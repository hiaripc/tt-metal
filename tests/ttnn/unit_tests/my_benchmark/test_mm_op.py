        ...
        for conf, dtype, math_fidelity, use_trace in matmul_configs:
            ...
            for m, k, n, in0_sharded, out_sharded, in0_block_w_div, num_out_blocks_h, num_out_blocks_w in matmul_shapes:
                ...
                writer.writerow([   
                        ...
                        dtype,
                        math_fidelity,
                        f"{kernel_config_time_acc * 1e6}",
                        f"{first_run_time_acc * 1e6}",
                        f"{second_run_time_acc * 1e6}",
                        f"{compile_time * 1e6}",
                        f"{offload_in0_acc * 1e6}",
                        f"{offload_in1_acc * 1e6}",
                    ]
                )
        ...
        for conf, dtype, math_fidelity, use_trace in matmul_configs:
            ...
            for m, k, n, in0_sharded, out_sharded, in0_block_w_div, num_out_blocks_h, num_out_blocks_w in matmul_shapes:
                ...
                for it in range(num_measurement_iterations):
                    ...
                    profiler.end(f"to_device_in0")
                    to_device_in0 = profiler.get("to_device_in0")
                    to_device_in0_acc += to_device_in0

                    profiler.start(f"to_device_in1")
                    in1_t = ttnn.from_torch(
                        in1,
                        tile=ttnn.Tile((32, 32)),
                        dtype=dtype,
                    ...
                    profiler.end(f"to_device_in1")
                    to_device_in1 = profiler.get("to_device_in1")
                    to_device_in1_acc += to_device_in1

                    profiler.start(f"tilize_in0")
                    in0_t = ttnn.tilize(in0_t)
                    profiler.end(f"tilize_in0")
                    tilize_in0 = profiler.get("tilize_in0")
                    tilize_in0_acc += tilize_in0

                    profiler.start(f"tilize_in1")
                    in1_t = ttnn.tilize(in1_t)
                    profiler.end(f"tilize_in1")
                ...
                writer.writerow([   
                        ...
                        dtype,
                        math_fidelity,
                        f"{to_device_in0_acc * 1e6:.2f}",
                        f"{to_device_in1_acc * 1e6:.2f}",
                        f"{tilize_in0_acc * 1e6:.2f}",
                        f"{tilize_in1_acc * 1e6:.2f}",
                    ]
                )
    pass
