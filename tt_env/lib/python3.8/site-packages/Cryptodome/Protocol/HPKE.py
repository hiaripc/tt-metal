...
class HPKE_Cipher:
    ...
    @staticmethod
    def _decap(enc: bytes, receiver_key: EccKey, kem_id: int, hashmod: ModuleType, sender_key: Optional[EccKey] = None):
    ...
    def _key_schedule(self, shared_secret: bytes, info: bytes, psk_id: bytes, psk: bytes):
        ...
        base_nonce = _labeled_expand(secret, b"base_nonce", key_schedule_context, self._Nn, suite_id, self._hashmod)

        exporter_secret = _labeled_expand(secret, b"exp", key_schedule_context, self._Nh, suite_id, self._hashmod)
    ...
    * a shared secret (e.g., a symmetric key derived from a password)
    ...
    if sender_key:
        ...
        if sender_key.curve != curve:
            raise ValueError("Sender key uses {} but recipient key {}".format(sender_key.curve, curve))
