...
class DependencyTree(object):
    ...
    def transitive_fingerprint(self, filename, module, compilation_options):
        ...
        try:
            ...
            m.update(
                str(
                    (module.language, getattr(module, "py_limited_api", False), getattr(module, "np_pythran", False))
                ).encode("UTF-8")
            )
...
def create_extension_list(
    patterns, exclude=None, ctx=None, aliases=None, quiet=False, language=None, exclude_failures=False
):
...
def cythonize_one(
    ...
    if any_failures:
        ...
        artifacts = list(
            filter(None, [getattr(result, attr, None) for attr in ("c_file", "h_file", "api_file", "i_file")])
        )
        if len(artifacts) == 1:
