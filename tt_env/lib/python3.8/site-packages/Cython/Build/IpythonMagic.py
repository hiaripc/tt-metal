...
class CythonMagics(Magics):
    ...
    @magic_arguments.argument(
        "-+", "--cplus", action="store_true", default=False, help="Output a C++ rather than C file."
    )
    @magic_arguments.argument(
        "-3", dest="language_level", action="store_const", const=3, default=None, help="Select Python 3 syntax."
    )
    @magic_arguments.argument(
        "-2", dest="language_level", action="store_const", const=2, default=None, help="Select Python 2 syntax."
    )
    @magic_arguments.argument(
        "-f",
        "--force",
        action="store_true",
        default=False,
        help="Force the compilation of a new module, even if the source has been " "previously compiled.",
    )
    @magic_arguments.argument(
        "-c",
        "--compile-args",
    ...
    @magic_arguments.argument(
        ...
        action="append",
        default=[],
        help="Add a path to the list of library directories (can be specified " "multiple times).",
    )
    @magic_arguments.argument(
        "-I",
        "--include",
    ...
    @magic_arguments.argument(
        ...
        action="append",
        default=[],
        help="Add a path to the list of src files (can be specified " "multiple times).",
    )
    @magic_arguments.argument(
        "--pgo",
        dest="pgo",
    ...
    @magic_arguments.argument(
        ...
        action="store_false",
        default=True,
        help=("Print debug information like generated .c/.cpp file location " "and exact gcc/g++ command invoked."),
    )
    @cell_magic
    ...
    def _add_pgo_flags(self, build_extension, step_name, temp_dir):
        ...
        if config and step_name in config:
            ...
            print(
                "No PGO %s configuration known for C compiler type '%s'" % (step_name, compiler_type), file=sys.stderr
            )
        return orig_flags
    ...
    def _get_build_extension(
        self, extension=None, lib_dir=None, temp_dir=None, pgo_step_name=None, _build_ext=build_ext
    ):
