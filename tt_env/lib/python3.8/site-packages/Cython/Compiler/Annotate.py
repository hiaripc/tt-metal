...
class AnnotationCCodeWriter(CCodeWriter):
    ...
    def _save_annotation_body(self, cython_code, generated_code, annotation_items, scopes, covered_lines=None):
        ...
        def annotate(match):
            ...
            calls[group_name] += 1
            return "<span class='%s'>%s</span>" % (group_name, match.group(group_name))
        ...
        for k, line in enumerate(lines, 1):
            ...
            if c_code:
                outlist.append(
                    "<pre class='cython code score-{score} {covered}'>{code}</pre>".format(
...
_parse_code = re.compile(
    (
        rb"(?P<refnanny>__Pyx_X?(?:GOT|GIVE)REF|__Pyx_RefNanny[A-Za-z]+)|"
        rb"(?P<trace>__Pyx_Trace[A-Za-z]+)|"
        rb"(?:"
        rb"(?P<pyx_macro_api>__Pyx_[A-Z][A-Z_]+)|"
        rb"(?P<pyx_c_api>(?:__Pyx_[A-Z][a-z_][A-Za-z_]*)|__pyx_convert_[A-Za-z_]*)|"
        rb"(?P<py_macro_api>Py[A-Z][a-z]+_[A-Z][A-Z_]+)|"
        rb"(?P<py_c_api>Py[A-Z][a-z]+_[A-Z][a-z][A-Za-z_]*)"
        rb")(?=\()|"  # look-ahead to exclude subsequent '(' from replacement
        rb"(?P<error_goto>(?:(?<=;) *if [^;]* +)?__PYX_ERR\([^)]+\))"
    ).decode("ascii")
).sub
