...
class AnnotationWriter(ExpressionWriter):
    ...
    def visit_Node(self, node):
        ...
        if self.description:
            warning(
                node.pos, "Failed to convert code to string representation in {0}".format(self.description), level=1
            )
    ...
    def visit_LambdaNode(self, node):
        ...
        if self.description:
            warning(
                node.pos, "Failed to convert lambda to string representation in {0}".format(self.description), level=1
            )
...
class EmbedSignature(CythonTransform):
    ...
    def visit_CFuncDefNode(self, node):
        ...
        signature = self._fmt_signature(
            self.class_name, node.declarator.base.name, node.declarator.args, return_type=node.return_type
        )
        if signature:
