...
def create_cython_argparser():
    ...
    parser.add_argument(
        "-o", "--output-file", dest="output_file", action="store", type=str, help="Specify name of generated C file"
    )
    parser.add_argument(
        "-t", "--timestamps", dest="timestamps", action="store_const", const=1, help="Only compile newer source files"
    )
    parser.add_argument(
        "-f",
        "--force",
    ...
    parser.add_argument(
        "-v", "--verbose", dest="verbose", action="count", help="Be verbose, print file names on multiple compilation"
    )
    parser.add_argument(
        "-p",
        "--embed-positions",
        dest="embed_pos_in_docstring",
        action="store_const",
        const=1,
        help="If specified, the positions in Cython files of each " "function definition is embedded in its docstring.",
    )
    parser.add_argument(
        "--cleanup",
        dest="generate_cleanup_code",
        action="store",
        type=int,
    ...
    parser.add_argument(
        "--gdb-outdir",
        action=SetGDBDebugOutputAction,
        type=str,
    ...
    parser.add_argument(
        "--annotate-coverage",
        dest="annotate_coverage_xml",
        action=SetAnnotateCoverageAction,
        type=str,
    ...
    parser.add_argument(
        "-+", "--cplus", dest="cplus", action="store_const", const=1, help="Output a C++ rather than C file."
    )
    parser.add_argument(
        "--embed",
        action="store_const",
    ...
    parser.add_argument(
        "--lenient",
        action=SetLenientAction,
        nargs=0,
        help="Change some compile time errors to runtime errors to " "improve Python compatibility",
    )
    parser.add_argument(
        "--capi-reexport-cincludes",
        dest="capi_reexport_cincludes",
        action="store_true",
        help="Add cincluded headers to any auto-generated header files.",
    )
    parser.add_argument(
        "--fast-fail", dest="fast_fail", action="store_true", help="Abort the compilation on the first error"
    )
    parser.add_argument(
        "-Werror", "--warning-errors", dest="warning_errors", action="store_true", help="Make all warnings into errors"
    )
    parser.add_argument(
        "-Wextra", "--warning-extra", action=ActivateAllWarningsAction, nargs=0, help="Enable extra warnings"
    )
