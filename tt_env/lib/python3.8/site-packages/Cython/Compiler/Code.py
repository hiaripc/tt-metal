...
special_py_methods = cython.declare(
    frozenset,
    frozenset(
        (
            "__cinit__",
    ...
    def __init__(self, include=None, verbatim=None, late=True, initial=False):
        ...
        if include:
            if include[0] == "<" and include[-1] == ">":
                self.pieces[0] = "#include {0}".format(include)
    ...
    def _add_utility(cls, utility, type, lines, begin_lineno, tags=None):
        ...
        if tags and "substitute" in tags and "naming" in tags["substitute"]:
            try:
                ...
                raise RuntimeError(
                    "Error parsing templated utility code of type '%s' at line %d: %s" % (type, begin_lineno, e)
                )
        ...
        def externalise(matchobj):
            ...
            try:
                ...
                cname = replacements[key] = output.get_py_string_const(
                    StringEncoding.EncodedString(name), identifier=str_type == "IDENT"
                ).cname
            return cname
        ...
        if self.temps_allocated:
            ...
            if leftovers:
                msg = "TEMPGUARD: Temps left over at end of '%s': %s" % (
                    self.scope.name,
                    ", ".join(["%s [%s]" % (name, ctype) for name, ctype, is_pytemp in sorted(leftovers)]),
                )
                # print(msg)
...
cython.declare(
    possible_unicode_identifier=object,
    possible_bytes_identifier=object,
    replace_identifier=object,
    find_alphanums=object,
)
possible_unicode_identifier = re.compile(rb"(?![0-9])\w+$".decode("ascii"), re.U).match
...
class GlobalState(object):
    ...
    def generate_string_constants(self):
        ...
        for _, cname, c in c_consts:
            ...
            decls_writer.putln(
                'static const char %s[] = "%s";' % (cname, StringEncoding.split_string_literal(c.escaped_value)),
                safe=True,
            )  # Braces in user strings are not for indentation.
            if conditional:
        ...
        if py_strings:
            ...
            init_constants.putln(
                "if (__Pyx_CreateStringTabAndInitStrings() < 0) %s;" % init_constants.error_goto(self.module_pos)
            )

    def generate_num_constants(self):
        consts = [
            (c.py_type, c.value[0] == "-", len(c.value), c.value, c.value_code, c)
        ...
        for py_type, _, _, value, value_code, c in consts:
            ...
            init_constants.putln(
                "%s = %s; %s"
                % (cname, function % value_code, init_constants.error_goto_if_null(cname, self.module_pos))
            )
    ...
    def commented_file_contents(self, source_desc):
        ...
        try:
            F = [
                " * "
                + line.rstrip()
                .replace("*/", "*[inserted by cython to avoid comment closer]/")
                .replace("/*", "/[inserted by cython to avoid comment start]*")
...
class CCodeWriter(object):
    ...
    def put_xdecref_clear(self, cname, type, clear_before_decref=False, nanny=True, have_gil=True):
        type.generate_xdecref_clear(
            self, cname, clear_before_decref=clear_before_decref, nanny=nanny, have_gil=have_gil
        )
    ...
    def put_pymethoddef(self, entry, term, allow_skip=True, wrapper_code_writer=None):
        ...
        self.putln(
            ...
            % (entry_name, func_ptr, "|".join(method_flags), entry.doc_cname if entry.doc else "0", term)
        )
