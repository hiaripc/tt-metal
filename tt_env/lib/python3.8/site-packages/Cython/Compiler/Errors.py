...
def format_position(position):
    if position:
        return "%s:%d:%d: " % (position[0].get_error_description(), position[1], position[2])
    return ""
...
class InternalError(Exception):
    ...
    def __init__(self, message):
        self.message_only = message
        Exception.__init__(self, "Internal compiler error: %s" % message)
...
def report_error(err, use_stack=True):
    ...
    if error_stack and use_stack:
        ...
        try:
            ...
            line = (
                format_error(
                    getattr(err, "message_only", "[unprintable exception message]"), getattr(err, "position", None)
                )
                + "\n"
            )
