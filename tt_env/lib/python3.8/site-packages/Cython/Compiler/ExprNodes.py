...
cython.declare(
    ...
    Symtab=object,
    Utils=object,
    find_coercion_error=object,
    debug_disposal_code=object,
    debug_temp_alloc=object,
    debug_coercion=object,
    bytearray_type=object,
    slice_type=object,
    memoryview_type=object,
    builtin_sequence_types=object,
    _py_int_types=object,
...
from .Builtin import (
    ...
    slice_type,
    long_type,
    sequence_types as builtin_sequence_types,
    memoryview_type,
)
from . import Builtin
from . import Symtab
...
def find_coercion_error(type_tuple, default, env):
    ...
    if err is None:
        return default
    elif env.directives["c_string_encoding"] and any(
        t in type_tuple
        for t in (
            PyrexTypes.c_char_ptr_type,
            PyrexTypes.c_uchar_ptr_type,
            PyrexTypes.c_const_char_ptr_type,
        ...
        if type_tuple[1].is_pyobject:
            ...
            return (
                "'%s' objects do not support coercion to C types with non-ascii or non-default c_string_encoding"
                % type_tuple[0].name
            )
    else:
        return err
...
def default_str_type(env):
    return {"bytes": bytes_type, "bytearray": bytearray_type, "str": str_type, "unicode": unicode_type}.get(
        env.directives["c_string_type"]
    )
...
def check_negative_indices(*nodes):
    ...
    for node in nodes:
        if node is None or (
            not isinstance(node.constant_result, _py_int_types) and not isinstance(node.constant_result, float)
        ):
            continue
...
def make_dedup_key(outer_type, item_nodes):
    ...
    item_keys = [
        ...
        else make_dedup_key(node.type, [node.mult_factor if node.is_literal else None] + node.args)
        if node.is_sequence_constructor
        else make_dedup_key(node.type, (node.start, node.stop, node.step))
        if node.is_slice
        # For constants, look at the Python value type if we don't know the concrete Cython type.
        else (node.type, node.constant_result, type(node.constant_result) if node.type is py_object_type else None)
        ...
        else (node.type, node.value, node.unicode_value, "IdentifierStringNode")
        if isinstance(node, IdentifierStringNode)
...
def get_exception_handler(exception_value):
    if exception_value is None:
        ...
        return (
            ...
            "} catch(...) { PyErr_SetNone(%s); }" % (exception_value.entry.cname, exception_value.entry.cname),
            False,
        )
    else:
        return (
            ...
            '"Error converting c++ exception.");' % (exception_value.entry.cname),
            False,
        )
...
class ExprNode(Node):
    ...
    def inferable_item_node(self, index=0):
        ...
        return IndexNode(
            self.pos,
            base=self,
            index=IntNode(self.pos, value=str(index), constant_result=index, type=PyrexTypes.c_py_ssize_t_type),
        )
    ...
    def release_temp_result(self, code):
        if not self.temp_code:
            ...
            if self.old_temp:
                raise RuntimeError(
                    "temp %s released multiple times in %s at %r" % (self.old_temp, self.__class__.__name__, pos)
                )
            else:
    ...
    def generate_assignment_code(
        self, rhs, code, overloaded_assignment=False, exception_check=None, exception_value=None
    ):
    ...
    def coerce_to(self, dst_type, env):
        ...
        if dst_type.is_memoryviewslice:
            ...
            if not src.type.is_memoryviewslice:
                ...
                if not src.type.conforms_to(
                    dst_type, broadcast=self.is_memview_broadcast, copying=self.is_memview_copy_assignment
                ):
            ...
            src_type is PyrexTypes.soft_complex_type and src_type != dst_type and not dst_type.assignable_from(src_type)
        ):
    ...
    def fail_assignment(self, dst_type):
        ...
        error(
            self.pos,
            "Cannot assign type '%s'%s to '%s'%s%s"
            % (self.type, src_resolved, dst_type, dst_resolved, extra_diagnostics),
        )
    ...
    def coerce_to_boolean(self, env):
        ...
        if type.is_enum or type.is_error:
            ...
            return SimpleCallNode(
                self.pos,
                function=AttributeNode(self.pos, obj=self, attribute=StringEncoding.EncodedString("operator bool")),
                args=[],
            ).analyse_types(env)
        elif type.is_ctuple:
...
class BoolNode(ConstNode):
    ...
    def coerce_to(self, dst_type, env):
        ...
        if dst_type.is_int and self.type.is_pyobject:
            return BoolNode(
                self.pos, value=self.value, constant_result=self.constant_result, type=PyrexTypes.c_bint_type
            )
    ...
    def coerce_to(self, dst_type, env):
        if self.type is dst_type:
            ...
            if self.has_constant_result():
                return FloatNode(
                    self.pos,
                    value="%d.0" % int(self.constant_result),
                    type=dst_type,
...
class BytesNode(ConstNode):
    ...
    def coerce_to(self, dst_type, env):
        ...
        if dst_type.is_pyobject:
            ...
            node.type = (
                PyrexTypes.c_const_char_ptr_type
                if dst_type == PyrexTypes.c_const_uchar_ptr_type
...
class StringNode(PyConstNode):
    ...
    def as_sliced_node(self, start, stop, step=None):
        ...
        if self.unicode_value is not None:
            ...
            unicode_value = None
        return StringNode(
            self.pos, value=value, unicode_value=unicode_value, constant_result=value, is_identifier=self.is_identifier
        )
    ...
    def generate_evaluation_code(self, code):
        self.result_code = code.get_py_string_const(
            self.value, identifier=self.is_identifier, is_str=True, unicode_value=self.unicode_value
        )
    ...
    def generate_result_code(self, code):
        if self.type.is_pyobject:
            code.putln(
                ...
                % (self.result(), float(self.value), code.error_goto_if_null(self.result(), self.pos))
            )
...
class NameNode(AtomicExprNode):
    ...
    def infer_type(self, env):
        ...
        if self.entry is None or self.entry.type is unspecified_type:
            ...
            self.entry.type.is_extension_type or self.entry.type.is_builtin_type
        ) and self.name == self.entry.type.name:
    ...
    def declare_from_annotation(self, env, as_target=False):
        ...
        if not entry:
            ...
            if as_target and env.is_c_class_scope and not (atype.is_pyobject or atype.is_error):
                ...
                warning(
                    annotation.pos,
                    "Annotation ignored since class-level attributes must be Python objects. "
                    "Were you trying to set up an instance attribute?",
                    2,
                )

            entry = self.entry = env.declare_var(
                name, atype, self.pos, is_cdef=not as_target, visibility=visibility, pytyping_modifiers=modifiers
            )
    ...
    def analyse_as_type(self, env):
        ...
        if not env.in_c_type_context and PyrexTypes.parse_basic_type(self.name):
            warning(
                self.pos,
                "Found C type '%s' in a Python annotation. Did you mean to use 'cython.%s'?" % (self.name, self.name),
            )
    ...
    def is_lvalue(self):
        return (self.entry.is_variable and not self.entry.is_readonly) or (
            self.entry.is_cfunction and self.entry.is_overridable
        )
    ...
    def generate_result_code(self, code):
        ...
        if entry.is_builtin and entry.is_const:
            ...
            code.putln(
                ...
                % (self.result(), interned_cname, code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            ...
            if (
                unbound_check_code
                and raise_unbound
                and (entry.type.is_pyobject or memslice_check or optional_cpp_check)
            ):
    ...
    def generate_assignment_code(
        ...
        if (
            self.entry.type.is_ptr
            and isinstance(rhs, ListNode)
            and not self.lhs_of_first_assignment
            and not rhs.in_module_scope
        ):
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        if self.entry is None:
            ...
            code.putln(
                ...
                " %s "
                "}" % (namespace, interned_cname, key_error_code, code.error_goto(self.pos))
            )
...
class BackquoteNode(ExprNode):
    ...
    def generate_result_code(self, code):
        code.putln(
            "%s = PyObject_Repr(%s); %s"
            % (self.result(), self.arg.py_result(), code.error_goto_if_null(self.result(), self.pos))
        )
        self.generate_gotref(code)
    ...
    def analyse_types(self, env):
        if self.level is None:
            ...
            if env.global_scope().parent_module and (
                env.directives["py2_import"]
                or Future.absolute_import not in env.global_scope().context.future_directives
            ):
                self.level = -1
            else:
                self.level = 0
...
class ScopedExprNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        for label, old_label in [(code.error_label, old_error_label)] + list(
            zip(code.get_loop_labels(), old_loop_labels)
        ):
...
class IteratorNode(ScopedExprNode):
    ...
    _func_iternext_type = PyrexTypes.CPtrType(
    ...
    def is_reversed_cpp_iteration(self):
        ...
        """
        if not (
            isinstance(self.sequence, SimpleCallNode)
            and self.sequence.arg_tuple
            and len(self.sequence.arg_tuple.args) == 1
        ):
            return False
    ...
    def generate_result_code(self, code):
        ...
        if not is_builtin_sequence:
            ...
            code.putln(
                ...
                % (self.result(), self.sequence.py_result(), code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            ...
            code.putln(
                "%s = __Pyx_PyObject_GetIterNextFunc(%s); %s"
                % (self.iter_func_ptr, self.py_result(), code.error_goto_if_null(self.iter_func_ptr, self.pos))
            )
        if self.may_be_a_sequence:
            code.putln("}")
...
class CppIteratorNode(ExprNode):
    ...
    def analyse_types(self, env):
        ...
        if iter_type.is_cpp_class:
            ...
            error(
                self.pos, "result type of %s() on %s must be a C++ class or pointer" % (begin_name, self.sequence.type)
            )
            self.type = error_type
        return self
    ...
    def generate_result_code(self, code):
        ...
        if self.sequence.is_simple():
            ...
            code.putln(
                "%s = %s%s;"
                % (self.cpp_sequence_cname, "&" if temp_type.is_ptr else "", self.sequence.move_result_rhs())
            )
    ...
    def infer_type(self, env, iterator_type=None):
        ...
        if iterator_type.is_ptr or iterator_type.is_array:
            ...
            fake_index_node = IndexNode(
                ...
                index=IntNode(self.pos, value="PY_SSIZE_T_MAX", type=PyrexTypes.c_py_ssize_t_type),
            )
...
class AsyncNextNode(AtomicExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            ...
            % (self.result(), self.iterator.py_result(), code.error_goto_if_null(self.result(), self.pos))
        )
        self.generate_gotref(code)
...
class WithExitCallNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        code.putln(
            "%s = __Pyx_PyObject_Call(%s, %s, NULL);" % (result_var, self.with_stat.exit_var, self.args.result())
        )
...
class JoinedStrNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        code.putln(
            "%s = __Pyx_PyUnicode_Join(%s, %d, %s, %s); %s"
            % (
                self.result(),
                list_var,
                num_items,
...
class FormattedValueNode(ExprNode):
    ...
    def generate_result_code(self, code):
        if self.c_format_spec is not None and not self.value.type.is_pyobject:
            ...
            code.putln(
                "%s = %s; %s" % (self.result(), convert_func_call, code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            return
        ...
        code.putln(
            "%s = %s(%s, %s); %s"
            % (self.result(), format_func, value_result, format_spec, code.error_goto_if_null(self.result(), self.pos))
        )
        self.generate_gotref(code)
...
class IndexNode(_IndexingBaseNode):
    ...
    def infer_type(self, env):
        ...
        if self.index.is_sequence_constructor and base_type.is_memoryviewslice:
            ...
            for a in self.index.args:
                ...
                inferred_type = IndexNode(
                    self.pos, base=ExprNode(self.base.pos, type=inferred_type), index=a
                ).infer_type(env)
            else:
    ...
    def analyse_as_pyobject(self, env, is_slice, getting, setting):
        ...
        if self.index.type.is_unicode_char and base_type is not dict_type:
            ...
            warning(
                ...
                "Use an explicit C integer cast to get back the previous integer lookup behaviour.",
                level=1,
            )
            self.index = self.index.coerce_to_pyobject(env)
            self.is_temp = 1
            ...
            if (
                ...
                and (
                    not self.index.type.signed
                    or not env.directives["wraparound"]
                    or (
                        isinstance(self.index, IntNode)
                        and self.index.has_constant_result()
        ...
        if base_type.is_memoryviewslice:
            ...
            if base_type.is_pythran_expr or len(indices) == base_type.ndim:
                ...
                if base_type.is_pythran_expr:
                    do_replacement = all(
                        index.type.is_int or index.is_slice or index.type.is_pythran_expr for index in indices
                    )
                    if do_replacement:
    ...
    def extra_index_params(self, code):
        if self.index.type.is_int:
            ...
            wraparound = (
                ...
                and self.original_index_type.signed
                and not (isinstance(self.index.constant_result, _py_int_types) and self.index.constant_result >= 0)
            )
    ...
    def generate_setitem_code(self, value_code, code):
        ...
        code.putln(
            code.error_goto_if_neg(
                "%s(%s, %s, %s%s)"
                % (function, self.base.py_result(), index_code, value_code, self.extra_index_params(code)),
                self.pos,
            )
        )
    ...
    def _check_byte_value(self, code, rhs):
        ...
        if rhs.has_constant_result():
            ...
            warning(
                rhs.pos, "value outside of range(0, 256)" " when assigning to byte: %s" % rhs.constant_result, level=1
            )
        else:
        ...
        if not self.nogil:
            ...
            if rhs.is_literal or not (
                rhs.is_temp and rhs.type in (PyrexTypes.c_uchar_type, PyrexTypes.c_char_type, PyrexTypes.c_schar_type)
            ):
            ...
            if conditions:
                ...
                code.putln(
                    ...
                    ' "byte must be in range(0, 256)"); %s' % code.error_goto(self.pos)
                )
                code.putln("}")
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        ...
        code.putln(
            code.error_goto_if_neg(
                "%s(%s, %s%s)" % (function, self.base.py_result(), index_code, self.extra_index_params(code)), self.pos
            )
        )
        self.generate_subexpr_disposal_code(code)
        self.free_subexpr_temps(code)
    ...
    def generate_result_code(self, code):
        if is_pythran_expr(self.base.type):
            ...
            code.putln(
                ...
                % (res, res, self.base.pythran_result(), pythran_indexing_code(self.indices))
            )
            return
...
class MemoryCopyScalar(MemoryCopyNode):
    ...
    def _generate_assignment_code(self, scalar, code):
        ...
        slice_iter_obj = MemoryView.slice_iter(
            self.dst.type, dst_temp, self.dst.type.ndim, code, force_strided=force_strided
        )
...
class SliceIndexNode(ExprNode):
    ...
    def analyse_types(self, env, getting=True):
        ...
        if self.base.type.is_buffer or self.base.type.is_pythran_expr or self.base.type.is_memoryviewslice:
            ...
            return index_node.analyse_base_and_index_types(
                env, getting=getting, setting=not getting, analyse_base=False
            )
    ...
    def coerce_to(self, dst_type, env):
        if (self.base.type.is_string or self.base.type.is_cpp_string) and dst_type in (
            bytes_type,
            bytearray_type,
            str_type,
            unicode_type,
            ...
            if dst_type not in (bytes_type, bytearray_type) and not env.directives["c_string_encoding"]:
                error(
                    self.pos, "default encoding required for conversion from '%s' to '%s'" % (self.base.type, dst_type)
                )
            self.type = dst_type
        ...
        if self.base.type.is_string:
            ...
            if self.stop is None:
                code.putln(
                    ...
                    % (result, type_name, base_result, start_code, code.error_goto_if_null(result, self.pos))
                )
            else:
                code.putln(
            ...
            if self.stop is None:
                code.putln(
                    ...
                    % (result, base_result, start_code, code.error_goto_if_null(result, self.pos))
                )
            ...
            code.putln(
                ...
                % (result, base_result, start_code, stop_code, code.error_goto_if_null(result, self.pos))
            )
    ...
    def generate_assignment_code(
        ...
        if self.type.is_pyobject:
            ...
            code.putln(
                ...
                % (self.base.result(), start_offset, rhs.result(), self.base.result(), array_length)
            )
...
class CallNode(ExprNode):
    ...
    def set_py_result_type(self, function, func_type=None):
        ...
        if func_type is Builtin.type_type and (
            ...
            and function.entry.name in Builtin.types_that_construct_their_instance
        ):
    ...
    def generate_evaluation_code(self, code):
        ...
        if self.overflowcheck and is_signed_int and function.result() in abs_function_cnames:
            ...
            code.putln(
                ...
                % (self.args[0].result(), self.args[0].type.empty_declaration_code(), code.error_goto(self.pos))
            )
        ...
        if arg is None:
            ...
            code.putln(
                ...
                % (self.result(), function.py_result(), code.error_goto_if_null(self.result(), self.pos))
            )
        else:
    ...
    def generate_result_code(self, code):
        ...
        if func_type.is_pyobject:
            ...
            code.putln(
                ...
                % (self.result(), self.function.py_result(), arg_code, code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            ...
            if self.has_optional_args:
                ...
                code.putln(
                    "%s.%s = %s;" % (self.opt_arg_struct, Naming.pyrex_prefix + "n", len(self.args) - expected_nargs)
                )
...
class NumPyMethodCallNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        code.putln(
            "new (&%s) decltype(%s){%s{}(%s)};"
            % (self.result(), self.result(), self.function_cname, ", ".join(a.pythran_result() for a in args))
        )
...
class GeneralCallNode(CallNode):
    ...
    def explicit_args_kwds(self):
        if (
            self.keyword_args
            and not self.keyword_args.is_dict_literal
            or not self.positional_args.is_sequence_constructor
        ):
        ...
        if not self.function.type.is_pyobject:
            ...
            if hasattr(self.function, "entry"):
                ...
                if node is not None and node is not self:
                    ...
                    error(
                        self.pos,
                        "Non-trivial keyword arguments and starred " "arguments not allowed in cdef functions.",
                    )
                else:
    ...
    def map_to_simple_call_node(self):
        ...
        if len(pos_args) > len(declared_args):
            error(
                ...
                "expected %d, got %s" % (len(declared_args), len(pos_args)),
            )
            return None
...
class AsTupleNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            "%s = %s(%s); %s"
            % (self.result(), cfunc, self.arg.py_result(), code.error_goto_if_null(self.result(), self.pos))
        )
        self.generate_gotref(code)
...
class MergedDictNode(ExprNode):
    ...
    def compile_time_value(self, denv):
        ...
        for item in self.keyword_args:
            if item.is_dict_literal:
                # process items in order
                items = [
                    (key.compile_time_value(denv), value.compile_time_value(denv))
    ...
    def generate_evaluation_code(self, code):
        ...
        if item.is_dict_literal:
            ...
            code.putln(
                "%s = PyDict_Copy(%s); %s"
                % (self.result(), item.py_result(), code.error_goto_if_null(self.result(), item.pos))
            )
            self.generate_gotref(code)
        ...
        if item.type is not dict_type:
            ...
            code.putln(
                ...
                % (self.result(), item.py_result(), code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            item.generate_disposal_code(code)
            code.putln("}")
                ...
                for arg in item.key_value_pairs:
                    ...
                    if self.reject_duplicates:
                        ...
                        code.putln(
                            ...
                            % (arg.key.py_result(), code.error_goto(self.pos))
                        )
                        code.putln("}")
                    code.put_error_if_neg(
                        arg.key.pos,
                        "PyDict_SetItem(%s, %s, %s)" % (self.result(), arg.key.py_result(), arg.value.py_result()),
                    )
                    arg.generate_disposal_code(code)
                    arg.free_temps(code)
            else:
...
class AttributeNode(ExprNode):
    ...
    def as_cython_attribute(self):
        if isinstance(self.obj, NameNode) and self.obj.is_cython_module and not self.attribute == "parallel":
    ...
    def analyse_as_cimported_attribute_node(self, env, target):
        ...
        if module_scope:
            ...
            if (
                entry
                and not entry.known_standard_library_import
                and (entry.is_cglobal or entry.is_cfunction or entry.is_type or entry.is_const)
            ):
    ...
    def analyse_as_python_attribute(self, env, obj_type=None, immutable_obj=False):
        ...
        if not obj_type.is_pyobject and not obj_type.is_error:
            ...
            if (
                obj_type.is_string
                or obj_type.is_cpp_string
                or obj_type.is_buffer
                or obj_type.is_memoryviewslice
                ...
                obj_type.is_cfunction
                and (self.obj.is_name or self.obj.is_attribute)
        ...
        if self.entry and self.entry.is_cmethod:
            if obj.type.is_extension_type and not self.entry.is_builtin_cmethod:
                ...
                return "((struct %s *)%s%s%s)->%s" % (
                    obj.type.vtabstruct_cname,
                    obj_code,
                    self.op,
                    obj.type.vtabslot_cname,
                    self.member,
                )
    ...
    def generate_assignment_code(
        ...
        if self.is_py_attr:
            ...
            code.put_error_if_neg(
                ...
                % (self.obj.py_result(), code.intern_identifier(self.attribute), rhs.py_result()),
            )
            rhs.generate_disposal_code(code)
            rhs.free_temps(code)
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        self.obj.generate_evaluation_code(code)
        if self.is_py_attr or (self.entry.scope.is_property_scope and "__del__" in self.entry.scope.entries):
            code.globalstate.use_utility_code(UtilityCode.load_cached("PyObjectSetAttrStr", "ObjectHandling.c"))
            code.put_error_if_neg(
                self.pos,
                "__Pyx_PyObject_DelAttrStr(%s, %s)" % (self.obj.py_result(), code.intern_identifier(self.attribute)),
            )
        else:
...
class SequenceNode(ExprNode):
    ...
    def _create_merge_node_if_necessary(self, env):
        ...
        if self.mult_factor:
            node = binop_node(
                ...
                "*",
                node,
                self.mult_factor.coerce_to_pyobject(env),
                inplace=True,
                type=self.type,
                is_temp=True,
            )
    ...
    def generate_sequence_packing_code(self, code, target=None, plain=False):
        ...
        if self.type is tuple_type and (self.is_literal or self.slow) and not c_mult:
            ...
            code.putln(
                "%s = %s(%s%s); %s"
                % (target, create_func, arg_count, size_factor, code.error_goto_if_null(target, self.pos))
            )
            code.put_gotref(target, py_object_type)
        ...
        if mult_factor is not None and mult_factor.type.is_pyobject:
            code.putln(
                "{ PyObject* %s = PyNumber_InPlaceMultiply(%s, %s); %s"
                % (
                    Naming.quick_temp_cname,
                    target,
                    mult_factor.py_result(),
    ...
    def generate_parallel_assignment_code(self, rhs, code):
        ...
        for item in self.unpacked_items:
            item.allocate(code)
        special_unpack = (
            rhs.type is py_object_type or rhs.type in (tuple_type, list_type) or not rhs.type.is_builtin_type
        )
    ...
    def generate_special_parallel_unpacking_code(self, code, rhs, use_loop):
        ...
        if not use_loop:
            for i, item in enumerate(self.unpacked_items):
                code.putln(
                    "%s = PySequence_ITEM(sequence, %d); %s"
                    % (item.result(), i, code.error_goto_if_null(item.result(), self.pos))
                )
            ...
            code.putln(
                "PyObject** temps[%s] = {%s};"
                % (len(self.unpacked_items), ",".join(["&%s" % item.result() for item in self.unpacked_items]))
            )
            code.putln("for (i=0; i < %s; i++) {" % len(self.unpacked_items))
            code.putln(
                "PyObject* item = PySequence_ITEM(sequence, i); %s" % (code.error_goto_if_null("item", self.pos))
            )
    ...
    def generate_generic_parallel_unpacking_code(self, code, rhs, unpacked_items, use_loop, terminate=True):
        ...
        if use_loop:
            code.putln(
                "PyObject** temps[%s] = {%s};"
                % (len(self.unpacked_items), ",".join(["&%s" % item.result() for item in unpacked_items]))
            )

        iterator_temp = code.funcstate.allocate_temp(py_object_type, manage_ref=True)
        code.putln(
            "%s = PyObject_GetIter(%s); %s"
            % (iterator_temp, rhs.py_result(), code.error_goto_if_null(iterator_temp, self.pos))
        )
    ...
    def generate_starred_assignment_code(self, rhs, code):
        ...
        if unpacked_fixed_items_left:
            ...
            iterator_temp = self.generate_generic_parallel_unpacking_code(
                code, rhs, unpacked_fixed_items_left, use_loop=True, terminate=False
            )
        ...
        code.putln(
            ...
            % (
                ...
                if (not iterator_temp and rhs.is_temp and rhs.type in (py_object_type, list_type))
        ...
        if unpacked_fixed_items_right:
            ...
            code.putln(
                "__Pyx_RaiseNeedMoreValuesError(%d+%s); %s"
                % (len(unpacked_fixed_items_left), length_temp, code.error_goto(self.pos))
            )
            code.putln("}")

            for item in unpacked_fixed_items_right[::-1]:
                item.allocate(code)
            for i, (item, coerced_arg) in enumerate(
                zip(unpacked_fixed_items_right[::-1], self.coerced_unpacked_items[::-1])
            ):
            ...
            code.putln(
                "%s = PySequence_GetSlice(%s, 0, %s-%d); %s"
                % (
                    sublist_temp,
                    target_list,
                    length_temp,
...
class TupleNode(SequenceNode):
    ...
    def infer_type(self, env):
        ...
        arg_types = [arg.infer_type(env) for arg in self.args]
        if any(
            type.is_pyobject or type.is_memoryviewslice or type.is_unspecified or type.is_fused for type in arg_types
        ):
            return tuple_type
    ...
    def analyse_types(self, env, skip_children=False):
        ...
        if not self.mult_factor and not any(
            (arg.is_starred or arg.type.is_pyobject or arg.type.is_memoryviewslice or arg.type.is_fused)
            for arg in self.args
        ):
        ...
        if not node.mult_factor or (
            node.mult_factor.is_literal and isinstance(node.mult_factor.constant_result, _py_int_types)
        ):
            node.is_temp = False
    ...
    def generate_operation_code(self, code):
        ...
        if self.is_literal or self.is_partly_literal:
            ...
            if self.is_literal:
                ...
                code.putln(
                    "%s = __Pyx_PySequence_Multiply(%s, %s); %s"
                    % (
                        self.result(),
                        tuple_target,
                        self.mult_factor.result(),
                        code.error_goto_if_null(self.result(), self.pos),
                    )
                )
                self.generate_gotref(code)
                ...
                code.putln(
                    "%s = PyNumber_Multiply(%s, %s); %s"
                    % (
                        self.result(),
                        tuple_target,
                        self.mult_factor.py_result(),
                        code.error_goto_if_null(self.result(), self.pos),
                    )
                )
                self.generate_gotref(code)
            ...
            for i, arg in enumerate(self.args):
                if arg.type.is_array:
                    ...
                    code.putln(
                        ...
                        % (self.result(), i, offset, arg.result(), self.result())
                    )
                else:
...
class ComprehensionAppendNode(Node):
    ...
    def generate_execution_code(self, code):
        ...
        code.putln(
            code.error_goto_if("%s(%s, (PyObject*)%s)" % (function, self.target.result(), self.expr.result()), self.pos)
        )
...
class DictComprehensionAppendNode(ComprehensionAppendNode):
    ...
    def generate_execution_code(self, code):
        ...
        code.putln(
            code.error_goto_if(
                "PyDict_SetItem(%s, (PyObject*)%s, (PyObject*)%s)"
                % (self.target.result(), self.key_expr.result(), self.value_expr.result()),
                self.pos,
            )
        )
...
class InlinedGeneratorExpressionNode(ExprNode):
    ...
    def generate_result_code(self, code):
        code.putln(
            "%s = __Pyx_Generator_Next(%s); %s"
            % (self.result(), self.gen.result(), code.error_goto_if_null(self.result(), self.pos))
        )
        self.generate_gotref(code)
...
class MergedSequenceNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        if is_set and item.is_set_literal or not is_set and item.is_sequence_constructor and item.type is list_type:
            ...
            code.putln(
                ...
                % (
                    ...
                    "PySet_New"
                    if is_set
                    else "__Pyx_PySequence_ListKeepNew"
                    if item.is_temp and item.type in (py_object_type, list_type)
                    else "PySequence_List",
                    item.py_result(),
                    code.error_goto_if_null(self.result(), self.pos),
                )
        ...
        for item in args:
            if (
                is_set
                and (item.is_set_literal or item.is_sequence_constructor)
                or (item.is_sequence_constructor and not item.mult_factor)
            ):
        ...
        if self.type is tuple_type:
            ...
            code.putln(
                "%s = %s; %s"
                % (self.result(), Naming.quick_temp_cname, code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            code.putln("}")
...
class DictNode(ExprNode):
    ...
    def compile_time_value(self, denv):
        pairs = [
            (item.key.compile_time_value(denv), item.value.compile_time_value(denv)) for item in self.key_value_pairs
        ]
        try:
            return dict(pairs)
        except Exception as e:
        ...
        if dst_type.is_pyobject:
            ...
            if self.type.is_struct_or_union:
                ...
                return DictNode(
                    ...
                    key_value_pairs=[
                        DictItemNode(
                            item.pos, key=item.key.coerce_to_pyobject(env), value=item.value.coerce_to_pyobject(env)
                        )
    ...
    def generate_evaluation_code(self, code):
        ...
        if is_dict:
            ...
            code.putln(
                ...
                % (self.result(), len(self.key_value_pairs), code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
            struct_scope = None
        else:
            ...
            if is_dict:
                ...
                if self.reject_duplicates:
                    ...
                    if keys_seen is None:
                        ...
                        code.putln(
                            '__Pyx_RaiseDoubleKeywordsError("function", %s); %s'
                            % (key.py_result(), code.error_goto(item.pos))
                        )
                        code.putln("} else {")

                code.put_error_if_neg(
                    self.pos,
                    "PyDict_SetItem(%s, %s, %s)" % (self.result(), item.key.py_result(), item.value.py_result()),
                )
                ...
                if item.value.type.is_array:
                    code.putln("memcpy(%s.%s, %s, sizeof(%s));" % (self.result(), key_cname, value_cname, value_cname))
                else:
...
class SortedDictKeysNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if self.arg.type is Builtin.dict_type:
            code.putln(
                "%s = PyDict_Keys(%s); %s"
                % (self.result(), dict_result, code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
        else:
            ...
            code.putln(
                ...
                % (self.result(), dict_result, keys_cname, code.error_goto_if_null(self.result(), self.pos))
            )
            self.generate_gotref(code)
...
class Py3ClassNode(ExprNode):
    ...
    def analyse_annotations(self, env):
        ...
        dict_items = [
            DictItemNode(
                entry.pos, key=IdentifierStringNode(entry.pos, value=entry.name), value=entry.annotation.string
            )
...
class ClassCellNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            ...
            '"super(): empty __class__ cell"); %s }' % (self.result(), code.error_goto(self.pos))
        )
...
class PyCFunctionNode(ExprNode, ModuleNameMixin):
    ...
    def analyse_default_args(self, env):
        ...
        if nonliteral_objects or nonliteral_other:
            ...
            for arg in nonliteral_other:
                entry = scope.declare_var(
                    arg.name, arg.type, None, Naming.arg_prefix + arg.name, allow_pyobject=False, allow_memoryview=True
                )
                self.defaults.append((arg, entry))
            ...
            if self.defaults_struct is None:
                ...
                defaults_getter = Nodes.DefNode(
                    ...
                    body=Nodes.ReturnStatNode(
                        self.pos,
                        return_type=py_object_type,
                        value=TupleNode(self.pos, args=[defaults_tuple, defaults_kwdict]),
                    ),
                    decorators=None,
        ...
        if annotations:
            annotations_dict = DictNode(
                self.pos,
                key_value_pairs=[
                    DictItemNode(pos, key=IdentifierStringNode(pos, value=name), value=value)
    ...
    def generate_cyfunction_code(self, code):
        ...
        if self.defaults:
            code.putln(
                ...
                % (self.result(), self.defaults_struct.name, self.defaults_pyobjects, code.error_goto(self.pos))
            )
        ...
        if not self.specialized_cpdefs:
            ...
            if self.defaults_kwdict:
                code.putln(
                    "__Pyx_CyFunction_SetDefaultsKwDict(%s, %s);" % (self.result(), self.defaults_kwdict.py_result())
                )
            ...
            if self.annotations_dict:
                code.putln(
                    "__Pyx_CyFunction_SetAnnotationsDict(%s, %s);" % (self.result(), self.annotations_dict.py_result())
                )
...
class CodeObjectNode(ExprNode):
    ...
    def __init__(self, def_node):
        ...
        self.varnames = TupleNode(
            def_node.pos,
            args=[IdentifierStringNode(arg.pos, value=arg.name) for arg in args + local_vars],
            is_temp=0,
            is_literal=1,
        )
...
class YieldExprNode(ExprNode):
    ...
    def generate_yield_code(self, code):
        ...
        code.putln(
            "/* return from %sgenerator, %sing value */"
            % ("async " if self.in_async_gen else "", "await" if self.is_await else "yield")
        )
...
class FuncLocalsExprNode(DictNode):
    def __init__(self, pos, env):
        ...
        items = [
            LocalsDictItemNode(
                pos, key=IdentifierStringNode(pos, value=var), value=NameNode(pos, name=var, allow_null=True)
            )
...
class UnopNode(ExprNode):
    ...
    def generate_result_code(self, code):
        if self.type.is_pythran_expr:
            ...
            code.putln(
                "new (&%s) decltype(%s){%s%s};"
                % (self.result(), self.result(), self.operator, self.operand.pythran_result())
            )
    ...
    def generate_py_operation_code(self, code):
        ...
        code.putln(
            "%s = %s(%s); %s"
            % (self.result(), function, self.operand.py_result(), code.error_goto_if_null(self.result(), self.pos))
        )
        self.generate_gotref(code)
    ...
    def type_error(self):
        if not self.operand.type.is_error:
            if self.is_prefix:
                ...
                error(
                    self.pos,
                    "No 'operator%s(int)' declared for postfix '%s' (operand type is '%s')"
    ...
    def analyse_types(self, env):
        ...
        if argtype.is_pyobject:
            self.error(
                ...
                % (
                    "variable '%s'" % self.operand.name
                    if self.operand.is_name
                    else "object attribute '%s'" % self.operand.attribute
                    if self.operand.is_attribute
                    else "object"
                )
            )
            return self
...
def unop_node(pos, operator, operand):
    ...
    if isinstance(operand, IntNode) and operator == "-":
        return IntNode(
            pos=operand.pos,
            value=str(-Utils.str_to_number(operand.value)),
            longness=operand.longness,
    ...
    def analyse_types(self, env):
        ...
        if to_py and not from_py:
            if self.type is bytes_type and self.operand.type.is_int:
                ...
                if self.operand.type.is_ptr:
                    ...
                    warning(
                        self.pos,
                        "No conversion from %s to %s, python object pointer used." % (self.operand.type, self.type),
                    )
            ...
            if self.type.create_from_py_utility_code(env):
                ...
                warning(
                    self.pos,
                    "No conversion from %s to %s, python object pointer used." % (self.type, self.operand.type),
                )
        ...
        if self.type.is_ptr and self.type.base_type.is_cfunction and self.type.base_type.nogil:
            ...
            if op_type.is_cfunction and not op_type.nogil:
                warning(
                    self.pos, "Casting a GIL-requiring function into a nogil function circumvents GIL validation", 1
                )
        return self
...
class CythonArrayNode(ExprNode):
    ...
    def analyse_types(self, env):
        ...
        if not (base_type.same_as(array_dtype) or base_type.is_void):
            ...
            error(
                self.operand.pos, "Expected %d dimensions, array has %d dimensions" % (ndim, len(array_dimension_sizes))
            )
            return self
        ...
        for axis_no, axis in enumerate(axes):
            ...
            if axis.stop.is_none:
                if array_dimension_sizes:
                    ...
                    axis.stop = IntNode(
                        self.pos, value=str(dimsize), constant_result=dimsize, type=PyrexTypes.c_int_type
                    )
                else:
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            "%s = __pyx_format_from_typeinfo(&%s); %s"
            % (
                format_temp,
                type_info,
        ...
        code.putln(
            '%s = __pyx_array_new(%s, %s, PyBytes_AS_STRING(%s), (char *) "%s", (char *) %s); %s'
            % (
                self.result(),
                shapes_temp,
                itemsize,
        ...
        if operand_as_type:
            ...
            if self.arg_type.is_fused:
                try:
                    ...
                    error(
                        self.operand.pos, "Type cannot be specialized since it is not a fused argument to this function"
                    )
...
class TypeidNode(ExprNode):
    ...
    def analyse_types(self, env):
        ...
        if as_type:
            ...
            self.is_type = True
            self.operand = (
                None  # nothing further uses self.operand - will only cause problems if its used in code generation
            )
        else:
    ...
    def generate_result_code(self, code):
        ...
        translate_cpp_exception(
            code, self.pos, "%s = typeid(%s);" % (self.temp_code, arg_code), None, None, self.in_nogil_context
        )
...
class BinopNode(ExprNode):
    ...
    def is_pythran_operation_types(self, type1, type2, env):
        ...
        return (
            has_np_pythran(env)
            and (is_pythran_supported_operation_type(type1) and is_pythran_supported_operation_type(type2))
            and (is_pythran_expr(type1) or is_pythran_expr(type2))
        )
        ...
        if self.type.is_pythran_expr:
            ...
            if self.operator == "**":
                code.putln(
                    ...
                    % (self.result(), self.result(), self.operand1.pythran_result(), self.operand2.pythran_result())
                )
            else:
                code.putln(
    ...
    def type_error(self):
        if not (self.operand1.type.is_error or self.operand2.type.is_error):
            error(
                self.pos,
                "Invalid operand types for '%s' (%s; %s)" % (self.operator, self.operand1.type, self.operand2.type),
            )
        self.type = PyrexTypes.error_type
    ...
    def analyse_c_operation(self, env):
        ...
        if self.type.is_int and env.directives["overflowcheck"] and self.operator in self.overflow_op_names:
            ...
            self.func = self.type.overflow_check_binop(
                self.overflow_op_names[self.operator], env, const_rhs=self.operand2.has_constant_result()
            )
            self.is_temp = True
    ...
    def analyse_operation(self, env):
        ...
        if not self.type.is_pyobject:
            self.zerodivision_check = (
                self.cdivision is None
                and not env.directives["cdivision"]
                and (not self.operand2.has_constant_result() or self.operand2.constant_result == 0)
            )
            ...
            if self.zerodivision_check:
                ...
                if self.type.is_int and self.type.signed and self.operator != "%":
                    ...
                    code.putln(
                        ...
                        % (self.type.empty_declaration_code(), minus1_check, self.operand1.result())
                    )
                    if in_nogil:
                        code.put_ensure_gil()
    ...
    def py_operation_function(self, code):
        ...
        if type1 is unicode_type:
            if self.operand1.may_be_none() or (
                type2.is_extension_type
                and type2.subtype_of(type1)
                or type2 is py_object_type
                and not isinstance(self.operand2, CoerceToPyTypeNode)
            ):
...
class PowNode(NumBinopNode):
        ...
        if self.type.is_complex:
            ...
            env.use_utility_code(
                UtilityCode.load_cached("IntPow", "CMath.c").specialize(
                    func_name=self.pow_func, type=self.type.empty_declaration_code(), signed=self.type.signed and 1 or 0
                )
            )
        elif not self.type.is_error:
            error(
                self.pos, "got unexpected types for C power operator: %s, %s" % (self.operand1.type, self.operand2.type)
            )
    ...
    def compute_c_result_type(self, type1, type2):
        ...
        c_result_type = None
        op1_is_definitely_positive = (self.operand1.has_constant_result() and self.operand1.constant_result >= 0) or (
            type1.is_int and type1.signed == 0  # definitely unsigned
        )
        type2_is_int = type2.is_int or (
            self.operand2.has_constant_result()
            and isinstance(self.operand2.constant_result, Real)
            and int(self.operand2.constant_result) == self.operand2.constant_result
        )
    ...
    def py_operation_function(self, code):
        if (
            self.type.is_pyobject
            and self.operand1.constant_result == 2
            and isinstance(self.operand1.constant_result, _py_int_types)
            and self.operand2.type is py_object_type
        ):
    ...
    def coerce_to(self, dst_type, env):
        ...
        if self.is_cpow is None and self.type_was_inferred and (dst_type.is_float or dst_type.is_int):
            ...
            if check_op1 and check_op2:
                warning(
                    self.pos,
                    "Treating '**' as if 'cython.cpow(True)' since it "
...
class BoolBinopNode(ExprNode):
    ...
    def coerce_to(self, dst_type, env):
        ...
        return BoolBinopNode.from_node(
            self, type=dst_type, operator=self.operator, operand1=operand1, operand2=operand2
        )

    def generate_bool_evaluation_code(
        self, code, final_result_temp, final_result_type, and_label, or_label, end_label, fall_through
    ):
        code.mark_pos(self.pos)
    ...
    def generate_operand1_test(self, code):
        ...
        if self.type.is_pyobject:
            ...
            code.putln(
                ...
                % (test_result, self.operand1.py_result(), code.error_goto_if_neg(test_result, self.pos))
            )
        else:
...
class BoolBinopResultNode(ExprNode):
    ...
    def __init__(self, arg, result_type, env):
        ...
        super(BoolBinopResultNode, self).__init__(
            arg.pos, arg=arg, type=result_type, value=CloneNode(arg).coerce_to(result_type, env)
        )
    ...
    def generate_operand_test(self, code):
        ...
        if self.arg.type.is_pyobject:
            ...
            code.putln(
                "%s = __Pyx_PyObject_IsTrue(%s); %s"
                % (test_result, self.arg.py_result(), code.error_goto_if_neg(test_result, self.pos))
            )
        else:
...
class CondExprNode(ExprNode):
    ...
    def type_error(self):
        if not (self.true_val.type.is_error or self.false_val.type.is_error):
            error(
                self.pos,
                "Incompatible types in conditional expression (%s; %s)" % (self.true_val.type, self.false_val.type),
            )
        self.type = PyrexTypes.error_type
...
class CmpNode(object):
    ...
    def calculate_cascaded_constant_result(self, operand1_result):
        ...
        if (
            isinstance(operand1_result, any_string_type)
            and isinstance(operand2_result, any_string_type)
            and type(operand1_result) != type(operand2_result)
        ):
    ...
    def find_common_type(self, env, op, operand1, common_type=None):
        ...
        if (
            type1 == str_type
            and (type2.is_string or type2 in (bytes_type, unicode_type))
            or type2 == str_type
            and (type1.is_string or type1 in (bytes_type, unicode_type))
        ):
    ...
    def is_python_result(self):
        return (
            self.has_python_operands()
            and self.special_bool_cmp_function is None
    ...
    def is_c_string_contains(self):
        return self.operator in ("in", "not_in") and (
            (self.operand1.type.is_int and (self.operand2.type.is_string or self.operand2.type is bytes_type))
            or (self.operand1.type.is_unicode_char and self.operand2.type is unicode_type)
        )
            ...
            if result_is_bool or (type1.is_builtin_type and type2.is_builtin_type):
                if type1 is Builtin.unicode_type or type2 is Builtin.unicode_type:
                    ...
                    result = optimise_numeric_binop(
                        "Eq" if self.operator == "==" else "Ne", self, PyrexTypes.c_bint_type, operand1, self.operand2
                    )
                    if result:
                        (
    ...
    def generate_operation_code(self, code, result_code, operand1, op, operand2):
        ...
        if self.special_bool_cmp_function:
            ...
            code.putln(
                "%s = PyObject_RichCompare%s(%s, %s, %s); %s%s"
                % (
                    result_code,
        ...
        if self.is_temp:
            ...
            if self.cascade:
                self.cascade.generate_evaluation_code(
                    code,
                    self.result(),
                    self.coerced_operand2 or self.operand2,
...
class CascadedCmpNode(Node, CmpNode):
    ...
    def generate_evaluation_code(self, code, result, operand1, needs_evaluation=False):
        ...
        if self.cascade:
            self.cascade.generate_evaluation_code(
                code, result, self.coerced_operand2 or self.operand2, needs_evaluation=self.coerced_operand2 is not None
            )
        if needs_evaluation:
...
def binop_node(pos, operator, operand1, operand2, inplace=False, **kwargs):
    ...
    return binop_node_classes[operator](
        pos, operator=operator, operand1=operand1, operand2=operand2, inplace=inplace, **kwargs
    )
...
class CoercionNode(ExprNode):
    ...
    def annotate(self, code):
        ...
        if self.arg.type != self.type:
            ...
            code.annotate(
                (file, line, col - 1),
                AnnotationItem(style="coerce", tag="coerce", text="[%s] to [%s]" % (self.arg.type, self.type)),
            )
...
class PyTypeTestNode(CoercionNode):
    ...
    def __init__(self, arg, dst_type, env, notnone=False):
        ...
        assert (
            dst_type.is_extension_type or dst_type.is_builtin_type
        ...
        if self.exception_format_args:
            code.putln(
                ...
                % (
                    ...
                    StringEncoding.escape_byte_string(self.exception_message.encode("UTF-8")),
                    ", ".join(['"%s"' % escape(str(arg).encode("UTF-8")) for arg in self.exception_format_args]),
                )
            )
        else:
            code.putln(
                'PyErr_SetString(%s, "%s");'
                % (self.exception_type_cname, escape(self.exception_message.encode("UTF-8")))
            )
...
class CoerceToPyTypeNode(CoercionNode):
    ...
    def generate_result_code(self, code):
        code.putln(
            "%s; %s"
            % (
                self.arg.type.to_py_call_code(self.arg.result(), self.result(), self.target_type),
                code.error_goto_if_null(self.result(), self.pos),
            )
        )
...
class CoerceIntToBytesNode(CoerceToPyTypeNode):
    ...
    def generate_result_code(self, code):
        ...
        if arg.type not in (PyrexTypes.c_char_type, PyrexTypes.c_uchar_type, PyrexTypes.c_schar_type):
            ...
            code.putln(
                "PyErr_SetString(PyExc_OverflowError, "
                '"value too large to pack into a byte"); %s' % (code.error_goto(self.pos))
            )
            code.putln("}")
        temp = None
        ...
        code.putln(
            ...
            % (self.result(), arg_result, code.error_goto_if_null(self.result(), self.pos))
        )
        if temp is not None:
            code.funcstate.release_temp(temp)
        self.generate_gotref(code)
...
class CoerceToBooleanNode(CoercionNode):
    ...
    def generate_result_code(self, code):
        ...
        if test_func is not None:
            ...
            code.putln(
                "%s = __Pyx_PyObject_IsTrue(%s); %s"
                % (self.result(), self.arg.py_result(), code.error_goto_if_neg(self.result(), self.pos))
            )
...
class AnnotationNode(ExprNode):
    ...
    def _warn_on_unknown_annotation(self, env, annotation):
        ...
        if annotation.is_name:
            ...
            if not env.lookup(annotation.name):
                warning(
                    annotation.pos, "Unknown type declaration '%s' in annotation, ignoring" % self.string.value, level=1
                )
            ...
            if not env.lookup(annotation.obj.name):
                ...
                if module_scope and module_scope.pxd_file_loaded:
                    warning(
                        annotation.pos,
                        "Unknown type declaration '%s' in annotation, ignoring" % self.string.value,
                        level=1,
                    )
        else:
    ...
    def analyse_type_annotation(self, env, assigned_value=None):
        ...
        if annotation.is_dict_literal:
            ...
            warning(
                annotation.pos,
                "Tuples cannot be declared as simple tuples of types. Use 'tuple[type1, type2, ...]'.",
                level=1,
            )
            return [], None
        ...
        with env.new_c_type_context(in_c_type_context=explicit_ctype):
            ...
            if explicit_pytype and not explicit_ctype and not (arg_type.is_pyobject or arg_type.equivalent_type):
                warning(
                    annotation.pos, "Python type declaration in signature annotation does not refer to a Python type"
                )
