            ...
            if annotate_defs:
                for stat in block.stats:
                    if isinstance(stat, NameAssignment):
                        label += "\n %s [%s %s]" % (
                            stat.entry.name,
                            "deletion" if stat.is_deletion else "definition",
                            stat.pos[1],
                        )
...
def check_definitions(flow, compiler_directives):
    ...
    for node, entry in references.items():
        if Uninitialized in node.cf_state:
            node.cf_maybe_null = True
            if (
                not entry.from_closure
                and len(node.cf_state) == 1
            ...
            if node.allow_null or entry.from_closure or entry.is_pyclass_attr or entry.type.is_error:
                ...
                if entry.error_on_uninitialized or (
                    Options.error_on_uninitialized and (entry.type.is_pyobject or entry.type.is_unspecified)
                ):
...
class ControlFlowAnalysis(CythonTransform):
    ...
    def visit_FuncDefNode(self, node):
        ...
        if node.star_arg:
            self.flow.mark_argument(
                node.star_arg, TypedExprNode(Builtin.tuple_type, may_be_none=False), node.star_arg.entry
            )
        if node.starstar_arg:
            self.flow.mark_argument(
                node.starstar_arg, TypedExprNode(Builtin.dict_type, may_be_none=False), node.starstar_arg.entry
            )
                        ...
                        if len(sequence.args) > 2:
                            self.mark_assignment(
                                ...
                                self.constant_folder(
                                    ExprNodes.binop_node(node.pos, "+", sequence.args[0], sequence.args[2])
                                ),
                                rhs_scope=node.iterator.expr_scope,
                            )
    ...
    def visit_ForFromStatNode(self, node):
        ...
        if node.step is not None:
            self.mark_assignment(
                node.target, self.constant_folder(ExprNodes.binop_node(node.pos, "+", node.bound1, node.step))
            )
        # Body block
        self.flow.nextblock()
        self._visit(node.body)
