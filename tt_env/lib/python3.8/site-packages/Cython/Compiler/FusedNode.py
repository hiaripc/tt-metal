...
from . import ExprNodes, PyrexTypes, MemoryView, ParseTreeTransforms, StringEncoding, Errors, Naming
...
class FusedCFuncDefNode(StatListNode):
    ...
    def copy_cdef(self, env):
        ...
        for cname, fused_to_specific in permutations:
            ...
            try:
                ...
                error(
                    copied_node.pos,
                    "Return type is a fused type that cannot " "be determined from the function arguments",
                )
            ...
            if copied_node.py_func:
                ...
                self.specialize_copied_def(
                    copied_node.py_func, cname, self.node.entry.as_variable, fused_to_specific, fused_types
                )
        ...
        for buffer_type in all_buffer_types:
            ...
            if dtype.is_typedef:
                if dtype_name not in seen_typedefs:
                    ...
                    decl_code.putln(
                        'ctypedef %s %s "%s"' % (dtype.resolve(), dtype_name, dtype.empty_declaration_code())
                    )
    ...
    def make_fused_cpdef(self, orig_py_func, env, is_def):
        ...
        context = {
            ...
            "min_positional_args": self.node.num_required_args - self.node.num_required_kw_args
            if is_def
            else sum(1 for arg in self.node.args if arg.default is None),
            ...
            if arg.type.is_fused:
                ...
                if len(arg_fused_types) > 1:
                    raise NotImplementedError(
                        "Determination of more than one fused base " "type per argument is not implemented."
                    )
    ...
    def generate_function_definitions(self, env, code):
        ...
        for stat in self.stats:
            if isinstance(stat, FuncDefNode) and (
                stat.entry.used or (Options.cimport_from_pyx and not stat.entry.visibility == "extern")
            ):
    ...
    def generate_execution_code(self, code):
        ...
        if self.__signatures__:
            ...
            code.putln(
                ...
                % (self.resulting_fused_function.result(), self.__signatures__.result())
            )
