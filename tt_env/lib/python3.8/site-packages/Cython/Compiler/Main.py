...
def _make_range_re(chrs):
    ...
    for i in range(0, len(chrs), 2):
        out.append("{0}-{1}".format(chrs[i], chrs[i + 1]))
    return "".join(out)
...
class Context(object):
    ...
    def from_options(cls, options):
        return cls(
            options.include_path, options.compiler_directives, options.cplus, options.language_level, options=options
        )
    ...
    def _check_pxd_filename(pos, pxd_pathname, qualified_name):
        ...
        if "." in qualified_name and qualified_name == os.path.splitext(pxd_filename)[0]:
            warning(
                ...
                " Please use the normal Python package directory layout." % pxd_filename,
                level=1,
            )

    def find_module(
        self, module_name, from_module=None, pos=None, need_pxd=1, absolute_fallback=True, relative_import=False
    ):
        ...
        if debug_find_module:
            print(
                "Context.find_module: module_name = %s, from_module = %s, pos = %s, need_pxd = %s"
        ...
        if not module_name_pattern.match(qualified_name):
            raise CompileError(pos or (module_name, 0, 0), "'%s' is not a valid module name" % module_name)
    ...
    def search_include_directories(
        self, qualified_name, suffix=None, source_pos=None, include=False, sys_path=False, source_file_path=None
    ):
...
def run_pipeline(source, options, full_module_name=None, context=None):
    ...
    if "." in full_module_name and "." in os.path.splitext(os.path.basename(abs_path))[0]:
        warning(
            (source_desc, 1, 0),
            "Dotted filenames ('%s') are deprecated."
            " Please use the normal Python package directory layout." % os.path.basename(abs_path),
            level=1,
        )
