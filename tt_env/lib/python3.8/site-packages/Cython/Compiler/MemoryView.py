...
ERR_UNINITIALIZED = (
    "Cannot check if memoryview %s is initialized without the " "GIL, consider using initializedcheck(False)"
)
...
def put_acquire_memoryviewslice(lhs_cname, lhs_type, lhs_pos, rhs, code, have_gil=False, first_assignment=True):
    ...
    put_assign_to_memviewslice(
        lhs_cname, rhs, rhstmp, lhs_type, code, have_gil=have_gil, first_assignment=first_assignment
    )
...
def put_assign_to_memviewslice(
    lhs_cname, rhs, rhs_cname, memviewslicetype, code, have_gil=False, first_assignment=False
):
...
def get_copy_new_utility(pos, from_memview, to_memview):
    if from_memview.dtype != to_memview.dtype and not (
        from_memview.dtype.is_cv_qualified and from_memview.dtype.cv_base_type == to_memview.dtype
    ):
    ...
    if is_contig:
        ...
        if contig_dim == len(axes) - 1:
            ...
            if contig_dim and not axes_specs[contig_dim - 1][0] in ("full", "ptr"):
                raise CompileError(
                    axes[contig_dim].pos, "Fortran contiguous specifier must follow an indirect dimension"
                )
...
def is_cf_contig(specs):
    ...
    if len(specs) == 1 and specs == [("direct", "contig")]:
        ...
        len(specs) > 1 and specs[0] == ("direct", "contig") and all(axis == ("direct", "follow") for axis in specs[1:])
    ):
...
memviewslice_declare_code = load_memview_c_utility("MemviewSliceStruct", context=context, requires=[])
