...
def generate_c_code_config(env, options):
    ...
    if hasattr(options, "emit_code_comments"):
        print(
            "Warning: option emit_code_comments is deprecated. " "Instead, use compiler directive emit_code_comments."
        )
...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def assure_safe_target(self, path, allow_failed=False):
        ...
        if not is_cython_generated_file(path, allow_failed=allow_failed, if_not_found=True):
            ...
            raise CompileError(
                self.pos,
                'The output file already exists and does not look like it was generated by Cython: "%s"'
    ...
    def generate_h_code(self, env, options, result):
        def h_entries(entries, api=0, pxd=0):
            return [
                ...
                if ((entry.visibility == "public") or (api and entry.api) or (pxd and entry.defined_in_pxd))
            ]
        ...
        if h_types or h_vars or h_funcs or h_extension_types:
            ...
            h_code_main.putln(
                "__attribute__ ((__deprecated__(%s), __unused__)) __inline__" % (warning_string.as_c_string_literal())
            )
            ...
            h_code_main.putln(
                "#define %s() __PYX_WARN_IF_%s_INIT_CALLED(%s())"
    ...
    def generate_cclass_header_code(self, type, h_code):
        h_code.putln(
            "%s %s %s;"
            % (Naming.extern_c_macro, PyrexTypes.public_decl("PyTypeObject", "DL_IMPORT"), type.typeobj_cname)
        )
    ...
    def generate_module_preamble(self, env, options, cimported_modules, metadata, code):
        ...
        )  # CYTHON_COMPILING_IN_LIMITED_API not yet defined
        ...
        code.putln(
            "    { %s = %s[f_index]; (void)%s; %s = lineno; (void)%s; %s (void)%s; }"
            % (
    ...
    def generate_extern_c_macro_definition(self, code, is_cpp):
        ...
        code.putln(
            "    #pragma message (\"Please do not define the '%s' macro externally. Use 'CYTHON_EXTERN_C' instead.\")"
            % name
        )
        code.putln("    #else")
    ...
    def generate_struct_union_predeclaration(self, entry, code):
        ...
        if type.is_cpp_class and type.templates:
            code.putln(
                "template <typename %s>" % ", typename ".join([T.empty_declaration_code() for T in type.templates])
            )
    ...
    def generate_cpp_class_definition(self, entry, code):
        ...
        if scope:
            if type.templates:
                code.putln(
                    "template <class %s>" % ", class ".join([T.empty_declaration_code() for T in type.templates])
                )
    ...
    def generate_typeobj_predeclaration(self, entry, code):
        ...
        if name:
            if entry.visibility == "extern" and not entry.in_cinclude:
                code.putln(
                    "%s %s %s;" % (Naming.extern_c_macro, PyrexTypes.public_decl("PyTypeObject", "DL_IMPORT"), name)
                )
            elif entry.visibility == "public":
                code.putln(
                    "%s %s %s;" % (Naming.extern_c_macro, PyrexTypes.public_decl("PyTypeObject", "DL_EXPORT"), name)
                )
        ...
        for entry in env.c_class_entries:
            if definition or entry.defined_in_pxd:
                ...
                module_state_defines.putln(
                    "#define %s %s->%s"
                    % (entry.type.typeptr_cname, Naming.modulestateglobal_cname, entry.type.typeptr_cname)
                )
                ...
                if entry.type.typeobj_cname is not None:
                    ...
                    module_state_defines_typeobj.putln(
                        "#define %s %s->%s"
                        % (entry.type.typeobj_cname, Naming.modulestateglobal_cname, entry.type.typeobj_cname)
                    )
    ...
    def generate_cvariable_declarations(self, env, code, definition):
        ...
        for entry in env.var_entries:
            if (
                entry.in_cinclude
                or entry.in_closure
                or (entry.visibility == "private" and not (entry.defined_in_pxd or entry.used))
            ):
                continue
                ...
                if scope:  # could be None if there was an error
                    ...
                    if scope.defines_any_special(["__getslice__", "__setslice__", "__delslice__"]):
                        warning(
                            ...
                            "use __getitem__, __setitem__, and __delitem__ instead",
                            1,
                        )
        ...
        if base_type:
            ...
            if freelist_size:
                ...
                if is_final_type:
                    ...
                    type_safety_check = (
                        " & (int)(!__Pyx_PyType_HasFeature(t, (Py_TPFLAGS_IS_ABSTRACT | Py_TPFLAGS_HEAPTYPE)))"
                    )
    ...
    def generate_dealloc_function(self, scope, code):
        ...
        for entry in py_attrs + memoryview_slices:
            code.put_xdecref_clear(
                "p->%s" % entry.cname, entry.type, nanny=False, clear_before_decref=True, have_gil=True
            )
        ...
        if base_type:
            ...
            if tp_dealloc is not None:
                ...
                code.putln(
                    "if (likely(%s)) __Pyx_PyType_GetSlot(%s, tp_dealloc, destructor)(o); "
                    "else __Pyx_call_next_tp_dealloc(o, %s);" % (base_cname, base_cname, slot_func_cname)
                )
            ...
            if freelist_size:
                ...
                code.putln(
                    ...
                    % (freecount_name, freelist_size, type.declaration_code("", deref=True), type_safety_check)
                )
    ...
    def generate_ass_subscript_function(self, scope, code):
        ...
        code.putln("")
        code.putln(
            "static int %s(PyObject *o, PyObject *i, PyObject *v) {" % (scope.mangle_internal("mp_ass_subscript"))
        )
        code.putln("if (v) {")
        if set_entry:
    ...
    def generate_richcmp_function(self, scope, code):
        ...
        if extern_parent and extern_parent.typeptr_cname:
            code.putln(
                "if (likely(%s->tp_richcompare)) return %s->tp_richcompare(o1, o2, op);"
    ...
    def generate_binop_function(self, scope, slot, code, pos):
        ...
        if get_slot_method_cname(slot.left_slot.method_name) and not get_slot_method_cname(slot.right_slot.method_name):
            warning(
                pos,
                "Extension type implements %s() but not %s(). "
    ...
    def generate_descr_get_function(self, scope, code):
        ...
        code.putln("")
        code.putln(
            "static PyObject *%s(PyObject *o, PyObject *i, PyObject *c) {" % (scope.mangle_internal("tp_descr_get"))
        )
    ...
    def generate_getset_table(self, env, code):
        if env.property_entries:
            ...
            for entry in env.property_entries:
                ...
                code.putln(
                    "{(char *)%s, %s, %s, (char *)%s, 0},"
                    % (entry.name.as_c_string_literal(), entry.getter_cname or "0", entry.setter_cname or "0", doc_code)
                )
        ...
        for name, entry in sorted(env.entries.items()):
            if entry.is_cglobal and entry.used and not entry.type.is_const:
                ...
                if entry.type.is_pyobject:
                    ...
                    code.putln(
                        'PyErr_Format(PyExc_TypeError, "Cannot convert Python object %s to %s");' % (name, entry.type)
                    )
    ...
    def generate_module_state_end(self, env, modules, globalstate):
        ...
        module_state.putln("")
        module_state.putln(
            "#define %s(o) ((%s *)__Pyx_PyModule_GetState(o))" % (Naming.modulestate_cname, Naming.modulestate_cname)
        )
        module_state.putln("")
        module_state.putln(
            "#define %s (%s(PyState_FindModule(&%s)))"
            % (Naming.modulestateglobal_cname, Naming.modulestate_cname, Naming.pymoduledef_cname)
        )
        module_state.putln("")
        ...
        module_state.putln(
            "static %s *%s = &%s_static;"
            % (Naming.modulestate_cname, Naming.modulestateglobal_cname, Naming.modulestateglobal_cname)
        )
        module_state.putln("#endif")
    ...
    def generate_module_state_defines(self, env, code):
        ...
        code.putln(
            "#define %s %s->%s"
            % (Naming.cython_runtime_cname, Naming.modulestateglobal_cname, Naming.cython_runtime_cname)
        )
        ...
        if Options.pre_import is not None:
            code.putln(
                "#define %s %s->%s" % (Naming.preimport_cname, Naming.modulestateglobal_cname, Naming.preimport_cname)
            )
    ...
    def generate_module_init_func(self, imported_modules, env, code):
        ...
        if wrong_punycode_module_name:
            ...
            code.putln(
                "void %s(void) {} /* workaround for https://bugs.python.org/issue39432 */" % wrong_punycode_module_name
            )
            code.putln("#endif")
        code.putln(header3)
        ...
        code.putln(
            "static CYTHON_SMALL_CODE int %s(PyObject *%s)"
            % (self.module_init_func_cname(), Naming.pymodinit_module_arg)
        )
        ...
        code.putln("#endif")
        code.putln(
            "%s = PyTuple_New(0); %s" % (Naming.empty_tuple, code.error_goto_if_null(Naming.empty_tuple, self.pos))
        )
        code.putln(
            '%s = PyBytes_FromStringAndSize("", 0); %s'
            % (Naming.empty_bytes, code.error_goto_if_null(Naming.empty_bytes, self.pos))
        )
        code.putln(
            '%s = PyUnicode_FromStringAndSize("", 0); %s'
    ...
    def generate_module_import_setup(self, env, code):
        ...
        if module_path:
            code.putln("if (!CYTHON_PEP489_MULTI_PHASE_INIT) {")
            code.putln(
                'if (PyObject_SetAttrString(%s, "__file__", %s) < 0) %s;'
                % (
                    env.module_cname,
                    code.globalstate.get_py_string_const(EncodedString(decode_filename(module_path))).cname,
                ...
                code.putln(
                    ...
                    % (
                        ...
                        code.globalstate.get_py_string_const(
                            EncodedString(decode_filename(os.path.dirname(module_path)))
                        ).cname,
            ...
            code.putln(
                code.error_goto_if_neg(
                    ...
                    % (code.globalstate.get_py_string_const(EncodedString(self.full_module_name)).cname),
                    self.pos,
                )
            )
            code.putln("}")
        ...
        code.putln(
            code.error_goto_if_neg(
                "PyDict_SetItemString(modules, %s, %s)" % (fq_module_name_cstring, env.module_cname), self.pos
            )
        )
        code.putln("}")
        code.putln("}")
    ...
    def generate_pymoduledef_struct(self, env, code):
        ...
        code.putln(
            "static PyObject* %s(PyObject *spec, PyModuleDef *def); /*proto*/" % Naming.pymodule_create_func_cname
        )
    ...
    def generate_module_creation_code(self, env, code):
        ...
        code.putln(
            ...
            % (env.module_cname, env.module_name.as_c_string_literal(), env.method_table_cname, doc, env.module_cname)
        )
        ...
        code.putln(
            ...
            % (module_temp, Naming.pymoduledef_cname, code.error_goto_if_null(module_temp, self.pos))
        )
        code.putln("{")
        ...
        code.putln(
            "%s = 0; /* transfer ownership from %s to %s pseudovariable */"
        ...
        code.putln(
            ...
            % (env.module_dict_cname, env.module_cname, code.error_goto_if_null(env.module_dict_cname, self.pos))
        )
        code.put_incref(env.module_dict_cname, py_object_type, nanny=False)

        code.putln(
            "%s = __Pyx_PyImport_AddModuleRef(__Pyx_BUILTIN_MODULE_NAME); %s"
            % (Naming.builtins_cname, code.error_goto_if_null(Naming.builtins_cname, self.pos))
        )
        code.putln(
            '%s = __Pyx_PyImport_AddModuleRef((const char *) "cython_runtime"); %s'
            % (Naming.cython_runtime_cname, code.error_goto_if_null(Naming.cython_runtime_cname, self.pos))
        )
        code.putln(
            'if (PyObject_SetAttrString(%s, "__builtins__", %s) < 0) %s'
            % (env.module_cname, Naming.builtins_cname, code.error_goto(self.pos))
        )
        if Options.pre_import is not None:
            code.putln(
        ...
        for name, entry in sorted(env.entries.items()):
            if entry.create_wrapper and not entry.is_type and entry.scope is env:
                ...
                code.putln(
                    ...
                    % (env.module_cname, name, code.error_goto(entry.pos))
                )
                code.putln("}")
        ...
        if entries:
            ...
            for entry in entries:
                ...
                code.putln(
                    ...
                    % (name, entry.cname, signature, code.error_goto(self.pos))
                )
        ...
        if entries:
            ...
            for entry in entries:
                ...
                code.putln(
                    ...
                    % (entry.name.as_c_string_literal(), entry.cname, signature, code.error_goto(self.pos))
                )
    ...
    def generate_c_variable_import_code_for_module(self, module, env, code):
        ...
        if entries:
            ...
            code.putln(
                ...
                % (temp, module.qualified_name, temp, code.error_goto(self.pos))
            )
            code.put_gotref(temp, py_object_type)
            for entry in entries:
                if env is module:
                ...
                code.putln(
                    ...
                    % (Naming.cyversion, temp, entry.name, cname, signature, code.error_goto(self.pos))
                )
            code.put_decref_clear(temp, py_object_type)
            code.funcstate.release_temp(temp)
    ...
    def generate_base_type_import_code(self, env, entry, code, import_generator):
        ...
        if (
            base_type
            and base_type.module_name != env.qualified_name
            and not (base_type.is_builtin_type or base_type.is_cython_builtin_type)
    ...
    def generate_type_import_code(self, env, type, pos, code, import_generator):
        ...
        if type.vtabptr_cname:
            ...
            code.putln(
                "%s = (struct %s*)__Pyx_GetVtable(%s); %s"
                % (
                    type.vtabptr_cname,
    ...
    def generate_type_import_call(self, type, code, import_generator, error_code=None, error_pos=None):
        ...
        if sizeof_objstruct != objstruct:
            ...
            code.putln("#else")
            code.putln(
                "sizeof(%s), __PYX_GET_STRUCT_ALIGNMENT_%s(%s),"
        ...
        if type.check_size and type.check_size in ("error", "warn", "ignore"):
            ...
            raise RuntimeError(
                "invalid value for check_size '%s' when compiling %s.%s" % (type.check_size, module_name, type.name)
            )
...
class ModuleImportGenerator(object):
    ...
    def imported_module(self, module_name_string, error_code):
        ...
        code.putln(
            "%s = PyImport_ImportModule(%s); if (unlikely(!%s)) %s" % (temp, module_name_string, temp, error_code)
        )
        code.put_gotref(temp, py_object_type)
...
def generate_cfunction_declaration(entry, env, code, definition):
    ...
    if (
        entry.used
        and entry.inline_func_in_pxd
        or (
            not entry.in_cinclude
            and (definition or entry.defined_in_pxd or entry.visibility == "extern" or from_cy_utility)
        )
    ):
        if entry.visibility == "extern":
            storage_class = Naming.extern_c_macro
...
packed_struct_utility_code = UtilityCode(
    proto="""
#if defined(__GNUC__)
