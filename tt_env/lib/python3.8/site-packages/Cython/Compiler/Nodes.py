...
cython.declare(
    ...
    LocalScope=object,
    ClosureScope=object,
    StructOrUnionScope=object,
    PyClassScope=object,
    CppClassScope=object,
    UtilityCode=object,
    ...
    def analyse(self, base_type, env, nonempty=0, visibility=None, in_pxd=False):
        ...
        if base_type.is_fused and env.fused_to_specific:
            try:
                ...
                error(
                    self.pos,
                    "'%s' cannot be specialized since its type is not a fused argument to this function" % self.name,
                )
...
class CArrayDeclaratorNode(CDeclaratorNode):
    ...
    def analyse(self, base_type, env, nonempty=0, visibility=None, in_pxd=False):
        if (
            (base_type.is_cpp_class and base_type.is_template_type())
            or base_type.is_cfunction
            or base_type.python_type_constructor_name
        ):
...
class CFuncDeclaratorNode(CDeclaratorNode):
    ...
    def analyse(self, return_type, env, nonempty=0, directive_locals=None, visibility=None, in_pxd=False):
        ...
        if (
            env.directives["legacy_implicit_noexcept"]
        ...
        if return_type.is_pyobject and (self.exception_value or self.exception_check) and self.exception_check != "+":
            ...
            return_type.is_pyobject
            and not self.exception_check
            and visibility != "extern"
            and self.has_explicit_exc_clause
        ):
            ...
            if self.exception_value:
                if self.exception_check == "+":
                    ...
                    if (
                        not exc_val_type.is_error
                        and not exc_val_type.is_pyobject
                        and not (
                            exc_val_type.is_cfunction
                            and not exc_val_type.return_type.is_pyobject
                            and not exc_val_type.args
                        )
                        and not (exc_val_type == PyrexTypes.c_char_type and self.exception_value.value == "*")
                    ):
                        error(
                            self.exception_value.pos,
                    ...
                    if (
                        ...
                        try:
                            ...
                            if self.exception_value.constant_result == type_default_value:
                                warning(
                                    self.pos,
                                    "Ambiguous exception value, same as default return value: %r"
    ...
    def declare_optional_arg_struct(self, func_type, env, fused_cname=None):
        ...
        op_args_struct = env.global_scope().declare_struct_or_union(
            name=struct_cname, kind="struct", scope=scope, typedef_flag=0, pos=self.pos, cname=struct_cname
        )
...
class TemplatedTypeNode(CBaseTypeNode):
    ...
    def _analyse_template_types(self, env, base_type):
        ...
        for template_node in self.positional_args:
            ...
            if ttype is None:
                ...
                require_python_types and not ttype.is_pyobject or require_optional_types and not ttype.can_be_optional()
            ):
    ...
    def analyse(self, env, could_be_name=False, base_type=None):
        ...
        if (base_type.is_cpp_class and base_type.is_template_type()) or base_type.python_type_constructor_name:
            ...
            options = Buffer.analyse_buffer_options(
                self.pos, env, self.positional_args, self.keyword_args, base_type.buffer_defaults
            )
    ...
    def analyse_pytyping_modifiers(self, env):
        ...
        modifier_node = self
        while (
            modifier_node.is_templated_type_node
            and modifier_node.base_type_node
            and len(modifier_node.positional_args) == 1
        ):
...
class CppClassNode(CStructOrUnionDefNode, BlockNode):
    ...
    def declare(self, env):
        if self.templates is None:
            ...
            template_types = [
                PyrexTypes.TemplatePlaceholderType(template_name, not required)
        ...
        self.entry = env.declare_cpp_class(
            self.name, None, self.pos, self.cname, base_classes=[], visibility=self.visibility, templates=template_types
        )
        ...
        if self.visibility == "public" or self.api:
            ...
            for item in self.entry.enum_values:
                ...
                code.putln(
                    ...
                    % (Naming.moddict_cname, item.name, temp, code.error_goto(item.pos))
                )
...
class FuncDefNode(StatNode, BlockNode):
    ...
    def generate_function_definitions(self, env, code):
        ...
        if self.needs_outer_scope:
            if self.is_cyfunction:
                code.putln(
                    ...
                    % (outer_scope_cname, cenv.scope_class.type.empty_declaration_code(), Naming.self_cname)
                )
            else:
                code.putln(
                    "%s = (%s) %s;"
        ...
        if self.entry.is_special and self.entry.name == "__hash__":
            ...
            code.putln(
                "if (unlikely(%s == -1) && !PyErr_Occurred()) %s = -2;" % (Naming.retval_cname, Naming.retval_cname)
            )
    ...
    def generate_arg_none_check(self, arg, code):
        ...
        code.putln(
            """PyErr_Format(PyExc_TypeError, "Argument '%%.%ds' must not be None", %s); %s"""
            % (max(200, len(arg.name_cstring)), arg.name_cstring, code.error_goto(arg.pos))
        )
        code.putln("}")
            ...
            for arg in self.args:
                if arg.default:
                    ...
                    if self.override or entry.cf_used:
                        ...
                        code.putln(
                            "%s = %s->%s;"
                            % (arg.cname, Naming.optional_args_cname, self.type.opt_arg_cname(declarator.name))
                        )
                        used += 1
                    i += 1
            for _ in range(used):
...
class DefNode(FuncDefNode):
    ...
    def as_cfunction(
        ...
        if cfunc is None:
            ...
            for i, (formal_arg, type_arg) in enumerate(zip(self.args, cfunc_type.args)):
                name_declarator, type = formal_arg.analyse(
                    scope, nonempty=1, is_self_arg=(i == 0 and scope.is_c_class_scope)
                )
        ...
        if env.is_py_class_scope or env.is_c_class_scope:
            if self.name == "__new__" and env.is_py_class_scope:
                ...
                self.decorators.insert(
                    0, DecoratorNode(self.pos, decorator=NameNode(self.pos, name=EncodedString("classmethod")))
                )
        ...
        for arg in self.args:
            ...
            if arg.type.is_pyobject or arg.type.is_buffer or arg.type.is_memoryviewslice:
                if arg.or_none:
                    ...
                    arg.type.is_extension_type
                    or arg.type.is_builtin_type
                    or arg.type.is_buffer
                    or arg.type.is_memoryviewslice
                ):
    ...
    def analyse_signature(self, env):
        if self.entry.is_special:
            ...
            not env.directives["always_allow_keywords"] or all([arg.pos_only for arg in self.args])
        ):
    ...
    def bad_signature(self):
        ...
        error(
            self.pos,
            "%s %s has wrong number of arguments (%d declared, %s expected)"
    ...
    def declare_pyfunction(self, env):
        ...
        if Options.docstrings:
            ...
            if entry.is_special:
                if (
                    entry.name in TypeSlots.invisible
                    or not entry.doc
                    or (entry.name in "__getattr__" and env.directives["fast_getattr"])
                ):
...
class DefNodeWrapper(FuncDefNode):
    ...
    def generate_function_header(self, code, with_pymethdef, proto_only=0):
        ...
        if (
            Options.docstrings
            and entry.doc
            and not self.target.fused_py_func
            and not entry.scope.is_property_scope
            and (not entry.is_special or entry.wrapperbase_cname)
        ):
    ...
    def generate_argument_parsing_code(self, env, code, decl_code):
        ...
        if self.signature_has_generic_args():
            ...
            code.putln(
                "%s = PyTuple_Size(%s); if (%s) return %s;"
                % (
                    Naming.nargs_cname,
                    Naming.args_cname,
    ...
    def generate_stararg_copy_code(self, code):
        ...
        if self.starstar_arg and self.starstar_arg.entry.cf_used:
            ...
            code.putln(
                ...
                % (self.starstar_arg.entry.cname, self.signature.fastvar, Naming.kwds_cname, Naming.kwvalues_cname)
            )
    ...
    def generate_tuple_and_keyword_parsing_code(self, args, code, decl_code):
        ...
        if accept_kwd_args:
            ...
            code.putln(
                "if (likely(__Pyx_ParseOptionalKeywords(%s, %s, %s, %s, %s, %s, %s) < 0)) %s"
                % (
                    Naming.kwds_cname,
        ...
        if code.label_used(argtuple_error_label):
            ...
            code.globalstate.use_utility_code(UtilityCode.load_cached("RaiseArgTupleInvalid", "FunctionArguments.c"))
            code.putln(
                "__Pyx_RaiseArgtupleInvalid(%s, %d, %d, %d, %s); %s"
                % (
                    self_name_csafe,
                    has_fixed_positional_count,
                    min_positional_args,
    ...
    def generate_stararg_init_code(self, max_positional_args, code):
        if self.starstar_arg:
            ...
            code.putln(
                "%s = PyDict_New(); if (unlikely(!%s)) return %s;"
        ...
        if self.star_arg:
            ...
            if max_positional_args == 0:
                ...
                code.putln(
                    "if (unlikely(!%s)) {" % self.star_arg.entry.type.nullcheck_string(self.star_arg.entry.cname)
                )
                if self.starstar_arg:
                    code.put_var_decref_clear(self.starstar_arg.entry)
                code.put_finish_refcount_context()
    ...
    def generate_argument_values_setup_code(self, args, code, decl_code):
        ...
        if self.target.defaults_struct:
            code.putln(
                ...
                % (
                    self.target.defaults_struct,
                    Naming.dynamic_args_cname,
                    self.target.defaults_struct,
                    Naming.self_cname,
                )
            )
        ...
        for i, arg in enumerate(args):
            if arg.default and arg.type.is_pyobject:
                ...
                code.putln(
                    "values[%d] = __Pyx_Arg_NewRef_%s(%s);"
    ...
    def generate_argument_values_cleanup_code(self, code):
        ...
        code.putln(
            "for (%s=0; %s < (Py_ssize_t)(sizeof(values)/sizeof(values[0])); ++%s) {" % (loop_var, loop_var, loop_var)
        )
        code.putln("__Pyx_Arg_XDECREF_%s(values[%s]);" % (self.signature.fastvar, loop_var))
        code.putln("}")
        code.putln("}")

    def generate_keyword_unpacking_code(
        self,
        min_positional_args,
        max_positional_args,
        has_fixed_positional_count,
        ...
        if self.num_required_args or max_positional_args > 0:
            ...
            for i, arg in enumerate(all_args[num_pos_only_args : last_required_arg + 1], num_pos_only_args):
                ...
                if arg.default:
                    ...
                    code.putln(
                        "PyObject* value = __Pyx_GetKwValue_%s(%s, %s, %s);"
                        % (self.signature.fastvar, Naming.kwds_cname, Naming.kwvalues_cname, pystring_cname)
                    )
                    code.putln(
                        "if (value) { values[%d] = __Pyx_Arg_NewRef_%s(value); kw_args--; }"
                    ...
                    code.putln(
                        "if (likely((values[%d] = __Pyx_GetKwValue_%s(%s, %s, %s)) != 0)) {"
        ...
        code.globalstate.use_utility_code(UtilityCode.load_cached("ParseKeywords", "FunctionArguments.c"))
        code.putln(
            "if (unlikely(__Pyx_ParseOptionalKeywords(%s, %s, %s, %s, %s, %s, %s) < 0)) %s"
            % (
                Naming.kwds_cname,
                Naming.kwvalues_cname,
                Naming.pykwdlist_cname,
    ...
    def generate_optional_kwonly_args_unpacking_code(self, all_args, code):
        ...
        if optional_args:
            if len(optional_args) > 1:
                ...
                code.putln(
                    "if (kw_args > 0 && %s(kw_args <= %d)) {"
                    % (not self.starstar_arg and "likely" or "", len(optional_args))
                )
                code.putln("Py_ssize_t index;")
                # not unrolling the loop here reduces the C code overhead
                code.putln(
                    "for (index = %d; index < %d && kw_args > 0; index++) {"
    ...
    def generate_arg_conversion_to_pyobject(self, arg, code):
        ...
        if func:
            code.putln(
                "%s = %s(%s); %s"
                % (arg.entry.cname, func, arg.hdr_cname, code.error_goto_if_null(arg.entry.cname, arg.pos))
            )
        ...
        if self.target.entry.is_special:
            ...
            if self.target.entry.name == "__ipow__" and len(self.args) != 2:
                ...
                code.putln(
                    ...
                    '"3-argument %s cannot be used in Python<3.8");' % (self.target.entry.qualified_name)
                )
                code.putln("%s;" % code.error_goto(self.pos))
                code.putln("}")
    ...
    def generate_function_body(self, env, code):
        ...
        code.putln(
            ...
            % (
                self.gen_type_name,
                body_cname,
                self.code_object.calculate_result_code(code) if self.code_object else "NULL",
                Naming.cur_scope_cname,
                name,
                qualname,
                module_name,
...
class GeneratorBodyDefNode(DefNode):
    ...
    def generate_function_definitions(self, env, code):
        ...
        if self.is_inlined and self.inlined_comprehension_type is not None:
            ...
            code.putln(
                "%s = %s; %s" % (Naming.retval_cname, comp_init, code.error_goto_if_null(Naming.retval_cname, self.pos))
            )
        ...
        if lenv.scope_class.type.scope.var_entries:
            closure_init_code.putln(
                "%s = %s;"
                % (
                    lenv.scope_class.type.declaration_code(Naming.cur_scope_cname),
                    lenv.scope_class.type.cast_code("%s->closure" % Naming.generator_cname),
                )
            )
...
class OverrideCheckNode(StatNode):
    ...
    def analyse_expressions(self, env):
        ...
        if env.return_type.is_void or env.return_type.is_returncode:
            self.body = StatListNode(
                self.pos, stats=[ExprStatNode(self.pos, expr=call_node), ReturnStatNode(self.pos, value=None)]
            )
        else:
    ...
    def generate_execution_code(self, code):
        ...
        # instance at a time.
        code.putln(
            "static PY_UINT64_T %s = __PYX_DICT_VERSION_INIT, %s = __PYX_DICT_VERSION_INIT;"
            % (Naming.tp_dict_version_temp, Naming.obj_dict_version_temp)
        )
        code.putln(
            "if (unlikely(!__Pyx_object_dict_version_matches(%s, %s, %s))) {"
        ...
        code.putln(
            ...
            % (func_node_temp, self_arg, interned_attr_cname, code.error_goto_if_null(func_node_temp, self.pos))
        )
...
class PyClassDefNode(ClassDefNode):
    ...
    update_bases_functype = PyrexTypes.CFuncType(
        PyrexTypes.py_object_type, [PyrexTypes.CFuncTypeArg("bases", PyrexTypes.py_object_type, None)]
    )
        ...
        if self.orig_bases:
            ...
            code.putln(
                code.error_goto_if_neg(
                    'PyDict_SetItemString(%s, "__orig_bases__", %s)' % (self.dict.result(), self.orig_bases.result()),
                    self.pos,
                )
            )
            code.putln("}")
...
class CClassDefNode(ClassDefNode):
        ...
        if not hasattr(self, "_buffer_defaults"):
            ...
            if self.buffer_defaults_node:
                self._buffer_defaults = Buffer.analyse_buffer_options(
                    self.buffer_defaults_pos, env, [], self.buffer_defaults_node, need_complete=False
                )
            else:
    ...
    def analyse_declarations(self, env):
        ...
        if self.bases.args:
            ...
            if base_type is None:
                ...
                error(
                    base.pos, "inheritance from PyVarObject types like '%s' is not currently supported" % base_type.name
                )
            else:
        ...
        if self.visibility == "extern":
            if (
                self.module_name == "__builtin__"
                and self.class_name in Builtin.builtin_types
                and env.qualified_name[:8] != "cpython."
            ):  # allow overloaded names for cimporting from cpython
        ...
        if len(self.bases.args) > 1:
            ...
            self.type_init_args = ExprNodes.TupleNode(
                self.pos,
                args=[
                    ExprNodes.IdentifierStringNode(self.pos, value=self.class_name),
                    self.bases,
    ...
    def generate_execution_code(self, code):
        ...
        if not self.entry.type.early_init:
            ...
            if self.type_init_args:
                ...
                code.putln(
                    "%s = __Pyx_PyType_GetSlot(&PyType_Type, tp_new, newfunc)(&PyType_Type, %s, NULL);"
                ...
                code.putln(
                    "if (__Pyx_PyType_GetSlot((PyTypeObject*) %s, tp_base, PyTypeObject*) != %s) {"
    ...
    def generate_type_ready_code(entry, code, bases_tuple_cname=None, check_heap_type_bases=False):
        ...
        # TODO: remove 'else:' and dedent
            ...
            if bases_tuple_cname or tuple_temp:
                if check_heap_type_bases:
                    ...
                    code.put_error_if_neg(
                        entry.pos,
                        "__Pyx_validate_bases_tuple(%s.name, %s, %s)"
                        % (
                            typespec_cname,
                ...
                code.putln(
                    "%s = (PyTypeObject *) __Pyx_PyType_FromModuleAndSpec(%s, &%s, %s);"
                    % (
                        typeptr_cname,
                        Naming.module_cname,
                        typespec_cname,
            ...
            if not buffer_slot.is_empty(scope):
                ...
                for buffer_method_name in ("__getbuffer__", "__releasebuffer__"):
                    ...
                    if buffer_slot.slot_code(scope) == "0" and not TypeSlots.get_base_slot_function(scope, buffer_slot):
                        ...
                        code.putln(
                            "%s->tp_as_buffer->%s = %s->tp_base->tp_as_buffer->%s;"
                            % (
                                typeptr_cname,
                                buffer_slot.slot_name,
                                typeptr_cname,
            ...
            code.put_error_if_neg(
                entry.pos, "__Pyx_fix_up_extension_type_from_spec(&%s, %s)" % (typespec_cname, typeptr_cname)
            )
            ...
            while base_type:
                if base_type.is_external and not base_type.objstruct_cname == "PyTypeObject":
                    ...
                    code.putln(
                        "if (sizeof(%s%s) != sizeof(%s%s)) {"
                        % (
                            "" if type.typedef_flag else "struct ",
                            type.objstruct_cname,
                            "" if base_type.typedef_flag else "struct ",
            ...
            for func in entry.type.scope.pyfunc_entries:
                ...
                if func.is_special and Options.docstrings and func.wrapperbase_cname and not is_buffer:
                    ...
                    code.putln(
                        ...
                        % (typeptr_cname, func.name, code.error_goto_if_null("wrapper", entry.pos))
                    )
            ...
            if not type.scope.is_internal and not type.scope.directives.get("internal"):
                ...
                code.put_error_if_neg(
                    entry.pos,
                    "PyObject_SetAttr(%s, %s, (PyObject *) %s)"
                    % (
                        Naming.module_cname,
            ...
            if weakref_entry:
                if weakref_entry.type is py_object_type:
                    ...
                    code.putln(
                        ...
                        % (tp_weaklistoffset, tp_weaklistoffset, objstruct, weakref_entry.cname)
                    )
                else:
    ...
    def analyse_declarations(self, env):
        ...
        if self.declaration_only:
            ...
            if self.is_assignment_expression:
                ...
                if (
                    (self.lhs.is_attribute or self.lhs.is_name)
                    and self.lhs.entry
                    and not self.lhs.entry.known_standard_library_import
                ):
                ...
                if stop_node:
                    ...
                    if node.type.is_array and node.type.size:
                        stop_node = ExprNodes.IntNode(
                            self.pos,
                            value=str(node.type.size),
                            constant_result=(
                                node.type.size
                                if isinstance(node.type.size, _py_int_types)
...
class PrintStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if len(self.arg_tuple.args) == 1 and self.append_newline:
            ...
            code.putln(
                "if (__Pyx_PrintOne(%s, %s) < 0) %s" % (stream_result, arg.py_result(), code.error_goto(self.pos))
            )
            ...
            code.putln(
                ...
                % (stream_result, self.arg_tuple.py_result(), self.append_newline, code.error_goto(self.pos))
            )
...
class DelStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        for arg in self.args:
            if (
                arg.type.is_pyobject
                or arg.type.is_memoryviewslice
                or arg.is_subscript
                and arg.base.type is Builtin.bytearray_type
            ):
...
class ReturnStatNode(StatNode):
    ...
    def analyse_expressions(self, env):
        ...
        if self.value:
            ...
            if not return_type.is_void and not return_type.is_pyobject and not return_type.is_returncode:
...
class RaiseStatNode(StatNode):
    ...
    def analyse_expressions(self, env):
        ...
        if self.exc_type and not self.exc_value and not self.exc_tb:
            ...
            if isinstance(exc, ExprNodes.SimpleCallNode) and not (
                exc.args or (exc.arg_tuple is not None and exc.arg_tuple.args)
            ):
            ...
            if exc.is_name and exc.entry.is_builtin:
                ...
                if self.builtin_exc_name == "MemoryError":
                    ...
                    self.builtin_exc_name == "StopIteration"
                    and env.is_local_scope
                    and env.name == "__next__"
                    and env.parent_scope
                    and env.parent_scope.is_c_class_scope
                    and not self.in_try_block
                ):
...
class DictIterationNextNode(Node):
    ...
    def generate_execution_code(self, code):
        ...
        assignments = []
        temp_addresses = []
        for var, result, target in [
            (self.key_ref, self.coerced_key_var, self.key_target),
        ...
        code.putln(
            "%s = __Pyx_dict_iter_next(%s, %s, &%s, %s, %s, %s, %s);"
            % (
                result_temp,
...
class SetIterationNextNode(Node):
    ...
    def generate_execution_code(self, code):
        ...
        result_temp = code.funcstate.allocate_temp(PyrexTypes.c_int_type, False)
        code.putln(
            "%s = __Pyx_set_iter_next(%s, %s, &%s, &%s, %s);"
            % (
                result_temp,
...
class _ForInStatNode(LoopNode, StatNode):
    ...
    def analyse_expressions(self, env):
        ...
        if (
            not self.is_async
            and (self.iterator.type.is_ptr or self.iterator.type.is_array)
            and self.target.type.assignable_from(self.iterator.type)
        ):
    ...
    def generate_handling_code(self, code, end_label):
        ...
        if self.pattern:
            has_non_literals = not all(
                pattern.is_literal or pattern.is_simple() and not pattern.is_temp for pattern in self.pattern
            )
            ...
            if exc_type:
                ...
                if len(patterns) == 2:
                    exc_tests.append(
                        "__Pyx_PyErr_GivenExceptionMatches2(%s, %s, %s)"
                        % (
                            exc_type,
                            patterns[0],
                            patterns[1],
                    ...
                    exc_tests.extend(
                        "__Pyx_PyErr_GivenExceptionMatches(%s, %s)" % (exc_type, pattern) for pattern in patterns
                    )
            elif len(patterns) == 2:
                code.globalstate.use_utility_code(UtilityCode.load_cached("FastTypeChecks", "ModuleSetupCode.c"))
                exc_tests.append(
                    "__Pyx_PyErr_ExceptionMatches2(%s, %s)"
                    % (
                        patterns[0],
                        patterns[1],
                    )
        ...
        if preserve_error:
            ...
            if needs_success_cleanup:
                exc_lineno_cnames = tuple(
                    [code.funcstate.allocate_temp(PyrexTypes.c_int_type, manage_ref=False) for _ in range(2)]
                )
                exc_filename_cname = code.funcstate.allocate_temp(
                    PyrexTypes.CPtrType(PyrexTypes.c_const_type(PyrexTypes.c_char_type)), manage_ref=False
                )
            else:
            ...
            if old_label == return_label:
                ...
                if not self.finally_clause.is_terminator:
                    ...
                    if (
                        ...
                        and not self.is_try_finally_in_nogil
                        and not isinstance(self.finally_clause, GILExitNode)
                    ):
...
class GILStatNode(NogilTryFinallyStatNode):
    ...
    def __init__(self, pos, state, body, condition=None):
        ...
        TryFinallyStatNode.__init__(
            self, pos, body=body, finally_clause=GILExitNode(pos, state=state, state_temp=self.state_temp)
        )
...
def cimport_numpy_check(node, code):
    ...
    for mod in code.globalstate.module_node.scope.cimported_modules:
        ...
        if (import_array or _import_array) and not used:  # at least one entry found
            ...
            if name == "*":
                ...
                if entry:
                    ...
                    submodule_scope = env.context.find_module(
                        name,
                        from_module=module_scope,
                        pos=self.pos,
                        absolute_fallback=False,
                        relative_import=is_relative_import,
                    )
            ...
            if name == "*":
                ...
                if entry.is_type and entry.type.name == name and hasattr(entry.type, "module_name"):
                    ...
                    try:
                        ...
                        module = env.find_module(
                            self.module.module_name.value, pos=self.pos, relative_level=self.module.level
                        )
    ...
    def analyse_expressions(self, env):
        ...
        if self.num_threads is not None:
            if self.parent and self.parent.num_threads is not None and not self.parent.is_prange:
                ...
                self.num_threads.type.is_int
                and self.num_threads.is_literal
                and self.num_threads.compile_time_value(env) <= 0
            ):
    ...
    def save_parallel_vars(self, code):
        ...
        for entry, (op, lastprivate) in sorted(self.privates.items()):
            ...
            if (
                ...
                and not entry.type.is_fake_reference
                and code.globalstate.directives["cpp_locals"]
            ):
    ...
    def analyse_expressions(self, env):
        ...
        if node.chunksize:
            if not node.schedule:
                ...
                node.chunksize.type.is_int and node.chunksize.is_literal and node.chunksize.compile_time_value(env) <= 0
            ):
