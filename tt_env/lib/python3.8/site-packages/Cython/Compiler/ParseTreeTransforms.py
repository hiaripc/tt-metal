    ...
    def visit_ExceptClauseNode(self, node):
        if node.is_except_as:
            ...
            del_target = Nodes.DelStatNode(
                node.pos, args=[ExprNodes.NameNode(node.target.pos, name=node.target.name)], ignore_nonexisting=True
            )
            node.body = Nodes.StatListNode(
                node.pos,
                stats=[
                    Nodes.TryFinallyStatNode(
                        node.pos, body=node.body, finally_clause=Nodes.StatListNode(node.pos, stats=[del_target])
                    )
                ],
            )
        self.visitchildren(node)
...
class _AssignmentExpressionChecker(TreeVisitor):
    ...
    def visit_AssignmentExpressionNode(self, node):
        ...
        if node.target_name in self.current_target_names:
            error(
                node.pos, "assignment expression cannot rebind comprehension iteration variable '%s'" % node.target_name
            )
        elif node.target_name in self.all_target_names:
            error(
                node.pos, "comprehension inner loop cannot rebind assignment expression target '%s'" % node.target_name
            )
...
def flatten_parallel_assignments(input, output):
    ...
    rhs = input[-1]
    if not (rhs.is_sequence_constructor or isinstance(rhs, ExprNodes.UnicodeNode)) or not sum(
        [lhs.is_sequence_constructor for lhs in input[:-1]]
    ):
        output.append(input)
        return
...
class PxdPostParse(CythonTransform, SkipDeclarations):
    ...
    def visit_FuncDefNode(self, node):
        ...
        if (
            isinstance(node, Nodes.DefNode)
            and self.scope_type == "cclass"
...
class InterpretCompilerDirectives(CythonTransform):
    ...
    def check_directive_scope(self, pos, directive, scope):
        ...
        if legal_scopes and scope not in legal_scopes:
            self.context.nonfatal_error(
                PostParseError(pos, "The %s compiler directive " "is not allowed in %s scope" % (directive, scope))
            )
            return False
        else:
    ...
    def _check_valid_cython_module(self, pos, module_name):
        ...
        error(
            pos,
            "'%s' is not a valid cython.* module%s%s"
            % (
                module_name,
                ". " if extra else "",
                extra,
    ...
    def _create_cimport_from_import(self, node_pos, module_name, level, imported_names):
        ...
        if module_name:
            ...
            return Nodes.FromCImportStatNode(
                node_pos, module_name=module_name, relative_level=level, imported_names=imported_names
            )
        else:
    ...
    def try_to_parse_directive(self, optname, args, kwds, pos):
        ...
        if len(args) == 1 and isinstance(args[0], ExprNodes.NoneNode):
            ...
            if (
                kwds is not None
                or len(args) != 1
                or not isinstance(args[0], (ExprNodes.StringNode, ExprNodes.UnicodeNode))
            ):
    ...
    def visit_with_directives(self, node, directives, contents_directives):
        ...
        if contents_directives is not None and new_contents_directives != new_directives:
            ...
            node.body = Nodes.StatListNode(
                node.body.pos,
                stats=[Nodes.CompilerDirectivesNode(node.body.pos, directives=new_contents_directives, body=node.body)],
            )
                ...
                for directive in new_directives:
                    if self.check_directive_scope(node.pos, directive[0], scope_name):
                        ...
                        if name in ("nogil", "with_gil"):
                            if value is None:
                                ...
                                if kwds or len(args) != 1 or not isinstance(args[0], ExprNodes.BoolNode):
                                    raise PostParseError(
                                        dec.pos, "The %s directive takes one compile-time boolean argument" % name
                                    )
                                value = args[0].value
                        ...
                        if current_opt_dict.get(name, missing) != value:
                            ...
                            warning(
                                dec.pos,
                                "Directive does not change previous value (%s%s)"
                    ...
                    if name in ("nogil", "gil"):
                        ...
                        if isinstance(node.manager, ExprNodes.SimpleCallNode) and len(node.manager.args) > 0:
                            if len(node.manager.args) == 1:
                                ...
                                self.context.nonfatal_error(
                                    PostParseError(
                                        node.pos, "Compiler directive %s accepts one positional argument." % name
                                    )
                                )
    ...
    def visit_WithStatNode(self, node):
        ...
        node.enter_call = ExprNodes.SimpleCallNode(
            ...
            function=ExprNodes.AttributeNode(
                pos,
                obj=ExprNodes.CloneNode(manager),
        ...
        if target is not None:
            body = Nodes.StatListNode(
                pos, stats=[Nodes.WithTargetAssignmentStatNode(pos, lhs=target, with_node=node), body]
            )
    ...
    def _handle_ExprNode(self, node, do_visit_children):
        if node.generator_arg_tag is not None and self.gen_node is not None and self.gen_node == node.generator_arg_tag:
            ...
            if not def_node.local_scope.lookup_here(name):
                ...
                name_decl.type = type
                new_arg = Nodes.CArgDeclNode(
                    pos=pos, declarator=name_decl, base_type=None, default=None, annotation=None
                )
...
class DecoratorTransform(ScopeTrackingTransform, SkipDeclarations):
    ...
    def visit_CFuncDefNode(self, node):
        ...
        if not node.decorators:
            ...
            if not (
                len(node.decorators) == 1
                and node.decorators[0].decorator.is_name
                and node.decorators[0].decorator.name == "staticmethod"
            ):
            ...
            if decorator.is_name:
                ...
                if handler_name:
                    if decorator.obj.name != node.name:
                        ...
                        error(
                            decorator_node.pos,
                            "Mismatching property names, expected '%s', got '%s'" % (decorator.obj.name, node.name),
                        )
    ...
    def chain_decorators(node, decorators, name):
        ...
        for decorator in decorators[::-1]:
            decorator_result = ExprNodes.SimpleCallNode(
                decorator.pos, function=decorator.decorator, args=[decorator_result]
            )
...
class CnameDirectivesTransform(CythonTransform, SkipDeclarations):
    ...
    def handle_function(self, node):
        ...
        for i, decorator in enumerate(node.decorators):
            ...
            if (
                isinstance(decorator, ExprNodes.CallNode)
                and decorator.function.is_name
                and decorator.function.name == "cname"
            ):
...
if VALUE is not None:
    ...
    def _inject_pickle_methods(self, node):
        ...
        non_py = [
            ...
            and (not e.type.can_coerce_to_pyobject(env) or not e.type.can_coerce_from_pyobject(env))
        ]
        ...
        if cinit or non_py or (structs and not auto_pickle_forced):
            if cinit:
                ...
                msg = "%s cannot be converted to a Python object for pickling" % ",".join(
                    "self.%s" % e.name for e in non_py
                )
            else:
                ...
                msg = (
                    "Pickling of struct members such as %s must be explicitly requested "
            ...
            unpickle_func = TreeFragment(
                ...
                % {
                    ...
                    "assignments": "; ".join(
                        "__pyx_result.%s = __pyx_state[%s]" % (v, ix) for ix, v in enumerate(all_members_names)
                    ),
            ...
            pickle_func = TreeFragment(
                ...
                % {
                    ...
                    "members": ", ".join("self.%s" % v for v in all_members_names)
                    + ("," if len(all_members_names) == 1 else ""),
                    # Even better, we could check PyType_IS_GC.
                    "any_notnone_members": " or ".join(
                        ["self.%s is not None" % e.name for e in all_members if e.type.is_pyobject] or ["False"]
                    ),
                },
                level="c_class",
                pipeline=[NormalizeTree(None)],
    ...
    def visit_DefNode(self, node):
        ...
        env = self.current_env()
        if (
            not isinstance(node, Nodes.DefNode)
            or node.fused_py_func
            or node.is_generator_body
            or not node.needs_assignment_synthesis(env)
        ):
            return node
    ...
    def _create_assignment(self, def_node, rhs, env):
        ...
        assmt = Nodes.SingleAssignmentNode(
            def_node.pos, lhs=ExprNodes.NameNode(def_node.pos, name=def_node.name), rhs=rhs
        )
    ...
    def visit_CStructOrUnionDefNode(self, node):
        ...
        self_value = ExprNodes.AttributeNode(
            pos=node.pos, obj=ExprNodes.NameNode(pos=node.pos, name="self"), attribute=EncodedString("value")
        )
    ...
    def create_Property(self, entry):
        ...
        property = template.substitute(
            {
                "ATTR": ExprNodes.AttributeNode(
                    pos=entry.pos, obj=ExprNodes.NameNode(pos=entry.pos, name="self"), attribute=entry.name
                ),
            },
            pos=entry.pos,
        ).stats[0]
...
class MarkClosureVisitor(CythonTransform):
    ...
    def visit_FuncDefNode(self, node):
        ...
        gbody = Nodes.GeneratorBodyDefNode(
            pos=node.pos, name=node.name, body=node.body, is_async_gen_body=node.is_async_def and collector.has_yield
        )
    ...
    def visit_cython_attribute(self, node):
        ...
        if attribute:
            if attribute == "__version__":
                ...
                node = ExprNodes.NameNode(
                    node.pos,
                    name=EncodedString(attribute),
    ...
    def _inject_locals(self, node, func_name):
        ...
        if func_name in ("locals", "vars"):
            if func_name == "locals" and len(node.args) > 0:
                error(
                    self.pos, "Builtin 'locals()' called with wrong number of args, expected 0, got %d" % len(node.args)
                )
                return node
            elif func_name == "vars":
                if len(node.args) > 1:
                    error(
                        self.pos,
                        "Builtin 'vars()' called with wrong number of args, expected 0-1, got %d" % len(node.args),
                    )
                if len(node.args) > 0:
            ...
            if len(node.args) > 1:
                error(
                    self.pos, "Builtin 'dir()' called with wrong number of args, expected 0-1, got %d" % len(node.args)
                )
            if len(node.args) > 0:
...
class DebugTransform(CythonTransform):
    ...
    def visit_ModuleNode(self, node):
        ...
        for k, v in node.scope.entries.items():
            if (
                v.qualified_name not in self.visited
                and not v.name.startswith("__pyx_")
                and not v.type.is_cfunction
                and not v.type.is_extension_type
            ):
                entries[k] = v
