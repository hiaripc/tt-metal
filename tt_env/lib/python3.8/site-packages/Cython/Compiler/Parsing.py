...
comparison_ops = cython.declare(frozenset, frozenset(("<", ">", "==", ">=", "<=", "<>", "!=", "in", "is", "not")))
    ...
    if keyword_args:
        ...
        for item in keyword_args:
            if isinstance(item, tuple):
                ...
                if dict_items:
                    kwargs.append(
                        ExprNodes.DictNode(dict_items[0].pos, key_value_pairs=dict_items, reject_duplicates=True)
                    )
                    dict_items = []
                kwargs.append(item)
    ...
    if value is None:
        ...
        if value.real:
            ...
            node = ExprNodes.binop_node(
                pos,
                "+",
                ExprNodes.FloatNode(pos, value=repr(value.real), constant_result=value.real),
                node,
...
def p_f_string_expr(s, unicode_value, pos, starting_index, is_raw):
    ...
    if terminal_char != "}":
        error(
            ...
            "missing '}' in format string expression" + (", found '%s'" % terminal_char if terminal_char else ""),
        )
    ...
    nodes.append(
        ExprNodes.FormattedValueNode(pos, value=expr, conversion_char=conversion_char, format_spec=format_spec)
    )
...
def p_comp_if(s, body):
    ...
    return Nodes.IfStatNode(
        pos, if_clauses=[Nodes.IfClauseNode(pos, condition=test, body=p_comp_iter(s, body))], else_clause=None
    )
...
expr_terminators = cython.declare(frozenset, frozenset((")", "]", "}", ":", "=", "NEWLINE")))
...
def p_from_import_statement(s, first_statement=0):
    ...
    if dotted_name == "__future__":
        ...
        return Nodes.FromCImportStatNode(
            pos, module_name=dotted_name, relative_level=level, imported_names=imported_names
        )
    else:
...
calling_convention_words = cython.declare(frozenset, frozenset(("__stdcall", "__cdecl", "__fastcall")))
    ...
    if is_const or is_volatile:
        ...
        if isinstance(base_type, Nodes.MemoryViewSliceTypeNode):
            ...
            base_type.base_type_node = Nodes.CConstOrVolatileTypeNode(
                pos, base_type=base_type.base_type_node, is_const=is_const, is_volatile=is_volatile
            )
            return base_type
...
def p_buffer_or_template(s, base_type_node, templates):
    ...
    keyword_dict = ExprNodes.DictNode(
        pos, key_value_pairs=[ExprNodes.DictItemNode(pos=key.pos, key=key, value=value) for key, value in keyword_args]
    )
    result = Nodes.TemplatedTypeNode(
        pos, positional_args=positional_args, keyword_args=keyword_dict, base_type_node=base_type_node
    )
    return result
...
basic_c_type_names = cython.declare(frozenset, frozenset(("void", "char", "int", "float", "double", "bint")))
...
sign_and_longness_words = cython.declare(frozenset, frozenset(("short", "long", "signed", "unsigned")))

base_type_start_words = cython.declare(
    frozenset, basic_c_type_names | sign_and_longness_words | frozenset(special_basic_c_types)
)

struct_enum_union = cython.declare(frozenset, frozenset(("struct", "union", "enum", "packed")))
...
def p_c_declarator(
    s, ctx=Ctx(), empty=0, is_type=0, cmethod_flag=0, assignable=0, nonempty=0, calling_convention_allowed=0
):
...
def p_c_simple_declarator(s, ctx, empty, is_type, cmethod_flag, assignable, nonempty):
    ...
    if s.sy in ("*", "**"):
        ...
        base = p_c_declarator(
            s, ctx, empty=empty, is_type=is_type, cmethod_flag=cmethod_flag, assignable=assignable, nonempty=nonempty
        )
        if is_const:
...
def p_c_arg_list(s, ctx=Ctx(), in_pyfunc=0, cmethod_flag=0, nonempty_declarators=0, kw_only=0, annotated=1):
    ...
    while s.sy not in c_arg_list_terminators:
        args.append(
            p_c_arg_decl(
                s, ctx, in_pyfunc, is_self_arg, nonempty=nonempty_declarators, kw_only=kw_only, annotated=annotated
            )
        )
        if s.sy != ",":
            break
...
def p_cdef_extern_block(s, pos, ctx):
    ...
    return Nodes.CDefExternNode(
        pos, include_file=include_file, verbatim_include=verbatim_include, body=body, namespace=ctx.namespace
    )
...
def p_c_enum_definition(s, pos, ctx):
    ...
    if scoped and s.sy == "(":
        ...
        underlying_type = Nodes.CSimpleBaseTypeNode(
            pos, name="int", module_path=[], is_basic_c_type=True, signed=1, complex=0, longness=0
        )
...
def _reject_cdef_modifier_in_py(s, name):
    ...
    if s.sy == "IDENT" and name in _CDEF_MODIFIERS:
        ...
        s.error(
            "Cannot use cdef modifier '%s' in Python function signature. Use a decorator instead." % name, fatal=False
        )
...
def p_def_statement(s, decorators=None, is_async_def=False):
    ...
    s.expect(
        ...
        "Use decorators and Python type annotations instead." % (s.systring if s.sy == "IDENT" else s.sy),
    )
...
_match_compiler_directive_comment = cython.declare(object, re.compile(r"^#\s*cython\s*:\s*((\w|[.])+\s*=.*)$").match)
