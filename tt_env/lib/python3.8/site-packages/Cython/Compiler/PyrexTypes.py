        ...
        for attr in subtypes:
            ...
            if list_or_subtype:
                if isinstance(list_or_subtype, BaseType):
                    list_or_subtype.get_fused_types(
                        result, seen, include_function_return_type=include_function_return_type
                    )
                else:
...
class PyrexType(BaseType):
    ...
    ) = generate_gotref = generate_xgotref = generate_giveref = generate_xgiveref = _generate_dummy_refcounting
...
class CTypedefType(BaseType):
                ...
                if type(base_type) is CIntType:
                    ...
                    env.use_utility_code(
                        TempitaUtilityCode.load_cached(
                            "CIntToPy",
                            "TypeConversion.c",
                            context={"TYPE": self.empty_declaration_code(), "TO_PY_FUNCTION": self.to_py_function},
                        )
                    )
                    return True
    ...
    def overflow_check_binop(self, binop, env, const_rhs=False):
        ...
        if binop == "lshift":
            env.use_utility_code(
                TempitaUtilityCode.load_cached(
                    "LeftShift", "Overflow.c", context={"TYPE": type, "NAME": name, "SIGNED": self.signed}
                )
            )
        else:
            if const_rhs:
            ...
            env.use_utility_code(
                TempitaUtilityCode.load_cached("SizeCheck", "Overflow.c", context={"TYPE": type, "NAME": name})
            )
            env.use_utility_code(
                TempitaUtilityCode.load_cached(
                    "Binop", "Overflow.c", context={"TYPE": type, "NAME": name, "BINOP": binop}
                )
            )
...
class MemoryViewSliceType(PyrexType):
    ...
    def same_as_resolved_type(self, other_type):
        return (
            other_type.is_memoryviewslice
            and
            # self.writable_needed == other_type.writable_needed and  # FIXME: should be only uni-directional
            self.dtype.same_as(other_type.dtype)
            and self.axes == other_type.axes
    ...
    def attributes_known(self):
        if self.scope is None:
            ...
            self.scope = scope = Symtab.CClassScope(
                "mvs_class_" + self.specialization_suffix(), None, visibility="extern", parent_type=self
            )
    ...
    def declare_attribute(self, attribute, env, pos):
        ...
        if attribute == "shape":
            scope.declare_var(
                "shape", c_array_type(c_py_ssize_t_type, Options.buffer_max_dims), pos, cname="shape", is_cdef=1
            )

        elif attribute == "strides":
            scope.declare_var(
                "strides", c_array_type(c_py_ssize_t_type, Options.buffer_max_dims), pos, cname="strides", is_cdef=1
            )
    ...
    def valid_dtype(self, dtype, i=0):
        ...
        return (
            ...
            (dtype.is_array and i < 8 and self.valid_dtype(dtype.base_type, i + 1))
            or dtype.is_numeric
...
class BufferType(BaseType):
    ...
    def same_as(self, other_type, compare_base=True):
        ...
        return (
            self.dtype.same_as(other_type.dtype)
            and self.ndim == other_type.ndim
            and self.mode == other_type.mode
            and self.cast == other_type.cast
            and (not compare_base or self.base.same_as(other_type.base))
        )
    ...
    def attributes_known(self):
        if self.scope is None:
            ...
            scope.declare_cproperty(
                "shape",
                c_ptr_type(c_long_type),
                "__Pyx_PythranShapeAccessor",
            ...
            if t.is_fused:
                ...
                if isinstance(t, FusedType):
                    ...
                    for substitution in product(*[fused_type.types for fused_type in t_fused_types]):
                        t_types.append(
                            t.specialize({fused_type: sub for fused_type, sub in zip(t_fused_types, substitution)})
                        )
    ...
    def convert_to_pystring(self, cvalue, code, format_spec=None):
        if self.to_pyunicode_utility is None:
            ...
            to_pyunicode_utility = TempitaUtilityCode.load_cached(
                "CIntToPyUnicode",
                "TypeConversion.c",
                context={"TYPE": self.empty_declaration_code(), "TO_PY_FUNCTION": utility_code_name},
            )
    ...
    def overflow_check_binop(self, binop, env, const_rhs=False):
        ...
        if binop == "lshift":
            ...
            if type in ("int", "long", "long long"):
                env.use_utility_code(
                    TempitaUtilityCode.load_cached("BaseCaseSigned", "Overflow.c", context={"INT": type, "NAME": name})
                )
            elif type in ("unsigned int", "unsigned long", "unsigned long long"):
                env.use_utility_code(
                    TempitaUtilityCode.load_cached(
                        "BaseCaseUnsigned", "Overflow.c", context={"UINT": type, "NAME": name}
                    )
                )
            elif self.rank <= 1:
...
def _load_overflow_base(env):
    ...
    for type in ("int", "long", "long long"):
        env.use_utility_code(
            TempitaUtilityCode.load_cached(
                "BaseCaseSigned", "Overflow.c", context={"INT": type, "NAME": type.replace(" ", "_")}
            )
        )
    for type in ("unsigned int", "unsigned long", "unsigned long long"):
        env.use_utility_code(
            TempitaUtilityCode.load_cached(
                "BaseCaseUnsigned", "Overflow.c", context={"UINT": type, "NAME": type.replace(" ", "_")}
            )
        )
...
class CComplexType(CNumericType):
    ...
    def assignable_from_resolved_type(self, src_type):
        return (
            src_type.is_complex
            and self.real_type.assignable_from_resolved_type(src_type.real_type)
            or src_type.is_numeric
            and self.real_type.assignable_from_resolved_type(src_type)
...
class CArrayType(CPointerBaseType):
    ...
    def declaration_code(self, entity_code, for_display=0, dll_linkage=None, pyrex=0):
        ...
        return self.base_type.declaration_code(
            "%s[%s]" % (entity_code, dimension_code), for_display, dll_linkage, pyrex
        )
    ...
    def assignable_from_resolved_type(self, other_type):
        ...
        if self.base_type.is_cfunction:
            ...
            if other_type.is_cfunction:
                ...
                return 0
        if (
            self.base_type.is_cpp_class
            and other_type.is_ptr
            and other_type.base_type.is_cpp_class
            and other_type.base_type.is_subclass(self.base_type)
        ):
            return 1
...
class CFuncType(CType):
    ...
    def with_with_gil(self, with_gil):
        if with_gil == self.with_gil:
            ...
            return CFuncType(
                ...
                with_gil,
                self.is_overridable,
                self.optional_arg_count,
                self.is_const_method,
                self.is_static_method,
                self.templates,
                self.is_strict_signature,
            )
    ...
    def same_c_signature_as_resolved_type(
        self, other_type, as_cmethod=False, as_pxd_definition=False, exact_semantics=True
    ):
    ...
    def narrower_c_signature_than_resolved_type(self, other_type, as_cmethod):
        ...
        for i in range(as_cmethod, nargs):
            if not self.args[i].type.subtype_of_resolved_type(other_type.args[i].type):
                ...
                self.args[i].needs_type_test = other_type.args[i].needs_type_test or not self.args[i].type.same_as(
    ...
    def same_as_resolved_type(self, other_type, as_cmethod=False):
        return (
            self.same_c_signature_as_resolved_type(other_type, as_cmethod=as_cmethod) and self.nogil == other_type.nogil
        )
    ...
    def pointer_assignable_from_resolved_type(self, rhs_type):
        ...
        if not rhs_type.is_cfunction:
            return 0
        return rhs_type.same_c_signature_as_resolved_type(self, exact_semantics=False) and not (
            self.nogil and not rhs_type.nogil
        )
    ...
    def declaration_code(self, entity_code, for_display=0, dll_linkage=None, pyrex=0, with_calling_convention=1):
        ...
        return self.return_type.declaration_code(
            "%s%s(%s)%s" % (cc, entity_code, arg_decl_code, trailer), for_display, dll_linkage, pyrex
        )
    ...
    def specialize(self, values):
        result = CFuncType(
            self.return_type.specialize(values),
...
class ToPyStructUtilityCode(object):
    ...
    def put_code(self, output):
        ...
        code.putln(
            "res = __Pyx_PyDict_NewPresized(%d); if (unlikely(!res)) return NULL;" % len(self.type.scope.var_entries)
        )
...
class CStructOrUnionType(CType):
    ...
    def can_be_complex(self):
        ...
        a, b = fields
        return (
            a.type.is_float and b.type.is_float and a.type.empty_declaration_code() == b.type.empty_declaration_code()
        )
...
class CppClassType(CType):
    ...
    def specialize_here(self, pos, env, template_values=None):
        ...
        if len(self.templates) - self.num_optional_templates <= len(template_values) < len(self.templates):
            ...
            template_values = template_values + [
                TemplatePlaceholderType("%s::%s" % (partial_specialization, param.name), True)
    ...
    def specialize(self, values):
        ...
        specialized = self.specializations[key] = CppClassType(
            self.name, None, self.cname, [], template_values, template_type=self
        )
    ...
    def declaration_code(self, entity_code, for_display=0, dll_linkage=None, pyrex=0, template_params=None):
        ...
        if self.templates:
            template_strings = [
                param.declaration_code("", for_display, None, pyrex)
    ...
    def cpp_optional_declaration_code(self, entity_code, dll_linkage=None, template_params=None):
        return "__Pyx_Optional_Type<%s> %s" % (
            self.declaration_code("", False, dll_linkage, False, template_params),
            entity_code,
        )
    ...
    def same_as_resolved_type(self, other_type):
        if other_type.is_cpp_class:
            ...
            # This messy logic is needed due to GH Issue #1852.
                ...
                self.template_type and other_type.template_type or self.templates or other_type.templates
            ):
...
cy_integral_type = FusedType([c_short_type, c_int_type, c_long_type], name="integral")
# Omitting long double as it might be slow
cy_floating_type = FusedType([c_float_type, c_double_type], name="floating")
cy_numeric_type = FusedType(
    [c_short_type, c_int_type, c_long_type, c_float_type, c_double_type, c_float_complex_type, c_double_complex_type],
    name="numeric",
)

# buffer-related structs
c_buf_diminfo_type = CStructOrUnionType("__Pyx_Buf_DimInfo", "struct", None, 1, "__Pyx_Buf_DimInfo")
...
c_pyx_buffer_nd_type = CStructOrUnionType("__Pyx_LocalBuf_ND", "struct", None, 1, "__Pyx_LocalBuf_ND")

cython_memoryview_type = CStructOrUnionType("__pyx_memoryview_obj", "struct", None, 0, "__pyx_memoryview_obj")

memoryviewslice_type = CStructOrUnionType("memoryviewslice", "struct", None, 1, "__Pyx_memviewslice")
    ...
    for func in functions:
        ...
        if func_type.templates:
            ...
            deductions = reduce(
                merge_template_deductions,
                [
                    pattern.type.deduce_template_params(actual)
                    for (pattern, actual) in zip(func_type.args, arg_types_for_deduction)
                ],
                {},
            )
            if deductions is None:
                errors.append(
                    (
                        func,
                        "Unable to deduce type parameters for %s given (%s)"
...
def widest_numeric_type(type1, type2):
    ...
    if type1 == type2:
        ...
        if type1 is soft_complex_type or type2 is soft_complex_type:
            ...
            if (
                not type1_is_other_complex
                and not type2_is_other_complex
                and widest_type.real_type == soft_complex_type.real_type
            ):
...
_escape_special_type_characters = partial(
    re.compile(
        # join substrings in reverse order to put longer matches first, e.g. "::" before ":"
        " ?(%s) ?"
        % "|".join(re.escape(s) for s in sorted(_special_type_characters, reverse=True))
    ).sub,
    lambda match: _special_type_characters[match.group(1)],
)
...
def write_noexcept_performance_hint(pos, env, function_name=None, void_return=False, is_call=False, is_from_pxd=False):
    ...
    solutions = [
        "Declare %s as 'noexcept' if you control the definition and "
        "you're sure you don't want the function to raise exceptions." % the_function
    ]
    if void_return:
