...
def c_safe_identifier(cname):
    ...
    if (
        cname[:2] == "__" and not (cname.startswith(Naming.pyrex_prefix) or cname in ("__weakref__", "__dict__"))
...
class Scope(object):
    ...
    def declare_struct_or_union(
        self, name, kind, scope, typedef_flag, pos, cname=None, visibility="private", api=0, packed=False
    ):
        ...
        if not entry:
            ...
            entry = self.declare_type(
                name, type, pos, cname, visibility=visibility, api=api, defining=scope is not None
            )
            self.sue_entries.append(entry)
            type.entry = entry
        else:
            if not (entry.is_type and entry.type.is_struct_or_union and entry.type.kind == kind):
    ...
    def check_previous_typedef_flag(self, entry, typedef_flag, pos):
        if typedef_flag != entry.type.typedef_flag:
            error(
                pos, "'%s' previously declared using '%s'" % (entry.name, ("cdef", "ctypedef")[entry.type.typedef_flag])
            )
    ...
    def declare_enum(
        self, name, pos, cname, scoped, typedef_flag, visibility="private", api=0, create_wrapper=0, doc=None
    ):
        if name:
            if not cname:
    ...
    def declare_cfunction(
        ...
        if entry:
            ...
            if visibility != "private" and visibility != entry.visibility:
                warning(
                    pos,
                    "Function '%s' previously declared as '%s', now as '%s'" % (name, entry.visibility, visibility),
                    1,
                )
            if overridable != entry.is_overridable:
                warning(
                    pos, "Function '%s' previously declared as '%s'" % (name, "cpdef" if overridable else "cdef"), 1
                )
        ...
        if (
            ...
            and type.exception_value is None
            and type.nogil
            and not pos[0].in_utility_code
            and
            # don't warn about external functions here - the user likely can't do anything
            defining
            and not in_pxd
            and not inline_in_pxd
        ):
    ...
    def declare_cgetter(
        ...
        **cfunc_type_config,
        ...
        cfunc_type = PyrexTypes.CFuncType(
            return_type, [PyrexTypes.CFuncTypeArg("self", self.parent_type, None)], **cfunc_type_config
        )
    ...
    def _emit_class_private_warning(self, pos, name):
        warning(
            pos,
            "Global name %s matched from within class scope "
            "in contradiction to to Python 'class private name' rules. "
            "This may change in a future release." % name,
            1,
        )
...
class BuiltinScope(Scope):
    ...
    def declare_builtin_type(
        self, name, cname, utility_code=None, objstruct_cname=None, type_class=PyrexTypes.BuiltinObjectType
    ):
        name = EncodedString(name)
...
class ModuleScope(Scope):
    ...
    def find_submodule(self, name, as_package=False):
        ...
        if not scope:
            scope = ModuleScope(
                name, parent_module=self, context=self.context, is_package=True if submodule else as_package
            )
    ...
    def declare_var(
        ...
        entry = Scope.declare_var(
            ...
            in_pxd=in_pxd,
            is_cdef=is_cdef,
            pytyping_modifiers=pytyping_modifiers,
        )
        if is_cdef:
            entry.is_cglobal = 1
...
class StructOrUnionScope(Scope):
    ...
    def declare_var(
        ...
        allow_memoryview=False,
        allow_refcounted=False,
    ):
    ...
    def lookup(self, name):
        ...
        if name == "classmethod":
            ...
            entry = Entry(
                ...
                PyrexTypes.CFuncType(py_object_type, [PyrexTypes.CFuncTypeArg("", py_object_type, None)], 0, 0),
            )
    ...
    def may_have_finalize(self):
        ...
        while current_type_scope:
            ...
            if del_entry and del_entry.is_special:
                return True
            if (
                current_type_scope.parent_type.is_external
                or not current_type_scope.implemented
                or current_type_scope.parent_type.multiple_bases
            ):
            ...
            if "typing.ClassVar" in pytyping_modifiers:
                ...
                if not type.is_pyobject:
                    if not type.equivalent_type:
                        warning(
                            pos,
                            "ClassVar[] requires the type to be a Python object type. Found '%s', using object instead."
                            % type,
                        )
                        type = py_object_type
                    else:
                ...
                if entry.is_final_cmethod and entry.is_inherited:
                    ...
                    if (
                        ...
                        warning(
                            ...
                            "This may cause incorrect vtables to be generated." % (name, self.class_name),
                            2,
                        )
            ...
            if entry.is_type:
                ...
                for e in entry.all_alternatives():
                    scope.declare_cfunction(
                        e.name, e.type.specialize(values), e.pos, e.cname, utility_code=e.utility_code
                    )
            else:
