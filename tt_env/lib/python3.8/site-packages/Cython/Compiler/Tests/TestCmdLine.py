...
class CmdLineParserTest(TestCase):
    ...
    def check_default_options(self, options, white_list=[]):
        ...
        for name in default_options.__dict__.keys():
            if name not in white_list:
                self.assertEqual(
                    getattr(options, name, no_value), getattr(default_options, name), msg="error in option " + name
                )
    ...
    def test_warning_extra_dont_overwrite(self):
        options, sources = parse_command_line(
            ["-X", "cdivision=True", "--warning-extra", "-X", "c_string_type=bytes", "source.pyx"]
        )
    ...
    def test_errors(self):
        ...
        error(
            ["--module-name", "foo.bar", "foo.pyx", "bar.pyx"], "Only one source file allowed when using --module-name"
        )
