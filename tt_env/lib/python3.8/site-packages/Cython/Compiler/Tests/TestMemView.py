...
class TestMemviewParsing(CythonTest):
    ...
    def test_non_slice_memview(self):
        self.not_parseable(
            "An axis specification in memoryview declaration does not have a ':'.", "cdef double[:foo, bar] x"
        )
        self.not_parseable(
            "An axis specification in memoryview declaration does not have a ':'.", "cdef double[0:foo, bar] x"
        )
