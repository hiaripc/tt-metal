    ...
    def _raise_compiler_error(self, child, e):
        ...
        for parent, attribute, index in self.access_path:
            ...
            if index is None:
                ...
                node = node[index]
                index = "[%d]" % index
            trace.append("%s.%s%s = %s" % (parent.__class__.__name__, attribute, index, self.dump_node(node)))
        stacktrace, called_nodes = self._find_node_path(sys.exc_info()[2])
        last_node = child
        for node, method_name, pos in called_nodes:
            last_node = node
            trace.append("File '%s', line %d, in %s: %s" % (pos[0], pos[1], method_name, self.dump_node(node)))
    ...
    def _visit_binop_node(self, node):
        ...
        if special_method_name:
            ...
            node = self._dispatch_to_method_handler(
                special_method_name, None, False, type_name, node, None, [operand1, operand2], None
            )
        return node
    ...
    def visit_UnopNode(self, node):
        ...
        if special_method_name:
            ...
            if obj_type.is_builtin_type:
                ...
                type_name = "object"  # safety measure
            node = self._dispatch_to_method_handler(
                special_method_name, None, False, type_name, node, None, [operand], None
            )
        return node
    ...
    def _dispatch_to_handler(self, node, function, arg_list, kwargs):
        if function.is_name:
            ...
            if obj_type.is_builtin_type:
                ...
                type_name = "object"  # safety measure
            return self._dispatch_to_method_handler(
                attr_name, self_arg, is_unbound_method, type_name, node, function, arg_list, kwargs
            )
        else:
            return node

    def _dispatch_to_method_handler(
        self, attr_name, self_arg, is_unbound_method, type_name, node, function, arg_list, kwargs
    ):
    ...
    def repr_of(self, node):
        if node is None:
            ...
            if isinstance(node, ExprNodes.NameNode):
                ...
                result += '(name="%s", type="%s")' % (node.declared_name(), getattr(node, "type", None))
