...
class Plugin(CoveragePlugin):
    ...
    def file_reporter(self, filename):
        ...
        return CythonModuleReporter(
            c_file, filename, rel_file_path, code, self._excluded_lines_map.get(rel_file_path, frozenset())
        )
    ...
    def _parse_cfile_lines(self, c_file):
        ...
        not_executable = re.compile(
            r"\s*c(?:type)?def\s+" r"(?:(?:public|external)\s+)?" r"(?:struct|union|enum|class)" r"(\s+[^:]+|)\s*:"
        ).match
