...
class CythonFunction(CythonVariable):
    def __init__(
        self, module, name, cname, pf_cname, qualified_name, lineno, type=CObject, is_initmodule_function="False"
    ):
...
class CythonParameters(object):
    ...
    def __init__(self):
        self.complete_unqualified = CompleteUnqualifiedFunctionNames(
            "cy_complete_unqualified", gdb.COMMAND_BREAKPOINTS, gdb.PARAM_BOOLEAN, True
        )
        ...
        self.terminal_background = TerminalBackground(
            "cy_terminal_background_color", gdb.COMMAND_FILES, gdb.PARAM_STRING, "dark"
        )
        ...
        if not func:
            ...
            if len(funcs) > 1:
                ...
                while True:
                    try:
                        result = input(
                            "Select a function, press 'a' for all " "functions or press 'q' or '^D' to quit: "
                        )
                    except EOFError:
                        return
                    else:
