...
def _PyObject_VAR_SIZE(typeobj, nitems):
    ...
    return (
        (typeobj.field("tp_basicsize") + nitems * typeobj.field("tp_itemsize") + (_sizeof_void_p() - 1))
        & ~(_sizeof_void_p() - 1)
...
class PyFrameObjectPtr(PyObjectPtr):
    ...
    def write_repr(self, out, visited):
        ...
        out.write(
            ...
            % (self.as_address(), self.co_filename.proxyval(visited), lineno, self.co_name.proxyval(visited))
        )
        first = True
        for pyop_name, pyop_value in self.iter_locals():
            if not first:
...
class PyUnicodeObjectPtr(PyObjectPtr):
    ...
    def proxyval(self, visited):
        ...
        result = "".join([(_unichr(ucs) if ucs <= 0x10FFFF else "\ufffd") for ucs in Py_UNICODEs])
        return result
        ...
        while i < len(proxy):
            ...
            # Non-ASCII characters
                ...
                if sys.maxunicode < 0x10000:
                    ...
                    if i < len(proxy) and 0xD800 <= ord(ch) < 0xDC00 and 0xDC00 <= ord(proxy[i]) <= 0xDFFF:
                        ch2 = proxy[i]
                        ucs = ch + ch2
                        i += 1
...
class ExecutionControlCommandBase(gdb.Command):
    ...
    def install_breakpoints(self):
        all_locations = itertools.chain(
            self.lang_info.static_break_functions(), self.lang_info.runtime_break_functions()
        )
    ...
    def filter_output(self, result):
        ...
        match_finish = re.search(r"^Value returned is \$\d+ = (.*)", result, re.MULTILINE)
        if match_finish:
    ...
    def alloc_pystring(self, string):
        ...
        try:
            result = gdb.parse_and_eval(
                "(PyObject *) %s((char *) %d, (size_t) %d)" % (PyString_FromStringAndSize, stringp, len(string))
            )
        finally:
            self.free(stringp)
