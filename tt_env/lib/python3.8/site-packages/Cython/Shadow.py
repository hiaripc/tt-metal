...
) = cpp_locals = py2_import = iterable_coroutine = remove_unreachable = lambda _: _EmptyDecoratorAndManager()
...
embedsignature.format = (
    overflowcheck.fold
) = optimize.use_switch = optimize.unpack_method_calls = lambda arg: _EmptyDecoratorAndManager()

final = internal = type_version_tag = no_gc_clear = no_gc = total_ordering = ufunc = _empty_decorator
...
class warn:
    undeclared = (
        unreachable
    ) = maybe_uninitialized = unused = unused_arg = unused_result = lambda _: _EmptyDecoratorAndManager()
