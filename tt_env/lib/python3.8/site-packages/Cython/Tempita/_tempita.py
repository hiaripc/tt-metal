...
class Template(object):
    ...
    def from_filename(
        cls, filename, namespace=None, encoding=None, default_inherit=None, get_template=get_file_template
    ):
        ...
        if encoding:
            c = c.decode(encoding)
        return cls(
            content=c, name=filename, namespace=namespace, default_inherit=default_inherit, get_template=get_template
        )
    ...
    def _interpret_inherit(self, body, defs, inherit_template, ns):
        ...
        if not self.get_template:
            raise TemplateError(
                "You cannot use inheritance without passing in get_template", position=None, name=self.name
            )
            ...
            if self._unicode and isinstance(value, bytes):
                if not self.default_encoding:
                    raise UnicodeDecodeError(
                        "Cannot decode bytes value %r into unicode " "(no default_encoding provided)" % value
                    )
                try:
                ...
                if not self.default_encoding:
                    raise UnicodeEncodeError(
                        "Cannot encode unicode value %r into bytes " "(no default_encoding provided)" % value
                    )
                value = value.encode(self.default_encoding)
            return value
...
class TemplateDef(object):
    ...
    def __get__(self, obj, type=None):
        ...
        return self.__class__(
            self._template, self._func_name, self._func_signature, self._body, self._ns, self._pos, bound_self=obj
        )
...
def trim_lex(tokens):
    ...
    for i, current in enumerate(tokens):
        ...
        if last_trim is not None and last_trim + 2 == i and not prev.strip():
            prev_ok = "last"
        if prev_ok and (
            not next_chunk or lead_whitespace_re.search(next_chunk) or (i == len(tokens) - 2 and not next_chunk.strip())
        ):
            if prev:
...
def parse_one_cond(tokens, name, context):
    ...
    while 1:
        ...
        if isinstance(tokens[0], tuple) and (
            tokens[0][0] == "endif" or tokens[0][0].startswith("elif ") or tokens[0][0] == "else"
        ):
            return part, tokens
...
def parse_for(tokens, name, context):
    ...
    if "(" in vars:
        raise TemplateError(
            "You cannot have () in the variable section of a for loop (%r)" % vars, position=pos, name=name
        )
...
def parse_default(tokens, name, context):
    ...
    if len(parts) == 1:
        raise TemplateError(
            "Expression must be {{default var=value}}; no = found in %r" % first, position=pos, name=name
        )
    var = parts[0].strip()
    if "," in var:
...
def fill_command(args=None):
    ...
    parser.add_option(
        "-o", "--output", dest="output", metavar="FILENAME", help="File to write output to (default stdout)"
    )
    parser.add_option(
        "--env", dest="use_env", action="store_true", help="Put the environment in as top-level variables"
    )
