...
cython.declare(
    ...
    _function_caches=list,
    _parse_file_version=object,
    _match_file_encoding=object,
)
...
def is_cython_generated_file(path, allow_failed=False, if_not_found=True):
    ...
    return (
        # Cython C file?
        file_content.startswith(b"/* Generated by Cython ")
        or
        # Cython output file after previous failures?
        (allow_failed and file_content == failure_marker)
        or
...
@cached_function
def find_versioned_file(
    directory, filename, suffix, _current_version=int(re.sub(r"^([0-9]+)[.]([0-9]+).*", r"\1\2", cython_version))
):
    """
    ...
    matching_files = glob.glob(
        (
            glob.escape(path_prefix)
            if sys.version_info >= (3, 4)
            else "".join(["[" + c + "]" if c in "[*?" else c for c in path_prefix])
        )
        + ".cython-*"
        + suffix
    )
...
_match_file_encoding = re.compile(rb"(\w*coding)[:=]\s*([-\w.]+)").search
