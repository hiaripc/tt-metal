...
class Alias(object):
    ...
    def validate(self):
        ...
        if self.name in self.blacklist:
            raise InvalidAliasError("The name %s can't be aliased " "because it is a keyword or builtin." % self.name)
        try:
            ...
            if not isinstance(caller, Alias):
                raise InvalidAliasError(
                    "The name %s can't be aliased " "because it is another magic command." % self.name
                )
    ...
    def __call__(self, rest=""):
        ...
        if nargs == 0:
            ...
            if len(args) < nargs:
                raise UsageError("Alias <%s> requires %s arguments, %s given." % (self.name, nargs, len(args)))
