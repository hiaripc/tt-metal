...
class BaseIPythonApplication(Application):
    ...
    builtin_profile_dir = Unicode(os.path.join(get_ipython_package_dir(), "config", "profile", "default"))
    ...
    def _profile_changed(self, change):
        self.builtin_profile_dir = os.path.join(get_ipython_package_dir(), "config", "profile", change["new"])
    ...
    def init_config_files(self):
        ...
        if self.copy_config_files:
            ...
            if path and (path / cfg).exists():
                self.log.warning(
                    "Staging %r from %s into %r [overwrite=%s]" % (cfg, src, self.profile_dir.location, self.overwrite)
                )
            ...
            for fullpath in files:
                ...
                if self.profile_dir.copy_config_file(cfg, path=path, overwrite=False):
                    # file was copied
                    self.log.warning(
                        "Staging bundled %s from %s into %r" % (cfg, self.profile, self.profile_dir.location)
                    )
