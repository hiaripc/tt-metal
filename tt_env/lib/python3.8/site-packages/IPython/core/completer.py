...
class Completer(Configurable):
    ...
    use_jedi = Bool(
        ...
        help="Experimental: Use Jedi to generate autocompletions. " "Default to True if jedi is installed.",
    ).tag(config=True)
    ...
    debug = Bool(
        default_value=False,
        help="Enable debug for the Completer. Mostly print extra " "information for experimental jedi integration.",
    ).tag(config=True)
    ...
    def global_matches(self, text):
        ...
        for lst in [list(self.namespace.keys()), list(self.global_namespace.keys())]:
            shortened = {
                "_".join([sub[0] for sub in word.split("_")]): word for word in lst if snake_case_re.match(word)
            }
        ...
        if isinstance(k, tuple):
            if filter_prefix_tuple(k):
                ...
                filtered_key_is_final[key_fragment] |= (
                    _DictKeyState.END_OF_TUPLE if len(k) == prefix_tuple_size + 1 else _DictKeyState.IN_TUPLE
                )
...
def _formatparamchildren(parameter) -> str:
    ...
    if not description.startswith("param "):
        raise ValueError(
            "Jedi function parameter description have change format." 'Expected "param ...", found %r".' % description
        )
...
def _make_signature(completion) -> str:
    ...
    return "(%s)" % ", ".join(
        [
            f
            for f in (
                _formatparamchildren(p) for signature in completion.get_signatures() for p in signature.defined_names()
            )
            if f
        ]
    )
...
class IPCompleter(Completer):
    ...
    profiler_output_dir = Unicode(
        default_value=".completion_profiles", help="Template for path at which to output profile data for completions."
    ).tag(config=True)
    ...
    def dict_key_matches(self, text: str) -> List[str]:
        ...
        can_close_tuple_item = (
            not continuation.startswith(",") and has_known_tuple_handling and self.auto_close_dict_keys
        )
    ...
    def _completions(self, full_text: str, offset: int, *, _timeout) -> Iterator[Completion]:
        ...
        non_jedi_results: Dict[str, SimpleMatcherResult] = {
            identifier: result for identifier, result in results.items() if is_non_jedi_result(result, identifier)
        }

        jedi_matches = (
            cast(_JediMatcherResult, results[jedi_matcher_id])["completions"] if jedi_matcher_id in results else ()
        )
    ...
    def _complete(self, *, cursor_line, cursor_pos, line_buffer=None, text=None, full_text=None) -> _CompleteResult:
        ...
        for matcher_id, matcher in matchers.items():
            ...
            if not suppressed_matchers:
                ...
                should_suppress = (
                    (suppression_config is True) or (suppression_recommended and (suppression_config is not False))
