...
class HistoryManager(HistoryAccessor):
    ...
    def __init__(self, shell=None, config=None, **traits):
        ...
        try:
            ...
            self.log.error(
                "Failed to create history session in %s. History will not be saved.", self.hist_file, exc_info=True
            )
            self.hist_file = ":memory:"
        ...
        with self.db:
            self.db.execute(
                """UPDATE sessions SET end=?, num_cmds=? WHERE
                            session==?""",
                (datetime.datetime.now(), len(self.input_hist_parsed) - 1, self.session_number),
            )
