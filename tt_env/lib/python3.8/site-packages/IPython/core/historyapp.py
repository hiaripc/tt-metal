...
class HistoryTrim(BaseIPythonApplication):
    ...
    def start(self):
        ...
        inputs = list(
            con.execute(
                "SELECT session, line, source, source_raw FROM " "history ORDER BY session DESC, line DESC LIMIT ?",
                (self.keep + 1,),
            )
        )
        ...
        if inputs:
            ...
            outputs = list(
                con.execute("SELECT session, line, output FROM " "output_history WHERE session >= ?", (first_session,))
            )
            sessions = list(
                con.execute(
                    "SELECT session, start, end, num_cmds, remark FROM " "sessions WHERE session >= ?", (first_session,)
                )
            )
        con.close()
...
class HistoryClear(HistoryTrim):
    ...
    flags = Dict(
        dict(force=({"HistoryClear": {"force": True}}, force.help), f=({"HistoryTrim": {"force": True}}, force.help))
    )
    aliases = Dict()
