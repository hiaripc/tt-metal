...
class InteractiveShell(SingletonConfigurable):
    ...
    banner1 = Unicode(default_banner, help="""The part of the banner to be printed before the profile""").tag(
        config=True
    )
    banner2 = Unicode("", help="""The part of the banner to be printed after the profile""").tag(config=True)
    ...
    inspector_class = Type(oinspect.Inspector, help="Class to use to instantiate the shell inspector").tag(config=True)
    ...
    input_transformers_post = List(
        [], help="A list of string input transformers, to be applied after IPython's " "own input transformations."
    )
    ...
    show_rewritten_input = Bool(True, help="Show rewritten input, e.g. for autocall.").tag(config=True)
    ...
    last_execution_result = Instance(
        "IPython.core.interactiveshell.ExecutionResult", help="Result of executing the last command", allow_none=True
    )

    def __init__(
        self, ipython_dir=None, profile_dir=None, user_module=None, user_ns=None, custom_exceptions=((), None), **kwargs
    ):
    ...
    call_pdb = property(_get_call_pdb, _set_call_pdb, None, "Control auto-activation of pdb at exceptions")
    ...
    def init_traceback_handlers(self, custom_exceptions):
        ...
        self.InteractiveTB = ultratb.AutoFormattedTB(
            mode="Plain", color_scheme="NoColor", tb_offset=1, debugger_cls=self.debugger_cls, parent=self
        )
    ...
    def showtraceback(
        self, exc_tuple=None, filename=None, tb_offset=None, exception_only=False, running_compiled_code=False
    ):
    ...
    def init_magics(self):
        ...
        self.magics_manager = magic.MagicsManager(shell=self, parent=self, user_magics=m.UserMagics(self))
    ...
    def run_line_magic(self, magic_name: str, line, _stack_depth=1):
        ...
        if fn is None:
            ...
            extra = (
                "" if cm is None else (" (But cell magic `%%%%%s` exists, " "did you mean that instead?)" % magic_name)
            )
    ...
    def run_cell_magic(self, magic_name, line, cell):
        ...
        if fn is None:
            ...
            extra = (
                ""
                if lm is None
                else (" (But line magic `%{0}` exists, " "did you mean that instead?)".format(magic_name))
            )
        ...
        with self.builtin_trap:
            ...
            with self.display_trap:
                ...
                has_raised = await self.run_ast_nodes(
                    code_ast.body, cell_name, interactivity=interactivity, compiler=compiler, result=result
                )
