...
class BasicMagics(Magics):
    ...
    def alias_magic(self, line=""):
        ...
        params = args.params
        if params and (
            (params.startswith('"') and params.endswith('"')) or (params.startswith("'") and params.endswith("'"))
        ):
            params = params[1:-1]
...
class AsyncMagics(BasicMagics):
    ...
    def autoawait(self, parameter_s):
        ...
        if not param:
            print(
                "IPython autoawait is `{}`, and set to use `{}`".format(d[self.shell.autoawait], self.shell.loop_runner)
            )
            return None
