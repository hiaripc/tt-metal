...
"""
    ...
    def test_recursion_one_frame(self):
        with tt.AssertPrints(re.compile(r"\[\.\.\. skipping similar frames: r1 at line 5 \(\d{2,3} times\)\]")):
            ip.run_cell("r1()")

    @recursionlimit(160)
    def test_recursion_three_frames(self):
        with tt.AssertPrints("[... skipping similar frames: "), tt.AssertPrints(
            re.compile(r"r3a at line 8 \(\d{2} times\)"), suppress=False
        ), tt.AssertPrints(re.compile(r"r3b at line 11 \(\d{2} times\)"), suppress=False), tt.AssertPrints(
