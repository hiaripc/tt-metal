    ...
    def structured_traceback(
        ...
        if exception and (id(exception[1]) not in chained_exc_ids):
            chained_exception_message = (
                self.prepare_chained_exception_message(evalue.__cause__)[0] if evalue is not None else ""
            )
...
class VerboseTB(TBTools):
    ...
    def format_exception(self, etype, evalue):
        ...
        try:
            ...
            etype_str, evalue_str = map(str, (etype, evalue))
        # ... and format it
        return ["%s%s%s: %s" % (colors.excName, etype_str, colorsnormal, py3compat.cast_unicode(evalue_str))]
