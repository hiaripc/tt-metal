...
class Demo(object):
    ...
    def __init__(
        self, src, title="", arg_str="", auto_all=None, format_rst=False, formatter="terminal", style="default"
    ):
    ...
    def show(self, index=None):
        ...
        print(self.marquee("<%s> block # %s (%s remaining)" % (self.title, index, self.nblocks - index - 1)))
    ...
    def show_all(self):
        ...
        for index, block in enumerate(self.src_blocks_colored):
            if silent[index]:
                print(marquee("<%s> SILENT block # %s (%s remaining)" % (title, index, nblocks - index - 1)))
            else:
                print(marquee("<%s> block # %s (%s remaining)" % (title, index, nblocks - index - 1)))
            print(block, end=" ")
        sys.stdout.flush()
    ...
    def __call__(self, index=None):
        ...
        try:
            ...
            if self._silent[index]:
                print(marquee("Executing silent block # %s (%s remaining)" % (index, self.nblocks - index - 1)))
            else:
                self.pre_cmd()
                self.show(index)
...
def slide(
    file_path, noclear=False, format_rst=True, formatter="terminal", style="native", auto_all=False, delimiter="..."
):
    if noclear:
        demo_class = Demo
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noclear", "-C", action="store_true", help="Do not clear terminal on each slide")
    parser.add_argument("--rst", "-r", action="store_true", help="Highlight comments and dostrings as rst")
    parser.add_argument(
        "--formatter",
        "-f",
        default="terminal",
    ...
    parser.add_argument("--auto", "-a", action="store_true", help="Run all blocks automatically without" "confirmation")
    parser.add_argument("--delimiter", "-d", default="...", help="slides delimiter added after each slide run")
