...
class TerminalInteractiveShell(InteractiveShell):
    ...
    autoformatter = Unicode(
        None, help="Autoformatter to reformat Terminal code. Can be `'black'`, `'yapf'` or `None`", allow_none=True
    ).tag(config=True)
    ...
    mouse_support = Bool(
        False, help="Enable mouse support in the prompt\n(Note: prevents selecting text with the mouse)"
    ).tag(config=True)
    ...
    editor = Unicode(get_default_editor(), help="Set the editor used by IPython (default to $EDITOR/vi/notepad).").tag(
        config=True
    )
    ...
    enable_history_search = Bool(True, help="Allows to enable/disable the prompt toolkit history search").tag(
        config=True
    )
    ...
    def _merge_shortcuts(self, user_shortcuts):
        ...
        for shortcut in user_shortcuts:
            ...
            if command_id not in known_commands:
                allowed_commands = "\n - ".join(known_commands)
                raise ValueError(
                    f"{command_id} is not a known shortcut command." f" Allowed commands are: \n - {allowed_commands}"
                )
            ...
            if new_keys != []:
                shortcuts_to_add.append(
                    RuntimeBinding(
                        ...
                        else (old_filter if old_filter is not None else filter_from_string("always")),
                    )
                )
    ...
    prompt_includes_vi_mode = Bool(True, help="Display the current vi mode (when using vi editing mode).").tag(
        config=True
    )
    ...
    def _extra_prompt_options(self):
        ...
        options = {
            ...
            "message": get_message,
            "prompt_continuation": (
                lambda width, lineno, is_soft_wrap: PygmentsTokens(self.prompts.continuation_prompt_tokens(width))
            ),
            "multiline": True,
            ...
            "input_processors": [
                ...
                ConditionalProcessor(
                    ...
                    filter=HasFocus(DEFAULT_BUFFER) & ~IsDone() & Condition(lambda: self.highlight_matching_brackets),
                ),
                ...
                ConditionalProcessor(
                    ...
                    & ~IsDone()
                    & Condition(lambda: isinstance(self.auto_suggest, NavigableAutoSuggestFromHistory)),
                ),
            ],
        }
        if not PTK3:
