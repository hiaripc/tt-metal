...
def skip(msg=None):
    ...
    if msg and not isinstance(msg, str):
        raise ValueError("invalid object passed to `@skip` decorator, did you " "meant `@skip()` with brackets ?")
...
skip_linux = skipif(sys.platform.startswith("linux"), "This test does not run under Linux")
skip_osx = skipif(sys.platform == "darwin", "This test does not run under OS X")


# Decorators to skip tests if not on specific platforms.
skip_if_not_win32 = skipif(sys.platform != "win32", "This test only runs under Windows")
skip_if_not_linux = skipif(not sys.platform.startswith("linux"), "This test only runs under Linux")
...
onlyif_unicode_paths = onlyif(unicode_paths, ("This test is only applicable " "where we can use unicode in filenames."))
