...
class Win32ShellCommandController(object):
    ...
    def run(self, stdout_func=None, stdin_func=None, stderr_func=None):
        ...
        if stdin_func:
            stdin_thread = threading.Thread(
                target=self._stdin_thread, args=(self.hstdin, self.piProcInfo.hProcess, stdin_func, stdout_func)
            )
    ...
    def _run_stdio(self):
        ...
        if self.mergeout:
            ...
            return self.run(
                stdout_func=self._stdout_raw, stdin_func=self._stdin_raw_block, stderr_func=self._stderr_raw
            )
