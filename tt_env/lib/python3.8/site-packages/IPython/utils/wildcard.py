...
def list_namespace(namespace, type_pattern, filter, ignore_case=False, show_all=False):
    ...
    if len(pattern_list) == 1:
        ...
        # modules
        filtered = filter_ns(
            namespace, name_pattern=pattern_list[0], type_pattern="all", ignore_case=ignore_case, show_all=show_all
        )
        results = {}
        for name, obj in filtered.items():
            ns = list_namespace(
                dict_dir(obj), type_pattern, ".".join(pattern_list[1:]), ignore_case=ignore_case, show_all=show_all
            )
