...
class EnumeratedValidator(BaseValidator):
    ...
    def build_regex_replacement(regex_str):
        ...
        match = re.match(r"\^(\w)\(\[2\-9\]\|\[1\-9\]\[0\-9\]\+\)\?\( domain\)\?\$", regex_str)
...
class AngleValidator(BaseValidator):
    ...
    (e.g. 270 is converted to -90).
        """.format(
            plotly_name=self.plotly_name,
            array_ok=", or a list, numpy array or other iterable thereof" if self.array_ok else "",
        )
...
""".format(
    ...
    def present(self, v):
        if v is None:
            ...
            if self.dimensions == 2 or self.dimensions == "1-2" and v and is_array(v[0]):
                # 2D case
                v = copy.deepcopy(v)
                for row in v:
