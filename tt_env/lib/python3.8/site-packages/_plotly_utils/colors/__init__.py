    ...
    if isinstance(colors, str):
        if colors in PLOTLY_SCALES:
            ...
            raise exceptions.PlotlyError(
                "If your colors variable is a string, it must be a " "Plotly scale, an rgb color or a hex color."
            )
...
def validate_scale_values(scale):
    ...
    if not all(x < y for x, y in zip(scale, scale[1:])):
        raise exceptions.PlotlyError(
            "'scale' must be a list that contains a strictly increasing " "sequence of numbers."
        )
...
def sample_colorscale(colorscale, samplepoints, low=0.0, high=1.0, colortype="rgb"):
    ...
    if isinstance(samplepoints, int):
        samplepoints = [low + idx / (samplepoints - 1) * (high - low) for idx in range(samplepoints)]
