...
class ClientRequest:
    ...
    _HOST_STRINGS = frozenset(map("".join, itertools.product(*zip("host".upper(), "host".lower()))))
    ...
    def __init__(
        ...
        *,
        ...
        if match := _CONTAINS_CONTROL_CHAR_RE.search(method):
            raise ValueError(
                f"Method cannot contain non-token characters {method!r} " f"(found at least {match.group()!r})"
            )
    ...
    async def send(self, conn: "Connection") -> "ClientResponse":
        ...
        writer = StreamWriter(
            ...
            self.loop,
            on_chunk_sent=(
                functools.partial(self._on_chunk_request_sent, self.method, self.url) if self._traces else None
            ),
            on_headers_sent=(
                functools.partial(self._on_headers_request_sent, self.method, self.url) if self._traces else None
            ),
        )
...
class ClientResponse(HeadersMixin):
    ...
    def __repr__(self) -> str:
        ...
        print(
            "<ClientResponse({}) [{} {}]>".format(ascii_encodable_url, self.status, ascii_encodable_reason),
            file=out,
        )
