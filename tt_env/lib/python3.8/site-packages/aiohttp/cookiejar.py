...
class CookieJar(AbstractCookieJar):
    ...
    DATE_TOKENS_RE = re.compile(
        r"[\x09\x20-\x2F\x3B-\x40\x5B-\x60\x7B-\x7E]*" r"(?P<token>[\x00-\x08\x0A-\x1F\d:a-zA-Z\x7F-\xFF]+)"
    )
    ...
    MAX_TIME = int(datetime.datetime.max.replace(tzinfo=datetime.timezone.utc).timestamp()) - 1
    try:
    ...
    def clear(self, predicate: Optional[ClearCookiePredicate] = None) -> None:
        ...
        to_del = [
            ...
            if ((key := (domain, path, name)) in self._expirations and self._expirations[key] <= now)
            or predicate(morsel)
        ]
        if to_del:
