    ...
    def parse_headers(self, lines: List[bytes]) -> Tuple["CIMultiDictProxy[str]", RawHeaders]:
        ...
        while line:
            ...
            if len(bname) > self.max_field_size:
                raise LineTooLong(
                    "request header name {}".format(bname.decode("utf8", "backslashreplace")),
            ...
            if continuation:
                ...
                while continuation:
                    ...
                    if header_length > self.max_field_size:
                        raise LineTooLong(
                            "request header field {}".format(bname.decode("utf8", "backslashreplace")),
                            str(self.max_field_size),
                            str(header_length),
                        )
                ...
                if pos >= start_pos:
                    ...
                    if self._lines[-1] == EMPTY:
                        ...
                        if not empty_body and (
                            ((length is not None and length > 0) or msg.chunked) and not self._upgraded
                        ):
