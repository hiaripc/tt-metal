        ...
        for fin, opcode, payload, compressed in self.parse_frame(data):
            if opcode in MESSAGE_TYPES_WITH_CONTENT:
                ...
                if not fin:
                    ...
                    if self._max_msg_size and len(self._partial) >= self._max_msg_size:
                        raise WebSocketError(
                            ...
                            "Message size {} exceeds limit {}".format(len(self._partial), self._max_msg_size),
                        )
                    continue
                ...
                # we should get continuation opcode
                    ...
                    raise WebSocketError(
                        WSCloseCode.PROTOCOL_ERROR,
                        "The opcode in non-fin frame is expected " "to be zero, got {!r}".format(opcode),
                    )
                ...
                if self._max_msg_size and len(assembled_payload) >= self._max_msg_size:
                    raise WebSocketError(
                        WSCloseCode.MESSAGE_TOO_BIG,
                        "Message size {} exceeds limit {}".format(len(assembled_payload), self._max_msg_size),
                    )
