...
def pytest_fixture_setup(fixturedef):  # type: ignore[no-untyped-def]
    ...
    def wrapper(*args, **kwargs):  # type: ignore[no-untyped-def]
        ...
        if "loop" not in request.fixturenames:
            raise Exception(
                "Asynchronous fixtures must depend on the 'loop' fixture or " "be used in tests depending from it."
            )
...
def pytest_pyfunc_call(pyfuncitem):  # type: ignore[no-untyped-def]
    ...
    if asyncio.iscoroutinefunction(pyfuncitem.function):
        existing_loop = pyfuncitem.funcargs.get("proactor_loop") or pyfuncitem.funcargs.get("loop", None)
...
def aiohttp_server(loop: asyncio.AbstractEventLoop) -> Iterator[AiohttpServer]:
    ...
    async def go(app: Application, *, port: Optional[int] = None, **kwargs: Any) -> TestServer:
