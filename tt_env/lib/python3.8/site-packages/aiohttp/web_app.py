...
class Application(MutableMapping[Union[str, AppKey[Any]], Any]):
    ...
    def __init__(
        ...
        *,
        ...
        if router is None:
            ...
            warnings.warn("router argument is deprecated", DeprecationWarning, stacklevel=2)
        ...
        if debug is not ...:
            warnings.warn("debug argument is deprecated", DeprecationWarning, stacklevel=2)
        self._debug = debug
    ...
    def __init_subclass__(cls: Type["Application"]) -> None:
        warnings.warn(
            "Inheritance class {} from web.Application " "is discouraged".format(cls.__name__),
            DeprecationWarning,
            stacklevel=3,
        )
    ...
    if DEBUG:  # pragma: no cover
        ...
        def __setattr__(self, name: str, val: Any) -> None:
            if name not in self.ATTRS:
                warnings.warn(
                    "Setting custom web.Application.{} attribute " "is discouraged".format(name),
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    def _add_subapp(self, resource_factory: Callable[[], _Resource], subapp: "Application") -> _Resource:
        if self.frozen:
    ...
    async def _handle(self, request: Request) -> StreamResponse:
        ...
        if debug:  # pragma: no cover
            if not isinstance(match_info, AbstractMatchInfo):
                raise TypeError("match_info should be AbstractMatchInfo " "instance, not {!r}".format(match_info))
        ...
        if self._run_middlewares:
            ...
            # is made each time.
            if not self._has_legacy_middlewares and not isinstance(match_info.route, SystemRoute):
