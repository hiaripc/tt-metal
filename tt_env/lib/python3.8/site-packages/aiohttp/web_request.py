...
_QDTEXT: Final[str] = r"[{}]".format(r"".join(chr(c) for c in (0x09, 0x20, 0x21) + tuple(range(0x23, 0x7F))))
# qdtext includes 0x5C to escape 0x5D ('\]')
# qdtext excludes obs-text (because obsoleted, and encoding not specified)

_QUOTED_PAIR: Final[str] = r"\\[\t !-~]"

_QUOTED_STRING: Final[str] = r'"(?:{quoted_pair}|{qdtext})*"'.format(qdtext=_QDTEXT, quoted_pair=_QUOTED_PAIR)
...
class BaseRequest(MutableMapping[str, Any], HeadersMixin):
    ...
    def clone(
        ...
        *,
        ...
        if headers is not sentinel:
            ...
            dct["raw_headers"] = tuple((k.encode("utf-8"), v.encode("utf-8")) for k, v in dct["headers"].items())
    ...
    def loop(self) -> asyncio.AbstractEventLoop:
        warnings.warn("request.loop property is deprecated", DeprecationWarning, stacklevel=2)
        return self._loop
    ...
    def has_body(self) -> bool:
        ...
        warnings.warn("Deprecated, use .can_read_body #2005", DeprecationWarning, stacklevel=2)
    ...
    if DEBUG:
        ...
        def __setattr__(self, name: str, val: Any) -> None:
            if name not in self.ATTRS:
                warnings.warn(
                    "Setting custom {}.{} attribute " "is discouraged".format(self.__class__.__name__, name),
                    DeprecationWarning,
                    stacklevel=2,
                )
