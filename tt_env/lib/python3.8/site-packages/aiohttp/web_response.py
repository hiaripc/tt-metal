...
class StreamResponse(BaseClass, HeadersMixin):
    ...
    def etag(self, value: Optional[Union[ETag, str]]) -> None:
        if value is None:
            ...
            raise ValueError(f"Unsupported etag type: {type(value)}. " f"etag must be str, ETag or None")
    ...
    async def _prepare_headers(self) -> None:
        ...
        if self._chunked:
            if version != HttpVersion11:
                raise RuntimeError(
                    "Using chunked encoding is forbidden " "for HTTP/{0.major}.{0.minor}".format(request.version)
                )
...
class Response(StreamResponse):
    ...
    def text(self, text: str) -> None:
        assert text is None or isinstance(text, str), "text argument must be str (%r)" % type(text)
    ...
    async def _start(self, request: "BaseRequest") -> AbstractStreamWriter:
        if hdrs.CONTENT_LENGTH in self._headers:
            ...
            if isinstance(self._body, Payload):
                ...
                if body_len != "0" or (self.status != 304 and request.method.upper() != hdrs.METH_HEAD):
