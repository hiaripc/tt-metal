...
class NamedPipeSite(BaseSite):
    ...
    def __init__(self, runner: "BaseRunner", path: str, *, shutdown_timeout: float = 60.0) -> None:
        loop = asyncio.get_event_loop()
        if not isinstance(loop, asyncio.ProactorEventLoop):  # type: ignore[attr-defined]
    ...
    async def start(self) -> None:
        ...
        server = self._runner.server
        assert server is not None
        _server = await loop.start_serving_pipe(server, self._path)  # type: ignore[attr-defined]
