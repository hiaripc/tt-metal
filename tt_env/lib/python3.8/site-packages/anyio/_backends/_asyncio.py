...
class TaskGroup(abc.TaskGroup):
    ...
    async def __aexit__(
        ...
        self._active = False
        if self._exceptions:
            raise BaseExceptionGroup("unhandled errors in a TaskGroup", self._exceptions)
...
class WorkerThread(Thread):
    ...
    def __init__(
        ...
        self.queue: Queue[tuple[Context, Callable, tuple, asyncio.Future, CancelScope] | None] = Queue(2)
        self.idle_since = AsyncIOBackend.current_time()
        self.stopping = False

    def _report_result(self, future: asyncio.Future, result: Any, exc: BaseException | None) -> None:
        self.idle_since = AsyncIOBackend.current_time()
        ...
        with self._send_guard:
            while True:
                try:
                    ...
                    self._raw_socket.sendmsg([message], [(socket.SOL_SOCKET, socket.SCM_RIGHTS, fdarray)])
                    break
                except BlockingIOError:
                    await self._wait_until_writable(loop)
                except OSError as exc:
...
class AsyncIOTaskInfo(TaskInfo):
    ...
    def has_pending_cancellation(self) -> bool:
        ...
        if sys.version_info >= (3, 11):
            if task.cancelling():
                return True
        elif isinstance(task._fut_waiter, asyncio.Future) and task._fut_waiter.cancelled():
            return True

        if task_state := _task_states.get(task):
            if cancel_scope := task_state.cancel_scope:
                return cancel_scope.cancel_called or (not cancel_scope.shield and cancel_scope._parent_cancelled())
...
class TestRunner(abc.TestRunner):
    ...
    def _raise_async_exceptions(self) -> None:
        ...
        if self._exceptions:
            ...
            if len(exceptions) == 1:
                ...
                raise BaseExceptionGroup("Multiple exceptions occurred in asynchronous callbacks", exceptions)

    async def _run_tests_and_fixtures(
        self,
        receive_stream: MemoryObjectReceiveStream[tuple[Awaitable[T_Retval], asyncio.Future[T_Retval]]],
    ) -> None:
    ...
    async def _call_in_runner_task(
        ...
        **kwargs: P.kwargs,
        ...
        if not self._runner_task:
            self._send_stream, receive_stream = create_memory_object_stream[Tuple[Awaitable[Any], asyncio.Future]](1)
            self._runner_task = self.get_loop().create_task(self._run_tests_and_fixtures(receive_stream))
    ...
    def run_asyncgen_fixture(
        ...
        fixturevalue: T_Retval = self.get_loop().run_until_complete(self._call_in_runner_task(asyncgen.asend, None))
    ...
    def run_fixture(
        ...
        retval = self.get_loop().run_until_complete(self._call_in_runner_task(fixture_func, **kwargs))
        self._raise_async_exceptions()
        return retval
...
class AsyncIOBackend(AsyncBackend):
    ...
    async def connect_tcp(cls, host: str, port: int, local_address: IPSockAddrType | None = None) -> abc.SocketStream:
        transport, protocol = cast(
            ...
            await get_running_loop().create_connection(StreamProtocol, host, port, local_addr=local_address),
        )
