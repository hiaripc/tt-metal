...
def expect_token(token, tok_type, tok_str=None):
    ...
    if not match_token(token, tok_type, tok_str):
        raise ValueError(
            ...
            % (token_repr(tok_type, tok_str), str(token), token.start[0], token.start[1] + 1)
        )
...
SINGLETONS = {
    ...
    if isinstance(c, type) and issubclass(c, (ast.expr_context, ast.boolop, ast.operator, ast.unaryop, ast.cmpop))
}
...
stmt_class_names = {n for n, c in ast.__dict__.items() if isinstance(c, type) and issubclass(c, ast.stmt)}
expr_class_names = {n for n, c in ast.__dict__.items() if isinstance(c, type) and issubclass(c, ast.expr)} | {
    "AssignName",
    "DelName",
    "Const",
    "AssignAttr",
...
def is_slice(node):
    ...
    return node.__class__.__name__ in ("Slice", "ExtSlice") or (
        node.__class__.__name__ == "Tuple" and any(map(is_slice, cast(ast.Tuple, node).elts))
    )
