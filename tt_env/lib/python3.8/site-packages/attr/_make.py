...
def _transform_attrs(cls, these, auto_attribs, kw_only, collect_by_mro, field_transformer) -> _Attributes:
    """
...
def _make_cached_property_getattr(cached_properties, original_getattr, cls):
    ...
    return _linecache_and_compile("\n".join(lines), unique_filename, glob, locals={"_cls": cls})["__getattr__"]
...
class _ClassBuilder:
    ...
    def _create_slots_class(self):
        ...
        cached_properties = {
            name: cached_prop.func for name, cached_prop in cd.items() if isinstance(cached_prop, cached_property)
        }
    ...
    def add_replace(self):
        self._cls_dict["__replace__"] = self._add_method_dunders(lambda self, **changes: evolve(self, **changes))
        return self
...
def attrs(
    ...
    *never* go away, though).
    ...
    if repr_ns is not None:
        ...
        warnings.warn(
            DeprecationWarning("The `repr_ns` argument is deprecated and will be removed in or after August 2025."),
            stacklevel=2,
        )
...
def _generate_unique_filename(cls: type, func_name: str) -> str:
    ...
    """
    return f"<attrs generated {func_name} {cls.__module__}." f"{getattr(cls, '__qualname__', cls.__name__)}>"


def _make_hash_script(cls: type, attrs: list[Attribute], frozen: bool, cache_hash: bool) -> tuple[str, dict]:
...
def _make_order(cls, attrs):
    ...
    def attrs_to_tuple(obj):
        ...
        """
        return tuple(
            key(value) if key else value for value, key in ((getattr(obj, a.name), a.order_key) for a in attrs)
        )
...
def _attrs_to_init_script(
    ...
    extra_lines, fmt_setter, fmt_setter_with_converter = _determine_setters(is_frozen, is_slotted, base_attr_map)
    ...
    for a in attrs:
        ...
        if a.init is False:
            ...
            if converter is not None:
                lines.append(fmt_setter_with_converter(attr_name, arg_name, has_on_setattr, converter))
                names_for_globals[converter._get_global_name(a.name)] = converter.converter
            else:
...
class Attribute:
    ...
    *Read-only* representation of an attribute.
    ...
    def __init__(
        ...
        bound_setattr(
            "metadata",
            (types.MappingProxyType(dict(metadata)) if metadata else _EMPTY_METADATA_SINGLETON),  # Shallow copy
        )
