...
def _format_fallback_interval(
    ...
    if skeleton in locale.datetime_skeletons:  # Use the given skeleton
        ...
        (isinstance(d, datetime.date) and not isinstance(d, datetime.datetime)) for d in (start, end)
    ):  # Both are just dates
        format = lambda dt: format_date(dt, locale=locale)
    elif all(
        (isinstance(d, datetime.time) and not isinstance(d, datetime.date)) for d in (start, end)
    ):  # Both are times
        ...
        for rule in rules:
            if "from" in rule and "before" in rule:
                if rule["from"] < rule["before"]:
                    ...
                    if rule["from"] <= seconds_past_midnight < 86400 or 0 <= seconds_past_midnight < rule["before"]:
                        return rule_id
...
def parse_date(
    ...
    iso_alike = re.match(r"^(\d{4})-?([01]\d)-?([0-3]\d)$", string, flags=re.ASCII)  # allow only ASCII digits
