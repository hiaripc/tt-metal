...
class CompileCatalog(CommandMixin):
    ...
    user_options = [
        ...
        ("output-file=", "o", "name of the output file (default " "'<output_dir>/<locale>/LC_MESSAGES/<domain>.mo')"),
...
class ExtractMessages(CommandMixin):
    ...
    user_options = [
        ...
        ("no-wrap", None, "do not break long message lines, longer than the output line width, " "into several lines"),
        ...
        ("last-translator=", None, "set the name and email of the last translator in output"),
    ]
    boolean_options = [
        "no-default-keywords",
        "no-location",
        ...
        if self.mapping_file:
            if self.mapping_file.endswith(".toml"):
                with open(self.mapping_file, "rb") as fileobj:
                    file_style = (
                        "pyproject.toml" if os.path.basename(self.mapping_file) == "pyproject.toml" else "standalone"
                    )
...
class InitCatalog(CommandMixin):
    ...
    user_options = [
        ...
        ("output-file=", "o", "name of the output file (default " "'<output_dir>/<locale>/LC_MESSAGES/<domain>.po')"),
...
class UpdateCatalog(CommandMixin):
    ...
    user_options = [
        ...
        ("init-missing=", None, "if any output files are missing, initialize them first"),
        ...
        ("ignore-pot-creation-date=", None, "ignore changes to POT-Creation-Date when updating or checking"),
    ]
    boolean_options = [
        "omit-header",
        "no-wrap",
    ...
    def run(self):
        ...
        if not self.output_file:
            if self.locale:
                po_files.append(
                    (self.locale, os.path.join(self.output_dir, self.locale, "LC_MESSAGES", f"{self.domain}.po"))
                )
            else:
