...
def parse_number(
    ...
    *,
    ...
    if (
        ...
        and group_symbol not in string  # if the grouping symbol is a kind of space,
        and SPACE_CHARS_RE.search(  # and the string to be parsed does not contain it,
            string
        ...
        if string != proper and proper != _remove_trailing_zeros_after_decimal(string, decimal_symbol):
            try:
                ...
                raise NumberFormatError(
                    f"{string!r} is not a properly formatted decimal number. " f"Did you mean {proper!r}?",
                    suggestions=[proper],
                ) from exc
            else:
...
class NumberPattern:
    ...
    def apply(
        ...
        *,
        ...
        retval = "".join(
            [self.prefix[is_negative], number if self.number_pattern != "" else "", self.suffix[is_negative]]
        )
        ...
        *,
        ...
        if group_separator:
            integer_part = self._format_int(
                a, self.int_prec[0], self.int_prec[1], locale, numbering_system=numbering_system
            )
        number = integer_part + self._format_frac(
            b or "0", locale=locale, force_frac=frac_prec, numbering_system=numbering_system
        )
        return number
