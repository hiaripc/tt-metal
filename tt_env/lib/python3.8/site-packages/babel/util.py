...
PYTHON_MAGIC_COMMENT_re = re.compile(rb"[ \t\f]* \# .* coding[=:][ \t]*([-\w.]+)", re.VERBOSE)
...
def wraptext(text: str, width: int = 70, initial_indent: str = "", subsequent_indent: str = "") -> list[str]:
    ...
    wrapper = TextWrapper(
        width=width, initial_indent=initial_indent, subsequent_indent=subsequent_indent, break_long_words=False
    )
