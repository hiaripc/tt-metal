...
class BeautifulTable(object):
    ...
    def sign(self, value):
        if not isinstance(value, enums.SignMode):
            allowed = ("{}.{}".format(type(self).__name__, i.name) for i in enums.SignMode)
    ...
    def _compute_width(self):
        ...
        for index, header in enumerate(self.columns.header):
            max_length = 0
            for i in pre_process(header, self.detect_numerics, self.precision, self.sign.value).split("\n"):
        ...
        for index, column in enumerate(zip(*self._data)):
            ...
            for i in column:
                for j in pre_process(i, self.detect_numerics, self.precision, self.sign.value).split("\n"):
                    output_str = pre_process(
                        j,
                        self.detect_numerics,
                        self.precision,
    ...
    def _get_horizontal_line(self, char, intersect_left, intersect_mid, intersect_right, mask=None):
                ...
                if not (self.columns.separator.isspace() and visible_junc):
                    ...
                    for i in range(len(self.columns) - 1):
                        ...
                        for j in range(length):
                            ...
                            line[index + j] = intersect_mid[j] if (mask[i] or mask[i + 1]) else " "
    ...
    def _get_top_border(self, *args, **kwargs):
        return self._get_horizontal_line(
            self.border.top, self.border.top_left, self.border.top_junction, self.border.top_right, *args, **kwargs
        )
    ...
    def _get_row_separator(self, *args, **kwargs):
        return self._get_horizontal_line(
            self.rows.separator, self.border.left_junction, self.junction, self.border.right_junction, *args, **kwargs
        )
    ...
    def _get_string(self, rows=None, append=False, recalculate_width=True):
        row_header_visible = bool("".join(x if x is not None else "" for x in self.rows.header).strip()) and (
            len(self.columns) > 0
        )

        column_header_visible = bool("".join(x if x is not None else "" for x in self.columns.header).strip()) and (
            len(self.rows) > 0 or rows is not None
        )

        # Preparing table for printing serialno, row headers and column headers
        if len(self.columns) > 0:
            if self._serialno:
                self.columns.insert(0, range(1, len(self.rows) + 1), self._serialno_header)
        ...
        if (self.columns._auto_width and recalculate_width) or sum(self.columns.width) == 0:
            self._compute_width()
        ...
        try:
            ...
            if column_header_visible:
                yield next(row_iterator)._get_string(align=self.columns.header.alignment)
    ...
    def to_csv(self, file_name, *args, **kwargs):
        ...
        if not isinstance(file_name, str):
            raise ValueError(("Expected 'file_name' to be string, got {}").format(type(file_name).__name__))

        with open(file_name, mode="wt", newline="") as csv_file:
            csv_writer = csv.writer(csv_file, *args, **kwargs)
            if bool("".join(x if x is not None else "" for x in self.columns.header).strip()):
    ...
    def to_df(self):
        ...
        try:
            ...
            warnings.warn("This method requires that 'pandas' is installed.", RuntimeWarning)
            raise
        ...
        return pd.DataFrame([list(row) for row in self.rows], columns=headers, index=index)
