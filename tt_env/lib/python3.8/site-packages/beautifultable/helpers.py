...
class BTRowData(BTBaseRow):
    ...
    def _clamp_row(self, row):
        ...
        if wep is enums.WidthExceedPolicy.WEP_STRIP or wep is enums.WidthExceedPolicy.WEP_ELLIPSIS:
            # Let's strip the row
    ...
    def _clamp_string(self, row_item, index, delimiter=""):
        ...
        if termwidth(row_item) <= width:
            ...
            if width - len(delimiter) >= 0:
                clamped_string = textwrap(row_item, width - len(delimiter))[0] + delimiter
            else:
    ...
    def _get_string(
        ...
        for row in map(list, zip_longest(*rows, fillvalue="")):
            ...
            for row_ in self._clamp_row(row):
                ...
                content = (table.border.left if mask[0] else " " * termwidth(table.border.left)) + content
                content += table.border.right if mask[-1] else " " * termwidth(table.border.right)
...
class BTRowCollection(object):
    ...
    def _canonical_key(self, key):
        ...
        raise TypeError(("row indices must be int, str or slices, not {}").format(type(key).__name__))
    ...
    def __getitem__(self, key):
        ...
        raise TypeError(("row indices must be int, str or a slice object, not {}").format(type(key).__name__))
    ...
    def pop(self, index=-1):
        ...
        if not isinstance(index, (int, basestring)):
            raise TypeError(("row index must be int or str, " "not {}").format(type(index).__name__))
...
class BTColumnCollection(object):
    ...
    def padding(self):
        ...
        raise AttributeError("cannot read attribute 'padding'. use 'padding_{left|right}'")
    ...
    def _reset_state(self, ncol):
        ...
        self._table._data = type(self._table._data)(
            self._table,
            [BTRowData(self._table, [None] * ncol) for i in range(len(self._table._data))],
        )
    ...
    def _canonical_key(self, key):
        if isinstance(key, (int, slice)):
            ...
            return self.header.index(key)
        raise TypeError(("column indices must be int, str or slices, not {}").format(type(key).__name__))
    ...
    def width_exceed_policy(self, value):
        if not isinstance(value, enums.WidthExceedPolicy):
            allowed = ("{}.{}".format(type(self).__name__, i.name) for i in enums.WidthExceedPolicy)
            error_msg = "allowed values for width_exceed_policy are: " + ", ".join(allowed)
    ...
    def default_alignment(self, value):
        if not isinstance(value, enums.Alignment):
            allowed = ("{}.{}".format(type(self).__name__, i.name) for i in enums.Alignment)
            error_msg = "allowed values for default_alignment are: " + ", ".join(allowed)
            raise ValueError(error_msg)
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, int):
            ...
            raise TypeError(
                ("column indices must be integers, strings or " "slices, not {}").format(type(key).__name__)
            )
    ...
    def __delitem__(self, key):
        ...
        if isinstance(key, (int, basestring, slice)):
            ...
            raise TypeError(("table indices must be int, str or " "slices, not {}").format(type(key).__name__))
    ...
    def pop(self, index=-1):
        ...
        """
        if not isinstance(index, (int, basestring)):
            raise TypeError(("column index must be int or str, " "not {}").format(type(index).__name__))
        if self._table._ncol == 0:
            raise IndexError("pop from empty table")
        else:
            res = []
    ...
    def insert(
        ...
        if not isinstance(padding_left, int):
            raise TypeError("'padding_left' should be of type 'int' not '{}'".format(type(padding_left).__name__))
        if not isinstance(padding_right, int):
            raise TypeError("'padding_right' should be of type 'int' not '{}'".format(type(padding_right).__name__))
        if not isinstance(alignment, enums.Alignment):
            raise TypeError(
                "alignment should be of type '{}' not '{}'".format(enums.Alignment.__name__, type(alignment).__name__)
            )
        ...
        if self._table._ncol == 0:
            ...
            self._table._data = type(self._table._data)(self._table, [BTRowData(self._table, [i]) for i in column])
        else:
            if (not isinstance(header, basestring)) and (header is not None):
                raise TypeError("header must be of type 'str' not '{}'".format(type(header).__name__))
            column_length = 0
            ...
            if column_length == len(self._table.rows):
                ...
                raise ValueError(
                    ("length of 'column' should be atleast {}, " "got {}").format(len(self._table.rows), column_length)
                )
