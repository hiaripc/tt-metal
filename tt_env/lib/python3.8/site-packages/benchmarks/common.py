...
class BuiltinsGenerator(object):
    ...
    def generate_decimal_list(self, n, none_prob=DEFAULT_NONE_PROB, use_nan=False):
        ...
        assert len(data) == n
        self.sprinkle(data, none_prob, value=decimal.Decimal("nan") if use_nan else None)
        return data
    ...
    def generate_fixed_binary_list(self, n, size, none_prob=DEFAULT_NONE_PROB):
        ...
        """
        return self._generate_varying_sequences(get_random_bytes, n, size, size, none_prob)

    def generate_varying_binary_list(self, n, min_size, max_size, none_prob=DEFAULT_NONE_PROB):
        """
        Generate a list of bytestrings with a random size between
        *min_size* and *max_size*.
        """
        return self._generate_varying_sequences(get_random_bytes, n, min_size, max_size, none_prob)

    def generate_ascii_string_list(self, n, min_size, max_size, none_prob=DEFAULT_NONE_PROB):
        """
        Generate a list of ASCII strings with a random size between
        *min_size* and *max_size*.
        """
        return self._generate_varying_sequences(get_random_ascii, n, min_size, max_size, none_prob)

    def generate_unicode_string_list(self, n, min_size, max_size, none_prob=DEFAULT_NONE_PROB):
        """
        Generate a list of unicode strings with a random size between
        *min_size* and *max_size*.
        """
        return self._generate_varying_sequences(get_random_unicode, n, min_size, max_size, none_prob)

    def generate_int_list_list(self, n, min_size, max_size, none_prob=DEFAULT_NONE_PROB):
        """
        Generate a list of lists of Python ints with a random size between
        *min_size* and *max_size*.
        """
        return self._generate_varying_sequences(
            partial(self.generate_int_list, none_prob=none_prob), n, min_size, max_size, none_prob
        )
    ...
    def generate_tuple_list(self, n, none_prob=DEFAULT_NONE_PROB):
        ...
        tuples = [(d.get("u"), d.get("v"), d.get("w")) if d is not None else None for d in dicts]
        assert len(tuples) == n
        return tuples
    ...
    def get_type_and_builtins(self, n, type_name):
        ...
        if kind in ("int", "float"):
            ...
            ty = pa.struct([pa.field("u", pa.int64()), pa.field("v", pa.float64()), pa.field("w", pa.bool_())])
        ...
        factories = {
            ...
            "varying binary": partial(self.generate_varying_binary_list, min_size=3, max_size=40),
            "ascii": partial(self.generate_ascii_string_list, min_size=3, max_size=40),
            "unicode": partial(self.generate_unicode_string_list, min_size=3, max_size=40),
