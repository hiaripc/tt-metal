...
class ParquetWriteBinary(object):
    def setup(self):
        ...
        unique_values = np.array([rands(value_size) for i in range(nuniques)], dtype="O")
        values = unique_values[np.random.randint(0, nuniques, size=length)]
        self.table = pa.table(
            [pa.array(values) for i in range(num_cols)], names=["f{}".format(i) for i in range(num_cols)]
        )
...
def generate_dict_table(num_cols, string_size, nunique, length, random_order=True):
    data = generate_dict_strings(string_size, nunique, length, random_order=random_order)
    return pa.table([data for i in range(num_cols)], names=["f{}".format(i) for i in range(num_cols)])
...
class ParquetWriteDictionaries(object):
    ...
    def setup(self, nunique):
        ...
        self.table = generate_dict_table(self.num_cols, self.value_size, self.nunique, self.length)
        self.table_sequential = generate_dict_table(
            self.num_cols, self.value_size, self.nunique, self.length, random_order=False
        )
...
class ParquetManyColumns(object):
    ...
    def setup(self, num_cols):
        ...
        self.table = pa.table({"c" + str(i): np.random.randn(num_rows) for i in range(num_cols)})
