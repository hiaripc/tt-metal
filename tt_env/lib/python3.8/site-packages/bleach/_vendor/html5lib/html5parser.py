    ...
    class InBodyPhase(Phase):
        ...
        def startTagButton(self, token):
            if self.tree.elementInScope("button"):
                self.parser.parseError(
                    "unexpected-start-tag-implies-end-tag", {"startName": "button", "endName": "button"}
                )
        ...
        def startTagImage(self, token):
            ...
            self.processStartTag(
                impliedTagToken("img", "StartTag", attributes=token["data"], selfClosing=token["selfClosing"])
            )
        ...
        def startTagIsIndex(self, token):
            ...
            self.processStartTag(
                impliedTagToken("input", "StartTag", attributes=attributes, selfClosing=token["selfClosing"])
            )
            ...
            if not self.tree.elementInScope("body"):
                ...
                for node in self.tree.openElements[2:]:
                    if node.name not in frozenset(
                        ...
                        self.parser.parseError(
                            "expected-one-end-tag-but-got-another", {"gotName": "body", "expectedName": node.name}
                        )
                        break
        ...
        def endTagTable(self, token):
            if self.tree.elementInScope("table", variant="table"):
                ...
                if self.tree.openElements[-1].name != "table":
                    self.parser.parseError(
                        "end-tag-too-early-named", {"gotName": "table", "expectedName": self.tree.openElements[-1].name}
                    )
    ...
    class InRowPhase(Phase):
        ...
        def clearStackToTableRowContext(self):
            while self.tree.openElements[-1].name not in ("tr", "html"):
                self.parser.parseError(
                    "unexpected-implied-end-tag-in-table-row", {"name": self.tree.openElements[-1].name}
                )
                self.tree.openElements.pop()
