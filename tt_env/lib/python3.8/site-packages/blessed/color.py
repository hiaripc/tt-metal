...
def dist_rgb_weighted(rgb1, rgb2):
    ...
    return (
        (2 + red_mean / 256) * pow(rgb1[0] - rgb2[0], 2)
...
def dist_cie94(rgb1, rgb2):
    ...
    return (
        (delta_l / (k_l * s_l)) ** 2
        + (delta_c / (k_c * s_c)) ** 2  # pylint: disable=superfluous-parens
        + (delta_h / (k_h * s_h)) ** 2
    )
...
def dist_cie2000(rgb1, rgb2):
    ...
    t_x = (
        1 - 0.17 * cos(h_mean - 30) + 0.24 * cos(2 * h_mean) + 0.32 * cos(3 * h_mean + 6) - 0.20 * cos(4 * h_mean - 63)
    )
