...
class ParameterizingString(six.text_type):
    ...
    def __new__(cls, cap, normal="", name="<not specified>"):
    ...
    def __call__(self, *args):
        ...
        try:
            ...
            if args and isinstance(args[0], six.string_types):
                raise TypeError(
                    "Unknown terminal capability, %r, or, TypeError " "for arguments %r: %s" % (self._name, args, err)
                )
...
class ParameterizingProxyString(six.text_type):
    ...
    def __new__(cls, fmt_pair, normal="", name="<not specified>"):
        # pylint: disable = missing-return-doc, missing-return-type-doc
        """
...
class FormattingString(six.text_type):
    ...
    def __new__(cls, sequence, normal=""):
        # pylint: disable = missing-return-doc, missing-return-type-doc
        """
        ...
        for idx, ucs_part in enumerate(args):
            if not isinstance(ucs_part, six.string_types):
                ...
                raise TypeError(
                    ...
                    "got %s" % (ucs_part, idx, expected_types, type(ucs_part).__name__)
                )
        postfix = ""
        if self and self._normal:
            postfix = self._normal
            _refresh = self._normal + self
            args = [_refresh.join(ucs_part.split(self._normal)) for ucs_part in args]
...
class NullCallableString(six.text_type):
    ...
    def __new__(cls):
        """Class constructor."""
        return six.text_type.__new__(cls, "")
...
def get_proxy_string(term, attr):
    ...
    _proxy_table = {  # pragma: no cover
        "screen": {
            ...
            "hpa": ParameterizingProxyString(("\x1b[{0}G", lambda *arg: (arg[0] + 1,)), term.normal, attr),
            "vpa": ParameterizingProxyString(("\x1b[{0}d", lambda *arg: (arg[0] + 1,)), term.normal, attr),
        },
        "ansi": {
            ...
            "civis": ParameterizingProxyString(("\x1b[?25l", lambda *arg: ()), term.normal, attr),
            "cnorm": ParameterizingProxyString(("\x1b[?25h", lambda *arg: ()), term.normal, attr),
            "hpa": ParameterizingProxyString(("\x1b[{0}G", lambda *arg: (arg[0] + 1,)), term.normal, attr),
            "vpa": ParameterizingProxyString(("\x1b[{0}d", lambda *arg: (arg[0] + 1,)), term.normal, attr),
            "sc": "\x1b[s",
            "rc": "\x1b[u",
...
def resolve_color(term, color):
    ...
    vga_color_cap = term._background_color if "on_" in color else term._foreground_color
    ...
    assert base_color in X11_COLORNAMES_TO_RGB, ("color not known", base_color)
...
def resolve_attribute(term, attr):
    ...
    if not tparm_capseq:
        ...
        proxy = get_proxy_string(term, term._sugar.get(attr, attr))  # pylint: disable=protected-access
