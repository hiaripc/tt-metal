...
if platform.system() == "Windows":
    ...
    try:
        ...
        _MSG_NOSUPPORT = (
            ...
            "unless a deriving class overrides them: {tty_methods}.".format(
                platform=platform.system(), tty_methods=", ".join(_TTY_METHODS)
            )
        )
...
class Terminal(object):
    ...
    def __init__(self, kind=None, stream=None, force_styling=False):
        ...
        global _CUR_TERM
        self.errors = ["parameters: kind=%r, stream=%r, force_styling=%r" % (kind, stream, force_styling)]
    ...
    def __init__capabilities(self):
        ...
        self._caps_compiled_any = re.compile(
            "|".join(cap.named_pattern for name, cap in self.caps.items()) + "|(?P<MISMATCH>.)"
        )
        self._caps_unnamed_any = re.compile(
            "|".join("({0})".format(cap.pattern) for name, cap in self.caps.items()) + "|(.)"
        )
    ...
    def _height_and_width(self):
        ...
        return WINSZ(
            ws_row=int(os.getenv("LINES", "25")), ws_col=int(os.getenv("COLUMNS", "80")), ws_xpixel=None, ws_ypixel=None
        )
    ...
    def get_location(self, timeout=None):
        ...
        match = self._query_response(self.u7 or "\x1b[6n", self.caps["cursor_report"].re_compiled, timeout)
    ...
    def get_fgcolor(self, timeout=None):
        ...
        match = self._query_response(
            "\x1b]10;?\x07", re.compile("\x1b]10;rgb:([0-9a-fA-F]+)/([0-9a-fA-F]+)/([0-9a-fA-F]+)\x07"), timeout
        )
    ...
    def get_bgcolor(self, timeout=None):
        ...
        """
        match = self._query_response(
            "\x1b]11;?\x07", re.compile("\x1b]11;rgb:([0-9a-fA-F]+)/([0-9a-fA-F]+)/([0-9a-fA-F]+)\x07"), timeout
        )
    ...
    def link(self, url, text, url_id=""):
        ...
        if not self.does_styling:
            return text
        return "\x1b]8;{0};{1}\x1b\\{2}" "\x1b]8;;\x1b\\".format(params, url, text)
    ...
    def ljust(self, text, width=None, fillchar=" "):
        """
    ...
    def rjust(self, text, width=None, fillchar=" "):
        """
    ...
    def center(self, text, width=None, fillchar=" "):
        """
        ...
        *canonical input* mode, it allows the tty driver to provide
        ...
        if HAS_TTY and self._keyboard_fd is not None:
            ...
            try:
                ...
                # Restore prior mode:
                termios.tcsetattr(self._keyboard_fd, termios.TCSAFLUSH, save_mode)
