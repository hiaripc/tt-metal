...
def sign_with_shared_key(req: Request, key: str) -> str:
    ...
    signature = base64.b64encode(hmac.digest(base64.b64decode(key), string_to_sign.encode("utf8"), "sha256")).decode(
        "utf8"
    )
...
def _can_access_container(
    ...
    if resp.status == 404 and auth[0] == ANONYMOUS:
        out_failures.append(
            RequestFailure.create_from_request_response("Could not access container", build_req(), resp)
        )
        return False
...
def _get_access_token(conf: Config, key: Any) -> Tuple[Any, float]:
    ...
    if azure_auth == "sakey":
        ...
        if resp.status == 400:
            if (
                (result["error"] == "invalid_grant" and "AADSTS700082" in result["error_description"])
                or (result["error"] == "interaction_required" and "AADSTS50078" in result["error_description"])
                or (result["error"] == "interaction_required" and "AADSTS50076" in result["error_description"])
            ):
                raise Error("Your refresh token is no longer valid, please run `az login` to get a new one")
            else:
                raise Error(
        ...
        def build_req() -> Request:
            return _create_access_token_request(creds=creds, scope="https://storage.azure.com/.default")
    ...
    # This enables the use of Managed Identity, Workload Identity, and other auth methods not implemented here
        ...
        try:
            ...
            raise RuntimeError("When setting AZURE_USE_IDENTITY=1, you must also install the azure-identity package")
    ...
    if _can_access_container(conf, account, container, anonymous_auth, out_failures=access_failures):
...
def execute_api_request(conf: Config, req: Request) -> "urllib3.BaseHTTPResponse":
    ...
    def build_req() -> Request:
        return create_api_request(req, auth=access_token_manager.get_token(conf, key=(account, container)))
...
def isdir(conf: Config, path: str) -> bool:
    ...
    if blob == "":
        ...
        it = create_page_iterator(
            ...
            url=build_url(account, "/{container}", container=container),
            method="GET",
            params=dict(comp="list", restype="container", prefix=blob, delimiter="/", maxresults="1"),
        )
        for result in it:
...
class StreamingWriteFile(BaseStreamingWriteFile):
    ...
    def _upload_chunk(self, chunk: memoryview, finalize: bool) -> None:
        ...
        if finalize:
            block_ids = [_block_index_to_block_id(i, self._upload_id) for i in range(self._block_index)]
            _finalize_blob(
                conf=self._conf,
                path=self._path,
                url=self._url,
...
def parallel_upload(
    ...
    while start < s.st_size:
        ...
        future = executor.submit(_upload_chunk, conf, src, start, min(part_size, s.st_size - start), dst_url, block_id)
...
def maybe_update_md5(conf: Config, path: str, etag: str, hexdigest: str) -> bool:
    ...
    headers["x-ms-blob-content-md5"] = base64.b64encode(binascii.unhexlify(hexdigest)).decode("utf8")
...
def _get_entries(conf: Config, account: str, container: str, result: Mapping[str, Any]) -> Iterator[DirEntry]:
    blobs = result["Blobs"]
    if blobs is None:
        return
...
def remote_copy(conf: Config, src: str, dst: str, return_md5: bool) -> Optional[str]:
    ...
    def build_req() -> Request:
        ...
        req = Request(
            url=build_url(dst_account, "/{container}/{blob}", container=dst_container, blob=dst_blob),
            method="PUT",
