    ...
    for attempt, backoff in enumerate(exponential_sleep_generator()):
        ...
        try:
            resp = conf.get_http_pool().request(
                ...
                timeout=urllib3.Timeout(connect=conf.connect_timeout, read=conf.read_timeout, total=total_timeout),
            ...
            if deadline is not None:
                if resp.headers.get("Transfer-Encoding") == "chunked":
                    ...
                    try:
                        ...
                        resp = urllib3.response.HTTPResponse(status=resp.status, headers=resp.headers, body=body)
                    except Timeout as e:
                        resp.close()
                        raise DeadlineExceeded.create_from_request_response(
