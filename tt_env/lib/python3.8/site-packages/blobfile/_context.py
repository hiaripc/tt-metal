...
class Context:
    ...
    def copy(
        ...
        if not overwrite:
            if self.exists(dst):
                raise FileExistsError(f"Destination '{dst}' already exists and overwrite is disabled")
    ...
    def scanglob(self, pattern: str, parallel: bool = False, shard_prefix_length: int = 0) -> Iterator[DirEntry]:
        if "?" in pattern or "[" in pattern or "]" in pattern:
            raise Error("Advanced glob queries are not supported")
        ...
        if _is_local_path(pattern):
            for filepath in _local_glob(pattern):
                ...
                yield DirEntry(
                    ...
                    stat=None
                    if is_dir
                    else Stat(size=s.st_size, mtime=s.st_mtime, ctime=s.st_ctime, md5=None, version=None),
                )
            ...
            if shard_prefix_length == 0:
                ...
                assert parallel, "You probably want to use parallel=True if you are setting shard_prefix_length > 0"
                initial_tasks = []
                valid_chars = [i for i in range(256) if i not in INVALID_CHARS.union([ord("/"), ord("*")])]
            ...
            if parallel:
                ...
                tasks_done = 0
                with mp_ctx.Pool(initializer=_glob_worker, initargs=(self._conf, root, tasks, results)):
    ...
    def scandir(self, path: RemoteOrLocalPath, shard_prefix_length: int = 0) -> Iterator[DirEntry]:
        ...
        if _is_local_path(path):
            ...
            if shard_prefix_length == 0:
                ...
                tasks_done = 0
                with mp_ctx.Pool(initializer=_sharded_listdir_worker, initargs=(self._conf, prefixes, items)):
                    while tasks_done < tasks_enqueued:
                        entry = items.get()
                        if entry is None:
                            tasks_done += 1
            ...
            if _is_gcp_path(path):
                ...
                def request_generator():
                    ...
                    for entry in azure.list_blobs(self._conf, path):
                        ...
                        req = Request(
                            url=azure.build_url(account, "/{container}/{blob}", container=container, blob=entry_blob),
                            method="DELETE",
                            # 404 is allowed in case a failed request successfully deleted the file
                            # before erroring out
                    ...
                    if cache_dir is None:
                        ...
                        with filelock.FileLock(lock_path):
                            ...
                            if remote_hash is None:
                                ...
                                expected_local_path = self.join(cache_dir, remote_hash, local_filename)
                                perform_copy = not self.exists(expected_local_path)

                            if perform_copy:
                                local_hexdigest = self.copy(remote_path, tmp_path, overwrite=True, return_md5=True)
                                ...
                                if remote_hash is None:
                                    if _is_azure_path(path):
                                        azure.maybe_update_md5(self._conf, path, remote_version, local_hexdigest)
                                    elif _is_gcp_path(path):
                                        gcp.maybe_update_md5(self._conf, path, remote_version, local_hexdigest)
                            else:
...
def _is_azure_path(path: str) -> bool:
    url = urllib.parse.urlparse(path)
    return (url.scheme == "https" and url.netloc.endswith(".blob.core.windows.net")) or url.scheme == "az"
...
def _download_chunk(conf: Config, src: str, dst: str, start: int, size: int, src_file_size: int) -> None:
    ctx = Context(conf)
    ...
    def close(self) -> None:
        ...
        try:
            if self._remote_path is not None and self._mode in ("w", "wb", "a", "ab"):
                self._ctx.copy(self._local_path, self._remote_path, overwrite=True, dst_version=self._version)
        finally:
