...
def show_session(
    ...
    controller: BrowserLike | None = None,
) -> None:
...
class ClientSession:
    ...
    def __init__(
        self,
        session_id: ID | None = None,
        websocket_url: str = DEFAULT_SERVER_WEBSOCKET_URL,
        io_loop: IOLoop | None = None,
        arguments: dict[str, str] | None = None,
        max_message_size: int = 20 * 1024 * 1024,
    ):
    ...
    def check_connection_errors(self) -> None:
        ...
        if not self.connected:
            if self.error_reason is ErrorReason.HTTP_ERROR:
                ...
                raise OSError(
                    f"We received an HTTP-Error. Disconnected with error code: {self.error_code}, given message: {self.error_detail}"
                )
    ...
    def show(
        self, obj: UIElement | None = None, browser: str | None = None, new: Literal["tab", "window"] = "tab"
    ) -> None:
