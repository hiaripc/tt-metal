...
class _ModelResolver:
    ...
    def add(self, cls: type[HasProps]) -> None:
        if not (issubclass(cls, Local) or cls.__name__.startswith("_")):
            ...
            if previous is not None and not hasattr(cls, "__implementation__"):
                raise Warning(
                    f"Duplicate qualified model declaration of '{cls.__qualified_model__}'. Previous definition: {previous}"
                )
    ...
    def clear_extensions(self) -> None:
        def is_extension(obj: type[HasProps]) -> bool:
            return (
                getattr(obj, "__implementation__", None) is not None
                or getattr(obj, "__javascript__", None) is not None
                or getattr(obj, "__css__", None) is not None
            )
...
class HasProps(Serializable, metaclass=MetaHasProps):
    ...
    def _raise_attribute_error_with_matches(self, name: str, properties: Iterable[str]) -> NoReturn:
        ...
        raise AttributeError(
            f"unexpected attribute {name!r} to {self.__class__.__name__}, {text} attributes are {nice_join(matches)}"
        )
    ...
    def properties_with_values(
        self, *, include_defaults: bool = True, include_undefined: bool = False
    ) -> dict[str, Any]:
        ...
        return self.query_properties_with_values(
            lambda prop: prop.serialized, include_defaults=include_defaults, include_undefined=include_undefined
        )
    ...
    def query_properties_with_values(
        self,
        query: Callable[[PropertyDescriptor[Any]], bool],
        *,
