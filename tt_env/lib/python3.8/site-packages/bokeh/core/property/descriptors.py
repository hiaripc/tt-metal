...
class PropertyDescriptor(Generic[T]):
    ...
    def __set__(self, obj: HasProps, value: T, *, setter: Setter | None = None) -> None:
        ...
        if not hasattr(obj, "_property_values"):
            ...
            raise RuntimeError(
                f"Cannot set a property value {self.name!r} on a {class_name} instance before HasProps.__init__"
            )
    ...
    def has_unstable_default(self, obj: HasProps) -> bool:
        ...
        return self.property._may_have_unstable_default() or self.is_unstable(
            obj.__overridden_defaults__.get(self.name, None)
        )
    ...
    def _get(self, obj: HasProps) -> T:
        ...
        if not hasattr(obj, "_property_values"):
            class_name = obj.__class__.__name__
            raise RuntimeError(
                f"Cannot get a property value {self.name!r} from a {class_name} instance before HasProps.__init__"
            )
