...
class NonNegative(SingleParameterizedProperty[T]):
    ...
    def __init__(
        self, type_param: TypeOrInst[Property[T]], *, default: Init[T] = Intrinsic, help: str | None = None
    ) -> None:
    ...
    def validate(self, value: Any, detail: bool = True) -> None:
        ...
        if not (self.type_param.is_valid(value) and value >= self.start and value <= self.end):
            msg = (
                ""
                if not detail
                else f"expected a value of type {self.type_param} in range [{self.start}, {self.end}], got {value!r}"
            )
