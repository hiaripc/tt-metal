...
MISSING_GLYPH = Error(1002, "MISSING_GLYPH", "Glyph renderer has no glyph set")
NO_SOURCE_FOR_GLYPH = Error(1003, "NO_SOURCE_FOR_GLYPH", "Glyph renderer has no data source")
REQUIRED_RANGE = Error(1004, "REQUIRED_RANGE", "A required Range object is missing")
MISSING_GOOGLE_API_KEY = Error(1005, "MISSING_GOOGLE_API_KEY", "Google now requires API keys for all Google Maps usage")
...
REQUIRED_SCALE = Error(1008, "REQUIRED_SCALE", "A required Scale object is missing")
...
MALFORMED_GRAPH_SOURCE = Error(1011, "MALFORMED_GRAPH_SOURCE", "The GraphSource is incorrectly configured")
...
BAD_EXTRA_RANGE_NAME = Error(
    1020, "BAD_EXTRA_RANGE_NAME", "An extra range name is configured with a name that does not correspond to any range"
)
EQUAL_SLIDER_START_END = Error(1021, "EQUAL_SLIDER_START_END", "Slider 'start' and 'end' cannot be equal")
MIN_PREFERRED_MAX_WIDTH = Error(1022, "MIN_PREFERRED_MAX_WIDTH", "Expected min_width <= width <= max_width")
MIN_PREFERRED_MAX_HEIGHT = Error(1023, "MIN_PREFERRED_MAX_HEIGHT", "Expected min_height <= height <= max_height")
...
REPEATED_LAYOUT_CHILD = Error(1027, "REPEATED_LAYOUT_CHILD", "The same model can't be used multiple times in a layout")
NOT_A_PROPERTY_OF = Error(1028, "NOT_A_PROPERTY_OF", "Invalid property name")
INVALID_PROPERTY_VALUE = Error(1029, "INVALID_PROPERTY_VALUE", "Invalid property value")
EXT = Error(9999, "EXT", "Custom extension reports error")
