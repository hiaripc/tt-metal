...
def get_screenshot_as_png(
    ...
    *,
    ...
    return Image.open(io.BytesIO(png)).convert("RGBA").crop((0, 0, width * dpr, height * dpr)).resize((width, height))
...
def get_layout_html(
    ...
    *,
    ...
    def html() -> str:
        return file_html(
            obj, resources, title="", template=template, theme=theme, suppress_callback_warning=True, _always_new=True
        )
    ...
    if width is not None or height is not None:
        ...
        if not isinstance(obj, Plot):
            warn(
                "Export method called with width or height argument on a non-Plot model. The size values will be ignored."
            )
        else:
