...
def layout(
    *args: UIElement, children: list[UIElement] | None = None, sizing_mode: SizingModeType | None = None, **kwargs: Any
) -> Column:
...
@overload
def grid(
    children: list[UIElement | list[UIElement | list[Any]]], *, sizing_mode: SizingModeType | None = ...
) -> GridBox:
    ...
...
@overload
def grid(
    children: list[UIElement | None], *, sizing_mode: SizingModeType | None = ..., nrows: int, ncols: int
) -> GridBox:
    ...
...
def grid(
    children: Any = [], sizing_mode: SizingModeType | None = None, nrows: int | None = None, ncols: int | None = None
) -> GridBox:
    """
...
def group_tools(
    tools: list[Tool | ToolProxy], *, merge: MergeFn[Tool] | None = None, ignore: set[str] | None = None
) -> list[Tool | ToolProxy]:
...
def _handle_child_sizing(children: list[UIElement], sizing_mode: SizingModeType | None, *, widget: str) -> None:
    for item in children:
        ...
        if not isinstance(item, LayoutDOM):
            raise ValueError(
                f"Only LayoutDOM items can be inserted into a {widget}. Tried to insert: {item} of type {type(item)}"
            )
        if sizing_mode is not None and _has_auto_sizing(item):
            item.sizing_mode = sizing_mode
...
def _create_grid(
    iterable: Iterable[UIElement | list[UIElement]], sizing_mode: SizingModeType | None, layer: int = 0, **kwargs
) -> Row | Column:
