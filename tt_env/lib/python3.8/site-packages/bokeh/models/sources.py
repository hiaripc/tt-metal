...
class DataSource(Model):
    ...
    selected = Readonly(
        Instance(Selection),
        default=InstanceDefault(Selection),
        help="""
...
class ColumnarDataSource(DataSource):
    ...
    selection_policy = Instance(
        SelectionPolicy,
        default=InstanceDefault(UnionRenderers),
        help="""
...
class ColumnDataSource(ColumnarDataSource):
    ...
    * A Pandas ``GroupBy`` object
    ...
    """,
        ...
        .accepts(Object("pandas.DataFrame"), lambda x: ColumnDataSource._data_from_df(x))
        .accepts(Object("pandas.core.groupby.GroupBy"), lambda x: ColumnDataSource._data_from_groupby(x))
        .asserts(
            lambda _, data: len({len(x) for x in data.values()}) <= 1,
    ...
    def _stream(
        self, new_data: DataDict | pd.Series | pd.DataFrame, rollover: int | None = None, setter: Setter | None = None
    ) -> None:
        ...
        if newkeys != oldkeys:
            ...
            if missing and extra:
                raise ValueError(
                    f"Must stream updates to all existing columns (missing: {', '.join(missing)}, extra: {', '.join(extra)})"
                )
            elif missing:
        ...
        for key, values in new_data.items():
            ...
            if (
                ...
                and values.dtype.kind.lower() == "m"
                and isinstance(old_values, np.ndarray)
                and old_values.dtype.kind.lower() != "m"
            ):
...
class CDSView(Model):
    ...
    filter = Instance(
        Filter,
        default=InstanceDefault(AllIndices),
        help="""
...
def _check_slice(s: slice) -> None:
    ...
    if (
        (s.start is not None and s.start < 0)
