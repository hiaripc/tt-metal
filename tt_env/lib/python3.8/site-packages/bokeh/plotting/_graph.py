...
__all__ = "get_graph_kwargs"

RENDERER_ARGS = ["name", "level", "visible", "x_range_name", "y_range_name", "selection_policy", "inspection_policy"]
...
def get_graph_kwargs(node_source, edge_source, **kwargs):
    ...
    mnode_visuals = pop_visuals(
        marker_type, kwargs, prefix="node_muted_", defaults=node_visuals, override_defaults={"alpha": 0.2}
    )
    ...
    medge_visuals = pop_visuals(
        MultiLine, kwargs, prefix="edge_muted_", defaults=edge_visuals, override_defaults={"alpha": 0.2}
    )
    ...
    ## node stuff
    node_kwargs = {
        k.lstrip("node_"): v for k, v in kwargs.copy().items() if k.lstrip("node_") in marker_type.properties()
    }
