...
def create_renderer(glyphclass, plot, **kwargs):
    ...
    nonselection_visuals = pop_visuals(
        glyphclass, kwargs, prefix="nonselection_", defaults=glyph_visuals, override_defaults={"alpha": 0.1}
    )
    ...
    muted_visuals = pop_visuals(
        glyphclass, kwargs, prefix="muted_", defaults=glyph_visuals, override_defaults={"alpha": 0.2}
    )
        ...
        if isinstance(val, np.ndarray):
            if isinstance(dataspecs[var], ColorSpec):
                if val.dtype == "uint32" and val.ndim == 1:  # 0xRRGGBBAA
                    ...
                    (val.dtype == "uint8" or val.dtype.kind == "f") and val.ndim == 2 and val.shape[1] in (3, 4)
                ):  # RGB/RGBA
                    pass
                else:
                    raise RuntimeError(
                        "Color columns need to be of type uint32[N], uint8[N] or uint8/float[N, {3, 4}]"
