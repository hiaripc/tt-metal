...
class BokehTornado(TornadoApplication):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if mem_log_frequency_milliseconds < 0:
            ...
            if psutil is None:
                log.warning(
                    "Memory logging requested, but is disabled. Optional dependency 'psutil' is missing. "
        ...
        super().__init__(
            all_patterns,  # type: ignore[arg-type] # TODO: this may be another bug in mypy (not sure; but looks suspicious)
    ...
    def new_connection(
        self, protocol: Protocol, socket: WSHandler, application_context: ApplicationContext, session: ServerSession
    ) -> ServerConnection:
...
def create_static_handler(
    prefix: str, key: str, app: Application
) -> tuple[str, type[StaticFileHandler | StaticHandler], dict[str, Any]]:
    route = prefix
