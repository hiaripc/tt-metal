...
class BokehOptionsDirective(BokehDirective):
    ...
    def run(self):
        ...
        try:
            ...
            raise SphinxError(
                f"Unable to generate options reference docs for {options_name}, couldn't import module {module_name}"
            )

        options = getattr(module, options_name, None)
        if options is None:
            raise SphinxError(
                f"Unable to generate options reference docs: no options {options_name} in module {module_name}"
            )
