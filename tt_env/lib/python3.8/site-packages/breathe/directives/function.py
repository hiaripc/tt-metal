...
class DoxygenFunctionDirective(BaseDirective):
    ...
    def run(self) -> List[Node]:
        ...
        finder_filter = self.filter_factory.create_function_and_all_friend_finder_filter(namespace, function_name)
        ...
        try:
            ...
            return warning.warn('doxygenfunction: Cannot find function "{namespace}{function}" ' "{tail}")
    ...
    def _parse_args(self, function_description: str) -> Optional[cpp.ASTParametersQualifiers]:
        ...
        parser = cpp.DefinitionParser(function_description, location=self.get_source_info(), config=self.config)
    ...
    def _create_function_signature(
        ...
        try:
            ...
            return format_parser_error("doxygenclass", e.error, e.filename, self.state, self.lineno, False)
    ...
    def _resolve_function(self, matches, args: Optional[cpp.ASTParametersQualifiers], project_info):
        ...
        for entry in matches:
            ...
            target_handler = create_target_handler({"no-link": ""}, project_info, self.state.document)
