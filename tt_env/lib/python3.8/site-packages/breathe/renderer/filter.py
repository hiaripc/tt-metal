...
class FilterFactory:
    ...
    def _create_description_filter(self, allow: bool, level: str, options: Dict[str, Any]) -> Filter:
    ...
    def _create_public_members_filter(self, options: Dict[str, Any]) -> Filter:
        ...
        if "members" in options:
            if options["members"].strip():
                ...
                # Accept any nodes which don't have a "sectiondef" as a parent or, if they do, only
                # accept them if their names are in the members list
                public_members_filter = (parent_is_sectiondef & node_name_is_in_members) | ~parent_is_sectiondef
            else:
    ...
    def _create_non_public_members_filter(self, prot: str, option_name: str, options: Dict[str, Any]) -> Filter:
    ...
    def create_class_member_filter(self, options: Dict[str, Any]) -> Filter:
        ...
        protected_members = self._create_non_public_members_filter("protected", "protected-members", options)

        private_members = self._create_non_public_members_filter("private", "private-members", options)
        ...
        filter_ = AndFilter(
            ...
            NotFilter(
                ...
                AndFilter(
                    ...
                    NotFilter(InFilter(LambdaAccessor(Node(), lambda x: x.content_[0].getValue()), valid_names)),
                )
            ),
            NotFilter(
            ...
            NotFilter(
                ...
                AndFilter(
                    ...
                    NotFilter(FilePathFilter(LambdaAccessor(Node(), lambda x: x.location), filename)),
                )
            ),
            NotFilter(
    ...
    def create_content_filter(self, kind: str, options: Dict[str, Any]) -> Filter:
        ...
        public_innerclass = parent_is_compounddef & parent_is_class & node_is_innerclass & node_is_public
    ...
    def create_member_finder_filter(self, namespace: str, name: str, kind: str) -> Filter:
        ...
        if namespace:
            ...
            is_implementation_file = parent.name.endswith(self.app.config.breathe_implementation_filename_extensions)
