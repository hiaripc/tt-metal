...
def get_param_decl(param):
    ...
    if not param_name:
        ...
        param_decl, number_of_subs = re.subn(r"(\((?:\w+::)*[*&]+)(\))", r"\g<1>" + param_name + r"\g<2>", param_type)
        if number_of_subs == 0:
...
class SphinxRenderer:
    ...
    def run_directive(
        ...
        if config.breathe_debug_trace_directives:
            ...
            print("{}Running directive: .. {}:: {}".format("  " * _debug_indent, directive.name, declaration))
            _debug_indent += 1
    ...
    def get_qualification(self) -> List[str]:
        ...
        if config.breathe_debug_trace_qualification:
            ...
            global _debug_indent
            print("{}{}".format(_debug_indent * "  ", debug_print_node(self.qualification_stack[0])))
            _debug_indent += 1
    ...
    def detaileddescription(self, node) -> List[Node]:
        ...
        for candNode in detailedCand:
            ...
            for para in candNode.traverse(nodes.paragraph):
                if para.parent and len(para.parent) == 1 and isinstance(para.parent, nodes.paragraph):
        ...
        if len(fieldLists) != 0 and sphinx.version_info[0:2] < (4, 3):
            ...
            if len(retvals) != 0:
                ...
                fRetvals = nodes.field("", nodes.field_name("", "returns"), nodes.field_body("", body))
    ...
    def visit_compounddef(self, node) -> List[Node]:
        ...
        if "members-only" not in options:
            if "allow-dot-graphs" in options:
                ...
                addnode("inheritancegraph", lambda: self.render_optional(node.get_inheritancegraph()))
                addnode(
                    "collaborationgraph",
            ...
            def render_derivedcompoundref(node):
                ...
                if not output:
                    return []
                return [nodes.paragraph("", "", nodes.Text("Subclassed by "), *intersperse(output, nodes.Text(", ")))]
    ...
    def visit_docformula(self, node) -> List[Node]:
        ...
        for item in node.content_:
            ...
            if latex.startswith("$") and latex.endswith("$"):
                latex = latex[1:-1]
                nodelist.append(nodes.math(text=latex, label=None, nowrap=False, docname=docname, number=None))
            # Else we're multiline
            else:
                if latex.startswith("\\[") and latex.endswith("\\]"):
                    latex = latex[2:-2:]

                nodelist.append(nodes.math_block(text=latex, label=None, nowrap=False, docname=docname, number=None))
        return nodelist
    ...
    def visit_enum(self, node) -> List[Node]:
        ...
        if (not dom or dom == "cpp") and node.strong == "yes":
            ...
            obj_type = "enum"
        return self.handle_declaration(node, declaration, obj_type=obj_type, content_callback=content)
    ...
    def visit_templateparam(self, node: compound.paramTypeSub, *, insertDeclNameByParsing: bool = False) -> List[Node]:
        nodelist: List[Node] = []
    ...
    def visit_docparamlist(self, node) -> List[Node]:
        ...
        for item in node.parameteritem:
            ...
            if len(bodyNodes) == 1:
                ...
                bodyNodes = [nodes.paragraph("", "", *(parameterDirectionNodes + bodyNodes[0].children))]
    ...
    def visit_docdotfile(self, node) -> List[Node]:
        ...
        if not os.path.isabs(dot_file_path):
            ...
            if os.path.isabs(project_path):
                ...
                dot_file_path = os.path.abspath(self.app.confdir + os.sep + project_path + os.sep + dot_file_path)
        try:
    ...
    def dispatch_memberdef(self, node) -> List[Node]:
        ...
        if node.kind in ("function", "signal", "slot") or (node.kind == "friend" and node.argsstring):
    ...
    def render_string(self, node: str) -> List[Node]:
        ...
        if stripped:
            ...
            if delimiter:
                ...
                return [
                    nodes.paragraph("", "", nodes.Text(line.strip())) for line in node.split(delimiter) if line.strip()
                ]
