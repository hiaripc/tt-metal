...
class BeautifulSoup(Tag):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if builder is None:
            ...
            if (
                ...
                and not (
                    ...
                    or (isinstance(original_features, str) and original_features in builder.ALTERNATE_NAMES)
                )
                and markup
            ):
        ...
        ) in self.builder.prepare_markup(markup, from_encoding, exclude_encodings=exclude_encodings):
            self.reset()
    ...
    def _markup_is_url(cls, markup: _RawMarkup) -> bool:
        ...
        problem: bool = False
        if isinstance(markup, bytes):
            problem = any(markup.startswith(prefix) for prefix in (b"http:", b"https:")) and b" " not in markup
        elif isinstance(markup, str):
            problem = any(markup.startswith(prefix) for prefix in ("http:", "https:")) and " " not in markup
        else:
            return False
    ...
    def string_container(self, base_class: Optional[Type[NavigableString]] = None) -> Type[NavigableString]:
    ...
    def decode(
        ...
        **kwargs: Any,
        ...
        if warning:
            ...
            indent_level = None
        return prefix + super(BeautifulSoup, self).decode(indent_level, eventual_encoding, formatter, iterator)
