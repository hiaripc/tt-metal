...
class MatchRule(object):
    ...
    def __init__(
        ...
        if len(values) == 0:
            raise ValueError("Either string, pattern, function, present, or exclude_everything must be provided.")
        if len(values) > 1:
            raise ValueError(
    ...
    def __init__(
        ...
        **kwargs: _StrainableAttribute,
        ...
        if name is None and not attrs and not string and not kwargs:
            ...
            self.name_rules = cast(List[TagNameMatchRule], list(self._make_match_rules(name, TagNameMatchRule)))
        ...
        self.string_rules = cast(List[StringMatchRule], list(self._make_match_rules(string, StringMatchRule)))
    ...
    def matches_tag(self, tag: Tag) -> bool:
        ...
        if self.name_rules:
            ...
            for rule in self.name_rules:
                ...
                if rule.matches_tag(tag) or (prefixed_name is not None and rule.matches_string(prefixed_name)):
                    name_matches = True
                    break
