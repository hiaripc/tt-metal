...
class TestMatchRule(SoupTest):
    def _tuple(self, rule: MatchRule) -> Tuple[Optional[str], Optional[str], Optional[Callable], Optional[bool]]:
        return (
            rule.string,
...
class TestSoupStrainer(SoupTest):
    ...
    def test_constructor_class_attribute(self):
        ...
        assert [] == strainer.name_rules
        assert [] == strainer.string_rules
        assert {"class_": [AttributeValueMatchRule(string="mainbody")]} == (strainer.attribute_rules)
    ...
    def test_one_name_rule_must_match(self):
        ...
        assert self.tag_matches(SoupStrainer(name=[re.compile("c-f"), re.compile("[ab]$")]), **kwargs)
    ...
    def test_parse_only_combining_tag_and_string(self, soupstrainer):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert str(warning.message).startswith("The given value for parse_only will exclude everything:")
...
"""
        ...
        assert '<a class="sister" href="http://example.com/lacie" id="link2">Lacie</a>' == id_soup.decode()
