...
class Parser(object):
        ...
        try:
            ...
            for decl in iterator:
                ...
                if isinstance(decl, pycparser.c_ast.Decl):
                    ...
                    quals = 0
                    if isinstance(decl.type.type, pycparser.c_ast.IdentifierType) and decl.type.type.names[
                        -1
                    ].startswith("__dotdotdot"):
                        ...
                        realtype, quals = self._get_type_and_quals(
                            decl.type, name=decl.name, partial_length_ok=True, typedef_example="*(%s *)0" % (decl.name,)
                        )
    ...
    def _declare(self, name, obj, included=False, quals=0):
        if name in self._declarations:
            ...
            if not self._options.get("override"):
                raise FFIError(
                    "multiple declarations of %s (for interactive usage, " "try cdef(xx, override=True))" % (name,)
                )
    ...
    def _parse_function_type(self, typenode, funcname=None):
        ...
        if ellipsis:
            ...
            if not params:
                raise CDefError(
                    "%s: a function with only '(...)' as argument" " is not correct C" % (funcname or "in expression")
                )
