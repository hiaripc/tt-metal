...
def main():
    ...
    p.add_argument("--diffstat", action="store_true", help="print a diffstat instead of applying the changes")
    p.add_argument(
        "--extensions",
    ...
    p.add_argument(
        "--staged", "--cached", action="store_true", help="format lines in the stage instead of the working dir"
    )
    ...
    changed_lines = compute_diff_and_extract_lines(commits, files, opts.staged, opts.diff_from_common_commit)
    if opts.verbose >= 1:
    ...
    if opts.verbose >= 1:
        ...
        if ignored_files:
            print("Ignoring the following files (wrong extension, symlink, or " "ignored by clang-format):")
    ...
    if len(commits) > 1:
        ...
        revision = None
    new_tree = run_clang_format_and_save_to_tree(changed_lines, revision, binary=opts.binary, style=opts.style)
    if opts.verbose >= 1:
...
def create_tree_from_index(filenames):
    ...
    def index_contents_generator():
        for filename in filenames:
            ...
            git_ls_files = subprocess.Popen(git_ls_files_cmd, env=env, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        for filename, line_ranges in iteritems(changed_lines):
            if revision is not None:
                ...
                git_metadata = subprocess.Popen(
                    git_metadata_cmd, env=env, stdin=subprocess.PIPE, stdout=subprocess.PIPE
                )
            ...
            blob_id = clang_format_to_blob(
                filename, line_ranges, revision=revision, binary=binary, style=style, env=env
            )
...
def clang_format_to_blob(filename, line_ranges, revision=None, binary="clang-format", style=None, env=None):
    ...
    if revision is not None:
        ...
        git_show = subprocess.Popen(git_show_cmd, env=env, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
...
def print_diff(old_tree, new_tree):
    ...
    # filter.
    return subprocess.run(["git", "diff", "--diff-filter=M", "--exit-code", old_tree, new_tree]).returncode
...
def print_diffstat(old_tree, new_tree):
    ...
    # that were modified, so unmodified files would show as deleted without the
    # filter.
    return subprocess.run(["git", "diff", "--diff-filter=M", "--exit-code", "--stat", old_tree, new_tree]).returncode
