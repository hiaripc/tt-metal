...
function(arrow_install_cmake_package PACKAGE_NAME EXPORT_NAME)
    ...
    write_basic_package_version_file("${BUILT_CONFIG_VERSION_CMAKE}" COMPATIBILITY SameMajorVersion)
    install(
        FILES
            "${BUILT_CONFIG_CMAKE}"
...
function(ADD_ARROW_LIB LIB_NAME)
    ...
    if(WIN32 OR CMAKE_GENERATOR STREQUAL Xcode OR NOT ARROW_POSITION_INDEPENDENT_CODE)
    ...
    if(BUILD_STATIC)
        ...
        if(ARG_STATIC_LINK_LIBS)
            target_link_libraries(${LIB_NAME}_static PUBLIC "$<BUILD_INTERFACE:${ARG_STATIC_LINK_LIBS}>")
            if(USE_OBJLIB)
                # Ensure that dependencies are built before compilation of objects in
                # object library, rather than only before the final link step
...
function(ARROW_ADD_PKG_CONFIG MODULE)
    configure_file(${MODULE}.pc.in "${CMAKE_CURRENT_BINARY_DIR}/${MODULE}.pc.generate.in" @ONLY)
    file(
        GENERATE
        OUTPUT
        "${CMAKE_CURRENT_BINARY_DIR}/$<CONFIG>/${MODULE}.pc"
        INPUT "${CMAKE_CURRENT_BINARY_DIR}/${MODULE}.pc.generate.in"
    )
    install(FILES "${CMAKE_CURRENT_BINARY_DIR}/$<CONFIG>/${MODULE}.pc" DESTINATION "${CMAKE_INSTALL_LIBDIR}/pkgconfig/")
endfunction()
