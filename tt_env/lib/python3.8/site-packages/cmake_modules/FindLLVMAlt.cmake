...
if(DEFINED LLVM_ROOT)
    ...
    foreach(ARROW_LLVM_VERSION ${ARROW_LLVM_VERSIONS})
        find_package(LLVM ${ARROW_LLVM_VERSION} CONFIG NO_DEFAULT_PATH HINTS ${LLVM_ROOT})
        if(LLVM_FOUND)
            break()
        endif()
    endforeach()
...
if(NOT LLVM_FOUND)
    foreach(ARROW_LLVM_VERSION ${ARROW_LLVM_VERSIONS})
        ...
        if(APPLE)
            ...
            if(BREW)
                string(REGEX REPLACE "^([0-9]+)(\\..+)?" "\\1" ARROW_LLVM_VERSION_MAJOR "${ARROW_LLVM_VERSION}")
                execute_process(
                    COMMAND
                        ${BREW} --prefix "llvm@${ARROW_LLVM_VERSION_MAJOR}"
...
if(LLVM_FOUND)
    ...
    if(ARROW_LLVM_USE_SHARED)
        ...
        if(TARGET LLVMSupport AND NOT ARROW_ZSTD_USE_SHARED)
            get_target_property(LLVM_SUPPORT_INTERFACE_LINK_LIBRARIES LLVMSupport INTERFACE_LINK_LIBRARIES)
            list(FIND LLVM_SUPPORT_INTERFACE_LINK_LIBRARIES zstd::libzstd_shared LLVM_SUPPORT_LIBZSTD_INDEX)
            if(NOT LLVM_SUPPORT_LIBZSTD_INDEX EQUAL -1)
                list(REMOVE_AT LLVM_SUPPORT_INTERFACE_LINK_LIBRARIES ${LLVM_SUPPORT_LIBZSTD_INDEX})
                list(INSERT LLVM_SUPPORT_INTERFACE_LINK_LIBRARIES ${LLVM_SUPPORT_LIBZSTD_INDEX} zstd::libzstd_static)
            endif()
            set_target_properties(
                LLVMSupport
                PROPERTIES
