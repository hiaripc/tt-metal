...
if(RAPIDJSON_INCLUDE_DIR)
    ...
    string(
        REGEX
        MATCH
        "#define RAPIDJSON_MAJOR_VERSION ([0-9]+)"
    ...
    string(
        REGEX
        MATCH
        "#define RAPIDJSON_MINOR_VERSION ([0-9]+)"
    ...
    string(
        REGEX
        MATCH
        "#define RAPIDJSON_PATCH_VERSION ([0-9]+)"
        RAPIDJSON_PATCH_VERSION_DEFINITION
        "${RAPIDJSON_H_CONTENT}"
    )
    string(REGEX REPLACE "^.+ ([0-9]+)$" "\\1" RAPIDJSON_PATCH_VERSION "${RAPIDJSON_PATCH_VERSION_DEFINITION}")
    if(
        "${RAPIDJSON_MAJOR_VERSION}"
            STREQUAL
            ""
        OR "${RAPIDJSON_MINOR_VERSION}"
            STREQUAL
            ""
        OR "${RAPIDJSON_PATCH_VERSION}"
            STREQUAL
            ""
    )
...
find_package_handle_standard_args(RapidJSONAlt REQUIRED_VARS RAPIDJSON_INCLUDE_DIR VERSION_VAR RAPIDJSON_VERSION)
...
if(RapidJSONAlt_FOUND)
    if(WIN32 AND "${RAPIDJSON_INCLUDE_DIR}" MATCHES "^/")
        ...
        execute_process(
            COMMAND
                "cygpath" "--windows" "${RAPIDJSON_INCLUDE_DIR}"
