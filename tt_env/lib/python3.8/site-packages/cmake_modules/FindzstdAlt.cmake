...
if(ARROW_ZSTD_USE_SHARED)
    ...
    if(CMAKE_IMPORT_LIBRARY_SUFFIX)
        list(APPEND ZSTD_LIB_NAMES "${CMAKE_IMPORT_LIBRARY_PREFIX}${ZSTD_LIB_NAME_BASE}${CMAKE_IMPORT_LIBRARY_SUFFIX}")
    endif()
    list(APPEND ZSTD_LIB_NAMES "${CMAKE_SHARED_LIBRARY_PREFIX}${ZSTD_LIB_NAME_BASE}${CMAKE_SHARED_LIBRARY_SUFFIX}")
else()
...
if("${zstdAlt_VERSION}" STREQUAL "" AND ZSTD_INCLUDE_DIR)
    ...
    string(REGEX MATCH "#define ZSTD_VERSION_MAJOR +([0-9]+)" ZSTD_VERSION_MAJOR_DEFINITION "${ZSTD_H_CONTENT}")
    string(REGEX REPLACE "^.+ ([0-9]+)$" "\\1" ZSTD_VERSION_MAJOR "${ZSTD_VERSION_MAJOR_DEFINITION}")
    string(REGEX MATCH "#define ZSTD_VERSION_MINOR +([0-9]+)" ZSTD_VERSION_MINOR_DEFINITION "${ZSTD_H_CONTENT}")
    string(REGEX REPLACE "^.+ ([0-9]+)$" "\\1" ZSTD_VERSION_MINOR "${ZSTD_VERSION_MINOR_DEFINITION}")
    string(REGEX MATCH "#define ZSTD_VERSION_RELEASE +([0-9]+)" ZSTD_VERSION_RELEASE_DEFINITION "${ZSTD_H_CONTENT}")
    string(REGEX REPLACE "^.+ ([0-9]+)$" "\\1" ZSTD_VERSION_RELEASE "${ZSTD_VERSION_RELEASE_DEFINITION}")
    if(
        "${ZSTD_VERSION_MAJOR}"
            STREQUAL
            ""
        OR "${ZSTD_VERSION_MINOR}"
            STREQUAL
            ""
        OR "${ZSTD_VERSION_RELEASE}"
            STREQUAL
            ""
    )
        set(zstdAlt_VERSION "0.0.0")
    else()
        set(zstdAlt_VERSION "${ZSTD_VERSION_MAJOR}.${ZSTD_VERSION_MINOR}.${ZSTD_VERSION_RELEASE}")
    endif()
endif()
