...
if(ARROW_CPU_FLAG STREQUAL "x86")
    ...
    if(MSVC)
        ...
        set(ARROW_AVX512_FLAG "${ARROW_AVX512_FLAG} -mavx512f -mavx512cd -mavx512vl -mavx512dq -mavx512bw")
    ...
    if(CXX_SUPPORTS_SSE4_2 AND ARROW_RUNTIME_SIMD_LEVEL MATCHES "^(SSE4_2|AVX2|AVX512|MAX)$")
...
set(UNKNOWN_COMPILER_MESSAGE "Unknown compiler: ${CMAKE_CXX_COMPILER_ID} ${CMAKE_CXX_COMPILER_VERSION}")
...
elseif(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    ...
    if(CMAKE_CXX_COMPILER_VERSION VERSION_EQUAL "13.0" OR CMAKE_CXX_COMPILER_VERSION VERSION_GREATER "13.0")
...
function(GET_GOLD_VERSION)
    ...
    execute_process(
        COMMAND
            ${CMAKE_CXX_COMPILER} "-Wl,--version" ${ARGN}
        ERROR_QUIET
    ...
    if(LINKER_OUTPUT MATCHES "GNU gold")
        ...
        if(NOT CMAKE_MATCH_1)
            message(
                SEND_ERROR
                "Could not extract GNU gold version. "
...
if(NOT WIN32 AND NOT APPLE)
    ...
    elseif(ARROW_USE_LD_GOLD)
        ...
        if("${GOLD_LOCATION}" MATCHES "^/opt/rh/devtoolset")
            message(
                STATUS
                "Skipping optional gold linker (version ${GOLD_VERSION}) because "
                "it's in devtoolset"
            )
            set(GOLD_VERSION)
        endif()
        ...
        if(MUST_USE_GOLD)
            ...
            if(ARROW_BUGGY_GOLD)
                if("${ARROW_LINK}" STREQUAL "d" AND "${CMAKE_BUILD_TYPE}" STREQUAL "RELEASE")
                    message(
                        SEND_ERROR
                        "Configured to use buggy gold with dynamic linking "
                        "in a RELEASE build"
                    )
                endif()
            endif()
    ...
    if(ARROW_USE_MOLD)
        ...
        if(LD_MOLD)
            ...
            elseif(CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
                ...
                message(
                    STATUS
                    "Using the default linker because compiler doesn't support mold: ${CMAKE_CXX_COMPILER_ID}"
                )
            endif()
...
if(NOT MSVC)
    ...
    string(APPEND CMAKE_C_FLAGS_RELWITHDEBINFO "${C_RELWITHDEBINFO_FLAGS} ${ARROW_C_FLAGS_RELWITHDEBINFO}")
    string(APPEND CMAKE_CXX_FLAGS_RELWITHDEBINFO "${CXX_RELWITHDEBINFO_FLAGS} ${ARROW_CXX_FLAGS_RELWITHDEBINFO}")
endif()
...
if(CMAKE_SYSTEM_NAME STREQUAL "Emscripten")
    ...
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fPIC -fexceptions -Wno-error=deprecated-literal-operator")
