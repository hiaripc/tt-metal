...
set(ARROW_RE2_LINKAGE "static" CACHE STRING "How to link the re2 library. static|shared (default static)")
...
function(provide_cmake_module MODULE_NAME ARROW_CMAKE_PACKAGE_NAME)
    ...
    if(EXISTS "${module}")
        ...
        install(FILES "${module}" DESTINATION "${ARROW_CMAKE_DIR}/${ARROW_CMAKE_PACKAGE_NAME}")
    endif()
endfunction()
...
macro(resolve_dependency DEPENDENCY_NAME)
    ...
    if(${DEPENDENCY_NAME}_SOURCE STREQUAL "SYSTEM" AND ARG_IS_RUNTIME_DEPENDENCY)
        ...
        string(REGEX REPLACE "([A-Z])" "_\\1" ARG_ARROW_CMAKE_PACKAGE_NAME_SNAKE ${ARG_ARROW_CMAKE_PACKAGE_NAME})
        # _Arrow_Flight -> Arrow_Flight
        string(SUBSTRING ${ARG_ARROW_CMAKE_PACKAGE_NAME_SNAKE} 1 -1 ARG_ARROW_CMAKE_PACKAGE_NAME_SNAKE)
        # Arrow_Flight -> ARROW_FLIGHT
        string(TOUPPER ${ARG_ARROW_CMAKE_PACKAGE_NAME_SNAKE} ARG_ARROW_CMAKE_PACKAGE_NAME_UPPER_SNAKE)
        provide_find_module(${PACKAGE_NAME} ${ARG_ARROW_CMAKE_PACKAGE_NAME})
        list(APPEND ${ARG_ARROW_CMAKE_PACKAGE_NAME_UPPER_SNAKE}_SYSTEM_DEPENDENCIES ${PACKAGE_NAME})
        ...
        # arrow_flight -> ARROW_FLIGHT
        string(TOUPPER ${ARG_ARROW_PC_PACKAGE_NAME_SNAKE} ARG_ARROW_PC_PACKAGE_NAME_UPPER_SNAKE)
        if(ARROW_BUILD_STATIC)
            find_package(PkgConfig QUIET)
            foreach(ARG_PC_PACKAGE_NAME ${ARG_PC_PACKAGE_NAMES})
                pkg_check_modules(
                    ${ARG_PC_PACKAGE_NAME}_PC
                    ${ARG_PC_PACKAGE_NAME}
                    NO_CMAKE_PATH
                    NO_CMAKE_ENVIRONMENT_PATH
                    QUIET
                )
                set(RESOLVE_DEPENDENCY_PC_PACKAGE "pkg-config package for ${ARG_PC_PACKAGE_NAME} ")
                string(
                    APPEND
                    RESOLVE_DEPENDENCY_PC_PACKAGE
    ...
    if(
        NOT (
            (
                _VERSION_ENTRY
                    MATCHES
                    "^[^#][A-Za-z0-9-_]+_VERSION="
            )
            OR (
                _VERSION_ENTRY
                    MATCHES
                    "^[^#][A-Za-z0-9-_]+_CHECKSUM="
            )
        )
    )
        continue()
...
if(DEFINED ENV{ARROW_CARES_URL})
    ...
    string(REPLACE "." "_" ARROW_CARES_BUILD_VERSION_UNDERSCORES ${ARROW_CARES_BUILD_VERSION})
...
foreach(CONFIG DEBUG MINSIZEREL RELEASE RELWITHDEBINFO)
    ...
    string(
        APPEND
        EP_CXX_FLAGS_${CONFIG}
        " ${CMAKE_CXX_COMPILE_OPTIONS_MSVC_RUNTIME_LIBRARY_${EP_MSVC_RUNTIME_LIBRARY}}"
    )
    string(APPEND EP_C_FLAGS_${CONFIG} " ${CMAKE_C_COMPILE_OPTIONS_MSVC_RUNTIME_LIBRARY_${EP_MSVC_RUNTIME_LIBRARY}}")
endforeach()
if(MSVC_TOOLCHAIN)
...
if(CMAKE_CROSSCOMPILING_EMULATOR)
    string(REPLACE ";" ${EP_LIST_SEPARATOR} EP_CMAKE_CROSSCOMPILING_EMULATOR "${CMAKE_CROSSCOMPILING_EMULATOR}")
    list(APPEND EP_COMMON_CMAKE_ARGS -DCMAKE_CROSSCOMPILING_EMULATOR=${EP_CMAKE_CROSSCOMPILING_EMULATOR})
endif()
...
macro(build_boost)
    ...
    if(ARROW_BOOST_REQUIRE_LIBRARY)
        ...
        if(MSVC)
            ...
            set(BOOST_LIBRARY_SUFFIX "${BOOST_LIBRARY_SUFFIX}-x64-${ARROW_BOOST_BUILD_VERSION_NO_MICRO_UNDERSCORE}")
        else()
...
if(CMAKE_CXX_COMPILER_ID STREQUAL "Clang" AND CMAKE_CXX_COMPILER_VERSION VERSION_GREATER 15)
...
if(
    ...
    OR (
        ARROW_S3
        AND ARROW_BUILD_BENCHMARKS
    )
)
    set(ARROW_USE_BOOST TRUE)
    set(ARROW_BOOST_REQUIRE_LIBRARY TRUE)
elseif(ARROW_GANDIVA OR ARROW_WITH_THRIFT OR (NOT ARROW_USE_NATIVE_INT128))
    set(ARROW_USE_BOOST TRUE)
...
if(ARROW_WITH_SNAPPY)
    ...
    if(${Snappy_SOURCE} STREQUAL "SYSTEM" AND NOT snappy_PC_FOUND AND ARROW_BUILD_STATIC)
...
macro(build_brotli)
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Emscripten")
        ...
        add_custom_command(
            ...
            COMMAND
                ...
                ${BROTLI_BUILD_DIR}/${CMAKE_STATIC_LIBRARY_PREFIX}*${CMAKE_STATIC_LIBRARY_SUFFIX} ${BROTLI_LIB_DIR}
            COMMAND
                ${CMAKE_COMMAND} -E copy_directory ${BROTLI_BUILD_INCLUDE_DIR} ${BROTLI_INCLUDE_DIR}/brotli
        )
    endif()
    ...
    target_include_directories(Brotli::brotlicommon BEFORE INTERFACE "${BROTLI_INCLUDE_DIR}")
...
if(ARROW_BUILD_TESTS OR ARROW_BUILD_BENCHMARKS OR ARROW_BUILD_INTEGRATION OR ARROW_USE_GLOG OR ARROW_WITH_GRPC)
...
macro(build_thrift)
    ...
    if(MSVC)
        ...
        set(THRIFT_LIB "${THRIFT_PREFIX}/lib/${CMAKE_STATIC_LIBRARY_PREFIX}thrift${CMAKE_STATIC_LIBRARY_SUFFIX}")
    endif()

    if(BOOST_VENDORED)
        set(THRIFT_DEPENDENCIES
            ${THRIFT_DEPENDENCIES}
            boost_ep
        )
    endif()
...
macro(build_protobuf)
    ...
    target_include_directories(arrow::protobuf::libprotobuf BEFORE INTERFACE "${PROTOBUF_INCLUDE_DIR}")
    ...
    target_include_directories(arrow::protobuf::libprotoc BEFORE INTERFACE "${PROTOBUF_INCLUDE_DIR}")
    add_executable(arrow::protobuf::protoc IMPORTED)
    set_target_properties(
        arrow::protobuf::protoc
        PROPERTIES
            IMPORTED_LOCATION
                "${PROTOBUF_COMPILER}"
    )
...
if(ARROW_WITH_PROTOBUF)
    ...
    if(ARROW_ORC OR ARROW_SUBSTRAIT OR ARROW_WITH_OPENTELEMETRY)
    ...
    get_target_property(PROTOBUF_PROTOC_EXECUTABLE ${ARROW_PROTOBUF_PROTOC} IMPORTED_LOCATION_RELEASE)
    if(NOT PROTOBUF_PROTOC_EXECUTABLE)
        get_target_property(PROTOBUF_PROTOC_EXECUTABLE ${ARROW_PROTOBUF_PROTOC} IMPORTED_LOCATION)
    endif()
...
macro(build_substrait)
    ...
    foreach(SUBSTRAIT_PROTO ${SUBSTRAIT_PROTOS})
        ...
        add_custom_command(
            OUTPUT
                "${SUBSTRAIT_PROTO_GEN}.cc"
                "${SUBSTRAIT_PROTO_GEN}.h"
            COMMAND
                ${ARROW_PROTOBUF_PROTOC} "-I${SUBSTRAIT_LOCAL_DIR}/proto" "--cpp_out=${SUBSTRAIT_CPP_DIR}"
    ...
    foreach(ARROW_SUBSTRAIT_PROTO ${ARROW_SUBSTRAIT_PROTOS})
        ...
        add_custom_command(
            OUTPUT
                "${ARROW_SUBSTRAIT_PROTO_GEN}.cc"
                "${ARROW_SUBSTRAIT_PROTO_GEN}.h"
            COMMAND
                ${ARROW_PROTOBUF_PROTOC} "-I${SUBSTRAIT_LOCAL_DIR}/proto" "-I${ARROW_SUBSTRAIT_PROTOS_DIR}"
...
macro(build_jemalloc)
    ...
    set(JEMALLOC_CONFIGURE_COMMAND
        ./configure
        "AR=${CMAKE_AR}"
        "CC=${CMAKE_C_COMPILER}"
    )
    ...
    target_include_directories(jemalloc::jemalloc BEFORE INTERFACE "${JEMALLOC_INCLUDE_DIR}")
...
if(ARROW_MIMALLOC)
    ...
    target_include_directories(mimalloc::mimalloc BEFORE INTERFACE "${MIMALLOC_INCLUDE_DIR}")
...
function(build_gtest)
    ...
    if(MSVC)
        install(
            FILES
                $<TARGET_PDB_FILE:gmock>
                $<TARGET_PDB_FILE:gmock_main>
                $<TARGET_PDB_FILE:gtest>
                $<TARGET_PDB_FILE:gtest_main>
            DESTINATION "${CMAKE_INSTALL_BINDIR}"
            OPTIONAL
        )
    endif()
...
if(ARROW_TESTING)
    ...
    if(GTest_SOURCE STREQUAL "SYSTEM")
        ...
        if(gtest_PC_FOUND)
            ...
            string(APPEND ARROW_TESTING_PC_CFLAGS "$<TARGET_PROPERTY:GTest::gtest,INTERFACE_INCLUDE_DIRECTORIES>")
...
macro(build_benchmark)
    ...
    target_include_directories(benchmark::benchmark BEFORE INTERFACE "${GBENCHMARK_INCLUDE_DIR}")
    ...
    target_include_directories(benchmark::benchmark_main BEFORE INTERFACE "${GBENCHMARK_INCLUDE_DIR}")
...
macro(build_zlib)
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Emscripten")
        ...
        if(NOT EXISTS ${EMSCRIPTEN_SYSROOT}/lib/wasm32-emscripten/pic/libz.a)
            execute_process(
                COMMAND
                    embuilder --pic --force build zlib
            )
        endif()
...
macro(build_bzip2)
    ...
    ExternalProject_Add(
        ...
        BUILD_IN_SOURCE 1
        BUILD_COMMAND
            ${MAKE} libbz2.a ${MAKE_BUILD_ARGS} ${BZIP2_EXTRA_ARGS}
        INSTALL_COMMAND
            ${MAKE} install -j1 PREFIX=${BZIP2_PREFIX} ${BZIP2_EXTRA_ARGS}
...
if(ARROW_WITH_BZ2)
    ...
    if(${BZip2_SOURCE} STREQUAL "SYSTEM" AND NOT bzip2_PC_FOUND AND ARROW_BUILD_STATIC)
        get_target_property(BZIP2_TYPE BZip2::BZip2 TYPE)
        if(BZIP2_TYPE STREQUAL "INTERFACE_LIBRARY")
            # Conan
            string(APPEND ARROW_PC_LIBS_PRIVATE " $<TARGET_FILE:CONAN_LIB::bzip2_bz2_$<CONFIG>>")
        else()
...
macro(build_utf8proc)
    ...
    target_include_directories(utf8proc::utf8proc BEFORE INTERFACE "${UTF8PROC_PREFIX}/include")
...
if(ARROW_WITH_UTF8PROC)
    set(utf8proc_resolve_dependency_args
        utf8proc
        PC_PACKAGE_NAMES
        libutf8proc
    )
    if(NOT VCPKG_TOOLCHAIN)
        ...
        list(
            APPEND
            utf8proc_resolve_dependency_args
            REQUIRED_VERSION
            "2.2.0"
        )
    endif()
...
macro(ensure_absl)
    if(NOT absl_FOUND)
        if(${absl_SOURCE} STREQUAL "AUTO")
            ...
            # released.
            set(ARROW_ABSL_REQUIRED_LTS_VERSIONS
                20230125
                20220623
                20211102
            )
...
macro(build_grpc)
    ...
    set(GRPC_STATIC_LIBRARY_UPB "${GRPC_PREFIX}/lib/${CMAKE_STATIC_LIBRARY_PREFIX}upb${CMAKE_STATIC_LIBRARY_SUFFIX}")
    ...
    get_target_property(GRPC_PROTOBUF_INCLUDE_DIR ${ARROW_PROTOBUF_LIBPROTOBUF} INTERFACE_INCLUDE_DIRECTORIES)
    get_filename_component(GRPC_PB_ROOT "${GRPC_PROTOBUF_INCLUDE_DIR}" DIRECTORY)
    get_target_property(GRPC_Protobuf_PROTOC_LIBRARY ${ARROW_PROTOBUF_LIBPROTOC} IMPORTED_LOCATION)
    ...
    target_include_directories(gRPC::grpc++_reflection BEFORE INTERFACE "${GRPC_INCLUDE_DIR}")
    ...
    add_custom_command(
        ...
        COMMAND
            ${CMAKE_COMMAND} -E copy $<TARGET_FILE:gRPC::grpc++> ${GRPC_STATIC_LIBRARY_GRPCPP_FOR_AR}
        DEPENDS
            grpc_ep
    )
...
if(ARROW_WITH_GRPC)
    ...
    if(GRPC_VENDORED)
        ...
        if(ARROW_USE_ASAN)
            ...
            target_compile_definitions(gRPC::grpc_asan_suppressed INTERFACE "GRPC_ASAN_SUPPRESSED")
...
macro(build_nlohmann_json)
    ...
    ExternalProject_Add(
        ...
        CMAKE_ARGS
            ${NLOHMANN_JSON_CMAKE_ARGS}
        BUILD_BYPRODUCTS
            ${NLOHMANN_JSON_BUILD_BYPRODUCTS}
    )
    ...
    target_include_directories(nlohmann_json::nlohmann_json BEFORE INTERFACE "${NLOHMANN_JSON_INCLUDE_DIR}")
...
macro(build_google_cloud_cpp_storage)
    ...
    string(JOIN ${EP_LIST_SEPARATOR} GOOGLE_CLOUD_CPP_PREFIX_PATH ${GOOGLE_CLOUD_CPP_PREFIX_PATH_LIST})

    set(GOOGLE_CLOUD_CPP_INSTALL_PREFIX "${CMAKE_CURRENT_BINARY_DIR}/google_cloud_cpp_ep-install")
    ...
    ExternalProject_Add(
        ...
        URL
            ${google_cloud_cpp_storage_SOURCE_URL}
        URL_HASH "SHA256=${ARROW_GOOGLE_CLOUD_CPP_BUILD_SHA256_CHECKSUM}"
        PATCH_COMMAND
            ${GOOGLE_CLOUD_CPP_PATCH_COMMAND}
        CMAKE_ARGS
            ${GOOGLE_CLOUD_CPP_CMAKE_ARGS}
        BUILD_BYPRODUCTS
    ...
    target_include_directories(google-cloud-cpp::common BEFORE INTERFACE "${GOOGLE_CLOUD_CPP_INCLUDE_DIR}")
    ...
    target_include_directories(google-cloud-cpp::rest-internal BEFORE INTERFACE "${GOOGLE_CLOUD_CPP_INCLUDE_DIR}")
    target_link_libraries(
        google-cloud-cpp::rest-internal
        INTERFACE
            absl::span
    ...
    target_include_directories(google-cloud-cpp::storage BEFORE INTERFACE "${GOOGLE_CLOUD_CPP_INCLUDE_DIR}")
...
if(ARROW_WITH_GOOGLE_CLOUD_CPP)
    ...
    get_target_property(google_cloud_cpp_storage_INCLUDE_DIR google-cloud-cpp::storage INTERFACE_INCLUDE_DIRECTORIES)
...
function(build_orc)
    ...
    if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.29)
        ...
        set(LZ4_HOME ${LZ4_ROOT} CACHE STRING "" FORCE)
        set(LZ4_LIBRARY LZ4::lz4 CACHE STRING "" FORCE)

        set(ORC_PREFER_STATIC_PROTOBUF OFF CACHE BOOL "" FORCE)
        get_target_property(PROTOBUF_INCLUDE_DIR ${ARROW_PROTOBUF_LIBPROTOBUF} INTERFACE_INCLUDE_DIRECTORIES)
        get_filename_component(Protobuf_ROOT "${PROTOBUF_INCLUDE_DIR}" DIRECTORY)
        set(PROTOBUF_HOME ${Protobuf_ROOT} CACHE STRING "" FORCE)
        # ORC uses this.
        target_include_directories(${ARROW_PROTOBUF_LIBPROTOC} INTERFACE "${PROTOBUF_INCLUDE_DIR}")
        ...
        set(SNAPPY_HOME ${Snappy_ROOT} CACHE STRING "" FORCE)
        set(SNAPPY_LIBRARY ${Snappy_TARGET} CACHE STRING "" FORCE)
        ...
        set(ZLIB_HOME ${ZLIB_ROOT} CACHE STRING "" FORCE)
        ...
        set(ZSTD_HOME ${ZSTD_ROOT} CACHE STRING "" FORCE)
        set(ZSTD_LIBRARY ${ARROW_ZSTD_LIBZSTD})

        set(BUILD_CPP_TESTS OFF CACHE BOOL "" FORCE)
        set(BUILD_JAVA OFF CACHE BOOL "" FORCE)
        set(BUILD_LIBHDFSPP OFF CACHE BOOL "" FORCE)
        set(BUILD_TOOLS OFF CACHE BOOL "" FORCE)
        set(INSTALL_VENDORED_LIBS OFF CACHE BOOL "" FORCE)
        ...
        get_target_property(ORC_PROTOBUF_ROOT ${ARROW_PROTOBUF_LIBPROTOBUF} INTERFACE_INCLUDE_DIRECTORIES)
    ...
    set(ORC_VENDORED TRUE PARENT_SCOPE)
    set(ARROW_BUNDLED_STATIC_LIBS ${ARROW_BUNDLED_STATIC_LIBS} PARENT_SCOPE)
endfunction()
...
macro(build_opentelemetry)
    ...
    if(Protobuf_VERSION VERSION_GREATER_EQUAL 3.22)
        message(
            FATAL_ERROR
            "GH-36013: Can't use bundled OpenTelemetry with Protobuf 3.22 or later. "
    ...
    get_target_property(OPENTELEMETRY_PROTOBUF_INCLUDE_DIR ${ARROW_PROTOBUF_LIBPROTOBUF} INTERFACE_INCLUDE_DIRECTORIES)
    get_target_property(OPENTELEMETRY_PROTOBUF_LIBRARY ${ARROW_PROTOBUF_LIBPROTOBUF} IMPORTED_LOCATION)
    get_target_property(OPENTELEMETRY_PROTOC_EXECUTABLE ${ARROW_PROTOBUF_PROTOC} IMPORTED_LOCATION)
    list(
        APPEND
    ...
    ExternalProject_Add(
        ...
        URL
            ${OPENTELEMETRY_PROTO_SOURCE_URL}
        BUILD_COMMAND
            ""
        CONFIGURE_COMMAND
            ""
    ...
    string(JOIN "${EP_LIST_SEPARATOR}" OPENTELEMETRY_PREFIX_PATH ${OPENTELEMETRY_PREFIX_PATH_LIST})
    ...
    if(CMAKE_SYSTEM_PROCESSOR STREQUAL "s390x")
        ...
        ExternalProject_Add(
            ...
            CONFIGURE_COMMAND
                ${CMAKE_COMMAND} -E env ARCH=s390x ${CMAKE_COMMAND} -G ${CMAKE_GENERATOR} "<SOURCE_DIR><SOURCE_SUBDIR>"
                ${OPENTELEMETRY_CMAKE_ARGS}
            BUILD_COMMAND
                ${CMAKE_COMMAND} --build "<BINARY_DIR>" --target all
            INSTALL_COMMAND
                ${CMAKE_COMMAND} --build "<BINARY_DIR>" --target install
            URL
                ${OPENTELEMETRY_SOURCE_URL}
            BUILD_BYPRODUCTS
                ${OPENTELEMETRY_BUILD_BYPRODUCTS}
            EXCLUDE_FROM_ALL NOT
            ${ARROW_WITH_OPENTELEMETRY}
            DEPENDS
                ${_OPENTELEMETRY_DEPENDENCIES}
        )
    else()
        ExternalProject_Add(
            opentelemetry_ep
    ...
    ExternalProject_Add_Step(
        ...
        download_proto
        COMMAND
            ${CMAKE_COMMAND} -E copy_directory $<TARGET_PROPERTY:opentelemetry_proto_ep,_EP_SOURCE_DIR>/opentelemetry
...
if(ARROW_WITH_OPENTELEMETRY)
    ...
    get_target_property(OPENTELEMETRY_INCLUDE_DIR opentelemetry-cpp::api INTERFACE_INCLUDE_DIRECTORIES)
...
macro(build_awssdk)
    ...
    if(ARROW_OPENSSL_USE_SHARED)
        list(APPEND AWSSDK_COMMON_CMAKE_ARGS -Dcrypto_SHARED_LIBRARY=${OPENSSL_CRYPTO_LIBRARY})
    else()
        list(APPEND AWSSDK_COMMON_CMAKE_ARGS -Dcrypto_STATIC_LIBRARY=${OPENSSL_CRYPTO_LIBRARY})
    endif()
    ...
    foreach(_AWSSDK_LIB ${_AWSSDK_LIBS})
        ...
        target_include_directories(${_AWSSDK_TARGET_NAME} BEFORE INTERFACE "${AWSSDK_INCLUDE_DIR}")
        if(${_AWSSDK_LIB} STREQUAL "aws-lc")
            ...
            target_include_directories(${_AWSSDK_TARGET_NAME} BEFORE INTERFACE "${AWS_LC_INCLUDE_DIR}")
        endif()
    ...
    ExternalProject_Add(
        ...
        CMAKE_ARGS
            ${AWS_CHECKSUMS_CMAKE_ARGS}
        BUILD_BYPRODUCTS
            ${AWS_CHECKSUMS_STATIC_LIBRARY}
        DEPENDS
            aws_c_common_ep
    )
    ...
    ExternalProject_Add(
        ...
        CMAKE_ARGS
            ${AWSSDK_COMMON_CMAKE_ARGS}
        BUILD_BYPRODUCTS
            ${AWS_C_EVENT_STREAM_STATIC_LIBRARY}
        DEPENDS
            aws_checksums_ep
            aws_c_io_ep
    )
...
function(build_azure_sdk)
    if(CMAKE_VERSION VERSION_LESS 3.22)
        ...
        message(
            FATAL_ERROR
            "Building Azure SDK for C++ requires at least CMake 3.22. "
    ...
    set(AZURE_SDK_VENDORED TRUE PARENT_SCOPE)
    list(
        APPEND
        ARROW_BUNDLED_STATIC_LIBS
        Azure::azure-core
