...
class ComparisonTestCase(TestCase):
    ...
    def comparison(n_prod, n_con, n_msgs):
        ...
        time_ff_many = run_test(
            Queue, num_producers=n_prod, num_consumers=n_con, msgs_per_prod=n_msgs, consume_many=100
        )
        time_mp = run_test(
            multiprocessing.Queue, num_producers=n_prod, num_consumers=n_con, msgs_per_prod=n_msgs, consume_many=1
        )

        if time_ff > time_mp:
            log.warning(
                f"faster-fifo took longer than mp.Queue ({time_ff=} vs {time_mp=}) on configuration ({n_prod}, {n_con}, {n_msgs})"
            )
        if time_ff_many > time_mp:
            log.warning(
                f"faster-fifo get_many() took longer than mp.Queue ({time_ff_many=} vs {time_mp=}) on configuration ({n_prod}, {n_con}, {n_msgs})"
            )
