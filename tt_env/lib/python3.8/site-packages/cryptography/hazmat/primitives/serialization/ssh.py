...
class _SSHFormatRSA:
    ...
    def load_private(self, data: memoryview, pubfields) -> tuple[rsa.RSAPrivateKey, memoryview]:
...
class _SSHFormatDSA:
    ...
    def load_private(self, data: memoryview, pubfields) -> tuple[dsa.DSAPrivateKey, memoryview]:
...
class _SSHFormatECDSA:
    ...
    def load_private(self, data: memoryview, pubfields) -> tuple[ec.EllipticCurvePrivateKey, memoryview]:
...
def load_application(data) -> tuple[memoryview, memoryview]:
    ...
    if not application.tobytes().startswith(b"ssh:"):
        raise ValueError("U2F application string does not start with b'ssh:' " f"({application})")
...
def _load_ssh_public_identity(
    ...
    if with_cert:
        ...
        if (sig_type == _SSH_RSA and inner_sig_type not in [_SSH_RSA_SHA256, _SSH_RSA_SHA512, _SSH_RSA]) or (
            sig_type != _SSH_RSA and inner_sig_type != sig_type
        ):
