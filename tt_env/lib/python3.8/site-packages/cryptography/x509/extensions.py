...
class Extensions:
    ...
    def get_extension_for_class(self, extclass: type[ExtensionTypeVar]) -> Extension[ExtensionTypeVar]:
...
class AuthorityKeyIdentifier(ExtensionType):
    ...
    def __init__(
        ...
        if authority_cert_serial_number is not None and not isinstance(authority_cert_serial_number, int):
...
class AccessDescription:
    ...
    def __repr__(self) -> str:
        return f"<AccessDescription(access_method={self.access_method}, " f"access_location={self.access_location})>"
...
class BasicConstraints(ExtensionType):
    ...
    def __repr__(self) -> str:
        return f"<BasicConstraints(ca={self.ca}, " f"path_length={self.path_length})>"
...
class DistributionPoint:
    def __init__(
        ...
        if reasons and (not isinstance(reasons, frozenset) or not all(isinstance(x, ReasonFlags) for x in reasons)):
            raise TypeError("reasons must be None or frozenset of ReasonFlags")

        if reasons and (ReasonFlags.unspecified in reasons or ReasonFlags.remove_from_crl in reasons):
...
class PolicyConstraints(ExtensionType):
    ...
    def __init__(
        ...
        if require_explicit_policy is not None and not isinstance(require_explicit_policy, int):
            raise TypeError("require_explicit_policy must be a non-negative integer or " "None")

        if inhibit_policy_mapping is not None and not isinstance(inhibit_policy_mapping, int):
...
class UserNotice:
    ...
    def __repr__(self) -> str:
        return f"<UserNotice(notice_reference={self.notice_reference}, " f"explicit_text={self.explicit_text!r})>"
...
class NoticeReference:
    ...
    def __repr__(self) -> str:
        return f"<NoticeReference(organization={self.organization!r}, " f"notice_numbers={self.notice_numbers})>"
...
class NameConstraints(ExtensionType):
    ...
    def _validate_ip_name(self, tree: typing.Iterable[GeneralName]) -> None:
        if any(
            isinstance(name, IPAddress) and not isinstance(name.value, (ipaddress.IPv4Network, ipaddress.IPv6Network))
            for name in tree
        ):
    ...
    def _validate_dns_name(self, tree: typing.Iterable[GeneralName]) -> None:
        if any(isinstance(name, DNSName) and "*" in name.value for name in tree):
...
class Extension(typing.Generic[ExtensionTypeVar]):
    ...
    def __repr__(self) -> str:
        return f"<Extension(oid={self.oid}, critical={self.critical}, " f"value={self.value})>"
...
class GeneralNames:
    def __init__(self, general_names: typing.Iterable[GeneralName]) -> None:
        ...
        if not all(isinstance(x, GeneralName) for x in general_names):
            raise TypeError(
                "Every item in the general_names list must be an " "object conforming to the GeneralName interface"
            )
    ...
    def get_values_for_type(
        self,
        type: type[DNSName] | type[UniformResourceIdentifier] | type[RFC822Name],
    ) -> list[str]:
        ...
    ...
    @typing.overload
    def get_values_for_type(self, type: type[IPAddress]) -> list[_IPAddressTypes]:
        ...
    ...
    ) -> list[_IPAddressTypes] | list[str] | list[OtherName] | list[Name] | list[ObjectIdentifier]:
...
class PrecertificateSignedCertificateTimestamps(ExtensionType):
    ...
    def __init__(
        ...
        if not all(isinstance(sct, SignedCertificateTimestamp) for sct in signed_certificate_timestamps):
            raise TypeError(
                "Every item in the signed_certificate_timestamps list must be " "a SignedCertificateTimestamp"
            )
...
class IssuingDistributionPoint(ExtensionType):
    ...
    def __init__(
        ...
        if only_some_reasons and (
            ReasonFlags.unspecified in only_some_reasons or ReasonFlags.remove_from_crl in only_some_reasons
        ):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.only_contains_attribute_certs == other.only_contains_attribute_certs
        )
...
class MSCertificateTemplate(ExtensionType):
    ...
    def __init__(
        ...
        if (major_version is not None and not isinstance(major_version, int)) or (
...
class NamingAuthority:
    ...
    def __repr__(self) -> str:
        return f"<NamingAuthority(" f"id={self.id}, url={self.url}, text={self.text})>"

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, NamingAuthority):
            return NotImplemented

        return self.id == other.id and self.url == other.url and self.text == other.text
...
class ProfessionInfo:
    def __init__(
        ...
        if naming_authority is not None and not isinstance(naming_authority, NamingAuthority):
        ...
        if not all(isinstance(item, str) for item in profession_items):
            raise TypeError("Every item in the profession_items list must be a str")

        if profession_oids is not None:
            profession_oids = list(profession_oids)
            if not all(isinstance(oid, ObjectIdentifier) for oid in profession_oids):
                raise TypeError("Every item in the profession_oids list must be an " "ObjectIdentifier")

        if registration_number is not None and not isinstance(registration_number, str):
            raise TypeError("registration_number must be a str")

        if add_profession_info is not None and not isinstance(add_profession_info, bytes):
...
class Admission:
    def __init__(
        ...
        if admission_authority is not None and not isinstance(admission_authority, GeneralName):
        ...
        if not all(isinstance(info, ProfessionInfo) for info in profession_infos):
            raise TypeError("Every item in the profession_infos list must be a " "ProfessionInfo")
...
class Admissions(ExtensionType):
    ...
    def __init__(
        ...
        if not all(isinstance(admission, Admission) for admission in admissions):
            raise TypeError("Every item in the contents_of_admissions list must be an " "Admission")
    ...
    def __repr__(self) -> str:
        return f"<Admissions(authority={self._authority}, " f"admissions={self._admissions})>"

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, Admissions):
            return NotImplemented

        return self.authority == other.authority and self._admissions == other._admissions
...
class UnrecognizedExtension(ExtensionType):
    ...
    def __repr__(self) -> str:
        return f"<UnrecognizedExtension(oid={self.oid}, " f"value={self.value!r})>"
