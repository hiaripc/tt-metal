...
def concat(left: Cycler[K, V], right: Cycler[K, U]) -> Cycler[K, V | U]:
    ...
    if left.keys != right.keys:
        raise ValueError(
            "Keys do not match:\n"
            "\tIntersection: {both!r}\n"
            "\tDisjoint: {just_one!r}".format(both=left.keys & right.keys, just_one=left.keys ^ right.keys)
        )
...
class Cycler(Generic[K, V]):
    ...
    def __add__(self, other: Cycler[L, U]) -> Cycler[K | L, V | U]:
        ...
        return Cycler(cast(Cycler[Union[K, L], Union[V, U]], self), cast(Cycler[Union[K, L], Union[V, U]], other), zip)
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Cycler):
            return Cycler(
                cast(Cycler[Union[K, L], Union[V, U]], self), cast(Cycler[Union[K, L], Union[V, U]], other), product
            )
...
def cycler(*args, **kwargs):
    ...
    if len(args) == 1:
        ...
        raise TypeError(
            "Only a single Cycler can be accepted as the lone " "positional argument. Use keyword arguments instead."
        )
