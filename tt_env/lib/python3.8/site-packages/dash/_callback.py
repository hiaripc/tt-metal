...
def callback(
    ...
    **_kwargs,
    ...
    config_prevent_initial_callbacks = _kwargs.pop("config_prevent_initial_callbacks", False)
    ...
    if background:
        ...
        if progress_default:
            ...
            if not len(long_spec["progress"]) == len(long_spec["progressDefault"]):
                raise Exception("Progress and progress default needs to be of same length")
        ...
        def add_context(*args, **kwargs):
            ...
            if long is not None:
                ...
                if not cache_key:
                    ...
                    if progress_default:
                        data["progressDefault"] = {str(o): x for o, x in zip(progress_outputs, progress_default)}
                    return to_json(data)
                if progress_outputs:
                    ...
                    if progress:
                        response["progress"] = {str(x): progress[i] for i, x in enumerate(progress_outputs)}
                ...
                if not job_running and output_value is callback_manager.UNDEFINED:
                    ...
                    raise LongCallbackError(f"An error occurred inside a long callback: {error['msg']}\n{error['tb']}")
                ...
                if multi and isinstance(output_value, (list, tuple)):
                    output_value = [NoUpdate() if NoUpdate.is_no_update(r) else r for r in output_value]
