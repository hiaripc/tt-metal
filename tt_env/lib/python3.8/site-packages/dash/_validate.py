...
def validate_index(name, checks, index):
    ...
    if missing:
        ...
        raise exceptions.InvalidIndexException(f"Missing item{plural} {', '.join(missing)} in {name}.")
...
def validate_template(template):
    ...
    for name in variable_names:
        if not name.isidentifier() or iskeyword(name):
            raise Exception(f'`{name}` is not a valid Python variable name in `path_template`: "{template}".')
