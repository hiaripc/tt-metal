...
def _get_traceback(secret, error: Exception):
    ...
    if hasattr(tbtools, "DebugTraceback"):
        ...
        return tbtools.DebugTraceback(error, skip=skip).render_debugger_html(True, secret, True)
...
class Dash:
    ...
    def __init__(  # pylint: disable=too-many-statements
        ...
        **obsolete,
        ...
        self.config = AttributeDict(
            name=name,
            assets_folder=os.path.join(flask.helpers.get_root_path(name), assets_folder),
            assets_url_path=assets_url_path,
            assets_ignore=assets_ignore,
            assets_external_path=get_combined_config("assets_external_path", assets_external_path, ""),
            pages_folder=pages_folder_config(name, pages_folder, use_pages),
            eager_loading=eager_loading,
            include_assets_files=get_combined_config("include_assets_files", include_assets_files, True),
    ...
    def init_app(self, app=None, **kwargs):
        ...
        self.server.register_blueprint(
            flask.Blueprint(
                ...
                static_url_path=config.routes_pathname_prefix + self.config.assets_url_path.lstrip("/"),
            )
        )
        ...
        if config.compress:
            try:
                ...
                _flask_compress_version = parse_version(_get_distribution_version("flask_compress"))

                if not hasattr(self.server.config, "COMPRESS_ALGORITHM") and _flask_compress_version >= parse_version(
                    "1.6.0"
                ):
    ...
    def _add_url(self, name, view_func, methods=("GET",)):
        ...
        self.server.add_url_rule(full_name, view_func=view_func, endpoint=full_name, methods=list(methods))
    ...
    def _setup_routes(self):
        ...
        if jupyter_dash.active:
            self._add_url("_alive_" + jupyter_dash.alive_token, jupyter_dash.serve_alive)
    ...
    def layout(self, value):
        ...
        if self._layout_is_function and not self.validation_layout and not self.config.suppress_callback_exceptions:
            ...
            def simple_clone(c, children=None):
                ...
                props = {
                    ...
                    if hasattr(c, p) and (p == "id" or not sig or sig.parameters[p].default == c.REQUIRED)
                }
    ...
    def _config(self):
        ...
        if self.validation_layout and not self.config.suppress_callback_exceptions:
            ...
            if self._extra_components:
                validation_layout = html.Div(children=[validation_layout] + self._extra_components)
    ...
    def _generate_scripts_html(self):
        ...
        deps = [
            {key: value[mode] if isinstance(value, dict) else value for key, value in js_dist_dependency.items()}
            for js_dist_dependency in _dash_renderer._js_dist_dependencies
        ]
        dev = self._dev_tools.serve_dev_bundles
        srcs = (
            self._collect_and_register_resources(self.scripts._resources._filter_resources(deps, dev_bundles=dev))
            + self.config.external_scripts
            + self._collect_and_register_resources(
                self.scripts.get_all_scripts(dev_bundles=dev)
                + self.scripts._resources._filter_resources(_dash_renderer._js_dist, dev_bundles=dev)
                ...
                + self.scripts._resources._filter_resources(dash_table._js_dist, dev_bundles=dev)
            )
        )
        ...
        return "\n".join(
            [format_tag("script", src) if isinstance(src, dict) else f'<script src="{src}"></script>' for src in srcs]
    ...
    def _generate_meta(self):
        meta_tags = []
        has_ie_compat = any(x.get("http-equiv", "") == "X-UA-Compatible" for x in self.config.meta_tags)
        ...
        if not has_viewport:
            meta_tags.append({"name": "viewport", "content": "width=device-width, initial-scale=1"})
    ...
    def index(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        if self._favicon:
            favicon_mod_time = os.path.getmtime(os.path.join(self.config.assets_folder, self._favicon))
        ...
        tags = "\n      ".join(format_tag("meta", x, opened=True, sanitize=True) for x in metas)
    ...
    def dispatch(self):
        ...
        g.inputs_list = inputs = body.get("inputs", [])  # pylint: disable=assigning-non-slot
        g.states_list = state = body.get("state", [])  # pylint: disable=assigning-non-slot
        output = body["output"]
        outputs_list = body.get("outputs") or split_callback_id(output)
        g.outputs_list = outputs_list  # pylint: disable=assigning-non-slot

        g.input_values = input_values = inputs_to_dict(inputs)  # pylint: disable=assigning-non-slot
        ...
        response = g.dash_response = flask.Response(mimetype="application/json")  # pylint: disable=assigning-non-slot
        ...
        try:
            ...
            args_grouping = map_grouping(lambda ind: inputs_state[ind], inputs_state_indices)

            g.args_grouping = args_grouping  # pylint: disable=assigning-non-slot
            g.using_args_grouping = not isinstance(  # pylint: disable=assigning-non-slot
                inputs_state_indices, int
            ) and (inputs_state_indices != list(range(grouping_len(inputs_state_indices))))
            ...
            outputs_grouping = map_grouping(lambda ind: flat_outputs[ind], outputs_indices)
            g.outputs_grouping = outputs_grouping  # pylint: disable=assigning-non-slot
            g.using_outputs_grouping = not isinstance(  # pylint: disable=assigning-non-slot
                outputs_indices, int
        ...
        for k in list(_callback.GLOBAL_CALLBACK_MAP):
            if k in self.callback_map:
                raise DuplicateCallback(
                    f"The callback `{k}` provided with `dash.callback` was already " "assigned with `app.callback`."
                )
    ...
    def _serve_default_favicon():
        return flask.Response(pkgutil.get_data("dash", "favicon.ico"), content_type="image/x-icon")
    ...
    def get_relative_path(self, path):
        ...
        ```
        """
        return _get_paths.app_get_relative_path(self.config.requests_pathname_prefix, path)
            ...
            for i, package in enumerate(packages):
                if hasattr(package, "path") and "dash/dash" in os.path.dirname(package.path):
                    component_packages_dist[i : i + 1] = [
                        os.path.join(os.path.dirname(package.path), x) for x in ["dcc", "html", "dash_table"]
                    ]
