...
def get_jl_prop_types(type_object):
    ...
    return dict(
        ...
        union=lambda: "{}".format(
            " | ".join(
                "{}".format(get_jl_type(subType)) for subType in type_object["value"] if get_jl_type(subType) != ""
            )
        ),
        # React's PropTypes.arrayOf
        arrayOf=lambda: (
            "Array"
            + (" of {}s".format(get_jl_type(type_object["value"])) if get_jl_type(type_object["value"]) != "" else "")
        ),
        # React's PropTypes.objectOf
        objectOf=lambda: "Dict with Strings as keys and values of type {}".format(get_jl_type(type_object["value"])),
...
def generate_metadata_strings(resources, metatype):
    ...
    return [
        jl_resource_tuple_string.format(
            ...
            type=metatype,
            async_string=":{}".format(str(resource.get("async")).lower()) if "async" in resource.keys() else "nothing",
        )
...
def generate_class_string(name, props, description, project_shortname, prefix):
    ...
    docstring = (
        create_docstring_jl(component_name=name, props=filtered_props, description=description)
        .replace("\r\n", "\n")
        .replace("$", "\\$")
    )
    ...
    for item in prop_keys[:]:
        if item.endswith("-*") or item == "setProps":
            ...
            warnings.warn(
                (
                    'WARNING: prop "{}" in component "{}" is a Julia keyword' " - REMOVED FROM THE JULIA COMPONENT"
                ).format(item, name)
            )
