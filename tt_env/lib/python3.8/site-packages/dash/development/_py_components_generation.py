...
'''
    ...
    default_arglist = [
        (f"{p:s}=Component.REQUIRED" if props[p]["required"] else f"{p:s}=Component.UNDEFINED")
        for p in prop_keys
...
def create_docstring(component_name, props, description, prop_reorder_exceptions=None):
    ...
    props = (
        props
        if (prop_reorder_exceptions is not None and component_name in prop_reorder_exceptions)
        or (prop_reorder_exceptions is not None and "ALL" in prop_reorder_exceptions)
...
def map_js_to_py_types_prop_types(type_object, indent_num):
    ...
    return dict(
        ...
        enum=lambda: ("a value equal to: " + ", ".join(str(t["value"]) for t in type_object["value"])),
        # React's PropTypes.oneOfType
        union=lambda: " | ".join(
            js_to_py_type(subType) for subType in type_object["value"] if js_to_py_type(subType) != ""
        ),
        # React's PropTypes.arrayOf
        arrayOf=array_of,
        # React's PropTypes.objectOf
        objectOf=lambda: ("dict with strings as keys and values of type " + js_to_py_type(type_object["value"])),
...
def map_js_to_py_types_flow_types(type_object):
    ...
    return dict(
        ...
        # React's PropTypes.oneOfType
        union=lambda: " | ".join(
            js_to_py_type(subType) for subType in type_object["elements"] if js_to_py_type(subType) != ""
        ),
        # Flow's Array type
        Array=lambda: "list"
        + (
...
def js_to_py_type(type_object, is_flow_type=False, indent_num=0):
    ...
    js_to_py_types = (
        ...
        if is_flow_type
        else map_js_to_py_types_prop_types(type_object=type_object, indent_num=indent_num)
    )
