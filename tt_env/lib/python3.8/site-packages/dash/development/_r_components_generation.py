...
def generate_class_string(name, props, project_shortname, prefix):
    ...
    for item in prop_keys[:]:
        if item.endswith("-*") or item == "setProps":
            ...
            prop_keys.remove(item)
            warnings.warn(
                ('WARNING: prop "{}" in component "{}" is an R keyword' " - REMOVED FROM THE R COMPONENT").format(
                    item, name
                )
            )
    ...
    default_paramtext += ", ".join(
        "{0}={0}".format(p) if p != "children" else "{}=children".format(p) for p in prop_keys
    )
...
def write_help_file(name, props, description, prefix, rpkg_data):
    ...
    item_text += "\n\n".join(
        "\\item{{{}}}{{{}{}}}".format(p, print_r_type(props[p]["type"]), props[p]["description"]) for p in prop_keys
    )
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(
            help_string.format(
                funcname=funcname,
                name=name,
                default_argtext=textwrap.fill(default_argtext, width=60, break_long_words=False),
                item_text=item_text,
...
def get_r_prop_types(type_object):
    ...
    return dict(
        ...
        union=lambda: "{}".format(
            " | ".join(
                "{}".format(get_r_type(subType)) for subType in type_object["value"] if get_r_type(subType) != ""
            )
        ),
        # React's PropTypes.arrayOf
        arrayOf=lambda: (
            "list"
            + (" of {}s".format(get_r_type(type_object["value"])) if get_r_type(type_object["value"]) != "" else "")
        ),
        # React's PropTypes.objectOf
        objectOf=lambda: "list with named elements and values of type {}".format(get_r_type(type_object["value"])),
