...
class ComponentMeta(abc.ABCMeta):
    ...
    def __new__(mcs, name, bases, attributes):
        ...
        ComponentRegistry.children_props[attributes.get("_namespace", module)][name] = attributes.get("_children_props")
...
def _check_if_has_indexable_children(item):
    if not hasattr(item, "children") or (
        not isinstance(item.children, Component) and not isinstance(item.children, (tuple, MutableSequence))
    ):
        raise KeyError
...
class Component(metaclass=ComponentMeta):
                ...
                if isinstance(v, dict):
                    for id_key, id_val in v.items():
                        if not isinstance(id_key, str):
                            raise TypeError("dict id keys must be strings,\n" + f"found {id_key!r} in id {v!r}")
    ...
    def to_plotly_json(self):
        ...
        props = {p: getattr(self, p) for p in self._prop_names if hasattr(self, p)}  # pylint: disable=no-member
