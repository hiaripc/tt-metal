...
coloredlogs.install(fmt="%(asctime)s,%(msecs)03d %(levelname)s - %(message)s", datefmt="%H:%M:%S")
...
def bootstrap_components(components_source, concurrency, install_type):
    ...
    source_glob = (
        components_source if components_source != "all" else "dash-core-components|dash-html-components|dash-table"
    )
