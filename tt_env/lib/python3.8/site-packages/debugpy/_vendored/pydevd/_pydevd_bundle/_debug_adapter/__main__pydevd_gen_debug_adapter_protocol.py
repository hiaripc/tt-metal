...
def _get_noqa_for_var(prop_name):
    return (
        "  # noqa (assign to builtin)" if prop_name in ("type", "format", "id", "hex", "breakpoint", "filter") else ""
    )
...
def update_class_to_generate_to_json(class_to_generate):
    ...
    if translate_prop_names:
        ...
        for prop_name in translate_prop_names:
            ...
            to_dict_body.append(
                "            %(prop_name)s = self._translate_id_to_dap(%(prop_name)s)%(noqa)s" % namespace
            )
    ...
    if not translate_prop_names:
        ...
        for prop_name in translate_prop_names:
            ...
            update_dict_ids_from_dap_body.append(
                "        dct[%(prop_name)r] = cls._translate_id_from_dap(dct[%(prop_name)r])" % namespace
            )
    ...
    for prop_name, prop in prop_name_and_prop:
        ...
        if prop_name in required:
            if use_to_dict:
                to_dict_body.append(
                    "        %(prop_name)r: %(prop_name)s.to_dict(update_ids_to_dap=update_ids_to_dap)," % namespace
                )
            else:
    ...
    if not translate_prop_names:
        ...
        for prop_name in translate_prop_names:
            ...
            update_dict_ids_to_dap_body.append(
                "        dct[%(prop_name)r] = cls._translate_id_to_dap(dct[%(prop_name)r])" % namespace
            )
            ...
            if prop["type"].__class__ == Ref:
                ...
                if prop["type"] == "array":
                    ...
                    if ref is not None:
                        ref_array_cls_name = ref.split("/")[-1]
                        init_body.append(
                            "    if update_ids_from_dap and self.%(prop_name)s:" % dict(prop_name=prop_name)
                        )
...
def update_class_to_generate_props(class_to_generate):
    ...
    class_to_generate["props"] = (
        "    __props__ = %s"
        % _indent_lines(json.dumps(properties, indent=4, default=default).replace("true", "True")).strip()
    )
...
def update_class_to_generate_refs(class_to_generate):
    ...
    class_to_generate["refs"] = (
        "    __refs__ = %s"
        % _OrderedSet(key for (key, val) in properties.items() if val["type"].__class__ == Ref).set_repr()
    )
