...
class PyDevdAPI(object):
        ...
        if not translated_absolute_filename.startswith("<"):
            ...
            if (
                ...
                and py_db.apply_files_filter(
                    self._DummyFrame(translated_absolute_filename), translated_absolute_filename, False
                )
            ):
        ...
        if file_to_id_to_breakpoint is None:
            ...
            try:
                ...
                py_db.consolidate_breakpoints(
                    canonical_normalized_filename, id_to_pybreakpoint, file_to_line_to_breakpoints
                )
                if py_db.plugin is not None:
                    py_db.has_plugin_line_breaks = py_db.plugin.has_line_breaks(py_db)
                    py_db.plugin.after_breakpoints_consolidated(
        ...
        for _ in range(50):  # Try this at most 50 times before giving up.
            ...
            if not children_pids:
                ...
                for pid in children_pids:
                    self._call(
                        ["taskkill", "/F", "/PID", str(pid), "/T"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
                    )
    ...
    def setup_auto_reload_watcher(
        ...
        py_db.setup_auto_reload_watcher(
            enable_auto_reload, watch_dirs, poll_target_time, exclude_patterns, include_patterns
        )
