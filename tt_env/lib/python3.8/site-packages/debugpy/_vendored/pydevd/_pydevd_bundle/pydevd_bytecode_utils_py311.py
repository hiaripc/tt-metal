...
def _get_smart_step_into_targets(code):
    ...
    for instr in dis.Bytecode(code):
        if instr.opname == "LOAD_CONST":
            ...
            targets_root.append(
                Target("".join(lines), positions.lineno, instr.offset, [], positions.end_lineno, startcol, endcol)
            )
    ...
    for targets in children:
        for child_target in targets:
            ...
            for outer_target in targets_root:
                outer_pos = _Pos(
                    outer_target.lineno, outer_target.endlineno, outer_target.startcol, outer_target.endcol
                )
