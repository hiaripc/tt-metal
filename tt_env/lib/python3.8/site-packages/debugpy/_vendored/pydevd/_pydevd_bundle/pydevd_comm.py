...
def internal_change_variable(dbg, seq, thread_id, frame_id, scope, attr, value):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq,
            "Error changing variable attr:%s expression:%s traceback:%s" % (attr, value, get_exception_traceback_str()),
        )
        dbg.writer.add_command(cmd)
...
def internal_evaluate_expression_json(py_db, request, thread_id):
    ...
    with ctx:
                    ...
                    try:
                        ...
                        if context == "repl":
                            ...
                            _evaluate_response_return_exception(
                                py_db, request, eval_result.etype, eval_result.result, eval_result.tb
                            )
                            return
        ...
        if frame_tracker is None:
            ...
            _evaluate_response(
                py_db, request, result="", error_message="Thread id: %s is not current thread id." % (thread_id,)
            )
            return
                        ...
                        for filename_in_utf8, lineno, method_name, line_text, line_col_info in frames[-max_frames:]:
                            if line_col_info is not None:
                                ...
                                if line_col_info.end_lineno is not None and lineno < line_col_info.end_lineno:
                                    line_text = "\n".join(
                                        linecache.getlines(filename_in_utf8)[lineno : line_col_info.end_lineno + 1]
                                    )
