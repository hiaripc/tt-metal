            ...
            if frame.f_code.co_flags & 0xA0:  # 0xa0 ==  CO_GENERATOR = 0x20 | CO_COROUTINE = 0x80
                ...
                if event == "line":
                    ...
                    if frame.f_code.co_firstlineno == frame.f_lineno:  # Check line to deal with async/await.
                        function_breakpoint_on_call_event = py_db.function_breakpoint_name_to_breakpoint.get(
                ...
                if info.pydev_state == 1:  # STATE_RUN = 1
                    ...
                    if step_cmd == -1:
                        ...
                        if py_db.apply_files_filter(frame, frame.f_code.co_filename, True) and (
                            ...
                            or py_db.apply_files_filter(frame.f_back, frame.f_back.f_code.co_filename, True)
                        ):
                            can_skip = True
            ...
            try:
                ...
                if py_db.show_return_values:
                    if is_return and (
                        ...
                        or (
                            info.pydev_step_cmd in (CMD_STEP_RETURN, CMD_STEP_RETURN_MY_CODE)
                ...
                if should_skip:
                    ...
                    if plugin_manager is not None:
                        result = plugin_manager.cmd_step_into(
                            py_db, frame, event, self._args[2], self._args[3], stop_info, stop
                        )
                        if result:
                    ...
                    for exc_break, is_user_uncaught in check_excs:
                        ...
                        if py_db.exclude_exception_by_filter(exc_break, trace):
                            pydev_log.debug(
                                ...
                                % (exception, frame.f_code.co_filename, frame.f_code.co_name)
                            )
                            should_stop = False

                        elif exc_break.condition is not None and not py_db.handle_breakpoint_condition(
    ...
    try:
        ...
        if py_db.ignore_exceptions_thrown_in_lines_with_ignore_exception:
            for check_trace_obj in (initial_trace_obj, trace_obj):
                ...
                from_user_input = py_db.filename_to_lines_where_exceptions_are_ignored.get(
                ...
                if exc_lineno not in merged:  # Note: check on merged but update lines_ignored.
                    try:
                        ...
                        pydev_log.exception(
                            "Error in linecache.getline(%r, %s, f_globals)", absolute_filename, exc_lineno
                        )
                        line = ""
