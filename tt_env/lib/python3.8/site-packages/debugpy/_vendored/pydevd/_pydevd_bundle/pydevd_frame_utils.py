...
def short_tb(exc_tb):
    ...
    while exc_tb:
        traceback.append(
            "{%r, %r, %r}" % (exc_tb.tb_frame.f_code.co_filename, exc_tb.tb_frame.f_code.co_name, exc_tb.tb_lineno)
        )
...
def create_frames_list_from_traceback(trace_obj, frame, exc_type, exc_desc, exception_type=None):
    ...
    for tb_frame, tb_lineno, line_col_info in reversed(lst):
        if frames_list is None and (
            (frame is tb_frame) or (frame is None) or (exception_type == EXCEPTION_TYPE_USER_UNHANDLED)
        ):
            frames_list = FramesList()
