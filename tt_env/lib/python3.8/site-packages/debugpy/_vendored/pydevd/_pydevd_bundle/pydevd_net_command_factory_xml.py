...
class NetCommandFactory(object):
    ...
    def make_custom_frame_created_message(self, frame_id, frame_description):
        ...
        return NetCommand(
            CMD_THREAD_CREATE, 0, '<xml><thread name="%s" id="%s"/></xml>' % (frame_description, frame_id)
        )
    ...
    def make_get_thread_stack_message(
        self, py_db, seq, thread_id, topmost_frame, fmt, must_be_suspended=False, start_frame=0, levels=0
    ):
        """
                        ...
                        if frame.f_code.co_name == "do_wait_suspend" and frame.f_code.co_filename.endswith("pydevd.py"):
                            ...
                            if arg is not None:
                                (
                                    ...
                                    _thread_suspend_str,
                                    thread_stack_str,
                                ) = self._make_send_curr_exception_trace_str(py_db, thread_id, *arg)
