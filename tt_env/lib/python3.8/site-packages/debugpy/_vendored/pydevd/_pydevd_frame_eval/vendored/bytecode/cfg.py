...
def _compute_stack_size(block, size, maxsize, *, check_pre_and_post=True):
    ...
    for instr in block:
        ...
        if instr.has_jump():
            effect = (
                instr.pre_and_post_stack_effect(jump=True) if check_pre_and_post else (instr.stack_effect(jump=True), 0)
            )
        ...
        effect = (
            instr.pre_and_post_stack_effect(jump=False) if check_pre_and_post else (instr.stack_effect(jump=False), 0)
        )
...
class ControlFlowGraph(_bytecode.BaseBytecode):
    ...
    def compute_stacksize(self, *, check_pre_and_post=True):
        ...
        if sys.version_info >= (3, 10) and self.flags & (
            CompilerFlags.GENERATOR | CompilerFlags.COROUTINE | CompilerFlags.ASYNC_GENERATOR
        ):
