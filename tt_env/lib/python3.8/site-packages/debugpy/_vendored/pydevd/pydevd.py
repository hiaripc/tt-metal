    ...
    def send_suspend_notification(self, thread_id, thread, stop_reason):
        ...
        if py_db is not None:
            py_db.writer.add_command(
                py_db.cmd_factory.make_thread_suspend_single_notification(py_db, thread_id, thread, stop_reason)
            )
...
class PyDB(object):
    ...
    def _internal_get_file_type(self, abs_real_path_and_basename):
        ...
        if basename.startswith(IGNORE_BASENAMES_STARTING_WITH) or abs_real_path_and_basename[0].startswith(
            IGNORE_BASENAMES_STARTING_WITH
        ):
                ...
                if force_check_project_scope:
                    ...
                    if DEBUG:
                        pydev_log.debug_once(
                            "File not traced (not in project - force_check_project_scope): %s", original_filename
                        )
    ...
    def add_break_on_exception(
        ...
        try:
            ...
            pydev_log.critical(
                "Error unable to add break on exception for: %s (exception could not be imported).", exception
            )
            return None
    ...
    def send_process_about_to_be_replaced(self):
        ...
        while not sent[0]:
            ...
            if (time.time() - initial_time) > timeout:
                pydev_log.critical(
                    "pydevd: Sending message related to process being replaced timed-out after %s seconds", timeout
                )
                break
            ...
            if PYDEVD_USE_SYS_MONITORING:
                if file_type is None:
                    if disable:
                        if DEBUG:
                            pydev_log.debug(
                                "Disable tracing of frame: %s - %s", frame.f_code.co_filename, frame.f_code.co_name
                            )
                        ...
                        if DEBUG:
                            pydev_log.debug(
                                "Set tracing of frame: %s - %s", frame.f_code.co_filename, frame.f_code.co_name
                            )
                        pydevd_sys_monitoring.enable_code_tracing(thread_ident, frame.f_code, frame)
                else:
                    if DEBUG:
                        pydev_log.debug(
                            "SKIP set tracing of frame: %s - %s", frame.f_code.co_filename, frame.f_code.co_name
                        )
            else:
    ...
    def run(self, file, globals=None, locals=None, is_module=False, set_trace=True):
        ...
        if self.thread_analyser is not None:
            ...
            send_concurrency_message(
                "threading_event", 0, t.name, thread_id, "thread", "start", file, 1, None, parent=thread_id
            )

        if self.asyncio_analyser is not None:
            # we don't have main thread in asyncio graph, so we should add a fake event
            send_concurrency_message(
                "asyncio_event", 0, "Task", "Task", "thread", "stop", file, 1, frame=None, parent=None
            )
    ...
    if SetupHolder.setup is not None:
        if port != SetupHolder.setup["port"]:
            raise AssertionError(
                "Unable to listen in port: %s (already listening in port: %s)" % (port, SetupHolder.setup["port"])
            )
    settrace(
        host=host,
        port=port,
...
def _log_initial_info():
    ...
    pydev_log.debug(
        "Using gevent mode: %s / imported gevent module support: %s", SUPPORT_GEVENT, bool(pydevd_gevent_integration)
    )
