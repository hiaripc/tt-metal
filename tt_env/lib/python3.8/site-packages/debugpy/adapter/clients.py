...
class Client(components.Component):
    ...
    def launch_request(self, request):
        ...
        if shell_expand_args:
            ...
            if console == "internalConsole":
                raise request.isnt_valid('Shell expansion in "args" is not available for "console":"internalConsole"')
    ...
    def configurationDone_request(self, request):
        if self.start_request is None or self.has_started:
            request.cant_handle(
                '"configurationDone" is only allowed during handling of a "launch" ' 'or an "attach" request'
            )
