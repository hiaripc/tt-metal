...
class Session(util.Observable):
    ...
    def wait_for(self, predicate, timeout=None):
        ...
        if timeout is not None:
            thread = threading.Thread(target=wait_for_timeout, name="Session.wait_for() timeout")
            thread.daemon = True
            thread.start()
        ...
        if self.server:
            if self.server.is_connected:
                if terminate_debuggee and self.launcher and self.launcher.is_connected:
                    ...
                    try:
                        self.server.channel.request("disconnect", {"terminateDebuggee": terminate_debuggee})
                    except Exception:
                        pass
