...
class JsonObject(object):
    ...
    def __format__(self, format_spec):
        ...
        if format_spec:
            ...
            encoder = eval(make_encoder, {"json_encoder_factory": self.json_encoder_factory})
        else:
...
def array(validate_item=False, vectorize=False, size=None):
    ...
    if size is None:
        ...
        validate_size = lambda value: (
            len(value) in size or "must have {0} elements".format(" or ".join(str(n) for n in sorted(size)))
        )
        ...
        validate_size = lambda value: (len(value) == size or "must have {0} elements".format(size))
