...
class Event(Message):
    ...
    def _handle(self):
        ...
        try:
            try:
                result = handler(self)
                assert result is None, f"Handler {util.srcnameof(handler)} tried to respond to {self.describe()}."
        ...
        try:
            ...
            if result is NO_RESPONSE:
                assert self.response is None, (
                    ...
                    "invoked request.respond().".format(util.srcnameof(handler), self.describe())
                )
            elif self.response is not None:
                assert result is None or result is self.response.body, (
                    "Handler {0} for {1} must not return a response body if it has "
                    "already invoked request.respond().".format(util.srcnameof(handler), self.describe())
                )
            else:
...
class Response(Message):
    ...
    def _parse(channel, message_dict, body=None):
        ...
        if known_request:
            ...
            raise response.isnt_valid("request_seq={0} does not match any known request", request_seq)
