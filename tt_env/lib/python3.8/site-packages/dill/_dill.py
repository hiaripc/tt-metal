...
def _create_code(*args):
    ...
    args = (
        fields[k].encode() if k in ENCODE_PARAMS and hasattr(fields[k], "encode") else fields[k] for k in CODE_PARAMS
    )
...
def _create_capsule(pointer, name, context, destructor):
    ...
    if attr_found:
        ...
        warnings.warn(
            "Creating a new PyCapsule %s for a C data structure that may not be present in memory. Segmentation faults or other memory errors are possible."
            % (name,),
            UnpicklingWarning,
        )
...
def _save_with_postproc(pickler, reduction, is_pickler_dill=None, obj=Getattr.NO_DEFAULT, postproc_list=None):
    ...
    if is_pickler_dill:
        ...
        if id(obj) in pickler._postproc:
            ...
            warnings.warn(
                "Cannot pickle %r: %shas recursive self-references that trigger a RecursionError." % (obj, name),
                PicklingWarning,
            )
...
if not OLD310 and MAPPING_PROXY_TRICK:
    ...
    def save_dict_view(dicttype):
        ...
        return (("keys", save_dict_keys), ("values", save_dict_values), ("items", save_dict_items))
...
def _save_file(pickler, obj, open_):
    ...
    pickler.save_reduce(
        _create_filehandle, (obj.name, obj.mode, position, obj.closed, open_, strictio, fmode, fdata), obj=obj
    )
    return
...
if IS_PYPY:
    ...
    def save_instancemethod0(pickler, obj):
        ...
        if code is not None and type(code) is not CodeType and getattr(obj.__self__, obj.__name__) == obj:
...
def _is_builtin_module(module):
    ...
    return (
        any(
            os.path.realpath(module.__file__).startswith(os.path.realpath(getattr(sys, name)))
            for name in names
            if hasattr(sys, name)
        )
...
def save_module(pickler, obj):
    if False:  # _use_diff:
        ...
        if (
            ...
            pickler.save_reduce(_import_module, (mod_name,), obj=obj, state=_main_dict)
            logger.trace(pickler, "# M1")
    ...
    if not _locate_function(obj, pickler):
        ...
        if _recurse:
            ...
            # If the globals is a module __dict__, do not save it in the pickle.
                ...
                and getattr(_import_module(obj.__module__, True), "__dict__", None) is globs_copy
            ):
                globs = globs_copy
            else:
                globs = {"__name__": obj.__module__}
        ...
        _save_with_postproc(
            pickler,
            (_create_function, (obj.__code__, globs, obj.__name__, obj.__defaults__, closure), state),
            obj=obj,
            postproc_list=postproc_list,
        )
...
if HAS_CTYPES and hasattr(ctypes, "pythonapi"):
    ...
    _testcapsule = _PyCapsule_New(
        ctypes.cast(_PyCapsule_New, ctypes.c_void_p), ctypes.create_string_buffer(b"dill._dill._testcapsule"), None
    )
    ...
    def save_capsule(pickler, obj):
        ...
        warnings.warn(
            "Pickling a PyCapsule (%s) does not pickle any C data structures and could cause segmentation faults or other memory errors when unpickling."
            % (name,),
            PicklingWarning,
        )
