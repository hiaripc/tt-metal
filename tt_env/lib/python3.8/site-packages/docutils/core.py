...
class Publisher(object):
    ...
    def set_source(self, source=None, source_path=None):
        ...
        try:
            self.source = self.source_class(
                source=source, source_path=source_path, encoding=self.settings.input_encoding
            )
        except TypeError:
            self.source = self.source_class(
                source=source, source_path=source_path, encoding=self.settings.input_encoding
    ...
    def apply_transforms(self):
        self.document.transformer.populate_from_components(
            (self.source, self.reader, self.reader.parser, self.writer, self.destination)
        )
    ...
    def publish(
        ...
        if (
            enable_exit_status
            and self.document
            and (self.document.reporter.max_level >= self.settings.exit_status_level)
        ):
    ...
    def debugging_dumps(self):
        ...
        if self.settings.dump_transforms:
            ...
            print(
                pprint.pformat(
                    [
                        (priority, "%s.%s" % (xclass.__module__, xclass.__name__), pending and pending.details, kwargs)
                        for priority, xclass, pending, kwargs in self.document.transformer.applied
                    ]
                ),
                file=self._stderr,
            )
...
command line used."""
    ...
    def report_SystemMessage(self, error):
        print(
            "Exiting due to level-%s (%s) system message." % (error.level, utils.Reporter.levels[error.level]),
            file=self._stderr,
        )
...
def publish_from_doctree(
    ...
    pub = Publisher(
        reader, None, writer, source=io.DocTreeInput(document), destination_class=io.StringOutput, settings=settings
    )
...
def publish_programmatically(
    ...
    * `enable_exit_status`: Boolean; enable exit status at end of processing?
    """
    pub = Publisher(
        reader, parser, writer, settings=settings, source_class=source_class, destination_class=destination_class
    )
    pub.set_components(reader_name, parser_name, writer_name)
    pub.process_programmatic_settings(settings_spec, settings_overrides, config_section)
    pub.set_source(source, source_path)
