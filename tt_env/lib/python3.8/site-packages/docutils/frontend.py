...
def validate_encoding_and_error_handler(setting, value, option_parser, config_parser=None, config_section=None):
    ...
    if ":" in value:
        ...
        validate_encoding_error_handler(
            setting + "_error_handler", handler, option_parser, config_parser, config_section
        )
        if config_parser:
...
def validate_smartquotes_locales(setting, value, option_parser, config_parser=None, config_section=None):
    ...
    for item in value:
        try:
            ...
            raise ValueError(
                'Invalid value "%s".' ' Format is "<language>:<quotes>".' % item.encode("ascii", "backslashreplace")
            )
...
def filter_settings_spec(settings_spec, *exclude, **replace):
    ...
    for i in range(2, len(settings), 3):
        ...
        for opt_spec in settings[i]:
            ...
            opt_name = [opt_string[2:].replace("-", "_") for opt_string in opt_spec[1] if opt_string.startswith("--")][
                0
            ]
...
class OptionParser(optparse.OptionParser, docutils.SettingsSpec):
    ...
    settings_spec = (
        ...
        None,
        (
            ("Specify the document title as metadata.", ["--title"], {}),
            (
                'Include a "Generated by Docutils" credit and link.',
                ["--generator", "-g"],
                {"action": "store_true", "validator": validate_boolean},
            ),
            ("Do not include a generator credit.", ["--no-generator"], {"action": "store_false", "dest": "generator"}),
            (
            ...
            (
                ...
                ["--time", "-t"],
                {"action": "store_const", "const": "%Y-%m-%d %H:%M UTC", "dest": "datestamp"},
            ),
            (
            ...
            ("Use <URL> for a source link; implies --source-link.", ["--source-url"], {"metavar": "<URL>"}),
            (
                'Do not include a "View document source" link.',
                ["--no-source-link"],
                {"action": "callback", "callback": store_multiple, "callback_args": ("source_link", "source_url")},
            ),
            (
                "Link from section headers to TOC entries.  (default)",
                ["--toc-entry-backlinks"],
                {"dest": "toc_backlinks", "action": "store_const", "const": "entry", "default": "entry"},
            ),
            (
            ...
            (
                ...
                ["--footnote-backlinks"],
                {"action": "store_true", "default": 1, "validator": validate_boolean},
            ),
            (
            ...
            (
                ...
                ["--section-numbering"],
                {"action": "store_true", "dest": "sectnum_xform", "default": 1, "validator": validate_boolean},
            ),
            (
            ...
            (
                ...
                "(Multiple-use option.)",
                ["--strip-class"],
                {"action": "append", "dest": "strip_classes", "metavar": "<class>", "validator": validate_strip_class},
            ),
            (
            ...
            ("Disable debug output.  (default)", ["--no-debug"], {"action": "store_false", "dest": "debug"}),
            (
            ...
            (
                ...
                ["--input-encoding", "-i"],
                {"metavar": "<name[:handler]>", "validator": validate_encoding_and_error_handler},
            ),
            (
            ...
            (
                ...
                ["--output-encoding", "-o"],
                {"metavar": "<name[:handler]>", "default": "utf-8", "validator": validate_encoding_and_error_handler},
            ),
            (
            ...
            (
                ...
                "Default: %s:%s." % (default_error_encoding, default_error_encoding_error_handler),
                ["--error-encoding", "-e"],
                {
            ...
            (
                ...
                "error output.  Default: %s." % default_error_encoding_error_handler,
                ["--error-encoding-error-handler"],
                {"default": default_error_encoding_error_handler, "validator": validate_encoding_error_handler},
            ),
            (
            ...
            (
                ...
                ["--record-dependencies"],
                {"metavar": "<file>", "validator": validate_dependency_file, "default": None},
            ),  # default set in Values class
            (
                "Read configuration settings from <file>, if it exists.",
                ["--config"],
                {"metavar": "<file>", "type": "string", "action": "callback", "callback": read_config_file},
            ),
            ("Show this program's version number and exit.", ["--version", "-V"], {"action": "version"}),
            ("Show this help message and exit.", ["--help", "-h"], {"action": "help"}),
            ...
            (
                ...
                {"action": "append", "dest": "expose_internals", "validator": validate_colon_separated_string_list},
            ),
    ...
    settings_defaults = {"_disable_config": None, "_source": None, "_destination": None, "_config_files": None}
    ...
    def check_args(self, args):
        ...
        if source and source == destination:
            self.error(
                "Do not specify the same file for both source and " "destination.  It will clobber the source file."
            )
