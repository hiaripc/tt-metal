...
class Input(TransformSpec):
    ...
    def __repr__(self):
        return "%s: source=%r, source_path=%r" % (self.__class__, self.source, self.source_path)
    ...
    def decode(self, data):
        ...
        if self.encoding and self.encoding.lower() == "unicode":
            assert isinstance(data, unicode), 'input encoding is "unicode" ' "but input is not a unicode object"
    ...
    coding_slug = re.compile(rb"coding[:=]\s*([-\w.]+)")
...
class Output(TransformSpec):
    ...
    def __repr__(self):
        return "%s: destination=%r, destination_path=%r" % (self.__class__, self.destination, self.destination_path)
    ...
    def encode(self, data):
        if self.encoding and self.encoding.lower() == "unicode":
            assert isinstance(data, unicode), (
                'the encoding given is "unicode" but the output is not ' "a Unicode string"
            )
            return data
...
class FileInput(Input):
    ...
    def __init__(
        ...
        elif sys.version_info >= (3, 0) and check_encoding(self.source, self.encoding) is False:
            # TODO: re-open, warn or raise error?
            raise UnicodeError(
                'Encoding clash: encoding given is "%s" '
                'but source is opened with encoding "%s".' % (self.encoding, self.source.encoding)
            )
        if not source_path:
            try:
...
class FileOutput(Output):
    ...
    def __init__(
        ...
        if destination is None:
            ...
            mode and hasattr(self.destination, "mode") and mode != self.destination.mode
        ):
            print(
                'Warning: Destination mode "%s" differs from specified ' 'mode "%s"' % (self.destination.mode, mode),
                file=self._stderr,
            )
        if not destination_path:
            try:
                ...
                try:
                    ...
                    if check_encoding(self.destination, self.encoding) is False:
                        raise ValueError(
                            ...
                            % (self.destination_path or "destination", self.destination.encoding, self.encoding)
                        )
                    else:
                        raise err
        except (UnicodeError, LookupError) as err:
            raise UnicodeError(
                "Unable to encode output data. output-encoding is: " "%s.\n(%s)" % (self.encoding, ErrorString(err))
            )
        finally:
            if self.autoclose:
                self.close()
