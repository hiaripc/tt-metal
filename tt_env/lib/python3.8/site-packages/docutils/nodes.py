...
class Node(object):
    ...
    def walk(self, visitor):
        ...
        visitor.document.reporter.debug(
            "docutils.nodes.Node.walk calling dispatch_visit for %s" % self.__class__.__name__
        )
        try:
            try:
    ...
    def walkabout(self, visitor):
        ...
        stop = False
        visitor.document.reporter.debug(
            "docutils.nodes.Node.walkabout calling dispatch_visit for %s" % self.__class__.__name__
        )
        try:
            try:
                visitor.dispatch_visit(self)
        ...
        if call_depart:
            visitor.document.reporter.debug(
                "docutils.nodes.Node.walkabout calling dispatch_departure " "for %s" % self.__class__.__name__
            )
    ...
    def traverse(self, condition=None, include_self=True, descend=True, siblings=False, ascend=False):
        ...
        warnings.warn("nodes.Node.traverse() is obsoleted by Node.findall().", PendingDeprecationWarning, stacklevel=2)
        return list(self.findall(condition, include_self, descend, siblings, ascend))

    def findall(self, condition=None, include_self=True, descend=True, siblings=False, ascend=False):
        """
        ...
        * the following siblings of the parent (if `ascend` is true) and
        ...
        if descend and len(self.children):
            for child in self:
                for subnode in child.findall(
                    condition=condition, include_self=True, descend=True, siblings=False, ascend=False
                ):
                    yield subnode
        if siblings or ascend:
            ...
            while node.parent:
                ...
                for sibling in node.parent[index + 1 :]:
                    for subnode in sibling.findall(
                        condition=condition, include_self=True, descend=descend, siblings=False, ascend=False
                    ):
                        yield subnode
                if not ascend:
                    break
...
class Text(Node, reprunicode):
    ...
    def __init__(self, data, rawsource=None):
        ...
        if rawsource is not None:
            warnings.warn(
                'nodes.Text: initialization argument "rawsource" ' "is ignored and will be removed in Docutils 1.3.",
                DeprecationWarning,
                stacklevel=2,
            )
        ...
        try:
            if self.document.settings.detailed:
                lines = ["%s%s" % (indent * level, "<#text>")] + [
                    indent * (level + 1) + repr(reprunicode(line)) for line in self.splitlines(True)
                ]
                return "\n".join(lines) + "\n"
        except AttributeError:
            pass
...
class Element(Node):
    ...
    def __repr__(self):
        ...
        for c in self.children:
            ...
            if len(data) > 60:
                ...
                break
        if self["names"]:
            return '<%s "%s": %s>' % (self.__class__.__name__, "; ".join([ensure_str(n) for n in self["names"]]), data)
        else:
    ...
    def __unicode__(self):
        if self.children:
            return "%s%s%s" % (self.starttag(), "".join([unicode(c) for c in self.children]), self.endtag())
        else:
    ...
    def set_class(self, name):
        ...
        warnings.warn(
            "docutils.nodes.Element.set_class() is deprecated; " "append to Element['classes'] list attribute directly",
            DeprecationWarning,
            stacklevel=2,
        )
        assert " " not in name
    ...
    def set_id(self, node, msgnode=None, suggested_prefix=""):
        ...
        for name in node["names"]:
            ...
            if base_id and auto_id_prefix.endswith("%"):
                ...
                if prefix.endswith("%"):
                    prefix = "%s%s-" % (prefix[:-1], suggested_prefix or make_id(node.tagname))
            while True:
        ...
        if explicit:
            if old_explicit:
                ...
                msg = self.reporter.system_message(
                    level, 'Duplicate explicit target name: "%s".' % name, backrefs=[id], base_node=node
                )
                if msgnode != None:
                    msgnode += msg
                dupname(node, name)
...
class pending(Special, Invisible, Element):
    ...
    def pformat(self, indent="    ", level=0):
        internals = [
            ".. internal attributes:",
            "     .transform: %s.%s" % (self.transform.__module__, self.transform.__name__),
            "     .details:",
        ]
        ...
        return Element.pformat(self, indent, level) + "".join(
...
class NodeVisitor(object):
    ...
    def dispatch_visit(self, node):
        ...
        self.document.reporter.debug(
            "docutils.nodes.NodeVisitor.dispatch_visit calling %s for %s" % (method.__name__, node_name)
        )
        return method(node)
    ...
    def dispatch_departure(self, node):
        ...
        self.document.reporter.debug(
            "docutils.nodes.NodeVisitor.dispatch_departure calling %s for %s" % (method.__name__, node_name)
        )
        return method(node)
