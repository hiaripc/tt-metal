        ...
        def parse(self, inputstring, document):
            ...
            for i, line in enumerate(inputstring.split("\n")):
                if len(line) > document.settings.line_length_limit:
                    error = document.reporter.error("Line %d exceeds the line-length-limit." % (i + 1))
                    document.append(error)
                    return
            ...
            try:
                ...
                error = document.reporter.error('Parsing with "recommonmark" ' "returned the error:\n%s" % err)
                document.append(error)
            ...
            for node in document.findall(nodes.TextElement):
                ...
                while i + 1 < len(children):
                    if isinstance(children[i], nodes.Text) and isinstance(children[i + 1], nodes.Text):
            ...
            for node in document.findall(lambda n: isinstance(n, (nodes.literal, nodes.literal_block))):
                ...
                if "level" in node:
                    ...
                    if node["level"] != section_level:
                        warning = document.reporter.warning(
                            "Title level inconsistent. Changing from %d to %d." % (node["level"], section_level),
            ...
            for node in document.findall(addnodes.pending_xref):
                ...
                if "name" not in reference:
                    reference["name"] = nodes.fully_normalize_name(reference.astext())
