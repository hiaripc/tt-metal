...
def directive(directive_name, language_module, document):
    ...
    try:
        ...
        messages.append(
            document.reporter.error(
                'Error importing directive module "%s" (directive "%s"):\n%s' % (modulename, directive_name, detail),
    ...
    try:
        ...
        messages.append(
            document.reporter.error(
                'No directive class "%s" in module "%s" (directive "%s").' % (classname, modulename, directive_name),
                line=document.current_line,
            )
        )
        return None, messages
...
unicode_pattern = re.compile(r"(?:0x|x|\\x|U\+?|\\u)([0-9a-f]+)$|&#x([0-9a-f]+);$", re.IGNORECASE)
...
def choice(argument, values):
    ...
    try:
        ...
        raise ValueError("must supply an argument; choose from %s" % format_values(values))
    if value in values:
        return value
    else:
        raise ValueError('"%s" unknown; choose from %s' % (argument, format_values(values)))
