...
class Include(Directive):
    ...
    def run(self):
        ...
        if after_text:
            ...
            if after_index < 0:
                raise self.severe(
                    'Problem with "start-after" option of "%s" ' "directive:\nText not found." % self.name
                )
        ...
        if not include_log:  # new document
            include_log.append((utils.relative_path(None, source), (None, None, None, None)))
        if (path, clip_options) in include_log:
            raise self.warning(
                'circular inclusion in "%s" directive: %s'
                % (self.name, " < ".join([path] + [pth for (pth, opt) in include_log[::-1]]))
            )
...
class Raw(Directive):
    ...
    def run(self):
        if not self.state.document.settings.raw_enabled or (
            not self.state.document.settings.file_insertion_enabled
        ...
        raw_node = nodes.raw("", text, classes=self.options.get("class", []), **attributes)
            ...
            if not node and isinstance(elem, nodes.paragraph):
                ...
                return [
                    self.state_machine.reporter.error(
                        'Error in "%s" directive: may contain a single paragraph ' "only." % (self.name),
                        line=self.lineno,
                    )
                ]
        if node:
...
class Unicode(Directive):
    ...
    optional_arguments = 0
    final_argument_whitespace = True
    option_spec = {"trim": directives.flag, "ltrim": directives.flag, "rtrim": directives.flag}
...
class Role(Directive):
    ...
    argument_pattern = re.compile(r"(%s)\s*(\(\s*(%s)\s*\)\s*)?$" % ((states.Inliner.simplename,) * 2))
    ...
    def run(self):
        ...
        if not match:
            raise self.error('"%s" directive arguments not valid role names: ' '"%s".' % (self.name, args))
        new_role_name = match.group(1)
        base_role_name = match.group(3)
        messages = []
        if base_role_name:
            base_role, messages = roles.role(
                base_role_name, self.state_machine.language, self.lineno, self.state.reporter
            )
        ...
        assert not hasattr(
            base_role, "arguments"
        ), 'Supplemental directive arguments for "%s" directive not ' 'supported (specified by "%r" role).' % (
            self.name,
            base_role,
        )
        try:
            converted_role = convert_directive_function(base_role)
            (arguments, options, content, content_offset) = self.state.parse_directive_block(
                self.content[1:], self.content_offset, converted_role, option_presets={}
            )
        ...
        for token in tokens[1:]:
            try:
                ...
                msg = self.reporter.error(
                    'Error parsing meta tag attribute "%s": %s.' % (token, detail), nodes.literal_block(line, line)
                )
                return msg, blank_finish
...
class Meta(Directive):
    ...
    def run(self):
        ...
        if (new_line_offset - self.content_offset) != len(self.content):
            ...
            error = self.state_machine.reporter.error(
                "Invalid meta directive.", nodes.literal_block(self.block_text, self.block_text), line=self.lineno
            )
            node += error
        # insert at begin of document
        index = self.state.document.first_child_not_matching_class((nodes.Titular, nodes.meta)) or 0
    ...
    def run(self):
        ...
        if sys.version_info < (3, 0):
            try:
                ...
                raise self.warning("Cannot encode date format string " 'with locale encoding "%s".' % locale_encoding)
        # @@@
        ...
        if sys.version_info < (3, 0):
            ...
            try:
                ...
                raise self.warning('Error decoding "%s"' 'with locale encoding "%s".' % (text, locale_encoding))
