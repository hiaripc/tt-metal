# $Id: tables.py 8860 2021-10-22 16:39:59Z milde $
    ...
    def check_table_dimensions(self, rows, header_rows, stub_columns):
        if len(rows) < header_rows:
            error = self.state_machine.reporter.error(
                ...
                'supplied ("%s" directive).' % (header_rows, len(rows), self.name),
        ...
        if len(rows) == header_rows > 0:
            error = self.state_machine.reporter.error(
                ...
                'for table body, required by "%s" directive.' % (len(rows), self.name),
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno,
            )
            raise SystemMessagePropagation(error)
        for row in rows:
            if len(row) < stub_columns:
                error = self.state_machine.reporter.error(
                    ...
                    'data supplied ("%s" directive).' % (stub_columns, len(row), self.name),
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                raise SystemMessagePropagation(error)
            if len(row) == stub_columns > 0:
                error = self.state_machine.reporter.error(
                    ...
                    'for table body, required by "%s" directive.' % (len(row), self.name),
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                raise SystemMessagePropagation(error)
    ...
    def get_csv_data(self):
        ...
        if self.content:
            ...
            try:
                ...
                severe = self.state_machine.reporter.severe(
                    'Problems with "%s" directive URL "%s":\n%s.' % (self.name, self.options["url"], SafeString(error)),
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                raise SystemMessagePropagation(severe)
