...
def role(role_name, language_module, lineno, reporter):
    ...
    if role_name:
        ...
        try:
            ...
            msg_text.append("Problem retrieving role entry from language " "module %r: %s." % (language_module, error))
        except KeyError:
            msg_text.append('No role entry for "%s" in module "%s".' % (role_name, language_module.__name__))
    else:
...
def rfc_reference_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    try:
        ...
        msg = inliner.reporter.error(
            "RFC number must be a number greater than or equal to 1; " '"%s" is invalid.' % text, line=lineno
        )
        prb = inliner.problematic(rawtext, rawtext, msg)
        return [prb], [msg]
