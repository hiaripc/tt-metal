...
class RSTState(StateWS):
    ...
    def no_match(self, context, transitions):
        ...
        self.reporter.severe(
            ...
            % (self.__class__.__name__, transitions, context, self.state_machine.line)
        )
        return context, None, []
    ...
    def nested_parse(
        self, block, input_offset, node, match_titles=False, state_machine_class=None, state_machine_kwargs=None
    ):
        """
    ...
    def new_subsection(self, title, lineno, messages):
        ...
        newabsoffset = self.nested_parse(
            self.state_machine.input_lines[offset:], input_offset=absoffset, node=section_node, match_titles=True
        )
...
class Inliner(object):
    ...
    def init_customizations(self, settings):
        ...
        self.patterns = Struct(
            ...
            emphasis=re.compile(self.non_whitespace_escape_before + r"(\*)" + end_string_suffix, re.UNICODE),
            strong=re.compile(self.non_whitespace_escape_before + r"(\*\*)" + end_string_suffix, re.UNICODE),
            ...
            target=re.compile(self.non_whitespace_escape_before + r"(`)" + end_string_suffix, re.UNICODE),
            substitution_ref=re.compile(
                self.non_whitespace_escape_before + r"(\|_{0,2})" + end_string_suffix, re.UNICODE
            ),
    ...
    def interpreted_or_phrase_ref(self, match, lineno):
        ...
        if endmatch and endmatch.start(1):  # 1 or more chars
            ...
            if rawsource[-1:] == "_":
                if role:
                    msg = self.reporter.warning(
                        "Mismatch: both interpreted text role %s and " "reference suffix." % position, line=lineno
                    )
                    text = unescape(string[rolestart:textend], True)
                    prb = self.problematic(text, text, msg)
                    return string[:rolestart], [prb], string[textend:], [msg]
                return self.phrase_ref(string[:matchstart], string[textend:], rawsource, escaped)
            else:
        ...
        msg = self.reporter.warning(
            "Inline interpreted text or phrase reference start-string " "without end-string.", line=lineno
        )
        text = unescape(string[matchstart:matchend], True)
        prb = self.problematic(text, text, msg)
        return string[:matchstart], [prb], string[matchend:], [msg]
    ...
    def phrase_ref(self, before, after, rawsource, escaped, text=None):
        ...
        if match:  # embedded <URI> or <alias_>
            ...
            if aliastext.endswith("_") and not (underscore_escaped or self.patterns.uri.match(aliastext)):
                ...
                target.indirect_reference_name = whitespace_normalize_name(unescape(aliastext[:-1]))
            else:
                aliastype = "uri"
        ...
        reference = nodes.reference(rawsource, text, name=whitespace_normalize_name(unescaped))
    ...
    def literal(self, match, lineno):
        before, inlines, remaining, sysmessages, endstring = self.inline_obj(
            match, lineno, self.patterns.literal, nodes.literal, restore_backslashes=True
        )
        return before, inlines, remaining, sysmessages
    ...
    def reference(self, match, lineno, anonymous=False):
        ...
        referencenode = nodes.reference(
            referencename + match.group("refend"), referencename, name=whitespace_normalize_name(referencename)
        )
...
class Body(RSTState):
    ...
    enum.sequences = ["arabic", "loweralpha", "upperalpha", "lowerroman", "upperroman"]  # ORDERED!
    enum.sequencepats = {
        "arabic": "[0-9]+",
        "loweralpha": "[a-z]",
        "upperalpha": "[A-Z]",
    ...
    def block_quote(self, indented, line_offset):
        ...
        while indented:
            ...
            (blockquote.source, blockquote.line) = self.state_machine.get_source_and_line(line_offset + 1)
            (
                blockquote_lines,
                attribution_lines,
                attribution_offset,
                indented,
                new_line_offset,
            ) = self.split_attribution(indented, line_offset)
            self.nested_parse(blockquote_lines, line_offset, blockquote)
            elements.append(blockquote)
            if attribution_lines:
                attribution, messages = self.parse_attribution(attribution_lines, line_offset + attribution_offset)
    ...
    def enumerator(self, match, context, next_state):
        ...
        if ordinal != 1:
            ...
            msg = self.reporter.info('Enumerated list start value not ordinal-1: "%s" (ordinal %s)' % (text, ordinal))
            self.parent += msg
        ...
        newline_offset, blank_finish = self.nested_list_parse(
            ...
            blank_finish=blank_finish,
            extra_settings={"lastordinal": ordinal, "format": format, "auto": sequence == "#"},
        )
    ...
    def parse_option_marker(self, match):
        ...
        for optionstring in optionstrings:
            ...
            if len(firstopt) > 1:
                ...
                (tokens[0].startswith("-") and not tokens[0].startswith("--")) or tokens[0].startswith("+")
            ):
                # "-ovalue" form
            ...
            if 0 < len(tokens) <= 2:
                ...
                raise MarkupError(
                    "wrong number of option tokens (=%s), should be 1 or 2: " '"%s"' % (len(tokens), optionstring)
                )
        return optlist
    ...
    def line_block(self, match, context, next_state):
        ...
        if not blank_finish:
            ...
            self.goto_line(new_line_offset)
        if not blank_finish:
            self.parent += self.reporter.warning("Line block ends without a blank line.", line=lineno + 1)
        if len(block):
    ...
    def line_block_line(self, match, lineno):
        ...
        indented, indent, line_offset, blank_finish = self.state_machine.get_first_known_indented(
            match.end(), until_blank=True
        )
        text = "\n".join(indented)
    ...
    def table_top(self, match, context, next_state, isolate_function, parser_class):
        ...
        if not blank_finish:
            msg = self.reporter.warning(
                "Blank line required after table.", line=self.state_machine.abs_line_number() + 1
            )
            self.parent += msg
        return [], next_state, []
            ...
            if match:
                if len(line.strip()) != toplen:
                    ...
                    messages = self.malformed_table(
                        lines[start : i + 1], "Bottom/header table border does " "not match top border."
                    )
    ...
    def hyperlink_target(self, match):
        ...
        lineno = self.state_machine.abs_line_number()
        block, indent, offset, blank_finish = self.state_machine.get_first_known_indented(
        ...
        for node in substitution_node.findall(nodes.Element):
            if self.disallowed_inside_substitution_definitions(node):
                ...
                msg = self.reporter.error(
                    "Substitution definition contains illegal element <%s>:" % node.tagname,
                    pformat,
                    nodes.literal_block(blocktext, blocktext),
                    source=src,
                    line=srcline,
    ...
    def run_directive(self, directive, match, type_name, option_presets):
        ...
        directive_instance = directive(
            type_name, arguments, options, content, lineno, content_offset, block_text, self, self.state_machine
        )
        try:
    ...
    def unknown_directive(self, type_name):
        ...
        text = "\n".join(indented)
        error = self.reporter.error(
            'Unknown directive type "%s".' % type_name, nodes.literal_block(text, text), line=lineno
        )
        return [error], blank_finish
...
class EnumeratedList(SpecializedBody):
    ...
    def enumerator(self, match, context, next_state):
        ...
        if (
            ...
            or (
                sequence != "#"
                and (sequence != self.parent["enumtype"] or self.auto or ordinal != (self.lastordinal + 1))
            )
...
class SubstitutionDef(Body):
    ...
    patterns = {"embedded_directive": re.compile(r"(%s)::( +|$)" % Inliner.simplename, re.UNICODE), "text": r""}
...
class Text(RSTState):
    ...
    patterns = {"underline": Body.patterns["line"], "text": r""}
    ...
    def underline(self, match, context, next_state):
        ...
        if column_width(title) > len(underline):
            if len(underline) < 4:
                ...
                msg = self.reporter.warning(
                    "Title underline too short.", nodes.literal_block(blocktext, blocktext), line=lineno
                )
                messages.append(msg)
        if not self.state_machine.match_titles:
            ...
            msg = self.reporter.severe(
                "Unexpected section title.", nodes.literal_block(blocktext, blocktext), source=src, line=srcline
            )
            self.parent += messages
            self.parent += msg
            return [], next_state, []
    ...
    def quoted_literal_block(self):
        ...
        new_abs_offset = self.nested_parse(
            ...
            node=parent_node,
            match_titles=False,
            state_machine_kwargs={"state_classes": (QuotedLiteralBlock,), "initial_state": "QuotedLiteralBlock"},
        )
...
class Line(SpecializedText):
    ...
    def text(self, match, context, next_state):
        ...
        try:
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.severe(
                    "Incomplete section title.", nodes.literal_block(blocktext, blocktext), line=lineno
                )
                self.parent += msg
                return [], "Body", []
        ...
        if not self.transitions["underline"][0].match(underline):
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.severe(
                    "Title overline & underline mismatch.", nodes.literal_block(source, source), line=lineno
                )
                self.parent += msg
                return [], "Body", []
        title = title.rstrip()
        ...
        if column_width(title) > len(overline):
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.warning(
                    "Title overline too short.", nodes.literal_block(source, source), line=lineno
                )
                messages.append(msg)
    ...
    def underline(self, match, context, next_state):
        ...
        msg = self.reporter.error(
            "Invalid section title or transition marker.", nodes.literal_block(blocktext, blocktext), line=lineno
        )
        self.parent += msg
        return [], "Body", []
...
class QuotedLiteralBlock(RSTState):
    ...
    patterns = {"initial_quoted": r"(%(nonalphanum7bit)s)" % Body.pats, "text": r""}
    ...
    def eof(self, context):
        if context:
            ...
            self.parent += self.reporter.warning(
                "Literal block expected; none found.", line=self.state_machine.abs_line_number()
            )
    ...
    def text(self, match, context, next_state):
        if context:
            self.messages.append(
                self.reporter.error("Inconsistent literal block quoting.", line=self.state_machine.abs_line_number())
            )
            self.state_machine.previous_line()
        raise EOFError
