...
class TableParser(object):
    ...
    def find_head_body_sep(self):
            ...
            if self.head_body_separator_pat.match(line):
                if self.head_body_sep:
                    raise TableMarkupError(
                        ...
                        "(table lines %s and %s); only one allowed." % (self.head_body_sep + 1, i + 1),
                        offset=i,
                    )
                else:
        ...
        if self.head_body_sep == 0 or self.head_body_sep == (len(self.block) - 1):
            raise TableMarkupError(
                "The head/body row separator may not be " "the first or last line of the table.", offset=i
            )
...
class GridTableParser(TableParser):
    ...
    def structure_from_cells(self):
        ...
        colspecs = [(colseps[i] - colseps[i - 1] - 1) for i in range(1, len(colseps))]  # list of column widths
    ...
    def init_row(self, colspec, offset):
        ...
        for start, end in colspec:
            ...
            try:
                ...
                raise TableMarkupError(
                    "Column span alignment problem " "in table line %s." % (offset + 2), offset=offset + 1
                )
        ...
        for i in range(len(columns) - 1):
            ...
            for line in lines:
                if i == lastcol and line[end:].strip():
                    ...
                    raise TableMarkupError(
                        "Text in column margin " "in table line %s." % (first_line + offset + 1),
