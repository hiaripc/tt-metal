...
class DocInfo(Transform):
    ...
    def apply(self):
        ...
        if isinstance(candidate, nodes.field_list):
            biblioindex = document.first_child_not_matching_class((nodes.Titular, nodes.Decorative, nodes.meta))
        ...
        for field in field_list:
            try:
                ...
                if issubclass(biblioclass, nodes.TextElement):
                    ...
                    topics[canonical] = biblioclass("", title, classes=[canonical], *field[1].children)
                else:
    ...
    def check_empty_biblio_field(self, field, name):
        if len(field[-1]) < 1:
            field[-1] += self.document.reporter.warning(
                'Cannot extract empty bibliographic field "%s".' % name, base_node=field
            )
            return None
        return 1
    ...
    def check_compound_biblio_field(self, field, name):
        if len(field[-1]) > 1:
            field[-1] += self.document.reporter.warning(
                'Cannot extract compound bibliographic field "%s".' % name, base_node=field
            )
            return None
        if not isinstance(field[-1][0], nodes.paragraph):
            field[-1] += self.document.reporter.warning(
                'Cannot extract bibliographic field "%s" containing ' "anything other than a single paragraph." % name,
                base_node=field,
            )
            return None
        return 1
    ...
    rcs_keyword_substitutions = [
        (re.compile(r"\$" r"Date: (\d\d\d\d)[-/](\d\d)[-/](\d\d)[ T][\d:]+" r"[^$]* \$", re.IGNORECASE), r"\1-\2-\3"),
    ...
    def extract_authors(self, field, name, docinfo):
        try:
            ...
            field[-1] += self.document.reporter.warning(
                ...
                "per item." % (name, "".join(self.language.author_separators)),
                base_node=field,
            )
            raise
    ...
    def authors_from_one_paragraph(self, field):
        ...
        text = "".join(unicode(node) for node in field[1].findall(nodes.Text))
        if not text:
            raise TransformError
