    ...
    def apply(self):
        for target in self.document.findall(nodes.target):
            ...
            if isinstance(target.parent, nodes.TextElement) or (
                target.hasattr("refid") or target.hasattr("refuri") or target.hasattr("refname")
            ):
                continue
...
class IndirectHyperlinks(Transform):
    ...
    def indirect_target_error(self, target, explanation):
        ...
        msg = self.document.reporter.error(
            'Indirect hyperlink target %s refers to target "%s", %s.' % (naming, target["refname"], explanation),
            base_node=target,
        )
        msgid = self.document.set_id(msg)
        ...
        for ref in self.document.autofootnote_refs:
            ...
            try:
                ...
                msg = self.document.reporter.error(
                    ...
                    "corresponding footnotes available." % len(self.autofootnote_labels),
                    base_node=ref,
                )
                msgid = self.document.set_id(msg)
    ...
    def symbolize_footnotes(self):
        ...
        for ref in self.document.symbol_footnote_refs:
            try:
                ...
                msg = self.document.reporter.error(
                    "Too many symbol footnote references: only %s " "corresponding footnotes available." % len(labels),
                    base_node=ref,
                )
                msgid = self.document.set_id(msg)
        ...
        for ref in subreflist:
            ...
            if key is None:
                ...
                if len(subdef.astext()) > line_length_limit:
                    msg = self.document.reporter.error(
                        'Substitution definition "%s" exceeds the' " line-length-limit." % (key)
                    )
            if msg:
                msgid = self.document.set_id(msg)
                prb = nodes.problematic(ref.rawsource, ref.rawsource, refid=msgid)
            ...
            try:
                ...
                if isinstance(parent, nodes.substitution_definition):
                    ...
                    msg = self.document.reporter.error(
                        "Circular substitution definition referenced: " '"%s".' % refname, base_node=ref_origin
                    )
                    msgid = self.document.set_id(msg)
                    prb = nodes.problematic(ref.rawsource, ref.rawsource, refid=msgid)
                    prbid = self.document.set_id(prb)
        ...
        if id is None:
            for resolver_function in self.unknown_reference_resolvers:
                ...
                if refname in self.document.nameids:
                    msg = self.document.reporter.error(
                        "Duplicate target name, cannot be used as a unique " 'reference: "%s".' % (node["refname"]),
                        base_node=node,
                    )
                else:
