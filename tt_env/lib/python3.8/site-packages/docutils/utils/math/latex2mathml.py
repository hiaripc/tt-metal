...
class math(object):
    ...
    def __repr__(self):
        ...
        content += ["%s=%r" % (k, v) for k, v in self.attributes.items() if v is not None]
    ...
    def _xml(self, level=0):
        return [self.xml_starttag()] + self._xml_body(level) + ["</%s>" % self.__class__.__name__]

    def xml_starttag(self):
        attrs = [
            '%s="%s"' % (k, str(v).replace("True", "true").replace("False", "false"))
    ...
    while len(string) > 0:
        ...
        if c == "\\":  # start of a LaTeX macro
            ...
            if c == "-" and node.children:
                ...
                if getattr(previous_node, "data", "-") in "([=" or previous_node.get("class") == "mathopen":
...
def handle_cmd(name, node, string):
    ...
    if name in bigdelimiters:
        ...
        if delimiter not in "()[]/|.":
            try:
                ...
                raise SyntaxError('Unsupported "\\%s" delimiter "%s"!' % (name, delimiter))
        if size:
    ...
    if name in layout_styles:  # 'displaystyle', 'textstyle', ...
        ...
        if isinstance(node, mrow) and len(node) == 0:
            ...
            raise SyntaxError('Declaration "\\%s" must be first command ' "in a group." % name)
        return new_node, string
...
def handle_script_or_limit(node, c, limits=""):
    ...
    elif limits == "movablelimits" or getattr(child, "data", "") in movablelimits:
    ...
    if c == "_":
        ...
        elif limits in ("limits", "movablelimits") or limits == "" and child.get("movablelimits", None) == True:
...
def align_attributes(rows):
    atts = {"class": "align", "displaystyle": True}
    tabs = max(row.count("&") - row.count(r"\&") for row in rows)
    if tabs:
...
def tex2mathml(tex_math, inline=True):
    ...
    if not inline:
        ...
        if len(rows) > 1:
            ...
            node = mtd()
            math_tree.append(mtable(mtr(node), **align_attributes(rows)))
