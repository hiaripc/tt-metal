...
class Writer(writers.Writer):
    ...
    settings_spec = (
        ...
        (
            (
                "Specify the template file (UTF-8 encoded). " "(default: writer dependent)",
                ["--template"],
                {"metavar": "<file>"},
            ),
            (
            ...
            (
                ...
                "(default: writer dependent)",
                ["--stylesheet-dirs"],
                {"metavar": "<dir[,dir,...]>", "validator": frontend.validate_comma_separated_list},
            ),
            (
                "Embed the stylesheet(s) in the output HTML file.  The stylesheet "
                "files must be accessible during processing. (default)",
                ["--embed-stylesheet"],
                {"default": 1, "action": "store_true", "validator": frontend.validate_boolean},
            ),
            (
...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document):
        ...
        if getattr(settings, "embed_images", None) is True:
            warnings.warn(
                'The configuration setting "embed_images" '
    ...
    def stylesheet_call(self, path, adjust_path=None):
        ...
        if self.settings.embed_stylesheet:
            try:
                ...
                msg = "Cannot embed stylesheet '%r': %s." % (path, SafeString(err.strerror))
    ...
    def visit_address(self, node):
        ...
        self.body.append(self.starttag(node, "pre", suffix="", CLASS="address"))
    ...
    def is_compactable(self, node):
        ...
        if isinstance(node, (nodes.field_list, nodes.definition_list)) and not self.settings.compact_field_lists:
            return False
        if isinstance(node, (nodes.enumerated_list, nodes.bullet_list)) and not self.settings.compact_lists:
            return False
        # Table of Contents:
    ...
    def visit_citation(self, node):
        ...
        self.body.append(self.starttag(node, "div", classes=[node.tagname], role="doc-biblioentry"))
    ...
    def visit_citation_reference(self, node):
        ...
        self.body.append(
            self.starttag(node, "a", suffix="[", href=href, classes=["citation-reference"], role="doc-biblioref")
        )
    ...
    def depart_colspec(self, node):
        ...
        if "colwidths-auto" in node.parent.parent["classes"] or (
            "colwidths-grid" not in self.settings.table_style and "colwidths-given" not in node.parent.parent["classes"]
        ):
            return
    ...
    def visit_docinfo_item(self, node, name, meta=True):
        ...
        self.body.append('<dt class="%s">%s<span class="colon">:</span></dt>\n' % (name, self.language.labels[name]))
    ...
    def visit_doctest_block(self, node):
        self.body.append(self.starttag(node, "pre", suffix="", classes=["code", "python", "doctest"]))
    ...
    def visit_field_list(self, node):
        ...
        for i, cls in enumerate(classes):
            if cls.startswith("field-indent-"):
                try:
                    indent_length = length_or_percentage_or_unitless(cls[13:], "px")
                except ValueError:
                    break
    ...
    def visit_field_name(self, node):
        self.body.append(self.starttag(node, "dt", "", classes=node.parent["classes"]))

    def depart_field_name(self, node):
        self.body.append('<span class="colon">:</span></dt>\n')

    def visit_field_body(self, node):
        self.body.append(self.starttag(node, "dd", "", classes=node.parent["classes"]))
    ...
    def visit_footnote(self, node):
        ...
        self.body.append(self.starttag(node, "aside", classes=classes, role="note"))
    ...
    def visit_image(self, node):
        ...
        if "scale" in node:
            ...
            for att_name in "width", "height":
                if att_name in atts:
                    ...
                    assert match
                    atts[att_name] = "%s%s" % (float(match.group(1)) * (float(node["scale"]) / 100), match.group(2))
        style = []
        for att_name in "width", "height":
            if att_name in atts:
        ...
        if isinstance(node.parent, nodes.TextElement) or (
            isinstance(node.parent, nodes.reference) and not isinstance(node.parent.parent, nodes.TextElement)
        ):
        ...
        if self.image_loading == "embed":
            ...
            if err_msg:
                self.document.reporter.error("Cannot embed image %r: %s" % (uri, err_msg))
            else:
                self.settings.record_dependencies.add(uri.replace("\\", "/"))
        ...
        if mimetype == "application/x-shockwave-flash":
            ...
            tag = self.starttag(node, "object", "", data=uri, **atts) + node.get("alt", uri) + "</object>" + suffix
        else:
    ...
    def visit_label(self, node):
        ...
        if self.settings.footnote_backlinks:
            ...
            if len(backrefs) == 1:
                self.body.append('<a role="doc-backlink"' ' href="#%s">' % backrefs[0])
    ...
    def depart_label(self, node):
        ...
        if len(backrefs) > 1:
            backlinks = ['<a role="doc-backlink" href="#%s">%s</a>' % (ref, i) for (i, ref) in enumerate(backrefs, 1)]
    ...
    def visit_math(self, node, math_env=""):
        ...
        if self.math_output not in self.math_tags:
            self.document.reporter.error(
                'math-output format "%s" not supported ' 'falling back to "latex"' % self.math_output
            )
        ...
        if self.math_output == "mathml" and (
            not self.math_output_options or self.math_output_options[0] == "blahtexml"
        ):
            wrapper = None
        ...
        if self.math_output == "mathjax" and not self.math_header:
            ...
            if self.math_output_options and not self.math_header:
                self.math_header = [
                    self.stylesheet_call(utils.find_file_in_dirs(s, self.settings.stylesheet_dirs), adjust_path=True)
            ...
            try:
                if converter == "latexml":
                    ...
                    math_code = tex2mathml_extern.blahtexml(
                        math_code, inline=not (math_env), reporter=self.document.reporter
                    )
                elif not converter:
    ...
    def visit_system_message(self, node):
        ...
        self.body.append(
            ...
            % (node["type"], node["level"], self.encode(node["source"]), line, backref_text)
        )
    ...
    def visit_term(self, node):
        if "details" in node.parent.parent["classes"]:
            ...
            self.body.append(self.starttag(node, "dt", "", classes=node.parent["classes"], ids=node.parent["ids"]))
...
class SimpleListChecker(nodes.GenericNodeVisitor):
    ...
    def visit_list_item(self, node):
        ...
        if (
            ...
            and (
                isinstance(children[-1], nodes.bullet_list)
