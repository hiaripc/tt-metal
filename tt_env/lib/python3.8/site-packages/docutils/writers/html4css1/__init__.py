...
class Writer(writers._html_base.Writer):
    ...
    default_stylesheet_dirs = [
        ...
        os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), "html5_polyglot")),  # for math.css
    ]
    default_template = os.path.join(os.path.dirname(os.path.abspath(__file__)), "template.txt")
    ...
    settings_spec = frontend.filter_settings_spec(
        ...
        initial_header_level=(
            ...
            {"choices": "1 2 3 4 5 6".split(), "default": "1", "metavar": "<level>"},
        ),
        xml_declaration=(
    ...
    settings_spec = settings_spec + (
        ...
        (
            (
                ...
                'Use 0 for "no limit".',
                ["--field-name-limit"],
                {"default": 14, "metavar": "<level>", "validator": frontend.validate_nonnegative_int},
            ),
            (
...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def depart_colspec(self, node):
        ...
        if "colwidths-auto" in node.parent.parent["classes"] or (
            "colwidths-auto" in self.settings.table_style and ("colwidths-given" not in node.parent.parent["classes"])
        ):
            return
        ...
        for node in self.colspecs:
            ...
            self.body.append(self.emptytag(node, "col", width="%i%%" % colwidth))
    ...
    def is_compactable(self, node):
        return "compact" in node["classes"] or (
            self.settings.compact_lists
    ...
    def visit_citation(self, node):
        self.body.append(self.starttag(node, "table", CLASS="docutils citation", frame="void", rules="none"))
    ...
    def visit_citation_reference(self, node):
        ...
        self.body.append(self.starttag(node, "a", suffix="[", href=href, classes=["citation-reference"]))
    ...
    def visit_docinfo(self, node):
        ...
        self.body.append(self.starttag(node, "table", CLASS="docinfo", frame="void", rules="none"))
    ...
    def visit_field_list(self, node):
        ...
        elif self.settings.compact_field_lists and "open" not in node["classes"]:
            self.compact_field_list = True
        if self.compact_field_list:
            for field in node:
                field_body = field[-1]
                assert isinstance(field_body, nodes.field_body)
                children = [n for n in field_body if not isinstance(n, nodes.Invisible)]
                if not (
                    len(children) == 0
                    or len(children) == 1
                    and isinstance(children[0], (nodes.paragraph, nodes.line_block))
                ):
                    self.compact_field_list = False
                    break
        self.body.append(self.starttag(node, "table", frame="void", rules="none", CLASS="docutils field-list"))
    ...
    def visit_field_name(self, node):
        ...
        if self.settings.field_name_limit and len(node.astext()) > self.settings.field_name_limit:
            atts["colspan"] = 2
            self.context.append("</tr>\n" + self.starttag(node.parent, "tr", "", CLASS="field") + "<td>&nbsp;</td>")
        else:
            self.context.append("")
    ...
    def visit_footnote(self, node):
        self.body.append(self.starttag(node, "table", CLASS="docutils footnote", frame="void", rules="none"))
        self.body.append('<colgroup><col class="label" /><col /></colgroup>\n' '<tbody valign="top">\n' "<tr>")
        self.footnote_backrefs(node)
    ...
    def visit_footnote_reference(self, node):
        ...
        self.body.append(self.starttag(node, "a", suffix, CLASS="footnote-reference", href=href))
    ...
    def visit_image(self, node):
        ...
        if ext in self.object_image_types:
            ...
            self.body.append(self.starttag(node, "object", "", **atts) + node.get("alt", uri) + "</object>" + suffix)
        else:
    ...
    def visit_label(self, node):
        self.body.append(self.starttag(node, "td", "%s[" % self.context.pop(), CLASS="label"))
    ...
    def visit_option_group(self, node):
        atts = {}
        if self.settings.option_limit and len(node.astext()) > self.settings.option_limit:
            atts["colspan"] = 2
            self.context.append("</tr>\n<tr><td>&nbsp;</td>")
        else:
            self.context.append("")
    ...
    def visit_option_list(self, node):
        self.body.append(self.starttag(node, "table", CLASS="docutils option-list", frame="void", rules="none"))
    ...
    def should_be_compact_paragraph(self, node):
        ...
        for key, value in node.attlist():
            if node.is_not_default(key) and not (
                key == "classes" and value in ([], ["first"], ["last"], ["first", "last"])
            ):
    ...
    def visit_subscript(self, node):
        if isinstance(node.parent, nodes.literal_block):
            self.body.append(self.starttag(node, "span", "", CLASS="subscript"))
        else:
    ...
    def visit_subtitle(self, node):
        if isinstance(node.parent, nodes.sidebar):
            self.body.append(self.starttag(node, "p", "", CLASS="sidebar-subtitle"))
            self.context.append("</p>\n")
    ...
    def visit_superscript(self, node):
        if isinstance(node.parent, nodes.literal_block):
            self.body.append(self.starttag(node, "span", "", CLASS="superscript"))
        else:
    ...
    def visit_table(self, node):
        ...
        self.body.append(self.starttag(node, "table", CLASS=" ".join(classes), **atts))
...
class SimpleListChecker(writers._html_base.SimpleListChecker):
    ...
    def visit_list_item(self, node):
        ...
        if (
            ...
            and (isinstance(children[-1], nodes.bullet_list) or isinstance(children[-1], nodes.enumerated_list))
        ):
            children.pop()
        if len(children) <= 1:
            return
