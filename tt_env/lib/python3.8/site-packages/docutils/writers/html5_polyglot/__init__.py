...
class Writer(writers._html_base.Writer):
    ...
    settings_spec = settings_spec + (
        ...
        (
            ...
            ('Obsoleted by "--image-loading".', ["--link-images"], {"dest": "embed_images", "action": "store_false"}),
            (
...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def visit_document(self, node):
        title = node.get("title", "") or os.path.basename(node["source"]) or "untitled Docutils document"
    ...
    def visit_image(self, node):
        ...
        self.body.append(
            ...
            % (self.starttag(node, "video", suffix, src=uri, **atts), uri, node.get("alt", uri), suffix, suffix)
        )
    ...
    supported_inline_tags = set(
        ("code", "kbd", "dfn", "samp", "var", "bdi", "del", "ins", "mark", "small", "b", "i", "q", "s", "u")
    )
    ...
    def visit_inline(self, node):
        ...
        if len(tags):
            ...
            classes == ["ln"] and isinstance(node.parent, nodes.literal_block) and "code" in node.parent.get("classes")
        ):
            if self.body[-1] == "<code>":
                del self.body[-1]
            else:
    ...
    def visit_literal(self, node):
        ...
        if tagname == "code":
            ...
            return
        self.body.append(self.starttag(node, tagname, "", CLASS="docutils literal"))
        text = node.astext()
    ...
    def section_title_tags(self, node):
        start_tag, close_tag = super(HTMLTranslator, self).section_title_tags(node)
        ids = node.parent["ids"]
        if ids and getattr(self.settings, "section_self_link", None) and not isinstance(node.parent, nodes.document):
            self_link = '<a class="self-link" title="link to this section"' ' href="#%s"></a>' % ids[0]
