# .. coding: utf-8
# $Id: __init__.py 8880 2021-11-05 11:11:18Z milde $
...
class Writer(writers.Writer):
    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                ["--use-latex-citations"],
                {"default": None, "action": "store_true", "validator": frontend.validate_boolean},
            ),
            (
                "Use figure floats for citations " "(might get mixed with real figures). (provisional default)",
                ["--figure-citations"],
                {"dest": "use_latex_citations", "action": "store_false", "validator": frontend.validate_boolean},
            ),
            (
                'Format for block quote attributions: one of "dash" (em-dash '
                'prefix), "parentheses"/"parens", or "none".  Default: "dash".',
                ["--attribution"],
                {"choices": ["dash", "parentheses", "parens", "none"], "default": "dash", "metavar": "<format>"},
            ),
            (
            ...
            (
                ...
                ["--embed-stylesheet"],
                {"default": False, "action": "store_true", "validator": frontend.validate_boolean},
            ),
            (
                ...
                'Default: ".".',
                ["--stylesheet-dirs"],
                {"metavar": "<dir[,dir,...]>", "validator": frontend.validate_comma_separated_list, "default": ["."]},
            ),
            (
            ...
            (
                ...
                ["--use-latex-toc"],
                {"default": True, "action": "store_true", "validator": frontend.validate_boolean},
            ),
            (
                "Table of contents by Docutils (without page numbers).",
                ["--use-docutils-toc"],
                {"dest": "use_latex_toc", "action": "store_false", "validator": frontend.validate_boolean},
            ),
            (
            ...
            (
                ...
                {"dest": "use_latex_docinfo", "action": "store_false", "validator": frontend.validate_boolean},
            ),
            (
            ...
            (
                ...
                ["--topic-abstract"],
                {"dest": "use_latex_abstract", "action": "store_false", "validator": frontend.validate_boolean},
            ),
            (
            ...
            ('Additional options to the "hyperref" package.', ["--hyperref-options"], {"default": ""}),
            (
            ...
            (
                ...
                ["--new-class-functions"],
                {"dest": "legacy_class_functions", "action": "store_false", "validator": frontend.validate_boolean},
            ),
            (
            ...
            (
                ...
                ["--new-column-widths"],
                {"dest": "legacy_column_widths", "action": "store_false", "validator": frontend.validate_boolean},
            ),
    ...
    def translate(self):
        ...
        try:
            template_file = open(self.document.settings.template, encoding="utf8")
        except IOError:
            template_file = open(
                os.path.join(self.default_template_path, self.document.settings.template), encoding="utf8"
            )
    ...
    def __call__(self):
        ...
        for c in "".join([self.active_chars.get(l, "") for l in languages]):
            if c not in shorthands:
                shorthands.append(c)
        if shorthands:
            self.setup.append(r"\AtBeginDocument{\shorthandoff{%s}}" % "".join(shorthands))
        ...
        if "estonian" in languages:
            self.setup.extend([r"\makeatletter", r"  \addto\extrasestonian{\bbl@deactivate{~}}", r"\makeatother"])
        if "basque" in languages:
            self.setup.extend([r"\makeatletter", r"  \addto\extrasbasque{\bbl@deactivate{~}}", r"\makeatother"])
        if languages[-1] == "english" and "french" in self.otherlanguages.keys():
            self.setup += [
                ...
                r"\AtBeginDocument{\selectlanguage{%s}" r"\noextrasfrench}" % self.language,
            ]
        return "\n".join(self.setup)
...
_du_sty = os.path.join(os.path.dirname(os.path.abspath(__file__)), "docutils.sty")
...
class Table(object):
    ...
    def set_table_style(self, node, settings):
        ...
        self.colwidths_auto = (
            ("colwidths-auto" in node["classes"] or "colwidths-auto" in settings.table_style)
        ...
        if not self.colwidths_auto:
            if self.borders == "standard" and not self.legacy_column_widths:
                opening.insert(
                    ...
                    r"{\dimexpr%s-%i\arrayrulewidth\relax}%%" % (width, len(self._col_specs) + 1),
                )
            else:
    ...
    def get_colspecs(self, node):
        ...
        if self.colwidths_auto:
            ...
            self._colwidths = [(factor * (node["colwidth"] + 1) / width) + 0.005 for node in self._col_specs]
            ...
            self._colwidths = [(node["colwidth"] + allowance) / norm_length for node in self._col_specs]
    ...
    def get_multicolumn_width(self, start, len_):
        ...
        try:
            multicol_width = sum([width for width in ([self._colwidths[start + co] for co in range(len_)])])
            if self.legacy_column_widths:
        ...
        if self._latex_type == "longtable":
            if 1 == self._translator.thead_depth():
                ...
                if twidth > 30 or (twidth > 12 and not self.colwidths_auto):
                    a.append(
                        r"\multicolumn{%d}{%s}" % (n_c, self.get_multicolumn_width(0, n_c))
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document, babel_class=Babel):
        ...
        if settings.use_latex_citations is None:
            ...
            warnings.warn(
                'The default for the setting "use_latex_citations" ' 'will change to "True" in Docutils 1.0.',
                FutureWarning,
                stacklevel=7,
            )
        if settings.legacy_column_widths is None:
            ...
            warnings.warn(
                'The default for the setting "legacy_column_widths" ' 'will change to "False" in Docutils 1.0.)',
                FutureWarning,
                stacklevel=7,
            )
        ...
        self.section_enumerator_separator = settings.section_enumerator_separator.replace("_", r"\_")
        # literal blocks:
        ...
        if self.settings.graphicx_option == "":
            ...
            self.graphicx_package = r"\usepackage[%s]{graphicx}" % self.settings.graphicx_option
        ...
        # Document parts
        self.head_prefix = [r"\documentclass[%s]{%s}" % (self.documentoptions, self.settings.documentclass)]
        ...
        if self.latex_encoding != "ascii":
            self.requirements["_inputenc"] = r"\usepackage[%s]{inputenc}" % self.latex_encoding
        # TeX font encoding
        if not self.is_xetex:
            if self.font_encoding:
                self.requirements["_fontenc"] = r"\usepackage[%s]{fontenc}" % self.font_encoding
            # ensure \textquotedbl is defined:
            for enc in self.font_encoding.split(","):
                enc = enc.strip()
                if enc == "OT1":
                    self.requirements["_textquotedblOT1"] = r"\DeclareTextSymbol{\textquotedbl}{OT1}{`\"}"
                elif enc not in ("T1", "T2A", "T2B", "T2C", "T4", "T5"):
                    self.requirements["_textquotedbl"] = r"\DeclareTextSymbolDefault{\textquotedbl}{T1}"
        # page layout with typearea (if there are relevant document options)
        if settings.documentclass.find("scr") == -1 and (
            self.documentoptions.find("DIV") != -1 or self.documentoptions.find("BCOR") != -1
        ):
        ...
        if self.fallback_stylesheet:
            stylesheet_list = [sheet for sheet in stylesheet_list if sheet != "docutils"]
        ...
        if settings.sectnum_xform:  # section numbering by Docutils
            ...
            if secnumdepth is not None:
                PreambleCmds.secnumdepth = r"\setcounter{secnumdepth}{%d}" % self.d_class.latex_section_depth(
                    secnumdepth
                )
    ...
    def stylesheet_call(self, path):
        ...
        if self.settings.embed_stylesheet:
            ...
            try:
                content = docutils.io.FileInput(source_path=path, encoding="utf-8").read()
                self.settings.record_dependencies.add(path)
            except IOError as err:
                msg = "Cannot embed stylesheet %r:\n  %s." % (path, SafeString(err.strerror))
                self.document.reporter.error(msg)
                return "% " + msg.replace("\n", "\n% ")
            if is_package:
                content = "\n".join([r"\makeatletter", content, r"\makeatother"])
        ...
        if not self.is_xetex:
            ...
            for ch in text:
                ...
                # preamble-definitions for unsupported Unicode characters
                    ...
                    self.requirements["_inputenc" + str(cp)] = "\\DeclareUnicodeCharacter{%04X}{%s}" % (
                        cp,
                        CharMaps.unsupported_unicode[cp],
                    )
    ...
    def depart_document(self, node):
        ...
        if self.title or (self.use_latex_docinfo and (self.author_stack or self.date)):
            ...
            if self.subtitle:
                title += [r"\\", r"\DUdocumentsubtitle{%s}" % "".join(self.subtitle)] + self.subtitle_labels
    ...
    def visit_entry(self, node):
        ...
        if "morerows" in node and "morecols" in node:
            raise NotImplementedError("Cells that " "span multiple rows *and* columns currently not supported, sorry.")
        ...
        if "morerows" in node:
            ...
            self.out.append(
                ...
                % (
                    mcols,
                    bar1,
                    self.active_table.get_multicolumn_width(self.active_table.get_entry_number(), mcols),
    ...
    def visit_enumerated_list(self, node):
        ...
        if enumeration_level <= 4:
            ...
            if (prefix, enumtype, suffix) != default_labels[enumeration_level - 1]:
    ...
    def visit_footnote(self, node):
        ...
        if self.docutils_footnotes:
            ...
            if self.settings.footnote_references == "brackets":
                num = "[%s]" % num
            self.out.append("%%\n\\DUfootnotetext{%s}{%s}{%s}{" % (node["ids"][0], backref, self.encode(num)))
    ...
    def to_latex_length(self, length_str, pxunit=None):
        ...
        if pxunit is not None:
            warnings.warn(
                "LaTeXTranslator.to_latex_length(): The optional "
    ...
    def visit_image(self, node):
        ...
        if "align" in attrs:
            ...
            try:
                ...
                pass  # TODO: warn?
        if "height" in attrs:
            include_graphics_options.append("height=%s" % self.to_latex_length(attrs["height"]))
        if "scale" in attrs:
            include_graphics_options.append("scale=%f" % (attrs["scale"] / 100.0))
        if "width" in attrs:
            include_graphics_options.append("width=%s" % self.to_latex_length(attrs["width"]))
        if not (self.is_inline(node) or isinstance(node.parent, (nodes.figure, nodes.compound))):
            pre.append("\n")
    ...
    def visit_paragraph(self, node):
        ...
        if index == 0 and isinstance(
            node.parent, (nodes.list_item, nodes.description, nodes.compound, nodes.container)
        ):
            pass
        elif (
            index > 0
            and isinstance(node.parent, nodes.compound)
            and not isinstance(node.parent[index - 1], (nodes.paragraph, nodes.compound))
        ):
            pass
    ...
    def visit_reference(self, node):
        ...
        if self._reference_label:
            self.out.append("\\%s{%s}}" % (self._reference_label, href.replace("#", "")))
            raise nodes.SkipNode
    ...
    def visit_system_message(self, node):
        ...
        if len(node["backrefs"]) == 1:
            ...
            backrefs = ["\\hyperlink{%s}{%d}" % (href, i + 1) for (i, href) in enumerate(node["backrefs"])]
    ...
    def visit_target(self, node):
        ...
        if (
            "refuri" in node or "refid" in node or "refname" in node  # external hyperlink  # resolved internal link
    ...
    def visit_contents(self, node):
        ...
        if self.use_latex_toc and "local" in node["classes"]:
            ...
            try:
                ...
                self.warn(
                    ...
                    '  Feature not supported with option "use-latex-toc"' % section_name,
                    base_node=node,
                )
                raise nodes.SkipNode
        ...
        if "local" in node["classes"]:
            ...
            self.requirements["minitoc-%s-depth" % mtc_name] = r"\mtcsetdepth{%stoc}{%d}" % (mtc_name, maxdepth)
