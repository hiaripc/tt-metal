...
try:
    ...
    from .pygmentsformatter import OdtPygmentsProgFormatter, OdtPygmentsLaTeXFormatter
...
class Writer(writers.Writer):
    ...
    default_stylesheet_path = utils.relative_path(
        os.path.join(os.getcwd(), "dummy"), os.path.join(os.path.dirname(__file__), default_stylesheet)
    )

    default_template = "template.txt"

    default_template_path = utils.relative_path(
        os.path.join(os.getcwd(), "dummy"), os.path.join(os.path.dirname(__file__), default_template)
    )
    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                {"default": None, "validator": frontend.validate_nonnegative_int},
            ),
            (
            ...
            for node in n2_nodes:
                ...
                if family == "paragraph" or family == "graphic":
                    ...
                    if language_code is not None:
                        n3.attrib[
                            "{urn:oasis:names:tc:opendocument:xmlns:" "xsl-fo-compatible:1.0}language"
                        ] = language_code
                        n3.attrib[
                            "{urn:oasis:names:tc:opendocument:xmlns:" "style:1.0}language-complex"
                        ] = language_code
                        updated = True
                        modified_nodes.add(n3)
                    if region_code is not None:
                        n3.attrib[
                            "{urn:oasis:names:tc:opendocument:xmlns:" "xsl-fo-compatible:1.0}country"
                        ] = region_code
    ...
    def create_meta(self):
        ...
        for prop, value in self.visitor.get_meta_dict().items():
            ...
            if prop == "keywords":
                ...
                el1 = SubElement(root, "meta:user-defined", attrib={"meta:name": prop}, nsdict=METNSD)
                el1.text = value
        s1 = ToString(doc)
        # doc = minidom.parseString(s1)
        # s1 = doc.toprettyxml('  ')
...
class ODFTranslator(nodes.GenericNodeVisitor):
        ...
        if self.settings.odf_config_file:
            ...
            for rststyle, format in parser.items("Formats"):
                if rststyle not in self.used_styles:
                    self.document.reporter.warning('Style "%s" is not a style used by odtwriter.' % (rststyle,))
                if sys.version_info >= (3, 0):
    ...
    def setup_paper(self, root_el):
        try:
            dimensions = subprocess.check_output(("paperconf", "-s"), stderr=subprocess.STDOUT)
        ...
        def walk(el):
            if (
                ...
                el.attrib["{%s}margin-left" % SNSD["fo"]] = el.attrib["{%s}margin-right" % SNSD["fo"]] = "%.3fpt" % (
                    0.1 * w
                )
                el.attrib["{%s}margin-top" % SNSD["fo"]] = el.attrib["{%s}margin-bottom" % SNSD["fo"]] = "%.3fpt" % (
                    0.1 * h
                )
            else:
                for subel in el:
    ...
    def visit_emphasis(self, node):
        el = SubElement(self.current_element, "text:span", attrib={"text:style-name": self.rststyle("emphasis")})
        self.set_current_element(el)
    ...
    def visit_list_item(self, node):
        ...
        if self.in_table_of_contents:
            if self.settings.generate_oowriter_toc:
                self.paragraph_style_stack.append(self.rststyle("contents-%d" % (self.list_level,)))
            else:
    ...
    def get_image_width_height(self, node, attr):
        ...
        if attr in node.attributes:
            ...
            try:
                ...
                self.document.reporter.warning(
                    'Invalid %s for image: "%s".  ' 'Error: "%s".' % (attr, node.attributes[attr], exp)
                )
        return size, unit
    ...
    def get_image_scale(self, node):
        if "scale" in node.attributes:
            ...
            try:
                ...
                self.document.reporter.warning('Invalid scale for image: "%s"' % (node.attributes["scale"],))
            if scale < 1:  # or scale > 100:
                self.document.reporter.warning("scale out of range (%s), using 1." % (scale,))
                scale = 1
            scale = scale * 0.01
        else:
            scale = 1.0
    ...
    def visit_literal(self, node):
        el = SubElement(self.current_element, "text:span", attrib={"text:style-name": self.rststyle("inlineliteral")})
        self.set_current_element(el)
    ...
    def _add_syntax_highlighting(self, insource, language):
        ...
        if language in ("latex", "tex"):
            fmtr = OdtPygmentsLaTeXFormatter(
                lambda name, parameters=(): self.rststyle(name, parameters), escape_function=escape_cdata
            )
        else:
    ...
    def visit_literal_block(self, node):
        ...
        if (
            pygments
            and self.settings.add_syntax_highlighting
            # and
    ...
    def visit_option_list(self, node):
        ...
        if not self.optiontablestyles_generated:
            ...
            el = SubElement(
                self.automatic_styles,
                "style:style",
                attrib={"style:name": self.rststyle(table_name), "style:family": "table"},
                nsdict=SNSD,
            )
            el1 = SubElement(
                el,
                "style:table-properties",
                attrib={"style:width": "17.59cm", "table:align": "left", "style:shadow": "none"},
                nsdict=SNSD,
            )
            el = SubElement(
                self.automatic_styles,
                "style:style",
                attrib={"style:name": self.rststyle("%s.%%c" % table_name, ("A",)), "style:family": "table-column"},
                nsdict=SNSD,
            )
            ...
            el = SubElement(
                self.automatic_styles,
                "style:style",
                attrib={"style:name": self.rststyle("%s.%%c" % table_name, ("B",)), "style:family": "table-column"},
                nsdict=SNSD,
            )
            el1 = SubElement(
                el, "style:table-column-properties", attrib={"style:column-width": "12.587cm"}, nsdict=SNSD
            )
            el = SubElement(
                self.automatic_styles,
                "style:style",
        ...
        el1 = SubElement(
            el, "table:table-column", attrib={"table:style-name": self.rststyle("%s.%%c" % table_name, ("A",))}
        )
        el1 = SubElement(
            el, "table:table-column", attrib={"table:style-name": self.rststyle("%s.%%c" % table_name, ("B",))}
        )
    ...
    def visit_reference(self, node):
        ...
        if self.settings.create_links:
            if "refuri" in node:
                ...
                self.document.reporter.warning('References must have "refuri" or "refid" attribute.')
        if (
        ...
        if table_name is not None:
            ...
            if table_style is None:
                ...
                if table_style is None:
                    ...
                    self.document.reporter.warning(
                        'Can\'t find default table style "%s".  ' "Using built-in default." % (table_name,)
                    )
    ...
    def visit_colspec(self, node):
        ...
        SubElement(
            el1, "style:table-column-properties", attrib={"style:column-width": "%.4fin" % colwidth}, nsdict=SNSD
        )
        self.append_child(
            "table:table-column",
            attrib={
    ...
    def visit_title_reference(self, node):
        el = self.append_child("text:span", attrib={"text:style-name": self.rststyle("quotation")})
...
class Reader(standalone.Reader):
    def get_transforms(self):
        ...
        if self.settings.create_links:
            return default
        return [i for i in default if i is not references.DanglingReferences]
