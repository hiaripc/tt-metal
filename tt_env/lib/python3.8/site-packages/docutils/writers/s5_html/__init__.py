...
themes_dir_path = utils.relative_path(
    os.path.join(os.getcwd(), "dummy"), os.path.join(os.path.dirname(__file__), "themes")
)
    ...
    settings_spec = html4css1.Writer.settings_spec + (
        ...
        (
            (
                ...
                ["--theme"],
                {"default": "default", "metavar": "<name>", "overrides": "theme_url"},
            ),
            (
            ...
            (
                ...
                ["--view-mode"],
                {"choices": ["slideshow", "outline"], "default": "slideshow", "metavar": "<mode>"},
            ),
            (
                "Normally hide the presentation controls in slideshow mode. " "This is the default.",
                ["--hidden-controls"],
                {"action": "store_true", "default": True, "validator": frontend.validate_boolean},
            ),
            (
...
%(title)s%(footer)s
    ...
    def __init__(self, *args):
        ...
        self.stylesheet.append(
            self.s5_stylesheet_template
            % {"path": self.theme_file_path, "view_mode": view_mode, "control_visibility": control_visibility}
        )
    ...
    def copy_file(self, name, source_dir, dest_dir):
        ...
        if os.path.isfile(source):
            ...
            if os.path.exists(dest) and not settings.overwrite_theme_files:
                ...
                dest_file.write(
                    src_data.replace(
                        b"ui/default", dest_dir[dest_dir.rfind("ui/") :].encode(sys.getfilesystemencoding())
                    )
                )
                dest_file.close()
