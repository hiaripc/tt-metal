...
class Babel(latex2e.Babel):
    ...
    def __call__(self):
        setup = [r"\usepackage{polyglossia}", r"\setdefaultlanguage{%s}" % self.language]
        if self.otherlanguages:
            setup.append(r"\setotherlanguages{%s}" % ",".join(sorted(self.otherlanguages.keys())))
        return "\n".join(setup)
...
class XeLaTeXTranslator(latex2e.LaTeXTranslator):
    ...
    def __init__(self, document):
        ...
        if self.latex_encoding == "utf8":
            ...
            self.requirements["_inputenc"] = r"\XeTeXinputencoding %s " % self.latex_encoding
