...
def _apply_recipe(recipe: TransformRecipe, tensor: Tensor, reduction_type: Reduction) -> Tensor:
    ...
    init_shapes, reduced_axes, axes_reordering, added_axes, final_shapes = _reconstruct_from_shape(
        recipe, backend.shape(tensor)
    )
...
def _prepare_transformation_recipe(
    pattern: str, operation: Reduction, axes_lengths: Tuple[Tuple, ...]
) -> TransformRecipe:
    ...
    reduced_axes: List[int] = [
        position for axis, position in axis_name2position.items() if axis not in rght.identifiers
    ]
