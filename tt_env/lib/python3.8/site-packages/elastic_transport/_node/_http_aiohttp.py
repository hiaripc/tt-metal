...
class AiohttpHttpNode(BaseAsyncNode):
    ...
    async def perform_request(  # type: ignore[override]
        ...
        resolved_timeout: Optional[float] = (
            self.config.request_timeout if request_timeout is DEFAULT else request_timeout
        )
        aiohttp_timeout = aiohttp.ClientTimeout(total=resolved_timeout if resolved_timeout is not None else 0)
...
def aiohttp_fingerprint(ssl_assert_fingerprint: str) -> "aiohttp.Fingerprint":
    ...
    """
    return aiohttp.Fingerprint(base64.b16decode(ssl_assert_fingerprint.replace(":", ""), casefold=True))
