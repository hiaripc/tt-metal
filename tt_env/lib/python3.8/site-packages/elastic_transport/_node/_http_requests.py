            ...
            **pool_kwargs: Any,
            ...
            if self._node_config.scheme == "https":
                ...
                if self._node_config.ssl_assert_fingerprint:
                    ...
                    pool_kwargs["assert_fingerprint"] = self._node_config.ssl_assert_fingerprint
...
class RequestsHttpNode(BaseNode):
    ...
    def __init__(self, config: NodeConfig):
        if not _REQUESTS_AVAILABLE:  # pragma: nocover
            raise ValueError("You must have 'requests' installed to use RequestsHttpNode")
        ...
        if config.scheme == "https":
            ...
            # Otherwise we go the traditional route of verifying certs.
                ...
                if config.scheme == "https" and not config.verify_certs and config.ssl_show_warn:
                    warnings.warn(
        ...
        try:
            requests_session_auth: Optional[AuthBase] = config._extras.pop("requests.session.auth", None)
        ...
        if hasattr(adapter, "get_connection_with_tls_context"):
            ...
            adapter.get_connection_with_tls_context(prepared_request, verify=self.session.verify)
        else:
    ...
    def perform_request(
        ...
        start = time.time()
        request = requests.Request(method=method, headers=request_headers, url=url, data=body_to_send)
        prepared_request = self.session.prepare_request(request)
        send_kwargs = {"timeout": (request_timeout if request_timeout is not DEFAULT else self.config.request_timeout)}
        send_kwargs.update(
        ...
        try:
            ...
            if isinstance(e, requests.Timeout):
                err = ConnectionTimeout("Connection timed out during request", errors=(e,))
