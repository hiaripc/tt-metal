...
class HTTPSConnectionPool(urllib3.HTTPSConnectionPool):
    ...
    def __init__(self, *args: Any, assert_fingerprint: Optional[str] = None, **kwargs: Any) -> None:
        self._elastic_assert_fingerprint = assert_fingerprint.replace(":", "").lower() if assert_fingerprint else None

        # Complain about fingerprint length earlier than urllib3 does.
        if self._elastic_assert_fingerprint and len(self._elastic_assert_fingerprint) not in _HASHES_BY_LENGTH:
    ...
    def _validate_conn(self, conn: HTTPSConnection) -> None:  # type: ignore[override]
        ...
        if self._elastic_assert_fingerprint:
            ...
            assert_fingerprint = unhexlify(self._elastic_assert_fingerprint.lower().replace(":", "").encode("ascii"))

            fingerprints: List[bytes]
            try:
                if sys.version_info >= (3, 13):
                    fingerprints = [hash_func(cert).digest() for cert in conn.sock.get_verified_chain()]  # type: ignore
                else:
