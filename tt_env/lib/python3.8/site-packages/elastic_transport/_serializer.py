...
class TextSerializer(Serializer):
    ...
    def loads(self, data: bytes) -> str:
        ...
        try:
            ...
            raise SerializationError(f"Unable to deserialize as text: {data!r}", errors=(e,))
...
class JsonSerializer(Serializer):
    ...
    def json_dumps(self, data: Any) -> bytes:
        return json.dumps(data, default=self.default, ensure_ascii=False, separators=(",", ":")).encode(
            "utf-8", "surrogatepass"
        )
...
if orjson is not None:
    ...
    class OrjsonSerializer(JsonSerializer):
        ...
        def json_dumps(self, data: Any) -> bytes:
            return orjson.dumps(data, default=self.default, option=orjson.OPT_SERIALIZE_NUMPY)
    ...
    def loads(self, data: bytes) -> Any:
        ...
        for line in re.split(b"[\n\r]", data):
            ...
            try:
                ...
                raise SerializationError(message=f"Unable to deserialize as NDJSON: {data!r}", errors=(e,))
        return ndjson
...
class SerializerCollection:
    ...
    def __init__(
        ...
        try:
            ...
            raise ValueError(f"Must configure a serializer for the default mimetype {default_mimetype!r}") from None
