    ...
    def __init__(
        ...
        if isinstance(node_class, str):
            if node_class not in NODE_CLASS_NAMES:
                ...
                raise ValueError(
                    f"Unknown option for node_class: '{node_class}'. " f"Available options are: '{options}'"
                )
        ...
        self.serializers = SerializerCollection(_serializers, default_mimetype=default_mimetype)
        ...
        *,
        ...
        if self.meta_header:
            request_headers["x-elastic-client-meta"] = ",".join(
                f"{k}={v}" for k, v in self._transport_client_meta + resolve_default(client_meta, ())
            )
        ...
        if body is not None:
            if "content-type" not in request_headers:
                raise ValueError("Must provide a 'Content-Type' header to requests with bodies")
            request_body = self.serializers.dumps(body, mimetype=request_headers["content-type"])
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if 200 <= resp.meta.status < 299 or resp.meta.status in NOT_DEAD_NODE_HTTP_STATUSES:
    ...
    def sniff(self, is_initial_sniff: bool = False) -> None:
        ...
        try:
            if should_sniff:
                ...
                options = SniffOptions(is_initial_sniff=is_initial_sniff, sniff_timeout=self._sniff_timeout)
                ...
                if not node_configs and is_initial_sniff:
                    raise SniffingError("No viable nodes were discovered on the initial sniff attempt")
