...
def create_user_agent(name: str, version: str) -> str:
    ...
    return f"{name}/{version} (Python/{python_version()}; elastic-transport/{__version__})"
...
def client_meta_version(version: str) -> str:
    ...
    if match is None:
        raise ValueError("Version {version!r} not formatted like a Python version string")
...
def to_str(value: Union[str, bytes], encoding: str = "utf-8", errors: str = "strict") -> str:
    if isinstance(value, bytes):
        return value.decode(encoding, errors)
    return value


def to_bytes(value: Union[str, bytes], encoding: str = "utf-8", errors: str = "strict") -> bytes:
...
_QUOTE_ALWAYS_SAFE = frozenset("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_.-~")
...
def basic_auth_to_header(basic_auth: Tuple[str, str]) -> str:
    ...
    if (
        ...
        raise ValueError("'basic_auth' must be a 2-tuple of str/bytes (username, password)")
...
def url_to_node_config(url: str, use_default_ports_for_scheme: bool = False) -> NodeConfig:
    ...
    if any(component in (None, "") for component in (parsed_url.scheme, parsed_url.host, parsed_port)):
        raise ValueError("URL must include a 'scheme', 'host', and 'port' component (ie 'https://localhost:9200')")
