...
_COMPAT_MIMETYPE_TEMPLATE = "application/vnd.elasticsearch+%s; compatible-with=" + str(__versionstr__.partition(".")[0])
...
def resolve_auth_headers(
    ...
    if resolved_http_auth is not None:
        ...
        if isinstance(http_auth, str) or (
            isinstance(resolved_http_auth, (list, tuple)) and all(isinstance(x, str) for x in resolved_http_auth)
        ):
...
def create_sniff_callback(
    host_info_callback: Optional[Callable[[Dict[str, Any], Dict[str, Any]], Optional[Dict[str, Any]]]] = None,
    sniffed_node_callback: Optional[Callable[[Dict[str, Any], NodeConfig], Optional[NodeConfig]]] = None,
    ...
    if host_info_callback is not None:
        ...
        def _sniffed_node_callback(node_info: Dict[str, Any], node_config: NodeConfig) -> Optional[NodeConfig]:
            ...
            if (
                host_info_callback(node_info, {"host": node_config.host, "port": node_config.port})  # type ignore[misc]
                is None
            ):
                return None
            return node_config
    ...
    def sniff_callback(transport: Transport, sniff_options: SniffOptions) -> List[NodeConfig]:
        for _ in transport.node_pool.all():
            try:
                meta, node_infos = transport.perform_request(
                    ...
                    request_timeout=(sniff_options.sniff_timeout if not sniff_options.is_initial_sniff else None),
                )
...
default_sniff_callback = create_sniff_callback(sniffed_node_callback=_default_sniffed_node_callback)
    ...
    def _perform_request(
        ...
        *,
        ...
        if not (method == "HEAD" and meta.status == 404) and (
            ...
            and (
                self._ignore_status is DEFAULT or self._ignore_status is None or meta.status not in self._ignore_status
            )
        ):
