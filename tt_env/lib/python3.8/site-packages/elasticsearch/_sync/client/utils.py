...
_TYPE_HOSTS = Union[str, Sequence[Union[str, Mapping[str, Union[str, int]], NodeConfig]]]

_TYPE_BODY = Union[bytes, str, Dict[str, Any]]

_TYPE_ASYNC_SNIFF_CALLBACK = Callable[[AsyncTransport, SniffOptions], Awaitable[List[NodeConfig]]]
...
def host_mapping_to_node_config(host: Mapping[str, Union[str, int]]) -> NodeConfig:
    ...
    if "url_prefix" in options:
        if "path_prefix" in options:
            raise ValueError(
                "Cannot specify conflicting options 'url_prefix' and " "'path_prefix'. Use 'path_prefix' only instead"
            )
...
def _merge_kwargs_no_duplicates(kwargs: Dict[str, Any], values: Dict[str, Any]) -> None:
    for key, val in values.items():
        if key in kwargs:
            raise ValueError(
                f"Received multiple values for '{key}', specify parameters " "directly instead of using 'params'"
            )
        kwargs[key] = val


def _merge_body_fields_no_duplicates(body: _TYPE_BODY, kwargs: Dict[str, Any], body_fields: Tuple[str, ...]) -> bool:
    mixed_body_and_params = False
    for key in list(kwargs.keys()):
        if key in body_fields:
            if isinstance(body, (str, bytes)):
                raise ValueError("Couldn't merge 'body' with other parameters as it wasn't a mapping.")
...
def _rewrite_parameters(
    ...
    def wrapper(api: F) -> F:
        ...
        def wrapped(*args: Any, **kwargs: Any) -> Any:
            ...
            if "params" in kwargs and (not ignore_deprecated_options or "params" not in ignore_deprecated_options):
            ...
            if maybe_transport_options:
                ...
                for option in maybe_transport_options:
                    if ignore_deprecated_options and option in ignore_deprecated_options:
                        continue
                    try:
            ...
            if "body" in kwargs and (not ignore_deprecated_options or "body" not in ignore_deprecated_options):
                ...
                if body is not None:
                    if body_name:
                        ...
                        mixed_body_and_params = _merge_body_fields_no_duplicates(body, kwargs, body_fields)
                        kwargs["body"] = body
...
def is_requests_node_class(node_class: Any) -> bool:
    ...
    return (
        ...
        and (node_class == "requests" or (inspect.isclass(node_class) and issubclass(node_class, RequestsHttpNode)))
    )
