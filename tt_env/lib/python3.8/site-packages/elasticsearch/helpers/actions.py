...
_TYPE_BULK_ACTION_HEADER_AND_BODY = Tuple[_TYPE_BULK_ACTION_HEADER, _TYPE_BULK_ACTION_BODY]
...
def _process_bulk_chunk_error(
    ...
    if raise_on_error and error.status_code not in ignore_status:
        raise BulkIndexError(f"{len(exc_errors)} document(s) failed to index.", exc_errors)
    else:
...
def streaming_bulk(
    ...
    raise_on_error: bool = True,
    expand_action_callback: Callable[[_TYPE_BULK_ACTION], _TYPE_BULK_ACTION_HEADER_AND_BODY] = expand_action,
    raise_on_exception: bool = True,
    max_retries: int = 0,
                ...
                try:
                    for data, (ok, info) in zip(
                        ...
                        if not ok:
                            ...
                            if max_retries and info["status"] in retry_on_status and (attempt + 1) <= max_retries:
    ...
    **kwargs: Any,
    ...
    class BlockingPool(ThreadPool):
        def _setup_queues(self) -> None:
            ...
            self._inqueue: Queue[
                Tuple[
                    List[Union[Tuple[Dict[str, Any]], Tuple[Dict[str, Any], Dict[str, Any]]]],
                    List[bytes],
                ]
...
def scan(
    ...
    **kwargs: Any,
    ...
    client = client.options(request_timeout=request_timeout, **pop_transport_kwargs(kwargs))
    ...
    try:
        while scroll_id and resp["hits"]["hits"]:
            ...
            resp = scroll_client.scroll(scroll_id=scroll_id, scroll=scroll, **scroll_kwargs)
            scroll_id = resp.get("_scroll_id")
...
def reindex(
    ...
    try:
        ...
        data_streams = target_client.indices.get_data_stream(name=target_index, expand_wildcards="all")
        is_data_stream = any(data_stream["name"] == target_index for data_stream in data_streams["data_streams"])
