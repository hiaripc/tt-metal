...
class AsyncVectorStore:
    ...
    async def delete(  # type: ignore[no-untyped-def]
        ...
        **delete_kwargs,
        ...
        try:
            if ids:
                body = [{"_op_type": "delete", "_index": self.index, "_id": _id} for _id in ids]
                await async_bulk(
                    self.client,
                    body,
                    refresh=refresh_indices,
    ...
    async def _create_index_if_not_exists(self) -> None:
        ...
        if self.custom_index_settings:
            conflicting_keys = set(self.custom_index_settings.keys()) & set(settings.keys())
            if conflicting_keys:
        ...
        await self.client.indices.create(index=self.index, mappings=mappings, settings=settings)
    ...
    async def max_marginal_relevance_search(
        ...
        *,
        ...
        selected_indices = maximal_marginal_relevance(query_vector, got_embeddings, lambda_mult=lambda_mult, k=k)
