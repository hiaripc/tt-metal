...
class BaseCounter(object):
    ...
    _placeholder_ = "___ENLIGHTEN_PLACEHOLDER___"
    ...
    def _resolve_color(self, value):
        ...
        if isinstance(value, Iterable) and len(value) == 3 and all(isinstance(_, int) and 0 <= _ <= 255 for _ in value):
...
class PrintableCounter(BaseCounter):  # pylint: disable=too-many-instance-attributes
    ...
    def fill(self, value):
        ...
        if char_len != 1:
            raise ValueError(
                "fill character must be a length of 1 " "when printed. Length: %d, Value given: %r" % (char_len, value)
            )
