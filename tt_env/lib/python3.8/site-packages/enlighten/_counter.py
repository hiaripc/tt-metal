...
from enlighten._util import EnlightenWarning, FORMAT_MAP_SUPPORT, format_time, raise_from_none, warn_best_level

COUNTER_FMT = "{desc}{desc_pad}{count:d} {unit}{unit_pad}" + "[{elapsed}, {rate:.2f}{unit_pad}{unit}/s]{fill}"
...
RESERVED_FIELDS = (
    {"count", "desc", "desc_pad", "elapsed", "interval", "rate", "unit", "unit_pad", "total"}
    | BAR_SPECIFIC_FIELDS
...
{count:{len_total}d}/{total:d} [{elapsed}<{eta}, {rate:.2f}{unit_pad}{unit}/s]'
    ...
    **Instance Attributes**
    ...
    def format(self, width=None, elapsed=None):
        ...
        if reserved_fields:
            warn_best_level(
                "Ignoring reserved fields specified as user-defined fields: %s" % ", ".join(reserved_fields),
                EnlightenWarning,
            )

        force_float = isinstance(count, float) or isinstance(total, float)
        fields["count"] = Float(count) if force_float else count
        fields["desc"] = self.desc or ""
        fields["total"] = Float(total) if force_float and total is not None else total
        fields["unit"] = self.unit or ""
        fields["desc_pad"] = " " if self.desc else ""
        fields["unit_pad"] = " " if self.unit else ""
    ...
    def _format_bar(self, fields, iterations, width, elapsed, force_float):
        ...
        if self.total == 0:
            ...
            fields["eta"] = format_time((self.total - iterations) / rate) if rate else "?"
    ...
    def update(self, incr=1, force=False, **fields):  # pylint: disable=arguments-differ
        ...
        if self.enabled:
            ...
            if force or self._count == self.total or currentTime - self.last_update >= self.min_delta:
