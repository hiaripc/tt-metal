    ...
    **companion_stream**
    ...
    def _flush_streams(self):
        ...
        if self.companion_stream is not None:
            if companion_buffer:
                self.companion_stream.write("".join(companion_buffer))
            ...
            if self.set_scroll:
                ...
                if self.companion_term:
                    self._companion_buffer.extend(
                        (term.normal_cursor, term.csr(0, self.height - 1), term.move(height, 0))
                    )
    ...
    def write(self, output="", flush=True, counter=None, **kwargs):
        ...
        try:
            self._buffer.extend((term.move(self.height - position, 0), "\r", term.clear_eol, output))
