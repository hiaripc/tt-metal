...
class FlightServer(flight.FlightServerBase):
    def __init__(self, delegate, **kwargs):
        ...
        if delegate:
            self.delegate = flight.connect(delegate, middleware=(TracingClientMiddlewareFactory(),))
        else:
            self.delegate = None
...
def main():
    ...
    if args.command == "server":
        server = FlightServer(
            args.delegate, location=args.listen, middleware={"trace": TracingServerMiddlewareFactory()}
        )
        server.serve()
    elif args.command == "client":
        client = flight.connect(args.server, middleware=(TracingClientMiddlewareFactory(),))
        if args.request_id:
