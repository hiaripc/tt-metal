...
class VaultClient(pe.KmsClient):
    ...
    def __init__(self, kms_connection_config):
        ...
        self.kms_url = kms_connection_config.kms_instance_url + VaultClient.DEFAULT_TRANSIT_ENGINE
    ...
    def wrap_key(self, key_bytes, master_key_identifier):
        ...
        headers = {VaultClient.TOKEN_HEADER: self.kms_connection_config.key_access_token}
        r = requests.post(
            endpoint + master_key_identifier, headers=headers, data={"plaintext": base64.b64encode(key_bytes)}
        )
        r.raise_for_status()
        r_dict = r.json()
    ...
    def unwrap_key(self, wrapped_key, master_key_identifier):
        ...
        headers = {VaultClient.TOKEN_HEADER: self.kms_connection_config.key_access_token}
        r = requests.post(endpoint + master_key_identifier, headers=headers, data={"ciphertext": wrapped_key})
        r.raise_for_status()
        r_dict = r.json()
...
def parquet_write_read_with_vault(parquet_filename):
    ...
    file_decryption_properties = crypto_factory.file_decryption_properties(kms_connection_config)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Filename of the parquet file to be created " "(default: /tmp/encrypted_table.vault.parquet",
    )
