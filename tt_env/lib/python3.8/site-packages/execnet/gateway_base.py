    ...
    def _try_send_to_primary_thread(self, reply: Reply) -> bool:
        ...
        if primary_thread_task_ready is not None:
            if not primary_thread_task_ready.is_set():
                ...
                return True
            elif self.execmodel.backend == "main_thread_only" and self._primary_thread_task is not None:
...
class Channel:
    ...
    def reconfigure(self, py2str_as_py3str: bool = True, py3str_as_py2str: bool = False) -> None:
...
MAIN_THREAD_ONLY_DEADLOCK_TEXT = "concurrent remote_exec would cause deadlock for main_thread_only execmodel"


class ChannelFactory:
    def __init__(self, gateway: BaseGateway, startcount: int = 1) -> None:
        self._channels: weakref.WeakValueDictionary[int, Channel] = weakref.WeakValueDictionary()
        # Channel ID => (callback, end marker, strconfig)
        self._callbacks: dict[int, tuple[Callable[[Any], Any], object, tuple[bool, bool]]] = {}
    ...
    def _terminate_execution(self) -> None:
        ...
        if not self._execpool.waitall(5.0):
            ...
            if not self._execpool.waitall(10.0):
                self._trace("execution did not finish in another 10 secs, " "calling os._exit()")
                os._exit(1)
        ...
        try:
            while True:
                ...
                try:
                    ...
                    raise LoadError(f"unknown opcode {opcode!r} - wire protocol corruption?") from None
                loader(self)
        except _Stop:
            if len(self.stack) != 1:
...
def loads(bytestring: bytes, py2str_as_py3str: bool = False, py3str_as_py2str: bool = False) -> Any:
    ...
    return load(io, py2str_as_py3str=py2str_as_py3str, py3str_as_py2str=py3str_as_py2str)


def load(io: ReadIO, py2str_as_py3str: bool = False, py3str_as_py2str: bool = False) -> Any:
...
class _Serializer:
    ...
    def _save(self, obj: object) -> None:
        ...
        try:
            ...
            meth: Callable[[_Serializer, object], None] | None = getattr(self.__class__, methodname, None)
            if meth is None:
