...
class Group:
    ...
    def __init__(self, xspecs: Iterable[XSpec | str | None] = (), execmodel: str = "thread") -> None:
    ...
    def set_execmodel(self, execmodel: str, remote_execmodel: str | None = None) -> None:
        ...
        """
        if self._gateways:
            raise ValueError("can not set execution models if " "gateways have been created already")
    ...
    def terminate(self, timeout: float | None = None) -> None:
        ...
        while self:
            ...
            safe_terminate(
                self.execmodel,
                timeout,
                [(partial(join_wait, gw), partial(kill, gw)) for gw in self._gateways_to_join],
            )
...
class MultiChannel:
    ...
    def receive_each(self, withchannel: bool = False) -> list[tuple[Channel, Any]] | list[Any]:
...
def safe_terminate(execmodel: ExecModel, timeout: float | None, list_of_paired_functions) -> None:
