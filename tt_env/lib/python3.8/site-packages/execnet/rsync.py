...
class RSync:
    ...
    def send(self, raises: bool = True) -> None:
        ...
        if not self._channels:
            if raises:
                raise OSError("no targets available, maybe you " "are trying call send() twice?")
            return
    ...
    def _send_link_structure(self, path: str) -> None:
        ...
        try:
            ...
            relpath = None
        if relpath is not None and relpath not in (os.curdir, os.pardir) and not relpath.startswith(os.pardir + os.sep):
