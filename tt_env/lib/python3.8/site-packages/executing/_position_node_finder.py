...
class PositionNodeFinder(object):
    ...
    def is_except_cleanup(inst: dis.Instruction, node: EnhancedAST) -> bool:
        ...
        return any(
            isinstance(n, ast.ExceptHandler) and n.name and mangled_name(n) == inst.argval for n in parents(node)
        )
    ...
    def verify(self, node: EnhancedAST, instruction: dis.Instruction) -> None:
        ...
        if (
            ...
            and any(
                mangled_name(cast(EnhancedAST, alias)) == instruction.argval for alias in cast(ast.Import, node).names
            )
        ):
        ...
        if node_match((ast.ListComp, ast.SetComp, ast.DictComp, ast.GeneratorExp, ast.For)) and inst_match(
            ("GET_ITER", "FOR_ITER")
        ):
            return
        ...
        if sys.version_info >= (3, 12):
            ...
            if node_match(ast.TypeAlias):
                if (
                    ...
                    or inst_match(("STORE_NAME", "STORE_FAST", "STORE_DEREF"), argrepr=node.name.id)
                    or inst_match("CALL")
                ):
                    return
