    ...
    def generate_required(self):
        ...
        with self.l("if {variable}_is_dict:"):
            ...
            if not self._definition.get("additionalProperties", True):
                ...
                if not_possible:
                    raise JsonSchemaDefinitionException(
                        "{}: items {} are required but not allowed".format(self._variable, not_possible)
                    )
            ...
            with self.l("if {variable}__missing_keys:"):
                ...
                self.exc(
                    "{name} must contain ", self.e(self._definition["required"]), rule="required", append_to_msg=dynamic
                )
