...
class CodeGeneratorDraft07(CodeGeneratorDraft06):
    ...
    def __init__(
        self, definition, resolver=None, formats={}, use_default=True, use_formats=True, detailed_exceptions=True
    ):
    ...
    def generate_if_then_else(self):
        ...
        with self.l("try:", optimize=False):
            self.generate_func_code_block(
                self._definition["if"], self._variable, self._variable_name, clear_variables=True
            )
        with self.l("except JsonSchemaValueException:"):
            if "else" in self._definition:
                self.generate_func_code_block(
                    self._definition["else"], self._variable, self._variable_name, clear_variables=True
                )
            else:
                self.l("pass")
        if "then" in self._definition:
            with self.l("else:"):
                self.generate_func_code_block(
                    self._definition["then"], self._variable, self._variable_name, clear_variables=True
                )
