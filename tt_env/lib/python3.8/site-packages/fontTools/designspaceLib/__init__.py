...
class BaseDocWriter(object):
    ...
    def _getEffectiveFormatTuple(self):
        ...
        if (
            ...
            or any(instance.locationLabel or instance.userLocation for instance in self.documentObject.instances)
        ):
            if minVersion < (5, 0):
                minVersion = (5, 0)
        if self.documentObject.axisMappings:
    ...
    def readRules(self):
        ...
        for ruleElement in self.root.findall(".rules/rule"):
            ...
            if externalConditions:
                ...
                self.log.info(
                    "Found stray rule conditions outside a conditionset. " "Wrapped them in a new conditionset."
                )
...
class DesignSpaceDocument(LogMixin, AsDictMixin):
    ...
    **Note:** Python attribute names are usually camelCased, the
    ...
    def map_backward(self, designLocation: AnisotropicLocationDict) -> SimpleLocationDict:
        ...
        """
        return {
            axis.name: (axis.map_backward(designLocation[axis.name]) if axis.name in designLocation else axis.default)
        ...
        for source in self.sources:
            ...
            if source.path in loaded:
                ...
                if source.path is None:
                    raise DesignSpaceDocumentError(
                        "Designspace source '%s' has no 'path' attribute" % (source.name or "<Unknown>")
                    )
