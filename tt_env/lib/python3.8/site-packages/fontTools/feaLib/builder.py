...
class Builder(object):
        ...
        if tables is None:
            ...
            if unsupported:
                ...
                raise NotImplementedError(
                    "The following tables were requested but are unsupported: " f"{unsupported_string}."
                )
        if "GSUB" in tables:
    ...
    def addAxisValueRecord(self, axisValueRecord, location):
        ...
        for record_ in self.stat_["AxisValueRecords"]:
            if (
                ...
                and {n.asFea() for n in record_.locations} == {n.asFea() for n in axisValueRecord.locations}
    ...
    def build_STAT(self):
        ...
        if "ElidedFallbackNameID" in self.stat_:
            ...
            if not name:
                raise FeatureLibError(
                    f"ElidedFallbackNameID {nameID} points " "to a nameID that does not exist in the " '"name" table',
                    None,
                )
    ...
    def makeTable(self, tag):
        ...
        for key, lookups in sorted(self.features_.items(), key=sortFeatureTag):
            ...
            lookup_indices = tuple(dict.fromkeys(l.lookup_index for l in lookups if l.lookup_index is not None))
            ...
            for ix in lookup_indices:
                try:
                    ...
                    warnings.warn(
                        "feaLib.Builder subclass needs upgrading to " "stash debug information. See fonttools#2065."
                    )
    ...
    def add_language_system(self, location, script, language):
        ...
        if script == "DFLT":
            if self.seen_non_DFLT_script_:
                raise FeatureLibError(
                    'languagesystems using the "DFLT" script tag must ' "precede all other languagesystems",
                    location,
                )
        else:
        ...
        if (script, language) in self.default_language_systems_:
            raise FeatureLibError(
                '"languagesystem %s %s" has already been specified' % (script.strip(), language.strip()),
                location,
            )
    ...
    def set_size_parameters(self, location, DesignSize, SubfamilyID, RangeStart, RangeEnd):
        if self.cur_feature_name_ != "size":
            raise FeatureLibError(
                "Parameters statements are not allowed " 'within "feature %s"' % self.cur_feature_name_,
                location,
            )
        ...
        for from_glyph, to_glyph in mapping.items():
            if from_glyph in lookup.mapping:
                if to_glyph == lookup.mapping[from_glyph]:
                    log.info(
                        "Removing duplicate single substitution from glyph" ' "%s" to "%s" at %s',
                        from_glyph,
                        to_glyph,
                        location,
                    )
    ...
    def add_chain_context_subst(self, location, prefix, glyphs, suffix, lookups):
        ...
        lookup.rules.append(ChainContextualRule(prefix, glyphs, suffix, self.find_lookup_builders_(lookups)))
            ...
            for markClassDef in markClass.definitions:
                for mark in markClassDef.glyphs.glyphSet():
                    if mark not in lookupBuilder.marks:
                        otMarkAnchor = self.makeOpenTypeAnchor(location, copy.deepcopy(markClassDef.anchor))
                        lookupBuilder.marks[mark] = (markClass.name, otMarkAnchor)
                    else:
                        existingMarkClass = lookupBuilder.marks[mark][0]
                        if markClass.name != existingMarkClass:
                            raise FeatureLibError(
                                "Glyph %s cannot be in both @%s and @%s" % (mark, existingMarkClass, markClass.name),
                                location,
                            )
    ...
    def add_conditionset(self, location, key, value):
        ...
        if "avar" in self.font:
            ...
            value = {
                axis: tuple(piecewiseLinearMap(v, mapping[axis]) if axis in mapping else v for v in condition_range)
