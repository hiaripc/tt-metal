...
class Parser(object):
    ...
    def __init__(self, featurefile, glyphNames=(), followIncludes=True, includeDir=None, **kwargs):
        if "glyphMap" in kwargs:
            ...
            if glyphNames:
                raise TypeError("'glyphNames' and (deprecated) 'glyphMap' are " "mutually exclusive")
    ...
    def parse(self):
        ...
        if self.missing:
            error = [" %s (first found at %s)" % (name, loc) for name, loc in self.missing.items()]
            raise FeatureLibError(
    ...
    def split_glyph_range_(self, name, location):
        ...
        if len(solutions) == 1:
            ...
            raise FeatureLibError(
                '"%s" is not a glyph in the font, and it can not be split ' "into a range of known glyphs" % name,
                location,
            )
        else:
            ranges = " or ".join(['"%s - %s"' % (s, l) for s, l in solutions])
            raise FeatureLibError(
                'Ambiguous glyph range "%s"; ' "please use %s to clarify what you mean" % (name, ranges),
                location,
            )
    ...
    def parse_glyphclass_(self, accept_glyphname, accept_null=False):
        ...
        while self.next_token_ != "]":
            if self.next_token_type_ is Lexer.NAME:
                ...
                raise FeatureLibError(
                    "Expected glyph name, glyph range, " f"or glyph class reference, found {self.next_token_!r}",
    ...
    def parse_substitute_(self):
        ...
        if not reverse and len(old) == 1 and len(new) == 1 and num_lookups == 0:
            ...
            if len(glyphs) != len(replacements):
                raise FeatureLibError(
                    ...
                    "but found a glyph class with %d elements" % (len(glyphs), len(replacements)),
                    location,
                )
        ...
        if reverse:
            if len(old) != 1:
                raise FeatureLibError(
                    "In reverse chaining single substitutions, " "only a single glyph or glyph class can be replaced",
                    location,
                )
            if len(new) != 1:
                raise FeatureLibError(
        ...
        if len(old) > 1 and len(new) > 1:
            raise FeatureLibError(
                "Direct substitution of multiple glyphs by multiple glyphs " "is not supported",
                location,
            )
    ...
    def parse_STAT_location(self):
        ...
        while self.next_token_ != ";":
            if self.next_token_type_ is Lexer.FLOAT:
                ...
                raise FeatureLibError(
                    f'Unexpected value "{self.next_token_}". ' "Expected integer or float.",
                    self.next_token_location_,
                )
        if len(values) == 3:
            ...
            if nominal < min_val or nominal > max_val:
                raise FeatureLibError(
                    f"Default value {nominal} is outside " f"of specified range " f"{min_val}-{max_val}.",
                    self.next_token_location_,
                )
    ...
    def parse_cvParameters_(self, tag):
        ...
        while self.next_token_ != "}" or self.cur_comments_:
            ...
            if self.cur_token_type_ is Lexer.COMMENT:
                ...
                raise FeatureLibError(
                    "Expected statement: got {} {}".format(self.cur_token_type_, self.cur_token_),
                    self.cur_token_location_,
                )
