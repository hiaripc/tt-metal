...
) = otTables.GPOS.mergeMap = otTables.BASE.mergeMap = otTables.JSTF.mergeMap = otTables.MATH.mergeMap = {
    "*": mergeObjects,
    "Version": max,
}

ttLib.getTableClass("GDEF").mergeMap = ttLib.getTableClass("GSUB").mergeMap = ttLib.getTableClass(
    "GPOS"
).mergeMap = ttLib.getTableClass("BASE").mergeMap = ttLib.getTableClass("JSTF").mergeMap = ttLib.getTableClass(
    "MATH"
).mergeMap = {
...
def layoutPreMerge(font):
    ...
    for t in [GSUB, GPOS]:
        ...
        if t.table.LookupList:
            ...
            if GDEF and GDEF.table.Version >= 0x00010002 and GDEF.table.MarkGlyphSetsDef:
    ...
    for t in [GSUB, GPOS]:
        ...
        if t.table.FeatureList and t.table.ScriptList:
            ...
            t.table.FeatureList.FeatureRecord = [
                f for i, f in enumerate(t.table.FeatureList.FeatureRecord) if i in usedIndices
            ]
