...
def buildLookup(subtables, flags=0, markFilterSet=None):
    ...
    ), "all subtables must have the same LookupType; got %s" % repr([t.LookupType for t in subtables])
    self = ot.Lookup()
    ...
    if markFilterSet is not None:
        ...
        assert (self.LookupFlag & LOOKUP_FLAG_USE_MARK_FILTERING_SET) == 0, (
            "if markFilterSet is None, flags must not set " "LOOKUP_FLAG_USE_MARK_FILTERING_SET; flags=0x%04x" % flags
        )
    return self
...
class ChainContextualBuilder(LookupBuilder):
    ...
    def newRuleSet_(self, format=1, chaining=True):
        st = getattr(ot, self.ruleSetAttr_(format, chaining))()  # ot.ChainPosRuleSet()/ot.SubRuleSet()/etc.
        st.populateDefaults()
        return st

    def newRule_(self, format=1, chaining=True):
        st = getattr(ot, self.ruleAttr_(format, chaining))()  # ot.ChainPosClassRule()/ot.SubClassRule()/etc.
        st.populateDefaults()
        return st
        ...
        for rule in self.rules[::-1]:
            ...
            for sub in rule.lookups:
                if not isinstance(sub, LigatureSubstBuilder):
                    continue
                if all(sub.ligatures.get(seq, replacement) == replacement for seq in itertools.product(*glyphs)):
                    res = sub
        return res
...
def buildPairPosGlyphs(pairs, glyphMap):
    ...
    return [
        buildPairPosGlyphsSubtable(pos, glyphMap, formatA, formatB) for ((formatA, formatB), pos) in sorted(p.items())
    ]
