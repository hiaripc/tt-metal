    ...
    def cost(self):
        if self._cost is None:
            self._cost = (
                ...
                + (self.ctx.valueFormat1_bytes + self.ctx.valueFormat2_bytes) * len(self.indices) * self.width
            )
        return self._cost
...
def cluster_pairs_by_class2_coverage_custom_cost(
    ...
    lines = [
        sum(1 << i if (class1, class2) in pairs else 0 for i, class2 in enumerate(all_class2)) for class1 in all_class1
    ]
