...
@_add_method(otTables.MultipleSubst)
def subset_glyphs(self, s):
    self.mapping = {g: v for g, v in self.mapping.items() if g in s.glyphs and all(sub in s.glyphs for sub in v)}
    return bool(self.mapping)
...
def subset_glyphs(self, s):
    ...
    self.ligatures = {
        g: [seq for seq in seqs if seq.LigGlyph in s.glyphs and all(c in s.glyphs for c in seq.Component)]
        for g, seqs in self.ligatures.items()
    }
...
def closure_glyphs(self, s, cur_glyphs):
    if self.Format == 1:
        ...
        if not indices or not all(c.intersect(s.glyphs) for c in self.LookAheadCoverage + self.BacktrackCoverage):
            return
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        return bool(
            self.GlyphCount and all(c.subset(s.glyphs) for c in self.LookAheadCoverage + self.BacktrackCoverage)
        )
    else:
        assert 0, "unknown format: %s" % self.Format
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        class1_map = [
            c
            for c in self.ClassDef1.subset(s.glyphs.intersection(self.Coverage.glyphs), remap=True)
        ...
        return bool(self.Class1Count and (self.Class2Count > 1) and self.Coverage.subset(s.glyphs))
    else:
        assert 0, "unknown format: %s" % self.Format
        ...
        for i in indices:
            ...
            for r in getattr(rss[i], c.Rule):
                if not r:
                    continue
                if not all(
                    all(c.Intersect(s.glyphs, cd, k) for k in klist) for cd, klist in zip(ContextData, c.RuleData(r))
                ):
                    continue
                chaos = set()
...
def subset_glyphs(self, s):
    ...
    if self.Format == 1:
        ...
        for rs in rss:
            ...
            ss = [r for r in ss if r and all(all(g in s.glyphs for g in glist) for glist in c.RuleData(r))]
        ...
        for rs in rss:
            ...
            ss = [
                r
                for r in ss
                if r and all(all(k in klass_map for k in klist) for klass_map, klist in zip(klass_maps, c.RuleData(r)))
            ]
            setattr(rs, c.Rule, ss)
            setattr(rs, c.RuleCount, len(ss))
            ...
            for r in ss:
                c.SetRuleData(
                    r,
                    [[klass_map.index(k) for k in klist] for klass_map, klist in zip(klass_maps, c.RuleData(r))],
                )
...
def subset_lookups(self, lookup_indices):
    ...
    if self.Format in [1, 2]:
        for rs in getattr(self, c.RuleSet):
            ...
            for r in getattr(rs, c.Rule):
                ...
                setattr(
                    r,
                    c.LookupRecord,
                    [ll for ll in getattr(r, c.LookupRecord) if ll and ll.LookupListIndex in lookup_indices],
                )
                for ll in getattr(r, c.LookupRecord):
                    if not ll:
                        continue
        ...
        setattr(
            self,
            c.LookupRecord,
            [ll for ll in getattr(self, c.LookupRecord) if ll and ll.LookupListIndex in lookup_indices],
        )
...
def subset_lookups(self, lookup_indices):
    ...
    return self.LookupCount or isinstance(self.FeatureParams, otTables.FeatureParamsSize)
...
def collect_lookups(self, feature_indices):
    return sum(
        (self.FeatureRecord[i].Feature.LookupListIndex for i in feature_indices if i < self.FeatureCount),
        [],
    )
...
def subset_lookups(self, lookup_indices):
    """Returns the indices of nonempty features."""
    return [r.FeatureIndex for r in self.SubstitutionRecord if r.Feature.subset_lookups(lookup_indices)]
...
def subset_lookups(self, lookup_indices):
    """Returns the indices of nonempty features."""
    return sum(
        (f.FeatureTableSubstitution.subset_lookups(lookup_indices) for f in self.FeatureVariationRecord),
        [],
    )
...
def subset_features(self, feature_indices):
    self.ensureDecompiled()
    self.SubstitutionRecord = [r for r in self.SubstitutionRecord if r.FeatureIndex in feature_indices]
...
def subset_features(self, feature_indices):
    ...
    while (
        self.FeatureVariationRecord and not self.FeatureVariationRecord[-1].FeatureTableSubstitution.SubstitutionCount
    ):
...
def subset_features(self, feature_indices, keepEmptyDefaultLangSys=False):
    if self.DefaultLangSys and not self.DefaultLangSys.subset_features(feature_indices) and not keepEmptyDefaultLangSys:
...
def subset_features(self, feature_indices, retain_empty):
    ...
    self.ScriptRecord = [
        s for s in self.ScriptRecord if s.Script.subset_features(feature_indices, s.ScriptTag == "DFLT") or retain_empty
    ]
...
def subset_glyphs(self, s):
    strikeData = [{g: strike[g] for g in s.glyphs if g in strike} for strike in self.strikeData]
    # Prune empty strikes
...
def subset_feature_tags(self, feature_tags):
    if self.table.FeatureList:
        feature_indices = [
            i for i, f in enumerate(self.table.FeatureList.FeatureRecord) if f.FeatureTag in feature_tags
        ]
        self.table.FeatureList.subset_features(feature_indices)
        if getattr(self.table, "FeatureVariations", None):
            self.table.FeatureVariations.subset_features(feature_indices)
        ...
        for record in self.table.ScriptList.ScriptRecord:
            if record.ScriptTag in langsys and "*   " not in langsys[record.ScriptTag]:
                record.Script.LangSysRecord = [
                    l for l in record.Script.LangSysRecord if l.LangSysTag in langsys[record.ScriptTag]
                ]
...
def prune_post_subset(self, font, options):
    ...
    if hasattr(table, "FeatureVariations"):
        ...
        if table.FeatureVariations and not (table.FeatureList and table.FeatureVariations.FeatureVariationRecord):
...
def subset_glyphs(self, s):
    ...
    if table.AttachList:
        ...
        table.AttachList.AttachPoint = [table.AttachList.AttachPoint[i] for i in indices if i < GlyphCount]
...
def prune_post_subset(self, font, options):
    ...
    if not hasattr(table, "MarkGlyphSetsDef") or not table.MarkGlyphSetsDef or not table.MarkGlyphSetsDef.Coverage:
...
def subset_glyphs(self, s):
    ...
    table.Anchors = {glyph: table.Anchors[glyph] for glyph in s.glyphs if glyph in table.Anchors}
...
def subset_glyphs(self, s):
    ...
    if table.Format in (1, 3):
        baselines = {glyph: table.BaselineValues.get(glyph, table.DefaultBaseline) for glyph in s.glyphs}
        if len(baselines) > 0:
...
def subset_glyphs(self, s):
    ...
    if table.Format in (0, 1):
        table.Carets = {glyph: table.Carets[glyph] for glyph in s.glyphs if glyph in table.Carets}
...
def subset_glyphs(self, s):
    ...
    if table.Format == 0:
        table.OpticalBoundsDeltas = {
            glyph: table.OpticalBoundsDeltas[glyph] for glyph in s.glyphs if glyph in table.OpticalBoundsDeltas
        }
        return len(table.OpticalBoundsDeltas) > 0
    elif table.Format == 1:
        table.OpticalBoundsPoints = {
            glyph: table.OpticalBoundsPoints[glyph] for glyph in s.glyphs if glyph in table.OpticalBoundsPoints
        }
...
def subset_glyphs(self, s):
    ...
    if prop.Format == 0:
        ...
        prop.Properties = {g: prop.Properties.get(g, prop.DefaultProperties) for g in s.glyphs}
        mostCommon, _cnt = Counter(prop.Properties.values()).most_common(1)[0]
        prop.DefaultProperties = mostCommon
        prop.Properties = {g: prop for g, prop in prop.Properties.items() if prop != mostCommon}
...
def closure_glyphs(self, s):
    if self.version > 0:
        ...
        self.ColorLayersV1 = {rec.BaseGlyph: rec.Paint for rec in self.table.BaseGlyphList.BaseGlyphPaintRecord}
...
def subset_glyphs(self, s):
    ...
    for t in self.tables:
        ...
        if t.format == 14:
            ...
            t.uvsDict = {
                v: [(u, g) for u, g in l if g in s.glyphs_requested or u in s.unicodes_requested]
...
class Options(object):
    ...
    _layout_features_default = _uniq_sort(sum(iter(_layout_features_groups.values()), []))
    ...
    def _prune_pre_subset(self, font):
        for tag in self._sort_tables(font):
            if (
                ...
                or (tag.strip() in self.options.hinting_tables and not self.options.hinting)
    ...
    def _closure_glyphs(self, font):
        ...
        if "VARC" in font:
            with timer("close glyph list over 'VARC'"):
                log.info("Closing glyph list over 'VARC': %d glyphs before", len(self.glyphs))
                log.glyphs(self.glyphs, font=font)
                font["VARC"].closure_glyphs(self)
                self.glyphs.intersection_update(realGlyphs)
                log.info("Closed glyph list over 'VARC': %d glyphs after", len(self.glyphs))
                log.glyphs(self.glyphs, font=font)
...
def main(args=None):
    ...
    if outfile is None:
        ...
        outfile = makeOutputFileName(fontfile, extension=ext, overWrite=True, suffix=".subset")
