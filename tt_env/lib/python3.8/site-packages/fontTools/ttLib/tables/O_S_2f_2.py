...
class table_O_S_2f_2(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        if self.version < 4 and self.fsSelection & 0b1110000000:
            log.warning(
                "fsSelection bits 7, 8 and 9 are only defined in " "OS/2 table version 4 and up: version %s",
                self.version,
            )
...
def _getUnicodeRanges():
    ...
    if not _unicodeStarts:
        unicodeRanges = [
            (start, (stop, bit)) for bit, blocks in enumerate(OS2_UNICODE_RANGES) for _, (start, stop) in blocks
        ]
