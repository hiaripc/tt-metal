...
class Silf(object):
    ...
    def decompile(self, data, ttFont, version=2.0):
        ...
        if numScriptTag:
            self.scriptTags = [
                struct.unpack("4s", data[x : x + 4])[0].decode("ascii") for x in range(1, 1 + 4 * numScriptTag, 4)
            ]
...
class Classes(object):
    ...
    def decompile(self, data, ttFont, version=2.0):
        ...
        for s, e in zip(oClasses[: self.numLinear], oClasses[1 : self.numLinear + 1]):
            self.linear.append(ttFont.getGlyphName(x) for x in struct.unpack((">%dH" % ((e - s) / 2)), data[s:e]))
        for s, e in zip(
...
class Pass(object):
    ...
    def toXML(self, writer, ttFont, version=2.0):
        ...
        wrapline(
            writer,
            ["{}={}".format(*x) for x in sorted(self.colMap.items(), key=lambda x: ttFont.getGlyphID(x[0]))],
        )
