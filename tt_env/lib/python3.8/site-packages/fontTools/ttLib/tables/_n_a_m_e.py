...
class table__n_a_m_e(DefaultTable.DefaultTable):
    ...
    def removeNames(self, nameID=None, platformID=None, platEncID=None, langID=None):
        ...
        if not args:
            ...
            return
        self.names = [
            rec for rec in self.names if any(argValue != getattr(rec, argName) for argName, argValue in args.items())
        ]
...
def _makeMacName(name, nameID, language, font=None):
    ...
    if font is not None:
        ...
        log.warning(
            "cannot store language %s into 'ltag' table " "without having access to the TTFont object" % language
        )
        return None
