        ...
        while True:
            try:
                ...
                if state == RepackerState.HB_FT:
                    ...
                    log.debug(
                        "Re-enabling sharing between extensions and switching back to " "harfbuzz+fontTools packing."
                    )
                    state = RepackerState.HB_FT
    ...
    def iterSubTables(self) -> Iterator[SubTableEntry]:
        ...
        for conv in self.getConverters():
            ...
            if isinstance(value, BaseTable):
                ...
                yield from (
                    self.SubTableEntry(name, v, index=i) for i, v in enumerate(value) if isinstance(v, BaseTable)
                )
