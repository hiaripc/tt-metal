...
class AATLookup(BaseConverter):
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        ...
        assert actualSize == dataSize, "AATLookup format %d claimed to write %d bytes, but wrote %d" % (
            lookupFormat,
            dataSize,
            actualSize,
        )
...
class STXHeader(BaseConverter):
    ...
    def _compilePerGlyphLookups(self, table, font):
        ...
        ), "len(AATStateTable.PerGlyphLookups) is %d, " "but the actions inside the table refer to %d" % (
            len(table.PerGlyphLookups),
            numLookups,
        )
...
class GlyphCIDMap(BaseConverter):
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        items = {font.getGlyphID(g): cid for g, cid in value.items() if cid is not None and cid != 0xFFFF}
        count = max(items) + 1 if items else 0
        writer.writeUShort(count)
...
class CFF2Index(BaseConverter):
    ...
    def write(self, writer, font, tableDict, values, repeatIndex=None):
        ...
        if self._itemClass is not None:
            ...
            items = [self._converter.write(writer, font, tableDict, item, i) for i, item in enumerate(items)]

        offsets = [len(item) for item in items]
        offsets = list(accumulate(offsets, initial=1))

        lastOffset = offsets[-1]
        offSize = 1 if lastOffset < 0x100 else 2 if lastOffset < 0x10000 else 3 if lastOffset < 0x1000000 else 4
