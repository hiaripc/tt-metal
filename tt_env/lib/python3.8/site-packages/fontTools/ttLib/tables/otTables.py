...
def _read_uint32var(data, i):
    ...
    if b0 < 0x80:
        ...
        return (b0 - 0xF0) << 32 | data[i + 1] << 24 | data[i + 2] << 16 | data[i + 3] << 8 | data[i + 4], i + 5
...
class VarComponent:
    ...
    def decompile(self, data, font, localState):
        ...
        def read_transform_component(values):
            ...
            if flags & values.flag:
                v = fi2fl(struct.unpack(">h", data[i : i + 2])[0], values.fractionalBits) * values.scale
                i += 2
                return v
            else:
    ...
    def compile(self, font):
        ...
        if numAxes:
            ...
            data.append(TupleVariation.compileDeltaValues_([fl2fi(v, 14) for v in self.axisValues]))
        else:
            flags &= ~VarComponentFlags.HAVE_AXES
    ...
    def toXML(self, writer, ttFont, attrs):
        ...
        if self.axisIndicesIndex is not None or self.flags & VarComponentFlags.RESET_UNSPECIFIED_AXES:
...
class BaseGlyphRecordArray(BaseTable):
    def preWrite(self, font):
        self.BaseGlyphRecord = sorted(self.BaseGlyphRecord, key=lambda rec: font.getGlyphID(rec.BaseGlyph))
        return self.__dict__.copy()


class BaseGlyphList(BaseTable):
    def preWrite(self, font):
        self.BaseGlyphPaintRecord = sorted(self.BaseGlyphPaintRecord, key=lambda rec: font.getGlyphID(rec.BaseGlyph))
        return self.__dict__.copy()
...
class Paint(getFormatSwitchingBaseTableClass("uint8")):
    ...
    def iterPaintSubTables(self, colr: COLR) -> Iterator[BaseTable.SubTableEntry]:
        if self.Format == PaintFormat.PaintColrLayers:
            ...
            yield from (
                ...
                for i, v in enumerate(layers[self.FirstLayerIndex : self.FirstLayerIndex + self.NumLayers])
            )
            return
    ...
    def getTransform(self) -> Transform:
        if self.Format == PaintFormat.PaintTransform:
            ...
            return (
                Identity.translate(self.centerX, self.centerY).scale(self.scale).translate(-self.centerX, -self.centerY)
            )
