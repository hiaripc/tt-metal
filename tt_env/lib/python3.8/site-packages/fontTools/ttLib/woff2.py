...
class WOFF2GlyfTable(getTableClass("glyf")):
    ...
    def reconstruct(self, data, ttFont):
        ...
        if offset != inputDataSize:
            raise TTLibError(
                "incorrect size of transformed 'glyf' table: expected %d, received %d bytes" % (offset, inputDataSize)
            )
        ...
        if self.glyphOrder is None:
            ...
            if len(self.glyphOrder) != self.numGlyphs:
                raise TTLibError(
                    "incorrect glyphOrder: expected %d glyphs, found %d" % (len(self.glyphOrder), self.numGlyphs)
                )
    ...
    def _encodeTriplets(self, glyph):
        ...
        for i in range(len(coordinates)):
            ...
            if x == 0 and absY < 1280:
                ...
                flags.append(
                    onCurveBit + 84 + 12 * (((absX - 1) & 0x300) >> 8) + (((absY - 1) & 0x300) >> 6) + xySignBits
                )
...
class WOFF2HmtxTable(getTableClass("hmtx")):
    ...
    def transform(self, ttFont):
        ...
        advanceWidthArray = array.array(
            "H",
            [self.metrics[glyphName][0] for i, glyphName in enumerate(glyphOrder) if i < numberOfHMetrics],
        )
        if sys.byteorder != "big":
            advanceWidthArray.byteswap()
        ...
        if hasLsbArray:
            lsbArray = array.array(
                "h",
                [self.metrics[glyphName][1] for i, glyphName in enumerate(glyphOrder) if i < numberOfHMetrics],
            )
            if sys.byteorder != "big":
                lsbArray.byteswap()
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(prog="fonttools ttLib.woff2", description=main.__doc__, add_help=False)
    ...
    parser_compress = parser_group.add_parser("compress", description="Compress a TTF or OTF font to WOFF2")
    parser_decompress = parser_group.add_parser("decompress", description="Decompress a WOFF2 font to OTF")
    ...
    if not options["output_file"]:
        ...
        options["output_file"] = makeOutputFileName(options["input_file"], outputDir=None, extension=extension)
