...
def _add_fvar(font, axes, instances: List[InstanceDescriptor]):
    ...
    for a in axes.values():
        ...
        axis.axisNameID = nameTable.addMultilingualName(a.labelNames, font, minNameID=256, mac=macNames)
    ...
    for instance in instances:
        ...
        inst = NamedInstance()
        inst.subfamilyNameID = nameTable.addMultilingualName(localisedStyleName, mac=macNames)
...
def _add_avar(font, axes, mappings, axisTags):
    ...
    for axis in axes.values():
        ...
        if axis.default not in keys:
            raise VarLibValidationError(
                f"Axis '{axis.name}': there must be a mapping for the axis default " f"value {axis.default}."
            )
        # No duplicate input values (output values can be >= their preceeding value).
        if len(set(keys)) != len(keys):
            raise VarLibValidationError(
                f"Axis '{axis.name}': All axis mapping input='...' values must be " "unique, but we found duplicates."
            )
        # Ascending values
        if sorted(vals) != vals:
...
def _add_gvar(font, masterModel, master_ttfs, tolerance=0.5, optimize=True):
    ...
    master_datas = [
        _MasterData(m["glyf"], m["hmtx"].metrics, getattr(m.get("vmtx"), "metrics", None)) for m in master_ttfs
    ]
...
def _get_advance_metrics(
    ...
    for glyph in glyphOrder:
        vhAdvances = [
            (metrics[glyph][0] if glyph in metrics and metrics[glyph][0] != sparse_advance else None)
            ...
            if tableTag in font:
                ...
                for master in master_ttfs:
                    if tableTag not in master or (
                        tag in specialTags and getattr(master[tableTag], itemName) == specialTags[tag]
                    ):
                        tables.append(None)
                    else:
...
def build_many(
    ...
    doBuildStatFromDSv5 = (
        ...
        and (any(a.axisLabels or a.axisOrdering is not None for a in designspace.axes) or designspace.locationLabels)
    )
...
def build(
    ...
    normalized_master_locs = [{ds.axes[k].tag: v for k, v in loc.items()} for loc in ds.normalized_master_locs]
    ...
    set_default_weight_width_slant(vf, location={axis.axisTag: axis.defaultValue for axis in vf["fvar"].axes})
...
def main(args=None):
    ...
    output_group.add_argument("-o", metavar="OUTPUTFILE", dest="outfile", default=None, help="output file")
