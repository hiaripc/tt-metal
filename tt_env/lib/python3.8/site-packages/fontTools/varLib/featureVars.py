...
def _checkSubstitutionGlyphsExist(glyphNames, substitutions):
    ...
    if missing:
        raise VarLibValidationError(
            "Missing glyphs are referenced in conditional substitution rules:" f" {', '.join(missing)}"
        )
...
def addFeatureVariationsRaw(font, table, conditionalSubstitutions, featureTag="rvrn"):
    ...
    existingTags = {
        feature.FeatureTag for feature in table.FeatureList.FeatureRecord if feature.FeatureTag in featureTags
    }
    ...
    if existingTags:
        ...
        varFeatureIndices.update(
            index for index, feature in enumerate(table.FeatureList.FeatureRecord) if feature.FeatureTag in existingTags
        )
    ...
    for conditionSet, lookupIndices in conditionalSubstitutions:
        ...
        for varFeatureIndex in sorted(varFeatureIndices):
            ...
            combinedLookupIndices = (
                existingLookupIndices + lookupIndices if processLast else lookupIndices + existingLookupIndices
            )
...
def findFeatureVariationRecord(featureVariations, conditionTable):
    ...
    if featureVariations.Version != 0x00010000:
        raise VarLibError(
            "Unsupported FeatureVariations table version: " f"0x{featureVariations.Version:08x} (expected 0x00010000)."
        )
