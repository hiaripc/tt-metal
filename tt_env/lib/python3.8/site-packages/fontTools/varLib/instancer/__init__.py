...
class AxisTriple(Sequence):
    ...
    def __post_init__(self):
        ...
        if (
            ...
            or (self.default is not None and self.maximum is not None and self.default > self.maximum)
...
def verticalMetricsKeptInSync(varfont):
    ...
    current_os2_vmetrics = [
        getattr(varfont["OS/2"], attr) for attr in ("sTypoAscender", "sTypoDescender", "sTypoLineGap")
    ]
...
def instantiateOTL(varfont, axisLimits):
    ...
    if "GDEF" not in varfont or varfont["GDEF"].table.Version < 0x00010003 or not varfont["GDEF"].table.VarStore:
        return
...
def parseArgs(args):
    ...
    parser.add_argument(
        "--update-name-table",
        action="store_true",
        help="Update the instantiated font's `name` table. Input font must have " "a STAT table with Axis Value Tables",
    )
    parser.add_argument(
        "--downgrade-cff2",
        action="store_true",
...
def main(args=None):
    ...
    outfile = makeOutputFileName(infile, overWrite=True, suffix=suffix) if not options.output else options.output
