    ...
    for glyph_name in glyphs:
        ...
        for m1idx in order:
            ...
            for ix, (contour0, contour1) in enumerate(zip(m0Isomorphisms, m1Isomorphisms)):
                ...
                this_tolerance, proposed_point, reverse = test_starting_point(glyph0, glyph1, ix, tolerance, matching)
...
def main(args=None):
    ...
    if len(args.inputs) == 1:
        ...
        if args.inputs[0].endswith(".designspace"):
            ...
            axis_triples = {
                a.name: (a.minimum, a.default, a.maximum) for a in designspace.axes if a.name not in discrete_axes
            }
            axis_mappings = {a.name: a.map for a in designspace.axes}
            axis_triples = {
                k: tuple(piecewiseLinearMap(v, dict(axis_mappings[k])) for v in vv) for k, vv in axis_triples.items()
            }
            ...
            if "gvar" in font:
                ...
                for glyphname in glyphs:
                    for var in gvar.variations[glyphname]:
                        ...
                        recursivelyAddGlyph(glyphname, glyphsets[locTuple], ttGlyphSets[locTuple], glyf)
        ...
        if not args.quiet:
            if args.json:
                ...
                for glyphname, p in problems_gen:
                    ...
                    master_idxs = (p["master_idx"],) if "master_idx" in p else (p["master_1_idx"], p["master_2_idx"])
                    if master_idxs != last_master_idxs:
                        master_names = (p["master"],) if "master" in p else (p["master_1"], p["master_2"])
