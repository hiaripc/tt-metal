...
class Merger(object):
    ...
    def merger(celf, clazzes, attrs=(None,)):
        ...
        def wrapper(method):
            ...
            for clazz in clazzes:
                ...
                for attr in attrs:
                    assert attr not in mergers, "Oops, class '%s' has merge function for '%s' defined already." % (
                        clazz.__name__,
                        attr,
                    )
                    mergers[attr] = method
    ...
    def mergeThings(self, out, lst):
        if not allEqualTo(out, lst, type):
            raise MismatchedTypes(self, expected=type(out).__name__, got=[type(x).__name__ for x in lst])
...
def _Lookup_SinglePos_get_effective_value(merger, subtables, glyph):
    for self in subtables:
        if self is None or type(self) != ot.SinglePos or self.Coverage is None or glyph not in self.Coverage.glyphs:
            continue
        if self.Format == 1:
            return self.Value
        elif self.Format == 2:
...
def merge(merger, self, lst):
    ...
    # Align them
    glyphs, padded = _merge_GlyphOrders(merger.font, [v.Coverage.glyphs for v in lst], [v.Value for v in lst])
    ...
    merger.mergeObjects(self, lst, exclude=("Format", "Coverage", "Value", "ValueCount", "ValueFormat"))
...
def _Lookup_PairPosFormat1_subtables_flatten(lst, font):
    ...
    # Align them
    glyphs, padded = _merge_GlyphOrders(font, [v.Coverage.glyphs for v in lst], [v.PairSet for v in lst])

    self.Coverage.glyphs = glyphs
    self.PairSet = [_PairSet_flatten([v for v in values if v is not None], font) for values in zip(*padded)]
    self.PairSetCount = len(self.PairSet)
    return self
...
def merge(merger, self, lst):
    ...
    if isPairPos:
        ...
        singlePosMapping = {gname: valRecord for gname, valRecord in zip(glyphs, singlePosTable.Value)}
...
def merge(merger, self, lst):
    for name, tableName in [
        ...
        if hasattr(self, name):
            value, deviceTable = buildVarDevTable(merger.store_builder, [getattr(a, name, 0) for a in lst])
            setattr(self, name, value)
            if deviceTable:
...
def merge(merger, self, lst):
    ...
    isVariable = varIndexBase != ot.NO_VARIATION_INDEX or any(id(table) in merger.varTableIds for table in subTables)
