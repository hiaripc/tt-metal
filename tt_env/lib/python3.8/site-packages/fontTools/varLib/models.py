...
def normalizeValue(v, triple, extrapolate=False):
    ...
    if not (lower <= default <= upper):
        raise ValueError(
            f"Invalid axis values, must be minimum, default, maximum: " f"{lower:3.3f}, {default:3.3f}, {upper:3.3f}"
        )
    if not extrapolate:
        ...
        def getKey(axisPoints, axisOrder):
            ...
            def key(loc):
                ...
                return (
                    ...
                    tuple(
                        axisOrder.index(axis) if axis in axisOrder else 0x10000 for axis in orderedAxes
                    ),  # Next, by known axes
