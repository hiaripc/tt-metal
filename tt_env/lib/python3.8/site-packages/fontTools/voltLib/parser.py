...
class Parser(object):
    ...
    def parse_def_lookup_(self):
        ...
        if self.next_token_ == "PROCESS_MARKS":
            ...
            if self.next_token_ == "MARK_GLYPH_SET":
                ...
                raise VoltLibError(
                    "Expected ALL, NONE, MARK_GLYPH_SET or an ID. " "Got %s" % (self.next_token_type_),
                    location,
                )
    ...
    def parse_substitution_(self, reversal):
        ...
        if (max_src > 1 and max_dest > 1) or (reversal and (max_src > 1 or max_dest > 1)):
            raise VoltLibError("Invalid substitution type", location)
        mapping = dict(zip(tuple(src), tuple(dest)))
        if max_src == 1 and max_dest == 1:
            if reversal:
                sub = ast.SubstitutionReverseChainingSingleDefinition(mapping, location=location)
            else:
    ...
    def parse_position_(self):
        ...
        if pos_type not in ("ATTACH", "ATTACH_CURSIVE", "ADJUST_PAIR", "ADJUST_SINGLE"):
            raise VoltLibError("Expected ATTACH, ATTACH_CURSIVE, ADJUST_PAIR, ADJUST_SINGLE", location)
        if pos_type == "ATTACH":
