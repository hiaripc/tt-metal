...
class GithubFileSystem(AbstractFileSystem):
    ...
    def repos(cls, org_or_user, is_org=True):
        ...
        """
        r = requests.get(
            "https://api.github.com/{part}/{org}/repos".format(part=["users", "orgs"][is_org], org=org_or_user)
        )
        r.raise_for_status()
    ...
    def tags(self):
        ...
        r = requests.get(
            "https://api.github.com/repos/{org}/{repo}/tags" "".format(org=self.org, repo=self.repo),
            **self.kw,
        )
        r.raise_for_status()
    ...
    def branches(self):
        ...
        r = requests.get(
            "https://api.github.com/repos/{org}/{repo}/branches" "".format(org=self.org, repo=self.repo),
            **self.kw,
        )
        r.raise_for_status()
        return [t["name"] for t in r.json()]
    ...
    def ls(self, path, detail=False, sha=None, _sha=None, **kwargs):
        ...
        if path not in self.dircache or sha not in [self.root, None]:
            r = requests.get(self.url.format(org=self.org, repo=self.repo, sha=_sha), **self.kw)
