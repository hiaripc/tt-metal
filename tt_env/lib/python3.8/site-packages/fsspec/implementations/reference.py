...
class LazyReferenceMapper(collections.abc.MutableMapping):
            ...
            fileinfo = [
                {
                    "name": name,
                    "type": "file",
                    "size": len(json.dumps(self.zmetadata[name]) if name in self.zmetadata else self._items[name]),
                }
                for name in others
            ]
    ...
    def __setitem__(self, key, value):
        if "/" in key and not self._is_meta(key):
            ...
            self.zmetadata[key] = json.loads(value.decode() if isinstance(value, bytes) else value)
    ...
    def write(self, field, record, base_url=None, storage_options=None):
        ...
        df.to_parquet(
            fn,
            engine="fastparquet",
            storage_options=storage_options or getattr(self.fs, "storage_options", None),
            compression="zstd",
            index=False,
            stats=False,
...
class ReferenceFileSystem(AsyncFileSystem):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(fs, dict):
            self.fss = {
                k: (fsspec.filesystem(k.split(":", 1)[0], **opts) if isinstance(opts, dict) else opts)
    ...
    def _process_gen(self, gens):
        ...
        for gen in gens:
            dimension = {
                k: v if isinstance(v, list) else range(v.get("start", 0), v["stop"], v.get("step", 1))
    ...
    def _dircache_from_items(self):
        ...
        for path, part in it:
            ...
            while par0 and par0 not in self.dircache:
                ...
                self.dircache[par0] = []
                self.dircache.setdefault(par0.rsplit("/", 1)[0] if "/" in par0 else "", []).append(
    ...
    async def _rm_file(self, path, **kwargs):
        self.references.pop(path, None)  # ignores FileNotFound, just as well for directories
