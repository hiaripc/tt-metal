...
def promote_pathlike_directory(
    directory: typing.Union[os.PathLike, str, None],
    /,
    *,
    ...
    See also:
        https://docs.python.org/3/glossary.html#term-path-like-object
    """
    return pathlib.Path(directory if directory is not None else default or os.curdir)


def deprecate_positional_args(
    *, supported_number: int, category: typing.Type[Warning] = PendingDeprecationWarning, stacklevel: int = 1
):
    ...
    def decorator(func):
        ...
        log.debug(
            "deprecate positional args: %s.%s(%r)", func.__module__, func.__qualname__, argnames[supported_number:]
        )
        ...
        def wrapper(*args, **kwargs):
            if len(args) > supported_number:
                ...
                wanted = ", ".join(f"{name}={value!r}" for name, value in deprecated.items())
                warnings.warn(
                    f"The signature of {func.__name__} will be reduced"
