...
class Dot(quoting.Quote, base.Base):
    ...
    def node(self, name: str, label: typing.Optional[str] = None, _attributes=None, **attrs) -> None:
        """Create a node.
    ...
    def edge(
        self, tail_name: str, head_name: str, label: typing.Optional[str] = None, _attributes=None, **attrs
    ) -> None:
    ...
    def edges(self, tail_head_iter) -> None:
        ...
        self.body += [edge(tail=quote(t), head=quote(h)) for t, h in tail_head_iter]

    @_tools.deprecate_positional_args(supported_number=2)
    def attr(self, kw: typing.Optional[str] = None, _attributes=None, **attrs) -> None:
        ...
        if kw is not None and kw.lower() not in ("graph", "node", "edge"):
            raise ValueError("attr statement must target graph, node, or edge:" f" {kw!r}")
    ...
    def subgraph(
        ...
        if graph.directed != self.directed:
            raise ValueError(f"{self!r} cannot add subgraph of different kind:" f" {graph!r}")
