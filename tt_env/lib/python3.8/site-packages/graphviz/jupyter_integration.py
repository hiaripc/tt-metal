...
JUPYTER_FORMATS = {"jpeg": _IMAGE_JPEG, "jpg": _IMAGE_JPEG, "png": "image/png", "svg": "image/svg+xml"}

SUPPORTED_JUPYTER_FORMATS = set(JUPYTER_FORMATS)

DEFAULT_JUPYTER_FORMAT = next(_ for _ in SUPPORTED_JUPYTER_FORMATS if _ == "svg")

MIME_TYPES = {"image/jpeg": "_repr_image_jpeg", "image/png": "_repr_image_png", "image/svg+xml": "_repr_image_svg_xml"}
...
def get_jupyter_format_mimetype(jupyter_format: str) -> str:
    try:
        ...
        raise ValueError(f"unknown jupyter_format: {jupyter_format!r}" f" (must be one of {sorted(JUPYTER_FORMATS)})")


def get_jupyter_mimetype_format(mimetype: str) -> str:
    if mimetype not in MIME_TYPES:
        raise ValueError(f"unsupported mimetype: {mimetype!r}" f" (must be one of {sorted(MIME_TYPES)})")
...
class JupyterIntegration(piping.Pipe):
    ...
    def _repr_mimebundle_(
        ...
        **_,
        ...
        return {
            mimetype: getattr(self, method_name)()
