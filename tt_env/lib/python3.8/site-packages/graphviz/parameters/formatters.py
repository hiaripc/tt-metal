...
def verify_formatter(formatter: typing.Optional[str], *, required: bool = REQUIRED) -> None:
    if formatter is None:
        if required:
            raise ValueError("missing formatter")
    elif formatter.lower() not in FORMATTERS:
        raise ValueError(f"unknown formatter: {formatter!r}" f" (must be None or one of {sorted(FORMATTERS)})")
