...
def verify_renderer(renderer: typing.Optional[str], *, required: bool = REQUIRED) -> None:
    if renderer is None:
        if required:
            raise ValueError("missing renderer")
    elif renderer.lower() not in RENDERERS:
        raise ValueError(f"unknown renderer: {renderer!r}" f" (must be None or one of {sorted(RENDERERS)})")
