...
def loop_tk(kernel):
    ...
    if hasattr(app, "createfilehandler"):
        ...
        app.tk.createfilehandler(kernel.shell_stream.getsockopt(zmq.FD), READABLE, process_stream_events)
...
def set_qt_api_env_from_gui(gui):
    ...
    if qt_api is not None and gui != "qt":
        if qt_env2gui[qt_api] != gui:
            print(f'Request for "{gui}" will be ignored because `QT_API` ' f'environment variable is set to "{qt_api}"')
            return
    else:
        if gui == "qt5":
            try:
...
def enable_gui(gui, kernel=None):
    ...
    if kernel is None:
        ...
        if kernel is None:
            msg = "You didn't specify a kernel," " and no IPython Application with a kernel appears to be running."
