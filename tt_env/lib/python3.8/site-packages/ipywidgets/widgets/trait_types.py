...
_color_rgbhsl = "({})|({})|({})|({})".format(_color_rgb, _color_rgba, _color_hsl, _color_hsla).format(
    ip=_color_int_percent, fp=_color_frac_percent
)
...
_custom_ident = rf"--([a-zA-Z0-9_\-{_non_ascii}]|{_escape})+"
...
_css_color = rf'({"|".join(_color_names)}|({_color_rgbhsl})|({_color_hex})|({_color_hexa}))'
_css_var_fallback_color = rf"var\({_custom_ident}(,\s*({_css_color}\s*)?)?\)"
...
def date_to_json(pydate, manager):
    ...
    if pydate is None:
        ...
        return dict(year=pydate.year, month=pydate.month - 1, date=pydate.day)  # Months are 0-based indices in JS
...
_number_format_types = {"e", "f", "g", "r", "s", "%", "p", "b", "o", "d", "x", "X", "c", ""}
