...
class Widget(LoggingHasTraits):
    ...
    def widgets():
        ...
        frame = _get_frame(2)
        if not (
            frame.f_code.co_filename == TRAITLETS_FILE
            and (frame.f_code.co_name in ("getmembers", "setup_instance", "setup_class"))
        ):
    ...
    def handle_control_comm_opened(cls, comm, msg):
        ...
        if version.split(".")[0] != CONTROL_PROTOCOL_VERSION_MAJOR:
            raise ValueError(
                "Incompatible widget control protocol versions: received version %r, expected version %r"
                % (version, __control_protocol_version__)
            )
    ...
    def _handle_control_comm_msg(cls, msg):
        ...
        if method == "request_states":
            ...
            cls._control_comm.send(
                dict(method="update_states", states=full_state, buffer_paths=buffer_paths), buffers=buffers
            )
    ...
    def _get_embed_state(self, drop_defaults=False):
        ...
        if len(buffers) > 0:
            state["buffers"] = [
                {"encoding": "base64", "path": p, "data": standard_b64encode(d).decode("ascii")}
    ...
    # Traits
    # -------------------------------------------------------------------------
    _model_name = Unicode("WidgetModel", help="Name of the model.", read_only=True).tag(sync=True)
    _model_module = Unicode("@jupyter-widgets/base", help="The namespace for the model.", read_only=True).tag(sync=True)
    ...
    _view_module = Unicode(None, allow_none=True, help="The namespace for the view.").tag(sync=True)
    _view_module_version = Unicode("", help="A semver requirement for the namespace version containing the view.").tag(
        sync=True
    )
