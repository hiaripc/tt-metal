...
from traitlets import Unicode, Bool, Int, Any, Dict, TraitError, CaselessStrEnum, Tuple, Union, observe, validate
...
class ToggleButtons(_Selection):
    ...
    icons = TypedTuple(Unicode(), help="Icons names for each button (FontAwesome names without the fa- prefix).").tag(
        sync=True
    )
...
class SelectionSlider(_SelectionNonempty):
    ...
    orientation = CaselessStrEnum(
        values=["horizontal", "vertical"], default_value="horizontal", help="Vertical or horizontal."
    ).tag(sync=True)
    readout = Bool(True, help="Display the current selected label next to the slider").tag(sync=True)
    continuous_update = Bool(True, help="Update the value of the widget as the user is holding the slider.").tag(
        sync=True
    )
