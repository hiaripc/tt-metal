    ...
    def _get_docstring(self):
        if self._cached_name is not None:
            return completion_cache.get_docstring(
                self._cached_name, self._name.get_public_name(), lambda: self._get_cache()
            )
...
class Name(BaseName):
    ...
    *Name* objects are returned from many different APIs including
    ...
    def defined_names(self):
        ...
        return sorted(
            unite(defined_names(self._inference_state, d) for d in defs), key=lambda s: s._name.start_pos or (0, 0)
        )
