    ...
    while leaf is not None:
        if leaf.type == "error_leaf" and ('"' in leaf.value or "'" in leaf.value):
            ...
            return (
                ...
                prefix_leaf or leaf,
                ("" if prefix_leaf is None else prefix_leaf.value) + cut_value_at_position(leaf, position),
            )
        if leaf.line != position[0]:
...
def extract_imported_names(node):
    ...
    if node.type in ["import_as_names", "dotted_as_names", "import_as_name"]:
        for index, child in enumerate(node.children):
            if child.type == "name":
                if index > 0 and node.children[index - 1].type == "keyword" and node.children[index - 1].value == "as":
                    continue
