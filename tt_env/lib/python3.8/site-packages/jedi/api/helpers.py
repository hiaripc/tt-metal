        ...
        for i, param_name in enumerate(param_names):
            ...
            if key_start is not None and not star_count == 1 or star_count == 2:
                if param_name.string_name not in used_names and (
                    kind == Parameter.KEYWORD_ONLY or kind == Parameter.POSITIONAL_OR_KEYWORD and positional_count <= i
                ):
                    if star_count:
                        return i
                    if had_equal:
        ...
        if node.type == "trailer" and node.children[0] == "(" or node.type == "decorator" and node.children[2] == "(":
            ...
            if not (leaf is node.children[-1] and position >= leaf.end_pos):
                ...
                if leaf is None:
                    return None
                return CallDetails(
                    node.children[0] if node.type == "trailer" else node.children[2], node.children, position
                )
