...
_VARIABLE_EXCTRACTABLE = (
    EXPRESSION_PARTS
    + ("atom testlist_star_expr testlist test lambdef lambdef_nocond " "keyword name number string fstring").split()
)
...
def extract_function(inference_state, path, module_context, name, pos, until_pos):
    ...
    if is_expression:
        ...
        if not has_ending_return_stmt:
            ...
            # defined variable.
            return_variables = (
                list(_find_needed_output_variables(context, nodes[0].parent, nodes[-1].end_pos, return_variables))
    ...
    function_call = "%s(%s)" % (("" if self_param is None else self_param + ".") + name, ", ".join(params))
    if is_expression:
    ...
    replacement_dct = _replace(nodes, replacement, function_code, pos, insert_before_leaf, remaining_prefix)
    if not is_expression:
