...
def _load_module(inference_state, path):
    return inference_state.parse(
        path=path, cache=True, diff_cache=settings.fast_parser, cache_path=settings.cache_directory
    ).get_root_node()
...
def _create(inference_state, compiled_value, module_context):
    ...
    return ValueSet(MixedObject(compiled_value, tree_value=tree_value) for tree_value in tree_values)
