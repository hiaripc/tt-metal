...
class CompiledValue(Value):
    ...
    def py__mro__(self):
        return (self,) + tuple(
            create_from_access_path(self.inference_state, access) for access in self.access_handle.py__mro__accesses()
        )

    @CheckAttribute()
    def py__bases__(self):
        return tuple(
            create_from_access_path(self.inference_state, access) for access in self.access_handle.py__bases__()
        )
        ...
        with reraise_getitem_errors(IndexError, KeyError, TypeError):
            try:
                access = self.access_handle.py__simple_getitem__(
                    index, safe=not self.inference_state.allow_unsafe_executions
                )
            except AttributeError:
            ...
            return ValueSet(
                [
                    create_from_access_path(
                        self.inference_state, self.access_handle.execute_operation(other.access_handle, operator)
                    )
                ]
            )
        except TypeError:
    ...
    def execute_annotation(self):
        ...
        if name == "Union":
            ...
            # objects.
            return ValueSet(
                [v.with_generics(arguments) for v in self.inference_state.typing_module.py__getattribute__(name)]
            ).execute_annotation()
...
def create_from_access_path(inference_state, access_path):
    ...
    for name, access in access_path.accesses:
        value = create_cached_compiled_value(
            inference_state, access, parent_context=None if value is None else value.as_context()
        )
    return value
