...
def _infer_annotation_string(context, string, index=None):
    ...
    if index is not None:
        value_set = value_set.filter(
            lambda value: (value.array_type == "tuple" and len(list(value.py__iter__())) >= index)
...
def _infer_param(function_value, param):
    ...
    if annotation is None:
        ...
        if len(params_comments) != len(all_params):
            debug.warning("Comments length != Params length %s %s", params_comments, all_params)
        ...
        return _infer_annotation_string(function_value.get_default_param_context(), param_comment)
...
def infer_return_types(function, arguments):
    ...
    return ValueSet.from_sets(
        ann.define_generics(type_var_dict) if isinstance(ann, (DefineGenericBaseClass, TypeVar)) else ValueSet({ann})
...
def infer_return_for_callable(arguments, param_values, result_values):
    ...
    return ValueSet.from_sets(
        v.define_generics(all_type_vars) if isinstance(v, (DefineGenericBaseClass, TypeVar)) else ValueSet({v})
