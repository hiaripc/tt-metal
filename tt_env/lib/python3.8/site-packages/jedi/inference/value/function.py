...
class BaseFunctionExecutionContext(ValueContext, TreeContextMixin):
    ...
    def merge_yield_values(self, is_async=False):
        return ValueSet.from_sets(lazy_value.infer() for lazy_value in self.get_yield_lazy_values())
    ...
    def infer(self):
        ...
        if is_coroutine:
            if self.is_generator():
                ...
                return ValueSet(
                    GenericClass(c, TupleGenericManager(generics)) for c in async_generator_classes
                ).execute_annotation()
            else:
