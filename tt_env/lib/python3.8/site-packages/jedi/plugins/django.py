...
def tree_name_to_values(func):
    def wrapper(inference_state, context, tree_name):
        ...
        if tree_name.value in _FILTER_LIKE_METHODS:
            ...
            for v in result:
                if (
                    ...
                    if len(generics) >= 1:
                        return ValueSet(QuerySetMethodWrapper(v, model) for model in generics[0])
        ...
        elif tree_name.value == "Field" and context.is_module() and context.py__name__() == "django.db.models.fields":
...
class GenericManagerWrapper(AttributeOverwrite, ClassMixin):
    def py__get__on_class(self, calling_instance, instance, class_value):
        return calling_instance.class_value.with_generics((ValueSet({class_value}),)).py__call__(
            calling_instance._arguments
        )
