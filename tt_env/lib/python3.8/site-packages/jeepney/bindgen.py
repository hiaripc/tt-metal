...
class Method:
    ...
    def make_code(self):
        ...
        return (
            ...
            "                           {tuple})\n"
        ).format(name=self.name, args=args, signature=signature, tuple=tuple)
...
class Interface:
    ...
    def make_code(self):
        ...
        chunks = [
            INTERFACE_CLASS_TEMPLATE.format(
                cls_name=cls_name,
                interface=self.name,
...
def main():
    ...
    ap.add_argument("-n", "--name", help="Bus name to introspect, required unless using file")
    ap.add_argument(
        "-p",
        "--path",
    ...
    ap.add_argument(
        "--bus", default="SESSION", help="Bus to connect to for introspection (SESSION/SYSTEM), default SESSION"
    )
    ap.add_argument("-f", "--file", help="XML file to use instead of D-Bus introspection")
