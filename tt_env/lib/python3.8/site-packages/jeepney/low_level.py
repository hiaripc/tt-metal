...
class FileDescriptor(FixedType):
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        if isinstance(data, bool) or not isinstance(data, int):
            raise TypeError(
                "Cannot use {data!r} as file descriptor. Expected " "an int or an object with fileno() method"
            )
...
class StringType:
    ...
    def __eq__(self, other):
        return (type(other) is StringType) and (self.length_type == other.length_type)
...
class ObjectPathType(StringType):
    ...
    def check_data(self, data):
        ...
        if any(c not in valid_chars for c in data):
            raise ValueError(f"Object path ({data!r}) can only contain A-Z, a-z, 0-9, / and _")
...
class Array:
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        if data_is_bytes:
            ...
            chunks = []
            for item in data:
                chunks.append(self.elt_type.serialise(item, data_pos, endianness, fds=fds))
...
class Variant:
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        return sig_buf + valtype.serialise(data, pos + len(sig_buf), endianness, fds=fds)
...
class Message:
    ...
    def from_buffer(cls, buf: bytes, fds=()) -> "Message":
        ...
        if n_fds > len(fds):
            raise ValueError(f"Message expects {n_fds} FDs, but only {len(fds)} were received")
        fds = fds[:n_fds]
        body = ()
