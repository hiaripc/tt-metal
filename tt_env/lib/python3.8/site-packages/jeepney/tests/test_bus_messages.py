...
def test_match_rule_path_namespace():
    ...
    assert "/freedesktop/" in (MatchRule(path_namespace="/org/freedesktop/portal").serialise())
...
def test_match_rule_arg_namespace():
    ...
    # Exact match
    assert rule.matches(new_signal(message_bus, "NameOwnerChanged", "s", ("com.example.backend1",)))

    # Parent of the name
    assert rule.matches(new_signal(message_bus, "NameOwnerChanged", "s", ("com.example.backend1.foo.bar",)))

    # Prefix but not a parent in the namespace
    assert not rule.matches(new_signal(message_bus, "NameOwnerChanged", "s", ("com.example.backend12",)))

    # Not a string
    assert not rule.matches(new_signal(message_bus, "NameOwnerChanged", "u", (1,)))
