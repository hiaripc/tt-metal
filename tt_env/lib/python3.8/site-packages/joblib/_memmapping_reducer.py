...
def add_maybe_unlink_finalizer(memmap):
    util.debug(
        ...
        "".format(type(memmap), id(memmap), os.path.basename(memmap.filename), os.getpid())
    )
...
def _reduce_memmap_backed(a, m):
    ...
    return (
        ...
        (m.filename, a.dtype, m.mode, offset, order, a.shape, strides, total_buffer_len, False),
    )
    ...
    **kwargs,
    ...
    if np is not None:
        ...
        forward_reduce_ndarray = ArrayMemmapForwardReducer(
            max_nbytes, temp_folder_resolver, mmap_mode, unlink_on_gc_collect, verbose, prewarm=prewarm
        )
