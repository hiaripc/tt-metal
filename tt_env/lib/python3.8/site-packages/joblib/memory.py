...
def _store_backend_factory(backend, location, verbose=0, backend_options=None):
    ...
    if isinstance(location, StoreBackendBase):
        ...
        warnings.warn(
            ...
            "supported by joblib. Returning None instead.".format(location.__class__.__name__),
            UserWarning,
        )
    ...
    def get(self):
        ...
        try:
            return self.store_backend.load_item(
                self._call_id, timestamp=self.timestamp, metadata=self.metadata, verbose=self.verbose
            )
...
class MemorizedFunc(Logger):
    ...
    def _cached_call(self, args, kwargs, shelving):
        ...
        if self._is_in_cache_and_valid(call_id):
            ...
            try:
                ...
                if self._verbose > 4:
                    self._print_duration(time.time() - start_time, context="cache loaded ")
                return output, {}
            except Exception:
        ...
        if self._verbose > 10:
            self.warn(f"Computing func {func_name}, argument hash {args_id} " f"in location {location}")
    ...
    def _check_previous_func_code(self, stacklevel=2):
        ...
        try:
            old_func_code, old_first_line = extract_first_line(self.store_backend.get_cached_func_code([self.func_id]))
        ...
        if old_first_line == first_line == -1 or func_name == "<lambda>":
            ...
            warnings.warn(
                JobLibCollisionWarning("Cannot detect name collisions for function '{0}'".format(func_description)),
            ...
            if possible_collision:
                warnings.warn(
                    JobLibCollisionWarning(
                        ...
                        "'%s' (%s:%i) and '%s' (%s:%i)"
                        % (func_name, source_file, old_first_line, func_name, source_file, first_line)
                    ),
                    stacklevel=stacklevel,
                )
        ...
        if self._verbose > 10:
            _, func_name = get_func_name(self.func, resolv_alias=False)
            self.warn("Function {0} (identified by {1}) has changed" ".".format(func_name, self.func_id))
    ...
    def _get_memorized_result(self, call_id, metadata=None):
        return MemorizedResult(
            self.store_backend, call_id, metadata=metadata, timestamp=self.timestamp, verbose=self._verbose - 1
        )
...
class Memory(Logger):
    ...
    def __init__(
        ...
        self.store_backend = _store_backend_factory(
            ...
            backend_options=dict(compress=compress, mmap_mode=mmap_mode, **backend_options),
        )
    ...
    def cache(self, func=None, ignore=None, verbose=None, mmap_mode=False, cache_validation_callback=None):
        ...
        """
        if cache_validation_callback is not None and not callable(cache_validation_callback):
            raise ValueError("cache_validation_callback needs to be callable. " f"Got {cache_validation_callback}.")
        if func is None:
        ...
        if self.store_backend is None:
            cls = AsyncNotMemorizedFunc if asyncio.iscoroutinefunction(func) else NotMemorizedFunc
            return cls(func)
        if verbose is None:
            verbose = self._verbose
    ...
    def __repr__(self):
        return "{class_name}(location={location})".format(
            class_name=self.__class__.__name__,
            location=(None if self.store_backend is None else self.store_backend.location),
        )
