...
def _get_active_backend(
    ...
    if prefer not in VALID_BACKEND_HINTS:
        raise ValueError(f"prefer={prefer} is not a valid backend hint, " f"expected one of {VALID_BACKEND_HINTS}")
    if require not in VALID_BACKEND_CONSTRAINTS:
        raise ValueError(
            f"require={require} is not a valid backend constraint, " f"expected one of {VALID_BACKEND_CONSTRAINTS}"
        )
        ...
        if isinstance(backend, str):
            ...
            if backend not in BACKENDS:
                if backend in EXTERNAL_BACKENDS:
                    ...
                    raise ValueError(f"Invalid backend: {backend}, expected one of " f"{sorted(BACKENDS.keys())}")
    ...
    * Ability to use shared memory efficiently with worker
    ...
    def __init__(
        ...
        if backend is default_parallel_config["backend"] or backend is None:
            ...
            warnings.warn(
                f"joblib backend '{backend}' is not available on " f"your system, falling back to {DEFAULT_BACKEND}.",
                UserWarning,
                stacklevel=2,
            )
            BACKENDS[backend] = BACKENDS[DEFAULT_BACKEND]
        ...
        try:
            ...
            if self.timeout is not None and not self._backend.supports_timeout:
                warnings.warn(
                    ...
                    "the 'timeout' parameter will not be used.".format(self._backend.__class__.__name__, self.timeout)
                )
    ...
    def print_progress(self):
        ...
        # tasks.
            ...
            if _verbosity_filter(self.n_dispatched_batches, self.verbose):
                return
            self._print(f"Done {self.n_completed_tasks:3d} tasks      | elapsed: " f"{short_format_time(elapsed_time)}")
        else:
    ...
    def __call__(self, iterable):
        ...
        if isinstance(self._backend, LokyBackend):
            ...
            def _batched_calls_reducer_callback():
                ...
                self._backend._workers._temp_folder_manager.set_current_context(self._id)  # noqa
