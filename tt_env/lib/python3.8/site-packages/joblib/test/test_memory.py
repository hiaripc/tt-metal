...
def test_memory_objects_repr(tmpdir):
    ...
    assert str(memorized_result) == memorized_result_repr.format(
        location=memory.store_backend.location, func=memorized_result.func_id, args_id=memorized_result.args_id
    )
...
@pytest.mark.parametrize(
    "memory_kwargs",
    [{"compress": 3, "verbose": 2}, {"mmap_mode": "r", "verbose": 5, "backend_options": {"parameter": "unused"}}],
)
def test_memory_pickle_dump_load(tmpdir, memory_kwargs):
    ...
    compare(
        memorized_result, memorized_result_reloaded, ignored_attrs=set(["store_backend", "timestamp", "_func_code_id"])
    )
...
def test_deprecated_bytes_limit(tmpdir):
    ...
    if __version__ >= "1.5":
        raise DeprecationWarning("Bytes limit is deprecated and should be removed by 1.4")
