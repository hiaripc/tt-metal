...
def exception_raiser(x, custom_exception=False):
    if x == 7:
        raise (MyExceptionWithFinickyInit("a", "b", "c", "d") if custom_exception else ValueError)
    return x
...
def test_simple_parallel(backend, n_jobs, verbose):
    assert [square(x) for x in range(5)] == Parallel(n_jobs=n_jobs, backend=backend, verbose=verbose)(
    ...
    if expected:
        if warninfo:
            ...
            warnings_have_the_right_length = (
                len(warninfo) >= 1 if getattr(sys.flags, "nogil", False) else len(warninfo) == 1
            )
...
def test_nested_parallel_warnings(parent_backend, child_backend, expected):
    ...
    Parallel(n_jobs=2, backend=parent_backend)(
        delayed(_assert_warning_nested)(backend=child_backend, inner_n_jobs=1, expected=False) for _ in range(5)
    )

    #  warnings if inner_n_jobs != 1 and expected
    res = Parallel(n_jobs=2, backend=parent_backend)(
        delayed(_assert_warning_nested)(backend=child_backend, inner_n_jobs=2, expected=expected) for _ in range(5)
    )
...
def test_parallel_pickling():
    ...
    with raises(PicklingError, match=r"the task to send"):
        Parallel(n_jobs=2, backend="loky")(delayed(id)(UnpicklableObject()) for _ in range(10))
...
def test_parallel_byteorder_corruption(byteorder):
    ...
    result = Parallel(n_jobs=2, backend="loky")(delayed(inspect_byteorder)(x) for _ in range(3))
...
def test_backend_context_manager(monkeypatch, backend, context):
    ...
    Parallel(n_jobs=2, backend="threading")(
        delayed(check_backend_context_manager)(context, b) for b in all_backends_for_context_manager if not b
    )
...
def test_nested_backend_in_sequential(backend, n_jobs, context):
    ...
    Parallel(n_jobs=1)(delayed(check_nested_backend)(DEFAULT_BACKEND, 1) for _ in range(10))

    with context(backend, n_jobs=n_jobs):
        Parallel(n_jobs=1)(delayed(check_nested_backend)(backend, n_jobs) for _ in range(10))
...
def test_backend_nesting_level(context, outer_backend, inner_backend):
    ...
    with context(inner_backend, n_jobs=2):
        Parallel()(delayed(check_nesting_level)(context, inner_backend, 1) for _ in range(10))
...
def test_parallel_with_interactively_defined_functions(backend):
    ...
    if backend == "multiprocessing" and mp.get_start_method() != "fork":
        pytest.skip("Require fork start method to use interactively defined " "functions with multiprocessing.")
    code = UNPICKLABLE_CALLABLE_SCRIPT_TEMPLATE_NO_MAIN.format(backend)
    check_subprocess_call([sys.executable, "-c", code], timeout=10, stdout_regex=r"\[0, 1, 4, 9, 16\]")
...
def test_parallel_with_unpicklable_functions_in_args(backend, define_func, callable_position, tmpdir):
    ...
    check_subprocess_call([sys.executable, code_file.strpath], timeout=10, stdout_regex=r"\[0, 1, 4, 9, 16\]")
...
def test_multiple_generator_call(backend, return_as, n_jobs):
    ...
    assert time.time() - t_start < 2, (
        "The error should be raised immediatly when submitting a new task " "but it took more than 2s."
    )
...
def _recursive_backend_info(limit=3, **kwargs):
    ...
    with Parallel(n_jobs=2) as p:
        ...
        if limit == 0:
            return this_level
        results = p(delayed(_recursive_backend_info)(limit=limit - 1, **kwargs) for i in range(1))
...
def check_child_num_threads(workers_info, parent_info, num_threads):
    ...
    for child_threadpool_info in workers_info:
        for child_module in child_threadpool_info:
            parent_max_num_threads = _parent_max_num_threads_for(child_module, parent_info)
...
def test_threadpool_limitation_in_child_loky(n_jobs):
    ...
    check_child_num_threads(workers_threadpool_infos, parent_info, expected_child_num_threads)
...
def test_threadpool_limitation_in_child_context(context, n_jobs, inner_max_num_threads):
    ...
    with context("loky", inner_max_num_threads=inner_max_num_threads):
        workers_threadpool_infos = Parallel(n_jobs=n_jobs)(delayed(_check_numpy_threadpool_limits)() for i in range(2))
