...
class JSON5Encoder:
    ...
    def _encode_str(self, obj: str, *, as_key: bool) -> str:
        if as_key and self.is_identifier(obj) and not self.quote_keys and not self.is_reserved_word(obj):
            return obj
    ...
    def _encode_array(self, obj: Any, seen: Set, level: int) -> str:
        ...
        item_sep = self.item_separator + indent_str
        return (
            "[" + indent_str + item_sep.join(self.encode(el, seen, level, as_key=False) for el in obj) + end_str + "]"
        )
