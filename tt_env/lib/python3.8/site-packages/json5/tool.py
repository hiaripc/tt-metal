...
def _parse_args(host, argv):
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="quote_keys",
        help="don't quote object keys that are identifiers" " (this is the default)",
    )
    parser.add_argument(
        "--trailing-commas",
        action="store_true",
        default=True,
        help="add commas after the last item in multi-line " "objects and arrays (this is the default)",
    )
    parser.add_argument(
        "--no-trailing-commas",
        dest="trailing_commas",
        action="store_false",
        help="do not add commas after the last item in " "multi-line lists and objects",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Controls how strings are encoded. By default they are always " 'double-quoted ("always_double")',
    )
    parser.add_argument(
        "file",
        metavar="FILE",
        nargs="?",
        default="-",
        help="optional file to read JSON5 document from; if " 'not specified or "-", will read from stdin ' "instead",
    )
