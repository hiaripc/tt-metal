    ...
    for each in instance:
        if contains_validator.is_valid(each):
            ...
            if matches > max_contains:
                yield ValidationError(
                    "Too many items match the given schema " f"(expected at most {max_contains})",
...
def minProperties(validator, mP, instance, schema):
    if validator.is_type(instance, "object") and len(instance) < mP:
        message = "should be non-empty" if mP == 1 else "does not have enough properties"
        yield ValidationError(f"{instance!r} {message}")
...
def oneOf(validator, oneOf, instance, schema):
    ...
    more_valid = [each for _, each in subschemas if validator.evolve(schema=each).is_valid(instance)]
    if more_valid:
