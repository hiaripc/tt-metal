...
def write_connection_file(
    ...
    **kwargs: Any,
    ...
    ports_needed = (
        int(shell_port <= 0) + int(iopub_port <= 0) + int(stdin_port <= 0) + int(control_port <= 0) + int(hb_port <= 0)
    )
    if transport == "tcp":
...
def find_connection_file(
    ...
    if profile is not None:
        warnings.warn("Jupyter has no profiles. profile=%s has been ignored." % profile, stacklevel=2)
    if path is None:
