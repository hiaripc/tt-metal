...
class Handler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            logger.debug("Incoming request to %s:%d failed: %s" % (self.chain_host, self.chain_port, repr(e)))
            return
        if chan is None:
            logger.debug(
                "Incoming request to %s:%d was rejected by the SSH server." % (self.chain_host, self.chain_port)
            )
            return
