...
if use_platform_dirs():
    SYSTEM_JUPYTER_PATH = platformdirs.site_data_dir(APPNAME, appauthor=False, multipath=True).split(os.pathsep)
else:
    deprecation(
...
if use_platform_dirs():
    SYSTEM_CONFIG_PATH = platformdirs.site_config_dir(APPNAME, appauthor=False, multipath=True).split(os.pathsep)
else:
    if os.name == "nt":
        programdata = os.environ.get("PROGRAMDATA", None)
...
def get_file_mode(fname: str) -> int:
    ...
    return stat.S_IMODE(Path(fname).stat().st_mode) & 0o6677  # Use 4 octal digits since S_IMODE does the same
