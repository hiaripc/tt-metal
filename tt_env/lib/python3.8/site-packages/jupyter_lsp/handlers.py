...
class LanguageServerWebSocketHandler(WebSocketMixin, WebSocketHandler, BaseJupyterHandler):  # type: ignore
...
class LanguageServersHandler(BaseHandler):
    ...
    async def get(self):
        ...
        response = {
            "version": 2,
            "sessions": {
                language_server: session.to_json() for language_server, session in self.manager.sessions.items()
            },
            "specs": {key: censored_spec(spec) for key, spec in self.manager.all_language_servers.items()},
        }
