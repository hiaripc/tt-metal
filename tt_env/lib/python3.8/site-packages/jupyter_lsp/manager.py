...
class LanguageServerManager(LanguageServerManagerAPI):
    ...
    def _default_conf_d_language_servers(self) -> KeyedLanguageServerSpecs:
        ...
        for app in APP_CONFIG_D_SECTIONS:
            language_servers.update(
                **manager.get(f"jupyter{app}config").get(self.__class__.__name__, {}).get("language_servers", {})
            )
    ...
    def subscribe(self, handler):
        ...
        if session is None:
            self.log.error("[{}] no session: handler subscription failed".format(handler.language_server))
            return
    ...
    async def on_client_message(self, message, handler):
        ...
        if session is None:
            self.log.error("[{}] no session: client message dropped".format(handler.language_server))
            return
    ...
    def unsubscribe(self, handler):
        ...
        if session is None:
            self.log.error("[{}] no session: handler unsubscription failed".format(handler.language_server))
            return
