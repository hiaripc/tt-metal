...
async def test_apply_change(tmp_path):
    ...
    # modifying a range
    editable_file.apply_change("ves", start={"line": 1, "character": 0}, end={"line": 1, "character": 3})
...
async def test_no_shadow_for_well_behaved_server(
    ...
    manager = SimpleNamespace(language_servers={"python-lsp-server": {"requires_documents_on_disk": False}})
...
async def test_shadow_created_for_ill_behaved_server(
    ...
    manager = SimpleNamespace(language_servers={"python-lsp-server": {"requires_documents_on_disk": True}})
