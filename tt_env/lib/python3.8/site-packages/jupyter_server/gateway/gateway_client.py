...
(JUPYTER_GATEWAY_VALIDATE_CERT env var)""",
    ...
    def _validate_cert_default(self):
        return bool(
            os.environ.get(self.validate_cert_env, str(self.validate_cert_default_value)) not in ["no", "false"]
        )
...
(JUPYTER_GATEWAY_RETRY_MAX env var)""",
    ...
    gateway_token_renewer_class_default_value = "jupyter_server.gateway.gateway_client.NoOpTokenRenewer"  # noqa: S105
...
(JUPYTER_GATEWAY_LAUNCH_TIMEOUT_PAD env var)""",
    ...
    def _accept_cookies_default(self):
        return bool(
            os.environ.get(self.accept_cookies_env, str(self.accept_cookies_value).lower()) not in ["no", "false"]
        )
    ...
    async def _fetch(self, endpoint: str, **kwargs: ty.Any) -> HTTPResponse:
        ...
        try:
            ...
            logging.getLogger("ServerApp").info(
                f"Attempting retry ({self.retry_count}) against " f"endpoint '{endpoint}'.  Retried error: '{e!r}'"
            )
...
async def gateway_request(endpoint: str, **kwargs: ty.Any) -> HTTPResponse:
    ...
    try:
        ...
        gateway_client.emit(data={STATUS_KEY: SUCCESS_STATUS, STATUS_CODE_KEY: 200, MESSAGE_KEY: "success"})
    ...
    # of the tree view.
        ...
        gateway_client.emit(data={STATUS_KEY: ERROR_STATUS, STATUS_CODE_KEY: e.code, MESSAGE_KEY: str(e.message)})
        error_reason = f"Exception while attempting to connect to Gateway server url '{gateway_client.url}'"
        ...
        gateway_client.emit(data={STATUS_KEY: ERROR_STATUS, STATUS_CODE_KEY: 503, MESSAGE_KEY: str(e)})
        raise web.HTTPError(
            503,
        ...
        gateway_client.emit(data={STATUS_KEY: ERROR_STATUS, STATUS_CODE_KEY: 404, MESSAGE_KEY: str(e)})
        raise web.HTTPError(
            404,
        ...
        gateway_client.emit(data={STATUS_KEY: ERROR_STATUS, STATUS_CODE_KEY: 505, MESSAGE_KEY: str(e)})
