...
class ServerApp(JupyterApp):
    ...
    tornado_settings = Dict(
        config=True,
        help=_i18n("Supply overrides for the tornado.web.Application that the Jupyter server uses."),
    )
    ...
    def _bind_http_server(self) -> None:
        ...
        if not success:
            self.log.critical(
                _i18n("ERROR: the Jupyter server could not be started because " "no available port could be found.")
            )
            self.exit(1)
    ...
    def _find_http_port(self) -> None:
        ...
        if not success:
            if self.port_retries:
                ...
                self.log.critical(
                    _i18n("ERROR: the Jupyter server could not be started because " "port %i is not available.") % port
                )
            self.exit(1)
