...
class ContentsHandler(ContentsAPIHandler):
    ...
    async def patch(self, path=""):
        ...
        if (
            ...
            and (await ensure_async(cm.is_hidden(path)) or await ensure_async(cm.is_hidden(old_path)))
        ):
        ...
        if model:
            ...
            if copy_from:
                if not cm.allow_hidden and (
                    await ensure_async(cm.is_hidden(path)) or await ensure_async(cm.is_hidden(copy_from))
                ):
