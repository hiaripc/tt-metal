...
class ZMQChannelsWebsocketConnection(BaseKernelWebsocketConnection):
    ...
    async def _register_session(self):
        ...
        if stale_handler:
            ...
            stale_handler.close()
        if self.kernel_id in self.multi_kernel_manager:  # only update open sessions if kernel is actively managed
            self._open_sessions[self.session_key] = t.cast(KernelWebsocketHandler, self.websocket_handler)
    ...
    def connect(self):
        ...
        self.multi_kernel_manager.add_restart_callback(self.kernel_id, self.on_restart_failed, "dead")
    ...
    def disconnect(self):
        ...
        if self.kernel_id in self.multi_kernel_manager:
            ...
            if (
                ...
                self.multi_kernel_manager.start_buffering(self.kernel_id, self.session_key, self.channels)
    ...
    def handle_incoming_message(self, incoming_msg: str) -> None:
        ...
        if am:
            ...
            if msg["header"]["msg_type"] not in am:  # type:ignore[unreachable]
                self.log.warning(
                    'Received message of type "%s", which is not allowed. Ignoring.' % msg["header"]["msg_type"]
                )
                ignore_msg = True
        if not ignore_msg:
            stream = self.channels[channel]
