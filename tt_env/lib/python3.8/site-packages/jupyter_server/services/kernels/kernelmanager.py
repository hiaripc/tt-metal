...
class MappingKernelManager(MultiKernelManager):
    ...
    allow_tracebacks = Bool(True, config=True, help=("Whether to send tracebacks to clients on exceptions."))
    ...
    async def _async_start_kernel(  # type:ignore[override]
        ...
        if kernel_id is None or kernel_id not in self:
            ...
            self.log.debug("Kernel args (excluding env): %r", {k: v for k, v in kwargs.items() if k != "env"})
    ...
    async def _async_shutdown_kernel(self, kernel_id, now=False, restart=False):
        ...
        return await self.pinned_superclass._async_shutdown_kernel(self, kernel_id, now=now, restart=restart)
        ...
        async def wrapped_method(self, *args, **kwargs):
            ...
            try:
                ...
                data = {
                    ...
                    "action": action,
                    "status": "success",
                    "msg": success_msg.format(kernel_id=self.kernel_id, kernel_name=self.kernel_name, action=action),
                }
                if self.kernel_id:
