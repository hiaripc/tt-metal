...
def get_app_dir():
    ...
    elif sys.prefix.startswith("/usr") and not osp.exists(app_dir) and osp.exists("/usr/local/share/jupyter/lab"):
...
def ensure_node_modules(cwd, logger=None):
    ...
    yarn_proc = ProgressProcess(["node", YARN_PATH, "--immutable", "--immutable-cache"], cwd=cwd, logger=logger)
    ret = yarn_proc.wait()
...
class _AppHandler:
    ...
    def toggle_extension(self, extension, value, level="sys_prefix"):
        ...
        level_page_config = get_static_page_config(app_settings_dir=app_settings_dir, logger=self.logger, level=level)
    ...
    def _format_no_compatible_package_version(self, name):
        ...
        parts = [
            "No version of {extension} could be found that is compatible with " "the current version of JupyterLab."
        ]
...
def _yarn_config(logger):
    ...
    try:
        ...
        logger.error(
            "Fail to get yarn configuration. {!s}{!s}".format(e.stderr.decode("utf-8"), e.output.decode("utf-8"))
        )
...
def _test_overlap(spec1, spec2, drop_prerelease1=False, drop_prerelease2=False):
    ...
    """
    cmp = _compare_ranges(spec1, spec2, drop_prerelease1=drop_prerelease1, drop_prerelease2=drop_prerelease2)
    if cmp is None:
        return
    return cmp == 0
