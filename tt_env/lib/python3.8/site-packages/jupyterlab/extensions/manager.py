...
class PluginManager(LoggingConfigurable):
    ...
    def __init__(
        ...
        self.log.debug("Plugins in %s will managed on the %s level", self.__class__.__name__, self.level)
        self.app_options = _ensure_options(app_options)
        plugin_options_field = {f.name for f in fields(PluginManagerOptions)}
        plugin_options = {
            option: value for option, value in (ext_options or {}).items() if option in plugin_options_field
        }
    ...
    async def disable(self, plugins: Union[str, List[str]]) -> ActionResult:
        ...
        if locked:
            return ActionResult(
                status="error",
                message=trans.gettext("The following plugins cannot be disabled as they are locked: ")
                + ", ".join(locked),
            )
        try:
    ...
    async def enable(self, plugins: Union[str, List[str]]) -> ActionResult:
        ...
        if locked:
            return ActionResult(
                status="error",
                message=trans.gettext("The following plugins cannot be enabled as they are locked: ")
                + ", ".join(locked),
            )
        try:
            for plugin in plugins:
...
class ExtensionManager(PluginManager):
    ...
    async def _fetch_listings(self) -> None:
        ...
        if self._listings_block_mode:
            if len(self.options.blocked_extensions_uris):
                self.log.info(f"Fetching blocked extensions from {self.options.blocked_extensions_uris}")
            ...
            self.log.info(f"Fetching allowed extensions from { self.options.allowed_extensions_uris}")
    ...
    async def _get_installed_extensions(self, get_latest_version=True) -> Dict[str, ExtensionPackage]:
    ...
    async def _update_extensions_list(self, query: Optional[str] = None, page: int = 1, per_page: int = 30) -> None:
