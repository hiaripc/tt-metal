...
https_proxy_url = environ.get("https_proxy") or environ.get("HTTPS_PROXY") or http_proxy_url or all_proxy_url
proxies = None
...
class PyPIExtensionManager(ExtensionManager):
    ...
    cache_timeout = CFloat(5 * 60.0, config=True, help="PyPI extensions list cache timeout in seconds.")

    package_metadata_cache_size = CInt(1500, config=True, help="The cache size for package metadata.")
    ...
    async def __throttleRequest(self, recursive: bool, fn: Callable, *args) -> Any:  # noqa
        ...
        try:
            ...
            if err.faultCode == -32500 and err.faultString.startswith("HTTPTooManyRequests:"):  # noqa PLR2004
                delay = 1.01
                match = re.search(r"Limit may reset in (\d+) seconds.", err.faultString)
                if match is not None:
                    delay = int(match.group(1) or "1")
                self.log.info(f"HTTPTooManyRequests - Perform next call to PyPI XMLRPC API in {delay}s.")
    ...
    async def install(self, name: str, version: Optional[str] = None) -> ActionResult:  # noqa
        ...
        with tempfile.TemporaryDirectory() as ve_dir, tempfile.NamedTemporaryFile(
            ...
            result = await current_loop.run_in_executor(None, partial(run, cmdline, capture_output=True))
    ...
    async def uninstall(self, extension: str) -> ActionResult:
        ...
        try:
            tmp_cmd = cmdline.copy()
            tmp_cmd.remove("--yes")
            result = await current_loop.run_in_executor(None, partial(run, tmp_cmd, capture_output=True))
            lines = filter(
