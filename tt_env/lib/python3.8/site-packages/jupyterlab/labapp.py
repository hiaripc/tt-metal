    ...
    * App mode: JupyterLab allows multiple JupyterLab "applications" to be
        ...
        if self.dev_mode:
            ...
            if not self.extensions_in_dev_mode:
                ...
                self.labextensions_path = (
                    [galata_extension] if galata_extension in map(os.path.abspath, self.labextensions_path) else []
                )
    ...
    def initialize_handlers(self):  # noqa
        ...
        if not self.core_mode and not errored:
            ...
            listings_config = {
                "blocked_extensions_uris": set(filter(lambda uri: len(uri) > 0, blocked_extensions_uris.split(","))),
                "allowed_extensions_uris": set(filter(lambda uri: len(uri) > 0, allowed_extensions_uris.split(","))),
