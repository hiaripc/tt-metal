...
def update_extension(target: str, vcs_ref: Optional[str] = None, interactive: bool = True) -> None:  # noqa
    ...
    has_test = (
        (target / "conftest.py").exists() or (target / "jest.config.js").exists() or (target / "ui-tests").exists()
    )
    ...
    for p in output_dir.rglob("*"):
        ...
        if not file_target.exists():
            ...
            choice = (input(f'overwrite "{relpath!s}"? [{default}]: ') or default) if interactive else "n"
            if choice.upper().startswith("Y"):
                shutil.copy(p, file_target)
                if file_target.name == "pyproject.toml":
                    override_pyproject = True
    ...
    if override_pyproject:
        if (target / "setup.cfg").exists():
            try:
                ...
                pyproject["project"]["dependencies"] = pyproject["project"].get("dependencies", []) + requirements
