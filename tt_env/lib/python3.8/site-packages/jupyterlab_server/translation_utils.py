    ...
    if not message:
        for package_name, package_root_path in found_package_locales.items():
            ...
            try:
                ...
                locales = {
                    loc.lower(): loc for loc in os.listdir(locale_path) if os.path.isdir(os.path.join(locale_path, loc))
                }
            except Exception:
                messages.append(traceback.format_exc())
        ...
        if invalid_locales:
            if PSEUDO_LANGUAGE in invalid_locales:
                ...
                locales[PSEUDO_LANGUAGE] = {
                    ...
                    "nativeName": ("to translate the UI" if display_locale != PSEUDO_LANGUAGE else "Pseudo-language"),
                }
...
def get_language_pack(locale_: str) -> tuple:
    ...
    locale_data = {}
    messages = message.split("\n")
    if not message and (locale_ == PSEUDO_LANGUAGE or is_valid_locale(locale_)) and locale_ in found_locales:
