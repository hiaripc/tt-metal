...
def _list_workspaces(directory: Path, prefix: str) -> list[dict[str, Any]]:
    ...
    items = [
        item for item in directory.iterdir() if item.name.startswith(prefix) and item.name.endswith(WORKSPACE_EXTENSION)
    ]
    items.sort()
...
def slugify(raw: str, base: str = "", sign: bool = True, max_length: int = 128 - len(WORKSPACE_EXTENSION)) -> str:
    """
