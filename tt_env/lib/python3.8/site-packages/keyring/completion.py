...
def install_completion(parser):
    preamble = dict(
        bash=files(__package__).joinpath("backend_complete.bash").read_text(encoding="utf-8"),
        zsh=files(__package__).joinpath("backend_complete.zsh").read_text(encoding="utf-8"),
    )
