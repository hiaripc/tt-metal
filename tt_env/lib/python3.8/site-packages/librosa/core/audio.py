...
@cache(level=20)
def autocorrelate(y: np.ndarray, *, max_size: Optional[int] = None, axis: int = -1) -> np.ndarray:
...
def mu_compress(x: Union[np.ndarray, _FloatLike_co], *, mu: float = 255, quantize: bool = True) -> np.ndarray:
    ...
    if quantize:
        y: np.ndarray = (
            np.digitize(x_comp, np.linspace(-1, 1, num=int(1 + mu), endpoint=True), right=True) - int(mu + 1) // 2
        )
        return y
