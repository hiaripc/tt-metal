...
NOTE_RE = re.compile(
    r"^(?P<note>[A-Ga-g])" r"(?P<accidental>[#♯𝄪b!♭𝄫♮]*)" r"(?P<octave>[+-]?\d+)?" r"(?P<cents>[+-]\d+)?$"
)

KEY_RE = re.compile(r"^(?P<tonic>[A-Ga-g])" r"(?P<accidental>[#♯b!♭]?)" r":(?P<scale>(maj|min)(or)?)$")
...
def fifths_to_note(*, unison: str, fifths: int, unicode: bool = True) -> str:
    ...
    acc_str = acc_map_inv[np.sign(acc_index) * 2] * int(abs(acc_index) // 2) + acc_map_inv[np.sign(acc_index)] * int(
        abs(acc_index) % 2
    )
