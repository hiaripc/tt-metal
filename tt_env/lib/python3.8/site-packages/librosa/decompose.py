...
def hpss(
    ...
    *,
    ...
    power: float = 2.0,
    mask: bool = False,
    margin: Union[_FloatLike_co, Tuple[_FloatLike_co, _FloatLike_co], List[_FloatLike_co]] = 1.0,
) -> Tuple[np.ndarray, np.ndarray]:
...
def nn_filter(
    ...
    **kwargs
    ...
    if rec_s.shape[0] != S.shape[axis] or rec_s.shape[0] != rec_s.shape[1]:
        raise ParameterError("Invalid self-similarity matrix shape " f"rec.shape={rec_s.shape} for S.shape={S.shape}")
