    ...
    if y is not None:
        ...
        if S.shape[-2] != frame_length // 2 + 1:
            raise ParameterError(
                ...
                "found {}".format(S.shape[-2], S.shape[-2] * 2 - 2, S.shape[-2] * 2 - 1, frame_length)
            )
...
def poly_features(
    *,
    ...
    if freq.ndim == 1:
        ...
        fitter = np.vectorize(lambda y: np.polyfit(freq, y, order), signature="(f,t)->(d,t)")  # type: ignore
...
def chroma_cqt(
    *,
    ...
    if bins_per_octave is None:
        ...
        raise ParameterError(
            f"bins_per_octave={bins_per_octave} must be an integer " f"multiple of n_chroma={n_chroma}"
        )
