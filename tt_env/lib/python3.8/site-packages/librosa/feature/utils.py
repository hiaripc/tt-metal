...
def delta(
    ...
    **kwargs : additional keyword arguments
    ...
    if mode == "interp" and width > data.shape[axis]:
        raise ParameterError(f"when mode='interp', width={width} " f"cannot exceed data.shape[axis]={data.shape[axis]}")
...
def stack_memory(data: np.ndarray, *, n_steps: int = 2, delay: int = 1, **kwargs: Any) -> np.ndarray:
    ...
    **kwargs : additional keyword arguments
    ...
    if t < 1:
        raise ParameterError("Cannot stack memory when input data has " f"no columns. Given data.shape={data.shape}")
