...
def recurrence_matrix(
    ...
    *,
    ...
    if width < 1 or width >= (t - 1) // 2:
        raise ParameterError(
            "width={} must be at least 1 and at most (data.shape[{}] - 1) // 2={}".format(width, axis, (t - 1) // 2)
        )
...
def subsegment(data: np.ndarray, frames: np.ndarray, *, n_segments: int = 4, axis: int = -1) -> np.ndarray:
    ...
    for seg_start, seg_end in zip(frames[:-1], frames[1:]):
        ...
        boundaries.extend(
            seg_start + agglomerative(data[tuple(idx_slices)], min(seg_end - seg_start, n_segments), axis=axis)
        )
...
def __affinity_bandwidth(
    ...
    if isinstance(bw_mode, np.ndarray):
        ...
        if bandwidth.shape != rec.shape:
            raise ParameterError(f"Invalid matrix bandwidth shape: {bandwidth.shape}." f"Should be {rec.shape}.")
        if (bandwidth <= 0).any():
