...
def dtw(
    ...
    *,
    ...
    if backtrack:
        ...
        if subseq and (
            (X is not None and Y is not None and X.shape[0] > Y.shape[0]) or c_is_transposed or C.shape[0] > C.shape[1]
        ):
            wp = np.fliplr(wp)
        return_values = [D, wp]
    else:
...
def __rqa_dp(
    ...
    for j in range(2, sim.shape[1]):
        ...
        if sim[i, j] > 0:
            ...
            vec[:init_limit] = (
                score_values[:init_limit] - t_values[:init_limit] * gap_onset - (~t_values[:init_limit]) * gap_extend
            )
...
def viterbi(
    ...
    *,
    ...
    if transition.shape != (n_states, n_states):
        raise ParameterError(
            f"transition.shape={transition.shape}, must be " f"(n_states, n_states)={n_states, n_states}"
        )
...
def viterbi_discriminative(
    ...
    *,
    ...
    if p_state is None:
        ...
        raise ParameterError(
            "Marginal distribution p_state must have shape (n_states,). " f"Got p_state.shape={p_state.shape}"
        )
...
def viterbi_binary(
    ...
    *,
    ...
    if transition.shape == (2, 2):
        ...
        raise ParameterError(f"transition.shape={transition.shape}, must be (2, 2) or " f"(n_states, 2, 2)={n_states}")
