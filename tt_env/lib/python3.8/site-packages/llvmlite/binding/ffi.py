    ...
    def _load_lib(self):
        try:
            with _suppress_cleanup_errors(
                _importlib_resources_path(__name__.rpartition(".")[0], get_library_name())
            ) as lib_path:
...
_importlib_resources_path = _importlib_resources_path_repl if sys.version_info[:2] >= (3, 9) else _impres.path
