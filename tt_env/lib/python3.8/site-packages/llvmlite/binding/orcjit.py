...
class JITLibraryBuilder:
    ...
    def link(self, lljit, library_name):
        ...
        return ResourceTracker(
            tracker, library_name, {name: exports[idx].address for idx, name in enumerate(self.__exports)}
        )
    ...
    def lookup(self, dylib, fn):
        ...
        with ffi.OutputString() as outerr:
            tracker = ffi.lib.LLVMPY_LLJITLookup(
                self, dylib.encode("utf-8"), fn.encode("utf-8"), ctypes.byref(address), outerr
            )
            if not tracker:
                raise RuntimeError(str(outerr))
...
ffi.lib.LLVMPY_LLJIT_Dylib_Tracker_Dispose.argtypes = [ffi.LLVMOrcDylibTrackerRef, POINTER(c_char_p)]
