...
class CallInstrAttributes(AttributeSet):
    _known = frozenset(["noreturn", "nounwind", "readonly", "readnone", "noinline", "alwaysinline"])
    ...
    def _descr(self, buf, add_metadata):
        ...
        buf.append(
            "{tail}{op}{fastmath} {callee}({args}){attr}{meta}\n".format(
                ...
                args=args,
                attr=fn_attrs,
                meta=(self._stringify_metadata(leading_comma=True) if add_metadata else ""),
            )
        )
...
class InvokeInstr(CallInstr):
    def __init__(
        self, parent, func, args, normal_to, unwind_to, name="", cconv=None, fastmath=(), attrs=(), arg_attrs=None
    ):
        assert isinstance(normal_to, Block)
        assert isinstance(unwind_to, Block)
        super(InvokeInstr, self).__init__(
            parent, func, args, name, cconv, tail=False, fastmath=fastmath, attrs=attrs, arg_attrs=arg_attrs
        )
        self.opname = "invoke"
...
class IndirectBranch(PredictableInstr, Terminator):
    ...
    def descr(self, buf):
        destinations = ["label {0}".format(blk.get_reference()) for blk in self.destinations]
        buf.append(
            "indirectbr {0} {1}, [{2}]  {3}\n".format(
                self.address.type,
...
class SwitchInstr(PredictableInstr, Terminator):
    ...
    def descr(self, buf):
        cases = [
            "{0} {1}, label {2}".format(val.type, val.get_reference(), blk.get_reference()) for val, blk in self.cases
        ]
        buf.append(
...
class ShuffleVector(Instruction):
    def __init__(self, parent, vector1, vector2, mask, name=""):
        ...
        if (
            ...
            or not (isinstance(mask.type.element, types.IntType) and mask.type.element.width == 32)
        ):
    ...
    def descr(self, buf):
        buf.append(
            "shufflevector {0} {1}\n".format(
                ", ".join("{0} {1}".format(op.type, op.get_reference()) for op in self.operands),
                self._stringify_metadata(leading_comma=True),
            )
        )
