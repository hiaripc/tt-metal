...
class FunctionType(Type):
    ...
    def __eq__(self, other):
        if isinstance(other, FunctionType):
            return self.return_type == other.return_type and self.args == other.args and self.var_arg == other.var_arg
        else:
            return False
        ...
        if not isinstance(values, (list, tuple)):
            if isinstance(values, Constant):
                if values.type != self.element:
                    raise TypeError("expected {} for {}".format(self.element, values.type))
