...
def _binop(opname):
    def wrap(fn):
        ...
        def wrapped(lhs, rhs):
            ...
            fmt = "{0} ({1} {2}, {3} {4})".format(opname, lhs.type, lhs.get_reference(), rhs.type, rhs.get_reference())
...
def _castop(opname):
    def wrap(fn):
        ...
        def wrapped(self, typ):
            ...
            op = "{0} ({1} {2} to {3})".format(opname, self.type, self.get_reference(), typ)
    ...
    def descr(self, buf):
        ...
        if self.initializer is not None:
            if self.initializer.type != self.value_type:
                raise TypeError(
                    "got initializer of type %s " "for global value type %s" % (self.initializer.type, self.value_type)
                )
...
class FunctionAttributes(AttributeSet):
    ...
    def _to_list(self, ret_type):
        ...
        if self.personality:
            attrs.append(
                "personality {persty} {persfn}".format(
                    persty=self.personality.type, persfn=self.personality.get_reference()
                )
            )
        return attrs
...
class Function(GlobalValue):
    ...
    def descr_prototype(self, buf):
        ...
        prototype = pt_str.format(
            prefix=prefix, name=name, args=args, vararg=vararg, attrs=attrs, section=section, metadata=metadata
        )
