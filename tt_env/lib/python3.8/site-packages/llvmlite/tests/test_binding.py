...
class TestModuleRef(BaseTest):
    ...
    def test_get_struct_type(self):
        ...
        self.assertIsNotNone(re.match(r"%struct\.glob_type(\.[\d]+)? = type { i64, \[2 x i64\] }", str(st_ty)))
    ...
    def test_structs(self):
        ...
        self.assertIsNotNone(re.match(r"%struct\.glob_type(\.[\d]+)? = type { i64, \[2 x i64\] }", str(structs[0])))
...
class TestOrcLLJIT(BaseTest):
    def jit(
        ...
        rt = (
            builder.add_ir(asm.format(triple=llvm.get_default_triple())).export_symbol(func_name).link(lljit, func_name)
        )
        cfptr = rt[func_name]
        self.assertTrue(cfptr)
    ...
    def test_thread_safe(self):
        ...
        def compile_many(i):
            def do_work():
                ...
                for c in range(50):
                    tracking.append(
                        llvm.JITLibraryBuilder().add_ir(llvm_ir).export_symbol("sum").link(lljit, f"sum_{i}_{c}")
                    )
...
class TestValueRef(BaseTest):
    ...
    def test_type_name(self):
        ...
        self.assertIsNotNone(re.match(r"struct\.glob_type(\.[\d]+)?", st.type.element_type.name))
    ...
    def test_type_printing_struct(self):
        ...
        self.assertIsNotNone(re.match(r"%struct\.glob_type(\.[\d]+)?\*", str(st.type)))
        self.assertIsNotNone(
            re.match(r"%struct\.glob_type(\.[\d]+)? = type { i64, \[2 x i64\] }", str(st.type.element_type))
        )
