...
class TestFunction(TestBase):
    ...
    def test_declare_attributes(self):
        ...
        self.assertEqual(
            asm,
            ("declare %s alwaysinline optsize alignstack(16) " 'personality i8 (...)* @"__gxx_personality_v0"')
            % self.proto,
        )
        # Check pickling
    ...
    def test_function_attr_section_meta(self):
        ...
        func.set_metadata("dbg", module.add_metadata([]))
        asm = self.descr(func).strip()
        self.assertEqual(asm, f'declare {self.proto} alwaysinline section "a_section" !dbg !0')  # noqa E501
        # Check pickling
        self.assert_pickle_correctly(func)
...
class TestIR(TestBase):
    ...
    def test_debug_info_gvar(self):
        ...
        di_float = mod.add_debug_info(
            "DIBasicType", {"name": "float", "size": 32, "encoding": ir.DIToken("DW_ATE_float")}
        )
        di_gvar = mod.add_debug_info(
            "DIGlobalVariableExpression",
            {
                ...
                "var": mod.add_debug_info(
                    "DIGlobalVariable", {"name": gvar.name, "type": di_float, "isDefinition": True}, is_distinct=True
                ),
            },
        )
        ...
        # Check output
        strmod = str(mod)
        self.assert_ir_line("!0 = !DIBasicType(encoding: DW_ATE_float, " 'name: "float", size: 32)', strmod)
        self.assert_ir_line("!1 = !DIExpression()", strmod)
        self.assert_ir_line("!2 = distinct !DIGlobalVariable(isDefinition: " 'true, name: "gvar", type: !0)', strmod)
        self.assert_ir_line("!3 = !DIGlobalVariableExpression(expr: !1, " "var: !2)", strmod)
        self.assert_ir_line('@"gvar" = global float 0x4045000000000000, ' "!dbg !3", strmod)
...
class TestTypes(TestBase):
    ...
    def test_str(self):
        ...
        self.assertEqual(str(ir.FunctionType(int1, (flt,), var_arg=True)), "i1 (float, ...)")
        self.assertEqual(str(ir.FunctionType(int1, (flt, dbl), var_arg=True)), "i1 (float, double, ...)")
        ...
        self.assertEqual(
            str(ir.LiteralStructType((ir.PointerType(int1), ir.LiteralStructType((int32, int8))))), "{i1*, {i32, i8}}"
        )
        self.assertEqual(str(ir.LiteralStructType((int1,), packed=True)), "<{i1}>")
        self.assertEqual(str(ir.LiteralStructType((int1, flt), packed=True)), "<{i1, float}>")
...
class TestConstant(TestBase):
    ...
    def test_gep(self):
        ...
        self.assertEqual(str(c), 'getelementptr ({float, i1}, {float, i1}* @"myconstant", i32 0, i32 1)')  # noqa E501
        ...
        self.assertEqual(str(c2), "getelementptr ({float, i1}, {float, i1}* null, i32 0)")  # noqa E501
        self.assertEqual(c.type, ir.PointerType(int1))
    ...
    def test_gep_addrspace_globalvar(self):
        ...
        self.assertEqual(
            str(c), ("getelementptr ({float, i1}, {float, i1} " 'addrspace(4)* @"myconstant", i32 0, i32 1)')
        )
    ...
    def test_ptrtoint_2(self):
        ...
        self.assertRaisesRegex(
            TypeError, r"can only ptrtoint\(\) to integer type, not 'i64\*'", gv.ptrtoint, int64.as_pointer()
        )

        c2 = ir.Constant(int32, 0)
        self.assertRaisesRegex(TypeError, r"can only call ptrtoint\(\) on pointer type, not 'i32'", c2.ptrtoint, int64)
