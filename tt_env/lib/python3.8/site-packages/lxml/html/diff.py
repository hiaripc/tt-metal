...
def compress_merge_back(tokens, tok):
    ...
    if type(last) is not token or type(tok) is not token:
        ...
        text += tok
        merged = token(
            text, pre_tags=last.pre_tags, post_tags=tok.post_tags, trailing_whitespace=tok.trailing_whitespace
        )
    ...
    for chunk in chunks:
        if isinstance(chunk, tuple):
            if chunk[0] == "img":
                ...
                cur_word = tag_token(
                    "img", src, html_repr=tag, pre_tags=tag_accum, trailing_whitespace=trailing_whitespace
                )
                tag_accum = []
...
def start_tag(el):
    ...
    return "<%s%s>" % (
        el.tag,
        "".join([' %s="%s"' % (name, html_escape(value, True)) for name, value in el.attrib.items()]),
    )
