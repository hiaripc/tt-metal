...
def process_inlines(tokens: list[Token], state: StateCore) -> None:
    ...
    for i, token in enumerate(tokens):
        ...
        while pos < maximum:
            ...
            isLastPunctChar = lastChar is not None and (isMdAsciiPunct(lastChar) or isPunctChar(chr(lastChar)))
            isNextPunctChar = nextChar is not None and (isMdAsciiPunct(nextChar) or isPunctChar(chr(nextChar)))
            ...
            if nextChar == 0x22 and t.group(0) == '"':  # 0x22: "  # noqa: SIM102
                if lastChar is not None and lastChar >= 0x30 and lastChar <= 0x39:  # 0x30: 0, 0x39: 9
            ...
            if not canOpen and not canClose:
                # middle of word
                if isSingle:
                    token.content = replaceAt(token.content, t.start(0) + lastIndex, APOSTROPHE)
                continue
            ...
            if canClose:
                ...
                for j in range(len(stack))[::-1]:
                    ...
                    if item["single"] == isSingle and stack[j]["level"] == thisLevel:
                        ...
                        token.content = replaceAt(token.content, t.start(0) + lastIndex, closeQuote)
