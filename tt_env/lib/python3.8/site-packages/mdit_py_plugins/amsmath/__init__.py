...
def amsmath_plugin(md: MarkdownIt, *, renderer: Callable[[str], str] | None = None) -> None:
...
def amsmath_block(state: StateBlock, startLine: int, endLine: int, silent: bool) -> bool:
    ...
    if not silent:
        ...
        token.block = True
        token.content = state.getLines(startLine, state.line, state.sCount[startLine], False)
