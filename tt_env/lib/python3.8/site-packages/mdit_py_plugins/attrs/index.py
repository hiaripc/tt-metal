...
def attrs_block_plugin(md: MarkdownIt, *, allowed: Sequence[str] | None = None) -> None:
    ...
    md.core.ruler.after(
        "block",
        "attr",
        partial(_attr_resolve_block_rule, allowed=None if allowed is None else set(allowed)),
    )
...
def _attr_block_rule(state: StateBlock, startLine: int, endLine: int, silent: bool) -> bool:
...
def _add_attrs(
    ...
    if allowed is not None and (disallowed := {k: v for k, v in attrs.items() if k not in allowed}):
