    ...
    with set_parent_type(state, "fieldlist"):
        while nextLine < endLine:
            ...
            first_line_body_indent = (
                state.sCount[nextLine] + posAfterName - (state.bMarks[startLine] + state.tShift[startLine])
            )
            ...
            while _line < endLine:
                ...
                if (state.bMarks[_line] + state.tShift[_line]) < state.eMarks[_line]:
                    if state.tShift[_line] <= 0:
                        ...
                        break
                    block_indent = (
                        state.tShift[_line] if block_indent is None else min(block_indent, state.tShift[_line])
                    )
