    ...
    if delimiters in rules:
        for rule_inline in rules[delimiters]["inline"]:
            ...
            def render_math_inline(
                ...
                return rule_inline["tmpl"].format(render(tokens[idx].content, False, macros))  # noqa: B023
...
def dollar_post(src: str, end: int) -> bool:
    try:
        ...
        return True
    return (not nxt) or (nxt < 0x30) or (nxt > 0x39)  # no decimal digit .. after closing '$'
...
rules: dict[str, dict[str, list[RuleDictType]]] = {
    ...
    "gitlab": {
        ...
        "block": [
            {
                "name": "math_block_eqno",
                "rex": re.compile(r"^`{3}math\s+?([^`]+?)\s+?`{3}\s*?\(([^)$\r\n]+?)\)", re.M),
