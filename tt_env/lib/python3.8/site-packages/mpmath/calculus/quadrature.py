    ...
    def summation(self, f, points, prec, epsilon, max_degree, verbose=False):
        ...
        for i in xrange(len(points) - 1):
            ...
            for degree in xrange(1, max_degree + 1):
                ...
                if verbose:
                    print(
                        "Integrating from %s to %s (degree %s of %s)" % (ctx.nstr(a), ctx.nstr(b), degree, max_degree)
                    )
        ...
        **References**
        ...
        try:
            ...
            if dim == 1:
                ...
                v, err = rule.summation(
                    lambda x: rule.summation(lambda y: f(x, y), points[1], prec, epsilon, m)[0],
                    points[0],
                    prec,
                    epsilon,
                    m,
                ...
                v, err = rule.summation(
                    lambda x: rule.summation(
                        lambda y: rule.summation(lambda z: f(x, y, z), points[2], prec, epsilon, m)[0],
                        points[1],
                        prec,
                        epsilon,
                        m,
