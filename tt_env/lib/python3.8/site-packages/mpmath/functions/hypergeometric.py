...
def _hyp1f2(ctx, a_s, b_s, z, **kwargs):
    ...
    can_use_asymptotic = (
        (not kwargs.get("force_series")) and (ctx.mag(absz) > 19) and (ctx.sqrt(absz) > 1.5 * orig)
    )  # and \
        ...
        try:
            try:
                ...
                def h(a1, b1, b2):
                    ...
                    c[2] = 2 * (
                        b1 * b2 + ctx.mpq_1_4 * (a1 - b1 - b2) * (3 * a1 + b1 + b2 - 2) - ctx.mpq_3_16
                    ) ** 2 + ctx.mpq_1_16 * (
                        -16 * (2 * a1 - 3) * b1 * b2 + 4 * (a1 - b1 - b2) * (-8 * a1**2 + 11 * a1 + b1 + b2 - 2) - 3
                    )
                    s1 = 0
                    s2 = 0
                    k = 0
