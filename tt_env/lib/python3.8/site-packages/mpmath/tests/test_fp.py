...
def test_fp_gamma():
    ...
    assert ae(fp.loggamma(1e50), 1.1412925464970229298e52)
    assert ae(fp.loggamma(1e25 + 1e25j), (5.6125802751733671621e26 + 5.7696599078528568383e26j))
...
def test_fp_erf():
    ...
    assert ae(fp.erfc(1e50), 0.0)
    assert ae(fp.erfc(-1e50), 2.0)
...
def test_fp_stress_ei_e1():
    ...
    v = fp.e1((-80.0 + 320.0j))
    assert ae(v, (3.3464697299634526706e31 - 1.6473152633843023919e32j), tol=ATOL)
    assert ae(v.real, 3.3464697299634526706e31, tol=PTOL)
    assert ae(v.imag, -1.6473152633843023919e32, tol=PTOL)
    v = fp.e1((-4.6566128730773925781e-10 + 1.1641532182693481445e-10j))
    ...
    v = fp.e1((-80.0 + 20.0j))
    assert ae(v, (-4.2139911108612653091e32 + 5.3367124741918251637e32j), tol=ATOL)
    assert ae(v.real, -4.2139911108612653091e32, tol=PTOL)
    assert ae(v.imag, 5.3367124741918251637e32, tol=PTOL)
    v = fp.e1((-120.0 + 30.0j))
    assert ae(v, (9.7760616203707508892e48 - 1.058257682317195792e50j), tol=ATOL)
    assert ae(v.real, 9.7760616203707508892e48, tol=PTOL)
    assert ae(v.imag, -1.058257682317195792e50, tol=PTOL)
    v = fp.e1((-160.0 + 40.0j))
    assert ae(v, (8.7065541466623638861e66 + 1.6577106725141739889e67j), tol=ATOL)
    assert ae(v.real, 8.7065541466623638861e66, tol=PTOL)
    assert ae(v.imag, 1.6577106725141739889e67, tol=PTOL)
    v = fp.e1((-200.0 + 50.0j))
    assert ae(v, (-3.070744996327018106e84 - 1.7243244846769415903e84j), tol=ATOL)
    assert ae(v.real, -3.070744996327018106e84, tol=PTOL)
    assert ae(v.imag, -1.7243244846769415903e84, tol=PTOL)
    v = fp.e1((-320.0 + 80.0j))
    assert ae(v, (9.9960598637998647276e135 - 2.6855081527595608863e136j), tol=ATOL)
    assert ae(v.real, 9.9960598637998647276e135, tol=PTOL)
    assert ae(v.imag, -2.6855081527595608863e136, tol=PTOL)
    v = fp.e1(-1.1641532182693481445e-10)
    ...
    assert ae(v.imag, -3.1415926535897932385, tol=PTOL)
    v = fp.e1(-50.0)
    assert ae(v, (-1.0585636897131690963e20 - 3.1415926535897932385j), tol=ATOL)
    assert ae(v.real, -1.0585636897131690963e20, tol=PTOL)
    assert ae(v.imag, -3.1415926535897932385, tol=PTOL)
    v = fp.e1(-80.0)
    assert ae(v, (-7.0146000049047999696e32 - 3.1415926535897932385j), tol=ATOL)
    assert ae(v.real, -7.0146000049047999696e32, tol=PTOL)
    assert ae(v.imag, -3.1415926535897932385, tol=PTOL)
    v = fp.e1((-1.1641532182693481445e-10 + 0.0j))
    assert ae(v, (22.296641293460247028 - 3.1415926535897932385j), tol=ATOL)
    assert ae(v.real, 22.296641293460247028, tol=PTOL)
    ...
    v = fp.e1((-80.0 - 80.0j))
    assert ae(v, (3.8674816337930010217e32 + 3.0540709639658071041e32j), tol=ATOL)
    assert ae(v.real, 3.8674816337930010217e32, tol=PTOL)
    assert ae(v.imag, 3.0540709639658071041e32, tol=PTOL)
    v = fp.e1((-1.1641532182693481445e-10 - 4.6566128730773925781e-10j))
    assert ae(v, (20.880034621432138988 + 1.8157749894560994861j), tol=ATOL)
    assert ae(v.real, 20.880034621432138988, tol=PTOL)
    assert ae(v.imag, 1.8157749894560994861, tol=PTOL)
    ...
    assert type(v) is float
    v = fp.ei(50.0)
    assert ae(v, 1.0585636897131690963e20, tol=ATOL)
    assert type(v) is float
    v = fp.ei(80.0)
    assert ae(v, 7.0146000049047999696e32, tol=ATOL)
    assert type(v) is float
    ...
    assert v.imag == 0
    v = fp.ei((50.0 + 0.0j))
    assert ae(v, (1.0585636897131690963e20 + 0.0j), tol=ATOL)
    assert ae(v.real, 1.0585636897131690963e20, tol=PTOL)
    assert v.imag == 0
    v = fp.ei((80.0 + 0.0j))
    assert ae(v, (7.0146000049047999696e32 + 0.0j), tol=ATOL)
    assert ae(v.real, 7.0146000049047999696e32, tol=PTOL)
    assert v.imag == 0
