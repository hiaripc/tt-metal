...
def test_bernoulli():
    ...
    assert bernoulli(2).ae(1.0 / 6)
    assert bernoulli(3) == 0
    assert bernoulli(4).ae(-1.0 / 30)
    assert bernoulli(5) == 0
    assert bernoulli(6).ae(1.0 / 42)
...
def test_altzeta():
    ...
    s = -3 + 4j
    assert altzeta(s).ae((1 - 2 ** (1 - s)) * zeta(s))
    assert altzeta(-100.5).ae(4.58595480083585913e108)
...
def test_zeta_negative():
    ...
    assert (
        str(zeta(a))
        == "2.5588049270871154960875033337384432038436330847333e+1233536161668617575553892558646631323374078"
    )
    mp.dps = 15
...
def test_polygamma():
    ...
    assert psi0(1e-40).ae(-1.000000000000000e40)
    assert psi0(1e-10 + 1e-10j).ae(-5000000000.577215 + 5000000000.000000j)
    assert psi0(1e-40 + 1e-40j).ae(-5.000000000000000e39 + 5.000000000000000e39j)
    assert psi0(inf) == inf
    assert psi1(inf) == 0
    assert psi(2, inf) == 0
...
def test_polygamma_high_prec():
    ...
    assert (
        str(psi(10, pi))
        == "-12.98876181434889529310283769414222588307175962213707170773803550518307617769657562747174101900659238"
    )
...
def test_polygamma_high_order():
    ...
    assert (
        str(psi(50, pi))
        == "-1344100348958402765749252447726432491812.641985273160531055707095989227897753035823152397679626136483"
    )
    assert (
        str(psi(50, pi + 14 * e))
        == "-0.00000000000000000189793739550804321623512073101895801993019919886375952881053090844591920308111549337295143780341396"
    )
    assert str(psi(50, pi + 14 * e * j)) == (
        "(-0.0000000000000000522516941152169248975225472155683565752375889510631513244785"
...
def test_harmonic():
    ...
    assert harmonic(3).ae(1.0 + 1.0 / 2 + 1.0 / 3)
...
def test_gamma_huge_3():
    ...
    y = gamma(x)
    assert str(y.real) == (
        "-6.82925203918106e+2636286142112569524501781477865238132302397236429627932441916" "056964386399485392600"
    )
    assert str(y.imag) == (
        "8.54647143678418e+26362861421125695245017814778652381323023972364296279324419160" "56964386399485392600"
    )
    mp.dps = 50
    y = gamma(x)
    assert str(y.real) == (
...
def test_polylog():
    ...
    for z in zs:
        assert polylog(1, z).ae(-log(1 - z))
    for z in zs:
        assert polylog(0, z).ae(z / (1 - z))
    for z in zs:
        assert polylog(-1, z).ae(z / (1 - z) ** 2)
    for z in zs:
        assert polylog(-2, z).ae(z * (1 + z) / (1 - z) ** 3)
    for z in zs:
        assert polylog(-3, z).ae(z * (1 + 4 * z + z**2) / (1 - z) ** 4)
    ...
    assert polylog(-2, -0.9).ae(-90 / 6859.0)
