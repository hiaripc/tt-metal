            ...
            try:
                ...
                if methodname not in exposed:
                    raise AttributeError(
                        "method %r of %r object is not in exposed=%r" % (methodname, type(obj), exposed)
                    )
        ...
        with self.mutex:
            ...
            for ident in keys:
                if ident != "0":
                    result.append(
                        "  %s:       refcount=%s\n    %s"
                        % (ident, self.id_to_refcount[ident], str(self.id_to_obj[ident][0])[:75])
                    )
            return "\n".join(result)
    ...
    def create(*args, **kwds):
        ...
        if len(args) >= 3:
            ...
            if len(args) >= 2:
                ...
                warnings.warn("Passing 'typeid' as keyword argument is deprecated", DeprecationWarning, stacklevel=2)
            else:
                if "c" not in kwds:
                    raise TypeError("create expected at least 2 positional " "arguments, got %d" % (len(args) - 1))
                c = kwds.pop("c")
                ...
                warnings.warn("Passing 'c' as keyword argument is deprecated", DeprecationWarning, stacklevel=2)
        args = tuple(args)
        ...
        with self.mutex:
            ...
            if method_to_typeid is not None:
                if not isinstance(method_to_typeid, dict):
                    raise TypeError(
                        "Method_to_typeid {0!r}: type {1!s}, not dict".format(method_to_typeid, type(method_to_typeid))
                    )
...
class BaseManager(object):
    ...
    def start(self, initializer=None, initargs=()):
        ...
        self._process = self._ctx.Process(
            ...
            args=(self._registry, self._address, self._authkey, self._serializer, writer, initializer, initargs),
        )
        ...
        self.shutdown = util.Finalize(
            ...
            args=(self._process, self._address, self._authkey, self._state, self._Client),
            exitpriority=0,
        )
...
class BaseProxy(object):
    ...
    def _incref(self):
        ...
        self._close = util.Finalize(
            self,
            BaseProxy._decref,
            args=(self._token, self._authkey, state, self._tls, self._idset, self._Client),
            exitpriority=10,
        )
