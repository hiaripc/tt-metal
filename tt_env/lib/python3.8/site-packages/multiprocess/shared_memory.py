                        ...
                        if last_error_code == _winapi.ERROR_ALREADY_EXISTS:
                            if name is not None:
                                raise FileExistsError(
                                    errno.EEXIST, os.strerror(errno.EEXIST), name, _winapi.ERROR_ALREADY_EXISTS
                                )
                            else:
                                continue
    ...
    def __init__(self, sequence=None, *, name=None):
        if sequence is not None:
            _formats = [
                ...
                else self._types_mapping[type(item)] % (self._alignment * (len(item) // self._alignment + 1),)
        ...
        if sequence is not None:
            _enc = _encoding
            struct.pack_into(
                "q" + self._format_size_metainfo, self.shm.buf, 0, self._list_len, *(self._allocated_bytes)
            )
            struct.pack_into(
                "".join(_formats),
                self.shm.buf,
            ...
            struct.pack_into(
                self._format_back_transform_codes, self.shm.buf, self._offset_back_transform_codes, *(_recreation_codes)
            )

        else:
            self._list_len = len(self)  # Obtains size from offset 0 in buffer.
            self._allocated_bytes = struct.unpack_from(self._format_size_metainfo, self.shm.buf, 1 * 8)
    ...
    def __setitem__(self, position, value):
        ...
        if not isinstance(value, (str, bytes)):
            ...
            if current_format[-1] == "s":
                ...
                new_format = self._types_mapping[str] % (self._allocated_bytes[position],)
