...
def build(
    ...
    def default_flush_errors(filename: str | None, new_messages: list[str], is_serious: bool) -> None:
    ...
    try:
        result = _build(sources, options, alt_lib_path, flush_errors, fscache, stdout, stderr, extra_plugins)
...
def load_plugins_from_config(options: Options, errors: Errors, stdout: TextIO) -> tuple[list[Plugin], dict[str, str]]:
    ...
    for plugin_path in options.plugins:
        ...
        if not hasattr(module, func_name):
            plugin_error('Plugin "{}" does not define entry point function "{}"'.format(plugin_path, func_name))
        ...
        if not issubclass(plugin_type, Plugin):
            plugin_error(
                'Return value of "plugin" must be a subclass of "mypy.plugin.Plugin" ' "(in {})".format(plugin_path)
            )
        try:
    ...
    def __init__(
        ...
        for module in CORE_BUILTIN_MODULES:
            ...
            if not isinstance(path, str):
                raise CompileError([f"Failed to find builtin module {module}, perhaps typeshed is broken?"])
            if is_typeshed_file(options.abs_custom_typeshed_dir, path) or is_stub_package_file(path):
                continue
    ...
    def all_imported_modules_in_file(self, file: MypyFile) -> list[tuple[int, str, int]]:
        ...
        def correct_rel_imp(imp: ImportFrom | ImportAll) -> str:
            ...
            if not new_id:
                ...
                self.errors.report(imp.line, 0, "No parent module -- cannot perform relative import", blocker=True)
...
def read_deps_cache(manager: BuildManager, graph: Graph) -> dict[str, FgDepMeta] | None:
    ...
    current_meta_snapshot = {id: st.meta_source_hash for id, st in graph.items() if st.meta_source_hash is not None}
...
def find_cache_meta(id: str, path: str, manager: BuildManager) -> CacheMeta | None:
    ...
    t0 = time.time()
    meta = _load_json_file(meta_json, manager, log_success=f"Meta {id} ", log_error=f"Could not load cache for {id}: ")
    t1 = time.time()
    if meta is None:
        return None
    ...
    t2 = time.time()
    manager.add_stats(load_meta_time=t2 - t0, load_meta_load_time=t1 - t0, load_meta_from_dict_time=t2 - t1)
        ...
        if manager.options.verbosity >= 2:
            for key in sorted(set(cached_options) | set(current_options)):
                if cached_options.get(key) != current_options.get(key):
                    manager.trace("    {}: {} != {}".format(key, cached_options.get(key), current_options.get(key)))
        return None
...
def validate_meta(
    ...
    if not bazel and (mtime != meta.mtime or path != meta.path):
        ...
        if source_hash != meta.hash:
            ...
            manager.log("Updating mtime for {}: file {}, meta {}, mtime {}".format(id, path, meta_json, meta.mtime))
            t1 = time.time()
...
class State:
    ...
    def is_fresh(self) -> bool:
        ...
        return self.meta is not None and self.is_interface_fresh() and self.dependencies == self.meta.dependencies
    ...
    def load_tree(self, temporary: bool = False) -> None:
        assert self.meta is not None, "Internal error: this method must be called only for cached modules"

        data = _load_json_file(self.meta.data_json, self.manager, "Load tree ", "Could not load tree: ")
        if data is None:
            return
        ...
        with self.wrap_context():
            ...
            if self.path and source is None:
                try:
                    ...
                    raise CompileError(
                        ["mypy: can't read file '{}': {}".format(self.path, os.strerror(ioerr.errno))],
    ...
    def compute_dependencies(self) -> None:
        ...
        dep_entries = manager.all_imported_modules_in_file(self.tree) + self.manager.plugin.get_additional_deps(
            self.tree
        )
    ...
    def detect_possibly_undefined_vars(self) -> None:
        ...
        if manager.errors.is_error_code_enabled(codes.POSSIBLY_UNDEFINED) or manager.errors.is_error_code_enabled(
            codes.USED_BEFORE_DEF
        ):
            self.tree.accept(
    ...
    def finish_passes(self) -> None:
        ...
        with self.wrap_context():
            ...
            self._patch_indirect_dependencies(self.type_checker().module_refs, expr_types | symbol_types)
            ...
            if manager.options.export_ref_info:
                write_undocumented_ref_info(self, manager.metastore, manager.options, self.type_map())
    ...
    def write_cache(self) -> None:
        ...
        if not self.path or self.options.cache_dir == os.devnull or self.options.fine_grained_incremental:
    ...
    def verify_dependencies(self, suppressed_only: bool = False) -> None:
        ...
        if suppressed_only:
            ...
            dependencies = [dep for dep in self.dependencies if self.priorities.get(dep) != PRI_INDIRECT]
    ...
    def generate_unused_ignore_notes(self) -> None:
        if (
            self.options.warn_unused_ignores or codes.UNUSED_IGNORE in self.options.enabled_error_codes
    ...
    def generate_ignore_without_code_notes(self) -> None:
        if self.manager.errors.is_error_code_enabled(codes.IGNORE_WITHOUT_CODE):
            self.manager.errors.generate_ignore_without_code_errors(self.xpath, self.options.warn_unused_ignores)
...
def exist_added_packages(suppressed: list[str], manager: BuildManager, options: Options) -> bool:
    ...
    for dep in suppressed:
        ...
        if not path:
            continue
        if options.follow_imports == "skip" and (not path.endswith(".pyi") or options.follow_imports_for_stubs):
            continue
...
def in_partial_package(id: str, manager: BuildManager) -> bool:
    ...
    while "." in id:
        ...
        if parent in manager.modules:
            ...
            try:
                parent_st = State(id=parent, path=None, source=None, manager=manager, temporary=True)
            except (ModuleNotFound, CompileError):
                parent_mod = None
            else:
...
def module_not_found(
    ...
    if target == "builtins":
        errors.report(line, 0, "Cannot find 'builtins' module. Typeshed appears broken!", blocker=True)
        errors.raise_error()
    else:
...
def skipping_ancestor(manager: BuildManager, id: str, path: str, ancestor_for: State) -> None:
    ...
    manager.errors.report(-1, -1, f'Ancestor package "{id}" ignored', severity="error", only_once=True)
    manager.errors.report(
        -1,
        -1,
...
def dispatch(sources: list[BuildSource], manager: BuildManager, stdout: TextIO) -> Graph:
    ...
    if manager.options.dump_deps:
        ...
        dump_all_dependencies(manager.modules, manager.all_types, manager.options.python_version, manager.options)
            ...
            for dep in state.dependencies:
                if dep in state.priorities:
                    ...
                    if dep in inv_nodes:
                        ...
                        if dep_id != node.node_id and (dep_id not in node.deps or pri < node.deps[dep_id]):
            ...
            if ignored and dep not in added:
                ...
                try:
                    if dep in st.ancestors:
                        ...
                        newst = State(id=dep, path=None, source=None, manager=manager, ancestor_for=st)
                    else:
                        newst = State(
                            id=dep,
                            path=None,
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    if sccs_left:
        manager.log(
            "{} fresh SCCs ({} nodes) left in queue (and will remain unprocessed)".format(sccs_left, nodes_left)
        )
...
def deps_filtered(graph: Graph, vertices: AbstractSet[str], id: str, pri_max: int) -> list[str]:
    ...
    if id not in vertices:
        return []
    state = graph[id]
    return [dep for dep in state.dependencies if dep in vertices and state.priorities.get(dep, PRI_HIGH) < pri_max]
...
def is_silent_import_module(manager: BuildManager, path: str) -> bool:
    ...
    return any(is_sub_path(path, dir) for dir in manager.search_paths.package_path + manager.search_paths.typeshed_path)
