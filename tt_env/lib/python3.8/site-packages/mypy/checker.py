...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def __init__(
        ...
        self.expr_checker = mypy.checkexpr.ExpressionChecker(self, self.msg, self.plugin, per_line_checking_time_ns)
    ...
    def check_first_pass(self) -> None:
        ...
        with state.strict_optional_set(self.options.strict_optional):
            self.errors.set_file(self.path, self.tree.fullname, scope=self.tscope, options=self.options)
            with self.tscope.module_scope(self.tree.fullname):
                ...
                if all_ is not None and all_.type is not None:
                    ...
                    seq_str = self.named_generic_type("typing.Sequence", [self.named_type("builtins.str")])
                    if not is_subtype(all_.type, seq_str):
                        str_seq_s, all_s = format_type_distinctly(seq_str, all_.type, options=self.options)
                        self.fail(message_registry.ALL_MUST_BE_SEQ_STR.format(str_seq_s, all_s), all_node)
        ...
        with state.strict_optional_set(self.options.strict_optional):
            ...
            with self.tscope.module_scope(self.tree.fullname):
                ...
                for node, active_typeinfo in todo:
                    ...
                    with self.tscope.class_scope(active_typeinfo) if active_typeinfo else nullcontext():
                        with self.scope.push_class(active_typeinfo) if active_typeinfo else nullcontext():
    ...
    def _visit_overloaded_func_def(self, defn: OverloadedFuncDef) -> None:
        ...
        if defn.info:
            ...
            if defn.is_explicit_override and not found_method_base_classes and found_method_base_classes is not None:
    ...
    def check_overlapping_overloads(self, defn: OverloadedFuncDef) -> None:
        ...
        if self.msg.errors.file in self.msg.errors.ignored_files or (self.is_typeshed_stub and self.options.test_env):
        ...
        for i, item in enumerate(defn.items):
            ...
            if impl_type is not None:
                ...
                if not (is_subtype(sig1.ret_type, impl.ret_type) or is_subtype(impl.ret_type, sig1.ret_type)):
    ...
    def get_generator_yield_type(self, return_type: Type, is_coroutine: bool) -> Type:
        ...
        elif not self.is_generator_return_type(return_type, is_coroutine) and not self.is_async_generator_return_type(
            return_type
        ):
        ...
        for item, typ in expanded:
            ...
            with self.binder.top_frame_context():
                ...
                if isinstance(item, FuncDef):
                    ...
                    check_for_explicit_any(fdef.type, self.options, self.is_typeshed_stub, self.msg, context=fdef)
            ...
            with self.binder.top_frame_context():
                ...
                for frame in old_binder.frames:
                    for key, narrowed_type in frame.types.items():
                        ...
                        if key_var is not None and not self.is_var_redefined_in_outer_context(key_var, defn.line):
            ...
            if not unreachable:
                if defn.is_generator or is_named_instance(self.return_types[-1], "typing.AwaitableGenerator"):
                    return_type = self.get_generator_return_type(self.return_types[-1], defn.is_coroutine)
    ...
    def check_unbound_return_typevar(self, typ: CallableType) -> None:
        ...
        if isinstance(typ.ret_type, TypeVarType) and typ.ret_type in typ.variables:
            ...
            if typ.ret_type not in arg_type_visitor.arg_types:
                ...
                if not (isinstance(upper_bound, Instance) and upper_bound.type.fullname == "builtins.object"):
                    self.note(
        ...
        if show_untyped and (self.options.disallow_untyped_defs or check_incomplete_defs):
            if fdef.type is None and self.options.disallow_untyped_defs:
                if not fdef.arguments or (
                    len(fdef.arguments) == 1 and (fdef.arg_names[0] == "self" or fdef.arg_names[0] == "cls")
                ):
                ...
                if is_unannotated_any(ret_type):
                    ...
                    if is_unannotated_any(self.get_generator_return_type(ret_type, fdef.is_coroutine)):
                        self.fail(message_registry.RETURN_TYPE_EXPECTED, fdef)
    ...
    def check___new___signature(self, fdef: FuncDef, typ: CallableType) -> None:
        ...
        elif not isinstance(get_proper_type(bound_type.ret_type), (AnyType, Instance, TupleType, UninhabitedType)):
            self.fail(
                message_registry.NON_INSTANCE_NEW_TYPE.format(format_type(bound_type.ret_type, self.options)),
                fdef,
            )
        else:
    ...
    def check_reverse_op_method(
        ...
        forward_type = self.expr_checker.analyze_external_member_access(forward_name, forward_base, context=defn)
    ...
    def check_inplace_operator_method(self, defn: FuncBase) -> None:
        ...
        if cls.has_readable_member(other_method):
            ...
            typ2 = get_proper_type(self.expr_checker.analyze_external_member_access(other_method, instance, defn))
            fail = False
    ...
    def check_slots_definition(self, typ: Type, context: Context) -> None:
        ...
        expected_type = UnionType([str_type, self.named_generic_type("typing.Iterable", [str_type])])
        self.check_subtype(
            typ,
            expected_type,
            context,
    ...
    def check_match_args(self, var: Var, typ: Type, context: Context) -> None:
        ...
        if not isinstance(typ, TupleType) or not all(is_string_literal(item) for item in typ.items):
            self.msg.note(
                "__match_args__ must be a tuple containing string literals for checking " "of match statements to work",
                context,
    ...
    def check_method_override(self, defn: FuncDef | OverloadedFuncDef | Decorator) -> list[TypeInfo] | None:
    ...
    def check_method_override_for_base_with_name(
        ...
        if base_attr:
            ...
            if isinstance(original_type, AnyType) or isinstance(typ, AnyType):
                ...
                # covariant overrides.
                pass
            elif original_node and not self.is_writable_attribute(original_node) and is_subtype(typ, original_type):
    ...
    def bind_and_map_method(
        ...
        """
        if isinstance(sym.node, (FuncDef, OverloadedFuncDef, Decorator)) and not is_static(sym.node):
    ...
    def check_override(
        ...
        if not is_subtype(override, original, ignore_pos_arg_names=True):
            ...
            if original_domain and override_domain and not is_subtype(override_domain, original_domain):
                fail = True
        ...
        if fail:
            ...
            if (
                ...
                for i in range(len(override.arg_types)):
                    if not is_subtype(original.arg_types[i], erase_override(override.arg_types[i])):
                ...
                if len(order) == len(original.items) and order != sorted(order):
                    self.msg.overload_signature_incompatible_with_supertype(name, name_in_super, supertype, node)
                    emitted_msg = True
    ...
    def check__exit__return_type(self, defn: FuncItem) -> None:
        ...
        if all(isinstance(ret.expr, NameExpr) and ret.expr.fullname == "builtins.False" for ret in returns):
        ...
        with self.tscope.class_scope(defn.info), self.enter_partial_types(is_class=True):
            ...
            if defn.decorators:
                ...
                for decorator in reversed(defn.decorators):
                    if isinstance(decorator, CallExpr) and isinstance(decorator.analyzed, PromoteExpr):
                    ...
                    sig, _ = self.expr_checker.check_call(dec, [temp], [nodes.ARG_POS], defn, callable_name=fullname)
            ...
            for base_inst in typ.bases:
                for base_tvar, base_decl_tvar in zip(base_inst.args, base_inst.type.defn.type_vars):
                    if (
                        ...
                        self.fail(
                            f'Variance of TypeVar "{base_tvar.name}" incompatible ' "with variance in parent type",
                            context=defn,
                            code=codes.TYPE_VAR,
                        )
    ...
    def check_enum(self, defn: ClassDef) -> None:
        ...
        if defn.info.fullname not in ENUM_BASES:
            for sym in defn.info.names.values():
                if isinstance(sym.node, Var) and sym.node.has_explicit_value and sym.node.name == "__members__":
    ...
    def check_enum_new(self, defn: ClassDef) -> None:
        def has_new_method(info: TypeInfo) -> bool:
            ...
            return bool(new_method and new_method.node and new_method.node.fullname != "builtins.object.__new__")
    ...
    def check_protocol_variance(self, defn: ClassDef) -> None:
        ...
        for i, tvar in enumerate(tvars):
            if not isinstance(tvar, TypeVarType):
                ...
                continue
            up_args: list[Type] = [
                object_type if i == j else AnyType(TypeOfAny.special_form) for j, _ in enumerate(tvars)
            ]
            down_args: list[Type] = [
                UninhabitedType() if i == j else AnyType(TypeOfAny.special_form) for j, _ in enumerate(tvars)
            ]
    ...
    def check_metaclass_compatibility(self, typ: TypeInfo) -> None:
        ...
        if typ.is_metaclass() or typ.is_protocol or typ.is_named_tuple or typ.is_enum or typ.typeddict_type is not None:
        ...
        if not metaclasses:
            return
        if typ.metaclass_type is not None and all(is_subtype(typ.metaclass_type, meta) for meta in metaclasses):
            return
        self.fail(
    ...
    def visit_assignment_stmt(self, s: AssignmentStmt) -> None:
        ...
        if s.type is not None and self.options.disallow_any_unimported and has_any_from_unimported_type(s.type):
            if isinstance(s.lvalues[-1], TupleExpr):
                # This is a multiple assignment. Instead of figuring out which type is problematic,
                # give a generic error message.
                self.msg.unimported_type_becomes_any("A type on this line", AnyType(TypeOfAny.special_form), s)
            else:
        ...
        self.check_final(s)
        if s.is_final_def and s.type and not has_no_typevars(s.type) and self.scope.active_class() is not None:
    ...
    def check_assignment(
        ...
        if isinstance(lvalue, (TupleExpr, ListExpr)):
            self.check_assignment_to_multiple_lvalues(lvalue.items, rvalue, rvalue, infer_lvalue_type)
        else:
            ...
            if lvalue_type:
                if isinstance(lvalue_type, PartialType) and lvalue_type.type is None:
                    ...
                    if isinstance(lvalue, RefExpr) and self.check_compatibility_all_supers(lvalue, lvalue_type, rvalue):
                        # We hit an error on this line; don't check for any others
                        return
                elif (
                ...
                elif isinstance(lvalue, MemberExpr) and lvalue.kind is None:  # Ignore member access to modules
                ...
                if (
                    ...
                    and (p_rvalue_type.type_object().is_abstract or p_rvalue_type.type_object().is_protocol)
        ...
        if (
            ...
            for base in lvalue_node.info.mro[1:]:
                ...
                if base_type:
                    ...
                    if not self.check_compatibility_super(lvalue, lvalue_type, rvalue, base, base_type, base_node):
    ...
    def check_assignment_to_slots(self, lvalue: Lvalue) -> None:
        ...
        self.fail(message_registry.NAME_NOT_IN_SLOTS.format(lvalue.name, inst.type.fullname), lvalue)
    ...
    def check_assignment_to_multiple_lvalues(
        ...
        if isinstance(rvalue, (TupleExpr, ListExpr)):
            ...
            for idx_rval, rval in enumerate(self.flatten_rvalues(rvalue.items)):
                if isinstance(rval, StarExpr):
                    ...
                    if self.type_is_iterable(typs) and isinstance(typs, Instance):
                        if iterable_type is not None and iterable_type != self.iterable_item_type(typs, rvalue):
                ...
                if isinstance(rval, StarExpr):
                    ...
                    if self.type_is_iterable(typs) and isinstance(typs, Instance):
                        if iterable_start is None:
                            iterable_start = i
                        iterable_end = i
            if iterable_start is not None and iterable_end is not None and iterable_type is not None:
            ...
            if self.check_rvalue_count_in_assignment(lvalues, len(rvalues), context):
                ...
                left_rvs, star_rvs, right_rvs = self.split_around_star(rvalues, star_index, len(lvalues))
    ...
    def check_multi_assignment(
        ...
        if isinstance(rvalue_type, TupleType) and find_unpack_in_list(rvalue_type.items) is not None:
        ...
        if isinstance(rvalue_type, AnyType):
            for lv in lvalues:
                if isinstance(lv, StarExpr):
                    lv = lv.expr
                temp_node = self.temp_node(AnyType(TypeOfAny.from_another_any, source_any=rvalue_type), context)
            ...
            self.check_multi_assignment_from_union(lvalues, rvalue, rvalue_type, context, infer_lvalue_type)
        elif isinstance(rvalue_type, Instance) and rvalue_type.type.fullname == "builtins.str":
            self.msg.unpacking_strings_disallowed(context)
        else:
            self.check_multi_assignment_from_iterable(lvalues, rvalue_type, context, infer_lvalue_type)
    ...
    def check_multi_assignment_from_tuple(
        ...
        if self.check_rvalue_count_in_assignment(lvalues, len(rvalue_type.items), context, rvalue_unpack=rvalue_unpack):
            star_index = next((i for i, lv in enumerate(lvalues) if isinstance(lv, StarExpr)), len(lvalues))
            ...
            if not undefined_rvalue:
                ...
                reinferred_rvalue_type = get_proper_type(self.expr_checker.accept(rvalue, lvalue_type))
    ...
    def type_is_iterable(self, type: Type) -> bool:
        ...
        if isinstance(type, FunctionLike) and type.is_type_obj():
            type = type.fallback
        return is_subtype(type, self.named_generic_type("typing.Iterable", [AnyType(TypeOfAny.special_form)]))
        ...
        if self.type_is_iterable(rvalue_type) and isinstance(
            ...
            for lv in lvalues:
                if isinstance(lv, StarExpr):
                    ...
                    self.check_assignment(lv.expr, self.temp_node(items_type, context), infer_lvalue_type)
                else:
    ...
    def check_lvalue(self, lvalue: Lvalue) -> tuple[Type | None, IndexExpr | None, Var | None]:
        ...
        if self.is_definition(lvalue) and (not isinstance(lvalue, NameExpr) or isinstance(lvalue.node, Var)):
    ...
    def infer_variable_type(self, name: Var, lvalue: Lvalue, init_type: Type, context: Context) -> None:
        ...
        elif not is_valid_inferred_type(init_type, is_lvalue_final=name.is_final) and not self.no_partial_types:
    ...
    def infer_partial_type(self, name: Var, lvalue: Lvalue, init_type: Type) -> bool:
        ...
        if isinstance(init_type, NoneType):
            ...
            if (
                ...
                and all(isinstance(t, (NoneType, UninhabitedType)) for t in get_proper_types(init_type.args))
            ):
                ...
                if isinstance(arg0, (NoneType, UninhabitedType)) and self.is_valid_defaultdict_partial_value_type(arg1):
    ...
    def check_simple_assignment(
        ...
        *,
        ...
        if self.is_stub and isinstance(rvalue, EllipsisExpr):
            ...
            always_allow_any = lvalue_type is not None and not isinstance(get_proper_type(lvalue_type), AnyType)
            rvalue_type = self.expr_checker.accept(rvalue, lvalue_type, always_allow_any=always_allow_any)
            if (
                ...
                with self.msg.filter_errors() as local_errors, self.local_type_map() as type_map:
                    alt_rvalue_type = self.expr_checker.accept(rvalue, None, always_allow_any=always_allow_any)
                if (
    ...
    def check_member_assignment(
        ...
        if dunder_set is None:
            self.fail(
                message_registry.DESCRIPTOR_SET_NOT_CALLABLE.format(attribute_type.str_with_options(self.options)),
                context,
            )
    ...
    def check_indexed_assignment(self, lvalue: IndexExpr, rvalue: Expression, context: Context) -> None:
        ...
        method_type = self.expr_checker.analyze_external_member_access("__setitem__", basetype, lvalue)
    ...
    def check_return_stmt(self, s: ReturnStmt) -> None:
        ...
        if defn is not None:
            ...
            if s.expr:
                ...
                # Return with a value.
                typ = get_proper_type(
                    self.expr_checker.accept(s.expr, return_type, allow_none_return=allow_none_func_call)
                )
                ...
                if isinstance(typ, AnyType):
                    ...
                    if (
                        ...
                        and not (defn.name in BINARY_MAGIC_METHODS and is_literal_not_implemented(s.expr))
                        and not (isinstance(return_type, Instance) and return_type.type.fullname == "builtins.object")
                        and not is_lambda
                    ):
    ...
    def visit_operator_assignment_stmt(self, s: OperatorAssignmentStmt) -> None:
        ...
        if inplace:
            ...
            expr.set_line(s)
            self.check_assignment(lvalue=s.lvalue, rvalue=expr, infer_lvalue_type=True, new_syntax=False)
        self.check_final(s)
    ...
    def type_check_raise(self, e: Expression, s: RaiseStmt, optional: bool = False) -> None:
        ...
        self.check_subtype(typ, UnionType.make_union(expected_type_items), s, message_registry.INVALID_EXCEPTION)
    ...
    def get_types_from_except_handler(self, typ: Type, n: Expression) -> list[Type]:
        ...
        if isinstance(typ, TupleType):
            ...
            return [
                union_typ for item in typ.relevant_items() for union_typ in self.get_types_from_except_handler(item, n)
            ]
    ...
    def analyze_async_iterable_item_type(self, expr: Expression) -> tuple[Type, Type]:
        ...
        item_type = echk.check_awaitable_expr(awaitable, expr, message_registry.INCOMPATIBLE_TYPES_IN_ASYNC_FOR)
    ...
    def analyze_iterable_item_type_without_expression(self, type: Type, context: Context) -> tuple[Type, Type]:
        ...
        if isinstance(iterable, TupleType) and iterable.partial_fallback.type.fullname == "builtins.tuple":
        ...
        if isinstance(s.expr, IndexExpr):
            ...
            for elt in flatten(s.expr):
                if isinstance(elt, NameExpr):
                    self.binder.assign_type(elt, DeletedType(source=elt.name), get_declaration(elt), False)
    ...
    def visit_decorator_inner(self, e: Decorator, allow_empty: bool = False) -> None:
        ...
        if e.func.info and not e.func.is_dynamic() and not e.is_overload:
            ...
            if e.func.is_explicit_override and not found_method_base_classes and found_method_base_classes is not None:
    ...
    def check_for_untyped_decorator(self, func: FuncDef, dec_type: Type, dec_expr: Expression) -> None:
        if self.options.disallow_untyped_decorators and is_typed_callable(func.type) and is_untyped_decorator(dec_type):
    ...
    def check_async_with_item(self, expr: Expression, target: Expression | None, infer_lvalue_type: bool) -> Type:
        ...
        obj = echk.check_awaitable_expr(obj, expr, message_registry.INCOMPATIBLE_TYPES_IN_ASYNC_WITH_AENTER)
        if target:
        ...
        res, _ = echk.check_method_call_by_name("__aexit__", ctx, [arg] * 3, [nodes.ARG_POS] * 3, expr)
        return echk.check_awaitable_expr(res, expr, message_registry.INCOMPATIBLE_TYPES_IN_ASYNC_WITH_AEXIT)
    ...
    def check_with_item(self, expr: Expression, target: Expression | None, infer_lvalue_type: bool) -> Type:
        ...
        if target:
            self.check_assignment(target, self.temp_node(obj, expr), infer_lvalue_type)
        arg = self.temp_node(AnyType(TypeOfAny.special_form), expr)
        res, _ = echk.check_method_call_by_name("__exit__", ctx, [arg] * 3, [nodes.ARG_POS] * 3, expr)
        return res
    ...
    def visit_match_stmt(self, s: MatchStmt) -> None:
        with self.binder.frame_context(can_skip=False, fall_through=0):
            ...
            for p, g, b in zip(s.patterns, s.guards, s.bodies):
                current_subject_type = self.expr_checker.narrow_type_from_binder(s.subject, subject_type)
                                ...
                                if isinstance(case_target, NameExpr):
                                    for type_map in (guard_map, else_map):
                                        ...
                                        for expr in list(type_map):
                                            if not (
                                                isinstance(expr, NameExpr) and expr.fullname == case_target.fullname
                                            ):
                                                continue
    ...
    def intersect_instances(
        ...
        def _make_fake_typeinfo_and_full_name(
            ...
            cdef, info_ = self.make_fake_typeinfo(curr_module_.fullname, full_name_, short_name, base_classes_)
        ...
        pretty_names_list = pretty_seq(format_type_distinctly(*base_classes, options=self.options, bare=True), "and")
    ...
    def partition_by_callable(self, typ: Type, unsound_partition: bool) -> tuple[list[Type], list[Type]]:
        ...
        if isinstance(typ, UnionType):
            ...
            for subtype in typ.items:
                ...
                subcallables, subuncallables = self.partition_by_callable(subtype, unsound_partition=True)
        ...
        if isinstance(typ, TypeVarType):
            ...
            callables, uncallables = self.partition_by_callable(erase_to_union_or_bound(typ), unsound_partition)
        ...
        if isinstance(ityp, Instance):
            ...
            if method and method.type:
                callables, uncallables = self.partition_by_callable(method.type, unsound_partition=False)
    ...
    def _is_truthy_type(self, t: ProperType) -> bool:
        return (
            ...
            or (isinstance(t, UnionType) and all(self._is_truthy_type(t) for t in get_proper_types(t.items)))
        )
    ...
    def find_type_equals_check(self, node: ComparisonExpr, expr_indices: list[int]) -> tuple[TypeMap, TypeMap]:
        ...
        for expr in exprs_in_type_calls:
            ...
            current_if_map, current_else_map = conditional_types_to_typemaps(expr, current_if_type, current_else_type)
    ...
    def find_isinstance_check_helper(self, node: Expression) -> tuple[TypeMap, TypeMap]:
        ...
        if isinstance(node, CallExpr) and len(node.args) != 0:
            ...
            for i, expr in enumerate(operands):
                ...
                if literal(expr) == LITERAL_TYPE and not is_literal_none(expr) and not self.is_literal_enum(expr):
                    h = literal_hash(expr)
                    if h is not None:
            ...
            for operator, expr_indices in simplified_operator_list:
                if operator in {"is", "is not", "==", "!="}:
                    ...
                    if right_index in narrowable_operand_index_to_hash:
                        if_type, else_type = self.conditional_types_for_iterable(item_type, iterable_type)
    ...
    def refine_identity_comparison_expression(
        ...
        if isinstance(target, LiteralType) and (target.is_enum_literal() or isinstance(target.value, bool)):
    ...
    def refine_tuple_type_with_len(self, typ: TupleType, op: str, size: int) -> tuple[Type | None, Type | None]:
        ...
        if op in ("==", "is"):
            ...
            if min_len < size:
                ...
                no_type = typ.copy_modified(items=prefix + [arg] * (size - min_len) + [unpack] + suffix)
                yes_items = []
    ...
    def refine_instance_type_with_len(self, typ: Instance, op: str, size: int) -> tuple[Type | None, Type | None]:
    ...
    def check_subtype(
        ...
        *,
        ...
        if subtype_label is not None or supertype_label is not None:
            subtype_str, supertype_str = format_type_distinctly(orig_subtype, orig_supertype, options=self.options)
            if subtype_label is not None:
                extra_info.append(subtype_label + " " + subtype_str)
            if supertype_label is not None:
                extra_info.append(supertype_label + " " + supertype_str)
            note_msg = make_inferred_type_note(outer_context or context, subtype, supertype, supertype_str)
    ...
    def check_possible_missing_await(
        ...
        with self.checking_await_set(), self.msg.filter_errors() as local_errors:
            ...
            if aw_type is None:
                return
            if not self.check_subtype(aw_type, supertype, context, msg=message_registry.INCOMPATIBLE_TYPES):
                return
    ...
    def in_checked_function(self) -> bool:
        ...
        - No otherwise.
        """
        return self.options.check_untyped_defs or not self.dynamic_funcs or not self.dynamic_funcs[-1]
    ...
    def enter_partial_types(self, *, is_function: bool = False, is_class: bool = False) -> Iterator[None]:
        ...
        if not self.current_node_deferred:
            for var, context in partial_types.items():
                ...
                if allow_none and isinstance(var.type, PartialType) and var.type.type is None and not permissive:
                    var.type = NoneType()
                else:
    ...
    def is_defined_in_base_class(self, var: Var) -> bool:
        if not var.info:
            return False
        return var.info.fallback_to_any or any(base.get(var.name) is not None for base in var.info.mro[1:])
    ...
    def find_partial_types_in_all_scopes(self, var: Var) -> tuple[bool, bool, dict[Var, Context] | None]:
        ...
        for scope in reversed(self.partial_types):
            if var in scope.map:
                ...
                scope_active = not disallow_other_scopes or scope.is_local == self.partial_types[-1].is_local
...
def conditional_types(
    ...
    if proposed_type_ranges:
        ...
        elif not any(type_range.is_upper_bound for type_range in proposed_type_ranges) and is_proper_subtype(
...
def detach_callable(typ: CallableType, class_type_vars: list[TypeVarLikeType]) -> CallableType:
    ...
    return typ.copy_modified(variables=list(typ.variables) + [tv for tv in class_type_vars if tv in seen_type_vars])
...
def overload_can_never_match(signature: CallableType, other: CallableType) -> bool:
    ...
    exp_signature = expand_type(signature, {tvar.id: erase_def_to_union_or_bound(tvar) for tvar in signature.variables})
    return is_callable_compatible(
...
def is_more_general_arg_prefix(t: FunctionLike, s: FunctionLike) -> bool:
    ...
    if isinstance(t, CallableType):
        if isinstance(s, CallableType):
            return is_callable_compatible(t, s, is_compat=is_proper_subtype, is_proper_subtype=True, ignore_return=True)
    elif isinstance(t, FunctionLike):
        if isinstance(s, FunctionLike):
            if len(t.items) == len(s.items):
                return all(is_same_arg_prefix(items, itemt) for items, itemt in zip(t.items, s.items))
    return False
    ...
    if not typ:
        ...
        if method:
            if isinstance(method, Decorator):
                return is_untyped_decorator(method.func.type) or is_untyped_decorator(method.var.type)
