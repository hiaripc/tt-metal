    ...
    def visit_class_pattern(self, o: ClassPattern) -> PatternType:
        ...
        if len(o.positionals) != 0:
            if self.should_self_match(typ):
                ...
                if has_local_errors:
                    self.msg.fail(
                        message_registry.MISSING_MATCH_ARGS.format(typ.str_with_options(self.options)),
                        o,
                    )
                    return self.early_non_match()
        ...
        for keyword, pattern in keyword_pairs:
            ...
            if has_local_errors or key_type is None:
                key_type = AnyType(TypeOfAny.from_error)
                self.msg.fail(
                    message_registry.CLASS_PATTERN_UNKNOWN_KEYWORD.format(typ.str_with_options(self.options), keyword),
                    pattern,
                )
