...
ErrorTuple: _TypeAlias = Tuple[Optional[str], int, int, int, int, str, str, bool, Optional[ErrorCode]]
...
class Errors:
    ...
    def __init__(
        ...
        *,
        ...
        self.hide_error_codes = hide_error_codes if hide_error_codes is not None else options.hide_error_codes
        ...
        if ignored_codes and info.code:
            ...
            if info.code in original_error_codes:
                ...
                if old_code in ignored_codes:
                    msg = f'Error code changed to {info.code.code}; "type: ignore" comment ' + "may be out of date"
            note = ErrorInfo(
    ...
    def has_many_errors(self) -> bool:
        ...
        if len(self.error_info_map) >= self.options.many_errors_threshold:
            return True
        if sum(len(errors) for errors in self.error_info_map.values()) >= self.options.many_errors_threshold:
            return True
        return False
    ...
    def raise_error(self, use_stdout: bool = True) -> NoReturn:
        ...
        raise CompileError(self.new_messages(), use_stdout=use_stdout, module_with_blocker=self.blocker_module())
    ...
    def format_messages(self, error_info: list[ErrorInfo], source_lines: list[str] | None) -> list[str]:
        ...
        for (
            ...
            if file is not None:
                ...
                s = message
            if not self.hide_error_codes and code and (severity != "note" or code in SHOW_NOTE_CODES):
    ...
    def targets(self) -> set[str]:
        ...
        return {info.target for errs in self.error_info_map.values() for info in errs if info.target}
        ...
        for e in errors:
            ...
            if not self.options.show_error_context:
                ...
                while i >= 0:
                    ...
                    result.append((None, -1, -1, -1, -1, "note", fmt.format(path, line), e.allow_dups, None))
                    i -= 1
            ...
            if not self.options.show_error_context:
                    ...
                    if e.type is None:
                        ...
                        result.append(
                            (
                                ...
                                -1,
                                "note",
                                'In member "{}" of class "{}":'.format(e.function_or_member, e.type),
                                e.allow_dups,
                                None,
                            )
                        )
                ...
                if e.type is None:
                    ...
                    result.append((file, -1, -1, -1, -1, "note", f'In class "{e.type}":', e.allow_dups, None))
...
def report_internal_error(
    ...
    if not options.show_traceback:
        if not options.pdb:
            print(
                "{}: note: please use --show-traceback to print a traceback " "when reporting a bug".format(prefix),
                file=stderr,
            )
    else:
