...
def parse(
    ...
    """
    ignore_errors = (options is not None and options.ignore_errors) or (fnam in errors.ignored_files)
...
class ASTConverter:
    ...
    def fail(self, msg: ErrorMessage, line: int, column: int, blocker: bool = True) -> None:
        if blocker or not self.options.ignore_errors:
            ...
            self.errors.set_file_ignored_lines(self.path, self.type_ignores, self.options.ignore_errors)
    ...
    def as_required_block(
        ...
        b = Block(
            self.fix_function_overloads(self.translate_stmt_list(stmts, can_strip=can_strip, is_coroutine=is_coroutine))
        )
        self.set_block_lines(b, stmts)
        return b
        ...
        for stmt in stmts:
            ...
            if isinstance(stmt, IfStmt) and seen_unconditional_func_def is False:
                ...
                if if_overload_name is not None:
                    (if_block_with_overload, if_unknown_truth_value) = self._get_executable_if_block_with_overloads(
                        stmt
                    )
    ...
    def _check_ifstmt_for_overloads(self, stmt: IfStmt, current_overload_name: str | None = None) -> str | None:
        ...
        if len(stmt.else_body.body) == 1:
            ...
            if (
                ...
                and self._check_ifstmt_for_overloads(stmt.else_body.body[0], current_overload_name) == overload_name
            ):
                return overload_name
        ...
        if no_type_check:
            ...
            try:
                ...
                if (
                    ...
                    arg_types = [
                        (a.type_annotation if a.type_annotation is not None else AnyType(TypeOfAny.unannotated))
                        for a in args
                    ]
                else:
                    # PEP 484 disallows both type annotations and type comments
