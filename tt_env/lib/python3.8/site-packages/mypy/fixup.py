...
class NodeFixer(NodeVisitor[None]):
    ...
    def visit_type_info(self, info: TypeInfo) -> None:
        ...
        try:
            ...
            if info._mro_refs:
                info.mro = [
                    lookup_fully_qualified_typeinfo(self.modules, name, allow_missing=self.allow_missing)
...
class TypeFixer(TypeVisitor[None]):
    ...
    def visit_instance(self, inst: Instance) -> None:
        ...
        inst.type_ref = None
        inst.type = lookup_fully_qualified_typeinfo(self.modules, type_ref, allow_missing=self.allow_missing)
    ...
    def visit_type_alias_type(self, t: TypeAliasType) -> None:
        ...
        if type_ref is None:
            return  # We've already been here.
        t.type_ref = None
        t.alias = lookup_fully_qualified_alias(self.modules, type_ref, allow_missing=self.allow_missing)
        for a in t.args:
            a.accept(self)
...
def lookup_fully_qualified_typeinfo(modules: dict[str, MypyFile], name: str, *, allow_missing: bool) -> TypeInfo:
    ...
    if isinstance(node, TypeInfo):
        ...
        assert allow_missing, "Should never get here in normal mode, got {}:{} instead of TypeInfo".format(
...
def lookup_fully_qualified_alias(modules: dict[str, MypyFile], name: str, *, allow_missing: bool) -> TypeAlias:
    ...
    if isinstance(node, TypeAlias):
        ...
        assert allow_missing, "Should never get here in normal mode, got {}:{} instead of TypeAlias".format(
            type(node).__name__, node.fullname if node else ""
        )
        return missing_alias()
