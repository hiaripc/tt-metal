...
def is_overlapping_types(
    ...
    """
    if isinstance(left, TypeGuardedType) or isinstance(right, TypeGuardedType):  # type: ignore[misc]
        # A type guard forces the new type even if it doesn't overlap the old.
        return True
    ...
    ) or is_proper_subtype(right, left, ignore_promotions=ignore_promotions, ignore_uninhabited=ignore_uninhabited):
        return True
    ...
    if isinstance(left, Instance) and isinstance(right, Instance):
        ...
        if is_subtype(
            left, right, ignore_promotions=ignore_promotions, ignore_uninhabited=ignore_uninhabited
        ) or is_subtype(right, left, ignore_promotions=ignore_promotions, ignore_uninhabited=ignore_uninhabited):
            return True
