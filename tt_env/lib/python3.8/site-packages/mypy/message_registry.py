...
# Invalid types
INVALID_TYPE_RAW_ENUM_VALUE: Final = ErrorMessage("Invalid type: try using Literal[{}.{}] instead?", codes.VALID_TYPE)
...
INCOMPATIBLE_RETURN_VALUE_TYPE: Final = ErrorMessage("Incompatible return value type", codes.RETURN_VALUE)
...
INVALID_EXCEPTION_GROUP: Final = ErrorMessage("Exception type in except* cannot derive from BaseExceptionGroup")
...
INCOMPATIBLE_TYPES_IN_ASSIGNMENT: Final = ErrorMessage("Incompatible types in assignment", code=codes.ASSIGNMENT)
...
INCOMPATIBLE_TYPES_IN_ASYNC_WITH_AENTER: Final = 'Incompatible types in "async with" for "__aenter__"'
...
TOO_MANY_TARGETS_FOR_VARIADIC_UNPACK: Final = ErrorMessage("Too many assignment targets for variadic unpack")
...
INCONSISTENT_ABSTRACT_OVERLOAD: Final = ErrorMessage("Overloaded method has both abstract and non-abstract variants")
MULTIPLE_OVERLOADS_REQUIRED: Final = ErrorMessage("Single overload definition, multiple required")
READ_ONLY_PROPERTY_OVERRIDES_READ_WRITE: Final = ErrorMessage("Read-only property cannot override read-write property")
FORMAT_REQUIRES_MAPPING: Final = "Format requires a mapping"
RETURN_TYPE_CANNOT_BE_CONTRAVARIANT: Final = ErrorMessage("Cannot use a contravariant type variable as return type")
FUNCTION_PARAMETER_CANNOT_BE_COVARIANT: Final = ErrorMessage("Cannot use a covariant type variable as a parameter")
...
INVALID_TYPEDDICT_ARGS: Final = ErrorMessage("Expected keyword arguments, {...}, or dict(...) in TypedDict constructor")
...
NAME_NOT_IN_SLOTS: Final = ErrorMessage('Trying to assign name "{}" that is not in "__slots__" of type "{}"')
...
# Generic
GENERIC_INSTANCE_VAR_CLASS_ACCESS: Final = "Access to generic instance variables via class is ambiguous"
...
IMPLICIT_GENERIC_ANY_BUILTIN: Final = 'Implicit generic "Any". Use "{}" and specify generic parameters'
...
DEPENDENT_FINAL_IN_CLASS_BODY: Final = ErrorMessage("Final name declared in class body cannot depend on type variables")
CANNOT_ACCESS_FINAL_INSTANCE_ATTR: Final = 'Cannot access final instance attribute "{}" on class object'
...
CANNOT_OVERRIDE_INSTANCE_VAR: Final = ErrorMessage(
    'Cannot override instance variable (previously declared on base class "{}") with class ' "variable"
)
CANNOT_OVERRIDE_CLASS_VAR: Final = ErrorMessage(
    'Cannot override class variable (previously declared on base class "{}") with instance ' "variable"
)
...
CLASS_PATTERN_GENERIC_TYPE_ALIAS: Final = "Class pattern class must not be a type alias with type parameters"
...
CLASS_PATTERN_KEYWORD_MATCHES_POSITIONAL: Final = 'Keyword "{}" already matches a positional pattern'
...
DATACLASS_FIELD_ALIAS_MUST_BE_LITERAL: Final = '"alias" argument to dataclass field must be a string literal'
DATACLASS_POST_INIT_MUST_BE_A_FUNCTION: Final = '"__post_init__" method must be an instance method'

# fastparse
FAILED_TO_MERGE_OVERLOADS: Final = ErrorMessage("Condition can't be inferred, unable to merge overloads")
