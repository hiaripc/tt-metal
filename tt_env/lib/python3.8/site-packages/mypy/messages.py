...
class MessageBuilder:
    ...
    def filter_errors(
        ...
        *,
        ...
        return ErrorWatcher(self.errors, filter_errors=filter_errors, save_filtered_errors=save_filtered_errors)
    ...
    def has_no_attr(
        ...
        if isinstance(original_type, Instance) and original_type.type.has_readable_member(member):
            ...
            if isinstance(original_type, FunctionLike) and original_type.is_type_obj():
                self.fail(
                    "The type {} is not generic and not indexable".format(format_type(original_type, self.options)),
                    context,
                )
                return None
            else:
            ...
            # Indexed set.
            self.fail(
                "Unsupported target for indexed assignment ({})".format(format_type(original_type, self.options)),
                context,
                code=codes.INDEX,
            )
            return codes.INDEX
            ...
            if not self.are_type_names_disabled():
                ...
                if isinstance(original_type, Instance) and original_type.type.names:
                    if (
                        ...
                        if module_symbol_table is not None:
                            alternatives |= {k for k, v in module_symbol_table.items() if v.module_public}
                ...
                if not failed:
                    self.fail(
                        '{} has no attribute "{}"{}'.format(format_type(original_type, self.options), member, extra),
                        context,
                        code=codes.ATTR_DEFINED,
                    )
                ...
                typ_format, orig_type_format = format_type_distinctly(typ, original_type, options=self.options)
                if typ_format == '"object"' and any(type(item) == NoneType for item in original_type.items):
                    typ_format = '"None"'
                self.fail(
                    'Item {} of {} has no attribute "{}"{}'.format(typ_format, orig_type_format, member, extra),
                    context,
    ...
    def incompatible_argument(
        ...
        if callee_name is not None:
                ...
                for variant in method, "__r" + method[2:]:
                    ...
                    if name.startswith(f'"{variant}" of'):
                        if op == "in" or variant != method:
                            ...
                            self.unsupported_operand_types(op, arg_type, base, context, code=codes.OPERATOR)
                        else:
                            self.unsupported_operand_types(op, base, arg_type, context, code=codes.OPERATOR)
                        return codes.OPERATOR

            if name.startswith('"__getitem__" of'):
                self.invalid_index_type(arg_type, callee.arg_types[n - 1], base, context, code=codes.INDEX)
                return codes.INDEX
            ...
            if name.startswith('"__setitem__" of'):
                if n == 1:
                    ...
                    info = f" (expression has type {arg_type_str}, " f"target has type {callee_type_str})"
                    error_msg = message_registry.INCOMPATIBLE_TYPES_IN_ASSIGNMENT.with_additional_msg(info)
        ...
        if callee_name == "<list>":
        ...
        elif callee_name == "<dict>" and isinstance(get_proper_type(callee.arg_types[n - 1]), TupleType):
            name = callee_name[1:-1]
            n -= 1
            ...
            msg = ("{} expression in dictionary comprehension has incompatible type {}; " "expected type {}").format(
                "Key" if n == 1 else "Value", actual_type_str, expected_type_str
            )
        elif callee_name == "<generator>":
            actual_type_str, expected_type_str = format_type_distinctly(
                arg_type, callee.arg_types[0], options=self.options
            )
            msg = "Generator has incompatible item type {}; expected {}".format(actual_type_str, expected_type_str)
        else:
    ...
    def incompatible_argument_note(
        ...
        if self.prefer_simple_messages():
            return
        if isinstance(original_caller_type, (Instance, TupleType, TypedDictType, TypeType, CallableType)):
            ...
            if isinstance(callee_type, UnionType):
                for item in callee_type.items:
                    ...
                    if isinstance(item, Instance) and item.type.is_protocol:
                        self.report_protocol_problems(original_caller_type, item, context, code=code)
        if isinstance(callee_type, CallableType) and isinstance(original_caller_type, Instance):
            call = find_member("__call__", original_caller_type, original_caller_type, is_operator=True)
            if call:
    ...
    def maybe_note_concatenate_pos_args(
        ...
        if (
            ...
            names: list[str] = []
            for c, o in zip(callee_type.formal_arguments(), original_caller_type.formal_arguments()):
    ...
    def invalid_index_type(
        ...
        *,
        ...
        index_str, expected_str = format_type_distinctly(index_type, expected_type, options=self.options)
        self.fail(
            "Invalid index type {} for {}; expected type {}".format(index_str, base_str, expected_str),
            context,
            code=code,
        )
    ...
    def unexpected_keyword_argument(self, callee: CallableType, name: str, arg_type: Type, context: Context) -> None:
        ...
        self.unexpected_keyword_argument_for_function(for_function(callee), name, context, matches=matches)
    ...
    def wrong_number_values_to_unpack(self, provided: int, expected: int, context: Context) -> None:
        if provided < expected:
            if provided == 1:
                ...
                self.fail(f"Need more than {provided} values to unpack ({expected} expected)", context)
        elif provided > expected:
            self.fail(f"Too many values to unpack ({expected} expected, {provided} provided)", context)
    ...
    def signature_incompatible_with_supertype(
        ...
        *,
        ...
        self.note("Superclass:", context, offset=ALIGN_OFFSET + OFFSET, allow_dups=ALLOW_DUPS, code=code)
        ...
        self.note("Subclass:", context, offset=ALIGN_OFFSET + OFFSET, allow_dups=ALLOW_DUPS, code=code)
    ...
    def argument_incompatible_with_supertype(
        ...
        self.fail(
            ...
            'supertype defines the argument type as "{}"'.format(arg_num, name, target, arg_type_in_supertype_f),
            context,
            code=codes.OVERRIDE,
            secondary_context=secondary_context,
        )
    ...
    def return_type_incompatible_with_supertype(
        ...
        target = self.override_target(name, name_in_supertype, supertype)
        override_str, original_str = format_type_distinctly(override, original, options=self.options)
        self.fail(
    ...
    def unsafe_super(self, method: str, cls: str, ctx: Context) -> None:
        self.fail(
            'Call to abstract method "{}" of "{}" with trivial body' " via super() is unsafe".format(method, cls),
            ctx,
    ...
    def unsupported_placeholder(self, placeholder: str, context: Context) -> None:
        self.fail(f'Unsupported format character "{placeholder}"', context, code=codes.STRING_FORMATTING)
    ...
    def requires_int_or_single_byte(self, context: Context, format_call: bool = False) -> None:
        self.fail(
            '"{}c" requires an integer in range(256) or a single byte'.format(":" if format_call else "%"),
            context,
            code=codes.STRING_FORMATTING,
        )
    ...
    def no_formal_self(self, name: str, item: CallableType, context: Context) -> None:
        ...
        self.fail(f'Attribute function "{name}" with type {type} does not accept self argument', context)
    ...
    def incompatible_self_argument(
        ...
        self.fail(f'Invalid self argument {arg_type} to {kind} "{name}" with type {sig_type}', context)
    ...
    def incompatible_conditional_function_def(
        ...
        if isinstance(old_type, (CallableType, Overloaded)) and isinstance(new_type, (CallableType, Overloaded)):
    ...
    def cannot_instantiate_abstract_class(
        ...
        attrs_with_none = [
            f'"{a}"' for a, implicit_and_can_return_none in abstract_attributes.items() if implicit_and_can_return_none
        ]
    ...
    def no_overridable_method(self, name: str, context: Context) -> None:
        self.fail(
            f'Method "{name}" is marked as an override, ' "but no base method was found with this name",
            context,
        )

    def explicit_override_decorator_missing(self, name: str, base_name: str, context: Context) -> None:
        self.fail(
            f'Method "{name}" is not using @override ' f'but is overriding a method in class "{base_name}"',
            context,
    ...
    def cant_override_final(self, name: str, base_name: str, ctx: Context) -> None:
        self.fail(
            'Cannot override final attribute "{}"' ' (previously declared in base class "{}")'.format(name, base_name),
            ctx,
        )
    ...
    def overload_inconsistently_applies_decorator(self, decorator: str, context: Context) -> None:
        self.fail(
            f'Overload does not consistently use the "@{decorator}" ' + "decorator on all function signatures.",
            context,
        )

    def overloaded_signatures_overlap(self, index1: int, index2: int, context: Context) -> None:
        self.fail(
            "Overloaded function signatures {} and {} overlap with " "incompatible return types".format(index1, index2),
            context,
    ...
    def overloaded_signature_will_never_match(self, index1: int, index2: int, context: Context) -> None:
        self.fail(
            ...
            "signature {index1}'s parameter type(s) are the same or broader".format(index1=index1, index2=index2),
            context,
        )
    ...
    def overloaded_signatures_ret_specific(self, index: int, context: Context) -> None:
        self.fail(
            "Overloaded function implementation cannot produce return type " "of signature {}".format(index),
            context,
        )
    ...
    def yield_from_invalid_operand_type(self, expr: Type, context: Context) -> Type:
        text = format_type(expr, self.options) if format_type(expr, self.options) != "object" else expr
    ...
    def unsupported_type_type(self, item: Type, context: Context) -> None:
        self.fail(f'Cannot instantiate type "Type[{format_type_bare(item, self.options)}]"', context)
    ...
    def unsupported_target_for_star_typeddict(self, typ: Type, ctx: Context) -> None:
        self.fail(
            "Unsupported type {} for ** expansion in TypedDict".format(format_type(typ, self.options)),
            ctx,
            code=codes.TYPEDDICT_ITEM,
        )
    ...
    def non_required_keys_absent_with_star(self, keys: list[str], ctx: Context) -> None:
        self.fail(
            "Non-required {} not explicitly found in any ** item".format(format_key_list(keys, short=True)),
            ctx,
            code=codes.TYPEDDICT_ITEM,
        )
    ...
    def typeddict_key_not_found(
        ...
        if typ.is_anonymous():
            ...
            if matches:
                self.note("Did you mean {}?".format(pretty_seq(matches, "or")), context, code=err_code)
    ...
    def typeddict_context_ambiguous(self, types: list[TypedDictType], context: Context) -> None:
        ...
        self.fail(f"Type of TypedDict is ambiguous, none of ({formatted_types}) matches cleanly", context)
    ...
    def bad_proto_variance(self, actual: int, tvar_name: str, expected: int, context: Context) -> None:
        msg = capitalize(
            ...
            " {} one is expected".format(variance_string(actual), tvar_name, variance_string(expected))
        )
        self.fail(msg, context)
    ...
    def unreachable_right_operand(self, op_name: str, context: Context) -> None:
        ...
        'op_name' should either be the string "and" or the string "or".
        """
        self.fail(f'Right operand of "{op_name}" is never evaluated', context, code=codes.UNREACHABLE)
    ...
    def impossible_intersection(self, formatted_base_class_list: str, reason: str, context: Context) -> None:
        ...
        self.fail(template.format(formatted_base_class_list, reason), context, code=codes.UNREACHABLE)
        ...
        *,
        ...
        if conflict_types and (
            ...
            for name, got, exp in conflict_types[:MAX_ITEMS]:
                ...
                if not isinstance(exp, (CallableType, Overloaded)) or not isinstance(got, (CallableType, Overloaded)):
                    self.note(
                        "{}: expected {}, got {}".format(name, *format_type_distinctly(exp, got, options=self.options)),
                        context,
                        offset=OFFSET,
                        code=code,
                    )
                    ...
                    if isinstance(exp, CallableType):
                        ...
                        self.pretty_overload(exp, context, 2 * OFFSET, code=code, skip_self=class_obj or is_module)
                    ...
                    if isinstance(got, CallableType):
                        ...
                        self.pretty_overload(got, context, 2 * OFFSET, code=code, skip_self=class_obj or is_module)
        ...
        for name, subflags, superflags in conflict_flags[:MAX_ITEMS]:
            ...
            if IS_CLASS_OR_STATIC in superflags and IS_CLASS_OR_STATIC not in subflags:
                self.note(
                    "Protocol member {}.{} expected class or static method".format(supertype.type.name, name),
                    context,
                    code=code,
                )
            if class_obj and IS_VAR in superflags and (IS_VAR in subflags and IS_CLASSVAR not in subflags):
                self.note(
    ...
    def try_report_long_tuple_assignment_error(
        ...
        if isinstance(subtype, TupleType):
            if (
                ...
                return True
            elif isinstance(supertype, TupleType) and (len(subtype.items) > 10 or len(supertype.items) > 10):
...
def format_type_inner(
    ...
    if isinstance(typ, Instance):
        ...
        if len(literal_items) > 1:
            literal_str = "Literal[{}]".format(", ".join(format_literal_value(t) for t in literal_items))
            ...
            print_as_optional = len(typ.items) - sum(isinstance(get_proper_type(t), NoneType) for t in typ.items) == 1
        ...
        if func.is_type_obj():
            ...
            args = format_callable_args(func.arg_types, func.arg_kinds, func.arg_names, format, verbosity)
...
def pretty_callable(tp: CallableType, options: Options, skip_self: bool = False) -> str:
    ...
    if isinstance(tp.definition, FuncDef) and hasattr(tp.definition, "arguments") and not tp.from_concatenate:
        definition_arg_names = [arg.variable.name for arg in tp.definition.arguments]
        if len(definition_arg_names) > len(tp.arg_names) and definition_arg_names[0] and not skip_self:
            if s:
                s = ", " + s
    ...
    if tp.variables:
        ...
        for tvar in tp.variables:
            if isinstance(tvar, TypeVarType):
                ...
                if isinstance(upper_bound, Instance) and upper_bound.type.fullname != "builtins.object":
...
def append_invariance_notes(notes: list[str], arg_type: Instance, expected_type: Instance) -> list[str]:
    ...
    if (
        ...
        invariant_type = "Dict"
        covariant_suggestion = 'Consider using "Mapping" instead, which is covariant in the value type'
