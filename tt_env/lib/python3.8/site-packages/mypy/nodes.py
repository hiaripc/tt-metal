...
class OverloadedFuncDef(FuncBase, SymbolNode, Statement):
    ...
    def deserialize(cls, data: JsonDict) -> OverloadedFuncDef:
        ...
        res = OverloadedFuncDef([cast(OverloadPart, SymbolNode.deserialize(d)) for d in data["items"]])
...
class FuncDef(FuncItem, SymbolNode, Statement):
    ...
    def serialize(self) -> JsonDict:
        ...
        return {
            ...
            "dataclass_transform_spec": (
                None if self.dataclass_transform_spec is None else self.dataclass_transform_spec.serialize()
            ),
        }
...
class TypeInfo(SymbolNode):
    ...
    def is_metaclass(self) -> bool:
        return self.has_base("builtins.type") or self.fullname == "abc.ABCMeta" or self.fallback_to_any
    ...
    def dump(self, str_conv: mypy.strconv.StrConv, type_str_conv: mypy.types.TypeStrVisitor) -> str:
        ...
        mro = "Mro({})".format(", ".join(item.fullname + str_conv.format_id(item) for item in self.mro))
        names = []
    ...
    def serialize(self) -> JsonDict:
        ...
        data = {
            ...
            "declared_metaclass": (None if self.declared_metaclass is None else self.declared_metaclass.serialize()),
            "metaclass_type": (None if self.metaclass_type is None else self.metaclass_type.serialize()),
            ...
            "dataclass_transform_spec": (
                self.dataclass_transform_spec.serialize() if self.dataclass_transform_spec is not None else None
            ),
        }
        return data
    ...
    def deserialize(cls, data: JsonDict) -> TypeInfo:
        ...
        ti.alt_promote = None if data["alt_promote"] is None else mypy.types.Instance.deserialize(data["alt_promote"])
        ti.declared_metaclass = (
            None if data["declared_metaclass"] is None else mypy.types.Instance.deserialize(data["declared_metaclass"])
        )
        ti.metaclass_type = (
            None if data["metaclass_type"] is None else mypy.types.Instance.deserialize(data["metaclass_type"])
        )
        ...
        ti.typeddict_type = (
            None if data["typeddict_type"] is None else mypy.types.TypedDictType.deserialize(data["typeddict_type"])
        )
        ...
        if data.get("dataclass_transform_spec") is not None:
            ti.dataclass_transform_spec = DataclassTransformSpec.deserialize(data["dataclass_transform_spec"])
        return ti
    ...
    def from_tuple_type(cls, info: TypeInfo) -> TypeAlias:
        ...
        return TypeAlias(
            info.tuple_type.copy_modified(
                ...
                fallback=mypy.types.Instance(info, mypy.types.type_vars_as_args(info.defn.type_vars))
            ),
            info.fullname,
            info.line,
            info.column,
...
class SymbolTableNode:
    ...
    def copy(self) -> SymbolTableNode:
        new = SymbolTableNode(self.kind, self.node, self.module_public, self.implicit, self.module_hidden)
    ...
    def __str__(self) -> str:
        ...
        for key, value in self.items():
            ...
            if isinstance(value, SymbolTableNode):
                if value.fullname != "builtins" and (value.fullname or "").split(".")[-1] not in implicit_module_attrs:
