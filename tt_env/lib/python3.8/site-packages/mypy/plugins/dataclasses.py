...
class DataclassTransformer:
    ...
    def transform(self) -> bool:
        ...
        if (
            ...
            and ("__match_args__" not in info.names or info.names["__match_args__"].plugin_generated)
            and py_version >= (3, 10)
        ):
            str_type = self._api.named_type("builtins.str")
            literals: list[Type] = [LiteralType(attr.name, str_type) for attr in attributes if attr.is_in_init]
    ...
    def _add_internal_post_init_method(self, attributes: list[DataclassAttribute]) -> None:
        add_method_to_class(
            ...
            args=[attr.to_argument(self._cls.info, of="__post_init__") for attr in attributes if attr.is_init_var],
            return_type=NoneType(),
        )
    ...
    def add_slots(self, info: TypeInfo, attributes: list[DataclassAttribute], *, correct_version: bool) -> None:
        ...
        if (info.slots is not None and info.slots != generated_slots) or info.names.get("__slots__"):
            ...
            self._api.fail(
                '"{}" both defines "__slots__" and is used with "slots=True"'.format(self._cls.name),
                self._cls,
            )
            return
    ...
    def collect_attributes(self) -> list[DataclassAttribute] | None:
        ...
        for attr in all_attrs:
            ...
            if found_default and attr.is_in_init and not attr.has_default and not attr.kw_only:
                ...
                self._api.fail("Attributes without a default cannot follow attributes with one", context)
            ...
            if found_kw_sentinel and self._is_kw_only_type(attr.type):
                ...
                if attr.name in current_attr_names:
                    context = Context(line=attr.line, column=attr.column)
                self._api.fail("There may not be more than one field with the KW_ONLY type", context)
    ...
    def _add_dataclass_fields_magic_attribute(self) -> None:
        ...
        if self._spec is _TRANSFORM_SPEC_FOR_DATACLASSES:
            ...
            field_type = any_type
        attr_type = self._api.named_type("builtins.dict", [self._api.named_type("builtins.str"), field_type])
        var = Var(name=attr_name, type=attr_type)
        var.info = self._cls.info
        var._fullname = self._cls.info.fullname + "." + attr_name
        var.is_classvar = True
        self._cls.info.names[attr_name] = SymbolTableNode(kind=MDEF, node=var, plugin_generated=True)
    ...
    def _get_bool_arg(self, name: str, default: bool) -> bool:
        ...
        if isinstance(self._reason, Expression):
            return _get_decorator_bool_argument(ClassDefContext(self._cls, self._reason, self._api), name, default)
...
def dataclass_class_maker_callback(ctx: ClassDefContext) -> bool:
    ...
    transformer = DataclassTransformer(ctx.cls, ctx.reason, _get_transform_spec(ctx.reason), ctx.api)
...
def _has_direct_dataclass_transform_metaclass(info: TypeInfo) -> bool:
    return info.declared_metaclass is not None and info.declared_metaclass.type.dataclass_transform_spec is not None
...
def _get_expanded_dataclasses_fields(
    ...
    if isinstance(typ, UnionType):
        ...
        return _get_expanded_dataclasses_fields(ctx, get_proper_type(typ.upper_bound), display_typ, parent_typ)
...
def _meet_replace_sigs(sigs: list[CallableType]) -> CallableType:
    ...
    """
    args = {name: (typ, kind) for name, typ, kind in zip(sigs[0].arg_names, sigs[0].arg_types, sigs[0].arg_kinds)}

    for sig in sigs[1:]:
        sig_args = {name: (typ, kind) for name, typ, kind in zip(sig.arg_names, sig.arg_types, sig.arg_kinds)}
