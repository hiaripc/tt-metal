...
class DefaultPlugin(Plugin):
    ...
    def get_class_decorator_hook_2(self, fullname: str) -> Callable[[ClassDefContext], bool] | None:
        ...
        if fullname in dataclasses.dataclass_makers:
            ...
            return partial(attrs.attr_class_maker_callback, auto_attribs_default=None, frozen_default=True)
        elif fullname in attrs.attr_define_makers:
            return partial(attrs.attr_class_maker_callback, auto_attribs_default=None, slots_default=True)
...
def typed_dict_get_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx.type, TypedDictType) and len(ctx.arg_types) >= 1 and len(ctx.arg_types[0]) == 1:
    ...
    if (
        ...
        for key in keys:
            ...
            if not is_subtype(default_type, value_type):
                ctx.api.msg.typeddict_setdefault_arguments_inconsistent(default_type, value_type, ctx.context)
                return AnyType(TypeOfAny.from_error)
    ...
    if isinstance(ctx.type, TypedDictType) and len(signature.arg_types) == 1:
        ...
        if ctx.args and ctx.args[0]:
            with ctx.api.msg.filter_errors():
                inferred = get_proper_type(ctx.api.get_expression_type(ctx.args[0][0], type_context=arg_type))
            possible_tds = []
            if isinstance(inferred, TypedDictType):
                possible_tds = [inferred]
            elif isinstance(inferred, UnionType):
                possible_tds = [t for t in get_proper_types(inferred.relevant_items()) if isinstance(t, TypedDictType)]
            items = []
            for td in possible_tds:
                item = arg_type.copy_modified(
                    required_keys=(arg_type.required_keys | td.required_keys) & arg_type.items.keys()
                )
