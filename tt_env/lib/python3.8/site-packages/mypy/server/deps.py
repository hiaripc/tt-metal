...
class DependencyVisitor(TraverserVisitor):
    ...
    def process_type_info(self, info: TypeInfo) -> None:
        ...
        if info.is_protocol:
            for base_info in info.mro[:-1]:
                ...
                self.add_dependency(make_wildcard_trigger(base_info.fullname), target=make_trigger(target))
    ...
    def visit_assignment_stmt(self, o: AssignmentStmt) -> None:
        ...
        if self.use_logical_deps() and o.unanalyzed_type is None:
            ...
            if isinstance(rvalue, CallExpr) and isinstance(rvalue.callee, RefExpr) and rvalue.callee.fullname:
                fname: str | None = None
    ...
    def process_isinstance_call(self, e: CallExpr) -> None:
        ...
        if len(e.args) == 2:
            arg = e.args[1]
            if isinstance(arg, RefExpr) and arg.kind == GDEF and isinstance(arg.node, TypeInfo) and arg.fullname:
    ...
    def add_dependency(self, trigger: str, target: str | None = None) -> None:
        ...
        """
        if trigger.startswith(("<builtins.", "<typing.", "<mypy_extensions.", "<typing_extensions.")):
    ...
    def attribute_triggers(self, typ: Type, name: str) -> list[str]:
        ...
        if isinstance(typ, Instance):
            ...
            if isinstance(typ.item, Instance) and typ.item.type.metaclass_type is not None:
                triggers.append(make_trigger(f"{typ.item.type.metaclass_type.type.fullname}.{name}"))
            return triggers
        else:
            return []
