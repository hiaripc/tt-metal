...
class FineGrainedBuildManager:
    ...
    def update(
        ...
        self.manager.log_fine_grained("==== update %s ====" % ", ".join(repr(id) for id, _ in changed_modules))
        ...
        while True:
            result = self.update_one(changed_modules, initial_set, removed_set, blocking_error, followed)
    ...
    def update_one(
        ...
        if next_id == blocking_error and next_id not in self.previous_modules and next_id not in initial_set:
            self.manager.log_fine_grained(f"skip {next_id!r} (module with blocking error not in import graph)")
        ...
        self.manager.log_fine_grained(f"update once: {next_id} in {t1 - t0:.3f}s - {len(changed_modules)} left")
    ...
    def update_module(
        ...
        result = update_module_isolated(module, path, manager, previous_modules, graph, force_removed, followed)
    ...
    if to_process:
        if is_verbose(manager):
            manager.log_fine_grained(
                "Calling process_fresh_modules on set of size {} ({})".format(len(to_process), sorted(to_process))
            )
...
def propagate_changes_using_dependencies(
    ...
    while triggered or targets_with_errors:
        ...
        todo, unloaded, stale_protos = find_targets_recursive(manager, graph, triggered, deps, up_to_date_modules)
...
def reprocess_nodes(
    ...
    changed = compare_symbol_table_snapshots(file_node.fullname, old_symbols_snapshot, new_symbols_snapshot)
...
def update_deps(
    ...
    for deferred in nodes:
        ...
        new_deps = get_dependencies_of_target(module_id, tree, node, type_map, options.python_version)
