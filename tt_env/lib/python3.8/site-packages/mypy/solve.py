...
def solve_constraints(
    ...
    if allow_polymorphic:
        if constraints:
            solutions, free_vars = solve_with_dependent(vars + extra_vars, constraints, vars, originals)
        else:
            solutions = {}
            free_vars = []
    else:
        ...
        for tv, cs in cmap.items():
            ...
            if solution is None or not get_vars(solution, [tv for tv in extra_vars if tv not in vars]):
...
def add_secondary_constraints(cs: set[Constraint], lower: Type, upper: Type) -> None:
    ...
    if isinstance(get_proper_type(upper), UnionType) and isinstance(get_proper_type(lower), UnionType):
