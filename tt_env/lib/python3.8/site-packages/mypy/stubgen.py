...
class ASTStubGenerator(BaseStubGenerator, mypy.traverser.TraverserVisitor):
    ...
    def _get_func_args(self, o: FuncDef, ctx: FunctionContext) -> list[ArgSig]:
        ...
        for i, arg_ in enumerate(o.arguments):
            ...
            args.append(ArgSig(name, typename, default=bool(arg_.initializer), default_value=default))

        if ctx.class_info is not None and all(arg.type is None and arg.default is False for arg in args):
            new_args = infer_method_arg_types(ctx.name, ctx.class_info.self_var, [arg.name for arg in args])
            if new_args is not None:
                args = new_args

        is_dataclass_generated = self.analyzed and self.processing_dataclass and o.info.names[o.name].plugin_generated
    ...
    def is_alias_expression(self, expr: Expression, top_level: bool = True) -> bool:
        ...
        if isinstance(expr, CallExpr) and self.get_fullname(expr.callee) in (
            ...
            return (
                top_level and isinstance(expr.node, (FuncDef, Decorator, MypyFile)) or isinstance(expr.node, TypeInfo)
            ) and not self.is_private_member(expr.node.fullname)
        elif (
            isinstance(expr, IndexExpr) and isinstance(expr.base, NameExpr) and not self.is_private_name(expr.base.name)
        ):
    ...
    def get_assign_initializer(self, rvalue: Expression) -> str:
        ...
        if self._current_class.info and self._current_class.info.is_named_tuple and not isinstance(rvalue, TempNode):
            return " = ..."
    ...
    def maybe_unwrap_unary_expr(self, expr: Expression) -> Expression:
        ...
        if expr.op in math_ops:
            while isinstance(expr, UnaryExpr):
                if expr.op not in math_ops or not isinstance(expr.expr, (IntExpr, FloatExpr, ComplexExpr, UnaryExpr)):
                    break
                expr = expr.expr
            return expr
...
class SelfTraverser(mypy.traverser.TraverserVisitor):
    ...
    def visit_assignment_stmt(self, o: AssignmentStmt) -> None:
        lvalue = o.lvalues[0]
        if isinstance(lvalue, MemberExpr) and isinstance(lvalue.expr, NameExpr) and lvalue.expr.name == "self":
...
def remove_blacklisted_modules(modules: list[StubSource]) -> list[StubSource]:
    return [module for module in modules if module.path is None or not is_blacklisted_path(module.path)]
...
def parse_source_file(mod: StubSource, mypy_options: MypyOptions) -> None:
    ...
    mod.ast = mypy.parse.parse(source, fnam=mod.path, module=mod.module, errors=errors, options=mypy_options)
...
def parse_options(args: list[str]) -> Options:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        default="",
        help="specify module search directories, separated by ':' " "(currently only used if --no-import is given)",
    )
    parser.add_argument(
        "-o",
        "--output",
