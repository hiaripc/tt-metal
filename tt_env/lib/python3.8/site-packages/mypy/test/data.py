                ...
                if arg.startswith("version"):
                    ...
                    if compare_op == ">=":
                        if version <= defaults.PYTHON3_VERSION:
                            _item_fail(f"{arg} always true since minimum runtime version is {defaults.PYTHON3_VERSION}")
...
class DataDrivenTestCase(pytest.Item):
    ...
    def __init__(
        ...
        *,
        ...
        if (platform == "windows" and sys.platform != "win32") or (platform == "posix" and sys.platform == "win32"):
            skip = True
        self.skip = skip
        self.xfail = xfail
        self.data = data
    ...
    def repr_failure(self, excinfo: pytest.ExceptionInfo[BaseException], style: Any | None = None) -> str:
        excrepr: object
...
def pytest_addoption(parser: Any) -> None:
    ...
    group.addoption("--mypy-verbose", action="count", help="Set the verbose flag when creating mypy Options")
    group.addoption(
        "--mypyc-showc",
        action="store_true",
        default=False,
...
def pytest_configure(config: pytest.Config) -> None:
    if config.getoption("--update-data") and config.getoption("--numprocesses", default=1) > 1:
        raise pytest.UsageError("--update-data incompatible with parallelized tests; re-run with -n 1")
...
def split_test_cases(parent: DataFileCollector, suite: DataSuite, file: str) -> Iterator[DataDrivenTestCase]:
    ...
    for case_id in cases_iter:
        ...
        if name in test_names:
            raise RuntimeError('Found a duplicate test name "{}" in {} on line {}'.format(name, parent.name, line_no))
...
class DataSuiteCollector(pytest.Class):
    def collect(self) -> Iterator[DataFileCollector]:
        ...
        assert os.path.isdir(suite.data_prefix), f"Test data prefix ({suite.data_prefix}) not set correctly"
...
class DataFileCollector(pytest.Collector):
    ...
    def from_parent(cls, parent: DataSuiteCollector, *, name: str) -> DataFileCollector:  # type: ignore[override]
