...
class TypeCheckSuite(DataSuite):
    ...
    def run_case(self, testcase: DataDrivenTestCase) -> None:
        ...
        incremental = (
            "incremental" in testcase.name.lower() or "incremental" in testcase.file or "serialize" in testcase.file
        )
        if incremental:
            ...
            for dn, dirs, files in os.walk(os.curdir):
                for file in files:
                    ...
                    if m and int(m.group(1)) > num_steps:
                        raise ValueError(
                            "Output file {} exists though test case only has {} runs".format(file, num_steps)
                        )
