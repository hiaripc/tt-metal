...
class TypeAliasType(Type):
    ...
    def _expand_once(self) -> Type:
        ...
        if self.alias.tvar_tuple_index is None:
            ...
            for tvar, sub in zip(self.alias.alias_tvars[:prefix] + self.alias.alias_tvars[prefix + 1 :], start + end):
                mapping[tvar.id] = sub
    ...
    def copy_modified(self, *, args: list[Type] | None = None) -> TypeAliasType:
        return TypeAliasType(self.alias, args if args is not None else self.args.copy(), self.line, self.column)
...
class TypeVarType(TypeVarLikeType):
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TypeVarType):
            return NotImplemented
        return self.id == other.id and self.upper_bound == other.upper_bound and self.values == other.values
...
class Instance(ProperType):
    ...
    def copy_modified(
        ...
        *,
        ...
        new = Instance(
            ...
            self.line,
            self.column,
            last_known_value=(last_known_value if last_known_value is not _dummy else self.last_known_value),
        )
    ...
    def is_singleton_type(self) -> bool:
        ...
        return self.type.is_enum and len(self.get_enum_values()) == 1 or self.type.fullname == "builtins.ellipsis"

    def get_enum_values(self) -> list[str]:
        """Return the list of values for an Enum."""
        return [name for name, sym in self.type.names.items() if isinstance(sym.node, mypy.nodes.Var)]
...
class Parameters(ProperType):
    ...
    def copy_modified(
        ...
        *,
        ...
        return Parameters(
            ...
            is_ellipsis_args=(is_ellipsis_args if is_ellipsis_args is not _dummy else self.is_ellipsis_args),
            variables=variables if variables is not _dummy else self.variables,
            imprecise_arg_kinds=(
                imprecise_arg_kinds if imprecise_arg_kinds is not _dummy else self.imprecise_arg_kinds
            ),
        )
    ...
    def argument_by_name(self, name: str | None) -> FormalArgument | None:
        if name is None:
            return None
        seen_star = False
        for i, (arg_name, kind, typ) in enumerate(zip(self.arg_names, self.arg_kinds, self.arg_types)):
...
class CallableType(FunctionLike):
    ...
    def copy_modified(
        ...
        modified = CallableType(
            ...
            from_concatenate=(from_concatenate if from_concatenate is not _dummy else self.from_concatenate),
            imprecise_arg_kinds=(
                imprecise_arg_kinds if imprecise_arg_kinds is not _dummy else self.imprecise_arg_kinds
            ),
    ...
    def is_type_obj(self) -> bool:
        return self.fallback.type.is_metaclass() and not isinstance(get_proper_type(self.ret_type), UninhabitedType)
    ...
    def with_unpacked_kwargs(self) -> NormalizedCallableType:
        ...
        extra_kinds = [
            ArgKind.ARG_NAMED if name in last_type.required_keys else ArgKind.ARG_NAMED_OPT for name in last_type.items
        ]
    ...
    def deserialize(cls, data: JsonDict) -> CallableType:
        ...
        return CallableType(
            ...
            type_guard=(deserialize_type(data["type_guard"]) if data["type_guard"] is not None else None),
        ...
        if any(isinstance(t, UnpackType) for t in self.items):
            ...
            if begin is None and end is None:
                ...
                if stride == -1:
                    ...
                    return None
            elif (begin is None or unpack_index >= begin >= 0) and (end is not None and unpack_index >= end >= 0):
                ...
                slice_items = self.items[begin:end:stride]
            elif (begin is None or unpack_index >= begin >= 0) and (end is None or unpack_index - total < end < 0):
...
class TypedDictType(ProperType):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and all(left_item_type == right_item_type for (_, left_item_type, right_item_type) in self.zip(other))
            and self.fallback == other.fallback
...
def get_proper_types(
    ...
    if isinstance(types, list):
        ...
        if not any(isinstance(t, (TypeAliasType, TypeGuardedType)) for t in typelist):  # type: ignore[misc]
            ...
            for var in t.variables:
                if isinstance(var, TypeVarType):
                    ...
                    if var.values:
                        ...
                        vs.append(f"{var.name}{f' = {var.default.accept(self)}' if var.has_default()  else ''}")
                else:
...
def flatten_nested_unions(types: Sequence[Type], handle_type_alias_type: bool = True) -> list[Type]:
    ...
    for t in typelist:
        ...
        if isinstance(tp, ProperType) and isinstance(tp, UnionType):
            flat_items.extend(flatten_nested_unions(tp.items, handle_type_alias_type=handle_type_alias_type))
        else:
...
def is_literal_type(typ: ProperType, fallback_fullname: str, value: LiteralValue) -> bool:
    ...
    return isinstance(typ, LiteralType) and typ.fallback.type.fullname == fallback_fullname and typ.value == value
