...
SPECIAL_DUNDERS: Final = frozenset(("__init__", "__new__", "__call__", "__init_subclass__", "__class_getitem__"))
        ...
        for filename, messages in messages_by_file.items():
            if filename is not None:
                xml += JUNIT_TESTCASE_FAIL_TEMPLATE.format(
                    ...
                    filename=filename,
                    time=dt,
                    name="mypy-py{ver}-{platform} {filename}".format(ver=version, platform=platform, filename=filename),
                )
            else:
                xml += JUNIT_TESTCASE_FAIL_TEMPLATE.format(
                    text=escape("\n".join(messages)),
...
def get_class_descriptors(cls: type[object]) -> Sequence[str]:
    ...
    if cls not in fields_cache:
        members = inspect.getmembers(cls, lambda o: inspect.isgetsetdescriptor(o) or inspect.ismemberdescriptor(o))
...
def get_terminal_width() -> int:
    ...
    return int(os.getenv("MYPY_FORCE_TERMINAL_WIDTH", "0")) or shutil.get_terminal_size().columns or DEFAULT_COLUMNS
