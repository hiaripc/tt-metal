...
def get_mypy_config(
    ...
    if compiler_options.separate:
        mypyc_sources = [src for src in mypyc_sources if src.path and not src.path.endswith("__init__.py")]
...
def build_using_shared_lib(
    ...
    for source in sources:
        ...
        extensions.append(get_extension()(full_module_name, sources=[shim_file], extra_compile_args=extra_compile_args))
...
def mypyc_build(
    ...
    *,
    ...
    fscache = FileSystemCache()
    mypyc_sources, all_sources, options = get_mypy_config(paths, only_compile_paths, compiler_options, fscache)
    ...
    use_shared_lib = len(mypyc_sources) > 1 or any("." in x.module for x in mypyc_sources) or always_use_shared_lib
