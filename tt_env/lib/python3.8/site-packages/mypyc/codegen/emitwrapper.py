...
def wrapper_function_header(fn: FuncIR, names: NameGenerator) -> str:
    ...
    """
    return (
        "PyObject *{prefix}{name}(" "PyObject *self, PyObject *const *args, size_t nargs, PyObject *kwnames)"
...
def generate_wrapper_function(fn: FuncIR, emitter: Emitter, source_path: str, module_name: str) -> None:
    ...
    emitter.emit_lines(
        "if (!{}(args, {}, kwnames, &parser{})) {{".format(parse_fn, nargs, "".join(", " + n for n in arg_ptrs)),
        "return NULL;",
        "}",
    )
...
def generate_get_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        "static PyObject *{name}(PyObject *self, PyObject *instance, PyObject *owner) {{".format(name=name)
    )
...
def generate_bool_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        "{}val = {}{}(self);".format(emitter.ctype_spaced(fn.ret_type), NATIVE_PREFIX, fn.cname(emitter.names))
    )
...
def generate_set_del_item_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    if del_name is not None:
        ...
        emitter.emit_line(
            'PyObject *result = PyObject_CallMethod(super, "__delitem__", "O", obj_{});'.format(args[1].name)
        )
...
class WrapperGenerator:
    ...
    def emit_header(self) -> None:
        ...
        self.emitter.emit_line(
            "static PyObject *{name}({input_args}) {{".format(name=self.wrapper_name(), input_args=input_args)
        )
