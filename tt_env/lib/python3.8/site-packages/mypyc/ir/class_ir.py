...
class ClassIR:
    ...
    def serialize(self) -> JsonDict:
        return {
            ...
            "method_decls": [(k, d.id if k in self.methods else d.serialize()) for k, d in self.method_decls.items()],
            # We serialize method fullnames out and put methods in a separate dict
            "methods": [(k, m.id) for k, m in self.methods.items()],
            "glue_methods": [((cir.fullname, k), m.id) for (cir, k), m in self.glue_methods.items()],
            ...
            "trait_vtables": [(cir.fullname, serialize_vtable(v)) for cir, v in self.trait_vtables.items()],
            ...
            "children": ([cir.fullname for cir in self.children] if self.children is not None else None),
