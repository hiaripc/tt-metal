...
def maybe_process_conditional_comparison(self: IRBuilder, e: Expression, true: BasicBlock, false: BasicBlock) -> bool:
    ...
    if not ((is_tagged(ltype) or is_fixed_width_rtype(ltype)) and (is_tagged(rtype) or is_fixed_width_rtype(rtype))):
        return False
    op = e.operators[0]
    if op not in ("==", "!=", "<", "<=", ">", ">="):
        return False
...
def is_borrow_friendly_expr(self: IRBuilder, expr: Expression) -> bool:
    ...
    if isinstance(expr, (IntExpr, FloatExpr, StrExpr, BytesExpr)):
        ...
        return True
    if isinstance(expr, (UnaryExpr, OpExpr, NameExpr, MemberExpr)) and constant_fold_expr(self, expr) is not None:
