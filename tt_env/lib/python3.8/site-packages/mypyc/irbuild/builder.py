...
class IRBuilder:
            ...
            if lvalue.kind == LDEF:
                if symbol not in self.symtables[-1]:
                    ...
                    if self.fn_info.is_generator:
                        return self.add_var_to_env_class(symbol, reg_type, self.fn_info.generator_class, reassign=False)
    ...
    def read(self, target: Value | AssignmentTarget, line: int = -1, can_borrow: bool = False) -> Value:
        ...
        if isinstance(target, AssignmentTargetIndex):
            reg = self.gen_method_call(target.base, "__getitem__", [target.index], target.type, line)
            if reg is not None:
                return reg
    ...
    def assign(self, target: Register | AssignmentTarget, rvalue_reg: Value, line: int) -> None:
        if isinstance(target, Register):
            ...
            target_reg2 = self.gen_method_call(target.base, "__setitem__", [target.index, rvalue_reg], None, line)
    ...
    def process_sequence_assignment(self, target: AssignmentTargetTuple, rvalue: Value, line: int) -> None:
        ...
        for i in range(len(target.items)):
            ...
            if is_list_rprimitive(rvalue.type):
                ...
                item_value = self.builder.gen_method_call(rvalue, "__getitem__", [index], item.type, line)
    ...
    def process_iterator_tuple_assignment_helper(self, litem: AssignmentTarget, ritem: Value, line: int) -> None:
        ...
        self.activate_block(error_block)
        self.add(RaiseStandardError(RaiseStandardError.VALUE_ERROR, "not enough values to unpack", line))
        self.add(Unreachable())
    ...
    def process_iterator_tuple_assignment(self, target: AssignmentTargetTuple, rvalue_reg: Value, line: int) -> None:
        ...
        # have not been assigned.
            ...
            self.activate_block(error_block)
            self.add(RaiseStandardError(RaiseStandardError.VALUE_ERROR, "too many values to unpack", line))
            self.add(Unreachable())

            self.activate_block(ok_block)

    def push_loop_stack(self, continue_block: BasicBlock, break_block: BasicBlock) -> None:
        self.nonlocal_control.append(LoopNonlocalControl(self.nonlocal_control[-1], continue_block, break_block))
    ...
    def get_sequence_type_from_type(self, target_type: Type) -> RType:
        ...
        if isinstance(target_type, UnionType):
            return RUnion.make_simplified_union([self.get_sequence_type_from_type(item) for item in target_type.items])
    ...
    def call_refexpr_with_args(self, expr: CallExpr, callee: RefExpr, arg_values: list[Value]) -> Value:
        ...
        if callee.fullname and expr.arg_kinds == [ARG_POS] * len(arg_values):
            ...
            target = self.builder.matching_call_c(call_c_ops_candidates, arg_values, expr.line, self.node_type(expr))
            if target:
                return target
        ...
        return self.py_call(function, arg_values, expr.line, arg_kinds=expr.arg_kinds, arg_names=expr.arg_names)
        ...
        if arg.initializer:
            ...
            def get_default() -> Value:
                ...
                # add the static/attribute to final_names/the class here.
                    ...
                    return builder.add(GetAttr(builder.fn_info.callable_class.self_reg, name, arg.line))
            ...
            if not reg.type.error_overlap:
                ...
                builder.assign_if_bitmap_unset(target.register, get_default, nb, arg.initializer.line)
                nb += 1
