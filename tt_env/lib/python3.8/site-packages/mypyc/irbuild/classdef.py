    ...
    for stmt in cdef.defs.body:
        if isinstance(stmt, OverloadedFuncDef) and stmt.is_property:
            ...
            if not isinstance(lvalue, NameExpr):
                builder.error("Only assignment to variables is supported in class bodies", stmt.line)
                continue
    ...
    for name, decl in ir.method_decls.items():
        if decl.implicit and decl.is_prop_getter:
            ...
            if setter_name in ir.method_decls:
                setter_ir = gen_property_setter_ir(builder, ir.method_decls[setter_name], cdef, ir.is_trait)
...
class NonExtClassBuilder(ClassBuilder):
    ...
    def add_attr(self, lvalue: NameExpr, stmt: AssignmentStmt) -> None:
        ...
        add_non_ext_class_attr(self.builder, self.non_ext, lvalue, stmt, self.cdef, self.attrs_to_cache)
...
class ExtClassBuilder(ClassBuilder):
    ...
    def add_attr(self, lvalue: NameExpr, stmt: AssignmentStmt) -> None:
        ...
        self.builder.call_c(py_setattr_op, [typ, self.builder.load_str(lvalue.name), value], stmt.line)
...
class DataClassBuilder(ExtClassBuilder):
    ...
    def add_attr(self, lvalue: NameExpr, stmt: AssignmentStmt) -> None:
        add_non_ext_class_attr_ann(self.builder, self.non_ext, lvalue, stmt, self.get_type_annotation)
        add_non_ext_class_attr(self.builder, self.non_ext, lvalue, stmt, self.cdef, self.attrs_to_cache)
    ...
    def finalize(self, ir: ClassIR) -> None:
        ...
        add_dunders_to_non_ext_dict(self.builder, self.non_ext, self.cdef.line, self.add_annotations_to_dict)
        dec = self.builder.accept(next(d for d in self.cdef.decorators if is_dataclass_decorator(d)))
        self.builder.call_c(
            dataclass_sleight_of_hand,
...
def allocate_class(builder: IRBuilder, cdef: ClassDef) -> Value:
    ...
    # Add it to the dict
    builder.call_c(dict_set_item_op, [builder.load_globals_dict(), builder.load_str(cdef.name), tp], cdef.line)
...
def find_non_ext_metaclass(builder: IRBuilder, cdef: ClassDef, bases: Value) -> Value:
    ...
    if cdef.metaclass:
        ...
        declared_metaclass = builder.add(LoadAddress(type_object_op.type, type_object_op.src, cdef.line))
...
def setup_non_ext_dict(builder: IRBuilder, cdef: ClassDef, metaclass: Value, bases: Value) -> Value:
    ...
    has_prepare = builder.call_c(py_hasattr_op, [metaclass, builder.load_str("__prepare__")], cdef.line)
...
def add_non_ext_class_attr_ann(
    ...
    if typ is None:
        ...
        ann_type = get_proper_type(stmt.type)
        if isinstance(stmt.unanalyzed_type, UnboundType) and stmt.unanalyzed_type.original_str_expr is not None:
    ...
    for attr in cl.deletable:
        if attr not in cl.attributes:
            ...
            for base in cl.mro:
                ...
                builder.error(
                    ('Attribute "{}" not defined in "{}" ' + '(defined in "{}")').format(attr, cl.name, base.name),
                    line,
                )
...
def gen_glue_ne_method(builder: IRBuilder, cls: ClassIR, line: int) -> None:
    ...
    with builder.enter_method(cls, "__ne__", object_rprimitive):
        ...
        not_implemented = builder.add(LoadAddress(not_implemented_op.type, not_implemented_op.src, line))
        builder.add(
            Branch(
...
def load_non_ext_class(builder: IRBuilder, ir: ClassIR, non_ext: NonExtClassInfo, line: int) -> Value:
    ...
    class_type_obj = builder.py_call(non_ext.metaclass, [cls_name, non_ext.bases, non_ext.dict], line)
