...
def transform_name_expr(builder: IRBuilder, expr: NameExpr) -> Value:
    ...
    if expr.kind == LDEF and not (isinstance(expr.node, Var) and expr.node.is_suppressed_import):
        ...
        if isinstance(expr.node, Var) and is_none_rprimitive(builder.node_type(expr)) and expr.node.is_inferred:
            builder.error(
                'Local variable "{}" has inferred type None; add an annotation'.format(expr.node.name),
                expr.node.line,
            )
        ...
        if isinstance(expr.node, MypyFile):
            ...
            obj = builder.call_c(dict_get_item_op, [mod_dict, builder.load_str(expr.node.fullname)], expr.line)
            return obj
        else:
    ...
    if isinstance(expr.expr, RefExpr):
        ...
        if isinstance(node, TypeInfo):
            ...
            if class_ir is not None and class_ir.is_ext_class:
                ...
                if sym is not None and isinstance(sym.node, Var) and not sym.node.is_classvar and not sym.node.is_final:
                    builder.error(
                        'Cannot access instance attribute "{}" through class object'.format(expr.name),
                        expr.line,
                    )
                    builder.note(
                        '(Hint: Use "x: Final = ..." or "x: ClassVar = ..." to define ' "a class attribute)",
                        expr.line,
                    )
...
def transform_call_expr(builder: IRBuilder, expr: CallExpr) -> Value:
    ...
    elif isinstance(callee, (NameExpr, MemberExpr)) and isinstance(callee.node, TypeInfo) and callee.node.is_newtype:
    ...
    if isinstance(callee, MemberExpr):
        ...
        return apply_method_specialization(builder, expr, callee) or translate_method_call(builder, expr, callee)
...
def translate_call(builder: IRBuilder, expr: CallExpr, callee: Expression) -> Value:
    ...
    if isinstance(callee, RefExpr):
        return apply_function_specialization(builder, expr, callee) or translate_refexpr_call(builder, expr, callee)

    function = builder.accept(callee)
    args = [builder.accept(arg) for arg in expr.args]
    return builder.py_call(function, args, expr.line, arg_kinds=expr.arg_kinds, arg_names=expr.arg_names)
...
def translate_super_method_call(builder: IRBuilder, expr: CallExpr, callee: SuperExpr) -> Value:
    ...
    if len(callee.call.args) == 2:
        ...
        if not isinstance(self_arg, NameExpr) or not isinstance(self_arg.node, Var) or not self_arg.node.is_self:
            return translate_call(builder, expr, callee)
...
def transform_index_expr(builder: IRBuilder, expr: IndexExpr) -> Value:
    ...
    return builder.gen_method_call(base, "__getitem__", [index_reg], builder.node_type(expr), expr.line)
...
def transform_comparison_expr(builder: IRBuilder, e: ComparisonExpr) -> Value:
    ...
    if first_op in ["in", "not in"] and len(e.operators) == 1 and isinstance(e.operands[1], (TupleExpr, ListExpr)):
    ...
    # x in {...}
    # x not in {...}
    if first_op in ("in", "not in") and len(e.operators) == 1 and isinstance(e.operands[1], SetExpr):
        set_literal = precompute_set_literal(builder, e.operands[1])
        if set_literal is not None:
            lhs = e.operands[0]
            result = builder.builder.call_c(set_in_op, [builder.accept(lhs), set_literal], e.line, bool_rprimitive)
            if first_op == "not in":
    ...
    def go(i: int, prev: Value) -> Value:
        if i == len(e.operators) - 1:
            return transform_basic_comparison(builder, e.operators[i], prev, builder.accept(e.operands[i + 1]), e.line)
...
def transform_basic_comparison(builder: IRBuilder, op: str, left: Value, right: Value, line: int) -> Value:
    if is_int_rprimitive(left.type) and is_int_rprimitive(right.type) and op in int_comparison_op_mapping:
    ...
    if is_fixed_width_rtype(left.type) and op in int_comparison_op_mapping:
        if right.type == left.type:
            ...
            if left.type.is_signed:
                ...
                op_id = ComparisonOp.unsigned_ops[op]
            return builder.builder.comparison_op(left, builder.coerce(right, left.type, line), op_id, line)
    elif is_fixed_width_rtype(right.type) and op in int_comparison_op_mapping and isinstance(left, Integer):
        if right.type.is_signed:
            op_id = ComparisonOp.signed_ops[op]
        else:
            op_id = ComparisonOp.unsigned_ops[op]
        return builder.builder.comparison_op(builder.coerce(left, right.type, line), right, op_id, line)
...
def _visit_list_display(builder: IRBuilder, items: list[Expression], line: int) -> Value:
    return _visit_display(builder, items, builder.new_list_op, list_append_op, list_extend_op, line, True)
...
def transform_tuple_expr(builder: IRBuilder, expr: TupleExpr) -> Value:
    ...
    types = tuple_type.types if isinstance(tuple_type, RTuple) else [object_rprimitive] * len(expr.items)
...
def transform_set_expr(builder: IRBuilder, expr: SetExpr) -> Value:
    return _visit_display(builder, expr.items, builder.new_set_op, set_add_op, set_update_op, expr.line, False)
