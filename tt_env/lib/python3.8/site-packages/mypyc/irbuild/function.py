...
def transform_decorator(builder: IRBuilder, dec: Decorator) -> None:
    func_ir, func_reg = gen_func_item(builder, dec.func, dec.func.name, builder.mapper.fdef_to_sig(dec.func))
...
def transform_lambda_expr(builder: IRBuilder, expr: LambdaExpr) -> Value:
    ...
    for arg, arg_type in zip(expr.arguments, typ.arg_types):
        ...
        runtime_args.append(RuntimeArg(arg.variable.name, builder.type_to_rtype(arg_type), arg.kind))
...
def gen_func_item(
    ...
    if builder.fn_info.is_generator:
        ...
        if builder.fn_info.is_nested and isinstance(fitem, FuncDef) and top_level and top_level.add_nested_funcs_to_env:
    ...
    if builder.fn_info.fitem in builder.encapsulating_funcs:
        for nested_fn in builder.encapsulating_funcs[builder.fn_info.fitem]:
            if isinstance(nested_fn, FuncDef):
                ...
                builder.add_var_to_env_class(nested_fn, object_rprimitive, env_for_func, reassign=False)
...
def has_nested_func_self_reference(builder: IRBuilder, fitem: FuncItem) -> bool:
    ...
    if any(isinstance(sym, FuncBase) for sym in builder.free_variables.get(fitem, set())):
        return True
    return any(has_nested_func_self_reference(builder, nested) for nested in builder.encapsulating_funcs.get(fitem, []))
...
def gen_func_ir(
    ...
    if fn_info.is_nested or fn_info.in_non_ext:
        ...
        if fn_info.is_decorated or is_singledispatch_main_func:
            ...
            func_ir = FuncIR(func_decl, args, blocks, fn_info.fitem.line, traceback_name=fn_info.fitem.name)
        else:
            func_ir = FuncIR(func_decl, args, blocks, fn_info.fitem.line, traceback_name=fn_info.fitem.name)
    return (func_ir, func_reg)
...
def handle_ext_method(builder: IRBuilder, cdef: ClassDef, fdef: FuncDef) -> None:
    ...
    for base in class_ir.mro[1:]:
        if (
            ...
            and name != "__init__"
            and not is_same_method_signature(class_ir.method_decls[name].sig, base.method_decls[name].sig)
        ):
...
def get_args(builder: IRBuilder, rt_args: Sequence[RuntimeArg], line: int) -> ArgInfo:
    ...
    args = [builder.read(builder.add_local_reg(var, type, is_arg=True), line) for var, type in fake_vars]
    arg_names = [
        arg.name if arg.kind.is_named() or (arg.kind.is_optional() and not arg.pos_only) else None for arg in rt_args
    ]
...
def gen_glue_method(
    ...
    if any(kind.is_star() for kind in arg_kinds) and any(not arg.kind.is_star() for arg in target.decl.sig.args):
        do_pycall = True
    ...
    if do_pycall:
        if target.decl.kind == FUNC_STATICMETHOD:
            ...
            first = args[0]
            st = 1
        retval = builder.builder.py_method_call(first, target.name, args[st:], line, arg_kinds[st:], arg_names[st:])
    else:
        retval = builder.builder.call(target.decl, args, arg_kinds, arg_names, line, bitmap_args=bitmap_args)
    ...
    for base_arg, sub_arg in zip(base_sig.real_args(), sub_sig.real_args()):
        ...
        if base_arg.type.error_overlap or sub_arg.type.error_overlap:
            if not is_same_type(base_arg.type, sub_arg.type):
                ...
                builder.error(
                    "Incompatible argument type " + f'"{sub_arg.type}" (base class has type "{base_arg.type}")',
                    line,
                )
...
def generate_singledispatch_dispatch_function(
    ...
    dispatch_cache = builder.builder.get_attr(dispatch_func_obj, "dispatch_cache", dict_rprimitive, line)
...
def gen_calls_to_correct_impl(
    ...
    def gen_native_func_call_and_return(fdef: FuncDef) -> None:
        ...
        ret_val = builder.builder.call(func_decl, arg_info.args, arg_info.arg_kinds, arg_info.arg_names, line)
    ...
    for impl, i in native_ids.items():
        ...
        builder.builder.compare_tagged_condition(passed_id, current_id, "==", call_impl, next_impl, line)
    ...
    ret_val = builder.py_call(impl_to_use, arg_info.args, line, arg_info.arg_kinds, arg_info.arg_names)
    coerced = builder.coerce(ret_val, current_func_decl.sig.ret_type, line)
    builder.add(Return(coerced))
...
def maybe_insert_into_registry_dict(builder: IRBuilder, fitem: FuncDef) -> None:
    ...
    if is_singledispatch_main_func:
        ...
        builder.call_c(py_setattr_op, [dispatch_func_obj, builder.load_str("registry"), registry_dict], line)
    ...
    for singledispatch_func, types in to_register.items():
        ...
        dispatch_func_obj = load_func(builder, singledispatch_func.name, singledispatch_func.fullname, line)
        registry = load_singledispatch_registry(builder, dispatch_func_obj, line)
        for typ in types:
