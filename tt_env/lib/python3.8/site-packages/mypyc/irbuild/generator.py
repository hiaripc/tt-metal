...
def add_helper_to_generator_class(
    ...
    helper_fn_decl = FuncDecl("__mypyc_generator_helper__", fn_info.generator_class.ir.name, builder.module_name, sig)
    helper_fn_ir = FuncIR(helper_fn_decl, arg_regs, blocks, fn_info.fitem.line, traceback_name=fn_info.fitem.name)
...
def add_close_to_generator_class(builder: IRBuilder, fn_info: FuncInfo) -> None:
    ...
    with builder.enter_method(fn_info.generator_class.ir, "close", object_rprimitive, fn_info):
        ...
        generator_exit = builder.load_module_attr_by_fullname("builtins.GeneratorExit", fn_info.fitem.line)
        builder.add(
            MethodCall(
                builder.self(),
                "throw",
        ...
        stop_iteration = builder.load_module_attr_by_fullname("builtins.StopIteration", fn_info.fitem.line)
...
def setup_env_for_generator_class(builder: IRBuilder) -> None:
    ...
    cls.next_label_target = builder.add_var_to_env_class(Var(NEXT_LABEL_ATTR_NAME), int_rprimitive, cls, reassign=False)
