...
class MatchVisitor(TraverserVisitor):
    ...
    def visit_class_pattern(self, pattern: ClassPattern) -> None:
        ...
        isinstance_op = (
            fast_isinstance_op if self.builder.is_builtin_ref_expr(pattern.class_ref) else slow_isinstance_op
        )

        cond = self.builder.call_c(isinstance_op, [self.subject, self.builder.accept(pattern.class_ref)], pattern.line)
    ...
    def visit_sequence_pattern(self, seq_pattern: SequencePattern) -> None:
        ...
        for i, pattern in enumerate(patterns):
            ...
            if star_index is not None and i >= star_index:
                current = self.builder.binary_op(actual_len, self.builder.load_int(min_len - i), "-", pattern.line)
