...
def transform_block(
    ...
    for i, op in enumerate(old_ops):
        ...
        if not dest.is_void and dest not in post_live[key] and not (isinstance(op, RegisterOp) and dest.is_borrowed):
            maybe_append_dec_ref(ops, dest, post_must_defined, key)
    block.ops = ops
...
def after_branch_increfs(
    ...
    if incref:
        return tuple(reg for reg in sorted(incref, key=lambda r: ordering[r]) if reg.type.is_refcounted)
    return ()
...
def make_value_ordering(ir: FuncIR) -> dict[Value, int]:
    ...
    for block in ir.blocks:
        for op in block.ops:
            if isinstance(op, LoadAddress) and isinstance(op.src, Register) and op.src not in result:
