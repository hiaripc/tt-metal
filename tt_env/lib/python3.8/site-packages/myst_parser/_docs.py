    ...
    def run(self):
        ...
        while stack:
            ...
            if len(path) >= min_heading_level:
                title = node[0]
                text = ".".join(str(i) for i in path[min_heading_level - 1 :]) + "." + ("&nbsp;" * 2)
...
class _ConfigBase(SphinxDirective):
    ...
    def field_type(field):
        ftypes: Sequence[str]
        ftypes = get_args(field.type) if get_origin(field.type) is Union else [field.type]
        ctype = " | ".join(
            str("None" if ftype == type(None) else ftype) for ftype in ftypes  # type: ignore[comparison-overlap]
        )
...
class MystConfigDirective(_ConfigBase):
    ...
    def run(self):
        ...
        for name, value, field in config.as_triple():
            ...
            if self.options.get("scope") == "local" and field.metadata.get("global_only"):
                continue
...
class DirectiveDoc(SphinxDirective):
    ...
    def run(self):
        ...
        klass, _ = directives.directive(name, self.state.memo.language, self.state.document)
        if klass is None:
...
class MystLexer(MarkdownLexer):
    ...
    tokens = {
        "root": [
            ...
            (
                r"^(\()([^\n]+)(\)=)(\n)",
                bygroups(token.Punctuation, token.Name.Label, token.Punctuation, token.Text),
            ),
            # :::
