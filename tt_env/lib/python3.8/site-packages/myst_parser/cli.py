...
def print_anchors(args=None):
    ...
    arg_parser.add_argument("-l", "--level", type=int, default=2, help="Maximum heading level.")
    ...
    def _filter_plugin(state: StateCore) -> None:
        state.tokens = [t for t in state.tokens if t.type.startswith("heading_") and int(t.tag[1]) <= args.level]
