...
class ValidatorType(Protocol):
    def __call__(self, inst: Any, field: dc.Field, value: Any, suffix: str = "") -> None:
        ...
...
def instance_of(type_: type[Any] | tuple[type[Any], ...]) -> ValidatorType:
    ...
    def _validator(inst, field, value, suffix=""):
        ...
        if not isinstance(value, type_):
            raise TypeError(
                f"'{field.name}{suffix}' must be of type {type_!r} " f"(got {value!r} that is a {value.__class__!r})."
            )
...
def is_callable(inst, field, value, suffix=""):
    ...
    if not callable(value):
        raise TypeError(f"'{field.name}{suffix}' must be callable " f"(got {value!r} that is a {value.__class__!r}).")
...
def in_(options: Sequence) -> ValidatorType:
    ...
    def _validator(inst, field, value, suffix=""):
        ...
        if not in_options:
            raise ValueError(f"'{field.name}{suffix}' must be in {options!r} (got {value!r})")

    return _validator


def deep_iterable(member_validator: ValidatorType, iterable_validator: ValidatorType | None = None) -> ValidatorType:
    """
