    ...
    for key, val in value.items():
        ...
        if val is None:
            ...
            if "url" in val and not isinstance(val["url"], str):
                raise TypeError(f"'{field.name}[{key}][url]' is not a string: {val['url']!r}")
            if "title" in val and not isinstance(val["title"], str):
                raise TypeError(f"'{field.name}[{key}][title]' is not a string: {val['title']!r}")
            if (
                "classes" in val
                and not isinstance(val["classes"], list)
                and not all(isinstance(c, str) for c in val["classes"])
            ):
                raise TypeError(f"'{field.name}[{key}][classes]' is not a list of str: {val['classes']!r}")
            new_dict[key] = val  # type: ignore[assignment]
        else:
            raise TypeError(f"'{field.name}[{key}]' value is not a string or dict: {val!r}")
...
def check_sub_delimiters(_: "MdParserConfig", field: dc.Field, value: Any) -> None:
    ...
    for delim in value:
        if (not isinstance(delim, str)) or len(delim) != 1:
            raise TypeError(f"'{field.name}' does not contain strings of length 1: {value}")
...
def check_inventories(_: "MdParserConfig", field: dc.Field, value: Any) -> None:
    ...
    for key, val in value.items():
        ...
        if not isinstance(val, (tuple, list)) or len(val) != 2:
            raise TypeError(f"'{field.name}[{key}]' value is not a 2-item list: {val!r}")
...
def check_fence_as_directive(inst: "MdParserConfig", field: dc.Field, value: Any) -> None:
...
def merge_file_level(
    ...
    if "substitutions" in topmatter:
        warning(
            MystWarnings.MD_TOPMATTER,
            "top-level 'substitutions' key is deprecated, " "place under 'myst' key instead",
        )
