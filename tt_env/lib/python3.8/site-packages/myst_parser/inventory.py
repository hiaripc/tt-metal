...
def _load_v2(stream: InventoryFileReader, base_url: str | None) -> InventoryType:
    ...
    for line in stream.read_compressed_lines():
        ...
        if ":" not in type:
            ...
            continue
        if type == "py:module" and type in invdata["objects"] and name in invdata["objects"][type]:
    ...
    for inv_name, inv_data in inventories.items():
        ...
        for domain_obj_name, data in inv_data.items():
            if ":" not in domain_obj_name:
                continue
            domain_name, obj_type = domain_obj_name.split(":", 1)
            if not (match_with_wildcard(domain_name, domains) and match_with_wildcard(obj_type, otypes)):
                continue
            for target in data:
                if match_with_wildcard(target, targets):
...
def fetch_inventory(uri: str, *, timeout: None | float = None, base_url: None | str = None) -> InventoryType:
...
def inventory_cli(inputs: None | list[str] = None):
    ...
    for match in filter_inventories(
        ...
        if args.loc and not match_with_wildcard(match.loc, args.loc):
            continue
        filtered["objects"].setdefault(match.domain, {}).setdefault(match.otype, {})[match.name] = {
            "loc": match.loc,
            "text": match.text,
        }
