...
"""
REGEX_URI_TEMPLATE = re.compile(r"{{\s*(uri|scheme|netloc|path|params|query|fragment)\s*}}")
...
class DocutilsRenderer(RendererProtocol):
    ...
    def setup_render(self, options: dict[str, Any], env: MutableMapping[str, Any]) -> None:
        ...
        self.language_module_rst: ModuleType = get_language_rst(self.document.settings.language_code)
        self._heading_offset: int = 0
        # a mapping of heading levels to its currently associated node
        self._level_to_section: dict[int, nodes.document | nodes.section] = {0: self.document}
    ...
    def render(self, tokens: Sequence[Token], options, md_env: MutableMapping[str, Any]) -> nodes.document:
    ...
    def _render_finalise(self) -> None:
        ...
        if self._heading_slugs and self.sphinx_env:
            self.sphinx_env.metadata[self.sphinx_env.docname]["myst_slugs"] = self._heading_slugs
        ...
        if wordcount_metadata:
            ...
            for key in ("words", "minutes"):
                ...
                if value is None:
                    continue
                substitution_node = nodes.substitution_definition(str(value), nodes.Text(str(value)))
    ...
    def nested_render_text(
        ...
        """
        tokens = self.md.parseInline(text, self.md_env) if inline else self.md.parse(text + "\n", self.md_env)
    ...
    @contextmanager
    def current_node_context(self, node: nodes.Element, append: bool = False) -> Iterator[None]:
    ...
    def add_line_and_source_path_r(self, nodes_: list[nodes.Element], token: SyntaxTreeNode) -> None:
    ...
    def update_section_level_state(self, section: nodes.section, level: int) -> None:
        ...
        parent_level = max(section_level for section_level in self._level_to_section if level > section_level)
    ...
    def create_highlighted_code_block(
        ...
        if self.sphinx_env is not None:
            ...
            if isinstance(emphasize_lines, str):
                try:
                    emphasize_lines = self._parse_linenos(emphasize_lines, len(text.splitlines()))
            ...
            node = node_cls(text, classes=["code"] + ([lexer_name] if lexer_name else []))
            try:
                lex_tokens = Lexer(
                    text,
                    lexer_name or "",
                ...
                self.reporter.warning(
                    str(err),
                    **{name: value for name, value in (("source", source), ("line", line)) if value is not None},
                )
                lex_tokens = Lexer(text, lexer_name or "", "none")

            if number_lines:
                lex_tokens = NumberLines(lex_tokens, lineno_start, lineno_start + len(text.splitlines()))
    ...
    def render_code_block(self, token: SyntaxTreeNode) -> None:
        ...
        lineno_start = 1
        number_lines = False
        emphasize_lines = str(token.attrs.get("emphasize-lines")) if "emphasize-lines" in token.attrs else None
    ...
    def render_fence(self, token: SyntaxTreeNode) -> None:
        ...
        if (not self.md_config.commonmark_only) and (not self.md_config.gfm_only):
            ...
            if name in self.md_config.fence_as_directive:
                ...
                return self.render_directive(token, name, arguments, additional_options=options)
    ...
    def render_heading(self, token: SyntaxTreeNode) -> None:
        ...
        parent_of_temp_root = (
            self.md_env.get("temp_root_node", None) is not None and self.current_node == self.md_env["temp_root_node"]
        )
        if not (parent_of_temp_root or isinstance(self.current_node, (nodes.document, nodes.section))):
    ...
    def render_link(self, token: SyntaxTreeNode) -> None:
        ...
        """
        if self.md_config.commonmark_only or self.md_config.gfm_only or self.md_config.all_links_external:
    ...
    def render_link_url(self, token: SyntaxTreeNode, conversion: None | UrlSchemeType = None) -> None:
        ...
        self.copy_attributes(token, ref_node, attribute_keys, aliases={"title": "reftitle"})
    ...
    def render_link_anchor(self, token: SyntaxTreeNode, target: str) -> None:
        ...
        self.copy_attributes(token, ref_node, ("class", "id", "reftitle"), aliases={"title": "reftitle"})
    ...
    def render_link_inventory(self, token: SyntaxTreeNode) -> None:
        ...
        # find the matches
        matches = self.get_inventory_matches(target=target, invs=invs, domains=domains, otypes=otypes)
        ...
        if len(matches) > 1:
            ...
            filter_str = inventory.filter_string(invs, domains, otypes, target)
            matches_str = ", ".join(
                [inventory.filter_string(m.inv, m.domain, m.otype, m.name) for m in matches[:show_num]]
            )
        ...
        ref_node["inv_match"] = inventory.filter_string(match.inv, match.domain, match.otype, match.name)
        self.add_line_and_source_path(ref_node, token)
        self.copy_attributes(token, ref_node, ("class", "id", "reftitle"), aliases={"title": "reftitle"})
        ref_node["refuri"] = posixpath.join(match.base_url, match.loc) if match.base_url else match.loc
    ...
    def render_image(self, token: SyntaxTreeNode) -> None:
        ...
        if self.md_env.get("relative-images", None) is not None and not REGEX_SCHEME.match(destination):
    ...
    def render_front_matter(self, token: SyntaxTreeNode) -> None:
        ...
        fields = {k: v for k, v in data.items() if k not in ("myst", "mystnb", "substitutions", "html_meta")}
        if fields:
            field_list = self.dict_to_fm_field_list(fields, language_code=self.document.settings.language_code)
    ...
    def dict_to_fm_field_list(self, data: dict[str, Any], language_code: str, line: int = 0) -> nodes.field_list:
    ...
    def render_table_row(self, token: SyntaxTreeNode) -> None:
        ...
        with self.current_node_context(row, append=True):
            for child in token.children or []:
                ...
                with self.current_node_context(entry, append=True), self.current_node_context(para, append=True):
    ...
    def render_amsmath(self, token: SyntaxTreeNode) -> None:
        ...
        node = nodes.math_block(token.content, token.content, nowrap=True, classes=["amsmath"])
    ...
    def render_myst_role(self, token: SyntaxTreeNode) -> None:
        ...
        role_func, messages = roles.role(name, self.language_module_rst, lineno, self.reporter)
        if not role_func:
            self.create_warning(
                ...
                if child.type == "dt":
                    ...
                    if item is None:
                        error = self.reporter.error(
                            ("Found a definition in a definition list, " "with no preceding term"),
                    ...
                    error_msg = self.reporter.error(
                        ("Expected a term/definition as a child of a definition list" f", but found a: {child.type}"),
                        # nodes.literal_block(content, content),
                        line=token_line(child),
                    )
        ...
        with self.current_node_context(field_list, append=True):
            ...
            while children:
                ...
                if child.type != "fieldlist_name":
                    error_msg = self.reporter.error(
                        ("Expected a fieldlist_name as a child of a field_list" f", but found a: {child.type}"),
                        # nodes.literal_block(content, content),
                        line=token_line(child),
                    )
                    self.current_node += [error_msg]
    ...
    def run_directive(
        ...
        try:
            ...
            msg_node = self.reporter.system_message(error.level, error.msg, line=position)
        ...
        assert isinstance(result, list), f'Directive "{name}" must return a list of nodes.'
    ...
    def render_substitution(self, token: SyntaxTreeNode, inline: bool) -> None:
        ...
        # try rendering
        try:
            rendered = env.from_string(f"{{{{{token.content}}}}}").render(variable_context)
        except Exception as error:
            self.create_warning(
        ...
        references = {n.name for n in ast.find_all(jinja2.nodes.Name) if n.name != "env"}
...
def compute_unique_slug(
    ...
    title = "".join(child.content for child in (inline_token.children or []) if child.type in ["text", "code_inline"])
