...
class SphinxRenderer(DocutilsRenderer):
    ...
    def _handle_relative_docs(self, destination: str) -> str:
        ...
        if relative_include is not None and destination.startswith(relative_include[0]):
            ...
            destination = os.path.relpath(os.path.join(include_dir, os.path.normpath(destination)), source_dir)
        return destination
    ...
    def render_link_unknown(self, token: SyntaxTreeNode) -> None:
        ...
        if potential_path and potential_path.is_file():
            ...
            if docname:
                wrap_node = addnodes.pending_xref(refdomain="doc", reftarget=docname, reftargetid=path_id, **kwargs)
                classes = ["xref", "myst"]
            else:
                wrap_node = addnodes.download_reference(refdomain=None, reftarget=path_dest, **kwargs)
                classes = ["xref", "download", "myst"]
        else:
            wrap_node = addnodes.pending_xref(refdomain=None, reftarget=destination, **kwargs)
            classes = ["xref", "myst"]
    ...
    def render_math_block_label(self, token: SyntaxTreeNode) -> None:
        ...
        node = nodes.math_block(content, content, nowrap=False, number=None, label=label)
    ...
    def render_amsmath(self, token: SyntaxTreeNode) -> None:
        ...
        if token.meta["numbered"] != "*":
            ...
            node = nodes.math_block(content, content, nowrap=True, number=None, classes=["amsmath"])
        self.add_line_and_source_path(node, token)
        self.current_node.append(node)
