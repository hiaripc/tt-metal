...
def parse_directive_text(
    ...
    *,
    ...
    return DirectiveParsingResult(arguments, options, body_lines, content_offset, parse_warnings)
...
def _parse_directive_options(
    ...
    if unknown_options:
        validation_errors.append(
            ParseWarnings(
                f"Unknown option keys: {sorted(unknown_options)} " f"(allowed: {sorted(options_spec)})",
                line,
                MystWarnings.DIRECTIVE_OPTION,
            )
        )
...
def parse_directive_arguments(directive_cls: type[Directive], arg_text: str) -> list[str]:
    ...
    if len(arguments) < required:
        ...
        if directive_cls.final_argument_whitespace:
            ...
            raise MarkupError(f"maximum {required + optional} argument(s) allowed, " f"{len(arguments)} supplied")
    return arguments
