...
def _validate_int(setting, value, option_parser, config_parser=None, config_section=None) -> int:
    """Validate an integer setting."""
    return int(value)


def _validate_comma_separated_set(setting, value, option_parser, config_parser=None, config_section=None) -> Set[str]:
    """Validate an integer setting."""
    value = frontend.validate_comma_separated_list(setting, value, option_parser, config_parser, config_section)
    return set(value)
...
def _create_validate_tuple(length: int) -> Callable[..., Tuple[str, ...]]:
    ...
    def _validate(setting, value, option_parser, config_parser=None, config_section=None):
        ...
        if len(string_list) != length:
            raise ValueError(f"Expecting {length} items in {setting}, got {len(string_list)}.")
...
def _create_validate_yaml(field: Field):
    ...
    def _validate_yaml(setting, value, option_parser, config_parser=None, config_section=None):
...
def _validate_url_schemes(setting, value, option_parser, config_parser=None, config_section=None):
...
class Parser(RstParser):
    ...
    def parse(self, inputstring: str, document: nodes.document) -> None:
            ...
            for i, line in enumerate(inputstring.split("\n")):
                if len(line) > document.settings.line_length_limit:
                    error = document.reporter.error(
                        f"Line {i+1} exceeds the line-length-limit:" f" {document.settings.line_length_limit}."
                    )
        ...
        if "attrs_image" in config.enable_extensions:
            create_warning(
                document,
                "The `attrs_image` extension is deprecated, " "please use `attrs_inline` instead.",
...
def _run_cli(writer_name: str, writer_description: str, argv: Optional[List[str]]):
    ...
    publish_cmdline(
        ...
        description=(f"Generates {writer_description} from standalone MyST sources.\n{default_description}"),
        argv=argv,
    )
...
def cli_html5_demo(argv: Optional[List[str]] = None):
    ...
    publish_cmdline(
        parser=Parser(),
        writer=SimpleWriter(),
        description=(f"Generates body HTML5 from standalone MyST sources.\n{default_description}"),
        settings_overrides={
