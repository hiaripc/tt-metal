...
class TokenizeError(Exception):
    ...
    def __str__(self) -> str:
        ...
        if self.context_mark is not None and (
            self.context_mark.line != self.problem_mark.line or self.context_mark.column != self.problem_mark.column
        ):
            lines.append(f"at line {self.context_mark.line}, column {self.context_mark.column}")
        if self.problem is not None:
            lines.append(self.problem)
        if self.problem_mark is not None:
            lines.append(f"at line {self.problem_mark.line}, column {self.problem_mark.column}")
        return "\n".join(lines)
...
def to_items(text: str, line_offset: int = 0, column_offset: int = 0) -> tuple[list[tuple[str, str]], State]:
    ...
    for key_token, value_token in _to_tokens(text, state, line_offset, column_offset):
        output.append((key_token.value, value_token.value if value_token is not None else ""))
    return output, state
...
def _tokenize(text: str, state: State) -> Iterable[Token]:
    ...
    while True:
        ...
        if not stream.column == 0:
            raise TokenizeError("expected key to start at column 0", stream.get_position())
...
def _scan_plain_scalar(stream: StreamBuffer, state: State, is_key: bool = False) -> KeyToken | ValueToken:
    chunks = []
    start_mark = stream.get_position()
    end_mark = start_mark
    ...
    while True:
        ...
        while True:
            ...
            if ch in _CHARS_END_SPACE_TAB_NEWLINE or (
                is_key and ch == ":" and stream.peek(length + 1) in _CHARS_END_SPACE_TAB_NEWLINE
            ):
                break
            length += 1
        if length == 0:
    ...
    return (
        KeyToken(start_mark, end_mark, "".join(chunks)) if is_key else ValueToken(start_mark, end_mark, "".join(chunks))
    )
...
def _scan_flow_scalar_non_spaces(stream: StreamBuffer, double: bool, start_mark: Position) -> list[str]:
    chunks = []
    while True:
        length = 0
...
def _scan_block_scalar(stream: StreamBuffer, style: Literal["|", ">"], state: State) -> ValueToken:
    indent = 0
    folded = style == ">"
    chunks = []
    start_mark = stream.get_position()
...
def _scan_block_scalar_indicators(stream: StreamBuffer, start_mark: Position) -> tuple[bool | None, int | None]:
    chomping = None
    increment = None
    ch = stream.peek()
    if ch in "+-":
...
def _scan_block_scalar_ignored_line(stream: StreamBuffer, start_mark: Position, state: State) -> None:
    while stream.peek() == " ":
        stream.forward()
    if stream.peek() == "#":
        state.has_comments = True
...
def _scan_block_scalar_breaks(stream: StreamBuffer, indent: int) -> tuple[list[str], Position]:
    chunks = []
    end_mark = stream.get_position()
