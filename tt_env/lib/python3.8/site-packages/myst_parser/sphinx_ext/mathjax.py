...
def override_mathjax(app: Sphinx):
    ...
    if "mathjax3_config" in app.config:
        ...
        if (
            ...
            and app.config.mathjax3_config["options"]["processHtmlClass"] != mjax_classes
        ):
...
def html_visit_displaymath(self: HTMLTranslator, node: nodes.math_block) -> None:
    ...
    if "amsmath" in node.get("classes", []):
        self.body.append(self.starttag(node, "div", CLASS="math notranslate nohighlight amsmath"))
        if node["number"]:
