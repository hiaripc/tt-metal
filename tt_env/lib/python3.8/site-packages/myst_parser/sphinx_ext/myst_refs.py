...
class MystReferenceResolver(ReferencesResolver):
    ...
    def log_warning(self, target: None | str, msg: str, subtype: MystWarnings, **kwargs: Any):
        ...
        if (
            ...
            and any(
                (re.fullmatch(ignore_type, "myst") and re.fullmatch(ignore_target, target))
    ...
    def run(self, **kwargs: Any) -> None:
        ...
        for node in findall(self.document)(addnodes.pending_xref):
            ...
            try:
                newnode = self.resolve_myst_ref_any(refdoc, node, contnode, search_domains)
            except NoUri:
                newnode = contnode
            if newnode is None:
                # If no local domain could resolve the reference, try to
                # resolve it as an inter-sphinx reference
                newnode = self._resolve_myst_ref_intersphinx(node, contnode, target, search_domains)
            if newnode is None:
            ...
            if len(newnode.children) == 1 and isinstance(newnode[0], nodes.inline) and not (newnode[0].children):
    ...
    def resolve_myst_ref_doc(self, node: pending_xref):
        ...
        if node["refexplicit"]:
            ...
            innernode = nodes.inline(implicit_text, implicit_text, classes=inner_classes)

        assert self.app.builder
        try:
            ref_node = make_refnode(self.app.builder, from_docname, ref_docname, targetid, innernode)
        except NoUri:
            ref_node = innernode
    ...
    def resolve_myst_ref_any(
        ...
        if only_domains is None or "std" in only_domains:
            ...
            for objtype in stddomain.object_types:
                ...
                if key in stddomain.objects:
                    ...
                    ref_node = make_refnode(self.app.builder, refdoc, docname, labelid, contnode)
        ...
        if len(results) > 1:
            ...
            self.log_warning(
                target,
                f"more than one target found for 'myst' cross-reference {target}: " f"could be {candidates}",
    ...
    def _resolve_ref_nested(self, node: pending_xref, fromdocname: str, target=None) -> Element | None:
    ...
    def _resolve_doc_nested(self, node: pending_xref, fromdocname: str) -> Element | None:
    ...
    def _resolve_myst_ref_intersphinx(
        ...
        if node.get("refexplicit"):
            ...
            newnode.append(contnode.__class__(match.text, match.text, classes=["iref", "myst"]))
        else:
            newnode.append(nodes.literal(match.name, match.name, classes=["iref", "myst"]))
