    ...
    if msg and "header" in msg:  # The test mocks don't provide a header, so tolerate that
        ...
        if "date" in msg_header and isinstance(msg_header["date"], datetime.datetime):
            try:
                ...
                formatted_time = datetime.datetime.strftime(msg_header["date"], "%Y-%m-%dT%H:%M:%S.%fZ")
                if formatted_time:  # docs indicate strftime may return empty string, so let's catch that too
...
class NotebookClient(LoggingConfigurable):
    ...
    def __init__(self, nb: NotebookNode, km: KernelManager | None = None, **kw: t.Any) -> None:
        ...
        self.widget_registry: dict[str, dict[str, t.Any]] = {"@jupyter-widgets/output": {"OutputModel": OutputWidget}}
        # comm_open_handlers should return an object with a .handle_msg(msg) method or None
        self.comm_open_handlers: dict[str, t.Any] = {"jupyter.widget": self.on_comm_open_jupyter_widget}
    ...
    async def async_start_new_kernel(self, **kwargs: t.Any) -> None:
        ...
        has_history_manager_arg = any(arg.startswith("--HistoryManager.hist_file") for arg in self.extra_arguments)
        if (
    ...
    async def _async_handle_timeout(self, timeout: int, cell: NotebookNode | None = None) -> None | dict[str, t.Any]:
    ...
    async def async_wait_for_reply(self, msg_id: str, cell: NotebookNode | None = None) -> dict[str, t.Any] | None:
    ...
    async def async_execute_cell(
        ...
        parent_msg_id = await ensure_async(
            self.kc.execute(cell.source, store_history=store_history, stop_on_error=not cell_allows_errors)
        )
        ...
        self.task_poll_for_reply = asyncio.ensure_future(
            self._async_poll_for_reply(parent_msg_id, cell, exec_timeout, task_poll_output_msg, task_poll_kernel_alive)
        )
        try:
    ...
    def process_message(self, msg: dict[str, t.Any], cell: NotebookNode, cell_index: int) -> NotebookNode | None:
        """
    ...
    def clear_output(self, outs: list[NotebookNode], msg: dict[str, t.Any], cell_index: int) -> None:
        """Clear output."""
        content = msg["content"]
    ...
    def handle_comm_msg(self, outs: list[NotebookNode], msg: dict[str, t.Any], cell_index: int) -> None:
