...
class Exporter(LoggingConfigurable):
    ...
    enabled = Bool(True, help="Disable this exporter (and any exporters inherited from it).").tag(config=True)

    file_extension = FilenameExtension(help="Extension of the file that should be written to disk").tag(config=True)
    ...
    preprocessors: List[t.Any] = List(help="""List of preprocessors, by name or namespace, to enable.""").tag(
        config=True
    )
    ...
    def from_filename(
        ...
        modified_date = datetime.datetime.fromtimestamp(os.path.getmtime(filename), tz=datetime.timezone.utc)
