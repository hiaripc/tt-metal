...
class TemplateExporter(Exporter):
    ...
    __doc__ = __doc__.format(filters="- " + "\n    - ".join(sorted(default_filters.keys()))) if __doc__ else None
    ...
    template_name = Unicode(help="Name of the template to use").tag(config=True, affects_template=True)
    ...
    exclude_raw = Bool(False, help="This allows you to exclude raw cells from all templates if set to True.").tag(
        config=True
    )
    ...
    extra_loaders: List[t.Any] = List(
        help="Jinja loaders to find templates. Will be tried in order " "before the default FileSystem ones.",
    ).tag(affects_environment=True)
    ...
    raw_mimetypes = List(Unicode(), help="""formats of raw cells to be included in this Exporter's output.""").tag(
        config=True
    )
    ...
    def __init__(self, config=None, **kw):
        ...
        self.observe(self._invalidate_environment_cache, list(self.traits(affects_environment=True)))
