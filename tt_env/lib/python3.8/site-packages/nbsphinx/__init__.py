...
class Exporter(nbconvert.RSTExporter):
    ...
    def __init__(
        ...
        def replace_attachments(text):
            for filename, bundle in attachment_storage:
                ...
                text = re.sub(
                    r"^(\s*\.\. ((\|[^|]*\| )?image|figure)::) attachment:{0}$".format(filename),
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        auto_execute = (
            ...
            not any(c.get("outputs") or c.get("execution_count") for c in nb.cells if c.cell_type == "code")
        )
        ...
        rststr, resources = super(Exporter, self).from_notebook_node(nb, resources, **kw)
        ...
        def warning(msg, *args):
            logger.warning(
                '"nbsphinx-thumbnail" in cell %s: ' + msg,
                cell_index,
                *args,
        ...
        for cell_index, cell in enumerate(nb.cells):
            ...
            if cell.cell_type != "code":
                warning('Only allowed in code cells; wrong cell type: "%s"', cell.cell_type)
                break
...
def _extract_thumbnail(cell, output_index):
    ...
    if output_index not in range(len(cell.outputs)):
        raise _ExtractThumbnailException('Invalid "output-index": %s', output_index)
    out = cell.outputs[output_index]
    if out.output_type not in {"display_data", "execute_result"}:
        raise _ExtractThumbnailException('Unsupported output type in output %s: "%s"', output_index, out.output_type)
    for mime_type in DISPLAY_DATA_PRIORITY_HTML:
        ...
        raise _ExtractThumbnailException("Unsupported MIME type(s) in output %s: %s", output_index, set(out.data))
...
class NotebookParser(rst.Parser):
    ...
    def parse(self, inputstring, document):
        ...
        *inputstring* is either the JSON representation of a notebook,
        ...
        if isinstance(converter, collections.abc.Sequence) and not isinstance(converter, str):
            if len(converter) != 2:
                raise NotebookError(
                    "The values of nbsphinx_custom_formats must be " "either strings or 2-element sequences"
                )
        ...
        try:
            ...
            raise NotebookError(type(e).__name__ + " in " + env.doc2path(env.docname, base=None) + ":\n" + str(e))
...
"""
        ...
        if env.config.nbsphinx_prolog:
            prolog = exporter.environment.from_string(env.config.nbsphinx_prolog).render(env=env)
        ...
        if env.config.nbsphinx_epilog:
            epilog = exporter.environment.from_string(env.config.nbsphinx_epilog).render(env=env)
...
def _create_code_nodes(directive):
    ...
    if "fancy" in directive.options:
        ...
        codearea_node = CodeAreaNode("", inner_node, prompt=prompt, stderr="stderr" in inner_classes)
...
_CITE_ROLES.update(
    {f"{k}-{suffix}": f"{v}:{suffix}" for k, v in _CITE_ROLES.items() for suffix in ("p", "ps", "t", "ts", "ct", "cts")}
)
...
def markdown2rst(text):
    ...
    def object_hook(obj):
        ...
        if obj.get("t") == "RawBlock" and obj["c"][0] == "latex":
            ...
            obj = {"t": "RawInline", "c": ["rst", ":nbsphinx-math:`{}`".format(obj["c"][1])]}
...
def _extract_gallery_or_toctree(cell):
    ...
    try:
        ...
        raise ValueError("invalid nbsphinx-[link-]gallery/nbsphinx-toctree option: {!r}".format(options))

    text = nbconvert.filters.markdown2rst(cell.source)
    settings = docutils.frontend.OptionParser(components=(rst.Parser,)).get_default_values()
...
class ReplaceAlertDivs(docutils.transforms.Transform):
    ...
    _start_re = re.compile(r'\s*<div\s*class\s*=\s*(?P<q>"|\')([a-z\s-]*)(?P=q)\s*>\s*\Z', flags=re.IGNORECASE)
    ...
    def apply(self):
        ...
        for node in self.document.traverse(docutils.nodes.reference):
            ...
            if not os.path.isfile(os.path.join(env.srcdir, file)):
                logger.warning("File not found: %r", file, location=node, type="nbsphinx", subtype="localfile")
                continue  # Link is ignored
            elif file.startswith(".."):
                logger.warning(
                    "Link outside source directory: %r", file, location=node, type="nbsphinx", subtype="localfile"
                )
                continue  # Link is ignored
...
def load_requirejs(app):
    ...
    if config.nbsphinx_requirejs_path:
        ...
        app.add_js_file(config.nbsphinx_requirejs_path, **config.nbsphinx_requirejs_options)
...
def builder_inited(app):
    ...
    if app.builder.format == "latex":
        sphinx.util.fileutil.copy_asset(
            os.path.join(os.path.dirname(__file__), "_texinputs"), os.path.join(app.builder.outdir)
        )
...
def html_collect_pages(app):
    ...
    for file in status_iterator(files, "copying linked files... ", sphinx.util.console.brown, len(files)):
        ...
        try:
            ...
            logger.warning("Cannot copy local file %r: %s", file, err, type="nbsphinx", subtype="localfile")
    notebooks = app.env.nbsphinx_notebooks.values()
    for notebook in status_iterator(notebooks, "copying notebooks ... ", "brown", len(notebooks)):
        sphinx.util.copyfile(
            os.path.join(app.env.nbsphinx_auxdir, notebook), os.path.join(app.builder.outdir, notebook)
        )
    ...
    if widgets_path is None:
        if env.nbsphinx_widgets:
            try:
                ...
                logger.warning(
                    "nbsphinx_widgets_path not given " "and ipywidgets module unavailable",
                    type="nbsphinx",
                    subtype="ipywidgets",
                )
            else:
...
def doctree_resolved(app, doctree, fromdocname):
    base = sphinx.util.osutil.relative_uri(app.builder.get_target_uri(fromdocname), "")

    # Replace GalleryToc with toctree + GalleryNode
    for node in doctree.traverse(GalleryToc):
        (toctree_wrapper,) = node
        if len(toctree_wrapper) != 1 or not isinstance(toctree_wrapper[0], (sphinx.addnodes.toctree, DummyTocTree)):
        ...
        for title, doc in toctree["entries"]:
            if doc in toctree["includefiles"]:
                        ...
                        if matched:
                            if has_wildcard(pattern):
                                ...
                                if has_wildcard(matched):
                                    logger.warning(
                                        "page %s matches two patterns in " "nbsphinx_thumbnails: %r and %r",
                                        doc,
                                        matched,
                                        pattern,
                                        type="nbsphinx",
                ...
                if filename is _BROKEN_THUMBNAIL:
                    filename = os.path.join(base, "_static", "nbsphinx-broken-thumbnail.svg")
                    ...
                    filename = os.path.join(base, "_static", "nbsphinx-no-thumbnail.svg")
        ...
        if isinstance(toctree, DummyTocTree):
            ...
            if caption:
                caption_node = docutils.nodes.title(caption, "", *[docutils.nodes.Text(caption)])
...
def depart_codearea_html(self, node):
    ...
    text = self.body[-1]
    text = text.replace("<pre>", "<pre>" + "<br/>" * node.get("empty-lines-before", 0))
    text = text.replace("</pre>", "<br/>" * node.get("empty-lines-after", 0) + "</pre>")
    self.body[-1] = text
...
"""
    ...
    if prompt:
        ...
        prefix = r"\llap{\color{" + promptcolor + "}" + prompt + r"\,\hspace{\fboxrule}\hspace{\fboxsep}}"
        assert code_lines
...
def visit_admonition_latex(self, node):
    kind = node["classes"][1]
    if len(node.children) >= 2 and isinstance(node.children[0], docutils.nodes.paragraph):
