...
def paper_1_case(float_edge_wt=False, explicit_node_wt=True, directed=False):
    ...
    # partitioning
    clusters_1 = {
        frozenset(x) for x in nx.community.lukes_partitioning(example_1, limit, node_weight=wtu, edge_weight=EWL)
    }
...
def paper_2_case(explicit_edge_wt=True, directed=False):
    ...
    clusters_2 = {
        frozenset(x)
        for x in nx.community.lukes_partitioning(example_2, byte_block_size, node_weight=NWL, edge_weight=wtu)
    }
