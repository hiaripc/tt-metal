...
def _detect_unboundedness(R):
    ...
    if nx.negative_edge_cycle(G):
        raise nx.NetworkXUnbounded(
            "Negative cost cycle of infinite capacity found. " "Min cost flow may be unbounded below."
        )
...
def _build_residual_network(G, demand, capacity, weight):
    ...
    if G.is_multigraph():
        edge_list = [
            (u, v, k, e) for u, v, k, e in G.edges(data=True, keys=True) if u != v and e.get(capacity, inf) > 0
        ]
    else:
    ...
    inf = (
        max(
            ...
            2 * sum(e[capacity] for u, v, k, e in edge_list if capacity in e and e[capacity] != inf),
        )
        or 1
    )
...
def _build_flow_dict(G, R, capacity, weight):
    ...
    if G.is_multigraph():
        for u in G:
            ...
            for v, es in G[u].items():
                flow_dict[u][v] = {
                    ...
                    k: (0 if (u != v or e.get(capacity, inf) <= 0 or e.get(weight, 0) >= 0) else e[capacity])
                    for k, e in es.items()
                }
        ...
        for u in G:
            flow_dict[u] = {
                # Always saturate negative selfloops.
                v: (0 if (u != v or e.get(capacity, inf) <= 0 or e.get(weight, 0) >= 0) else e[capacity])
...
def capacity_scaling(G, demand="demand", capacity="capacity", weight="weight", heap=BinaryHeap):
    ...
    flow_cost = sum(
        0 if e.get(capacity, inf) <= 0 or e.get(weight, 0) >= 0 else e[capacity] * e[weight]
        for u, v, e in nx.selfloop_edges(G, data=True)
    )
