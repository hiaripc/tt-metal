...
def _find_candidates(u, graph_params, state_params, G1_degree):  # todo: make the 4th argument the degree of u
    ...
    if not covered_neighbors:
        ...
        if G1.is_multigraph():
            candidates.difference_update(
                {node for node in candidates if G1.number_of_edges(u, u) != G2.number_of_edges(node, node)}
            )
        return candidates
    ...
    if G1.is_multigraph():
        common_nodes.difference_update(
            {node for node in common_nodes if G1.number_of_edges(u, u) != G2.number_of_edges(node, node)}
        )
    return common_nodes
