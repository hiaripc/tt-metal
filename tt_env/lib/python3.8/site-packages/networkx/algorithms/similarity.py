...
def optimize_edit_paths(
    ...
    def match_edges(u, v, pending_g, pending_h, Ce, matched_uv=None):
        ...
        if matched_uv is None or len(matched_uv) == 0:
            ...
            g_ind = [
                ...
                or any(pending_g[i][:2] in ((p, u), (u, p), (p, p)) for p, q in matched_uv)
            ]
            h_ind = [
                j
                for j in range(N)
                if pending_h[j][:2] == (v, v)
                or any(pending_h[j][:2] in ((q, v), (v, q), (q, q)) for p, q in matched_uv)
            ]
            ...
            for k, i in enumerate(g_ind):
                ...
                for l, j in enumerate(h_ind):
                    ...
                    if nx.is_directed(G1) or nx.is_directed(G2):
                        ...
                        if any(g in ((p, u), (u, p)) and h in ((q, v), (v, q)) for p, q in matched_uv):
                            continue
    ...
    if node_subst_cost:
        C[0:m, 0:n] = np.array(
            [node_subst_cost(G1.nodes[u], G2.nodes[v]) for u in pending_u for v in pending_v]
        ).reshape(m, n)
        if roots:
            initial_cost = node_subst_cost(G1.nodes[root_u], G2.nodes[root_v])
    elif node_match:
        C[0:m, 0:n] = np.array(
            [1 - int(node_match(G1.nodes[u], G2.nodes[v])) for u in pending_u for v in pending_v]
        ).reshape(m, n)
        if roots:
    ...
    if edge_subst_cost:
        C[0:m, 0:n] = np.array(
            [edge_subst_cost(G1.edges[g], G2.edges[h]) for g in pending_g for h in pending_h]
        ).reshape(m, n)
    elif edge_match:
        C[0:m, 0:n] = np.array(
            [1 - int(edge_match(G1.edges[g], G2.edges[h])) for g in pending_g for h in pending_h]
        ).reshape(m, n)
    else:
        # all zeroes
        pass
...
def _simrank_similarity_python(
    ...
    for its in range(max_iterations):
        ...
        is_close = all(
            all(abs(newsim[u][v] - old) <= tolerance * (1 + abs(old)) for v, old in nbrs.items())
...
def panther_similarity(G, source, k=5, path_length=5, c=0.5, delta=0.1, eps=None):
    ...
    index_map = {}
    _ = list(generate_random_paths(G, sample_size, path_length=path_length, index_map=index_map))
