...
def to_pydot(N):
    ...
    for n, nodedata in N.nodes(data=True):
        ...
        raise_error = _check_colon_quotes(n) or (
            any((_check_colon_quotes(k) or _check_colon_quotes(v)) for k, v in str_nodedata.items())
        )
        if raise_error:
            raise ValueError(
    ...
    if N.is_multigraph():
        for u, v, key, edgedata in N.edges(data=True, keys=True):
            ...
            raise_error = (
                ...
                or (any((_check_colon_quotes(k) or _check_colon_quotes(val)) for k, val in str_edgedata.items()))
            )
            if raise_error:
                raise ValueError(
                    f'Node names and attributes should not contain ":" unless they are quoted with "".\
