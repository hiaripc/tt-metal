    ...
    if initial_graph is None:
        ...
        if len(initial_graph) < max(m1, m2) or len(initial_graph) > n:
            raise nx.NetworkXError(
                f"Barabási–Albert initial graph must have between " f"max(m1, m2) = {max(m1, m2)} and n = {n} nodes"
            )
        G = initial_graph.copy()
    ...
    while new_node < n:
        ...
        if a_probability < p and G.size() <= clique_size - m:
            ...
            for i in range(m):
                ...
                if G.degree(dest_node) == clique_degree and dest_node in elligible_nodes:
