...
def test_fiedler_vector_tracemin_unknown():
    ...
    with pytest.raises(nx.NetworkXError, match="Unknown linear system solver"):
        nx.linalg.algebraicconnectivity._tracemin_fiedler(L, X, normalized=False, tol=1e-8, method="tracemin_unknown")
...
class TestAlgebraicConnectivity:
    ...
    def test_two_nodes_multigraph(self, method):
        ...
        assert nx.algebraic_connectivity(G, weight="spam", tol=1e-12, method=method) == pytest.approx(6, abs=1e-7)
    ...
    def test_buckminsterfullerene(self, normalized, sigma, laplacian_fn, method):
        ...
        A = laplacian_fn(G)
        try:
            assert nx.algebraic_connectivity(G, normalized=normalized, tol=1e-12, method=method) == pytest.approx(
                sigma, abs=1e-7
            )
