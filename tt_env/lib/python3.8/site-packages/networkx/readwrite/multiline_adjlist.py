...
def generate_multiline_adjlist(G, delimiter=" "):
    ...
    if G.is_directed():
        ...
        if G.is_multigraph():
            ...
            for s, nbrs in G.adjacency():
                nbr_edges = [
                    (u, data) for u, datadict in nbrs.items() if u not in seen for key, data in datadict.items()
                ]
                deg = len(nbr_edges)
                yield str(s) + delimiter + str(deg)
                for u, d in nbr_edges:
    ...
    for line in lines:
        ...
        if nodetype is not None:
            try:
                ...
                raise TypeError(f"Failed to convert node ({u}) to " f"type {nodetype}") from err
        G.add_node(u)
        for i in range(deg):
            while True:
                try:
            ...
            if nodetype is not None:
                try:
                    ...
                    raise TypeError(f"Failed to convert node ({v}) " f"to type {nodetype}") from err
            if edgetype is not None:
                try:
                    edgedata = {"weight": edgetype(data)}
                except BaseException as err:
                    raise TypeError(f"Failed to convert edge data ({data}) " f"to type {edgetype}") from err
            else:
                try:  # try to evaluate
