...
def _relabel_inplace(G, mapping):
    if len(mapping.keys() & mapping.values()) > 0:
        ...
        try:
            ...
            raise nx.NetworkXUnfeasible(
                "The node label sets are overlapping and no ordering can " "resolve the mapping. Use copy=True."
            ) from err
    else:
    ...
    for old in nodes:
        ...
        if multigraph:
            ...
            if directed:
                new_edges += [
                    (new if old == source else source, new, data) for (source, _, data) in G.in_edges(old, data=True)
                ]
        G.remove_node(old)
...
def _relabel_copy(G, mapping):
    ...
    if G.is_multigraph():
        new_edges = [
            (mapping.get(n1, n1), mapping.get(n2, n2), k, d.copy()) for (n1, n2, k, d) in G.edges(keys=True, data=True)
        ]
