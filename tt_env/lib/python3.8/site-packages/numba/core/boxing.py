    ...
    for field_name in ("start", "stop", "step"):
        ...
        if isinstance(field_obj, int):
            try:
                ...
                raise ValueError(
                    (f"Unable to create literal slice. " f"Error encountered with {field_name} " f"attribute. {str(e)}")
                )
...
def unbox_tuple(typ, obj, c):
    ...
    with c.builder.if_then(c.builder.not_(size_matches), likely=False):
        c.pyapi.err_format(
            "PyExc_ValueError", "size mismatch for tuple, expected %d element(s) but got %%zd" % (n,), actual_size
        )
    ...
    with c.builder.if_else(ok, likely=True) as (if_ok, if_not_ok):
        with if_ok:
            ...
            with c.pyapi.set_iterate(obj) as loop:
                ...
                with c.builder.if_else(cgutils.is_null(c.builder, expected_typobj), likely=False) as (
                    if_first,
                    if_not_first,
                ):
                    with if_first:
