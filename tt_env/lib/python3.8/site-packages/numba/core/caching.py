...
class CacheImpl(metaclass=ABCMeta):
    ...
    _locator_classes = [_UserProvidedCacheLocator, _InTreeCacheLocator, _UserWideCacheLocator, _IPythonCacheLocator]
    ...
    def __init__(self, py_func):
        ...
        for cls in self._locator_classes:
            ...
            raise RuntimeError(
                "cannot cache function %r: no locator available " "for file %r" % (qualname, source_path)
            )
...
class Cache(_Cache):
    ...
    def __init__(self, py_func):
        ...
        self._cache_file = IndexDataCacheFile(
            cache_path=self._cache_path, filename_base=filename_base, source_stamp=source_stamp
        )
        self.enable()
