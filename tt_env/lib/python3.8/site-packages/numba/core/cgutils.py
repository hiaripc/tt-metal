        ...
        if value.type != ptr.type.pointee:
            if (
                ...
                raise TypeError(
                    ...
                    "{ptr.type.pointee} in "
                    "{self._datamodel} "
                    "(trying to write member #{index})".format(value=value, ptr=ptr, self=self, index=index)
                )
    ...
    def _dbg():
        ...
        if axis is not None:
            if isinstance(axis, int):
                printf(
                    builder,
                    "debug: IndexError: index %d is out of bounds " "for axis {} with size %d\n".format(axis),
                    ind,
                    dimlen,
                )
            else:
...
def _raw_memcpy(builder, func_name, dst, src, count, itemsize, align):
    ...
    builder.call(
        memcpy,
        [builder.bitcast(dst, voidptr_t), builder.bitcast(src, voidptr_t), builder.mul(count, itemsize), is_volatile],
    )
