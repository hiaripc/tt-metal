...
def dump(header, body, lang):
    if config.HIGHLIGHT_DUMPS:
        try:
            ...
            def printer(arg):
                print(highlight(arg, lexer(), Terminal256Formatter(style=by_colorscheme())))
...
class _CFG(object):
    ...
    def pretty_printer(
        ...
        def init_digraph(name, fname, fontsize):
            ...
            cmax = 200
            if len(fname) > cmax:
                wstr = f'CFG output filename "{fname}" exceeds maximum ' f"supported length, it will be truncated."
        ...
        for obj in jzon["objects"]:
            ...
            dat = "".join(new_lines)
            if dat:
                tab = ('<table id="%s" BORDER="1" CELLBORDER="0" ' 'CELLPADDING="0" CELLSPACING="0">%s</table>') % (
                    idc,
                    dat,
                )
                label = "<{}>".format(tab)
    ...
    def display(self, filename=None, format="pdf", view=False):
        ...
        """
        rawbyt = self.pretty_printer(filename=filename, view=view, render_format=format, **self.kwargs)
...
class CPUCodegen(Codegen):
    ...
    def _init(self, llvm_module):
        ...
        self._mpm_cheap = self._module_pass_manager(
            loop_vectorize=loopvect, slp_vectorize=False, opt=opt_level, cost="cheap"
        )

        self._mpm_full = self._module_pass_manager()

        self._engine.set_object_cache(
            self._library_class._object_compiled_hook, self._library_class._object_getbuffer_hook
        )
    ...
    def _pass_manager_builder(self, **kwargs):
        ...
        pmb = create_pass_manager_builder(
            opt=opt_level, loop_vectorize=loop_vectorize, slp_vectorize=slp_vectorize, **kwargs
        )
