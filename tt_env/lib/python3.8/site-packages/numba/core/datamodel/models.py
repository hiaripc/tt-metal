...
class EphemeralArrayModel(PointerModel):
    ...
    def as_data(self, builder, value):
        values = [builder.load(cgutils.gep_inbounds(builder, value, i)) for i in range(self._fe_type.count)]
...
class EnumerateType(StructModel):
    def __init__(self, dmm, fe_type):
        members = [("count", types.EphemeralPointer(types.intp)), ("iter", fe_type.source_type)]
...
class GeneratorModel(CompositeModel):
    def __init__(self, dmm, fe_type):
        ...
        self._be_type = ir.LiteralStructType(
            [self._dmm.lookup(types.int32).get_value_type(), self._args_be_type, self._state_be_type]
        )
        self._be_ptr_type = self._be_type.as_pointer()
