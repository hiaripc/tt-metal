...
_msg_deprecated_signature_arg = (
    "Deprecated keyword argument `{0}`. " "Signatures should be passed as the first " "positional argument."
)
...
def jit(signature_or_function=None, locals={}, cache=False, pipeline_class=None, boundscheck=None, **options):
    ...
    wrapper = _jit(sigs, locals=locals, target=target, cache=cache, targetoptions=options, **dispatcher_args)
...
def _jit(sigs, locals, target, cache, targetoptions, **dispatcher_args):
    ...
    def wrapper(func):
        ...
        if not inspect.isfunction(func):
            raise TypeError("The decorated object is not a function (got type " f"{type(func)}).")
        ...
        if config.DISABLE_JIT and not target == "npyufunc":
            return func
        disp = dispatcher(py_func=func, locals=locals, targetoptions=targetoptions, **dispatcher_args)
        if cache:
            disp.enable_caching()
        if sigs is not None:
...
def generated_jit(function=None, cache=False, pipeline_class=None, **options):
    ...
    if pipeline_class is not None:
        dispatcher_args["pipeline_class"] = pipeline_class
    wrapper = _jit(
        sigs=None, locals={}, target="cpu", cache=cache, targetoptions=options, impl_kind="generated", **dispatcher_args
    )
