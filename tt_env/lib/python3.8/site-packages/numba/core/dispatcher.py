        ...
        if ok:
            for pyparam, implparam in zip(pysig.parameters.values(), implsig.parameters.values()):
                ...
                if (
                    ...
                    or (implparam.default is not implparam.empty and implparam.default != pyparam.default)
                ):
                    ok = False
        if not ok:
            raise TypeError(
...
class Dispatcher(serialize.ReduceMixin, _MemoMixin, _DispatcherBase):
    ...
    def __init__(self, py_func, locals={}, targetoptions={}, impl_kind="direct", pipeline_class=compiler.Compiler):
        ...
        _DispatcherBase.__init__(self, arg_count, py_func, pysig, can_fallback, exact_match_required=False)
        ...
        self._compiler = compiler_class(py_func, self.targetdescr, targetoptions, locals, pipeline_class)
    ...
    def dump(self, tab=""):
        print(f"{tab}DUMP {type(self).__name__}[{self.py_func.__name__}" f", type code={self._type._code}]")
    ...
    @classmethod
    def _rebuild(cls, uuid, py_func, locals, targetoptions, impl_kind, can_compile, sigs):
        """
            ...
            with self._compiling_counter:
                ...
                if cres is not None:
                    ...
                    if not cres.objectmode:
                        self.targetctx.insert_user_function(cres.entry_point, cres.fndesc, [cres.library])
...
_dispatcher.typeof_init(OmittedArg, dict((str(t), t._code) for t in types.number_domain))
