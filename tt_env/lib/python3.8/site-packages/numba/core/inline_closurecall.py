...
class InlineClosureCallPass(object):
    ...
    def _inline_reduction(self, work_list, block, i, expr, call_name):
        ...
        if len(expr.args) not in (2, 3):
            raise TypeError(
                "invalid reduce call, " "two arguments are required (optional initial " "value can also be specified)"
            )
    ...
    def _inline_stencil(self, instr, call_name, func_def):
        ...
        if "neighborhood" in options:
            ...
            if not fixed:
                raise ValueError(
                    "stencil neighborhood option should be a tuple" " with constant structure such as ((-w, w),)"
                )
        if "index_offsets" in options:
            fixed = guard(self._fix_stencil_index_offsets, options)
            if not fixed:
                raise ValueError(
                    "stencil index_offsets option should be a tuple" " with constant structure such as (offset, )"
                )
...
def _find_iter_range(func_ir, range_iter_var, swapped):
    ...
    require(
        isinstance(func_def, ir.Global) and (func_def.value == range or func_def.value == numba.misc.special.prange)
    )
...
def _inline_arraycall(func_ir, cfg, visited, loop, swapped, enable_prange=False, typed=False):
    ...
    if range_def:
        ...
        if typed:
            ...
            stmts.append(
                _new_definition(
                    func_ir, len_func_var, ir.Global("length_of_iterator", length_of_iterator, loc=loc), loc
                )
            )
    ...
    if dtype_def and dtype_mod_def:
        ...
        if typed:
            ...
            stmts.append(
                _new_definition(
                    func_ir, empty_func, ir.Global("unsafe_empty_inferred", unsafe_empty_inferred, loc=loc), loc
                )
            )
            array_kws = []
        else:
    ...
    stmts.append(
        _new_definition(func_ir, array_var, ir.Expr.call(empty_func, (size_tuple_var,), list(array_kws), loc=loc), loc)
    )
    ...
    if range_def:
        if range_def[0] != 0:
            ...
            blk.body.insert(
                ...
                _new_definition(
                    func_ir,
                    index_var,
                    ir.Expr.binop(fn=operator.sub, lhs=iter_first_var, rhs=range_def[0], loc=loc),
                    loc,
                ),
            )
    else:
        ...
        stmts.append(
            _new_definition(
                func_ir, next_index_var, ir.Expr.binop(fn=operator.add, lhs=index_var, rhs=one, loc=loc), loc
            )
        )
    ...
    for i in range(len(append_block.body)):
        if append_block.body[i] is append_stmt:
            ...
            append_block.body[i] = ir.SetItem(
                target=array_var, index=index_var, value=append_stmt.value.args[0], loc=append_stmt.loc
            )
                    ...
                    if inst.value is expr:
                        ...
                        for var in varlist:
                            ...
                            if var.name in defined or (
                                var.name in livemap[label] and not (var.name in usedefs.defmap[label])
                            ):
            ...
            if removed_var in state.list_items:
                ...
                if state.list_vars == []:
                    ...
                    for inst in state.stmts:
                        if (isinstance(inst, ir.Assign) and inst.target.name in state.dead_vars) or (
