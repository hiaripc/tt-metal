...
def _call_function_ex_replace_kws_large(
    old_body, buildmap_name, buildmap_idx, search_end, new_body, func_ir, errmsg, already_deleted_defs
):
    """
    Extracts the kws args passed as varkwarg
    for CALL_FUNCTION_EX. This pass is taken when
        ...
        for i, stmt in enumerate(blk.body):
            if (
                ...
                if (
                    kws
                    or not found
                    or not (isinstance(keyword_def.value, ir.Expr) and keyword_def.value.op == "build_map")
                ):
...
class Interpreter(object):
    ...
    def op_STORE_ATTR(self, inst, target, value):
        ...
        sa = ir.SetAttr(target=self.get(target), value=self.get(value), attr=attr, loc=self.loc)
    ...
    def op_BUILD_CONST_KEY_MAP(self, inst, keys, keytmps, values, res):
        ...
        expr = ir.Expr.build_map(
            items=items, size=2, literal_value=literal_dict, value_indexes=value_indexes, loc=self.loc
        )
    ...
    def op_FOR_ITER(self, inst, iterator, pair, indval, pred):
        ...
        # Conditional jump
        br = ir.Branch(cond=self.get(pred), truebr=inst.next, falsebr=inst.get_jump_target(), loc=self.loc)
    ...
    def op_BUILD_MAP(self, inst, items, size, res):
        ...
        expr = ir.Expr.build_map(
            items=got_items, size=size, literal_value=literal_dict, value_indexes=value_indexes, loc=self.loc
        )
        self.store(expr, res)

    def op_STORE_MAP(self, inst, dct, key, value):
        stmt = ir.StoreMap(dct=self.get(dct), key=self.get(key), value=self.get(value), loc=self.loc)
        self.current_block.append(stmt)
    ...
    def op_RERAISE(self, inst, exc):
        ...
        if tryblk is not None:
            ...
            msg = f"Unreachable condition reached (op code RERAISE executed)" f"{error_extras['reportable']}"
    ...
    def op_MAKE_FUNCTION(self, inst, name, code, closure, annotations, kwdefaults, defaults, res):
        ...
        if not isinstance(assume_code_const, ir.Const):
            msg = (
                "Unsupported use of closure. " "Probably caused by complex control-flow constructs; " "e.g. try-except"
            )
