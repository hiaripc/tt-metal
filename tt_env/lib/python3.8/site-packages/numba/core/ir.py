...
class DynamicRaise(Terminator):
    ...
    def __str__(self):
        if self.exc_class is None:
            ...
            return "<dynamic> raise %s(%s)" % (self.exc_class, ", ".join(map(repr, self.exc_args)))
...
class FunctionIR(object):
    ...
    def render_dot(self, filename_prefix="numba_ir", include_ir=True):
        ...
        try:
            ...
            raise ImportError(
                "The feature requires `graphviz` but it is not available. " "Please install with `pip install graphviz`"
            )
        g = gv.Digraph(
            filename="{}{}.dot".format(
                filename_prefix,
