...
def mk_alloc(typingctx, typemap, calltypes, lhs, size_var, dtype, scope, loc, lhs_typ):
    ...
    if calltypes:
        cac = typemap[attr_var.name].get_call_type(typingctx, [size_typ, types.functions.NumberClass(dtype)], {})
    ...
    if lhs_typ.layout == "F":
        ...
        if calltypes:
            calltypes[asfortranarray_call] = typemap[afa_attr_var.name].get_call_type(typingctx, [empty_c_typ], {})

        asfortranarray_assign = ir.Assign(asfortranarray_call, lhs, loc)

        out.extend([g_np_assign, attr_assign, typ_var_assign, empty_c_assign, afa_attr_assign, asfortranarray_assign])
    else:
...
def is_immutable_type(var, typemap):
    ...
    if isinstance(typ, (types.Number, types.scalars._NPDatetimeBase, types.iterators.RangeType)):
        return True
    if typ == types.string:
        return True
