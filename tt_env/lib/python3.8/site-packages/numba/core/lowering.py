...
class BaseLower(object):
    ...
    def __init__(self, context, library, fndesc, func_ir, metadata=None):
        ...
        self.debuginfo = dibuildercls(
            module=self.module, filepath=func_ir.loc.filename, cgctx=context, directives_only=directives_only
        )
        ...
        if config.DUMP_LLVM:
            ...
            if config.HIGHLIGHT_DUMPS:
                try:
                    ...
                    print(highlight(self.module.__repr__(), lexer(), Terminal256Formatter(style=by_colorscheme())))
                except ImportError:
    ...
    def create_cpython_wrapper(self, release_gil=False):
        ...
        if self.genlower:
            self.context.create_cpython_wrapper(
                self.library, self.genlower.gendesc, self.env, self.call_helper, release_gil=release_gil
            )
        self.context.create_cpython_wrapper(
            self.library, self.fndesc, self.env, self.call_helper, release_gil=release_gil
        )
...
class Lower(BaseLower):
    ...
    def lower_inst(self, inst):
        ...
        if isinstance(inst, ir.Assign):
            ...
            assert pred.type == llvmlite.ir.IntType(1), "cond is not i1: %s" % pred.type
    ...
    def fold_call_args(self, fnty, signature, pos_args, vararg, kw_args):
        ...
        if pysig is None:
            ...
            argvals = typing.fold_arguments(
                pysig, pos_args, dict(kw_args), normal_handler, default_handler, stararg_handler
            )
        return argvals
    ...
    def _lower_call_ObjModeDispatcher(self, fnty, expr, signature):
        ...
        argobjs = [
            self.pyapi.from_native_value(atyp, aval, self.env_manager) for atyp, aval in zip(argtypes, argvalues)
        ]
    ...
    def _lower_call_FunctionType(self, fnty, expr, signature):
        ...
        if not fnty.check_signature(signature):
            ...
            raise UnsupportedError(f"mismatch of function types:" f" expected {fnty} but got {types.FunctionType(sig)}")
        ftype = fnty.ftype
        argvals = self.fold_call_args(
            fnty,
            sig,
    ...
    def lower_expr(self, resty, expr):
        if expr.op == "binop":
            ...
            if isinstance(resty, types.LiteralList):
                ...
                castvals = [
                    self.context.cast(self.builder, val, fromty, resty.dtype) for val, fromty in zip(itemvals, itemtys)
                ]
            ...
            return self.context.build_map(
                self.builder, resty, list(zip(key_types, value_types)), list(zip(keys, values))
            )
    ...
    def storevar(self, value, name, argidx=None):
        ...
        if name in self._singly_assigned_vars and not self._disable_sroa_like_opt:
            ...
            if value.type != ptr.type.pointee:
                msg = ("Storing {value.type} to ptr of {ptr.type.pointee} " "('{name}'). FE type {fetype}").format(
                    value=value, ptr=ptr, fetype=fetype, name=name
                )
