...
class ObjectModeFrontEnd(FunctionPass):
    ...
    def _frontend_looplift(self, state):
        ...
        main, loops = transforms.loop_lifting(
            state.func_ir, typingctx=state.typingctx, targetctx=state.targetctx, locals=state.locals, flags=loop_flags
        )
        if loops:
    ...
    def run_pass(self, state):
        ...
        if not state.flags.force_pyobject:
            ...
            if len(state.lifted) > 0:
                warn_msg = (
                    'Function "%s" was compiled in object mode without'
                    " forceobj=True, but has lifted loops." % (state.func_id.func_name,)
                )
            else:
                warn_msg = 'Function "%s" was compiled in object mode without' " forceobj=True." % (
