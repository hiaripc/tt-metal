...
def _loop_lift_modify_blocks(func_ir, loopinfo, blocks, typingctx, targetctx, flags, locals):
    ...
    lifted_ir = func_ir.derive(
        blocks=loopblocks, arg_names=tuple(loopinfo.inputs), arg_count=len(loopinfo.inputs), force_non_generator=True
    )
    ...
    callblock = _loop_lift_modify_call_block(
        liftedloop, blocks[loopinfo.callfrom], loopinfo.inputs, loopinfo.outputs, loopinfo.returnto
    )
    # remove blocks
    ...
    for _, p in with_ranges_tuple:
        ...
        if ir_utils.is_return(func_ir.blocks[target_block.terminator.get_targets()[0]].terminator):
            if PYVERSION == (3, 8):
                ...
                raise errors.CompilerError("unsupported control flow: due to return statements " "inside with block")
