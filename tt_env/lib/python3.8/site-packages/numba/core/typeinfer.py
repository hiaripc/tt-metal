            ...
            with typeinfer.warnings.catch_warnings(filename=loc.filename, lineno=loc.line):
                try:
                    ...
                    if utils.use_old_style_errors():
                        ...
                        msg = "Unknown CAPTURED_ERRORS style: " f"'{config.CAPTURED_ERRORS}'."
                        assert 0, msg
...
class BuildMapConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of dict at {0}", self.loc):
            ...
            if not tsets:
                typeinfer.add_type(
                    self.target, types.DictType(types.undefined, types.undefined, self.special_value), loc=self.loc
                )
            else:
...
"""
    ...
    def typeof_static_setitem(self, inst):
        constraint = StaticSetItemConstraint(
            target=inst.target, index=inst.index, index_var=inst.index_var, value=inst.value, loc=inst.loc
        )
        self.constraints.append(constraint)
        self.calls.append((inst, constraint))
    ...
    def typeof_expr(self, inst, target, expr):
        if expr.op == "call":
            ...
            constraint = StaticGetItemConstraint(
                target.name, value=expr.value, index=expr.index, index_var=expr.index_var, loc=expr.loc
            )
            self.constraints.append(constraint)
            ...
            constraint = TypedGetItemConstraint(
                target.name, value=expr.value, dtype=expr.dtype, index=expr.index, loc=expr.loc
            )
            self.constraints.append(constraint)
            self.calls.append((inst.value, constraint))
