    ...
    def format(self):
        ...
        msgbuf = [
            _header_template.format(
                the_function=self._function_type, fname=fname, signature=argstr, ncandidates=ncandidates
            )
        ]
...
class Dispatcher(WeakType, Callable, Dummy):
    ...
    def dump(self, tab=""):
        print((f"{tab}DUMP {type(self).__name__}[code={self._code}, " f"name={self.name}]"))
        ...
        if self.requires_gil:
            ...
            class GilRequiringDefn(AbstractTemplate):
                ...
                def generic(self, args, kws):
                    ...
                    # ffi_forced_object.
                    coerced = [
                        actual if formal == ffi_forced_object else formal for actual, formal in zip(args, self.key.args)
                    ]
