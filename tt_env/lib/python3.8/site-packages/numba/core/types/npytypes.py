        ...
        if isinstance(other, Record):
            ...
            for other_fd, self_fd in zip(other.fields.items(), self.fields.items()):
                if not other_fd == self_fd:
                    return
            warnings.warn(
                f"{self} has been considered a subtype of {other} " f" This is an experimental feature.",
...
class Array(Buffer):
    ...
    def __repr__(self):
        return (
            f"Array({repr(self.dtype)}, {self.ndim}, '{self.layout}', " f"{not self.mutable}, aligned={self.aligned})"
        )
