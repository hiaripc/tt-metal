...
class Numpy_rules_ufunc(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        if not all(
            np.can_cast(fromty, toty, "unsafe") for (fromty, toty) in zip(ufunc_loop.numpy_outputs, explicit_outputs_np)
        ):
            ...
            if ndims > 0:
                ...
                if array_ufunc_type is not None:
                    ...
                    if output_type is NotImplemented:
                        msg = f"unsupported use of ufunc {ufunc} on " f"{array_ufunc_type}"
                        # raise TypeError here because
                        # NumpyRulesArrayOperator.generic is capturing
                        # TypingError
                        raise NumbaTypeError(msg)
                    elif not issubclass(output_type, types.Array):
                        msg = f"ufunc {ufunc} on {array_ufunc_type}" f"cannot return non-array {output_type}"
                        # raise TypeError here because
                        # NumpyRulesArrayOperator.generic is capturing
                        # TypingError
                        raise TypeError(msg)
...
def _numpy_redirect(fname):
    ...
    cls = type(
        "Numpy_redirect_{0}".format(fname), (Numpy_method_redirection,), dict(key=numpy_function, method_name=fname)
    )
