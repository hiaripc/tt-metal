        ...
        if sig is not None:
            if not isinstance(sig, Signature):
                raise AssertionError(
                    "generic() must return a Signature or None. " "{} returned {}".format(generic, type(sig)),
                )
...
def make_overload_attribute_template(
    typ, attr, overload_func, inline, prefer_literal=False, base=_OverloadAttributeTemplate, **kwargs
):
    """
        ...
        if typ is not None:
            ...
            if typing_key is val:
                ...
                if getattr(mod, val.__name__) is not val:
                    raise ValueError("%r is not globally reachable as '%s.%s'" % (mod, val.__module__, val.__name__))
