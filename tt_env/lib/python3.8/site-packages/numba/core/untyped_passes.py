...
class InlineClosureLikes(FunctionPass):
    ...
    def run_pass(self, state):
        ...
        inline_pass = InlineClosureCallPass(
            state.func_ir, state.flags.auto_parallel, state.parfor_diagnostics.replaced_fns, typed_pass
        )
        inline_pass.run()
...
class InlineInlinables(FunctionPass):
    ...
    def run_pass(self, state):
        ...
        inline_worker = InlineWorker(
            state.typingctx, state.targetctx, state.locals, state.pipeline, state.flags, validator=callee_ir_validator
        )
                        ...
                        if isinstance(stmt.value, ir.Expr) and stmt.value.op == "typed_getitem":
                            if isinstance(branch_ty, types.Literal):
                                ...
                                new_typed_getitem = ir.Expr.typed_getitem(
                                    value=orig.value, dtype=branch_ty, index=orig.index, loc=orig.loc
                                )
