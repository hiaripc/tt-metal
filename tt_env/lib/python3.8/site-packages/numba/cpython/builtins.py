...
def ol_sum(iterable, start=0):
    ...
    if isinstance(
        iterable, (types.containers._HomogeneousTuple, types.List, types.ListType, types.Array, types.RangeType)
    ):
        iterator = iter
...
def resolve_getattr(tyctx, obj, name, default):
    ...
    if fn is None:  # No attribute
        ...
        if not (isinstance(default, types.NamedTuple) and default.instance_class == _getattr_default_type):
