...
unicode_kind = {1: unicode.PY_UNICODE_1BYTE_KIND, 2: unicode.PY_UNICODE_2BYTE_KIND, 4: unicode.PY_UNICODE_4BYTE_KIND}[
    unicode_byte_width
]
...
def unicode_to_unicode_charseq(context, builder, fromty, toty, val):
    ...
    with builder.if_then(kind2):
        if unicode_byte_width >= 2:
            ...
            context.call_conv.return_user_exc(
                builder, ValueError, ("cannot cast 16-bit unicode_type to %s-bit %s" % (unicode_byte_width * 8, toty))
            )
    ...
    with builder.if_then(kind4):
        if unicode_byte_width >= 4:
            ...
            context.call_conv.return_user_exc(
                builder, ValueError, ("cannot cast 32-bit unicode_type to %s-bit %s" % (unicode_byte_width * 8, toty))
            )
