...
def get_next_double(context, builder, state_ptr):
    ...
    return builder.fdiv(
        builder.fadd(b, builder.fmul(a, ir.Constant(double, 67108864.0))), ir.Constant(double, 9007199254740992.0)
    )
    ...
    with builder.if_else(is_32b) as (ifsmall, iflarge):
        ...
        with iflarge:
            ...
            if not is_numpy:
                ...
                high = get_shifted_int(builder.sub(nbits, c32))
            total = builder.add(
                builder.zext(low, int64_t), builder.shl(builder.zext(high, int64_t), ir.Constant(int64_t, 32))
            )
...
def _gauss_impl(state, loc_preprocessor, scale_preprocessor):
    def _impl(context, builder, sig, args):
        ...
        mu, sigma = args
        return builder.fadd(
            loc_preprocessor(builder, mu), builder.fmul(scale_preprocessor(builder, sigma), builder.load(ret))
        )
...
def poisson_impl2(lam, size):
    ...
    if isinstance(lam, (types.Float, types.Integer)) and (
        isinstance(size, types.Integer) or (isinstance(size, types.UniTuple) and isinstance(size.dtype, types.Integer))
    ):
...
def noncentral_chisquare(df, nonc, size=None):
    if size in (None, types.none):
        ...
        raise NumbaTypeError(
            "np.random.noncentral_chisquare(): size should be int or " "tuple of ints or None, got %s" % size
        )
