...
class SetInstance(object):
    ...
    def issubset(self, other, strict=False):
        ...
        with builder.if_else(builder.icmp_unsigned(cmp_op, payload.used, other_payload.used)) as (
            if_smaller,
            if_larger,
        ):
            with if_larger:
    ...
    def equals(self, other):
        ...
        res = cgutils.alloca_once_value(builder, cgutils.true_bit)
        with builder.if_else(builder.icmp_unsigned("==", payload.used, other_payload.used)) as (
            if_same_size,
            otherwise,
        ):
            with if_same_size:
    ...
    def upsize(self, nitems):
        ...
        with builder.if_then(need_resize, likely=False):
            ...
            if DEBUG_ALLOCS:
                context.printf(
                    builder,
                    "upsize to %zd items: current size = %zd, " "min entries = %zd, new size = %zd\n",
                    nitems,
                    size,
                    min_entries,
                    new_size,
    ...
    def downsize(self, nitems):
        ...
        need_resize = builder.and_(
            builder.icmp_unsigned("<=", max_size, size), builder.icmp_unsigned("<", minsize, size)
        )
        ...
        with builder.if_then(need_resize, likely=False):
            ...
            if DEBUG_ALLOCS:
                context.printf(
                    builder,
                    "downsize to %zd items: current size = %zd, " "min entries = %zd, new size = %zd\n",
                    nitems,
                    size,
                    min_entries,
                    new_size,
    ...
    def _allocate_payload(self, nentries, realloc=False):
        ...
        allocsize, ovf = cgutils.muladd_with_overflow(
            builder, nentries, ir.Constant(intp_t, entry_size), ir.Constant(intp_t, payload_size)
        )
        ...
        with builder.if_then(builder.load(ok), likely=True):
            ...
            with builder.if_else(alloc_ok, likely=False) as (if_error, if_ok):
                ...
                with if_ok:
                    ...
                    if DEBUG_ALLOCS:
                        context.printf(
                            builder, "allocated %zd bytes for set at %p: mask = %zd\n", allocsize, payload.ptr, new_mask
                        )
    ...
    def _copy_payload(self, src_payload):
        ...
        allocsize = builder.add(
            ir.Constant(intp_t, payload_size), builder.mul(ir.Constant(intp_t, entry_size), nentries)
        )
