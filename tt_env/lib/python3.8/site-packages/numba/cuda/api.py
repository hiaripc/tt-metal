...
@require_context
def managed_array(shape, dtype=np.float_, strides=None, order="C", stream=0, attach_global=True):
...
@require_context
def mapped_array(shape, dtype=np.float_, strides=None, order="C", stream=0, portable=False, wc=False):
...
def device_array_like(ary, stream=0):
    ...
    return device_array(shape=ary.shape, dtype=ary.dtype, strides=strides, order=order, stream=stream)
...
def mapped_array_like(ary, stream=0, portable=False, wc=False):
    ...
    order = _order_like_array(ary)
    return mapped_array(
        shape=ary.shape, dtype=ary.dtype, strides=strides, order=order, stream=stream, portable=portable, wc=wc
    )
