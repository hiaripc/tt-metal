...
class Driver(object):
    ...
    def _initialize_extras(self):
        ...
        call_proto = ctypes.CFUNCTYPE(
            c_int, ctypes.POINTER(drvapi.cu_device_ptr), ctypes.POINTER(drvapi.cu_ipc_mem_handle), ctypes.c_uint
        )
...
class CtypesFunction(Function):
    ...
    def read_func_attr_all(self):
        ...
        maxtpb = self.read_func_attr(enums.CU_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK)
...
def launch_kernel(cufunc_handle, gx, gy, gz, bx, by, bz, sharedmem, hstream, args, cooperative=False):
    ...
    if cooperative:
        driver.cuLaunchCooperativeKernel(cufunc_handle, gx, gy, gz, bx, by, bz, sharedmem, hstream, params_for_launch)
    else:
        driver.cuLaunchKernel(cufunc_handle, gx, gy, gz, bx, by, bz, sharedmem, hstream, params_for_launch, extra)
...
class Linker(metaclass=ABCMeta):
    ...
    def add_file_guess_ext(self, path):
        ...
        if ext == "":
            ...
            if kind is None:
                raise RuntimeError("Don't know how to link file with extension " f".{ext}")
...
_MVC_ERROR_MESSAGE = "Minor version compatibility requires ptxcompiler and cubinlinker packages " "to be available"
