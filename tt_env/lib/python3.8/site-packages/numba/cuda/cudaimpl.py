...
def ptx_shfl_sync_i32(context, builder, sig, args):
    ...
    fnty = ir.FunctionType(
        ...
        (ir.IntType(32), ir.IntType(32), ir.IntType(32), ir.IntType(32), ir.IntType(32)),
    )
...
def ptx_vote_sync(context, builder, sig, args):
    ...
    lmod = builder.module
    fnty = ir.FunctionType(
        ir.LiteralStructType((ir.IntType(32), ir.IntType(1))), (ir.IntType(32), ir.IntType(32), ir.IntType(1))
    )
    func = cgutils.get_or_insert_function(lmod, fnty, fname)
...
def ptx_brev_u8(context, builder, sig, args):
    ...
    fn = cgutils.get_or_insert_function(
        builder.module, ir.FunctionType(ir.IntType(64), (ir.IntType(64),)), "__nv_brevll"
    )
    return builder.call(fn, args)
...
def ptx_ffs_64(context, builder, sig, args):
    fn = cgutils.get_or_insert_function(
        builder.module, ir.FunctionType(ir.IntType(32), (ir.IntType(64),)), "__nv_ffsll"
    )
    return builder.call(fn, args)
...
def ptx_max_f4(context, builder, sig, args):
    fn = cgutils.get_or_insert_function(
        builder.module, ir.FunctionType(ir.FloatType(), (ir.FloatType(), ir.FloatType())), "__nv_fmaxf"
    )
    return builder.call(fn, args)
...
def ptx_max_f8(context, builder, sig, args):
    fn = cgutils.get_or_insert_function(
        builder.module, ir.FunctionType(ir.DoubleType(), (ir.DoubleType(), ir.DoubleType())), "__nv_fmax"
    )
...
def ptx_min_f4(context, builder, sig, args):
    fn = cgutils.get_or_insert_function(
        builder.module, ir.FunctionType(ir.FloatType(), (ir.FloatType(), ir.FloatType())), "__nv_fminf"
    )
    return builder.call(fn, args)
...
def ptx_min_f8(context, builder, sig, args):
    fn = cgutils.get_or_insert_function(
        builder.module, ir.FunctionType(ir.DoubleType(), (ir.DoubleType(), ir.DoubleType())), "__nv_fmin"
    )
...
def ptx_nanosleep(context, builder, sig, args):
    nanosleep = ir.InlineAsm(
        ir.FunctionType(ir.VoidType(), [ir.IntType(32)]), "nanosleep.u32 $0;", "r", side_effect=True
    )
    ns = args[0]
...
def _generic_array(context, builder, shape, dtype, symbol_name, addrspace, can_dynsized=False):
    ...
    if dynamic_smem:
        ...
        get_dynshared_size = ir.InlineAsm(
            ir.FunctionType(ir.IntType(32), []), "mov.u32 $0, %dynamic_smem_size;", "=r", side_effect=True
        )
