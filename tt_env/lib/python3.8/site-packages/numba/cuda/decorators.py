...
def jit(
    func_or_sig=None, device=False, inline=False, link=[], debug=None, opt=True, lineinfo=False, cache=False, **kws
):
    """
    ...
    if signatures is not None:
        ...
        if func_or_sig is None:
            if config.ENABLE_CUDASIM:
                ...
                def autojitwrapper(func):
                    return jit(
                        func, device=device, debug=debug, opt=opt, lineinfo=lineinfo, link=link, cache=cache, **kws
                    )
