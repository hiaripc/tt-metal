...
class Reduce(object):
    ...
    def __call__(self, arr, size=None, res=None, init=0, stream=0):
        ...
        if size_full:
            ...
            kernel[full_blockct, blocksize, stream](arr[:size_full], partials[:full_blockct], init, True)

        if size_partial:
            # kernel for partially populated threadblocks
            kernel[1, size_partial, stream](arr[size_full:], partials[full_blockct:], init, not full_blockct)
