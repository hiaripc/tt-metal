...
class CUDATargetContext(BaseContext):
    ...
    def prepare_cuda_kernel(
        self, codelib, fndesc, debug, lineinfo, nvvm_options, filename, linenum, max_registers=None
    ):
        """
        ...
        library = self.codegen().create_library(
            f"{codelib.name}_kernel_", entry_name=kernel_name, nvvm_options=nvvm_options, max_registers=max_registers
        )
    ...
    def generate_kernel_wrapper(self, library, fndesc, kernel_name, debug, lineinfo, filename, linenum):
        ...
        if debug or lineinfo:
            ...
            debuginfo = self.DIBuilder(
                module=wrapper_module, filepath=filename, cgctx=self, directives_only=directives_only
            )
