...
def lower_constant_function_type(context, builder, typ, pyval):
    ...
    raise NotImplementedError(
        "lower_constant_struct_function_type({}, {}, {}, {})".format(context, builder, typ, pyval)
    )
...
def _get_wrapper_address(func, sig):
    ...
    if addr <= 0 and addr != -1:
        raise ValueError(
            f"wrapper address of {type(func)} instance must be" f" a positive integer but got {addr} [sig={sig}]"
        )
