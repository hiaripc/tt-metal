...
def get_os_spec_info(os_name):
    ...
    os_specific_funcs = {
        "Linux": {_libc_version: lambda: " ".join(platform.libc_ver())},
        "Windows": {
            _os_spec_version: lambda: " ".join(s for s in platform.win32_ver()),
        },
        "Darwin": {
            _os_spec_version: lambda: "".join(i or " " for s in tuple(platform.mac_ver()) for i in s),
        },
    }
...
def get_sysinfo():
    ...
    sys_info = {
        ...
        _numba_env_vars: {k: v for (k, v) in os.environ.items() if k.startswith("NUMBA_")},
...
def display_sysinfo(info=None, sep_pos=45):
    ...
    template = (
        ...
        ("CFS Restrictions (CPUs worth of runtime)", info.get(_cfs_restrict, "None")),
        ("",),
        (
            "CPU Features",
            "\n".join(
                " " * (sep_pos + 3) + l if i else l
                for i, l in enumerate(textwrap.wrap(info.get(_cpu_features, "?"), width=79 - sep_pos))
            ),
        ),
        ("",),
        ...
        (
            ...
            info.get(_mem_available, 0) // MB or "?",
        ),
        ("",),
        ("__OS Information__",),
        ...
        ("CUDA NVIDIA Bindings In Use", info.get(_cu_nvidia_bindings_used, "?")),
        ("CUDA Minor Version Compatibility Available", info.get(_cu_mvc_available, "?")),
        ("CUDA Minor Version Compatibility Needed", info.get(_cu_mvc_needed, "?")),
        ("CUDA Minor Version Compatibility In Use", info.get(_cu_mvc_in_use, "?")),
        ("CUDA Detect Output:",),
        ...
        ("NumPy Supported SIMD features", DisplaySeq(info.get(_numpy_supported_simd_features, []) or ("None found.",))),
        ("NumPy Supported SIMD dispatch", DisplaySeq(info.get(_numpy_supported_simd_dispatch, []) or ("None found.",))),
        ("NumPy Supported SIMD baseline", DisplaySeq(info.get(_numpy_supported_simd_baseline, []) or ("None found.",))),
        ("NumPy AVX512_SKX support detected", info.get(_numpy_AVX512_SKX_detected, "?")),
        ("",),
        ...
        (DisplayMap({k: v for k, v in info.items() if k.startswith("Conda")}) or ("Conda not available.",)),
        ("",),
        ("__Installed Packages__",),
        DisplaySeq(info.get(_inst_pkg, ("Couldn't retrieve packages info.",))),
        ("",),
        ("__Error log__" if info.get(_errors, []) else "No errors reported.",),
        DisplaySeq(info.get(_errors, [])),
        ("",),
        ("__Warning log__" if info.get(_warnings, []) else "No warnings reported.",),
