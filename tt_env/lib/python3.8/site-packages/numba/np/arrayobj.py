...
def normalize_indices(context, builder, index_types, indices):
    ...
    if len(indices):
        index_types, indices = zip(
            *[normalize_index(context, builder, idxty, idx) for idxty, idx in zip(index_types, indices)]
        )
...
def populate_array(array, data, shape, strides, itemsize, meminfo, parent=None):
    ...
    *shape* and *strides* can be Python tuples or LLVM arrays.
    ...
    if (required_fields & datamodel_fields) != required_fields:
        ...
        msg = (
            f"The datamodel for type {array._fe_type} is missing " f"field{'s' if len(missing) > 1 else ''} {missing}."
        )
...
def basic_indexing(context, builder, aryty, ary, index_types, indices, boundscheck=None):
    ...
    dataptr = cgutils.get_item_pointer(
        context, builder, aryty, ary, output_indices, wraparound=False, boundscheck=False
    )
    ...
    def get_size(self):
        ...
        with cgutils.for_range(builder, self.size) as loop:
            ...
            pred = _getitem_array_single_int(
                self.context, builder, self.idxty.dtype, self.idxty, self.idxary, loop.index
            )
    ...
    with builder.if_then(use_copy, likely=False):
        ...
        with cgutils.loop_nest(builder, src_shapes, intp_t) as indices:
            src_ptr = cgutils.get_item_pointer2(
                context, builder, src_data, src_shapes, src_strides, srcty.layout, indices
            )
    ...
    for arg in args:
        ...
        if unified_dtype is None:
            ...
            raise errors.TypingError(
                "Mismatch of argument types. Numba cannot " "broadcast arrays with different types. " f"Got {args}"
            )
...
def _change_dtype(context, builder, oldty, newty, ary):
    ...
    populate_array(
        shape_ary, data=shape_data, shape=shape_constant, strides=strides_constant, itemsize=sizeof_intp, meminfo=None
    )
        ...
        if dtype_size != a.itemsize:
            ...
            if dtype_size < a.itemsize:
                if dtype_size == 0 or a.itemsize % dtype_size != 0:
                    msg3 = (
                        "When changing to a smaller dtype, its size must " "be a divisor of the size of original dtype"
                    )
...
def ol_np_empty(shape, dtype=float):
    ...
    if (
        dtype is float or (isinstance(dtype, types.Function) and dtype.typing_key is float) or is_nonelike(dtype)
    ):  # default
...
def impl_np_frombuffer(buffer, dtype=float):
    ...
    if nb_dtype is not None:
        ...
        msg = "Cannot parse input types to function " f"np.frombuffer({buffer}, {dtype})"
        raise errors.TypingError(msg)
...
def assign_sequence_to_array(context, builder, data, shapes, strides, arrty, seqty, seq):
    ...
    def assign_item(indices, valty, val):
        ptr = cgutils.get_item_pointer2(
            context, builder, data, shapes, strides, arrty.layout, indices, wraparound=False
        )
...
def _atleast_nd(minimum, axes):
    ...
    def impl(typingcontext, *args):
        ...
        return (
            signature(types.Tuple(rettys) if len(rettys) > 1 else rettys[0], types.StarArgTuple.from_types(args)),
            codegen,
        )
...
def _do_concatenate(context, builder, axis, arrtys, arrs, arr_shapes, arr_strides, retty, ret_shapes):
    ...
    for arrty, arr, arr_sh, arr_st, offset in zip(arrtys, arrs, arr_shapes, arr_strides, copy_offsets):
        ...
        with loop_nest as indices:
            ...
            dest_ptr = cgutils.get_item_pointer2(
                context, builder, ret_data, ret_shapes, ret_strides, retty.layout, indices
            )
        ...
        with builder.if_else(is_axis) as (on_axis, on_other_dim):
            ...
            with on_other_dim:
                ...
                with builder.if_then(builder.not_(is_ok), likely=False):
                    context.call_conv.return_user_exc(
                        builder, ValueError, ("np.concatenate(): input sizes over " "dimension %d do not match" % dim,)
                    )
...
def _np_stack(context, builder, arrtys, arrs, retty, axis):
    ...
    for arr in arrs[1:]:
        ...
        for sh, orig_sh in zip(cgutils.unpack_tuple(builder, arr.shape), orig_shape):
            ...
            with builder.if_then(builder.not_(is_ok), likely=False):
                context.call_conv.return_user_exc(
                    builder, ValueError, ("np.stack(): all input arrays must have the same shape",)
                )
...
def np_concatenate(typingctx, arrays, axis):
    ...
    def codegen(context, builder, sig, args):
        ...
        return _np_concatenate(
            context, builder, list(sig.args[0]), cgutils.unpack_tuple(builder, args[0]), sig.return_type, axis
        )
    ...
    try:
        ...
        if kind == "quicksort":
            sort = quicksort.make_jit_quicksort(
                lt=lt_floats if is_float else None, is_argsort=is_argsort, is_np_array=True
            )
            func = sort.run_quicksort
        elif kind == "mergesort":
            sort = mergesort.make_jit_mergesort(lt=lt_floats if is_float else None, is_argsort=is_argsort)
...
def array_sort(context, builder, sig, args):
    arytype = sig.args[0]
    sort_func = get_sort_func(kind="quicksort", is_float=isinstance(arytype.dtype, types.Float))
...
def array_argsort(context, builder, sig, args):
    ...
    sort_func = get_sort_func(kind=kind.literal_value, is_float=isinstance(arytype.dtype, types.Float), is_argsort=True)
