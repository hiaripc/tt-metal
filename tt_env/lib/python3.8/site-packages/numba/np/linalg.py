...
class _BLAS:
    ...
    def numba_xxnrm2(cls, dtype):
        ...
        sig = types.intc(
            types.char, types.intp, types.CPointer(dtype), types.intp, types.CPointer(rtype)  # kind  # n  # x  # incx
        )  # returned
...
class _LAPACK:
    ...
    def numba_xxpotrf(cls, dtype):
        sig = types.intc(
            types.char, types.char, types.intp, types.CPointer(dtype), types.intp  # kind  # uplo  # n  # a  # lda
        )
...
def vdot(left, right):
    ...
    if isinstance(left, types.Array) and isinstance(right, types.Array):
        ...
        if left.layout not in "CF" or right.layout not in "CF":
            warnings.warn(
                "np.vdot() is faster on contiguous arrays, called on %s" % ((left, right),), NumbaPerformanceWarning
            )
...
def eig_impl(a):
    ...
    def real_eig_impl(a):
        ...
        r = numba_ez_rgeev(
            kind, JOBVL, JOBVR, n, acpy.ctypes, n, wr.ctypes, wi.ctypes, vl.ctypes, ldvl, vr.ctypes, ldvr
        )
...
def eigh_impl(a):
    ...
    def eigh_impl(a):
        ...
        r = numba_ez_xxxevd(kind, JOBZ, UPLO, n, acpy.ctypes, n, w.ctypes)  # kind  # jobz  # uplo  # n  # a  # lda  # w
        _handle_err_maybe_convergence_problem(r)
...
def qr_impl(a):
    ...
    def qr_impl(a):
        ...
        ret = numba_ez_geqrf(kind, m, n, q.ctypes, m, tau.ctypes)  # kind  # m  # n  # a  # lda  # tau
        if ret < 0:
            fatal_error_func()
            assert 0  # unreachable
        ...
        ret = numba_ez_xxgqr(kind, m, minmn, minmn, q.ctypes, m, tau.ctypes)  # kind  # m  # n  # k  # a  # lda  # tau
...
def _oneD_norm_2_impl(a):
    ...
    def impl(a):
        ...
        r = xxnrm2(kind, n, a.ctypes, jmp, ret.ctypes)  # kind  # n  # x  # incx  # result
        if r < 0:
            fatal_error_func()
            assert 0  # unreachable
...
def _check_scalar_or_lt_2d_mat(a, func_name, la_prefix=True):
    ...
    if isinstance(a, types.Array):
        if not a.ndim <= 2:
            raise TypingError("%s.%s() only supported on 1 and 2-D arrays " % interp, highlighting=False)
