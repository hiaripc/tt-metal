...
normal_year_months = create_constant_array(TIMEDELTA64, [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
leap_year_months = create_constant_array(TIMEDELTA64, [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
normal_year_months_acc = create_constant_array(TIMEDELTA64, [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334])
leap_year_months_acc = create_constant_array(TIMEDELTA64, [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335])
...
def timedelta_times_number(context, builder, sig, args):
    res = _timedelta_times_number(context, builder, args[0], sig.args[0], args[1], sig.args[1], sig.return_type)
    return impl_ret_untracked(context, builder, sig.return_type, res)
...
def is_leap_year(builder, year_val):
    ...
    not_multiple_of_100 = cgutils.is_not_null(builder, builder.srem(actual_year, Constant(DATETIME64, 100)))
...
def reduce_datetime_for_unit(builder, dt_val, src_unit, dest_unit):
    ...
    if src_unit_code == 0:
        ...
        normal_array = cgutils.global_constant(builder, "normal_year_months_acc", normal_year_months_acc)
...
def datetime_minus_timedelta(context, builder, sig, args):
    dt_arg, td_arg = args
    dt_type, td_type = sig.args
    res = _datetime_minus_timedelta(context, builder, dt_arg, dt_type.unit, td_arg, td_type.unit, sig.return_type.unit)
    return impl_ret_untracked(context, builder, sig.return_type, res)
