...
class _ArrayHelper(
    namedtuple(
        "_ArrayHelper", ("context", "builder", "shape", "strides", "data", "layout", "base_type", "ndim", "return_val")
    )
):
...
def _build_array(context, builder, array_ty, input_types, inputs):
    ...
    for arg_number, arg in enumerate(inputs):
        ...
        arg_result = context.compile_internal(
            builder, _broadcast_onto, _broadcast_onto_sig, [arg_ndim, src_shape, dest_ndim, dest_shape]
        )
    ...
    return _ArrayHelper(
        context, builder, shape, strides, array_val.data, array_ty.layout, array_ty.dtype, ndim, out_val
    )
...
def numpy_ufunc_kernel(context, builder, sig, args, ufunc, kernel_class):
    ...
    if len(arguments) < ufunc.nin:
        raise RuntimeError(
            "Not enough inputs to {}, expected {} got {}".format(ufunc.__name__, ufunc.nin, len(arguments))
        )
