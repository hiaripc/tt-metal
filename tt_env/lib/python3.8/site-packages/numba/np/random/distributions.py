...
def random_standard_normal(bitgen):
    while 1:
        ...
        if idx == 0:
            ...
            if ((fi_double[idx - 1] - fi_double[idx]) * next_double(bitgen) + fi_double[idx]) < np.exp(-0.5 * x * x):
                return x
...
def random_standard_normal_f(bitgen):
    while 1:
        ...
        if idx == 0:
            ...
            if ((fi_float[idx - 1] - fi_float[idx]) * next_float(bitgen) + fi_float[idx]) < float32(
                np.exp(-float32(0.5) * x * x)
            ):
                return x
...
def random_standard_exponential_f(bitgen):
    while 1:
        ...
        if ri < ke_float[idx]:
            ...
            elif (fe_float[idx - 1] - fe_float[idx]) * next_float(bitgen) + fe_float[idx] < float32(
                np.exp(float32(-x))
            ):
                return x
    ...
    while 1:
        if case == 10:
            ...
            if A > (
                ...
                + (13680.0 - (462.0 - (132.0 - (99.0 - 140.0 / w2) / w2) / w2) / w2) / w / 66320.0
            ):
                case = 10
                continue
        elif case == 60:
