...
def check_size(size):
    if not any(
        [
            isinstance(size, UniTuple) and isinstance(size.dtype, types.Integer),
            isinstance(size, Tuple) and size.count == 0,
            isinstance(size, types.Integer),
        ]
    ):
        raise TypingError(
            "Argument size is not one of the"
            + " expected type(s): "
...
def NumPyRandomGeneratorType_integers(inst, low, high, size=None, dtype=np.int64, endpoint=False):
    ...
    if isinstance(dtype, type):
        ...
        raise TypingError(
            "Argument dtype is not one of the" + " expected type(s): " + "np.int32, np.int64, np.int16, np.int8, "
