...
class UFuncMechanism(object):
    ...
    def _resolve_signature(self):
        ...
        if not matches:
            ...
            for formaltys in self.typemap:
                all_matches = all(
                    actual is None or formal == actual for formal, actual in zip(formaltys, self.argtypes)
                )
                if all_matches:
                    matches.append(formaltys)
        ...
        for i, (ity, val) in enumerate(zip(indtypes, self.inputs)):
            if ity != val.dtype:
                if not hasattr(val, "astype"):
                    msg = ("compatible signature is possible by casting but " "{0} does not support .astype()").format(
                        type(val)
                    )
                    raise TypeError(msg)
                # Cast types
