...
def _build_ufunc_loop_body(
    load, store, context, func, builder, arrays, out, offsets, store_offset, signature, pyapi, env
):
    elems = load()
...
def _build_ufunc_loop_body_objmode(
    load, store, context, func, builder, arrays, out, offsets, store_offset, signature, env, pyapi
):
    elems = load()
...
def build_slow_loop_body(context, func, builder, arrays, out, offsets, store_offset, signature, pyapi, env):
    ...
    return _build_ufunc_loop_body(
        load, store, context, func, builder, arrays, out, offsets, store_offset, signature, pyapi, env=env
    )
...
def build_obj_loop_body(context, func, builder, arrays, out, offsets, store_offset, signature, pyapi, envptr, env):
    ...
    return _build_ufunc_loop_body_objmode(
        load, store, context, func, builder, arrays, out, offsets, store_offset, signature, envptr, pyapi
    )
...
class _GufuncWrapper(object):
    ...
    def gen_loop_body(self, builder, pyapi, func, args):
        status, retval = self.call_conv.call_function(
            builder, func, self.signature.return_type, self.signature.args, args
        )
...
class GUArrayArg(object):
    def __init__(self, context, builder, args, steps, i, step_offset, typ, syms, sym_dim):
        ...
        if isinstance(typ, types.Array):
            ...
            self._loader = ldcls(
                dtype=typ.dtype, ndim=ndim, core_step=core_step, as_scalar=as_scalar, shape=shape, strides=strides
            )
        else:
...
class _ArrayArgLoader(object):
    ...
    def load(self, context, builder, data, ind):
        ...
        context.populate_array(
            array,
            data=builder.bitcast(offseted_data, array.data.type),
            shape=shape,
            strides=strides,
