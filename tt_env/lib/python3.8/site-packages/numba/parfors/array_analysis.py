...
random_calls = (
    random_int_args + random_1arg_size + random_2arg_sizelast + random_3arg_sizelast + ["randint", "triangular"]
)
...
def assert_equiv(typingctx, *val):
    ...
    assert all(
        isinstance(a, (types.ArrayCompatible, types.BaseTuple, types.SliceType, types.Integer)) for a in val[0][1:]
    )
...
class SymbolicEquivSet(ShapeEquivSet):
    ...
    def __repr__(self):
        return "SymbolicEquivSet({}, ind_to_var={}, def_by={}, " "ref_by={}, ext_shapes={})".format(
            self.ind_to_obj,
            self.ind_to_var,
            self.def_by,
            self.ref_by,
...
class ArrayAnalysis(object):
    ...
    def run(self, blocks=None, equiv_set=None):
        ...
        self.alias_map, self.arg_aliases = find_potential_aliases(
            blocks, self.func_ir.arg_names, self.typemap, self.func_ir
        )
        ...
        if isinstance(inst, ir.Assign):
            ...
            if isinstance(typ, types.ArrayCompatible) and typ.ndim == 0:
                ...
                if isinstance(typ, types.containers.UniTuple) and isinstance(typ.dtype, types.Integer):
                    shape = inst.value
                elif isinstance(typ, types.containers.Tuple) and all(
                    [isinstance(x, (types.Integer, types.IntegerLiteral)) for x in typ.types]
                ):
                    shape = inst.value
            ...
            if isinstance(typ, types.ArrayCompatible):
                if (
                    ...
                    and isinstance(self.typemap[shape.name], types.containers.BaseTuple)
                ):
                    pass
                elif (
                    shape is None
                    or isinstance(shape, tuple)
                    or (isinstance(shape, ir.Var) and not equiv_set.has_shape(shape))
                ):
            ...
            def handle_call_binop(cond_def):
                ...
                if br is not None and (
                    (isinstance(lhs_typ, types.Integer) and isinstance(rhs_typ, types.Integer))
                    or (isinstance(lhs_typ, types.BaseTuple) and isinstance(rhs_typ, types.BaseTuple))
                ):
                    loc = inst.loc
    ...
    def slice_size(self, index, dsize, equiv_set, scope, stmts):
        ...
        if config.DEBUG_ARRAY_OPT >= 2:
            print(
                f"slice_size index={index} dsize={dsize} "
        ...
        if lhs_known and rhs_known:
            ...
            return (
                self.gen_static_slice_size(lhs_rel, rhs_rel, loc, scope, stmts, equiv_set),
