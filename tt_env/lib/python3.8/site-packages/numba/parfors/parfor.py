...
class LoopNest(object):
    ...
    def __repr__(self):
        return "LoopNest(index_variable = {}, range = ({}, {}, {}))".format(
            self.index_variable, self.start, self.stop, self.step
        )
        ...
        for p in self.params:
            ...
            if isinstance(ty, types.BaseTuple):
                if ty.count > config.PARFOR_MAX_TUPLE_SIZE:
                    msg = (
                        "Use of a tuple (%s) of length %d in a parallel region "
            ...
            for inst in stmt:
                if isinstance(inst.value, ir.Expr):
                    try:
                        ...
                        if attr == "empty":
                            msg = (
                                "The memory allocation derived from the "
...
class PreParforPass(object):
    ...
    def __init__(
        self, func_ir, typemap, calltypes, typingctx, targetctx, options, swapped={}, replace_functions_map=None
    ):
        self.func_ir = func_ir
                    ...
                    if isinstance(expr, ir.Expr) and expr.op == "call":
                        ...
                        def replace_func():
                            ...
                            if (
                                repl_func is None
                                and len(callname) == 2
                                and isinstance(callname[1], ir.Var)
                                and isinstance(self.typemap[callname[1].name], types.npytypes.Array)
                            ):
                            ...
                            new_blocks, _ = inline_closure_call(
                                self.func_ir,
                                g,
                                block,
                                i,
                    ...
                    if isinstance(expr, ir.Expr) and expr.op == "inplace_binop":
                        ...
                        if isinstance(target_typ, types.npytypes.Array):
                            ...
                            if isinstance(value_typ, types.npytypes.Array):
                                new_instr = self._inplace_binop_to_parfor(
                                    equiv_set, loc, expr.immutable_fn, target, value
                                )
                                self.rewritten.append(
                                    dict(old=instr, new=new_instr, reason="inplace_binop"),
                                )
        ...
        for label in topo_order:
            ...
            for instr in block.body:
                if isinstance(instr, (ir.StaticSetItem, ir.SetItem)):
                    ...
                    if isinstance(target_typ, types.npytypes.Array):
                        if (
                            isinstance(index_typ, types.npytypes.Array)
                            and isinstance(index_typ.dtype, types.Boolean)
                            and target_typ.ndim == index_typ.ndim
                        ):
                            ...
                            if isinstance(value_typ, types.Number):
                                ...
                                self.rewritten.append(
                                    dict(old=instr, new=new_instr, reason="masked_assign_broadcast_scalar"),
                                )
                                instr = new_instr
                            # RHS is an array
                                ...
                                if (
                                    isinstance(val_def, ir.Expr)
                                    and val_def.op == "getitem"
                                    and val_def.index.name == index.name
                                ):
                                    new_instr = self._setitem_to_parfor(equiv_set, loc, target, index, val_def.value)
                                    self.rewritten.append(
                                        dict(old=instr, new=new_instr, reason="masked_assign_array"),
                                    )
                                    instr = new_instr
                        else:
                            # Handle A[:] = x
                            ...
                            if isinstance(index_typ, types.BaseTuple):
                                ...
                                # in the index tuple.
                                sliced_dims = len(
                                    list(filter(lambda x: isinstance(x, types.misc.SliceType), index_typ.types))
                                )
                            ...
                            if shape is not None and (
                                not isinstance(value_typ, types.npytypes.Array) or sliced_dims == value_typ.ndim
                            ):
                                new_instr = self._setitem_to_parfor(equiv_set, loc, target, index, value, shape=shape)
                                self.rewritten.append(
                                    dict(old=instr, new=new_instr, reason="slice"),
                                )
                                instr = new_instr
                new_body.append(instr)
            block.body = new_body
    ...
    def _setitem_to_parfor(self, equiv_set, loc, target, index, value, shape=None):
        ...
        if shape:
            ...
            body_block.body.extend(
                [ir.Assign(mask_val, mask_var, loc), ir.Branch(mask_var, true_label, end_label, loc)]
            )
...
class ConvertNumpyPass:
    ...
    def _numpy_to_parfor(self, equiv_set, lhs, expr):
        ...
        raise errors.UnsupportedRewriteError(
            f"parfor translation failed for {expr}",
            loc=expr.loc,
        )
...
class ConvertReducePass:
    ...
    def _reduce_to_parfor(self, equiv_set, lhs, args, loc):
        ...
        parfor = Parfor(
            ...
            index_var,
            equiv_set,
            ("{} function".format(call_name), "reduction"),
            pass_states.flags,
        )
        if config.DEBUG_ARRAY_OPT >= 1:
    ...
    def _mk_reduction_body(self, call_name, scope, loc, index_vars, in_arr, acc_var):
        ...
        reduce_f_ir = compile_to_numba_ir(
            fcode,
...
class ConvertLoopPass:
        ...
        for loop, s in sorted(sized_loops, key=lambda tup: tup[1]):
            ...
            for inst in blocks[entry].body:
                ...
                if (
                    ...
                    races = races.intersection(
                        {x for x in races if not isinstance(pass_states.typemap[x], types.misc.Module)}
                    )
                    ...
                    def find_mask_from_size(size_var):
                        ...
                        require(
                            size_def
                            and isinstance(size_def, ir.Expr)
                            and size_def.op == "getattr"
                            and size_def.attr == "shape"
                        )
                    ...
                    if loop_kind == "pndindex":
                        ...
                        index_var, index_var_typ = _make_index_var(
                            pass_states.typemap, scope, index_vars, body_block, force_tuple=True
                        )
                        ...
                        if mask_var is not None:
                            ...
                            body_block.body.extend(
                                [ir.Assign(mask_val, mask, loc), ir.Branch(mask, true_label, false_label, loc)]
                            )
                    else:  # prange
                        start = 0
                        step = 1
    ...
    def _is_parallel_loop(self, func_var, call_table):
        ...
        return len(call) > 0 and (
            call[0] == "prange"
            or call[0] == prange
            or call[0] == "internal_prange"
            or call[0] == internal_prange
            or call[0] == "pndindex"
            or call[0] == pndindex
        )
            ...
            for stmt in block.body:
                if isinstance(stmt, ir.Assign) and isinstance(stmt.value, ir.Var):
                    ...
                    # make sure parallel index is not overwritten
                        ...
                        if unver(stmt.target.name) in map(unver, index_set) and unver(stmt.target.name) != unver(
                            stmt.value.name
                        ):
                            raise errors.UnsupportedRewriteError(
                ...
                while i < len(block.body) - 1:
                    ...
                    if isinstance(stmt, Parfor) and isinstance(next_stmt, Parfor):
                        ...
                        fused_node, fuse_report = try_fuse(
                            equiv_set, stmt, next_stmt, self.metadata["parfors"], func_ir, typemap
                        )
...
def lower_parfor_sequential(typingctx, func_ir, typemap, calltypes, metadata):
    ...
    for block_label, block in func_ir.blocks.items():
        block_label, parfor_found = _lower_parfor_sequential_block(
            block_label, block, new_blocks, typemap, calltypes, parfor_found, scope=scope
        )
    ...
    while i != -1:
        ...
        for l, b in inst.loop_body.items():
            l, parfor_found = _lower_parfor_sequential_block(
                l, b, new_blocks, typemap, calltypes, parfor_found, scope=scope
            )
        ...
        for stmt in blk.body:
            if isinstance(stmt, (ir.StaticSetItem, ir.SetItem)):
                ...
                if isinstance(setarray_index, ir.Var) and (
                    setarray_index.name == index or _is_indirect_index(func_ir, setarray_index, nest_indices)
                ):
                ...
                if isinstance(getarray_index, ir.Var) and (
                    getarray_index.name == index or _is_indirect_index(func_ir, getarray_index, nest_indices)
                ):
                ...
                get_array_indexed_with_parfor_index_internal(
                    stmt.loop_body.values(), index, ret_indexed, ret_not_indexed, nest_indices, func_ir
                )
        ...
        if isinstance(node, ir.Assign) and isinstance(node.value, ir.Expr) and node.value.op == "inplace_binop":
            if first_red_func is None:
                ...
                if first_red_func != node.value.fn:
                    msg = (
                        "Reduction variable %s has multiple conflicting "
        ...
        if isinstance(rhs, ir.Expr):
            ...
            if name in in_vars:
                ...
                if (
                    not (
                        i + 1 < len(nodes)
                        and isinstance(nodes[i + 1], ir.Assign)
                ...
                if not supported_reduction(rhs, func_ir):
                    raise ValueError(
                        ("Use of reduction variable " + unversioned_name + " in an unsupported reduction function.")
                    )
...
def maximize_fusion_inner(func_ir, block, call_table, alias_map, arg_aliases, up_direction=True):
    ...
    while i < len(block.body) - 2:
        ...
        next_stmt = block.body[i + 1]
        can_reorder = (
            _can_reorder_stmts(stmt, next_stmt, func_ir, call_table, alias_map, arg_aliases)
            if up_direction
            else _can_reorder_stmts(next_stmt, stmt, func_ir, call_table, alias_map, arg_aliases)
        )
        if can_reorder:
...
def try_fuse(equiv_set, parfor1, parfor2, metadata, func_ir, typemap):
    ...
    for i in range(ndims):
        ...
        if not (
            is_equiv(nest1.start, nest2.start) and is_equiv(nest1.stop, nest2.stop) and is_equiv(nest1.step, nest2.step)
        ):
    ...
    if len(overlap) != 0:
        ...
        _, p2arraynotindexed = get_array_indexed_with_parfor_index(
            parfor2.loop_body.values(), parfor2.index_var.name, parfor2.get_loop_nest_vars(), func_ir
        )
...
def has_cross_iter_dep(parfor, func_ir, typemap, index_positions=None, indexed_arrays=None, non_indexed_arrays=None):
    ...
    def check_index(
        stmt_index, array_accessed, index_positions, indexed_arrays, non_indexed_arrays, derived_from_indices
    ):
        ...
        if isinstance(stmt_index, ir.Var):
            ...
            if isinstance(typemap[stmt_index.name], types.BaseTuple):
                ...
                if fbs_res is not None:
                    ...
                    if all([x.name in indices or x.name not in derived_from_indices for x in ind_seq]):
                        ...
                        return add_check_position(
                            new_index_positions, array_accessed, index_positions, indexed_arrays, non_indexed_arrays
                        )
                    else:
                ...
                if stmt_index.name in indices:
                    ...
                    return add_check_position(
                        (True,), array_accessed, index_positions, indexed_arrays, non_indexed_arrays
                    )
                    ...
                    return add_check_position(
                        (False,), array_accessed, index_positions, indexed_arrays, non_indexed_arrays
                    )
        else:
...
def remove_dead_parfor(parfor, lives, lives_n_aliases, arg_aliases, alias_map, func_ir, typemap):
    ...
    _update_parfor_get_setitems(
        parfor.loop_body[first_label].body, parfor.index_var, alias_map, first_block_saved_values, lives_n_aliases
    )
...
def _update_parfor_get_setitems(block_body, index_var, alias_map, saved_values, lives):
    ...
    for stmt in block_body:
        if (
            isinstance(stmt, (ir.StaticSetItem, ir.SetItem))
            and get_index_var(stmt).name == index_var.name
