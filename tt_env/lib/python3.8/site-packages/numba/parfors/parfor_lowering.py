...
def _lower_parfor_parallel_std(lowerer, parfor):
    ...
    try:
        (func, func_args, func_sig, func_arg_types, exp_name_to_tuple_var) = _create_gufunc_for_parfor_body(
            lowerer, parfor, typemap, typingctx, targetctx, flags, {}, bool(alias_map), index_var_typ, parfor.races
        )
    finally:
    ...
    gu_signature = _create_shape_signature(
        parfor.get_shape_classes, num_inputs, num_reductions, func_args, func_sig, parfor.races, typemap
    )
    if config.DEBUG_ARRAY_OPT:
...
def _parfor_lowering_finalize_reduction(
    ...
    for redvar_name, redarr_var in redarrs.items():
        ...
        handler = (
            _lower_trivial_inplace_binops if reduce_info.redvar_info.redop is not None else _lower_non_trivial_reduce
        )
            ...
            if empty_container_allocator_hoist(inst, dep_on_param, call_table, hoisted, not_hoisted, typemap, itemsset):
                ...
                for ib_inst in inst.init_block.body:
                    if empty_container_allocator_hoist(
                        ib_inst, dep_on_param, call_table, hoisted, not_hoisted, typemap, itemsset
                    ):
                        continue
...
def _create_gufunc_for_parfor_body(
    lowerer, parfor, typemap, typingctx, targetctx, flags, locals, has_aliases, index_var_typ, races
):
    """
    ...
    for pindex in range(len(parfor_inputs)):
        if ascontig and isinstance(param_types[pindex], types.npytypes.Array):
            gufunc_txt += (
                "    " + parfor_params_orig[pindex] + " = np.ascontiguousarray(" + parfor_params[pindex] + ")\n"
            )
    ...
    for arr, var in zip(parfor_redarrs, parfor_redvars):
        if config.DEBUG_ARRAY_OPT_RUNTIME:
            gufunc_txt += (
                '    print("final reduction value",ParallelAcceleratorGufuncThreadId,' + param_dict[var] + ")\n"
            )
            gufunc_txt += (
                '    print("final reduction array",ParallelAcceleratorGufuncThreadId,' + param_dict[arr] + ")\n"
            )
...
def call_parallel_gufunc(
    ...
    dim_starts = cgutils.alloca_once(
        builder, sched_type, size=context.get_constant(types.uintp, num_dim), name="dim_starts"
    )
    dim_stops = cgutils.alloca_once(
        builder, sched_type, size=context.get_constant(types.uintp, num_dim), name="dim_stops"
    )
    for i in range(num_dim):
        start, stop, step = loop_ranges[i]
    ...
    set_chunksize = cgutils.get_or_insert_function(
        builder.module, llvmlite.ir.FunctionType(llvmlite.ir.VoidType(), [uintp_t]), name="set_parallel_chunksize"
    )

    get_num_threads = cgutils.get_or_insert_function(
        builder.module, llvmlite.ir.FunctionType(llvmlite.ir.IntType(types.intp.bitwidth), []), "get_num_threads"
    )
    ...
    with cgutils.if_unlikely(builder, builder.icmp_signed("<=", num_threads, num_threads.type(0))):
        ...
        context.call_conv.return_user_exc(
            builder, RuntimeError, ("Invalid number of threads. " "This likely indicates a bug in Numba.",)
        )
    ...
    num_divisions = builder.call(
        get_sched_size, [num_threads, context.get_constant(types.uintp, num_dim), dim_starts, dim_stops]
    )
