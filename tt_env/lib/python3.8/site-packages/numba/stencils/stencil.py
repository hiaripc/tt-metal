...
def raise_if_incompatible_array_sizes(a, *args):
    ...
    for arg in literal_unroll(args):
        if a.ndim != arg.ndim:
            raise ValueError(
                "Secondary stencil array does not have same number " " of dimensions as the first stencil input."
            )
        argshape = arg.shape
...
class StencilFunc(object):
    ...
    def add_indices_to_kernel(self, kernel, index_names, ndim, neighborhood, standard_indexed, typemap, calltypes):
        ...
        if neighborhood is None:
            ...
            if len(neighborhood) != ndim:
                raise ValueError(
                    "%d dimensional neighborhood specified for %d "
        ...
        for block in kernel.blocks.values():
            ...
            for stmt in block.body:
                ...
                ) or (isinstance(stmt, ir.SetItem) and stmt.target.name in kernel.arg_names):
                ...
                if (
                    ...
                    if ndim == 1:
                        ...
                        new_body.append(ir.Assign(tuple_call, s_index_var, loc))
                        new_body.append(
                            ir.Assign(ir.Expr.getitem(stmt.value.value, s_index_var, loc), stmt.target, loc)
                        )
                else:
                    new_body.append(stmt)
            block.body = new_body
    ...
    def _type_me(self, argtys, kwtys):
        ...
        if self.neighborhood is not None and len(self.neighborhood) != argtys[0].ndim:
            raise NumbaValueError(
                ...
                "for %d dimensional input array" % (len(self.neighborhood), argtys[0].ndim)
            )
    ...
    def _stencil_wrapper(self, result, sigret, return_type, typemap, calltypes, *args):
        ...
        kernel_size, relatively_indexed = self.add_indices_to_kernel(
            kernel_copy, index_vars, the_array.ndim, self.neighborhood, standard_indexed, typemap, copy_calltypes
        )
        ...
        new_func = compiler.compile_ir(
            self._typingctx, self._targetctx, stencil_ir, new_stencil_param_types, None, compiler.DEFAULT_FLAGS, {}
        )
        return new_func
    ...
    def __call__(self, *args, **kwargs):
        if self.neighborhood is not None and len(self.neighborhood) != args[0].ndim:
            raise ValueError(
                ...
                "dimensional input array".format(len(self.neighborhood), args[0].ndim)
            )
