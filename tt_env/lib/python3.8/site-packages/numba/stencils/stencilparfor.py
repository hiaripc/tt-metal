...
class StencilPass(object):
        ...
        for label, block in self.func_ir.blocks.items():
            for i, stmt in reversed(list(enumerate(block.body))):
                ...
                if (
                    ...
                    for arg_type in arg_typemap:
                        if isinstance(arg_type, types.BaseTuple):
                            raise ValueError(
                                "Tuple parameters not supported " "for stencil kernels in parallel=True mode."
                            )
                    ...
                    gen_nodes = self._mk_stencil_parfor(
                        label, in_args, out_arr, stencil_ir, index_offsets, stmt.target, rt, sf, arg_to_arr_dict
                    )
                ...
                # Found a call to a stencil via numba.stencil().
                    ...
                    and isinstance(stmt.value, ir.Expr)
                    and stmt.value.op == "call"
                    and guard(find_callname, self.func_ir, stmt.value) == ("stencil", "numba")
                ):
    ...
    def _mk_stencil_parfor(
        self, label, in_args, out_arr, stencil_ir, index_offsets, target, return_type, stencil_func, arg_to_arr_dict
    ):
        ...
        if config.DEBUG_ARRAY_OPT >= 1:
            print(
                "_mk_stencil_parfor", label, in_args, out_arr, index_offsets, return_type, stencil_func, stencil_blocks
            )
        ...
        start_lengths, end_lengths = self._replace_stencil_accesses(
            stencil_ir, parfor_vars, in_args, index_offsets, stencil_func, arg_to_arr_dict
        )
        ...
        if out_arr is None:
            ...
            stmts = ir_utils.gen_np_call(
                "empty", np.empty, out_arr, [shape_var, dtype_attr_var], self.typingctx, self.typemap, self.calltypes
            )
            # ------------------
        ...
        self.calltypes[setitem_call] = signature(
            types.none, self.typemap[out_arr.name], self.typemap[parfor_ind_var.name], self.typemap[out_arr.name].dtype
        )
        ...
        parfor = numba.parfors.parfor.Parfor(
            loopnests, init_block, stencil_blocks, loc, parfor_ind_var, equiv_set, pattern, self.flags
        )
    ...
    def _get_stencil_start_ind(self, start_length, gen_nodes, scope, loc):
        ...
        f_ir = compile_to_numba_ir(
            get_start_ind, {}, self.typingctx, self.targetctx, (types.intp,), self.typemap, self.calltypes
        )
            ...
            for stmt in block.body:
                ...
                if (
                    ...
                    self.calltypes[getitem_call] = signature(
                        getitem_return_typ, self.typemap[stmt.value.value.name], self.typemap[ind_var.name]
                    )
...
def get_stencil_ir(sf, typingctx, args, scope, loc, input_dict, typemap, calltypes):
    ...
    with cpu_target.nested_context(typingctx, targetctx):
        ...
        tp.state.typemap, tp.state.return_type, tp.state.calltypes, _ = type_inference_stage(
            tp.state.typingctx, tp.state.targetctx, tp.state.func_ir, tp.state.args, None
        )
