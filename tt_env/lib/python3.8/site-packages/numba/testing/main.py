    ...
    * a -l option to (only) list tests
    ...
    def _getParentArgParser(self):
        ...
        if self.testRunner is None:
            parser.add_argument(
                "-R", "--refleak", dest="refleak", action="store_true", help="Detect reference / memory leaks"
            )
        parser.add_argument(
            "-m",
            "--multiprocess",
        ...
        parser.add_argument(
            "--tags", dest="tags", type=str, help="Comma-separated list of tags to select " "a subset of the test suite"
        )
        parser.add_argument(
            "--exclude-tags",
            dest="exclude_tags",
            type=str,
            help="Comma-separated list of tags to de-select " "a subset of the test suite",
        )
        ...
        parser.add_argument(
            "-j", "--slice", dest="useslice", nargs="?", type=str, const="None", help="Shard the test sequence"
        )
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "origin/release0.58 as identified by `git diff`. "
    ...
    def runTests(self):
        if self.refleak:
            ...
            self.testRunner = BasicTestRunner(
                self.useslice, verbosity=self.verbosity, failfast=self.failfast, buffer=self.buffer
            )
