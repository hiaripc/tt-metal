...
from numba.core.compiler import compile_extra, compile_isolated, Flags, DEFAULT_FLAGS, CompilerBase, DefaultPassBuilder
...
skip_ppc64le_issue4563 = unittest.skipIf(
    platform.machine() == "ppc64le", ("Hits: 'Parameter area must exist " "to pass an argument in memory'")
)
...
skip_ppc64le_issue6465 = unittest.skipIf(
    platform.machine() == "ppc64le", ("Hits: 'mismatch in size of " "parameter area' in " "LowerCall_64SVR4")
)
...
skip_m1_llvm_rtdyld_failure = unittest.skipIf(
    IS_OSX_ARM64,
    "skip tests that contribute to triggering an AssertionError in LLVM's "
    "RuntimeDyLd on OSX arm64. (see: numba#8567)",
)

skip_m1_fenv_errors = unittest.skipIf(IS_OSX_ARM64, "fenv.h-like functionality unreliable on OSX arm64")
    ...
    def compile(self, func, args, return_type=None, flags=DEFAULT_FLAGS):
        ...
        if cache_key in self.cr_cache:
            ...
            with cpu_target.nested_context(self.typingctx, self.targetctx):
                cr = compile_extra(self.typingctx, self.targetctx, func, args, return_type, flags, locals={})
...
class TestCase(unittest.TestCase):
    ...
    def assertRefCountEqual(self, *objects):
        ...
        for i in range(len(objects))[1:]:
            rc_i = rc[i]
            if rc_0 != rc_i:
                self.fail(f"Refcount for objects does not match. " f"#0({rc_0}) != #{i}({rc_i}) does not match.")
    ...
    def assertPreciseEqual(
        self, first, second, prec="exact", ulps=1, msg=None, ignore_sign_on_zero=False, abs_tol=None
    ):
        """
    ...
    def subprocess_test_runner(self, test_module, test_class=None, test_name=None, envvars=None, timeout=60):
        ...
        status = subprocess.run(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout, env=env_copy, universal_newlines=True
        )
...
def strace(work, syscalls, timeout=10):
    ...
    with tempfile.NamedTemporaryFile("w+t") as ntf:
        ...
        def check_return(problem=""):
            ...
            if ret != 0:
                msg = "strace exited non-zero, process return code was:" f"{ret}. {problem}"
...
class IRPreservingTestPipeline(CompilerBase):
    ...
    def define_pipelines(self):
        pipeline = DefaultPassBuilder.define_nopython_pipeline(self.state, "ir_preserving_custom_pipe")
