...
from numba.tests.support import TestCase, CompilationCache, MemoryLeakMixin, tag, needs_blas
import unittest
...
class TestArrayMethods(MemoryLeakMixin, TestCase):
    ...
    def test_sum(self):
        ...
        all_test_arrays = [
            [np.ones((7, 6, 5, 4, 3), arr_dtype), np.ones(1, arr_dtype), np.ones((7, 3), arr_dtype) * -5]
                    ...
                    with self.subTest("Testing np.sum(axis) with {} " "input ".format(arr.dtype)):
                        ...
                        if isinstance(numba_res, np.ndarray):
                            self.assertPreciseEqual(
                                npy_res.astype(out_dtypes[arr.dtype]), numba_res.astype(out_dtypes[arr.dtype])
                            )
                        else:
