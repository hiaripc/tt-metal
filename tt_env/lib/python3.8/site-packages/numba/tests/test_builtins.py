...
class TestBuiltins(TestCase):
    ...
    def test_divmod_ints(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(pyfunc, (types.int64, types.int64), flags=flags)
        cfunc = cr.entry_point
    ...
    def test_divmod_floats(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(pyfunc, (types.float64, types.float64), flags=flags)
        cfunc = cr.entry_point
    ...
    def test_filter(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(pyfunc, (types.Dummy("list"), types.Dummy("function_ptr")), flags=flags)
        cfunc = cr.entry_point
    ...
    def test_iter_next(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(pyfunc, (types.UniTuple(types.int32, 3),), flags=flags)
        cfunc = cr.entry_point
        self.assertPreciseEqual(cfunc((1, 42, 5)), (1, 42))

        cr = compile_isolated(pyfunc, (types.UniTuple(types.int32, 1),), flags=flags)
        cfunc = cr.entry_point
    ...
    def check_min_max_invalid_types(self, pyfunc, flags=enable_pyobj_flags):
        cr = compile_isolated(pyfunc, (types.int32, types.Dummy("list")), flags=flags)
        cfunc = cr.entry_point
        cfunc(1, [1])
    ...
    def test_reduce(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(pyfunc, (types.Dummy("function_ptr"), types.Dummy("list")), flags=flags)
        cfunc = cr.entry_point
    ...
    def test_pow_op_usecase(self):
        ...
        for x, y in args:
            cres = compile_isolated(pow_op_usecase, (typeof(x), typeof(y)), flags=no_pyobj_flags)
    ...
    def test_pow_usecase(self):
        ...
        for x, y in args:
            cres = compile_isolated(pow_usecase, (typeof(x), typeof(y)), flags=no_pyobj_flags)
            r = cres.entry_point(x, y)
    ...
    def test_isinstance_exceptions(self):
        fns = [
            ...
            (
                ...
                ('isinstance() cannot determine the type of variable "z" due to a ' "branch."),
            ),
            (
                invalid_isinstance_optional_usecase,
                ('isinstance() cannot determine the type of variable "z" due to a ' "branch."),
            ),
            (invalid_isinstance_unsupported_type_usecase(), ('isinstance() does not support variables of type "ntpl(')),
        ]
