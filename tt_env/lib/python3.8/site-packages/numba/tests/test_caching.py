    ...
    def test_ipython(self):
        ...
        def execute_with_input():
            ...
            with open(inputfn, "rb") as stdin:
                p = subprocess.Popen(
                    base_cmd, stdin=stdin, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
                )
...
"""
    ...
    def test_no_collision(self):
        ...
        proc = mp.Process(
            target=cache_file_collision_tester, args=(q, self.tempdir, self.modname_bar1, self.modname_bar2)
        )
        proc.start()
...
class TestCFuncCache(BaseCacheTest):
    ...
    def run_in_separate_process(self):
        ...
        out, err = popen.communicate()
        if popen.returncode != 0:
            raise AssertionError(f"process failed with code {popen.returncode}:" f"stderr follows\n{err.decode()}\n")
