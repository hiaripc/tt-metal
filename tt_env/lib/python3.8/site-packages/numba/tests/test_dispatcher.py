...
class TestSignatureHandling(BaseTest):
    ...
    def test_default_args(self):
        ...
        with self.assertRaises(TypeError) as cm:
            f()
        self.assertIn("not enough arguments: expected at least 1, got 0", str(cm.exception))
        with self.assertRaises(TypeError) as cm:
            f(y=6, z=7)
...
class TestGeneratedDispatcher(TestCase):
    ...
    def test_signature_errors(self):
        ...
        with self.assertRaises(TypeError) as raises:
            f(1j)
        self.assertIn(
            "should be compatible with signature '(x, y=5)', " "but has signature '(x)'", str(raises.exception)
        )
        # Mismatching defaults
        with self.assertRaises(TypeError) as raises:
            f(1)
        self.assertIn(
            "should be compatible with signature '(x, y=5)', " "but has signature '(x, y=6)'", str(raises.exception)
        )
