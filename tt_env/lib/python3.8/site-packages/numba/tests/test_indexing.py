...
class TestGetItem(TestCase):
    ...
    def test_2d_integer_indexing(self, flags=enable_pyobj_flags, pyfunc=integer_indexing_2d_usecase):
        ...
        a = np.arange(100, dtype="i4").reshape(10, 10)
        arraytype = types.Array(types.int32, 2, "C")
        cr = compile_isolated(pyfunc, (arraytype, types.int32, types.int32), flags=flags)
        cfunc = cr.entry_point
        ...
        cr = compile_isolated(pyfunc, (arraytype, indextype, indextype), flags=flags)
        cfunc = cr.entry_point
    ...
    def test_2d_float_indexing(self, flags=enable_pyobj_flags):
        ...
        arraytype = types.Array(types.int32, 2, "C")
        cr = compile_isolated(pyfunc, (arraytype, types.float32, types.int32), flags=flags)
        cfunc = cr.entry_point
    ...
    def check_ellipsis(self, pyfunc, flags):
        def compile_func(arr):
            cr = compile_isolated(pyfunc, (typeof(arr), types.intp, types.intp), flags=flags)
