...
"""
        ...
        for jit_f, (py_cls, jit_cls), (x, y) in itertools.product(
            ...
            [(PyFloatWrapper, JitFloatWrapper), (PyFloatUpdateWrapper, JitFloatUpdateWrapper)],
            test_values,
        ):
            py_f = jit_f.py_func
        ...
        for jit_f, (py_cls, jit_cls), (x, y) in itertools.product(
            ...
            [(PyIntWrapper, JitIntWrapper), (PyIntUpdateWrapper, JitIntUpdateWrapper)],
            test_values,
        ):
            x, y = int(x), int(y)
            py_f = jit_f.py_func
    ...
    def test_arithmetic_logical_reflection(self):
        ...
        for test_type, test_op, test_value in [
            (int32, int_op, (2, 4)),
            (float64, float_op, (2.0, 4.0)),
            (float64[::1], float_op, (np.array([1.0, 2.0, 4.0]), np.array([20.0, -24.0, 1.0]))),
        ]:
            spec = {"x": test_type}
