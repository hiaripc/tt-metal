...
from numba.tests.support import TestCase, tag, needs_lapack, needs_blas, _is_armv7l, EnableNRTStatsMixin
...
class TestLinalgSvd(TestLinalgBase):
    ...
    def check_reconstruction(self, a, got, expected):
        ...
        np.testing.assert_allclose(a, rec, rtol=10 * resolution, atol=100 * resolution)  # zeros tend to be fuzzy
        ...
        def check(a, **kwargs):
            ...
            if use_reconstruction:
                ...
                np.testing.assert_allclose(
                    a, rec, rtol=10 * resolution, atol=100 * resolution  # zeros tend to be fuzzy
                )
            ...
            if use_reconstruction:
                ...
                try:
                    ...
                    for k in out_array_idx:
                        try:
                            np.testing.assert_allclose(
                                expected[k], got[k], rtol=100 * resolution, atol=100 * resolution
                            )
                        except AssertionError:
    ...
    def test_linalg_solve(self):
        ...
        def check(a, b, **kwargs):
            ...
            if use_reconstruction:
                ...
                resolution = np.finfo(a.dtype).resolution
                np.testing.assert_allclose(
                    b, rec, rtol=10 * resolution, atol=100 * resolution  # zeros tend to be fuzzy
                )
        ...
        def check(a, **kwargs):
            ...
            if use_reconstruction:
                ...
                try:
                    ...
                    rec = cfunc(got)
                    np.testing.assert_allclose(
                        rec, a, rtol=10 * resolution, atol=100 * resolution  # zeros tend to be fuzzy
                    )
                    if a.shape[0] >= a.shape[1]:
                        ...
                        np.testing.assert_allclose(
                            got, lstsq_pinv, rtol=10 * resolution, atol=100 * resolution  # zeros tend to be fuzzy
                        )
...
class TestTrace(TestLinalgBase):
    ...
    def test_trace(self):
        ...
        # Dimension issue
        self.assert_wrong_dimensions(rn, self.cfunc_w_offset, (np.ones(10, dtype=np.float64), 1), False)
        self.assert_wrong_dimensions(rn, self.cfunc_no_offset, (np.ones(10, dtype=np.float64),), False)
