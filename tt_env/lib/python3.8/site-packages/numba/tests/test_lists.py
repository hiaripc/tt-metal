...
class TestLists(MemoryLeakMixin, TestCase):
    ...
    def test_create_nested_list(self):
        ...
        cr = compile_isolated(pyfunc, (types.int32, types.int32, types.int32, types.int32, types.int32, types.int32))
        cfunc = cr.entry_point
    ...
    def test_mul_error(self):
        ...
        if sys.platform.startswith("darwin"):
            ...
            libc.printf(
                "###Please ignore the above error message i.e. \
...
class TestUnboxing(MemoryLeakMixin, TestCase):
    ...
    def test_errors(self):
        ...
        with self.assertRaises(TypeError) as raises:
            cfunc(lst)
        msg = "can't unbox heterogeneous list: " "UniTuple({0} x 1) != UniTuple({0} x 2)"
...
class TestListOfList(ManagedListTestCase):
    ...
    def test_heterogeneous_list_error(self):
        ...
        self.assertIn(
            ("reflected list(array(float64, 1d, C)) != " "reflected list(array(complex128, 1d, C))"),
        ...
        self.assertIn(
            ("reflected list(array(float64, 1d, C)) != " "reflected list((int64 x 1))"), str(raises.exception)
        )
...
class TestLiteralLists(MemoryLeakMixin, TestCase):
    ...
    def test_literal_value_involved_passthrough(self):
        ...
        def ol_bar(x):
            ...
            outer_literal = {
                types.literal("a"): types.LiteralList(
                    [
                        types.literal(1),
                        types.literal("a"),
                        types.DictType(types.unicode_type, types.intp, initial_value={"f": 1}),
                        inner_dict,
                    ]
                ),
                types.literal("b"): types.literal(2),
                types.literal("c"): types.List(types.complex128, reflected=False),
            }
