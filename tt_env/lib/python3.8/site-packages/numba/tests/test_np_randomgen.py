...
class TestRandomGenerators(MemoryLeakMixin, TestCase):
    def check_numpy_parity(
        self, distribution_func, bitgen_type=None, seed=None, test_size=None, test_dtype=None, ulp_prec=5
    ):
        ...
        if (isinstance(numba_res, np.ndarray) and np.issubdtype(numba_res.dtype, np.floating)) or isinstance(
            numba_res, float
        ):
    ...
    def test_triangular(self):
        ...
        self._check_invalid_types(
            dist_func, ["left", "mode", "right", "size"], [0, 3, 5, (1,)], ["x", "x", "x", ("x",)]
        )
