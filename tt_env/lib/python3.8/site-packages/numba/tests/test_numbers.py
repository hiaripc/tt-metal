...
class TestViewIntFloat(TestCase):
    ...
    def test_32_bits(self):
        ...
        #        Value  Initial Type   Expected answers using dtypes
        inputs = (
            (1, np.uint32, (1, 1, 1.401298464324817e-45)),
    ...
    def test_64_bits(self):
        ...
        #        Value  Initial Type   Expected answers using dtypes
        inputs = (
            (1, np.uint64, (1, 1, 5e-324)),
            (-1, np.int64, (18446744073709551615, -1, np.nan)),
            (1.0, np.float64, (4607182418800017408, 4607182418800017408, 1.0)),
        )
        self.do_testing(inputs, dtypes)
    ...
    def test_python_scalar_exception(self):
        ...
        with self.assertRaises(TypingError) as e:
            myview()
        self.assertIn(
            "'view' can only be called on NumPy dtypes, " "try wrapping the variable 'a' with 'np.<dtype>()'",
            str(e.exception),
        )
    ...
    def do_testing_exceptions(self, pair):
        with self.assertRaises(TypingError) as e:
            ...
            view(1)
        self.assertIn(
            "Changing the dtype of a 0d array is only supported " "if the itemsize is unchanged", str(e.exception)
        )
