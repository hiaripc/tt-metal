...
class TestFromDtype(TestCase):
    ...
    def test_struct_types(self):
        ...
        check(
            dtype, fields={"a": (types.int16, 0, None, None), "b": (types.int32, 2, None, None)}, size=6, aligned=False
        )

        dtype = np.dtype([("a", np.int16), ("b", np.int32)], align=True)
        check(
            dtype, fields={"a": (types.int16, 0, None, None), "b": (types.int32, 4, None, None)}, size=8, aligned=True
        )

        dtype = np.dtype([("m", np.int32), ("n", "S5")])
        check(
            dtype,
            fields={"m": (types.int32, 0, None, None), "n": (types.CharSeq(5), 4, None, None)},
            size=9,
            aligned=False,
        )
...
class TestUFuncs(TestCase):
    ...
    def test_ufunc_find_matching_loop(self):
        ...
        check_exact(
            np_add, (types.NPTimedelta("s"), types.NPTimedelta("s")), "mm->m", output_types=(types.NPTimedelta("s"),)
        )
        check_exact(
            np_add, (types.NPTimedelta("ms"), types.NPDatetime("s")), "mM->M", output_types=(types.NPDatetime("ms"),)
        )
        check_exact(
            np_add, (types.NPDatetime("s"), types.NPTimedelta("s")), "Mm->M", output_types=(types.NPDatetime("s"),)
        )
        check_exact(
            np_add, (types.NPDatetime("s"), types.NPTimedelta("")), "Mm->M", output_types=(types.NPDatetime("s"),)
        )
        check_exact(
            np_add, (types.NPDatetime("ns"), types.NPTimedelta("")), "Mm->M", output_types=(types.NPDatetime("ns"),)
        )
        check_exact(
            np_add, (types.NPTimedelta(""), types.NPDatetime("s")), "mM->M", output_types=(types.NPDatetime("s"),)
        )
        check_exact(
            np_add, (types.NPTimedelta(""), types.NPDatetime("ns")), "mM->M", output_types=(types.NPDatetime("ns"),)
        )
