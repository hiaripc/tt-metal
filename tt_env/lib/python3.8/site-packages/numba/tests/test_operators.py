...
class TestOperators(TestCase):
        ...
        for arg_types in types_list:
            ...
            for x, y in itertools.product(x_operands, y_operands):
                ...
                self.assertPreciseEqual(
                    got,
                    expected,
                    msg="mismatch for (%r, %r) with types %s: %r != %r" % (x, y, arg_types, got, expected),
                )
                self.assertPreciseEqual(
                    x_got,
                    x_expected,
    ...
    def run_test_floats(self, pyfunc, x_operands, y_operands, types_list, flags=force_pyobj_flags):
        for arg_types in types_list:
            cr = compile_isolated(pyfunc, arg_types, flags=flags)
            cfunc = cr.entry_point
            for x, y in itertools.product(x_operands, y_operands):
    ...
    compare_types = [types.int32, types.int64, types.uint32, types.uint64, types.float32, types.float64]
    ...
    def test_truediv_errors(self, flags=force_pyobj_flags):
        self.check_div_errors("truediv_usecase", "division by zero", flags=flags, allow_complex=True)
    ...
    generate_binop_tests(
        locals(),
        ("bitwise_and", "bitwise_iand", "bitwise_or", "bitwise_ior", "bitwise_xor", "bitwise_ixor"),
        {
            "ints": "run_logical",
    ...
    def _check_in(self, pyfunc, flags):
        dtype = types.int64
        cres = compile_isolated(pyfunc, (dtype, types.UniTuple(dtype, 3)), flags=flags)
        cfunc = cres.entry_point
        for i in (3, 4, 5, 6, 42):
            tup = (3, 42, 5)
...
class TestMixedInts(TestCase):
    ...
    def run_binary(
        self, pyfunc, control_func, operands, types, expected_type=int, force_type=lambda x: x, **assertPreciseEqualArgs
    ):
        for xt, yt in types:
    ...
    def run_unary(self, pyfunc, control_func, operands, types, expected_type=int):
        for xt in types:
            ...
            for x in operands:
                ...
                self.assertIsInstance(got, expected_type)
                self.assertPreciseEqual(
                    got, expected, msg="mismatch for %r with type %s: %r != %r" % (x, xt, got, expected)
                )

    def run_arith_binop(
        self, pyfunc, opname, samples, expected_type=int, force_type=lambda x: x, **assertPreciseEqualArgs
    ):
        self.run_binary(
            pyfunc,
