...
class TestCC(BasePYCCTest):
    ...
    @unittest.skipIf(
        sys.platform == "darwin" and utils.PYVERSION == (3, 8), "distutils incorrectly using gcc on python 3.8 builds"
    )
...
class TestDistutilsSupport(TestCase):
    ...
    def check_setup_py(self, setup_py_file):
        ...
        def run_python(args):
            p = subprocess.Popen(
                [sys.executable] + args, cwd=self.usecase_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env
            )
            out, _ = p.communicate()
            rc = p.wait()
            if rc != 0:
