...
import unittest
from numba.tests.support import TestCase, enable_pyobj_flags, MemoryLeakMixin, tag, compile_function
...
class TestSetReflection(BaseTest):
    ...
    def check_reflection(self, pyfunc):
        cfunc = jit(nopython=True)(pyfunc)
        samples = [
            (set([1.0, 2.0, 3.0, 4.0]), set([0.0])),
            (set([1.0, 2.0, 3.0, 4.0]), set([5.0, 6.0, 7.0, 8.0, 9.0])),
        ]
