...
class TestSlices(MemoryLeakMixin, TestCase):
        ...
        for args in arg_cases:
            try:
                ...
                self.assertRegexpMatches(
                    str(py_type_e), r"slice expected at (most|least) (3|1) arguments?, got {}".format(n_args)
                )
    ...
    def test_slice_indices(self):
        ...
        slices = starmap(
            slice, product(chain(range(-5, 5), (None,)), chain(range(-5, 5), (None,)), chain(range(-5, 5), (None,)))
        )
        lengths = range(-2, 3)
