...
class TestStencilBase(unittest.TestCase):
    ...
    def _compile_this(self, func, sig, flags):
        return compile_extra(
            registry.cpu_target.typing_context, registry.cpu_target.target_context, func, sig, None, flags, {}
        )
    ...
    def test_stencil_standard_indexing_2d(self):
        ...
        def test_impl_seq(n):
            ...
            for i in range(1, n - 1):
                for j in range(1, n - 1):
                    C[i, j] = (
                        A[i, j + 1] * B[0, 1] + A[i + 1, j] * B[1, 0] + A[i, j - 1] * B[0, -1] + A[i - 1, j] * B[-1, 0]
                    )
            for i in range(1, n - 1):
                for j in range(1, n - 1):
                    D[i, j] = (
                        C[i, j + 1] * B[0, 1] + C[i + 1, j] * B[1, 0] + C[i, j - 1] * B[0, -1] + C[i - 1, j] * B[-1, 0]
                    )
            return D
...
class TestManyStencils(TestStencilBase):
    ...
    def check_against_expected(self, pyfunc, expected, *args, **kwargs):
        ...
        * from the njit(parallel=True) of a trivial wrapper function around
        ...
        if should_fail:
            ...
            raise RuntimeError(
                ("The following implementations should have " "raised an exception but did not:\n%s") % msg
            )
                    ...
                    for __b in range(0, a.shape[1] - 1):
                        for __a in range(0, a.shape[0]):
                            __b0[__a, __b, __c, __d] = (
                                a[__a + 0, __b + 0, __c + 0, __d + 0] - a[__a + 0, __b + 1, __c + 0, __d + -1] + 1.0
                            )
            return __b0
        ...
        def __kernel(a, neighborhood):
            ...
            for __b in range(1, a.shape[1] - 1):
                for __a in range(2, a.shape[0] - 1):
                    g = 4.0 + a[__a + 0, __b + 1]
                    __b0[__a, __b] = g + (
                        a[__a + 0, __b + 1] + a[__a + 1, __b + 0] + a[__a + 0, __b + -1] + np.sin(a[__a + -2, __b + 0])
                    )
            return __b0
    ...
    def test_basic36(self):
        ...
        def __kernel(a, neighborhood):
            ...
            for __b in range(1, a.shape[1] - 1):
                for __a in range(0, a.shape[0] - 1):
                    __b0[__a, __b] = (
                        a[__a + 0, __b + 1] + a[__a + 0, __b + -1] + a[__a + 1, __b + -1] + a[__a + 1, __b + -1]
                    )
            return __b0
    ...
    def test_basic43(self):
        ...
        def __kernel(a, b, neighborhood):
            ...
            for __b in range(1, a.shape[1] - 2):
                for __a in range(2, a.shape[0] - 1):
                    __b0[__a, __b] = (
                        a[__a + 0, __b + 1] + a[__a + 1, __b + 2] + b[__a + -2, __b + 0] + b[__a + 0, __b + -1]
                    )
            return __b0
    ...
    def test_basic51(self):
        ...
        b = np.arange(12.0).reshape(3, 4)
        self.check_exceptions(
            kernel, a, b, options={"standard_indexing": ["a", "b"]}, expected_exception=[ValueError, NumbaValueError]
        )
    ...
    def test_basic64(self):
        ...
        a = np.arange(12.0).reshape(3, 4)
        self.check_exceptions(
            kernel, a, options={"standard_indexing": "a"}, expected_exception=[ValueError, NumbaValueError]
        )
    ...
    def test_basic67b(self):
        ...
        a = np.arange(10.0 * 20.0).reshape(10, 20)
        self.check_exceptions(
            kernel, a, options={"neighborhood": ((-10, 0),)}, expected_exception=[TypingError, ValueError]
        )
