...
class BasicUFuncTest(BaseUFuncTest):
    ...
    def basic_ufunc_test(
        ...
        for input_tuple in inputs:
            ...
            for expected_i, result_i in zip(expected, results):
                msg = "\n".join(
                    ["ufunc '{0}' failed", "inputs ({1}):", "{2}", "got({3})", "{4}", "expected ({5}):", "{6}"]
                ).format(ufunc.__name__, input_type, input_operand, output_type, result_i, expected_i.dtype, expected_i)
                try:
    ...
    def signed_unsigned_cmp_test(self, comparison_ufunc):
        ...
        additional_inputs = (
            ...
            (np.uint64(1), np.int64(1)),
            (np.array([-1, -1, 0, 0, 1, 1], dtype=np.int64), np.array([0, 1, 0, 1, 0, 1], dtype=np.uint64)),
            (np.array([0, 1, 0, 1, 0, 1], dtype=np.uint64), np.array([-1, -1, 0, 0, 1, 1], dtype=np.int64)),
        )
...
class TestUFuncs(BasicUFuncTest, TestCase):
    ...
    def test_spacing_ufunc(self):
        ...
        # to NumPy
        additional = [
            (np.array([np.inf, -np.inf], dtype=np.float64), types.Array(types.float64, 1, "C")),
        ]
    ...
    def binary_ufunc_mixed_types_test(self, ufunc):
        ...
        for vals in itertools.product(inputs1, inputs2, output_types):
            ...
            if ufunc_name == "divide" and (
                input2_type == types.Array(types.uint32, 1, "C") or input2_type == types.Array(types.uint64, 1, "C")
            ):
                continue
            ...
            if (isinstance(input1_type, types.Array) or isinstance(input2_type, types.Array)) and not isinstance(
...
class TestArrayOperators(BaseUFuncTest, TestCase):
    ...
    def unary_op_test(
        self, operator, nrt=True, skip_inputs=[], additional_inputs=[], int_output_type=None, float_output_type=None
    ):
