...
class TestUnicode(BaseTest):
        ...
        for s in cpython_str + UNICODE_EXAMPLES:
            ...
            for sub_str in cpython_subs + default_subs + extra_subs:
                for start in list(range(-20, 20)) + [None]:
                    msg = 'Results "{}".endswith("{}", {}) must be equal'
                    self.assertEqual(
                        pyfunc(s, sub_str, start), cfunc(s, sub_str, start), msg=msg.format(s, sub_str, start)
                    )
    ...
    def test_count_optional_arg_type_check(self):
        ...
        def try_compile_bad_optional(*args):
            bad_sig = types.int64(
                types.unicode_type, types.unicode_type, types.Optional(types.float64), types.Optional(types.float64)
            )
            njit([bad_sig])(pyfunc)
        ...
        sig_optional = types.int64(
            types.unicode_type, types.unicode_type, types.Optional(types.int64), types.Optional(types.int64)
        )
    ...
    def test_rfind_wrong_start_end_optional(self):
        ...
        def try_compile_wrong_start_optional(*args):
            wrong_sig_optional = types.int64(
                types.unicode_type, types.unicode_type, types.Optional(types.float64), types.Optional(types.intp)
            )
        ...
        def try_compile_wrong_end_optional(*args):
            wrong_sig_optional = types.int64(
                types.unicode_type, types.unicode_type, types.Optional(types.intp), types.Optional(types.float64)
            )
            njit([wrong_sig_optional])(rfind_with_start_end_usecase)
        ...
        for pyfunc, msg in zip(pyfuncs, messages):
            ...
            for s in unicode_examples:
                ...
                cases = [
                    ...
                    (s[:-2], [0] + list(range(-10, 1 - l)), [-2, -1] + list(range(l - 2, 10))),
    ...
    def test_split(self):
        ...
        for test_str, splitter in CASES:
            self.assertEqual(
                pyfunc(test_str, splitter), cfunc(test_str, splitter), "'%s'.split('%s')?" % (test_str, splitter)
            )
    ...
    def test_strip(self):
        ...
        for pyfunc, case_name in [
            ...
            for fn in cfunc, cfunc_optional:
                for string, chars in STRIP_CASES:
                    self.assertEqual(
                        pyfunc(string, chars), fn(string, chars), "'%s'.%s('%s')?" % (string, case_name, chars)
                    )
    ...
    def test_isalpha(self):
        ...
        cpython = ["\u1FFc", "\U00010401", "\U00010427", "\U00010429", "\U0001044E", "\U0001F40D", "\U0001F46F"]
        ...
        for test_str, old_str, new_str in CASES:
            for count in count_test:
                self.assertEqual(
                    ...
                    "'%s'.replace('%s', '%s', '%s')?" % (test_str, old_str, new_str, count),
                )
