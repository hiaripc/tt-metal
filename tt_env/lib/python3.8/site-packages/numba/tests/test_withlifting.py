...
class TestLiftObj(MemoryLeak, TestCase):
    ...
    def test_objmode_gv_variable_error(self):
        ...
        with self.assertRaisesRegex(
            errors.CompilerError, ("Error handling objmode argument 'val'. " "Global 'gv_type2' is not defined\.")
        ):
            global_var()
    ...
    def test_objmode_gv_mod_attr_error(self):
        ...
        with self.assertRaisesRegex(
            ...
            ("Error handling objmode argument 'val'. " "Getattr cannot be resolved at compile-time"),
        ):
            moderror()
    ...
    def test_objmode_gv_mod_attr_error_multiple(self):
        ...
        with self.assertRaisesRegex(
            errors.CompilerError,
            ("Error handling objmode argument 'v2'. " "Getattr cannot be resolved at compile-time"),
        ):
            moderror()
    ...
    def test_objmode_closure_type_in_overload_error(self):
        ...
        with self.assertRaisesRegex(
            errors.TypingError,
            ("Error handling objmode argument 'out'. " "Freevar 'shrubbery' is not defined"),
        ):
            bar()
    ...
    def test_objmode_typed_dict(self):
        ...
        with self.assertRaises(TypeError) as raises:
            test4()
        self.assertIn(
            ("can't unbox a <class 'dict'> " "as a <class 'numba.typed.typeddict.Dict'>"),
            str(raises.exception),
        )
    ...
    def test_objmode_typed_list(self):
        ...
        self.assertRegex(
            str(raises.exception),
            (r"can't unbox a <class 'list'> " r"as a (<class ')?numba.typed.typedlist.List('>)?"),
        )
