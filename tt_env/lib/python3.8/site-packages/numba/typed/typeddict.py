...
class Dict(MutableMapping):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if args:
            ...
            _len = len(args)
            if _len > 1:
                raise errors.TypingError("Dict expect at most 1 argument, " f"got {_len}")

            # check if argument is iterable
            arg = args[0]
            if not isinstance(arg, Iterable):
                msg = (
                    f"'{type(arg)}' object is not iterable. Supported type " "constructor are Dict() and Dict(iterable)"
                )
            ...
            for idx, item in enumerate(arg):
                if len(item) != 2:
                    msg = f"dictionary update sequence element #{idx} has " f"length {len(item)}; 2 is required"
