...
def array_ufunc_errmsg_formatter(dummy, ufunc, method, *inputs, **kwargs):
    ...
    return "operand type(s) all returned NotImplemented from " "__array_ufunc__({!r}, {!r}, {}): {}".format(
...
def array_function_errmsg_formatter(public_api, types):
    ...
    return "no implementation found for '{}' on types that implement " "__array_function__: {}".format(
        func_name, list(types)
    )
...
def _ufunc_doc_signature_formatter(ufunc):
    ...
    if ufunc.nout == 0:
        ...
        out_args = "[, {positional}], / [, out={default}]".format(
            positional=", ".join("out{}".format(i + 1) for i in range(ufunc.nout)), default=repr((None,) * ufunc.nout)
        )
