...
def set_printoptions(
    ...
    *,
    ...
    opt = _make_options_dict(
        precision, threshold, edgeitems, linewidth, suppress, nanstr, infstr, sign, formatter, floatmode, legacy
    )
...
def _get_formatdict(data, *, precision, floatmode, suppress, sign, legacy, formatter, **kwargs):
    ...
    formatdict = {
        ...
        "longfloat": lambda: FloatingFormat(data, precision, floatmode, suppress, sign, legacy=legacy),
        "complexfloat": lambda: ComplexFloatingFormat(data, precision, floatmode, suppress, sign, legacy=legacy),
        "longcomplexfloat": lambda: ComplexFloatingFormat(data, precision, floatmode, suppress, sign, legacy=legacy),
...
def array2string(
    ...
    *,
    ...
    overrides = _make_options_dict(
        precision, threshold, edgeitems, max_line_width, suppress_small, None, None, sign, formatter, floatmode, legacy
    )
    ...
    def fillFormat(self, data):
        ...
        if len(abs_non_zero) != 0:
            ...
            with errstate(over="ignore"):  # division can overflow
                if max_val >= 1.0e8 or (not self.suppress_small and (min_val < 0.0001 or max_val / min_val > 1000.0)):
        ...
        if len(finite_vals) == 0:
            ...
            if self.floatmode == "fixed" or self._legacy <= 113:
                trim, unique = "k", False
            strs = (
                dragon4_scientific(x, precision=self.precision, unique=unique, trim=trim, sign=self.sign == "+")
            ...
            strs = (
                dragon4_positional(
                    x, precision=self.precision, fractional=True, unique=unique, trim=trim, sign=self.sign == "+"
                )
                for x in finite_vals
            )
...
def format_float_scientific(
    x, precision=None, unique=True, trim="k", sign=False, pad_left=None, exp_digits=None, min_digits=None
):
    """
...
def _array_repr_implementation(
    arr, max_line_width=None, precision=None, suppress_small=None, array2string=array2string
):
