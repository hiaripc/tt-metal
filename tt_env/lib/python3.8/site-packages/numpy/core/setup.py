...
def can_link_svml():
    ...
    return "x86_64" in platform and "linux" in platform and sys.maxsize > 2**31


def check_svml_submodule(svmlpath):
    if not os.path.exists(svmlpath + "/README.md"):
        raise RuntimeError("Missing `SVML` submodule! Run `git submodule " "update --init` to fix this.")
    return True
...
def check_math_capabilities(config, ext, moredefs, mathlibs):
    ...
    def check_funcs_once(
        funcs_name, headers=["feature_detection_math.h", "feature_detection_cmath.h"], add_to_moredefs=True
    ):
    ...
    def check_funcs(funcs_name, headers=["feature_detection_math.h", "feature_detection_cmath.h"]):
    ...
    if not check_funcs_once(MANDATORY_FUNCS, add_to_moredefs=False):
        raise SystemError(
            "One of the required function to build numpy is not" " available (the list is %s)." % str(MANDATORY_FUNCS)
        )
    ...
    for tup in OPTIONAL_INTRINSICS:
        ...
        if config.check_func(f, decl=False, call=True, call_args=args, headers=headers):
            moredefs.append((m, 1))
...
def check_ieee_macros(config):
    ...
    for f in _macros:
        ...
        already_declared = config.check_decl(py_symbol, headers=["Python.h", "math.h"])
        if already_declared:
            if config.check_macro_true(py_symbol, headers=["Python.h", "math.h"]):
                pub.append("NPY_%s" % fname2def("decl_%s" % f))
        else:
            macros.append(f)
...
def check_types(config_cmd, ext, build_dir):
    ...
    if not result:
        ...
        if "__pypy__" in sys.builtin_module_names:
            python = "pypy"
        raise SystemError("Cannot compile 'Python.h'. Perhaps you need to " "install {0}-dev|{0}-devel.".format(python))
    ...
    for type in ("Py_intptr_t", "off_t"):
        res = config_cmd.check_type_size(
            type, headers=["Python.h"], library_dirs=[pythonlib_dir()], expected=expected[type]
        )
    ...
    if config_cmd.check_decl("PY_LONG_LONG", headers=["Python.h"]):
        res = config_cmd.check_type_size(
            "PY_LONG_LONG", headers=["Python.h"], library_dirs=[pythonlib_dir()], expected=expected["PY_LONG_LONG"]
        )
        if res >= 0:
        ...
        res = config_cmd.check_type_size("long long", expected=expected["long long"])
        if res >= 0:
...
def check_mathlib(config_cmd):
    ...
    for libs in mathlibs_choices:
        if config_cmd.check_func("log", libraries=libs, call_args="0", decl="double log(double);", call=True):
            mathlibs = libs
            break
    else:
        raise RuntimeError("math library missing; rerun setup.py after setting the " "MATHLIB env variable")
    return mathlibs
...
def configuration(parent_package="", top_path=None):
    from numpy.distutils.misc_util import Configuration, dot_join, exec_mod_from_location
    ...
    generate_umath = exec_mod_from_location("_".join(n.split(".")), generate_umath_py)
        ...
        if newer(__file__, target):
            ...
            with open(target, "w") as target_f:
                if sys.platform == "darwin":
                    target_f.write("/* may be overridden by numpyconfig.h on darwin */\n")
                for d in moredefs:
                    if isinstance(d, str):
    ...
    def get_mathlib_info(*args):
        ...
        for lang, test_code, note in (
            ...
            (
                "c++",
                ("int main(void)" "{ auto x = 0.0; return static_cast<int>(x); }"),
            ...
            if not st:
                ...
                raise RuntimeError(f"Broken toolchain: cannot link a simple {lang.upper()} " f"program. {note}")
            if is_cpp:
    ...
    def generate_umath_doc_header(ext, build_dir):
        ...
        if newer(generate_umath_doc_py, target):
            ...
            generate_umath_doc = exec_mod_from_location("_".join(n.split(".")), generate_umath_doc_py)
    ...
    config.add_extension("_rational_tests", sources=[join("src", "umath", "_rational_tests.c")])

    #######################################################################
    #                        struct_ufunc_test module                     #
    #######################################################################

    config.add_extension("_struct_ufunc_tests", sources=[join("src", "umath", "_struct_ufunc_tests.c")])

    #######################################################################
    #                        operand_flag_tests module                    #
    #######################################################################

    config.add_extension("_operand_flag_tests", sources=[join("src", "umath", "_operand_flag_tests.c")])
