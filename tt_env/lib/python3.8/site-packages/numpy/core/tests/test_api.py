...
import pytest
from numpy.testing import assert_, assert_equal, assert_array_equal, assert_raises, assert_warns, HAS_REFCOUNT
...
@pytest.mark.parametrize(
    "a",
    (np.array(2), np.array([3, 2, 7, 0]), np.arange(6).reshape(2, 3)),  # 0D array  # 1D array  # 2D array
)
...
@pytest.mark.parametrize(
    ["dtype", "out_dtype"], [(np.bytes_, np.bool_), (np.unicode_, np.bool_), (np.dtype("S10,S9"), np.dtype("?,?"))]
)
