...
class _DeprecationTestCase:
    ...
    def assert_deprecated(
        self, function, num=1, ignore_others=False, function_fails=False, exceptions=np._NoValue, args=(), kwargs={}
    ):
...
class FlatteningConcatenateUnsafeCast(_DeprecationTestCase):
    ...
    def test_deprecated(self):
        self.assert_deprecated(
            np.concatenate, args=(([0.0], [1.0]),), kwargs=dict(axis=None, out=np.empty(2, dtype=np.int64))
        )
    ...
    def test_not_deprecated(self):
        self.assert_not_deprecated(
            np.concatenate,
            args=(([0.0], [1.0]),),
            kwargs={"axis": None, "out": np.empty(2, dtype=np.int64), "casting": "unsafe"},
        )
...
class TestFutureWarningArrayLikeNotIterable(_DeprecationTestCase):
    ...
    def test_strange_dtype_handling(self, protocol):
        ...
        MyArr = type("MyArr", (), {protocol: getattr(blueprint, protocol), "__float__": lambda _: 0.5})
        myarr = MyArr()
...
class TestNoseDecoratorsDeprecated(_DeprecationTestCase):
    ...
    def test_parametrize(self):
        def _test_parametrize():
            ...
            # results.
            @np.testing.dec.parametrize("base, power, expected", [(1, 1, 1), (2, 1, 2), (2, 2, 4)])
