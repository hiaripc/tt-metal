...
class TestBuiltin:
    ...
    def test_bad_param(self):
        ...
        assert_raises(
            ValueError, np.dtype, {"names": ["f0", "f1"], "formats": ["i4", "i1"], "offsets": [0, 4], "itemsize": 4}
        )
        # If alignment is enabled, the alignment (4) must divide the itemsize
        assert_raises(
            ValueError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": ["i4", "i1"], "offsets": [0, 4], "itemsize": 9},
            align=True,
        )
        # If alignment is enabled, the individual fields must be aligned
        assert_raises(
            ValueError, np.dtype, {"names": ["f0", "f1"], "formats": ["i1", "f4"], "offsets": [0, 2]}, align=True
        )
...
class TestRecord:
    ...
    def test_aligned_size(self):
        ...
        dt3 = np.dtype(
            {"f0": ("i4", 0), "f1": ([("f1", "i1"), ("f2", "i4"), ("f3", "i1")], 4), "f2": ("i1", 16)}, align=True
        )
        ...
        dt3 = np.dtype(
            {"f0": ("i4", 0), "f1": ([("f1", "i1"), ("f2", "i4"), ("f3", "i1")], 4), "f2": ("i1", 10)}, align=False
        )
    ...
    def test_union_struct(self):
        ...
        dt2 = np.dtype(
            {"names": ["f2", "f0", "f1"], "formats": ["<u4", "<u2", "<u2"], "offsets": [4, 0, 2]}, align=True
        )
...
class TestString:
    def test_complex_dtype_str(self):
        dt = np.dtype(
            [
                ("top", [("tiles", (">f4", (64, 64)), (1,)), ("rtile", ">f4", (64, 36))], (3,)),
        ...
        assert_equal(
            str(dt), "[(('Red pixel', 'r'), 'u1'), " "(('Green pixel', 'g'), 'u1'), " "(('Blue pixel', 'b'), 'u1')]"
        )
...
class TestDTypeMakeCanonical:
    ...
    @hypothesis.given(
        dtype=hypothesis.extra.numpy.array_dtypes(
            subtype_strategy=hypothesis.extra.numpy.array_dtypes(), min_size=5, max_size=10, allow_subarrays=True
        )
    )
...
class TestPickling:
    ...
    @pytest.mark.parametrize("t", [int, float, complex, np.int32, str, object, np.compat.unicode, bool])
    ...
    def test_structured_padded(self):
        dt = np.dtype({"names": ["A", "B"], "formats": ["f4", "f4"], "offsets": [0, 8], "itemsize": 16})
        self.check_pickling(dt)
...
class TestPromotion:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.longdouble(np.nextafter(1.7e308, 0.0)), np.complex128, np.clongdouble),
            # Additionally use "nextafter" so the cast can't round down:
            (np.longdouble(np.nextafter(1.7e308, np.inf)), np.clongdouble, None),
            # repeat for complex scalars:
            (np.complex64(2), np.complex64, None),
            (np.clongdouble(2), np.complex64, np.clongdouble),
            # Base of the double value to sidestep any rounding issues:
            (np.clongdouble(np.nextafter(1.7e308, 0.0) * 1j), np.complex128, np.clongdouble),
            # Additionally use "nextafter" so the cast can't round down:
            (np.clongdouble(np.nextafter(1.7e308, np.inf)), np.clongdouble, None),
        ],
    )
