...
class TestEinsum:
    def test_einsum_errors(self):
        for do_opt in [True, False]:
            ...
            assert_raises(
                ValueError, np.einsum, "i->i", [[0, 1], [0, 1]], out=np.arange(4).reshape(2, 2), optimize=do_opt
            )
    ...
    def check_einsum_sums(self, dtype, do_opt=False):
        ...
        for n in range(1, 17):
            ...
            assert_equal(np.einsum(a, np_array, optimize=do_opt), np.trace(a).astype(dtype))
            assert_equal(np.einsum(a, list(np_array), optimize=do_opt), np.trace(a).astype(dtype))
        ...
        assert_equal(
            np.einsum("i,i,i->i", a, b, c, dtype="?", casting="unsafe", optimize=do_opt),
        ...
        assert_array_equal(np.einsum("ji,i->", x, y, optimize=optimize), [2.0])  # contig_contig_outstride0_two
        assert_array_equal(np.einsum("i,ij->", y, x, optimize=optimize), [2.0])  # stride0_contig_outstride0_two
        assert_array_equal(np.einsum("ij,i->", x, y, optimize=optimize), [2.0])  # contig_stride0_outstride0_two
    ...
    def test_einsum_misc(self):
        ...
        assert_equal(
            np.einsum("ijklm,ijn,ijn->", a, b, b, optimize=True), np.einsum("ijklm,ijn->", a, b, optimize=True)
        )
        ...
        assert_equal(
            ...
            [[[1, 3], [3, 9], [5, 15], [7, 21]], [[8, 16], [16, 32], [24, 48], [32, 64]]],
        )
        assert_equal(
    ...
    def test_einsum_broadcast(self):
        ...
        for opt in [True, False]:
            assert_equal(np.einsum("...lmn,lmno->...o", A, B, optimize=opt), ref)  # used to raise error
    ...
    @pytest.mark.parametrize("dtype", np.typecodes["AllFloat"] + np.typecodes["AllInteger"])
