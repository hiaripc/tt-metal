...
class TestIndexing:
    ...
    def test_subclass_writeable(self):
        d = np.rec.array(
            [("NGC1001", 11), ("NGC1002", 1.0), ("NGC1003", 1.0)], dtype=[("target", "S20"), ("V_mag", ">f4")]
        )
    ...
    def _get_multi_index(self, arr, indices):
        ...
        for ax, indx in enumerate(in_indices):
            ...
            if isinstance(indx, np.ndarray) and indx.dtype == bool:
                ...
                if indx.ndim != 0:
                    arr = arr.reshape(
                        (arr.shape[:ax] + (np.prod(arr.shape[ax : ax + indx.ndim]),) + arr.shape[ax + indx.ndim :])
                    )
                    indx = flat_indx
                else:
        ...
        for indx in indices:
            if indx[0] == "f":
                ...
                arr = arr.reshape(
                    (arr.shape[:ax] + (np.prod(orig_slice).astype(int),) + arr.shape[ax + len(indx[1:]) :])
                )
