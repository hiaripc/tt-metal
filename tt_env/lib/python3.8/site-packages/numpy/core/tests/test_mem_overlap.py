...
def iter_random_view_pairs(x, same_steps=True, equal_size=False):
    ...
    while True:
        steps = tuple(
            rng.randint(1, 11, dtype=np.intp) if rng.randint(0, 5, dtype=np.intp) == 0 else 1 for j in range(x.ndim)
        )
...
def test_may_share_memory_harder_fuzz():
    ...
    check_may_share_memory_easy_fuzz(
        get_max_work=lambda a, b: max(a.size, b.size) // 2, same_steps=False, min_count=2000
    )
