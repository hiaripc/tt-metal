...
class TestNonarrayArgs:
    ...
    @pytest.mark.parametrize(
        "val, ndigits",
        [
            pytest.param(2**31 - 1, -1, marks=pytest.mark.xfail(reason="Out of range of int32")),
...
class TestTypes:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [np.dtype([("name1", "i8")]), np.dtype([("name1", np.dtype("i8").newbyteorder())])],
    ...
    @pytest.mark.parametrize(
        ["dtype1", "dtype2"],
        itertools.product(list(np.typecodes["All"]) + ["i,i", "S3", "S100", "U3", "U100", rational], repeat=2),
    )
...
class TestNonzero:
    ...
    def test_nonzero_twodim(self):
        ...
        x = np.array(
            [[(0, 1), (0, 0), (1, 11)], [(1, 1), (1, 0), (0, 0)], [(0, 0), (1, 5), (0, 1)]],
            dtype=[("a", "f4"), ("b", "u1")],
        )
...
class TestArrayComparisons:
    def test_array_equal(self):
        ...
        assert_(res)
        assert_(type(res) is bool)
        res = np.array_equal(np.array([("a", 1)], dtype="S1,u4"), np.array([("a", 1)], dtype="S1,u4"))
        assert_(res)
        assert_(type(res) is bool)
...
class TestClip:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.zeros(10, dtype=np.int64), 0, -(2**64) + 1, np.full(10, -(2**64) + 1, dtype=object)),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.zeros(10, dtype=np.int64), np.array(np.nan), np.zeros(10, dtype=np.int32)),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "arr, amin, amax",
        [
            (np.array([1] * 10, dtype="m8"), np.timedelta64("NaT"), np.zeros(10, dtype=np.int32)),
        ],
    )
    @pytest.mark.filterwarnings("ignore::DeprecationWarning")
    ...
    @given(
        data=st.data(), arr=hynp.arrays(dtype=hynp.integer_dtypes() | hynp.floating_dtypes(), shape=hynp.array_shapes())
    )
...
class TestCreationFuncs:
    ...
    def setup_method(self):
        ...
        self.dtypes = sorted(
            ...
            | {np.dtype(tp.str.replace("0", str(i))) for tp in variable_sized for i in range(1, 10)},
    ...
    def check_like_function(self, like_function, value, fill_value=False):
        ...
        for d, dtype in self.data:
            ...
            for s in self.shapes:
                ...
                if d.ndim != len(s):
                    assert_equal(
                        np.argsort(like_function(d, dtype=dtype, shape=s, order="K", **fill_kwarg).strides),
...
class TestCorrelate:
    def _setup(self, dt):
        ...
        self.zs = np.array([-3.0, -14.0, -30.0, -48.0, -66.0, -84.0, -102.0, -54.0, -19.0], dtype=dt)
...
class TestBroadcast:
    ...
    def test_shape_mismatch_error_message(self):
        with pytest.raises(ValueError, match=r"arg 0 with shape \(1, 3\) and " r"arg 2 with shape \(2,\)"):
