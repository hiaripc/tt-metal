...
def test_float_types(tp):
    ...
    if tp(1e16).itemsize > 4:
        assert_equal(str(tp(1e16)), str(float("1e16")), err_msg="Failed str formatting for type %s" % tp)
    else:
        ref = "1e+16"
        assert_equal(str(tp(1e16)), ref, err_msg="Failed str formatting for type %s" % tp)
...
def test_complex_types(tp):
    ...
    if tp(1e16).itemsize > 8:
        assert_equal(str(tp(1e16)), str(complex(1e16)), err_msg="Failed str formatting for type %s" % tp)
    else:
        ref = "(1e+16+0j)"
        assert_equal(str(tp(1e16)), ref, err_msg="Failed str formatting for type %s" % tp)
...
def _test_redirected_print(x, tp, ref=None):
    ...
    assert_equal(file.getvalue(), file_tp.getvalue(), err_msg="print failed for type%s" % tp)
