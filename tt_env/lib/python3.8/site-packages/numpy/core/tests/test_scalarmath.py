        ...
        for k, atype in enumerate(types):
            ...
            for l, btype in enumerate(types):
                ...
                assert_equal(
                    c_scalar.dtype,
                    c_array.dtype,
                    "error with types (%d/'%c' + %d/'%c')" % (k, np.dtype(atype).char, l, np.dtype(btype).char),
                )
    ...
    def test_mixed_types(self):
        ...
        for t1 in typelist:
            for t2 in typelist:
                ...
                b = t2(2)
                result = a**b
                msg = ("error with %r and %r:" "got %r, expected %r") % (t1, t2, result, 9)
    ...
    def test_numpy_scalar_relational_operators(self):
        ...
        for dt1 in np.typecodes["AllInteger"]:
            ...
            for dt2 in np.typecodes["AllInteger"]:
                ...
                assert_(
                    not np.array(1, dtype=dt1)[()] < np.array(0, dtype=dt2)[()], "type %s and %s failed" % (dt1, dt2)
                )
        ...
        for dt1 in "BHILQP":
            ...
            for dt2 in "bhilqp":
                ...
                assert_(
                    not np.array(1, dtype=dt1)[()] < np.array(-1, dtype=dt2)[()], "type %s and %s failed" % (dt1, dt2)
                )
                assert_(np.array(1, dtype=dt1)[()] != np.array(-1, dtype=dt2)[()], "type %s and %s failed" % (dt1, dt2))
        ...
        for dt1 in "bhlqp" + np.typecodes["Float"]:
            ...
            for dt2 in "bhlqp" + np.typecodes["Float"]:
                ...
                assert_(
                    np.array(-1, dtype=dt1)[()] == np.array(-1, dtype=dt2)[()], "type %s and %s failed" % (dt1, dt2)
                )
...
@pytest.mark.parametrize(
    "operation",
    [lambda min, max: max + max, lambda min, max: min - max, lambda min, max: max * max],
    ids=["+", "-", "*"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(lambda min, neg_1: min // neg_1, marks=pytest.mark.skip(reason="broken on some platforms")),
    ],
    ids=["neg", "abs", "*", "//"],
)
