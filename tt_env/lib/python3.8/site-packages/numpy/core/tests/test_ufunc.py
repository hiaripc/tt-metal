...
class TestUfunc:
    ...
    def test_signature5(self):
        ...
        assert_equal(ixs, (0, 1, 1, 2, 0, 2))
        assert_equal(
            flags, (self.size_inferred | self.can_ignore, self.size_inferred, self.size_inferred | self.can_ignore)
        )
        assert_equal(sizes, (-1, -1, -1))
    ...
    @pytest.mark.parametrize("get_kwarg", [lambda dt: dict(dtype=x), lambda dt: dict(signature=(x, None, None))])
    ...
    @pytest.mark.parametrize(
        "get_kwarg",
        [param(lambda x: dict(dtype=x), id="dtype"), param(lambda x: dict(signature=(x, None, None)), id="signature")],
    )
    def test_signature_dtype_instances_allowed(self, get_kwarg):
    ...
    def test_keepdims_argument(self):
        ...
        b = np.arange(10, 16).reshape((2, 3))
        w = np.arange(20, 26).reshape((2, 3))
        assert_array_equal(umt.innerwt(a, b, w, keepdims=True), np.sum(a * b * w, axis=-1, keepdims=True))
        assert_array_equal(umt.innerwt(a, b, w, axis=0, keepdims=True), np.sum(a * b * w, axis=0, keepdims=True))
        # Check errors.
        # Not a boolean
                    ...
                    for s2 in slice_3:
                        ...
                        if a1.shape[-1] == a2.shape[-2] and broadcastable(a1.shape[0], a2.shape[0]):
                            assert_array_almost_equal(
                                ...
                                np.sum(a2[..., np.newaxis].swapaxes(-3, -1) * a1[..., np.newaxis, :], axis=-1),
    ...
    @pytest.mark.parametrize(
        ("axis", "where"), ((0, np.array([True, False, True])), (1, [True, True, False]), (None, True))
    )
    ...
    def test_custom_ufunc(self):
        a = np.array(
            [_rational_tests.rational(1, 2), _rational_tests.rational(1, 3), _rational_tests.rational(1, 4)],
        ...
        expected = np.array(
            [_rational_tests.rational(1), _rational_tests.rational(2, 3), _rational_tests.rational(1, 2)],
            dtype=_rational_tests.rational,
        )
    ...
    def test_custom_ufunc_forced_sig(self):
        ...
        with assert_raises(TypeError):
            np.multiply(_rational_tests.rational(1), 1, signature=(_rational_tests.rational, int, None))
    ...
    @pytest.mark.parametrize("ufunc", [np.logical_and, np.logical_or])  # logical_xor object loop is bad
