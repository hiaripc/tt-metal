...
UFUNCS = [obj for obj in np.core.umath.__dict__.values() if isinstance(obj, np.ufunc)]
...
UFUNCS_BINARY_ACC = [uf for uf in UFUNCS_BINARY if hasattr(uf, "accumulate") and uf.nout == 1]
...
class TestRemainder:
    ...
    @pytest.mark.xfail(
        ...
        reason="MacOS seems to not give the correct 'invalid' warning for " "`fmod`.  Hopefully, others always do.",
    )
    @pytest.mark.parametrize("dtype", np.typecodes["Float"])
...
class TestDivisionIntegerOverflowsAndDivideByZero:
    ...
    overflow_results = {
        ...
        operator.floordiv: result_type(helper_lambdas["min"], helper_lambdas["neg_min"]),
        np.floor_divide: result_type(helper_lambdas["min"], helper_lambdas["neg_min"]),
        np.divmod: result_type(helper_lambdas["min-zero"], helper_lambdas["neg_min-zero"]),
    }
    ...
    @pytest.mark.parametrize(
        "operation", [np.remainder, np.fmod, np.divmod, np.floor_divide, operator.mod, operator.floordiv]
    )
    ...
    def test_overflows(self, dividend_dtype, divisor_dtype, operation):
        ...
        if np.dtype(dividend_dtype).itemsize >= np.dtype(divisor_dtype).itemsize and operation in (
            np.divmod,
            np.floor_divide,
            operator.floordiv,
        ):
            ...
            for a in arrays:
                ...
                with pytest.warns(RuntimeWarning, match="overflow encountered in"):
                    ...
                    expected_array = np.array(
                        [self.overflow_results[operation].nocast(dividend_dtype)] * len(a)
                    ).flatten()
...
class TestSpecialFloats:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            # ninf
            ([0.5, 0.5, 0.5, -np.inf], NEG_INVALID_ERR + INF_INVALID_ERR + LTONE_INVALID_ERR),
            ([0.5, 0.5, 0.5, -np.inf] * 32, NEG_INVALID_ERR + INF_INVALID_ERR + LTONE_INVALID_ERR),
...
class TestFPClass:
    ...
    def test_fpclass(self, stride):
        arr_f64 = np.array(
            [np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0, -0.0, 0.0, 2.2251e-308, -2.2251e-308], dtype="d"
        )
        arr_f32 = np.array(
            [np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0, -0.0, 0.0, 1.4013e-045, -1.4013e-045], dtype="f"
        )
...
class TestFRExp:
    ...
    @pytest.mark.parametrize("dtype", ["f", "d"])
    @pytest.mark.skipif(
        not sys.platform.startswith("linux"), reason="np.frexp gives different answers for NAN/INF on windows and linux"
    )
...
class TestBool:
    ...
    def test_truth_table_logical(self):
        ...
        typecodes = np.typecodes["AllFloat"] + np.typecodes["AllInteger"] + "?"  # boolean
...
class TestSpecialMethods:
    ...
    def test_ufunc_override_methods(self):
        ...
        res = np.multiply.reduce(
            a, axis="axis0", dtype="dtype0", out="out0", keepdims="keep0", initial="init0", where="where0"
        )
        assert_equal(res[0], a)
        assert_equal(res[1], np.multiply)
        assert_equal(res[2], "reduce")
...
class TestComplexFunctions:
    ...
    def test_loss_of_precision(self, dtype):
        ...
        for func in (np.arcsinh, np.arcsinh, np.arcsin, np.arctanh, np.arctan):
            pts = [rp + 1j * ip for rp in (-1e-3, 0, 1e-3) for ip in (-1e-3, 0, 1e-3) if rp != 0 or ip != 0]
            check(func, pts, 1)
            check(func, pts, 1j)
            check(func, pts, 1 + 1j)
...
@pytest.mark.parametrize(
    "ufunc, dtype",
    [(ufunc, t[0]) for ufunc in UFUNCS_BINARY_ACC for t in ufunc.types if t[-1] == "?" and t[0] not in "DFGMmO"],
)
