...
class _Config:
    ...
    conf_features = dict(
        ...
        AVX512F=dict(interest=20, implies="FMA3 AVX2", implies_detect=False, extra_checks="AVX512F_REDUCE"),
        AVX512CD=dict(interest=21, implies="AVX512F"),
        AVX512_KNL=dict(
            interest=40, implies="AVX512CD", group="AVX512ER AVX512PF", detect="AVX512_KNL", implies_detect=False
        ),
        AVX512_KNM=dict(
            interest=41,
            implies="AVX512_KNL",
        ...
        AVX512_CNL=dict(
            interest=44, implies="AVX512_SKX", group="AVX512IFMA AVX512VBMI", detect="AVX512_CNL", implies_detect=False
        ),
        AVX512_ICL=dict(
            interest=45,
        ...
        ## Power10/ISA 3.1
        VSX4=dict(interest=4, implies="VSX3", implies_detect=False, extra_checks="VSX4_MMA"),
        # IBM/Z
        ## VX(z13) support
    ...
    def conf_features_partial(self):
        ...
        if on_power:
            partial = dict(
                ...
                VSX4=dict(flags="-mcpu=power10 -mtune=power10", implies_detect=False),
            )
            if self.cc_is_clang:
        ...
        if on_zarch:
            partial = dict(
                VX=dict(flags="-march=arch11 -mzvector"),
                VXE=dict(flags="-march=arch12", implies_detect=False),
                VXE2=dict(flags="-march=arch13", implies_detect=False),
            )
        ...
        try:
            ...
            if o and re.match(_Distutils._dist_warn_regex, o):
                _Distutils.dist_error(
                    "Flags in command", cmd, "aren't supported by the compiler" ", output -> \n%s" % o
                )
...
class _CCompiler:
    ...
    def __init__(self):
        ...
        detect_arch = (
            ...
            (
                "cc_on_ppc64le",
                ".*(powerpc|ppc)64(el|le).*|.*powerpc.*",
                "defined(__powerpc64__) && " "defined(__LITTLE_ENDIAN__)",
            ),
            ("cc_on_ppc64", ".*(powerpc|ppc).*|.*powerpc.*", "defined(__powerpc64__) && " "defined(__BIG_ENDIAN__)"),
            ("cc_on_aarch64", ".*(aarch64|arm64).*", ""),
            ("cc_on_armhf", ".*arm.*", "defined(__ARM_ARCH_7__) || " "defined(__ARM_ARCH_7A__)"),
            ("cc_on_s390x", ".*s390x.*", ""),
            # undefined platform
            ("cc_on_noarch", "", ""),
        )
...
class _Parse:
    ...
    def __init__(self, cpu_baseline, cpu_dispatch):
        self._parse_policies = dict(
            ...
            KEEP_SORT=(self._parse_policy_keepsort, self._parse_policy_not_keepsort, []),
    ...
    def _parse_arg_features(self, arg_name, req_features):
        ...
        for tok in tokens:
            if tok[0] in ("#", "$"):
                self.dist_fatal(
                    arg_name,
                    "target groups and policies " "aren't allowed from arguments, " "only from dispatch-able sources",
                )
            if tok == "+":
                append = True
                continue
    ...
    def _parse_token_policy(self, token):
        ...
        if token not in self._parse_policies:
            self.dist_fatal(
                "'%s' is an invalid policy name, available policies are" % token, self._parse_policies.keys()
            )
        return token
    ...
    def _parse_policy_maxopt(self, has_baseline, final_targets, extra_flags):
        ...
        if self.cc_has_debug:
            ...
            if not flags:
                self.dist_log(
                    "current compiler doesn't support optimization flags, " "policy 'maxopt' is skipped", stderr=True
                )
            else:
                extra_flags += flags
        return has_baseline, final_targets, extra_flags
    ...
    def _parse_policy_werror(self, has_baseline, final_targets, extra_flags):
        ...
        if not flags:
            self.dist_log(
                "current compiler doesn't support werror flags, " "warnings will 'not' treated as errors", stderr=True
            )
        else:
...
class CCompilerOpt(_Config, _Distutils, _Cache, _CCompiler, _Feature, _Parse):
    ...
    def generate_dispatch_header(self, header_path):
        ...
        with open(header_path, "w") as f:
            baseline_calls = " \\\n".join(
                [
                    ("\t%sWITH_CPU_EXPAND_(MACRO_TO_CALL(%s, __VA_ARGS__))") % (self.conf_c_prefix, f)
    ...
    def _generate_config(self, output_dir, dispatch_src, targets, has_baseline=False):
        ...
        for tar in targets:
            ...
            dispatch_calls.append(
                "\t%sCPU_DISPATCH_EXPAND_(CB((%s), %s, __VA_ARGS__))" % (self.conf_c_prefix_, req_detect, target_name)
            )
        dispatch_calls = " \\\n".join(dispatch_calls)
