        ...
        try:
            ...
            if (
                ...
                and (
                    ...
                    or (curses.tigetstr("setaf") is not None and curses.tigetstr("setab") is not None)
        ...
        try:
            ...
            if not hasattr(setup_module, "configuration"):
                ...
                config = Configuration(
                    subpackage_name, parent_name, self.top_path, subpackage_path, caller_level=caller_level + 1
                )
            else:
            ...
            if config.name != dot_join(parent_name, subpackage_name):
                self.warn(
                    "Subpackage %r configuration returned as %r" % (dot_join(parent_name, subpackage_name), config.name)
                )
        finally:
            del sys.path[0]
        return config
        ...
        if not os.path.isfile(setup_py):
            if not self.options["assume_default_configuration"]:
                self.warn(
                    ...
                    "(%s/{setup_%s,setup}.py was not found)" % (os.path.dirname(setup_py), subpackage_name)
                )
            config = Configuration(
                subpackage_name, parent_name, self.top_path, subpackage_path, caller_level=caller_level + 1
            )
            ...
            config = self._get_configuration_from_setup_py(
                setup_py, subpackage_name, subpackage_path, parent_name, caller_level=caller_level + 1
            )
        if config:
            return [config]
        else:
