...
class system_info:
    ...
    def get_option_single(self, *options):
        ...
        *options : list of str
        ...
        raise AliasedOptionError(
            AliasedOptionError.__doc__.format(section=self.section, options="[{}]".format(", ".join(options)))
        )
...
class fftw_info(system_info):
    ...
    ver_info = [
        {"name": "fftw3", "libs": ["fftw3"], "includes": ["fftw3.h"], "macros": [("SCIPY_FFTW3_H", None)]},
        {
            "name": "fftw2",
            "libs": ["rfftw", "fftw"],
...
class fftw3_armpl_info(fftw_info):
    ...
    notfounderror = FFTWNotFoundError
    ver_info = [
        {"name": "fftw3", "libs": ["armpl_lp64_mp"], "includes": ["fftw3.h"], "macros": [("SCIPY_FFTW3_H", None)]}
    ]
...
class lapack_opt_info(system_info):
    ...
    lapack_order = ["armpl", "mkl", "openblas", "flame", "accelerate", "atlas", "lapack"]
    ...
    def calc_info(self):
        ...
        if len(unknown_order) > 0:
            raise ValueError(
                "lapack_opt_info user defined " "LAPACK order has unacceptable " "values: {}".format(unknown_order)
            )
...
class blas_opt_info(system_info):
    ...
    blas_order = ["armpl", "mkl", "blis", "openblas", "accelerate", "atlas", "blas"]
...
class openblas_info(blas_info):
    ...
    def check_symbols(self, info):
        ...
        calls = "\n".join(
            "%s%s%s();" % (self.symbol_prefix, symbol_name, self.symbol_suffix) for symbol_name in self._require_symbols
        )
        s = (
            textwrap.dedent(
                """\
        ...
        try:
            ...
            try:
                c.link_executable(
                    obj, out, libraries=info["libraries"], library_dirs=info["library_dirs"], extra_postargs=extra_args
                )
                res = True
            except distutils.ccompiler.LinkError:
                res = False
...
class boost_python_info(system_info):
    ...
    def calc_info(self):
        ...
        bpl_srcs += glob(os.path.join(srcs_dir, "*", "*.cpp"))
        info = {
            "libraries": [("boost_python_src", {"include_dirs": [src_dir] + py_incl_dirs, "sources": bpl_srcs})],
...
class umfpack_info(system_info):
    ...
    def calc_info(self):
        ...
        if inc_dir is not None:
            dict_append(
                info, include_dirs=[inc_dir], define_macros=[("SCIPY_UMFPACK_H", None)], swig_opts=["-I" + inc_dir]
            )
...
def parseCmdLine(argv=(None,)):
    ...
    parser.add_option(
        "-v", "--verbose", action="store_true", dest="verbose", default=False, help="be verbose and print more messages"
    )
