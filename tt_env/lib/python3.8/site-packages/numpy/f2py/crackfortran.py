...
##
fortrantypes = (
    r"character|logical|integer|real|complex|double\s*(precision\s*(complex|)|complex)|type(?=\s*\([\w\s,=(*)]*\))|byte"
)
typespattern = re.compile(beforethisafter % ("", fortrantypes, fortrantypes, ".*"), re.I), "type"
typespattern4implicit = re.compile(
    beforethisafter
    % ("", fortrantypes + "|static|automatic|undefined", fortrantypes + "|static|automatic|undefined", ".*"),
    re.I,
)
#
functionpattern = (
    re.compile(beforethisafter % (r"([a-z]+[\w\s(=*+-/)]*?|)", "function", "function", ".*"), re.I),
    "begin",
)
...
groupbegins90 = groupbegins77 + r"|module(?!\s*procedure)|python\s*module|(abstract|)\s*interface|" + r"type(?!\s*\()"
...
endpattern = re.compile(beforethisafter % ("", groupends, groupends, r".*"), re.I), "end"
endifs = r"end\s*(if|do|where|select|while|forall|associate|block|" + r"critical|enum|team)"
...
moduleprocedurepattern = (
    re.compile(beforethisafter % ("", moduleprocedures, moduleprocedures, r".*"), re.I),
    "moduleprocedure",
)
...
intentpattern = (
    re.compile(beforethisafter % ("", "intent|depend|note|check", "intent|depend|note|check", r"\s*\(.*?\).*"), re.I),
    "intent",
)
    ...
    if not m:
        ...
        if (m1) and (not m1.group("this") == groupname[groupcounter]):
            raise Exception(
                ...
                % (repr(m1.group("this")), repr(groupname[groupcounter]), filepositiontext)
            )
        if skipblocksuntil == groupcounter:
            skipblocksuntil = -1
        grouplist[groupcounter - 1].append(groupcache[groupcounter])
...
typedefpattern = re.compile(
    r"(?:,(?P<attributes>[\w(),]+))?(::)?(?P<name>\b[a-z$_][\w$]*\b)" r"(?:\((?P<params>[\w,]*)\))?\Z", re.I
)
nameargspattern = re.compile(
    r"\s*(?P<name>\b[\w$]+\b)\s*(@\(@\s*(?P<args>[\w\s,]*)\s*@\)@|)\s*((result(\s*@\(@\s*(?P<result>\b[\w$]+\b)\s*@\)@|))|(bind\s*@\(@\s*(?P<bind>.*)\s*@\)@))*\s*\Z",
    re.I,
)
operatorpattern = re.compile(r"\s*(?P<scheme>(operator|assignment))" r"@\(@\s*(?P<name>[^)]+)\s*@\)@\s*\Z", re.I)
    ...
    for n in param_names:
        if "=" in vars[n]:
            ...
            if isdouble(vars[n]):
                ...
                outmess(f"get_parameters[TODO]: " f"implement evaluation of complex expression {v}\n")
    ...
    if isinstance(obj, list):
        ...
        for index, value in enumerate(obj):
            new_index, new_item = traverse(
                (index, value), visit, parents=parents + [parent], result=result, *args, **kwargs
            )
        ...
        for key, value in obj.items():
            new_key, new_value = traverse(
                (key, value), visit, parents=parents + [parent], result=result, *args, **kwargs
            )
...
def character_backward_compatibility_hook(item, parents, result, *args, **kwargs):
    ...
    if vars_dict is not None:
        ...
        for varname, vd in vars_dict.items():
            if ischaracter(vd):
                ...
                new_value = re.sub(r"(?<![&])\b" + varname + r"\b", "&" + varname, new_value)
    ...
    if new_value is not None:
        if new_value != value:
            ...
            outmess(f"character_bc_hook[{parent_key}.{key}]:" f" replaced `{value}` -> `{new_value}`\n", 1)
