...
class Expr:
    ...
    def linear_solve(self, symbol):
        ...
        if zero != as_number(0):
            raise RuntimeError(f"not a {symbol}-linear equation:" f" {a} * {symbol} + {b} == {self}")
        return a, b
...
def eliminate_quotes(s):
    ...
    new_s = re.sub(
        r"({kind}_|)({single_quoted}|{double_quoted})".format(
            kind=r"\w[\w\d_]*", single_quoted=r"('([^'\\]|(\\.))*')", double_quoted=r'("([^"\\]|(\\.))*")'
        ),
        repl,
        s,
    )
...
def replace_parenthesis(s):
    ...
    mn_i = len(s)
    for left_, right_ in (("(/", "/)"), "()", "{}", "[]"):  # to support C literal structs
        i = s.find(left_)
        if i == -1:
            continue
        if i < mn_i:
...
class _FromStringWorker:
    ...
    def process(self, s, context="expr"):
        ...
        operands = re.split(
            r"(?<=[@\w\d_])\s*([*]|/)", (r if self.language is Language.C else r.replace("**", "@__f2py_DOUBLE_STAR@"))
        )
        if len(operands) > 1:
        ...
        m = re.match(r"\A({digit_string})({kind}|)\Z".format(digit_string=r"\d+", kind=r"_(\d+|\w[\w\d_]*)"), r)
        if m:
        ...
        m = re.match(
            r"\A({significant}({exponent}|)|\d+{exponent})({kind}|)\Z".format(
                significant=r"[.]\d+|\d+[.]\d*", exponent=r"[edED][+-]?\d+", kind=r"_(\d+|\w[\w\d_]*)"
            ),
            r,
        )
        if m:
