...
def _wrap_header_guess_version(header):
    ...
    try:
        ...
        warnings.warn("Stored array in format 2.0. It can only be" "read by NumPy >= 1.9", UserWarning, stacklevel=2)
        return ret
    ...
    if array.dtype.hasobject:
        ...
        if _isfileobj(fp):
            ...
            for chunk in numpy.nditer(
                array, flags=["external_loop", "buffered", "zerosize_ok"], buffersize=buffersize, order="F"
            ):
        ...
        if _isfileobj(fp):
            ...
            for chunk in numpy.nditer(
                array, flags=["external_loop", "buffered", "zerosize_ok"], buffersize=buffersize, order="C"
            ):
                fp.write(chunk.tobytes("C"))
...
def open_memmap(
    filename, mode="r+", dtype=None, shape=None, fortran_order=False, version=None, *, max_header_size=_MAX_HEADER_SIZE
):
    """
