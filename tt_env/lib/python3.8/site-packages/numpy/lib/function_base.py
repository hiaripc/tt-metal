...
def _create_arrays(broadcast_shape, dim_sizes, list_of_core_dims, dtypes, results=None):
    ...
    if results is None:
        ...
        arrays = tuple(
            np.empty_like(result, shape=shape, dtype=dtype) for result, shape, dtype in zip(results, shapes, dtypes)
        )
    return arrays
...
class vectorize:
    ...
    def _vectorize_call_with_signature(self, func, args):
        ...
        if len(args) != len(input_core_dims):
            raise TypeError(
                "wrong number of positional arguments: " "expected %r, got %r" % (len(input_core_dims), len(args))
            )
...
def corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *, dtype=None):
    ...
    if bias is not np._NoValue or ddof is not np._NoValue:
        # 2015-03-15, 1.10
        warnings.warn("bias and ddof have no effect and are deprecated", DeprecationWarning, stacklevel=3)
...
def _percentile_dispatcher(
    a, q, axis=None, out=None, overwrite_input=None, method=None, keepdims=None, *, interpolation=None
):
    return (a, q, out)
...
def _quantile(
    ...
    try:
        ...
        raise ValueError(f"{method!r} is not a valid method. Use one of: " f"{_QuantileMethods.keys()}") from None
...
def insert(arr, obj, values, axis=None):
    ...
    if indices.size == 1:
        ...
        if index < -N or index > N:
            raise IndexError(f"index {obj} is out of bounds for axis {axis} " f"with size {N}")
        if index < 0:
            index += N
