...
class TestSelect:
    ...
    conditions = [np.array([False, False, False]), np.array([False, True, False]), np.array([False, False, True])]
    ...
    def test_basic(self):
        ...
        assert_array_equal(select(conditions, choices, default=15), self._select(conditions, choices, default=15))
...
class TestInsert:
    ...
    def test_multidim(self):
        ...
        c = np.concatenate((a[:, 0:1], np.arange(1, 4).repeat(3).reshape(3, 3).T, a[:, 1:2]), axis=1)
    ...
    def test_basic(self):
        ...
        for ctype in [np.int16, np.uint16, np.int32, np.uint32, np.float32, np.float64, np.complex64, np.complex128]:
            ...
            if ctype in ["1", "b"]:
                ...
                assert_array_equal(
                    np.cumprod(a2, axis=0), np.array([[1, 2, 3, 4], [5, 12, 21, 36], [50, 36, 84, 180]], ctype)
                )
                assert_array_equal(
                    np.cumprod(a2, axis=-1), np.array([[1, 2, 6, 24], [5, 30, 210, 1890], [10, 30, 120, 600]], ctype)
                )
...
class TestGradient:
    ...
    def test_datetime64(self):
        ...
        x = np.array(
            ["1910-08-16", "1910-08-11", "1910-08-10", "1910-08-12", "1910-10-12", "1910-12-12", "1912-12-12"],
            dtype="datetime64[D]",
        )
...
class TestExtins:
    ...
    def test_place(self):
        ...
        assert_raises_regex(
            ValueError, "Cannot insert from an empty array", lambda: place(a, [0, 0, 0, 0, 0, 1, 0], [])
        )
...
class TestVectorize:
    ...
    def test_parse_gufunc_signature(self):
        ...
        assert_equal(nfb._parse_gufunc_signature("(x)->(y),()"), ([("x",)], [("y",), ()]))
        assert_equal(
            nfb._parse_gufunc_signature("(),(a,b,c),(d)->(d,e)"), ([(), ("a", "b", "c"), ("d",)], [("d", "e")])
        )
...
class TestCorrCoef:
    ...
    B = np.array(
        [[0.10377691, 0.5417086, 0.49807457], [0.82872117, 0.77801674, 0.39226705], [0.9314666, 0.66800209, 0.03538394]]
    )
...
class TestKaiser:
    def test_simple(self):
        ...
        assert_almost_equal(kaiser(5, 1.0), np.array([0.78984831, 0.94503323, 1.0, 0.94503323, 0.78984831]))
        assert_almost_equal(kaiser(5, 1.56789), np.array([0.58285404, 0.88409679, 1.0, 0.88409679, 0.58285404]))
...
class TestInterp:
    ...
    def test_complex_interp(self):
        ...
        y = [7.5 + 1.5j, 5.0 + 1.0j, 8.75 + 1.75j, 6.25 + 1.25j, 3.0 + 3j, 3.25 + 3.25j, 3.5 + 3.5j, 3.75 + 3.75j]
...
class TestPercentile:
    ...
    def test_extended_axis(self):
        ...
        assert_equal(np.percentile(x, [25, 60], axis=(0, 1, 2)), np.percentile(x, [25, 60], axis=None))
        assert_equal(np.percentile(x, [25, 60], axis=(0,)), np.percentile(x, [25, 60], axis=0))

        d = np.arange(3 * 5 * 7 * 11).reshape((3, 5, 7, 11))
        np.random.shuffle(d.ravel())
        assert_equal(np.percentile(d, 25, axis=(0, 1, 2))[0], np.percentile(d[:, :, :, 0].flatten(), 25))
        assert_equal(np.percentile(d, [10, 90], axis=(0, 1, 3))[:, 1], np.percentile(d[:, :, 1, :].flatten(), [10, 90]))
        assert_equal(np.percentile(d, 25, axis=(3, 1, -4))[2], np.percentile(d[:, :, 2, :].flatten(), 25))
        assert_equal(np.percentile(d, 25, axis=(3, 1, 2))[2], np.percentile(d[2, :, :, :].flatten(), 25))
        assert_equal(np.percentile(d, 25, axis=(3, 2))[2, 1], np.percentile(d[2, 1, :, :].flatten(), 25))
        assert_equal(np.percentile(d, 25, axis=(1, -2))[2, 1], np.percentile(d[2, :, :, 1].flatten(), 25))
        assert_equal(np.percentile(d, 25, axis=(1, 3))[2, 2], np.percentile(d[2, :, 2, :].flatten(), 25))
    ...
    def test_nan_behavior(self):
        ...
        b = np.percentile(np.arange(24, dtype=float).reshape(2, 3, 4), [0.3, 0.6], (0, 2), method="nearest")
        b[:, 1] = np.nan
        b[:, 2] = np.nan
...
class TestQuantile:
    ...
    def test_quantile_monotonic(self, method):
        ...
        quantile = np.quantile(
            np.array([0, 1, 1, 2, 2, 3, 3, 4, 5, 5, 1, 1, 9, 9, 9, 8, 8, 7]) * 0.1, p0, method=method
        )
    ...
    @hypothesis.given(
        arr=arrays(
            ...
            elements=st.floats(allow_infinity=False, allow_nan=False, min_value=-1e300, max_value=1e300),
        )
    )
