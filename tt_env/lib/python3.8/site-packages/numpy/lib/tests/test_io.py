    ...
    def test_unicode_roundtrip(self):
        ...
        with tempdir() as tmpdir:
            for suffix in suffixes:
                ...
                b = np.loadtxt(os.path.join(tmpdir, "test.csv" + suffix), encoding="UTF-16-LE", dtype=np.unicode_)
                assert_array_equal(a, b)
        ...
        def check_large_zip(memoryerror_raised):
            ...
            try:
                ...
                test_data = np.asarray(
                    [np.random.rand(np.random.randint(50, 100), 4) for i in range(800000)], dtype=object
                )
                with tempdir() as tmpdir:
...
class LoadTxtBase:
    ...
    def test_converters_decode(self):
        ...
        c.write(b"\xcf\x96")
        c.seek(0)
        x = self.loadfunc(c, dtype=np.unicode_, converters={0: lambda x: x.decode("UTF-8")})
    ...
    def test_converters_nodecode(self):
        ...
        with temppath() as path:
            with io.open(path, "wt", encoding="UTF-8") as f:
                f.write(utf8)
            x = self.loadfunc(path, dtype=np.unicode_, converters={0: lambda x: x + "t"}, encoding="UTF-8")
            a = np.array([utf8 + "t"])
            assert_array_equal(x, a)
...
class TestLoadTxt(LoadTxtBase):
    ...
    def test_usecols(self):
        ...
        bogus_idx = 1.5
        assert_raises_regex(
            TypeError, "^usecols must be.*%s" % type(bogus_idx).__name__, np.loadtxt, c, usecols=bogus_idx
        )

        assert_raises_regex(
            TypeError, "^usecols must be.*%s" % type(bogus_idx).__name__, np.loadtxt, c, usecols=[0, bogus_idx, 0]
        )
    ...
    def test_bad_usecols(self):
        ...
        with pytest.raises(
            TypeError, match="If a structured dtype .*. But 1 usecols were given and " "the number of fields is 3."
        ):
...
M   33  21.99
    ...
    def test_invalid_converter(self):
        strip_rand = lambda x: float(
            (b"r" in x.lower() and x.split()[-1]) or (b"r" not in x.lower() and x.strip() or 0.0)
        )
        strip_per = lambda x: float(
            (b"%" in x.lower() and x.split()[0]) or (b"%" not in x.lower() and x.strip() or 0.0)
        )
        s = TextIO(
    ...
    def test_withmissing_float(self):
        ...
        control = ma.array(
            [(0, 1.5), (2, -1.0)], mask=[(False, False), (False, True)], dtype=[("A", int), ("B", float)]
        )
        assert_equal(test, control)
        assert_equal(test.mask, control.mask)
    ...
    def test_with_masked_column_various(self):
        ...
        test = np.genfromtxt(data, dtype=None, missing_values="2,5", usemask=True)
        control = ma.array(
            [(1, 2, 3), (0, 5, 6)], mask=[(0, 1, 0), (0, 1, 0)], dtype=[("f0", bool), ("f1", bool), ("f2", int)]
        )
        assert_equal(test, control)
    ...
    def test_utf8_file_nodtype_unicode(self):
        ...
        with temppath() as path:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                assert_(w[0].category is np.VisibleDeprecationWarning)
            ctl = np.array(
                [["norm1", "norm2", "norm3"], ["norm1", latin1, "norm3"], ["test1", "testNonethe" + utf8, "test3"]],
                dtype=np.unicode_,
            )
            assert_array_equal(test, ctl)
