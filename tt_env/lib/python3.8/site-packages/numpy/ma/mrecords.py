...
def fromtextfile(
    fname, delimiter=None, commentchar="#", missingchar="", varnames=None, vartypes=None, *, delimitor=np._NoValue
    ...
    _variables = masked_array(
        [line.strip().split(delimiter) for line in ftext if line[0] != commentchar and len(line) > 1]
    )
    ...
    _datalist = [
        masked_array(a, mask=m, dtype=t, fill_value=f) for (a, m, t, f) in zip(_variables.T, _mask, vartypes, mfillv)
    ]
