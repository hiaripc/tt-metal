...
class NumpyDocTestCase(npd.DocTestCase):
    def __init__(self, test, optionflags=0, setUp=None, tearDown=None, checker=None, obj=None, result_var="_"):
        ...
        doctest.DocTestCase.__init__(
            self, test, optionflags=optionflags, setUp=setUp, tearDown=tearDown, checker=checker
        )
            ...
            if old_mode != new_mode:
                try:
                    raise AssertionError(
                        "FPU mode changed from {0:#x} to {1:#x} during the " "test".format(old_mode, new_mode)
                    )
    ...
    def runTests(self):
        ...
        if self.testRunner is None:
            self.testRunner = nose.core.TextTestRunner(
                stream=self.config.stream, verbosity=self.config.verbosity, config=self.config
            )
