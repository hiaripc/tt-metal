...
def build_err_msg(
    arrays, err_msg, header="Items are not equal:", verbose=True, names=("ACTUAL", "DESIRED"), precision=8
):
    msg = ["\n" + header]
    if err_msg:
...
def assert_array_compare(
    comparison, x, y, err_msg="", verbose=True, header="", precision=6, equal_nan=True, equal_inf=True, *, strict=False
):
    __tracebackhide__ = True  # Hide traceback for py.test
    ...
    try:
        ...
        if not cond:
            ...
            msg = build_err_msg(
                [x, y], err_msg + reason, verbose=verbose, header=header, names=("x", "y"), precision=precision
            )
            raise AssertionError(msg)
        ...
        if cond != True:
            ...
            remarks = ["Mismatched elements: {} / {} ({:.3g}%)".format(n_mismatch, n_elements, percent_mismatch)]
            ...
            err_msg += "\n" + "\n".join(remarks)
            msg = build_err_msg(
                [ox, oy], err_msg, verbose=verbose, header=header, names=("x", "y"), precision=precision
            )
            raise AssertionError(msg)
    except ValueError:
        import traceback
...
def assert_array_equal(x, y, err_msg="", verbose=True, *, strict=False):
    ...
    __tracebackhide__ = True  # Hide traceback for py.test
    assert_array_compare(
        operator.__eq__, x, y, err_msg=err_msg, verbose=verbose, header="Arrays are not equal", strict=strict
    )
...
def assert_array_less(x, y, err_msg="", verbose=True):
    ...
    __tracebackhide__ = True  # Hide traceback for py.test
    assert_array_compare(
        operator.__lt__, x, y, err_msg=err_msg, verbose=verbose, header="Arrays are not less-ordered", equal_inf=False
    )
...
def assert_array_max_ulp(a, b, maxulp=1, dtype=None):
    ...
    if not np.all(ret <= maxulp):
        raise AssertionError(
            "Arrays are not almost equal up to %g " "ULP (max difference is %g ULP)" % (maxulp, np.max(ret))
        )
    return ret
...
def _assert_no_gc_cycles_context(name=None):
    ...
    try:
        for i in range(100):
            ...
            raise RuntimeError(
                "Unable to fully collect garbage - perhaps a __del__ method " "is creating more reference cycles?"
            )
...
def check_free_memory(free_bytes):
    ...
    if env_value is not None:
        ...
        msg = f"{free_bytes/1e9} GB memory required, but environment variable " f"NPY_AVAILABLE_MEM={env_value} set"
    else:
