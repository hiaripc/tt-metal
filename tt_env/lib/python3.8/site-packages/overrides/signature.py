    ...
    for super_index, (name, super_param) in enumerate(super_sig.parameters.items()):
        ...
        if not is_param_defined_in_sub(name, True, sub_has_var_kwargs, sub_sig, super_param):
            ...
            if super_param.kind != sub_param.kind and not (
                super_param.kind == Parameter.KEYWORD_ONLY and sub_param.kind == Parameter.POSITIONAL_OR_KEYWORD
            ):
...
def ensure_all_positional_args_defined_in_sub(
    ...
    sub_parameter_values = [
        v for v in sub_sig.parameters.values() if v.kind not in (Parameter.KEYWORD_ONLY, Parameter.VAR_KEYWORD)
    ]
    super_parameter_values = [
        v for v in super_sig.parameters.values() if v.kind not in (Parameter.KEYWORD_ONLY, Parameter.VAR_KEYWORD)
    ]
    ...
    for index, sub_param in enumerate(sub_parameter_values):
        ...
        if (
            ...
            and not (
                super_param.kind == Parameter.POSITIONAL_ONLY and sub_param.kind == Parameter.POSITIONAL_OR_KEYWORD
            )
...
def is_param_defined_in_sub(
    ...
    return (
        ...
        or (super_param.kind == Parameter.POSITIONAL_OR_KEYWORD and sub_has_var_args and sub_has_var_kwargs)
