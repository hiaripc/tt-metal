...
def pytest_collection_modifyitems(items, config) -> None:
    ...
    for item in items:
        if config.getoption("--doctest-modules") or config.getoption("--doctest-cython", default=False):
...
@pytest.fixture(params=[key for key, value in indices_dict.items() if not isinstance(value, MultiIndex)])
...
_narrow_series = {
    f"{dtype.__name__}-series": tm.make_rand_series(name="a", dtype=dtype) for dtype in tm.NARROW_NP_DTYPES
}
