...
class ArrowExtensionArray(OpsMixin, ExtensionArraySupportsAnyAll, BaseStringArrayMethods):
    ...
    def _from_sequence(cls, scalars, *, dtype: Dtype | None = None, copy: bool = False):
        ...
        if (
            ...
            and (pa.types.is_large_binary(pa_dtype) or pa.types.is_large_string(pa_dtype))
        ):
    ...
    def _from_sequence_of_strings(cls, strings, *, dtype: Dtype | None = None, copy: bool = False):
        ...
        if pa_type is None or pa.types.is_binary(pa_type) or pa.types.is_string(pa_type):
    ...
    def argsort(
        ...
        **kwargs,
        ...
        result = pc.array_sort_indices(self._data, order=order, null_placement=null_placement)
        np_result = result.to_numpy()
        return np_result.astype(np.intp, copy=False)

    def _argmin_max(self, skipna: bool, method: str) -> int:
        if self._data.length() in (0, self._data.null_count) or (self._hasna and not skipna):
    ...
    def fillna(
        ...
        if is_array_like(value):
            ...
            if len(value) != len(self):
                raise ValueError(f"Length of 'value' does not match. Got ({len(value)}) " f" expected {len(self)}")
    ...
    def reshape(self, *args, **kwargs):
        raise NotImplementedError(f"{type(self)} does not support reshape " f"as backed by a 1D pyarrow.ChunkedArray.")

    def round(self: ArrowExtensionArrayT, decimals: int = 0, *args, **kwargs) -> ArrowExtensionArrayT:
        """
    ...
    @classmethod
    def _concat_same_type(cls: type[ArrowExtensionArrayT], to_concat) -> ArrowExtensionArrayT:
        """
    ...
    def _quantile(self: ArrowExtensionArrayT, qs: npt.NDArray[np.float64], interpolation: str) -> ArrowExtensionArrayT:
        """
    ...
    def _apply_elementwise(self, func: Callable) -> list[list[Any]]:
        ...
        return [
            [None if val is None else func(val) for val in chunk.to_numpy(zero_copy_only=False)]
    ...
    def _str_find(self, sub: str, start: int = 0, end: int | None = None):
        if start != 0 and end is not None:
            ...
            raise NotImplementedError(f"find not implemented with {sub=}, {start=}, {end=}")
        return type(self)(result)
    ...
    def _str_get(self, i: int):
        ...
        selected = pc.utf8_slice_codeunits(self._data, start=start, stop=stop, step=step)
    ...
    def _str_slice(self, start: int | None = None, stop: int | None = None, step: int | None = None):
        ...
        if step is None:
            step = 1
        return type(self)(pc.utf8_slice_codeunits(self._data, start=start, stop=stop, step=step))
    ...
    def _str_extract(self, pat: str, flags: int = 0, expand: bool = True):
        raise NotImplementedError("str.extract not supported with pd.ArrowDtype(pa.string()).")
    ...
    def _dt_tz_localize(
        ...
        if tz is None:
            ...
            result = pc.assume_timezone(self._data, str(tz), ambiguous=ambiguous, nonexistent=nonexistent_pa)
        return type(self)(result)
