...
class DatetimeArray(dtl.TimelikeOps, dtl.DatelikeOps):
    ...
    _datetimelike_ops: list[str] = _field_ops + _object_ops + _bool_ops + _other_ops + ["unit"]
    ...
    def astype(self, dtype, copy: bool = True):
        ...
        elif self.tz is None and is_datetime64_dtype(dtype) and dtype != self.dtype and is_unitless(dtype):
            raise TypeError(
    ...
    def _format_native_types(
        ...
        return tslib.format_array_from_datetime(self.asi8, tz=self.tz, format=fmt, na_rep=na_rep, reso=self._creso)
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo | None) -> tzinfo | None:
    ...
    if tz is None:
        ...
        raise TypeError(f"data is already tz-aware {inferred_tz}, unable to " f"set specified tz: {tz}")
    return tz
...
def _validate_dt64_dtype(dtype):
    ...
    if dtype is not None:
        ...
        if (
            isinstance(dtype, np.dtype) and (dtype.kind != "M" or not is_supported_unit(get_unit_from_dtype(dtype)))
