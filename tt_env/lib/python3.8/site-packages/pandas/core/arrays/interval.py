...
class IntervalArray(IntervalMixin, ExtensionArray):
    ...
    def __new__(
        ...
        if isinstance(data, cls):
            ...
            # don't allow scalars
            if is_scalar(data):
                msg = f"{cls.__name__}(...) must be called with a collection " f"of some kind, {data} was passed"
                raise TypeError(msg)
    ...
    def _ensure_simple_new_inputs(
        ...
        if type(left) != type(right):
            msg = f"must not have differing left [{type(left).__name__}] and " f"right [{type(right).__name__}] types"
        ...
        if isinstance(left, ABCDatetimeIndex) and str(left.tz) != str(right.tz):
            msg = "left and right must have the same time zone, got " f"'{left.tz}' and '{right.tz}'"
            raise ValueError(msg)
    ...
    @classmethod
    def _from_factorized(cls: type[IntervalArrayT], values: np.ndarray, original: IntervalArrayT) -> IntervalArrayT:
        if len(values) == 0:
    ...
    def __getitem__(self: IntervalArrayT, key: PositionalIndexer) -> IntervalArrayT | IntervalOrNA:
    ...
    def fillna(self: IntervalArrayT, value=None, method=None, limit=None) -> IntervalArrayT:
        """
    ...
    @classmethod
    def _concat_same_type(cls: type[IntervalArrayT], to_concat: Sequence[IntervalArrayT]) -> IntervalArrayT:
        """
    ...
    def _validate_listlike(self, value):
        ...
        try:
            ...
            msg = "'value' should be a compatible interval type, " f"got {type(value)} instead."
            raise TypeError(msg) from err
    ...
    def unique(self) -> IntervalArray:
        ...
        nc = unique(self._combined.view("complex128")[:, 0])  # type: ignore[call-overload]
        nc = nc[:, None]
