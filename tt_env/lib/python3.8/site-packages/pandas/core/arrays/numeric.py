...
class NumericArray(BaseMaskedArray):
    ...
    def __init__(self, values: np.ndarray, mask: npt.NDArray[np.bool_], copy: bool = False) -> None:
        ...
        if not (isinstance(values, np.ndarray) and checker(values.dtype)):
            descr = "floating" if self._dtype_cls.kind == "f" else "integer"  # type: ignore[comparison-overlap]
    ...
    def _coerce_to_array(cls, value, *, dtype: DtypeObj, copy: bool = False) -> tuple[np.ndarray, np.ndarray]:
        ...
        mask = None
        values, mask, _, _ = _coerce_to_data_and_mask(value, mask, dtype, copy, dtype_cls, default_dtype)
        return values, mask

    @classmethod
    def _from_sequence_of_strings(cls: type[T], strings, *, dtype: Dtype | None = None, copy: bool = False) -> T:
