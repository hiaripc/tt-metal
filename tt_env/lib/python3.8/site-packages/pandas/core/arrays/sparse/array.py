...
def _sparse_array_op(left: SparseArray, right: SparseArray, op: Callable, name: str) -> SparseArray:
    ...
    if left.sp_index.ngaps == 0 or right.sp_index.ngaps == 0:
        ...
        if name in ["floordiv", "mod"] and (right == 0).any() and left.dtype.kind in ["i", "u"]:
...
class SparseArray(OpsMixin, PandasObject, ExtensionArray):
    ...
    def __init__(
        ...
        if isinstance(data, type(self)) and sparse_index is None:
            ...
            sparse_values = np.asarray(data.sp_values, dtype=dtype)  # type: ignore[arg-type]
            ...
            if len(sparse_values) != sparse_index.npoints:
                raise AssertionError(
                    f"Non array-like type {type(sparse_values)} must " "have the same length as the index"
                )
    ...
    def take(self: SparseArrayT, indices, *, allow_fill: bool = False, fill_value=None) -> SparseArrayT:
    ...
    @classmethod
    def _concat_same_type(cls: type[SparseArrayT], to_concat: Sequence[SparseArrayT]) -> SparseArrayT:
