...
class TimedeltaArray(dtl.TimelikeOps):
    ...
    def astype(self, dtype, copy: bool = True):
        ...
        if isinstance(dtype, np.dtype) and dtype.kind == "m":
            ...
            if is_supported_unit(get_unit_from_dtype(dtype)):
                ...
                raise ValueError(
                    f"Cannot convert from {self.dtype} to {dtype}. " "Supported resolutions are 's', 'ms', 'us', 'ns'"
                )
    ...
    def __truediv__(self, other):
        ...
        if is_timedelta64_dtype(other.dtype) or is_integer_dtype(other.dtype) or is_float_dtype(other.dtype):
...
def _validate_td64_dtype(dtype) -> DtypeObj:
    ...
    if not isinstance(dtype, np.dtype) or dtype.kind != "m" or not is_supported_unit(get_unit_from_dtype(dtype)):
