...
def random_state(state: RandomState | None = None):
    ...
    """
    if is_integer(state) or is_array_like(state) or isinstance(state, np.random.BitGenerator):
...
def require_length_match(data, index: Index) -> None:
    ...
    if len(data) != len(index):
        raise ValueError("Length of values " f"({len(data)}) " "does not match length of index " f"({len(index)})")
