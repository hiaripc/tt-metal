...
def is_any_int_dtype(arr_or_dtype) -> bool:
    ...
    False
    """
    return _is_dtype_type(arr_or_dtype, classes(np.integer, np.timedelta64)) or _is_dtype(
...
def is_integer_dtype(arr_or_dtype) -> bool:
    ...
    False
    """
    return _is_dtype_type(arr_or_dtype, classes_and_not_datetimelike(np.integer)) or _is_dtype(
        arr_or_dtype, lambda typ: isinstance(typ, ExtensionDtype) and typ.kind in "iu"
    )
...
def is_signed_integer_dtype(arr_or_dtype) -> bool:
    ...
    False
    """
    return _is_dtype_type(arr_or_dtype, classes_and_not_datetimelike(np.signedinteger)) or _is_dtype(
...
def is_unsigned_integer_dtype(arr_or_dtype) -> bool:
    ...
    True
    """
    return _is_dtype_type(arr_or_dtype, classes_and_not_datetimelike(np.unsignedinteger)) or _is_dtype(
...
def is_numeric_dtype(arr_or_dtype) -> bool:
    ...
    False
    """
    return _is_dtype_type(arr_or_dtype, classes_and_not_datetimelike(np.number, np.bool_)) or _is_dtype(
...
def is_1d_only_ea_obj(obj: Any) -> bool:
    ...
    return isinstance(obj, ExtensionArray) and not isinstance(obj, (DatetimeArray, TimedeltaArray, PeriodArray))
...
def is_1d_only_ea_dtype(dtype: DtypeObj | None) -> bool:
    ...
    return isinstance(dtype, ExtensionDtype) and not isinstance(dtype, (DatetimeTZDtype, PeriodDtype))
...
def is_ea_or_datetimelike_dtype(dtype: DtypeObj | None) -> bool:
    ...
    """
    return isinstance(dtype, ExtensionDtype) or (isinstance(dtype, np.dtype) and dtype.kind in ["m", "M"])
...
def _validate_date_like_dtype(dtype) -> None:
    ...
    if typ not in ["generic", "ns"]:
        raise ValueError(
            f"{repr(dtype.name)} is too specific of a frequency, " f"try passing {repr(dtype.type.__name__)}"
        )
