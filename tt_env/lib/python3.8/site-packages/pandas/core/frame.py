...
class DataFrame(NDFrame, OpsMixin):
    ...
    def dot(self, other: AnyArrayLike | DataFrame) -> DataFrame | Series:
        ...
        if isinstance(other, DataFrame):
            ...
            return self._constructor_sliced(np.dot(lvals, rvals), index=left.index, copy=False)
    ...
    def from_dict(
        ...
        if orient == "index":
            ...
            raise ValueError(f"Expected 'index', 'columns' or 'tight' for orient parameter. " f"Got '{orient}' instead")
    ...
    def transpose(self, *args, copy: bool = False) -> DataFrame:
        ...
        **Non-square DataFrame with mixed dtypes**
        ...
        if self._can_fast_transpose:
            ...
            result = self._constructor(new_vals, index=self.columns, columns=self.index, copy=False)
    ...
    def __getitem__(self, key):
        ...
        if is_hashable(key) and not is_iterator(key):
            ...
            if not is_mi and (
                self.columns.is_unique and key in self.columns or key in self.columns.drop_duplicates(keep=False)
            ):
    ...
    def _set_item_frame_value(self, key, value: DataFrame) -> None:
        ...
        if len(value.columns) != 1:
            raise ValueError("Cannot set a DataFrame with multiple columns to the single " f"column {key}")
        ...
        * To select Pandas datetimetz dtypes, use ``'datetimetz'`` (new in
        ...
        def dtype_predicate(dtype: DtypeObj, dtypes_set) -> bool:
            ...
            return issubclass(dtype.type, tuple(dtypes_set)) or (
                np.number in dtypes_set and getattr(dtype, "_is_numeric", False) and not is_bool_dtype(dtype)
            )
    ...
    def insert(
        ...
        if allow_duplicates and not self.flags.allows_duplicate_labels:
            raise ValueError(
                "Cannot specify 'allow_duplicates=True' when " "'self.flags.allows_duplicate_labels' is False."
            )
    ...
    def shift(
        ...
        if axis == 1 and periods != 0 and freq is None and fill_value is lib.no_default and ncols > 0:
        ...
        *,
        ...
        for col in keys:
            ...
            if len(arrays[-1]) != len(self):
                ...
                raise ValueError(
                    f"Length mismatch: Expected {len(self)} rows, " f"received array of length {len(arrays[-1])}"
                )
                ...
                if multi_col:
                    ...
                    if col_fill is None:
                        if len(col_name) not in (1, self.columns.nlevels):
                            raise ValueError("col_fill=None is incompatible " f"with incomplete column name {name}")
                        col_fill = col_name[0]
...
"""
    ...
    def _append(
        ...
        if isinstance(other, (Series, dict)):
            ...
            index = Index(
                [other.name],
                name=self.index.names if isinstance(self.index, MultiIndex) else self.index.name,
            )
    ...
    def round(self, decimals: int | dict[IndexLabel, int] | Series = 0, *args, **kwargs) -> DataFrame:
        ...
        **kwargs
        ...
        if new_cols is not None and len(new_cols) > 0:
            return self._constructor(concat(new_cols, axis=1), index=self.index, columns=self.columns).__finalize__(
                self, method="round"
            )
        else:
    ...
    def corr(
        ...
        * `Spearman's rank correlation coefficient <https://en.wikipedia.org/wiki/Spearman%27s_rank_correlation_coefficient>`_
        ...
        if method == "pearson":
            ...
            raise ValueError(
                "method must be either 'pearson', " "'spearman', 'kendall', or a callable, " f"'{method}' was supplied"
            )
    ...
    def count(self, axis: Axis = 0, numeric_only: bool = False):
        ...
        if len(frame._get_axis(axis)) == 0:
            ...
            if frame._is_mixed_type or frame._mgr.any_extension_types:
                ...
                result = self._constructor_sliced(counts, index=frame._get_agg_axis(axis), copy=False)
    ...
    def quantile(
        ...
        if method == "single":
            ...
            if interpolation not in valid_interpolation:
                raise ValueError(
                    f"Invalid interpolation: {interpolation}. " f"Interpolation must be in {valid_interpolation}"
                )
    ...
    def isin(self, values: Series | DataFrame | Sequence | Mapping) -> DataFrame:
        ...
        if isinstance(values, dict):
            ...
            result = self._constructor(
                algorithms.isin(self.values.ravel(), values).reshape(self.shape),  # type: ignore[arg-type]
                self.index,
                self.columns,
                copy=False,
            )
    ...
    def _to_dict_of_blocks(self, copy: bool = True):
        ...
        return {k: self._constructor(v).__finalize__(self) for k, v, in mgr.to_dict(copy=copy).items()}
