...
class NDFrame(PandasObject, indexing.IndexingMixin):
    ...
    def _validate_dtype(cls, dtype) -> DtypeObj | None:
        ...
        if dtype is not None:
            ...
            # a compound dtype
            if dtype.kind == "V":
                raise NotImplementedError("compound dtypes are not implemented " f"in the {cls.__name__} constructor")
    ...
    @final
    def _set_axis_nocheck(self, labels, axis: Axis, inplace: bool_t, copy: bool_t | None):
        if inplace:
    ...
    @final
    def swapaxes(self: NDFrameT, axis1: Axis, axis2: Axis, copy: bool_t | None = None) -> NDFrameT:
        """
        ...
        if using_copy_on_write() and self._mgr.is_single_block and isinstance(self._mgr, BlockManager):
            ...
            new_mgr.blocks[0].refs.add_reference(new_mgr.blocks[0])  # type: ignore[arg-type]
    ...
    def squeeze(self, axis: Axis | None = None):
        ...
        return self.iloc[tuple(0 if i in axes and len(a) == 1 else slice(None) for i, a in enumerate(self.axes))]
    ...
    def rename_axis(
        ...
        **MultiIndex**
        ...
        if mapper is not lib.no_default:
            ...
            if non_mapper:
                return self._set_axis_name(mapper, axis=axis, inplace=inplace, copy=copy)
            else:
    ...
    def bool(self) -> bool_t:
        ...
        if isinstance(v, (bool, np.bool_)):
            ...
            raise ValueError("bool cannot act on a non-boolean single element " f"{type(self).__name__}")
    ...
    def _drop_labels_or_levels(self, keys, axis: AxisInt = 0):
        ...
        if invalid_keys:
            raise ValueError("The following keys are not valid labels or " f"levels for axis {axis}: {invalid_keys}")
    ...
    def __array__(self, dtype: npt.DTypeLike | None = None) -> np.ndarray:
        ...
        if astype_is_view(values.dtype, arr.dtype) and using_copy_on_write() and self._mgr.is_single_block:
    ...
    def to_latex(
        ...
        render_kwargs_ = {
            ...
            "multicol_align": multicolumn_format if multicolumn else f"naive-{multicolumn_format}",
            ...
            "clines": "skip-last;data" if (multirow and isinstance(self.index, MultiIndex)) else None,
            "bold_rows": bold_rows,
        }
    ...
    def _take(
        ...
        if not isinstance(indices, slice):
            ...
            if axis == 0 and indices.ndim == 1 and using_copy_on_write() and is_range_indexer(indices, len(self)):
    ...
    def xs(
        ...
        if is_scalar(loc) and axis == 0:
            ...
            result = self._constructor_sliced(new_mgr, name=self.index[loc]).__finalize__(self)
        elif is_scalar(loc):
    ...
    def reindex(
        ...
        * ``(labels, axis={{'index', 'columns'}}, ...)``
        ...
        return self._reindex_axes(axes, level, limit, tolerance, method, fill_value, copy).__finalize__(
            self, method="reindex"
        )
    ...
    def _needs_reindex_multi(self, axes, method, level) -> bool_t:
        ...
        return (
            ...
            and not (self.ndim == 2 and len(self.dtypes) == 1 and is_extension_array_dtype(self.dtypes.iloc[0]))
        )
    ...
    def astype(self: NDFrameT, dtype, copy: bool_t | None = None, errors: IgnoreRaise = "raise") -> NDFrameT:
        """
        ...
        if value is None:
            ...
            if self.ndim == 1:
                ...
                for k, v in value.items():
                    ...
                    # attribute "get"
                    downcast_k = downcast if not is_dict else downcast.get(k)  # type: ignore[union-attr]
        ...
        if value is lib.no_default or method is not lib.no_default:
            ...
            if is_dict_like(to_replace):
                ...
                if len(to_replace) != len(value):
                    raise ValueError(
                        f"Replacement lists must match in length. " f"Expecting {len(to_replace)} got {len(value)} "
                    )
    ...
    @final
    def at_time(self: NDFrameT, time, asof: bool_t = False, axis: Axis | None = None) -> NDFrameT:
        """
    ...
    def _align_series(
        ...
        if not axis:
            ...
            if is_series:
                ...
                left = self._constructor(self._mgr.reindex_indexer(join_index, lidx, axis=1, copy=copy))
    ...
    def shift(
        ...
        if freq is None:
            ...
            axis = self._get_axis_number(axis)
            new_data = self._mgr.shift(periods=periods, axis=axis, fill_value=fill_value)
        ...
        if isinstance(index, PeriodIndex):
            ...
            if freq != orig_freq:
                ...
                raise ValueError(
                    f"Given freq {freq.rule_code} does not match " f"PeriodIndex freq {orig_freq.rule_code}"
                )
    ...
    @final
    @doc(klass=_shared_doc_kwargs["klass"])
    def tz_convert(self: NDFrameT, tz, axis: Axis = 0, level=None, copy: bool_t | None = None) -> NDFrameT:
        """
    ...
    def _accum_func(
        ...
        **kwargs,
        ...
        if axis == 1:
            return self.T._accum_func(name, func, axis=0, skipna=skipna, *args, **kwargs).T  # noqa: B026
    ...
    def _inplace_method(self, other, op):
        ...
        if self.ndim == 1 and result._indexed_same(self) and is_dtype_equal(result.dtype, self.dtype):
            ...
            self._mgr.setitem_inplace(slice(None), result._values)  # type: ignore[union-attr]
            return self
    ...
    def __itruediv__(self: NDFrameT, other) -> NDFrameT:
        # error: Unsupported left operand type for / ("Type[NDFrame]")
        return self._inplace_method(other, type(self).__truediv__)  # type: ignore[operator]

    def __ifloordiv__(self: NDFrameT, other) -> NDFrameT:
        # error: Unsupported left operand type for // ("Type[NDFrame]")
        return self._inplace_method(other, type(self).__floordiv__)  # type: ignore[operator]
...
def _align_as_utc(left: NDFrameT, right: NDFrameT, join_index: Index | None) -> tuple[NDFrameT, NDFrameT]:
    """
