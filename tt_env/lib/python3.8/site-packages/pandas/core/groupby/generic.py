...
class SeriesGroupBy(GroupBy[Series]):
    ...
    def _get_data_to_aggregate(self, *, numeric_only: bool = False, name: str | None = None) -> SingleManager:
        ...
        if numeric_only and not is_numeric_dtype(ser.dtype):
            ...
            raise TypeError(f"Cannot use {kwd_name}=True with " f"{type(self).__name__}.{name} and non-numeric dtypes.")
        return single
    ...
    def _wrap_applied_output(
        ...
        if isinstance(values[0], dict):
            ...
            # GH #6265 #24880
            result = self.obj._constructor(data=values, index=self.grouper.result_index, name=self.obj.name)
            if not self.as_index:
                result = self._insert_inaxis_grouper(result)
                result.index = default_index(len(result))
