...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def _indexed_output_to_ndframe(self, result: Mapping[base.OutputKey, ArrayLike]) -> Series | DataFrame:
        raise AbstractMethodError(self)
    ...
    def _numba_prep(self, data: DataFrame):
        ...
        if len(self.grouper.groupings) > 1:
            raise NotImplementedError("More than 1 grouping labels are not supported with engine='numba'")
        # GH 46867
    ...
    def _agg_py_fallback(self, values: ArrayLike, ndim: int, alt: Callable) -> ArrayLike:
        """
    ...
    def std(
        ...
        if maybe_use_numba(engine):
            ...
            def _postprocessing(vals, inference, nullable: bool = False, result_mask=None) -> ArrayLike:
                if nullable:
    ...
    def _value_counts(
        ...
        in_axis_names = {grouping.name for grouping in self.grouper.groupings if grouping.in_axis}
        if isinstance(obj, Series):
            ...
            if subset is not None:
                ...
                if doesnt_exist:
                    raise ValueError(f"Keys {doesnt_exist} in subset do not " f"exist in the DataFrame.")
            else:
        ...
        if any(
            isinstance(grouping.grouping_vector, (Categorical, CategoricalIndex)) and not grouping._observed
            for grouping in groupings
        ):
            levels_list = [ping.result_index for ping in groupings]
            multi_index, _ = MultiIndex.from_product(levels_list, names=[ping.name for ping in groupings]).sortlevel()
    ...
    def sem(self, ddof: int = 1, numeric_only: bool = False):
        ...
        if numeric_only and self.obj.ndim == 1 and not is_numeric_dtype(self.obj.dtype):
            raise TypeError(
                f"{type(self).__name__}.sem called with " f"numeric_only={numeric_only} and dtype {self.obj.dtype}"
            )
    ...
    def ohlc(self) -> DataFrame:
        ...
        if self.obj.ndim == 1:
            ...
            res_values = self.grouper._cython_operation("aggregate", obj._values, "ohlc", axis=0, min_count=-1)

            agg_names = ["open", "high", "low", "close"]
            result = self.obj._constructor_expanddim(res_values, index=self.grouper.result_index, columns=agg_names)
            return self._reindex_output(result)

        result = self._apply_to_column_groupbys(lambda x: x.ohlc(), self._obj_with_exclusions)
        if not self.as_index:
            result = self._insert_inaxis_grouper(result)
    ...
    def quantile(
                ...
                if isinstance(orig_vals, BaseMaskedArray):
                    ...
                    if interpolation in {"linear", "midpoint"} and not is_float_dtype(orig_vals):
                        ...
                        return type(orig_vals)(
                            vals.astype(inference.numpy_dtype),  # type: ignore[union-attr]
                            result_mask,
                        )
                    ...
                    if needs_i8_conversion(inference):
                        ...
                        vals = vals.astype("i8").view(orig_vals._ndarray.dtype)  # type: ignore[union-attr]
                        # error: Item "ExtensionArray" of "Union[ExtensionArray,
                        # ndarray[Any, Any]]" has no attribute "_from_backing_data"
                        return orig_vals._from_backing_data(vals)  # type: ignore[union-attr]
        ...
        func = partial(libgroupby.group_quantile, labels=ids, qs=qs, interpolation=interpolation)
        ...
        def blk_func(values: ArrayLike) -> ArrayLike:
            ...
            if vals.ndim == 2:
                ncols = vals.shape[0]
                shaped_labels = np.broadcast_to(labels_for_lexsort, (ncols, len(labels_for_lexsort)))
            else:
    ...
    def pct_change(
        ...
        fill_grp = filled.groupby(self.grouper.codes, axis=self.axis, group_keys=self.group_keys)
    ...
    def _reindex_output(
        ...
        in_axis_grps = list((i, ping.name) for (i, ping) in enumerate(groupings) if ping.in_axis)
