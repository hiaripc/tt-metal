...
def unpack_1tuple(tup):
    ...
    if len(tup) == 1 and isinstance(tup[0], slice):
        ...
        if isinstance(tup, list):
            # GH#31299
            raise ValueError(
                "Indexing with a single-item list containing a " "slice is not allowed. Pass a tuple instead.",
            )
...
def check_array_indexer(array: AnyArrayLike, indexer: Any) -> Any:
    ...
    if is_bool_dtype(dtype):
        ...
        if len(indexer) != len(array):
            raise IndexError(f"Boolean index has wrong length: " f"{len(indexer)} instead of {len(array)}")
