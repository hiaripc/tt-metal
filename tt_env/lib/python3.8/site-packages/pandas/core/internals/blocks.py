...
class Block(PandasObject):
    ...
    @final
    def make_block(self, values, placement=None, refs: BlockValuesRefs | None = None) -> Block:
        """
    ...
    @final
    def _maybe_downcast(self, blocks: list[Block], downcast=None, using_cow: bool = False) -> list[Block]:
        ...
        if self.dtype == _dtype_obj:
            ...
            return extend_blocks([blk.convert(using_cow=using_cow, copy=not using_cow) for blk in blocks])
            ...
            for blk_num, blk in enumerate(rb):
                ...
                if convert and blk.is_object and not all(x is None for x in dest_list):
                    ...
                    result = extend_blocks(
                        [b.convert(copy=True and not using_cow, using_cow=using_cow) for b in result]
                    )
    ...
    def where(self, other, cond, _downcast: str | bool = "infer", using_cow: bool = False) -> list[Block]:
        ...
        try:
            ...
            if self.ndim == 1 or self.shape[0] == 1:
                ...
                return self._maybe_downcast(blocks, downcast=_downcast, using_cow=using_cow)
                ...
                for i, nb in enumerate(nbs):
                    ...
                    submask = cond[:, i : i + 1]
                    rbs = nb.where(oth, submask, _downcast=_downcast, using_cow=using_cow)
                    res_blocks.extend(rbs)
                return res_blocks
    ...
    def fillna(
        ...
        return extend_blocks([blk._maybe_downcast([blk], downcast=downcast, using_cow=using_cow) for blk in nbs])
...
class EABackedBlock(Block):
        ...
        try:
            ...
            if self.ndim == 1 or self.shape[0] == 1:
                if is_interval_dtype(self.dtype):
                    ...
                    return self._maybe_downcast(nbs, downcast=_downcast, using_cow=using_cow)
    ...
    def putmask(self, mask, new, using_cow: bool = False) -> list[Block]:
        ...
        if using_cow and self.refs.has_reference():
            values = values.copy()
            self = self.make_block_same_class(values.T if values.ndim == 2 else values)  # type: ignore[assignment]
    ...
    def interpolate(
        ...
        **kwargs,
        ...
        if method == "linear":  # type: ignore[comparison-overlap]
            ...
            missing.interpolate_array_2d(data_out, method=method, limit=limit, index=index, axis=axis)
...
def check_ndim(values, placement: BlockPlacement, ndim: int) -> None:
    ...
    if values.ndim > ndim:
        ...
        raise ValueError("Wrong number of dimensions. " f"values.ndim > ndim [{values.ndim} > {ndim}]")
    ...
    if not is_1d_only_ea_dtype(values.dtype):
        ...
        if len(placement) != len(values):
            raise ValueError(f"Wrong number of items passed {len(values)}, " f"placement implies {len(placement)}")
