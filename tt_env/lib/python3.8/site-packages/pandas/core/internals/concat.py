...
def _concatenate_array_managers(mgrs_indexers, axes: list[Index], concat_axis: AxisInt, copy: bool) -> Manager:
    """
    ...
    for mgr, indexers in mgrs_indexers:
        ...
        for ax, indexer in indexers.items():
            mgr = mgr.reindex_indexer(axes[ax], indexer, axis=ax, allow_dups=True, use_na_proxy=True)
...
def _concat_managers_axis0(mgrs_indexers, axes: list[Index], copy: bool) -> BlockManager:
    """
    concat_managers specialized to concat_axis=0, with reindexing already
    having been done in _maybe_reindex_columns_na_proxy.
    """
    had_reindexers = {i: len(mgrs_indexers[i][1]) > 0 for i in range(len(mgrs_indexers))}
    mgrs_indexers = _maybe_reindex_columns_na_proxy(axes, mgrs_indexers)
...
def _concatenate_join_units(join_units: list[JoinUnit], copy: bool) -> ArrayLike:
    ...
    if len(to_concat) == 1:
        ...
        to_concat = [t if is_1d_only_ea_dtype(t.dtype) else t[0, :] for t in to_concat]  # type: ignore[call-overload]
