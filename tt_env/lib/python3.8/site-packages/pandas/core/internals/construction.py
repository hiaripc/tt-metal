    ...
    if verify_integrity:
        ...
        for arr in arrays:
            if not isinstance(arr, (np.ndarray, ExtensionArray)) or arr.ndim != 1 or len(arr) != len(index):
                raise ValueError(
                    "Arrays must be 1-dimensional np.ndarray or ExtensionArray " "with length matching len(index)"
                )
...
def to_arrays(data, columns: Index | None, dtype: DtypeObj | None = None) -> tuple[list[ArrayLike], Index]:
    ...
    if isinstance(data, ABCDataFrame):
        ...
        if columns is not None:
            arrays = [data._ixs(i, axis=1)._values for i, col in enumerate(data.columns) if col in columns]
        else:
...
def _validate_or_indexify_columns(content: list[np.ndarray], columns: Index | None) -> Index:
    ...
    if columns is None:
        ...
        if not is_mi_list and len(columns) != len(content):  # pragma: no cover
            ...
            raise AssertionError(f"{len(columns)} columns passed, passed data had " f"{len(content)} columns")
        if is_mi_list:
            ...
            if columns and len(columns[0]) != len(content):
                raise ValueError(f"{len(columns[0])} columns passed, passed data had " f"{len(content)} columns")
    return columns
