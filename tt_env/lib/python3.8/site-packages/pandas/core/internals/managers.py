...
class BaseBlockManager(DataManager):
    ...
    def interpolate(self: T, inplace: bool, **kwargs) -> T:
        return self.apply("interpolate", inplace=inplace, **kwargs, using_cow=using_copy_on_write())
    ...
    def _combine(self: T, blocks: list[Block], copy: bool = True, index: Index | None = None) -> T:
        ...
        if self.is_single_block:
            ...
            if sl_type == "slice":
                ...
                if not allow_fill and only_slice:
                    ...
                    blocks = [
                        blk.getitem_block_columns(slice(ml, ml + 1), new_mgr_locs=BlockPlacement(i))
...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
            ...
            for block in blocks:
                if self.ndim != block.ndim:
                    raise AssertionError(
                        f"Number of Block dimensions ({block.ndim}) must equal " f"number of axes ({self.ndim})"
                    )
    ...
    def fast_xs(self, loc: int) -> SingleBlockManager:
        ...
        if isinstance(dtype, ExtensionDtype) and not immutable_ea:
            ...
            # "None"
            result = np.empty(n, dtype=object if immutable_ea else dtype)  # type: ignore[arg-type]
    ...
    def _iset_single(self, loc: int, value: ArrayLike, inplace: bool, blkno: int, blk: Block) -> None:
        """
    ...
    def quantile(
        ...
        *,
        ...
        blocks = [blk.quantile(axis=axis, qs=qs, interpolation=interpolation) for blk in self.blocks]
    ...
    def _interleave(
        ...
        if not dtype:
            ...
            dtype = interleaved_dtype([blk.dtype for blk in self.blocks])  # type: ignore[assignment]
...
def _form_blocks(arrays: list[ArrayLike], consolidate: bool, refs: list) -> list[Block]:
    ...
    for (_, _, dtype), tup_block in grouper:
        ...
        if isinstance(dtype, np.dtype):
            ...
            dtype_blocks = [
                block_type(ensure_block_shape(x[1], 2), placement=BlockPlacement(x[0]), ndim=2) for x in tup_block
            ]
            nbs.extend(dtype_blocks)
    return nbs
...
def _tuples_to_blocks_no_consolidate(tuples, refs) -> list[Block]:
    ...
    return [
        new_block_2d(ensure_block_shape(arr, ndim=2), placement=BlockPlacement(i), refs=ref)
