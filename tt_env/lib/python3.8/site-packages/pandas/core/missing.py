...
def check_value_size(value, mask: npt.NDArray[np.bool_], length: int):
    ...
    if is_array_like(value):
        if len(value) != length:
            raise ValueError(f"Length of 'value' does not match. Got ({len(value)}) " f" expected {length}")
        value = value[mask]
...
def _interpolate_2d_with_fill(
    ...
    **kwargs,
    ...
    if limit_direction not in valid_limit_directions:
        raise ValueError(
            "Invalid limit_direction: expecting one of " f"{valid_limit_directions}, got '{limit_direction}'."
        )
    ...
    if limit_area is not None:
        ...
        if limit_area not in valid_limit_areas:
            raise ValueError(f"Invalid limit_area: expecting one of {valid_limit_areas}, got " f"{limit_area}.")
...
def _from_derivatives(xi, yi, x, order=None, der: int | list[int] | None = 0, extrapolate: bool = False):
    """
...
def _interpolate_with_limit_area(values: np.ndarray, method: str, limit: int | None, limit_area: str | None) -> None:
    """
