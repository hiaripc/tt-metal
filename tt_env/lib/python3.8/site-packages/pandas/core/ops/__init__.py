    ...
    if isinstance(right, np.ndarray):
        if right.ndim == 1:
            ...
            if right.shape == left.shape:
                ...
                raise ValueError("Unable to coerce to DataFrame, shape " f"must be {left.shape}: given {right.shape}")

        elif right.ndim > 2:
            raise ValueError("Unable to coerce to Series/DataFrame, " f"dimension must be <= 2: {right.shape}")
...
def _maybe_align_series_as_frame(frame: DataFrame, series: Series, axis: AxisInt):
    ...
    return type(frame)(rvalues, index=frame.index, columns=frame.columns, dtype=rvalues.dtype)
