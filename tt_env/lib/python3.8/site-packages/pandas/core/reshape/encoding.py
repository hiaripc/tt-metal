...
def from_dummies(
    ...
    if not isinstance(data, DataFrame):
        raise TypeError("Expected 'data' to be a 'DataFrame'; " f"Received 'data' of type: {type(data).__name__}")

    if data.isna().any().any():
        raise ValueError("Dummy DataFrame contains NA value in column: " f"'{data.isna().any().idxmax()}'")
    ...
    if sep is None:
        ...
        raise TypeError(
            "Expected 'sep' to be of type 'str' or 'None'; " f"Received 'sep' of type: {type(sep).__name__}"
        )
    ...
    for prefix, prefix_slice in variables_slice.items():
        ...
        if any(assigned > 1):
            raise ValueError(
                "Dummy DataFrame contains multi-assignment(s); " f"First instance in row: {assigned.idxmax()}"
            )
        if any(assigned == 0):
            if isinstance(default_category, dict):
                ...
                raise ValueError(
                    "Dummy DataFrame contains unassigned value(s); " f"First instance in row: {assigned.idxmin()}"
                )
