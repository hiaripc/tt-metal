...
def melt(
    ...
    if id_vars is not None:
        if not is_list_like(id_vars):
            ...
            if not missing.empty:
                raise KeyError("The following 'id_vars' are not present " f"in the DataFrame: {list(missing)}")
    else:
        id_vars = []
    ...
    if value_vars is not None:
        if not is_list_like(value_vars):
            ...
            if not missing.empty:
                raise KeyError("The following 'value_vars' are not present in " f"the DataFrame: {list(missing)}")
    ...
    if frame.shape[1] > 0:
        mdata[value_name] = concat([frame.iloc[:, i] for i in range(frame.shape[1])]).values
    else:
