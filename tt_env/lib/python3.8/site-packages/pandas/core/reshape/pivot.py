...
def _generate_marginal_results(table, data, values, rows, cols, aggfunc, observed, margins_name: Hashable = "All"):
    if len(cols) > 0:
...
def pivot(
    ...
    *,
    ...
    data.index.names = [name if name is not None else lib.NoDefault for name in data.index.names]
    ...
    if values is lib.NoDefault:
        ...
        indexed = data.set_index(cols + columns_listlike, append=append)  # type: ignore[operator]
    else:
    ...
    result.index.names = [name if name is not lib.NoDefault else None for name in result.index.names]
