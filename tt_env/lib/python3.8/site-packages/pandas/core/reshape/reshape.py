...
class _Unstacker:
    ...
    def __init__(self, index: MultiIndex, level=-1, constructor=None) -> None:
        ...
        if num_cells > np.iinfo(np.int32).max:
            warnings.warn(
                f"The following operation may generate {num_cells} cells " f"in the resulting pandas object.",
                PerformanceWarning,
    ...
    def _indexer_and_to_sort(
        self,
    ) -> tuple[npt.NDArray[np.intp], list[np.ndarray],]:  # each has _some_ signed integer dtype
        v = self.level
    ...
    def get_new_columns(self, value_columns: Index | None):
        ...
        if isinstance(value_columns, MultiIndex):
            ...
            new_levels = value_columns.levels + (self.removed_level_full,)  # type: ignore[has-type]
