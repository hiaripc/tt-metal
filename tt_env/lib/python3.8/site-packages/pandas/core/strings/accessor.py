    ...
    def _forbid_nonstring_types(func: F) -> F:
        ...
        def wrapper(self, *args, **kwargs):
            if self._inferred_dtype not in allowed_types:
                msg = f"Cannot use .str.{func_name} with values of " f"inferred dtype '{self._inferred_dtype}'."
                raise TypeError(msg)
    ...
    def _wrap_result(
        ...
        if expand is None:
            ...
            if isinstance(result.dtype, ArrowDtype):
                ...
                result = {
                    label: ArrowExtensionArray(pa.array(res)) for label, res in zip(labels, (zip(*result.tolist())))
                }
