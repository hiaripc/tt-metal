...
def _box_as_indexlike(dt_array: ArrayLike, utc: bool = False, name: Hashable = None) -> Index:
    """
...
def _return_parsed_timezone_results(result: np.ndarray, timezones, utc: bool, name) -> Index:
    """
...
def _assemble_from_unit_mappings(arg, errors: DateTimeErrorChoices, utc: bool):
    ...
    if len(req):
        _required = ",".join(req)
        raise ValueError(
            "to assemble mappings requires at least that " f"[year, month, day] be specified: [{_required}] is missing"
        )
