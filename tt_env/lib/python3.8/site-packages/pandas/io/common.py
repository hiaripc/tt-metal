    ...
    *,
    ...
    if compression:
        ...
        # ZIP Compression
            ...
            handle = _BytesZipFile(handle, ioargs.mode, **compression_args)  # type: ignore[arg-type]
            if handle.buffer.mode == "r":
                ...
                if len(zip_names) == 1:
                    ...
                    raise ValueError("Multiple files found in ZIP file. " f"Only one file per ZIP: {zip_names}")
        ...
        # TAR Encoding
            ...
            if isinstance(handle, str):
                ...
                handle = _BytesTarFile(fileobj=handle, **compression_args)  # type: ignore[arg-type]
            ...
            if "r" in handle.buffer.mode:
                ...
                if len(files) == 1:
                    ...
                    raise ValueError("Multiple files found in TAR archive. " f"Only one file per TAR archive: {files}")
    ...
    if "r" in ioargs.mode and not hasattr(handle, "read"):
        raise TypeError("Expected file path name or file-like object, " f"got {type(ioargs.filepath_or_buffer)} type")
...
def _maybe_memory_map(handle: str | BaseBuffer, memory_map: bool) -> tuple[str | BaseBuffer, bool, list[BaseBuffer]]:
    ...
    try:
        ...
        # expected "BaseBuffer"
        wrapped = _IOWrapper(mmap.mmap(handle.fileno(), 0, access=mmap.ACCESS_READ))  # type: ignore[arg-type]
    finally:
