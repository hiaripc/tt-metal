    ...
    *{"row": "MY_ROW_CLASS", "col_trim": "", "row_trim": ""}*.
    ...
    def concat(self, other: Styler) -> Styler:
        ...
        if not self.data.index.nlevels == other.data.index.nlevels:
            raise ValueError(
                "number of index levels must be same in `other` " "as in `Styler`. See documentation for suggestions."
            )
    ...
    def to_latex(
        ...
        **CSS Conversion**
        ...
        encoding = (
            (encoding or get_option("styler.render.encoding")) if isinstance(buf, str) else encoding  # i.e. a filepath
        )
    ...
    def _update_ctx(self, attrs: DataFrame) -> None:
        ...
        if not self.index.is_unique or not self.columns.is_unique:
            raise KeyError("`Styler.apply` and `.applymap` are not compatible " "with non-unique index or columns.")
    ...
    def _applymap(self, func: Callable, subset: Subset | None = None, **kwargs) -> Styler:
    ...
    def set_sticky(
        ...
        if not isinstance(obj, pd.MultiIndex):
                    ...
                    styles.append(
                        {
                            ...
                            "props": props + f"top:{pixel_size}px; z-index:2; height:{pixel_size}px; ",
                        }
                    )
            else:
            ...
            if axis == 1:
                    ...
                    styles.append(
                        {
                            ...
                            "props": props + (f"top:{i * pixel_size}px; height:{pixel_size}px; " "z-index:2;"),
                        }
                    )
                    ...
                    styles.append(
                        {
                            ...
                            "props": props
                            + (f"top:{(len(levels_)) * pixel_size}px; " f"height:{pixel_size}px; z-index:2;"),
                        }
                    )
                ...
                for i, level in enumerate(levels_):
                    props_ = props + (
                        f"left:{i * pixel_size}px; " f"min-width:{pixel_size}px; " f"max-width:{pixel_size}px; "
                    )
                    styles.extend(
                        [
                            {
...
def _validate_apply_axis_arg(
    ...
    if isinstance(arg, Series) and isinstance(data, DataFrame):
        raise ValueError(
            f"'{arg_name}' is a Series but underlying data for operations " f"is a DataFrame since 'axis=None'"
        )
    if isinstance(arg, DataFrame) and isinstance(data, Series):
        raise ValueError(
            f"'{arg_name}' is a DataFrame but underlying data for " f"operations is a Series with 'axis in [0,1]'"
        )
    ...
    with _mpl(Styler.background_gradient) as (_, _matplotlib):
        ...
        def css(rgba, text_only) -> str:
            if not text_only:
                ...
                return f"background-color: {_matplotlib.colors.rgb2hex(rgba)};" + f"color: {text_color};"
            else:
...
def _highlight_between(
    ...
    if inclusive == "both":
        ...
        raise ValueError(f"'inclusive' values can be 'both', 'left', 'right', or 'neither' " f"got {inclusive}")
