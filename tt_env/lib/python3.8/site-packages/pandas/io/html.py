...
class _HtmlFrameParser:
    ...
    def _expand_colspan_rowspan(self, rows, section: Literal["header", "footer", "body"]):
        ...
        text: str | tuple
        remainder: list[tuple[int, str | tuple, int]] = []  # list of (index, text, nrows)
...
def _validate_flavor(flavor):
    if flavor is None:
        ...
        if not all(isinstance(flav, str) for flav in flavor):
            raise TypeError(f"Object of type {repr(type(flavor).__name__)} " f"is not an iterable of strings")
    else:
...
def read_html(
    ...
    **after** `skiprows` is applied.
    ...
    if extract_links not in [None, "header", "footer", "body", "all"]:
        raise ValueError(
            "`extract_links` must be one of " '{None, "header", "footer", "body", "all"}, got ' f'"{extract_links}"'
        )
