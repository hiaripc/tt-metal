...
class CParserWrapper(ParserBase):
    ...
    def __init__(self, src: ReadCsvBuffer[str], **kwds) -> None:
        ...
        if not self._has_complex_date_col:
            # error: Cannot determine type of 'index_col'
            if self._reader.leading_cols == 0 and is_index_col(self.index_col):  # type: ignore[has-type]
    ...
    def read(
        self,
        nrows: int | None = None,
    ) -> tuple[Index | MultiIndex | None, Sequence[Hashable] | MultiIndex, Mapping[Hashable, ArrayLike],]:
...
def _concatenate_chunks(chunks: list[dict[int, ArrayLike]]) -> dict:
    ...
    if warning_columns:
        ...
        warning_message = " ".join(
            [f"Columns ({warning_names}) have mixed types. " f"Specify dtype option on import or set low_memory=False."]
        )
