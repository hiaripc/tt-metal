...
class PythonParser(ParserBase):
    def __init__(self, f: ReadCsvBuffer[str] | list, **kwds) -> None:
        ...
        if self.thousands is None:
            ...
            regex = rf"^[\-\+]?([0-9]+{thousands}|[0-9])*({decimal}[0-9]*)?" rf"([0-9]?(E|e)\-?[0-9]+)?$"
            ...
            for level, hr in enumerate(header):
                try:
                    ...
                    if 0 < self.line_pos <= hr and (not have_mi_columns or hr != header[-1]):
                        ...
                        raise ValueError(f"Passed header={msg}" f"but only {self.line_pos} lines in file") from err
...
class FixedWidthReader(abc.Iterator):
    ...
    def __init__(
        ...
        if not isinstance(self.colspecs, (tuple, list)):
            raise TypeError("column specifications must be a list or tuple, " f"input was a {type(colspecs).__name__}")
