...
def _datetime_to_stata_elapsed_vec(dates: Series, fmt: str) -> Series:
    ...
    def parse_dates_safe(dates, delta: bool = False, year: bool = False, days: bool = False):
        d = {}
        if is_datetime64_dtype(dates.dtype):
            ...
            raise ValueError(
                "Columns containing dates must contain either " "datetime64, datetime.datetime or null values."
            )
    ...
    for col in data:
        ...
        if dtype == np.int8:
            ...
            if np.isinf(data[col]).any():
                raise ValueError(
                    f"Column {col} contains infinity or -infinity" "which is outside the range supported by Stata."
                )
            value = data[col].max()
...
class StataValueLabel:
    ...
    def __init__(self, catarray: Series, encoding: Literal["latin-1", "utf-8"] = "latin-1") -> None:
        ...
        self.value_labels: list[tuple[float, str]] = list(zip(np.arange(len(categories)), categories))
    ...
    def _prepare_value_labels(self):
        ...
        if self.text_len > 32000:
            raise ValueError(
                "Stata value labels for a single variable must " "have a combined length less than 32,000 characters."
            )
...
class StataNonCatValueLabel(StataValueLabel):
    ...
    def __init__(
        ...
        self._encoding = encoding
        self.value_labels: list[tuple[float, str]] = sorted(value_labels.items(), key=lambda x: x[0])
        self._prepare_value_labels()
...
the string values returned are correct."""
    ...
    def read(
        ...
        for i in cols_:
            if self._dtyplist[i] is not None:
                ...
                if dtype != np.dtype(object) and dtype != self._dtyplist[i]:
                    ...
                    data_formatted.append((col, Series(data[col], ix, self._dtyplist[i])))
                else:
        ...
        if convert_dates:
            ...
            for i in cols:
                col = data.columns[i]
                data[col] = _stata_elapsed_date_to_datetime_vec(data[col], self._fmtlist[i])
    ...
    def _do_select_columns(self, data: DataFrame, columns: Sequence[str]) -> DataFrame:
        if not self._column_selector_set:
            ...
            if unmatched:
                ...
                raise ValueError("The following columns were not " f"found in the Stata data set: {joined}")
    ...
    def _prepare_non_cat_value_labels(self, data: DataFrame) -> list[StataNonCatValueLabel]:
        ...
        for labname, labels in self._non_cat_value_labels.items():
            ...
            if not is_numeric_dtype(data[colname].dtype):
                ...
                raise ValueError(
                    f"Can't create value labels for {labname}, value labels " "can only be applied to numeric columns."
                )
        ...
        for col in self.data:
            if col in self._variable_labels:
                ...
                try:
                    ...
                    raise ValueError(
                        "Variable labels must contain only characters that " f"can be encoded in {self._encoding}"
                    ) from err
...
class StataWriterUTF8(StataWriter117):
    ...
    def __init__(
        ...
        *,
        ...
        if version is None:
            ...
            raise ValueError("You must use version 119 for data sets containing more than" "32,767 variables")
    ...
    def _validate_variable_name(self, name: str) -> str:
        ...
        for c in name:
            if (
                (ord(c) < 128 and (c < "A" or c > "Z") and (c < "a" or c > "z") and (c < "0" or c > "9") and c != "_")
                or 128 <= ord(c) < 192
                or c in {"×", "÷"}
            ):
                name = name.replace(c, "_")
