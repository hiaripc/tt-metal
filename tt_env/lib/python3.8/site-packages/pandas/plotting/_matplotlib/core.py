...
class MPLPlot(ABC):
    ...
    def __init__(
        ...
        **kwds,
        ...
        self.errors = {kw: self._parse_errorbars(kw, err) for kw, err in zip(["xerr", "yerr"], [xerr, yerr])}
    ...
    def _validate_subplots_kwarg(self, subplots: bool | Sequence[Sequence[str]]) -> bool | list[tuple[int, ...]]:
        """
        ...
        if self._kind not in supported_kinds:
            raise ValueError(
                "When subplots is an iterable, kind must be " f"one of {', '.join(supported_kinds)}. Got {self._kind}."
            )
    ...
    def _setup_subplots(self):
        ...
        if input_log - valid_log:
            ...
            raise ValueError(f"Boolean, None and 'sym' are valid options, '{invalid_log}' is given.")
    ...
    def result(self):
        ...
        if self.subplots:
            ...
            all_sec = is_list_like(self.secondary_y) and len(self.secondary_y) == self.nseries  # type: ignore[arg-type]
    ...
    def _convert_to_ndarray(self, data):
        ...
        if (is_integer_dtype(data.dtype) or is_float_dtype(data.dtype)) and is_extension_array_dtype(data.dtype):
        ...
        # errors are a column in the dataframe
            ...
            if isinstance(self.data, ABCSeries) and err_shape[0] == 2:
                ...
                if err_shape[2] != len(self.data):
                    raise ValueError(
                        "Asymmetrical error bars should be provided " f"with the shape (2, {len(self.data)})"
                    )
    ...
    def _get_errorbars(self, label=None, index=None, xerr: bool = True, yerr: bool = True):
        errors = {}
    ...
    def _get_subplots(self):
        ...
        return [ax for ax in self.fig.get_axes() if (isinstance(ax, Subplot) and ax.get_subplotspec() is not None)]
...
class LinePlot(MPLPlot):
    ...
    @classmethod
    def _plot(cls, ax: Axes, x, y, style=None, column_num=None, stacking_id=None, **kwds):  # type: ignore[override]
