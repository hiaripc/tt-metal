...
def test_transform_partial_failure(op, request):
    ...
    if op in ("ffill", "bfill", "pad", "backfill", "shift"):
        request.node.add_marker(pytest.mark.xfail(reason=f"{op} is successful on any dtype"))
...
def test_map_empty(request, index):
    if isinstance(index, MultiIndex):
        request.node.add_marker(pytest.mark.xfail(reason="Initializing a Series from a MultiIndex is not supported"))
