...
class TestAstype:
    def test_astype(self):
        ...
        expected = SparseArray._simple_new(np.array([0.0, 2.0], dtype=dtype.subtype), IntIndex(4, [2, 3]), dtype)
        tm.assert_sp_array_equal(result, expected)

        dtype = SparseDtype("int64", 0)
        result = arr.astype(dtype)
        expected = SparseArray._simple_new(np.array([0, 2], dtype=np.int64), IntIndex(4, [2, 3]), dtype)
        tm.assert_sp_array_equal(result, expected)
...
def test_dtype_sparse_with_fill_value_not_present_in_data():
    ...
    expected = Series(["a", "b", "b"], name="A", dtype=SparseDtype("object", fill_value="c"))
