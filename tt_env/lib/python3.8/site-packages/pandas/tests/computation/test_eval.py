        ...
        pytest.param(
            ...
            marks=[
                pytest.mark.skipif(
                    ...
                    reason=f"numexpr enabled->{USE_NUMEXPR}, " f"installed->{NUMEXPR_INSTALLED}",
                ),
                td.skip_if_no_ne,
            ],
        )
...
class TestEval:
    ...
    def test_floor_division(self, lhs, rhs, engine, parser):
        ...
        if engine == "python":
            ...
            msg = r"unsupported operand type\(s\) for //: 'VariableNode' and " "'VariableNode'"
            with pytest.raises(TypeError, match=msg):
                pd.eval(
                    ex,
                    local_dict={"lhs": lhs, "rhs": rhs},
    ...
    def test_unary_in_array(self):
        ...
        result = np.array(
            pd.eval("[-True, True, ~True, +True," "-False, False, ~False, +False," "-37, 37, ~37, +37]"),
            dtype=np.object_,
        )
        expected = np.array(
            [
...
def should_warn(*args):
    ...
    only_one_dt = reduce(operator.xor, map(lambda x: issubclass(x.dtype.type, np.datetime64), args))
...
class TestAlignment:
    ...
    def test_basic_frame_alignment(self, engine, parser, lr_idx_type, rr_idx_type, c_idx_type):
    ...
    @pytest.mark.parametrize("c_idx_type", index_types)
    @pytest.mark.parametrize("r_idx_type", lhs_index_types)
    def test_basic_frame_series_alignment(self, engine, parser, index_name, r_idx_type, c_idx_type):
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always", RuntimeWarning)
            df = tm.makeCustomDataframe(10, 10, data_gen_f=f, r_idx_type=r_idx_type, c_idx_type=c_idx_type)
    ...
    def test_basic_series_frame_alignment(self, request, engine, parser, index_name, r_idx_type, c_idx_type):
        if (
            engine == "numexpr"
        ...
        df = tm.makeCustomDataframe(10, 7, data_gen_f=f, r_idx_type=r_idx_type, c_idx_type=c_idx_type)
        index = getattr(df, index_name)
        s = Series(np.random.randn(5), index[:5])
    ...
    @pytest.mark.parametrize("index_name", ["index", "columns"])
    @pytest.mark.parametrize("op", ["+", "*"])
    def test_series_frame_commutativity(self, engine, parser, index_name, op, r_idx_type, c_idx_type):
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always", RuntimeWarning)
...
class TestOperations:
    ...
    def test_simple_arith_ops(self, engine, parser):
        ...
        arith_ops = [op for op in expr.ARITH_OPS_SYMS + expr.CMP_OPS_SYMS if op not in exclude_arith]
...
def test_numexpr_option_incompatible_op():
    ...
    with pd.option_context("compute.use_numexpr", False):
        df = DataFrame({"A": [True, False, True, False, None, None], "B": [1, 2, 3, 4, 5, 6]})
