    ...
    if request.param == "numpy":
        ...
        def make_dataframe(*args, **kwargs):
            ...
            for col in df.columns:
                if is_float_dtype(df[col].dtype) and not is_float_dtype(df_nullable[col].dtype):
...
def test_subset_loc_rows_columns(
    ...
    if (
        ...
        and (using_array_manager or (dtype == "int64" and dtype_backend == "numpy" and not using_copy_on_write))
    ):
        df_orig.iloc[1, 1] = 0
    tm.assert_frame_equal(df, df_orig)
...
def test_subset_set_with_mask(backend, using_copy_on_write):
    ...
    expected = DataFrame({"a": [2, 3, 0], "b": [0, 0, 0], "c": [0.20, 0.3, 0.4]}, index=range(1, 4))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        # original parent dataframe is not modified (CoW)
        tm.assert_frame_equal(df, df_orig)
...
@pytest.mark.parametrize("dtype", ["int64", "float64"], ids=["single-block", "mixed-block"])
def test_subset_set_column_with_loc(backend, using_copy_on_write, using_array_manager, dtype):
...
@pytest.mark.parametrize("dtype", ["int64", "float64"], ids=["single-block", "mixed-block"])
def test_subset_chained_getitem(request, backend, method, dtype, using_copy_on_write, using_array_manager):
    ...
    if not using_array_manager:
        ...
        subset_is_view = test_callspec.endswith("column-iloc-slice") or test_callspec.endswith("column-loc-slice")
...
def test_series_subset_set_with_indexer(backend, indexer_si, indexer, using_copy_on_write):
...
def test_column_as_series_set_with_upcast(backend, using_copy_on_write, using_array_manager):
...
def test_column_as_series_no_item_cache(request, backend, method, using_copy_on_write, using_array_manager):
...
def test_set_value_copy_only_necessary_column(using_copy_on_write, indexer_func, indexer, val):
...
def test_series_midx_slice(using_copy_on_write):
    ...
    result.iloc[0] = 100
    if using_copy_on_write:
        expected = Series([1, 2, 3], index=pd.MultiIndex.from_arrays([[1, 1, 2], [3, 4, 5]]))
