...
class BaseArithmeticOpsTests(BaseOpsUtil):
    ...
    * divmod_exc = TypeError
    ...
    def test_arith_series_with_array(self, data, all_arithmetic_operators):
        ...
        op_name = all_arithmetic_operators
        ser = pd.Series(data)
        self.check_opname(ser, op_name, pd.Series([ser.iloc[0]] * len(ser)), exc=self.series_array_exc)
    ...
    def test_direct_arith_with_ndframe_returns_not_implemented(self, request, data, box):
        ...
        if not hasattr(data, "__add__"):
            request.node.add_marker(pytest.mark.xfail(reason=f"{type(data).__name__} does not implement add"))
