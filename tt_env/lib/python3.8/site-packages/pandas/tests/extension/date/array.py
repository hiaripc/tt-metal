...
class DateArray(ExtensionArray):
    def __init__(
        ...
        if isinstance(dates, list):
            ...
            # populate them
            for i, (y, m, d) in enumerate(map(lambda date: (date.year, date.month, date.day), dates)):
                self._year[i] = y
                self._month[i] = m
                self._day[i] = d
            ...
            if not ly == lm == ld:
                raise ValueError(f"tuple members must have the same length: {(ly, lm, ld)}")
    ...
    def isna(self) -> np.ndarray:
        return np.logical_and(
            np.logical_and(self._year == dt.date.min.year, self._month == dt.date.min.month),
