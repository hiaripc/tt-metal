...
def data(dtype):
    ...
    if pa.types.is_boolean(pa_dtype):
        ...
        data = (
            [time(12, 0), time(0, 12)] * 4 + [None] + [time(0, 0), time(1, 1)] * 44 + [None] + [time(0, 5), time(5, 0)]
        )
...
class TestBaseAccumulateTests(base.BaseAccumulateTests):
    ...
    def test_accumulate_series_raises(self, data, all_numeric_accumulations, skipna):
        ...
        if (
            (pa.types.is_integer(pa_type) or pa.types.is_floating(pa_type) or pa.types.is_duration(pa_type))
    ...
    def test_accumulate_series(self, data, all_numeric_accumulations, skipna, request):
        ...
        if do_skip:
            pytest.skip(
                "These should *not* work, we test in test_accumulate_series_raises " "that these correctly raise."
            )
...
class TestBaseNumericReduce(base.BaseNumericReduceTests):
    def check_reduce(self, ser, op_name, skipna):
        ...
        if pa.types.is_boolean(pa_dtype):
            ...
            pytest.skip("pandas boolean data with NA does not fully support all reductions")
    ...
    def test_reduce_series(self, data, all_numeric_reductions, skipna, request):
        ...
        if pa.types.is_temporal(pa_dtype) and opname in [
            ...
            if pa.types.is_duration(pa_dtype) and opname in ["sum"]:
                ...
                should_work = False
        elif (pa.types.is_string(pa_dtype) or pa.types.is_binary(pa_dtype)) and opname in [
            "sum",
            "mean",
            "median",
            "prod",
        ...
        xfail_mark = pytest.mark.xfail(
            raises=TypeError,
            reason=(f"{all_numeric_reductions} is not implemented in " f"pyarrow={pa.__version__} for {pa_dtype}"),
        )
        if all_numeric_reductions in {"skew", "kurt"}:
            request.node.add_marker(xfail_mark)
        elif (
            all_numeric_reductions in {"var", "std", "median"} and pa_version_under7p0 and pa.types.is_decimal(pa_dtype)
        ):
            request.node.add_marker(xfail_mark)
...
class TestBaseBooleanReduce(base.BaseBooleanReduceTests):
    @pytest.mark.parametrize("skipna", [True, False])
    def test_reduce_series(self, data, all_boolean_reductions, skipna, na_value, request):
        pa_dtype = data.dtype.pyarrow_dtype
        xfail_mark = pytest.mark.xfail(
            raises=TypeError,
            reason=(f"{all_boolean_reductions} is not implemented in " f"pyarrow={pa.__version__} for {pa_dtype}"),
        )
...
class TestBaseDtype(base.BaseDtypeTests):
    ...
    def test_get_common_dtype(self, dtype, request):
        ...
        if (
            ...
            or (pa.types.is_timestamp(pa_dtype) and (pa_dtype.unit != "ns" or pa_dtype.tz is not None))
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=(f"{pa_dtype} does not have associated numpy " f"dtype findable by find_common_type")
                )
            )
...
class TestBaseMissing(base.BaseMissingTests):
    ...
    def test_fillna_series_method(self, data_missing, fillna_method):
        with tm.maybe_produces_warning(PerformanceWarning, fillna_method is not None, check_stacklevel=False):
...
class TestBaseParsing(base.BaseParsingTests):
    ...
    def test_EA_types(self, engine, data, dtype_backend, request):
        ...
        if pa.types.is_decimal(pa_dtype):
            ...
            request.node.add_marker(pytest.mark.xfail(reason="CSV parsers don't correctly handle binary"))
        elif pa.types.is_duration(pa_dtype) and dtype_backend == "pyarrow" and engine == "python":
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=TypeError,
...
class TestBaseMethods(base.BaseMethodsTests):
    ...
    def test_argmin_argmax(self, data_for_sorting, data_missing_for_sorting, na_value, request):
    ...
    def test_argreduce_series(self, data_missing_for_sorting, op_name, skipna, expected, request):
        ...
        super().test_argreduce_series(data_missing_for_sorting, op_name, skipna, expected)
...
class TestBaseArithmeticOps(base.BaseArithmeticOpsTests):
    ...
    def _get_scalar_exception(self, opname, pa_dtype):
        ...
        if opname in {
            ...
            exc = None
        elif opname in ["__add__", "__radd__"] and (pa.types.is_string(pa_dtype) or pa.types.is_binary(pa_dtype)):
            exc = None
    ...
    def _get_arith_xfail_marker(self, opname, pa_dtype):
        ...
        if (
            ...
            mark = pytest.mark.xfail(
                reason=(f"GH#29997: 1**pandas.NA == 1 while 1**pyarrow.NA == NULL " f"for {pa_dtype}")
            )
        elif arrow_temporal_supported:
            mark = pytest.mark.xfail(
                raises=TypeError,
                reason=(f"{opname} not supported between" f"pd.NA and {pa_dtype} Python scalar"),
            )
        elif (
        ...
        elif opname == "__rtruediv__" and pa.types.is_decimal(pa_dtype) and not pa_version_under7p0:
            mark = pytest.mark.xfail(
                raises=pa.ArrowInvalid,
                reason="divide by 0",
            )
        elif opname == "__pow__" and pa.types.is_decimal(pa_dtype) and pa_version_under7p0:
            mark = pytest.mark.xfail(
                raises=pa.ArrowInvalid,
    ...
    def test_arith_series_with_scalar(self, data, all_arithmetic_operators, request, monkeypatch):
        pa_dtype = data.dtype.pyarrow_dtype

        if all_arithmetic_operators == "__rmod__" and (pa.types.is_string(pa_dtype) or pa.types.is_binary(pa_dtype)):
            pytest.skip("Skip testing Python string formatting")

        self.series_scalar_exc = self._get_scalar_exception(all_arithmetic_operators, pa_dtype)
        ...
        if (
            (all_arithmetic_operators == "__floordiv__" and pa.types.is_integer(pa_dtype))
    ...
    def test_arith_frame_with_scalar(self, data, all_arithmetic_operators, request, monkeypatch):
        pa_dtype = data.dtype.pyarrow_dtype

        if all_arithmetic_operators == "__rmod__" and (pa.types.is_string(pa_dtype) or pa.types.is_binary(pa_dtype)):
            pytest.skip("Skip testing Python string formatting")

        self.frame_scalar_exc = self._get_scalar_exception(all_arithmetic_operators, pa_dtype)
    ...
    def test_arith_series_with_array(self, data, all_arithmetic_operators, request, monkeypatch):
        pa_dtype = data.dtype.pyarrow_dtype

        self.series_array_exc = self._get_scalar_exception(all_arithmetic_operators, pa_dtype)
        ...
        if (
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=pa.ArrowInvalid,
                    reason=(
                        f"Implemented pyarrow.compute.subtract_checked " f"which raises on overflow for {pa_dtype}"
                    ),
                )
            )
        ...
        if (
            ...
            or (pa.types.is_integer(pa_dtype) and all_arithmetic_operators not in ["__truediv__", "__rtruediv__"])
            or pa.types.is_duration(pa_dtype)
            or pa.types.is_timestamp(pa_dtype)
        ):
            monkeypatch.setattr(TestBaseArithmeticOps, "_combine", self._patch_combine)
    ...
    def test_add_series_with_extension_array(self, data, request):
        ...
        if (pa_version_under8p0 and pa.types.is_duration(pa_dtype)) or (pa.types.is_boolean(pa_dtype)):
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=NotImplementedError,
...
def test_arrowdtype_construct_from_string_type_only_one_pyarrow():
    ...
    msg = r"Passing pyarrow type specific parameters \(\[pyarrow\]\) in the " r"string is not supported\."
    with pytest.raises(NotImplementedError, match=msg):
...
def test_setitem_null_slice(data):
    ...
    result = orig.copy()
    result[:] = data[0]
    expected = ArrowExtensionArray(pa.array([data[0]] * len(data), type=data._data.type))
...
def test_str_partition():
    ...
    expected = pd.DataFrame([["a", "b", "cba"], [None, None, None]], dtype=ArrowDtype(pa.string()))
    tm.assert_frame_equal(result, expected)
    ...
    expected = pd.DataFrame([["abc", "b", "a"], [None, None, None]], dtype=ArrowDtype(pa.string()))
    tm.assert_frame_equal(result, expected)
...
def test_str_unsupported_extract():
    ser = pd.Series(["abc", None], dtype=ArrowDtype(pa.string()))
    with pytest.raises(NotImplementedError, match="str.extract not supported with pd.ArrowDtype"):
...
@pytest.mark.xfail(pa_version_under7p0, reason="Methods not supported for pyarrow < 7.0")
...
def test_describe_datetime_data(pa_type):
    ...
    result = data.describe()
    expected = pd.Series(
        [9] + [pd.Timestamp(v, tz=pa_type.tz, unit=pa_type.unit) for v in [5, 1, 3, 5, 7, 9]],
        dtype=object,
