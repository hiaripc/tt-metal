...
class TestDataFrameIndexing:
    ...
    def test_setitem_list(self, float_frame):
        ...
        msg = rf"Length of values \({len(newcolumndata)}\) " rf"does not match length of index \({len(data)}\)"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_getitem_boolean_casting(self, datetime_frame):
        ...
        expected = Series(
            [np.dtype("float64")] * 4 + [np.dtype("int32")] * 2 + [np.dtype("int64")] * 2,
    ...
    def test_fancy_getitem_slice_mixed(self, float_frame, float_string_frame, using_copy_on_write):
    ...
    def test_getitem_setitem_float_labels(self, using_array_manager):
        ...
        msg = "cannot do positional indexing on Index with " r"these indexers \[1.0\] of type float"
        with pytest.raises(TypeError, match=msg):
            df.iloc[1.0:5]
    ...
    def test_loc_datetime_assignment_dtype_does_not_change(self, utc, indexer):
        ...
        df = DataFrame(
            {
                "date": to_datetime([datetime(2022, 1, 20), datetime(2022, 1, 22)], utc=utc),
                "update": [True, False],
            }
        )
