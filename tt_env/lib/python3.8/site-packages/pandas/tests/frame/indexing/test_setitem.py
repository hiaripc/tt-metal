...
class TestDataFrameSetItem:
    ...
    def test_setitem_wrong_length_categorical_dtype_raises(self):
        ...
        msg = rf"Length of values \({len(cat)}\) " rf"does not match length of index \({len(df)}\)"
        with pytest.raises(ValueError, match=msg):
            df["foo"] = cat
    ...
    def test_setitem_categorical(self):
        ...
        expected = DataFrame({"h": Categorical(["m", "n"]).reorder_categories(["n", "m"])})
        tm.assert_frame_equal(df, expected)
...
class TestDataFrameSetItemBooleanMask:
    ...
    def test_setitem_boolean_indexing(self):
        ...
        df1 = DataFrame(
            index=idx,
            columns=cols,
            data=np.array([[0.0, 0.5, 1.0], [1.5, 2.0, 2.5], [3.0, 3.5, 4.0]], dtype=float),
        )
