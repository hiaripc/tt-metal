...
class TestDataFrameQuantile:
    ...
    def test_quantile(self, datetime_frame, interp_method, using_array_manager, request):
        ...
        if interpolation == "linear":
            ...
            request.node.add_marker(
                pytest.mark.xfail(using_array_manager, reason="Name set incorrectly for arraymanager")
            )
    ...
    def test_non_numeric_exclusion(self, interp_method, request, using_array_manager):
        ...
        if method == "table" and using_array_manager:
            request.node.add_marker(pytest.mark.xfail(reason="Axis name incorrectly set."))
    ...
    def test_quantile_datetime(self):
        ...
        # datetime w/ multi
        result = df.quantile([0.5], numeric_only=False)
        expected = DataFrame([[Timestamp("2010-07-02 12:00:00"), 2.5]], index=[0.5], columns=["a", "b"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_quantile_nat(self, interp_method, request, using_array_manager):
        ...
        res = df.quantile([0.5], numeric_only=False, interpolation=interpolation, method=method)
        exp = DataFrame([[Timestamp("2012-01-02"), pd.NaT]], index=[0.5], columns=["a", "b"])
        tm.assert_frame_equal(res, exp)
    ...
    def test_quantile_item_cache(self, using_array_manager, interp_method, using_copy_on_write):
