...
class TestDataFrameReplace:
    ...
    def test_regex_replace_list_obj(self, to_replace, values, expected, inplace, use_value_regex_args):
    ...
    def test_regex_replace_dict_mixed(self, mix_abc):
        ...
        res2 = dfmix.copy()
        return_value = res2.replace({"b": r"\s*\.\s*"}, {"b": np.nan}, inplace=True, regex=True)
        assert return_value is None
        ...
        res2 = dfmix.copy()
        return_value = res2.replace({"b": r"\s*(\.)\s*"}, {"b": r"\1ty"}, inplace=True, regex=True)
        assert return_value is None
    ...
    def test_regex_replace_dict_nested(self, mix_abc):
        ...
        res2 = dfmix.copy()
        res4 = dfmix.copy()
        return_value = res2.replace({"b": {r"\s*\.\s*": np.nan}}, inplace=True, regex=True)
        assert return_value is None
    ...
    def test_regex_replace_string_types(self, data, to_replace, expected, frame_or_series, any_string_dtype):
        # GH-41333, GH-35977
        dtype = any_string_dtype
    ...
    def test_categorical_replace_with_dict(self, replace_dict, final_data):
        ...
        tm.assert_frame_equal(result, expected)
        msg = r"Attributes of DataFrame.iloc\[:, 0\] \(column name=\"a\"\) are " "different"
...
class TestDataFrameReplaceRegex:
    ...
    @pytest.mark.parametrize("to_replace,value", [(r"\s*\.\s*", np.nan), (r"\s*(\.)\s*", r"\1\1\1")])
