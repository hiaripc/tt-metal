...
class TestResetIndex:
    ...
    def test_reset_index_multiindex_columns(self, multiindex_df):
        ...
        with pytest.raises(
            ValueError,
            match=("col_fill=None is incompatible with " r"incomplete column name \('C', 'c'\)"),
        ):
    ...
    def test_reset_index_datetime(self, tz_naive_fixture):
        ...
        expected["idx1"] = expected["idx1"].apply(lambda d: Timestamp(d, tz=tz))
        expected["idx3"] = expected["idx3"].apply(lambda d: Timestamp(d, tz="Europe/Paris"))
        tm.assert_frame_equal(df.reset_index(), expected)

        # GH#7793
        idx = MultiIndex.from_product([["a", "b"], date_range("20130101", periods=3, tz=tz)])
