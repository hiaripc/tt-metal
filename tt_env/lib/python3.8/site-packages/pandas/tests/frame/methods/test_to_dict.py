...
class TestDataFrameToDict:
    ...
    def test_to_dict_returns_native_types(self, orient, data, expected_types):
        ...
        if orient == "dict":
            assertion_iterator = (
                (i, key, value) for key, index_value_map in result.items() for i, value in index_value_map.items()
            )
        elif orient == "list":
            assertion_iterator = ((i, key, value) for key, values in result.items() for i, value in enumerate(values))
        elif orient in {"split", "tight"}:
            assertion_iterator = (
                (i, key, result["data"][i][j]) for i in result["index"] for j, key in enumerate(result["columns"])
            )
        elif orient == "records":
