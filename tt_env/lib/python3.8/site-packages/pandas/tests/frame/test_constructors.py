...
class TestDataFrameConstructors:
    ...
    def test_constructor_dtype_nocast_view_2d_array(self, using_array_manager, using_copy_on_write):
    ...
    def test_constructor_with_datetimes(self):
        ...
        result = df.dtypes
        expected = Series(
            [np.dtype("int64")] + [np.dtype(objectname)] * 2 + [np.dtype(datetime64name)] * 2,
            index=list("ABCDE"),
        )
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize(
        "dtype",
        tm.ALL_NUMERIC_DTYPES + tm.DATETIME64_DTYPES + tm.TIMEDELTA64_DTYPES + tm.BOOL_DTYPES,
    )
