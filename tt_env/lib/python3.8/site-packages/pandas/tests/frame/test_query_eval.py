...
class TestDataFrameEval:
    ...
    def test_ops(self, op_str, op, rop, n):
        ...
        base = DataFrame(np.tile(m.values, n).reshape(n, -1), columns=list("abcd"))  # noqa:F841
...
class TestDataFrameQueryNumExprPython(TestDataFrameQueryNumExprPandas):
    ...
    def test_query_numexpr_with_min_and_max_columns(self):
        ...
        regex_to_match = r"Variables in expression \"\(min\) == \(1\)\" " r"overlap with builtins: \('min'\)"
        with pytest.raises(NumExprClobberingError, match=regex_to_match):
            df.query("min == 1")

        regex_to_match = r"Variables in expression \"\(max\) == \(1\)\" " r"overlap with builtins: \('max'\)"
        with pytest.raises(NumExprClobberingError, match=regex_to_match):
            df.query("max == 1")
