...
class TestDataFrameAnalytics:
    ...
    def test_std_datetime64_with_nat(self, values, skipna, using_array_manager, request):
        # GH#51335
...
class TestDataFrameReductions:
    ...
    def test_reductions_skipna_none_raises(self, request, frame_or_series, all_reductions):
        if all_reductions == "count":
            request.node.add_marker(pytest.mark.xfail(reason="Count does not accept skipna"))
        ...
        df = DataFrame(
            {
                ...
                "b": Series([Timestamp("2019-12-31 00:00:00.123")], dtype="datetime64[ms]"),
            }
        )
        result = df.max()
        expected = Series(
...
def test_minmax_extensionarray(method, numeric_only):
    ...
    result = getattr(df, method)(numeric_only=numeric_only)
    expected = Series([getattr(int64_info, method)], index=Index(["Int64"], dtype="object"))
    tm.assert_series_equal(result, expected)
