...
class TestDataFrameSubclassing:
    ...
    def test_subclass_stack_multi(self):
        ...
        df = tm.SubclassedDataFrame(
            ...
            index=MultiIndex.from_tuples(list(zip(list("AABB"), list("cdcd"))), names=["aaa", "ccc"]),
            columns=MultiIndex.from_tuples(list(zip(list("WWXX"), list("yzyz"))), names=["www", "yyy"]),
        )
    ...
    def test_corrwith(self):
        ...
        df1 = tm.SubclassedDataFrame(np.random.randn(5, 4), index=index, columns=columns)
        df2 = tm.SubclassedDataFrame(np.random.randn(4, 4), index=index[:4], columns=columns)
