...
def test_unary_binary(request, dtype):
    ...
    if is_extension_array_dtype(dtype) or isinstance(dtype, dict):
        request.node.add_marker(pytest.mark.xfail(reason="Extension / mixed with multiple outputs not implemented."))
...
def test_binary_input_aligns_columns(request, dtype_a, dtype_b):
    if (
        ...
        request.node.add_marker(pytest.mark.xfail(reason="Extension / mixed with multiple inputs not implemented."))
...
def test_alignment_deprecation_many_inputs_enforced():
    ...
    # mixed frame / arrays
    msg = r"operands could not be broadcast together with shapes \(3,3\) \(3,3\) \(3,2\)"
    ...
    msg = r"operands could not be broadcast together with shapes \(3,2\) \(3,3\) \(3,3\)"
    with pytest.raises(ValueError, match=msg):
