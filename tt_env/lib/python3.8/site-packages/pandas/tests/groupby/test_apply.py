...
2011.05.18,05:00,1.40649"""
    ...
    exp_idx = Index(["2011.05.16", "2011.05.17", "2011.05.18"], dtype=object, name="date")
...
def test_apply_trivial():
    ...
    result = df.groupby([str(x) for x in df.dtypes], axis=1).apply(lambda x: df.iloc[1:])
...
def test_apply_trivial_fail():
    ...
    result = df.groupby([str(x) for x in df.dtypes], axis=1, group_keys=True).apply(lambda x: df)
...
def test_apply_aggregating_timedelta_and_datetime():
    ...
    result = df.groupby("clientid").apply(
        lambda ddf: Series({"clientid_age": ddf.time_delta_zero.min(), "date": ddf.datetime.min()})
    )
    expected = DataFrame(
        {
            "clientid": ["A", "B", "C"],
...
def test_groupby_apply_return_empty_chunk():
    ...
    expected = Series(
        [0],
        name="value",
        index=MultiIndex.from_product([["empty", "filled"], [0]], names=["group", None]).drop("empty"),
    )
    tm.assert_series_equal(result, expected)
...
def test_apply_datetime_issue(group_column_dtlike):
    ...
    expected = DataFrame(["spam"], Index(["foo"], dtype="object", name="a"), columns=[42])
    tm.assert_frame_equal(result, expected)
...
def test_apply_by_cols_equals_apply_by_rows_transposed():
    ...
    by_rows = df.T.groupby(axis=0, level=0).apply(lambda x: x.droplevel(axis=0, level=0))
