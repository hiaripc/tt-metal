    ...
    df = DataFrame(
        {
            ...
            "key2": Categorical(list(pd.date_range("2018-06-01 00", freq="1T", periods=3)) * 3),
            "values": np.arange(9),
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            Series(
                index=MultiIndex.from_arrays([Series([1, 1, 2], dtype="category"), [1, 2, 2]], names=["a", "b"]),
                data=[1, 2, 3],
                name="x",
            ),
        ),
...
@pytest.mark.parametrize("func", ["first", "last"])
def test_df_groupby_first_on_categorical_col_grouped_on_2_categoricals(func: str, observed: bool):
    # GH 34951
    cat = Categorical([0, 0, 1, 1])
    val = [0, 1, 1, 0]
    df = DataFrame({"a": cat, "b": cat, "c": val})
...
def test_groupby_categorical_observed_nunique():
    ...
    expected = Series(
        [1, 1],
        index=MultiIndex.from_arrays([CategoricalIndex([1, 2], name="a"), CategoricalIndex([1, 2], name="b")]),
        name="c",
    )
    tm.assert_series_equal(result, expected)
...
def test_groupby_categorical_aggregate_functions():
    ...
    df = DataFrame([[1, "small"], [1, "big"], [2, "small"]], columns=["grp", "description"]).astype(
        {"description": dtype}
    )
...
def test_category_order_reducer(request, as_index, sort, observed, reduction_func, index_kind, ordered):
    # GH#48749
    if reduction_func in ("idxmax", "idxmin") and not observed and index_kind != "multi":
...
def test_agg_list(request, as_index, observed, reduction_func, test_series, keys):
    ...
    if not test_series:
        ...
        expected.columns = MultiIndex(levels=[["b"], [reduction_func]], codes=[[0], [0]])
    elif not as_index:
