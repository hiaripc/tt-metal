...
def test_basic(dtype):
    ...
    tm.assert_series_equal(value_grouped.aggregate(np.mean), agged, check_index_type=False)
...
def test_groupby_with_hier_columns():
    ...
    columns = MultiIndex.from_tuples([("A", "cat"), ("B", "dog"), ("B", "cat"), ("A", "dog")])
...
def test_groupby_sort_multi():
    ...
    df = DataFrame({"a": [0, 1, 2, 0, 1, 2], "b": [0, 0, 0, 1, 1, 1], "d": np.random.randn(6)})
...
def test_groupby_multiindex_not_lexsorted():
    ...
    # GH 14776
    df = DataFrame({"x": ["a", "a", "b", "a"], "y": [1, 1, 2, 2], "z": [1, 2, 3, 4]}).set_index(["x", "y"])
...
def test_empty_groupby(columns, keys, values, method, op, request, using_array_manager, dropna):
    # GH8093 & GH26411
    override_dtype = None

    if isinstance(values, Categorical) and len(keys) == 1 and op in ["idxmax", "idxmin"]:
        mark = pytest.mark.xfail(raises=ValueError, match="attempt to get arg(min|max) of an empty sequence")
...
def test_groupby_numerical_stability_cumsum():
    ...
    exp_data = [1e16] * 2 + [1e16 + 96, 1e16 + 98] + [5e15 + 97, 5e15 + 98] + [97.0, 98.0]
