...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "string[pyarrow]",
            marks=pytest.mark.skipif(pa_version_under7p0, reason="pyarrow is not installed"),
        ),
        "datetime64[ns]",
        "period[d]",
        "Sparse[float]",
...
def test_categorical_reducers(request, reduction_func, observed, sort, as_index, index_kind):
    # GH#36327
    if reduction_func in ("idxmin", "idxmax") and not observed and index_kind != "multi":
