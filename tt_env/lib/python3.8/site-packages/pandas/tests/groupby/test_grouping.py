...
class TestSelection:
    ...
    def test_indices_grouped_by_tuple_with_lambda(self):
        # GH 36158
        df = DataFrame({"Tuples": ((x, y) for x in [0, 1] for y in np.random.randint(3, 5, 5))})
...
class TestGrouping:
    ...
    def test_grouper_getting_correct_binner(self):
        ...
        df = DataFrame(
            {"A": 1},
            index=MultiIndex.from_product([list("ab"), date_range("20130101", periods=80)], names=["one", "two"]),
        )
        result = df.groupby([Grouper(level="one"), Grouper(level="two", freq="M")]).sum()
        expected = DataFrame(
            {"A": [31, 28, 21, 31, 28, 21]},
            index=MultiIndex.from_product(
