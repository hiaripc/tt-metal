...
def multiindex_data():
    ...
    for date in dates:
        ...
        levels = [
            (item, random.randint(0, 10000) / 100, random.randint(0, 10000) / 100) for item in items[:nitems_for_date]
        ]
...
def test_against_head_and_tail(arg, method, simulated):
    ...
    data = {
        ...
        "value": [f"group {g} row {j}" for j in range(n_rows_per_group) for g in range(n_groups)],
    }
