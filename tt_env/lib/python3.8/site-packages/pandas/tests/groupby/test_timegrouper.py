...
class TestGroupBy:
    def test_groupby_with_timegrouper(self):
        ...
        for df in [df_original, df_reordered]:
            ...
            expected = DataFrame(
                {"Buyer": 0, "Quantity": 0},
                index=date_range("20130901", "20131205", freq="5D", name="Date", inclusive="left"),
            )
    ...
    def test_timegrouper_with_reg_groups(self):
        ...
        for df in [df_original, df_sorted]:
            ...
            with pytest.raises(ValueError, match=msg):
                df.groupby([Grouper(freq="1M", key="Date", level="Date"), "Buyer"]).sum()

            # single groupers
            expected = DataFrame(
                [[31]],
                columns=["Quantity"],
                index=DatetimeIndex([datetime(2013, 10, 31, 0, 0)], freq=offsets.MonthEnd(), name="Date"),
            )
...
4,2000-01-01 16:50:00,America/New_York"""
    ...
    def test_datetime_count(self):
        df = DataFrame({"a": [1, 2, 3] * 2, "dates": date_range("now", periods=6, freq="T")})
    ...
    def test_grouper_period_index(self):
        ...
        expected = Series(range(0, periods), index=Index(range(1, periods + 1), name=index.name))
        tm.assert_series_equal(result, expected)
