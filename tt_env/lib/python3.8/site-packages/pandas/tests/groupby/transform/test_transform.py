...
def test_transform_axis_ts(tsframe):
    ...
    r = len(base.index)
    c = len(base.columns)
    tso = DataFrame(np.random.randn(r, c), index=base.index, columns=base.columns, dtype="float64")
    # monotonic
    ts = tso
...
def test_transform_casting():
    ...
    """
    df = pd.read_csv(StringIO(data), sep=r"\s+", index_col=[0], parse_dates=["DATETIME"])
...
def test_transform_with_non_scalar_group():
    ...
    df = DataFrame(np.random.randint(1, 10, (4, 12)), columns=cols, index=["A", "C", "G", "T"])
...
def test_transform_agg_by_name(request, reduction_func, frame_or_series):
    ...
    if func == "corrwith" and isinstance(obj, Series):  # GH#32293
        request.node.add_marker(pytest.mark.xfail(reason="TODO: implement SeriesGroupBy.corrwith"))
