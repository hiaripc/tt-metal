...
def test_constructor_single_level():
    result = MultiIndex(levels=[["foo", "bar", "baz", "qux"]], codes=[[0, 1, 2, 3]], names=["first"])
...
def test_constructor_mismatched_codes_levels(idx):
    ...
    length_error = (
        r"On level 0, code max \(3\) >= length of level \(1\)\. " "NOTE: this index is in an inconsistent state"
    )
...
def test_na_levels():
    ...
    result = MultiIndex(levels=[[1, 2, 3, 4, 5]], codes=[[0, -1, 1, 2, 3, 4]]).set_levels(
        [[np.nan, "s", pd.NaT, 128, None]]
    )
    tm.assert_index_equal(result, expected)

    result = MultiIndex(levels=[[np.nan, "s", pd.NaT, 128, None]], codes=[[1, 2, 2, 2, 2, 2]]).set_codes(
        [[0, -1, 1, 2, 3, 4]]
    )
    tm.assert_index_equal(result, expected)
...
@pytest.mark.parametrize("first, second", [([], []), (["foo", "bar", "baz"], []), ([], ["a", "b", "c"])])
...
def test_dtype_representation():
    ...
    result = pmidx.dtypes
    expected = Series(["int64", "object"], index=MultiIndex.from_tuples([("a", "b"), ("c", "d")]))
