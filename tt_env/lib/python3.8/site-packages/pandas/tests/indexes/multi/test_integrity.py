...
def test_hash_collisions():
    ...
    index = MultiIndex.from_product([np.arange(1000), np.arange(1000)], names=["one", "two"])
...
def test_large_multiindex_error():
    # GH12527
    df_below_1000000 = pd.DataFrame(1, index=MultiIndex.from_product([[1, 2], range(499999)]), columns=["dest"])
    with pytest.raises(KeyError, match=r"^\(-1, 0\)$"):
        df_below_1000000.loc[(-1, 0), "dest"]
    with pytest.raises(KeyError, match=r"^\(3, 0\)$"):
        df_below_1000000.loc[(3, 0), "dest"]
    df_above_1000000 = pd.DataFrame(1, index=MultiIndex.from_product([[1, 2], range(500001)]), columns=["dest"])
    with pytest.raises(KeyError, match=r"^\(-1, 0\)$"):
...
def test_million_record_attribute_error():
    ...
    df = pd.DataFrame({"a": r, "b": r}, index=MultiIndex.from_tuples([(x, x) for x in r]))
