...
class TestIndex(Base):
    ...
    def test_isin_nan_common_float64(self, nulls_fixture, float_numpy_dtype):
        ...
        if nulls_fixture is pd.NaT or nulls_fixture is pd.NA:
            ...
            msg = (
                r"float\(\) argument must be a string or a (real )?number, " f"not {repr(type(nulls_fixture).__name__)}"
            )
            with pytest.raises(TypeError, match=msg):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                True,
                MultiIndex.from_tuples([("a", "b", "c"), ("d", "e", np.nan), ("f", np.nan, np.nan)]),
            ),
        ],
    )
    ...
    def test_reindex_doesnt_preserve_type_if_target_is_empty_index_numeric(self, any_real_numpy_dtype):
        # GH7774
