...
def test_union_different_types(index_flat, index_flat2, request):
    ...
    if not len(idx1) or not len(idx2):
        ...
        idx1.dtype.kind == "c" and (idx2.dtype.kind not in ["i", "u", "f", "c"] or not isinstance(idx2.dtype, np.dtype))
    ) or (
        idx2.dtype.kind == "c" and (idx1.dtype.kind not in ["i", "u", "f", "c"] or not isinstance(idx1.dtype, np.dtype))
    ):
...
class TestSetOps:
    ...
    @pytest.mark.filterwarnings("ignore:Falling back on a non-pyarrow:pandas.errors.PerformanceWarning")
