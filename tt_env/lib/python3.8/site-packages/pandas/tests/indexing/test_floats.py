...
def gen_obj(klass, index):
    if klass is Series:
        ...
        obj = DataFrame(np.random.randn(len(index), len(index)), index=index, columns=index)
    return obj
...
class TestFloatIndexers:
    ...
    def test_slice_integer(self):
        ...
        for index, oob in [
            ...
            # positional indexing
            msg = (
                "cannot do slice indexing " rf"on {type(index).__name__} with these indexers \[-6\.0\] of " "type float"
            )
            with pytest.raises(TypeError, match=msg):
                s[slice(-6.0, 6.0)]
    ...
    def test_slice_integer_frame_getitem(self, index_func):
        ...
        # positional indexing
        msg = "cannot do slice indexing " rf"on {type(index).__name__} with these indexers \[-10\.0\] of " "type float"
        with pytest.raises(TypeError, match=msg):
            s[slice(-10.0, 10.0)]
        ...
        for idx, res in [
            ...
            # positional indexing
            msg = (
                "cannot do slice indexing " rf"on {type(index).__name__} with these indexers \[0\.5\] of " "type float"
            )
            with pytest.raises(TypeError, match=msg):
                s[idx]
    ...
    def test_float_slice_getitem_with_integer_index_raises(self, idx, index_func):
        ...
        # positional indexing
        msg = (
            "cannot do slice indexing " rf"on {type(index).__name__} with these indexers \[(3|4)\.0\] of " "type float"
        )
        with pytest.raises(TypeError, match=msg):
            s[idx] = 0
