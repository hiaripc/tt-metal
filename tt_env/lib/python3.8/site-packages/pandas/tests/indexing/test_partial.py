...
class TestEmptyFrameSetitemExpansion:
    def test_empty_frame_setitem_index_name_retained(self):
        ...
        df["series"] = series
        expected = DataFrame({"series": [1.23] * 4}, index=pd.RangeIndex(4, name="df_index"))
    ...
    def test_empty_frame_setitem_index_name_inherited(self):
        ...
        series = Series(1.23, index=pd.RangeIndex(4, name="series_index"))
        df["series"] = series
        expected = DataFrame({"series": [1.23] * 4}, index=pd.RangeIndex(4, name="series_index"))
        tm.assert_frame_equal(df, expected)
...
class TestPartialSetting:
    ...
    def test_series_partial_set(self):
        ...
        msg = rf"\"None of \[Index\(\[3, 3, 3\], dtype='{np.int_().dtype}'\)\] " r"are in the \[index\]\""
    ...
    def test_series_partial_set_with_name(self):
        ...
        # raises as nothing is in the index
        msg = rf"\"None of \[Index\(\[3, 3, 3\], dtype='{np.int_().dtype}', " r"name='idx'\)\] are in the \[index\]\""
        with pytest.raises(KeyError, match=msg):
            ser.loc[[3, 3, 3]]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                (r"None of \[Index\(\['2000-01-04', '2000-01-08'\], " r"dtype='object'\)\] are in the \[index\]"),
            ),
        ],
    )
