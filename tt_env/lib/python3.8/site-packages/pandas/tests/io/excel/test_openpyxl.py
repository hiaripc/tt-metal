...
@pytest.mark.parametrize(
    ...
    [
        (
            "invalid",
            "'invalid' is not valid for if_sheet_exists. Valid options " "are 'error', 'new', 'replace' and 'overlay'.",
        ),
        (
            "error",
...
def test_to_excel_with_openpyxl_engine(ext):
    ...
    with tm.ensure_clean(ext) as filename:
        ...
        styled = df.style.applymap(lambda val: f"color: {'red' if val < 0 else 'black'}").highlight_max()
...
def test_read_multiindex_header_no_index_names(datapath, ext):
    ...
    expected = DataFrame(
        ...
        columns=pd.MultiIndex.from_tuples([("X", "Y", "A1"), ("X", "Y", "A2"), ("XX", "YY", "B1"), ("XX", "YY", "B2")]),
