...
class TestReaders:
    ...
    def test_usecols_list(self, request, read_ext, df_ref):
        if read_ext == ".xlsb":
            request.node.add_marker(pytest.mark.xfail(reason="Sheets containing datetimes not supported by pyxlsb"))
    ...
    def test_usecols_diff_positional_int_columns_order(self, request, read_ext, usecols, df_ref):
        if read_ext == ".xlsb":
            request.node.add_marker(pytest.mark.xfail(reason="Sheets containing datetimes not supported by pyxlsb"))
    ...
    def test_usecols_excel_range_str(self, request, read_ext, df_ref):
        ...
        expected = df_ref[["C", "D"]]
        result = pd.read_excel("test1" + read_ext, sheet_name="Sheet1", index_col=0, usecols="A,D:E")
        tm.assert_frame_equal(result, expected, check_names=False)
    ...
    def test_dtype_backend(self, read_ext, dtype_backend):
        ...
        if dtype_backend == "pyarrow":
            ...
            expected = DataFrame({col: ArrowExtensionArray(pa.array(df[col], from_pandas=True)) for col in df.columns})
            # pyarrow by default infers timestamp resolution as us, not ns
            expected["i"] = ArrowExtensionArray(expected["i"].array._data.cast(pa.timestamp(unit="us")))
    ...
    def test_date_conversion_overflow(self, request, engine, read_ext):
        ...
        if engine == "openpyxl":
            request.node.add_marker(pytest.mark.xfail(reason="Maybe not supported by openpyxl"))

        if engine is None and read_ext in (".xlsx", ".xlsm"):
            # GH 35029
            request.node.add_marker(pytest.mark.xfail(reason="Defaults to openpyxl, maybe not supported"))
    ...
    @pytest.mark.network
    @tm.network(
        url=("https://raw.githubusercontent.com/pandas-dev/pandas/main/" "pandas/tests/io/data/excel/test1.xlsx"),
        check_before_test=True,
    )
    def test_read_from_http_url(self, read_ext):
        url = "https://raw.githubusercontent.com/pandas-dev/pandas/main/" "pandas/tests/io/data/excel/test1" + read_ext
    ...
    def test_read_excel_multiindex_blank_after_name(self, request, read_ext, sheet_name, idx_lvl2):
        # GH34673
        if read_ext == ".xlsb":
            request.node.add_marker(
                pytest.mark.xfail(reason="Sheets containing datetimes not supported by pyxlsb (GH4679")
            )
    ...
    def test_read_excel_skiprows(self, request, read_ext):
        ...
        actual = pd.read_excel("testskiprows" + read_ext, sheet_name="skiprows_list", skiprows=[0, 2])
        expected = DataFrame(
            [
                [1, 2.5, pd.Timestamp("2015-01-01"), True],
                [2, 3.5, pd.Timestamp("2015-01-02"), False],
    ...
    def test_ignore_chartsheets_by_str(self, request, engine, read_ext):
        ...
        if engine == "pyxlsb":
            request.node.add_marker(pytest.mark.xfail(reason="pyxlsb can't distinguish chartsheets from worksheets"))
