...
class TestDataFrameFormatting:
    ...
    def test_truncate_with_different_dtypes(self):
        ...
        # 11594
        s = Series([datetime(2012, 1, 1)] * 10 + [datetime(1012, 1, 2)] + [datetime(2012, 1, 3)] * 10)
    ...
    def test_wide_repr_wide_columns(self):
        with option_context("mode.sim_interactive", True, "display.max_columns", 20):
            df = DataFrame(np.random.randn(5, 3), columns=["a" * 90, "b" * 90, "c" * 90])
            rep_str = repr(df)
    ...
    def test_wide_repr_multiindex_cols(self):
        with option_context("mode.sim_interactive", True, "display.max_columns", 20):
            ...
            df = DataFrame(tm.rands_array(25, (10, max_cols - 1)), index=midx, columns=mcols)
            df.index.names = ["Level 0", "Level 1"]
            with option_context("display.expand_frame_repr", False):
                rep_str = repr(df)
            with option_context("display.expand_frame_repr", True):
...
"""
    ...
    def test_repr_html_long_multiindex(self):
        ...
        idx = MultiIndex.from_tuples(tuples, names=["first", "second"])
        df = DataFrame(np.random.randn((max_L1 + 1) * 2, 2), index=idx, columns=["A", "B"])
        long_repr = df._repr_html_()
        assert "..." in long_repr
...
class TestSeriesFormatting:
    ...
    def test_freq_name_separation(self):
        s = Series(np.random.randn(10), index=date_range("1/1/2000", periods=10), name=0)
...
class TestGenericArrayFormatter:
    ...
    def test_3d_array(self):
        obj = fmt.GenericArrayFormatter(np.array([[[True, True], [False, False]], [[False, True], [True, False]]]))
        res = obj.get_result()
        assert len(res) == 2
...
class TestDatetimeIndexFormat:
    ...
    def test_date_explicit_date_format(self):
        formatted = pd.to_datetime([datetime(2003, 2, 1), NaT]).format(date_format="%m-%d-%Y", na_rep="UT")
