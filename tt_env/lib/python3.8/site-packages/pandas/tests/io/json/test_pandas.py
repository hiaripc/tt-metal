...
class TestPandasContainer:
    ...
    @pytest.mark.parametrize("convert_axes", [True, False])
    def test_roundtrip_categorical(self, request, orient, categorical_frame, convert_axes):
        # TODO: create a better frame to test with and improve coverage
        if orient in ("index", "columns"):
            request.node.add_marker(
                pytest.mark.xfail(reason=f"Can't have duplicate index values for orient '{orient}')")
            )
    ...
    def test_series_non_unique_index(self):
        ...
        tm.assert_series_equal(s, read_json(s.to_json(orient="split"), orient="split", typ="series"))
        unserialized = read_json(s.to_json(orient="records"), orient="records", typ="series")
    ...
    def test_read_s3_jsonl(self, s3_resource, s3so):
        ...
        result = read_json("s3n://pandas-test/items.jsonl", lines=True, storage_options=s3so)
        expected = DataFrame([[1, 2], [1, 2]], columns=["a", "b"])
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame.from_dict(
            {
                ...
                "Datetime": Series(["2020-01-01", None, "2020-01-03"], dtype="datetime64[ns]"),
            }
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                DataFrame([[1, 2], [4, 5]], columns=["a", "b"], index=[["a", "b"], ["c", "d"]]),
                {"columns": ["a", "b"], "data": [[1, 2], [4, 5]]},
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "split",
                "",
                marks=pytest.mark.xfail(reason="Produces JSON but not in a consistent manner"),
            ),
            pytest.param(
                "table",
                "",
...
{spaces}}}
    ...
    def test_emca_262_nan_inf_support(self):
        ...
        expected = DataFrame(["a", np.nan, "NaN", np.inf, "Infinity", -np.inf, "-Infinity"])
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                '{"(0, \'x\')":1,"(0, \'y\')":"a","(1, \'x\')":2,' '"(1, \'y\')":"b","(2, \'x\')":3,"(2, \'y\')":"c"}',
            )
        ],
    )
    ...
    def test_to_s3(self, s3_resource, s3so):
        ...
        timeout = 5
        while True:
            if target_file in (obj.key for obj in s3_resource.Bucket("pandas-test").objects.all()):
                break
            time.sleep(0.1)
            timeout -= 0.1
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                '{"0":{"imag":8.0,"real":-6.0},' '"1":{"imag":1.0,"real":0.0},' '"2":{"imag":-5.0,"real":9.0}}',
            ),
            (
                Series({0: -9.39 + 0.66j, 1: 3.95 + 9.32j, 2: 4.03 - 0.17j}),
                '{"0":{"imag":0.66,"real":-9.39},' '"1":{"imag":9.32,"real":3.95},' '"2":{"imag":-0.17,"real":4.03}}',
            ),
            (
            ...
            (
                DataFrame([[-0.28 + 0.34j, -1.08 - 0.39j], [0.41 - 0.34j, -0.78 - 1.35j]]),
    ...
    @pytest.mark.parametrize("orient", ["split", "records", "values", "index", "columns"])
    ...
    def test_read_json_nullable_series(self, string_storage, dtype_backend, orient):
        ...
        with pd.option_context("mode.string_storage", string_storage):
            result = read_json(out, dtype_backend=dtype_backend, orient=orient, typ="series")
