...
2001,106380451,67"""
    ...
    msg = "Integer column has NA values" if parser.engine == "c" else "Unable to convert column DOY"
...
def decimal_number_check(request, parser, numeric_decimal, thousands, float_precision):
    ...
    if thousands is None and value in ("1_,", "1_234,56", "1_234,56e0"):
        request.node.add_marker(pytest.mark.xfail(reason=f"thousands={thousands} and sep is in {value}"))
    df = parser.read_csv(
        StringIO(value),
    ...
    expected = DataFrame(
        {
            ...
            "h": pd.Series([pd.NA if parser.engine != "pyarrow" else "", "a"], dtype="string"),
            "i": pd.Series([Timestamp("2019-12-31")] * 2),
            "j": pd.Series([pd.NA, pd.NA], dtype="Int64" if parser.engine != "pyarrow" else object),
        }
    )
    tm.assert_frame_equal(result, expected)
...
"""
    ...
    expected = DataFrame(
        {
            "a": IntegerArray(np.array([1, 1, 1582218195625938945]), np.array([False, True, False])),
            "b": 1,
        }
    )
    tm.assert_frame_equal(result, expected)
