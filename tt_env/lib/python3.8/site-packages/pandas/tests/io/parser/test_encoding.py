...
def test_utf8_bom(all_parsers, data, kwargs, expected, request):
    ...
    if parser.engine == "pyarrow" and data == "\n1" and kwargs.get("skip_blank_lines", True):
        ...
        request.node.add_marker(pytest.mark.xfail(reason="Pyarrow can't read blank lines"))
...
def test_readcsv_memmap_utf8(all_parsers):
    ...
    with tm.ensure_clean("utf8test.csv") as fname:
        df.to_csv(fname, index=False, header=False, encoding="utf-8")
        dfr = parser.read_csv(fname, header=None, memory_map=True, engine="c", encoding="utf-8")
