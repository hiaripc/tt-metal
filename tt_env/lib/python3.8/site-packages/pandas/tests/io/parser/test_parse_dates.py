...
def test_multiple_date_cols_int_cast(all_parsers):
    ...
    result = parser.read_csv_check_warnings(FutureWarning, "use 'date_format' instead", StringIO(data), **kwds)
...
def test_nat_parse(all_parsers):
    # see gh-3062
    parser = all_parsers
    df = DataFrame(dict({"A": np.arange(10, dtype="float64"), "B": Timestamp("20010101")}))
    df.iloc[3:6, :] = np.nan
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"parse_dates": ["a", "b"]},
            DataFrame([[datetime(2016, 4, 15), datetime(2013, 9, 16)]], columns=["a", "b"]),
        ),
        (
            "a,b\n04.15.2016,09.16.2013",
            {"parse_dates": True, "index_col": [0, 1]},
            DataFrame(
                index=MultiIndex.from_tuples([(datetime(2016, 4, 15), datetime(2013, 9, 16))], names=["a", "b"]),
                columns=[],
            ),
        ),
    ],
...
def test_parse_delimited_date_swap_no_warning(all_parsers, date_string, dayfirst, expected):
    parser = all_parsers
...
def test_parse_multiple_delimited_dates_with_swap_warnings():
    ...
    with pytest.raises(
        ValueError,
        match=(r'^time data "31/05/2000" doesn\'t match format "%m/%d/%Y", ' r"at position 1. You might want to try:"),
    ):
...
def test_hypothesis_delimited_date(request, date_format, dayfirst, delimiter, test_datetime):
    if date_format == "%m %Y" and delimiter == ".":
        request.node.add_marker(
            pytest.mark.xfail(
                reason="parse_datetime_string cannot reliably tell whether " "e.g. %m.%Y is a float or a date"
            )
        )
...
03.08.2003 15:20:00.000,2"""
    ...
    result = parser.read_csv_check_warnings(warn, msg, StringIO(data), parse_dates=True, index_col=0)
