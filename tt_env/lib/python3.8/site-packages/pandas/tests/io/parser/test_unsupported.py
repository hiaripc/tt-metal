...
1,2,3,4,"""
        ...
        for default in py_unsupported:
            msg = f"The {repr(default)} option is not " f"supported with the {repr(python_engine)} engine"
...
def test_invalid_file_inputs(request, all_parsers):
    ...
    if parser.engine == "python":
        request.node.add_marker(pytest.mark.xfail(reason=f"{parser.engine} engine supports lists."))
