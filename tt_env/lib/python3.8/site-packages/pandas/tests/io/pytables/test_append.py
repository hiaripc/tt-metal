...
def test_append(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        with catch_warnings(record=True):
            ...
            uint_data = DataFrame(
                {
                    "u08": Series(np.random.randint(0, high=255, size=5), dtype=np.uint8),
                    "u16": Series(np.random.randint(0, high=65535, size=5), dtype=np.uint16),
                    "u32": Series(np.random.randint(0, high=2**30, size=5), dtype=np.uint32),
                    "u64": Series(
...
because its data contents are not [string] but [mixed] object dtype"""
        ...
        # series directly
        msg = re.escape(
            "cannot properly create the storer for: " "[group->df,value-><class 'pandas.core.series.Series'>]"
        )
        with pytest.raises(TypeError, match=msg):
    ...
    df = DataFrame(
        {
            ...
            "B": [Timestamp("20130101") + timedelta(days=i, seconds=10) for i in range(10)],
        }
    )
    df["C"] = df["A"] - df["B"]
    df.loc[3:5, "C"] = np.nan
