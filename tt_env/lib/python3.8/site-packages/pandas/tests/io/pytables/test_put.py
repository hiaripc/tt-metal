...
def test_put_string_index(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        # mixed length
        index = Index(
            ["abcdefghijklmnopqrstuvwxyz1234567890"] + [f"I am a very long string index: {i}" for i in range(20)]
        )
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        def make_index(names=None):
            return MultiIndex.from_tuples(
                [(datetime.datetime(2013, 12, d), s, t) for d in range(1, 3) for s in range(2) for t in range(3)],
                names=names,
            )
...
def test_store_periodindex(tmp_path, setup_path, format):
    ...
    df = DataFrame(np.random.randn(5, 1), index=pd.period_range("20220101", freq="M", periods=5))
