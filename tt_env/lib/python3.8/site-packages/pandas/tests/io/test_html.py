...
class TestReadHtml:
    ...
    @pytest.mark.network
    @tm.network(
        url=("https://raw.githubusercontent.com/pandas-dev/pandas/main/" "pandas/tests/io/data/html/spam.html"),
        check_before_test=True,
    )
    ...
    def test_invalid_url(self):
        msg = "Name or service not known|Temporary failure in name resolution|" "No tables found"
    ...
    def test_file_url(self, banklist_data):
        url = banklist_data
        dfs = self.read_html(file_path_to_url(os.path.abspath(url)), match="First", attrs={"id": "table"})
    ...
    def test_invalid_table_attrs(self, banklist_data):
        ...
        with pytest.raises(ValueError, match="No tables found"):
            self.read_html(url, match="First Federal Bank of Florida", attrs={"id": "tasdfable"})
    ...
    def test_multiindex_header_index_skiprows(self, banklist_data):
        df = self._bank_data(banklist_data, header=[0, 1], index_col=[0, 1], skiprows=1)[0]
        assert isinstance(df.index, MultiIndex)
        assert isinstance(df.columns, MultiIndex)
    ...
    def test_gold_canyon(self, banklist_data):
        ...
        df = self.read_html(banklist_data, match="Gold Canyon", attrs={"id": "table"})[0]
    ...
    def test_parser_error_on_empty_header_row(self):
        ...
        expected = DataFrame(
            [["a", "b"]],
            columns=MultiIndex.from_tuples([("Unnamed: 0_level_0", "A"), ("Unnamed: 1_level_0", "B")]),
        )
    ...
    @pytest.mark.filterwarnings(
        "ignore:You provided Unicode markup but also provided a value for " "from_encoding.*:UserWarning"
    )
    def test_encode(self, html_encoding_file):
        ...
        try:
            with open(html_encoding_file, "rb") as fobj:
                from_string = self.read_html(fobj.read(), encoding=encoding, index_col=0).pop()

            with open(html_encoding_file, "rb") as fobj:
                from_file_like = self.read_html(BytesIO(fobj.read()), encoding=encoding, index_col=0).pop()

            from_filename = self.read_html(html_encoding_file, encoding=encoding, index_col=0).pop()
