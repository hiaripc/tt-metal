...
sqlalchemy_connectable_iris = mysql_connectable + postgresql_connectable + sqlite_iris_connectable
...
def test_copy_from_callable_insertion_method(conn, expected_count, request):
    ...
    result_count = expected.to_sql("test_frame", conn, index=False, method=psql_insert_copy)
    # GH 46891
...
def test_execute_typeerror(sqlite_iris_engine):
    with pytest.raises(TypeError, match="pandas.io.sql.execute requires a connection"):
        with tm.assert_produces_warning(
            FutureWarning,
            match="`pandas.io.sql.execute` is deprecated and " "will be removed in the future version.",
        ):
...
class PandasSQLTest:
    ...
    def _to_sql_with_sql_engine(self, test_frame1, engine="auto", **engine_kwargs):
        ...
        assert self.pandasSQL.to_sql(test_frame1, "test_frame1", engine=engine, **engine_kwargs) == 4
...
class _TestSQLApi(PandasSQLTest):
    ...
    def test_to_sql_append(self, test_frame1):
        ...
        # Add to table again
        assert sql.to_sql(test_frame1, "test_frame4", self.conn, if_exists="append") == 4
    ...
    def test_date_parsing(self):
        ...
        df = sql.read_sql_query("SELECT * FROM types", self.conn, parse_dates=["DateCol"])
        ...
        df = sql.read_sql_query("SELECT * FROM types", self.conn, parse_dates=["IntDateCol"])
        ...
        df = sql.read_sql_query("SELECT * FROM types", self.conn, parse_dates={"IntDateCol": "s"})
        assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
        assert df.IntDateCol.tolist() == [
            Timestamp(1986, 12, 25, 0, 0, 0),
            Timestamp(2013, 1, 1, 0, 0, 0),
    ...
    def test_custom_dateparsing_error(self, read_sql, text, mode, error, types_data_frame):
    ...
    def test_to_sql_index_label_multiindex(self):
        ...
        result = sql.to_sql(temp_frame, "test_index_label", self.conn, if_exists="replace")
        assert result == expected_row_count
        frame = sql.read_sql_query("SELECT * FROM test_index_label", self.conn)
        assert frame.columns[:2].tolist() == ["A", "B"]
    ...
    def test_multiindex_roundtrip(self):
        ...
        result = sql.read_sql_query("SELECT * FROM test_multiindex_roundtrip", self.conn, index_col=["A", "B"])
    ...
    def test_dtype_argument(self, dtype):
        ...
        result = sql.read_sql_query("SELECT A, B FROM test_dtype_argument", con=self.conn, dtype=dtype)
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def test_read_table_index_col(self, test_frame1):
        ...
        result = sql.read_sql_table("test_frame", self.conn, index_col=["A", "B"], columns=["C", "D"])
        assert result.index.names == ["A", "B"]
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_datetime_with_timezone(self, request):
        ...
        if not hasattr(df, "DateColWithTz"):
            request.node.add_marker(pytest.mark.xfail(reason="no column with datetime with time zone"))
        ...
        df = read_sql_query("select * from types", self.conn, parse_dates=["DateColWithTz"])
        if not hasattr(df, "DateColWithTz"):
            request.node.add_marker(pytest.mark.xfail(reason="no column with datetime with time zone"))
        col = df.DateColWithTz
        assert is_datetime64tz_dtype(col.dtype)
    ...
    def test_date_parsing(self):
        ...
        df = sql.read_sql_table("types", self.conn, parse_dates={"DateCol": "%Y-%m-%d %H:%M:%S"})
        assert issubclass(df.DateCol.dtype.type, np.datetime64)
        ...
        df = sql.read_sql_table("types", self.conn, parse_dates={"IntDateCol": {"unit": "s"}})
        assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
    ...
    def test_connectable_issue_example(self):
        ...
        assert DataFrame({"test_foo_data": [0, 1, 2]}).to_sql("test_foo_data", self.conn) == 3
        main(self.conn)
    ...
    def test_read_sql_dtype_backend(self, string_storage, func, dtype_backend):
        ...
        with pd.option_context("mode.string_storage", string_storage):
            result = getattr(pd, func)(f"Select * from {table}", self.conn, dtype_backend=dtype_backend)
...
class TestSQLiteAlchemy(_TestSQLAlchemy):
    ...
    def test_row_object_is_named_tuple(self):
        ...
        with Session() as session:
            ...
            assert df.to_sql("test_frame", con=self.conn, index=False, if_exists="replace") == 2
            session.commit()
...
class TestPostgreSQLAlchemy(_TestSQLAlchemy):
    ...
    def test_schema_support(self):
        ...
        assert df.to_sql("test_schema_public_explicit", self.conn, index=False, schema="public") == 2
        assert df.to_sql("test_schema_other", self.conn, index=False, schema="other") == 2
        ...
        tm.assert_frame_equal(df, res2)
        res3 = sql.read_sql_table("test_schema_public_explicit", self.conn, schema="public")
        ...
        assert df.to_sql("test_schema_other", self.conn, schema="other", index=False) == 2
        df.to_sql(
            "test_schema_other",
            self.conn,
            schema="other",
        ...
        if isinstance(self.conn, Engine):
            ...
            assert pdsql.to_sql(df, "test_schema_other2", index=False, if_exists="replace") == 2
            assert pdsql.to_sql(df, "test_schema_other2", index=False, if_exists="append") == 2
...
class TestSQLiteFallback(SQLiteMixIn, PandasSQLTest):
    ...
    def _get_index_columns(self, tbl_name):
        ixs = sql.read_sql_query(
            "SELECT * FROM sqlite_master WHERE type = 'index' " + f"AND tbl_name = '{tbl_name}'",
            self.conn,
        )
        ix_cols = []
