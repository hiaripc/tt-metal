...
class TestStata:
    ...
    def test_read_dta2(self, datapath):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            parsed_114 = self.read_dta(datapath("io", "data", "stata", "stata2_114.dta"))
            parsed_115 = self.read_dta(datapath("io", "data", "stata", "stata2_115.dta"))
            parsed_117 = self.read_dta(datapath("io", "data", "stata", "stata2_117.dta"))
    ...
    def test_categorical_warnings_and_errors(self):
        ...
        with tm.ensure_clean() as path:
            msg = (
                "Stata value labels for a single variable must have " r"a combined length less than 32,000 characters\."
            )
            with pytest.raises(ValueError, match=msg):
                original.to_stata(path)
...
"""
    ...
    def test_out_of_range_double(self):
        ...
        msg = r"Column ColumnTooBig has a maximum value \(.+\) outside the range " r"supported by Stata \(.+\)"
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                df.to_stata(path)
    ...
    def test_inf(self, infval):
        ...
        msg = "Column WithInf contains infinity or -infinity" "which is outside the range supported by Stata."
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                df.to_stata(path)
    ...
    def test_invalid_file_not_written(self, version):
        ...
        with tm.ensure_clean() as path:
            msg1 = r"'latin-1' codec can't encode character '\\ufffd' " r"in position 14: ordinal not in range\(256\)"
            msg2 = "'ascii' codec can't decode byte 0xef in position 14: " r"ordinal not in range\(128\)"
