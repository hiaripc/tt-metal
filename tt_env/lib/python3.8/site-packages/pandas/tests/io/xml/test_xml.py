...
@pytest.mark.slow
@tm.network(
    url=("https://data.cityofchicago.org/api/views/" "8pix-ypme/rows.xml?accessType=DOWNLOAD"),
...
def test_bad_xpath_etree(datapath):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(SyntaxError, match=("You have used an incorrect or unsupported XPath")):
...
def test_prefix_namespace(parser):
    ...
    df_iter = read_xml_iterparse(xml_prefix_nmsp, parser=parser, iterparse={"row": ["shape", "degrees", "sides"]})
...
def test_names_option_output(datapath, parser):
    filename = datapath("io", "data", "xml", "books.xml")
    df_file = read_xml(filename, names=["Col1", "Col2", "Col3", "Col4", "Col5"], parser=parser)
    df_iter = read_xml(
        filename,
        parser=parser,
...
<shapes>
  ...
  <shape type="3D">
    ...
    df_xpath = read_xml(xml, xpath=".//shape", parser=parser, names=["type_dim", "shape", "type_edge"])
...
def test_utf16_encoding(datapath, parser):
    ...
    with pytest.raises(
        UnicodeError,
        match=("UTF-16 stream does not start with BOM|" "'utf-16-le' codec can't decode byte"),
    ):
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    with pytest.raises(XMLSyntaxError, match=("Extra content at the end of the document")):
        read_xml(kml, stylesheet=xsl)
...
def test_empty_stylesheet(val):
    ...
    with pytest.raises(XMLSyntaxError, match=("Document is empty|Start tag expected, '<' not found")):
...
def test_wrong_dict_value(datapath, parser):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(TypeError, match="<class 'str'> is not a valid type for value in iterparse"):
...
def test_no_result(datapath, parser):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(ParserError, match="No result from selected items in iterparse."):
        read_xml(
            filename,
            parser=parser,
...
@pytest.mark.network
@td.skip_if_no("lxml")
@tm.network(url="https://www.w3schools.com/xml/cdcatalog_with_xsl.xml", check_before_test=True)
