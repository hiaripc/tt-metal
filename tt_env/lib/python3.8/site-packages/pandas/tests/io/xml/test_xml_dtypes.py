...
def test_converters_date(parser):
    ...
    df_result = read_xml(xml_dates, converters={"date": convert_to_datetime}, parser=parser)
    df_iter = read_xml_iterparse(
        xml_dates,
        parser=parser,
...
def test_callable_func_converters(datapath, parser, iterparse):
    ...
    with pytest.raises(TypeError, match=("'float' object is not callable")):
        read_xml(filename, converters={"year": float()}, parser=parser, iterparse=iterparse)
...
<data>
  ...
  <row>
    ...
    df_result = read_xml(xml, parse_dates={"date_end": ["year", "month", "day"]}, parser=parser)
    df_iter = read_xml_iterparse(
        xml,
        parser=parser,
