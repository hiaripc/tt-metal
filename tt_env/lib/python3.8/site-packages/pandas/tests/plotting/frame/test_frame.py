...
class TestDataFramePlots(TestPlotBase):
    ...
    def test_errorbar_scatter(self):
        df = DataFrame(np.abs(np.random.randn(5, 2)), index=range(5), columns=["x", "y"])
        df_err = DataFrame(np.abs(np.random.randn(5, 2)) / 5, index=range(5), columns=["x", "y"])
        ...
        def _check_errorbar_color(containers, expected, has_err="has_xerr"):
            ...
            self._check_colors(err_lines, linecolors=np.array([expected] * len(err_lines)))

        # GH 8081
        df = DataFrame(np.abs(np.random.randn(10, 5)), columns=["a", "b", "c", "d", "e"])
    ...
    def test_secondary_y(self, secondary_y):
        ax_df = DataFrame([0]).plot(secondary_y=secondary_y, ylabel="Y", ylim=(0, 100), yticks=[99])
