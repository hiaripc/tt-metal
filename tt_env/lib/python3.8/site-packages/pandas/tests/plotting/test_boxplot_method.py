...
class TestDataFrameGroupByPlots(TestPlotBase):
    ...
    def test_grouped_box_layout(self, hist_df):
        ...
        # _check_plot_works adds an ax so catch warning. see GH #13188
        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            box = _check_plot_works(df.groupby("gender").boxplot, column="height", return_type="dict")
        self._check_axes_shape(self.plt.gcf().axes, axes_num=2, layout=(1, 2))

        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            box = _check_plot_works(df.groupby("category").boxplot, column="height", return_type="dict")
        self._check_axes_shape(self.plt.gcf().axes, axes_num=4, layout=(2, 2))

        # GH 6769
        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            box = _check_plot_works(df.groupby("classroom").boxplot, column="height", return_type="dict")
        ...
        box = df.groupby("classroom").boxplot(column=["height", "weight", "category"], return_type="dict")
        self._check_axes_shape(self.plt.gcf().axes, axes_num=3, layout=(2, 2))
        ...
        box = df.boxplot(column=["height", "weight", "category"], by="gender", layout=(-1, 1))
    ...
    def test_fontsize(self):
        ...
        self._check_ticks_props(df.boxplot("a", by="b", fontsize=16), xlabelsize=16, ylabelsize=16)
