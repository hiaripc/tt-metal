...
def test_resample_interpolate(frame):
    # # 12925
    df = frame
    tm.assert_frame_equal(df.resample("1T").asfreq().interpolate(), df.resample("1T").interpolate())
...
def test_resample_empty_series(freq, empty_series_dti, resample_method, request):
    ...
    if resample_method == "ohlc" and isinstance(empty_series_dti.index, PeriodIndex):
        request.node.add_marker(pytest.mark.xfail(reason=f"GH13083: {resample_method} fails for PeriodIndex"))
