...
def test_resample_origin_with_day_freq_on_dst(unit):
    ...
    def _create_series(values, timestamps, freq="D"):
        return Series(
            values,
            index=DatetimeIndex([Timestamp(t, tz=tz) for t in timestamps], freq=freq, ambiguous=True).as_unit(unit),
        )
...
def test_how_lambda_functions(simple_date_range_series, unit):
    ...
    # doesn't make sense
    result = ts.resample("M").aggregate({"foo": lambda x: x.mean(), "bar": lambda x: x.std(ddof=1)})
...
def test_groupby_with_dst_time_change(unit):
    # GH 24972
    index = (
        DatetimeIndex([1478064900001000000, 1480037118776792000], tz="UTC").tz_convert("America/Chicago").as_unit(unit)
    )
...
def test_resample_dst_anchor(unit):
    ...
    tm.assert_frame_equal(
        df_daily.resample("D").agg({"a": "min", "b": "max", "c": "count"})[["a", "b", "c"]],
        DataFrame(
            {
...
def test_downsample_across_dst_weekly(unit):
    ...
    expected = DataFrame(
        [23, 42],
        index=DatetimeIndex(["2017-03-26", "2017-04-02"], tz="Europe/Amsterdam", freq="W").as_unit(unit),
    )
    tm.assert_frame_equal(result, expected)
...
def test_resample_apply_product(duplicates, unit):
    ...
    expected = DataFrame(
        ...
        index=DatetimeIndex(["2012-03-31", "2012-06-30", "2012-09-30", "2012-12-31"], freq="Q-DEC").as_unit(unit),
        columns=df.columns,
    )
    tm.assert_frame_equal(result, expected)
