...
def test_resample_as_freq_with_subperiod():
    ...
    expected = DataFrame(data=expected_data, index=timedelta_range("00:00:00", "00:10:00", freq="2T"))
    tm.assert_frame_equal(result, expected)
...
def test_resample_closed_right():
    ...
    expected = Series(
        [0, 3, 7, 11, 15, 9],
        index=pd.TimedeltaIndex([pd.Timedelta(seconds=120 + i * 60) for i in range(6)], freq="T"),
    )
    tm.assert_series_equal(result, expected)
