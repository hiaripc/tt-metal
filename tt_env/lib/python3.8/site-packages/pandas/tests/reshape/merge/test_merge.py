...
class TestMerge:
    ...
    @pytest.fixture
    def left(self):
        return DataFrame({"key": ["a", "b", "c", "d", "e", "e", "a"], "v1": np.random.randn(7)})
    ...
    def test_merge_period_values(self):
        ...
        right = DataFrame({"key": [2, 3], "value": pd.period_range("20151011", periods=2, freq="D")})
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                DatetimeIndex(["2001-01-01", "2002-02-02", "2003-03-03", pd.NaT, pd.NaT, pd.NaT]),
            ),
            *[
                (
...
def left():
    ...
    return DataFrame(
        {
            "X": Series(np.random.choice(["foo", "bar"], size=(10,))).astype(CDT(["foo", "bar"])),
...
def right():
    np.random.seed(1234)
    return DataFrame({"X": Series(["foo", "bar"]).astype(CDT(["foo", "bar"])), "Z": [1, 2]})
...
def test_merge_equal_cat_dtypes2():
    ...
    expected = DataFrame({"left": [1, 2], "right": [3, 2], "foo": Series(["a", "b"]).astype(cat_dtype)}).set_index(
        "foo"
    )
...
def test_merge_multiindex_columns():
    ...
    expected_labels = [letter + l_suf for letter in letters] + [letter + r_suf for letter in letters]
    expected_index = MultiIndex.from_product([expected_labels, numbers], names=["outer", "inner"])
...
def test_merge_join_cols_error_reporting_on_and_index(func, kwargs):
    ...
    left = DataFrame({"a": [1, 2], "b": [3, 4]})
    right = DataFrame({"a": [1, 1], "c": [5, 6]})
    msg = r'Can only pass argument "on" OR "left_index" ' r'and "right_index", not a combination of both\.'
    with pytest.raises(MergeError, match=msg):
