...
def test_cut_return_intervals():
    ...
    expected = Series(IntervalIndex.from_breaks(exp_bins, closed="right").take([0, 0, 0, 1, 1, 1, 2, 2, 2])).astype(
        CDT(ordered=True)
    )
...
def test_series_ret_bins():
    ...
    expected = Series(IntervalIndex.from_breaks([-0.003, 1.5, 3], closed="right").repeat(2)).astype(CDT(ordered=True))
    tm.assert_series_equal(result, expected)
    ...
    expected = Series(
        IntervalIndex(
            [
                ...
                Interval(Timestamp("2013-01-01 16:00:00"), Timestamp("2013-01-02 08:00:00")),
                Interval(Timestamp("2013-01-02 08:00:00"), Timestamp("2013-01-03 00:00:00")),
            ]
        )
    ).astype(CDT(ordered=True))
