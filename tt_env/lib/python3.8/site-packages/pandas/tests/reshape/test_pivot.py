...
class TestPivotTable:
    ...
    def test_pivot_with_non_observable_dropna(self, dropna):
        ...
        expected = DataFrame(
            {"B": [2.0, 3.0]},
            index=Index(
                Categorical.from_codes([0, 1], categories=["low", "high"], ordered=True),
                name="A",
            ),
        )
    ...
    def test_pivot_no_values(self):
        ...
        res = df.pivot_table(index=Grouper(freq="A"), columns=Grouper(key="dt", freq="M"))
        exp = DataFrame([3], index=pd.DatetimeIndex(["2011-12-31"], freq="A"), columns=exp_columns)
        tm.assert_frame_equal(res, exp)
    ...
    def test_margins(self, data):
        # column specified
        result = data.pivot_table(values="D", index=["A", "B"], columns="C", margins=True, aggfunc=np.mean)
        ...
        # no column specified
        table = data.pivot_table(index=["A", "B"], columns="C", margins=True, aggfunc=np.mean)
    ...
    def test_no_col(self, data):
        ...
        table = data.drop(columns="CC").pivot_table(index=["AA", "BB"], margins=True, aggfunc=np.mean)
    ...
    def test_margins_dtype(self, data):
        ...
        expected = DataFrame({"dull": [12, 21, 3, 9, 45], "shiny": [33, 0, 36, 51, 120]}, index=mi).rename_axis(
            "C", axis=1
        )
    ...
    def test_categorical_margins(self, observed, request):
        if observed:
            request.node.add_marker(pytest.mark.xfail(reason="GH#17035 (np.mean of ints is casted back to ints)"))
        # GH 10989
    ...
    def test_pivot_table_with_mixed_nested_tuples(self, using_array_manager):
        ...
        result = pivot_table(df, values="D", index=["A", "B"], columns=[(7, "seven")], aggfunc=np.sum)
        expected = DataFrame(
            [[4.0, 5.0], [7.0, 6.0], [4.0, 1.0], [np.nan, 6.0]],
            columns=Index(["large", "small"], name=(7, "seven")),
            index=MultiIndex.from_arrays([["bar", "bar", "foo", "foo"], ["one", "two"] * 2], names=["A", "B"]),
        )
        if using_array_manager:
            # INFO(ArrayManager) column without NaNs can preserve int dtype
