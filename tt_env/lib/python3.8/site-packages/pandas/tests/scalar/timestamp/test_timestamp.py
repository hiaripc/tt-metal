...
class TestTimestampProperties:
    ...
    def test_woy_boundary(self):
        ...
        result = np.array(
            [Timestamp(datetime(*args)).week for args in [(2000, 1, 1), (2000, 1, 2), (2005, 1, 1), (2005, 1, 2)]]
        )
    ...
    def test_dow_parametric(self, ts, sign):
        # GH 53738
        ts = f"{sign}{str(ts.year).zfill(4)}" f"-{str(ts.month).zfill(2)}" f"-{str(ts.day).zfill(2)}"
...
class TestTimestamp:
    ...
    def test_class_ops_dateutil(self):
        def compare(x, y):
            assert int(np.round(Timestamp(x)._value / 1e9) - np.round(Timestamp(y)._value / 1e9)) == 0
