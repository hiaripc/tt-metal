...
def test_reindex_corner(datetime_series):
    ...
    msg = r"Invalid fill method\. Expecting pad \(ffill\), backfill " r"\(bfill\) or nearest\. Got foo"
...
def test_reindex_datetimeindexes_tz_naive_and_aware():
    ...
    msg = r"Cannot compare dtypes datetime64\[ns, America/Chicago\] " r"and datetime64\[ns\]"
...
def test_reindex_empty_with_level(values):
    # GH41170
    ser = Series(range(len(values[0])), index=MultiIndex.from_arrays(values), dtype="object")
    result = ser.reindex(np.array(["b"]), level=0)
    expected = Series(index=MultiIndex(levels=[["b"], values[1]], codes=[[], []]), dtype="object")
    tm.assert_series_equal(result, expected)
