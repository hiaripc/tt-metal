...
class TestTimeSeriesArithmetic:
    def test_series_add_tz_mismatch_converts_to_utc(self):
        ...
        ser1 = Series(np.random.randn(90), index=rng.take(perm).tz_convert("US/Eastern"))

        perm = np.random.permutation(100)[:90]
        ser2 = Series(np.random.randn(90), index=rng.take(perm).tz_convert("Europe/Berlin"))
...
def test_none_comparison(request, series_with_simple_index):
    ...
    if len(series) < 1:
        request.node.add_marker(pytest.mark.xfail(reason="Test doesn't make sense on empty data"))
