...
def test_contains(any_string_dtype):
    ...
    with tm.maybe_produces_warning(PerformanceWarning, any_string_dtype == "string[pyarrow]"):
...
def test_match_mixed_object():
    ...
    expected = Series([True, np.nan, True, np.nan, np.nan, False, np.nan, np.nan, np.nan])
...
def test_flags_kwarg(any_string_dtype):
    ...
    with tm.assert_produces_warning(UserWarning, match=msg, raise_on_extra_warnings=not using_pyarrow):
