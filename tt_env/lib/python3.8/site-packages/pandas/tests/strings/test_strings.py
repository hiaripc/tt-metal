...
def test_repeat_mixed_object():
    ...
    result = ser.str.repeat(3)
    expected = Series(["aaa", np.nan, "bbb", np.nan, np.nan, "foofoofoo", np.nan, np.nan, np.nan])
    tm.assert_series_equal(result, expected)
...
def test_spilt_join_roundtrip_mixed_object():
    ...
    result = ser.str.split("_").str.join("_")
    expected = Series(["a_b", np.nan, "asdf_cas_asdf", np.nan, np.nan, "foo", np.nan, np.nan, np.nan])
    tm.assert_series_equal(result, expected)
...
def test_encode_errors_kwarg(any_string_dtype):
    ...
    msg = r"'charmap' codec can't encode character '\\x9d' in position 1: " "character maps to <undefined>"
