...
def test_apply_out_of_range(request, tz_naive_fixture, _offset):
    ...
    try:
        ...
        if isinstance(tz, tzlocal) and not IS64 and _offset is not DateOffset:
            ...
            request.node.add_marker(pytest.mark.xfail(reason="OverflowError inside tzlocal past 2038"))
        elif isinstance(tz, tzlocal) and is_platform_windows() and _offset in (QuarterEnd, BQuarterBegin, BQuarterEnd):
            request.node.add_marker(pytest.mark.xfail(reason="After GH#49737 t.tzinfo is None on CI"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            ...
            CustomBusinessHour(holidays=["2014-06-27", Timestamp(2014, 6, 30), Timestamp("2014-07-02")]),
        ],
