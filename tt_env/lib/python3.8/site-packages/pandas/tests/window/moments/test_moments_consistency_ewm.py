...
def create_mock_weights(obj, com, adjust, ignore_na):
    if isinstance(obj, DataFrame):
        ...
        w = concat(
            [
                create_mock_series_weights(obj.iloc[:, i], com=com, adjust=adjust, ignore_na=ignore_na)
...
def test_ewm_consistency_mean(all_data, adjust, ignore_na, min_periods):
    ...
    expected = all_data.multiply(weights).cumsum().divide(weights.cumsum()).fillna(method="ffill")
...
def test_ewm_consistency_consistent(consistent_data, adjust, ignore_na, min_periods):
    ...
    corr_x_x = consistent_data.ewm(com=com, min_periods=min_periods, adjust=adjust, ignore_na=ignore_na).corr(
        consistent_data
    )
...
def test_ewm_consistency_std(all_data, adjust, ignore_na, min_periods, bias):
    ...
    cov_x_x = all_data.ewm(com=com, min_periods=min_periods, adjust=adjust, ignore_na=ignore_na).cov(
        all_data, bias=bias
    )
...
def test_ewm_consistency_series_cov_corr(series_data, adjust, ignore_na, min_periods, bias):
    ...
    cov_x_y = series_data.ewm(com=com, min_periods=min_periods, adjust=adjust, ignore_na=ignore_na).cov(
        series_data, bias=bias
    )
    ...
    if bias:
        ...
        mean_x_times_y = (
            (series_data * series_data).ewm(com=com, min_periods=min_periods, adjust=adjust, ignore_na=ignore_na).mean()
        )
