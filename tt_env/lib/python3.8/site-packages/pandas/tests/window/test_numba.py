...
pytestmark = pytest.mark.skipif(
    ...
    reason="On GHA CI, Windows can fail with " "'Windows fatal exception: stack overflow' " "and macOS can timeout",
)
...
class TestTableMethod:
    ...
    def test_table_method_rolling_apply(self, axis, nogil, parallel, nopython, step):
        ...
        df = DataFrame(np.eye(3))
        result = df.rolling(2, method="table", axis=axis, min_periods=0, step=step).apply(
            f, raw=True, engine_kwargs=engine_kwargs, engine="numba"
        )
        expected = df.rolling(2, method="single", axis=axis, min_periods=0, step=step).apply(
            f, raw=True, engine_kwargs=engine_kwargs, engine="numba"
        )
        tm.assert_frame_equal(result, expected)
