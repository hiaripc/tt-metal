...
class TestRollingTS:
    ...
    def test_invalid_minp(self, minp, regular):
        ...
        msg = r"local variable 'minp' referenced before assignment|" "min_periods must be an integer"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_on(self, regular):
        ...
        # not a valid column
        msg = r"invalid on specified as foobar, must be a column " "\\(of DataFrame\\), an Index or None"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_non_monotonic_on(self):
        ...
        msg = r"invalid on specified as A, must be a column " "\\(of DataFrame\\), an Index or None"
        with pytest.raises(ValueError, match=msg):
            df.rolling("2s", on="A").sum()
    ...
    def test_perf_min(self):
        ...
        dfp = DataFrame({"B": np.random.randn(N)}, index=date_range("20130101", periods=N, freq="s"))
    ...
    def test_rolling_on_multi_index_level(self):
        ...
        df = DataFrame(
            {"column": range(6)},
            index=MultiIndex.from_product([date_range("20190101", periods=3), range(2)], names=["date", "seq"]),
        )
