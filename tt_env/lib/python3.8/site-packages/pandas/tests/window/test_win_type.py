...
def test_window_with_args(step):
    ...
    r = Series(np.random.randn(100)).rolling(window=10, min_periods=1, win_type="gaussian", step=step)
...
def test_cmov_window_na_min_periods(step):
    ...
    rs = vals.rolling(5, win_type="boxcar", min_periods=4, center=True, step=step).mean()
