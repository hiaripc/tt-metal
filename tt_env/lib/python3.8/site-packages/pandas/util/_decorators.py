        ...
        def wrapper(*args, **kwargs) -> Callable[..., Any]:
            ...
            if old_arg_value is not None:
                if new_arg_name is None:
                    ...
                    msg = f"the {repr(old_arg_name)} keyword is deprecated, " f"use {repr(new_arg_name)} instead."

                warnings.warn(msg, FutureWarning, stacklevel=stacklevel)
                if kwargs.get(new_arg_name) is not None:
                    msg = f"Can only specify {repr(old_arg_name)} " f"or {repr(new_arg_name)}, not both."
                    raise TypeError(msg)
...
def doc(*docstrings: None | str | Callable, **params) -> Callable[[F], F]:
    ...
    **params
    ...
    def decorator(decorated: F) -> F:
        ...
        params_applied = [
            component.format(**params) if isinstance(component, str) and len(params) > 0 else component
