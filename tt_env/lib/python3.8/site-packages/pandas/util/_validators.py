...
def _check_arg_length(fname, args, max_fname_arg_count, compat_args):
    ...
    if len(args) > len(compat_args):
        ...
        raise TypeError(f"{fname}() takes at most {max_arg_count} {argument} " f"({actual_arg_count} given)")
...
def _check_for_default_values(fname, arg_val_dict, compat_args):
    ...
    for key in arg_val_dict:
        ...
        if not match:
            raise ValueError(f"the '{key}' parameter is not supported in " f"the pandas implementation of {fname}()")
...
def validate_bool_kwarg(
    ...
    if not good_value:
        raise ValueError(f'For argument "{arg_name}" expected type bool, received ' f"type {type(value).__name__}.")
    return value
...
def validate_fillna_kwargs(value, method, validate_scalar_dict_value: bool = True):
    ...
    if value is None and method is not None:
        ...
        if validate_scalar_dict_value and isinstance(value, (list, tuple)):
            raise TypeError('"value" parameter must be a scalar or dict, but ' f'you passed a "{type(value).__name__}"')
...
def check_dtype_backend(dtype_backend) -> None:
    if dtype_backend is not lib.no_default:
        if dtype_backend not in ["numpy_nullable", "pyarrow"]:
            raise ValueError(
                f"dtype_backend {dtype_backend} is invalid, only 'numpy_nullable' and " f"'pyarrow' are allowed.",
            )
