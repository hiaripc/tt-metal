...
class PEP8Normalizer(ErrorFinder):
    def __init__(self, *args, **kwargs):
        ...
        self._indentation_tos = self._last_indentation_tos = IndentationNode(self._config, indentation="")
    ...
    def _visit_node(self, node):
        ...
        if in_introducer:
            ...
            self._indentation_tos = IndentationNode(
                self._config, self._indentation_tos.indentation + self._config.indentation, parent=self._indentation_tos
            )
            ...
            if not self._check_tabs_spaces(spacing):
                ...
                if self._new_statement:
                    ...
                    if (
                        ...
                        and indentation == node.get_latest_suite_node().indentation + self._config.indentation
                    ):
                            ...
                            if value in "])}":
                                if node.type == IndentationTypes.VERTICAL_BRACKET:
                                    ...
                                    self.add_issue(
                                        part,
                                        123,
                                        "Losing bracket does not match " "indentation of opening bracket's line",
                                    )
                            else:
        ...
        if value and value in "()[]{}" and type_ != "error_leaf" and part.parent.type != "error_node":
            if value in _OPENING_BRACKETS:
                self._indentation_tos = BracketNode(
                    self._config, part, parent=self._indentation_tos, in_suite_introducer=self._in_suite_introducer
                )
            else:
