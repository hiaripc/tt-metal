    ...
    def match_file(self, file: str) -> Optional[Any]:
        ...
        *file* (:class:`str`) is the normalized file path to match against.
        ...
        raise NotImplementedError(
            ("{cls.__module__}.{cls.__qualname__} must override match_file().").format(cls=self.__class__)
        )
    ...
    def __init__(
        ...
        *pattern* is a precompiled regular expression (:class:`re.Pattern`) in which
        ...
        if isinstance(pattern, (str, bytes)):
            assert include is None, f"include:{include!r} must be null when pattern:{pattern!r} is a string."
            ...
            # null-operation.
            assert include is None, f"include:{include!r} must be null when pattern:{pattern!r} is null."
