...
class InvalidPkgInfo(ValueError):
    def __init__(self, fqp, candidates):
        ...
        super().__init__(f"Invalid PKG-INFO in archive: {fqp} " f'(no "Metadata-Version" found)')
...
class InvalidUnpackedSDist(ValueError):
    def __init__(self, fqp, raised):
        self.fqp = fqp
        super().__init__(f"Could not load {fqp} as an unpacked sdist: {raised}")
...
class UnpackedSDist(SDist):
    def __init__(self, filename, metadata_version=None):
        ...
        super(UnpackedSDist, self).__init__(filename, metadata_version=metadata_version)
