...
def test_singleline__init___():
    single = _make_single_line(_Options(fields=None, item_delim="I", sequence_delim="S"))
    assert single._item_delim == "I"
    assert single._sequence_delim == "S"


def test_singleline__call__wo_fields_wo_list():
    single = _make_single_line(_Options(fields=(), item_delim="|", sequence_delim=object()))  # raise if used
...
def test_singleline__call__w_fields_w_list():
    single = _make_single_line(_Options(fields=("foo", "bar"), item_delim="|", sequence_delim="*"))
...
def test_csv__init___():
    csv = _make_csv(_Options(fields=None, sequence_delim="S"))
    assert csv._sequence_delim == "S"


def test_csv__call__wo_fields_wo_list():
    meta = _Meta(foo="Foo", bar="Bar")
    csv = _make_csv(_Options(fields=None, sequence_delim=object()))  # raise if used
...
def test_csv__call__w_fields_w_list():
    meta = _Meta(foo="Foo", bar=["Bar1", "Bar2"], baz="Baz")
    csv = _make_csv(_Options(fields=("foo", "bar"), item_delim="|", sequence_delim="*"))
    output = _capture_output(csv, meta)
...
def test_json__call___wo_fields_wo_list():
    ...
    meta = _Meta(name="foo", version="0.1", foo="Foo")
    json(meta)
    expected = collections.OrderedDict([("foo", "Foo"), ("name", "foo"), ("version", "0.1")])
    assert expected == json._mapping


def test_json__call___w_fields_w_list():
    json = _make_json(_Options(fields=("foo", "bar")))
    meta = _Meta(name="foo", version="0.1", foo="Foo", bar=["Bar1", "Bar2"], baz="Baz")
    json(meta)
    expected = collections.OrderedDict([("foo", "Foo"), ("bar", ["Bar1", "Bar2"])])
    assert expected == json._mapping
...
def test_json__call___output():
    ...
    output = json_module.loads(output, object_pairs_hook=collections.OrderedDict)
    expected = collections.OrderedDict([("foo", "Foo"), ("name", "foo"), ("version", "0.1")])
...
def test_main_w_dist_wo_download_url():
    ...
    with mock.patch("pkginfo.commandline.get_metadata") as _get_metadata:
        ...
        formatter = _call_main(["-d", "http://example.com", "/path/to/foo"])
