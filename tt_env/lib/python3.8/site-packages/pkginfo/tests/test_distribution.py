...
This is a description without newlines or leading whitespace."""
    assert _collapse_leading_ws("Description", DESCRIPTION) == DESCRIPTION
...
def test__collapse_leading_ws_w_descr_one_line_w_leading_ws():
    ...
    assert _collapse_leading_ws("Description", DESCRIPTION) == DESCRIPTION.strip()
...
The newlines should be preserved, and the whitespace stripped"""
    ...
    assert _collapse_leading_ws("Description", DESCRIPTION) == EXPECTED
...
This is a field value without newlines or leading whitespace."""
    assert _collapse_leading_ws("Other", OTHER) == OTHER
...
def test__collapse_leading_ws_w_other_one_line_w_leading_ws():
    ...
    assert _collapse_leading_ws("Other", OTHER) == OTHER.strip()
...
We expect them to be converted to spaces."""
    EXPECTED = "This is a field value with newlines.  " "We expect them to be converted to spaces."
    assert _collapse_leading_ws("Other", OTHER) == EXPECTED
...
def test_distribution_parse_Description():
    ...
    assert dist.description == "This package enables integration with foo servers."


def test_distribution_parse_Description_multiline():
    dist = _make_distribution()
    dist.parse("Description: This package enables integration with\n" "        foo servers.")
    assert dist.description == "This package enables integration with\nfoo servers."
...
def test_distribution_parse_Requires_External_multiple():
    ...
    assert list(dist.requires_external) == [
        "libfoo",
        "libbar (1.4)",
        "libbaz (>=2.3,<3.0)",
    ]
...
def test_distribution_parse_Requires_Dist_single_w_env_marker():
    ...
    assert list(dist.requires_dist) == ["SpanishInquisition; python_version == '1.4'"]
...
def test_distribution_parse_Requires_Dist_multiple():
    ...
    assert list(dist.requires_dist) == [
        "SpanishInquisition",
...
def test_distribution_parse_Provides_Dist_multiple():
    ...
    assert list(dist.provides_dist) == [
        "SillyWalks",
...
def test_distribution_parse_Obsoletes_Dist_multiple():
    ...
    assert list(dist.obsoletes_dist) == [
        "kniggits",
...
def test_distribution_parse_Project_URL_single_no_version():
    ...
    assert list(dist.project_urls) == ["Bug tracker, http://bugs.example.com/grail"]
