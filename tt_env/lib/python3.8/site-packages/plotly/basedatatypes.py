...
%s""" % (
                    trailing_underscores,
                    path,
                    display_string_positions(prop_idcs, disp_i, length=dict_item_len, char="^"),
                )
            else:
...
%s""" % (
                    path,
                    display_string_positions(prop_idcs, i, length=_len_dict_item(prop[i]), char="^"),
                )
...
%s"""
                    ...
                    for k, v in d.items():
                        ...
                        if update_target == () or overwrite:
                            if k == "data":
                                ...
                                # Accept v
                                self[k] = v
                        elif (isinstance(update_target, BasePlotlyType) and isinstance(v, (dict, BasePlotlyType))) or (
                            isinstance(update_target, tuple) and isinstance(update_target[0], BasePlotlyType)
                        ):
    ...
    def _select_layout_subplots_by_prefix(self, prefix, selector=None, row=None, col=None, secondary_y=None):
        ...
        layout_keys_filters = [
            ...
            lambda k: row is None or container_to_row_col.get(k, (None, None, None))[0] == row,
            lambda k: col is None or container_to_row_col.get(k, (None, None, None))[1] == col,
            lambda k: (secondary_y is None or container_to_row_col.get(k, (None, None, None))[2] == secondary_y),
        ]
        layout_keys = reduce(
    ...
    def _add_annotation_like(
        ...
        if row is not None:
            ...
            if row > len(grid_ref):
                raise IndexError(
                    "row index %d out-of-bounds, row index must be between 1 and %d, inclusive." % (row, len(grid_ref))
                )
            if col > len(grid_ref[row - 1]):
                raise IndexError(
                    "column index %d out-of-bounds, "
                    "column index must be between 1 and %d, inclusive." % (row, len(grid_ref[row - 1]))
                )
...
is of type {subplot_type}.""".format(
    ...
    because subplot does not have a secondary y-axis""".format(
            ...
            if exclude_empty_subplots and (
                not self._subplot_not_empty(xref, yref, selector=bool(exclude_empty_subplots))
            ):
                return self
...
""".format(
    ...
    def add_traces(
        ...
        if exclude_empty_subplots:
            data = list(
                filter(
                    lambda trace: self._subplot_not_empty(trace["xaxis"], trace["yaxis"], bool(exclude_empty_subplots)),
                    data,
                )
            )
    ...
    def _perform_batch_animate(self, animation_opts):
        ...
        if self._batch_trace_edits:
            animate_styles, animate_trace_indexes = zip(
                *[(trace_style, trace_index) for trace_index, trace_style in self._batch_trace_edits.items()]
            )
        else:
...
class BasePlotlyType(object):
                ...
                if self._compound_array_props.get(prop, None) is None:
                    ...
                    if self._props is not None:
                        self._compound_array_props[prop] = [
                            validator.data_class(_parent=self) for _ in self._props.get(prop, [])
                        ]
                    else:
                        self._compound_array_props[prop] = []
    ...
    def __repr__(self):
        ...
        props = {
            ...
            if p in self._valid_props and not isinstance(self._get_validator(p), LiteralValidator)
        }
...
class BaseLayoutType(BaseLayoutHierarchyType):
    ...
    def _set_subplotid_prop(self, prop, value):
        ...
        if suffix_digit == 0:
            raise TypeError(
                "Subplot properties may only be suffixed by an " "integer >= 1\n" "Received {k}".format(k=prop)
            )
