...
trendline_functions = dict(lowess=lowess, rolling=rolling, ewm=ewm, expanding=expanding, ols=ols)
...
def make_mapping(args, variable):
    ...
    return Mapping(
        ...
        updater=lambda trace, v: trace.update({parent: {".".join([variable] + other_variables): v}}),
        facet=None,
    )
    ...
    for attr_name in trace_spec.attrs:
        ...
        if attr_name == "dimensions":
            dims = [
                ...
                and (trace_spec.constructor != go.Parcoords or _is_continuous(args["data_frame"], name))
                and (
                    ...
                    if x.dtype.type == np.datetime64:
                        ...
                        try:
                            ...
                            raise ValueError(
                                ...
                                "If 'x' contains stringified dates, please convert to a datetime column." % args["x"]
                            )
            ...
            trace_spec.constructor in [go.Histogram2d, go.Histogram2dContour] and attr_name == "z"
        ):
        ...
        fig.layout.sliders = [
            {
                ...
                "yanchor": "top",
                "xanchor": "left",
                "currentvalue": {"prefix": get_label(args, args["animation_frame"]) + "="},
                "pad": {"b": 10, "t": 60},
                "len": 0.9,
                "x": 0.1,
                "y": 0,
...
def make_trace_spec(args, constructor, attrs, trace_patch):
    if constructor in [go.Scatter, go.Scatterpolar]:
        if "render_mode" in args and (
            ...
            or (args["render_mode"] == "auto" and len(args["data_frame"]) > 1000 and args["animation_frame"] is None)
        ):
...
def make_trendline_spec(args, constructor):
    trace_spec = TraceSpec(
        constructor=go.Scattergl if constructor == go.Scattergl else go.Scatter,  # could be contour
        attrs=["trendline"],
                ...
                # Check validity of column name
                    ...
                    if wide_mode and argument in (value_name, var_name):
                        ...
                        err_msg = (
                            ...
                            "Expected one of %s but received: %s" % (field, str(list(df_input.columns)), argument)
                        )
                        if argument == "index":
...
def process_dataframe_hierarchy(args):
    ...
    if args["values"]:
        try:
            ...
            raise ValueError("Column `%s` of `df` could not be converted to a numerical data type." % args["values"])
        ...
        count_colname = "count" if "count" not in df.columns else "".join([str(el) for el in list(df.columns)])
...
def infer_config(args, constructor, trace_patch, layout_patch):
    ...
    if "color" in args:
        ...
        show_colorbar = bool(
            ...
            and (constructor not in [go.Treemap, go.Sunburst, go.Icicle] or args.get("color_is_continuous"))
        )
    else:
        show_colorbar = False
    ...
    if "line_group" in args or "line_dash" in args:  # px.line, px.line_*, px.area, px.ecdf
        modes = set()
    ...
    if "trendline" in args and args["trendline"] is not None:
        if args["trendline"] not in trendline_functions:
            raise ValueError(
                "Value '%s' for `trendline` must be one of %s" % (args["trendline"], trendline_functions.keys())
            )
    ...
    if "ecdfnorm" in args:
        if args.get("ecdfnorm", None) not in [None, "percent", "probability"]:
            raise ValueError(
                "`ecdfnorm` must be one of None, 'percent' or 'probability'. " + "'%s' was provided." % args["ecdfnorm"]
            )
...
def make_figure(args, constructor, trace_patch=None, layout_patch=None):
            ...
            for i, m in enumerate(grouped_mappings):
                ...
                try:
                    ...
                    if (
                        ...
                        or (trace_spec.constructor in [go.Histogram] and m.variable in ["symbol", "dash"])
                    ):
                        pass
                    elif (
                        trace_spec != trace_specs[0]
    ...
    if show_colorbar:
        ...
        layout_patch["coloraxis1"] = dict(
            ...
            cmin=range_color[0],
            cmax=range_color[1],
            colorbar=dict(title_text=get_decorated_label(args, args[colorvar], colorvar)),
        )
