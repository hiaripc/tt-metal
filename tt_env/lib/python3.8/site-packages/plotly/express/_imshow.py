...
def _vectorize_zvalue(z, mode="max"):
    ...
    if z is None:
        ...
        raise ValueError(
            "zmax can be a scalar, or an iterable of length 1, 3 or 4. " "A value of %s was passed for zmax." % str(z)
        )
...
def imshow(
    ...
    slice_dimensions = (facet_col is not None) + (animation_frame is not None)  # 0, 1, or 2
    facet_label = None
    ...
    if animation_frame is not None:
        ...
        args["animation_frame"] = (
            "animation_frame" if labels.get("animation_frame") is None else labels["animation_frame"]
        )
    iterables = ()
    if animation_frame is not None:
    ...
    if img.ndim == 2 + slice_dimensions and not binary_string:
        ...
        if y is not None and img.shape[y_index] != len(y):
            raise ValueError(
                "The length of the y vector must match the length of the first " + "dimension of the img matrix."
            )
        x_index = slice_dimensions + 1
        if x is not None and img.shape[x_index] != len(x):
            raise ValueError(
                "The length of the x vector must match the length of the second " + "dimension of the img matrix."
            )
    ...
    # For 2D+RGB data, use Image trace
    elif (img.ndim >= 3 and (img.shape[-1] in [3, 4] or slice_dimensions and binary_string)) or (
        img.ndim == 2 and binary_string
    ):
        ...
        if x is not None:
            ...
            if np.issubdtype(x.dtype, np.number):
                ...
                error_msg = (
                    error_msg_xarray
                    if img_is_xarray
                    else ("Only numerical values are accepted for the `x` parameter " "when an Image trace is used.")
                )
        ...
        if y is not None:
            ...
            if np.issubdtype(y.dtype, np.number):
                ...
                error_msg = (
                    error_msg_xarray
                    if img_is_xarray
                    else ("Only numerical values are accepted for the `y` parameter " "when an Image trace is used.")
                )
                raise ValueError(error_msg)
        if binary_string:
        ...
        raise ValueError(
            ...
            "visualized using the `facet_col` or/and `animation_frame` arguments." % str(img.shape)
        )
