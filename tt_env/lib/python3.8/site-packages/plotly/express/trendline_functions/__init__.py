...
def ols(trendline_options, x_raw, x, y, x_label, y_label, non_missing):
    ...
    for k in trendline_options.keys():
        if k not in valid_options:
            raise ValueError(
                "OLS trendline_options keys must be one of [%s] but got '%s'" % (", ".join(valid_options), k)
            )
    ...
    if log_y:
        if np.any(y <= 0):
            raise ValueError("Can't do OLS trendline with `log_y=True` when `y` contains non-positive values.")
        y = np.log10(y)
        y_label = "log10(%s)" % y_label
    if log_x:
        if np.any(x <= 0):
            raise ValueError("Can't do OLS trendline with `log_x=True` when `x`  contains non-positive values.")
        x = np.log10(x)
...
def lowess(trendline_options, x_raw, x, y, x_label, y_label, non_missing):
    ...
    for k in trendline_options.keys():
        if k not in valid_options:
            raise ValueError(
                "LOWESS trendline_options keys must be one of [%s] but got '%s'" % (", ".join(valid_options), k)
            )
