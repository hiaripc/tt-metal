...
def _ternary_layout(title="Ternary contour plot", width=550, height=525, pole_labels=["a", "b", "c"]):
    ...
    return dict(
        ...
        ternary=dict(
            sum=1,
            aaxis=dict(title=dict(text=pole_labels[0]), min=0.01, linewidth=2, ticks="outside"),
            baxis=dict(title=dict(text=pole_labels[1]), min=0.01, linewidth=2, ticks="outside"),
            caxis=dict(title=dict(text=pole_labels[2]), min=0.01, linewidth=2, ticks="outside"),
        ),
        showlegend=False,
    )
...
def _replace_zero_coords(ternary_data, delta=0.0005):
    ...
    if np.any(unity_complement) < 0:
        raise ValueError("The provided value of delta led to negative" "ternary coords.Set a smaller delta")
