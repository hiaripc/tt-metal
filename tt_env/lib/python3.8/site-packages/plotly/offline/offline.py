...
def get_image_download_script(caller):
    ...
    return (
        ...
        "width: width, filename: filename}});}};" + check_start + "downloadimage('{format}', {height}, {width}, "
        "'{filename}');" + check_end
    )
...
def iplot(
    ...
    post_script = build_save_image_post_script(image, filename, image_height, image_width, "iplot")
...
def plot(
    ...
    if not filename.endswith(".html") and output_type == "file":
        warnings.warn(
            "Your filename `" + filename + "` didn't end with .html. " "Adding .html to the end of your file."
        )
        filename += ".html"
    ...
    # Handle image request
    post_script = build_save_image_post_script(image, image_filename, image_height, image_width, "plot")
...
def enable_mpl_offline(
    ...
    formatter.for_type(
        ...
        lambda fig: iplot_mpl(fig, resize, strip_style, verbose, show_link, link_text, validate),
    )
