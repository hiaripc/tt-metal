    ...
    * ``ALLOW_ABBREV`` - Controls whether partial switch names are supported, for example '--ver' will match
            ...
            for obj in cls.__dict__.values():
                ...
                for name in swinfo.names:
                    ...
                    if name in self._switches_by_name and not self._switches_by_name[name].overridable:
                        raise SwitchError(T_("Switch {name} already defined and is not overridable").format(name=name))
        ...
        while argv:
            ...
            if a.startswith("--") and len(a) >= 3:
                ...
                if self.ALLOW_ABBREV:
                    ...
                    if len(partials) == 1:
                        ...
                        raise UnknownSwitch(T_("Ambiguous partial switch {0}").format("--" + name))
                ...
                if swinfo.argtype:
                    if not argv:
                        raise MissingArgument(T_("Switch {0} requires an argument").format(swname))
                    a = argv.pop(0)
                    if a and a[0] == "=":
                        if len(a) >= 2:
                            val = a[1:]
            ...
            if swinfo.func in swfuncs:
                if swinfo.list:
                    ...
                    raise SwitchError(
                        T_("Switch {0} already given ({1} is equivalent)").format(swfuncs[swinfo.func].swname, swname)
                    )
            else:
                if swinfo.list:
    ...
    def _validate_args(self, swfuncs, tailargs):
        ...
        for swinfo in self._switches_by_func.values():
            ...
            requirements[swinfo.func] = {self._switches_by_name[req] for req in swinfo.requires}
        ...
        if hasattr(self.main, "positional"):
            ...
            for item in m.annotations:
                annotation = sig.parameters[item].annotation if item != "return" else sig.return_annotation
            ...
            tailargs = self._positional_validate(tailargs, positional, varargs, m.args[1:], m.varargs)
    ...
    def _parse_kwd_args(self, switches):
        ...
        for index, (swname, val) in enumerate(switches.items(), 1):
            ...
            if isinstance(switch_local, CountOf):
                ...
                raise SwitchError(T_("Switch {0} must be a sequence (iterable)").format(swname))
        ...
        for switch_info, prefix, color in switchs(by_groups, True):
            ...
            if switch_info.requires:
                help_txt += T_("; requires {0}").format(
                    ", ".join((("-" if len(switch) == 1 else "--") + switch) for switch in switch_info.requires)
                )
            if switch_info.excludes:
                help_txt += T_("; excludes {0}").format(
                    ", ".join((("-" if len(switch) == 1 else "--") + switch) for switch in switch_info.excludes)
                )

            msg = indentation.join(wrapper.wrap(" ".join(ln.strip() for ln in help_txt.splitlines())))
            ...
            for name, subcls in sorted(self._subcommands.items()):
                with gc:
                    ...
                    if self.SUBCOMMAND_HELPMSG:
                        ...
                        help_str += self.SUBCOMMAND_HELPMSG.format(parent=self.PROGNAME, sub=name)
                    else:
                        help_str = doc or ""

                    msg = indentation.join(wrapper.wrap(" ".join(ln.strip() for ln in help_str.splitlines())))
                    ...
                    print(description_indent.format(subcls.name, padding, bodycolor | colors.filter(msg)))
