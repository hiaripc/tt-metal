...
class Color:
    ...
    def hex_code(self):
        ...
        return "#000000" if self.isreset else f"#{self.rgb[0]:02X}{self.rgb[1]:02X}{self.rgb[2]:02X}"
...
class Style(metaclass=ABCMeta):
    ...
    def __repr__(self):
        ...
        neg_attributes = ", ".join(f"-{a}" for a in self.attributes if not self.attributes[a])
        colors = ", ".join(repr(c) for c in (self.fg, self.bg) if c)
        string = "; ".join(s for s in (attributes, neg_attributes, colors) if s) or "empty"
        if self.isreset:
            string = "reset"
...
class ANSIStyle(Style):
    ...
    def __str__(self):
        return self.limit_representation(self.use_color).ansi_sequence if self.use_color else ""
