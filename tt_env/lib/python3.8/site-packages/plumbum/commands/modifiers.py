...
class _RETCODE(ExecutionModifier):
    ...
    def __rand__(self, cmd):
        if self.foreground:
            result = cmd.run(retcode=None, stdin=None, stdout=None, stderr=None, timeout=self.timeout)
            return result[0]
...
class LogPipe:
    ...
    def __rand__(self, cmd):
        ...
        for typ, lines in popen.iter_lines(line_timeout=self.line_timeout, mode=BY_TYPE, **self.kw):
            if not lines:
                continue
