...
class RemoteEnv(BaseEnv):
    ...
    def __init__(self, remote):
        ...
        if env0[0] == 0 and not env0[2].rstrip():
            _curr = dict(line.split("=", 1) for line in env0[1].split("\x00") if "=" in line)
        else:
    ...
    def update(self, *args, **kwargs):
        ...
        self.remote._session.run("export " + " ".join(f"{k}={shquote(v)}" for k, v in self.getdict().items()))
...
class BaseRemoteMachine(BaseMachine):
    ...
    * ``connect_timeout`` - the connection timeout
    ...
    def _get_uname(self):
        ...
        rc, out, _ = self._session.run("python3 -c 'import platform;print(platform.uname()[0])'", retcode=None)
        if rc == 0:
            return out.strip()
    ...
    def __getitem__(self, cmd):
        ...
        if not isinstance(cmd, LocalPath):
            return self.RemoteCommand(self, self.path(cmd) if "/" in cmd or "\\" in cmd else self.which(cmd))
    ...
    def tempdir(self):
        ...
        the context exits"""
        _, out, _ = self._session.run("mktemp -d 2>/dev/null || mktemp -d tmp.XXXXXXXXXX")
    ...
    def _path_glob(self, fn, pattern):
        ...
        fn = fn.replace(" ", r"\ ")
        matches = self._session.run(rf"for fn in {fn}/{pattern}; do echo $fn; done")[1].splitlines()
