...
class RemotePath(Path):
    ...
    def glob(self, pattern):
        return self._glob(
            pattern,
            lambda pat: [RemotePath(self.remote, m) for m in self.remote._path_glob(self, pat)],
        )
    ...
    def move(self, dst):
        if isinstance(dst, RemotePath):
            ...
            raise TypeError(f"dst must be a string or a RemotePath (to the same remote machine), got {dst!r}")
    ...
    def mkdir(self, mode=None, parents=True, exist_ok=True):
        if parents and exist_ok:
            ...
            try:
                ...
                if not exist_ok:
                    raise OSError(errno.EEXIST, "File exists (on remote end)", str(self)) from None
    ...
    def symlink(self, dst):
        if isinstance(dst, RemotePath):
            ...
            raise TypeError("dst must be a string or a RemotePath (to the same remote machine), got {dst!r}")
    ...
    def open(self, mode="r", bufsize=-1, *, encoding=None):
        ...
        if encoding is not None:
            raise NotImplementedError("encoding not supported for ParamikoMachine paths")
