...
class Pooch:
    ...
    def load_registry_from_doi(self):
        ...
        if not isinstance(downloader, DOIDownloader):
            raise ValueError(
                f"Invalid base_url '{self.base_url}': " + "Pooch.load_registry_from_doi is only implemented for DOIs"
            )
...
def stream_download(url, fname, known_hash, downloader, pooch=None, retry_if_failed=0):
    ...
    for i in range(download_attempts):
        try:
            ...
            get_logger().info(
                "Failed to download '%s'. " "Will attempt the download again %d more time%s.",
                str(fname.name),
                retries_left,
