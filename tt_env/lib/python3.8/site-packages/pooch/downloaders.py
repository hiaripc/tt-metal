...
class ZenodoRepository(DataRepository):  # pylint: disable=missing-class-docstring
    ...
    def api_version(self):
        ...
        if self._api_version is None:
            if all("key" in file for file in self.api_response["files"]):
                ...
                raise ValueError(
                    "Couldn't determine the version of the Zenodo API for " f"{self.archive_url} (doi:{self.doi})."
                )
    ...
    def download_url(self, file_name):
        ...
        if file_name not in files:
            raise ValueError(f"File '{file_name}' not found in data archive " f"{self.archive_url} (doi:{self.doi}).")
        # Build download url
        if self.api_version == "legacy":
    ...
    def api_response(self):
        ...
        if self._api_response is None:
            ...
            if version is None:
                ...
                api_url = "https://api.figshare.com/v2/articles/" f"{article_id}/versions/{version}"
...
class DataverseRepository(DataRepository):  # pylint: disable=missing-class-docstring
    ...
    def _get_api_response(cls, doi, archive_url):
        ...
        response = requests.get(
            f"{parsed['protocol']}://{parsed['netloc']}/api/datasets/" f":persistentId?persistentId=doi:{doi}",
            timeout=DEFAULT_TIMEOUT,
        )
        return response
    ...
    def api_response(self):
        ...
        if self._api_response is None:
            self._api_response = self._get_api_response(self.doi, self.archive_url)  # pragma: no cover
    ...
    def download_url(self, file_name):
        ...
        download_url = f"{parsed['protocol']}://{parsed['netloc']}/api/access/datafile/" f"{files[file_name]['id']}"
        return download_url
