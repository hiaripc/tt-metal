            ...
            if self.members is None:
                ...
                for member in self.members:
                    ...
                    subdir_members = [
                        ...
                        if os.path.normpath(info.name).startswith(os.path.normpath(member))
                    ]
                    # Extract the data file from within the archive
...
class Decompress:  # pylint: disable=too-few-public-methods
    ...
    def _compression_module(self, fname):
        ...
        if self.method not in self.modules:
            message = f"Invalid compression method '{self.method}'. " f"Must be one of '{list(self.modules.keys())}'."
        ...
        if self.method == "auto":
            ...
            if ext not in self.extensions:
                message = f"Unrecognized file extension '{ext}'. " f"Must be one of '{list(self.extensions.keys())}'."
                if ext in {".zip", ".tar"}:
                    message = " ".join([message, error_archives])
                raise ValueError(message)
