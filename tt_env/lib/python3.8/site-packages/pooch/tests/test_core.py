...
def test_pooch_download_retry(monkeypatch):
    ...
    with TemporaryDirectory() as local_store:
        ...
        true_path = str(path / "tiny-data.txt")
        retries = 11
        pup = Pooch(path=path, base_url=BASEURL, registry=REGISTRY, retry_if_failed=retries)
...
def test_pooch_file_not_in_registry():
    ...
    pup = Pooch(path="it shouldn't matter", base_url="this shouldn't either", registry=REGISTRY)
    with pytest.raises(ValueError):
