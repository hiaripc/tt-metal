...
def _check_args_length(hook_type: str, args: Sequence[str]) -> None:
    if hook_type == "prepare-commit-msg":
        if len(args) < 1 or len(args) > 3:
            raise SystemExit(
                f"hook-impl for {hook_type} expected 1, 2, or 3 arguments " f"but got {len(args)}: {args}",
            )
        ...
        if len(args) != expected:
            ...
            raise SystemExit(
                f"hook-impl for {hook_type} expected {expected} {arguments_s} " f"but got {len(args)}: {args}",
            )
    else:
