...
RE_PREFIX = re.compile(
    r"(?P<value>[-+]?\d+\.?(?:\d+)?(?:[eE]?\d)?) ?" r"(?P<prefix>(?:[a-zA-Z\u03bc\u00B5]|\xce\xbc|\xc2\xb5)i?)$"
)
...
def _convert(value, spec):
    ...
    if magnitude:
        value /= magnitude
        prefix = "%s%s%s" % (
            "" if spec["prefix_space"] is None else " ",
            prefixes[magnitude],
    ...
    def __new__(cls, value=0.0):
        ...
        if isinstance(value, BASESTRING):
            ...
            if match:
                ...
                elif prefix in {"µ", "µ"}:  # pylint: disable=duplicate-value  # Python 2.7
        ...
        try:
            ...
            raise_from_none(ValueError("Could not convert %s to Float: %r" % (value.__class__.__name__, value)))
        except TypeError:
            raise_from_none(TypeError("Can't convert %s to Float: %r" % (value.__class__.__name__, value)))
