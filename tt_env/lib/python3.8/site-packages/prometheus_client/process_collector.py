    ...
    def collect(self) -> Iterable[Metric]:
        ...
        try:
            ...
            vmem = GaugeMetricFamily(
                self._prefix + "virtual_memory_bytes", "Virtual memory size in bytes.", value=float(parts[20])
            )
        ...
        try:
            ...
            open_fds = GaugeMetricFamily(
                self._prefix + "open_fds", "Number of open file descriptors.", len(os.listdir(os.path.join(pid, "fd")))
            )
