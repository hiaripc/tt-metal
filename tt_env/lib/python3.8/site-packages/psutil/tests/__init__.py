...
class TestMemoryLeak(PsutilTestCase):
    ...
    def _check_fds(self, fun):
        ...
        diff = after - before
        if diff < 0:
            msg = f"negative diff {diff!r} (gc probably collected a" " resource from a previous test)"
            raise self.fail(msg)
        if diff > 0:
    ...
    def test_class_coverage(cls, test_class, ls):
        ...
        for fun_name, _, _ in ls:
            ...
            if not hasattr(test_class, meth_name):
                msg = f"{test_class.__class__.__name__!r} class should define a" f" {meth_name!r} method"
