...
class FreeBSDPsutilTestCase(PsutilTestCase):
    ...
    def test_cmdline(self):
        ...
        assert " ".join(psutil.Process(self.pid).cmdline()) == " ".join(out.split("\n")[1].split()[2:])
...
class NetBSDTestCase(PsutilTestCase):
    ...
    def test_vmem_free(self):
        assert abs(psutil.virtual_memory().free - self.parse_meminfo("MemFree:")) < TOLERANCE_SYS_MEM

    def test_vmem_buffers(self):
        assert abs(psutil.virtual_memory().buffers - self.parse_meminfo("Buffers:")) < TOLERANCE_SYS_MEM

    def test_vmem_shared(self):
        assert abs(psutil.virtual_memory().shared - self.parse_meminfo("MemShared:")) < TOLERANCE_SYS_MEM

    def test_vmem_cached(self):
        assert abs(psutil.virtual_memory().cached - self.parse_meminfo("Cached:")) < TOLERANCE_SYS_MEM

    # --- swap mem

    def test_swapmem_total(self):
        assert abs(psutil.swap_memory().total - self.parse_meminfo("SwapTotal:")) < TOLERANCE_SYS_MEM
