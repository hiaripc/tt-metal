...
def get_ipv4_netmask(ifname):
    ...
    ifname = bytes(ifname[:15], "ascii")
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        return socket.inet_ntoa(fcntl.ioctl(s.fileno(), SIOCGIFNETMASK, struct.pack("256s", ifname))[20:24])
...
def get_ipv4_broadcast(ifname):
    ...
    ifname = bytes(ifname[:15], "ascii")
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        return socket.inet_ntoa(fcntl.ioctl(s.fileno(), SIOCGIFBRDADDR, struct.pack("256s", ifname))[20:24])
...
class TestSystemVirtualMemoryAgainstFree(PsutilTestCase):
    ...
    def test_shared(self):
        ...
        assert abs(free_value - psutil_value) < TOLERANCE_SYS_MEM, f"{free_value} {psutil_value} \n{free.output}"
    ...
    def test_no_vmstat_mocked(self):
        ...
        with mock_open_exception("/proc/vmstat", FileNotFoundError) as m:
            with warnings.catch_warnings(record=True) as ws:
                ...
                assert len(ws) == 1
                w = ws[0]
                assert "'sin' and 'sout' swap memory stats couldn't " "be determined and were set to 0" in str(
                    w.message
                )
                assert ret.sin == 0
                assert ret.sout == 0
