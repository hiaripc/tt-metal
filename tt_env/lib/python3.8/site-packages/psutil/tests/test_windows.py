...
class TestSystemAPIs(WindowsTestCase):
    ...
    def test_disk_partitions(self):
        sys_value = [
            x + "\\" for x in win32api.GetLogicalDriveStrings().split("\\\x00") if x and not x.startswith("A:")
        ]
        psutil_value = [x.mountpoint for x in psutil.disk_partitions(all=True) if not x.mountpoint.startswith("A:")]
        assert sys_value == psutil_value
