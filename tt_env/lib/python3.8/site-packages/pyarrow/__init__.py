...
try:
    ...
    try:
        ...
        def parse_git(root, **kwargs):
            ...
            kwargs["describe_command"] = "git describe --dirty --tags --long --match 'apache-arrow-[0-9]*.*'"
...
def show_versions():
    ...
    print_entry(
        "Package kind", cpp_build_info.package_kind if len(cpp_build_info.package_kind) > 0 else "not indicated"
    )
    print_entry("Arrow C++ library version", cpp_build_info.version)
    print_entry("Arrow C++ compiler", f"{cpp_build_info.compiler_id} {cpp_build_info.compiler_version}")
...
def show_info():
    ...
    print("\nOptional modules:")
    modules = ["csv", "cuda", "dataset", "feather", "flight", "fs", "gandiva", "json", "orc", "parquet"]
...
from pyarrow.lib import Buffer, ResizableBuffer, foreign_buffer, py_buffer, Codec, compress, decompress, allocate_buffer
        ...
        if _has_pkg_config(pkgname):
            ...
            if library_dir:
                if not library_dir.startswith("-L"):
                    raise ValueError("pkg-config --libs-only-L returned unexpected " "value {!r}".format(library_dir))
