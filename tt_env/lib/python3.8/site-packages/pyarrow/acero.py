...
try:
    ...
    raise ImportError(f"The pyarrow installation is not built with support for 'acero' ({str(exc)})") from None
    ...
    if coalesce_keys and join_type == "full outer":
        ...
        for idx, col in enumerate(left_columns + right_columns):
            if idx < len(left_columns) and col in left_column_keys_indices:
                ...
                projections.append(
                    Expression._call(
                        "coalesce", [Expression._field(idx), Expression._field(right_operand_index + right_key_index)]
                    )
                )
        ...
        projection = Declaration("project", ProjectNodeOptions(projections, projected_col_names))
...
def _perform_join_asof(
    left_operand, left_on, left_by, right_operand, right_on, right_by, tolerance, use_threads=True, output_type=Table
):
    """
    ...
    right_columns = [col for col in right_operand.schema.names if col not in [right_on] + right_by]
    ...
    decl = Declaration("asofjoin", options=join_opts, inputs=[left_source, right_source])
