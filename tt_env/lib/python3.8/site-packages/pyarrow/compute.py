...
def _decorate_compute_function(wrapper, exposed_name, func, options_class):
    ...
    wrapper.__arrow_compute_function__ = dict(
        name=func.name, arity=func.arity, options_class=cpp_doc.options_class, options_required=cpp_doc.options_required
    )
    ...
    if options_class is not None:
        ...
        if options_class_doc:
            ...
            warnings.warn(f"Options class {options_class.__name__} " f"does not have a docstring", RuntimeWarning)
...
def _handle_options(name, options_class, options, args, kwargs):
    if args or kwargs:
        if options is not None:
            raise TypeError(
                "Function {!r} called with both an 'options' argument " "and additional arguments".format(name)
            )
    ...
    if options is not None:
        if isinstance(options, dict):
            ...
            return options
        raise TypeError("Function {!r} expected a {} parameter, got {}".format(name, options_class, type(options)))
    ...
    if options_class is None:
        ...
        def wrapper(*args, memory_pool=None):
            if arity is not Ellipsis and len(args) != arity:
                raise TypeError(f"{func_name} takes {arity} positional argument(s), " f"but {len(args)} were given")
    ...
    *name_or_index : string, multiple strings, tuple or int
    ...
    if n == 1:
        if isinstance(name_or_index[0], (str, int)):
            ...
            raise TypeError(
                "field reference should be str, multiple str, tuple or " f"integer, got {type(name_or_index[0])}"
            )
