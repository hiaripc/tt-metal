    ...
    if flavor is None:
        ...
        if schema is not None:
            ...
            if isinstance(field_names, list):
                ...
                raise ValueError("Expected list of field names, got {}".format(type(field_names)))
        else:
    ...
    if flavor == "filename":
        ...
        if field_names is not None:
            ...
            if isinstance(schema, pa.Schema):
                ...
                raise ValueError("Expected Schema for 'schema', got {}".format(type(schema)))
        else:
...
def _ensure_partitioning(scheme):
    ...
    if scheme is None:
        ...
        raise ValueError("Expected Partitioning or PartitioningFactory, got {}".format(type(scheme)))
    return scheme
...
def _ensure_multiple_sources(paths, filesystem=None):
    ...
    is_local = isinstance(filesystem, (LocalFileSystem, _MockFileSystem)) or (
        isinstance(filesystem, SubTreeFileSystem) and isinstance(filesystem.base_fs, LocalFileSystem)
    )
...
def parquet_dataset(
    ...
    options = ParquetFactoryOptions(
        partition_base_dir=partition_base_dir, partitioning=_ensure_partitioning(partitioning)
    )
