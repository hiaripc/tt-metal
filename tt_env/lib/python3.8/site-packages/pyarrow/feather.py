...
def write_feather(df, dest, compression=None, compression_level=None, chunksize=None, version=2):
    ...
    if version == 1:
        ...
        if compression is None and Codec.is_available("lz4_frame"):
            ...
            raise ValueError(
                'compression="{}" not supported, must be ' "one of {}".format(compression, _FEATHER_SUPPORTED_CODECS)
            )
...
def read_feather(source, columns=None, use_threads=True, memory_map=False, **kwargs):
    ...
    **kwargs
    ...
    """
    return read_table(source, columns=columns, memory_map=memory_map, use_threads=use_threads).to_pandas(
...
def read_table(source, columns=None, memory_map=False, use_threads=True):
    ...
    if all(map(lambda t: t == int, column_types)):
        ...
        raise TypeError(
            "Columns must be indices or names. " "Got columns {} of types {}".format(columns, column_type_names)
        )
