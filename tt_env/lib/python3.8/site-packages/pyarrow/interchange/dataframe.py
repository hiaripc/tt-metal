...
class _PyArrowDataFrame:
    ...
    def __init__(self, df: pa.Table | pa.RecordBatch, nan_as_null: bool = False, allow_copy: bool = True) -> None:
        ...
        if nan_as_null is True:
            raise RuntimeError("nan_as_null=True currently has no effect, " "use the default nan_as_null=False")
    ...
    def metadata(self) -> dict[str, Any]:
        ...
        if self._df.schema.metadata:
            schema_metadata = {
                "pyarrow." + k.decode("utf8"): v.decode("utf8") for k, v in self._df.schema.metadata.items()
            }
    ...
    def get_columns(self) -> Iterable[_PyArrowColumn]:
        ...
        """
        return [_PyArrowColumn(col, allow_copy=self._allow_copy) for col in self._df.columns]

    def select_columns(self, indices: Sequence[int]) -> _PyArrowDataFrame:
        """
        Create a new DataFrame by selecting a subset of columns by index.
        """
        return _PyArrowDataFrame(self._df.select(list(indices)), self._nan_as_null, self._allow_copy)

    def select_columns_by_name(self, names: Sequence[str]) -> _PyArrowDataFrame:
        """
        Create a new DataFrame by selecting a subset of columns by name.
        """
        return _PyArrowDataFrame(self._df.select(list(names)), self._nan_as_null, self._allow_copy)
    ...
    def get_chunks(self, n_chunks: Optional[int] = None) -> Iterable[_PyArrowDataFrame]:
        ...
        iterator = [_PyArrowDataFrame(batch, self._nan_as_null, self._allow_copy) for batch in batches]
        return iterator
