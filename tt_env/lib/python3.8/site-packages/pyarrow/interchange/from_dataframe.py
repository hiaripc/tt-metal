...
def bool_column_to_array(
    ...
    if size == 8 and not allow_copy:
        raise RuntimeError(
            "Boolean column will be casted from uint8 and a copy " "is required which is forbidden by allow_copy=False"
        )
...
def validity_buffer_from_mask(
    ...
    if null_kind == ColumnNullType.NON_NULLABLE:
        ...
        if null_kind == ColumnNullType.USE_BYTEMASK:
            if not allow_copy:
                raise RuntimeError(
                    "To create a bitmask a copy of the data is " "required which is forbidden by allow_copy=False"
                )
...
def validity_buffer_nan_sentinel(
    ...
    if null_kind == ColumnNullType.USE_NAN:
        ...
        if kind == DtypeKind.FLOAT and bit_width == 16:
            ...
            # for float16
            raise NotImplementedError(f"{data_type} with {null_kind} is not yet supported.")
        else:
    ...
    # Check for sentinel values
        ...
        pyarrow_data = pa.Array.from_buffers(sentinel_dtype, length, [None, data_pa_buffer], offset=offset)
