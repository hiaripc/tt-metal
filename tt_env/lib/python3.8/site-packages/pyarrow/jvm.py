...
def field(jvm_field):
    ...
    if not jvm_type.isComplex():
        ...
        raise NotImplementedError("JVM field conversion only implemented for primitive types.")
    ...
    if jvm_metadata.isEmpty():
        ...
        metadata = {str(entry.getKey()): str(entry.getValue()) for entry in jvm_metadata.entrySet()}
...
def array(jvm_array):
    ...
    if jvm_array.getField().getType().isComplex():
        ...
        raise NotImplementedError(
            "Cannot convert JVM Arrow array of type {}," " complex types not yet implemented.".format(minor_type_str)
        )
