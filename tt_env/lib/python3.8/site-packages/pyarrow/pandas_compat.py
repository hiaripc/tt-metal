...
def construct_metadata(columns_to_convert, df, column_names, index_levels, index_descriptors, preserve_index, types):
    ...
    dict
    """
    num_serialized_index_levels = len([descr for descr in index_descriptors if not isinstance(descr, dict)])
    ...
    column_metadata = []
    for col, sanitized_name, arrow_type in zip(columns_to_convert, column_names, df_types):
        metadata = get_column_metadata(col, name=sanitized_name, arrow_type=arrow_type, field_name=sanitized_name)
        column_metadata.append(metadata)

    index_column_metadata = []
    if preserve_index is not False:
        non_str_index_names = []
        for level, arrow_type, descriptor in zip(index_levels, index_types, index_descriptors):
    ...
    for name in schema.names:
        try:
            ...
            try:
                ...
                raise KeyError(
                    "name '{}' present in the specified schema is not found " "in the columns or index".format(name)
                )
...
def dataframe_to_types(df, preserve_index, columns=None):
    (all_names, column_names, _, index_descriptors, index_columns, columns_to_convert, _) = _get_columns_to_convert(
    ...
    metadata = construct_metadata(
        columns_to_convert, df, column_names, index_columns, index_descriptors, preserve_index, types
    )
...
def _reconstruct_block(item, columns=None, extension_columns=None, return_block=True):
    ...
    if "dictionary" in item:
        arr = _pandas_api.categorical_type.from_codes(block_arr, categories=item["dictionary"], ordered=item["ordered"])
...
def table_to_dataframe(options, table, categories=None, ignore_metadata=False, types_mapper=None):
    ...
    result = pa.lib.table_to_blocks(options, table, categories, list(ext_columns_dtypes.keys()))
    if _pandas_api.is_ge_v3():
        from pandas.api.internals import create_dataframe_from_blocks

        blocks = [_reconstruct_block(item, column_names, ext_columns_dtypes, return_block=False) for item in result]
        ...
        blocks = [_reconstruct_block(item, column_names, ext_columns_dtypes) for item in result]
        axes = [columns, index]
...
def _deserialize_column_index(block_table, all_columns, column_indexes):
    if all_columns:
        ...
        columns_values = [columns_name_dict.get(name, name) for name in block_table.column_names]
    else:
    ...
    if len(column_indexes) > 1:
        ...
        columns = _pandas_api.pd.Index(columns_values, name=column_indexes[0]["name"] if column_indexes else None)
...
def _reconstruct_columns_from_metadata(columns, column_indexes):
    ...
    * Part of :func:`~pyarrow.pandas_compat.table_to_blockmanager`
    ...
    levels_dtypes = [
        (level, col_index.get("pandas_type", str(level.dtype)), col_index.get("numpy_type", None))
