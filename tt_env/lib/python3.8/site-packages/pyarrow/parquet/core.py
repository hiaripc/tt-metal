...
try:
    ...
    raise ImportError(
        "The pyarrow installation is not built with support " f"for the Parquet file format ({str(exc)})"
    ) from None
...
from pyarrow.fs import LocalFileSystem, FileSystem, FileType, _resolve_filesystem_and_path, _ensure_filesystem
...
_filters_to_expression = _deprecate_api(
    "_filters_to_expression", "filters_to_expression", filters_to_expression, "10.0.0", DeprecationWarning
)
...
class ParquetFile:
    ...
    def __init__(
        ...
        *,
        ...
        filesystem, source = _resolve_filesystem_and_path(source, filesystem, memory_map=memory_map)
    ...
    def iter_batches(
        self, batch_size=65536, row_groups=None, columns=None, use_threads=True, use_pandas_metadata=False
    ):
        """
        ...
        batches = self.reader.iter_batches(
            batch_size, row_groups=row_groups, column_indices=column_indices, use_threads=use_threads
        )
        return batches
    ...
    def _get_column_indices(self, column_names, use_pandas_metadata=False):
        ...
        if use_pandas_metadata:
            ...
            if indices is not None and index_columns:
                indices += [
                    self.reader.column_name_idx(descr) for descr in index_columns if not isinstance(descr, dict)
                ]
    ...
    def __init__(
        ...
        **options,
        ...
        if filesystem is not None:
            ...
            sink = self.file_handle = filesystem.open_output_stream(path, compression=None)
        else:
            sink = where
    ...
    def read(self, columns=None, use_threads=True, use_pandas_metadata=False):
        ...
        if columns is not None and use_pandas_metadata:
            if metadata and b"pandas" in metadata:
                ...
                index_columns = [col for col in _get_pandas_index_columns(metadata) if not isinstance(col, dict)]
                columns = list(columns) + list(set(index_columns) - set(columns))

        table = self._dataset.to_table(columns=columns, filter=self._filter_expression, use_threads=use_threads)
...
def read_table(
    ...
    *,
    ...
    try:
        ...
        if filters is not None:
            raise ValueError(
                "the 'filters' keyword is not supported when the " "pyarrow.dataset module is not available"
            )
        if partitioning != "hive":
            raise ValueError(
                "the 'partitioning' keyword is not supported when the " "pyarrow.dataset module is not available"
            )
        if schema is not None:
            raise ValueError(
                "the 'schema' argument is not supported when the " "pyarrow.dataset module is not available"
            )
...
def read_metadata(where, memory_map=False, decryption_properties=None, filesystem=None):
    ...
    with file_ctx:
        file = ParquetFile(where, memory_map=memory_map, decryption_properties=decryption_properties)
