...
def test_pandas_to_pyarrow_with_missing(np_float):
    ...
    expected = pa.table(
        {"a": pa.array(np_array, from_pandas=True), "dt": pa.array(datetime_array, type=pa.timestamp("ns"))}
    )
    ...
    table = pa.table(
        {
            "a": pa.array(arr, type=uint),
            "b": pa.array(arr, type=int),
            "c": pa.array(np.array(arr, dtype=np_float), type=float, from_pandas=True),
            "d": [True, False, True],
            "e": [True, False, None],
            "f": ["a", None, "c"],
