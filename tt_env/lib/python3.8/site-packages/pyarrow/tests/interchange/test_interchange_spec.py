...
@pytest.mark.parametrize("uint, uint_bw", [(pa.uint8(), 8), (pa.uint16(), 16), (pa.uint32(), 32)])
@pytest.mark.parametrize("int, int_bw", [(pa.int8(), 8), (pa.int16(), 16), (pa.int32(), 32), (pa.int64(), 64)])
@pytest.mark.parametrize(
    "float, float_bw, np_float",
    [(pa.float16(), 16, np.float16), (pa.float32(), 32, np.float32), (pa.float64(), 64, np.float64)],
)
@pytest.mark.parametrize("unit", ["s", "ms", "us", "ns"])
@pytest.mark.parametrize("tz", ["", "America/New_York", "+07:30", "-04:30"])
@pytest.mark.parametrize("use_batch", [False, True])
def test_mixed_dtypes(uint, uint_bw, int, int_bw, float, float_bw, np_float, unit, tz, use_batch):
...
def test_dataframe(use_batch):
    ...
    a = pa.chunked_array([["Flamingo", "Parrot", "Cow"], ["Horse", "Brittle stars", "Centipede"]])
    ...
    assert list(df.select_columns((1,)).column_names()) == list(df.select_columns_by_name(("animals",)).column_names())
...
@pytest.mark.parametrize(
    ...
    [(pa.int8(), 8, "c"), (pa.int16(), 16, "s"), (pa.int32(), 32, "i"), (pa.int64(), 64, "l")],
)
