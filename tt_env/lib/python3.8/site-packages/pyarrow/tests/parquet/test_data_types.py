...
try:
    ...
    from pyarrow.tests.pandas_examples import dataframe_with_arrays, dataframe_with_lists
    ...
    for precision in range(1, 39):
        for scale in range(0, precision + 1):
            with util.random_seed(0):
                random_decimal_values = [util.randdecimal(precision, scale) for _ in range(num_values)]
...
def test_large_list_records():
    ...
    list_values = [
        list(map(int, np.random.randint(0, 100, size=x))) if i % 8 else None for i, x in enumerate(list_lengths)
    ]
...
def test_byte_array_exactly_2gb():
    ...
    cases = [[b"x" * 1023], [b"x" * 1024], [b"x" * 1025]]  # 2^31 - 1  # 2^31  # 2^31 + 1
    for case in cases:
