...
from pyarrow.fs import FileSelector, FileSystem, LocalFileSystem, PyFileSystem, SubTreeFileSystem, FSSpecHandler
...
def test_filters_cutoff_exclusive_datetime(tempdir):
    ...
    expected = pd.Categorical(
        np.array([datetime.date(2018, 4, 11)], dtype="datetime64"), categories=np.array(date_keys, dtype="datetime64")
    )
...
def test_filters_inclusive_datetime(tempdir):
    ...
    pd.DataFrame({"dates": pd.date_range("2020-01-01", periods=10, freq="D"), "id": range(10)}).to_parquet(
        path, use_deprecated_int96_timestamps=True
    )
...
def test_filters_inclusive_set(tempdir):
    ...
    dataset = pq.ParquetDataset(
        base_path,
        filesystem=local,
        filters=[("integer", "in", [1]), ("string", "in", ("a", "b")), ("boolean", "not in", {"False"})],
    )
    table = dataset.read()
    result_df = table.to_pandas().reset_index(drop=True)
...
def test_filters_invalid_pred_op(tempdir):
    ...
    dataset = pq.ParquetDataset(base_path, filesystem=local, filters=[("integers", "!=", {3})])
...
def test_read_multiple_files(tempdir):
    ...
    expected = pa.Table.from_arrays(
        [result.column(i) for i in to_read], names=col_names, metadata=result.schema.metadata
    )
    ...
    output_df = pd.DataFrame(
        {
            ...
            "num": list(range(10)),
            "nan": [np.nan] * 10,
            "date": np.arange("2017-01-01", "2017-01-11", dtype="datetime64[D]").astype("datetime64[ns]"),
        }
    )
