...
def test_coerce_timestamps(tempdir):
    ...
    arrays["datetime64"] = [
        np.array(["2007-07-13T01:23:34.123456789", None, "2010-08-13T05:46:57.437699912"], dtype="datetime64[ms]"),
        None,
        None,
        np.array(["2007-07-13T02", None, "2010-08-13T05:46:57.437699912"], dtype="datetime64[ms]"),
    ]
...
def test_date_time_types(tempdir):
    ...
    table = pa.Table.from_arrays(
        ...
        ["date32", "date64", "timestamp[us]", "time32[s]", "time64[us]", "time32_from64[s]", "timestamp[ns]"],
    )
...
def test_coerce_int96_timestamp_unit(unit):
    ...
    _check_roundtrip(
        table, expected, version="2.6", read_table_kwargs=read_table_kwargs, use_deprecated_int96_timestamps=True
    )
...
def test_noncoerced_nanoseconds_written_without_exception(tempdir):
    ...
    n = 9
    df = pd.DataFrame({"x": range(n)}, index=pd.date_range("2017-01-01", freq="ns", periods=n))
