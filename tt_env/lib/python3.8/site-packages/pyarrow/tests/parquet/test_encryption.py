...
try:
    ...
    from pyarrow.tests.parquet.encryption import InMemoryKmsClient, verify_file_encrypted
...
def data_table():
    data_table = pa.Table.from_pydict(
        {"a": pa.array([1, 2, 3]), "b": pa.array(["a", "b", "c"]), "c": pa.array(["x", "y", "z"])}
    )
    return data_table
...
def write_encrypted_file(path, data_table, footer_key_name, col_key_name, footer_key, col_key, encryption_config):
    """
    ...
    kms_connection_config, crypto_factory = setup_encryption_environment(custom_kms_conf)
...
def test_encrypted_parquet_write_read(tempdir, data_table):
    ...
    kms_connection_config, crypto_factory = write_encrypted_file(
        path, data_table, FOOTER_KEY_NAME, COL_KEY_NAME, FOOTER_KEY, COL_KEY, encryption_config
    )
...
def test_encrypted_parquet_write_read_wrong_key(tempdir, data_table):
    ...
    write_encrypted_file(path, data_table, FOOTER_KEY_NAME, COL_KEY_NAME, FOOTER_KEY, COL_KEY, encryption_config)
    ...
    with pytest.raises(ValueError, match=r"Incorrect master key used"):
        read_encrypted_parquet(path, decryption_config, wrong_kms_connection_config, wrong_crypto_factory)
...
def test_encrypted_parquet_write_no_col_key(tempdir, data_table):
    ...
    with pytest.raises(OSError, match="Either column_keys or uniform_encryption " "must be set"):
        ...
        write_encrypted_file(path, data_table, FOOTER_KEY_NAME, COL_KEY_NAME, FOOTER_KEY, b"", encryption_config)
...
def test_encrypted_parquet_write_kms_factory_type_error(tempdir, data_table, basic_encryption_config):
...
def test_encrypted_parquet_loop(tempdir, data_table, basic_encryption_config):
    ...
    kms_connection_config, crypto_factory = write_encrypted_file(
        path, data_table, FOOTER_KEY_NAME, COL_KEY_NAME, FOOTER_KEY, COL_KEY, basic_encryption_config
    )
