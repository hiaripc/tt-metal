...
@pytest.mark.parametrize(
    ("data", "type", "physical_type", "min_value", "max_value", "null_count", "num_values", "distinct_count"),
    [
        ...
        (["", "b", chr(1000), None, "aaa"], pa.binary(), "BYTE_ARRAY", b"", chr(1000).encode("utf-8"), 1, 4, None),
        ([True, False, False, True, True], pa.bool_(), "BOOLEAN", False, True, 0, 5, None),
        ([b"\x00", b"b", b"12", None, b"aaa"], pa.binary(), "BYTE_ARRAY", b"\x00", b"b", 1, 4, None),
    ],
)
...
def test_statistics_convert_logical_types(tempdir):
    ...
    cases = [
        ...
        (datetime.time(10, 30, 0, 1000), datetime.time(15, 30, 0, 1000), pa.time32("ms")),
        (datetime.time(10, 30, 0, 1000), datetime.time(15, 30, 0, 1000), pa.time64("us")),
        (
...
def test_parquet_sorting_column():
    ...
    assert sorting_col.to_dict() == {"column_index": 10, "descending": False, "nulls_first": False}

    sorting_col = pq.SortingColumn(0, descending=True, nulls_first=True)
    assert sorting_col.to_dict() == {"column_index": 0, "descending": True, "nulls_first": True}
...
def test_parquet_sorting_column_nested():
    ...
    sorting_columns = [pq.SortingColumn(0, descending=True), pq.SortingColumn(2, descending=False)]  # a.x  # b
...
def test_field_id_metadata():
    ...
    fields = [
        ...
        pa.field(
            "list", pa.list_(pa.field("list-inner", pa.int32(), metadata={field_id: b"10"})), metadata={field_id: b"11"}
        ),
...
def test_write_metadata(tempdir):
    ...
    with pytest.raises(RuntimeError, match=msg):
        pq.write_metadata(
            pa.schema([("a", "int32"), ("b", "null")]), path, metadata_collector=[parquet_meta, parquet_meta]
        )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"col1": range(10)}, {"col2": range(10)}, "The two columns with index 0 differ."),
        ({"col1": range(10), "col2": range(10)}, {"col3": range(10)}, "This schema has 2 columns, other has 1"),
    ),
)
...
def test_write_metadata_fs_file_combinations(tempdir, s3_example_s3fs):
    ...
    assert (
        meta1.read_bytes() == meta2.read_bytes() == meta3.read_bytes() == meta4.read_bytes() == s3_fs.open(meta5).read()
    )
