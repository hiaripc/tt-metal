...
try:
    ...
    from pyarrow.tests.parquet.common import _roundtrip_pandas_dataframe, alltypes_sample
except ImportError:
    pd = tm = None
...
carat        cut  color  clarity  depth  table  price     x     y     z
 ...
 0.23  Very Good      H      VS1   59.4   61.0    338  4.00  4.05  2.39"""
    expected = pd.read_csv(io.BytesIO(expected_string), sep=r"\s{2,}", index_col=None, header=0, engine="python")
...
carat        cut  color  clarity  depth  table  price     x     y     z
 ...
 0.23  Very Good      H      VS1   59.4   61.0    338  4.00  4.05  2.39"""
    expected = pd.read_csv(
        io.BytesIO(expected_string), sep=r"\s{2,}", index_col=["cut", "color", "clarity"], header=0, engine="python"
    ).sort_index()
...
def test_backwards_compatible_column_metadata_handling(datadir):
    ...
    expected = pd.DataFrame(
        {"a": [1, 2, 3], "b": [0.1, 0.2, 0.3], "c": pd.date_range("2017-01-01", periods=3, tz="Europe/Brussels")}
    )
    expected.index = pd.MultiIndex.from_arrays(
        [["a", "b", "c"], pd.date_range("2017-01-01", periods=3, tz="Europe/Brussels")], names=["index", None]
    )
...
def test_categories_with_string_pyarrow_dtype(tempdir):
    ...
    assert pa.array(df1["x"].cat.categories.values).to_pylist() == pa.array(df2["x"].cat.categories.values).to_pylist()
