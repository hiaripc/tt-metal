...
binary_like_types = st.one_of(binary_type, string_type, large_binary_type, large_string_type, fixed_size_binary_type)

signed_integer_types = st.sampled_from([pa.int8(), pa.int16(), pa.int32(), pa.int64()])
unsigned_integer_types = st.sampled_from([pa.uint8(), pa.uint16(), pa.uint32(), pa.uint64()])
...
decimal128_type = st.builds(
    pa.decimal128, precision=st.integers(min_value=1, max_value=38), scale=st.integers(min_value=1, max_value=38)
)
decimal256_type = st.builds(
    pa.decimal256, precision=st.integers(min_value=1, max_value=76), scale=st.integers(min_value=1, max_value=76)
)
numeric_types = st.one_of(integer_types, floating_types, decimal128_type, decimal256_type)

date_types = st.sampled_from([pa.date32(), pa.date64()])
time_types = st.sampled_from([pa.time32("s"), pa.time32("ms"), pa.time64("us"), pa.time64("ns")])
...
if tzst and zoneinfo:
    ...
    timezones = st.none()
timestamp_types = st.builds(pa.timestamp, unit=st.sampled_from(["s", "ms", "us", "ns"]), tz=timezones)
duration_types = st.builds(pa.duration, st.sampled_from(["s", "ms", "us", "ns"]))
interval_types = st.just(pa.month_day_nano_interval())
temporal_types = st.one_of(date_types, time_types, timestamp_types, duration_types, interval_types)

primitive_types = st.one_of(null_type, bool_type, numeric_types, temporal_types, binary_like_types)
...
def arrays(draw, type, size=None, nullable=True):
    ...
    if pa.types.is_null(ty):
        ...
        min_int64 = -(2**63)
        max_int64 = 2**63 - 1
        min_datetime = datetime.datetime.fromtimestamp(min_int64 // 10**9) + datetime.timedelta(hours=12)
        max_datetime = datetime.datetime.fromtimestamp(max_int64 // 10**9) - datetime.timedelta(hours=12)
        try:
            offset = ty.tz.split(":")
        ...
        value = st.datetimes(timezones=st.just(tz), min_value=min_datetime, max_value=max_datetime)
        ...
        value = _pymap(ty.key_type, ty.item_type, size=_default_array_sizes, nullable=nullable)
