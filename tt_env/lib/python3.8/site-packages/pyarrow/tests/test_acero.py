...
def test_declaration():
    ...
    decl = Declaration.from_sequence(
        [Declaration("table_source", options=table_opts), Declaration("filter", options=filter_opts)]
    )
...
def test_filter(table_source):
    ...
    decl = Declaration.from_sequence([table_source, Declaration("filter", options=FilterNodeOptions(field("c") > 1))])
...
def test_project(table_source):
    ...
    decl = Declaration.from_sequence(
        [table_source, Declaration("project", ProjectNodeOptions([pc.multiply(field("a"), 2)]))]
    )
    result = decl.to_table()
    ...
    # provide name
    decl = Declaration.from_sequence(
        [table_source, Declaration("project", ProjectNodeOptions([pc.multiply(field("a"), 2)], ["a2"]))]
    )
    result = decl.to_table()
    ...
    decl = Declaration.from_sequence([table_source, Declaration("project", ProjectNodeOptions([pc.sum(field("a"))]))])
...
def test_aggregate_scalar(table_source):
    decl = Declaration.from_sequence(
        [table_source, Declaration("aggregate", AggregateNodeOptions([("a", "sum", None, "a_sum")]))]
    )
    result = decl.to_table()
...
def test_hash_join():
    ...
    expected = pa.table([[2, 3], [5, 6], [2, 3], [4, 5]], names=["key", "a", "key", "b"])
    assert result.equals(expected)
    ...
    joined = Declaration("hashjoin", options=join_opts, inputs=[left_source, right_source])
    result = joined.to_table()
    expected = pa.table([[1, 2, 3], [4, 5, 6], [None, 2, 3], [None, 4, 5]], names=["key", "a", "key", "b"])
    ...
    joined = Declaration("hashjoin", options=join_opts, inputs=[left_source, right_source])
    result = joined.to_table()
    expected = pa.table([[1, 2, 3], [4, 5, 6], [None, 2, 3], [None, 4, 5]], names=["key_left", "a", "key_right", "b"])
    assert result.sort_by("a").equals(expected)

    # manually specifying output columns
    join_opts = HashJoinNodeOptions(
        "left outer", left_keys="key", right_keys="key", left_output=["key", "a"], right_output=[field("b")]
    )
    joined = Declaration("hashjoin", options=join_opts, inputs=[left_source, right_source])
    result = joined.to_table()
...
def test_asof_join():
    ...
    joined = Declaration("asofjoin", options=join_opts, inputs=[left_source, right_source])
    result = joined.to_table()
    expected = pa.table([[1, 2, 3], [1, 1, 1], [4, 5, 6], [None, 4, None]], names=["key", "ts", "a", "b"])
...
def test_scan(tempdir):
    ...
    result = decl.to_table()
    assert result.schema.names == ["a", "b", "__fragment_index", "__batch_index", "__last_in_fragment", "__filename"]
