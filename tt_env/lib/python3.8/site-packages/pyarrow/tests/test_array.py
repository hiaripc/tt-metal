...
def test_array_from_scalar():
    ...
    cases = [
        ...
        (now_with_tz, 2, pa.array([now_utc] * 2, type=pa.timestamp("us", tz=pytz.timezone("US/Eastern")))),
        (now.time(), 9, pa.array([now.time()] * 9)),
        (oneday, 4, pa.array([oneday] * 4)),
        (False, 9, pa.array([False] * 9)),
        ([1, 2], 2, pa.array([[1, 2], [1, 2]])),
        (pa.scalar([-1, 3], type=pa.large_list(pa.int8())), 5, pa.array([[-1, 3]] * 5, type=pa.large_list(pa.int8()))),
...
@pytest.mark.parametrize(
    ("list_array_type", "list_type_factory"), [(pa.ListArray, pa.list_), (pa.LargeListArray, pa.large_list)]
)
...
def test_map_from_arrays():
    ...
    result = pa.MapArray.from_arrays(
        [0, 2, 2, 6], keys, items, pa.map_(keys.type, items.type), mask=pa.array([False, True, False], type=pa.bool_())
    )
    assert result.null_count == 1
    assert result.equals(expected)

    # pass in null bitmap without the type
    result = pa.MapArray.from_arrays([0, 2, 2, 6], keys, items, mask=pa.array([False, True, False], type=pa.bool_()))
    assert result.equals(expected)
    ...
    result = pa.MapArray.from_arrays(
        [0, 2, 2, 6], keys, items, pa.map_(keys.type, items.type), mask=pa.array([True, True, False], type=pa.bool_())
    )
    expected = pa.array(pyentries, type=pa.map_(pa.binary(), pa.int32()))
    ...
    with pytest.raises(pa.ArrowInvalid, match=msg1):
        pa.MapArray.from_arrays(
            offsets, keys, items, pa.map_(keys.type, items.type), mask=pa.array([False, True, False], type=pa.bool_())
        )
...
def test_union_from_dense():
    ...
    # with type codes
    check_result(
        pa.UnionArray.from_dense(logical_types, value_offsets, [binary, int64], type_codes=[11, 13]),
        expected_field_names=["0", "1"],
    ...
    check_result(
        pa.UnionArray.from_dense(logical_types, value_offsets, [binary, int64], ["bin", "int"], [11, 13]),
        expected_field_names=["bin", "int"],
        expected_type_codes=[11, 13],
        expected_type_code_values=logical_types,
    )
    ...
    arr = pa.UnionArray.from_dense(types, value_offsets, [binary, int64], type_codes=[11, 13])
    with pytest.raises(pa.ArrowInvalid):
        arr.validate(full=True)
...
def test_union_from_sparse():
    ...
    # with field names and type codes
    check_result(
        pa.UnionArray.from_sparse(logical_types, [binary, int64], ["bin", "int"], [11, 13]),
        expected_field_names=["bin", "int"],
        expected_type_codes=[11, 13],
        expected_type_code_values=logical_types,
    )
...
def test_union_array_slice():
    # ARROW-2314
    arr = pa.UnionArray.from_sparse(
        pa.array([0, 0, 1, 1], type=pa.int8()), [pa.array(["a", "b", "c", "d"]), pa.array([1, 2, 3, 4])]
    )
...
def test_cast_integers_safe():
    safe_cases = [
        (np.array([0, 1, 2, 3], dtype="i1"), "int8", np.array([0, 1, 2, 3], dtype="i4"), pa.int32()),
        (np.array([0, 1, 2, 3], dtype="i1"), "int8", np.array([0, 1, 2, 3], dtype="u4"), pa.uint16()),
        (np.array([0, 1, 2, 3], dtype="i1"), "int8", np.array([0, 1, 2, 3], dtype="u1"), pa.uint8()),
        (np.array([0, 1, 2, 3], dtype="i1"), "int8", np.array([0, 1, 2, 3], dtype="f8"), pa.float64()),
    ]
...
def test_cast_integers_unsafe():
    ...
    unsafe_cases = [
        (np.array([50000], dtype="i4"), "int32", np.array([50000]).astype(dtype="i2"), pa.int16()),
        (np.array([70000], dtype="i4"), "int32", np.array([70000]).astype(dtype="u2"), pa.uint16()),
        (np.array([-1], dtype="i4"), "int32", np.array([-1]).astype(dtype="u2"), pa.uint16()),
        (np.array([50000], dtype="u2"), pa.uint16(), np.array([50000]).astype(dtype="i2"), pa.int16()),
    ]
...
def test_floating_point_truncate_safe():
    safe_cases = [
        (np.array([1.0, 2.0, 3.0], dtype="float32"), "float32", np.array([1, 2, 3], dtype="i4"), pa.int32()),
        (np.array([1.0, 2.0, 3.0], dtype="float64"), "float64", np.array([1, 2, 3], dtype="i4"), pa.int32()),
        (np.array([-10.0, 20.0, -30.0], dtype="float64"), "float64", np.array([-10, 20, -30], dtype="i4"), pa.int32()),
    ]
    for case in safe_cases:
...
def test_floating_point_truncate_unsafe():
    unsafe_cases = [
        (np.array([1.1, 2.2, 3.3], dtype="float32"), "float32", np.array([1, 2, 3], dtype="i4"), pa.int32()),
        (np.array([1.1, 2.2, 3.3], dtype="float64"), "float64", np.array([1, 2, 3], dtype="i4"), pa.int32()),
        (np.array([-10.1, 20.2, -30.3], dtype="float64"), "float64", np.array([-10, 20, -30], dtype="i4"), pa.int32()),
    ]
    for case in unsafe_cases:
...
def test_decimal_to_int_safe():
    safe_cases = [
        ...
        ([decimal.Decimal("123"), None, decimal.Decimal("-91")], pa.decimal128(19, 10), [123, None, -91], pa.int8()),
    ]
    for case in safe_cases:
        _check_cast_case(case)
        _check_cast_case(case, safe=True)
...
def test_decimal_to_int_value_out_of_bounds():
    out_of_bounds_cases = [
        (
            np.array([decimal.Decimal("1234567890123"), None, decimal.Decimal("-912345678901234")]),
            pa.decimal128(32, 5),
        ...
        ([decimal.Decimal("1234"), None, decimal.Decimal("-9123")], pa.decimal128(32, 5), [-46, None, 93], pa.int8()),
    ]
...
def test_cast_signed_to_unsigned():
    safe_cases = [
        (np.array([0, 1, 2, 3], dtype="i1"), pa.uint8(), np.array([0, 1, 2, 3], dtype="u1"), pa.uint8()),
        (np.array([0, 1, 2, 3], dtype="i2"), pa.uint16(), np.array([0, 1, 2, 3], dtype="u2"), pa.uint16()),
    ]
...
def test_cast_from_null():
    ...
    out_types = [
        ...
        pa.struct([pa.field("a", pa.int32()), pa.field("b", pa.list_(pa.int8())), pa.field("c", pa.string())]),
...
def test_cast_string_to_number_roundtrip():
    cases = [
        ...
        (pa.array([None, "18446744073709551615"]), pa.array([None, 18446744073709551615], type=pa.uint64())),
    ]
...
def test_unique_simple():
    cases = [
        ...
        (pa.array(["foo", None, "bar", "foo"], pa.large_binary()), pa.array(["foo", None, "bar"], pa.large_binary())),
    ]
...
def test_value_counts_simple():
    cases = [
        (pa.array([1, 2, 3, 1, 2, 3]), pa.array([1, 2, 3]), pa.array([2, 2, 2], type=pa.int64())),
        (pa.array(["foo", None, "bar", "foo"]), pa.array(["foo", None, "bar"]), pa.array([2, 1, 1], type=pa.int64())),
        (
...
def test_dictionary_encode_simple():
    cases = [
        (
            ...
            pa.DictionaryArray.from_arrays(pa.array([0, 1, 2, None, 0, 1, 2], type="int32"), [1, 2, 3]),
        ),
        (
            pa.array(["foo", None, "bar", "foo"]),
            pa.DictionaryArray.from_arrays(pa.array([0, None, 1, 0], type="int32"), ["foo", "bar"]),
        ),
        (
            pa.array(["foo", None, "bar", "foo"], type=pa.large_binary()),
            pa.DictionaryArray.from_arrays(
                pa.array([0, None, 1, 0], type="int32"), pa.array(["foo", "bar"], type=pa.large_binary())
            ),
        ),
    ]
    for arr, expected in cases:
...
def test_dictionary_encode_sliced():
    cases = [
        (
            ...
            pa.DictionaryArray.from_arrays(pa.array([0, 1, None, 2, 0], type="int32"), [2, 3, 1]),
        ),
        (
            pa.array([None, "foo", "bar", "foo", "xyzzy"])[1:-1],
            pa.DictionaryArray.from_arrays(pa.array([0, 1, 0], type="int32"), ["foo", "bar"]),
        ),
        (
            pa.array([None, "foo", "bar", "foo", "xyzzy"], type=pa.large_string())[1:-1],
            pa.DictionaryArray.from_arrays(
                pa.array([0, 1, 0], type="int32"), pa.array(["foo", "bar"], type=pa.large_string())
            ),
        ),
    ]
    for arr, expected in cases:
...
def test_cast_date32_to_int():
    ...
    expected1 = pa.array([datetime.date(1970, 1, 1), datetime.date(1970, 1, 2), datetime.date(1970, 1, 3)]).cast(
        "date32"
    )
...
pickle_test_parametrize = pytest.mark.parametrize(
    ...
    [
        ...
        ([(1, "a"), (2, "c"), None], pa.struct([pa.field("a", pa.int64()), pa.field("b", pa.string())])),
    ],
)
...
def test_interval_array_from_relativedelta():
    ...
    data = [
        None,
        relativedelta(years=1, months=1, days=1, seconds=1, microseconds=1, minutes=1, hours=1, weeks=1, leapdays=1),
    ]
    ...
    assert arr.to_pandas().tolist() == [
        ...
        DateOffset(
            ...
            days=8,
            microseconds=(
                datetime.timedelta(seconds=1, microseconds=1, minutes=1, hours=1) // datetime.timedelta(microseconds=1)
            ),
            nanoseconds=0,
        ),
    ]
...
def test_interval_array_from_dateoffset():
    ...
    data = [
        None,
        DateOffset(years=1, months=1, days=1, seconds=1, microseconds=1, minutes=1, hours=1, weeks=1, nanoseconds=1),
        DateOffset(),
    ]
...
@pytest.mark.parametrize(
    "list_type", [pa.list_(pa.int32()), pa.list_(pa.int32(), list_size=2), pa.large_list(pa.int32())]
)
...
def test_array_supported_masks():
    ...
    with pytest.raises(pa.ArrowTypeError):
        arr = pa.array(
            [4, None, 4, 3], mask=pa.array([False, True, False, True], mask=pa.array([True, True, True, True]))
        )
...
def test_array_invalid_mask_raises():
    # ARROW-10742
    cases = [
        ([1, 2], np.array([False, False], dtype="O"), TypeError, "must be boolean dtype"),
        ([1, 2], np.array([[False], [False]]), pa.ArrowInvalid, "must be 1D array"),
        ([1, 2, 3], np.array([False, False]), pa.ArrowInvalid, "different length"),
        (np.array([1, 2]), np.array([False, False], dtype="O"), TypeError, "must be boolean dtype"),
        (np.array([1, 2]), np.array([[False], [False]]), ValueError, "must be 1D array"),
...
def test_c_array_protocol_device_unsupported_keyword():
    ...
    with pytest.raises(NotImplementedError, match=r"Received unsupported keyword argument\(s\): \['other'\]"):
...
def test_struct_array_sort():
    ...
    arr_with_nulls = pa.StructArray.from_arrays(
        [pa.array([5, 7, 7, 35], type=pa.int64()), pa.array(["foo", "car", "bar", "foobar"])],
        names=["a", "b"],
...
def test_list_view_from_arrays_fails(list_array_type, list_type_factory):
    ...
    with pytest.raises(pa.lib.ArrowInvalid):
        list_array_type.from_arrays(
            array_slice.offsets, array_slice.sizes, array_slice.values, mask=array_slice.is_null()
        )


@pytest.mark.parametrize(
    ("list_array_type", "list_type_factory", "offset_type"),
    [(pa.ListViewArray, pa.list_view, pa.int32()), (pa.LargeListViewArray, pa.large_list_view, pa.int64())],
)
...
def test_list_view_slice(list_view_type):
    ...
    array = list_view_type.from_arrays(offsets=[0, 3, 4], sizes=[3, 1, 2], values=[1, 2, 3, 4, 5, 6])
