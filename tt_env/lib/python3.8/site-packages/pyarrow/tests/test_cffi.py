...
assert_schema_released = pytest.raises(ValueError, match="Cannot import released ArrowSchema")

assert_array_released = pytest.raises(ValueError, match="Cannot import released ArrowArray")

assert_stream_released = pytest.raises(ValueError, match="Cannot import released Arrow Stream")
...
@pytest.mark.parametrize(
    "obj", [pa.int32(), pa.field("foo", pa.int32()), pa.schema({"foo": pa.int32()})], ids=["type", "field", "schema"]
)
...
def test_roundtrip_array_capsule(arr, schema_accessor, bad_type, good_type):
    ...
    with pytest.raises(ValueError, match=r"Could not cast.* string to requested .* int32"):
...
def test_roundtrip_device_array_capsule(arr, schema_accessor, bad_type, good_type):
    ...
    schema_capsule, array_capsule = arr.__arrow_c_device_array__(good_type.__arrow_c_schema__())
    arr_out = import_array(schema_capsule, array_capsule)
    assert schema_accessor(arr_out) == good_type
...
def test_roundtrip_chunked_array_capsule_requested_schema():
    ...
    requested_capsule = requested_type.__arrow_c_schema__()
    with pytest.raises(ValueError, match="Could not cast string to requested type int64"):
