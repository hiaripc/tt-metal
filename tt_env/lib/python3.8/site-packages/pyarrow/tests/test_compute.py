...
exported_functions = [
    func for (name, func) in sorted(pc.__dict__.items()) if hasattr(func, "__arrow_compute_function__")
]

exported_option_classes = [
    cls
    for (name, cls) in sorted(pc.__dict__.items())
    if (isinstance(cls, type) and cls is not pc.FunctionOptions and issubclass(cls, pc.FunctionOptions))
]
...
def test_option_class_equality(request):
    options = [
        ...
        pc.MakeStructOptions(
            ...
            field_metadata=[pa.KeyValueMetadata({"a": "1"}), pa.KeyValueMetadata({"b": "2"})],
        ),
        ...
        pc.QuantileOptions(),
        pc.RandomOptions(),
        pc.RankOptions(sort_keys="ascending", null_placement="at_start", tiebreaker="max"),
        ...
        pc.VarianceOptions(),
        pc.WeekOptions(week_starts_monday=True, count_from_zero=False, first_week_is_fully_in_year=False),
    ]
...
def test_sum_chunked_array(arrow_type):
    ...
    arr = pa.chunked_array(
        [pa.array([1, 2], type=arrow_type), pa.array([], type=arrow_type), pa.array([3, 4], type=arrow_type)]
    )
    assert pc.sum(arr).as_py() == 10
...
def test_generated_signatures():
    ...
    assert str(sig) == (
        "(*strings, null_handling='emit_null', " "null_replacement='', options=None, " "memory_pool=None)"
    )
...
def test_drop_null_record_batch():
    ...
    batch = pa.record_batch(
        [pa.array(["a", None, "c", "d", None]), pa.array([None, None, "c", None, "e"])], names=["a'", "b'"]
    )
...
def test_fill_null():
    ...
    with pytest.raises(pa.ArrowInvalid, match="Array arguments must all be the same length"):
...
def test_fill_null_chunked_array(arrow_type):
    ...
    arr = pa.chunked_array(
        [pa.array([1, 2], type=arrow_type), pa.array([], type=arrow_type), pa.array([None, 4], type=arrow_type)]
    )
    expected = pa.chunked_array(
        [pa.array([1, 2], type=arrow_type), pa.array([], type=arrow_type), pa.array([5, 4], type=arrow_type)]
    )
    result = arr.fill_null(fill_value)
    assert result.equals(expected)
...
DecimalTypeTraits = namedtuple("DecimalTypeTraits", ("name", "factory", "max_precision"))

FloatToDecimalCase = namedtuple("FloatToDecimalCase", ("precision", "scale", "float_val"))
...
@pytest.mark.parametrize(
    "case_generator",
    [integral_float_to_decimal_cast_cases, real_float_to_decimal_cast_cases, random_float_to_decimal_cast_cases],
    ...
    with decimal.localcontext() as ctx:
        for case in case_generator(float_ty, decimal_ty.max_precision):
            check_cast_float_to_decimal(
                float_ty, case.float_val, decimal_ty.factory(case.precision, case.scale), ctx, decimal_ty.max_precision
            )
...
def test_strptime():
    ...
    expected = pa.array([datetime.datetime(2020, 5, 1), None, datetime.datetime(1900, 12, 13)], type=pa.timestamp("s"))
    assert got == expected
    # Positional format
    assert pc.strptime(arr, "%m/%d/%Y", unit="s") == got

    expected = pa.array([datetime.datetime(2020, 1, 5), None, None], type=pa.timestamp("s"))
...
def test_strftime():
    ...
    assert result.equals(expected)
    with pytest.raises(pa.ArrowInvalid, match="Timezone not present, cannot convert to string"):
...
def test_iso_calendar_longer_array(unit):
    ...
    expected = pa.StructArray.from_arrays(
        [[2021] * 50, [52] * 50, [7] * 50], names=["iso_year", "iso_week", "iso_day_of_week"]
    )
    assert result.equals(expected)
...
def test_assume_timezone():
    ...
    with pytest.raises(ValueError, match="Timestamp doesn't exist in " f"timezone '{timezone}'"):
    ...
    with pytest.raises(ValueError, match="Timestamp is ambiguous in " f"timezone '{timezone}'"):
...
@pytest.mark.timezone_data
@pytest.mark.parametrize("unit", ("nanosecond", "microsecond", "millisecond", "second", "minute", "hour", "day"))
@pytest.mark.pandas
...
def test_cumulative_sum(start, skip_nulls):
    ...
    for strt in starts:
        arrays = [pa.array([1, 2, 3]), pa.array([0, None, 20, 30]), pa.chunked_array([[0, None], [20, 30]])]
        expected_arrays = [
            pa.array([1, 3, 6]),
            pa.array([0, None, 20, 50]) if skip_nulls else pa.array([0, None, None, None]),
            pa.chunked_array([[0, None, 20, 50]]) if skip_nulls else pa.chunked_array([[0, None, None, None]]),
        ]
    ...
    for strt in starts:
        ...
        for i, arr in enumerate(arrays):
            ...
            np.testing.assert_array_almost_equal(
                result.to_numpy(zero_copy_only=False), expected.to_numpy(zero_copy_only=False)
            )
...
def test_cumulative_prod(start, skip_nulls):
    ...
    for strt in starts:
        ...
        expected_arrays = [
            pa.array([1, 2, 6]),
            pa.array([1, None, 20, 100]) if skip_nulls else pa.array([1, None, None, None]),
            pa.chunked_array([[1, None, 20, 100]]) if skip_nulls else pa.chunked_array([[1, None, None, None]]),
        ]
        for i, arr in enumerate(arrays):
...
def test_cumulative_max(start, skip_nulls):
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            pa.array([2, 2, None, 5, 5, None]) if skip_nulls else pa.array([2, 2, None, None, None, None]),
        ...
        for i, arr in enumerate(arrays):
            ...
            expected = pc.max_element_wise(
                expected_arrays[i], strt if strt is not None else int(-1e9), skip_nulls=False
            )
            assert result.equals(expected)
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            np.array([2.5, 2.5, None, 2.5, 4.9, None]) if skip_nulls else np.array([2.5, 2.5, None, None, None, None]),
        ]
        for i, arr in enumerate(arrays):
            result = pc.cumulative_max(arr, start=strt, skip_nulls=skip_nulls)
            # Max `start` offset with expected array before comparing
            expected = pc.max_element_wise(expected_arrays[i], strt if strt is not None else -1e9, skip_nulls=False)
            np.testing.assert_array_almost_equal(
                result.to_numpy(zero_copy_only=False), expected.to_numpy(zero_copy_only=False)
            )
...
def test_cumulative_min(start, skip_nulls):
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            pa.array([5, 5, None, 2, 2, None]) if skip_nulls else pa.array([5, 5, None, None, None, None]),
        ...
        for i, arr in enumerate(arrays):
            ...
            expected = pc.min_element_wise(expected_arrays[i], strt if strt is not None else int(1e9), skip_nulls=False)
            assert result.equals(expected)
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            np.array([5.5, 5.5, None, 5.5, 3.9, None]) if skip_nulls else np.array([5.5, 5.5, None, None, None, None]),
        ]
        for i, arr in enumerate(arrays):
            result = pc.cumulative_min(arr, start=strt, skip_nulls=skip_nulls)
            # Min `start` offset with expected array before comparing
            expected = pc.min_element_wise(expected_arrays[i], strt if strt is not None else 1e9, skip_nulls=False)
            np.testing.assert_array_almost_equal(
                result.to_numpy(zero_copy_only=False), expected.to_numpy(zero_copy_only=False)
            )
...
def test_case_when():
    assert pc.case_when(pc.make_struct([True, False, None], [False, True, None]), [1, 2, 3], [11, 12, 13]) == pa.array(
        [1, 12, None]
    )
...
def test_random():
    ...
    arrays = [tuple(pc.random(100, initializer=i % 7).to_pylist()) for i in range(0, 100)]
    ...
    with pytest.raises(
        TypeError, match=r"initializer should be 'system', an integer, " r"or a hashable object; got \[\]"
    ):
...
def test_list_slice_output_fixed(start, stop, step, expected, value_type, list_type):
    if list_type == "fixed":
        arr = pa.array([[1, 2, 3], [4, 5, None], [6, None, None], None], pa.list_(pa.int8(), 3)).cast(
            pa.list_(value_type(), 3)
        )
    else:
