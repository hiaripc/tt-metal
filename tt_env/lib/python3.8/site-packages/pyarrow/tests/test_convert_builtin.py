...
def test_nested_arrays(seq, factory):
    arr = pa.array(
        seq([np.array([], dtype=np.int64), np.array([1, 2], dtype=np.int64), None]), type=factory(pa.int64())
    )
    assert len(arr) == 3
    assert arr.null_count == 1
...
@pytest.mark.parametrize(
    "np_scalar,pa_type", [(np.float16, pa.float16()), (np.float32, pa.float32()), (np.float64, pa.float64())]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"a": 1, "b": 2}, None, {"a": 5, "b": None}], pa.struct([("a", pa.int8()), ("b", pa.int16())])),
    ],
)
...
def test_sequence_timestamp_with_timezone(timezone, unit):
    ...
    data = [
        ...
        None,
        pytz.timezone("US/Eastern").localize(datetime.datetime(2006, 1, 13, 12, 34, 56, 432539)),
        pytz.timezone("Europe/Moscow").localize(datetime.datetime(2010, 8, 13, 5, 0, 0, 437699)),
    ]
    utcdata = [
...
def test_struct_from_list_of_pairs_errors():
    ...
    # type inference
    template = r"Could not convert {} with type {}: was expecting tuple of " r"(key, value) pair"
    cases = [
...
def test_struct_from_dicts_inference():
    ...
    expected_type = pa.struct(
        [
            pa.field("a", pa.struct([pa.field("aa", pa.list_(pa.int64())), pa.field("ab", pa.bool_())])),
...
def test_map_from_dicts():
    data = [
        ...
        [{"key": b"c", "value": 3}],
        [{"key": b"d", "value": 4}, {"key": b"e", "value": 5}, {"key": b"f", "value": None}],
        [{"key": b"g", "value": 7}],
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([None, datetime.date.today()], [None, pa.scalar(datetime.date.today())], pa.date32()),
        ([None, datetime.date.today()], [None, pa.scalar(datetime.date.today(), pa.date64())], pa.date64()),
        ([datetime.time(1, 1, 1), None], [pa.scalar(datetime.time(1, 1, 1)), None], pa.time64("us")),
        ([datetime.timedelta(seconds=10)], [pa.scalar(datetime.timedelta(seconds=10))], pa.duration("us")),
        ([None, datetime.datetime(2014, 1, 1)], [None, pa.scalar(datetime.datetime(2014, 1, 1))], pa.timestamp("us")),
        (
        ...
        ([b"a", b"b"], [pa.scalar(b"a", pa.binary(1)), pa.scalar(b"b", pa.binary(1))], pa.binary(1)),
        ([[1, 2, 3]], [pa.scalar([1, 2, 3])], pa.list_(pa.int64())),
        ([["a", "b"]], [pa.scalar(["a", "b"])], pa.list_(pa.string())),
        ([[1, 2, 3]], [pa.scalar([1, 2, 3], type=pa.list_view(pa.int64()))], pa.list_view(pa.int64())),
        ([["a", "b"]], [pa.scalar(["a", "b"], type=pa.list_view(pa.string()))], pa.list_view(pa.string())),
        ...
        (["a"], [pa.scalar("a", type=pa.dictionary(pa.int64(), pa.string()))], pa.dictionary(pa.int64(), pa.string())),
        (
            ["a", "b"],
            [
        ...
        ([1], [pa.scalar(1, type=pa.dictionary(pa.int64(), pa.int32()))], pa.dictionary(pa.int64(), pa.int32())),
        (
            [(1, 2)],
...
def test_array_accepts_pyarrow_scalar_errors(seq):
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="pyarrow scalars cannot be mixed with other " "Python scalar values currently"
    ):
        pa.array(sequence)
