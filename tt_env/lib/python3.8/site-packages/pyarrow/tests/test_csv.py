...
def test_write_options():
    ...
    check_options_class(
        cls, include_header=[True, False], delimiter=[",", "\t", "|"], quoting_style=["needed", "none", "all_valid"]
    )
...
class BaseCSVTableRead(BaseTestCSV):
    ...
    def test_simple_timestamps(self):
        ...
        assert table.to_pydict() == {
            ...
            "c": [datetime(1970, 1, 1, 0, 0, 0, 123000), datetime(1989, 7, 14, 1, 0, 0, 123456)],
        }
    ...
    def test_column_types(self):
        ...
        opts = ConvertOptions(
            column_types={"b": "float32", "c": "string", "d": "boolean", "e": pa.decimal128(11, 2), "zz": "null"}
        )
        rows = b"a,b,c,d,e\n1,2,3,true,1.0\n4,-5,6,false,0\n"
        table = self.read_bytes(rows, convert_options=opts)
        schema = pa.schema(
            [("a", pa.int64()), ("b", pa.float32()), ("c", pa.string()), ("d", pa.bool_()), ("e", pa.decimal128(11, 2))]
        )
        expected = {
            "a": [1, 4],
            "b": [2.0, -5.0],
...
class BaseStreamingCSVRead(BaseTestCSV):
    ...
    def test_inference(self):
        ...
        reader = self.open_bytes(rows, read_options=read_options)
        self.check_reader(
            reader, expected_schema, [{"a": ["123", "abc"], "b": [b"456", b"de\xff"]}, {"a": ["gh"], "b": [b"ij"]}]
        )
...
def test_write_read_round_trip():
    ...
    for read_options, parse_options, write_options in [
        ...
        (ReadOptions(column_names=t.column_names), None, WriteOptions(include_header=False)),
        (None, ParseOptions(delimiter="|"), WriteOptions(include_header=True, delimiter="|")),
        (
