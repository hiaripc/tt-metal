...
cuda_ipc = pytest.mark.skipif(not has_ipc_support, reason="CUDA IPC not supported in platform `%s`" % (platform))
...
def make_table():
    ...
    a2 = pa.array(
        [("ab", True), ("cde", False), (None, None), None], type=pa.struct([("strs", pa.utf8()), ("bools", pa.bool_())])
    )
    ...
    a3 = pa.DictionaryArray.from_arrays(indices=[0, 1, 1, None], dictionary=pa.array(["foo", "bar"]))
...
def _arr_copy_to_host(carr):
    ...
    for cbuf in carr.buffers():
        if cbuf is None:
            ...
            buf = global_context.foreign_buffer(cbuf.address, cbuf.size, cbuf).copy_to_host()
            buffers.append(buf)
...
def test_device_interface_array():
    ...
    carr = pa.Array.from_buffers(
        typ, 2, cbuffers[:2], children=[pa.Array.from_buffers(typ.value_type, 3, cbuffers[2:])]
    )
...
def _batch_copy_to_host(cbatch):
    ...
    for col in cbatch.columns:
        buffers = [
            global_context.foreign_buffer(buf.address, buf.size, buf).copy_to_host() if buf is not None else None
