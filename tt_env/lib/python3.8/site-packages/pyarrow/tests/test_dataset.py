...
from pyarrow.tests.util import FSProtocolClass, ProxyHandler, _configure_s3_limited_user, _filesystem_uri, change_cwd
...
def test_dataset(dataset, dataset_reader):
    ...
    assert result["new"] == [False, False, True, True, False, False, False, False, True, True]
...
def test_scanner(dataset, dataset_reader):
    ...
    scanner = dataset_reader.scanner(dataset, columns=["i64"], memory_pool=pa.default_memory_pool())
    ...
    scanner = dataset_reader.scanner(
        dataset,
        columns=["__filename", "__fragment_index", "__batch_index", "__last_in_fragment"],
        memory_pool=pa.default_memory_pool(),
    )
    table = scanner.to_table()
    expected_names = ["__filename", "__fragment_index", "__batch_index", "__last_in_fragment"]
...
def test_head(dataset, dataset_reader):
    ...
    result = dataset_reader.head(dataset, 2, columns=["i64"], filter=ds.field("i64") > 1).to_pydict()
...
def test_take(dataset, dataset_reader):
    ...
    for indices in [[1, 7], pa.array([1, 7])]:
        assert dataset_reader.take(dataset, indices) == dataset_reader.to_table(dataset).take(indices)
    with pytest.raises(IndexError):
...
def test_partitioning():
    ...
    partitioning = ds.FilenamePartitioning(
        pa.schema([pa.field("group", pa.int64()), pa.field("key", pa.dictionary(pa.int8(), pa.string()))]),
        dictionaries={
    ...
    table = pa.table(
        [pa.array(range(20)), pa.array(np.random.randn(20)), pa.array(np.repeat(["a", "b"], 10))],
        names=["f1", "f2", "part"],
    )
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        ds.write_dataset(table, tempdir, format="ipc", partitioning=partitioning)
        load_back = None
        with pytest.raises(ValueError, match="Expected Partitioning or PartitioningFactory"):
...
def test_file_format_pickling(pickle_module):
    formats = [
        ...
        ds.CsvFileFormat(read_options=pa.csv.ReadOptions(skip_rows=3, column_names=["foo"])),
        ds.CsvFileFormat(read_options=pa.csv.ReadOptions(skip_rows=3, block_size=2**20)),
        ds.JsonFileFormat(),
        ds.JsonFileFormat(
            parse_options=pa.json.ParseOptions(newlines_in_values=True, unexpected_field_behavior="ignore")
        ),
        ds.JsonFileFormat(read_options=pa.json.ReadOptions(use_threads=False, block_size=14)),
    ]
    try:
...
def test_fragment_scan_options_pickling(pickle_module):
    options = [
        ...
        ds.CsvFragmentScanOptions(convert_options=pa.csv.ConvertOptions(strings_can_be_null=True)),
        ds.CsvFragmentScanOptions(read_options=pa.csv.ReadOptions(block_size=2**16)),
        ds.JsonFragmentScanOptions(),
        ds.JsonFragmentScanOptions(pa.json.ParseOptions(newlines_in_values=False, unexpected_field_behavior="error")),
        ds.JsonFragmentScanOptions(read_options=pa.json.ReadOptions(use_threads=True, block_size=512)),
    ]
...
def test_filesystem_factory(mockfs, paths_or_selector, pre_buffer):
    ...
    expected_str = pa.DictionaryArray.from_arrays(
        pa.array([0, 1, 2, 3, 4], type=pa.int32()), pa.array("0 1 2 3 4".split(), type=pa.string())
    )
...
def test_make_fragment_with_size(s3_example_simple):
    ...
    fragments_with_size = [
        file_format.make_fragment(path, fs, file_size=size) for path, size in zip(paths, sizes_toosmall)
    ]
    ...
    fragments_with_size = [
        file_format.make_fragment(path, fs, file_size=size) for path, size in zip(paths, sizes_toolarge)
    ]
...
def test_fragments_reconstruct(tempdir, dataset_reader, pickle_module):
    ...
    new_fragment = parquet_format.make_fragment(
        fragment.path, fragment.filesystem, partition_expression=fragment.partition_expression
    )
    ...
    pattern = r"No match for FieldRef.Name\(part\) in " + fragment.physical_schema.to_string(False, False, False)
...
def test_fragments_parquet_row_groups_reconstruct(tempdir, dataset_reader, pickle_module):
    ...
    new_fragment = parquet_format.make_fragment(
        fragment.path, fragment.filesystem, partition_expression=fragment.partition_expression, row_groups=[0]
    )
    ...
    new_fragment = parquet_format.make_fragment(
        fragment.path, fragment.filesystem, partition_expression=fragment.partition_expression, row_groups={1}
    )
    ...
    new_fragment = parquet_format.make_fragment(
        fragment.path, fragment.filesystem, partition_expression=fragment.partition_expression, row_groups={2}
    )
...
def test_fragments_repr(tempdir, dataset):
    ...
    assert (
        ...
        repr(fragment) == "<pyarrow.dataset.ParquetFileFragment path=subdir/1/xxx/file0.parquet "
        "partition=[key=xxx, group=1]>"
        or repr(fragment) == "<pyarrow.dataset.ParquetFileFragment path=subdir/1/xxx/file0.parquet "
...
def test_partitioning_factory_hive_segment_encoding_key_encoded(pickled, pickle_module):
    ...
    partition_schema_en = pa.schema(
        [("test%27%3B%20date", pa.timestamp("s")), ("test%27%3B%5B%20string%27", pa.string())]
    )
    ...
    fragments = list(factory.finish().get_fragments())
    assert fragments[0].partition_expression.equals(
        (ds.field("test'; date") == "2021-05-04 00:00:00") & (ds.field("test';[ string'") == "$")
    )
    ...
    fragments = list(factory.finish().get_fragments())
    assert fragments[0].partition_expression.equals(
        (ds.field("test%27%3B%20date") == "2021-05-04 00%3A00%3A00") & (ds.field("test%27%3B%5B%20string%27") == "%24")
    )
...
def test_hive_partitioning_dictionary_key(multisourcefs):
    ...
    dataset = ds.dataset("hive", format="parquet", filesystem=multisourcefs, partitioning=part)
    assert dataset.partitioning.schema == schema
    table = dataset.to_table()
...
def test_scan_iterator(use_threads):
    ...
    for factory, schema in (
        ...
        scanner = ds.Scanner.from_batches(factory(), schema=schema, use_threads=use_threads)
...
def test_partition_discovery(tempdir, partitioning, null_fallback, infer_dictionary, partition_keys):
    ...
    expected_schema = table.schema.append(pa.field("part1", expected_type(part_keys1[0]))).append(
...
def s3_example_simple(s3_server):
    ...
    uri = "s3://{}:{}@mybucket/data.parquet?scheme=http&endpoint_override={}:{}" "&allow_bucket_creation=True".format(
...
def test_open_dataset_from_uri_s3_fsspec(s3_example_simple):
    ...
    fs = s3fs.S3FileSystem(
        key=access_key, secret=secret_key, client_kwargs={"endpoint_url": "http://{}:{}".format(host, port)}
    )
...
def test_open_dataset_from_s3_with_filesystem_uri(s3_server):
    ...
    uri = "s3://{}:{}@{}/{}?scheme=http&endpoint_override={}:{}" "&allow_bucket_creation=true".format(
...
def test_union_dataset_from_other_datasets(tempdir, multisourcefs):
    ...
    expected_schema = pa.schema(
        [("month", pa.int32()), ("color", pa.string()), ("unknown", pa.string())]  # fill with nulls
    )
    assembled = ds.dataset([child1, child3], schema=expected_schema)
    assert assembled.to_table().schema.equals(expected_schema)
...
def test_orc_writer_not_implemented_for_dataset():
    with pytest.raises(NotImplementedError, match="Writing datasets not yet implemented for this file format"):
...
def test_csv_format_options_generate_columns(tempdir, dataset_reader):
    ...
    assert result.equals(
        pa.table({"f0": pa.array([1]), "f1": pa.array(["a"]), "f2": pa.array([True]), "f3": pa.array([1])})
    )
...
def test_csv_fragment_options(tempdir, dataset_reader):
    ...
    options = ds.CsvFragmentScanOptions(
        convert_options=convert_options, read_options=pa.csv.ReadOptions(block_size=2**16)
    )
...
def test_json_fragment_options(tempdir, dataset_reader):
    ...
    with pytest.raises(ValueError, match="try to increase block size"):
        options = ds.JsonFragmentScanOptions(read_options=pa.json.ReadOptions(block_size=4))
...
def test_encoding(tempdir, dataset_reader):
    ...
    for encoding, input_rows in [
        ("latin-1", b"a,b\nun,\xe9l\xe9phant"),
        ("utf16", b"\xff\xfea\x00,\x00b\x00\n\x00u\x00n\x00," b"\x00\xe9\x00l\x00\xe9\x00p\x00h\x00a\x00n\x00t\x00"),
    ]:
        ...
        expected_table = pa.table({"a": ["un"], "b": ["éléphant"]}, schema=expected_schema)
...
def test_dataset_preserved_partitioning(tempdir):
    ...
    dataset2 = ds.FileSystemDataset(
        list(dataset.get_fragments()), schema=dataset.schema, format=dataset.format, filesystem=dataset.filesystem
    )
...
def test_write_dataset_with_field_names_hive(tempdir):
    ...
    ds.write_dataset(table, tempdir, format="ipc", partitioning=["b"], partitioning_flavor="hive")
...
def test_write_dataset_with_scanner(tempdir):
    ...
    with tempfile.TemporaryDirectory() as tempdir2:
        ds.write_dataset(dataset.scanner(columns=["b", "c"]), tempdir2, format="ipc", partitioning=["b"])
...
def test_write_dataset_with_backpressure(tempdir):
    ...
    write_thread = threading.Thread(
        target=lambda: ds.write_dataset(scanner, str(tempdir), format="parquet", filesystem=gating_fs)
    )
    write_thread.start()
...
def test_write_dataset_existing_data(tempdir):
    ...
    ds.write_dataset(
        table, directory, partitioning=partitioning, format="ipc", existing_data_behavior="overwrite_or_ignore"
    )
    ...
    ds.write_dataset(
        table, directory, partitioning=partitioning, format="ipc", existing_data_behavior="delete_matching"
    )
...
def test_write_dataset_partitioned_dict(tempdir):
    ...
    partitioning = ds.partitioning(
        pa.schema([dataset.schema.field("part")]), dictionaries={"part": pa.array(["a", "b"])}
    )
...
def test_write_dataset_use_threads(tempdir):
    ...
    ds.write_dataset(
        dataset, target1, format="feather", partitioning=partitioning, use_threads=True, file_visitor=file_visitor
    )
...
def test_write_table_multiple_fragments(tempdir):
    table = pa.table(
        [pa.array(range(10)), pa.array(np.random.randn(10)), pa.array(np.repeat(["a", "b"], 5))],
        names=["f1", "f2", "part"],
    )
...
def test_write_dataset_parquet(tempdir):
    table = pa.table(
        [
            ...
            pa.array(np.arange("2012-01-01", 20, dtype="datetime64[D]").astype("datetime64[ns]")),
...
def test_partition_dataset_parquet_file_visitor(tempdir):
    ...
    ds.write_dataset(
        table, root_path, format="parquet", partitioning=partitioning, use_threads=True, file_visitor=file_visitor
    )
...
def test_write_dataset_s3_put_only(s3_server):
    ...
    fs = S3FileSystem(
        access_key="limited", secret_key="limited123", endpoint_override="{}:{}".format(host, port), scheme="http"
    )
...
def test_dataset_join(tempdir):
    ...
    assert result.to_table().sort_by("colA") == pa.table(
        {"colA": [1, 2, 6, 99], "col2": ["a", "b", "f", None], "col3": ["A", "B", None, "Z"]}
    )
...
def test_dataset_join_asof(tempdir):
    t1 = pa.Table.from_pydict({"colA": [1, 1, 5, 6, 7], "col2": ["a", "b", "a", "b", "f"]})
    ds.write_dataset(t1, tempdir / "t1", format="ipc")
    ds1 = ds.dataset(tempdir / "t1", format="ipc")

    t2 = pa.Table.from_pydict({"colB": [2, 9, 15], "col3": ["a", "b", "g"], "colC": [1.0, 3.0, 5.0]})
    ds.write_dataset(t2, tempdir / "t2", format="ipc")
    ds2 = ds.dataset(tempdir / "t2", format="ipc")
...
def test_dataset_join_asof_collisions(tempdir):
    t1 = pa.table({"colA": [1, 2, 6], "colB": [10, 20, 60], "on": [1, 2, 3], "colVals": ["a", "b", "f"]})
    ds.write_dataset(t1, tempdir / "t1", format="ipc")
    ds1 = ds.dataset(tempdir / "t1", format="ipc")
    ...
    msg = "Columns {'colVals'} present in both tables. " "AsofJoin does not support column collisions."
    with pytest.raises(ValueError, match=msg):
        ds1.join_asof(
            ds2,
            on="on",
...
def test_dataset_filter(tempdir, dstype):
    ...
    joined = result.join(
        ds.dataset(pa.table({"colB": [10, 20], "col2": ["a", "b"]})), keys="col2", join_type="right outer"
    )
...
def test_union_dataset_filter(tempdir, dstype):
    ...
    joined = filtered_union_ds.join(
        ds.dataset(pa.table({"colB": [10, 20], "col2": ["a", "b"]})), keys="col2", join_type="left outer"
    )
...
def test_write_dataset_with_scanner_use_projected_schema(tempdir):
    ...
    with pytest.raises(KeyError, match=r"'Column original_column does not exist in schema"):
...
def test_read_table_nested_columns(tempdir, format):
    ...
    table = pa.table(
        {
            ...
            "a.dotted.field": [1, 2],
            "interaction": [
                {"type": None, "element": "button", "values": [1, 2], "structs": [{"foo": "bar"}, None]},
                {"type": "scroll", "element": "window", "values": [None, 3, 4], "structs": [{"fizz": "buzz"}]},
            ],
        }
    )
    ...
    table = ds1.to_table(
        columns=["user_id", "interaction.type", "interaction.values", "interaction.structs", "a.dotted.field"]
    )
...
def test_dataset_sort_by(tempdir, dstype):
    ...
    assert dt.filter((pc.field("values") < 4)).sort_by("values").to_table().to_pydict() == {
        "keys": ["a", "a", "b"],
        "values": [1, 2, 3],
    }

    table = pa.Table.from_arrays(
        [pa.array([5, 7, 7, 35], type=pa.int64()), pa.array(["foo", "car", "bar", "foobar"])], names=["a", "b"]
    )
    dt = ds.dataset(table)
...
def test_make_write_options_error():
    ...
    msg_1 = "make_write_options() should be called on an " "instance of ParquetFileFormat"
