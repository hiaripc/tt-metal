...
def _check_pandas_roundtrip(
    ...
    write_feather(df, path, compression=compression, compression_level=compression_level, version=version)
...
def test_dataset(version):
    ...
    data = {"col_" + str(i): np.random.randn(num_values[0]) for i in range(num_values[1])}
    table = pa.table(data)
...
def test_boolean_object_nulls(version):
    repeats = 100
    table = pa.Table.from_arrays([np.array([False, None, True] * repeats, dtype=object)], names=["arr"])
    _check_arrow_roundtrip(table)
...
def test_delete_partial_file_on_error(version):
    ...
    # strings will fail
    df = pd.DataFrame(
        {"numbers": range(5), "strings": [b"foo", None, "bar", CustomClass(), np.nan]}, columns=["numbers", "strings"]
    )
...
def test_v1_unsupported_types():
    ...
    with pytest.raises(
        TypeError,
        match=("Unsupported Feather V1 type: " "list<item: int64>. " "Use V2 format to serialize all Arrow types."),
    ):
