...
class ClientHeaderAuthMiddleware(ClientMiddleware):
    ...
    def received_headers(self, headers):
        ...
        self.factory.set_call_credential([b"authorization", auth_header[0].encode("utf-8")])
...
def test_repr():
    ...
    endpoint_repr = "<pyarrow.flight.FlightEndpoint " "ticket=<pyarrow.flight.Ticket ticket=b'foo'> " "locations=[]>"
    info_repr = (
    ...
    info = flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), [], -1, -1)
    ...
    assert (
        repr(flight.SchemaResult(pa.schema([("int", "int64")]))) == "<pyarrow.flight.SchemaResult schema=(int: int64)>"
    )
...
def test_eq():
    items = [
        ...
        lambda: (flight.FlightDescriptor.for_command("foo"), flight.FlightDescriptor.for_path("foo")),
        lambda: (flight.FlightEndpoint(b"foo", []), flight.FlightEndpoint(b"", [])),
        lambda: (
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), [], -1, -1),
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_command(b"foo"), [], -1, 42),
        ),
        lambda: (flight.Location("grpc+tcp://localhost:1234"), flight.Location("grpc+tls://localhost:1234")),
        lambda: (flight.Result(b"foo"), flight.Result(b"bar")),
        lambda: (flight.SchemaResult(pa.schema([])), flight.SchemaResult(pa.schema([("ints", pa.int64())]))),
...
def test_flight_do_get_ticket():
    ...
    data1 = [pa.array([-10, -5, 0, 5, 10], type=pa.int32())]
    table = pa.Table.from_arrays(data1, names=["a"])
    with CheckTicketFlightServer(expected_ticket=b"the-ticket") as server, flight.connect(
...
def test_read_options():
    ...
    with ConstantFlightServer() as server, FlightClient(("localhost", server.port)) as client:
        options = flight.FlightCallOptions(read_options=IpcReadOptions(included_fields=[1]))
...
def test_http_basic_unauth():
    ...
    with EchoStreamFlightServer(auth_handler=basic_auth_handler) as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        action = flight.Action("who-am-i", b"")
        with pytest.raises(flight.FlightUnauthenticatedError, match=".*unauthenticated.*"):
            list(client.do_action(action))


@pytest.mark.skipif(os.name == "nt", reason="ARROW-10013: gRPC on Windows corrupts peer()")
...
def test_http_basic_auth_invalid_password():
    ...
    with EchoStreamFlightServer(auth_handler=basic_auth_handler) as server, FlightClient(
        ...
        action = flight.Action("who-am-i", b"")
        with pytest.raises(flight.FlightUnauthenticatedError, match=".*wrong password.*"):
...
def test_token_auth():
    ...
    with EchoStreamFlightServer(auth_handler=token_auth_handler) as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        action = flight.Action("who-am-i", b"")
...
def test_authenticate_basic_token():
    ...
    with HeaderAuthFlightServer(
        auth_handler=no_op_auth_handler, middleware={"auth": HeaderAuthServerMiddlewareFactory()}
...
def test_arbitrary_headers_in_flight_call_options():
    ...
    with ArbitraryHeadersFlightServer(
        ...
        options = flight.FlightCallOptions(
            headers=[token_pair, (b"test-header-1", b"value1"), (b"test-header-2", b"value2")]
        )
...
def test_tls_fails():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s, FlightClient(
        "grpc+tls://localhost:" + str(s.port)
    ) as client:
        # gRPC error messages change based on version, so don't look
        # for a particular error
...
def test_tls_override_hostname():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s, flight.connect(
        ("localhost", s.port), tls_root_certs=certs["root_cert"], override_hostname="fakehostname"
    ) as client:
        with pytest.raises(flight.FlightUnavailableError):
...
def test_middleware_mapping():
    ...
    with FlightServerBase(middleware={"test": server_middleware}) as server, FlightClient(
...
def test_mtls():
    ...
    with ConstantFlightServer(
        tls_certificates=[certs["certificates"][0]], verify_client=True, root_certificates=certs["root_cert"]
    ) as s, FlightClient(
        ("localhost", s.port),
...
def test_interrupt():
    ...
    with CancelFlightServer() as server, FlightClient(("localhost", server.port)) as client:
        reader = client.do_get(flight.Ticket(b""))
...
def test_large_descriptor():
    ...
    with FlightServerBase() as server, flight.connect(("localhost", server.port)) as client:
        with pytest.raises(OSError, match="Failed to serialize Flight descriptor"):
            writer, _ = client.do_put(large_descriptor, pa.schema([]))
            writer.close()
        with pytest.raises(pa.ArrowException, match="Failed to serialize Flight descriptor"):
    ...
    with TracingFlightServer(
        ...
        options = flight.FlightCallOptions(
            headers=[
                ...
                (b"traceparent", b"00-000ff00f00f0ff000f0f00ff0f00fff0-" b"000f0000f0f00000-00"),
                (b"tracestate", b""),
            ]
        )
...
def test_do_put_does_not_crash_when_schema_is_none():
    ...
    msg = "Argument 'schema' has incorrect type " r"\(expected pyarrow.lib.Schema, got NoneType\)"
