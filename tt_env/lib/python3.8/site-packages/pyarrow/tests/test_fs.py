...
def py_fsspec_s3fs(request, s3_server):
    ...
    fs = s3fs.S3FileSystem(
        key=access_key, secret=secret_key, client_kwargs=dict(endpoint_url="http://{}:{}".format(host, port))
    )
    fs = PyFileSystem(FSSpecHandler(fs))
    fs.create_dir(bucket)
...
@pytest.fixture(
    params=[
        ...
        pytest.param("py_fsspec_localfs", id="PyFileSystem(FSSpecHandler(fsspec.LocalFileSystem()))"),
        pytest.param("py_fsspec_memoryfs", id='PyFileSystem(FSSpecHandler(fsspec.filesystem("memory")))'),
        pytest.param("py_fsspec_s3fs", id="PyFileSystem(FSSpecHandler(s3fs.S3FileSystem()))", marks=pytest.mark.s3),
    ]
)
...
def test_open_append_stream(fs, pathfn, compression, buffer_size, compressor, decompressor, allow_append_to_file):
    ...
    if allow_append_to_file:
        with fs.open_append_stream(p, compression=compression, buffer_size=buffer_size) as f:
            f.write(b"\nnewly added")
...
def test_gcs_options(pickle_module):
    ...
    fs = GcsFileSystem(default_metadata={"ACL": "authenticated-read", "Content-Type": "text/plain"})
    assert isinstance(fs, GcsFileSystem)
    assert pickle_module.loads(pickle_module.dumps(fs)) == fs
...
def test_s3_options(pickle_module):
    ...
    fs2 = S3FileSystem(
        background_writes=True, default_metadata={"ACL": "authenticated-read", "Content-Type": "text/plain"}
    )
    assert isinstance(fs2, S3FileSystem)
    assert pickle_module.loads(pickle_module.dumps(fs2)) == fs2
    assert fs2 != fs
    ...
    fs = S3FileSystem(
        allow_bucket_creation=True, allow_bucket_deletion=True, check_directory_existence_before_creation=True
    )
    assert isinstance(fs, S3FileSystem)
    assert pickle_module.loads(pickle_module.dumps(fs)) == fs
...
def test_filesystem_from_uri_s3(s3_server):
    ...
    uri = "s3://{}:{}@mybucket/foo/bar?scheme=http&endpoint_override={}:{}" "&allow_bucket_creation=True".format(
...
def test_copy_files(s3_connection, s3fs, tempdir):
    ...
    source_uri = f"s3://{access_key}:{secret_key}@{path}" f"?scheme=http&endpoint_override={host}:{port}"
...
def test_copy_files_directory(tempdir):
    ...
    copy_files(str(source_dir), str(destination_dir2), source_filesystem=localfs, destination_filesystem=localfs)
