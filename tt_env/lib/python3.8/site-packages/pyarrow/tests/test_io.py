...
@pytest.mark.parametrize("val, expected_hex_buffer", [(b"check", b"636865636B"), (b"\a0", b"0730"), (b"", b"")])
...
def test_non_cpu_buffer(pickle_module):
    ...
    arr_short = pa.FixedSizeBinaryArray.from_buffers(pa.binary(5), 1, [None, cuda_buf_short])
...
def test_cache_options_pickling(pickle_module):
    options = [
        pa.CacheOptions(),
        pa.CacheOptions(hole_size_limit=4096, range_size_limit=8192, lazy=True, prefetch_limit=5),
    ]
...
@pytest.mark.parametrize(
    "compression",
    [
        pytest.param("bz2", marks=pytest.mark.xfail(raises=pa.lib.ArrowNotImplementedError)),
        "brotli",
        "gzip",
        "lz4",
        "zstd",
...
def test_compression_level(compression):
    ...
    for compression_level in range(min_level, max_level + 1):
        ...
        decompressed_bytes = codec.decompress(compressed_bytes, INPUT_SIZE, asbytes=True)
...
def test_memory_map_large_seeks():
    if sys.maxsize >= 2**32:
        ...
        expected_error = pytest.raises(
            pa.ArrowCapacityError, match="Requested memory map length 4294967306 " "does not fit in a C size_t"
        )
...
@pytest.mark.xfail(sys.platform == "emscripten", reason="umask doesn't work on Emscripten")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "lz4",
        "zstd",
        pytest.param("snappy", marks=pytest.mark.xfail(raises=pa.lib.ArrowNotImplementedError)),
    ],
)
