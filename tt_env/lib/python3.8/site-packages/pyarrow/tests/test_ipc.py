...
@pytest.fixture(
    params=[pytest.param("file_fixture", id="File Format"), pytest.param("stream_fixture", id="Stream Format")]
)
    ...
    df = pd.DataFrame(
        {
            ...
            "two": pd.Categorical(["foo", np.nan, "bar", "foo", "foo"], categories=["foo", "bar"], ordered=True),
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pa.ipc.IpcWriteOptions(use_legacy_format=True, metadata_version=pa.ipc.MetadataVersion.V4),
    ],
)
...
def test_read_options():
    ...
    options = pa.ipc.IpcReadOptions(use_threads=False, ensure_native_endian=False, included_fields=[1])
    assert options.use_threads is False
    assert options.ensure_native_endian is False
...
def test_schema_batch_serialize_methods():
    nrows = 5
    df = pd.DataFrame({"one": np.random.randn(nrows), "two": ["foo", np.nan, "bar", "bazbaz", "qux"]})
    batch = pa.RecordBatch.from_pandas(df)
...
def test_record_batch_reader_from_arrow_stream():
    ...
    with pytest.raises(pa.lib.ArrowTypeError, match="Field 0 cannot be cast"):
        pa.RecordBatchReader.from_stream(wrapper, schema=pa.schema([pa.field("a", pa.list_(pa.int32()))]))
