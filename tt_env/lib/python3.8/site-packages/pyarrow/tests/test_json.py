...
def test_read_options(pickle_module):
    ...
    check_options_class_pickling(cls, pickler=pickle_module, block_size=1234, use_threads=False)
...
def test_parse_options(pickle_module):
    ...
    check_options_class_pickling(
        cls, pickler=pickle_module, explicit_schema=schema, newlines_in_values=False, unexpected_field_behavior="ignore"
    )
...
class BaseTestJSONRead:
    ...
    def test_simple_nulls(self):
        ...
        table = self.read_bytes(rows)
        schema = pa.schema(
            [("a", pa.float64()), ("b", pa.int64()), ("c", pa.string()), ("d", pa.null()), ("e", pa.bool_())]
        )
        assert table.schema == schema
        assert table.to_pydict() == {
            "a": [1.0, None, 4.5],
