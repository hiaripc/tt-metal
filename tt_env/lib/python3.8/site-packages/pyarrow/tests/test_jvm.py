...
def root_allocator():
    ...
    jar_path = os.path.join(
        arrow_dir, "java", "tools", "target", "arrow-tools-{}-jar-with-dependencies.jar".format(version)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pa.timestamp("ms"), '{"name":"timestamp","unit":"MILLISECOND",' '"timezone":null}'),
        (pa.timestamp("us"), '{"name":"timestamp","unit":"MICROSECOND",' '"timezone":null}'),
        (pa.timestamp("ns"), '{"name":"timestamp","unit":"NANOSECOND",' '"timezone":null}'),
        (pa.timestamp("ns", tz="UTC"), '{"name":"timestamp","unit":"NANOSECOND"' ',"timezone":"UTC"}'),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pa.uint8(), list(range(128)), "UInt1Vector", '{"name":"int","bitWidth":8,"isSigned":false}'),
        (pa.uint16(), list(range(128)), "UInt2Vector", '{"name":"int","bitWidth":16,"isSigned":false}'),
        (pa.uint32(), list(range(128)), "UInt4Vector", '{"name":"int","bitWidth":32,"isSigned":false}'),
        (pa.uint64(), list(range(128)), "UInt8Vector", '{"name":"int","bitWidth":64,"isSigned":false}'),
        (pa.int8(), list(range(128)), "TinyIntVector", '{"name":"int","bitWidth":8,"isSigned":true}'),
        (pa.int16(), list(range(128)), "SmallIntVector", '{"name":"int","bitWidth":16,"isSigned":true}'),
        (pa.int32(), list(range(128)), "IntVector", '{"name":"int","bitWidth":32,"isSigned":true}'),
        (pa.int64(), list(range(128)), "BigIntVector", '{"name":"int","bitWidth":64,"isSigned":true}'),
        # TODO: float16
        (pa.float32(), list(range(128)), "Float4Vector", '{"name":"floatingpoint","precision":"SINGLE"}'),
        (pa.float64(), list(range(128)), "Float8Vector", '{"name":"floatingpoint","precision":"DOUBLE"}'),
        (
            pa.timestamp("s"),
            list(range(128)),
            "TimeStampSecVector",
        ...
        #  * pa.time64('us')
        #  * pa.time64('ns')
        (pa.date32(), list(range(128)), "DateDayVector", '{"name":"date","unit":"DAY"}'),
        (pa.date64(), list(range(128)), "DateMilliVector", '{"name":"date","unit":"MILLISECOND"}'),
        # TODO(ARROW-2606): pa.decimal128(19, 4)
    ],
)
