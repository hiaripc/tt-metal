...
try:
    ...
    from numpy.exceptions import VisibleDeprecationWarning as _np_VisibleDeprecationWarning
    ...
    return pd.DataFrame(
        {
            ...
            "datetime[s]": np.arange("2016-01-01T00:00:00.001", size, dtype="datetime64[s]"),
            "datetime[ms]": np.arange("2016-01-01T00:00:00.001", size, dtype="datetime64[ms]"),
            "datetime[us]": np.arange("2016-01-01T00:00:00.001", size, dtype="datetime64[us]"),
            "datetime[ns]": np.arange("2016-01-01T00:00:00.001", size, dtype="datetime64[ns]"),
...
def _check_pandas_roundtrip(
    ...
    with warnings.catch_warnings():
        ...
        tm.assert_frame_equal(
            result, expected, check_dtype=check_dtype, check_index_type=("equiv" if preserve_index else False)
        )
...
class TestConvertMetadata:
    ...
    def test_column_index_names_with_decimal(self):
        ...
        df = pd.DataFrame(
            ...
            columns=pd.MultiIndex.from_product([[decimal.Decimal(1)], [decimal.Decimal(2), decimal.Decimal(3)]]),
    ...
    def test_index_metadata_field_name(self):
        ...
        df = pd.DataFrame(
            ...
            index=pd.MultiIndex.from_arrays([["c", "b", "a"], [3, 2, 1]], names=[None, "foo"]),
    ...
    def test_categorical_column_index(self):
        df = pd.DataFrame(
            [(1, "a", 2.0), (2, "b", 3.0), (3, "c", 4.0)], columns=pd.Index(list("def"), dtype="category")
        )
        t = pa.Table.from_pandas(df, preserve_index=True)
        js = t.schema.pandas_metadata
    ...
    def test_integer_no_nulls(self):
        ...
        for dtype, arrow_dtype in numpy_dtypes:
            ...
            values = np.random.randint(
                max(info.min, np.iinfo(np.int_).min), min(info.max, np.iinfo(np.int_).max), size=num_values, dtype=dtype
            )
...
class TestConvertDateTimeLikeTypes:
    ...
    def test_timestamps_notimezone_no_nulls(self):
        df = pd.DataFrame(
            {
                "datetime64": np.array(
                    ["2007-07-13T01:23:34.123456789", "2006-01-13T12:34:56.432539784", "2010-08-13T05:46:57.437699912"],
                    dtype="datetime64[ns]",
                )
            }
        )
    ...
    def test_timestamps_notimezone_nulls(self):
        df = pd.DataFrame(
            {
                "datetime64": np.array(
                    ["2007-07-13T01:23:34.123456789", None, "2010-08-13T05:46:57.437699912"], dtype="datetime64[ns]"
                )
            }
        )
        field = pa.field("datetime64", pa.timestamp("ns"))
    ...
    def test_timestamps_with_timezone(self, unit):
        ...
        df = pd.DataFrame(
            {
                "datetime64": np.array(
                    ["2007-07-13T01:23:34.123", "2006-01-13T12:34:56.432", "2010-08-13T05:46:57.437"],
    ...
    def test_arrow_time_to_pandas(self):
        ...
        expected_df = pd.DataFrame(
            {"time64[us]": expected, "time64[ns]": expected, "time32[ms]": expected_ms, "time32[s]": expected_s},
            columns=names,
        )
    ...
    def test_numpy_datetime64_columns(self):
        datetime64_ns = np.array(
            ["2007-07-13T01:23:34.123456789", None, "2006-01-13T12:34:56.432539784", "2010-08-13T05:46:57.437699912"],
            dtype="datetime64[ns]",
        )
        _check_array_from_pandas_roundtrip(datetime64_ns)

        datetime64_us = np.array(
            ["2007-07-13T01:23:34.123456", None, "2006-01-13T12:34:56.432539", "2010-08-13T05:46:57.437699"],
            dtype="datetime64[us]",
        )
        _check_array_from_pandas_roundtrip(datetime64_us)

        datetime64_ms = np.array(
            ["2007-07-13T01:23:34.123", None, "2006-01-13T12:34:56.432", "2010-08-13T05:46:57.437"],
            dtype="datetime64[ms]",
        )
        _check_array_from_pandas_roundtrip(datetime64_ms)

        datetime64_s = np.array(
            ["2007-07-13T01:23:34", None, "2006-01-13T12:34:56", "2010-08-13T05:46:57"], dtype="datetime64[s]"
        )
    ...
    @pytest.mark.skipif(
        Version("1.16.0") <= Version(np.__version__) < Version("1.16.1"), reason="Until numpy/numpy#12745 is resolved"
    )
...
class TestConvertStringLikeTypes:
    ...
    def test_numpy_string_array_to_fixed_size_binary(self):
        ...
        with pytest.raises(pa.lib.ArrowInvalid, match=r"Got bytestring of length 3 \(expected 4\)"):
            arr = np.array([b"foo", b"bar", b"baz"], dtype="|S3")
            pa.array(arr, type=pa.binary(4))

        with pytest.raises(pa.lib.ArrowInvalid, match=r"Got bytestring of length 12 \(expected 3\)"):
...
class TestConvertListTypes:
    ...
    def test_column_of_arrays_to_py(self):
        ...
        dtype = "i1"
        arr = np.array(
            [np.arange(10, dtype=dtype), np.arange(5, dtype=dtype), None, np.arange(1, dtype=dtype)], dtype=object
        )
    ...
    def test_column_of_decimal_list(self):
        array = pa.array(
            [[decimal.Decimal("1"), decimal.Decimal("2")], [decimal.Decimal("3.3")]], type=pa.list_(pa.decimal128(2, 1))
        )
        table = pa.Table.from_arrays([array], names=["col1"])
        df = table.to_pandas()
    ...
    def test_to_list_of_structs_pandas(self):
        ...
        expected = pd.Series(
            [[{"f1": 1, "f2": ["a", "b"]}], [{"f1": 2, "f2": ["c", "d"]}, {"f1": 3, "f2": ["e", "f"]}]]
        )
    ...
    def test_map_array_with_nulls(self):
        ...
        expected = [
            [(k, float(v) if v is not None else None) for k, v in row] if row is not None else None for row in data
        ]
    ...
    def test_list_view_to_pandas_multiple_chunks(self, klass):
        ...
        arr1 = klass.from_arrays(offsets=pa.array([2, 1, 0]), sizes=pa.array([2, 2, 2]), values=pa.array([1, 2, 3, 4]))
...
class TestConvertStructTypes:
    ...
    def test_from_numpy_nested(self):
        ...
        ty = pa.struct(
            [
                pa.field("x", pa.struct([pa.field("xx", pa.int8()), pa.field("yy", pa.bool_())])),
                pa.field("y", pa.int16()),
...
class TestConvertMisc:
    ...
    def test_category(self):
        ...
        arrays = {
            ...
            "cat_strings_with_na": pd.Categorical(v1 * repeats, categories=["foo", "bar"]),
            "cat_ints": pd.Categorical(v2 * repeats),
            "cat_binary": pd.Categorical(v3 * repeats),
            "cat_strings_ordered": pd.Categorical(v1 * repeats, categories=["bar", "qux", "foo"], ordered=True),
            "ints": v2 * repeats,
            "ints2": v2 * repeats,
            "strings": v1 * repeats,
            "strings2": v1 * repeats,
    ...
    def test_partial_schema(self):
        data = OrderedDict(
            [("a", [0, 1, 2, 3, 4]), ("b", np.array([-10, -5, 0, 5, 10], dtype=np.int32)), ("c", [-10, -5, 0, 5, 10])]
        )
        df = pd.DataFrame(data)
...
def _fully_loaded_dataframe_example():
    index = pd.MultiIndex.from_arrays(
        [pd.date_range("2000-01-01", periods=5).repeat(2), np.tile(np.array(["foo", "bar"], dtype=object), 5)]
    )
...
def test_table_from_pandas_checks_field_nullability():
    ...
    schema = pa.schema([pa.field("a", pa.float64(), nullable=False), pa.field("b", pa.utf8(), nullable=False)])
...
def test_table_from_pandas_columns_argument_only_does_filtering():
    ...
    schema1 = pa.schema([("arrays", pa.list_(pa.int64())), ("floats", pa.float64()), ("partition", pa.int64())])
...
def test_table_from_pandas_schema_field_order_metadata():
    ...
    # mangle metadata
    df = pd.DataFrame(
        {"datetime": pd.date_range("2020-01-01T00:00:00Z", freq="h", periods=2), "float": np.random.randn(2)}
    )
...
def test_array_uses_memory_pool():
    # ARROW-6570
    N = 10000
    arr = pa.array(np.arange(N, dtype=np.int64), mask=np.random.randint(0, 2, size=N).astype(np.bool_))
...
def test_dictionary_with_pandas():
    ...
    for index_type in ["uint8", "int8", "uint16", "int16", "uint32", "int32", "uint64", "int64"]:
        ...
        ex_pandas2 = pd.Categorical.from_codes(np.where(mask, -1, signed_indices), categories=dictionary)
...
def test_dictionary_encoded_nested_to_pandas():
    ...
    result = arr.to_pandas()
    expected = pd.Series([np.array(["a", "a", "a"], dtype=object), np.array(["b", "b"], dtype=object)])
...
def test_roundtrip_empty_table_with_extension_dtype_index():
    ...
    if Version(pd.__version__) > Version("1.0"):
        ...
        tm.assert_index_equal(
            ...
            pd.Index([{"left": 0, "right": 1}, {"left": 1, "right": 2}, {"left": 2, "right": 3}], dtype="object"),
        )
            ...
            b"pandas": json.dumps(
                {
                    ...
                    "columns": [
                        {"metadata": None, "name": "a", "numpy_type": "int64", "pandas_type": "int64"},
                        {"metadata": None, "name": "b", "numpy_type": "object", "pandas_type": "unicode"},
                    ],
                    "index_columns": [{"kind": "range", "name": "qux", "start": 0, "step": 2, "stop": 8}],
