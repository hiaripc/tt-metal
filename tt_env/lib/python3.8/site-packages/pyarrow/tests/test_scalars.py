...
@pytest.mark.parametrize(
    ...
    [
        ...
        (decimal.Decimal("1.1234567890123456789012345678901234567890"), None, pa.Decimal256Scalar),
        ...
        (datetime.datetime.now().time().replace(microsecond=0), pa.time32("s"), pa.Time32Scalar),
...
@pytest.mark.parametrize(
    "ty",
    [pa.list_(pa.int64()), pa.large_list(pa.int64()), pa.list_view(pa.int64()), pa.large_list_view(pa.int64()), None],
)
...
def test_struct():
    ...
    assert list(s.values()) == [pa.scalar(2, type=pa.int16()), pa.scalar(3.5, type=pa.float32())]
    assert list(s.items()) == [("x", pa.scalar(2, type=pa.int16())), ("y", pa.scalar(3.5, type=pa.float32()))]
    assert "x" in s
    assert "y" in s
    assert "z" not in s
    assert 0 not in s
...
def test_union(pickle_module):
    ...
    arr = pa.UnionArray.from_dense(
        ...
        children=[pa.array([b"a", b"b", b"c", b"d"], type="binary"), pa.array([1, 2, 3], type="int64")],
    )
    for s in arr:
        s.validate(full=True)
        assert isinstance(s, pa.UnionScalar)
