...
def get_many_types():
    ...
    return (
        ...
        pa.union(
            [pa.field("a", pa.binary(10)), pa.field("b", pa.string())], mode=pa.lib.UnionMode_DENSE, type_codes=[4, 8]
        ),
        pa.union([pa.field("a", pa.binary(10)), pa.field("b", pa.string())], mode=pa.lib.UnionMode_SPARSE),
        pa.union(
            [pa.field("a", pa.binary(10), nullable=False), pa.field("b", pa.string())], mode=pa.lib.UnionMode_SPARSE
        ),
...
def test_is_map():
    ...
    fields = pa.map_(pa.field("key_name", pa.utf8(), nullable=False), pa.field("value_name", pa.int32()))
...
def test_is_union():
    for mode in [pa.lib.UnionMode_SPARSE, pa.lib.UnionMode_DENSE]:
        assert types.is_union(
            pa.union([pa.field("a", pa.int32()), pa.field("b", pa.int8()), pa.field("c", pa.string())], mode=mode)
        )
...
@pytest.mark.parametrize(
    ("tz", "expected"),
    [(datetime.timezone.utc, "UTC"), (datetime.timezone(datetime.timedelta(hours=1, minutes=30)), "+01:30")],
)
...
def test_convert_custom_tzinfo_objects_to_string():
    ...
    msg = r"Object returned by tzinfo.utcoffset\(None\) is not an instance " r"of datetime.timedelta"
...
def test_list_view_type():
    ...
    # metadata doesn't matter by default
    ty_metadata = pa.list_view(pa.field("item", pa.int64(), metadata={"hello": "world"}))
    assert ty == ty_metadata
    assert not ty.equals(ty_metadata, check_metadata=True)
