...
def make_dataframe():
    ...
    N = 30
    df = pd.DataFrame(
        {col: np.random.randn(N) for col in string.ascii_uppercase[:4]}, index=pd.Index([rands(10) for _ in range(N)])
    )
    return df
    ...
    def _leak_check():
        ...
        if current_use - baseline_use > threshold:
            raise Exception(
                ...
                "Departure from baseline {} after {} iterations".format(current_use - baseline_use, i)
            )
