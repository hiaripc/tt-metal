...
class DtdLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(<!\[)([^\[\s]+)(\s*)(\[)", bygroups(Keyword, Name.Entity, Text, Keyword)),  # conditional sections
...
class VueLexer(HtmlLexer):
    ...
    tokens = {
        "root": [
            (r"(\{\{)(.*?)(\}\})", bygroups(Comment.Preproc, using(JavascriptLexer), Comment.Preproc)),
            ("[^<&{]+", Text),
            inherit,
        ],
        "tag": [
        ...
        "attr-directive": [
            (r'(["\'])(.*?)(\1)', bygroups(String, using(JavascriptLexer), String), "#pop"),
