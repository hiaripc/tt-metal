...
@pytest.mark.parametrize(
    "test_path",
    [pytest.param(each, id=f"{each.parent.name}-{each.stem}") for each in SUITE.glob("*/**/*.json")],
)
def test_referencing_suite(test_path, subtests):
    ...
    registry = Registry().with_resources(
        (uri, specification.create_resource(contents)) for uri, contents in loaded["registry"].items()
    )
