...
def parse_escape(source, info, in_set):
    ...
    if ch in HEX_ESCAPES:
        ...
        return Atomic(Branch([String([0x0D, 0x0A]), SetUnion(info, [Character(c) for c in charset])]))
...
class Character(RegexBase):
    ...
    def dump(self, indent, reverse):
        ...
        print(
            "{}CHARACTER {} {}{}".format(INDENT * indent, POS_TEXT[self.positive], display, CASE_TEXT[self.case_flags])
        )
...
class Conditional(RegexBase):
    ...
    def __eq__(self, other):
        return type(self) is type(other) and (self.group, self.yes_item, self.no_item) == (
            other.group,
            other.yes_item,
            other.no_item,
        )
...
class Fuzzy(RegexBase):
    ...
    def __eq__(self, other):
        return (
            type(self) is type(other) and self.subpattern == other.subpattern and self.constraints == other.constraints
        )
...
class GreedyRepeat(RegexBase):
    ...
    def __eq__(self, other):
        return type(self) is type(other) and (self.subpattern, self.min_count, self.max_count) == (
            other.subpattern,
            other.min_count,
            other.max_count,
        )
...
class LookAround(RegexBase):
    ...
    def __eq__(self, other):
        return type(self) is type(other) and (self.behind, self.positive, self.subpattern) == (
            other.behind,
            other.positive,
            other.subpattern,
        )
...
class LookAroundConditional(RegexBase):
    ...
    def __eq__(self, other):
        return type(self) is type(other) and (self.subpattern, self.yes_item, self.no_item) == (
            other.subpattern,
            other.yes_item,
            other.no_item,
        )
...
class Property(RegexBase):
    ...
    def dump(self, indent, reverse):
        ...
        print(
            "{}PROPERTY {} {}:{}{}".format(
                INDENT * indent, POS_TEXT[self.positive], name, value, CASE_TEXT[self.case_flags]
            )
        )
...
class Range(RegexBase):
    ...
    def dump(self, indent, reverse):
        ...
        print(
            "{}RANGE {} {} {}{}".format(
                INDENT * indent, POS_TEXT[self.positive], display_lower, display_upper, CASE_TEXT[self.case_flags]
            )
        )
...
class SetUnion(SetBase):
    ...
    def optimise(self, info, reverse, in_set=False):
        ...
        if len(items) == 1:
            i = items[0]
            return i.with_flags(
                positive=i.positive == self.positive, case_flags=self.case_flags, zerowidth=self.zerowidth
            ).optimise(info, reverse, in_set)
