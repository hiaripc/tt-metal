...
class X509Adapter(HTTPAdapter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if cert_bytes is None or not isinstance(cert_bytes, bytes):
            raise ValueError(
                "Invalid cert content provided. " "You must provide an X.509 cert " "formatted as a byte array."
            )
        if pk_bytes is None or not isinstance(pk_bytes, bytes):
            raise ValueError(
                "Invalid private key content provided. " "You must provide a private key " "formatted as a byte array."
            )
...
def check_cert_dates(cert):
    ...
    if cert.not_valid_after < now or cert.not_valid_before > now:
        raise ValueError(
            ...
            "{:%Y-%m-%d %H:%M:%SZ} "
            "Not Before: {:%Y-%m-%d %H:%M:%SZ}".format(cert.not_valid_after, cert.not_valid_before)
        )
