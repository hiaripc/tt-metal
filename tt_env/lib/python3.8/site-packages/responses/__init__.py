...
def _get_url_and_path(url):
    ...
    url_and_path = urlunparse([url_parsed.scheme, url_parsed.netloc, url_parsed.path, None, None, None])
...
class Response(BaseResponse):
    ...
    def get_response(self, request):
        ...
        if self.auto_calculate_content_length and isinstance(body, BufferIO) and "Content-Length" not in headers:
    ...
    def __repr__(self):
        return "<Response(url='{url}' status={status} " "content_type='{content_type}' headers='{headers}')>".format(
            url=self.url,
            status=self.status,
...
class CallbackResponse(BaseResponse):
    def __init__(self, method, url, callback, stream=None, content_type="text/plain", **kwargs):
    ...
    def get_response(self, request):
        ...
        if isinstance(r_headers, dict) and "Content-Type" in r_headers:
            ...
            has_content_type = any([h for h in r_headers if h and h[0].lower() == "content-type"])
        if has_content_type:
...
class RequestsMock(object):
    ...
    def _on_request(self, adapter, request, **kwargs):
        ...
        if match is None:
            if any(
                [
                    p.match(request.url) if isinstance(p, Pattern) else request.url.startswith(p)
            ...
            for i, m in enumerate(self.registered()):
                error_msg += "- {} {} {}\n".format(m.method, m.url, match_failed_reasons[i])
    ...
    def stop(self, allow_assert=True):
        ...
        if not_called:
            raise AssertionError(
                "Not all requests have been executed {0!r}".format([(match.method, match.url) for match in not_called])
            )

    def assert_call_count(self, url, count):
        call_count = len([1 for call in self.calls if call.request.url == _ensure_url_default_path(url)])
        if call_count == count:
            return True
        else:
            raise AssertionError(
                "Expected URL '{0}' to be called {1} times. Called {2} times.".format(url, count, call_count)
            )
