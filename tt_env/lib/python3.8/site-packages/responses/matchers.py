...
def fragment_identifier_matcher(identifier):
    def match(request):
        ...
        if not valid:
            reason = "URL fragment identifier is different: {} doesn't match {}".format(identifier, url_fragment)
        return valid, reason
...
def query_param_matcher(params):
    ...
    def match(request):
        ...
        params_dict = params or {}
        valid = params is None if request_params is None else params_dict == request_params_dict
...
def request_kwargs_matcher(kwargs):
    ...
    def match(request):
        ...
        request_kwargs = {k: v for k, v in request.req_kwargs.items() if k in kwargs_dict}

        valid = not kwargs_dict if not request_kwargs else sorted(kwargs.items()) == sorted(request_kwargs.items())
...
def multipart_matcher(files, data=None):
    ...
    def match(request):
        ...
        prepared_content_type = prepared.headers["Content-Type"].replace(prepared_boundary, request_boundary)
        ...
        if not body_valid:
            return False, reason + "Request body differs. {} aren't equal {}".format(request_body, prepared_body)
