...
def test_fail_matchers_error():
    ...
    def run():
        with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
            ...
            assert "request.body doesn't match: JSONDecodeError: Cannot parse request.body" in msg
...
def test_multipart_matcher(req_file, match_file):
    ...
    def run():
        ...
        responses.add(
            ...
            match=[matchers.multipart_matcher(files={"file_name": match_file}, data=req_data)],
        )
        resp = requests.post("http://httpbin.org/post", data=req_data, files={"file_name": req_file})
        assert resp.status_code == 200
...
def test_multipart_matcher_fail():
    ...
    def run():
        ...
        with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
            ...
            msg = str(excinfo.value)
            assert "multipart/form-data doesn't match. Request headers['Content-Type'] is different." in msg
            assert "application/x-www-form-urlencoded isn't equal to multipart/form-data; boundary=" in msg
...
def test_query_string_matcher_raises():
    ...
    def run():
        with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
            ...
            msg = str(excinfo.value)
            assert "Query string doesn't match. {didi: pro, test: 1} doesn't match {didi: pro}" in msg
...
def test_request_matches_headers():
    ...
    def run():
        ...
        resp = requests.get(url, headers={"Accept": "application/json", "Accept-Charset": "utf-8"})
...
def test_request_matches_headers_no_match():
    ...
    def run():
        ...
        msg = str(excinfo.value)
        assert ("Headers do not match: {Accept: application/xml} doesn't match " "{Accept: application/json}") in msg
...
def test_request_matches_headers_strict_match():
    ...
    def run():
        ...
        msg = str(excinfo.value)
        assert (
            "Headers do not match: {Accept: text/plain, Accept-Charset: utf-8} " "doesn't match {Accept: text/plain}"
        ) in msg
...
def test_fragment_identifier_matcher_error():
    ...
    def run():
        ...
        msg = str(excinfo.value)
        assert ("URL fragment identifier is different: test=1 doesn't match test=2") in msg
        assert ("URL fragment identifier is different: None doesn't match test=2") in msg
