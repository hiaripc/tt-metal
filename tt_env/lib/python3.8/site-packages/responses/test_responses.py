...
def test_replace_error(original, replacement):
    ...
    def run():
        ...
        assert "Response is not registered for URL %s" % replacement in str(excinfo.value)
...
def test_replace_response_object_error():
    ...
    def run():
        ...
        with pytest.raises(ValueError) as excinfo:
            responses.replace(Response(method=responses.GET, url="http://example.com/two"))
        assert "Response is not registered for URL http://example.com/two" in str(excinfo.value)
...
def test_content_length_error(monkeypatch):
    ...
    monkeypatch.setattr(requests.packages.urllib3.HTTPResponse, "__init__", patched_init)  # type: ignore
...
def test_auto_calculate_content_length_string_body():
    ...
    def run():
        url = "http://example.com/"
        responses.add(responses.GET, url, body="test", auto_calculate_content_length=True)
        resp = requests.get(url)
        assert_response(resp, "test")
...
def test_auto_calculate_content_length_bytes_body():
    ...
    def run():
        url = "http://example.com/"
        responses.add(responses.GET, url, body=b"test bytes", auto_calculate_content_length=True)
        resp = requests.get(url)
...
def test_auto_calculate_content_length_unicode_body():
    ...
    def run():
        url = "http://example.com/test"
        responses.add(responses.GET, url, body="михољско лето", auto_calculate_content_length=True)
        resp = requests.get(url)
        assert_response(resp, "михољско лето", content_type="text/plain; charset=utf-8")
...
@pytest.mark.parametrize("url", ("http://example.com", "http://example.com?hello=world"))
def test_assert_call_count(url):
    ...
    def run():
        ...
        assert "Expected URL '{0}' to be called 2 times. Called 0 times.".format(url) in str(excinfo.value)
        ...
        assert "Expected URL '{0}' to be called 3 times. Called 2 times.".format(url) in str(excinfo.value)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            ("<Response(url='http://example.com/' status=200 " "content_type='text/plain' headers='null')>"),
        ),
        (
            {
...
def test_mocked_responses_list_registered():
    ...
    def run():
        ...
        second_response = Response(responses.GET, "http://example.com/", body="", headers={"X-Test": "foo"})
