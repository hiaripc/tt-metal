...
class PytestTester:
    ...
    def __call__(
        self, label="fast", verbose=1, extra_argv=None, doctests=False, coverage=False, tests=None, parallel=None
    ):
        import pytest
        ...
        if parallel is not None and parallel > 1:
            if _pytest_has_xdist():
                ...
                warnings.warn("Could not run tests in parallel because " "pytest-xdist plugin is not available.")
...
def check_free_memory(free_mb):
    ...
    try:
        ...
        msg = "{0} MB memory required, but {1} MB available".format(free_mb, mem_free / 1e6)
...
def _parse_size(size_str):
    ...
    m = re.match(r"^\s*(\d+)\s*({0})\s*$".format("|".join(suffixes.keys())), size_str, re.I)
