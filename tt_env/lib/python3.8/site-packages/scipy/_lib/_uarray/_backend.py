...
def pickle_function(func):
    ...
    if test is not func:
        raise pickle.PicklingError("Can't pickle {}: it's not the same object as {}".format(func, test))
...
class Dispatchable:
    ...
    def __str__(self):
        return "<{0}: type={1!r}, value={2!r}>".format(type(self).__name__, self.type, self.value)
...
def all_of_type(arg_type):
    ...
    def outer(func):
        ...
        def inner(*args, **kwargs):
            ...
            return tuple(
                Dispatchable(arg, arg_type) if not isinstance(arg, Dispatchable) else arg for arg in extracted_args
            )
...
def determine_backend_multi(dispatchables, *, domain, only=True, coerce=False, **kwargs):
