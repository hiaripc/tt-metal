    ...
    def __init__(self, pool=1):
        ...
        if callable(pool):
            ...
            if int(pool) == -1:
                ...
                raise RuntimeError(
                    "Number of workers specified must be -1," " an int >= 1, or an object with a 'map' " "method"
                )
        ...
        def wrapper(*args, **kwargs):
            if old_name in kwargs:
                ...
                if new_name in kwargs:
                    message = f"{fun.__name__}() got multiple values for " f"argument now known as `{new_name}`"
