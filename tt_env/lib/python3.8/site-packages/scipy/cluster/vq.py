...
def whiten(obs, check_finite=True):
    ...
    if zero_std_mask.any():
        ...
        warnings.warn(
            "Some columns have standard deviation zero. " "The values of these columns will not change.", RuntimeWarning
        )
    return obs / std_dev
...
def _kmeans(obs, guess, thresh=1e-5):
    ...
    while diff > thresh:
        ...
        code_book, has_members = _vq.update_cluster_means(obs, obs_code, code_book.shape[0])
...
def _missing_warn():
    ...
    warnings.warn("One of the clusters is empty. " "Re-run kmeans with a different initialization.")
...
def kmeans2(data, k, iter=10, thresh=1e-5, minit="random", missing="warn", check_finite=True, *, seed=None):
    """
