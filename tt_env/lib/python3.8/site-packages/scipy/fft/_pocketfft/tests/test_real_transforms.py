...
from scipy.fft._pocketfft.realtransforms import dct, idct, dst, idst, dctn, idctn, dstn, idstn
...
def is_longdouble_binary_compatible():
    try:
        one = np.frombuffer(b"\x00\x00\x00\x00\x00\x00\x00\x80\xff\x3f\x00\x00\x00\x00\x00\x00", dtype="<f16")
        return one == np.longfloat(1.0)
    except TypeError:
        return False
...
def test_overwrite(routine, dtype, shape, axis, type, norm, overwrite_x):
    ...
    sig = "%s(%s%r, %r, axis=%r, overwrite_x=%r)" % (routine.__name__, x.dtype, x.shape, None, axis, overwrite_x)
    if not overwrite_x:
