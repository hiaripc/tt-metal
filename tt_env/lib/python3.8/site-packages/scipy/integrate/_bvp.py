...
def solve_bvp(
    fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None, tol=1e-3, max_nodes=1000, verbose=0, bc_tol=None
):
    ...
    if bc_res.shape != (n + k,):
        raise ValueError(
            "`bc` return is expected to have shape {}, " "but actually has {}.".format((n + k,), bc_res.shape)
        )
    ...
    if verbose > 0:
        if status == 0:
            ...
            print(
                "Number of nodes is exceeded after iteration {}. \n"
                "Maximum relative residual: {:.2e} \n"
