...
def test_integration():
    ...
    for vectorized, method, t_span, jac in product(
        ...
        with suppress_warnings() as sup:
            ...
            res = solve_ivp(
                fun, t_span, y0, rtol=rtol, atol=atol, method=method, dense_output=True, jac=jac, vectorized=vectorized
            )
...
def test_events():
    ...
    for method in ["RK23", "RK45", "DOP853", "Radau", "BDF", "LSODA"]:
        res = solve_ivp(
            fun_rational, [5, 8], [1 / 3, 2 / 9], method=method, events=(event_rational_1, event_rational_2)
        )
        assert_equal(res.status, 0)
    ...
    for method in ["RK23", "RK45", "DOP853", "Radau", "BDF", "LSODA"]:
        res = solve_ivp(
            fun_rational, [8, 5], [4 / 9, 20 / 81], method=method, events=(event_rational_1, event_rational_2)
        )
        assert_equal(res.status, 0)
        assert_equal(res.t_events[0].size, 1)
        assert_equal(res.t_events[1].size, 1)
...
def test_max_step():
    ...
    for method in [RK23, RK45, DOP853, Radau, BDF, LSODA]:
        for t_span in ([5, 9], [5, 1]):
            res = solve_ivp(
                fun_rational, t_span, y0, rtol=rtol, max_step=0.5, atol=atol, method=method, dense_output=True
            )
            assert_equal(res.t[0], t_span[0])
...
def test_num_jac():
    def fun(t, y):
        return np.vstack(
            [-0.04 * y[0] + 1e4 * y[1] * y[2], 0.04 * y[0] - 1e4 * y[1] * y[2] - 3e7 * y[1] ** 2, 3e7 * y[1] ** 2]
        )

    def jac(t, y):
        return np.array(
            [[-0.04, 1e4 * y[2], 1e4 * y[1]], [0.04, -1e4 * y[2] - 6e7 * y[1], -1e4 * y[1]], [0, 6e7 * y[1], 0]]
        )
...
def test_num_jac_sparse():
    ...
    J_num_sparse, factor_sparse = num_jac(fun, 0, y.ravel(), f, 1e-8, None, sparsity=(A, groups))
    ...
    J_num_sparse, factor_sparse = num_jac(fun, 0, y.ravel(), f, 1e-8, factor, sparsity=(A, groups))
...
def test_args():
    ...
    assert_allclose(w[2], 1 / (((1 - z0) / z0) * np.exp(-k * t) + 1), rtol=1e-9, atol=1e-12)
