...
class ode:
    ...
    *Note*: The first two arguments of ``f(t, y, ...)`` are in the
    ...
    def integrate(self, t, step=False, relax=False):
        ...
        try:
            self._y, self.t = mth(
                self.f, self.jac or (lambda: None), self._y, self.t, t, self.f_params, self.jac_params
            )
...
class vode(IntegratorBase):
    ...
    def run(self, f, jac, y0, t0, t1, f_params, jac_params):
        ...
        if istate < 0:
            ...
            warnings.warn(
                "{:s}: {:s}".format(self.__class__.__name__, self.messages.get(istate, unexpected_istate_msg))
            )
            self.success = 0
        else:
