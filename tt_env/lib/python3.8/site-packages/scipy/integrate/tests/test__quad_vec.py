...
def test_quad_vec_simple(quadrature):
    ...
    for epsabs in [0.1, 1e-3, 1e-6]:
        ...
        res, err, *rest = quad_vec(f, 0, 2, norm="max", epsrel=1e-8, full_output=True, limit=10000, **kwargs)
        assert_allclose(res, exact, rtol=0, atol=epsabs)
...
def test_quad_vec_simple_inf(quadrature):
    ...
    res, err, info = quad_vec(
        f, -np.inf, np.inf, limit=1000, norm="max", epsabs=epsabs, quadrature=quadrature, full_output=True
    )
