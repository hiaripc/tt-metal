...
class TestODEClass:
    ...
    def _do_problem(self, problem, integrator, method="adams"):
        ...
        ig.set_integrator(
            integrator, atol=problem.atol / 10, rtol=problem.rtol / 10, method=method, **integrator_params
        )
    ...
    def verify(self, zs, t):
        ...
        u = np.vstack(
            (
                ...
                + lmbd[1] * lmbd[0] * self.z0[0] / d10 * (1 / d20 * (e0 - e2) - 1 / d21 * (e1 - e2)),
            )
        ).transpose()
        return allclose(u, zs, atol=self.atol, rtol=self.rtol)
...
class ODECheckParameterUse:
    ...
    def _get_solver(self, f, jac):
        ...
        if self.solver_uses_jac:
            solver.set_integrator(self.solver_name, atol=1e-9, rtol=1e-7, with_jacobian=self.solver_uses_jac)
        else:
...
def test_odeint_banded_jacobian():
    ...
    def bjac_rows(y, t, c):
        jac = np.row_stack((np.r_[0, np.diag(c, 1)], np.diag(c), np.r_[np.diag(c, -1), 0], np.r_[np.diag(c, -2), 0, 0]))
        return jac

    def bjac_cols(y, t, c):
        return bjac_rows(y, t, c).T.copy(order="C")

    c = array([[-205, 0.01, 0.00, 0.0], [0.1, -2.50, 0.02, 0.0], [1e-3, 0.01, -2.0, 0.01], [0.00, 0.00, 0.1, -1.0]])
    ...
    sol3, info3 = odeint(
        func, y0, t, args=(c,), full_output=True, atol=1e-13, rtol=1e-11, mxstep=10000, Dfun=bjac_rows, ml=2, mu=1
    )
