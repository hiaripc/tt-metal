...
class TestCtypesQuad:
    ...
    def test_ctypes_variants(self):
        ...
        sin_1 = get_clib_test_routine(
            "_sin_1", ctypes.c_double, ctypes.c_int, ctypes.POINTER(ctypes.c_double), ctypes.c_void_p
        )
...
class TestMultivariateCtypesQuad:
    ...
    def test_typical(self):
        ...
        assert_quad(quad(self._multivariate_typical, 0, pi, (2, 1.8)), 0.30614353532540296487)

    def test_indefinite(self):
        # 2) Infinite integration limits --- Euler's constant
        assert_quad(quad(self._multivariate_indefinite, 0, Inf), 0.577215664901532860606512)
...
class TestQuad:
    ...
    def test_sine_weighted_infinite(self):
        ...
        a = 4.0
        ome = 3.0
        assert_quad(quad(myfunc, 0, Inf, args=a, weight="sin", wvar=ome), ome / (a**2 + ome**2))
    ...
    def test_cosine_weighted_infinite(self):
        ...
        a = 2.5
        ome = 2.3
        assert_quad(quad(myfunc, -Inf, 0, args=a, weight="cos", wvar=ome), a / (a**2 + ome**2))
    ...
    def test_cauchypv_weight(self):
        ...
        assert_quad(quad(myfunc, 0, 5, args=0.4, weight="cauchy", wvar=2.0), tabledValue, error_tolerance=1.9e-8)
    ...
    def test_double_integral_improper(self, x_lower, x_upper, y_lower, y_upper, expected):
        ...
        assert_quad(dblquad(f, x_lower, x_upper, y_lower, y_upper), expected, error_tolerance=3e-8)
    ...
    def test_triple_integral(self):
        ...
        a, b = 1.0, 2.0
        assert_quad(
            tplquad(simpfunc, a, b, lambda x: x, lambda x: 2 * x, lambda x, y: x - y, lambda x, y: x + y, (2.0,)),
            2 * 8 / 3.0 * (b**4.0 - a**4.0),
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (-np.inf, -1, -np.inf, -1, -np.inf, 0, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 2)),
            (-np.inf, -1, -np.inf, 0, -np.inf, -1, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 2)),
            (-np.inf, 0, -np.inf, -1, -np.inf, -1, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 2)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, -1] for all n.
            (-np.inf, -1, -np.inf, -1, -np.inf, -1, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 3)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain Dx = [-inf, -1] and Dy = Dz = [-inf, 1].
            (-np.inf, -1, -np.inf, 1, -np.inf, 1, (np.pi ** (3 / 2)) / 8 * (((erf(1) + 1) ** 2) * erfc(1))),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain Dx = Dy = [-inf, -1] and Dz = [-inf, 1].
            (-np.inf, -1, -np.inf, -1, -np.inf, 1, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) * (erfc(1) ** 2))),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            ...
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, 1] for each n (two at a time).
            (-np.inf, 1, -np.inf, 1, -np.inf, 0, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 2)),
            (-np.inf, 1, -np.inf, 0, -np.inf, 1, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 2)),
            (-np.inf, 0, -np.inf, 1, -np.inf, 1, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 2)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, 1] for all n.
            (-np.inf, 1, -np.inf, 1, -np.inf, 1, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 3)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [0, inf] for all n.
            ...
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-1, inf] for each n (two at a time).
            (-1, np.inf, -1, np.inf, 0, np.inf, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 2)),
            (-1, np.inf, 0, np.inf, -1, np.inf, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 2)),
            (0, np.inf, -1, np.inf, -1, np.inf, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 2)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-1, inf] for all n.
            (-1, np.inf, -1, np.inf, -1, np.inf, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) ** 3)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain Dx = [1, inf] and Dy = Dz = [-1, inf].
            (1, np.inf, -1, np.inf, -1, np.inf, (np.pi ** (3 / 2)) / 8 * (((erf(1) + 1) ** 2) * erfc(1))),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain Dx = Dy = [1, inf] and Dz = [-1, inf].
            (1, np.inf, 1, np.inf, -1, np.inf, (np.pi ** (3 / 2)) / 8 * ((erf(1) + 1) * (erfc(1) ** 2))),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
    ...
    def test_triple_integral_improper(self, x_lower, x_upper, y_lower, y_upper, z_lower, z_upper, expected):
        ...
        assert_quad(tplquad(f, x_lower, x_upper, y_lower, y_upper, z_lower, z_upper), expected, error_tolerance=6e-8)
