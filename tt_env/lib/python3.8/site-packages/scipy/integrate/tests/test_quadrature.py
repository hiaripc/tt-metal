...
from numpy.testing import assert_equal, assert_almost_equal, assert_allclose, assert_, suppress_warnings
...
class TestQuadrature:
    ...
    def test_simps(self):
        ...
        y = np.arange(4)
        x = 2**y
        assert_equal(simpson(y, x=x, dx=0.5, even="first"), simps(y, x=x, dx=0.5, even="first"))
...
class TestCumulative_trapezoid:
    ...
    def test_y_nd_x_1d(self):
        ...
        ys_expected = (
            np.array(
                [[[4.0, 5.0, 6.0, 7.0], [8.0, 9.0, 10.0, 11.0]], [[40.0, 44.0, 48.0, 52.0], [56.0, 60.0, 64.0, 68.0]]]
            ),
            np.array([[[2.0, 3.0, 4.0, 5.0]], [[10.0, 11.0, 12.0, 13.0]], [[18.0, 19.0, 20.0, 21.0]]]),
            np.array(
                [
    ...
    def test_cumtrapz(self):
        ...
        y = x
        assert_allclose(
            cumulative_trapezoid(y, x, dx=0.5, axis=0, initial=0), cumtrapz(y, x, dx=0.5, axis=0, initial=0), rtol=1e-14
        )
...
class TestQMCQuad:
    ...
    def basic_test(self, n_points=2**8, n_estimates=8, signs=np.ones(2)):
        ...
        res = qmc_quad(func, a, b, n_points=n_points, n_estimates=n_estimates, args=(mean, cov), qrng=qrng)
