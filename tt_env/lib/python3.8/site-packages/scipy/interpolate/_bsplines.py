...
from scipy.linalg import get_lapack_funcs, LinAlgError, cholesky_banded, cho_solve_banded, solve, solve_banded
...
class BSpline:
    ...
    **Implementation details**
    ...
    def design_matrix(cls, x, t, k, extrapolate=False):
        ...
        data, indices = _bspl._make_design_matrix(x, t, k, extrapolate, indices)
    ...
    def integrate(self, a, b, extrapolate=None):
        ...
        if extrapolate == "periodic":
            ...
            if n_periods > 0:
                ...
                _bspl.evaluate_spline(ta, ca.reshape(ca.shape[0], -1), ka, x, 0, False, out)
            ...
            # Evaluate the difference of antiderivatives.
            x = np.asarray([a, b], dtype=np.float_)
            _bspl.evaluate_spline(ta, ca.reshape(ca.shape[0], -1), ka, x, 0, extrapolate, out)
            integral = out[1] - out[0]
    ...
    def from_power_basis(cls, pp, bc_type="not-a-knot"):
        ...
        if not isinstance(pp, CubicSpline):
            raise NotImplementedError("Only CubicSpline objects are accepted" "for now. Got %s instead." % type(pp))
        x = pp.x
        coef = pp.c
        k = pp.c.shape[0] - 1
        n = x.shape[0]
...
def make_lsq_spline(x, y, t, k=3, w=None, axis=0, check_finite=True):
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result is
    ...
    if k > 0 and np.any((x < t[k]) | (x > t[-k])):
        raise ValueError("Out of bounds w/ x = %s." % x)
    if x.size != w.size:
        raise ValueError("Shapes of x {} and w {} are incompatible".format(x.shape, w.shape))
