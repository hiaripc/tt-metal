...
import numpy as np
from numpy import array, transpose, searchsorted, atleast_1d, atleast_2d, ravel, poly1d, asarray, intp
...
class interp2d:
    ...
    def __init__(self, x, y, z, kind="linear", copy=True, bounds_error=False, fill_value=None):
        ...
        if rectangular_grid:
            ...
            z = ravel(z)
            if len(x) != len(y):
                raise ValueError("x and y must have equal lengths for non rectangular grid")
            if len(z) != len(x):
        ...
        if not rectangular_grid:
            ...
            nx, tx, ny, ty, c, fp, ier = dfitpack.regrid_smth(x, y, z, None, None, None, None, kx=kx, ky=ky, s=0.0)
    ...
    def __call__(self, x, y, dx=0, dy=0, assume_sorted=False):
        ...
        if self.bounds_error and (any_out_of_bounds_x or any_out_of_bounds_y):
            raise ValueError(
                "Values out of range; x must be in %r, y in %r" % ((self.x_min, self.x_max), (self.y_min, self.y_max))
            )
...
def _check_broadcast_up_to(arr_from, shape_to, name):
    ...
    raise ValueError(
        "%s argument must be able to broadcast up " "to shape %s but had shape %s" % (name, shape_to, shape_from)
    )
...
class interp1d(_Interpolator1D):
    ...
    def __init__(
        self, x, y, kind="linear", axis=-1, copy=True, bounds_error=None, fill_value=np.nan, assume_sorted=False
    ):
    ...
    def _check_bounds(self, x_new):
        ...
        if self.bounds_error and below_bounds.any():
            ...
            raise ValueError(
                "A value ({}) in x_new is below "
                "the interpolation range's minimum value ({}).".format(below_bounds_value, self.x[0])
            )
        if self.bounds_error and above_bounds.any():
            above_bounds_value = x_new[np.argmax(above_bounds)]
            raise ValueError(
                "A value ({}) in x_new is above "
                "the interpolation range's maximum value ({}).".format(above_bounds_value, self.x[-1])
            )
...
class PPoly(_PPolyBase):
    ...
    def _evaluate(self, x, nu, extrapolate, out):
        _ppoly.evaluate(self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, x, nu, bool(extrapolate), out)
    ...
    def integrate(self, a, b, extrapolate=None):
        ...
        if extrapolate == "periodic":
            ...
            if n_periods > 0:
                _ppoly.integrate(
                    self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, xs, xe, False, out=range_int
                )
                range_int *= n_periods
            else:
                range_int.fill(0)
            ...
            if b <= xe:
                _ppoly.integrate(
                    self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, a, b, False, out=remainder_int
                )
                range_int += remainder_int
            else:
                _ppoly.integrate(
                    self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, a, xe, False, out=remainder_int
                )
                range_int += remainder_int
            ...
            _ppoly.integrate(
                self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, a, b, bool(extrapolate), out=range_int
            )
    ...
    def solve(self, y=0.0, discontinuity=True, extrapolate=None):
        ...
        y = float(y)
        r = _ppoly.real_roots(
            self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, y, bool(discontinuity), bool(extrapolate)
        )
        if self.c.ndim == 2:
            return r[0]
        else:
...
class BPoly(_PPolyBase):
    ...
    def _evaluate(self, x, nu, extrapolate, out):
        _ppoly.evaluate_bernstein(
            self.c.reshape(self.c.shape[0], self.c.shape[1], -1), self.x, x, nu, bool(extrapolate), out
        )
    ...
    def from_derivatives(cls, xi, yi, orders=None, extrapolate=None):
        ...
        for i in range(m):
            ...
            if orders[i] is None:
                ...
                n1 = min(n - n2, len(y2))
                if n1 + n2 != n:
                    mesg = "Point %g has %d derivatives, point %g" " has %d derivatives, but order %d requested" % (
                        xi[i],
                        len(y1),
                        xi[i + 1],
                        len(y2),
