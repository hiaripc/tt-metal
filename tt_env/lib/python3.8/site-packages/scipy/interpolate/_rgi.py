...
def interpn(points, values, xi, method="linear", bounds_error=True, fill_value=np.nan):
    ...
    if method not in ["linear", "nearest", "cubic", "quintic", "pchip", "splinef2d", "slinear"]:
        raise ValueError(
    ...
    if len(points) != ndim and method == "splinef2d":
        raise ValueError("The method splinef2d can only be used for " "scalar data with one point per coordinate")
    ...
    if method in ["linear", "nearest", "slinear", "cubic", "quintic", "pchip"]:
        ...
        idx_valid = np.all(
            (grid[0][0] <= xi[:, 0], xi[:, 0] <= grid[0][-1], grid[1][0] <= xi[:, 1], xi[:, 1] <= grid[1][-1]), axis=0
        )
