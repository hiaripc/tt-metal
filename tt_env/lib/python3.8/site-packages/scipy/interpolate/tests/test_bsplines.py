...
class TestBSpline:
    ...
    def test_endpoints(self):
        ...
        for extrap in (True, False):
            assert_allclose(b([tm, tp], extrap), b([tm + 1e-10, tp - 1e-10], extrap), atol=1e-9)

    def test_continuity(self):
        # assert continuity at internal knots
        b = _make_random_spline()
        t, _, k = b.tck
        assert_allclose(b(t[k + 1 : -k - 1] - 1e-10), b(t[k + 1 : -k - 1] + 1e-10), atol=1e-9)
...
def test_knots_multiplicity():
    ...
    def check_splev(b, j, der=0, atol=1e-14, rtol=1e-14):
        ...
        assert_allclose(
            splev(x, (t, c, k), der), b(x, der), atol=atol, rtol=rtol, err_msg="der = %s  k = %s" % (der, b.k)
        )
...
def B_012(x):
    ...
    x = np.atleast_1d(x)
    return np.piecewise(
        x, [(x < 0) | (x > 2), (x >= 0) & (x < 1), (x >= 1) & (x <= 2)], [lambda x: 0.0, lambda x: x, lambda x: 2.0 - x]
    )
