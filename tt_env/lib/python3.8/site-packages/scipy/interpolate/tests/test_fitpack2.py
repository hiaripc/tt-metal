...
class TestUnivariateSpline:
    ...
    def test_derivatives_2(self):
        ...
        ders = spalde(3, tck)
        assert_allclose(
            ders, [45.0, 39.0, 22.0, 6.0], atol=1e-15  # 3**3 + 2*(3)**2  # 3*(3)**2 + 4*(3)  # 6*(3) + 4  # 6*3**0
        )
    ...
    def test_array_like_input(self):
        ...
        with suppress_warnings() as sup:
            ...
            spl2 = LSQBivariateSpline(
                x.tolist(), y.tolist(), z.tolist(), tx.tolist(), ty.tolist(), w=w.tolist(), bbox=bbox
            )
...
class TestSmoothSphereBivariateSpline:
    ...
    def test_invalid_input(self):
        ...
        with assert_raises(ValueError) as exc_info:
            invalid_theta = array(
                [-0.1 * pi, 0.25 * pi, 0.25 * pi, 0.5 * pi, 0.5 * pi, 0.5 * pi, 0.75 * pi, 0.75 * pi, 0.75 * pi]
            )
        ...
        with assert_raises(ValueError) as exc_info:
            invalid_theta = array(
                [0.25 * pi, 0.25 * pi, 0.25 * pi, 0.5 * pi, 0.5 * pi, 0.5 * pi, 0.75 * pi, 0.75 * pi, 1.1 * pi]
            )
            SmoothSphereBivariateSpline(invalid_theta, phi, r, s=1e10)
        assert "theta should be between [0, pi]" in str(exc_info.value)
    ...
    def test_array_like_input(self):
        theta = np.array(
            [0.25 * pi, 0.25 * pi, 0.25 * pi, 0.5 * pi, 0.5 * pi, 0.5 * pi, 0.75 * pi, 0.75 * pi, 0.75 * pi]
        )
        ...
        # list input
        spl2 = SmoothSphereBivariateSpline(theta.tolist(), phi.tolist(), r.tolist(), w=w.tolist(), s=1e10)
        assert_array_almost_equal(spl1(1.0, 1.0), spl2(1.0, 1.0))
...
class TestRectBivariateSpline:
    ...
    def test_derivatives_grid(self):
        ...
        dy = array(
            [[4, -1, 0, 1, -4], [4, -1, 0, 1, -4], [0, 1.5, 0, -1.5, 0], [2, 0.25, 0, -0.25, -2], [4, -1, 0, 1, -4]]
        )
        dxdy = (
            array(
                [
...
class TestRectSphereBivariateSpline:
    ...
    def test_derivatives_grid(self):
        ...
        assert_allclose(lut(x, y, dtheta=1), _numdiff_2d(lut, x, y, dx=1), rtol=1e-4, atol=1e-4)
        assert_allclose(lut(x, y, dphi=1), _numdiff_2d(lut, x, y, dy=1), rtol=1e-4, atol=1e-4)
        assert_allclose(lut(x, y, dtheta=1, dphi=1), _numdiff_2d(lut, x, y, dx=1, dy=1, eps=1e-6), rtol=1e-3, atol=1e-3)
...
def _numdiff_2d(func, x, y, dx=0, dy=0, eps=1e-8):
    if dx == 0 and dy == 0:
        ...
        return (func(x + eps, y + eps) - func(x - eps, y + eps) - func(x + eps, y - eps) + func(x - eps, y - eps)) / (
            2 * eps
        ) ** 2
    else:
...
class Test_DerivedBivariateSpline(object):
    ...
    def setup_method(self):
        ...
        with suppress_warnings() as sup:
            ...
            self.lut_lsq = LSQBivariateSpline(x, y, z, linspace(0.5, 19.5, 4), linspace(1.5, 20.5, 4), eps=1e-2)
