...
class TestInterp1D:
    ...
    def test_validation(self):
        ...
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=np.array((-1, -1, -1))
        )  # doesn't broadcast
        assert_raises(ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=[[-1]])  # doesn't broadcast
        assert_raises(ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=[-1, -1])  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=np.array([])
        )  # doesn't broadcast
        assert_raises(ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=())  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x2, self.y210, kind="linear", axis=0, fill_value=[-1, -1]
        )  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x2, self.y210, kind="linear", axis=0, fill_value=(0.0, [-1, -1])
        )  # above doesn't bc
            ...
            for dty in dt_rc:
                ...
                for dtn in dt_r:
                    ...
                    for kind in spline_kinds:
                        ...
                        assert_allclose(f(xnew), y, atol=1e-7, err_msg="%s, %s %s" % (dtx, dty, dtn))
    ...
    def test_previous(self):
        ...
        assert_allclose(
            interpolator2D([-1, -2, 5, 8, 12, 25]), [[np.nan, np.nan, 5, 8, 9, 9], [np.nan, np.nan, 15, 18, 19, 19]]
        )
        ...
        interpolator1D = interp1d(
            [2, 0, 1], [-1, 0, 1], kind="previous", fill_value="extrapolate", assume_sorted=False  # x is not ascending
        )
        assert_allclose(interpolator1D([-2, -1, 0, 1, 2, 3, 5]), [np.nan, np.nan, 0, 1, -1, -1, -1])
    ...
    def test_next(self):
        ...
        assert_allclose(
            interpolator2D([-1, -2, 5, 8, 12, 25]), [[0, 0, 5, 8, np.nan, np.nan], [10, 10, 15, 18, np.nan, np.nan]]
        )
        ...
        interpolator1D = interp1d(
            [2, 0, 1], [-1, 0, 1], kind="next", fill_value="extrapolate", assume_sorted=False  # x is not ascending
        )
        assert_allclose(interpolator1D([-2, -1, 0, 1, 2, 3, 5]), [0, 0, 0, 1, -1, np.nan, np.nan])

        interpolator2D = interp1d(self.x10, self.y210_edge_updated, kind="next", fill_value="extrapolate")
        assert_allclose(
            interpolator2D([-1, -2, 5, 8, 12, 25]), [[30, 30, 5, 8, np.nan, np.nan], [30, 30, 15, 18, np.nan, np.nan]]
        )
    ...
    def _check_fill_value(self, kind):
        ...
        for y in (self.y325, self.y225):
            assert_raises(
                ValueError, interp1d, self.x5, y, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
            )
        ...
        fill_value = [100, 200]
        assert_raises(
            ValueError, interp1d, self.x5, self.y235, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
        )
    ...
    def test_antiderivative_vs_derivative(self):
        ...
        for dx in range(0, 10):
            ...
            for k in range(dx):
                ...
                assert_allclose(pp2(pp2.x[1:]), pp2(endpoint), rtol=1e-7, err_msg="dx=%d k=%d" % (dx, k))
...
class TestBPoly:
    ...
    def test_simple5(self):
        ...
        bp = BPoly(c, x)
        assert_allclose(
            bp(0.3), 0.7**4 + 4 * 0.7**3 * 0.3 + 8 * 6 * 0.7**2 * 0.3**2 + 2 * 4 * 0.7 * 0.3**3 + 0.3**4
        )
...
class TestBPolyCalculus:
    ...
    def test_antiderivative_simple(self):
        ...
        assert_allclose(
            bi(xx), np.where(xx < 1, xx**2 / 2.0, 0.5 * xx * (xx / 2.0 - 1) + 3.0 / 4), atol=1e-12, rtol=1e-12
        )
...
class TestBPolyFromDerivatives:
    ...
    def test_orders_too_high(self):
        ...
        assert_raises(ValueError, BPoly.from_derivatives, **dict(xi=xi, yi=yi, orders=2 * k))  # but this is not
...
class TestNdPPoly:
    ...
    def test_integrate_2d(self):
        ...
        for ranges in [[(0, 1), (0, 1)], [(0, 0.5), (0, 1)], [(0, 1), (0, 0.5)], [(0.3, 0.7), (0.6, 0.2)]]:
            ...
            ig2, err2 = nquad(lambda x, y: p((x, y)), ranges, opts=[dict(epsrel=1e-5, epsabs=1e-5)] * 2)
            assert_allclose(ig, ig2, rtol=1e-5, atol=1e-5, err_msg=repr(ranges))
    ...
    for jout, (x, y) in enumerate(zip(xnew, ynew)):
        ...
        for k1 in range(c.shape[0]):
            for k2 in range(c.shape[1]):
                val += c[nx - k1 - 1, ny - k2 - 1, j1, j2] * _dpow(s1, k1, nu[0]) * _dpow(s2, k2, nu[1])
