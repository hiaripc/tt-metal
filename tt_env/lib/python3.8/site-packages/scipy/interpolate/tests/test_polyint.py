...
def check_shape(interpolator_cls, x_shape, y_shape, deriv_shape=None, axis=0, extra_args={}):
    np.random.seed(1234)
    ...
    if x_shape and y.size > 0:
        if interpolator_cls is CubicHermiteSpline:
            interpolator_cls(list(x), list(y), list(dydx), axis=axis, **extra_args)(list(xi))
        else:
...
class TestCubicSpline:
    ...
    def check_correctness(S, bc_start="not-a-knot", bc_end="not-a-knot", tol=1e-14):
        ...
        # Check C2 continuity.
        assert_allclose(
            c[3, 1:], c[0, :-1] * dxi**3 + c[1, :-1] * dxi**2 + c[2, :-1] * dxi + c[3, :-1], rtol=tol, atol=tol
        )
