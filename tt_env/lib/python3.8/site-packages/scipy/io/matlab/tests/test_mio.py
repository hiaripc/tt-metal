...
from numpy.testing import assert_array_equal, assert_array_almost_equal, assert_equal, assert_, assert_warns
import pytest
...
CA = mlarr(
    ([], mlarr([1]), mlarr([[1, 2]]), mlarr([[1, 2, 3]])), dtype=object  # tuple for object array creation
).reshape(1, -1)
...
CAE = mlarr(
    (mlarr(1), mlarr(2), mlarr([]), mlarr([]), mlarr(3)), dtype=object  # tuple for object array creation
).reshape(1, -1)
objarr = np.empty((1, 1), dtype=object)
objarr[0, 0] = mlarr(1)
case_table5.append(
    {"name": "scalarcell", "classes": {"testscalarcell": "cell"}, "expected": {"testscalarcell": objarr}}
)
...
def _check_level(label, expected, actual):
    ...
    assert_(
        types_compatible(expected, actual), "Expected type %s, got %s at %s" % (type(expected), type(actual), label)
    )
    ...
    assert_(
        expected.shape == actual.shape, msg="Expected shape %s, got %s at %s" % (expected.shape, actual.shape, label)
    )
...
def _cases(version, filt="test%(name)s_*.mat"):
    ...
    for case in cases:
        ...
        if filt is None:
            ...
            files = glob(use_filt)
            assert len(files) > 0, "No files for test %s using filter %s" % (name, filt)
...
def test_matfile_version(version, filt, regex):
    ...
    assert len(files) > 0, "No files for version %s using filter %s" % (version, filt)
    for file in files:
