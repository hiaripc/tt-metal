...
class TestMMIOSparseCSR(TestMMIOArray):
    ...
    def test_simple_integer(self, typeval, dtype):
        self.check_exact(
            scipy.sparse.csr_matrix([[1, 2], [3, 4]], dtype=dtype), (2, 2, 4, "coordinate", typeval, "general")
        )
    ...
    def test_simple_upper_triangle_integer(self, typeval, dtype):
        self.check_exact(
            scipy.sparse.csr_matrix([[0, 1], [0, 0]], dtype=dtype), (2, 2, 1, "coordinate", typeval, "general")
        )

    @pytest.mark.parametrize("typeval, dtype", parametrize_args)
    def test_simple_lower_triangle_integer(self, typeval, dtype):
        self.check_exact(
            scipy.sparse.csr_matrix([[0, 0], [1, 0]], dtype=dtype), (2, 2, 1, "coordinate", typeval, "general")
        )

    @pytest.mark.parametrize("typeval, dtype", parametrize_args)
    def test_simple_rectangular_integer(self, typeval, dtype):
        self.check_exact(
            scipy.sparse.csr_matrix([[1, 2, 3], [4, 5, 6]], dtype=dtype), (2, 3, 6, "coordinate", typeval, "general")
        )
    ...
    def test_simple_symmetric_integer(self, typeval, dtype):
        self.check_exact(
            scipy.sparse.csr_matrix([[1, 2], [2, 4]], dtype=dtype), (2, 2, 3, "coordinate", typeval, "symmetric")
        )

    def test_simple_skew_symmetric_integer(self):
        self.check_exact(
            scipy.sparse.csr_matrix([[0, 2], [-2, 0]]), (2, 2, 1, "coordinate", "integer", "skew-symmetric")
        )

    def test_simple_skew_symmetric_float(self):
        self.check(
            scipy.sparse.csr_matrix(array([[0, 2], [-2.0, 0]], "f")), (2, 2, 1, "coordinate", "real", "skew-symmetric")
        )
...
class TestMMIOCoordinate:
    ...
    def test_complex_write_read(self):
        ...
        J = array([0, 3, 1, 2, 1, 3, 4, 4])
        V = array(
            [1.0 + 3j, 6.0 + 2j, 10.50 + 0.9j, 0.015 + -4.4j, 250.5 + 0j, -280.0 + 5j, 33.32 + 6.4j, 12.00 + 0.8j]
        )
