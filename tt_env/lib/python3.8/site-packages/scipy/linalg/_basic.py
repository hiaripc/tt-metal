...
def _solve_check(n, info, lamch=None, rcond=None):
    ...
    if info < 0:
        raise ValueError("LAPACK reported an illegal value in {}-th argument" ".".format(-info))
    elif 0 < info:
    ...
    if rcond < E:
        warn(
            "Ill-conditioned matrix (rcond={:.6g}): " "result may not be accurate.".format(rcond),
            LinAlgWarning,
            stacklevel=3,
        )
...
def solve(
    ...
    if sym_pos:
        message = (
            "The 'sym_pos' keyword is deprecated and should be "
    ...
    if transposed:
        ...
        if np.iscomplexobj(a1):
            raise NotImplementedError(
                "scipy.linalg.solve can currently " "not solve a^T x = b or a^H x = b " "for complex matrices."
            )
    else:
        trans = 0
        norm = "1"
...
def solve_triangular(a, b, trans=0, lower=False, unit_diagonal=False, overwrite_b=False, check_finite=True):
    ...
    if a1.shape[0] != b1.shape[0]:
        raise ValueError("shapes of a {} and b {} are incompatible".format(a1.shape, b1.shape))
    overwrite_b = overwrite_b or _datacopied(b1, b)
    ...
    if a1.flags.f_contiguous or trans == 2:
        ...
        x, info = trtrs(a1.T, b1, overwrite_b=overwrite_b, lower=not lower, trans=not trans, unitdiag=unit_diagonal)
...
def solve_toeplitz(c_or_cr, b, check_finite=True):
    ...
    r, c, b, dtype, b_shape = _validate_args_for_toeplitz_ops(c_or_cr, b, check_finite, keep_b_shape=True)
    ...
    if b.ndim == 1:
        ...
        x = np.column_stack([levinson(vals, np.ascontiguousarray(b[:, i]))[0] for i in range(b.shape[1])])
        x = x.reshape(*b_shape)
...
def solve_circulant(c, b, singular="raise", tol=None, caxis=-1, baxis=0, outaxis=0):
    ...
    if nc != nb:
        raise ValueError("Shapes of c {} and b {} are incompatible".format(c.shape, b.shape))
...
def inv(a, overwrite_a=False, check_finite=True):
    ...
    if info > 0:
        raise LinAlgError("singular matrix")
    if info < 0:
        raise ValueError("illegal value in %d-th argument of internal " "getrf|getri" % -info)
    return inv_a
...
def det(a, overwrite_a=False, check_finite=True):
    ...
    if info < 0:
        raise ValueError("illegal value in %d-th argument of internal " "det.getrf" % -info)
    return a_det
...
def lstsq(a, b, cond=None, overwrite_a=False, overwrite_b=False, check_finite=True, lapack_driver=None):
    ...
    if m != b1.shape[0]:
        raise ValueError(
            "Shape mismatch: a and b should have the same number" " of rows ({} != {}).".format(m, b1.shape[0])
        )
    ...
    if driver in ("gelss", "gelsd"):
        ...
        if info < 0:
            raise ValueError("illegal value in %d-th argument of internal %s" % (-info, lapack_driver))
        ...
        if info < 0:
            raise ValueError("illegal value in %d-th argument of internal " "gelsy" % -info)
        if m > n:
            x1 = x[:n]
            x = x1
...
def pinv(a, atol=None, rtol=None, return_rank=False, check_finite=True, cond=None, rcond=None):
    ...
    if rcond or cond:
        warn(
            'Use of the "cond" and "rcond" keywords are deprecated and '
            'will be removed in future versions of SciPy. Use "atol" and '
            '"rtol" keywords instead',
            DeprecationWarning,
            stacklevel=2,
        )
