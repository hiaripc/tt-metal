...
def eig(
    ...
    _check_info(
        info, "eig algorithm (geev)", positive="did not converge (only eigenvalues " "with order >= %d have converged)"
    )
...
def eigh(
    ...
    if turbo:
        warnings.warn(
            "Keyword argument 'turbo' is deprecated in favour of '"
    ...
    if eigvals:
        warnings.warn(
            "Keyword argument 'eigvals' is deprecated in favour of "
    ...
    if driver not in drv_str:
        raise ValueError(
            '"{}" is unknown. Possible values are "None", "{}".' "".format(driver, '", "'.join(drv_str[1:]))
        )
    ...
    if driver:
        if b is None and (driver in ["gv", "gvd", "gvx"]):
            raise ValueError(
                "{} requires input b array to be supplied " "for generalized eigenvalue problems." "".format(driver)
            )
        if (b is not None) and (driver in ["ev", "evd", "evr", "evx"]):
            raise ValueError(
                '"{}" does not accept input b array ' "for standard eigenvalue problems." "".format(driver)
            )
    ...
    if info == 0:
        ...
        if info < -1:
            raise LinAlgError(
                "Illegal value in argument {} of internal {}" "".format(-info, drv.typecode + pfx + driver)
            )
        elif info > n:
            raise LinAlgError(
                "The leading minor of order {} of B is not "
    ...
    if select != 0:  # (non-all)
        ...
        if select == 1:  # (value)
            ...
            if sr.dtype.char.lower() not in "hilqp":
                raise ValueError(
                    'when using select="i", select_range must '
                    "contain integers, got dtype %s (%s)" % (sr.dtype, sr.dtype.char)
                )
...
def eigh_tridiagonal(
    d, e, eigvals_only=False, select="a", select_range=None, check_finite=True, tol=0.0, lapack_driver="auto"
):
    """
    Solve eigenvalue problem for a real symmetric tridiagonal matrix.
