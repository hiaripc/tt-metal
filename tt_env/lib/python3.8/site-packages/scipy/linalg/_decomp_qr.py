...
def qr_multiply(a, c, mode="right", pivoting=False, conjugate=False, overwrite_a=False, overwrite_c=False):
    ...
    if mode == "left":
        if c.shape[0] != min(M, N + overwrite_c * (M - N)):
            raise ValueError(
                "Array shapes are not compatible for Q @ c" " operation: {} vs {}".format(a.shape, c.shape)
            )
    else:
        if M != c.shape[1]:
            raise ValueError(
                "Array shapes are not compatible for c @ Q" " operation: {} vs {}".format(c.shape, a.shape)
            )
