...
def _qz(A, B, output="real", lwork=None, sort=None, overwrite_a=False, overwrite_b=False, check_finite=True):
    if sort is not None:
        ...
        raise ValueError(
            "The 'sort' input of qz() has to be None and will be " "removed in a future release. Use ordqz instead."
        )
...
def ordqz(A, B, sort="lhp", output="real", overwrite_a=False, overwrite_b=False, check_finite=True):
    ...
    (AA, BB, _, *ab, Q, Z, _, _), typ = _qz(
        A, B, output=output, sort=None, overwrite_a=overwrite_a, overwrite_b=overwrite_b, check_finite=check_finite
    )
