...
import numpy as np
from numpy import Inf, dot, diag, prod, logical_not, ravel, transpose, conjugate, absolute, amax, sign, isfinite
...
def expm(A):
    ...
    if a.shape[-2:] == (2, 2):
        ...
        mu = csqrt((a1 - a4) ** 2 + 4 * a2 * a3) / 2.0  # csqrt slow but handles neg.vals

        eApD2 = np.exp((a1 + a4) / 2.0)
        AmD2 = (a1 - a4) / 2.0
        coshMu = np.cosh(mu)
