...
for routine in [ssyevr, dsyevr, cheevr, zheevr, ssyevx, dsyevx, cheevx, zheevx, ssygvd, dsygvd, chegvd, zhegvd]:
    if routine.__doc__:
...
def get_lapack_funcs(names, arrays=(), dtype=None, ilp64=False):
    ...
    if not ilp64:
        return _get_funcs(
            names, arrays, dtype, "LAPACK", _flapack, _clapack, "flapack", "clapack", _lapack_alias, ilp64=False
        )
    else:
        if not HAS_ILP64:
            raise RuntimeError("LAPACK ILP64 routine requested, but Scipy " "compiled only with 32-bit BLAS")
        return _get_funcs(
            names, arrays, dtype, "LAPACK", _flapack_64, None, "flapack_64", None, _lapack_alias, ilp64=True
        )
...
def _check_work_float(value, dtype, int_dtype):
    ...
    if int_dtype.itemsize == 4:
        ...
        if value < 0 or value > _int64_max:
            raise ValueError(
                "Too large work array required -- computation" " cannot be performed with standard 64-bit" " LAPACK."
            )
    return value
