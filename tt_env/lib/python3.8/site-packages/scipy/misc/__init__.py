...
def __getattr__(name):
    if name not in __all__:
        raise AttributeError("scipy.misc is deprecated and has no attribute " f"{name}.")

    if name in dataset_methods:
        msg = (
            "The module `scipy.misc` is deprecated and will be "
        ...
        msg = (
            f"The method `{name}` from the `scipy.misc` namespace is"
