...
class VisitingDistribution:
    ...
    def __init__(self, lb, ub, visiting_param, rand_gen):
        ...
        self._factor6 = (
            np.pi * (1.0 - self._factor5) / np.sin(np.pi * (1.0 - self._factor5)) / np.exp(gammaln(self._d1))
        )
...
class StrategyChain:
    ...
    def accept_reject(self, j, e, x_visit):
        ...
        pqv_temp = 1.0 - (
            (1.0 - self.acceptance_param) * (e - self.energy_state.current_energy) / self.temperature_step
        )
        if pqv_temp <= 0.0:
            pqv = 0.0
        else:
