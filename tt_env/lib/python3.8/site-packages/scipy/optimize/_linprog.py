...
def linprog_verbose_callback(res):
    ...
    np.set_printoptions(linewidth=500, formatter={"float": lambda x: "{0: 12.4f}".format(x)})
    if status:
...
def linprog(
    ...
    **m**\ ethod [13]_; it features a crossover routine, so it is as accurate
    ...
    if meth.startswith("highs"):
        ...
        sol["status"], sol["message"] = _check_result(
            sol["x"], sol["fun"], sol["status"], sol["slack"], sol["con"], lp.bounds, tol, sol["message"]
        )
        sol["success"] = sol["status"] == 0
        return OptimizeResult(sol)

    warn(
        f"`method='{meth}'` is deprecated and will be removed in SciPy "
    ...
    if not complete:
        ...
        if meth == "simplex":
            x, status, message, iteration = _linprog_simplex(
                c, c0=c0, A=A, b=b, callback=callback, postsolve_args=postsolve_args, **solver_options
            )
            ...
            x, status, message, iteration = _linprog_rs(
                c, c0=c0, A=A, b=b, x0=x0, callback=callback, postsolve_args=postsolve_args, **solver_options
            )
