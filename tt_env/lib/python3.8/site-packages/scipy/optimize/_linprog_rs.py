...
def _phase_one(A, b, x0, callback, postsolve_args, maxiter, tol, disp, maxupdate, mast, pivot):
    ...
    x, basis, status, iter_k = _phase_two(
        c, A, x, basis, callback, postsolve_args, maxiter, tol, disp, maxupdate, mast, pivot, iter_k, phase_one_n
    )
...
def _generate_auxiliary_problem(A, b, x0, tol):
    ...
    elif len(nonzero_constraints) > m - len(basis) or np.any(x < 0):  # can't get trivial BFS
        c = np.zeros(n)
        status = 6
        return A, b, c, basis, x, status
...
def _phase_two(
    c, A, x, b, callback, postsolve_args, maxiter, tol, disp, maxupdate, mast, pivot, iteration=0, phase_one_n=None
):
    """
    ...
    **unknown_options,
    ...
    if status == 0:
        x, basis, status, iteration = _phase_two(
            c, A, x, basis, callback, postsolve_args, maxiter, tol, disp, maxupdate, mast, pivot, iteration
        )
