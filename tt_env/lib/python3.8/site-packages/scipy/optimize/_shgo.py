...
def shgo(
    ...
    if len(shc.LMC.xl_maps) == 0:
        ...
        shc.fail_routine(
            mes="Failed to find a feasible minimizer point. " "Lowest sampling point = {}".format(shc.f_lowest)
        )
...
class SHGO:
    def __init__(
        ...
        if (
            ...
            and (minimizer_kwargs is not None and "constraints" not in minimizer_kwargs and constraints is not None)
        ...
        if sampling_method == "simplicial":
            ...
            if sampling_method in ["halton", "sobol"]:
                if sampling_method == "sobol":
                    ...
                    self.qmc_engine = qmc.Sobol(d=self.dim, scramble=False, seed=np.random.RandomState())
                else:
                    self.sampling_method = "halton"
                    self.qmc_engine = qmc.Halton(d=self.dim, scramble=True, seed=np.random.RandomState())
    ...
    def iterate_hypercube(self):
        ...
        if self.n_sampled == 0:
            ...
            # account.
            self.HC = Complex(self.dim, self._raw_func, self.args, self.symmetry, self.bounds, self.g_cons, self.g_args)
        else:
    ...
    def sampling_subspace(self):
        ...
        for ind, g in enumerate(self.g_cons):
            ...
            if self.C.size == 0:
                self.res.message = (
                    "No sampling point found within the " + "feasible set. Increasing sampling " + "size."
                )
