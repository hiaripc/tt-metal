...
class Complex:
    def __init__(self, dim, func, func_args=(), symmetry=False, bounds=None, g_cons=None, g_args=()):
        self.dim = dim
        self.bounds = bounds
                    ...
                    for v in c():
                        if self.bounds is None:
                            ...
                            for i in range(len(self.bounds)):
                                x_a[i] = x_a[i] * (self.bounds[i][1] - self.bounds[i][0]) + self.bounds[i][0]
                        ...
                        for vn in v.nn:
                            if self.bounds is None:
                                ...
                                xn_a = np.array(vn.x, dtype=float)
                                for i in range(len(self.bounds)):
                                    xn_a[i] = xn_a[i] * (self.bounds[i][1] - self.bounds[i][0]) + self.bounds[i][0]
...
class VertexCache:
    def __init__(self, func, func_args=(), bounds=None, g_cons=None, g_cons_args=(), indexed=True):
        self.cache = {}
        self.func = func
        self.g_cons = g_cons
