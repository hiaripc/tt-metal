...
class TestProjectCG(TestCase):
    ...
    def test_active_box_constraints_hits_boundaries(self):
        ...
        Z, _, Y = projections(A)
        x, info = projected_cg(
            H, c, Z, Y, b, tol=0, ub=[np.inf, np.inf, 1.6, np.inf], trust_radius=trust_radius, return_all=True
        )
        assert_equal(info["stop_cond"], 2)
        assert_equal(info["hits_boundary"], True)
    ...
    def test_active_box_constraints_hits_boundaries_infeasible_iter(self):
        ...
        Z, _, Y = projections(A)
        x, info = projected_cg(
            H, c, Z, Y, b, tol=0, ub=[np.inf, 0.1, np.inf, np.inf], trust_radius=trust_radius, return_all=True
        )
        assert_equal(info["stop_cond"], 2)
        assert_equal(info["hits_boundary"], True)
