...
class TestBasinHopping:
    ...
    def test_jac(self):
        ...
        res = basinhopping(
            func2d_easyderiv, [0.0, 0.0], minimizer_kwargs=minimizer_kwargs, niter=self.niter, disp=self.disp
        )
    ...
    def test_all_nograd_minimizers(self):
        ...
        for method in methods:
            minimizer_kwargs["method"] = method
            res = basinhopping(
                func2d_nograd, self.x0[i], minimizer_kwargs=minimizer_kwargs, niter=self.niter, disp=self.disp
            )
            tol = self.tol
            if method == "COBYLA":
                tol = 2
    ...
    def test_pass_takestep(self):
        ...
        i = 1
        res = basinhopping(
            func2d, self.x0[i], minimizer_kwargs=self.kwargs, niter=self.niter, disp=self.disp, take_step=takestep
        )
        assert_almost_equal(res.x, self.sol[i], self.tol)
    ...
    def test_pass_accept_test(self):
        ...
        basinhopping(
            func2d, self.x0[i], minimizer_kwargs=self.kwargs, niter=10, disp=self.disp, accept_test=accept_test
        )
    ...
    def test_pass_callback(self):
        ...
        # there's no point in running it more than a few steps.
        res = basinhopping(
            func2d, self.x0[i], minimizer_kwargs=self.kwargs, niter=30, disp=self.disp, callback=callback
        )
