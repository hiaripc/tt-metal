...
class TestNewToOld:
    def test_multiple_constraint_objects(self):
        ...
        # mixed old and new
        coni.append(
            [{"type": "ineq", "fun": lambda x: x[0] - 2 * x[1] + 2}, NonlinearConstraint(lambda x: x[0] - x[1], -1, 1)]
        )
    ...
    def test_individual_constraint_objects(self):
        ...
        # multiple equalities
        cone.append(NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], 1.21, 1.21))  # two same equalities
        cone.append(
            NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], [1.21, 1.4], [1.21, 1.4])
        )  # two different equalities
        cone.append(
            NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], [1.21, 1.21], 1.21)
        )  # equality specified two ways
        cone.append(
            NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], [1.21, -np.inf], [1.21, np.inf])
        )  # equality + unbounded
        ...
        coni.append(NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], 1.21, np.inf))  # two same inequalities
        cone.append(
            NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], [1.21, -np.inf], [1.21, 1.4])
        )  # mixed equality/inequality
        coni.append(
            NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], [1.1, 0.8], [1.2, 1.4])
        )  # bounded above and below
        coni.append(
            NonlinearConstraint(lambda x: [x[0] - x[1], x[1] - x[2]], [-1.2, -1.4], [-1.1, -0.8])
        )  # - bounded above and below
