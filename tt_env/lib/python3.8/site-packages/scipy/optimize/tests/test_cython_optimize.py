...
def test_bisect():
    npt.assert_allclose(
        EXPECTED, list(_zeros.loop_example("bisect", A0, ARGS, XLO, XHI, XTOL, RTOL, MITR)), rtol=RTOL, atol=XTOL
    )
...
def test_ridder():
    npt.assert_allclose(
        EXPECTED, list(_zeros.loop_example("ridder", A0, ARGS, XLO, XHI, XTOL, RTOL, MITR)), rtol=RTOL, atol=XTOL
    )
...
def test_brenth():
    npt.assert_allclose(
        EXPECTED, list(_zeros.loop_example("brenth", A0, ARGS, XLO, XHI, XTOL, RTOL, MITR)), rtol=RTOL, atol=XTOL
    )
...
def test_brentq():
    npt.assert_allclose(
        EXPECTED, list(_zeros.loop_example("brentq", A0, ARGS, XLO, XHI, XTOL, RTOL, MITR)), rtol=RTOL, atol=XTOL
    )
