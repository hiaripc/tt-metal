...
class TestDIRECT:
    def setup_method(self):
        ...
        self.bounds_stylinski_tang = Bounds([-4.0, -4.0], [4.0, 4.0])
        self.maxiter = 1000
    ...
    def test_direct_callback(self, locally_biased):
        ...
        # test accuracy
        assert_allclose(res_callback.x, self.optimum_sphere_pos, rtol=1e-3, atol=1e-3)
        assert_allclose(res_callback.fun, self.optimum_sphere, atol=1e-5, rtol=1e-5)
    ...
    def test_len_tol(self, len_tol, locally_biased):
        ...
        message = (
            "The side length measure of the hyperrectangle containing "
    ...
    def test_vol_tol(self, vol_tol, locally_biased):
        ...
        assert_allclose(res.x, np.zeros((4,)))
        message = (
            "The volume of the hyperrectangle containing the lowest " f"function value found is below vol_tol={vol_tol}"
        )
        assert res.message == message
    ...
    def test_f_min(self, f_min_rtol, locally_biased):
        ...
        message = (
            "The best function value found is within a relative "
    ...
    def test_f_circle_with_args(self, locally_biased):
        ...
        assert_allclose(res.x, np.array([1.0, 1.0]), rtol=1e-5)
    ...
    def test_failure_maxfun(self, locally_biased):
        ...
        message = "Number of function evaluations done is " f"larger than maxfun={maxfun}"
    ...
    def test_failure_maxiter(self, locally_biased):
        ...
        maxiter = 10
        result = direct(
            self.styblinski_tang, self.bounds_stylinski_tang, maxiter=maxiter, locally_biased=locally_biased
        )
        assert result.success is False
    ...
    def test_bounds_variants(self, locally_biased):
        ...
        res_old_bounds = direct(self.sphere, bounds_old, locally_biased=locally_biased)
        res_new_bounds = direct(self.sphere, bounds_new, locally_biased=locally_biased)
    ...
    def test_epsilon(self, eps, locally_biased):
        result = direct(
            self.styblinski_tang, self.bounds_stylinski_tang, eps=eps, vol_tol=1e-6, locally_biased=locally_biased
        )
    ...
    @pytest.mark.parametrize("locally_biased", ["bias", [0, 0], 2.0])
    def test_locally_biased_validation(self, locally_biased):
        error_msg = "locally_biased must be True or False."
        with pytest.raises(ValueError, match=error_msg):
            direct(self.styblinski_tang, self.bounds_stylinski_tang, locally_biased=locally_biased)
