...
class BaseMixin:
    ...
    def test_x_scale_options(self):
        ...
        assert_raises(ValueError, least_squares, fun_trivial, 2.0, x_scale=1.0 + 2.0j, method=self.method)
    ...
    def test_rosenbrock(self):
        ...
        for jac, x_scale, tr_solver in product(
            ["2-point", "3-point", "cs", jac_rosenbrock], [1.0, np.array([1.0, 0.2]), "jac"], ["exact", "lsmr"]
        ):
            with suppress_warnings() as sup:
                sup.filter(UserWarning, "jac='(3-point|cs)' works equivalently to '2-point' for method='lm'")
    ...
    def test_rosenbrock_cropped(self):
        ...
        if self.method == "lm":
            ...
            for jac, x_scale, tr_solver in product(
                ...
                res = least_squares(
                    fun_rosenbrock_cropped, x0, jac, x_scale=x_scale, tr_solver=tr_solver, method=self.method
                )
    ...
    def test_convergence_with_only_one_tolerance_enabled(self):
        ...
        for ftol, xtol, gtol in [(1e-8, None, None), (None, 1e-8, None), (None, None, 1e-8)]:
            res = least_squares(
                fun_rosenbrock, x0, jac=jac_rosenbrock, ftol=ftol, gtol=gtol, xtol=xtol, method=self.method
            )
            assert_allclose(res.x, x_opt)
...
class BoundsMixin:
    ...
    def test_inconsistent_shape(self):
        ...
        assert_raises(
            ValueError, least_squares, fun_rosenbrock, [1.0, 2.0], bounds=([0.0], [3.0, 4.0]), method=self.method
        )
    ...
    def test_bounds_shape(self):
        ...
        for jac in ["2-point", "3-point", "cs", jac_2d_trivial]:
            for bounds_func in [get_bounds_direct, get_bounds_instances]:
                ...
                res = least_squares(
                    fun_2d_trivial, x0, jac=jac, bounds=bounds_func(0.5, [2.0, 2.0]), method=self.method
                )
                assert_allclose(res.x, [0.5, 0.5])
                res = least_squares(
                    fun_2d_trivial, x0, jac=jac, bounds=bounds_func([0.3, 0.2], 3.0), method=self.method
                )
                assert_allclose(res.x, [0.3, 0.2])
                res = least_squares(
                    fun_2d_trivial, x0, jac=jac, bounds=bounds_func([-1, 0.5], [1.0, 3.0]), method=self.method
                )
    ...
    def test_rosenbrock_bounds(self):
        ...
        for x0, bounds in problems:
            for jac, x_scale, tr_solver in product(
                ["2-point", "3-point", "cs", jac_rosenbrock], [1.0, [1.0, 0.5], "jac"], ["exact", "lsmr"]
            ):
                res = least_squares(
                    fun_rosenbrock, x0, jac, bounds, x_scale=x_scale, tr_solver=tr_solver, method=self.method
                )
...
class SparseMixin:
    def test_exact_tr_solver(self):
        ...
        assert_raises(
            ValueError, least_squares, p.fun, p.x0, tr_solver="exact", jac_sparsity=p.sparsity, method=self.method
        )
    ...
    def test_with_bounds(self):
        ...
        for jac, jac_sparsity in product([p.jac, "2-point", "3-point", "cs"], [None, p.sparsity]):
            res_1 = least_squares(
                p.fun, p.x0, jac, bounds=(p.lb, np.inf), method=self.method, jac_sparsity=jac_sparsity
            )
            res_2 = least_squares(
                p.fun, p.x0, jac, bounds=(-np.inf, p.ub), method=self.method, jac_sparsity=jac_sparsity
            )
