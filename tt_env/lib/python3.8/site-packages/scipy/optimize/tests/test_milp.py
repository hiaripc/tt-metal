...
def test_milp_2():
    ...
    res6 = milp(
        c=c,
        integrality=True,
        constraints=[LinearConstraint(A[:1], b_l[:1], b_u[:1]), LinearConstraint(A[1:], b_l[1:], b_u[1:])],
    )
    res7 = milp(
        c=c, integrality=True, constraints=[(A[:1], b_l[:1], b_u[:1]), LinearConstraint(A[1:], b_l[1:], b_u[1:])]
    )
...
@pytest.mark.parametrize(["options", "msg"], [({"time_limit": 10}, _msg_time), ({"node_limit": 1}, _msg_iter)])
