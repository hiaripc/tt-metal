...
from scipy.optimize import NonlinearConstraint, LinearConstraint, Bounds, minimize, BFGS, SR1
...
class TestTrustRegionConstr(TestCase):
    ...
    def test_issue_9044(self):
        ...
        result = minimize(
            lambda x: x**2, [0], jac=lambda x: 2 * x, hess=lambda x: 2, callback=callback, method="trust-constr"
        )
...
class TestBoundedNelderMead:
    ...
    def test_invalid_bounds(self):
        prob = Rosenbrock()
        with raises(ValueError, match=r"one of the lower bounds is greater " r"than an upper bound."):
    ...
    def test_outside_bounds_warning(self):
        prob = Rosenbrock()
        with raises(UserWarning, match=r"Initial guess is not within " r"the specified bounds"):
            bounds = Bounds([-np.inf, 1.0], [4.0, 5.0])
