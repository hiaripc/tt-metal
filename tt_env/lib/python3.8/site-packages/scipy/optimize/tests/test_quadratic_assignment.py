...
class Test2opt(QAPCommonTests):
    ...
    def test_partial_guess(self):
        ...
        res3 = quadratic_assignment(
            A, B, method=self.method, options={"rng": 0, "partial_guess": guess, "partial_match": match}
        )
    ...
    def test_specific_input_validation(self):
        ...
        with pytest.raises(ValueError, match="`partial_guess` can have only as many entries as"):
            quadratic_assignment(
                np.identity(3), np.identity(3), method=self.method, options={"partial_guess": _rm(5, 2)}
            )
        # test for only two seed columns
        with pytest.raises(ValueError, match="`partial_guess` must have two columns"):
            quadratic_assignment(
                np.identity(3), np.identity(3), method=self.method, options={"partial_guess": _range_matrix(2, 3)}
            )
        # test that seed has no more than two dimensions
        with pytest.raises(ValueError, match="`partial_guess` must have exactly two"):
            quadratic_assignment(
                np.identity(3), np.identity(3), method=self.method, options={"partial_guess": np.random.rand(3, 2, 2)}
            )
        # seeds cannot be negative valued
        with pytest.raises(ValueError, match="`partial_guess` must contain only pos"):
            quadratic_assignment(
                np.identity(3), np.identity(3), method=self.method, options={"partial_guess": -1 * _range_matrix(2, 2)}
            )
        ...
        with pytest.raises(ValueError, match="`partial_guess` entries must be less than number"):
            quadratic_assignment(
                np.identity(5), np.identity(5), method=self.method, options={"partial_guess": 2 * _range_matrix(4, 2)}
            )
        # columns of seed matrix must be unique
        with pytest.raises(ValueError, match="`partial_guess` column entries must be unique"):
            quadratic_assignment(
                np.identity(3), np.identity(3), method=self.method, options={"partial_guess": np.ones((2, 2))}
            )
