...
class TestSLSQP:
    ...
    def test_minimize_equality_given2(self):
        ...
        res = minimize(
            ...
            jac=self.jac,
            args=(-1.0,),
            constraints={"type": "eq", "fun": self.f_eqcon, "args": (-1.0,), "jac": self.fprime_eqcon},
            options=self.opts,
        )
        assert_(res["success"], res["message"])
        assert_allclose(res.x, [1, 1])
    ...
    def test_minimize_equality_given_cons_scalar(self):
        ...
        res = minimize(
            ...
            jac=self.jac,
            args=(-1.0,),
            constraints={"type": "eq", "fun": self.f_eqcon_scalar, "args": (-1.0,), "jac": self.fprime_eqcon_scalar},
            options=self.opts,
        )
        assert_(res["success"], res["message"])
        assert_allclose(res.x, [1, 1])
    ...
    def test_equality_given(self):
        ...
        res = fmin_slsqp(
            self.fun, [-1.0, 1.0], fprime=self.jac, args=(-1.0,), eqcons=[self.f_eqcon], iprint=0, full_output=1
        )
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
        assert_array_almost_equal(x, [1, 1])
    ...
    def test_inequality_given(self):
        ...
        res = fmin_slsqp(
            self.fun, [-1.0, 1.0], fprime=self.jac, args=(-1.0,), ieqcons=[self.f_ieqcon], iprint=0, full_output=1
        )
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
    ...
    def test_regression_5743(self):
        ...
        sol = minimize(
            lambda x: x[0] ** 2 + x[1] ** 2,
            x,
            constraints=({"type": "eq", "fun": lambda x: x[0] + x[1] - 1}, {"type": "ineq", "fun": lambda x: x[0] - 2}),
            bounds=((0, None), (0, None)),
            method="SLSQP",
        )
    ...
    def test_gh9640(self):
        ...
        res = minimize(
            target, x0, method="SLSQP", bounds=bnds, constraints=cons, options={"disp": False, "maxiter": 10000}
        )
