...
class TestTnc:
    ...
    def f38(self, x):
        return (
            100.0 * pow(x[1] - pow(x[0], 2), 2)
            + pow(1.0 - x[0], 2)
            + 90.0 * pow(x[3] - pow(x[2], 2), 2)
            + pow(1.0 - x[2], 2)
    ...
    def test_minimize_tnc1(self):
        ...
        res = optimize.minimize(
            self.f1, x0, method="TNC", jac=self.g1, bounds=bnds, options=self.opts, callback=iterx.append
        )
    ...
    def test_tnc1(self):
        ...
        assert_allclose(
            self.f1(x), self.f1(xopt), atol=1e-8, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_tnc1b(self):
        ...
        x, nf, rc = optimize.fmin_tnc(
            self.f1, x, approx_grad=True, bounds=bounds, messages=optimize._tnc.MSG_NONE, maxfun=200
        )

        assert_allclose(
            self.f1(x), self.f1(xopt), atol=1e-4, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_tnc1c(self):
        ...
        x, nf, rc = optimize.fmin_tnc(
            self.f1, x, fprime=self.g1, bounds=bounds, messages=optimize._tnc.MSG_NONE, maxfun=200
        )
    ...
    def test_tnc3(self):
        ...
        assert_allclose(
            self.f3(x), self.f3(xopt), atol=1e-8, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_tnc4(self):
        ...
        assert_allclose(
            self.f4(x), self.f4(xopt), atol=1e-8, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_tnc5(self):
        ...
        assert_allclose(
            self.f5(x), self.f5(xopt), atol=1e-8, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_tnc38(self):
        ...
        assert_allclose(
            self.f38(x), self.f38(xopt), atol=1e-8, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_tnc45(self):
        ...
        assert_allclose(
            self.f45(x), self.f45(xopt), atol=1e-8, err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc]
        )
    ...
    def test_maxiter_depreciations(self):
        ...
        with pytest.warns(DeprecationWarning, match=msg):
            optimize.minimize(self.f1, [1, 3], method="TNC", options={"maxiter": 1})
