...
class TestIterativeSubproblem:
    def test_for_the_easy_case(self):
        ...
        subprob = IterativeSubproblem(
            x=0, fun=lambda x: 0, jac=lambda x: np.array(g), hess=lambda x: np.array(H), k_easy=1e-10, k_hard=1e-10
        )
            ...
            for min_eig, max_eig in eig_limits:
                ...
                for trust_radius in trust_radius_list:
                    ...
                    subprob_ac = IterativeSubproblem(
                        0, lambda x: 0, lambda x: g, lambda x: H, k_easy=1e-10, k_hard=1e-10
                    )
                    ...
                    for k_opt, k_trf in stop_criteria:
                        ...
                        # Solve subproblem
                        subprob = IterativeSubproblem(
                            0, lambda x: 0, lambda x: g, lambda x: H, k_easy=k_easy, k_hard=k_hard
                        )
                        p, hits_boundary = subprob.solve(trust_radius)
