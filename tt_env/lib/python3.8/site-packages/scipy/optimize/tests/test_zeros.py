...
from numpy.testing import assert_warns, assert_, assert_allclose, assert_equal, assert_array_equal, suppress_warnings
import numpy as np
...
class TestBasic:
    def run_check_by_name(self, name, smoothness=0, **kwargs):
        ...
        for function, fname in zip(tstutils_functions, tstutils_fstrings):
            ...
            zero = r.root
            assert_(r.converged)
            assert_allclose(zero, 1.0, atol=xtol, rtol=rtol, err_msg="method %s, function %s" % (name, fname))
    ...
    def run_check(self, method, name):
        ...
        rtol = 4 * _FLOAT_EPS
        for function, fname in zip(tstutils_functions, tstutils_fstrings):
            zero, r = method(function, a, b, xtol=xtol, rtol=rtol, full_output=True)
            assert_(r.converged)
            assert_allclose(zero, 1.0, atol=xtol, rtol=rtol, err_msg="method %s, function %s" % (name, fname))
    ...
    def run_check_lru_cached(self, method, name):
        ...
        assert_(r.converged)
        assert_allclose(zero, 0, err_msg="method %s, function %s" % (name, "f_lrucached"))
    ...
    def run_tests(self, tests, method, name, xtol=4 * _FLOAT_EPS, rtol=4 * _FLOAT_EPS, known_fail=None, **kwargs):
        ...
        results = [
            list(self._run_one_test(tc, method, sig_args_keys=sig_args_keys, sig_kwargs_keys=sig_kwargs_keys, **kwargs))
            for tc in tests
        ]
    ...
    def run_collection(
        ...
        **kwargs,
        ...
        self.run_tests(tests, method, name, xtol=xtol, rtol=rtol, known_fail=known_fail, **kwargs)
    ...
    def test_brentq(self):
        ...
        self.run_collection("aps", zeros.brentq, "brentq", smoothness=1, xtol=1e-14, rtol=1e-14)

    def test_brenth(self):
        self.run_check(zeros.brenth, "brenth")
        self.run_check_lru_cached(zeros.brenth, "brenth")
        self.run_check_by_name("brenth")
        self.run_collection("aps", zeros.brenth, "brenth", smoothness=1, xtol=1e-14, rtol=1e-14)
...
def test_array_newton_failures():
    ...
    with pytest.warns(RuntimeWarning):
        result = zeros.newton(
            colebrook_eqn, x0=[0.01, 0.2, 0.02223, 0.3], maxiter=2, args=[reynolds_number, diameter], full_output=True
        )
        assert not result.converged.all()
    # they all fail
    with pytest.raises(RuntimeError):
        result = zeros.newton(
            colebrook_eqn, x0=[0.01] * 2, maxiter=2, args=[reynolds_number, diameter], full_output=True
        )
...
def test_gh9551_raise_error_if_disp_true():
    ...
    with pytest.raises(
        RuntimeError, match=r"^Derivative was zero\. Failed to converge after \d+ iterations, value is [+-]?\d*\.\d+\.$"
    ):
