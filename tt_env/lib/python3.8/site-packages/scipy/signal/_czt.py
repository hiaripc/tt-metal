...
def _validate_sizes(n, m):
    if n < 1 or not isinstance(n, numbers.Integral):
        raise ValueError(
            "Invalid number of CZT data " f"points ({n}) specified. " "n must be positive and integer type."
        )
    ...
    if m is None:
        ...
        raise ValueError(
            "Invalid number of CZT output " f"points ({m}) specified. " "m must be positive and integer type."
        )
...
class CZT:
    ...
    def __call__(self, x, *, axis=-1):
        ...
        if x.shape[axis] != self.n:
            raise ValueError(f"CZT defined for length {self.n}, not " f"{x.shape[axis]}")
