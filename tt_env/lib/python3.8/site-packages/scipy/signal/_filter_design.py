...
def findfreqs(num, den, N, kind="ba"):
    ...
    ez = r_[
        "-1", numpy.compress(ep.imag >= 0, ep, axis=-1), numpy.compress((abs(tz) < 1e5) & (tz.imag >= 0), tz, axis=-1)
    ]
...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2 * pi, include_nyquist=False):
    ...
    if _is_int_type(worN):
        ...
        if a.size == 1 and N >= b.shape[0] and sp_fft.next_fast_len(N) == N and (b.ndim == 1 or (b.shape[-1] == 1)):
...
def group_delay(system, w=512, whole=False, fs=2 * pi):
    ...
    if np.any(singular):
        ...
        warnings.warn(
            ...
            around which a singularity may be present".format(
                ", ".join("{0:.3f}".format(ws) for ws in w[near_singular])
            ),
            stacklevel=2,
        )
...
def zpk2tf(z, p, k):
    ...
    if issubclass(b.dtype.type, numpy.complexfloating):
        ...
        if len(pos_roots) == len(neg_roots):
            if numpy.all(numpy.sort_complex(neg_roots) == numpy.sort_complex(pos_roots)):
                b = b.real.copy()
...
def zpk2sos(z, p, k, pairing=None, *, analog=False):
    ...
    *Algorithms*
    ...
    for si in range(n_sections - 1, -1, -1):
        ...
        elif len(p) + 1 == len(z) and not np.isreal(p1) and np.isreal(p).sum() == 1 and np.isreal(z).sum() == 1:
...
def normalize(b, a):
    ...
    if leading_zeros > 0:
        warnings.warn(
            "Badly conditioned filter coefficients (numerator): the " "results may be meaningless", BadCoefficients
        )
...
def iirdesign(wp, ws, gpass, gstop, analog=False, ftype="ellip", output="ba", fs=None):
    ...
    if wp.shape[0] != ws.shape[0] or wp.shape not in [(1,), (2,)]:
        raise ValueError(
            "wp and ws must have one or two elements each, and" "the same shape, got %s and %s" % (wp.shape, ws.shape)
        )
...
def iirfilter(N, Wn, rp=None, rs=None, btype="band", analog=False, ftype="butter", output="ba", fs=None):
    ...
    if not analog:
        if numpy.any(Wn <= 0) or numpy.any(Wn >= 1):
            if fs is not None:
                raise ValueError(
                    "Digital filter critical frequencies must " f"be 0 < Wn < fs/2 (fs={fs} -> fs/2={fs/2})"
                )
...
def lp2bp_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    p_bp = concatenate((p_lp + sqrt(p_lp**2 - wo**2), p_lp - sqrt(p_lp**2 - wo**2)))
...
def lp2bs_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    # Duplicate poles and zeros and shift from baseband to +wo and -wo
    z_bs = concatenate((z_hp + sqrt(z_hp**2 - wo**2), z_hp - sqrt(z_hp**2 - wo**2)))
    p_bs = concatenate((p_hp + sqrt(p_hp**2 - wo**2), p_hp - sqrt(p_hp**2 - wo**2)))
...
def buttord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    if filter_type == 1:  # low
        ...
        wp0 = optimize.fminbound(
            band_stop_obj, passb[0], stopb[0] - 1e-12, args=(0, passb, stopb, gpass, gstop, "butter"), disp=0
        )
        passb[0] = wp0
        wp1 = optimize.fminbound(
            band_stop_obj, stopb[1] + 1e-12, passb[1], args=(1, passb, stopb, gpass, gstop, "butter"), disp=0
        )
        passb[1] = wp1
...
def cheb1ord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    if filter_type == 1:  # low
        ...
        wp0 = optimize.fminbound(
            band_stop_obj, passb[0], stopb[0] - 1e-12, args=(0, passb, stopb, gpass, gstop, "cheby"), disp=0
        )
        passb[0] = wp0
        wp1 = optimize.fminbound(
            band_stop_obj, stopb[1] + 1e-12, passb[1], args=(1, passb, stopb, gpass, gstop, "cheby"), disp=0
        )
        passb[1] = wp1
        nat = (stopb * (passb[0] - passb[1])) / (stopb**2 - passb[0] * passb[1])
    elif filter_type == 4:  # pass
...
def ellipord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    if filter_type == 1:  # low
        ...
        wp0 = optimize.fminbound(
            band_stop_obj, passb[0], stopb[0] - 1e-12, args=(0, passb, stopb, gpass, gstop, "ellip"), disp=0
        )
        passb[0] = wp0
        wp1 = optimize.fminbound(
            band_stop_obj, stopb[1] + 1e-12, passb[1], args=(1, passb, stopb, gpass, gstop, "ellip"), disp=0
        )
        passb[1] = wp1
        nat = (stopb * (passb[0] - passb[1])) / (stopb**2 - passb[0] * passb[1])
    elif filter_type == 4:  # pass
...
def ellipap(N, rp, rs):
    ...
    if N % 2:
        newp = numpy.compress(
            abs(p.imag) > EPSILON * numpy.sqrt(numpy.sum(p * numpy.conjugate(p), axis=0).real), p, axis=-1
        )
...
bessel_norms = {"bessel": "phase", "bessel_phase": "phase", "bessel_delay": "delay", "bessel_mag": "mag"}
