    ...
    def __new__(cls, *system, **kwargs):
        ...
        if cls is LinearTimeInvariant:
            raise NotImplementedError(
                "The LinearTimeInvariant class is not " "meant to be used directly, use `lti` " "or `dlti` instead."
            )
    ...
    *system : arguments
    ...
    def to_discrete(self, dt, method="zoh", alpha=None):
        ...
        return ZerosPolesGain(
            *cont2discrete((self.zeros, self.poles, self.gain), dt, method=method, alpha=alpha)[:-1], dt=dt
        )
...
class StateSpace(LinearTimeInvariant):
    ...
    *system: arguments
    ...
    def _check_binop_other(self, other):
        return isinstance(other, (StateSpace, np.ndarray, float, complex, np.number, int))
    ...
    def __add__(self, other):
        ...
        if isinstance(other, StateSpace):
            ...
            if self.D.shape == other.shape:
                ...
                raise ValueError(
                    "Cannot add systems with incompatible " "dimensions ({} and {})".format(self.D.shape, other.shape)
                )
...
def lsim(system, U, T, X0=None, interp=True):
    ...
    if not np.allclose((T[1:] - T[:-1]) / dt, 1.0):
        warnings.warn(
            "Non-uniform timesteps are deprecated. Results may be " "slow and/or inaccurate.", DeprecationWarning
        )
    ...
    if not interp:
        ...
        M = np.vstack(
            [
                ...
                np.hstack([np.zeros((n_inputs, n_states + n_inputs)), np.identity(n_inputs)]),
...
def _YT_real(ker_pole, Q, transfer_matrix, i, j):
    ...
    transfer_matrix_ij = np.dot(np.dot(ker_pole_mu_nu, ker_pole_mu_nu.T), transfer_matrix_j_mo_transfer_matrix_j)
    if not np.allclose(transfer_matrix_ij, 0):
        transfer_matrix_ij = np.sqrt(2) * transfer_matrix_ij / np.linalg.norm(transfer_matrix_ij)
...
def _YT_complex(ker_pole, Q, transfer_matrix, i, j):
    ...
    m = np.dot(np.dot(np.conj(ker_pole_ij.T), np.dot(u, np.conj(u).T) - np.dot(np.conj(u), u.T)), ker_pole_ij)
    ...
    transfer_matrix_j_mo_transfer_matrix_j = transfer_matrix[:, i, np.newaxis] + 1j * transfer_matrix[:, j, np.newaxis]
    ...
    transfer_matrix_i_j = np.dot(np.dot(ker_pole_mu, np.conj(ker_pole_mu.T)), transfer_matrix_j_mo_transfer_matrix_j)
...
def dlsim(system, u, t=None, x0=None):
    ...
    # Last point
    yout[out_samples - 1, :] = np.dot(system.C, xout[out_samples - 1, :]) + np.dot(system.D, u_dt[out_samples - 1, :])
