...
def _valfrommode(mode):
    try:
        ...
        raise ValueError("Acceptable mode flags are 'valid'," " 'same', or 'full'.") from e
...
def _bvalfromboundary(boundary):
    try:
        ...
        raise ValueError(
            "Acceptable boundary flags are 'fill', 'circular' " "(or 'wrap'), and 'symmetric' (or 'symm')."
        ) from e
...
def _conv_ops(x_shape, h_shape, mode):
    ...
    if mode == "full":
        ...
        raise ValueError("Acceptable mode flags are 'valid'," " 'same', or 'full', not mode={}".format(mode))
    ...
    constants = (
        {
            ...
            "same": (3.2646654e-9, 2.8478277e-10, offset) if h.size <= x.size else (3.21635404e-9, 1.1773253e-8, -1e-5),
        }
        if x.ndim == 1
        else {
            ...
            if zi.shape != expected_shape:
                ...
                for k in range(zi.ndim):
                    if k == axis and zi.shape[k] == expected_shape[k]:
                        ...
                        raise ValueError(
                            "Unexpected shape for zi: expected " "%s, found %s." % (expected_shape, zi.shape)
                        )
...
def residue(b, a, tol=1e-3, rtype="avg"):
    ...
    b = np.asarray(b)
    a = np.asarray(a)
    if np.issubdtype(b.dtype, np.complexfloating) or np.issubdtype(a.dtype, np.complexfloating):
        b = b.astype(complex)
        a = a.astype(complex)
    else:
        b = b.astype(float)
...
def resample_poly(x, up, down, axis=0, window=("kaiser", 5.0), padtype="constant", cval=None):
    ...
    funcs = {"mean": np.mean, "median": np.median, "minimum": np.amin, "maximum": np.amax}
    ...
    if zi is not None:
        ...
        if zi.shape != x_zi_shape:
            raise ValueError(
                ...
                "must have shape %r, got %r." % (axis, x.shape, n_sections, x_zi_shape, zi.shape)
            )
        return_zi = True
    else:
