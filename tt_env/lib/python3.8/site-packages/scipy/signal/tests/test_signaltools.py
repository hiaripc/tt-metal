...
class _TestConvolve2d:
    ...
    def test_valid_mode_complx(self):
        ...
        h = array([[62.0 + 24.0j, 80.0 + 30.0j, 98.0 + 36.0j, 116.0 + 42.0j, 134.0 + 48.0j]])
    ...
    @pytest.mark.parametrize(
        "boundary, expected", [("symm", [[37.0, 42.0, 44.0, 45.0]]), ("wrap", [[43.0, 44.0, 42.0, 39.0]])]
    )
...
class TestFFTConvolve:
    ...
    def test_valid_mode_complex(self, axes):
        ...
        expected = array([45.0 + 12.0j, 30.0 + 23.0j, 48 + 32.0j])
    ...
    @pytest.mark.parametrize(
        "n",
        list(range(1, 100)) + list(range(1000, 1500)) + np.random.RandomState(1234).randint(1001, 10000, 5).tolist(),
    )
...
class TestAllFreqConvolves:
    ...
    def test_invalid_shapes(self, convapproach):
        ...
        b = np.arange(-6, 0).reshape((3, 2))
        with assert_raises(
            ValueError, match="For 'valid' mode, one must be at least " "as large as the other in every dimension"
        ):
    ...
    def test_invalid_shapes_axes(self, convapproach):
        ...
        with assert_raises(
            ValueError,
            match=r"incompatible shapes for in1 and in2:" r" \(5L?, 6L?, 2L?, 1L?\) and" r" \(5L?, 6L?, 3L?, 1L?\)",
        ):
...
class TestResample:
    ...
    def test_input_domain(self):
        ...
        fsig = fft(tsig)
        num = 256
        assert_allclose(signal.resample(fsig, num, domain="freq"), signal.resample(tsig, num, domain="time"), atol=1e-9)
    ...
    def test_resample_methods(self, method, ext, padtype):
        ...
        if method == "fft":
            ...
            y1_true = np.array([1.0 + 0.0j, 0.5 + 0.0j, 0.0 + 0.0j, 0.5 + 0.0j])
...
class TestOrderFilt:
    def test_basic(self):
        assert_array_equal(signal.order_filter([1, 2, 3], [1, 0, 1], 1), [2, 3, 2])
...
class TestCorrelateComplex:
    ...
    def test_swap_full(self, dt):
        d = np.array([0.0 + 0.0j, 1.0 + 1.0j, 2.0 + 2.0j], dtype=dt)
        k = np.array([1.0 + 3.0j, 2.0 + 4.0j, 3.0 + 5.0j, 4.0 + 6.0j], dtype=dt)
        y = correlate(d, k)
        assert_equal(y, [0.0 + 0.0j, 10.0 - 2.0j, 28.0 - 6.0j, 22.0 - 6.0j, 16.0 - 6.0j, 8.0 - 4.0j])

    def test_swap_same(self, dt):
        d = [0.0 + 0.0j, 1.0 + 1.0j, 2.0 + 2.0j]
        k = [1.0 + 3.0j, 2.0 + 4.0j, 3.0 + 5.0j, 4.0 + 6.0j]
        y = correlate(d, k, mode="same")
        assert_equal(y, [10.0 - 2.0j, 28.0 - 6.0j, 22.0 - 6.0j])
    ...
    def test_consistency_correlate_funcs(self):
        ...
        for mode in ["full", "valid", "same"]:
            ...
            if mode == "valid":
                ...
                assert_almost_equal(
                    np.squeeze(signal.correlate2d([b], [a], mode=mode)), signal.correlate(b, a, mode=mode)
                )
...
class TestFiltFilt:
    ...
    def test_acoeff(self):
        ...
        out = signal.filtfilt([0.5, 0.5], 1, np.arange(10))
    ...
    def test_gust_simple(self):
        ...
        assert_allclose(
            y, [z1[0] + 0.25 * z2[0] + 0.25 * x[0] + 0.125 * x[1], 0.25 * z1[0] + z2[0] + 0.125 * x[0] + 0.25 * x[1]]
        )
...
def filtfilt_gust_opt(b, a, x):
    ...
    if warnflag > 0:
        raise RuntimeError("minimization failed in filtfilt_gust_opt: " "warnflag=%d" % warnflag)
    z0f = opt[:m]
    z0b = opt[m:]
...
class TestDecimate:
    ...
    def _test_phaseshift(self, method, zero_phase):
        ...
        for rate_to in rates_to:
            ...
            assert_allclose(np.angle(h_resps.conj() * h_resamps)[subnyq], 0, atol=1e-3, rtol=1e-3)
    ...
    def test_auto_n(self):
        ...
        assert_allclose(np.linalg.norm(x), 1.0, rtol=1e-3)
...
class TestPartialFractionExpansion:
    ...
    def test_residuez_general(self):
        ...
        self.assert_rp_almost_equal(
            r, p, [-0.9041 - 5.9928j, -0.9041 + 5.9928j], [0.5 + 0.3257j, 0.5 - 0.3257j], decimal=4
        )
        ...
        self.assert_rp_almost_equal(
            r,
            p,
            [0.2618 + 0.1902j, 0.2618 - 0.1902j, 0.4, 0.0382 - 0.1176j, 0.0382 + 0.1176j],
            [-0.8090 + 0.5878j, -0.8090 - 0.5878j, 1.0, 0.3090 + 0.9511j, 0.3090 - 0.9511j],
            decimal=4,
        )
        assert_equal(k.size, 0)
...
class TestVectorstrength:
    ...
    def test_equal_1dperiod(self):
        events = np.array([0.25, 0.25, 0.25, 0.25, 0.25, 0.25])
        period = 2
        targ_strength = 1.0
        targ_phase = 0.125
    ...
    def test_equal_2dperiod(self):
        ...
        targ_strength = [1.0] * 2
        targ_phase = np.array([0.25, 0.125])
    ...
    def test_partial_2dperiod(self):
        ...
        targ_strength = [1.0 / 3.0] * 4
        targ_phase = np.array([0.5, 0.5, 0.5, 0.5])
...
class TestUniqueRoots:
    ...
    def test_complex_repeat(self):
        p = [-1.0, -1.0 + 0.05j, -0.95 + 0.15j, -0.90 + 0.15j, 0.0, 0.5 + 0.5j, 0.45 + 0.55j]
        ...
        unique, multiplicity = unique_roots(p, tol=1e-1, rtype="max")
        assert_almost_equal(unique, [-1.0 + 0.05j, -0.90 + 0.15j, 0.0, 0.5 + 0.5j], decimal=15)
        assert_equal(multiplicity, [2, 2, 1, 2])
