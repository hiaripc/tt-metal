...
class TestWelch:
    ...
    def test_unk_scaling(self):
        assert_raises(ValueError, welch, np.zeros(4, np.complex128), scaling="foo", nperseg=4)
    ...
    def test_detrend_external_nd_0(self):
        ...
        x = x.reshape((2, 1, 10))
        x = np.moveaxis(x, 2, 0)
        f, p = welch(x, nperseg=10, axis=0, detrend=lambda seg: signal.detrend(seg, axis=0, type="l"))
        assert_allclose(p, np.zeros_like(p), atol=1e-15)
    ...
    def test_window_external(self):
        ...
        assert_raises(ValueError, welch, x, 10, win, nperseg=4)  # because nperseg != win.shape[-1]
        win_err = signal.get_window("hann", 32)
        assert_raises(ValueError, welch, x, 10, win_err, nperseg=None)  # win longer than signal
    ...
    def test_real_twosided_32(self):
        ...
        assert_allclose(f, fftfreq(8, 1.0))
        q = np.array(
            [0.08333333, 0.07638889, 0.11111111, 0.11111111, 0.11111111, 0.11111111, 0.11111111, 0.07638889], "f"
        )
        assert_allclose(p, q, atol=1e-7, rtol=1e-7)
        assert_(p.dtype == q.dtype)
    ...
    def test_complex_32(self):
        ...
        assert_allclose(f, fftfreq(8, 1.0))
        q = np.array(
            [0.41666666, 0.38194442, 0.55555552, 0.55555552, 0.55555558, 0.55555552, 0.55555552, 0.38194442], "f"
        )
        assert_allclose(p, q, atol=1e-7, rtol=1e-7)
    ...
    def test_window_correction(self):
        ...
        for window in ["hann", "bartlett", ("tukey", 0.1), "flattop"]:
            ...
            assert_allclose(np.sqrt(np.trapz(p_dens, freq)), A * np.sqrt(2) / 2, rtol=1e-3)
...
class TestCSD:
    ...
    def test_window_external(self):
        ...
        assert_array_equal(fe.shape, (5,))  # because win length used as nperseg
        assert_array_equal(pe.shape, (5,))
        assert_raises(ValueError, csd, x, x, 10, win, nperseg=256)  # because nperseg != win.shape[-1]
        win_err = signal.get_window("hann", 32)
        assert_raises(ValueError, csd, x, x, 10, win_err, nperseg=None)  # because win longer than signal
...
class TestSpectrogram:
    ...
    def test_window_external(self):
        ...
        assert_raises(ValueError, spectrogram, x, fs, win, nperseg=8)  # because nperseg != win.shape[-1]
        win_err = signal.get_window(("tukey", 0.25), 2048)
        assert_raises(ValueError, spectrogram, x, fs, win_err, nperseg=None)  # win longer than signal
    ...
    def test_short_data(self):
        ...
        with suppress_warnings() as sup:
            ...
            f1, _, p1 = spectrogram(x, fs, window=("tukey", 0.25), nperseg=1025)  # user-specified nperseg
...
class TestSTFT:
    def test_input_validation(self):
        ...
        with chk_VE("Unknown value for mode foo, must be one of: " + r"\{'psd', 'stft'\}"):
            _spectral_helper(x, x, mode="foo")
        with chk_VE("x and y must be equal if mode is 'stft'"):
            _spectral_helper(x[:512], x[512:], mode="stft")
        with chk_VE(
            "Unknown boundary option 'foo', must be one of: " + r"\['even', 'odd', 'constant', 'zeros', None\]"
        ):
    ...
    def test_roundtrip_real(self, scaling):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            _, _, zz = stft(
                x, nperseg=nperseg, noverlap=noverlap, window=window, detrend=None, padded=False, scaling=scaling
            )
    ...
    def test_roundtrip_not_nola(self):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            _, _, zz = stft(
                x, nperseg=nperseg, noverlap=noverlap, window=window, detrend=None, padded=True, boundary="zeros"
            )
    ...
    def test_roundtrip_boundary_extension(self):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            _, _, zz = stft(
                x, nperseg=nperseg, noverlap=noverlap, window=window, detrend=None, padded=True, boundary=None
            )
            ...
            for boundary in ["even", "odd", "constant", "zeros"]:
                _, _, zz_ext = stft(
                    x, nperseg=nperseg, noverlap=noverlap, window=window, detrend=None, padded=True, boundary=boundary
                )
