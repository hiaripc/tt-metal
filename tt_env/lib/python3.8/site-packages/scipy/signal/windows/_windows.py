...
def kaiser_bessel_derived(M, beta, *, sym=True):
    ...
    """
    if not sym:
        raise ValueError("Kaiser-Bessel Derived windows are only defined for symmetric " "shapes")
    elif M < 1:
        return np.array([])
    elif M % 2:
        raise ValueError("Kaiser-Bessel Derived windows are only defined for even number " "of points")
...
def taylor(M, nbar=4, sll=30, norm=True, sym=True):
    ...
    def W(n):
        return 1 + 2 * np.dot(Fm, np.cos(2 * np.pi * ma[:, np.newaxis] * (n - M / 2.0 + 0.5) / M))
...
_win_equiv_raw = {
    ...
    ("general gaussian", "general_gaussian", "general gauss", "general_gauss", "ggs"): (general_gaussian, True),
...
def get_window(window, Nx, fftbins=True):
    ...
    try:
        ...
        if isinstance(window, tuple):
            ...
            raise ValueError("%s as window type is not supported." % str(type(window))) from e
