...
class spmatrix:
    ...
    def resize(self, shape):
        ...
        raise NotImplementedError("{}.resize is not implemented".format(type(self).__name__))
    ...
    def count_nonzero(self):
        ...
        """
        raise NotImplementedError("count_nonzero not implemented for %s." % self.__class__.__name__)
    ...
    def __sub__(self, other):  # self - other
        if isscalarlike(other):
            if other == 0:
                return self.copy()
            raise NotImplementedError("subtracting a nonzero scalar from a " "sparse matrix is not supported")
        elif isspmatrix(other):
            if other.shape != self.shape:
                raise ValueError("inconsistent shapes")
    ...
    def __rsub__(self, other):  # other - self
        if isscalarlike(other):
            if other == 0:
                return -self.copy()
            raise NotImplementedError("subtracting a sparse matrix from a " "nonzero scalar is not supported")
        elif isdense(other):
            other = np.broadcast_to(other, self.shape)
    ...
    def _divide(self, other, true_divide=False, rdivide=False):
        if isscalarlike(other):
            ...
            if true_divide and np.can_cast(self.dtype, np.float_):
                ...
                if np.issubdtype(self.dtype, np.integer) and np.issubdtype(scalar_dtype, np.integer):
    ...
    def mean(self, axis=None, dtype=None, out=None):
        ...
        if axis is None:
            return (inter_self / np.array(self.shape[0] * self.shape[1])).sum(dtype=res_dtype, out=out)
