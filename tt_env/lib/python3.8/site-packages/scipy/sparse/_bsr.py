...
class bsr_matrix(_cs_matrix, _minmax_mixin):
    ...
    **Blocksize**
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, blocksize=None):
        ...
        if isspmatrix(arg1):
            ...
            if isshape(arg1):
                ...
                if self.data.ndim != 3:
                    raise ValueError("BSR data must be 3-dimensional, got shape=%s" % (self.data.shape,))
                if blocksize is not None:
                    if not isshape(blocksize):
                        raise ValueError("invalid blocksize=%s" % (blocksize,))
                    if tuple(blocksize) != self.data.shape[1:]:
                        raise ValueError("mismatching blocksize=%s vs %s" % (blocksize, self.data.shape[1:]))
            else:
    ...
    def _mul_multivector(self, other):
        ...
        bsr_matvecs(
            M // R, N // C, n_vecs, R, C, self.indptr, self.indices, self.data.ravel(), other.ravel(), result.ravel()
        )
    ...
    def _mul_sparse_matrix(self, other):
        ...
        idx_dtype = get_index_dtype((self.indptr, self.indices, other.indptr, other.indices), maxval=bnnz)
    ...
    def _binopt(self, other, op, in_shape=None, out_shape=None):
        ...
        idx_dtype = get_index_dtype((self.indptr, self.indices, other.indptr, other.indices), maxval=max_bnnz)
        indptr = np.empty(self.indptr.shape, dtype=idx_dtype)
