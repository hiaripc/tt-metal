...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
        ...
        if isspmatrix(arg1):
            ...
            if isshape(arg1):
                ...
                if len(arg1) == 2:
                    ...
                    raise ValueError("unrecognized {}_matrix " "constructor usage".format(self.format))
    ...
    def check_format(self, full_check=True):
        ...
        if self.indptr.dtype.kind != "i":
            warn("indptr array has non-integer dtype ({})" "".format(self.indptr.dtype.name), stacklevel=3)
        if self.indices.dtype.kind != "i":
            warn("indices array has non-integer dtype ({})" "".format(self.indices.dtype.name), stacklevel=3)
        ...
        if len(self.indptr) != major_dim + 1:
            raise ValueError("index pointer size ({}) should be ({})" "".format(len(self.indptr), major_dim + 1))
        if self.indptr[0] != 0:
        ...
        if full_check:
            ...
            if self.nnz > 0:
                if self.indices.max() >= minor_dim:
                    raise ValueError("{} index values must be < {}" "".format(minor_name, minor_dim))
    ...
    def __ne__(self, other):
        ...
        if isscalarlike(other):
            if np.isnan(other):
                ...
                warn(
                    "Comparing a sparse matrix with a nonzero scalar using !=" " is inefficient, try using == instead.",
                    SparseEfficiencyWarning,
                    stacklevel=3,
                )
                all_true = self.__class__(np.ones(self.shape), dtype=np.bool_)
    ...
    def _inequality(self, other, op, op_name, bad_scalar_msg):
        ...
        # Sparse other.
            ...
            warn(
                "Comparing sparse matrices using >= and <= is inefficient, " "using <, >, or !=, instead.",
                SparseEfficiencyWarning,
            )
            all_true = self.__class__(np.ones(self.shape, dtype=np.bool_))
    ...
    def multiply(self, other):
        ...
        if isspmatrix(other):
            ...
            # Row vector times matrix. other is a row.
                ...
                other = self._dia_container((other.toarray().ravel(), [0]), shape=(other.shape[1], other.shape[1]))
                return self._mul_sparse_matrix(other)
            # self is a row.
            elif self.shape[0] == 1 and self.shape[1] == other.shape[1]:
                copy = self._dia_container((self.toarray().ravel(), [0]), shape=(self.shape[1], self.shape[1]))
                return other._mul_sparse_matrix(copy)
            # Column vector times matrix. other is a column.
            elif other.shape[1] == 1 and self.shape[0] == other.shape[0]:
                other = self._dia_container((other.toarray().ravel(), [0]), shape=(other.shape[0], other.shape[0]))
                return other._mul_sparse_matrix(self)
            # self is a column.
            elif self.shape[1] == 1 and self.shape[0] == other.shape[0]:
                copy = self._dia_container((self.toarray().ravel(), [0]), shape=(self.shape[0], self.shape[0]))
        ...
        # Sparse row vector times...
            ...
            return self._coo_container(
                (data.view(np.ndarray).ravel(), (row, col)), shape=(other.shape[0], self.shape[1]), copy=False
            )
        # Sparse column vector times...
            ...
            return self._coo_container(
                (data.view(np.ndarray).ravel(), (row, col)), shape=(self.shape[0], other.shape[1]), copy=False
            )
    ...
    def _mul_sparse_matrix(self, other):
        ...
        idx_dtype = get_index_dtype((self.indptr, self.indices, other.indptr, other.indices), maxval=nnz)
    ...
    def _get_arrayXarray(self, row, col):
        ...
        csr_sample_values(M, N, self.indptr, self.indices, self.data, major.size, major.ravel(), minor.ravel(), val)
        if major.ndim == 1:
    ...
    def _major_slice(self, idx, copy=False):
        ...
        if step == 1:
            ...
            res_indices = np.empty(nnz, dtype=idx_dtype)
            res_data = np.empty(nnz, dtype=self.dtype)
            csr_row_slice(start, stop, step, self.indptr, self.indices, self.data, res_indices, res_data)
    ...
    def _minor_index_fancy(self, idx):
        ...
        res_indices = np.empty(nnz, dtype=idx_dtype)
        res_data = np.empty(nnz, dtype=self.dtype)
        csr_column_index2(col_order, col_offsets, len(self.indices), self.indices, self.data, res_indices, res_data)
        return self.__class__((res_data, res_indices, res_indptr), shape=new_shape, copy=False)
    ...
    def tocoo(self, copy=True):
        ...
        return self._coo_container((self.data, (row, col)), self.shape, copy=copy, dtype=self.dtype)
    ...
    def __get_has_canonical_format(self):
        ...
        if not getattr(self, "_has_sorted_indices", True):
            ...
            self.has_canonical_format = bool(
                _sparsetools.csr_has_canonical_format(len(self.indptr) - 1, self.indptr, self.indices)
            )
    ...
    def __get_sorted(self):
        ...
        if not hasattr(self, "_has_sorted_indices"):
            self._has_sorted_indices = bool(
                _sparsetools.csr_has_sorted_indices(len(self.indptr) - 1, self.indptr, self.indices)
            )
    ...
    def sort_indices(self):
        ...
        if not self.has_sorted_indices:
            _sparsetools.csr_sort_indices(len(self.indptr) - 1, self.indptr, self.indices, self.data)
            self.has_sorted_indices = True
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, "blocksize"):
            ...
            if rm or rn:
                raise ValueError("shape must be divisible into %s blocks. " "Got %s" % (self.blocksize, shape))
    ...
    def _binopt(self, other, op):
        ...
        idx_dtype = get_index_dtype((self.indptr, self.indices, other.indptr, other.indices), maxval=maxnnz)
