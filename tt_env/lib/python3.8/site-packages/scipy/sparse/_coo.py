...
class coo_matrix(_data_matrix, _minmax_mixin):
    ...
    def tocsc(self, copy=False):
        ...
        if self.nnz == 0:
            ...
            M, N = self.shape
            idx_dtype = get_index_dtype((self.col, self.row), maxval=max(self.nnz, M))
    ...
    def tocsr(self, copy=False):
        ...
        if self.nnz == 0:
            ...
            M, N = self.shape
            idx_dtype = get_index_dtype((self.row, self.col), maxval=max(self.nnz, N))
            row = self.row.astype(idx_dtype, copy=False)
            col = self.col.astype(idx_dtype, copy=False)
    ...
    def todia(self, copy=False):
        ...
        if len(diags) > 100:
            ...
            warn("Constructing a DIA matrix with %d diagonals " "is inefficient" % len(diags), SparseEfficiencyWarning)
