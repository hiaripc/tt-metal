...
class _minmax_mixin:
    ...
    def _min_or_max_axis(self, axis, min_or_max):
        ...
        if axis == 0:
            return self._coo_container((value, (np.zeros(len(value)), major_index)), dtype=self.dtype, shape=(1, M))
        else:
            return self._coo_container((value, (major_index, np.zeros(len(value)))), dtype=self.dtype, shape=(M, 1))
