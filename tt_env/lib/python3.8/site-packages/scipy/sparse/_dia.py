...
from ._sputils import isshape, upcast_char, getdtype, get_index_dtype, get_sum_dtype, validateaxis, check_shape
...
class dia_matrix(_data_matrix):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False):
        ...
        if isspmatrix_dia(arg1):
            ...
            if isshape(arg1):
                ...
                try:
                    ...
                    self.offsets = np.atleast_1d(np.array(arg1[1], dtype=get_index_dtype(maxval=max(shape)), copy=copy))
        ...
        if self.data.shape[0] != len(self.offsets):
            raise ValueError(
                ...
                "does not match the number of offsets (%d)" % (self.data.shape[0], len(self.offsets))
            )
    ...
    def sum(self, axis=None, dtype=None, out=None):
        ...
        if axis == 0:
            ...
            dia_matvec(
                num_rows, num_cols, len(self.offsets), self.data.shape[1], self.offsets, self.data, one, row_sums
            )
    ...
    def transpose(self, axes=None, copy=False):
        ...
        data = np.hstack((self.data, np.zeros((self.data.shape[0], pad_amount), dtype=self.data.dtype)))
        data = data[r, c]
