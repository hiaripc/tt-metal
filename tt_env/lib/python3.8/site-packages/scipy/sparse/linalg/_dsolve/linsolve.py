...
def splu(A, permc_spec=None, diag_pivot_thresh=None, relax=None, panel_size=None, options=dict()):
    """
    ...
    _options = dict(DiagPivotThresh=diag_pivot_thresh, ColPerm=permc_spec, PanelSize=panel_size, Relax=relax)
    ...
    return _superlu.gstrf(
        N, A.nnz, A.data, A.indices, A.indptr, csc_construct_func=csc_construct_func, ilu=False, options=_options
    )
...
def spilu(
    ...
    return _superlu.gstrf(
        N, A.nnz, A.data, A.indices, A.indptr, csc_construct_func=csc_construct_func, ilu=True, options=_options
    )
...
def spsolve_triangular(A, b, lower=True, overwrite_A=False, overwrite_b=False, unit_diagonal=False):
    ...
    if not isspmatrix_csr(A):
        warn("CSR matrix format is required. Converting to CSR matrix.", SparseEfficiencyWarning)
        A = csr_matrix(A)
    ...
    if overwrite_b:
        if np.can_cast(b.dtype, x_dtype, casting="same_kind"):
            ...
            raise ValueError("Cannot overwrite b (dtype {}) with result " "of type {}.".format(b.dtype, x_dtype))
    else:
    ...
    for i in row_indices:
        ...
        if not unit_diagonal and (indptr_stop <= indptr_start or A.indices[A_diagonal_index_row_i] < i):
            raise LinAlgError("A is singular: diagonal {} is zero.".format(i))
        if not unit_diagonal and A.indices[A_diagonal_index_row_i] > i:
            raise LinAlgError(
                "A is not triangular: A[{}, {}] is nonzero." "".format(i, A.indices[A_diagonal_index_row_i])
            )
