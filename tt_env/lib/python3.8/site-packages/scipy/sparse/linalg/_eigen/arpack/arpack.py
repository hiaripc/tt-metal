...
class _SymmetricArpackParams(_ArpackParams):
    ...
    def iterate(self):
        self.ido, self.tol, self.resid, self.v, self.iparam, self.ipntr, self.info = self._arpack_solver(
            self.ido,
            self.bmat,
            self.which,
            self.k,
...
class IterInv(LinearOperator):
    ...
    def _matvec(self, x):
        ...
        if info != 0:
            raise ValueError(
                "Error in inverting M: function " "%s did not converge (info = %i)." % (self.ifunc.__name__, info)
            )
        return b
...
def _fast_spmatrix_to_csc(A, hermitian=False):
    ...
    if isspmatrix_csr(A) and hermitian and not np.issubdtype(A.dtype, np.complexfloating):
        return A.T
...
def eigs(
    ...
    if M is not None:
        ...
        if np.dtype(M.dtype).char.lower() != np.dtype(A.dtype).char.lower():
            warnings.warn(
                "M does not have the same type precision as A. " "This may adversely affect ARPACK convergence"
            )
    ...
    if k >= n - 1:
        warnings.warn(
            "k >= N - 1 for N * N square matrix. " "Attempting to use scipy.linalg.eig instead.", RuntimeWarning
        )
    ...
    params = _UnsymmetricArpackParams(
        n, k, A.dtype.char, matvec, mode, M_matvec, Minv_matvec, sigma, ncv, v0, maxiter, which, tol
    )
