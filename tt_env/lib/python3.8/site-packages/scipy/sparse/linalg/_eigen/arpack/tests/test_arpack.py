...
def generate_matrix(N, complex_=False, hermitian=False, pos_definite=False, sparse=False):
...
def eval_evec(symmetric, d, typ, k, which, v0=None, sigma=None, mattype=np.asarray, OPpart=None, mode="normal"):
    ...
    if general:
        err = "error for %s:general, typ=%s, which=%s, sigma=%s, " "mattype=%s, OPpart=%s, mode=%s" % (
            eigs_func.__name__,
            typ,
            which,
            sigma,
        ...
        err = "error for %s:standard, typ=%s, which=%s, sigma=%s, " "mattype=%s, OPpart=%s, mode=%s" % (
            eigs_func.__name__,
            typ,
            which,
            sigma,
            ...
            for which in params.which:
                for mattype in params.mattypes:
                    for sigma, modes in params.sigmas_modes.items():
                        for mode in modes:
                            eval_evec(symmetric, D, typ, k, which, None, sigma, mattype, None, mode)
                ...
                for mattype in params.mattypes:
                    for sigma, OPparts in params.sigmas_OPparts.items():
                        for OPpart in OPparts:
                            eval_evec(symmetric, D, typ, k, which, None, sigma, mattype, OPpart)
