...
@pytest.mark.skipif(platform.machine() == "aarch64", reason="issue #15935")
...
def test_hermitian():
    ...
    for s, k, gen in itertools.product(sizes, ks, gens):
        ...
        for wx, vx in zip(w, v.T):
            # Check eigenvector
            assert_allclose(
                np.linalg.norm(H.dot(vx) - B.dot(vx) * wx) / np.linalg.norm(H.dot(vx)), 0, atol=5e-4, rtol=0
            )
...
def test_maxit():
    ...
    for maxiter in range(1, 4):
        with pytest.warns(UserWarning, match="Exited at iteration"):
            _, _, l_h, r_h = lobpcg(
                A, X, tol=1e-8, maxiter=maxiter, retLambdaHistory=True, retResidualNormsHistory=True
            )
        assert_allclose(np.shape(l_h)[0], maxiter + 3)
        assert_allclose(np.shape(r_h)[0], maxiter + 3)
    with pytest.warns(UserWarning, match="Exited at iteration"):
        l, _, l_h, r_h = lobpcg(A, X, tol=1e-8, retLambdaHistory=True, retResidualNormsHistory=True)
...
def test_diagonal_data_types(n, m):
    ...
    for s_f_i, s_f in enumerate(list_sparse_format):
        ...
        for A, B, M, X, Y in tests:
            eigvals, _ = lobpcg(A, X, B=B, M=M, Y=Y, tol=1e-4, maxiter=100, largest=False)
            assert_allclose(eigvals, np.arange(1 + m_excluded, 1 + m_excluded + m), atol=1e-5)
