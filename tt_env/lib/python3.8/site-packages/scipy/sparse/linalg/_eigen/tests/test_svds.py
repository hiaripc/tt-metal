...
def _check_svds(A, k, u, s, vh, which="LM", check_usvh_A=False, check_svd=True, atol=1e-10, rtol=1e-7):
    n, m = A.shape
...
def _check_svds_n(A, k, u, s, vh, which="LM", check_res=True, check_svd=True, atol=1e-10, rtol=1e-7):
    n, m = A.shape
...
class SVDSCommonTests:
    ...
    def test_svds_parameter_tol(self):
        ...
        accuracies = {"propack": [1e-12, 1e-6, 1e-4], "arpack": [2e-15, 1e-10, 1e-10], "lobpcg": [1e-11, 1e-3, 10]}
        ...
        for n, m, k in nmks:
            ...
            if k < min(n, m) - 1:
                ...
                for dt, eps in [(complex, 1e-7), (np.complex64, 1e-3)]:
                    ...
                    assert_allclose(
                        np.dot(U1, np.dot(np.diag(s1), VH1)), np.dot(U2, np.dot(np.diag(s2), VH2)), rtol=eps
                    )
