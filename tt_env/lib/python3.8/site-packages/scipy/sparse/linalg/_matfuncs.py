...
def _smart_matrix_product(A, B, alpha=None, structure=None):
    ...
    if structure == UPPER_TRIANGULAR:
        if not isspmatrix(A) and not isspmatrix(B) and not is_pydata_spmatrix(A) and not is_pydata_spmatrix(B):
...
class _ExpmPadeHelper:
    ...
    def pade13_scaled(self, s):
        ...
        U = _smart_matrix_product(
            B, (U2 + b[7] * B6 + b[5] * B4 + b[3] * B2 + b[1] * self.ident), structure=self.structure
        )
...
def _expm(A, use_exact_onenorm):
    ...
    if (isinstance(A, np.ndarray) or isspmatrix(A) or is_pydata_spmatrix(A)) and not np.issubdtype(A.dtype, np.inexact):
        A = A.astype(float)
