...
def _svdp(
    ...
    if kmax < k:
        raise ValueError("kmax must be greater than or equal to k, " f"but kmax ({kmax}) < k ({k})")
    ...
    if info > 0:
        ...
        raise LinAlgError(f"k={k} singular triplets did not converge within " f"kmax={kmax} iterations")
