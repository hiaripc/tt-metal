...
import numpy as np
import pytest
from numpy.testing import assert_allclose, assert_, assert_equal, suppress_warnings
...
REAL_DTYPES = {np.intc, np.int_, np.longlong, np.single, np.double, np.longdouble}
...
def test_expm_multiply_dtype(dtype_a, dtype_b, b_is_matrix):
    ...
    assert_allclose_ = (
        partial(assert_allclose, rtol=1.2e-3, atol=1e-5) if {dtype_a, dtype_b} & IMPRECISE else assert_allclose
    )
