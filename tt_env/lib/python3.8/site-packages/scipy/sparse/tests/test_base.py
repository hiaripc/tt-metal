...
from scipy.sparse._sputils import supported_dtypes, isscalarlike, get_index_dtype, asmatrix, matrix
...
def with_64bit_maxval_limit(
    maxval_limit=None, random=False, fixed_dtype=None, downcast_maxval=None, assert_32bit=False
):
    """
...
class _TestCommon:
    ...
    def test_round(self):
        ...
        assert_equal(np.around(A, decimals=decimal), round(self.spmatrix(A), ndigits=decimal).toarray())
        ...
        def check_setdiag(a, b, k):
            ...
            for r in [-1, len(np.diag(a, k)), 2, 30]:
                ...
                with suppress_warnings() as sup:
                    sup.filter(
                        SparseEfficiencyWarning, "Changing the sparsity structure of a cs[cr]_matrix is expensive"
                    )
                    b.setdiag(v, k)
    ...
    def test_from_list(self):
        ...
        assert_array_equal(self.spmatrix(A, dtype="int16").toarray(), array(A).astype("int16"))
    ...
    def test_sub(self):
        def check(dtype):
            ...
            A = self.spmatrix(np.array([[1, 0, 0, 4], [-1, 0, 0, 0], [0, 8, 0, -5]], "d"))
    ...
    def test_matmul(self):
        ...
        # check matrix-vector
        assert_array_almost_equal(operator.matmul(M, col), M.toarray() @ col)

        # check matrix-matrix
        assert_array_almost_equal(operator.matmul(M, B).toarray(), (M * B).toarray())
        assert_array_almost_equal(operator.matmul(M.toarray(), B), (M * B).toarray())
        assert_array_almost_equal(operator.matmul(M, B.toarray()), (M * B).toarray())
    ...
    def test_maximum_minimum(self):
        ...
        def check(dtype, dtype2, btype):
            ...
            with suppress_warnings() as sup:
                sup.filter(
                    SparseEfficiencyWarning, "Taking maximum .minimum. with > 0 .< 0. number results to a dense matrix"
                )
...
class _TestMinMax:
    ...
    def test_minmax_axis(self):
        ...
        axes = [-2, -1, 0, 1]
        for axis in axes:
            assert_array_equal(X.max(axis=axis).A, D.max(axis=axis, keepdims=True))
            assert_array_equal(X.min(axis=axis).A, D.min(axis=axis, keepdims=True))
    ...
    def test_argmax(self):
        ...
        for D in [D1, D2]:
            ...
            assert_equal(mat.argmax(axis=1), asmatrix(np.argmax(D, axis=1).reshape(-1, 1)))
            assert_equal(mat.argmin(axis=1), asmatrix(np.argmin(D, axis=1).reshape(-1, 1)))
...
def sparse_test_class(
    ...
    bases = (
        ...
        _possibly_unimplemented(_TestFancyMultidim, fancy_indexing and fancy_multidim_indexing),
        _possibly_unimplemented(_TestFancyMultidimAssign, fancy_multidim_assign and fancy_assign),
    ...
    for cls in bases:
        for name in cls.__dict__:
            ...
            if old_cls is not None:
                raise ValueError(
                    "Test class %s overloads test %s defined in %s" % (cls.__name__, name, old_cls.__name__)
                )
            names[name] = cls
...
class TestCOO(
    sparse_test_class(getset=False, slicing=False, slicing_assign=False, fancy_indexing=False, fancy_assign=False)
):
    spmatrix = coo_matrix
    math_dtypes = [np.int_, np.float_, np.complex_]
...
class TestBSR(
    sparse_test_class(
        getset=False, slicing=False, slicing_assign=False, fancy_indexing=False, fancy_assign=False, nnz_axis=False
    )
):
    spmatrix = bsr_matrix
    math_dtypes = [np.int_, np.float_, np.complex_]
