...
@pytest.mark.skipif(
    not (sys.platform.startswith("linux") and np.dtype(np.intp).itemsize >= 8), reason="test requires 64-bit Linux"
)
...
def test_upcast():
    ...
    for a_dtype in supported_dtypes:
        for b_dtype in supported_dtypes:
            ...
            if (np.issubdtype(a_dtype, np.complexfloating) and not np.issubdtype(b_dtype, np.complexfloating)) or (
                not np.issubdtype(a_dtype, np.complexfloating) and np.issubdtype(b_dtype, np.complexfloating)
            ):
