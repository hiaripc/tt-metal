...
def test_from_square_quat_matrix():
    ...
    x = np.array([[3, 0, 0, 4], [5, 0, 12, 0], [0, 0, 0, 1], [0, 0, 0, -1]])
    r = Rotation.from_quat(x)
...
def test_from_rotvec_small_angle():
    rotvec = np.array([[5e-4 / np.sqrt(3), -5e-4 / np.sqrt(3), 5e-4 / np.sqrt(3)], [0.2, 0.3, 0.4], [0, 0, 0]])
    ...
    assert_allclose(quat[1, :3], np.array([0.09879603932153465, 0.14819405898230198, 0.19759207864306931]))
...
def test_align_vectors_noise():
    ...
    est, rmsd, cov = Rotation.align_vectors(noisy_result, vectors, return_sensitivity=True)
