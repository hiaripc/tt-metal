...
def roots_jacobi(n, alpha, beta, mu=False):
    ...
    if a + b == 0.0:
        ...
        an_func = lambda k: np.where(
            k == 0, (b - a) / (2 + a + b), (b * b - a * a) / ((2.0 * k + a + b) * (2.0 * k + a + b + 2))
        )
    ...
    df = lambda n, x: (0.5 * (n + a + b + 1) * _ufuncs.eval_jacobi(n - 1, a + 1, b + 1, x))
...
def sh_jacobi(n, p, q, monic=False):
    ...
    kn = 1.0
    pp = orthopoly1d(
        x, w, hn, kn, wfunc=wfunc, limits=(0, 1), monic=monic, eval_func=lambda x: _ufuncs.eval_sh_jacobi(n, p, q, x)
    )
    return pp
...
def roots_genlaguerre(n, alpha, mu=False):
    ...
    df = (
        lambda n, x: (
            n * _ufuncs.eval_genlaguerre(n, alpha, x) - (n + alpha) * _ufuncs.eval_genlaguerre(n - 1, alpha, x)
        )
        / x
    )
    return _gen_roots_and_weights(m, mu0, an_func, bn_func, f, df, False, mu)
...
def _pbcf(n, theta):
    ...
    u3 = (
        -4042.0 * ctp[9, :] + 18189.0 * ctp[7, :] - 28287.0 * ctp[5, :] - 151995.0 * ctp[3, :] - 259290.0 * ct
    ) / 414720.0
    u4 = (
        72756.0 * ctp[10, :]
        - 321339.0 * ctp[8, :]
    ...
    v3 = (
        -4042.0 * ctp[9, :] + 18189.0 * ctp[7, :] - 36387.0 * ctp[5, :] + 238425.0 * ctp[3, :] + 259290.0 * ct
    ) / 414720.0
    v4 = (
        -121260.0 * ctp[10, :]
        + 551733.0 * ctp[8, :]
    ...
    A2 = (
        b4 * u0 + phip[0, :] * b3 * u1 + phip[1, :] * b2 * u2 + phip[2, :] * b1 * u3 + phip[3, :] * b0 * u4
    ) / zeta**6
    ...
    U = P * (
        Ai * (A0 + A1 / mu**2.0 + A2 / mu**4.0) + Aip * (B0 + B1 / mu**2.0 + B2 / mu**4.0) / mu ** (8.0 / 6.0)
    )
    ...
    D2 = (
        a4 * v0 + phip[0, :] * a3 * v1 + phip[1, :] * a2 * v2 + phip[2, :] * a1 * v3 + phip[3, :] * a0 * v4
    ) / zeta**6
    # Derivative of U
    # https://dlmf.nist.gov/12.10#E36
    Ud = Pd * (
        Ai * (C0 + C1 / mu**2.0 + C2 / mu**4.0) / mu ** (4.0 / 6.0) + Aip * (D0 + D1 / mu**2.0 + D2 / mu**4.0)
    )
    return U, Ud
...
def hermitenorm(n, monic=False):
    ...
    kn = 1.0
    p = orthopoly1d(
        x, w, hn, kn, wfunc=wfunc, limits=(-inf, inf), monic=monic, eval_func=lambda x: _ufuncs.eval_hermitenorm(n, x)
    )
    return p
...
def roots_gegenbauer(n, alpha, mu=False):
    ...
    an_func = lambda k: 0.0 * k
    bn_func = lambda k: np.sqrt(k * (k + 2 * alpha - 1) / (4 * (k + alpha) * (k + alpha - 1)))
...
def roots_legendre(n, mu=False):
    ...
    df = lambda n, x: (-n * x * _ufuncs.eval_legendre(n, x) + n * _ufuncs.eval_legendre(n - 1, x)) / (1 - x**2)
    return _gen_roots_and_weights(m, mu0, an_func, bn_func, f, df, True, mu)
...
def legendre(n, monic=False):
    ...
    p = orthopoly1d(
        x, w, hn, kn, wfunc=lambda x: 1.0, limits=(-1, 1), monic=monic, eval_func=lambda x: _ufuncs.eval_legendre(n, x)
    )
    return p
