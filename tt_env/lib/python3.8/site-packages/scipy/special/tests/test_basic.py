...
class TestCephes:
    ...
    def test_binom(self):
        ...
        rknown = np.array(
            [
                [-0.097152, 0.9263051596159367, 0.01858423645695389, -0.007581020651518199],
    ...
    def test_beta(self):
        ...
        assert_allclose(cephes.beta(0.0342, 171), 24.070498359873497, rtol=1e-13, atol=0)
    ...
    def test_betaincinv(self):
        ...
        assert_allclose(cephes.betaincinv(0.0342, 171, 0.25), 8.4231316935498957e-21, rtol=3e-12, atol=0)
    ...
    def test_fdtri(self):
        assert_allclose(cephes.fdtri(1, 1, [0.499, 0.501]), array([0.9937365, 1.00630298]), rtol=1e-6)
        # From Wolfram Alpha:
    ...
    def test_wofz(self):
        ...
        w = [
            complex(
                -3.78270245518980507452677445620103199303131110e-7, 0.000903861276433172057331093754199933411710053155
            ),
            complex(
            ...
            complex(
                7.317131068972378096865595229600561710140617977e34, 8.321873499714402777186848353320412813066170427e34
            ),
            complex(
            ...
            complex(
                0.3960793007699874918961319170187598400134746631, -5.593152259116644920546186222529802777409274656e-9
            ),
            complex(
            ...
            complex(
                9.11463368405637174660562096516414499772662584e304, 3.97101807145263333769664875189354358563218932e305
            ),
            complex(
                -4.4927207857715598976165541011143706155432296e281, -2.8019591213423077494444700357168707775769028e281
            ),
            complex(
                2.820947917809305132678577516325951485807107151e-6, 2.820947917668257736791638444590253942253354058e-6
            ),
            complex(
                2.82094791773878143474039725787438662716372268e-15, 2.82094791773878143474039725773333923127678361e-15
            ),
        ]
...
class TestAiry:
    ...
    def test_ai_zeros_big(self):
        ...
        assert_allclose(
            z[:6],
            [-2.3381074105, -4.0879494441, -5.5205598281, -6.7867080901, -7.9441335871, -9.0226508533],
            rtol=1e-10,
        )
        assert_allclose(
            zp[:6],
            [-1.0187929716, -3.2481975822, -4.8200992112, -6.1633073556, -7.3721772550, -8.4884867340],
            rtol=1e-10,
        )
    ...
    def test_bi_zeros_big(self):
        ...
        assert_allclose(
            z[:6],
            [-1.1737132227, -3.2710933028, -4.8307378417, -6.1698521283, -7.3767620794, -8.4919488465],
            rtol=1e-10,
        )
        assert_allclose(
            zp[:6],
            [-2.2944396826, -4.0731550891, -5.5123957297, -6.7812944460, -7.9401786892, -9.0195833588],
            rtol=1e-10,
        )
...
class TestKelvin:
    ...
    def test_beip_zeros(self):
        ...
        assert_array_almost_equal(
            bip,
            array([3.772673304934953, 8.280987849760042, 12.742147523633703, 17.193431752512542, 21.641143941167325]),
            8,
        )
    ...
    def test_berp_zeros(self):
        ...
        assert_array_almost_equal(brp, array([6.03871, 10.51364, 14.96844, 19.41758, 23.86430]), 4)
    ...
    def test_kelvin_zeros(self):
        ...
        assert_array_almost_equal(berpz, array([6.03871, 10.51364, 14.96844, 19.41758, 23.86430]), 4)
        assert_array_almost_equal(
            beipz,
            array(
                [
...
class TestBernoulli:
    def test_bernoulli(self):
        ...
        assert_array_almost_equal(brn, array([1.0000, -0.5000, 0.1667, 0.0000, -0.0333, 0.0000]), 4)
...
class TestCombinatorics:
    def test_comb(self):
        ...
        assert_allclose(
            [special.comb(20, k, exact=True) for k in range(21)], special.comb(20, list(range(21))), atol=1e-15
        )
...
class TestEllipCarlson(object):
    ...
    def test_elliprc(self):
        ...
        args = array([[0.0, 0.25], [2.25, 2.0], [0.0, 1.0j], [-1.0j, 1.0j], [0.25, -2.0], [1.0j, -1.0]])
    ...
    def test_elliprj_hard(self):
        assert_allclose(
            elliprj(6.483625725195452e-08, 1.1649136528196886e-27, 3.6767340167168e13, 0.493704617023468),
            8.63426920644241857617477551054e-6,
            rtol=5e-15,
            atol=1e-20,
        )
        assert_allclose(
            elliprj(14.375105857849121, 9.993988969725365e-11, 1.72844262269944e-26, 5.898871222598245e-06),
...
class TestErf:
    ...
    def test_erfc_consistent(self):
        self._check_variant_func(
            cephes.erfc, lambda z: 1 - cephes.erf(z), rtol=1e-12, atol=1e-14  # <- the test function loses precision
        )
...
class TestFactorialFunctions:
    def test_factorial(self):
        ...
        assert_array_almost_equal([6.0, 24.0, 120.0], special.factorial([3, 4, 5], exact=False))
        assert_array_almost_equal(special.factorial([[5, 3], [4, 3]]), [[120, 6], [24, 6]])
        ...
        assert_equal(special.factorial([7, 4, 15, 10], exact=True), [5040, 24, 1307674368000, 3628800])

        assert_equal(special.factorial([[5, 3], [4, 3]], True), [[120, 6], [24, 6]])

        # object arrays
        assert_equal(special.factorial(np.arange(-3, 22), True), special.factorial(np.arange(-3, 22), False))

        # int64 array
        assert_equal(special.factorial(np.arange(-3, 15), True), special.factorial(np.arange(-3, 15), False))

        # int32 array
        assert_equal(special.factorial(np.arange(-3, 5), True), special.factorial(np.arange(-3, 5), False))
    ...
    def test_factorial2(self):
        assert_array_almost_equal([105.0, 384.0, 945.0], special.factorial2([7, 8, 9], exact=False))
    ...
    # single NaN.
    @pytest.mark.skipif(sys.version_info >= (3, 10), reason="Python 3.10+ math.factorial() requires int")
...
class TestFresnel:
    ...
    def test_fresnel_zeros(self):
        ...
        assert_array_almost_equal(
            szo, array([2.0093 + 0.2885j, 2.8335 + 0.2443j, 3.4675 + 0.2185j, 4.0026 + 0.2009j, 4.4742 + 0.1877j]), 3
        )
        assert_array_almost_equal(
            czo, array([1.7437 + 0.3057j, 2.6515 + 0.2529j, 3.3204 + 0.2240j, 3.8757 + 0.2047j, 4.3611 + 0.1907j]), 3
        )
...
class TestGamma:
    ...
    def test_975(self):
        ...
        pts = [0.25, np.nextafter(0.25, 0), 0.25 - 1e-12, np.nextafter(0.25, 1), 0.25 + 1e-12]
        for xp in pts:
...
class TestHyper:
    ...
    def test_hyp0f1(self):
        ...
        expected = np.array([0.58493659229143, 0.70566805723127, 1.0, 1.37789689539747, 1.60373685288480])
    ...
    def test_hyperu(self):
        ...
        hprl = (pi / sin(pi * b)) * (
            ...
            - z ** (1 - b) * special.hyp1f1(1 + a - b, 2 - b, z) / (special.gamma(a) * special.gamma(2 - b))
        )
...
class TestBessel:
    ...
    def test_jn_zeros(self):
        ...
        assert_array_almost_equal(
            jn0, array([2.4048255577, 5.5200781103, 8.6537279129, 11.7915344391, 14.9309177086]), 4
        )
    ...
    def test_jn_zeros_slow(self):
        ...
        assert_allclose(
            jn3010,
            array([3036.86590780927, 3057.06598526482, 3073.66360690272, 3088.37736494778, 3101.86438139042]),
            rtol=1e-8,
        )
    ...
    def test_jnp_zeros(self):
        ...
        assert_array_almost_equal(jnp, array([1.84118, 5.33144, 8.53632, 11.70600, 14.86359]), 4)
...
class TestLambda:
    def test_lmbda(self):
        ...
        lamr = (
            ...
            array([special.jvp(0, 0.1), -2 * special.jv(1, 0.1) / 0.01 + 2 * special.jvp(1, 0.1) / 0.1]),
        )
...
class TestLegendreFunctions:
    ...
    def test_clpmn_close_to_real_2(self):
        ...
        assert_array_almost_equal(
            array([clp_plus, clp_minus]), array([special.lpmv(m, n, x), special.lpmv(m, n, x)]), 7
        )
    ...
    def test_clpmn_close_to_real_3(self):
        ...
        assert_array_almost_equal(
            ...
            array(
                [special.lpmv(m, n, x) * np.exp(-0.5j * m * np.pi), special.lpmv(m, n, x) * np.exp(0.5j * m * np.pi)]
            ),
            7,
        )
    ...
    def test_clpmn_across_unit_circle(self):
        ...
        for type in [2, 3]:
            assert_almost_equal(
                special.clpmn(m, n, x + 1j * eps, type)[0][m, n], special.clpmn(m, n, x - 1j * eps, type)[0][m, n], 6
            )
    ...
    def test_deriv_clpmn(self):
        ...
        for type in [2, 3]:
            for z in zvals:
                for h in [1e-3, 1e-3j]:
                    approx_derivative = (
                        special.clpmn(m, n, z + 0.5 * h, type)[0] - special.clpmn(m, n, z - 0.5 * h, type)[0]
                    ) / h
                    assert_allclose(special.clpmn(m, n, z, type)[1], approx_derivative, rtol=1e-4)

    def test_lpmn(self):
        lp = special.lpmn(0, 2, 0.5)
        assert_array_almost_equal(lp, (array([[1.00000, 0.50000, -0.12500]]), array([[0.00000, 1.00000, 1.50000]])), 4)

    def test_lpn(self):
        lpnf = special.lpn(2, 0.5)
        assert_array_almost_equal(lpnf, (array([1.00000, 0.50000, -0.12500]), array([0.00000, 1.00000, 1.50000])), 4)
...
class TestOblCvSeq:
    def test_obl_cv_seq(self):
        ...
        assert_array_almost_equal(obl, array([-0.348602, 1.393206, 5.486800, 11.492120]), 5)
...
class TestProCvSeq:
    def test_pro_cv_seq(self):
        ...
        assert_array_almost_equal(prol, array([0.319000, 2.593084, 6.533471, 12.514462]), 5)
...
def test_sph_harm():
    ...
    assert_array_almost_equal(
        sh(2, 2, pi, pi / 2), 0.25 * sqrt(15 / (2.0 * pi)) * exp(0 + 2.0 * pi * 1j) * sin(pi / 2.0) ** 2.0
    )
    assert_array_almost_equal(
    ...
    assert_array_almost_equal(
        ...
        (3.0 / 16.0) * sqrt(35.0 / (2.0 * pi)) * exp(0 + 4.0 * pi / 8.0 * 1j) * sin(pi / 6.0) ** 4.0,
    )
...
def test_agm_simple():
    ...
    assert_allclose(special.agm(0.75 * fi.max, fi.max), 1.564904312298045e308, rtol=rtol)
