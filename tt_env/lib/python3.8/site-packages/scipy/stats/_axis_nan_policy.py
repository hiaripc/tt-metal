...
def _broadcast_shapes(shapes, axis=None):
    ...
    if axis is not None:
        ...
        if not np.array_equal(axis_int, axis):
            raise np.AxisError("`axis` must be an integer, a " "tuple of integers, or `None`.")
        axis = axis_int
...
def _axis_nan_policy_factory(
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            if intersection:
                message = f"{hypotest_fun_in.__name__}() got multiple values " f"for argument '{list(intersection)[0]}'"
