...
from ._constants import _XMIN, _EULER, _ZETA3, _SQRT_PI, _SQRT_2_OVER_PI, _LOG_SQRT_2_OVER_PI
...
class FitDataError(ValueError):
    ...
    def __init__(self, distr, lower, upper):
        self.args = (
            ...
            "(x - loc)/scale  < {upper!r} for each x in `data`.".format(distr=distr, lower=lower, upper=upper),
        )
    ...
    def fit(self, data, *args, **kwds):
        ...
        if f0 is not None or f1 is not None:
            ...
            theta, info, ier, mesg = optimize.fsolve(
                _beta_mle_a, a, args=(b, len(data), np.log(data).sum()), full_output=True
            )
            if ier != 1:
...
class betaprime_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        if n == 1.0:
            ...
            return np.where(b > 3, a * (a + 1.0) * (a + 2.0) / ((b - 3.0) * (b - 2.0) * (b - 1.0)), np.inf)
        elif n == 4.0:
            return np.where(
                b > 4, (a * (a + 1.0) * (a + 2.0) * (a + 3.0) / ((b - 4.0) * (b - 3.0) * (b - 2.0) * (b - 1.0))), np.inf
            )
        else:
...
class burr_gen(rv_continuous):
    ...
    def _pdf(self, x, c, d):
        ...
        output = _lazywhere(
            ...
            f2=lambda x_, c_, d_: (c_ * d_ * (x_ ** (-c_ - 1.0)) / ((1 + x_ ** (-c_)) ** (d_ + 1.0))),
        )
        if output.ndim == 0:
            return output[()]
        return output
    ...
    def _logpdf(self, x, c, d):
        output = _lazywhere(
            x == 0,
            [x, c, d],
            lambda x_, c_, d_: (np.log(c_) + np.log(d_) + sc.xlogy(c_ * d_ - 1, x_) - (d_ + 1) * sc.log1p(x_ ** (c_))),
            f2=lambda x_, c_, d_: (np.log(c_) + np.log(d_) + sc.xlogy(-c_ - 1, x_) - sc.xlog1py(d_ + 1, x_ ** (-c_))),
        )
        if output.ndim == 0:
            return output[()]
        return output
...
class f_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd):
        ...
        g1 = _lazywhere(
            v2 > 6,
            (v1, v2_2, v2_4, v2_6),
            lambda v1, v2_2, v2_4, v2_6: (2 * v1 + v2_2) / v2_6 * np.sqrt(v2_4 / (v1 * (v1 + v2_2))),
            np.nan,
        )
        g1 *= np.sqrt(8.0)
    ...
    def _stats(self, c, moments="mv"):
        ...
        if "k" not in moments:
            ...
            k = _lazywhere(
                c < 1 / 4,
                (c,),
                lambda xi: 3 * (1 - 2 * xi) * (2 * xi**2 + xi + 3) / (1 - 3 * xi) / (1 - 4 * xi) - 3,
                np.nan,
            )
        return m, v, s, k
...
class genextreme_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        gam2k = np.where(
            abs(c) < 1e-7, np.pi**2.0 / 6.0, sc.expm1(sc.gammaln(2.0 * c + 1.0) - 2 * sc.gammaln(c + 1.0)) / c**2.0
        )
        eps = 1e-14
        ...
        sk1 = _lazywhere(
            c >= -1.0 / 3,
            (c, g1, g2, g3, g2mg12),
            lambda c, g1, g2, g3, g2gm12: np.sign(c) * (-g3 + (g2 + 2 * g2mg12) * g1) / g2mg12**1.5,
            fillvalue=np.nan,
        )
        ...
        ku1 = _lazywhere(
            c >= -1.0 / 4,
            (g1, g2, g3, g4, g2mg12),
            lambda g1, g2, g3, g4, g2mg12: (g4 + (-4 * g3 + 3 * (g2 + g2mg12) * g1) * g1) / g2mg12**2,
            fillvalue=np.nan,
        )
...
class erlang_gen(gamma_gen):
    ...
    def _argcheck(self, a):
        ...
        if not allint:
            ...
            warnings.warn(
                "The shape parameter of the erlang distribution " "has been given a non-integer value %r." % (a,),
                RuntimeWarning,
            )
        return a > 0
    ...
    def _stats(self, p, a, b):
        ...
        m4e = (
            ...
            * (
            ...
            + np.float_power(b, 2)
            * np.float_power(t2, 3)
            * (6 * r3 - 12 * b2 * b1 * np.float_power(b0, -2) + 6 * np.float_power(r1, 3))
...
class laplace_asymmetric_gen(rv_continuous):
    ...
    def _cdf(self, x, kappa):
        ...
        return np.where(
            x >= 0, 1 - np.exp(-x * kappa) * (kapinv / kappkapinv), np.exp(x * kapinv) * (kappa / kappkapinv)
        )
    ...
    def _sf(self, x, kappa):
        ...
        kappkapinv = kappa + kapinv
        return np.where(
            x >= 0, np.exp(-x * kappa) * (kapinv / kappkapinv), 1 - np.exp(x * kapinv) * (kappa / kappkapinv)
        )
...
def _lognorm_logpdf(x, s):
    return _lazywhere(
        x != 0, (x, s), lambda x, s: -np.log(x) ** 2 / (2 * s**2) - np.log(s * x * np.sqrt(2 * np.pi)), -np.inf
    )
...
class lognorm_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        for name in ["f0", "fs", "fix_s", "floc", "fscale", "loc", "scale", "optimizer", "method"]:
            kwds.pop(name, None)
        if kwds:
            raise TypeError("Unknown arguments: %s." % kwds)
...
for m in _gibrat_method_names:
    wrapper = np.deprecate(getattr(gilbrat, m), f"gilbrat.{m}", f"gibrat.{m}", deprmsg)
    ...
    def _logpdf(self, x, df):
        return _lazywhere(
            ...
            f2=lambda x, df: (
                sc.gammaln((df + 1) / 2)
                - sc.gammaln(df / 2)
                - (0.5 * np.log(df * np.pi) + (df + 1) / 2 * np.log(1 + (x**2) / df))
            ),
        )
...
class powerlaw_gen(rv_continuous):
    ...
    def _support_mask(self, x, a):
        return super(powerlaw_gen, self)._support_mask(x, a) & ((x != 0) | (a >= 1))
...
class powerlognorm_gen(rv_continuous):
    ...
    def _pdf(self, x, c, s):
        ...
        #                                         (Phi(-log(x)/s))**(c-1),
        return c / (x * s) * _norm_pdf(np.log(x) / s) * pow(_norm_cdf(-np.log(x) / s), c * 1.0 - 1.0)
...
class recipinvgauss_gen(rv_continuous):
    ...
    def _logpdf(self, x, mu):
        return _lazywhere(
            x > 0,
            (x, mu),
            lambda x, mu: (-((1 - mu * x) ** 2.0) / (2 * x * mu**2.0) - 0.5 * np.log(2 * np.pi * x)),
            fillvalue=-np.inf,
        )
...
class skew_norm_gen(rv_continuous):
    ...
    def _pdf(self, x, a):
        return _lazywhere(
            a == 0, (x, a), lambda x, a: _norm_pdf(x), f2=lambda x, a: 2.0 * _norm_pdf(x) * _norm_cdf(a * x)
        )
    ...
    def _skewnorm_odd_moments(self):
        skewnorm_odd_moments = {
            ...
            15: Polynomial([2027025, -4729725, 8513505, -10135125, 7882875, -3869775, 1091475, -135135]),
            17: Polynomial(
                [34459425, -91891800, 192972780, -275675400, 268017750, -175429800, 74220300, -18378360, 2027025]
            ),
            19: Polynomial(
                [
                    654729075,
    ...
    def _munp(self, order, a):
        if order & 1:
            if order > 19:
                raise NotImplementedError(
                    "skewnorm noncentral moments not " "implemented for odd orders greater " "than 19."
                )
...
class trapezoid_gen(rv_continuous):
    ...
    def _pdf(self, x, c, d):
        ...
        return _lazyselect(
            [x < c, (c <= x) & (x <= d), x > d],
            [lambda x, c, d, u: u * x / c, lambda x, c, d, u: u, lambda x, c, d, u: u * (1 - x) / (1 - d)],
            (x, c, d, u),
        )
    ...
    def _munp(self, n, c, d):
        ...
        dc_term = _lazyselect(
            ...
            [lambda d: 1.0, lambda d: np.expm1((n + 2) * np.log(d)) / (d - 1.0), lambda d: n + 2],
            [d],
        )
...
class triang_gen(rv_continuous):
    ...
    def _pdf(self, x, c):
        ...
        r = _lazyselect(
            [c == 0, x < c, (x >= c) & (c != 1), c == 1],
            [lambda x, c: 2 - 2 * x, lambda x, c: 2 * x / c, lambda x, c: 2 * (1 - x) / (1 - c), lambda x, c: 2 * x],
            (x, c),
        )
        return r
...
class truncnorm_gen(rv_continuous):
    ...
    def _ppf(self, q, a, b):
        ...
        def ppf_left(q, a, b):
            log_Phi_x = _log_sum(sc.log_ndtr(a), np.log(q) + _log_gauss_mass(a, b))
            return sc.ndtri_exp(log_Phi_x)

        def ppf_right(q, a, b):
            log_Phi_x = _log_sum(sc.log_ndtr(-b), np.log1p(-q) + _log_gauss_mass(a, b))
            return -sc.ndtri_exp(log_Phi_x)
    ...
    def _isf(self, q, a, b):
        ...
        def isf_left(q, a, b):
            log_Phi_x = _log_diff(sc.log_ndtr(b), np.log(q) + _log_gauss_mass(a, b))
            return sc.ndtri_exp(np.real(log_Phi_x))

        def isf_right(q, a, b):
            log_Phi_x = _log_diff(sc.log_ndtr(-a), np.log1p(-q) + _log_gauss_mass(a, b))
            return -sc.ndtri_exp(np.real(log_Phi_x))
    ...
    def _munp(self, n, a, b):
        ...
        return _lazywhere(
            (n >= 0) & (a == a) & (b == b), (n, a, b), np.vectorize(n_th_moment, otypes=[np.float64]), np.nan
        )
...
class truncpareto_gen(rv_continuous):
    ...
    def _entropy(self, b, c):
        return -(np.log(b / (1 - c**-b)) + (b + 1) * (np.log(c) / (c**b - 1) - 1 / b))
...
class FitUniformFixedScaleDataError(FitDataError):
    def __init__(self, ptp, fscale):
        self.args = (
            ...
            "data.ptp() <= fscale, but data.ptp() = %r and fscale = %r." % (ptp, fscale),
        )
