...
from ._distn_infrastructure import rv_discrete, get_distribution_names, _check_shape, _ShapeInfo
...
boltzmann = boltzmann_gen(name="boltzmann", a=0, longname="A truncated discrete exponential ")
...
class skellam_gen(rv_discrete):
    ...
    def _cdf(self, x, mu1, mu2):
        x = floor(x)
        px = np.where(
            x < 0, _boost._ncx2_cdf(2 * mu2, -2 * x, 2 * mu1), 1 - _boost._ncx2_cdf(2 * mu1, 2 * (x + 1), 2 * mu2)
        )
        return px
...
class yulesimon_gen(rv_discrete):
    ...
    def _stats(self, alpha):
        ...
        g2 = np.where(
            alpha > 4, (alpha + 3) + (alpha**3 - 49 * alpha - 22) / (alpha * (alpha - 4) * (alpha - 3)), np.inf
        )
...
nchypergeom_fisher = nchypergeom_fisher_gen(name="nchypergeom_fisher", longname="A Fisher's noncentral hypergeometric")
