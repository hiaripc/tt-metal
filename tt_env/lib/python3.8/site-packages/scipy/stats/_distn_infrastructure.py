...
class rv_generic:
    ...
    def _construct_doc(self, docdict, shapes_vals=None):
        ...
        for i in range(2):
            ...
            try:
                ...
                raise Exception(
                    "Unable to construct docstring for " 'distribution "%s": %s' % (self.name, repr(e))
                ) from e

        # correct for empty shapes
        self.__doc__ = self.__doc__.replace("(, ", "(").replace(", )", ")")

    def _construct_default_doc(self, longname=None, extradoc=None, docdict=None, discrete="continuous"):
        ...
        self.__doc__ = "".join(
            [
                "%s %s random variable." % (longname, discrete),
                "\n\n%(before_notes)s\n",
                docheaders["notes"],
                extradoc,
                "\n%(example)s",
    ...
    def _argcheck_rvs(self, *args, **kwargs):
        ...
        if not ok:
            raise ValueError(
                "size does not match the broadcast shape of " "the parameters. %s, %s, %s" % (size, size_, bcast_shape)
            )
    ...
    def moment(self, order=None, *args, **kwds):
        ...
        has_shape_n = self.shapes is not None and "n" in (self.shapes.split(", "))
        ...
        #    error (order of the moment is not provided), and it is still an
        #    error for the same reason.
        if not got_order and ((not got_keyword_n) or (got_keyword_n and has_shape_n)):  # A4 and B4  # A2
            message = "moment() missing 1 required " "positional argument: `order`"
        ...
        if got_keyword_n and not has_shape_n:
            if got_order:  # B1
                ...
                message = (
                    "Use of keyword argument 'n' for method 'moment is"
    ...
    def interval(self, confidence=None, *args, **kwds):
        ...
        has_shape_alpha = self.shapes is not None and "alpha" in (self.shapes.split(", "))
        got_confidence = confidence is not None
        got_keyword_alpha = kwds.get("alpha", None) is not None

        if not got_confidence and ((not got_keyword_alpha) or (got_keyword_alpha and has_shape_alpha)):
            message = "interval() missing 1 required positional argument: " "`confidence`"
            raise TypeError(message)
        ...
        if got_keyword_alpha and not has_shape_alpha:
            if got_confidence:
                ...
                message = (
                    "Use of keyword argument 'alpha' for method "
...
def _get_fixed_fit_value(kwds, names):
    ...
    if len(vals) > 1:
        ...
        raise ValueError(
            "fit method got multiple keyword arguments to " "specify the same fixed parameter: " + ", ".join(repeated)
        )
    ...
    **Statistics**
    ...
    def __init__(
        ...
        if extradoc is not None:
            warnings.warn("extradoc is deprecated and will be removed in " "SciPy 1.11.0", DeprecationWarning)
        ...
        self._construct_argparser(
            meths_to_inspect=[self._pdf, self._cdf], locscale_in="loc=0, scale=1", locscale_out="loc, scale"
        )
        self._attach_methods()
        ...
        if sys.flags.optimize < 2:
            ...
            if self.__doc__ is None:
                self._construct_default_doc(
                    longname=longname, extradoc=extradoc, docdict=docdict, discrete="continuous"
                )
            else:
                dct = dict(distcont)
    ...
    def _reduce_func(self, args, kwds, data=None):
        ...
        if method == "mm":
            ...
            raise ValueError("Method '{0}' not available; must be one of {1}".format(method, methods))
        ...
        **kwds : floats, optional
        ...
        if method == "mm":
            if not np.isfinite(obj):
                raise FitError(
                    "Optimization failed: either a data moment " "or fitted distribution moment is " "non-finite."
                )
    ...
    **Discrete distributions from a list of probabilities**
    ...
    def _construct_docstrings(self, name, longname, extradoc):
        ...
        if sys.flags.optimize < 2:
            ...
            if self.__doc__ is None:
                self._construct_default_doc(
                    longname=longname, extradoc=extradoc, docdict=docdict_discrete, discrete="discrete"
                )
            else:
