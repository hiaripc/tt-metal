...
def entropy(
    pk: np.typing.ArrayLike, qk: Optional[np.typing.ArrayLike] = None, base: Optional[float] = None, axis: int = 0
...
def differential_entropy(
    ...
    * ``'auto'`` selects the method automatically (default). Currently,
    ...
    if not 2 <= 2 * window_length < n:
        raise ValueError(
            f"Window length ({window_length}) must be positive and less " f"than half the sample size ({n}).",
        )
