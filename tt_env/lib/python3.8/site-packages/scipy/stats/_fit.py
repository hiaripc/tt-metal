...
def _combine_bounds(name, user_bounds, shape_domain, integral):
    ...
    if user_bounds[0] > user_bounds[1]:
        message = f"There are no values for `{name}` on the interval " f"{list(user_bounds)}."
...
class FitResult:
    ...
    def __repr__(self):
        ...
        return "\n".join(
            [key.rjust(m) + ": " + repr(getattr(self, key)) for key in keys if getattr(self, key) is not None]
        )
    ...
    def _qp_plot(self, ax, fit_params, qq):
        ...
        if self.discrete and qq:
            ...
            ax.plot(
                q_ideal, q_ideal, "o", label="Reference", color="k", alpha=0.25, markerfacecolor="none", clip_on=True
            )
...
def fit(dist, data, bounds=None, *, guess=None, method="mle", optimizer=optimize.differential_evolution):
    ...
    if isinstance(user_bounds, dict):
        ...
        if user_bounds:
            message = (
                "Bounds provided for the following unrecognized " f"parameters will be ignored: {set(user_bounds)}"
            )
        ...
        if user_bounds.ndim != 2 or user_bounds.shape[1] != 2:
            message = (
                "Each element of `bounds` must be a tuple specifying " "the lower and upper bounds of a shape parameter"
            )
            raise ValueError(message)
    ...
    if user_guess is None:
        ...
        default_guess = {param.name: np.mean(bound) for param, bound in zip(param_info, bounds)}
        unrecognized = set(user_guess) - set(default_guess)
        if unrecognized:
            message = "Guesses provided for the following unrecognized " f"parameters will be ignored: {unrecognized}"
            warnings.warn(message, RuntimeWarning, stacklevel=2)
        default_guess.update(user_guess)

        message = "Each element of `guess` must be a scalar " "guess for a distribution parameter."
        try:
            guess_array = np.asarray([default_guess[param.name] for param in param_info], dtype=float)
        except ValueError as e:
            raise ValueError(message) from e
    ...
    if guess_array is not None:
        ...
        for i in rounded:
            message = (
                f"Guess for parameter `{param_info[i].name}` " f"rounded from {guess_array[i]} to {guess_rounded[i]}."
            )
            warnings.warn(message, RuntimeWarning, stacklevel=2)
...
GoodnessOfFitResult = namedtuple("GoodnessOfFitResult", ("fit_result", "statistic", "pvalue", "null_distribution"))
...
def goodness_of_fit(
    ...
    *observed data*. This tends to reduce the value of the statistic for the
    ...
    res = stats.monte_carlo_test(
        data, rvs, statistic_fun, vectorized=True, n_resamples=n_mc_samples, axis=-1, alternative="greater"
    )
...
_compare_dict = {"ad": _anderson_darling, "ks": _kolmogorov_smirnov, "cvm": _cramer_von_mises}
