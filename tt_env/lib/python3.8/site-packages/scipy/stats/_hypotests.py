...
BarnardExactResult = make_dataclass("BarnardExactResult", [("statistic", float), ("pvalue", float)])
...
def barnard_exact(table, alternative="two-sided", pooled=True, n=32):
    ...
    * :math:`T(X) \geq |T(X_0)|` when `alternative` = "two-sided".
    ...
    """
    if n <= 0:
        raise ValueError("Number of points `n` must be strictly positive, " f"found {n!r}")
    ...
    if alternative == "two-sided":
        ...
        msg = "`alternative` should be one of {'two-sided', 'less', 'greater'}," f" found {alternative!r}"
...
BoschlooExactResult = make_dataclass("BoschlooExactResult", [("statistic", float), ("pvalue", float)])
...
def boschloo_exact(table, alternative="two-sided", n=32):
    ...
    if alternative == "less":
        ...
        msg = f"`alternative` should be one of {'two-sided', 'less', 'greater'}," f" found {alternative!r}"
        raise ValueError(msg)
...
class TukeyHSDResult:
    ...
    def __str__(self):
        ...
        s = "Tukey's HSD Pairwise Group Comparisons" f" ({self._ci_cl*100:.1f}% Confidence Interval)\n"
