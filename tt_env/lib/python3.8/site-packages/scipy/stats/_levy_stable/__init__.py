...
def _Phi_Z0(alpha, t):
    return (
        -np.tan(np.pi * alpha / 2) * (np.abs(t) ** (1 - alpha) - 1) if alpha != 1 else -2.0 * np.log(np.abs(t)) / np.pi
    )
...
def _pdf_single_value_piecewise_Z0(x0, alpha, beta, **kwds):
    ...
    return _pdf_single_value_piecewise_post_rounding_Z0(x0, alpha, beta, quad_eps)
...
def _cdf_single_value_piecewise_Z0(x0, alpha, beta, **kwds):
    ...
    return _cdf_single_value_piecewise_post_rounding_Z0(x0, alpha, beta, quad_eps)
...
def _cdf_single_value_piecewise_post_rounding_Z0(x0, alpha, beta, quad_eps):
    ...
    if (alpha == 1 and beta < 0) or x0 < zeta:
        ...
        return 1 - _cdf_single_value_piecewise_post_rounding_Z0(-x0, alpha, -beta, quad_eps)
    elif x0 == zeta:
        return 0.5 - xi / np.pi
...
def _rvs_Z1(alpha, beta, size=None, random_state=None):
    ...
    def otherwise(alpha, beta, TH, aTH, bTH, cosTH, tanTH, W):
        ...
        res3 = val3 * ((np.cos(aTH) + np.sin(aTH) * tanTH - val0 * (np.sin(aTH) - np.cos(aTH) * tanTH)) / W) ** (
            1.0 / alpha
        )
        return res3
    ...
    def _cdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            ...
            if cdf_single_value_method is not None:
                ...
                data_out[data_mask] = np.array([f.integral(self.a, x_1) for x_1 in _x]).reshape(
                    data_out[data_mask].shape
                )
...
def pdf_from_cf_with_fft(cf, h=0.01, q=9, level=3):
    ...
    if level > 1:
        ...
        s1 = np.sum(
            ...
            * np.exp(1j * np.pi * indices / (n - 1) - 2 * 1j * np.pi * indices * steps / (N * (n - 1))),
            axis=0,
        )
    else:
