...
def _chk_size(a, b):
    ...
    (na, nb) = (a.size, b.size)
    if na != nb:
        raise ValueError("The size of the input array should match!" " (%s <> %s)" % (na, nb))
    return (a, b, na)
...
def kendalltau(x, y, use_ties=True, use_missing=False, method="auto", alternative="two-sided"):
    ...
    (rx, ry) = (rx[idx], ry[idx])
    C = np.sum(
        [((ry[i + 1 :] > ry[i]) * (rx[i + 1 :] > rx[i])).filled(0).sum() for i in range(len(ry) - 1)], dtype=float
    )
    D = np.sum(
        [((ry[i + 1 :] < ry[i]) * (rx[i + 1 :] > rx[i])).filled(0).sum() for i in range(len(ry) - 1)], dtype=float
    )
    ...
    if not xties and not yties and method == "exact":
        ...
        if use_ties:
            ...
            v1 = np.sum([v * k * (k - 1) for (k, v) in xties.items()], dtype=float) * np.sum(
                [v * k * (k - 1) for (k, v) in yties.items()], dtype=float
            )
            v1 /= 2.0 * n * (n - 1)
            if n > 2:
                v2 = np.sum([v * k * (k - 1) * (k - 2) for (k, v) in xties.items()], dtype=float) * np.sum(
    ...
    if m is not nomask:
        ...
        if np.any(~m):
            ...
            result = stats_LinregressResult(
                slope=None, intercept=None, rvalue=None, pvalue=None, stderr=None, intercept_stderr=None
            )
    else:
