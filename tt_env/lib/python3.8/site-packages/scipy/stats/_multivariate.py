...
class multivariate_normal_gen(multi_rv_generic):
    ...
    def logcdf(
        self, x, mean=None, cov=1, allow_singular=False, maxpts=None, abseps=1e-5, releps=1e-5, *, lower_limit=None
    ):
...
class multivariate_normal_frozen(multi_rv_frozen):
    def __init__(self, mean=None, cov=1, allow_singular=False, seed=None, maxpts=None, abseps=1e-5, releps=1e-5):
        """Create a frozen multivariate normal distribution.
    ...
    def cdf(self, x, *, lower_limit=None):
        x = self._dist._process_quantiles(x, self.dim)
        out = self._dist._cdf(
            x, self.mean, self.cov_object.covariance, self.maxpts, self.abseps, self.releps, lower_limit
        )
        return _squeeze_output(out)
...
class matrix_normal_frozen(multi_rv_frozen):
    ...
    def logpdf(self, X):
        ...
        out = self._dist._logpdf(
            self.dims, X, self.mean, self.rowpsd.U, self.rowpsd.log_pdet, self.colpsd.U, self.colpsd.log_pdet
        )
        return _squeeze_output(out)
...
def _dirichlet_check_parameters(alpha):
    ...
    if np.min(alpha) <= 0:
        ...
        raise ValueError("Parameter vector 'a' must be one dimensional, " "but a.shape = %s." % (alpha.shape,))
    return alpha
...
def _dirichlet_check_input(alpha, x):
    ...
    if (np.abs(np.sum(x, 0) - 1.0) > 10e-10).any():
        raise ValueError(
            "The input vector 'x' must lie within the normal " "simplex. but np.sum(x, 0) = %s." % np.sum(x, 0)
        )
...
class wishart_gen(multi_rv_generic):
    ...
    def _process_parameters(self, df, scale):
        ...
        if scale.ndim == 0:
            ...
            raise ValueError(
                "Array 'scale' must be square if it is two" " dimensional, but scale.scale = %s." % str(scale.shape)
            )
        elif scale.ndim > 2:
    ...
    def _process_quantiles(self, x, dim):
        ...
        if x.ndim == 1:
            ...
            if not x.shape[0] == x.shape[1]:
                raise ValueError(
                    "Quantiles must be square if they are two" " dimensional, but x.shape = %s." % str(x.shape)
                )
            x = x[:, :, np.newaxis]
        elif x.ndim == 3:
            if not x.shape[0] == x.shape[1]:
        ...
        if not x.shape[0:2] == (dim, dim):
            raise ValueError(
                "Quantiles have incompatible dimensions: should" " be %s, got %s." % ((dim, dim), x.shape[0:2])
            )
    ...
    def _standard_rvs(self, n, shape, dim, df, random_state):
        ...
        variances = (
            np.r_[[random_state.chisquare(df - (i + 1) + 1, size=n) ** 0.5 for i in range(dim)]]
            .reshape((dim,) + shape[::-1])
            .T
        )
    ...
    def _entropy(self, dim, df, log_det_scale):
        ...
        return (
            ...
            - 0.5 * (df - dim - 1) * np.sum([psi(0.5 * (df + 1 - (i + 1))) for i in range(dim)])
            + 0.5 * df * dim
        )
...
for name in ["logpmf", "pmf", "mean", "cov", "rvs"]:
    ...
    method_frozen.__doc__ = doccer.docformat(method.__doc__, multinomial_docdict_noparams)
...
class unitary_group_gen(multi_rv_generic):
    ...
    def rvs(self, dim, size=1, random_state=None):
        ...
        z = (
            1
            / math.sqrt(2)
            * (random_state.normal(size=size + (dim, dim)) + 1j * random_state.normal(size=size + (dim, dim)))
        )
        q, r = np.linalg.qr(z)
        # The last two dimensions are the rows and columns of R matrices.
        # Extract the diagonals. Note that this eliminates a dimension.
...
class random_table_gen(multi_rv_generic):
    ...
    def logpmf(self, x, row, col):
        ...
        res[mask] = (
            np.sum(lnfac(r), axis=-1) + np.sum(lnfac(c), axis=-1) - lnfac(n) - np.sum(lnfac(x[mask]), axis=(-1, -2))
        )
        res[~mask] = -np.inf
    ...
    def _process_rvs_method(cls, method, r, c, n):
        ...
        try:
            ...
            raise ValueError(f"'{method}' not recognized, " f"must be one of {set(known_methods)}")
