...
def check_kurt_expect(distfn, arg, m, v, k, msg):
    if np.isfinite(k):
        ...
        npt.assert_allclose(m4e, (k + 3.0) * np.power(v, 2), atol=1e-5, rtol=1e-5, err_msg=msg + " - kurtosis")
...
def check_cmplx_deriv(distfn, arg):
    ...
    for x in x_cast:
        ...
        assert_allclose(deriv(distfn.logpdf, x, *arg), deriv(distfn.pdf, x, *arg) / distfn.pdf(x, *arg), rtol=1e-5)
...
def check_deprecation_warning_gh5982_interval(distfn, arg, distname):
    ...
    def my_interval(*args, **kwds):
        return (distfn.ppf(0.25, *args, **kwds), distfn.ppf(0.75, *args, **kwds))
