...
fail_fit_test_mm = (
    ...
    + ["genhyperbolic", "johnsonsu", "ksone", "kstwo", "nct", "pareto", "powernorm", "powerlognorm"]
    + ["pearson3"]
)
...
fail_fit_fix_test_mm = (
    ...
    + ["genhyperbolic", "johnsonsu", "ksone", "kstwo", "pareto", "powernorm", "powerlognorm"]
    + ["pearson3"]
)
...
def cases_test_moments():
    ...
    for distname, arg in distcont[:] + histogram_test_instances:
        ...
        if distname in distxslow_test_moments:
            yield pytest.param(distname, arg, True, True, True, marks=pytest.mark.xslow(reason="too slow"))
            continue
        ...
        if not cond1 or not cond2:
            ...
            yield pytest.param(distname, arg, True, True, True, marks=[pytest.mark.xfail] + marks)


@pytest.mark.slow
@pytest.mark.parametrize("distname,arg,normalization_ok,higher_ok," "is_xfailing", cases_test_moments())
...
@pytest.mark.parametrize(
    "x,n,sf,cdf,pdf,rtol",
    [
        (2.0e-5, 1000000000, 0.44932297307934442379, 0.55067702692065557621, 35946.137394996276407, 5e-15),
        (2.0e-9, 1000000000, 0.99999999061111115519, 9.3888888448132728224e-9, 8.6666665852962971765, 5e-14),
        (5.0e-4, 1000000000, 7.1222019433090374624e-218, 1.0, 1.4244408634752704094e-211, 5e-14),
    ],
)
...
def check_cdf_ppf(distfn, arg, msg):
    ...
    npt.assert_almost_equal(
        distfn.cdf(distfn.ppf(values, *arg), *arg), values, decimal=DECIMAL, err_msg=msg + " - cdf-ppf roundtrip"
    )
