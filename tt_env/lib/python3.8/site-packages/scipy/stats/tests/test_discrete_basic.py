...
def test_ppf_with_loc(dist, args):
    ...
    for loc in re_locs:
        npt.assert_array_equal(
            [_a - 1 + loc, _b + loc], [distfn.ppf(0.0, *args, loc=loc), distfn.ppf(1.0, *args, loc=loc)]
        )
...
def check_cdf_ppf(distfn, arg, supp, msg):
    ...
    npt.assert_array_equal(distfn.ppf(distfn.cdf(supp, *arg) - 1e-8, *arg), supp, msg + "-roundtrip")

    if not hasattr(distfn, "xk"):
        _a, _b = distfn.support(*arg)
        supp1 = supp[supp < _b]
        npt.assert_array_equal(
            distfn.ppf(distfn.cdf(supp1, *arg) + 1e-8, *arg), supp1 + distfn.inc, msg + " ppf-cdf-next"
        )
