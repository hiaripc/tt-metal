...
@pytest.mark.parametrize(
    "dist, params", [(stats.norm, (0.5, 2.5)), (stats.binom, (10, 0.3, 2))]  # type: ignore[attr-defined] # noqa
...
def cases_test_fit_mse():
    ...
    skip_basic_fit = {
        ...
        "vonmises",
    }  # can have negative CDF; doesn't play nice
    slow_basic_fit = {
        "wald",
        "genextreme",
        "anglit",
...
def cases_test_fitstart():
    for distname, shapes in dict(distcont).items():
        if not isinstance(distname, str) or distname in {"studentized_range", "recipinvgauss"}:  # slow
            continue
...
def assert_nlff_less_or_close(dist, data, params1, params0, rtol=1e-7, atol=0, nlff_name="nnlf"):
...
class TestGoodnessOfFit:
    ...
    def test_params_effects(self):
        ...
        assert not np.allclose(res2.fit_result.params.c, res1.fit_result.params.c, rtol=1e-8)
        assert not np.allclose(res2.null_distribution, res1.null_distribution, rtol=1e-8)
