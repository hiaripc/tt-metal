...
import functools
import pytest
from numpy.testing import assert_, assert_equal, assert_allclose, assert_almost_equal  # avoid new uses
...
class TestCvm:
    ...
    def test_cdf_4(self):
        assert_allclose(_cdf_cvm([0.02983, 0.04111, 0.12331, 0.94251], 4), [0.01, 0.05, 0.5, 0.999], atol=1e-4)

    def test_cdf_10(self):
        assert_allclose(_cdf_cvm([0.02657, 0.03830, 0.12068, 0.56643], 10), [0.01, 0.05, 0.5, 0.975], atol=1e-4)

    def test_cdf_1000(self):
        assert_allclose(_cdf_cvm([0.02481, 0.03658, 0.11889, 1.16120], 1000), [0.01, 0.05, 0.5, 0.999], atol=1e-4)

    def test_cdf_inf(self):
        assert_allclose(_cdf_cvm([0.02480, 0.03656, 0.11888, 1.16204]), [0.01, 0.05, 0.5, 0.999], atol=1e-4)
...
class TestMannWhitneyU:
    ...
    pm5 = {
        ...
        5: [0.004, 0.008, 0.016, 0.028, 0.048, 0.075, 0.111, 0.155, 0.21, 0.274, 0.345, 0.421, 0.5, 0.579],
    }
    pm6 = {
    ...
    def test_exact_distribution(self):
        ...
        for n, table in p_tables.items():
            for m, p in table.items():
                ...
                assert_allclose(
                    _mwu_state.cdf(k=u2, m=m, n=n) + _mwu_state.sf(k=u2, m=m, n=n) - _mwu_state.pmf(k=u2, m=m, n=n), 1
                )
    ...
    cases_11355 = [
        ...
        ([1, 2, 3, 4], [3, 6, 7, 8, np.inf, np.inf, 2, 1, 4, 4, 5], 8.5, 0.08735617507695),
        ([1, 2, np.inf, 4], [3, 6, 7, 8, np.inf, 3, 2, 1, 4, 4, 5], 17.5, 0.5988856695752),
        ([1, 2, np.inf, 4], [3, 6, 7, 8, np.inf, np.inf, 2, 1, 4, 4, 5], 16, 0.4687165824462),
        ([1, np.inf, np.inf, 4], [3, 6, 7, 8, np.inf, np.inf, 2, 1, 4, 4, 5], 24.5, 0.7912517950119),
    ]
...
class TestBarnardExact:
    ...
    def test_less_greater(self, input_sample, expected, alternative):
        ...
        statistic, pvalue = res.statistic, res.pvalue
        assert_allclose([statistic, pvalue], [expected_stat, less_pvalue_expect], atol=1e-7)
...
class TestBoschlooExact:
    ...
    def test_raises(self):
        ...
        # Test value error on wrong alternative param
        error_msg = r"`alternative` should be one of \('two-sided', 'less', " r"'greater'\), found .*"
        with assert_raises(ValueError, match=error_msg):
...
class TestCvm_2samp:
    ...
    @pytest.mark.parametrize(
        "statistic, m, n, pval",
        [(710, 5, 6, 48.0 / 462), (1897, 7, 7, 117.0 / 1716), (576, 4, 6, 2.0 / 210), (1764, 6, 7, 2.0 / 1716)],
    )
...
class TestTukeyHSD:
    ...
    @pytest.mark.parametrize(
        ...
        ids=["equal size sample", "unequal sample size", "extreme sample size differences"],
    )
    ...
    @pytest.mark.parametrize(
        "data,res_expect_str,atol",
        ((data_same_size, matlab_sm_siz, 1e-12), (data_diff_size, matlab_diff_sz, 1e-7)),
