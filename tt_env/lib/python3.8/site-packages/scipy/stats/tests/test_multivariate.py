...
class TestMultivariateNormal:
    ...
    def test_R_values(self):
        ...
        r_pdf = np.array([0.0002214706, 0.0013819953, 0.0049138692, 0.0103803050, 0.0140250800])
...
class TestMatrixNormal:
    ...
    def test_samples(self):
        ...
        expected = np.array(
            [
                [[1.56228264238181, -1.24136424071189], [2.46865788392114, 6.22964440489445]],
                [[3.86405716144353, 10.73714311429529], [2.59428444080606, 5.79987854490876]],
            ]
        )
...
class TestWishart:
    def test_scale_dimensions(self):
        ...
        scales = [1, [1], np.array(1), np.r_[1], np.array(1, ndmin=2)]  # scalar  # iterable  # 0-dim  # 1-dim  # 2-dim
        for scale in scales:
            w = wishart(1, scale)
        ...
        scales = [[1, 2], np.r_[1, 2], np.array([[1, 0], [0, 2]])]  # iterable  # 1-dim  # 2-dim
        for scale in scales:
            w = wishart(2, scale)
            assert_equal(w.scale, true_scale)
            assert_equal(w.scale.shape, true_scale.shape)
    ...
    def test_quantile_dimensions(self):
        ...
        X = [[1, 2, 3], np.r_[1, 2, 3], np.array([1, 2, 3], ndmin=3)]  # iterable  # 1-dim  # 3-dim
...
class TestMultinomial:
    ...
    def test_entropy_broadcasting(self):
        ...
        assert_allclose(
            ent2,
            [[binom.entropy(7, 0.3), binom.entropy(7, 0.4)], [binom.entropy(8, 0.3), binom.entropy(8, 0.4)]],
            rtol=1e-8,
        )
...
class TestOrthoGroup:
    def test_reproducibility(self):
        ...
        expected = np.array(
            [[0.381686, -0.090374, 0.919863], [0.905794, -0.161537, -0.391718], [-0.183993, -0.98272, -0.020204]]
        )
        assert_array_almost_equal(x, expected)
