...
def test_bootstrap_against_theory(method):
    ...
    res = bootstrap((data,), np.mean, n_resamples=5000, confidence_level=alpha, method=method, random_state=0)
...
def test_bootstrap_vectorized_3samp(method, axis):
    ...
    res2 = bootstrap(
        (x, y, z), statistic_1d, vectorized=False, axis=axis, n_resamples=100, method=method, random_state=0
    )
    assert_allclose(res1.confidence_interval, res2.confidence_interval)
    assert_allclose(res1.standard_error, res2.standard_error)
...
def test_bootstrap_vectorized_1samp(method, axis):
    ...
    x = np.random.rand(4, 5)
    res1 = bootstrap(
        (x,), statistic, vectorized=True, axis=axis, n_resamples=100, batch=None, method=method, random_state=0
    )
    res2 = bootstrap(
        (x,), statistic_1d, vectorized=False, axis=axis, n_resamples=100, batch=10, method=method, random_state=0
    )
    assert_allclose(res1.confidence_interval, res2.confidence_interval)
    assert_allclose(res1.standard_error, res2.standard_error)
...
def test_bootstrap_gh15678(method):
    ...
    ref = bootstrap(
        data, stats.skew, method=method, n_resamples=100, random_state=np.random.default_rng(9563), vectorized=False
    )
...
def test_re_boostrap(additional_resamples):
    ...
    res = stats.bootstrap(
        (x,), np.mean, n_resamples=n2, random_state=rng, confidence_level=0.90, method="BCa", bootstrap_result=res
    )

    rng = np.random.default_rng(296689032789913033)
    ref = stats.bootstrap((x,), np.mean, n_resamples=n3, random_state=rng, confidence_level=0.90, method="BCa")
...
def test_vector_valued_statistic_gh17715():
    ...
    data = [[4, 0, 0, 2], [2, 1, 2, 1], [0, 6, 0, 0], [0, 6, 3, 0], [0, 8, 1, 0]]  # (tp, tn, fp, fn)
...
class TestPermutationTest:
    ...
    @pytest.mark.parametrize(
        ...
        [("pairings", special.factorial(3) ** 2), ("samples", 2**3), ("independent", special.binom(6, 3))],
    )
    ...
    def test_against_kendalltau(self):
        ...
        res = permutation_test(
            (x,), statistic1d, permutation_type="pairings", n_resamples=np.inf, random_state=self.rng
        )
    ...
    def test_against_fisher_exact(self, alternative):
        ...
        res = permutation_test(
            (x,), statistic, permutation_type="pairings", n_resamples=np.inf, alternative=alternative, random_state=rng
        )
    ...
    @pytest.mark.parametrize(
        ...
        (("less", 0.9708333333333), ("greater", 0.05138888888889), ("two-sided", 0.1027777777778)),
    )
    def test_against_spearmanr_in_R(self, alternative, expected_pvalue):
        ...
        res = permutation_test(
            (x,), statistic1d, permutation_type="pairings", n_resamples=np.inf, alternative=alternative
        )
...
def test_all_partitions_concatenated():
    ...
    expected = np.product([special.binom(sum(n[i:]), sum(n[i + 1 :])) for i in range(len(n) - 1)])
...
def test_parameter_vectorized(fun_name):
    ...
    fun_options = {
        "bootstrap": {"data": (sample,), "random_state": rng, "method": "percentile"},
        "permutation_test": {"data": (sample,), "random_state": rng, "permutation_type": "samples"},
