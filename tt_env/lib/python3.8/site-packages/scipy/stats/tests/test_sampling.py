...
# domains.
@pytest.mark.parametrize("domain, err, msg", bad_domains + bad_sized_domains + nan_domains)  # type: ignore[operator]
...
@pytest.mark.parametrize("size", [None, 0, (0,), 1, (10, 3), (2, 3, 4, 5), (0, 0), (0, 1)])
...
@pytest.mark.parametrize("method", ["SimpleRatioUniforms", "NumericalInversePolynomial", "TransformedDensityRejection"])
...
class TestQRVS:
    ...
    sizes = [(None, tuple()), (1, (1,)), (4, (4,)), ((4,), (4,)), ((2, 4), (2, 4))]  # type: ignore
...
class TestTransformedDensityRejection:
    ...
    def test_bad_construction_points_scalar(self, construction_points):
        with pytest.raises(ValueError, match=r"`construction_points` must be " r"a positive integer."):
    ...
    def test_bad_construction_points_array(self):
        ...
        construction_points = []
        with pytest.raises(
            ValueError, match=r"`construction_points` must " r"either be a " r"scalar or a non-empty array."
        ):
            TransformedDensityRejection(StandardNormal(), construction_points=construction_points)

        # construction_points not monotonically increasing
        construction_points = [1, 1, 1, 1, 1, 1]
        with pytest.warns(RuntimeWarning, match=r"33 : starting points not " r"strictly monotonically " r"increasing"):
            TransformedDensityRejection(StandardNormal(), construction_points=construction_points)
        ...
        with pytest.warns(RuntimeWarning, match=r"50 : starting point out of " r"domain"):
...
class TestDiscreteAliasUrn:
    ...
    def test_bad_args(self):
        msg = r"`domain` must be provided when the " r"probability vector is not available."
...
class TestNumericalInversePolynomial:
    ...
    fail_dists = ["ncf", "pareto", "chi2", "fatiguelife", "halfgennorm", "gibrat", "lognorm", "ncx2", "t"]
...
class TestNumericalInverseHermite:
    ...
    def test_inaccurate_CDF(self):
        ...
        match = (
            "98 : one or more intervals very short; possibly due to " "numerical problems with a pole or very flat tail"
        )
...
class TestSimpleRatioUniforms:
    ...
    def test_basic(self, dist, mv_ex):
        ...
        check_cont_samples(rng, dist, mv_ex)
        rng = SimpleRatioUniforms(dist, mode=dist.mode, cdf_at_mode=dist.cdf(dist.mode), random_state=42)
        check_cont_samples(rng, dist, mv_ex)
