...
def assert_does_not_leak(f, iterations=10, check_interval=1, tolerance=5):
    ...
    for i in range(iterations):
        ...
        if i % check_interval == 0:
            ...
            if diff > tolerance:
                raise Exception("Memory increased by {0} megabytes after {1} " "iterations".format(diff, i + 1))
    gc.collect()
    usage = memory_profiler.memory_usage()[0]
    diff = usage - baseline
...
def test_leak3():
    ...
    writer = pq.ParquetWriter("leak_test_" + rands(5) + ".parquet", table.schema)
...
def test_ARROW_8801():
    x = pd.to_datetime(np.random.randint(0, 2**32, size=2**20, dtype=np.int64), unit="ms", utc=True)
    table = pa.table(pd.DataFrame({"x": x}))

    assert_does_not_leak(
        lambda: table.to_pandas(split_blocks=False), iterations=1000, check_interval=50, tolerance=1000
    )
