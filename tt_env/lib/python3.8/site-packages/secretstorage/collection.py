...
class Collection:
    ...
    def __init__(
        self, connection: DBusConnection, collection_path: str = DEFAULT_COLLECTION, session: Optional[Session] = None
    ) -> None:
    ...
    def create_item(
        ...
        item_path, prompt = self._collection.call("CreateItem", "a{sv}(oayays)b", properties, _secret, replace)
        if len(item_path) > 1:
...
def create_collection(
    connection: DBusConnection, label: str, alias: str = "", session: Optional[Session] = None
) -> Collection:
