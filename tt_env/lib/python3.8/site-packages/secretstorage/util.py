...
def open_session(connection: DBusConnection) -> Session:
    ...
    session = Session()
    try:
        output, result = service.call("OpenSession", "sv", ALGORITHM_DH, ("ay", int_to_bytes(session.my_public_key)))
    except DBusErrorResponse as resp:
        if resp.name != DBUS_NOT_SUPPORTED:
            raise
        output, result = service.call("OpenSession", "sv", ALGORITHM_PLAIN, ("s", ""))
