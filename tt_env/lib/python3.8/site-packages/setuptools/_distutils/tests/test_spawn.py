...
class TestSpawn(support.TempdirManager):
    ...
    def test_find_executable(self, tmp_path):
        ...
        with os_helper.EnvironmentVarGuard() as env:
            env["PATH"] = ""
            with mock.patch("distutils.spawn.os.confstr", return_value=tmp_dir, create=True), mock.patch(
                "distutils.spawn.os.defpath", tmp_dir
            ):
                rv = find_executable(program)
                assert rv is None
        ...
        with os_helper.EnvironmentVarGuard() as env:
            env["PATH"] = os.pathsep
            with mock.patch("distutils.spawn.os.confstr", return_value="", create=True), mock.patch(
                "distutils.spawn.os.defpath", ""
            ):
                rv = find_executable(program)
                assert rv is None
        ...
        with os_helper.EnvironmentVarGuard() as env:
            ...
            # without confstr
            with mock.patch("distutils.spawn.os.confstr", side_effect=ValueError, create=True), mock.patch(
                "distutils.spawn.os.defpath", tmp_dir
            ):
                rv = find_executable(program)
                assert rv == filename
