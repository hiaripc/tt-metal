...
TypeCheckerCallable: TypeAlias = Callable[[Any, Any, Tuple[Any, ...], TypeCheckMemo], Any]
TypeCheckLookupCallback: TypeAlias = Callable[[Any, Tuple[Any, ...], Tuple[Any, ...]], Optional[TypeCheckerCallable]]
...
protocol_check_cache: WeakKeyDictionary[type[Any], dict[type[Any], TypeCheckError | None]] = WeakKeyDictionary()
        ...
        if isinstance(argument_types, list) and not any(type(item) is ParamSpec for item in argument_types):
            ...
            if unfulfilled_kwonlyargs:
                raise TypeCheckError(
                    f"has mandatory keyword-only arguments in its declaration: " f'{", ".join(unfulfilled_kwonlyargs)}'
                )
...
def check_tuple(
    ...
    if use_ellipsis:
        ...
        if len(value) != len(tuple_params):
            raise TypeCheckError(
                f"has wrong number of elements (expected {len(tuple_params)}, got " f"{len(value)} instead)"
            )
    ...
    *,
    ...
    if origin_type.__bound__ is not None:
        ...
        for constraint in origin_type.__constraints__:
            ...
            raise TypeCheckError(f"does not match any of the constraints " f"({formatted_constraints})")
...
def check_protocol(
    ...
    for attrname in dir(origin_type):
        ...
        if callable(member):
            ...
            return_annotation = (
                signature.return_annotation if signature.return_annotation is not Parameter.empty else Any
            )
...
def check_self(
    ...
    if isclass(value):
        if not issubclass(value, memo.self_type):
            raise TypeCheckError(f"is not an instance of the self type " f"({qualified_name(memo.self_type)})")
...
if sys.version_info >= (3, 11):
    origin_type_checkers.update({typing.LiteralString: check_literal_string, typing.Self: check_self})
