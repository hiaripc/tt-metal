        ...
        if self.node:
            for index, child in enumerate(self.node.body):
                if isinstance(child, ImportFrom) and child.module == "__future__":
                    ...
                    isinstance(child, Expr) and isinstance(child.value, Constant) and isinstance(child.value.value, str)
                ):
                    continue  # docstring
        ...
        if node.slice:
            ...
            if isinstance(slice_value, Tuple):
                ...
                if self._memo.name_matches(node.value, "typing.Union") and any(
                    item is None or (isinstance(item, expr) and self._memo.name_matches(item, *anytype_names))
                    for item in items
                ):
                    return None
                ...
                if detector.contains_yields and new_memo.name_matches(return_annotation, *generator_names):
                    if isinstance(return_annotation, Subscript):
                        ...
                        if isinstance(annotation_slice, Index):
                            annotation_slice = annotation_slice.value  # type: ignore[attr-defined]
        ...
        if isinstance(self._memo.node, (FunctionDef, AsyncFunctionDef)):
            ...
            if check_required:
                ...
                if len(targets) == 1 and len(targets[0]) == 1:
                    ...
                    targets_arg = List(
                        [Dict(keys=list(target), values=list(target.values())) for target in targets],
                        ctx=Load(),
                    )
                    node.value = Call(
                        func_name,
