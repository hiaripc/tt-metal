...
try:  # packaged lib (in _soundfile_data which should be on python path)
    if _sys.platform == "darwin":
        ...
        if _architecture()[0] == "64bit":
            ...
            raise OSError("no packaged library for Windows {} {}".format(_architecture(), _machine()))
    ...
    try:  # system-wide libsndfile:
        ...
        if _sys.platform == "darwin" and _machine() == "arm64":
            _hbrew_path = "/opt/homebrew/lib/" if _os.path.isdir("/opt/homebrew/lib/") else "/usr/local/lib/"
...
def write(
    ...
    with SoundFile(
        file, "w", samplerate, channels, subtype, endian, format, closefd, compression_level, bitrate_mode
    ) as f:
        f.write(data)
...
class SoundFile(object):
    ...
    def __repr__(self):
        compression_setting = (
            ", compression_level={0}".format(self.compression_level) if self.compression_level is not None else ""
        )
        compression_setting += ", bitrate_mode='{0}'".format(self.bitrate_mode) if self.bitrate_mode is not None else ""
        return (
    ...
    def truncate(self, frames=None):
        ...
        if frames is None:
            frames = self.tell()
        err = _snd.sf_command(
            self._file, _snd.SFC_FILE_TRUNCATE, _ffi.new("sf_count_t*", frames), _ffi.sizeof("sf_count_t")
        )
        if err:
        ...
        if array.ndim not in (1, 2):
            raise ValueError(
                "Invalid shape: {0!r} ({1})".format(
                    array.shape, "0 dimensions not supported" if array.ndim < 1 else "too many dimensions"
                )
            )
    ...
    def _set_bitrate_mode(self, bitrate_mode):
        ...
        err = _snd.sf_command(
            self._file, _snd.SFC_SET_BITRATE_MODE, pointer_bitrate_mode, _ffi.sizeof(pointer_bitrate_mode)
        )
        if err != _snd.SF_TRUE:
            err = _snd.sf_error(self._file)
...
def _create_info_struct(file, mode, samplerate, channels, format, subtype, endian):
    ...
    if "r" not in mode or format.upper() == "RAW":
        ...
        if any(arg is not None for arg in (samplerate, channels, original_format, subtype, endian)):
            raise TypeError(
                "Not allowed for existing files (except 'RAW'): " "samplerate, channels, format, subtype, endian"
            )
    return info
