...
# Regular expressions
# CSS escape pattern
RE_CSS_ESC = re.compile(rf"(?:(\\[a-f0-9]{{1,6}}{WSC}?)|(\\[^\r\n\f])|(\\$))", re.I)
RE_CSS_STR_ESC = re.compile(rf"(?:(\\[a-f0-9]{{1,6}}{WS}?)|(\\[^\r\n\f])|(\\$)|(\\{NEWLINE}))", re.I)
# Pattern to break up `nth` specifiers
RE_NTH = re.compile(rf"(?P<s1>[-+])?(?P<a>[0-9]+n?|n)(?:(?<=n){WSC}*(?P<s2>[-+]){WSC}*(?P<b>[0-9]+))?", re.I)
# Pattern to iterate multiple values.
RE_VALUES = re.compile(rf"(?:(?P<value>{VALUE})|(?P<split>{WSC}*,{WSC}*))", re.X)
# Whitespace checks
RE_WS = re.compile(WS)
...
def _cached_css_compile(
    pattern: str, namespaces: ct.Namespaces | None, custom: ct.CustomSelectors | None, flags: int
) -> cm.SoupSieve:
    """Cached CSS compile."""

    custom_selectors = process_custom(custom)
    return cm.SoupSieve(
        pattern, CSSParser(pattern, custom=custom_selectors, flags=flags).process_selectors(), namespaces, custom, flags
    )
...
class CSSParser:
    ...
    def parse_pseudo_class_custom(self, sel: _Selector, m: Match[str], has_selector: bool) -> bool:
        ...
        if selector is None:
            raise SelectorSyntaxError(
                f"Undefined custom selector '{pseudo}' found at position {m.end(0)}", self.pattern, m.end(0)
            )
    ...
    def parse_pseudo_nth(
        self, sel: _Selector, m: Match[str], has_selector: bool, iselector: Iterator[tuple[str, Match[str]]]
    ) -> bool:
        """Parse `nth` pseudo."""
    ...
    def parse_pseudo_open(
        self, sel: _Selector, name: str, has_selector: bool, iselector: Iterator[tuple[str, Match[str]]], index: int
    ) -> bool:
    ...
    def parse_has_combinator(
        self, sel: _Selector, m: Match[str], has_selector: bool, selectors: list[_Selector], rel_type: str, index: int
            ...
            while True:
                ...
                if key == "at_rule":
                    ...
                    if not has_selector:
                        if not is_forgive:
                            raise SelectorSyntaxError(
                                f"Expected a selector at position {m.start(0)}", self.pattern, m.start(0)
                            )
                        sel.no_match = True
                    if is_open:
                        closed = True
                        ...
                        raise SelectorSyntaxError(
                            f"Unmatched pseudo-class close at position {m.start(0)}", self.pattern, m.start(0)
                        )
                elif key == "combine":
                    if is_relative:
                    ...
                    if has_selector:
                        raise SelectorSyntaxError(
                            f"Tag name found at position {m.start(0)} instead of at the start", self.pattern, m.start(0)
                        )
