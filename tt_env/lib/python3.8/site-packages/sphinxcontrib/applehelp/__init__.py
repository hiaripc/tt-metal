...
if sphinx.version_info[:2] >= (6, 1):
    ...
    from sphinx.util import SkipProgressMessage, progress_message  # type: ignore[attr-defined,no-redef]
...
class AppleHelpBuilder(StandaloneHTMLBuilder):
    ...
    def init(self) -> None:
        ...
        if self.config.applehelp_bundle_id is None:
            raise SphinxError(__("You must set applehelp_bundle_id before " "building Apple Help output"))

        self.bundle_path = path.join(self.outdir, self.config.applehelp_bundle_name + ".help")  # type: ignore
        self.outdir = path.join(self.bundle_path, "Contents", "Resources", self.config.applehelp_locale + ".lproj")
...
def setup(app: Sphinx) -> Dict[str, Any]:
    ...
    app.add_config_value("applehelp_bundle_name", lambda self: make_filename(self.project), "applehelp")
    ...
    app.add_config_value(
        "applehelp_kb_product", lambda self: "%s-%s" % (make_filename(self.project), self.release), "applehelp"
    )
    ...
    app.add_config_value("applehelp_stopwords", lambda self: self.language or "en", "applehelp")
    ...
    app.add_config_value(
        "applehelp_codesign_identity", lambda self: environ.get("CODE_SIGN_IDENTITY", None), "applehelp"
    )
    app.add_config_value(
        "applehelp_codesign_flags", lambda self: shlex.split(environ.get("OTHER_CODE_SIGN_FLAGS", "")), "applehelp"
    )
