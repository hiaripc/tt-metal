...
from typing import Iterator, List, Tuple, Optional, NamedTuple, Any, Iterable, Callable, Union, Sequence
...
MarkerInLine = NamedTuple("MarkerInLine", [("position", int), ("is_start", bool), ("string", str)])
    ...
    def _raw_split_into_pieces(
        ...
        for name, body in ast.iter_fields(stmt):
            if (
                ...
                and body
                and isinstance(body[0], (ast.stmt, ast.ExceptHandler, getattr(ast, "match_case", ())))
            ):
...
class FrameInfo(object):
    ...
    def filename(self) -> str:
        ...
        if os.path.isabs(result) or (result.startswith("<") and result.endswith(">")):
            return result
    ...
    def lines(self) -> List[Union[Line, LineGap, BlankLineRange]]:
        ...
        for i, piece in enumerate(pieces):
            if i == 1 and self.scope and pieces[0] == self.scope_pieces[0] and pieces[1] != self.scope_pieces[1]:
    ...
    def variables(self) -> List[Variable]:
        ...
        scope = self.scope
        node_values = [
            pair for pair in evaluator.find_expressions(scope) if is_expression_interesting(*pair)
    ...
    def variables_in_lines(self) -> List[Variable]:
        ...
        return unique_in_order(
            var for line in self.lines if isinstance(line, Line) for var, node in self.variables_by_lineno[line.lineno]
        )
    ...
    def variables_in_executing_piece(self) -> List[Variable]:
        ...
        """
        return unique_in_order(var for lineno in self.executing_piece for var, node in self.variables_by_lineno[lineno])
