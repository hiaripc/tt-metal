...
class Formatter:
        ...
        *,
        ...
        if pygmented and not options.pygments_formatter:
            if show_executing_node:
                pygments_style = style_with_executing_node(pygments_style, executing_node_modifier)

            if pygments_formatter_cls is None:
                from pygments.formatters.terminal256 import Terminal256Formatter as pygments_formatter_cls
        ...
        if not self.show_linenos and self.options.blank_lines == BlankLines.SINGLE:
            raise ValueError("BlankLines.SINGLE option can only be used when show_linenos=True")
    ...
    def format_frame_header(self, frame_info: FrameInfo) -> str:
        return ' File "{frame_info.filename}", line {frame_info.lineno}, in {name}\n'.format(
            ...
            name=(frame_info.executing.code_qualname() if self.use_code_qualname else frame_info.code.co_name),
        )
    ...
    def format_line(self, line: Line) -> str:
        ...
        if self.show_executing_node and not self.pygmented:
            for line_range in line.executing_node_ranges:
                ...
                if end > start:
                    result += " " * (start + len(prefix)) + self.executing_node_underline * (end - start) + "\n"
        return result
