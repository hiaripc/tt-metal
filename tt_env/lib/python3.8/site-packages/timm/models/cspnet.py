...
default_cfgs = {
    ...
    "cs3darknet_s": _cfg(url="", interpolation="bicubic"),
    "cs3darknet_m": _cfg(
    ...
    "cs3darknet_focus_s": _cfg(url="", interpolation="bicubic"),
    ...
    "cs3darknet_focus_x": _cfg(url="", interpolation="bicubic"),
    "cs3sedarknet_l": _cfg(
    ...
    "cs3sedarknet_xdw": _cfg(url="", interpolation="bicubic"),
    "cs3edgenet_x": _cfg(
...
class CspStagesCfg:
    ...
    bottle_ratio: Union[float, Tuple[float, ...]] = 1.0  # bottleneck-ratio of blocks in stage
...
model_cfgs = dict(
    ...
    cs3sedarknet_l=_cs3_cfg(attn_layer="se", attn_kwargs=dict(rd_ratio=0.25)),
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = ConvNormAct(
            mid_chs, mid_chs, kernel_size=3, dilation=dilation, groups=groups, drop_layer=drop_block, **ckwargs
        )
...
class DarkBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = ConvNormAct(
            mid_chs, out_chs, kernel_size=3, dilation=dilation, groups=groups, drop_layer=drop_block, **ckwargs
        )
        self.drop_path = DropPath(drop_path) if drop_path else nn.Identity()
...
class EdgeBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = ConvNormAct(
            in_chs, mid_chs, kernel_size=3, dilation=dilation, groups=groups, drop_layer=drop_block, **ckwargs
        )
        self.attn = attn_layer(mid_chs, act_layer=act_layer) if attn_layer is not None else nn.Identity()
...
def create_csp_stages(cfg: CspModelCfg, drop_path_rate: float, output_stride: int, stem_feat: Dict[str, Any]):
    ...
    cfg_dict["block_dpr"] = (
        ...
        else [x.tolist() for x in torch.linspace(0, drop_path_rate, sum(cfg.stages.depth)).split(cfg.stages.depth)]
    )
