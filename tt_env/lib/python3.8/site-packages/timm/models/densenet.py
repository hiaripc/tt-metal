...
class DenseLayer(nn.Module):
    def __init__(
        self, num_input_features, growth_rate, bn_size, norm_layer=BatchNormAct2d, drop_rate=0.0, memory_efficient=False
    ):
    ...
    def __init__(
        ...
        if aa_layer is None:
            ...
            stem_pool = nn.Sequential(
                *[nn.MaxPool2d(kernel_size=3, stride=1, padding=1), aa_layer(channels=num_init_features, stride=2)]
            )
        if deep_stem:
...
def densenet121d(pretrained=False, **kwargs):
    ...
    """
    model = _create_densenet(
        "densenet121d", growth_rate=32, block_config=(6, 12, 24, 16), stem_type="deep", pretrained=pretrained, **kwargs
    )
    return model
