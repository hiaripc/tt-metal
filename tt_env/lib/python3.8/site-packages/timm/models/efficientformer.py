...
class EfficientFormer(nn.Module):
    ...
    def group_matcher(self, coarse=False):
        matcher = dict(stem=r"^stem", blocks=[(r"^stages\.(\d+)", None), (r"^norm", (99999,))])  # stem and embed
        return matcher
...
def _create_efficientformer(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        EfficientFormer, variant, pretrained, pretrained_filter_fn=_checkpoint_filter_fn, **kwargs
    )
    return model
...
def efficientformer_l1(pretrained=False, **kwargs):
    model_kwargs = dict(depths=EfficientFormer_depth["l1"], embed_dims=EfficientFormer_width["l1"], num_vit=1, **kwargs)
...
def efficientformer_l3(pretrained=False, **kwargs):
    model_kwargs = dict(depths=EfficientFormer_depth["l3"], embed_dims=EfficientFormer_width["l3"], num_vit=4, **kwargs)
...
def efficientformer_l7(pretrained=False, **kwargs):
    model_kwargs = dict(depths=EfficientFormer_depth["l7"], embed_dims=EfficientFormer_width["l7"], num_vit=8, **kwargs)
