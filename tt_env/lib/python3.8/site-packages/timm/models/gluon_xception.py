...
class Xception65(nn.Module):
    ...
    def __init__(
        ...
        self.mid = nn.Sequential(
            OrderedDict(
                [
                    ("block%d" % i, Block(728, 728, stride=1, dilation=middle_dilation, norm_layer=norm_layer))
                    for i in range(4, 20)
                ]
            )
        )
...
def _create_gluon_xception(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(Xception65, variant, pretrained, feature_cfg=dict(feature_cls="hook"), **kwargs)
