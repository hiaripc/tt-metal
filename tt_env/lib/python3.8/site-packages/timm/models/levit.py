...
default_cfgs = dict(
    levit_128s=_cfg(url="https://dl.fbaipublicfiles.com/LeViT/LeViT-128S-96703c44.pth"),
    levit_128=_cfg(url="https://dl.fbaipublicfiles.com/LeViT/LeViT-128-b88c2750.pth"),
    levit_192=_cfg(url="https://dl.fbaipublicfiles.com/LeViT/LeViT-192-92712e41.pth"),
    levit_256=_cfg(url="https://dl.fbaipublicfiles.com/LeViT/LeViT-256-13b5763e.pth"),
    levit_384=_cfg(url="https://dl.fbaipublicfiles.com/LeViT/LeViT-384-9bdaf2e2.pth"),
...
def levit_128s(pretrained=False, use_conv=False, **kwargs):
    return create_levit("levit_128s", pretrained=pretrained, use_conv=use_conv, **kwargs)


@register_model
def levit_128(pretrained=False, use_conv=False, **kwargs):
    return create_levit("levit_128", pretrained=pretrained, use_conv=use_conv, **kwargs)


@register_model
def levit_192(pretrained=False, use_conv=False, **kwargs):
    return create_levit("levit_192", pretrained=pretrained, use_conv=use_conv, **kwargs)


@register_model
def levit_256(pretrained=False, use_conv=False, **kwargs):
    return create_levit("levit_256", pretrained=pretrained, use_conv=use_conv, **kwargs)


@register_model
def levit_384(pretrained=False, use_conv=False, **kwargs):
    return create_levit("levit_384", pretrained=pretrained, use_conv=use_conv, **kwargs)


@register_model
def levit_256d(pretrained=False, use_conv=False, **kwargs):
    return create_levit("levit_256d", pretrained=pretrained, use_conv=use_conv, distilled=False, **kwargs)


class ConvNorm(nn.Sequential):
    def __init__(
        self, in_chs, out_chs, kernel_size=1, stride=1, pad=0, dilation=1, groups=1, bn_weight_init=1, resolution=-10000
    ):
        super().__init__()
...
class Residual(nn.Module):
    ...
    def forward(self, x):
        if self.training and self.drop > 0:
            return (
                x + self.m(x) * torch.rand(x.size(0), 1, 1, device=x.device).ge_(self.drop).div(1 - self.drop).detach()
            )
        else:
            return x + self.m(x)
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, key_dim, num_heads=8, attn_ratio=4, act_layer=None, resolution=14, use_conv=False):
        super().__init__()
        ...
        self.proj = nn.Sequential(
            act_layer(), ln_layer(self.val_attn_dim, dim, bn_weight_init=0, resolution=resolution)
        )
