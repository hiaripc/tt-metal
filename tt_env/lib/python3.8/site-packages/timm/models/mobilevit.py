...
def _mobilevit_block(d, c, s, transformer_dim, transformer_depth, patch_size=4, br=4.0):
    ...
    return (
        ...
        ByoBlockCfg(
            ...
            s=1,
            block_kwargs=dict(
                transformer_dim=transformer_dim, transformer_depth=transformer_depth, patch_size=patch_size
            ),
        ),
    )
...
def _mobilevitv2_block(d, c, s, transformer_depth, patch_size=2, br=2.0, transformer_br=0.5):
    ...
    return (
        ...
        ByoBlockCfg(
            ...
            br=transformer_br,
            gs=1,
            block_kwargs=dict(transformer_depth=transformer_depth, patch_size=patch_size),
        ),
    )
    ...
    def __init__(
        ...
        **kwargs,  # eat unused args
        ...
        self.conv_kxk = layers.conv_norm_act(
            in_chs, in_chs, kernel_size=kernel_size, stride=stride, groups=groups, dilation=dilation[0]
        )
...
class LinearTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = ConvMlp(
            in_features=embed_dim, hidden_features=int(embed_dim * mlp_ratio), act_layer=act_layer, drop=drop
        )
...
class MobileVitV2Block(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,  # eat unused args
        ...
        self.conv_kxk = layers.conv_norm_act(
            in_chs, in_chs, kernel_size=kernel_size, stride=1, groups=groups, dilation=dilation[0]
        )
        self.conv_1x1 = nn.Conv2d(in_chs, transformer_dim, kernel_size=1, bias=False)
...
def mobilevitv2_150_in22ft1k(pretrained=False, **kwargs):
    return _create_mobilevit("mobilevitv2_150_in22ft1k", cfg_variant="mobilevitv2_150", pretrained=pretrained, **kwargs)


@register_model
def mobilevitv2_175_in22ft1k(pretrained=False, **kwargs):
    return _create_mobilevit("mobilevitv2_175_in22ft1k", cfg_variant="mobilevitv2_175", pretrained=pretrained, **kwargs)


@register_model
def mobilevitv2_200_in22ft1k(pretrained=False, **kwargs):
    return _create_mobilevit("mobilevitv2_200_in22ft1k", cfg_variant="mobilevitv2_200", pretrained=pretrained, **kwargs)
