...
class MlpWithDepthwiseConv(nn.Module):
    def __init__(
        self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.0, extra_relu=False
    ):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, sr_ratio=1, linear_attn=False, qkv_bias=True, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def group_matcher(self, coarse=False):
        matcher = dict(stem=r"^patch_embed", blocks=r"^stages\.(\d+)")  # stem and embed
        return matcher
...
def _create_pvt2(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        PyramidVisionTransformerV2, variant, pretrained, pretrained_filter_fn=_checkpoint_filter_fn, **kwargs
    )
    return model
