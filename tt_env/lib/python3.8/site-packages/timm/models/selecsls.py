...
default_cfgs = {
    "selecsls42": _cfg(url="", interpolation="bicubic"),
    "selecsls42b": _cfg(
    ...
    "selecsls84": _cfg(url="", interpolation="bicubic"),
}
...
class SelecSLS(nn.Module):
    ...
    def group_matcher(self, coarse=False):
        return dict(stem=r"^stem", blocks=r"^features\.(\d+)", blocks_head=r"^head")
...
def _create_selecsls(variant, pretrained, **kwargs):
    ...
    if variant.startswith("selecsls42"):
        ...
        cfg["num_features"] = 1280
        feature_info.extend(
            [dict(num_chs=1024, reduction=32, module="head.1"), dict(num_chs=1280, reduction=64, module="head.3")]
        )
    else:
