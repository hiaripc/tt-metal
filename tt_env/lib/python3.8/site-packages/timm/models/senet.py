...
class SENet(nn.Module):
    def __init__(
        ...
        if input_3x3:
            ...
            layer0_modules = [
                ("conv1", nn.Conv2d(in_chans, inplanes, kernel_size=7, stride=2, padding=3, bias=False)),
...
def legacy_seresnet18(pretrained=False, **kwargs):
    model_args = dict(block=SEResNetBlock, layers=[2, 2, 2, 2], groups=1, reduction=16, **kwargs)
    return _create_senet("legacy_seresnet18", pretrained, **model_args)


@register_model
def legacy_seresnet34(pretrained=False, **kwargs):
    model_args = dict(block=SEResNetBlock, layers=[3, 4, 6, 3], groups=1, reduction=16, **kwargs)
...
def legacy_seresnet101(pretrained=False, **kwargs):
    model_args = dict(block=SEResNetBottleneck, layers=[3, 4, 23, 3], groups=1, reduction=16, **kwargs)
    return _create_senet("legacy_seresnet101", pretrained, **model_args)


@register_model
def legacy_seresnet152(pretrained=False, **kwargs):
    model_args = dict(block=SEResNetBottleneck, layers=[3, 8, 36, 3], groups=1, reduction=16, **kwargs)
...
def legacy_seresnext26_32x4d(pretrained=False, **kwargs):
    model_args = dict(block=SEResNeXtBottleneck, layers=[2, 2, 2, 2], groups=32, reduction=16, **kwargs)
    return _create_senet("legacy_seresnext26_32x4d", pretrained, **model_args)


@register_model
def legacy_seresnext50_32x4d(pretrained=False, **kwargs):
    model_args = dict(block=SEResNeXtBottleneck, layers=[3, 4, 6, 3], groups=32, reduction=16, **kwargs)
