...
class Attention(nn.Module):
    ...
    def __init__(self, dim, hidden_dim, num_heads=8, qkv_bias=False, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn_in = Attention(
            in_dim, in_dim, num_heads=in_num_head, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop
        )

        self.norm_mlp_in = norm_layer(in_dim)
        self.mlp_in = Mlp(
            in_features=in_dim, hidden_features=int(in_dim * 4), out_features=in_dim, act_layer=act_layer, drop=drop
        )
        ...
        self.attn_out = Attention(dim, dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
def checkpoint_filter_fn(state_dict, model):
    ...
    if state_dict["patch_pos"].shape != model.patch_pos.shape:
        state_dict["patch_pos"] = resize_pos_embed(
            state_dict["patch_pos"], model.patch_pos, getattr(model, "num_tokens", 1), model.pixel_embed.grid_size
        )
    return state_dict
...
def _create_tnt(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(TNT, variant, pretrained, pretrained_filter_fn=checkpoint_filter_fn, **kwargs)
    return model
...
def tnt_s_patch16_224(pretrained=False, **kwargs):
    model_cfg = dict(
        patch_size=16, embed_dim=384, in_dim=24, depth=12, num_heads=6, in_num_head=4, qkv_bias=False, **kwargs
    )
...
def tnt_b_patch16_224(pretrained=False, **kwargs):
    model_cfg = dict(
        patch_size=16, embed_dim=640, in_dim=40, depth=12, num_heads=10, in_num_head=4, qkv_bias=False, **kwargs
    )
