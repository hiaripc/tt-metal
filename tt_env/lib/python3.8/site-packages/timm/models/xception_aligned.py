...
class SeparableConv2d(nn.Module):
    def __init__(
        ...
        self.conv_dw = create_conv2d(
            in_chs, in_chs, kernel_size, stride=stride, padding=padding, dilation=dilation, depthwise=True
        )
...
class PreXceptionModule(nn.Module):
    def __init__(
        self, in_chs, out_chs, stride=1, dilation=1, pad_type="", no_skip=False, act_layer=nn.ReLU, norm_layer=None
    ):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            *[
                ...
                create_conv2d(32, 64, kernel_size=3, stride=1)
                if preact
        ...
        self.feature_info += [
            dict(num_chs=self.num_features, reduction=curr_stride, module="blocks." + str(len(self.blocks) - 1))
        ]
...
def _xception(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        XceptionAligned, variant, pretrained, feature_cfg=dict(flatten_sequential=True, feature_cls="hook"), **kwargs
    )
...
def xception65p(pretrained=False, **kwargs):
    ...
    model_args = dict(
        block_cfg=block_cfg, preact=True, norm_layer=partial(nn.BatchNorm2d, eps=0.001, momentum=0.1), **kwargs
    )
