...
def _parse_declaration(first_token, tokens):
    ...
    if colon is None:
        return ParseError(
            name.source_line, name.source_column, "invalid", "Expected ':' after declaration name, got EOF"
        )
    elif colon != ":":
        return ParseError(
            colon.source_line,
...
def _consume_at_rule(at_keyword, tokens):
    ...
    for token in tokens:
        ...
        prelude.append(token)
    return AtRule(
        at_keyword.source_line, at_keyword.source_column, at_keyword.value, at_keyword.lower_value, prelude, content
    )
