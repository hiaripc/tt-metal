...
def serialize_identifier(value):
    ...
    result += (
        c
        if c in ("abcdefghijklmnopqrstuvwxyz_" "ABCDEFGHIJKLMNOPQRSTUVWXYZ") or ord(c) > 0x7F
        else r"\A "
        if c == "\n"
        else r"\D "
        if c == "\r"
...
def serialize_name(value):
    return "".join(
        c
        if c in ("abcdefghijklmnopqrstuvwxyz-_0123456789" "ABCDEFGHIJKLMNOPQRSTUVWXYZ") or ord(c) > 0x7F
        else r"\A "
        if c == "\n"
        else r"\D "
        if c == "\r"
...
def _serialize_to(nodes, write):
    ...
    for node in nodes:
        ...
        if (previous_type, serialization_type) in bad_pairs:
            write("/**/")
        elif previous_type == "\\" and not (serialization_type == "whitespace" and node.value.startswith("\n")):
            write("\n")
...
BAD_PAIRS = set(
    [
        ...
        for b in ("ident", "function", "url", "number", "percentage", "dimension", "unicode-range")
    ]
    + [(a, b) for a in ("ident", "at-keyword", "hash", "dimension") for b in ("-", "-->")]
    + [(a, b) for a in ("#", "-", "number", "@") for b in ("ident", "function", "url")]
    + [(a, b) for a in ("unicode-range", ".", "+") for b in ("number", "percentage", "dimension")]
