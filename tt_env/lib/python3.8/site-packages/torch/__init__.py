...
def set_deterministic_debug_mode(debug_mode: Union[builtins.int, str]) -> None:
    ...
    if isinstance(debug_mode, str):
        if debug_mode == "default":
            ...
            raise RuntimeError(
                "invalid value of debug_mode, expected one of `default`, " f"`warn`, `error`, but got {debug_mode}"
            )
    ...
    if debug_mode == 0:
        ...
        raise RuntimeError("invalid value of debug_mode, expected 0, 1, or 2, " f"but got {debug_mode}")
...
def _register_device_module(device_type, module):
    ...
    if hasattr(m, device_type):
        raise RuntimeError(
            f"The runtime module of '{device_type}' has already " f"been registered with '{getattr(m, device_type)}'"
        )
...
if TYPE_CHECKING:
    ...
    def __getattr__(name):
        ...
        if replacement is not None:
            ...
            warnings.warn(
                f"'{name}' is deprecated, please use '{replacement.__module__}.{replacement.__name__}()'", stacklevel=2
            )
            return replacement()
