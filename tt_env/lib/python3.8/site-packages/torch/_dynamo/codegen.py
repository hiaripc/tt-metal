...
class PyCodegen:
    ...
    def __call__(self, value, allow_cache=True):
        ...
        if value.source is not None and allow_cache:
        ...
        elif value.is_python_constant() and is_safe_constant(value.as_python_constant()):
            output.append(self.create_load_const(value.as_python_constant()))
        elif isinstance(value, TensorWithTFOverrideVariable):
            graph_outputs_key = self.add_graph_output(value)
            output.append(self.create_load_global(value.global_mangled_class_name(), True, add=True))
            ...
            if isinstance(value, NumpyNdarrayVariable):
                ...
                output.extend([self.create_load_attr("item")] + create_call_function(0, True))
    ...
    def load_import_from(self, module_name, object_name) -> None:
        self.extend_output(AttrSource(self.tx.import_source(module_name), object_name).reconstruct(self))
