...
class _TorchDynamoContext:
    ...
    def __call__(self, fn):
        ...
        if (
            ...
            and (getattr(fn, "__name__", "") not in ["_call_impl", "_wrapped_call_impl"])
...
def optimize(
    ...
    *,
    ...
    return _optimize_catch_errors(
        ...
        dynamic=dynamic,
        save_config=save_config,
        compiler_config=backend.get_compiler_config() if hasattr(backend, "get_compiler_config") else None,
    )
...
class FlattenInputOutputSignature(torch.fx.interpreter.Transformer):
    def __init__(
        ...
        matched_input_elements_to_fake = {
            val: example_fake_inputs[ix] for ix, val in enumerate(matched_input_elements_positions)
        }
    ...
    def run_node(self, n):
        ...
        if "example_value" in self.current_node.meta:
            result_proxy.node.meta["example_value"] = self.current_node.meta["example_value"]
...
def rewrite_signature(
    ...
    def produce_matching(sources, candidates):
        source_types = " or ".join(
            [
                desc + " of types: (" + ", ".join([str(type(val)) for val in vals]) + ")"
    ...
    def argument_names(f_sig, args, kwargs) -> List[str]:
        def signature_to_fullargspec(sig: inspect.Signature):
            ...
            defaults = tuple(
                p.default
                for p in params
                if p.kind == inspect.Parameter.POSITIONAL_OR_KEYWORD and p.default is not inspect.Parameter.empty
            )
            kwonlydefaults = {
                p.name: p.default
                for p in params
    ...
    **extra_kwargs,
    ...
    def inner(*args, **kwargs):
        ...
        if aten_graph:
            ...
            for node in graph.graph.nodes:
                if node.op == "get_attr" and isinstance(getattr(graph, node.target), torch.Tensor):
                    node.meta["val"] = fake_mode.from_tensor(getattr(graph, node.target), static_shapes=True)
        ...
        graph.meta["input_shape_constraints"] = (
            [constraint.serializable_spec for constraint in constraints] if constraints else []
        )
