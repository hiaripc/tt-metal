...
recompiles_verbose_log = torch._logging.getArtifactLogger(__name__, "recompiles_verbose")
...
CLOSURE_VARS = {
    ...
    "___current_backend": (lambda: torch._dynamo.eval_frame.guarded_backend_cache.current_backend),
    "___lookup_backend": (
        lambda backend_obj_id: torch._dynamo.eval_frame.guarded_backend_cache.cached_backends[backend_obj_id]
    ),
    "___skip_backend_check": (
        lambda: torch._dynamo.eval_frame.guarded_backend_cache.skip_backend_check_for_run_only_mode
    ),
    "___compile_config_hash": (lambda: torch._dynamo.eval_frame.get_saved_else_current_config_hash().hex()),
...
def must_add_nn_module_guards(guard):
    ...
    return (
        ...
        or (config.guard_nn_modules_using_dict_tags and guard.create_fn is GuardBuilder.NN_MODULE)
    )
...
class CheckFunctionManager:
    ...
    def compile_check_fn(self, builder, guards_out, guard_fail_fn):
        ...
        if tensor_check_names:
            ...
            dynamic_dims_sizes = [
                convert(self.output_graph.tensor_weakref_to_sizes_strides[t]["size"]) for t in tensor_check_examples
            ]

            dynamic_dims_strides = [
                convert(self.output_graph.tensor_weakref_to_sizes_strides[t]["stride"]) for t in tensor_check_examples
            ]
            ...
            for i, name in enumerate(tensor_check_names):
                ...
                pytype = type(t)
                dispatch_key = (
                    torch._C._dispatch_keys(t) | torch._C._dispatch_tls_local_include_set()
        ...
        aotautograd_guards: List[GuardEnvExpr] = (
            self.output_graph.tracing_context.guards_context.aotautograd_guards if self.output_graph else []
        )
