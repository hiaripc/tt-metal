    ...
    def subtracer(self, source_target, prior_tracer):
        ...
        try:
            ...
            tracer = (
                prior_tracer
                if prior_tracer
                else SubgraphTracer(self, parent=self.current_tracer, source_target=source_target)
            )
...
def check_pt2_compliant_op(output_graph, kind, target, args, kwargs):
    ...
    if isinstance(target, torch._ops.OpOverload):
        ...
        encountered_non_compliant_op(
            target,
            f"Encountered the torch.ops.OpOverload {target} " f"that is not PT2 compliant.",
        )
        return
