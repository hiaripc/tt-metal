...
class SpeculationLog:
    ...
    def next(self, filename: str, lineno: int, instruction_pointer) -> SpeculationEntry:
        ...
        self.index += 1
        assert (
            entry.instruction_pointer == instruction_pointer and entry.filename == filename and entry.lineno == lineno
        ), textwrap.dedent(
            f"""
    ...
    def inner(self: "InstructionTranslatorBase", inst: Instruction):
        ...
        if value.is_python_constant():
            ...
            self.output.compile_subgraph(
                self,
                reason=GraphCompileReason(f"generic_jump {typestr(value)}", [self.frame_summary()]),
            )
            self.pop()
    ...
    def decorator(inner_fn):
        ...
        def handle_graph_break(
            ...
            if sys.version_info >= (3, 11) and inst.opname == "CALL":
                ...
                stack_effect = dis.stack_effect(dis.opmap["PRECALL"], inst.arg) + dis.stack_effect(
                    dis.opmap["CALL"], inst.arg
                )
            else:
...
class InstructionTranslatorBase(Checkpointable[InstructionTranslatorGraphState]):
    ...
    def cell_and_freevars(self):
        if not hasattr(self, "_cell_and_freevars"):
            self._cell_and_freevars = tuple(self.code_options["co_cellvars"] or []) + tuple(
                self.code_options["co_freevars"] or []
            )
        ...
        if (
            ...
            self.push(
                ConstantVariable.create(
                    supported_any[op](left.as_python_constant(), right.as_python_constant()),
                )
            )
    ...
    def __init__(
        ...
        self.f_locals: Dict[str, Any] = f_locals  # needed for recording accessed locals for replay
...
class InstructionTranslator(InstructionTranslatorBase):
    ...
    def RETURN_VALUE(self, inst):
        ...
        self.output.compile_subgraph(
            self,
            reason=GraphCompileReason("return_value", [self.frame_summary()], graph_break=False),
...
class InliningInstructionTranslator(InstructionTranslatorBase):
    ...
    def inline_call_(parent, func: VariableTracker, args: List[VariableTracker], kwargs):
        ...
        if code.co_name in ("__setitem__", "__setattr__") and not (
            args is not None and len(args) > 0 and isinstance(args[0], variables.CustomizedDictVariable)
        ):
        ...
        if inst.argval in self.closure_cells:
            ...
            if isinstance(
                ...
                if (
                    ...
                    and maybe_cell.source.name() not in self.output.root_tx.mutated_closure_cell_contents
                ):
