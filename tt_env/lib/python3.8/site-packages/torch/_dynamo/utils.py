...
def compile_times(repr="str", aggregate=False):
    ...
    if repr == "str":
        rows = [
            (k, fmt_fn(compilation_time_metrics[k], item_fn=lambda x: f"{x:.4f}")) for k in compilation_time_metrics
        ]
...
def write_record_to_file(filename, exec_record):
    try:
        if os.path.exists(filename):
            log.warning("Unable to write execution record %s; file already exists.", filename)
        else:
...
def proxy_args_kwargs(args, kwargs):
    try:
        ...
        raise unimplemented(f"call_function args: {typestr(*args)} {typestr(*list(kwargs.values()))}") from e
...
def check_numpy_ndarray_args(args, kwargs):
    ...
    return any(isinstance(x, NumpyNdarrayVariable) for x in itertools.chain(args, kwargs.values()))
...
def iter_contains(items, search, tx, check_tensor_identity=False):
    ...
    if search.is_python_constant():
        found_const = any(
            x.is_python_constant() and x.as_python_constant() == search.as_python_constant() for x in items
        )
...
def dict_const_keys_repr(const_keys, *, local):
    if any(isinstance(k, enum.Enum) for k in const_keys):
        ...
        const_keys_str = (
            f"{ {enum_repr(k, local=local) if isinstance(k, enum.Enum) else repr(k) for k in const_keys} }".replace(
                "'", ""
            )
        )
    else:
        ...
        if cos_similarity:
            ...
            if fp64_ref.dtype == torch.float64:
                ...
                if math.isnan(ref_error):
                    log.warning("Found nan in reference. Consider running in higher precision.")
...
def get_fake_values_from_nodes(tx, nodes):
    ...
    return tree_map_only(torch.Tensor, functools.partial(ensure_graph_fake, tx=tx), args_kwargs)
...
def get_fake_value(node, tx, allow_non_graph_fake=False):
    ...
    try:
        with tx.fake_mode, enable_python_dispatcher():
            ret_val = wrap_fake_exception(lambda: run_node(tx.output, node, args, kwargs, nnmodule))
    except Unsupported:
        raise
        ...
        if isinstance(cause, torch._subclasses.fake_tensor.DataDependentOutputException):
            unimplemented(f"data dependent operator: {cause.func}")
        elif isinstance(cause, torch._subclasses.fake_tensor.DynamicOutputShapeException):
            unimplemented(f"dynamic shape operator: {cause.func}")
        elif isinstance(cause, torch._subclasses.fake_tensor.UnsupportedOperatorException):
            unimplemented(
                f"unsupported operator: {cause.func} (see "
                "https://docs.google.com/document/d/1GgvOe7C8_NVOMLOCwDaYV1mXXyHMXY7ExoewHqooxrs/edit#heading=h.64r4npvq0w0"
                " for how to fix)"
            )
        elif isinstance(cause, torch.fx.experimental.symbolic_shapes.GuardOnDataDependentSymNode):
    ...
    if not allow_non_graph_fake:
        _ = tree_map_only(torch.Tensor, functools.partial(ensure_graph_fake, tx=tx), ret_val)
    return ret_val
...
def has_torch_function(vt: "torch._dynamo.variables.base.VariableTracker") -> bool:
    ...
    return isinstance(vt, TensorWithTFOverrideVariable) or (
        isinstance(vt, UserDefinedObjectVariable) and hasattr(vt.value, "__torch_function__")
    )
