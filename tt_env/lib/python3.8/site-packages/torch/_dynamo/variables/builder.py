...
class VariableBuilder:
    ...
    def _wrap(self, value):
        ...
        elif istype(value, (dict, collections.defaultdict, collections.OrderedDict)) and all(
            ConstantVariable.is_literal(k) or self.tensor_can_be_dict_key(k) or isinstance(k, enum.Enum)
            for k in value.keys()
        ):
        ...
        # NB: These can't be put in type_dispatch, they have to run later
            ...
            return self.tx.output.side_effects.track_list(
                ...
                RestrictedListSubclassVariable(
                    [
                        LazyVariableTracker.create(value=value[i], source=GetItemSource(self.source, i))
    ...
    def wrap_tuple_iterator(self, value: tuple_iterator):
        ...
        output = [
            VariableBuilder(self.tx, TupleIteratorGetItemSource(self.get_source(), i))(tuple_iterator_getitem(value, i))
    ...
    def wrap_slice_range(self, value: Union[slice, range]):
        items = [
            VariableBuilder(self.tx, AttrSource(self.get_source(), k))(getattr(value, k))
    ...
    def wrap_literal(self, value):
        ...
        if unspec and type(value) is torch.Size:
            self.install_guards(GuardBuilder.LIST_LENGTH)
            return SizeVariable(
                [VariableBuilder(self.tx, GetItemSource(self.get_source(), i))(v) for i, v in enumerate(value)]
            )
    ...
    def wrap_tensor(self, value: torch.Tensor):
        ...
        if (
            source.guard_source().is_nn_module() or get_static_address_type(value) is not None
        ...
        self.install_guards(
            functools.partial(
                ...
                value=value if isinstance(source, NumpyTensorSource) else TensorWeakRef(value),
            )
        )
...
def _dataclasses_fields_lambda(obj):
    ...
    for field in dataclasses.fields(value):
        source = None
        if obj.source:
            source = GetItemSource(AttrSource(obj.source, "__dataclass_fields__"), field.name)
...
def wrap_fx_proxy_cls(target_cls, tx, proxy, example_value=None, subclass_type=None, **options):
    ...
    if isinstance(example_value, torch.Tensor):
            ...
            if val is None:
                ...
                unpacked.append(
                    wrap_fx_proxy_cls(
                        target_cls,
                        tx,
                        proxy.tracer.create_proxy("call_function", operator.getitem, (proxy, i), {}),
                        example_value=val,
                        **options,
                    )
                )
    ...
    elif (inspect.isclass(proxy.node.target) and issubclass(proxy.node.target, _StreamBase)) or proxy.node.target in [
    ...
    elif (inspect.isclass(proxy.node.target) and issubclass(proxy.node.target, _EventBase)) or proxy.node.target in [
        ...
        unimplemented(
            "torch.* op returned non-Tensor " + f"{typestr(example_value)} {proxy.node.op} {proxy.node.target}"
        )
...
def _automatic_dynamic(e, tx, source, static_shapes) -> SymbolicContext:
    ...
    if any(isinstance(s, SymInt) for s in e.size()):
        return StatefulSymbolicContext(
            dynamic_sizes=[DimDynamic.DYNAMIC if isinstance(s, SymInt) else DimDynamic.STATIC for s in e.size()],
            constraint_sizes=[None] * e.dim(),
            tensor_source=source,
            source_to_symint_node_cache=source_to_symint_node_cache,
        )
