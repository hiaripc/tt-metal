...
class BuiltinVariable(VariableTracker):
    ...
    def _binop_handlers():
        ...
        for (
            ...
            def dynamic_handler(tx, a, b, options, fn=op):
                ...
                return wrap_fx_proxy(
                    tx,
                    tx.output.create_proxy("call_function", fn, *proxy_args_kwargs([a, b], {})),
                    **options,
                )
        ...
        list_like_addition_handlers = [
            ...
            (
                ...
                lambda tx, a, b, options: TupleVariable(list(a.unpack_var_sequence(tx)) + b.items, **options),
            ),
            (
    ...
    def tensor_args(self, *args, **kwargs):
        return any(isinstance(i, variables.TensorVariable) for i in itertools.chain(args, kwargs.values())) and not any(
            isinstance(i, variables.GetAttrVariable) for i in itertools.chain(args, kwargs.values())
        )
    ...
    def _dyn_proxy(self, tx, *args, **kwargs):
        ...
        return wrap_fx_proxy(
            tx,
            tx.output.create_proxy("call_function", self.fn, *proxy_args_kwargs(args, kwargs)),
        )
    ...
    def call_sum(self, tx, seq, **kwargs):
        ...
        if (
            ...
            and all(isinstance(x, variables.ConstantVariable) and isinstance(x.value, (int, float)) for x in seq.items)
            and not kwargs
        ):
    ...
    def _comparison(self, tx, left, right):
        ...
        if (
            all(isinstance(x, (NNModuleVariable, ConstantVariable)) for x in [left, right])
    ...
    def call_and_(self, tx, a, b):
        ...
        if isinstance(a, (SymNodeVariable, ConstantVariable)) and isinstance(b, (SymNodeVariable, ConstantVariable)):
            return SymNodeVariable.create(
                tx,
                tx.output.create_proxy("call_function", operator.and_, *proxy_args_kwargs([a, b], {})),
                sym_num=None,
            )
        # None no-ops this handler and lets the driving function proceed
        return None
    ...
    def call_or_(self, tx, a, b):
        ...
        if isinstance(a, (SymNodeVariable, ConstantVariable)) and isinstance(b, (SymNodeVariable, ConstantVariable)):
            return SymNodeVariable.create(
                tx,
                tx.output.create_proxy("call_function", operator.or_, *proxy_args_kwargs([a, b], {})),
                sym_num=None,
            )
        # None no-ops this handler and lets the driving function proceed
        return None
    ...
    def call_not_(self, tx, a):
        if isinstance(a, SymNodeVariable):
            return SymNodeVariable.create(
                tx,
                tx.output.create_proxy("call_function", operator.not_, *proxy_args_kwargs([a], {})),
                sym_num=None,
            )
