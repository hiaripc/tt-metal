...
class UserFunctionVariable(BaseUserFunctionVariable):
    ...
    def bind_args(self, parent, args, kwargs):
        ...
        fake_func = types.FunctionType(
            ...
            fn.__globals__,
            fn.__name__,
            tuple([wrap(val=arg, source=source) for arg, source in zip(defaults, defaults_sources)]),
            fn.__closure__,
        )
        if fn.__kwdefaults__:
            kwdefaults_sources = {
                k: None if self.source is None else DefaultsSource(self.source, k, is_kw=True)
                for k in fn.__kwdefaults__
            }
            fake_func.__kwdefaults__ = {
                k: wrap(val=v, source=kwdefaults_sources[k]) for k, v in fn.__kwdefaults__.items()
            }
    ...
    def __init__(self, kernel, kernel_idx, grid, **kwargs):
        ...
        if isinstance(kernel, Autotuner):
            ...
            if (
                ...
                or (
                    "prune_configs_by" in defaults and defaults["prune_configs_by"].default != kernel.early_config_prune
                )
            ):
