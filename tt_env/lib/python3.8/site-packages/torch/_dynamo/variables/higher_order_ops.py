...
def _assert_tensors_nonaliasing(inputs, outputs):
    ...
    assert input_tensor_ids.isdisjoint(output_tensor_ids), "inputs to function body cannot alias outputs"
...
def validate_args_and_maybe_create_graph_inputs(sub_args, tracer, tx, manually_set_subgraph_inputs):
    ...
    for a in sub_args:
        ...
        if isinstance(a, ConstantVariable):
            ...
            if manually_set_subgraph_inputs:
                raise unimplemented(
                    f"HigherOrderOperator with body that accepts non-Tensors as input. " f"Got: {a.python_type()}"
                )
            else:
                # leverage tracer's lifting mechanism to lift these args.
                if only_consist_of(a, (ConstantVariable, SymNodeVariable, TensorVariable)):
                    new_arg = a
                else:
                    unimplemented(
    ...
    *,
    ...
    try:
        ...
        with tx.output.subtracer(source_target, tracer) as subtracer:
            ...
            autograd_ctx = dynamo_enable_grad(tx, enable_grad) if enable_grad is not None else contextlib.nullcontext()
            ...
            if should_flatten_outputs:
                ...
                output, treespec = _make_inlined(tx, pytree.tree_flatten)(output).unpack_var_sequence(tx)
        ...
        tx.restore_graphstate(checkpoint)
        raise Unsupported(
            f"{msg} Scroll up for the stack trace " f"of the initial exception. The reason was: {ex.msg}"
        ) from ex
    ...
    def call_function(
        ...
        def dedup_and_sort_lifted_freevars(true_lifted_freevars, false_lifted_freevars):
            ...
            def shared_getattrs(true_lifted_proxies, false_lifted_proxies):
                true_targets = {
                    proxy.node.target: proxy for proxy in true_lifted_proxies if proxy.node.op == "get_attr"
                }
            ...
            true_shared_freevars = (true_lifted_freevars.keys() & false_lifted_freevars.keys()).union(
                true_fn_shared_getattrs.keys()
            )
            false_shared_freevars = (true_lifted_freevars.keys() & false_lifted_freevars.keys()).union(
                false_fn_shared_getattrs.keys()
            )
...
def non_single_tensor_return_unsupported(api, ret):
    ...
    if not isinstance(ret, TensorVariable):
        raise Unsupported(f"{api} over function that returns something " f"other than one Tensor")
        ...
        if not has_aux.value:
            if isinstance(argnums_v, int):
                ...
                for idx in range(len(argnums_v)):
                    proxy = grads.call_method(tx, "__getitem__", (ConstantVariable.create(idx),), {}).call_method(
                        tx, "contiguous", (), {}
                    )
                    items.append(proxy)
...
class CheckpointHigherOrderVariable(WrapHigherOrderVariable):
    def call_function(self, tx, args: List[VariableTracker], kwargs: Dict[str, VariableTracker]) -> VariableTracker:
        ...
        (
            ...
            treespec,
            checkpointed_gmod,
        ) = self.create_wrapped_node(tx, args, gmod_kwargs, "torch.utils.checkpoint.checkpoint")
