...
def initialize_lazy_module(tx, mod, args, kwargs):
    ...
    if hasattr(mod, "_initialize_hook"):
        ...
        input = [
            type(arg)([convert_to_fake(x) for x in arg]) if isinstance(arg, (list, tuple)) else convert_to_fake(arg)
...
class NNModuleVariable(VariableTracker):
    ...
    def call_method(
        ...
        if name == "named_children":
            ...
            if isinstance(args[0], SliceVariable):
                ...
                new_module_variable = tx.output.register_attr_or_module(
                    ...
                    source=NNModuleSource(GetItemSource(self.source, args[0].as_python_constant())),
                )
            ...
            or (isinstance(module, torch.nn.modules.conv._ConvNd) and name == "_conv_forward")
            or (isinstance(module, torch.nn.modules.conv._ConvTransposeNd) and name == "_output_padding")
        ):
            # Inline the function
            fn = getattr(module, name).__func__
        ...
        # manual handling of inputs
            ...
            and all(isinstance(x, variables.TensorVariable) for x in itertools.chain(args, kwargs.values()))
        ):
            return generic_call_method_helper(name)
        else:
...
class UnspecializedNNModuleVariable(UserDefinedObjectVariable):
    ...
    def unpack_var_sequence(self, tx):
        ...
        if fn in (
            ...
            assert self.source
            return [
                VariableBuilder(tx, source=GetItemSource(self.source, idx))(item) for idx, item in enumerate(self.value)
            ]
