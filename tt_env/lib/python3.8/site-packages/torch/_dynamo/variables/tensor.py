    ...
    def specialize(value: torch.Tensor):
        ...
        if not has_free_symbols(value):
            ...
            props["is_contiguous"] = tuple(
                [x for x in torch._prims_common._memory_formats if value.is_contiguous(memory_format=x)]
            )
        return props
    ...
    def call_method(
        ...
        if name in ("stride", "size"):
            ...
            torch_fn = VariableBuilder(
                tx,
                AttrSource(AttrSource(args[0].source, "__torch_function__"), "__func__"),
        ...
        elif name == "requires_grad_" and self.as_proxy().node.meta["example_value"].requires_grad != (
...
class NumpyNdarrayVariable(TensorVariable):
    ...
    def var_getattr(self, tx, name):
        ...
        def insert_into_graph():
            return wrap_fx_proxy(
                tx,
                tx.output.create_proxy("call_function", numpy_attr_wrapper, (self.as_proxy(), name), {}),
            )
