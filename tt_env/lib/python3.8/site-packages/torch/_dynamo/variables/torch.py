...
class TorchCtxManagerClassVariable(BaseTorchVariable):
    ...
    def call_function(
        ...
        if self.value is torch.no_grad:
            if len(args) == 1 and isinstance(args[0], variables.functions.BaseUserFunctionVariable):
            ...
            return GradModeVariable.create(tx, args[0].as_python_constant(), initialized=True)
...
class TorchInGraphFunctionVariable(BaseTorchVariable):
    ...
    def call_function(
        ...
        if self.can_constant_fold_through() and (constant_args or unspec_python_args):
            ...
            for method in [device_interface.stream for _, device_interface in get_registered_device_interfaces()]
        ):
            assert len(args) == 1
        ...
        # generically support torch.ops that return int
            ...
            all_ints_or_floats = all(
                isinstance(x, (variables.ConstantVariable, variables.SymNodeVariable)) for x in args
            )
...
Either create the tensor outside the compiled region, or do not set the tensor to require_grad"""
            ...
            if "out" in kwargs and not (
                isinstance(kwargs["out"], variables.ConstantVariable) and kwargs["out"].as_python_constant() is None
            ):
    ...
    def call_function(
        ...
        if self.can_constant_fold_through() and (constant_args or unspec_python_args):
            ...
            if self.value is torch.nn.CrossEntropyLoss:
                ...
                return variables.UserDefinedClassVariable(self.value, source=self.source).call_function(
                    tx, args, kwargs
                )
        elif can_dispatch_torch_function(tx, args, kwargs):
            return dispatch_torch_function(tx, self, args, kwargs)
