...
class TensorWithTFOverrideVariable(TensorVariable):
    ...
    def var_getattr(self, tx, name):
        ...
        if tx.output.torch_function_enabled:
            if self.source:
                install_guard(
                    AttrSource(AttrSource(self.source, "__class__"), name).make_guard(GuardBuilder.FUNCTION_MATCH)
                )
    ...
    def call_method(
        ...
        if tx.output.torch_function_enabled:
            ...
            if self.source:
                func_var = VariableBuilder(tx, AttrSource(AttrSource(self.source, "__class__"), name))(
