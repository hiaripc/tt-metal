...
class UserDefinedObjectVariable(UserDefinedVariable):
    ...
    def call_method(
        ...
        if method is not None:
            ...
            if (
                ...
                and inspect.getattr_static(type(self.value), "keys") in (collections.OrderedDict.keys, dict.keys)
            ):
                assert not kwargs
                assert self.source  # OrderedDict, dict subtypes must always have source
                install_guard(self.source.make_guard(GuardBuilder.ODICT_KEYS))
    ...
    def call_function(
        ...
        if (
            ...
            if (
                ...
                and trace_rules.lookup(obj.__class__) == variables.TorchCtxManagerClassVariable
            ...
            if self.source:
                install_guard(
                    ...
                    AttrSource(self.source, "args").make_guard(GuardBuilder.CONSTANT_MATCH),
                    AttrSource(self.source, "keywords").make_guard(GuardBuilder.CONSTANT_MATCH),
                )
    ...
    def var_getattr(self, tx, name):
        ...
        try:
            ...
            if isinstance(getattr_fn, types.FunctionType):
                return variables.UserMethodVariable(getattr_fn, self, source=source).call_function(
                    tx, [ConstantVariable.create(name)], {}
                )
        ...
        if isinstance(subobj, property):
            ...
            return variables.UserMethodVariable(subobj.__get__.__func__, subobj_var, source=source).call_function(
                tx, [self], {}
            )
        ...
        if (
            ...
            if not source:
                ...
                source = AttrSource(
                    AttrSource(tx.import_source(type(value).__module__), type(value).__name__),
                    name,
                )
    ...
    def odict_getitem(self, tx, key):
        ...
        index = (
            key.source if ConstDictVariable.is_valid_key(key) and key.source is not None else key.as_python_constant()
        )
