    ...
    def tree_zip(combined_args, dynamic_shapes):
        if isinstance(combined_args, (tuple, list)):
            ...
            if dynamic_shapes is not None:
                raise UserError(
                    UserErrorType.INVALID_INPUT,
                    f"Expected dynamic_shapes of a {type(combined_args)} to be None, " f"got {dynamic_shapes} instead",
                )
    ...
    def check_same_bounds(dim):
        if dim.__name__ in symbols:
            ...
            if dim.min != min_ or dim.max != max_:
                ...
                raise UserError(
                    UserErrorType.INVALID_INPUT,
                    f"Found different definitions {this_} and {that_} " f"for the same symbolic dimension {dim}!",
                )
            ...
            for i, dim in shape.items():
                if isinstance(dim, _Dim):
                    ...
                    if dim is not None:
                        raise UserError(
                            UserErrorType.INVALID_INPUT,
                            f"Unexpected item #{i} ({dim}) in dynamic_shape {shape} of Tensor, " "try None instead",
                        )
...
def dynamic_dim(t: torch.Tensor, index: int, debug_name: Optional[str] = None):
    if not isinstance(t, torch.Tensor):
        raise UserError(UserErrorType.DYNAMIC_DIM, f"Expected tensor as input to dynamic_dim but got {type(t)}")

    if t.dim() < 1:
        raise UserError(UserErrorType.DYNAMIC_DIM, "Cannot mark 0-dimension tensors to be dynamic")
    ...
    return _create_constraint(
        ...
        id(t),
        index,
        StrictMinMaxConstraint(vr=ValueRanges(lower=2, upper=sympy.oo), warn_only=False),
...
def capture_pre_autograd_graph(
    ...
    with torch._dynamo.config.patch(dataclasses.asdict(DEFAULT_EXPORT_DYNAMO_CONFIG)):
        ...
        m.meta["inline_constraints"] = {
            k: v for k, v in fake_mode.shape_env.runtime_var_to_range.items() if re.match(r"^[if]\d+$", str(k))
        }
...
def _convert_input_to_fake(gm, args, kwargs):
    ...
    fake_params_buffers = pytree.tree_map_only(
        ...
        {**dict(gm.named_parameters(remove_duplicate=False)), **dict(gm.named_buffers(remove_duplicate=False))},
    )
        ...
        for node in gm.graph.nodes:
            ...
            if add_root:
                ...
                node.meta["nn_module_stack"] = {
                    key: (normalize_path(path), ty) for key, (path, ty) in nn_module_stack.items()
                }
...
def _lift_buffers_to_user_inputs(
    gm: torch.fx.GraphModule, graph_signature: GraphSignature, user_input_names: List[str]
) -> Dict[str, str]:
    ...
    *,
    ...
    if dynamo_fake_mode:
        gm.meta["inline_constraints"] = {
            k: v for k, v in dynamo_fake_mode.shape_env.runtime_var_to_range.items() if re.match(r"^[if]\d+$", str(k))
        }
    ...
    *,
    ...
    with zipfile.ZipFile(f, "r") as zipf:
        ...
        if version != SCHEMA_VERSION:
            raise RuntimeError(
                f"Serialized version {version} does not match our current " f"schema version {SCHEMA_VERSION}."
            )
...
def aot_compile(
    ...
    *,
    ...
    """
    if constraints is not None:
        warnings.warn("The constraints field is deprecated. " "Please use dynamic_shapes instead.")
    ...
    gm = _export_to_torch_ir(f, args, kwargs, constraints, disable_constraint_solver=disable_constraint_solver)
