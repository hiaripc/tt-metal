...
def serialize_sym_bool(s: Union[bool, torch.SymBool]) -> SymBool:
    if isinstance(s, (torch.SymBool, bool)):
        ...
        raise SerializeError(f"SymBool should be either symbol or bool, got `{s}` of type `{type(s)}`")
...
class GraphModuleSerializer:
    ...
    def serialize_input(self, arg) -> Argument:
        ...
        if isinstance(arg, torch.fx.Node):
            ...
            if not (
                arg._has_method("__getstate__")
                and arg._has_method(  # type: ignore[attr-defined]
                    "__setstate__"
                )  # type: ignore[attr-defined]
            ):
                raise SerializeError(
                    f"Unable to serialize custom class {arg}. Please define " "serialization methods via def_pickle()."
                )
    ...
    def serialize_outputs(self, node: torch.fx.Node) -> List[Argument]:
        ...
        for idx, (meta, return_schema) in enumerate(zip(meta_val, returns)):
            if meta is None:
                ...
                assert isinstance(return_schema.real_type, torch.ListType) and isinstance(
...
class GraphModuleDeserializer:
    ...
    def deserialize_node(self, serialized_node: Node, target: Callable) -> None:
        if target.__module__ == "_operator":  # TODO(zhxchen17) Follow up on this.
            ...
            assert len(serialized_node.outputs) == 1 and serialized_node.outputs[0].type in (
                "as_tensors",
                "as_tensor",
            ...
            name = output.value.name if output.type == "as_tensor" else None  # FX will generate a name for us.
    ...
    def deserialize_input_spec(self, i: InputSpec) -> ep.InputSpec:
        if i.user_input is not None:
            return ep.InputSpec(
                kind=ep.InputKind.USER_INPUT, arg=self.deserialize_argument_spec(i.user_input.arg), target=None
            )
    ...
    def deserialize_metadata(self, metadata: Dict[str, str]) -> Dict[str, Any]:
        ...
        if nn_module_stack_str := metadata.get("nn_module_stack"):
            ...
            nn_module_stack = dict(
                import_nn_module_stack(*item.split(",")) for item in nn_module_stack_str.split(ST_DELIMITER)
            )
...
def serialize(
    ...
    serialized_artifact = ExportedProgramSerializer(opset_version).serialize(exported_program)
    assert isinstance(serialized_artifact.exported_program, ExportedProgram)
    json_program = json.dumps(dataclasses.asdict(serialized_artifact.exported_program), cls=EnumEncoder)
    json_bytes = json_program.encode("utf-8")
    artifact = SerializedArtifact(json_bytes, serialized_artifact.state_dict, serialized_artifact.constants)
    return artifact
...
def deserialize(
    ...
    return ExportedProgramDeserializer(expected_opset_version).deserialize(
        SerializedArtifact(serialized_exported_program, artifact.state_dict, artifact.constants)
    )
