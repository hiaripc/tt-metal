    ...
    def __init__(
        ...
        if parent is not None:
            ...
            parent.graph_module.add_submodule(
                accessor,
                self.graph_module if self.cached_graph_module is None else self.cached_graph_module,
            )
        ...
        if signature is not None and self.parent is not None:
            ...
            with self.graph_module.graph.inserting_after(None):
                ...
                for idx, arg in enumerate(signature.inputs):
                    flat_arg_node = self.graph_module.graph.create_node(
                        ...
                        args=(flat_args, idx),
                        name=arg.name if not isinstance(arg, ConstantArgument) else f"_constant_{idx}",
                    )
            ...
            with self.parent.graph.inserting_before(self.parent_call_module):
                ...
                kwarg_nodes = {
                    k: self.parent.graph.call_function(operator.getitem, (kwargs_node, k)) for k in kwargs_spec.context
                }
    ...
    def finalize_outputs(self):
        ...
        if signature is not None and self.parent is not None:
            ...
            tree_out_node = _generate_unflatten(
                self.graph_module,
                tuple(self.node_map[self.seen_nodes[output.name]] for output in orig_outputs),
                signature.out_spec,
            )
...
def _outline_submodules(orig_graph: torch.fx.Graph, root_module: torch.fx.GraphModule):
    ...
    ModuleFrame(
        ...
        [""],
        "",
        {entry.fqn: entry.signature for entry in root_module.module_call_graph if entry.signature},
