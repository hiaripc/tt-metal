...
def aot_dispatch_base(
    ...
    *,
    ...
    with context(), track_graph_compiling(aot_config, "inference"):
        compiler = (
            aot_config.inference_compiler if aot_config.inference_compiler is not None else aot_config.fw_compiler
        )
    ...
    *,
    ...
    class CompiledFunction(torch.autograd.Function):
        ...
        def forward(ctx, *deduped_flat_tensor_args):
            ...
            assert all(isinstance(x, (int, float, torch.SymInt, torch.SymFloat)) for x in symint_outs), str(
                [type(x) for x in symint_outs]
            )
            ...
            raw_returns_meta = [
                x for x in CompiledFunction.metadata.input_info if x.mutation_type == MutationType.MUTATED_OUT_GRAPH
...
Got grad_output types: {str(grad_output_types)}"""
            ...
            all_args = [
                t.contiguous() if tangents_start_idx <= i < tangents_end_idx else t for i, t in enumerate(all_args)
            ]
