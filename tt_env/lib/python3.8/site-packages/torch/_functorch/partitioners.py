...
def _extract_fwd_bwd_modules(joint_module: fx.GraphModule, saved_values, saved_sym_nodes, *, num_fwd_outputs):
    ...
    fwd_graph = _extract_graph_with_inputs_outputs(
        joint_module.graph, primal_inputs + fwd_seed_offset_inputs, fwd_outputs + saved_values + saved_sym_nodes
    )
    bwd_graph = _extract_graph_with_inputs_outputs(
        joint_module.graph, saved_sym_nodes + saved_values + tangent_inputs + bwd_seed_offset_inputs, bwd_outputs
    )
...
def _size_of(node: fx.Node) -> int:
    if "val" in node.meta:
        ...
        # layering violation)
            ...
            return sum(
                _tensor_nbytes(hint_int(n.numel(), fallback=4098), n.dtype) for n in val if isinstance(n, torch.Tensor)
            )
    ...
    for base_node, node_pair in recomputable_rng_ops_map.items():
        ...
        with fw_graph.inserting_before(fw_node):
            functional_fw_node = fw_graph.create_node(
                "call_function", run_and_save_rng, args=(fw_node.target, *fw_node.args), kwargs=fw_node.kwargs
            )
...
def min_cut_rematerialization_partition(
    joint_module: fx.GraphModule, _joint_inputs, compiler="inductor", recomputable_ops=None, *, num_fwd_outputs
) -> Tuple[fx.GraphModule, fx.GraphModule]:
    """
    ...
    def classify_nodes(joint_module):
        ...
        unclaimed_nodes = {
            node for node in joint_module.graph.nodes if node not in required_fw_nodes and node not in required_bw_nodes
        }
