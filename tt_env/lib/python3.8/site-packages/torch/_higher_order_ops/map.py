...
def create_fw_bw_graph(f, num_mapped_args, *args):
    ...
    with suspend_functionalization(), unset_functional_temporarily():
        with disable_proxy_modes_tracing():
            ...
            example_pos_args = [_from_fun(arg) if isinstance(arg, torch.Tensor) else arg for arg in pos_args]
            example_flat_out = pytree.tree_map(_from_fun, f(*example_xs, *example_pos_args))
        ...
        def joint_f(*example_args):
            ...
            def fw_with_masks(*args):
                fw_out = f(*args)
                return fw_out, [
                    True if isinstance(ret, torch.Tensor) and ret.requires_grad else False for ret in fw_out
                ]
            ...
            input_storage = {
                StorageWeakRef(arg._typed_storage()) for arg in example_args if isinstance(arg, torch.Tensor)
            }
...
def _unstack_pytree(xs):
    ...
    ctx = FunctionalTensorMode if any(isinstance(x, FunctionalTensor) for x in flat_xs) else nullcontext
    with ctx():
        a = zip(*flat_xs)
