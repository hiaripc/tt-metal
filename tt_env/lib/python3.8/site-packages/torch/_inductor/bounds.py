...
class BoundVars:
    ...
    def __init__(self, loop_body: LoopBody) -> None:
        ...
        self.replacement_vals = {
            k: ValueRanges(0, v - 1) if (isinstance(v, int) or v.is_number) else bound_sympy(v)
        ...
        self.unbounded_vars = dominated_nodes(
            ...
            if node.target in ["load", "reduction", operator.getitem] or "masked_subblock" in node.target
        )
    ...
    def get_bounds(self) -> Dict[torch.fx.Node, ValueRanges]:
        ...
        for node in self.unbounded_vars:
            ...
            if not isinstance(node.target, str) or (
                "masked_subblock" not in node.target and "set_indirect" not in node.target
            ):
