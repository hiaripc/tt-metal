    ...
    def lookup(
        ...
        if config.max_autotune or config.max_autotune_gemm:
            ...
            if not check_cache(local_cache) and not (
                use_global_cache() and check_cache(self.get_global_cache(), callback=log_stats)
            ):
                try:
...
def get_include_and_linking_paths(
    ...
    if sys.platform == "linux" and (
        include_pytorch or vec_isa != invalid_vec_isa or cuda or config.cpp.enable_kernel_profile
    ):
...
class CudaKernelParamCache:
    ...
    def set(cls, key: str, params: Dict[str, str], cubin: str) -> None:
        ...
        _, path = write(
            ...
            bin_type,
            hash_type=bin_type,
            specified_dir=split_aot_inductor_output_path(config.aot_inductor.output_path)[0],
        )
    ...
    def compile(
        ...
        if key not in cls.cache or (
            ...
            with lock:
                ...
                output_so = (
                    config.aot_inductor.output_path if specified_so_name else os.path.splitext(input_path)[0] + ".so"
                )
                ...
                if not os.path.exists(output_so):
                    ...
                    if fbcode_aot_cpu_re:
                        cmd = (
                            f"{ld_command} -r -b binary -o {os.path.basename(consts_o)} {os.path.basename(consts_path)}"
                        )
...
def _cuda_lib_options() -> List[str]:
    ...
    if is_linux():
        extra_lib_dir = "lib64"
        if not os.path.exists(cpp_extension._join_cuda_home(extra_lib_dir)) and os.path.exists(
            cpp_extension._join_cuda_home("lib")
        ):
...
def cuda_compile_command(
    ...
    options = (
        ...
        + [f"-Xcompiler {opt}" if "=" in opt else f"-Xcompiler={opt}" for opt in nvcc_host_compiler_options]
