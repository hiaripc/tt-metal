...
class CppVecKernel(CppKernel):
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        if is_broadcast:
            ...
            line = (
                f"masked_load({loadbuf}, {load_mask})" if load_mask else f"at::vec::Vectorized<float>::loadu({loadbuf})"
            )
...
class CppTile2DKernel(CppVecKernel):
    ...
    def need_vec_transpose(self, index):
        return (
            ...
            and not stride_at(self.itervars[self.tiling_idx], index).has(self.itervars[self.tiling_idx])
            and not stride_at(self.itervars[self.tiling_idx], index).has(self.itervars[self.outer_idx])
        )
    ...
    def gen_transposed_tile_load_store(self, name, var, index, is_store):
        ...
        need_define = True
        load_or_store = (
            f"at::vec::transpose_mxn<{DTYPE_TO_CPP[dtype]},{factor},{factor}>({src}, {ld_src}, {dst}, {ld_dst});"
        )
        if is_store:
    ...
    def set_ranges(self, group, reduction_group):
        ...
        self.outer_idx, self.tiling_idx = (
            self.tiling_indices if self.tiling_indices[1] < self.reduction_depth else reversed(self.tiling_indices)
        )
        return vars
        ...
        class VecCheckerProxy:
            ...
            def constant(val, dtype):
                with RecordOptimizationContext(__name__) as node_ctx:
                    ...
                    if opt_ctx.dtype not in supported_dtypes or (
                        opt_ctx.dtype == torch.int32
                        and not all(user.target in VecCheckerProxy.bin_cmp_ops for user in node_ctx.current_node.users)
                    ):
            ...
            def index_expr(expr, dtype):
                ...
                if not len(self.ranges) or not all(
                    not isinstance(range, sympy.Expr) or sympy.simplify(range).is_number for range in self.ranges
                ):
                ...
                def can_use_int32():
                    ...
                    return range_expressable_in_32_bits(
                        ValueRanges(int(expr_ranges.lower), int(expr_ranges.upper) + 1)  # type: ignore[arg-type]
                    )
                ...
                with RecordOptimizationContext(__name__) as node_ctx:
                    ...
                    if (
                        dtype == torch.int64
                        and can_use_int32()
                        and all(user.target in VecCheckerProxy.bin_cmp_ops for user in node_ctx.current_node.users)
                    ):
                        opt_ctx.dtype = torch.int32
                    else:
                        opt_ctx.dtype = dtype
                ...
                with RecordOptimizationContext(__name__) as node_ctx:
                    ...
                    if dtype == torch.float:
                        ...
                        is_to_uint8_and_to_float = all(
                            (usr.target in ["to_dtype"] and usr.args[2] == torch.float32) for usr in cur_node.users
                        )
...
class CppKernelProxy(CppKernel):
                        ...
                        for node, users in node_users.items():
                            if node in sub_graph.nodes and (
                                ...
                                or (
                                    ...
                                    and all(usr.args[-1] in DTYPE_LOWP_FP for usr in users)
                                )
                            ):
    ...
    def codegen_nodes(self, nodes):
        ...
        vec_dtype = (
            ...
            if all(
                hasattr(_node, "_lowp_fp_type") and _node._lowp_fp_type == first_node._lowp_fp_type for _node in nodes
            )
            else torch.float
        )
        ...
        with torch._inductor.config.patch(inplace_buffers=False):
            ...
            if len(tiling_indices) == 1:
                ...
                inner_main_loop.set_kernel(
                    codegen_kernel(CppTile2DKernel, tiling_factors[0], tiling_indices, vec_dtype)
                )
                inner_tail_loop.set_kernel(
                    codegen_kernel(CppVecKernel, tiling_factors[0], tiling_indices[0], vec_dtype)
                )
...
class LoopLevel:
    ...
    def lines(self):
        ...
        if self.reduction_var_map:
            reduction = " " + " ".join(
                f"reduction({RTYPE_TO_CPP[rtype]}:{var})" for var, rtype in self.reduction_var_map.items()
            )
        else:
            reduction = ""
