    ...
    def gen_ops(self) -> "List[cutlass_gemm_op.GemmOperation]":  # type: ignore[name-defined]
        ...
        for op in res.values():
            if op.gemm_kind == cutlass_lib.GemmKind.Universal3x:
                ...
                num_2x_ops += 1
        log.debug(
            "Got cutlass configs: total number of ops: %d, " "total number of 3x ops: %d, total number of 2x ops: %d",
            len(res),
            num_3x_ops,
            num_2x_ops,
        )
        ...
        if op.gemm_kind == cutlass_lib.GemmKind.Universal3x:
            if Bias is not None and self.has_tma_epilogue(op):
                if self.should_swap_XW(Bias, self.beta):
                    ...
                    should_swap_xw = True
            if epilogue_nodes is not None and len(epilogue_nodes) > 0:
                epilogue_args = CutlassEVTEpilogueArgumentFormatter.ir_to_evt_argument_string(
