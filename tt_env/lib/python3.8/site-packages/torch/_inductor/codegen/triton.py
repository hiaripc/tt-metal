...
class TritonOverrides(OpOverrides):
    ...
    def to_dtype(x, dtype: torch.dtype, src_dtype: Optional[torch.dtype] = None):
        def _get_min_elements_per_thread(src_dtype: torch.dtype, dst_dtype: torch.dtype) -> int:
            ...
            assert not (
                src_dtype in fp8_dtypes and dst_dtype in fp8_dtypes and src_dtype != dst_dtype
...
class TritonKernel(Kernel):
    ...
    def initialize_range_tree(self, pid_cache):
        ...
        for i in range(len(self.numels)):
            ...
            self.range_trees.append(
                IterationRangesRoot(names[i], self.numels[i], names[i][0], pid_idx, self, pid_cache)
            )
        for tree in self.range_trees:
    ...
    def split_and_set_ranges(self, lengths: List[List[sympy.Expr]]):
        ...
        if len(lengths) == len(self.range_trees) and all(
            V.graph.sizevars.simplify(sympy_product(x) - g) == 0 for x, g in zip(lengths, groups)
        ):
    ...
    def is_broadcasted(self, index: sympy.Expr):
        ...
        return any(
            simplify(idx_range) != simplify(iter_range) for idx_range, iter_range in zip(index_numels, self.numels)
        )
    ...
    def codegen_kernel_benchmark(self):
        ...
        with result.indent():
            ...
            with result.indent():
                result.writeline(f"torch.cuda.set_device({index})")  # no-op to ensure context
                for tree in self.range_trees:
        ...
        for arg_name in call_args:
            ...
            if buf and len(buf.layout.size) == 4:
                ...
                if uniform_stride_order is None:
                    ...
                    size_list = [
                        V.graph.get_buffer(name).layout.size if V.graph.get_buffer(name) else None for name in call_args
                    ]
                    source_list = [
                        "GraphInput"
...
class TritonScheduling(BaseScheduling):
        ...
        if not node1.is_reduction() and node2.is_reduction():
            ...
            if numel1 == numel2 * rnumel2:
                ...
                if config.triton.tiling_prevents_reduction_fusion and not node1.is_template():
                    is_reduction_tiling_valid = self.select_tiling(node1.get_nodes(), numel1) in (
                        (numel1, 1),
                        (numel2, rnumel2, 1),
                    )
    ...
    def can_use_32bit_indexing(numel: sympy.Expr, buffers: Iterable[Union[ir.Buffer, ir.TensorBox]]) -> bool:
        ...
        buf_sizes = [
            buf.get_layout().storage_size() for buf in buffers if not isinstance(buf.get_layout(), ir.MultiOutputLayout)
        ]
        ...
        def _get_buffer(name: str) -> Union[ir.Buffer, ir.TensorBox]:
            if name in V.graph.name_to_buffer:
                ...
                return ir.ConstantBuffer(
                    name,
                    ir.FixedLayout(data.device, data.dtype, *V.graph.static_sizes_strides(data)),
                )
    ...
    def get_kernel_args(self, node_schedule, numel, reduction_numel):
        reductions = list(
            filter(
                lambda n: n not in (EnableReduction, DisableReduction) and n.is_reduction(),
                node_schedule,
            )
        )
        if len(reductions) > 0:
    ...
    def candidate_tilings(node):
        ...
        for dep in deps:
            ...
            score = V.graph.sizevars.size_hint(
                sympy_product(size for size, stride in zip(ranges, strides) if stride != 0)
            )
