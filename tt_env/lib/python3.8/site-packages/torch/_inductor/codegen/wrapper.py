...
class EnterCudaDeviceContextManagerLine:
    ...
    def codegen(self, code: IndentedBuffer, device_cm_stack: contextlib.ExitStack):
        if V.graph.cpp_wrapper:
            ...
            if V.graph.aot_mode:
                ...
                if self.last_seen_device_guard_index is None:
                    if config.aot_inductor.abi_compatible:
                        code.writeline("AOTICudaStreamGuard stream_guard(stream, this->device_idx_);")
                    else:
                        code.writeline(
            ...
            code.writeline(f"torch.cuda.set_device({self.device_idx}) # no-op to ensure context")
...
class ReuseLine(MemoryPlanningLine):
    ...
    def codegen(self, code: IndentedBuffer):
        assert self.node.get_name() not in V.graph.removed_buffers
        assert self.reused_as.get_name() not in V.graph.removed_buffers
        code.writeline(self.wrapper.make_buffer_reuse(self.node, self.reused_as, self.delete_old))
...
class WrapperCodeGen(CodeGen):
    ...
    def __init__(self):
        ...
        self.write_get_raw_stream = functools.lru_cache(None)(self.write_get_raw_stream)  # type: ignore[assignment]
    ...
    def make_allocation(self, name, device, dtype, shape, stride):
        try:
            ...
            expected = None
        if stride == expected:
            return f"{name} = empty(" f"{self.codegen_shape_tuple(shape)}, " f"device='{device.type}', dtype={dtype})"
        else:
            return (
...
class CppWrapperCodeGen(WrapperCodeGen):
                ...
                if V.graph.aot_mode:
                    ...
                    if config.aot_inductor.abi_compatible:
                        ...
                        self.prefix.writeline(
                            f"auto {constants_key} = *tensor_handle_to_tensor_pointer(" + f"""constants_.at({idx}));"""
                        )
                else:
    ...
    def codegen_model_constructor(self):
        ...
        with self.prefix.indent():
            for idx, (name, inp) in enumerate(V.graph.graph_inputs.items()):
                assert not isinstance(inp, sympy.Expr), f"input {name=} cannot be symbolic"
            ...
            for idx, output in enumerate(V.graph.graph_outputs):
                assert not isinstance(output, sympy.Expr), f"output {name=} cannot be symbolic"
                name = f"output{idx}"
    ...
    def generate_extern_kernel_args_decl_if_needed(self, op_overload, raw_args, output_args):
        ...
        def fill_args(arg, arg_type):
            ...
            if isinstance(arg_type, torch.TensorType):
                ...
                elif isinstance(arg_type.getElementType(), torch.OptionalType) and isinstance(
    ...
    def generate_extern_kernel_alloc_and_find_schema_if_needed_fbcode(
        ...
        (
            tensor_call_args,
            int_call_args,
        ) = self.generate_extern_kernel_args_decl_if_needed(op_overload, raw_args, output_args)
...
class CudaWrapperCodeGen(CppWrapperCodeGen):
    ...
    def generate_kernel_call(self, name, call_args, grid=None, device_index=None, cuda=True, triton=True):
        ...
        assert isinstance(grid, (list, tuple)), f"expected grid to be a list or tuple but got: {grid=}"
