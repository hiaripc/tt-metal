    ...
    for snode in order:
        if cur_comm_node is None:
            ...
            if isinstance(snode.node, ir.CollectiveKernel):
                ...
                overlap_log.debug(f"| {node_summary(snode)}")  # noqa: G004
    overlap_log.debug(f"Est. runtime (ms): {total_est_runtime / 1000 / 1000}")  # noqa: G004
...
def reorder_compute_and_comm_for_overlap(
    ...
    for p in config.reorder_for_compute_comm_overlap_passes:
        ...
        if torch.distributed.get_rank() == 0:
            overlap_log.debug(f"==== Visualize overlap before reordering pass {p} ====")  # noqa: G004
            try:
        ...
        if torch.distributed.get_rank() == 0:
            overlap_log.debug(f"==== Visualize overlap after reordering pass {p} ====")  # noqa: G004
            try:
                visualize_overlap(order)
            except Exception as e:
                overlap_log.debug(str(e))
