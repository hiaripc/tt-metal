...
def compile_fx_inner(
    ...
    if config.fx_graph_cache and not aot_mode:
        ...
        compiled_graph = fx_codegen_and_compile(gm, example_inputs, **graph_kwargs)  # type: ignore[arg-type]
    ...
    if cudagraphs:
        ...
        complex_memory_overlap_inputs = any(
            complex_memory_overlap(t) for t in example_inputs if isinstance(t, torch.Tensor)
        )
        ...
        cudagraph_tests = [
            ...
            (
                all(isinstance(t, (torch.Tensor, torch.SymInt)) for t in example_inputs),
                "non-Tensor inputs",
            ),
            (
                (len(compiled_graph.device_idxs) == 1 or not config.triton.cudagraph_trees),
    ...
    _step_logger()(
        logging.INFO,
        "torchinductor done compiling " f"{'BACKWARDS' if is_backward else 'FORWARDS'} " f"graph {graph_id}",
    )
...
def fx_codegen_and_compile(
    ...
    _step_logger()(
        logging.INFO,
        "torchinductor compiling " f"{'BACKWARDS' if is_backward else 'FORWARDS'} " f"graph {graph_id}",
    )
...
def get_input_idxs_to_check(
    ...
    for i, input in enumerate(inputs):
        if (
            ...
            and (i not in static_input_idxs or not is_aligned(input.storage_offset(), input.dtype))
...
def cudagraphify_impl(
    ...
    static_inputs = [
        x if not isinstance(x, torch.Tensor) else static_input(x) if idx not in static_input_idxs else x.detach()
...
def compile_fx_aot(
    ...
    config_patches: Dict[str, Any] = (
        {"cpp_wrapper": True} if config_patches is None else {**config_patches, "cpp_wrapper": True}
    )
