...
def invalidate_eager_modules():
    for mod in torch._guards.TracingContext.get().module_context.nn_modules.values():
        ...
        for attr_name, tensor in list(
            itertools.chain(mod.named_parameters(recurse=False), mod.named_buffers(recurse=False))
        ):
...
def enforce_output_layout(gm: torch.fx.GraphModule):
    ...
    *_, output_node = gm.graph.nodes
    ...
    with gm.graph.inserting_before(output_node):
        for n in out_list:
            if not isinstance(n.meta["val"], torch.Tensor) or not torch._prims_common.is_non_overlapping_and_dense(
                n.meta["val"]
            ):
                continue
...
def convert_conv_weights_to_channels_last(gm: torch.fx.GraphModule):
    ...
    for conv in convs:
        ...
        if len(weight_node.meta["val"].size()) != 4 or weight_node.meta["val"].is_contiguous(
