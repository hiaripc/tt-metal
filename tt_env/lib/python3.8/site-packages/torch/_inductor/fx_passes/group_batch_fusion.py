    ...
    def match(self, node: torch.fx.Node):
        if CallFunctionVarArgs(torch.nn.functional.layer_norm).match(node):
            ...
            group_key = (
                ...
                if "example_value" in input.meta and is_node_meta_valid(weight) and is_node_meta_valid(bias)
                else None
            )
        else:
            group_key = None
    ...
    for node in reversed(graph.nodes):
        ...
        for key, candidate_nodes in candidates.items():
            ...
            for subset in find_independent_subset_greedy(candidate_nodes, rule.graph_search_options):
                ...
                log.info(f"{rule.__class__.__name__}: key = {key}; subset size = {len(subset)}")  # noqa: G004
