...
if torch._C._has_mkldnn:
                    ...
                    CallFunction(
                        ...
                        CallFunction(
                            ...
                            CallFunction(
                                ...
                                CallFunction(
                                    aten.mul,
                                    CallFunction(aten.mul, computation_call, computation_call),
                                    computation_call,
                                ),
                                0.044715,
                            ),
    ...
    def _hardswish_fusion(computation_call):
        return CallFunction(
            ...
            CallFunction(
                ...
                CallFunction(
                    aten.clamp_max,
                    CallFunction(aten.clamp_min, CallFunction(aten.add, computation_call, 3), 0),
                    6,
                ),
            ),
            6,
    ...
    def _is_valid_binary(match, fn):
        ...
        if len(binary_nodes) < 1:
            return False
        if any(
            not (hasattr(n.args[0], "meta") and isinstance(n.args[0].meta.get("val", None), torch.Tensor))
        ...
        # check alpha is one.
        if any(
            get_arg_value(n, 2, kwarg_name="alpha") != 1.0 and get_arg_value(n, 2, kwarg_name="alpha") is not None
            for n in binary_nodes
        ):
            return False
        if any(
    ...
    def _register_binary_unary_maybe_inplace_fusion_lowering(
        ...
        @register_lowering_pattern(
            pattern,
            extra_check=_is_valid_computation_binary_inplace(computation_op, binary_op, other_index),
        )
        def fn(match, *args, **kwargs):
            other = kwargs.get("other")
            assert isinstance(other, ir.TensorBox)
    ...
    def _register_unary_fusion():
        ...
        def _unary_fusion_patterns(is_bf16):
            replacement_unary_fusion_patterns = {
                UnaryAttr("gelu", algorithm_attr="tanh"): [
                    _unary_fusion_pattern(_gelu_fusion_2, call_fn, 4, is_bf16) for call_fn in computation_call_fns
                ],
                UnaryAttr("gelu", algorithm_attr="none"): [
                    _unary_fusion_pattern(_gelu_fusion_1, call_fn, 2, is_bf16) for call_fn in computation_call_fns
                ],
                UnaryAttr("hardswish"): [
                    _unary_fusion_pattern(_hardswish_fusion, call_fn, 2, is_bf16) for call_fn in computation_call_fns
                ],
                UnaryAttr("hardsigmoid"): [
                    _unary_fusion_pattern(_hardsigmoid_fusion, call_fn, 1, is_bf16) for call_fn in computation_call_fns
                ],
                UnaryAttr("swish"): [
                    _unary_fusion_pattern(_silu_fusion, call_fn, 2, is_bf16) for call_fn in computation_call_fns
                ],
            }
            if not is_bf16:
                ...
                replacement_unary_fusion_patterns.update(
                    {
                        ...
                        UnaryAttr("sigmoid"): [_combined_fusion(u, aten.sigmoid) for u in call_user1],
        ...
        for is_bf16 in [True, False]:
            ...
            _leaky_relu_patterns = [
                _unary_fusion_pattern(_leaky_relu_fusion, call_fn, 3, is_bf16) for call_fn in computation_call_fns
            ]
            ...
            hardtanh_patterns = [
                _unary_fusion_pattern(_hardtanh_fusion, call_fn, 1, is_bf16) for call_fn in computation_call_fns
            ]
    ...
    def _is_packable_mkldnn_rnn_layer(match):
        ...
        # Check dtype
        if any(
            lstm_node.args[POS_ARG].meta.get("val").dtype == torch.bfloat16 and not mkldnn._is_mkldnn_bf16_supported()
    ...
    def _register_weight_pack_pass():
        ...
        def linear(match, *args, **kwargs):
            ...
            with graph.inserting_before(linear_node):
                ...
                packed_weight_op = (
                    mkldnn._reorder_linear_weight if is_bf16_weight else torch.ops.mkl._mkl_reorder_linear_weight
                )
                packed_weight_node = graph.create_node("call_function", packed_weight_op, args=packed_weight_inputs)
