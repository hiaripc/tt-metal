...
def reorder_for_locality(graph: torch.fx.Graph):
    ...
    first_copy = next(
        (node for node in graph.nodes if node.op == "call_function" and node.target == torch.ops.aten.copy_.default),
        None,
    )
...
@register_lowering_pattern(
    CallFunction(aten.cat, ListOf(CallFunction(aten.addmm, Arg(), Arg(), Arg())), Arg()),
)
    ...
    for node in graph.nodes:
        if node.target in noop_registry:
            ...
            if get_node_storage(node) in output_storages and (
                get_node_storage(src) in input_storages or get_node_storage(src) in output_storages
            ):
                continue
...
class ConstructorMoverPass:
    def __init__(self, target: str, allow_outputs: bool = False) -> None:
        ...
        assert isinstance(target, str), (
            "target should be a string representing the device type. " f"Got: {type(target).__name__}"
        )
