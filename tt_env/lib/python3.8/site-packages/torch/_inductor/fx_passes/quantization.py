...
dequantize_per_channel_to_bf16_weight_pattern = _may_generate_pattern_with_dtype_convert(
...
def _register_quantized_conv_lowering(
    ...
    def qconv(match: Match, *args, **kwargs):
        ...
        assert kwargs["attr"] == "none"  # Expected no post op fused in weight prepack phase
...
def _register_quantized_linear_lowering(
    ...
    def qlinear(match: Match, *args, **kwargs):
        ...
        assert kwargs["postop_name"] == "none"  # Expected no post op fused in weight prepack phase
...
def _register_quantization_unary_fusion():
    ...
    for original_pattern_output_dtype in [torch.float32, torch.bfloat16]:
        ...
        conv_unary_replace_float_out_patterns = {
            UnaryAttr("relu", [], ""): generate_pattern_with_unary(dequantize_qconv_pt2e_pattern, aten.relu.default),
        ...
        linear_unary_replace_float_out_patterns = {
            UnaryAttr("relu", [], ""): generate_pattern_with_unary(qlinear_pt2e_pattern, aten.relu.default),
        }
    ...
    for int8_mixed_bf16_with_inplace_add in [False, True]:
        ...
        binary_replace_patterns = {
            BinaryUnaryAttr("add", 1.0, "none", [], ""): generate_pattern_with_output_quant(
                ...
                dtype=torch.bfloat16 if int8_mixed_bf16_with_inplace_add else torch.float32,
            ),
...
def _is_input_output_same_scale_zp(check_node):
    def fn(match):
        ...
        scales = [
            (mul_node.args[1] if mul_node.args[0].target is check_node else 1.0 / mul_node.args[1])
