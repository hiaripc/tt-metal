...
class SplitCatSimplifier:
    ...
    def replace_split(
        ...
        if len(split_ranges) == 1:  # We can completely eliminate the split node
            ...
            with graph.inserting_after(new_split):
                split_items = [
                    graph.call_function(operator.getitem, args=(new_split, i)) for i in range(len(split_ranges))
                ]
    ...
    def get_transform_params(
        ...
        for user_node, user_inputs in zip(next_users, user_inputs_list):
            ...
            for user_input in user_inputs:
                ...
                elif user_node.target == torch.stack:  # We need to unsqueeze inputs not coming through unbind into cat
                    transform_params.append((None, None, (cat_dim,), None))
...
def merge_split_squeeze(match: Match, split_input: torch.fx.Node, split_sizes: List[int], dim: int):
    ...
    with graph.inserting_before(match.output_node()):
        ...
        for item_index, getitem_node in sorted(
            [(getitem_node.args[1], getitem_node) for getitem_node in split.users.keys()]
        ):
