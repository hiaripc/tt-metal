...
class GraphLowering(torch.fx.Interpreter):
    ...
    def __init__(
        ...
        self.extern_node_serializer: Optional[Callable[[List[ir.ExternKernelNode]], Any]] = extern_node_serializer
        ...
        self.nodes_prefer_channels_last = self.find_nodes_prefer_channels_last() if self.layout_opt else set()
    ...
    def decide_layout_opt(gm, *, is_inference) -> bool:
        ...
        if (
            all(n.args[idx].meta["val"].device == torch.device("cpu") for n in conv_nodes for idx in [0, 1])
        ...
        def is_in_out_channel(n):
            return (
                n.args[1].meta["val"].size(0) * 2 <= n.args[1].meta["val"].size(1) and n.args[1].meta["val"].size(2) > 1
            )
    ...
    def call_function(self, target, args, kwargs):
        ...
        if target not in lowerings:
            ...
            if base_name in FALLBACK_ALLOW_LIST:
                ...
                error = MissingOperatorWithDecomp if get_decompositions([target]) else MissingOperatorWithoutDecomp
                log.info(
    ...
    def output(self, target, args, kwargs):
        ...
        for name, value in self.graph_inputs.items():
            assert isinstance(value, (TensorBox, sympy.Expr)), f"Unsupported inductor graph input type: {type(value)}"
    ...
    def run_node(self, n: torch.fx.Node):
        ...
        with ir.IRNode.current_origins(origins), self.set_current_node(n), V.set_current_node(n):
            if (
                n.op == "call_function"
                ...
                result = fallback_handler(n.target, add_to_fallback_set=False)(*args, **kwargs)
            ...
            is_input_for_as_strided = any(user.target in as_strided_ops for user in n.users)
            if (is_output or is_input_for_as_strided) and isinstance(n.meta["val"], torch.Tensor):
        ...
        if isinstance(result, TensorBox) and isinstance(result.data, ir.StorageBox):
            if isinstance(result.data.data, ir.Loops):
                ...
                result.data.data.origin_node = n
                if isinstance(result.data.data, ir.ComputedBuffer) and isinstance(result.data.data.data, ir.Loops):
                    result.data.data.data.origin_node = n
                # Not really multi-output, can straightforwardly recurse in
                elif isinstance(result.data.data, ir.MultiOutput) and not result.data.data.indices:
    ...
    def get_output_names(self):
        return [
            ...
            if not isinstance(node, ir.NoneAsConstantBuffer) and not isinstance(node, ir.ShapeAsConstantBuffer)
        ]
