...
class Reduction(Loops):
    ...
    def __str__(self):
        return Loops.__str__(self, names=("ranges", "reduction_ranges", "reduction_type"))  # type: ignore[call-arg]
    ...
    def num_splits(
        ...
        def inner_reduction_splits(reduction_numel_hint, numel_hint):
            ...
            if reduction_numel_hint * numel_hint <= min_elements_per_device:
                ...
                tmp_split_size = (reduction_numel_hint + num_threads * blocks_per_output - 1) // (
        ...
        def outer_reduction_splits(reduction_numel_hint, numel_hint):
            ...
            if reduction_numel_hint * numel_hint < min_elements_per_device:
                ...
                tmp_split_size = (reduction_numel_hint + rvals_per_thread * target_blocks - 1) // (
        ...
        def get_read_indices(r):
            ...
            range_vars = [
                r for r in read_writes.range_vars if isinstance(r, sympy.Expr) and not isinstance(r, sympy.Number)
            ]
            indices = []
            changed = False
    ...
    def _unroll_reduction_fn(inner_fn, reduction_ranges, reduction_type, src_dtype):
        ...
        def fn(index):
            return functools.reduce(
                combine_fn,
                (value_fn(index, rindex) for rindex in itertools.product(*[range(x) for x in reduction_ranges])),
            )
    ...
    def create(  # type: ignore[override]
        ...
        if (
            ...
            and V.graph.sizevars.size_hint(reduction_numel) < config.unroll_reductions_threshold
            and sympy_product(ranges) != 1
        ):
            return Pointwise.create(
                device,
                dst_dtype,
                cls._unroll_reduction_fn(inner_fn, reduction_ranges, reduction_type, src_dtype),
                ranges,
            )
        ...
        if split == -1:
            ...
            new_ranges, new_reduction_ranges = extract_input_node_reduction_ranges(input_node)  # type: ignore[arg-type]
    ...
    def realize_into(cls, src, dst, unsafe_alias=False):
        ...
        if not unsafe_alias:
            src = Pointwise.create(
                ...
                ranges=[V.graph.sizevars.guard_equals(a, b) for a, b in zip(src.get_size(), dst.get_size())],
            ).data
...
class ConstantBuffer(InputBuffer):
    ...
    def constant_to_device(self, device):
        return ConstantBuffer(V.graph.constant_name(self.get_name(), device), self.layout)
...
class ComputedBuffer(Buffer):
    ...
    def get_fill_order(self):
        ...
        if isinstance(self.layout, FlexibleLayout):
            ...
            reads_bufs = [
                V.graph.name_to_buffer[r.name] if r.name in V.graph.name_to_buffer.keys() else None for r in reads
            ]
            # only consider reads to buffer of same size
            # ignore StarDeps because they don't contribute stride information
            assert all(isinstance(r, (dependencies.StarDep, dependencies.MemoryDep)) for r in reads)
            reads = [
                sympy_subs(r.index, {v: sympy.Integer(0) for v in reduction_vars if v != 0})
                for r in reads
    ...
    def simplify_and_reorder(self):
        ...
        reads_bufs = [
            V.graph.name_to_buffer[reads_name] if reads_name in V.graph.name_to_buffer.keys() else None
...
class IndexPutFallback(ExternKernel):
    ...
    def codegen(self, wrapper):
        ...
        wrapper.writeline(wrapper.wrap_kernel_call(self.cpp_kernel if V.graph.cpp_wrapper else self.kernel, args))
...
class DeviceCopy(ExternKernelOut):
    ...
    def create(cls, x, device):
        if not x.is_extern() and all(
            (r.name in V.graph.constants and isinstance(r, dependencies.MemoryDep)) for r in x.get_reads()
        ):
...
class FallbackKernel(ExternKernelAlloc):
    ...
    def codegen_args(self):
        ...
        if V.graph.cpp_wrapper and isinstance(self.op_overload, torch._ops.OpOverload):
            args = [
                V.graph.wrapper_code.val_to_cpp_arg_str(param.real_type, x, self.is_legacy_abi_kernel())
        ...
        def handle_single_output(return_type, output):
            if isinstance(return_type, torch.TensorType):
                ...
                return export_schema.Argument.create(
                    as_tensors=[export_schema.TensorArgument(name=out.get_name()) for out in output]
                )
            else:
        ...
        if kernel.namespace == "aten":
            ...
            if V.graph.cpp_wrapper:
                if config.is_fbcode() and kernel not in has_c_shim:
                    ...
                    self.args_default_value = [
                        {"type": x.real_type, "value": x.default_value} for x in schema.arguments if not x.kwarg_only
                    ]
    ...
    def create(cls, kernel, *args, **kwargs):
        ...
        def generate_output(output, indices):
            if isinstance(output, (list, tuple)):
                return type(output)(
                    generate_output(output[i], indices + [(type(output), i)]) for i in range(len(output))
                )
...
class MultiOutput(ExternKernel):
    ...
    def get_alias_names(self):
        return [
            ...
            if isinstance(inp, (FallbackKernel, ComplexView)) and len(inp.get_alias_names()) > 0
        ]
...
class ConvolutionBinary(ExternKernelAlloc):
    ...
    def create(
        ...
        (
            ...
            kernel_layout,
            req_stride_order,
        ) = _prepare_convolution_fusion_create(cls, x, weight, bias, padding_, stride_, dilation_, groups)
    ...
    def create(cls, x, packed_w, orig_w, batch_size):
        ...
        *m, _ = x.get_size()
        ...
        return MKLPackedLinear(
            layout=FixedLayout(x.get_device(), x.get_dtype(), output_size, output_stride),
            inputs=inputs,
            constant_args=constant_args,
        )
...
class MkldnnRnnLayer(ExternKernelAlloc):
    ...
    def create(
        ...
        output_ir = [
            ...
            for i, (output_size, output_stride) in enumerate(zip(output_sizes, output_strides))
        ]
...
class StorageBox(MutableBox):
    ...
    def is_pointwise_non_scalar_tensor_num_reads_larger_than_one(self):
        ...
        return (
            ...
            and all(not isinstance(read, dependencies.StarDep) for read in self.data.get_reads())
            else True
        )
    ...
    def debug_str(self):
        ...
        lines.extend(
            [
                ...
                for name, block in itertools.chain([("body", self.root_block)], self.subblocks.items())
            ]
        )
        return "\n".join(lines)
