...
def make_fallback(op, layout_constraint=None, warn=True):
    ...
    if (
        ...
        and not (config.fallback_random and op in torch._decomp.decompositions_for_rng.extra_random_decomps)
    ):
...
def sdpa_constraint(fx_node, *args, **kwargs):
    ...
    def apply_constraint(arg, fx_arg):
        ...
        def is_aligned_backward(x):
            aligned_strides = all(
                (V.graph.sizevars.size_hint(x.get_stride()[i]) % ALIGNMENT) == 0 for i in range(len(x.get_stride()) - 1)
            )
...
def slice_scatter(x, src, dim=0, start=None, end=None, step=1):
    ...
    def inner_fn(idx):
        ...
        if step != 1:
            mask.append(
                ops.eq(
                    ops.index_expr(ModularIndexing(idx[dim] - start, 1, step), torch.int64),
...
def check_and_broadcast_indices(indices, device):
    assert all(
        i.get_dtype() in (torch.int64, torch.int32, torch.bool, torch.uint8) for i in indices if i is not None
...
def index(x, indices):
    try:
        ...
        x.realize()
        return fallback_handler(aten.index.Tensor, add_to_fallback_set=False)(x, indices)
...
def _upsample_nearest_exact2d(x, output_size, scales_h: Optional[float] = None, scales_w: Optional[float] = None):
    ...
    def load(index):
        *prefix, ih, iw = index
        ...
        return (
            ops.masked(
                mask,
                lambda: constant_boundary_condition_2d(x, pad_fill_value)([*prefix, ih, iw]),
                fill_value,
            )
            if padding
...
fallback_adaptive_avg_pool2d = fallback_handler(aten._adaptive_avg_pool2d.default, add_to_fallback_set=False)
...
fallback_avg_pool2d = fallback_handler(aten.avg_pool2d.default, add_to_fallback_set=False)
...
fallback_avg_pool2d_backward = fallback_handler(aten.avg_pool2d_backward.default, add_to_fallback_set=False)
...
def make_reduction(reduction_type: str, override_return_dtype=None):
    def inner(x, axis=None, keepdims=False, *, dtype=None):
        ...
        if isinstance(result.data.data, Reduction):  # Only realize if reduction isn't unrolled
            result.realize()
        return result
...
fallback_pow_tensor_tensor = fallback_handler(aten.pow.Tensor_Tensor, add_to_fallback_set=False)
fallback_pow_scalar = fallback_handler(aten.pow.Scalar, add_to_fallback_set=False)
fallback_pow_tensor_scalar = fallback_handler(aten.pow.Tensor_Scalar, add_to_fallback_set=False)
...
foreach_add_list = register_foreach_pointwise(aten._foreach_add.List, add, allow_alpha=True)
foreach_add_scalar = register_foreach_pointwise(aten._foreach_add.Scalar, add, allow_alpha=True)
...
register_foreach_inplace(aten._foreach_add_.List, aten._foreach_add.List, foreach_add_list)
register_foreach_inplace(aten._foreach_add_.Scalar, aten._foreach_add.Scalar, foreach_add_scalar)
register_foreach_inplace(aten._foreach_mul_.List, aten._foreach_mul.List, foreach_mul_list)
register_foreach_inplace(aten._foreach_mul_.Scalar, aten._foreach_mul.Scalar, foreach_mul_scalar)
register_foreach_inplace(aten._foreach_div_.List, aten._foreach_div.List, foreach_div_list)
register_foreach_inplace(aten._foreach_div_.Scalar, aten._foreach_div.Scalar, foreach_div_scalar)
...
try:
    ...
    def all_gather_into_tensor(shard, tag, ranks, group_size):
        return TensorBox.create(
            ir.AllGatherIntoTensor.create(ir.ExternKernel.require_contiguous(shard), tag, ranks, group_size)
        )
    ...
    def all_to_all_single(self, output_split_sizes, input_split_sizes, tag, ranks, group_size):
        return TensorBox.create(
            ir.AllToAllSingle.create(self, output_split_sizes, input_split_sizes, tag, ranks, group_size)
        )
