...
class BaseSchedulerNode:
    def __init__(self, scheduler: "Scheduler", node: ir.Buffer):
        ...
        self.min_order: int
        self.max_order: int
        self.last_usage: Set[str] = set()  # buffers that won't be used after this kernel
        self.written = False
    ...
    def prune_deps(self):
        self.unmet_dependencies = {
            dep for dep in self.unmet_dependencies if dep.name not in self.scheduler.available_buffer_names
        }
    ...
    def allocate(self):
        ...
        if hasattr(V.kernel, "args") and self.get_name() in V.kernel.inplace_update_buffers:
            V.graph.wrapper_code.codegen_inplace_reuse(
                self.scheduler.name_to_node[V.kernel.inplace_update_buffers[self.get_name()]].node,
                self.node,
            )
        else:
            V.graph.wrapper_code.codegen_allocation(self.node)
    ...
    def codegen_originating_info(self, buffer, only_once=True):
        ...
        for o in origins:
            ...
            if "stack_trace" in o.meta:
                ...
                out_lines.append(
                    "#pragma CMT " + stack_trace_last_line.replace("{", "{{").replace("}", "}}").replace("\n", "\\")
                )
    ...
    def get_read_write_buffers_sizes(self) -> int:
        ...
        if isinstance(self, SchedulerNode):
            node_numel = V.graph.sizevars.size_hint(
                sympy_product(self.get_ranges()[0]) * sympy_product(self.get_ranges()[1])
            )
        else:
            node_numel = int(1e9)
    ...
    def get_estimated_runtime(self) -> float:
        ...
        if not hasattr(self, "node") or not self.node:
            assert isinstance(self, (FusedSchedulerNode, ForeachKernelSchedulerNode)), f"{type(self)=}"
            assert self.snodes
...
class SchedulerNode(BaseSchedulerNode):
    ...
    def is_reduction(self):
        assert isinstance(self.node, (ir.ComputedBuffer, ir.TemplateBuffer)), f"{type(self.node)=}"
            ...
            for node in self._body.get_nodes():
                if (
                    ...
                    buffers_store_as_atomic_add.add(
                        node.kwargs["name"] if "name" in node.kwargs else (node.args[1] if len(node.args) >= 2 else "")
                    )
...
class FusedSchedulerNode(BaseSchedulerNode):
    ...
    def __init__(self, scheduler: "Scheduler", snodes: List[SchedulerNode]):
        ...
        self.unmet_dependencies = {
            dep for dep in set.union(*[x.unmet_dependencies for x in snodes]) if dep.name not in self.get_names()
        } - self.read_writes.writes
    ...
    def has_atomic_add(self, check_buf):
        return any(
            (isinstance(sub_schedule_node1, SchedulerNode) and sub_schedule_node1.has_atomic_add(check_buf))
...
class ForeachKernelSchedulerNode(FusedSchedulerNode):
    ...
    def can_fuse(cls, producer, consumer):
        ...
        if producer.is_foreach() and consumer.is_foreach():
            ...
            return foreach_match and all(
                producer.scheduler.can_fuse(l, r) for l, r in zip(producer.snodes, consumer.snodes)
            )
    ...
    def __init__(
        ...
        if prev_node_1 is None or prev_node_2 is None:
            ...
            self.unmet_dependencies = {
                dep
                for dep in set.union(prev_node_1.unmet_dependencies, prev_node_2.unmet_dependencies)
...
class Scheduler:
    ...
    def __init__(self, nodes):
        ...
        self.name_to_node: Dict[str, BaseSchedulerNode] = {n.get_name(): n for n in self.nodes}
        self.name_to_fused_node: Dict[str, BaseSchedulerNode] = dict()  # set in fuse_nods()
    ...
    def create_foreach_nodes(self):
        ...
        for names in V.graph.lists.values():
            names = [
                name
                for name in names
                if name in kept_node_names and not isinstance(self.name_to_node[name], NopKernelSchedulerNode)
            ]
            if not names:
            ...
            if isinstance(node, FusedSchedulerNode) and node not in visited:
                ...
                if node.get_names().issubset(combined_ancestors):
                    ...
                    return bool(combined_names & node.ancestors) or any(
                        found_path(self.name_to_fused_node[n]) for n in node.ancestors - combined_ancestors
                    )
            return False
    ...
    def can_fuse(self, node1: BaseSchedulerNode, node2: BaseSchedulerNode):
        ...
        if isinstance(node1, (ExternKernelSchedulerNode, NopKernelSchedulerNode)) and not node1.is_template():
            why("node1 is extern or nop")
            return False
        if isinstance(node2, (ExternKernelSchedulerNode, NopKernelSchedulerNode)) and not node2.is_template():
        ...
        if node1.is_template() and (
            node2.has_aliasing_or_mutation() or node2.is_reduction() or not config.epilogue_fusion
        ):
        ...
        if no_shared_data and (not config.aggressive_fusion or node1.is_reduction() or node2.is_reduction()):
            why("no shared data")
    ...
    def remove_inplace_buffer(self, name):
        ...
        V.kernel.args.inplace_buffers[name] = inner_name.replace("in_out_ptr", "REMOVED")
        V.kernel.removed_buffers.add(name)
    ...
    def create_backend(self, device: torch.device):
        assert device.type != "cuda" or device.index is not None, f"{device} should have been normalized in lowering"
