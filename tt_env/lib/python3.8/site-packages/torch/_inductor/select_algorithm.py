...
class TritonTemplate(KernelTemplate):
    ...
    def generate(
        ...
        **kwargs,
        ...
        with patch.object(V.graph, "get_dtype", self._fake_get_dtype(fake_out)), TritonTemplateKernel(
...
class ExternKernelCaller(ChoiceCaller):
    ...
    def hash_key(self):
        return "-".join(
            [
                self.choice.name,
                *[f"{kwarg}={repr(self.kwargs[kwarg])}" for kwarg in sorted(self.kwargs.keys())],
...
class AlgorithmSelectorCache(PersistentCache):
    ...
    def make_benchmark_fn(
        ...
        unique_example_inputs = {
            x.get_name(): input_gen_fns.get(i, cls.benchmark_example_value)(x) for i, x in enumerate(input_nodes)
        }
        ...
        sizes = ", ".join(
            [
                "x".join(
                    map(
                        str,
                        V.graph.sizevars.size_hints(n.get_size(), fallback=config.unbacked_symint_fallback),
                    )
                )
