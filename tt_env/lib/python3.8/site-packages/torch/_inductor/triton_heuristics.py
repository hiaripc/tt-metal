...
class CachingAutotuner(KernelInterface):
    ...
    def coordinate_descent_tuning(self, launcher, *args, **kwargs):
        ...
        assert not (
            self.heuristic_type == HeuristicType.PERSISTENT_REDUCTION and "RBLOCK" in launcher.config.kwargs
...
def triton_config(
    ...
    while (
        ...
        and (y * maxGridSize[1] < size_hints[1] or conditional_product(x, y, z) < target)
    ):
        y *= 2
    while (
        z
        and z < min(size_hints[2], config.triton.max_block["Z"])
        and (z * maxGridSize[2] < size_hints[2] or conditional_product(x, y, z) < target)
    ):
        z *= 2
