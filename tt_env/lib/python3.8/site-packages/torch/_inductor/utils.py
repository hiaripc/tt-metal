...
def do_bench_using_profiling(fn: Callable[[], Any], warmup=25, rep=100) -> float:
    ...
    if len(filtered_events) % n_repeat != 0:
        raise RuntimeError(
            "Failed to divide all profiling events into #repeat groups. " "#CUDA events: %d, #repeats: %s",
            len(filtered_events),
            n_repeat,
        )
...
def do_bench(*args, **kwargs):
    ...
    def load_triton():
        ...
        return triton_do_bench, (
            "quantiles" if inspect.signature(triton_do_bench).parameters.get("quantiles") is not None else "percentiles"
        )
...
def has_torchvision_roi_align() -> bool:
    try:
        ...
        return roi_align is not None and hasattr(getattr(torch.ops, "torchvision", None), "roi_align")
    except ImportError:
        return False
...
def ceildiv(numer: Union[int, sympy.Expr], denom: Union[int, sympy.Expr]) -> Union[int, sympy.Expr]:
...
def timed(model: Callable[..., Any], example_inputs, times: int = 1, device: str = "cuda") -> float:
    synchronize(device)
...
def print_performance(fn, args=(), times=10, repeat=10, baseline=1.0, device: str = "cuda"):
...
def aggregate_origins(node_schedule):
    ...
    if isinstance(node_schedule, list):
        return functools.reduce(
            operator.or_,
            [node.node.origins for node in node_schedule if hasattr(node, "node") and node.node],
            set(),
        )
                ...
                if isinstance(cache_entries, dict):
                    ...
                    if os.path.exists(triton_cache_dir):
                        ...
                        cache_entries.update(
                            {f: os.path.getsize(os.path.join(triton_cache_dir, f)) for f in files if ".lock" not in f}
                        )
...
def use_max_autotune() -> bool:
    return config.max_autotune or config.max_autotune_gemm or config.search_autotune_cache
...
def use_triton_template(layout, *, enable_int32=False):
    ...
    return _use_template_for_cuda(layout, layout_dtypes) and _use_autotune_backend("TRITON")
...
def get_benchmark_name():
    ...
    try:
        ...
        if idx + 1 < len(sys.argv) and len(sys.argv[idx + 1]) > 0 and sys.argv[idx + 1][0] != "-":
...
def is_cpu_device(inputs):
    return all(item.device == torch.device("cpu") for item in inputs if isinstance(item, torch.Tensor))


def get_sympy_Expr_dtype(val: sympy.Expr) -> torch.dtype:
    assert isinstance(val, sympy.Expr), "only support sympy.Expr as input to get_sympy_Expr_dtype"
    if val.is_integer:
        return torch.int64
    else:
