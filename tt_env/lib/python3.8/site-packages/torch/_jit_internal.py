...
def get_type_hint_captures(fn):
    ...
    valid_return_annotation_type = return_annotation is not inspect.Parameter.empty and not isinstance(
...
def ignore(drop=False, **kwargs):
    ...
    if not isinstance(drop, bool):
        raise RuntimeError("Argument to @torch.jit.ignore must be a bool or " f"a function but got {drop}")
...
def is_union(ann):
    ...
    return isinstance(ann, BuiltinUnionType) or (
        hasattr(ann, "__module__") and ann.__module__ == "typing" and (get_origin(ann) is Union)
    )
...
def is_future(ann) -> bool:
    if ann is Future:
        raise RuntimeError(
            "Attempted to use Future without a " "contained type. Please add a contained type, e.g. " "Future[int]"
        )
...
if torch.distributed.rpc.is_available():
    ...
    def is_rref(ann) -> bool:
        if ann is RRef:
            raise RuntimeError(
                "Attempted to use RRef without a " "contained type. Please add a contained type, e.g. " "RRef[int]"
            )
...
def _qualified_name(obj, mangle_name=True) -> str:
    ...
    if "." in name:
        raise RuntimeError(f"Could not get qualified name for class '{name}': " f"'{name}' is not a valid identifier")
...
def raise_error_container_parameter_missing(target_type) -> None:
    if target_type == "Dict":
        raise RuntimeError(
            "Attempted to use Dict without " "contained types. Please add contained type, e.g. " "Dict[int, int]"
        )
    raise RuntimeError(
