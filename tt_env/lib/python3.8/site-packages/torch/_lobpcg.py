...
def _matrix_polynomial_value(poly, x, zero_power=None):
    ...
    if zero_power is None:
        zero_power = torch.eye(x.size(-1), x.size(-1), dtype=x.dtype, device=x.device).view(
            *([1] * len(list(x.shape[:-2]))), x.size(-1), x.size(-1)
        )
...
def _symeig_backward_partial_eigenspace(D_grad, U_grad, A, D, U, largest):
    ...
    res -= U_ortho.matmul(
        ...
        * torch.cholesky_solve(U_ortho_t.matmul(series_acc), chr_poly_D_at_A_to_U_ortho_L)
    ).matmul(Ut)
...
class LOBPCGAutogradFunction(torch.autograd.Function):
    ...
    def backward(ctx, D_grad, U_grad):
        ...
        if A.is_sparse or (B is not None and B.is_sparse and ctx.needs_input_grad[2]):
            raise ValueError(
                "lobpcg.backward does not support sparse input yet." "Note that lobpcg.forward does though."
            )
        if (
            ...
            raise ValueError(
                "lobpcg.backward does not support complex input yet." "Note that lobpcg.forward does though."
            )
        if B is not None:
...
class LOBPCG:
    ...
    def _get_svqb(self, U: Tensor, drop: bool, tau: float) -> Tensor:  # Tensor  # bool  # float
