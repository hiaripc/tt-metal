...
def help_message(verbose=False):
    ...
    lines = (
        ...
        + sorted([f"{pad_to(name)}\t{log_registry.artifact_descriptions[name]}" for name in printed_artifacts])
    )
    setting_info = "  " + "\n  ".join(lines)
    examples = """
Examples:
...
def _invalid_settings_err_msg(settings, verbose=False):
    valid_settings = ", ".join(
        ["all"] + list(log_registry.log_alias_to_log_qnames.keys()) + list(log_registry.artifact_names)
    )
    msg = f"""
