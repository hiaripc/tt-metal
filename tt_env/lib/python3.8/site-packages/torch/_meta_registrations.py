...
def linalg_cross(self, other, *, dim=-1):
    ...
    torch._check(
        ...
        lambda: (
            f"linalg.cross: inputs dimension {dim} must have length 3. " f"Got {self.size(dim)} and {other.size(dim)}"
        ),
    )
...
def linearSolveCheckInputs(
    ...
    torch._check(
        ...
        lambda: (
            f"Expected b and A to be on the same device, but found b on " f"{self.device} and A on {A.device} instead."
        ),
    )
    ...
    torch._check(
        A.size(-1) == A.size(-2),
        lambda: (f"A must be batches of square matrices, " f"but they are {A.size(-2)} by {A.size(-1)} matrices"),
    )
...
def linalg_householder_product(input: Tensor, tau: Tensor) -> Tensor:
    ...
    torch._check(
        ...
        lambda: (
            f"torch.linalg.householder_product: tau dtype {tau.dtype}" f" does not match input dtype {input.dtype}"
        ),
    )
...
def _pad2d_common(input, padding, *, is_reflection):
    ...
    torch._check(
        ...
        lambda: (f"input (H: {input_h} W: {input_w}) is too small. " f"Calculated output H: {output_h} W: {output_w}"),
    )
...
if torch._C._has_mkldnn:
    _meta_lib_dont_use_me_use_register_meta_for_mkldnn = torch.library.Library("mkldnn", "IMPL", "Meta")
    ...
    if torch._C.has_mkl:
        _meta_lib_dont_use_me_use_register_meta_for_mkl = torch.library.Library("mkl", "IMPL", "Meta")
    ...
    _meta_lib_dont_use_me_use_register_meta_for_onednn = torch.library.Library("onednn", "IMPL", "Meta")
    ...
    _meta_lib_dont_use_me_use_register_meta_for_quantized = torch.library.Library("quantized", "IMPL", "Meta")
    ...
    def meta_quantized_max_pool2d(
        ...
        (
            ...
            outputHeight,
            outputWidth,
        ) = max_pool2d_checks_and_compute_shape(input, kernel_size, stride, padding, dilation, ceil_mode)
...
def _meta_foreach_out_of_place(*args, _scalar_op=None, **kwargs):
    ...
    for iarg, arg in enumerate(args[1:]):
        if isinstance(arg, list):
            ...
            torch._check(
                len(arg) == nelem,
                lambda: (f"self and argument-{iarg+2} must match in length, " f"but got {nelem} and {len(arg)}."),
            )
...
def _check_foreach_binop_tensor_lists(self, other):
    torch._check(
        ...
        lambda: ("The first two arguments of must be List[Tensor], " f"but got {type(self)} and {type(other)}."),
    )
    torch._check(
        len(self) > 0 and len(self) == len(other),
        lambda: ("self and other must be non-empty and match in length, " f"but got {len(self)} and {len(other)}."),
    )
...
def meta__foreach_addcop_scalar(self, tensor1, tensor2, scalar=1):
    ...
    torch._check(
        ...
        lambda: ("All arguments must be List[Tensor], " f"but got {type(self)}, {type(tensor1)}, and {type(tensor2)}"),
    )
...
def meta__foreach_addcop_tensor(self, tensor1, tensor2, scalars):
    torch._check(
        all(isinstance(l, List) for l in [self, tensor1, tensor2]) and isinstance(scalars, torch.Tensor),
        lambda: (
...
def meta__int_mm(a, b):
    ...
    torch._check(
        a.size(1) == b.size(0),
        lambda: (f"Incompatible matrix sizes for _int_mm ({a.size(0)}x{a.size(1)} " f"and {b.size(0)}x{b.size(1)})"),
    )
...
def meta_round(self, **kwargs):
    return elementwise_meta(self, type_promotion=ELEMENTWISE_TYPE_PROMOTION_KIND.DEFAULT)
...
def meta_rshifts(self, other):
    ...
    return elementwise_meta(self, other, type_promotion=ELEMENTWISE_TYPE_PROMOTION_KIND.DEFAULT)
...
def pool2d_shape_check(
    ...
    if memory_format == torch.channels_last:
        ...
        torch._check(
            (ndim == 3 and input.size(0) != 0 and valid_dims) or (ndim == 4 and valid_dims and input.size(3) != 0),
...
def pool3d_shape_check(
    ...
    torch._check(
        ...
        lambda: (f"kernel size should be greater than zero, but got " f"kT: {kT}, kH: {kH}, kW: {kW}"),
    )
    torch._check(
        dT > 0 and dW > 0 and dH > 0,
        lambda: (f"stride should be greater than zero, but got " f"dT: {dT}, dH: {dH}, dW: {dW}"),
    )
    torch._check(
...
def meta_max_pool2d_with_indices_backward(
    ...
    (
        ...
        outputHeight,
        outputWidth,
    ) = max_pool2d_checks_and_compute_shape(self, kernel_size, stride, padding, dilation, ceil_mode)
...
def meta_max_unpool2d(self_, indices, output_size):
    ...
    torch._check(
        self_.ndim in (3, 4),
        lambda: (
            f"Input to max_unpooling2d should be a 3d or 4d Tensor, " f"but got a tensor with {self_.ndim} dimensions."
        ),
    )
    torch._check(
...
def meta_max_pool3d_with_indices(
    ...
    if input.ndim == 4:
        ...
        channels_last = (not input_channels_last_check.is_contiguous()) and input_channels_last_check.is_contiguous(
...
def check_grid_sampler_3d(input: Tensor, grid: Tensor, interpolation_mode: int):
    ...
    torch._check(
        not (input.ndim == 5 and interpolation_mode == GridSamplerInterpolation.BICUBIC.value),
...
def meta_select(self, dim, index):
    ...
    torch._check_index(
        ...
        lambda: f"select(): index {index} out of range for tensor of size " f"{self.size()} at dimension {dim}",
    )
...
def meta_scaled_mm(
    ...
    return torch.empty(self.size(0), mat2.size(1), dtype=_out_dtype, device=self.device), torch.empty(
...
def rnn_cell_checkSizes(input_gates, hidden_gates, input_bias, hidden_bias, factor, prev_hidden):
    ...
    torch._check(
        all(x.device == input_gates.device for x in [hidden_gates, input_bias, hidden_bias, prev_hidden]),
...
def _create_unary_float_meta_func(func):
    ...
    @out_wrapper()
    def _f(x):
        return elementwise_meta(x, type_promotion=ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT)
