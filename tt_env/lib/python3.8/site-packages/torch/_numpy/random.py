...
def deco_stream(func):
    ...
    def inner(*args, **kwds):
        if not use_numpy_random():
            ...
            args = tuple(arg.tensor.numpy() if isinstance(arg, ndarray) else arg for arg in args)
            kwds = {key: val.tensor.numpy() if isinstance(val, ndarray) else val for key, val in kwds.items()}
