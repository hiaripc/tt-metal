...
def check_significant_strides(a: TensorLikeType, b: TensorLikeType, *, only_cuda=True) -> Tuple[bool, Optional[int]]:
    return _check_strides_helper(a, b, only_cuda=only_cuda, significant_only=True)


def check_all_strides(a: TensorLikeType, b: TensorLikeType, *, only_cuda=True) -> Tuple[bool, Optional[int]]:
...
def is_contiguous_for_memory_format(a: Tensor, *, memory_format: torch.memory_format) -> bool:  # type: ignore[return]
...
def get_higher_dtype(
    ...
    def _extract_dtype(x: Optional[Union[torch.dtype, TensorLikeType, NumberType]]) -> Optional[torch.dtype]:
        if x is None:
            return None
...
def check(b: bool, s: Callable[[], str], exc_type: Type[Exception] = RuntimeError) -> None:
    ...
    warnings.warn(
        DeprecationWarning(
            "'torch._prims_common.check' will be removed in the future. Please use " "'torch._check*' functions instead"
        )
    )
