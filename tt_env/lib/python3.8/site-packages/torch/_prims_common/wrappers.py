...
def _safe_copy_out(*, copy_from: TensorLikeType, copy_to: TensorLikeType, exact_dtype: bool = False):
    ...
    if exact_dtype:
        torch._check(
            ...
            lambda: f"Expected out tensor to have dtype {copy_from.dtype} " f"but got {copy_to.dtype} instead",
        )
    else:
        torch._check(
...
def out_wrapper(*out_names: str, exact_dtype: bool = False):
    ...
    def _out_wrapper(fn: Callable) -> Callable:
        ...
        return_type = (
            TensorLikeType
            if is_tensor
            else NamedTuple(f"return_types_{fn.__name__}", [(o, TensorLikeType) for o in out_names])
        )
