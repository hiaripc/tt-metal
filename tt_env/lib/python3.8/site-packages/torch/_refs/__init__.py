...
def imag(a: TensorLikeType) -> TensorLikeType:
    assert isinstance(a, TensorLike)
    torch._check(utils.is_complex_dtype(a.dtype), lambda: "imag only supports complex tensors.")
    return prims.imag(a)
...
@_make_elementwise_unary_reference(ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT, aten_op=aten.special_i0)
def i0(a):
...
def logsumexp(self: TensorLikeType, dim: DimsType, keepdim: bool = False) -> TensorLikeType:
...
def _make_elementwise_binary_reference(
    ...
    def inner(prim: Callable):
        ...
        def _ref(
            ...
            torch._check_value(
                supports_two_python_scalars or not (isinstance(a, Number) and isinstance(b, Number)),
...
def float_power(
    ...
    if isinstance(a, Number) and isinstance(b, Number):
        raise ValueError("Receive two Number inputs to an elementwise binary operation!")
...
def floor_divide(a: Union[TensorLikeType, NumberType], b: Union[TensorLikeType, NumberType]):
...
def isclose(
    ...
    result = logical_or(close, logical_and(isfinite(actual_error), le(actual_error, allowed_error)))
...
def logaddexp(a: TensorLikeType, b: TensorLikeType) -> TensorLikeType:
    ...
    inf_mask = torch.logical_and(torch.logical_not(torch.isfinite(torch.real(a))), torch.real(a) == torch.real(b))
...
@out_wrapper()
def clone(a: TensorLikeType, *, memory_format: torch.memory_format = torch.preserve_format) -> TensorLikeType:
        ...
        if kw in to_kwargs:
            if (
                ...
                or (
                    ...
                    and (not to_kwargs[kw].index or to_kwargs[kw].index == a.device.index)
                )
                or (getattr(a, kw, None) == to_kwargs[kw])  # this also handles {"memory_format": None}
            ):
                to_kwargs.pop(kw)
...
def sum_to_size(
    ...
    *shape,
    ...
    reduce_dims = tuple(range(leading_dims)) + tuple(
        i for i in range(leading_dims, len(shape)) if shape[i - leading_dims] == 1 and a.shape[i] != 1
    )
...
def _unsqueeze_atleast(at_least_fn: Callable, dim: int, arg: TensorLikeType) -> TensorLikeType:
...
def as_strided(
    ...
    storage_offset_int = storage_offset if storage_offset is not None else a.storage_offset()
...
def column_stack(tensors: TensorSequenceType) -> TensorLikeType:
    aligned_tensors = tuple(x if x.ndim > 1 else x.reshape((x.numel(), 1)) for x in tensors)
...
def constant_pad_nd(input: TensorLikeType, pad: List[int], value: NumberType = 0) -> TensorLikeType:
    ...
    for i in range(l_diff, l_inp):
        pad_idx = 2 * (l_inp - i - 1)
        if pad[pad_idx] > 0:
            c_output = c_output.narrow(i, pad[pad_idx], c_output.shape[i] - pad[pad_idx])
        if pad[pad_idx + 1] > 0:
...
def contiguous(a: Tensor, *, memory_format: torch.memory_format = torch.contiguous_format) -> Tensor:
    torch._check(
...
def _normalize(a: Tensor, norm_dims: DimsType, eps: float) -> Tuple[Tensor, Tensor, Tensor]:
...
def native_layer_norm(
    ...
    torch._check(
        input.ndim >= normalized_ndim and input.shape[(input.ndim - normalized_ndim) :] == tuple(normalized_shape),
...
def permute(a: TensorLikeType, *dims) -> TensorLikeType:
    _permutation = utils.canonicalize_dims(a.ndim, utils.extract_dims_from_varargs(dims))
...
def stft(
    ...
    if return_complex is None:
        return_complex_ = input.is_complex() or (window is not None and utils.is_complex_dtype(window.dtype))
        torch._check(
            return_complex_,
            (
...
def istft(
    ...
    torch._check(0 < win_length_ <= n_fft, lambda: "istft expected 0 < win_length <= n_fft")
    torch._check(
        window is None or window.shape == (win_length_,),
...
def repeat(a: Tensor, *repeat_shape) -> Tensor:
    ...
    target_shape = tuple(padded_size * repeat_size for padded_size, repeat_size in zip(padded_shape, repeat_shape))
...
@out_wrapper()
def index_fill(x: TensorLike, dim: int, index: TensorLike, value: Union[NumberType, TensorLike]):
...
def _index_fill(
    ...
    *,
    ...
    if isinstance(value, TensorLike):
        ...
        value = torch.scalar_tensor(value, dtype=x.dtype, layout=x.layout, device=x.device)  # type: ignore[arg-type]
...
def tensor_split(
    ...
    if isinstance(indices_or_sections, TensorLike):
        if not indices_or_sections.device.type == "cpu":
            msg = (
                "tensor_split: if indices_or_sections is a tensor it must be on the CPU, but received one on {}".format(
...
def diag(
    ...
    ndim = self.dim()
    torch._check(ndim in (1, 2), lambda: f"diag(): Supports 1D or 2D tensors. Got {ndim}D")
    if ndim == 1:
...
def diag_embed(
    ...
    b_range = torch.arange(offset, last_dim + offset, device=t.device, dtype=torch.int64)
...
def t(a: TensorLikeType):
    ...
    #         )
    if a.ndim > 2:
        raise RuntimeError(f"t() expects a tensor with <= 2 dimensions, but self is {a.ndim}D")
...
def T(a: TensorLikeType) -> TensorLikeType:
    ...
    torch._check(
        a.ndim in (0, 2),
        lambda: (
            "The use of `x.T` on tensors of dimension other than 0 or 2 " "to reverse their shape is not supported."
        ),
    )
    return a.t()
...
def unfold(self: TensorLikeType, dimension: int, size: int, step: int) -> TensorLikeType:
    shape, strides = _get_unfold_shape_stride(self.shape, self.stride(), dimension, size, step)
...
def unfold_copy(self: TensorLikeType, dimension: int, size: int, step: int):
    return self.unfold(dimension, size, step).clone(memory_format=torch.contiguous_format)
...
def take_along_dim(a: torch.Tensor, indices: torch.Tensor, dim: Optional[int] = None) -> torch.Tensor:
    ...
    torch._check(
        ...
        lambda: ("torch.take_along_dim(): dtype of indices should be int but got " f"{indices.dtype} instead"),
    )
...
({type(start).__name__}, {type(end).__name__}, {type(steps).__name__})",
    ...
    dtype_red = torch.int64 if (utils.is_boolean_dtype(dtype) or utils.is_integer_dtype(dtype)) else dtype
    computation_dtype, _ = utils.reduction_dtypes(rg, REDUCTION_OUTPUT_TYPE_KIND.SAME, dtype_red)
...
def meshgrid(
    *tensors: Union[TensorLikeType, List[TensorLikeType], Tuple[TensorLikeType]],
    ...
    if indexing == "xy":
        ...
        torch._check(
            indexing == "ij",
            lambda: ('torch.meshgrid: indexing must be one of "xy" or "ij", ' f"but received: {indexing}"),
        )
...
def norm(
    ...
    *,
    ...
    if (p == "fro" and (dim is None or isinstance(dim, Dim) or len(dim) <= 2)) or p is None:
        p = 2
    if isinstance(dim, Dim):
        dim = [dim]
    if isinstance(p, str):
...
def trace(self: TensorLikeType) -> TensorLikeType:
    torch._check(self.ndim == 2, lambda: "expected a matrix, but got tensor with dim {self.ndim}")
...
def triu(a: TensorLikeType, diagonal: int = 0) -> TensorLikeType:
    torch._check(a.ndim >= 2, lambda: "triu: input tensor must have at least 2 dimensions")
    h, w = a.shape[-2:]
    mask = (torch.arange(w, device=a.device).unsqueeze(-2) - torch.arange(h, device=a.device).unsqueeze(-1)) >= diagonal
...
def tril(a: TensorLikeType, diagonal: int = 0) -> TensorLikeType:
    torch._check(a.ndim >= 2, lambda: "tril: input tensor must have at least 2 dimensions")
    h, w = a.shape[-2:]
    mask = (torch.arange(w, device=a.device).unsqueeze(-2) - torch.arange(h, device=a.device).unsqueeze(-1)) <= diagonal
...
def tril_indices(
    ...
    *,
    ...
    arange_kw = partial(torch.arange, layout=layout, device=device, pin_memory=pin_memory)
    ...
    return torch.stack((torch.cat((row_inds1, row_inds2)), torch.cat((col_inds1, col_inds2))))
...
def triu_indices(
    ...
    *,
    ...
    return torch.stack((torch.cat((row_inds2, row_inds1)), torch.cat((col_inds2, col_inds1))))
...
def geometric(self, p, generator=None):
    ...
    torch._check(
        not utils.is_complex_dtype(self.dtype) and not utils.is_boolean_dtype(self.dtype),
...
def _compute_sizes(seq, scalar_type):
    ...
    while isinstance(seq, (list, tuple)):
        ...
        try:
            ...
            raise ValueError(f"could not determine the shape of object type '{type(seq).__name__}'")  # noqa: TRY200
        seq = handle
...
def _recursive_build(sizes, dim, scalarType, obj):
    ...
    seq_size = len(seq)
    if seq_size != n:
        raise ValueError(f"expected sequence of length {n} at dim {dim} (got {seq_size})")
    return torch.stack([_recursive_build(sizes, dim + 1, scalarType, item) for item in seq])
