...
def matrix_norm(
    ...
    *,
    ...
    if isinstance(dim, Dim):
        dim = (dim,)  # type: ignore[assignment]
    torch._check(len(dim) == 2, lambda: "linalg.matrix_norm: dim must be a 2-tuple. Got {dim}")
    torch._check(
        dim[0] != dim[1],
    ...
    if isinstance(ord, str):
        ...
        # dtype
        check_fp_or_complex(A.dtype, "linalg.matrix_norm", allow_low_precision_dtypes=ord != "nuc")
        ...
        # dtype
        check_fp_or_complex(A.dtype, "linalg.matrix_norm", allow_low_precision_dtypes=ord != 2)
...
def norm(
    ...
    *,
    ...
    if ord is not None and ((dim is not None and len(dim) == 2) or (dim is None and A.ndim == 2)):
        if dim is None:
            dim = (0, 1)
