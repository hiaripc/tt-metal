...
def _is_tensor_constructor(func: OpOverload):
    ...
    return len(schema.returns) == 1 and schema.returns[0].type is torch._C.TensorType.get()
...
class FakeTensorConverter:
    ...
    def from_meta_and_device(self, fake_mode, t, device):
        assert t.device.type == "meta", f"tensor's device must be `meta`, got {t.device.type} instead"
        maybe_memo = self._get_memo(t)
        if maybe_memo is not None:
            return maybe_memo
...
@register_op_impl(lambda func: func in (aten.convolution.default, aten.convolution_backward.default))
def conv(fake_mode, func, *args, **kwargs):
    ...
    with fake_mode:
        ...
        if not has_hint(batch):
            ...
            mem_fmt = torch._C._conv_determine_backend_memory_format(kwargs["input"], kwargs["weight"], conv_backend)
...
def init_cuda_context():
    ...
    if torch.cuda.is_available():
        torch.empty(1, device="cuda") if torch.version.hip is None else torch.zeros(1, device="cuda")
...
def should_allow_numbers_as_tensors(func: OpOverload):
    return torch._C._should_allow_numbers_as_tensors(func.name().split("::")[-1].split(".")[0])
...
class FakeTensorMode(TorchDispatchMode):
    ...
    def dispatch(self, func, types, args=(), kwargs=None):
        ...
        has_symbolic_sizes = any(i._has_symbolic_sizes_strides for i in flat_arg_fake_tensors) or any(
            isinstance(a, torch.SymInt) for a in flat_args
        )
        ...
        if (func in self.lift_fns and not flat_arg_fake_tensors) or (
            should_allow_numbers_as_tensors(func) and not has_symbolic_sizes and not flat_arg_fake_tensors
        ):
            assert all(
...
def run_fallback_kernel(fake_mode, func, flat_args, args_spec, orig_not_implemented_exception):
    ...
    def map_out(e):
        if id(e) not in inp_impls and (
            isinstance(e, torch.Tensor) and not e.is_sparse and e._typed_storage()._cdata in storages
        ):
            raise orig_not_implemented_exception
