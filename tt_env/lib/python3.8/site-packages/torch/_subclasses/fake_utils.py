...
def outputs_alias_inputs(outputs, inputs):
    input_storages = {
        inp._typed_storage()._cdata for inp in tree_flatten_only(torch.Tensor, inputs) if torch._C._has_storage(inp)
    }
    return any(
...
def is_sdpa_error(func, idx, e):
    if (
        (func is aten._scaled_dot_product_flash_attention.default or func is aten._flash_attention_forward.default)
        and idx in (6, 7)
...
class CrossRefFakeMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        context = f"When comparing the output of {func} on FakeTensor and concrete Tensors, " f"found"
        r = func(*args, **kwargs)
