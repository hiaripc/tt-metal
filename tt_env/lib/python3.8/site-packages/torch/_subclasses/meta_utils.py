                ...
                if t.is_sparse:
                    ...
                    def is_c_of_r(complex_dtype, real_dtype):
                        return (
                            ...
                            and utils.corresponding_real_dtype(complex_dtype) == real_dtype
                        )
                        ...
                        if t.is_nested:
                            ...
                            for attr in attrs:
                                ...
                                if orig_shape_env is None:
                                    orig_shape_env = (
                                        inner_t.fake_mode.shape_env if isinstance(inner_t, FakeTensor) else None
                                    )
                                transformed_tensors_dict[attr] = callback(
                                    lambda: empty_create(inner_t, AttrSource(source, attr))
                                )
                    ...
                    if swr not in self.storage_memo and (
                        r.is_nested or (r.stride() == strides and r.storage_offset() == storage_offset)
                    ):
