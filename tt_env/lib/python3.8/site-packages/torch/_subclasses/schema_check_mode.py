...
class SchemaCheckMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        cloned_metadata = {
            name: [parse_metadata(a) for a in pytree.tree_leaves(pre_arguments.get(name))] for name in pre_arguments
        }
            ...
            if arguments.get(name) is not None:
                ...
                for j in range(len(tuple_out)):
                    ...
                    if after is tuple_out[j] and isinstance(after, torch.Tensor):
                        ...
                        if not schema_info.is_mutable(SchemaArgument(SchemaArgType.input, i)) and func not in [
...
However, we found that `outputs[{str(j)}] is {name}"""
                ...
                if any(
                    has_mutated(a, b, c) for a, b, c in zip(pytree.tree_leaves(before), pytree.tree_leaves(after), md)
                ):
