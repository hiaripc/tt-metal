...
class Tensor(torch._C.TensorBase):
    def __deepcopy__(self, memo):
        ...
        with torch.no_grad():
            ...
            if (
                self.is_sparse
                or self.device.type in ["lazy", "xla", "mtia", "mps", "ort", "meta", "ipu"]
                or (not torch._C._has_storage(self) and self.device.type == torch._C._get_privateuse1_backend_name())
    ...
    def _reduce_ex_internal(self, proto):
        ...
        if self.device.type in ["xla", "mtia", "ort"] or (
            not torch._C._has_storage(self) and self.device.type == torch._C._get_privateuse1_backend_name()
        ):
    ...
    def __cuda_array_interface__(self):
        ...
        if not self.is_cuda:
            raise AttributeError(
                ...
                "If CUDA data is required use tensor.cuda() to copy tensor to device memory." % self.type()
            )

        if self.is_sparse:
            raise AttributeError(
                "Can't get __cuda_array_interface__ on sparse type: %s "
                "Use Tensor.to_dense() to convert to a dense tensor first." % self.type()
            )
