...
class LinearBn1d(nn.modules.linear.Linear, nni._FusedModule):
    ...
    def from_float(cls, mod):
        ...
        assert type(mod) == nni.LinearBn1d, (
            "qat." + cls.__name__ + ".from_float only works for " + nni.LinearBn1d.__name__
        )
        ...
        qat_linearbn = cls(
            linear.in_features, linear.out_features, linear.bias is not None, bn.eps, bn.momentum, False, qconfig
        )
    ...
    def to_float(self):
        ...
        linear.weight, linear.bias = fuse_linear_bn_weights(
            self.weight, self.bias, self.bn.running_mean, self.bn.running_var, self.bn.eps, self.bn.weight, self.bn.bias
        )
        return linear
