...
class ConvTranspose2d(nnq.ConvTranspose2d):
    ...
    def forward(self, input: Tensor, reduce_range: bool = True) -> Tensor:
        ...
        if len(input.shape) != 4:
            raise ValueError("Input shape must be `(N, C, H, W)`!")
        return ops.quantized.conv_transpose2d_dynamic(input, self._packed_params, reduce_range)
...
class ConvTranspose3d(nnq.ConvTranspose3d):
    ...
    def forward(self, input: Tensor, reduce_range: bool = True) -> Tensor:
        ...
        return ops.quantized.conv_transpose3d_dynamic(input, self._packed_params, reduce_range)
