...
class Linear(nnq.Linear):
    ...
    def forward(self, x):
        ...
        if self._packed_params.dtype == torch.qint8:
            if self.version is None or self.version < 4:
                Y = torch.ops.quantized.linear_dynamic(x, self._packed_params._packed_params)
            else:
                Y = torch.ops.quantized.linear_dynamic(x, self._packed_params._packed_params, reduce_range=True)
    ...
    def from_float(cls, mod):
        ...
        assert type(mod) in float_modules, "nn.quantized.dynamic.Linear.from_float only works for one of" + str(
        ...
        assert dtype in [torch.qint8, torch.float16], (
            "The only supported dtypes for " f"dynamic quantized linear are qint8 and float16 got: {dtype}"
        )
