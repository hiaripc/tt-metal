...
class RNNBase(torch.nn.Module):
    ...
    def __init__(
        ...
        if (
            not isinstance(dropout, numbers.Number) or not 0 <= dropout <= 1 or isinstance(dropout, bool)
...
class RNNCellBase(torch.nn.Module):
    ...
    def from_float(cls, mod):
        ...
        if type(mod) == torch.nn.LSTMCell:
            ...
            qRNNCellBase = RNNCell(
                mod.input_size, mod.hidden_size, bias=mod.bias, nonlinearity=mod.nonlinearity, dtype=dtype
            )
        else:
    ...
    def set_weight_bias(self, weight_bias_dict):
        ...
        self._packed_weight_ih = pack_weight_bias(
            weight_bias_dict["weight"]["weight_ih"], weight_bias_dict["bias"]["bias_ih"], self.weight_dtype
        )
        self._packed_weight_hh = pack_weight_bias(
            weight_bias_dict["weight"]["weight_hh"], weight_bias_dict["bias"]["bias_hh"], self.weight_dtype
        )
    ...
    def forward(self, input: Tensor, hx: Optional[Tensor] = None) -> Tensor:
        ...
        if self.nonlinearity == "tanh":
            ret = torch.ops.quantized.quantized_rnn_tanh_cell_dynamic(
                input, hx, self._packed_weight_ih, self._packed_weight_hh, self.bias_ih, self.bias_hh
            )
