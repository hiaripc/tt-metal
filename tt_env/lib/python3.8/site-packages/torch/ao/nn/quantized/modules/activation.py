...
class Softmax(torch.nn.Softmax):
    ...
    def forward(self, input):
        ...
        if dim is None:
            ...
            dim = torch.nn.functional._get_softmax_dim("softmax", input.dim(), stacklevel)  # type: ignore[attr-defined]
...
class MultiheadAttention(torch.ao.nn.quantizable.MultiheadAttention):
    ...
    def from_observed(cls, other):
        converted = torch.ao.quantization.convert(
            other, mapping=None, inplace=False, remove_qconfig=True, convert_custom_config_dict=None
        )
...
class PReLU(torch.nn.Module):
    ...
    def from_float(cls, mod):
        ...
        if observer.dtype != torch.quint8:
            warn(f"PReLU's weight observer should have dtype quint8 but got {observer.dtype}")
        wt_scale, wt_zp = observer.calculate_qparams()
        qweight = torch.quantize_per_tensor(float_wt, float(wt_scale), int(wt_zp), torch.quint8)
