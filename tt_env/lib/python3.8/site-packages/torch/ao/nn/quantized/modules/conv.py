...
class _ConvNd(WeightedQuantizedModule):
    ...
    def _init(
        ...
        bias_float = (
            torch.zeros(out_channels, dtype=torch.float, **{k: v for k, v in factory_kwargs.items() if k != "dtype"})
            if bias
            else None
        )
    ...
    def from_float(cls, mod):
        if hasattr(mod, "weight_fake_quant"):
            ...
            activation_post_process = (
                None if not hasattr(mod, "activation_post_process") else mod.activation_post_process
            )
...
class Conv3d(_ConvNd):
    ...
    def forward(self, input):
        ...
        if self.padding_mode != "zeros":
            _reversed_padding_repeated_twice = _reverse_repeat_padding(self.padding)
            input = F.pad(input, _reversed_padding_repeated_twice, mode=self.padding_mode)
        return ops.quantized.conv3d(input, self._packed_params, self.scale, self.zero_point)
...
class _ConvTransposeNd(_ConvNd):
    ...
    def from_float(cls, mod):
        ...
        msg = (
            " nnq." + cls.__name__ + ".from_float only works for " + cls._FLOAT_MODULE.__name__
...
class ConvTranspose1d(_ConvTransposeNd):
    ...
    def set_weight_bias(self, w: torch.Tensor, b: Optional[torch.Tensor]) -> None:
        self._packed_params = torch.ops.quantized.conv_transpose1d_prepack(
            w, b, self.stride, self.padding, self.output_padding, self.dilation, self.groups
        )
    ...
    def forward(self, input):
        ...
        if len(input.shape) != 3:
            raise ValueError("Input shape must be `(N, C, L)`!")
        return torch.ops.quantized.conv_transpose1d(input, self._packed_params, self.scale, self.zero_point)
...
class ConvTranspose2d(_ConvTransposeNd):
    ...
    def forward(self, input):
        ...
        if len(input.shape) != 4:
            raise ValueError("Input shape must be `(N, C, H, W)`!")
        return ops.quantized.conv_transpose2d(input, self._packed_params, self.scale, self.zero_point)
...
class ConvTranspose3d(_ConvTransposeNd):
    ...
    def forward(self, input):
        ...
        return ops.quantized.conv_transpose3d(input, self._packed_params, self.scale, self.zero_point)
