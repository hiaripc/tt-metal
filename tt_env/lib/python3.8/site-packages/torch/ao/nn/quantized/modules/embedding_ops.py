    ...
    def __init__(self, num_embeddings, embedding_dim, dtype=torch.quint8):
        ...
        if self.dtype in [torch.quint8, torch.quint4x2]:
            ...
            wq = torch._empty_per_channel_affine_quantized(
                [num_embeddings, embedding_dim], scales=scales, zero_points=zero_points, axis=0, dtype=self.dtype
            )
            self.set_weight(wq)
        else:
    ...
    def __init__(
        ...
        if _weight is None:
            ...
            qweight = torch._empty_per_channel_affine_quantized(
                [num_embeddings, embedding_dim], scales=scales, zero_points=zero_points, axis=0, dtype=torch.quint8
            )
        else:
    ...
    def from_float(cls, mod):
        ...
        if hasattr(mod, "weight_fake_quant"):
            ...
            assert type(mod) == nn.Embedding, (
                "nnq." + cls.__name__ + ".from_float only works for " + nn.Embedding.__name__
            )
    ...
    def from_float(cls, mod):
        ...
        if hasattr(mod, "weight_fake_quant"):
            ...
            assert type(mod) == nn.EmbeddingBag, (
                "nnq." + cls.__name__ + ".from_float only works for " + nn.EmbeddingBag.__name__
            )
