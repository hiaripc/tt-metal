...
class RNNCellBase(nn.RNNCellBase):
    ...
    def _init_weight_qparams_dict(self, weight_qparams_dict, device):
        ...
        for key, weight_qparams in weight_qparams_dict.items():
            ...
            assert weight_qscheme in [None, torch.per_tensor_affine, torch.per_channel_affine], Exception(
...
class RNNCell(RNNCellBase):
    ...
    def forward(self, input: Tensor, hx: Optional[Tensor] = None) -> Tensor:
        assert input.dim() in (1, 2), f"RNNCell: Expected input to be 1-D or 2-D but received {input.dim()}-D tensor"
...
class LSTMCell(RNNCellBase):
    ...
    def from_float(cls, mod, weight_qparams_dict):
        ref_mod = cls(
            mod.input_size, mod.hidden_size, mod.bias, mod.weight_ih.device, mod.weight_ih.dtype, weight_qparams_dict
        )
        ref_mod.weight_ih = mod.weight_ih
        ref_mod.weight_hh = mod.weight_hh
        ref_mod.bias_ih = mod.bias_ih
        ...
        for key, weight_qparams in weight_qparams_dict.items():
            ...
            if weight_qscheme is not None:
                self.register_buffer(
                    key + "_scale", torch.tensor(weight_qparams["scale"], dtype=torch.float, device=device)
                )
                self.register_buffer(
                    key + "_zero_point", torch.tensor(weight_qparams["zero_point"], dtype=torch.int, device=device)
                )
                if weight_qscheme == torch.per_channel_affine:
                    self.register_buffer(
                        key + "_axis", torch.tensor(weight_qparams["axis"], dtype=torch.int, device=device)
                    )
                else:
                    # added for TorchScriptability, not used
                    self.register_buffer(key + "_axis", torch.tensor(0, dtype=torch.int, device=device))
...
class LSTM(RNNBase):
    ...
    def permute_hidden(
        self, hx: Tuple[Tensor, Tensor], permutation: Optional[Tensor]  # type: ignore[override]
    ) -> Tuple[Tensor, Tensor]:
    ...
    def check_forward_args(
        ...
        self.check_hidden_size(
            hidden[0], self.get_expected_hidden_size(input, batch_sizes), "Expected hidden[0] size {}, got {}"
        )
        self.check_hidden_size(
            hidden[1], self.get_expected_cell_size(input, batch_sizes), "Expected hidden[1] size {}, got {}"
        )
        ...
        if isinstance(orig_input, PackedSequence):
            ...
            if not is_batched:
                ...
                if hx is not None and hx.dim() != 3:
                    raise RuntimeError(f"For batched 3-D input, hx should also be 3-D but got {hx.dim()}-D tensor")
            max_batch_size = input.size(0) if self.batch_first else input.size(1)
            sorted_indices = None
            unsorted_indices = None
