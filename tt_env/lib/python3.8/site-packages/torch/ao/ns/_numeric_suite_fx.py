...
def _extract_weights_impl(
    ...
    _extract_weights_one_model(
        model_name_a, gm_a, nodes_and_names_to_instrument_a, results, op_to_type_to_weight_extraction_fn
    )
    _extract_weights_one_model(
        model_name_b, gm_b, nodes_and_names_to_instrument_b, results, op_to_type_to_weight_extraction_fn
    )
...
def _add_loggers_one_model(
    ...
    model = add_loggers_to_model(
        model, node_to_instrument_inputs_to_ref_name, node_to_instrument_outputs_to_ref_name, logger_cls, model_name
    )
    return model
...
def _add_loggers_impl(
    ...
    new_model_a = _add_loggers_one_model(
        name_a, gm_a, nodes_and_names_to_instrument_inputs_a, nodes_and_names_to_instrument_outputs_a, logger_cls
    )
    new_model_b = _add_loggers_one_model(
        name_b, gm_b, nodes_and_names_to_instrument_inputs_b, nodes_and_names_to_instrument_outputs_b, logger_cls
    )
...
def _extract_logger_info_one_model(
    ...
    for gm_name, mod in model.named_modules():
        ...
        is_logger = isinstance(mod, logger_cls) or (  # type: ignore[arg-type]
            isinstance(mod, torch.jit.RecursiveScriptModule) and mod.original_name == "OutputLogger"
        )
        if is_logger:
            key = mod.ref_name
