...
def create_one_transformed_and_logged_copy_of_subgraph(
    ...
    if subgraph_candidate_idx == 0:
        ...
        logger_mod_orig = _get_logger_for_subgraph(
            mt, first_node, last_node, subgraph_idx, subgraph_candidate_idx, qconfig_str, OutputLogger, fqn
        )
        ...
        logger_mod_orig = _get_logger_for_subgraph(
            mt, first_node, last_node, subgraph_idx, subgraph_candidate_idx, str(qconfig), OutputComparisonLogger, fqn
        )
...
def create_add_loggers_graph(
    ...
    for n in orig_nodes:
        ...
        if insert_submodule_copy:
            ...
            logger_mod_orig = _get_logger_for_subgraph(
                model, first_node, last_node, cur_subgraph_idx, subgraph_candidate_idx, qconfig_str, OutputLogger, fqn
            )
...
def print_n_shadows_summary(
    ...
    for subgraph_data in results_comparison.values():
        mean_all_candidates = [
            candidate["cmp_mean"] for candidate_name, candidate in subgraph_data["candidates"].items()
        ]
