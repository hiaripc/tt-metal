...
class BaseDataScheduler:
    ...
    def __init__(self, data_sparsifier, schedule_param: str, last_epoch=-1, verbose=False):
        ...
        if not isinstance(data_sparsifier, BaseDataSparsifier):
            raise TypeError(
                "{} is not an instance of torch.ao.pruning.BaseDataSparsifier".format(type(data_sparsifier).__name__)
            )
        ...
        self.base_param = {
            name: config.get(schedule_param, None) for name, config in self.data_sparsifier.data_groups.items()
        }
    ...
    def step(self):
        ...
        self._last_param = {
            name: config.get(self.schedule_param, None) for name, config in self.data_sparsifier.data_groups.items()
        }
