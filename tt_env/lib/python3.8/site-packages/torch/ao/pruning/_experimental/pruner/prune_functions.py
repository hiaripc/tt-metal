...
def _get_adjusted_next_layer_bias(next_layer: nn.Module, pruned_biases: Tensor, mask: Tensor) -> nn.Parameter:
    ...
    if isinstance(next_layer, nn.Conv2d):  # checking for Conv2d
        ...
        sum_range = list(range(len(scaling_product.shape)))[1:]  # all but the first dimension
    ...
    if (
        parametrize.is_parametrized(next_layer) and getattr(next_layer, "_bias", None) is not None
...
def prune_conv2d_padded(conv2d_1: nn.Conv2d) -> None:
    ...
    if getattr(conv2d_1, "_bias", None) is not None:
        if conv2d_1.bias is not None:  # conv2d_1 has original bias and bias propagated from previous layer
        ...
        if conv2d_1.bias is not None:  # conv2d_1 has bias propagated from previous layer
