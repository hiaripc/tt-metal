...
class _InputEqualizationObserver(nn.Module):
    ...
    def __init__(
        self, dtype=torch.quint8, qscheme=torch.per_tensor_affine, quant_min=None, quant_max=None, factory_kwargs=None
    ) -> None:
        super().__init__()
...
class _WeightEqualizationObserver(nn.Module):
    ...
    def __init__(
        self, dtype=torch.qint8, qscheme=torch.per_tensor_affine, quant_min=None, quant_max=None, factory_kwargs=None
    ) -> None:
        super().__init__()
    ...
    for node_arg in op_node.args:
        if node_arg_is_weight(op_node, node_arg):
            assert (
                ...
                and isinstance(modules[str(node_arg.target)], _WeightEqualizationObserver)
            )
            return node_arg
    return None
...
def get_layer_sqnr_dict(model_a: nn.Module, model_b: nn.Module, x: torch.Tensor) -> Dict[str, float]:
    ...
    model_a_ns, model_b_ns = ns.add_loggers(
        "fp32", model_a, "int8", model_b, ns.OutputLogger, unmatchable_types_map=unmatchable_types_map
    )
    ...
    ns.extend_logger_results_with_comparison(
        activation_comparison_dict, "fp32", "int8", torch.ao.ns.fx.utils.compute_sqnr, "sqnr"
    )
