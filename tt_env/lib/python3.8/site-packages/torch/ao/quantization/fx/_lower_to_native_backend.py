...
QOP_TO_ARG_NAMES_TO_SKIP = {
    ...
    torch._ops.ops.quantized.instance_norm: ["running_mean", "running_var", "use_input_stats", "momentum"],
}
    ...
    for n in model.graph.nodes:
        ...
        with model.graph.inserting_after(q_node):
            qop_node = create_node_from_old_node_preserve_meta(
                model.graph, ("call_function", qbin_op, tuple(qop_node_args), {}), bop_node
            )
...
def _lower_to_native_backend(
    model: GraphModule, qconfig_map: Dict[str, QConfigAny], node_name_to_scope: Dict[str, Tuple[str, type]]
) -> GraphModule:
