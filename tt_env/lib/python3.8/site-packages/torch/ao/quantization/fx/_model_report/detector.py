    ...
    def _generate_dict_info(self, model: GraphModule) -> Dict[str, Any]:
        ...
        for fqn, module in model.named_modules():
            ...
            if self._is_supported(module):
                ...
                is_supported_type = (
                    sum([isinstance(module, x) for x in self.DEFAULT_DYNAMIC_STATIC_CHECK_SUPPORTED]) > 0
                )
...
class InputWeightEqualizationDetector(DetectorBase):
    ...
    * :attr:`DEFAULT_PRE_OBSERVER_NAME`: The name of the pre-observer to be inserted for this detector
    ...
    def _calculate_range_ratio(self, info_dict: Dict, info_str: str, module_fqn: str) -> torch.Tensor:
        ...
        per_channel_range = (
            info_dict[prefix_str + self.PER_CHANNEL_MAX_KEY] - info_dict[prefix_str + self.PER_CHANNEL_MIN_KEY]
        )
    ...
    def generate_detector_report(self, model: GraphModule) -> Tuple[str, Dict[str, Any]]:
        ...
        input_weight_benefit_str = (
            "{}/{} channels would benefit and we expect significant reduction in quantization error."
        )
        ...
        for module_fqn in input_weight_equalization_info:
            ...
            if num_recs / len(recommendation_per_channel) >= self.DEFAULT_RECOMMEND_INPUT_WEIGHT_CHANNEL_RATIO:
                ...
                non_benefit_reason_formatted = input_weight_non_benefit_reasoning.format(
...
class OutlierDetector(DetectorBase):
    ...
    * :attr:`DEFAULT_PRE_OBSERVER_NAME`: The name of the pre-observer to be inserted for this detector
    ...
    def generate_detector_report(self, model: GraphModule) -> Tuple[str, Dict[str, Any]]:
        ...
        note_string = (
            "Note: outlier detection is only reliable for {}. We recommend {} to ensure the most accurate results."
        )
        note_distribution = "stationary distributions"
        note_rec = (
            "running the static vs. dynamic detector to ensure activation data before modules above is stationary"
        )
        ...
        constant_suggestion = (
            "We recommend taking a look at the dict and data to see how frequent this occurred and why."
        )
