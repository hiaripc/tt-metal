...
def _is_conversion_supported(activation_post_process: torch.nn.Module) -> bool:
    ...
    return (
        (
            dtype in [torch.quint8, torch.qint8, torch.qint32, torch.uint8, torch.int8, torch.int16, torch.int32]
            and (not is_dynamic)
        )
...
def _get_module_path_and_prefix(
    obs_node: Node, node_name_to_scope: Dict[str, Tuple[str, type]], node_name_to_qconfig: Dict[str, QConfigAny]
) -> Tuple[str, str]:
    ...
    is_input_observer_only = (
        node_name_to_qconfig[observed_node.name] is None if observed_node.name in node_name_to_qconfig else None
    )
    ...
    for node in list(model.graph.nodes):
        if node.op == "placeholder":
            ...
            if _is_activation_post_process(mod):
                ...
                if observed_node in statically_quantized_custom_module_nodes:
                    ...
                    if is_decomposed:
                        _replace_observer_with_quantize_dequantize_node_decomposed(
                            model, model.graph, node, modules, node_name_to_scope, node_name_to_qconfig
                        )
                    else:
                ...
                convert_standalone_module(node, modules, model, is_reference, backend_config)
            # below this point `type_before_parametrizations` is used
            # instead of `type` to handle situations with fx quant + sparsity
            elif type_before_parametrizations(mod) in set(root_module_classes).union(qat_module_classes).union(
                ...
                convert_custom_module(
                    node, model.graph, modules, custom_module_class_mapping, statically_quantized_custom_module_nodes
                )
