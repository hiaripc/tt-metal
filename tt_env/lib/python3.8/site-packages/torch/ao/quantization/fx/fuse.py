...
def fuse(
    ...
    for node in model.graph.nodes:
        maybe_last_node, pattern, matched_node_pattern, obj, node_to_subpattern = fusion_pairs.get(
        ...
        if node.name not in match_map:
            for pattern, fuse_handler_cls in pattern_to_fuse_handler_cls.items():
                ...
                if _is_match(modules, node, pattern):
                    apply_match(
                        pattern, node, (node, pattern, fuse_handler_cls(node)), matched_node_pattern, node_to_subpattern
                    )
                    break
