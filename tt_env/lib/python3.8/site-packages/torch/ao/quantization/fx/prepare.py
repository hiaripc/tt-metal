...
def _get_qspec_for_arg(
    arg: Node, input_qspec_map: Dict[Node, QuantizationSpecBase], named_modules: Dict[str, torch.nn.Module]
...
def _is_input_arg_dtype_supported_by_backend(
    ...
    if isinstance(arg, (list, tuple)):
        return all(
            _is_input_arg_dtype_supported_by_backend(a, node, qconfig, dtype_config, backend_config) for a in arg
        )
    ...
    if is_activation:
        ...
        return (dtype_config.input_dtype is None) or (
            ...
            and _qconfig_satisfies_dtype_config_constraints(qconfig, dtype_config.input_dtype_with_constraints)
        )
    elif is_weight:
...
def _qat_swap_modules(root: torch.nn.Module, module_to_qat_module: Dict[Pattern, Type[torch.nn.Module]]) -> None:
...
def _get_target_activation_dtype_for_node(
    ...
    if qconfig is not None:
        ...
        bias_dtype = (
            torch.float16
            if (act_dtype == torch.float16 and weight_dtype == torch.float16 and (not input_act_is_dynamic))
            else torch.float
        )
        ...
        if (
            ...
            weight_index = backend_config._pattern_complex_format_to_config[node.target]._input_type_to_index.get(
                "weight"
            )
...
def _maybe_insert_input_observer_for_arg_or_kwarg(
    ...
    if not is_standalone_module:
        ...
        arg_as_input_target_dtype, arg_as_input_target_is_dynamic = _get_dtype_and_is_dynamic(
            arg_as_input_act_obs_or_fq
        )

        arg_as_output_act_obs_or_fq = _get_output_act_obs_or_fq(arg, named_modules, obs_or_fq_map, is_qat)
        arg_as_output_target_dtype, arg_as_output_target_is_dynamic = _get_dtype_and_is_dynamic(
        ...
        if cur_input_idx is None:
            ...
            arg_as_input_target_dtype = torch.quint8 if cur_input_idx in sm_input_quantized_idxs else torch.float
            needs_obs_or_fq = (arg_as_output_target_dtype != arg_as_input_target_dtype) and (
    ...
    if needs_obs_or_fq:
        ...
        if existing_obs_node is None:
            new_obs_node = _insert_obs_or_fq(arg, arg_as_input_act_obs_or_fq, model, named_modules, graph)
...
def _maybe_insert_observers_before_graph_output(
    ...
    def _recursive_maybe_replace_node_with_obs(
        ...
        if isinstance(maybe_node, Node):
            ...
            need_obs = (
                arg_as_output_target_dtype != arg_as_input_target_dtype and arg_as_input_target_dtype != torch.float
            )
            if need_obs:
    ...
    for old_arg in graph_output_node.args:
        new_args.append(_recursive_maybe_replace_node_with_obs(old_arg, model, named_modules, graph))
                ...
                if node.op != "output":
                    ...
                    if len(node.args) > 0 and isinstance(node.args[0], Node) and len(node.args[0].users) > 1:
                        for user in node.args[0].users:
                            ...
                            is_user_quantized = node_name_to_qconfig.get(user.name, None) is not None or (
                    ...
                    if is_input_node_of_the_pattern:
                        ...
                        _maybe_insert_input_equalization_observers_for_node(
                            node, equalization_qconfig, model, named_modules, model.graph, is_quantized_branch
                        )
...
def _run_prepare_fx_on_standalone_modules(
    ...
    for root_node, _, pattern, qhandler, qconfig in node_name_to_match_result_with_qconfig.values():
        ...
        (
            ...
            sm_backend_config,
        ) = _get_standalone_module_configs(root_node, named_modules, prepare_custom_config, qconfig, backend_config)
...
def prepare(
    ...
    if is_standalone_module:
        ...
        observed_graph_module_attrs.standalone_module_input_quantized_idxs = input_quantized_idxs
        observed_graph_module_attrs.standalone_module_output_quantized_idxs = output_quantized_idxs
    return model
