...
def _is_qconfig_supported_by_dtype_configs(qconfig: QConfig, dtype_configs: List[DTypeConfig]):
    for dtype_config in dtype_configs:
        ...
        qconfig_bias_dtype = (
            torch.float16
            if (qconfig_activation_dtype == torch.float16 and qconfig_weight_dtype == torch.float16 and not is_dynamic)
            else torch.float
        )
...
def _get_object_type_qconfig(
    qconfig_mapping: QConfigMapping, object_type: Union[Callable, str], fallback_qconfig: QConfigAny
) -> QConfigAny:
