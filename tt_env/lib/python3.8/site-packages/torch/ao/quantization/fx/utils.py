...
def assert_and_get_unique_device(module: torch.nn.Module) -> Any:
    ...
    ""
    assert len(devices) <= 1, "prepare only works with cpu or single-device CUDA modules, " f"but got devices {devices}"
...
NON_OBSERVABLE_ARG_DICT: Dict[NodeInfo, Dict[Union[type, torch.dtype], Callable[[Node], List[int]]]] = {
    NodeInfo("call_method", "masked_fill"): {torch.bool: return_arg_list([1]), float: return_arg_list([2])},
...
def maybe_get_next_module(
    ...
    for user in node.users.keys():
        if (
            ...
            user.op == "call_function" and target_functional_type is not None and user.target == target_functional_type
        ):
            return user
...
def _qconfig_satisfies_dtype_config_constraints(
    qconfig: QConfigAny, dtype_with_constraints: DTypeWithConstraints, is_activation: bool = True
) -> bool:
    """
    ...
    def _activation_post_process_satisfies_dtype_config_constraints(
        ...
        if backend_scale_exact_match is not None and backend_zero_point_exact_match is not None:
            ...
            if not isinstance(activation_post_process, FixedQParamsObserver) and not isinstance(
