...
from torch.ao.quantization.observer import _is_activation_post_process as is_activation_post_process  # noqa: F401
...
def _add_observer_(
    module, qconfig_propagation_list=None, non_leaf_module_list=None, device=None, custom_module_class_mapping=None
):
    ...
    def insert_activation_post_process(m, special_act_post_process=None):
        ...
        if needs_observation(m) and not isinstance(m, DeQuantStub):
            ...
            m.add_module(
                "activation_post_process", get_activation_post_process(m.qconfig, device, special_act_post_process)
            )
...
def convert(
    module, mapping=None, inplace=False, remove_qconfig=True, is_reference=False, convert_custom_config_dict=None
):
    ...
    _convert(
        module, mapping, inplace=True, is_reference=is_reference, convert_custom_config_dict=convert_custom_config_dict
    )
    if remove_qconfig:
...
def _convert(module, mapping=None, inplace=False, is_reference=False, convert_custom_config_dict=None):
    ...
    for name, mod in module.named_children():
        ...
        if not isinstance(mod, _FusedModule) and type_before_parametrizations(mod) not in custom_module_class_mapping:
            _convert(mod, mapping, True, is_reference, convert_custom_config_dict)  # inplace
