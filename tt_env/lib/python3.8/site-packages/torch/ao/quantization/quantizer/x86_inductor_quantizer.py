...
class X86InductorQuantizer(Quantizer):
    ...
    def _annotate_qat_conv2d_bn_binary_unary(
        ...
        for fused_partition in fused_partitions:
            ...
            (
                ...
                binary_node,
                unary_node,
            ) = self._get_output_nodes_of_partitions([conv_partition, bn_partition, binary_partition, unary_partition])
    ...
    def _annotate_qat_conv2d_bn_binary(self, gm: torch.fx.GraphModule, quantization_config: QuantizationConfig) -> None:
        ...
        for fused_partition in fused_partitions:
            ...
            (
                ...
                bn_output_node,
                binary_node,
            ) = self._get_output_nodes_of_partitions([conv_partition, bn_partition, binary_partition])
            if len(bn_output_node.users) != 1:
                # Conv BN pattern should only has 1 user.
                continue
            (
    ...
    def _annotate_qat_conv2d_bn_unary(self, gm: torch.fx.GraphModule, quantization_config: QuantizationConfig) -> None:
        ...
        for fused_partition in fused_partitions:
            ...
            (
                ...
                bn_output_node,
                unary_node,
            ) = self._get_output_nodes_of_partitions([conv_partition, bn_partition, unary_partition])
    ...
    def _annotate_qat_conv2d_bn(self, gm: torch.fx.GraphModule, quantization_config: QuantizationConfig) -> None:
        ...
        for fused_partition in fused_partitions:
            ...
            self._annotate_conv_node_helper(conv_node, False, quantization_config)
            bn_output_node.meta[QUANT_ANNOTATION_KEY] = _X86InductorQuantizationAnnotation(
                # TODO<leslie> Remove the annotate of output in QAT when qat util support pattern matcher.
                output_qspec=get_output_act_qspec(quantization_config),  # type: ignore[arg-type]
                _annotated=True,
                _is_output_of_quantized_pattern=True,
    ...
    def _annotate_output_share_observer_as_input(self, input_node: Node, source_node: Node):
        source_node_quantization_annotation = (
            source_node.meta[QUANT_ANNOTATION_KEY] if QUANT_ANNOTATION_KEY in source_node.meta else None
        )
        ...
        if (node.target in int8_in_int8_out_ops_pt2e) and (_is_any_annotated([node])):
            if node.target == torch.ops.aten.max_pool2d.default:
                ...
                maxpool_node_quantization_annotation = (
                    maxpool_node.meta[QUANT_ANNOTATION_KEY] if QUANT_ANNOTATION_KEY in maxpool_node.meta else None
                )
                if (
                    maxpool_node_quantization_annotation
