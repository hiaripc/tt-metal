...
def get_swapped_custom_module_class(custom_module, custom_module_class_mapping, qconfig):
    ...
    assert type(custom_module) in class_mapping, (
        "did not find corresponding observed " f"module class for {type(custom_module)} in mapping: {class_mapping}"
    )
...
def get_quant_type(qconfig):
    ...
    raise Exception(
        f"Unrecognized dtype combination in get_quant_type: activation({activation.dtype})," f"weight({weight.dtype})"
    )
