        ...
        if isinstance(grad, torch.Tensor):
            ...
            if out.requires_grad:
                ...
                if not out.dtype.is_floating_point:
                    msg = "grad can be implicitly created only for real scalar outputs" f" but got {out.dtype}"
...
def grad(
    ...
    if materialize_grads:
        if any(result[i] is None and not is_tensor_like(inputs[i]) for i in range(len(inputs))):
            raise RuntimeError("materialize_grads cannot be used when the given input is a GradientEdge")
        result = tuple(
            output if output is not None else torch.zeros_like(input, requires_grad=True)
...
torch._C._add_docstr(is_view_replay_enabled, "Returns True if view-replay is currently enabled.")
