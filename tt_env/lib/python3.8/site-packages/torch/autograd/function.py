...
def once_differentiable(fn):
    ...
    def wrapper(ctx, *args):
        ...
        err_fn = _functions.DelayedError(
            b"trying to differentiate twice a function that was marked " b"with @once_differentiable",
            len(outputs),
        )
    ...
    def _map(obj):
        if condition(obj):
            ...
            raise ValueError(
                ...
                + torch.typename(obj)
                + (". Accepted types: " + condition_msg + ", or lists/tuples of them" if condition_msg else "")
            )
