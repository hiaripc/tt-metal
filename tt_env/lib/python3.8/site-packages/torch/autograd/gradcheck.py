...
def _densify(x):
    ...
    if isinstance(x, (list, tuple)):
        ...
        if x_coalesced.numel() > 0:
            stride = tmp.stride()
            flat_indices = (
                x_coalesced.indices().mul(torch.tensor(stride, dtype=indices_dtype, device=device).unsqueeze(1)).sum(0)
            )
...
def _iter_tensor(x_tensor):
    ...
    if _is_sparse_any_tensor(x_tensor):
        ...
        if x_tensor.layout is torch.sparse_coo:
            ...
            x_indices = (
                torch._convert_indices_from_csr_to_coo(x_tensor.crow_indices(), x_tensor.col_indices())
                .repeat_interleave(x_blocksize[0] * x_blocksize[1], 1)
                .mul_(torch.tensor(x_blocksize, device=x_tensor.device).reshape(2, 1))
                .add_(torch.stack(torch.where(torch.ones(x_blocksize, device=x_tensor.device))).repeat(1, x_nnz))
                .t()
            )
            ...
            x_blocksize = x_block_values.size()[1:3]
            x_indices = (
                torch._convert_indices_from_csr_to_coo(x_tensor.ccol_indices(), x_tensor.row_indices(), transpose=True)
                .repeat_interleave(x_blocksize[0] * x_blocksize[1], 1)
                .mul_(torch.tensor(x_blocksize, device=x_tensor.device).reshape(2, 1))
                .add_(torch.stack(torch.where(torch.ones(x_blocksize, device=x_tensor.device))).repeat(1, x_nnz))
                .t()
...
def get_numerical_jacobian(fn, inputs, target=None, eps=1e-3, grad_out=1.0):
    ...
    **very careful** in this to not clone `target`.
    ...
    if grad_out != 1.0:  # grad_out param is only kept for backward compatibility reasons
        raise ValueError(
            "Expected grad_out to be 1.0. get_numerical_jacobian no longer " "supports values of grad_out != 1.0."
        )
...
def _with_prepare_inputs(fn, inputs, input_idx, input_to_perturb, fast_mode=False):
    ...
    def wrapped_fn():
        inp = tuple(
            _prepare_input(a, input_to_perturb if i == input_idx else None, fast_mode) if is_tensor_like(a) else a
...
def get_analytical_jacobian(inputs, output, nondet_tol=0.0, grad_out=1.0):
    ...
    if grad_out != 1.0:  # grad_out param is only kept for backward compatibility reasons
        raise ValueError(
            "Expected grad_out to be 1.0. get_analytical_jacobian no longer " "supports values of grad_out != 1.0."
        )
        ...
        def jvp(tangent: torch.Tensor):
            with fwAD.dual_level():
                ...
                inputs_with_dual = tuple(
                    dual if idx == input_idx else (inp.detach() if is_tensor_like(inp) else inp)
                ...
                for dual_output in dual_outputs:
                    ...
                    if tangent_out is not None:
                        ...
                        ret.append(
                            torch.zeros([], dtype=primal_out.dtype, device=primal_out.device).expand(primal_out.shape)
                        )
                return tuple(ret)
...
def _test_batched_grad(input, output, output_idx) -> bool:
    ...
    def vjp(v):
        results = grad(v)
        results = tuple(
            grad if grad is not None else torch.zeros([], dtype=inp.dtype, device=inp.device).expand(inp.shape)
...
def _run_slow_mode_and_get_error(func, tupled_inputs, outputs, input_idx, output_idx, rtol, atol, is_forward_ad):
    ...
    slow_numerical = _get_numerical_jacobian(func, tupled_inputs, outputs, is_forward_ad=is_forward_ad)[input_idx][
        output_idx
    ]
    if is_forward_ad:
