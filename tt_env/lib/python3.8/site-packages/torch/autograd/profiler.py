...
class profile:
    ...
    def __init__(
        ...
        *,
        ...
        if not self.use_cpu:
            assert use_kineto, "Device-only events supported only with Kineto (use_kineto=True)"
        ...
        assert len(self.kineto_activities) > 0, "No activities specified for the profiler"
    ...
    def _parse_kineto_results(self, result: _ProfilerResult):
        ...
        def _cpu_memory_usage(mem_record):
            return (
                mem_record.nbytes()
                if mem_record.device_type() in [DeviceType.CPU, DeviceType.MKLDNN, DeviceType.IDEEP]
                else 0
            )
