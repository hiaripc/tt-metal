...
from .sharding_spec import ShardingSpec, ChunkShardingSpec
from .sharding_plan import ShardingPlan
from .sharder import Sharder


def _shard_tensor(tensor: torch.Tensor, sharding_spec: ShardingSpec, src_rank=0, process_group=None) -> ShardedTensor:
    """
...
def shard_parameter(
    module: torch.nn.Module, param_name: str, sharding_spec: ShardingSpec, src_rank=0, process_group=None
):
    """
...
def _collect_local_shard(module: torch.nn.Module) -> torch.nn.Module:
    ...
    def hook_func(_module, _input, output):
        if isinstance(output, ShardedTensor):
            ...
            if (
                isinstance(sharding_spec, ChunkShardingSpec) and local_tensor.size(sharding_spec.dim) == 1
            ):  # type: ignore[attr-defined, arg-type]
                local_tensor = local_tensor.squeeze(output._sharding_spec.dim)  # type: ignore[attr-defined]
            return local_tensor
