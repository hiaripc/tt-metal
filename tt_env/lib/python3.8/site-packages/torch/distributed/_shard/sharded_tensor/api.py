...
class ShardedTensor(ShardedTensorBase):
        ...
        for shard_metadata in self._metadata.shards_metadata:
            ...
            if rank == current_rank:
                local_tensor = _create_tensor_from_params(
                    shard_metadata.shard_sizes, local_device=device, tensor_properties=self._metadata.tensor_properties
                )
    ...
    def __del__(self):
        ...
        with _sharded_tensor_lock:
            global _sharded_tensor_current_id, _sharded_tensor_map
            if hasattr(self, "_sharded_tensor_id") and self._sharded_tensor_id in _sharded_tensor_map:
    ...
    def _init_rpc(self):
        ...
        for rank in range(dist.get_world_size()):
            ...
            if len(self.local_shards()) != 0:
                ...
                fut = rpc.rpc_async(
                    rank, _register_remote_shards, args=(all_tensor_ids[rank_to_name[rank]], rrefs, rpc_rank)
                )
                futs.append(fut)
    ...
    def cpu(self, memory_format=torch.preserve_format, process_group=None) -> ShardedTensor:
        """
        ...
        st_cpu = ShardedTensor._init_from_local_shards_and_global_metadata(
            list_shards, sharded_tensor_metadata=st_meta, process_group=pg, init_rrefs=self._init_rrefs
        )
        return st_cpu

    def cuda(
        self, device=None, non_blocking=False, memory_format=torch.preserve_format, process_group=None
    ) -> ShardedTensor:
        """
        ...
        if device is not None:
            ...
            assert (
                isinstance(device, torch.device) and device.index == torch.cuda.current_device()
        ...
        for shard in self._local_shards:
            cuda_tensor = shard.tensor.cuda(
                device=current_device, non_blocking=non_blocking, memory_format=memory_format
    ...
    def reshard(self, resharding_spec: shard_spec.ShardingSpec) -> ShardedTensor:
        ...
        if not isinstance(resharding_spec, shard_spec.ChunkShardingSpec) or not isinstance(
            self._sharding_spec, shard_spec.ChunkShardingSpec
        ):
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        def dispatch(st: ShardedTensor, func: Callable):
            ...
            if _has_custom_op(st._sharding_spec, func):
                return _dispatch_custom_op(st._sharding_spec, func, types, args, kwargs, st._process_group)
    ...
    def __setstate__(self, state):
        ...
        if not distributed_c10d.is_initialized():
            raise RuntimeError(
                "Need to initialize default process group using " '"init_process_group" before loading ShardedTensor'
            )
        ...
        if pg_state.local_rank != local_rank:
            raise RuntimeError(
                f"Local rank at save time was {pg_state.local_rank}, but at " f"load time was {local_rank}"
            )

        global_rank = distributed_c10d.get_rank()
        if pg_state.global_rank != global_rank:
            raise RuntimeError(
                f"Global rank at save time was {pg_state.global_rank}, but at " f"load time was {global_rank}"
            )
