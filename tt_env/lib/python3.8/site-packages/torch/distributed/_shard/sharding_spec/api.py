...
from ._internals import check_tensor, get_chunked_dim_size, get_split_size, validate_non_overlapping_shards_metadata
...
def _infer_sharding_spec_from_shards_metadata(shards_metadata):
    ...
    if chunk_sharding_dim is not None:
        ...
        chunk_shard_sizes = sorted(
            [get_chunked_dim_size(shard_total_length, split_size, idx) for idx in range(len(placements))]
        )
