...
def mark_data_parallel_shardings(
    ...
    for node in train_step_graph.graph.nodes:
        ...
        if node.op == "placeholder":
            ...
            assert (
                isinstance(node_strategy, DataParallelStrategy) and node_strategy.node_type == NodeType.NON_TENSOR
    ...
    if isinstance(val, torch.Tensor):
        ...
        for idx, placement in enumerate(spec.placements):
            if placement.is_shard():
                ...
                local_shard = placement._split_tensor(local_shard, num_chunks, with_padding=False, contiguous=False)[0][
    ...
    for node in graph.graph.nodes:
        ...
        if node.op == "placeholder":
            ...
            if node.target == torch.ops.aten.repeat.default:
                ...
                local_shape = compute_local_shape(output_val.shape, out_spec.mesh, out_spec.placements)
                ...
                def infer_repeat_sizes(repeated_shape, input_shape):
                    ...
                    for i in range(len(repeated_shape)):
                        if i < padded_length:
                            ...
                            repeated_size[i] = repeated_shape[i] // input_shape[i - padded_length]
