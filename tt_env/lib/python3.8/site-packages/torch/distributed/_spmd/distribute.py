...
def unpack_sizes_and_dims(sizes: List[Union[DSymInt, int]], mesh: DeviceMesh) -> Tuple[List[int], List[Placement]]:
    ...
    placements: List[Placement] = [
        Shard(i) for i, a in enumerate(sizes) if (isinstance(a, DSymInt) and a.is_shard())
    ] or [Replicate()]
...
def slice_backwad_sym_int_consumer_rule(node: fx.Node, args: Tuple[Any, ...]) -> DTensor:
    ...
    return DTensor.from_local(
        local_tensor=torch.slice_scatter(input_tensor, grad_output.to_local(), dim, start, end, step),
...
def factory_with_sizes_rule(
    ...
    assert not any(isinstance(a, DTensor) for a in flat_args), (
        f"Not expect DTensor argument for factory op, but got {node.target} " f"with arguments {args}."
    )
    ...
    with torch.no_grad():
        ...
        if any(a.is_shard() for a in pytree.arg_tree_leaves(*args) if isinstance(a, DSymInt)):
            if op_overload in VIEW_SYM_INT_CONSUMERS:
                ...
                logger.warning(
                    "Assuming using local_value from SymInt for %s" "is mathematically correct. Full args are %s.",
                    op_overload,
                    args,
                )
        ...
        if node.op == OP.PLACEHOLDER:
            ...
            for inp_arg in node.args[0]:
                if isinstance(inp_arg, fx.Node):
                    ...
                    if isinstance(obj, DTensor):
                        output_schemas[inp_arg.name] = Schema(obj.device_mesh, obj.placements)  # type: ignore[arg-type]
            ...
            if node.target in operators and len(dsymints) > 0:
                ...
                assert len(dsymints) == 0, (
                    "SPMD expansion does not support SymInt in non-operator " f"nodes, got {node.target}."
                )
