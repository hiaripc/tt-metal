...
def _prop__foreach_binop_list(op_schema: OpSchema) -> OutputSharding:
    ...
    assert len(self) == len(other), "Two tensor lists must match in length, " f"but got {len(self)} and {len(other)}"
...
def _prop__fused_adam(op_schema: OpSchema):
    ...
    assert all(len(s) == len(tensor_schemas[0]) for s in tensor_schemas), (
        "expect the same number of gradients and states, but got " f"{[len(s) for s in tensor_schemas]}."
    )
