...
def _mark_sharding(
    ...
    """
    placement_strategies: Dict[Node, PlacementStrategy] = _mark_tensor_parallel_shardings(
        gm, graph_signature, mesh, parameter_placements
    )
    ...
    for node in gm.graph.nodes:
        if node.op == "placeholder":
            ...
            if node.target == operator.getitem:
                ...
                if (
                    op_schema.op not in DTensor._op_dispatcher.sharding_propagator.op_strategy_funcs
                    and op_schema.op not in DTensor._op_dispatcher.sharding_propagator.op_to_rules
                ):
...
def _generate_default_output_sharding(
    ...
    new_op_schema = OpSchema(
        op=op_schema.op,
        args_schema=pytree.tree_map_only(DTensorSpec, update_arg_spec, op_schema.args_schema),
    ...
    return OutputSharding(
        output_spec=pytree.tree_map_only(FakeTensor, create_output_spec, node.meta["val"]),
    ...
    if isinstance(val, torch.Tensor):
        ...
        for idx, placement in enumerate(spec.placements):
            if placement.is_shard():
                ...
                local_shard = placement._split_tensor(local_shard, num_chunks, with_padding=False, contiguous=True)[0][
...
def _shard_state_dict(
    ...
    for node, placement_strategy in placement_strategies.items():
        ...
        state_dict[fqn] = (
            torch.nn.Parameter(local_param) if isinstance(original_param, torch.nn.Parameter) else local_param
        )
