...
register_op_strategy(aten._to_copy.default, schema_info=RuntimeSchemaInfo(static_kwargkey=["dtype"]))(default_strategy)
...
def equal_strategy(mesh: DeviceMesh, op_schema: OpSchema) -> StrategyType:
    ...
    select_strategy = (
        self_strategy if self_strategy.max_num_shards() >= other_strategy.max_num_shards() else other_strategy
    )
    ...
    for arg_strategy in select_strategy.strategies:
        ...
        if is_tensor_partial(arg_spec):
            ...
            output_spec = DTensorSpec(
                mesh=arg_spec.mesh,
                placements=tuple(Replicate() if isinstance(p, _Partial) else p for p in arg_spec.placements),
            )
