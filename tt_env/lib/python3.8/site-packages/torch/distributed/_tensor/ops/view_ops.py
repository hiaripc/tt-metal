...
def dim_squeeze(shape: Shape, dim: Optional[int] = None) -> DimMap:
    ...
    return tuple(
        InputDim(i) for i, s in enumerate(shape) if s > 1 or (dim is not None and i != normalize_dim(dim, len(shape)))
    )
...
def dim_reduction(ndim: int, dim_or_dims: Optional[Union[int, Sequence[int]]], keepdim: bool) -> DimMap:
    ...
    return tuple(
        InputDim(i) if i not in dim_or_dims else Singleton() for i in range(ndim) if i not in dim_or_dims or keepdim
    )
...
ops: Dict[Callable[..., torch.Tensor], Op] = {
    ...
    torch.movedim: Op(dim_map=lambda input, source, destination: dim_movedim(input.ndim, source, destination)),
    torch.permute: Op(dim_map=lambda input, dims: tuple(InputDim(i) for i in normalize_dims(dims, input.ndim))),
    ...
    torch.transpose: Op(dim_map=lambda input, dim0, dim1: dim_transpose(input.ndim, dim0, dim1)),
...
def propagate_shape_and_sharding(
    ...
    def get_dim_size(cmd: DimSpec) -> Tuple[int, Optional[InputDim]]:
        if isinstance(cmd, InputDim):
            ...
            return (
                ...
                dim0 if isinstance(dim0, InputDim) and dim0.input_dim in sharded_in_dims else None,
            )
    ...
    output_placements = (
        None if needs_reshard else [Shard(dim_map[s.dim]) if isinstance(s, Shard) else s for s in in_shard]
    )
    ...
    def reshape_prop(op_schema: OpSchema) -> OutputSharding:
        ...
        if shard_out is not None:
            ...
            suggested_placements = [
                p if not isinstance(p, Shard) or shardable_dims[p.dim][mesh_dim] else Replicate()
