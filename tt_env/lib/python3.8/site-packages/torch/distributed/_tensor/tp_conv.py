...
def _is_supported(input_size, kernel_size, stride, padding, dilation):
    ...
    if padding[1] != 0:
        if stride[1] != 1:
            raise RuntimeError("Stride must be 1 when there is padding for tensor parallel convolution.")
...
def convolution_backward_handler(
    ...
    # local propagation
    local_results = tp_convolution_backward(op_call, tuple(op_info.local_args), op_info.local_kwargs)
