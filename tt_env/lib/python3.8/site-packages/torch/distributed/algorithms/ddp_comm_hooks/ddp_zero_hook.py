...
def hook_with_zero_step(
    ...
    if not zero._overlap_with_ddp:
        raise ValueError(
            "ZeroRedundancyOptimizer must be constructed with " "`overlap_with_ddp=True` to use this hook properly"
        )
    ...
    if (pg != dist.Backend.NCCL) and (pg != "hccl"):
        raise RuntimeError(
            "Overlapping DDP with ZeRO using this approach currently requires " "NCCL/HCCL backend to avoid hangs"
        )
        ...
        for bucket_index in range(num_buckets):
            ...
            if rank in assigned_ranks:
                ...
                assert bucket_index in overlap_info.bucket_index_to_future, (
                    f"All-reduce future for bucket {bucket_index} not saved " f"on rank {rank}"
                )
