...
def all_gather(
    ...
    if pg is not None:
        ...
        if len(exception_list) > 0:
            raise RuntimeError(error_msg, exception_list) from exception_list[0]  # type: ignore[misc]
        return ret_list
    else:
        if not sync_obj.success:
            raise RuntimeError(
...
def all_gather_object_enforce_type(
    ...
    for i in range(1, list_len):
        if not type_checker(first_obj, object_list[i]):
            raise TypeError(
                f"Object type at index {i} is {type(object_list[i])}, " f"while first object type is {type(first_obj)}"
            )
