...
class P2POp:
    ...
    def __init__(
        self, op: Callable, tensor: torch.Tensor, peer: int, group: Optional[ProcessGroup] = None, tag: int = 0
    ):
        """Init."""
        self.op = op
        self.tensor = tensor
...
def _check_p2p_op_list(p2p_op_list):
    ...
    if not isinstance(p2p_op_list, list) or not all(isinstance(p2p_op, P2POp) for p2p_op in p2p_op_list):
        raise ValueError(
            "Invalid ``p2p_op_list``. Each op is expected to " "to be of type ``torch.distributed.P2POp``."
        )
...
def send(tensor: torch.Tensor, dst: int, group: Optional[ProcessGroup] = None, tag: int = 0) -> None:
    ...
    if get_rank() == dst:
        raise ValueError(
            "Invalid destination rank: destination rank should not be the same as " "the rank of the current process."
        )
...
def broadcast_object_list(object_list, src=0, group=None, device=None):
    ...
    if my_rank == src:
        ...
        object_tensor = torch.empty(  # type: ignore[call-overload]
            torch.sum(object_sizes_tensor).item(), dtype=torch.uint8, device=current_device  # type: ignore[arg-type]
        )
