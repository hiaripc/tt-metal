...
class SimpleElasticAgent(ElasticAgent):
    ...
    def _assign_worker_ranks(self, store, group_rank: int, group_world_size: int, spec: WorkerSpec) -> List[Worker]:
        ...
        role_pos = next(
            idx for idx, role_info in enumerate(role_infos) if _RoleInstanceInfo.compare(role_info, my_role_info) == 0
        )
    ...
    def _invoke_run(self, role: str = DEFAULT_ROLE) -> RunResult:
        ...
        while True:
            ...
            if state == WorkerState.SUCCEEDED:
                log.info(
                    "[%s] worker group successfully finished." " Waiting %s seconds for other agents to finish.",
                    role,
                ...
                if self._remaining_restarts > 0:
                    log.info(
                        "[%s] Worker group %s. " "%s/%s attempts left;" " will restart worker group",
                        role,
                        state.name,
                ...
                if num_nodes_waiting > 0:
                    log.info(
                        "[%s] Detected %s " "new nodes from group_rank=%s; " "will restart worker group",
                        role,
                        num_nodes_waiting,
                        group_rank,
                    )
