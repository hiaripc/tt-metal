...
class ProcessFailure:
    ...
    def __post_init__(self):
        ...
        if os.path.isfile(self.error_file):
            try:
                with open(self.error_file) as fp:
                    ...
                    log.debug("User process failed with error data: %s", json.dumps(self.error_file_data, indent=2))
                    self.message, self.timestamp = self._get_error_data(self.error_file_data)
            except Exception:
        ...
        if not self.message:
            ...
            if self.exitcode < 0:
                self.message = f"Signal {-self.exitcode} ({self.signal_name()})" f" received by PID {self.pid}"
            else:
...
class ChildFailedError(Exception):
    ...
    def __init__(self, name: str, failures: Dict[GlobalRank, ProcessFailure]):
        ...
        assert self.failures  # does not make sense to create a ChildFaileError with no failures
