...
def _parse_rendezvous_config(config_str: str) -> Dict[str, str]:
    ...
    for kv in key_values:
        ...
        if not key:
            raise ValueError(
                "The rendezvous configuration string must be in format " "<key1>=<value1>,...,<keyN>=<valueN>."
            )
...
def parse_rendezvous_endpoint(endpoint: Optional[str], default_port: int) -> Tuple[str, int]:
    ...
    if len(rest) == 1:
        ...
        if port is None or port >= 2**16:
            raise ValueError(
                f"The port number of the rendezvous endpoint '{endpoint}' must be an integer " "between 0 and 65536."
            )
    else:
        port = default_port
