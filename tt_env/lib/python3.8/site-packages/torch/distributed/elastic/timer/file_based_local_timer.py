...
class FileTimerClient(TimerClient):
    ...
    def __init__(
        self, file_path: str, signal=(signal.SIGKILL if sys.platform != "win32" else signal.CTRL_C_EVENT)
    ...
    def _send_request(self, request: FileTimerRequest) -> None:
        ...
        with file:
            ...
            if len(json_request) > select.PIPE_BUF:
                raise RuntimeError(
                    f"FileTimerRequest larger than {select.PIPE_BUF} bytes " f"is not supported: {json_request}"
                )
    ...
    def acquire(self, scope_id: str, expiration_time: float) -> None:
        self._send_request(
            request=FileTimerRequest(
                worker_pid=os.getpid(), scope_id=scope_id, expiration_time=expiration_time, signal=self.signal
            ),
        )

    def release(self, scope_id: str) -> None:
        self._send_request(
            request=FileTimerRequest(worker_pid=os.getpid(), scope_id=scope_id, expiration_time=-1, signal=0),
        )
...
class FileTimerServer:
    ...
    def start(self) -> None:
        log.info(
            "Starting %s..." " max_interval=%s," " daemon=%s", type(self).__name__, self._max_interval, self._daemon
        )
    ...
    def _get_requests(self, fd: io.TextIOWrapper, max_interval: float) -> List[FileTimerRequest]:
        ...
        while not self._stop_signaled or self._run_once:
            ...
            if len(json_request) == 0:
                ...
                requests.append(
                    FileTimerRequest(worker_pid=pid, scope_id=scope_id, expiration_time=expiration_time, signal=signal)
                )
            now = time.time()
