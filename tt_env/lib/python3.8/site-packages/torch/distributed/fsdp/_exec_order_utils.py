        ...
        if self.is_first_iter:
            ...
            if not torch.distributed._functional_collectives.is_torchdynamo_compiling():
                ...
                for (r1, n1), (r2, n2) in itertools.combinations(
                    ((rank, world_num_valid_indices[rank]) for rank in range(self.world_size)),
                    2,
                ):
                    if n1 != n2:
                        raise RuntimeError(
            ...
            if not torch.distributed._functional_collectives.is_torchdynamo_compiling():
                ...
                for (r1, i1), (r2, i2) in itertools.combinations(
                    (
                        (
                            rank,
                            world_indices[rank * num_valid_indices : (rank + 1) * num_valid_indices],
                        )
            ...
            if self.current_order_index >= len(self.handles_pre_forward_order):
                ...
                if expected_handle != handle:
                    ...
                    msg_prefix = (
                        f"Expected to all-gather for {expected_param_names} " "but trying to all-gather parameters for "
                    )
