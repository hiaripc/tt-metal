...
class FlatParamHandle:
    ...
    def __init__(
        ...
        *,
        ...
        if self._use_fake_all_gather or self._use_fake_reduce:
            self._fake_process_group = FakeProcessGroup(rank=process_group.rank(), world_size=process_group.size())
    ...
    def _init_flat_param_and_metadata(
        ...
        if len(params_to_flatten) == 0:
            raise ValueError(f"`params` were not found in `module`'s tree" f"params: {params}\nmodule: {module}")
        if self.rank == 0 and aligned_numel > 0 and total_numel != total_numel_without_padding:
            log.info(
                "FSDP FlatParameter address alignment created " "%s numel of padding (%s vs. %s)",
        ...
        if aligned_numel > 0:
            ...
            if numel_to_pad > 0 and numel_to_pad < self.world_size:
                if self.rank == 0:
                    log.info(
                        "FSDP FlatParameter world size divisibility created " "%s numel of padding",
                        numel_to_pad,
                    )
                padding_tensor = _construct_padding_tensor(numel_to_pad, dtype, False, device)
                params_to_flatten.append(padding_tensor)
    ...
    def _validate_tensors_to_flatten(self, tensors: List[Union[Tensor, nn.Parameter]]) -> Tuple:
        ...
        for tensor in tensors:
            ...
            if dtype is not None and tensor.dtype != dtype:
                raise ValueError(f"Must flatten tensors with uniform dtype but got {dtype} " f"and {tensor.dtype}")
            if (
            ...
            if device is not None and tensor.device != device:
                raise ValueError(
                    "Must flatten tensors on the same device but got both " f"{device} and {tensor.device}"
                )
            dtype = tensor.dtype
    ...
    def _init_shard_metadata(
        ...
        _p_assert(
            ...
            f"numel_padded: {numel_padded} " f"sharded_flat_param_numel: {sharded_flat_param_numel}",
        )
    ...
    def _get_shard_metadata(
        ...
        for i, (
            ...
            in_sharded_flat_param = (
                unsharded_start_idx <= unsharded_param_end_idx and unsharded_end_idx >= unsharded_param_start_idx
            )
            if not in_sharded_flat_param:
                ...
                if unsharded_start_idx <= unsharded_param_start_idx:
                    ...
                    offset_in_shard = 0
                assert offset_in_shard >= 0 and offset_in_shard < sharded_flat_param_numel, (
    ...
    def init_flat_param_attributes(self) -> None:
        ...
        if self.uses_sharded_strategy:
            ...
            unsharded_param_dtype = (
                self._fwd_bwd_param_dtype if self._uses_param_mixed_precision else flat_param.dtype
    ...
    def _use_low_precision_shard(self):
        ...
        flat_param = self.flat_param
        _alloc_storage(flat_param._mp_shard, flat_param._local_shard.size())  # type: ignore[attr-defined]
        # `copy_()` implicitly casts to the low precision
        flat_param._mp_shard.copy_(  # type: ignore[attr-defined]
            flat_param._local_shard.to(self.device, non_blocking=True)  # type: ignore[attr-defined]
        )
    ...
    def _use_unsharded_flat_param(
        ...
        self.flat_param.data = padded_unsharded_flat_param[: unsharded_size.numel()].view(
            unsharded_size
    ...
    def prepare_gradient_for_backward(self):
        ...
        """
        _p_assert(
            self._training_state in (HandleTrainingState.BACKWARD_PRE, HandleTrainingState.IDLE),
        ...
        if flat_param.grad is not None and (
            ...
            _p_assert(
                ...
                f"Expects the sharded gradient to be on {self.device} " f"but got {flat_param.grad.device}",
            )
    ...
    def _use_sharded_flat_param(self) -> None:
        ...
        if self._use_orig_params:
            ...
            skip_use_sharded_views = (
                ...
                and in_forward
                and self._sharding_strategy in NO_RESHARD_AFTER_FORWARD_HANDLE_STRATEGIES
            )
    ...
    def _use_sharded_views(self) -> None:
        ...
        ) in enumerate(zip(self.flat_param._shared_params, self.flat_param._shared_param_infos)):
            self._setattr_param(module, param_name, param)
            prim_param = getattr(prim_module, prim_param_name)
    ...
    def _writeback_orig_params(self) -> bool:
        ...
        if self._skipped_use_sharded_views and self.uses_sharded_strategy:
            ...
            _p_assert(
                ...
                "If skipped using sharded views, the unsharded flat parameter " "should be allocated",
            )
        else:
        ...
        # `._saved_grad_shard`.
        flat_param_grad = (
            flat_param.grad if self.uses_sharded_strategy or not self._offload_params else flat_param._cpu_grad
        )
        ...
        for i, (
            ...
            needs_param_writeback = param_changed or not _same_storage_as_data_ptr(  # changed parameter variable itself
    ...
    def sharded_grad(self) -> Optional[Tensor]:
        ...
        if hasattr(flat_param, "_cpu_grad"):
            ...
            _p_assert(
                ...
                or self._training_state in (HandleTrainingState.FORWARD, HandleTrainingState.IDLE),
                "Sharded strategies should use `_cpu_grad` or `_saved_grad_shard` " "unless in IDLE or FORWARD",
            )
    ...
    def _force_full_precision(self) -> bool:
        return (self._uses_param_mixed_precision or self._uses_reduce_mixed_precision) and (
...
def _construct_padding_tensor(padding_numel: int, dtype: torch.dtype, requires_grad: bool, device: torch.device):
    ...
    return (
        torch.ones((padding_numel,), dtype=dtype, requires_grad=requires_grad, device=device)
