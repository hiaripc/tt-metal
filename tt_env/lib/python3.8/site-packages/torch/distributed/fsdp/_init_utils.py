...
def _init_process_group_state(
    ...
    if process_group is not None and device_mesh is not None:
        raise ValueError(
            "Cannot pass both process_group and device_mesh at the " "same time. Please just pass only one of them."
        )
    ...
    state._gradient_predivide_factor = default_hooks.DefaultState._get_gradient_predivide_factor(
...
def _init_process_group_state_for_hybrid_shard(
    ...
    if device_mesh:
        if _is_valid_hybrid_shard_device_mesh(device_mesh):
            ...
            raise ValueError("Expected device_mesh to have ndim=2 " f"but got {len(device_mesh.get_group())}")
...
def _init_ignored_module_states(
    ...
    if ignored_modules is not None and ignored_states is not None:
        raise ValueError(
            "Cannot pass both ignored_modules and ignored_states at the " "same time. Please just pass ignored_states."
        )
...
def _check_ignored_states(ignored_states: List[Any], passed_as_ignored_states: bool) -> None:
    ...
    if passed_as_ignored_states:
        ...
        if not all(isinstance(state, nn.Module) for state in ignored_states):
            sorted_types = sorted({type(state) for state in ignored_states}, key=repr)
            raise ValueError("ignored_modules expects nn.Module list elements but got " f"types {sorted_types}")
...
def _check_single_device_module(
    ...
    if len(devices) == 2 and torch.device("cpu") in devices:
        if device_id is None:
            raise RuntimeError(
                "To support a module with both CPU and GPU params, " "please pass in device_id argument."
            )
    elif len(devices) > 1:
...
def _need_to_materialize_module(
    ...
    is_torchdistX_deferred_init = (
        not is_meta_module and _TORCHDISTX_AVAIL and any(fake.is_fake(param) for param in managed_params)
    )
...
def _check_orig_params_flattened(
    ...
    for param_name, param in _named_parameters_with_duplicates(fsdp_module):
        if param not in ignored_params and not _is_fsdp_flattened(param):
            raise RuntimeError(f"Found an unflattened parameter: {param_name}; " f"{param.size()} {param.__class__}")
