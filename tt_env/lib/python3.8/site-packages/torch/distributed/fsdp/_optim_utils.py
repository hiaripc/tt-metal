    ...
    for unflat_param_state in unflat_param_states:
        ...
        if state_names is None:
            ...
            if state_names != set(unflat_param_state.keys()):
                raise ValueError(
                    "Differing optimizer state names for the unflattened " f"parameters: {unflat_param_names}"
                )
...
def _flatten_tensor_optim_state(
    ...
    for tensor, shape in zip(pos_dim_tensors, unflat_param_shapes):
        if tensor is None and len(shape) == 0:
            ...
            raise ValueError(
                "Tensor optimizer state does not have same shape as its " f"parameter: {tensor.shape} {shape}"
            )
    ...
    assert flat_tensor.shape == flat_param_shape, (
        f"tensor optim state: {flat_tensor.shape} " f"flat parameter: {flat_param_shape}"
    )
    return flat_tensor
...
def _rekey_sharded_optim_state_dict(
    ...
    param_to_param_key: Dict[nn.Parameter, Union[int, str]] = cast(
        ...
        (
            ...
            else _get_param_to_param_key(optim, model, is_named_optimizer, param_to_fqns, flat_param_to_fqn)
        ),
    )
...
def _get_param_id_to_param_from_optim_input(
    ...
    try:
        ...
        raise TypeError("Optimizer input should be an iterable of Tensors or dicts, " f"but got {optim_input}") from e
    if len(params) == 0:
    ...
    for fqn, gathered_state in output_states.items():
        ...
        for rank, object_state in enumerate(state_info):
            ...
            for name, non_tensor_value in object_state.non_tensors.items():
                ...
                assert curr_non_tensor_value is None or curr_non_tensor_value == non_tensor_value, (
            ...
            for name, scalar_tensor_value in object_state.scalar_tensors.items():
                ...
                assert curr_scalar_tensor_value is None or torch.equal(scalar_tensor_value, curr_scalar_tensor_value), (
...
def _allgather_orig_param_states(
    ...
    has_state_params: List[bool] = [
        True if fqn in output_states else False for fqn, idx in fsdp_param_info.param_indices.items()
    ]
    ...
    for state_name, buffers in state_buffers.items():
        ...
        for numel, is_padding in zip(flat_param._numels_with_padding, flat_param._is_padding_mask):
            frozen_and_no_state = not is_padding and (
                not fsdp_param_info.param_requires_grad[param_idx] and not has_state_params[param_idx]
            )
...
def _convert_state_with_orig_params(
    ...
    for _all_states in all_states.values():
        ...
        assert len(fsdp_param_info.param_requires_grad) > 0, (
            "With use_orig_params, FSDPParamInfo should have requires_grad " "information. However, the length is zero."
        )
        for key, idx in fsdp_param_info.param_indices.items():
    ...
    with SimpleProfiler.profile("preprocessing"):
        ...
        param_key_to_param = cast(
            ...
            (
                ...
                if using_optim_input
                else _get_param_key_to_param(optim, model, is_named_optimizer, param_to_fqns, flat_param_to_fqn)
            ),
        )
        fqn_to_fsdp_param_info = _get_fqn_to_fsdp_param_info(model)
