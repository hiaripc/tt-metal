...
def _catch_all_reshard(
    ...
    try:
        if state._handle:
            ...
            already_resharded = (
                state._handle.flat_param.data_ptr() == state._handle.flat_param._local_shard.data_ptr()
...
def _get_handle_to_prefetch(
    ...
    _p_assert(
        ...
        f"Prefetching is only supported in {valid_training_states} but " f"currently in {training_state}",
    )
    eod = state._exec_order_data
    target_handle: Optional[FlatParamHandle] = None
    if (
        training_state == HandleTrainingState.BACKWARD_PRE and state.backward_prefetch == BackwardPrefetch.BACKWARD_PRE
    ) or (
