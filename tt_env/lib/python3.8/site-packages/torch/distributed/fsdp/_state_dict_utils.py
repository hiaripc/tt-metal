...
def _shared_param_name_infos(module: nn.Module, fsdp_state) -> Iterator[Tuple[str, str, str]]:
    for param_name, module_name in _module_handle(fsdp_state, module).shared_param_module_names():
        module_name = _convert_to_wrapped_module_name(module_name)
        fqn = f"{module_name}{param_name}"
        yield fqn, param_name, module_name
...
@no_type_check
def _sharded_post_load_state_dict_hook(module: nn.Module, fsdp_state: _FSDPState, *args, **kwargs) -> None:
...
def _sharded_pre_load_state_dict_hook(
    ...
    for fqn, _, _ in _param_name_infos(module, fsdp_state):
        ...
        if not fsdp_state._state_dict_config._use_dtensor:
            ...
            assert len(shards) < 2, (
                "Expects 0 or 1 shard per rank " f"but got {len(shards)} shards on rank {fsdp_state.rank}."
            )
