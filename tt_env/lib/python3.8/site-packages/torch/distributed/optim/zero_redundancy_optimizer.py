...
class _ZeROJoinHook(JoinHook):
    def __init__(self, zero):
        assert isinstance(zero, ZeroRedundancyOptimizer), (
            "ZeRO join hook requires passing in a ZeroRedundancyOptimizer " "instance as the state"
        )
        self.zero = zero
        super().__init__()
...
class ZeroRedundancyOptimizer(Optimizer, Joinable):
    ...
    def add_param_group(self, param_group: Dict[str, Any]) -> None:
        ...
        if self.initialized and self._overlap_with_ddp:
            raise RuntimeError(
                "ZeroRedundancyOptimizer with `overlap_with_ddp=True` only " "supports a single parameter group"
            )
    ...
    def _partition_parameters(
        ...
        assert len(self._partition_parameters_cache) == 0, (
            "Specifying `params_per_rank` should only be done when the " "parameters have not been partitioned yet"
        )
    ...
    def _device_to_params_per_rank(
        ...
        assert self.parameters_as_bucket_view, (
            "`_device_to_params_per_rank` should only be used if " "`parameters_as_bucket_view=True`"
        )
    ...
    def _assign_bucket_subset_to_rank(
        ...
        self._bucket_assignments_per_rank_cache[assigned_rank][bucket_index] = _DDPBucketAssignment(
            bucket_index, bucket_params, bucket_offset
        )
            ...
            for bucket_assignment in bucket_assignments.values():
                ...
                for param in params:
                    assert _is_trainable(param), (
                        "Model parameter " "corresponding to a gradient in a DDP bucket should " "require a gradient"
                    )
                    bucket_size += param.numel()
                    dtype = param.dtype  # assumes all same dtype
    ...
    def _verify_and_init_params(
        ...
        if isinstance(params, torch.Tensor):
            raise TypeError("`params` argument should be an iterable of " f"Tensors, but got {torch.typename(params)}")
        try:
            all_params = list(params)
        except TypeError as e:
            raise TypeError(
                "`params` argument should be an iterable of Tensors" f" or dicts, but got {torch.typename(params)}"
            ) from e
        if len(all_params) == 0:
    ...
    def _init_local_optimizer(self) -> None:
        ...
        if self._overlap_with_ddp:
            ...
            assert len(param_groups) == 1, (
                "Initializing the local " "functional optimizer with more than one parameter group"
            )
        ...
        if self._overlap_with_ddp and not hasattr(self.optim, "param_groups"):
            assert hasattr(self.optim, "param_group"), (
                "The functional optimizer should set at least one of the " "attributes `param_group` or `param_groups`"
            )
    ...
    def _init_zero_for_overlap(self) -> None:
        ...
        assert self._overlap_with_ddp, (
            "`_init_zero_for_overlap()` should only be called when " "`overlap_with_ddp=True`"
        )
    ...
    def _get_optimizer_constructor(self, optimizer_class: Any) -> Any:
        ...
        if not self._overlap_with_ddp:
            if optimizer_class in functional_optims:
                ...
                raise ValueError(
                    f"Passing in a functional optimizer {optimizer_class} " "when `overlap_with_ddp=False`"
                )
            else:
            ...
            if optimizer_class in functional_optims:
                ...
                logger.info(
                    "Using the functional optimizer %s " "instead of %s since " "`overlap_with_ddp=True`",
                    optim_constructor,
                    optimizer_class,
                )
