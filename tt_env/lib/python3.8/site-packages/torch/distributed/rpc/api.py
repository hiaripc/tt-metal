...
def _all_gather(obj, worker_names=None, timeout: float = UNSET_RPC_TIMEOUT):
    ...
    """
    if not worker_names:
        assert _ALL_WORKER_NAMES is not None, "`_ALL_WORKER_NAMES` is not initialized for `def _all_gather`."
        worker_names = _ALL_WORKER_NAMES
    ...
    if is_leader:
        ...
        for follower_name in worker_names - {leader_name}:
            fut = rpc_async(
                follower_name, _broadcast_to_followers, args=(sequence_id, states.gathered_objects), timeout=rpc_timeout
            )
...
def _wait_all_workers(timeout=DEFAULT_SHUTDOWN_TIMEOUT):
    ...
    try:
        ...
        logger.error("Failed to respond to 'Shutdown Proceed' in time, got error %s", ex)
        raise ex
...
def remote(to, func, args=None, kwargs=None, timeout=UNSET_RPC_TIMEOUT):
    ...
    with ctx_manager as rf:
        ...
        if qualified_name is not None:
            ...
            (pickled_python_udf, tensors) = _default_pickler.serialize(PythonUDF(func, args, kwargs))
            rref = _invoke_remote_python_udf(dst_worker_info, pickled_python_udf, tensors, timeout, is_async_exec)
...
def _invoke_rpc(to, func, rpc_type, args=None, kwargs=None, rpc_timeout: float = UNSET_RPC_TIMEOUT):
    ...
    with ctx_manager as rf:
        ...
        if qualified_name is not None:
            ...
            (pickled_python_udf, tensors) = _default_pickler.serialize(PythonUDF(func, args, kwargs))
            fut = _invoke_rpc_python_udf(dst_worker_info, pickled_python_udf, tensors, rpc_timeout, is_async_exec)
        if should_profile:
            assert torch.autograd._profiler_enabled()
            assert rf is not None
