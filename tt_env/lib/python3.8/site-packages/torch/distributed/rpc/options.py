...
def _to_device(device: DeviceType) -> torch.device:
    ...
    if device.type != "cuda":
        raise ValueError("`set_devices` expect a list of CUDA devices, but got " f"device type {device.type}.")
    return device
...
def _to_device_map(device_map: Dict[DeviceType, DeviceType]) -> Dict[torch.device, torch.device]:
    ...
    for k, v in device_map.items():
        ...
        if v in reverse_map:
            raise ValueError(
                "`device_map` only supports 1-to-1 mapping, " f"trying to map {k} and {reverse_map[v]} to {v}"
            )
        full_device_map[k] = v
        reverse_map[v] = k
...
class TensorPipeRpcBackendOptions(_TensorPipeRpcBackendOptionsBase):
    ...
    def __init__(
        ...
        *,
        ...
        full_device_maps = {} if device_maps is None else {k: _to_device_map(v) for k, v in device_maps.items()}
