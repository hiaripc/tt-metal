...
_PrepareInputType = Callable[[Union[torch.Tensor, DTensor], Optional[DeviceMesh], Optional[int]], DTensor]

_PrepareOutputType = Callable[[DTensor, Optional[DeviceMesh], Optional[int]], Union[torch.Tensor, DTensor]]
...
def _prepare_output_validate(
    ...
    def wrapper(*args, **kwargs):  # pyre-ignore[2, 3]
        ...
        assert isinstance(output, DTensor), (
            "Expect output of Tensor Parallel to be a DTensor, but found" f" {type(output)}."
        )
...
def _create_1d_device_mesh(device_mesh: DeviceMesh, tp_mesh_dim: int = 0) -> DeviceMesh:
    ...
    assert tp_mesh_dim < device_mesh.ndim and tp_mesh_dim >= -device_mesh.ndim, (
        f"Expect tp_mesh_dim within range [{-device_mesh.ndim}," f" {device_mesh.ndim}), but found {tp_mesh_dim}."
    )
