...
class ExportedProgram:
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if hasattr(self.graph_signature, "lifted_tensor_constants"):
            ordered_tensor_constants = tuple(
                self.tensor_constants[name] for name in self.graph_signature.lifted_tensor_constants
            )
        else:
        ...
        if self.call_spec.out_spec is not None:
            ...
            try:
                ...
                user_inputs = [spec for spec in self.graph_signature.input_specs if spec.kind == InputKind.USER_INPUT]
                for i, value in enumerate(mutated_values):
                    ...
                    if output_spec.kind == OutputKind.BUFFER_MUTATION:
                        ...
                        assert output_spec.target is not None
                        index = next(i for i, spec in enumerate(user_inputs) if spec.arg.name == output_spec.target)
    ...
    def run_decompositions(
        ...
        old_new_placeholder_map = {
            old_node.name: new_node.name for old_node, new_node in zip(old_placeholders, new_placeholders)
        }
        ...
        if len(new_range_constraints) > 0 or len(new_equality_constraints) > 0:
            exported_program = exported_program._transform(
                _AddRuntimeAssertionsForInlineConstraintsPass(new_range_constraints, new_equality_constraints)
            )
