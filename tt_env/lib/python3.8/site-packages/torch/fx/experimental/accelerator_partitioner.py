...
class Partitioner:
    ...
    def saturate_host(self) -> None:
        ...
        while len(used_devices) * 2 + len(replicated_device_to_used_device) <= len(self.devices):
            ...
            idle_devices = [
                d for d in self.devices if d not in used_devices and d not in replicated_device_to_used_device
            ]
            ...
            for used_device in used_devices:
                ...
                available_devices = [
                    ...
                    if d.available_mem_bytes >= used_device.available_mem_bytes - device_to_left_mem_bytes[used_device]
                ]
    ...
    def sparse_nn_partition(self, available_mem_bytes: int) -> None:
        ...
        if len(embedding_partitions) > len(self.devices):
            msg = (
                "Need " + str(len(embedding_partitions)) + " devices, but only " + str(len(self.devices)) + " provided"
            )
