...
def index_select_inference_rule(n: Node, symbols, constraints, counter):
    ...
    c2 = Conj(
        [
            ...
            Disj(
                [
                    IndexSelect(i + 1, symbols[n.args[0]], dims[0], n.args[1], index_select)
    ...
    c3 = Conj(
        [
            is_dyn,
            Disj(
                [IndexSelect(i + 1, symbols[n.args[0]], Dyn, n.args[1], index_select) for i in range(MAX_TENSOR_RANK)]
            ),
        ]
    )
...
def expand_inference_rule(n: Node, symbols, constraints, counter):
    ...
    e2_constraint = BinConstraintT(
        e2, TensorType([arg if isinstance(arg, int) else symbols[arg] for arg in n.args[1:]]), op_eq
    )
...
def cumsum_inference_rule(n: Node, symbols, constraints, counter):
    ...
    for i in range(1, MAX_TENSOR_RANK + 1):
        ...
        c_tensor_i = Conj(
            [BinConstraintT(input, TensorType(new_dims), op_eq), BinConstraintT(output, TensorType(new_dims), op_eq)]
            + [range_check(arg_1, i)]
            + nat_constraints
        )
...
def arange_inference_rule(n: Node, symbols, constraints, counter):
    ...
    c11 = Conj(
        [BinConstraintD(end, Dyn, op_neq), BinConstraintD(start, Dyn, op_neq), BinConstraintD(step, Dyn, op_neq)]
    )
...
def broadcasting_inference_rule(n: Node, symbols, constraints, counter):
    ...
    if isinstance(n.args[0], Node) and isinstance(n.args[1], Node):
        ...
        if isinstance(symbols[n.args[0]], TVar):
            ...
            c = Conj(
                [
                    BinConstraintD(my_output, BinConstraintD(e1, n.args[1], op_code), op_eq),
        ...
        if isinstance(symbols[n.args[1]], TVar):
            ...
            c = Conj(
                [
                    BinConstraintD(my_output, BinConstraintD(e2, n.args[0], op_code), op_eq),
                    BinConstraintD(0, my_output, op_leq),
                ]
            )
            return [c], counter
...
def adaptive_inference_rule(n: Node, module_instance, symbols, constraints, counter):
    ...
    c2 = BinConstraintT(
        avg_pool, TensorType([d1, d2, module_instance.output_size[0], module_instance.output_size[1]]), op_eq
    )
