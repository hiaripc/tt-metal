...
def optimize_for_inference(
    model: torch.nn.Module, pass_config: Optional[Dict[str, Any]] = None, tracer: Type[fx.Tracer] = fx.Tracer
) -> torch.nn.Module:
    """
    ...
    for cur_idx, node in enumerate(fx_graph.nodes):
        if node.op == "call_method" and node.target == "to_mkldnn":
            ...
            cur_colors = [
                get_color(i) for i in node.all_input_nodes if isinstance(i, fx.Node) if get_color(i) is not None
            ]
