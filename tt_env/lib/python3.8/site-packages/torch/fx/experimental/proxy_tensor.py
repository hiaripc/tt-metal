...
def proxy_call(proxy_mode, func, pre_dispatch, args, kwargs):
    ...
    if not pytree.tree_all_only(torch.Tensor, can_handle_tensor, (args, kwargs)):
        not_implemented_log.debug(
            "ProxyTensorMode tensors without proxy had unrecognized subclasses: %s", unrecognized_types
        )
...
class ProxyTorchDispatchMode(TorchDispatchMode):
    def __init__(
        self, tracer, tracing_mode, pre_dispatch=False, _allow_fake_constant=False, _error_on_data_dependent_ops=True
    ):
        dk = torch._C.DispatchKey.PreDispatch if pre_dispatch else None
        super().__init__(dk)
        self.tracer = tracer
    ...
    def wrapped(*args):
        ...
        if tracing_mode == "real":
            ...
            if fake_tensor_mode is None:
                shape_env = ShapeEnv()
                fake_tensor_mode = FakeTensorMode(
                    allow_fallback_kernels=False, allow_non_fake_inputs=_allow_non_fake_inputs, shape_env=shape_env
                )
            else:
        ...
        def wrap_fake(x):
            ...
            # NB: don't match on bools
                ...
                return shape_env.create_symintnode(
                    shape_env.create_symbol(x, source, positive=None), hint=x, source=source
                )
        ...
        ), pre_dispatch_mode, proxy_function_mode, sym_mode, proxy_mode, disable_autocast_cache():
            t = dispatch_trace(
                wrap_key(func, args, fx_tracer, pre_dispatch), tracer=fx_tracer, concrete_args=tuple(phs)
            )
