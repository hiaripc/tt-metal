...
def _constrain_symbol_range(
    shape_env, s: sympy.Symbol, compiler_min: int, compiler_max: int, runtime_min: int, runtime_max: int
):
...
def _constrain_range_for_size(a, min: Optional[int] = None, max: Optional[int] = None):
    ...
    _constrain_symbol_range(
        a.node.shape_env, a.node.expr, compiler_min=compiler_min, compiler_max=max, runtime_min=min, runtime_max=max
    )
...
def constrain_range(a, *, min: Optional[int], max: Optional[int] = None):
    ...
    # SymInt).
    _constrain_symbol_range(
        a.node.shape_env, a.node.expr, compiler_min=min, compiler_max=max, runtime_min=min, runtime_max=max
    )
...
def _assert_bound_is_rational(expr: sympy.Expr, bound: ValueRanges):
    ...
    """
    assert bound.lower.is_rational or bound.lower.is_Boolean or not bound.lower.is_finite or expr.has(sympy.Pow), (
        bound,
        expr,
    )
    assert bound.upper.is_rational or bound.upper.is_Boolean or not bound.upper.is_finite or expr.has(sympy.Pow), (
        bound,
        expr,
    )
...
class DynamicDimConstraintPrinter(StrPrinter):
    ...
    def _print_Relational(self, expr):
        return "{} {} {}".format(
            self.parenthesize(expr.lhs, precedence(expr)), expr.rel_op, self.parenthesize(expr.rhs, precedence(expr))
        )
...
class DimConstraints:
        ...
        if len(free_symbols) > 1:
            ...
            if reduced == sympy.false:
                self._inconsistencies.append(
                    f"{expr}, obtained by rewriting {orig_expr} with congruences, " "is inconsistent!"
                )
        ...
        for s, congruences in self._congruences.items():
            ...
            if remainder_modulus_pairs:
                ...
                reduced_congruences[s].update(
                    congruence for congruence in congruences_to_check if not sympy.checksol(congruence, substitution)
                )
            else:
    ...
    def specialize_divisor_symbols(self):
        ...
        self._univariate_inequalities = {
            s: exprs for s, exprs in self._univariate_inequalities.items() if s not in self._substitutions
        }
        self._congruences = {
            s: congruences for s, congruences in self._congruences.items() if s not in self._substitutions
        }
        ...
        if self._dcp.source_name_to_debug_name:
            ...
            if forced_specializations:
                ...
                for s, val in forced_specializations.items():
                    buf += (
                        f"  - {s} must be specialized to {val} because the guards generated for it are too complex.\n"
                    )
...
class ShapeEnv:
    ...
    def __init__(
        self, *, should_record_events: Optional[bool] = None, tracked_fakes: Optional[List[Any]] = None, **kwargs
    ) -> None:
        self._init(**kwargs)
        ...
        self.should_record_events = (
            ...
            else (self._translation_validation_enabled and not config.translation_validation_no_bisect)
        )
    ...
    def _create_symbolic_sizes_strides_storage_offset(
        ...
        *,
        ...
        dynamic_strides_offset = (
            DimDynamic.STATIC if all(r == DimDynamic.STATIC for r in dynamic_dims) else DimDynamic.DUCK
        )
    ...
    def create_unspecified_symbol(
        ...
        return self.create_symbol(
            val, source, dynamic_dim, constraint_dim, positive=None, do_not_specialize_zero_one=True
        )
        ...
        if not _simplified:
            for source, expr in input_guards:
                ...
                if (
                    ...
                    and not equalities_inputs.is_equal(source, symbol_to_source[expr][0])
                ):
                    msg = (
                ...
                if not is_trivial and len(expr.free_symbols) == 1:
                    ...
                    for c in constraints:
                        if isinstance(c, StrictMinMaxConstraint):
                            ...
                            msg = f"Not all values of {var_with_range} " f"satisfy the generated guard {guard_expr}."
