        ...
        def validate(self) -> None:
            ...
            if r == z3.sat:
                ...
                raise ValidationException(
                    ...
                    self._assertions,
                    self._target_exprs,
                    failed_source_exprs=[inp for inp in self._source_exprs if not model.evaluate(inp)],
                )
            else:
                if r == z3.unknown:
...
def bisect(shape_env):
    ...
    assert len(args) >= 2, (
        f"bisecting expects {event.name} to have at least 2 positional arguments. " f"Got: {len(args)}"
    )
    assert isinstance(args[1], sympy.Basic), (
        f"bisecting expects {event.name} to have a SymPy expression as its second argument. " f"Got: {type(args[1])}"
    )
