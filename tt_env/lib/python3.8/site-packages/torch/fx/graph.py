            ...
            if has_args_kwargs_tuple:
                ...
                fn_kwargs = (
                    ...
                    + ", ".join(
                        f"'{k}':{v}"
                        for k, v in zip(
                            self.pytree_info.in_spec.children_specs[1].context, self.pytree_info.orig_args[count_args:]
                        )
                    )
                    + "}"
                )
...
class Graph:
    ...
    @compatibility(is_backward_compatible=True)
    def placeholder(
        self, name: str, type_expr: Optional[Any] = None, default_value: Any = inspect.Signature.empty
    ) -> Node:
        """
    ...
    def get_attr(self, qualified_name: str, type_expr: Optional[Any] = None) -> Node:
        ...
        if self.owning_module and not _get_attr_reference_exists(self.owning_module, qualified_name):
            warnings.warn(
            ...
            for node in self.nodes:
                ...
                if node.op in ["get_attr", "call_module"]:
                    ...
                    for i, atom in enumerate(target_atoms):
                        ...
                        if node.op == "call_module" and not isinstance(new_m_itr, torch.nn.Module):
                            raise RuntimeError(
