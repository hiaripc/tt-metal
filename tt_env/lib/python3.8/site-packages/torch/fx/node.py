...
def _get_qualified_name(func: Callable[..., Any]) -> str:
    ...
    # torch.Tensor.{fn}
    if isinstance(func, (types.MethodDescriptorType, types.WrapperDescriptorType)) and func is getattr(
        torch.Tensor, func.__name__, None
    ):
...
class Node:
    ...
    @compatibility(is_backward_compatible=True)
    def format_node(
        self, placeholder_names: Optional[List[str]] = None, maybe_return_typename: Optional[List[str]] = None
    ) -> Optional[str]:
        """
