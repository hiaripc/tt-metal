...
def _maybe_get_inplace_op(op):
    ...
    maybe_inplace_op = (
        None if maybe_namespace_module is None else getattr(maybe_namespace_module, f"{op_base_name}_", None)
    )
...
def reinplace(gm, *sample_args):
    ...
    input_storages = {
        StorageWeakRef(node.meta["fake_result"]._typed_storage()) for node in gm.graph.nodes if node.op == "placeholder"
    }
