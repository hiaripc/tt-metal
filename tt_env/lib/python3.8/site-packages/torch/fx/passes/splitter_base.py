...
class FxNetAccNodesFinder:
    ...
    def __call__(self) -> NodeSet:
        ...
        self.acc_nodes = {
            ...
            if n.op in CALLABLE_NODE_OPS and self.operator_support.is_node_supported(submodules, n)
        }
...
class _SplitterBase:
    ...
    def node_support_preview(self, dump_graph: bool = False):
        ...
        for node in self.module.graph.nodes:
            ...
            last_index = len(arg_dtypes) - next(
                (i for i, dtype in enumerate(reversed(arg_dtypes)) if dtype is not None),
                len(arg_dtypes),
            )
            ...
            kwarg_dtypes_tuple = tuple(
                (k, get_dtype(arg)) for k, arg in node.kwargs.items() if isinstance(arg, torch.fx.Node)
            )
    ...
    def tag(self, subgraphs: List[Subgraph]):
        ...
        for subgraph in subgraphs:
            tag = (
                f"_run_on_acc_{len(self.tags)}" if subgraph.is_acc else f"{self.non_acc_submodule_name}{len(self.tags)}"
            )
