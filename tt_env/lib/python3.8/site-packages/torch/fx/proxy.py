            ...
            for k, v in a.items():
                ...
                def no_node(arg):
                    if isinstance(arg, Node):
                        raise RuntimeError(
                            "Keys for dictionaries used as an argument cannot contain a " f"Node. Got key: {k}"
                        )
    ...
    def __torch_function__(cls, orig_method, types, args=None, kwargs=None):
        ...
        if torch.overrides.is_tensor_method_or_property(orig_method):
            ...
            return tracer.create_proxy(
                "call_function", orig_method, args, kwargs, name=tracer.graph._target_to_str(orig_method.__name__)
            )
