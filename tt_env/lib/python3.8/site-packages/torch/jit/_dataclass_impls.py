...
def compose_fn(cls, name: str, body_lines: List[str], signature: str) -> ParsedDef:
    ...
    return ParsedDef(
        py_ast,
        ctx=SourceContext(source=decl, filename=fake_filename, file_lineno=0, leading_whitespace_len=0),
        source=decl,
...
def synthesize__repr__(cls) -> ParsedDef:
    return compose_fn(
        ...
        [
            ...
            + ", ".join([f"{field.name}=self.{field.name}" for field in dataclasses.fields(cls) if field.repr])
            + ")'"
        ],
        signature="(self) -> str",
    )
