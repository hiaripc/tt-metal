...
def get_jit_class_def(cls, self_name):
    ...
    method_defs = [
        get_jit_def(obj, name, self_name=self_name, is_classmethod=is_classmethod(obj)) for (name, obj) in methods
    ]
...
def build_class_def(ctx, py_def, methods, properties, self_name, assigns):
    r = ctx.make_range(py_def.lineno, py_def.col_offset, py_def.col_offset + len("class"))
...
def build_param_list(ctx, py_args, self_name, pdt_arg_types=None):
    ...
    arg_and_types = [
        (
            arg,
            pdt_arg_types[arg.arg] if pdt_arg_types and bool(pdt_arg_types[arg.arg]) else None,
        )
    ...
    result = [
        build_param(ctx, arg, self_name, kwarg_only=False, pdt_arg_type=arg_type) for arg, arg_type in arg_and_types
    ]
    result += [
...
class StmtBuilder(Builder):
    ...
    def build_AnnAssign(ctx, stmt):
        ...
        if (
            ...
            raise ValueError(
                "Type annotations on instance attributes must be declared in " f"__init__, not '{ctx.funcname}': {sr}"
            )
...
class ExprBuilder(Builder):
    ...
    def build_Name(ctx, expr):
        ...
        if expr.id.startswith(_reserved_prefix):
            raise NotSupportedError(
                r,
                "names of variables used in JIT-ed functions " "can't start with " + _reserved_prefix,
            )
        if expr.id == "True":
