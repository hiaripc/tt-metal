...
def _binary_helper(fn, args, kwargs, inplace):
    ...
    same_layout = (torch.is_tensor(data_args[1]) or is_masked_tensor(data_args[1])) and (
    ...
    if args0_layout == torch.sparse_coo:
        ...
        if same_layout:
            if not (
                ...
                and _tensors_match(data_args[0].col_indices(), data_args[1].col_indices())
            ):
                raise ValueError(
...
NATIVE_BINARY_MAP = {getattr(torch.ops.aten, name): _torch_binary(name) for name in BINARY_NAMES}
NATIVE_INPLACE_BINARY_MAP = {
    getattr(torch.ops.aten, name): _torch_inplace_binary(name) for name in INPLACE_BINARY_NAMES
}
