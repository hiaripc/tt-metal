...
def _tensors_match(a, b, exact=True, rtol=1e-05, atol=1e-08):
    ...
    if a.layout == b.layout == torch.sparse_coo:
        return _tensors_match(a.values(), b.values(), exact) and _tensors_match(a.indices(), b.indices(), exact)
...
def _masked_tensor_str(data, mask, formatter):
    ...
    if data.dim() == 1:
        formatted_elements = [
            formatter.format(d.item()) if isinstance(d.item(), float) else str(d.item()) for d in data
        ]
        max_len = max(8 if x[1] else len(x[0]) for x in zip(formatted_elements, ~mask))
        return "[" + ", ".join(["--".rjust(max_len) if m else e for (e, m) in zip(formatted_elements, ~mask)]) + "]"
...
class MaskedTensor(torch.Tensor):
    ...
    def _validate_members(self):
        ...
        if data.layout == torch.sparse_coo:
            ...
            if not _tensors_match(data.crow_indices(), mask.crow_indices(), exact=True) or not _tensors_match(
                data.col_indices(), mask.col_indices(), exact=True
            ):
                raise ValueError(
