...
def as_nested_tensor(
    tensor_list: List[Tensor], dtype: Optional[DType] = None, device: Optional[Device] = None, layout=None
) -> Tensor:
    r"""
...
def nested_tensor(
    tensor_list, *, dtype=None, layout=None, device=None, requires_grad=False, pin_memory=False
) -> Tensor:
    r"""
    ...
    if layout == torch.strided:
        return _nested.nested_tensor(
            tensor_list, dtype=dtype, device=device, requires_grad=requires_grad, pin_memory=pin_memory
        )
    elif layout == torch.jagged:
        # Need to:
        #   * Detach tensors to discard autograd history
        #   * Wrap lists of scalars as tensors
        list_of_tensors = [t.detach() if isinstance(t, Tensor) else torch.as_tensor(t) for t in tensor_list]
...
def narrow(
    tensor: Tensor, dim: int, start: Union[int, Tensor], length: Union[int, Tensor], layout=torch.strided
) -> Tensor:
    r"""
