...
def check_schema(schema_str: str, func, *args, **kwargs) -> None:
    ...
    arg_type_check_fns = {
        ...
        "jt": lambda x: isinstance(x, NestedTensor) and x._lengths is None,  # ops with "jt" require contiguous JT only
        "jt_all": lambda x: isinstance(x, NestedTensor),  # ops with "jt_all" can accept all kinds of JT
        "any": lambda x: True,
    }
    for i, named_arg_type in enumerate(named_arg_types):
        ...
        if i >= len(args):
            if not is_optional:
                raise ValueError(f"NestedTensor {func.__name__}({schema_str}) " f"missing required argument: {name}")
            continue
...
def check_ragged_dim_same(func, a: NestedTensor, a_name: str, b: NestedTensor, b_name: str) -> None:
    ...
    if a._size[a._ragged_idx] != b._size[b._ragged_idx]:
        raise RuntimeError(
            f"NestedTensor {func.__name__}: expected {a_name} and {b_name} to have the " "same exact offsets tensor."
        )
...
def jagged_binary_pointwise(func, *args, **kwargs):
    ...
    mismatch_error_msg = (
        f"cannot call binary pointwise function {func.__name__} with inputs of shapes " f"{a.shape} and {b.shape}"
    )
...
register_jagged_func(torch.ops.aten.is_contiguous.memory_format, "self: jt_all, memory_format: any?")(
...
register_jagged_func(torch.ops.aten._softmax.default, "self: jt, dim: any, half_to_float: any")(jagged_unary_pointwise)
