...
def _check_for_seq_len_0_nested(params: SDPAParams, debug=False) -> bool:
    ...
    q_is_safe = (
        _check_for_seq_len_0_and_consistent_head_dim_nested_helper(params.query, "query", debug)
    ...
    k_is_safe = (
        _check_for_seq_len_0_and_consistent_head_dim_nested_helper(params.key, "key", debug)
    ...
    v_is_safe = (
        _check_for_seq_len_0_and_consistent_head_dim_nested_helper(params.value, "value", debug)
...
def jagged_scaled_dot_product_attention(
    ...
    if query.dim() > 3 and key.dim() > 3 and value.dim() > 3 and query._ragged_idx == 1:
        ...
        output = F.scaled_dot_product_attention(
            ...
            key._values,
            value._values,
            attn_mask=(attn_mask._values if isinstance(attn_mask, NestedTensor) else attn_mask),
            dropout_p=dropout_p,
            is_causal=is_causal,
            scale=scale,
        )
