    ...
    * :attr:`head_bias` if set to True, adds a bias term to the 'head' of the
    ...
    def forward(self, input_: Tensor, target_: Tensor) -> _ASMoutput:
        ...
        if targ_dim == 1:
            ...
            if input_.dim() != 2:
                raise RuntimeError(
                    "1D target tensor expects 2D input tensors, " "but found inputs with size", input_.size()
                )
        elif targ_dim == 0:
            if input_.dim() != 1:
                raise RuntimeError(
                    "0D target tensor expects 1D input tensors, " "but found inputs with size", input_.size()
                )
        else:
