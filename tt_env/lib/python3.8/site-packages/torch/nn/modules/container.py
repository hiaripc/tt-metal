...
class Sequential(Module):
    ...
    def __add__(self, other) -> "Sequential":
        if isinstance(other, Sequential):
            ...
            raise ValueError(
                "add operator supports only objects " f"of Sequential class, but {str(type(other))} is given."
            )
...
class ParameterList(Module):
    ...
    def extend(self, values: Iterable[Any]) -> Self:
        ...
        if not isinstance(values, container_abcs.Iterable) or isinstance(values, torch.Tensor):
            raise TypeError(
                "ParameterList.extend should be called with an " "iterable, but got " + type(values).__name__
            )
        for value in values:
            self.append(value)
        return self
        ...
        for k, p in enumerate(self):
            if isinstance(p, torch.Tensor):
                ...
                parastr = "{} containing: [{} of size {}{}]".format(
                    "Parameter" if isinstance(p, Parameter) else "Tensor", p.dtype, size_str, device_str
                )
        ...
        for k, p in self.items():
            if isinstance(p, torch.Tensor):
                ...
                if p.device.type in ["cuda", torch._C._get_privateuse1_backend_name()]:
                    ...
                    device_str = ""
                parastr = "{} containing: [{} of size {}{}]".format(
                    "Parameter" if isinstance(p, Parameter) else "Tensor", torch.typename(p), size_str, device_str
                )
                child_lines.append("  (" + str(k) + "): " + parastr)
            else:
                child_lines.append("  (" + str(k) + "): Object of type: " + type(p).__name__)
