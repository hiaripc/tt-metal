    ...
    * :attr:`dilation` controls the spacing between the kernel points; also known as the à trous algorithm.
    ...
    def forward(self, input: Tensor, output_size: Optional[List[int]] = None) -> Tensor:
        ...
        return F.conv_transpose1d(
            input, self.weight, self.bias, self.stride, self.padding, output_padding, self.groups, self.dilation
        )
...
class _ConvTransposeMixin(_ConvTransposeNd):
    def __init__(self, *args, **kwargs):
        warnings.warn("_ConvTransposeMixin is a deprecated internal class. " "Please consider using public APIs.")
...
class _LazyConvXdMixin(LazyModuleMixin):
    ...
    def initialize_parameters(self, input) -> None:  # type: ignore[override]
        ...
        if self.has_uninitialized_params():  # type: ignore[misc]
            ...
            if self.transposed:
                self.weight.materialize((self.in_channels, self.out_channels // self.groups, *self.kernel_size))
            else:
                self.weight.materialize((self.out_channels, self.in_channels // self.groups, *self.kernel_size))
            if self.bias is not None:
    ...
    def _get_in_channels(self, input: Tensor) -> int:
        ...
        if input.dim() not in (num_dims_no_batch, num_dims_batch):
            raise RuntimeError(
                ...
                "got input of size: {}".format(num_dims_no_batch, num_dims_batch, self.__class__.__name__, input.shape)
            )
