...
class Unfold(Module):
    ...
    * For the case of two input spatial dimensions this operation is sometimes
    ...
    def __init__(
        self, kernel_size: _size_any_t, dilation: _size_any_t = 1, padding: _size_any_t = 0, stride: _size_any_t = 1
    ) -> None:
        super().__init__()
        self.kernel_size = kernel_size
        self.dilation = dilation
    ...
    def extra_repr(self) -> str:
        return "kernel_size={kernel_size}, dilation={dilation}, padding={padding}," " stride={stride}".format(
            **self.__dict__
        )
