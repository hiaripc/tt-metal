...
class _DDPSink(Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        if static_graph and not delay_ar_enqueued:
            Variable._execution_engine.queue_callback(reducer._delay_all_reduce)  # type: ignore[call-arg,misc]
...
class DistributedDataParallel(Module, Joinable):
    ...
    def __init__(
        ...
        if bool(delay_all_reduce_named_params is not None) != bool(param_to_hook_all_reduce is not None):
            self._log_and_throw(
                ValueError,
                "delay_all_reduce_named_params and param_to_hook_all_reduce " "need to be set at the same time.",
            )
