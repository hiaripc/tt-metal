...
class GroupNormPerSampleGrad(torch.autograd.Function):
    ...
    def forward(ctx, kwarg_names, _, *expanded_args_and_kwargs):
        ...
        output, mean, rstd = forward_helper(
            torch.native_group_norm, (input, weight, bias, N, C, HxW, num_groups, eps), {}
        )
    ...
    def backward(ctx, grad_output):
        ...
        if input.requires_grad:
            ...
            results.append(
                bw_fn(grad_output_c, input_c, mean, rstd, weight_c, N, C, HxW, num_groups, (True, False, False))[0]
            )
        else:
            results.append(None)
        ...
        if hasattr(ctx, "weight"):
            set_grad_sample_if_exists(
                weight, lambda _: torch.einsum("ni...->ni", F.group_norm(input, num_groups, eps=eps) * grad_output)
            )
        if hasattr(ctx, "bias"):
