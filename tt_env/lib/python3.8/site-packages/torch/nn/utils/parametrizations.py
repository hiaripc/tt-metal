...
class _Orthogonal(Module):
    ...
    def right_inverse(self, Q: torch.Tensor) -> torch.Tensor:
        if Q.shape != self.shape:
            raise ValueError(
                f"Expected a matrix or batch of matrices of shape {self.shape}. " f"Got a tensor of shape {Q.shape}."
            )
...
def orthogonal(
    module: Module, name: str = "weight", orthogonal_map: Optional[str] = None, *, use_trivialization: bool = True
) -> Module:
    ...
    if weight.ndim < 2:
        raise ValueError("Expected a matrix or batch of matrices. " f"Got a tensor of {weight.ndim} dimensions.")
    ...
    if orth_enum is None:
        raise ValueError(
            'orthogonal_map has to be one of "matrix_exp", "cayley", "householder". ' f"Got: {orthogonal_map}"
        )
...
class _SpectralNorm(Module):
    def __init__(self, weight: torch.Tensor, n_power_iterations: int = 1, dim: int = 0, eps: float = 1e-12) -> None:
        ...
        if n_power_iterations <= 0:
            raise ValueError(
                "Expected n_power_iterations to be positive, but " f"got n_power_iterations={n_power_iterations}"
            )
    ...
    def _power_method(self, weight_mat: torch.Tensor, n_power_iterations: int) -> None:
        ...
        for _ in range(n_power_iterations):
            ...
            self._u = F.normalize(
                torch.mv(weight_mat, self._v), dim=0, eps=self.eps, out=self._u  # type: ignore[has-type]
...
def spectral_norm(
    module: Module, name: str = "weight", n_power_iterations: int = 1, eps: float = 1e-12, dim: Optional[int] = None
) -> Module:
