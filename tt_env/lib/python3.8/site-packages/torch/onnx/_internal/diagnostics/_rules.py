...
class _MissingStandardSymbolicFunction(infra.Rule):
    ...
    def format(  # type: ignore[override]
        ...
        return (
            self,
            level,
            self.format_message(op_name=op_name, opset_version=opset_version, issue_url=issue_url),
        )
...
class _FxGraphToOnnx(infra.Rule):
    ...
    def format(self, level: infra.Level, graph_name) -> Tuple[infra.Rule, infra.Level, str]:  # type: ignore[override]
        """Returns a tuple of (Rule, Level, message) for this Rule.
...
class _FxNodeToOnnx(infra.Rule):
    ...
    def format(self, level: infra.Level, node_repr) -> Tuple[infra.Rule, infra.Level, str]:  # type: ignore[override]
        """Returns a tuple of (Rule, Level, message) for this Rule.
...
class _FxPass(infra.Rule):
    ...
    def format(self, level: infra.Level, pass_name) -> Tuple[infra.Rule, infra.Level, str]:  # type: ignore[override]
        """Returns a tuple of (Rule, Level, message) for this Rule.
...
class _NoSymbolicFunctionForCallFunction(infra.Rule):
    ...
    def format(self, level: infra.Level, target) -> Tuple[infra.Rule, infra.Level, str]:  # type: ignore[override]
        """Returns a tuple of (Rule, Level, message) for this Rule.
...
class _UnsupportedFxNodeAnalysis(infra.Rule):
    ...
    def format_message(self, node_op_to_target_mapping) -> str:  # type: ignore[override]
        """Returns the formatted default message of this Rule.
...
class _FindOperatorOverloadsInOnnxRegistry(infra.Rule):
    ...
    def format(self, level: infra.Level, node) -> Tuple[infra.Rule, infra.Level, str]:  # type: ignore[override]
        """Returns a tuple of (Rule, Level, message) for this Rule.
    ...
    fx_graph_to_onnx: _FxGraphToOnnx = dataclasses.field(
        default=_FxGraphToOnnx.from_sarif(
            **{
                ...
                "message_strings": {"default": {"text": "Transforming FX graph {graph_name} to ONNX graph."}},
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
        ),
    ...
    fx_node_to_onnx: _FxNodeToOnnx = dataclasses.field(
        default=_FxNodeToOnnx.from_sarif(
            **{
                ...
                "message_strings": {"default": {"text": "Transforming FX node {node_repr} to ONNX node."}},
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
        ),
    ...
    no_symbolic_function_for_call_function: _NoSymbolicFunctionForCallFunction = dataclasses.field(
        default=_NoSymbolicFunctionForCallFunction.from_sarif(
            **{
                ...
                "message_strings": {
                    "default": {"text": 'No symbolic function to convert the "call_function" node {target} to ONNX. '}
                },
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
    ...
    unsupported_fx_node_analysis: _UnsupportedFxNodeAnalysis = dataclasses.field(
        default=_UnsupportedFxNodeAnalysis.from_sarif(
            **{
                ...
                "short_description": {"text": "Result from FX graph analysis to reveal unsupported FX nodes."},
                "full_description": {
                ...
                "message_strings": {"default": {"text": "Unsupported FX nodes: {node_op_to_target_mapping}. "}},
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
        ),
    ...
    op_level_debugging: _OpLevelDebugging = dataclasses.field(
        default=_OpLevelDebugging.from_sarif(
            **{
                ...
                "short_description": {"text": "Report any op level validation failure in warnings."},
                "full_description": {
    ...
    find_opschema_matched_symbolic_function: _FindOpschemaMatchedSymbolicFunction = dataclasses.field(
        default=_FindOpschemaMatchedSymbolicFunction.from_sarif(
            **{
                ...
                "message_strings": {
                    "default": {"text": "The OnnxFunction: {symbolic_fn} is the nearest match of the node {node}."}
                },
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
    ...
    fx_node_insert_type_promotion: _FxNodeInsertTypePromotion = dataclasses.field(
        default=_FxNodeInsertTypePromotion.from_sarif(
            **{
                ...
                "message_strings": {"default": {"text": "Performing explicit type promotion for node {target}. "}},
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
        ),
    ...
    find_operator_overloads_in_onnx_registry: _FindOperatorOverloadsInOnnxRegistry = dataclasses.field(
        default=_FindOperatorOverloadsInOnnxRegistry.from_sarif(
            **{
                ...
                "message_strings": {
                    "default": {"text": "Checking if the FX node: {node} is supported in onnx registry."}
                },
                "help_uri": None,
                "properties": {"deprecated": False, "tags": []},
            }
