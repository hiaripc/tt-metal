...
class Diagnostic:
    ...
    thread_flow_locations: List[infra.ThreadFlowLocation] = dataclasses.field(default_factory=list)
    ...
    def sarif(self) -> sarif.Result:
        ...
        kind: Literal["informational", "fail"] = "informational" if self.level == infra.Level.NONE else "fail"
        ...
        sarif_result.code_flows = [
            sarif.CodeFlow(
                thread_flows=[sarif.ThreadFlow(locations=[loc.sarif() for loc in self.thread_flow_locations])]
            )
        ]
