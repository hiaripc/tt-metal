...
def enable_fake_mode():
    ...
    fake_mode = fake_tensor.FakeTensorMode(
        ...
        shape_env=ShapeEnv(allow_scalar_outputs=False, allow_dynamic_output_shape_ops=False),
    )
...
class ONNXRuntimeOptions:
    ...
    execution_providers: Optional[Sequence[Union[str, Tuple[str, Dict[Any, Any]]]]] = None
...
class ONNXProgram:
    ...
    def __init__(
        ...
        *,
        ...
        model_torch: Optional[Union[torch.nn.Module, Callable, torch_export.ExportedProgram]] = None,
    ):
    ...
    def __call__(
        self,
        *args: Any,
        model_with_state_dict: Optional[Union[torch.nn.Module, Callable, torch_export.ExportedProgram]] = None,
...
class Exporter:
    ...
    def _assert_fake_tensor_mode(self):
        ...
        has_any_non_fake_tensors = pytree.tree_any(
            lambda x: isinstance(x, torch.Tensor) and not isinstance(x, torch._subclasses.FakeTensor),
            (self.model_args, self.model_kwargs),
        )
...
def _assert_dependencies(export_options: ResolvedExportOptions):
    ...
    def missing_package(package_name: str, exc_info: logging._ExcInfoType):
        message = f"Please install the `{package_name}` package " f"(e.g. `python -m pip install {package_name}`)."
...
def common_pre_export_passes(
    ...
    analysis.UnsupportedFxNodesAnalysis(diagnostic_context, module, options.onnxfunction_dispatcher).analyze(
        infra.levels.ERROR
    )
