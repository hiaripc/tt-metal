...
class UnsupportedFxNodesAnalysis(_pass.Analysis):
    ...
    def _lint(
        ...
        normalized_op_targets_map = {
            op: list(targets.keys()) for op, targets in analysis_result.unsupported_op_to_target_mapping.items()
        }
    ...
    def analyze(self, diagnostic_level: diagnostics.infra.Level) -> UnsupportedFxNodesAnalysisResult:
        ...
        for node in self.module.graph.nodes:
            if node.op == "call_function":
                ...
                internal_opname: registration.OpName = self.onnxfunction_dispatcher._get_aten_name(
                    node=node, diagnostic_context=self.diagnostic_context
                )
                overload_registration = self.onnxfunction_dispatcher.onnx_registry.is_registered_op(
                ...
                default_registration = self.onnxfunction_dispatcher.onnx_registry.is_registered_op(
                    namespace=internal_opname.namespace,
                    op_name=internal_opname.op_name,
                    overload=None,
                )
                if not overload_registration and not default_registration:
                    op_to_target_mapping.setdefault(node.op, {}).setdefault(str(node.target), None)
