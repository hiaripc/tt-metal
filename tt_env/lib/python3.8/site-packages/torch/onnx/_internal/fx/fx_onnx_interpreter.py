...
def _retrieve_or_adapt_input_to_graph_set(
    ...
    if isinstance(onnx_tensor, (tuple, list)) and any(
        isinstance(node, torch.fx.Node) and fx_type_utils.is_torch_symbolic_type(node.meta.get("val"))
...
def _wrap_fx_args_as_onnxscript_args(
    ...
    onnxscript_args = tuple(
        _retrieve_or_adapt_input_to_graph_set(arg, fx_name_to_onnxscript_value, tracer) for arg in complete_args
    )
...
class FxOnnxInterpreter:
    ...
    def run(
        ...
        onnxfunction_dispatcher: onnxfunction_dispatcher.OnnxFunctionDispatcher,
        op_level_debug: bool,
        parent_onnxscript_graph: Optional[onnxscript_graph_building.TorchScriptGraph] = None,
    ...
    def call_module(
        ...
        outputs: Union[  # type: ignore[no-redef]
            onnxscript_graph_building.TorchScriptTensor,
            Tuple[onnxscript_graph_building.TorchScriptTensor, ...],
        ] = parent_onnxscript_graph.add_module_call(unique_module_name, sub_onnxscript_graph, onnx_args)
