...
class OnnxFunctionDispatcher:
    ...
    def dispatch(
        self,
        node: torch.fx.Node,
        onnx_args: Sequence[Optional[Union[fx_type_utils.TensorLike, str, int, float, bool, list]]],
    ...
    def _filter_or_keep_complex(
        ...
        if any(
            ...
            if isinstance(arg, torch.fx.Node) and "val" in arg.meta and isinstance(arg.meta["val"], torch.Tensor)
        ):
    ...
    def get_function_overloads(
        ...
        diagnostic = diagnostics.UnsupportedFxNodeDiagnostic(
            diagnostics.rules.no_symbolic_function_for_call_function,
            diagnostics.levels.ERROR,
            f"Cannot find symbolic function for {op_full_name}, " f"which should be registered under {node.target}.",
            unsupported_fx_node=node,
        )
        diagnostic_context.log(diagnostic)
        raise diagnostics.RuntimeErrorWithDiagnostic(diagnostic)
...
class _OnnxSchemaChecker:
    ...
    def perfect_match_inputs(
        ...
        args: Sequence[Optional[Union[fx_type_utils.TensorLike, str, int, float, bool, list]]],
        ...
        with diagnostic.log_section(logging.INFO, "Checking perfect match..."):
            ...
            for schema_input, torch_input in zip(self.op_schema.inputs, function_inputs):
                ...
                if not allowed_types.intersection(torch_input_compatible_types) and not any(
                    fx_type_utils.is_optional_onnx_dtype_str(onnx_type_str) for onnx_type_str in allowed_types
                ):
    ...
    def _match_onnx_attribute_type(
        self,
        attribute_name: str,
        attribute: Union[fx_type_utils.Argument, onnxscript_graph_building.TorchScriptTensor],
    ...
    def _record_matching_score(
        self,
        inputs: Sequence[Optional[Union[fx_type_utils.TensorLike, str, int, float, bool, list]]],
...
def _find_onnx_data_type(
    torch_input: Optional[Union[fx_type_utils.TensorLike, str, int, float, bool, list, tuple]]
) -> Set[str]:
    ...
    if (
        torch_input is None
        or (isinstance(torch_input, fx_type_utils.TensorLike) and torch_input.dtype is None)
