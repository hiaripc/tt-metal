...
class _ModuleStackMeta:
    ...
    def __init__(
        self,
        nn_module_stack_meta: Optional[Union[_FX_TRACER_NN_MODULE_STACK_META_TYPE, _DYNAMO_NN_MODULE_STACK_META_TYPE]],
    ):
        self._module_stack = []
    ...
    def add_leaf_node(self, leaf_node: _LeafNode) -> None:
        ...
        if self.is_same_module_as(leaf_node) or leaf_node.fx_op == "call_module":
            ...
            if isinstance(last_node, _ModuleNode) and (
                last_node.is_parent_module_of(leaf_node) or last_node.is_same_module_as(leaf_node)
            ):
            ...
            raise AssertionError(
                f"Node {leaf_node} ({leaf_node.stack_meta}) does not belong to module " f"{self._stack_meta}."
            )
