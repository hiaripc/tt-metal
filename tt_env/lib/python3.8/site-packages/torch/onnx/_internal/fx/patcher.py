...
class ONNXTorchPatcher:
    ...
    def __init__(self):
        ...
        if has_safetensors_and_transformers:
                    ...
                    for k in f.keys():
                        ...
                        if not fake_mode:
                            ...
                            result[k] = torch.empty(
                                ...
                                dtype=safetensors.torch._getdtype(empty_tensor.get_dtype()),
                            )
                return result
            ...
            self.transformers_modeling_utils_safe_load_file = transformers.modeling_utils.safe_load_file

    def __enter__(self):
        torch.load = self.torch_load_wrapper
        torch._utils._rebuild_tensor = self.torch__util_rebuild_tensor_wrapper

        self.torch_fx__symbolic_trace__wrapped_methods_to_patch = torch.fx._symbolic_trace._wrapped_methods_to_patch
        desired_wrapped_methods = copy.deepcopy(torch.fx._symbolic_trace._wrapped_methods_to_patch)
        ...
        if has_safetensors_and_transformers:
            ...
            transformers.modeling_utils.safe_load_file = self.safetensors_torch_load_file_wrapper

    def __exit__(self, exc_type, exc_value, traceback):
        torch.load = self.torch_load
        torch._utils._rebuild_tensor = self.torch__util_rebuild_tensor
        torch.fx._symbolic_trace._wrapped_methods_to_patch = self.torch_fx__symbolic_trace__wrapped_methods_to_patch
        if has_safetensors_and_transformers:
