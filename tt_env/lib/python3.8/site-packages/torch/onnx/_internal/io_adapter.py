...
class InputAdaptStep(Protocol):
    ...
    def apply(
        ...
        model: Optional[Union[torch.nn.Module, Callable, torch_export.ExportedProgram]] = None,
...
class ConvertComplexToRealRepresentationInputStep(InputAdaptStep):
    ...
    def apply(
        ...
        return (
            tuple(
                torch.view_as_real(arg) if isinstance(arg, torch.Tensor) and arg.is_complex() else arg
...
class RemoveNonTensorInputStep(InputAdaptStep):
    ...
    def apply(
        ...
        assert not model_kwargs
        return (
            tuple(arg for arg in model_args if not isinstance(arg, (int, float, bool, str))),
            {},
        )
...
class ConvertComplexToRealRepresentationOutputStep(OutputAdaptStep):
    ...
    def apply(
        ...
        """
        return [
            torch.view_as_real(output) if isinstance(output, torch.Tensor) and torch.is_complex(output) else output
