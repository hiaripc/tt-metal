...
class OrtBackend:
    ...
    def __init__(self, options: Optional[OrtBackendOptions] = None):
        ...
        self._resolved_onnx_exporter_options = torch.onnx._internal.exporter.ResolvedExportOptions(
            torch.onnx.ExportOptions() if self._options.export_options is None else self._options.export_options
        )
        ...
        self.run = (
            _run_onnx_session_with_ortvaluevector if hasattr(ORTC, "push_back_batch") else _run_onnx_session_with_fetch
        )
    ...
    def get_cached_instance_for_options(
        ...
        def reusable(a: OrtBackendOptions, b: OrtBackendOptions):
            ...
            if a.export_options is not None and b.export_options is not None:
                return (
                    ...
                    and a.export_options.op_level_debug == b.export_options.op_level_debug
                    and a.export_options.diagnostic_options == b.export_options.diagnostic_options
        ...
        if backend is None:
            assert len(OrtBackend.__instance_cache) < OrtBackend.__instance_cache_max_count, (
