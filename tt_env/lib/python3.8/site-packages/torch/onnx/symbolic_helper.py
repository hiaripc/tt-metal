...
def _parse_arg(
    ...
    if node.kind() == "onnx::Constant":
        ...
        if desc == "i":
            ...
            raise errors.SymbolicValueError(
                f"ONNX symbolic does not understand the Constant node '{node}' " f"specified with descriptor '{desc}'.",
                value,
            )
    ...
    raise errors.SymbolicValueError(
        "Expected node type 'onnx::Constant' " f"for argument '{arg_name}' of node '{node_name}', got '{node.kind()}'.",
        value,
    )
...
def _get_const(value, desc, arg_name):
    if not _is_constant(value):
        raise errors.SymbolicValueError(
            f"ONNX symbolic expected a constant value of the '{arg_name}' argument, " f"got '{value}'",
            value,
        )
...
def _unpack_list(list_value: _C.Value) -> List[_C.Value]:
    ...
    if list_node.kind() != "prim::ListConstruct":
        raise errors.SymbolicValueError(
            f"ONNX symbolic expected node type prim::ListConstruct, " f"got '{list_node}'.",
            list_value,
        )
...
def _unpack_tuple(tuple_value: _C.Value) -> Tuple[_C.Value, ...]:
    ...
    if not _is_tuple_construct(tuple_value):
        raise errors.SymbolicValueError(
            f"ONNX symbolic expected node type 'prim::TupleConstruct', " f"got '{tuple_node.kind()}'.",
            tuple_value,
        )
...
def is_complex_value(x: _C.Value) -> bool:
    assert _is_value(x)
    return _type_utils.JitScalarType.from_value(x, _type_utils.JitScalarType.UNDEFINED) in {
...
def is_caffe2_aten_fallback() -> bool:
    return (
        GLOBALS.operator_export_type == _C_onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK and _C_onnx._CAFFE2_ATEN_FALLBACK
    )
...
def _is_fp(value) -> bool:
    return _type_utils.JitScalarType.from_value(value, _type_utils.JitScalarType.UNDEFINED) in {
...
def _interpolate_warning(interpolate_mode):
    ...
    warnings.warn(
        "You are trying to export the model with " + onnx_op + " for ONNX opset version "
...
def _interpolate_size_to_scales(g: jit_utils.GraphContext, input, output_size, dim):
    ...
    if _is_value(output_size):
        ...
        scales_constant = [
            1.0 if i < 2 else float(output_size[-(dim - i)]) / float(input.type().sizes()[-(dim - i)])
...
def _interpolate_helper(name, dim, interpolate_mode):
    ...
    def symbolic_fn(g, input, output_size, *args):
        ...
        coordinate_transformation_mode = (
            "asymmetric" if interpolate_mode == "nearest" else "align_corners" if align_corners else "half_pixel"
        )
...
def __interpolate_helper(
    ...
    coordinate_transformation_mode = (
        "asymmetric" if mode == "nearest" else "align_corners" if align_corners else "half_pixel"
    )
...
) -> Tuple[_type_utils.JitScalarType, Optional[_C.Value], Optional[_C.Value], Optional[_C.Value],]:
...
def quantize_helper(
    ...
    if _type_utils.JitScalarType.from_value(zero_point, _type_utils.JitScalarType.UNDEFINED) not in {
