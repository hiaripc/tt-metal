...
def clamp(g: jit_utils.GraphContext, self, min, max):
    ...
    if symbolic_helper._is_none(min):
        ...
        if symbolic_helper._get_tensor_rank(min) == 0 and symbolic_helper._get_tensor_rank(max) == 0:
...
def index_put(g: jit_utils.GraphContext, self, indices_list_value, values, accumulate=False):
    ...
    if len(indices_list) > 1:
        ...
        indices_list = [
            symbolic_helper._unsqueeze_helper(g, opset9.expand(g, ind, broadcast_index_shape, None), [-1])
...
def split(g: jit_utils.GraphContext, self, split_size_or_sizes, dim, _outputs=None):
    if not symbolic_helper._is_split_static(split_size_or_sizes, _outputs):
        ...
        if (
            ...
            split_sizes = [
                symbolic_helper._unsqueeze_helper(g, v, [0]) for v in symbolic_helper._unpack_list(split_size_or_sizes)
            ]
...
def _prepare_onnx_paddings(g: jit_utils.GraphContext, input, pad):
    ...
    paddings = g.op(
        "Concat",
        pad,
        g.op("ConstantOfShape", extension, value_t=torch.tensor([0], dtype=torch.int64)),
        axis_i=0,
    )
...
def index(g: jit_utils.GraphContext, self, index):
    ...
    if len(indices) == 1:
        ...
        if not symbolic_helper._is_none(index) and (
            ...
            or _type_utils.JitScalarType.from_value(index) == _type_utils.JitScalarType.UINT8
        ):
...
def _get_im2col_indices_along_dim(g: jit_utils.GraphContext, input_d, kernel_size_d, dilation_d, padding_d, stride_d):
    ...
    blocks_d_indices = symbolic_helper._unsqueeze_helper(g, blocks_d_indices, [0])  # Reshape to [1, -1]
...
def embedding_bag(
    ...
    indices_len = symbolic_helper._unsqueeze_helper(
        g,
        symbolic_helper._size_helper(g, indices, g.op("Constant", value_t=torch.tensor(0))),
        [0],
    )
...
def embedding_renorm(g: jit_utils.GraphContext, weight, indices, max_norm, norm_type):
    ...
    if norm_i == 1:
        ...
        raise errors.SymbolicValueError(
            f"Unsupported: ONNX export of embedding_renorm with norm: {norm_i}. " "Only 1. and 2. are supported.",
            weight,
        )
