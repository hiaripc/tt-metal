...
def sqrt(g: jit_utils.GraphContext, self):
    if _type_utils.JitScalarType.from_value(self, _type_utils.JitScalarType.UNDEFINED) in {
...
def _reduce_op_symbolic(onnx_op_name, allow_multi_dim_support=True):
    ...
    def symbolic(g, self, dim=None, keepdim=None):
        ...
        if dim is None or dim == tuple():
            ...
            dim, keepdim = symbolic_helper._get_const(dim, desc, "dim"), symbolic_helper._get_const(
                keepdim, "i", "keepdim"
            )
...
@_onnx_symbolic(
    "aten::max_pool1d",
    decorate=[
        _apply_params("max_pool1d", torch.nn.modules.utils._single, 1, return_indices=False),
        _export("max_pool1d"),
    ],
)
@_onnx_symbolic(
    "aten::max_pool2d",
    decorate=[
        _apply_params("max_pool2d", torch.nn.modules.utils._pair, 2, return_indices=False),
        _export("max_pool2d"),
    ],
)
@_onnx_symbolic(
    "aten::max_pool3d",
    decorate=[
        _apply_params("max_pool3d", torch.nn.modules.utils._triple, 3, return_indices=False),
        _export("max_pool3d"),
    ],
)
@_beartype.beartype
...
@_onnx_symbolic(
    ...
    decorate=[
        _apply_params("adaptive_avg_pool1d", "AveragePool", torch.nn.modules.utils._single),
...
@_onnx_symbolic(
    ...
    decorate=[
        _apply_params("adaptive_avg_pool2d", "AveragePool", torch.nn.modules.utils._pair),
...
@_onnx_symbolic(
    ...
    decorate=[
        _apply_params("adaptive_avg_pool3d", "AveragePool", torch.nn.modules.utils._triple),
...
def bitwise_not(g: jit_utils.GraphContext, input):
    if not symbolic_helper._is_bool(input):
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting bitwise Not " "for non-boolean input values",
            input,
        )
...
def bitwise_or(g, self, other):
    if not symbolic_helper._is_bool(self):
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting bitwise OR " "for non-boolean input values. self: ",
            self,
        )
    if not symbolic_helper._is_bool(other):
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting bitwise OR " "for non-boolean input values. other: ",
            other,
        )
...
def __and_(g: jit_utils.GraphContext, input, other):
    if not symbolic_helper._is_bool(input):
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting bitwise AND " "for non-boolean input values",
            input,
        )
    if not symbolic_helper._is_bool(other):
        raise errors.SymbolicValueError(
...
def __or_(g: jit_utils.GraphContext, input, other):
    if not symbolic_helper._is_bool(input):
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting bitwise OR " "for non-boolean input values",
            input,
        )
    if not symbolic_helper._is_bool(other):
        raise errors.SymbolicValueError(
...
def __xor_(g: jit_utils.GraphContext, input, other):
    if not symbolic_helper._is_bool(input):
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting bitwise XOR " "for non-boolean input values",
            input,
        )
    if not symbolic_helper._is_bool(other):
        raise errors.SymbolicValueError(
...
def log_softmax(g: jit_utils.GraphContext, input, dim, dtype=None):
    ...
    if input_dim is None:
        return symbolic_helper._unimplemented(
            "dim",
            "ONNX and PyTorch use different strategies to split the input. " "Input rank must be known at export time.",
        )
    if dim < 0:
        dim = input_dim + dim
    is_transpose_required = input_dim != dim + 1
...
def _log_softmax(g: jit_utils.GraphContext, input, dim, half_to_float):
    if (
        half_to_float
        and _type_utils.JitScalarType.from_value(input, _type_utils.JitScalarType.UNDEFINED)
...
def batch_norm(
    ...
    if (
        ...
        and not symbolic_helper.args_have_same_dtype([input, weight, bias, running_mean, running_var])
        ...
        return symbolic_helper._onnx_opset_unsupported_detailed(
            ...
            9,
            15,
            "All input tensors must have the same `dtype`." " Turn off Autocast or export using opset version 15.",
            input,
        )
...
def instance_norm(
    ...
    if (
        ...
        if n is None:
            raise errors.SymbolicValueError(
                "Unsupported: ONNX export of instance_norm training for unknown " "batch size.",
                input,
            )
        c = input_size[1]
...
def unfold(g: jit_utils.GraphContext, input, dimension, size, step):
    ...
    if sizedim is not None:
        ...
        stack = [
            symbolic_helper._slice_helper(g, input, axes=[dimension], starts=[low], ends=[hi])
        ...
        unsqueeze = [
            symbolic_helper._unsqueeze_helper(g, g.op("Transpose", t, perm_i=perm), [dimension]) for t in stack
        ]
...
@_onnx_symbolic("aten::alpha_dropout_", decorate=[_apply_params("aten::alpha_dropout_")])  # See Note [Export inplace]
@_onnx_symbolic(
...
def _pack_padded_sequence(g: jit_utils.GraphContext, input, lengths, batch_first):
    ...
    if (
        _type_utils.JitScalarType.from_value(lengths, _type_utils.JitScalarType.UNDEFINED)
        != _type_utils.JitScalarType.INT
    ):
...
def one_hot(g: jit_utils.GraphContext, self, num_classes):
    ...
    if _type_utils.JitScalarType.from_value(num_classes, _type_utils.JitScalarType.UNDEFINED) in {
        _type_utils.JitScalarType.UINT8,
        _type_utils.JitScalarType.INT8,
        _type_utils.JitScalarType.INT,
        _type_utils.JitScalarType.INT16,
...
def arange(g: jit_utils.GraphContext, *args):
    ...
    if len(args) == 2 or len(args) == 5:
        ...
        arange_tensor = g.op(
            "Add",
            symbolic_helper._squeeze_helper(g, nonzero(g, ones(g, range_tensor, dtype, *(args[3:]))), [1]),
            start,
        )
        return g.op("Cast", arange_tensor, to_i=_type_utils.JitScalarType(dtype).onnx_type())
...
def index(g: jit_utils.GraphContext, self, index):
    ...
    def try_mask_to_index(index):
        if not symbolic_helper._is_none(index) and (
            _type_utils.JitScalarType.from_value(index, _type_utils.JitScalarType.UNDEFINED)
            == _type_utils.JitScalarType.UINT8
...
def meshgrid(g: jit_utils.GraphContext, tensor_list, indexing: Optional[str] = None):
    ...
    tensors = [
        symbolic_helper._reshape_helper(g, t, g.op("Constant", value_t=torch.LongTensor([-1])))
...
def index_add(g: jit_utils.GraphContext, self, dim, index, other, alpha=None):
    ...
    if dim is None:
        raise errors.SymbolicValueError(
            "ONNX export does NOT support exporting 'index_add_()' function with " "unknown 'dim' value.",
            self,
        )
...
def prim_constant(g: jit_utils.GraphContext, *inputs, **attrs):
    ...
    if node.output().type().isSubtypeOf(_C.ListType.ofInts()) or node.output().type().isSubtypeOf(
        _C.ListType.ofFloats()
    ):
