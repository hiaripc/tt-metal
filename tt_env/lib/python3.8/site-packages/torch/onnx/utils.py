...
def exporter_context(model, mode: _C_onnx.TrainingMode, verbose: bool):
    with select_model_mode_for_export(model, mode) as mode_ctx, disable_apex_o2_state_dict_hook(
        model
...
def _optimize_graph(
    ...
    _C._jit_pass_onnx_unpack_quantized_weights(graph, params_dict, symbolic_helper.is_caffe2_aten_fallback())
    ...
    if GLOBALS.onnx_shape_inference:
        try:
            ...
            if (
                ...
                and exc.args[0] == "ScalarType UNKNOWN_SCALAR is an unexpected tensor scalar type!"
            ):
...
def _trace_and_get_graph_from_model(model, args):
    ...
    if orig_state_dict_keys != torch.jit._unique_state_dict(model).keys():
        raise RuntimeError(
            "state_dict changed after running the tracer; " "something weird is happening in your model!"
        )
...
def _pre_trace_quant_model(model, args):
    ...
    """
    if any(hasattr(m, "_packed_params") for m in getattr(model, "modules", list)()) or any(
        getattr(arg, "is_quantized", False) for arg in args
    ):
...
def unconvertible_ops(
    ...
    try:
        ...
        raise errors.OnnxExporterError(
            "Failed to discover unconvertible ops because of errors during the JIT graph " "generation process."
        ) from e
...
def _export(
    ...
    try:
        ...
        if isinstance(model, (torch.nn.Module, torch.jit.ScriptModule)):
            module_typenames_to_export_as_functions = _setup_trace_module_map(model, export_modules_as_functions)
        ...
        with exporter_context(model, training, verbose):
            ...
            defer_weight_export = export_type is not _exporter_states.ExportTypes.PROTOBUF_FILE
            if custom_opsets is None:
                custom_opsets = {}
            ...
            # string in memory.
            if (operator_export_type is _C_onnx.OperatorExportTypes.ONNX) and (not val_use_external_data_format):
                try:
...
def _run_symbolic_function(
    ...
    try:
        ...
        raise errors.UnsupportedOperatorError(
            ...
            opset_version,
            symbolic_function_group.get_min_supported() if symbolic_function_group else None,
        )
