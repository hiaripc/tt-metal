...
def verify(
    ...
    dynamic_axes: Optional[Mapping[str, Union[Mapping[int, str], Mapping[str, Sequence[int]]]]] = None,
...
class GraphInfoPrettyPrinter:
    ...
    def _node_count_segment_str(self) -> str:
        ...
        error_node_kind = (
            f"({self.graph_info.essential_node_kinds().pop()})" if node_count == 1 and has_mismatch else ""
        )
...
class GraphInfo:
    ...
    def all_mismatch_leaf_graph_info(self) -> List["GraphInfo"]:
        ...
        no_mismatch_children = (self.upper_graph_info is None or not self.upper_graph_info.has_mismatch()) and (
    ...
    def _graph_partition_pivot(self) -> int:
        ...
        included_node_indices = [
            i for i, n in enumerate(self.graph.nodes()) if n.kind() not in self._EXCLUDED_NODE_KINDS
        ]
    ...
    def find_mismatch(
        ...
        assert len(self.input_args) + len(self.params_dict) == len(list(self.graph.inputs())), (
