...
def _check_verbose_deprecated_warning(verbose):
    ...
    if verbose != "deprecated":
        warnings.warn(
            "The verbose parameter is deprecated. Please use get_last_lr() " "to access the learning rate.", UserWarning
        )
        return verbose
    return False
        ...
        if last_epoch == -1:
            ...
            for i, group in enumerate(optimizer.param_groups):
                if "initial_lr" not in group:
                    raise KeyError(
                        "param 'initial_lr' is not specified " f"in param_groups[{i}] when resuming an optimizer"
                    )
...
class LinearLR(LRScheduler):
    ...
    def __init__(
        self, optimizer, start_factor=1.0 / 3, end_factor=1.0, total_iters=5, last_epoch=-1, verbose="deprecated"
    ):
