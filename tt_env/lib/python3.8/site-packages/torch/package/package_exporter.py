...
class PackageExporter:
    ...
    def __init__(
        ...
        if isinstance(importer, Importer):
            ...
            if not isinstance(importer, collections.abc.Sequence):
                raise TypeError(
                    "importer arg should be an Importer or a sequence of Importers, " f"got {type(importer)} instead."
                )
    ...
    def _write(self, filename, str_or_bytes):
        if filename in self._written_files:
            raise AssertionError(
                f"Tried to write file '{filename}', but it already exists in this archive. " "Please file a bug."
            )
    ...
    def _can_implicitly_extern(self, module_name: str):
        ...
        return top_level_package_name == "torch" or (
            top_level_package_name not in _DISALLOWED_MODULES and is_stdlib_module(top_level_package_name)
        )
