...
class SynchronizedDataLoaderPattern(Pattern):
    ...
    def match(self, event: _ProfilerEvent):
        def is_dataloader_function(name: str, function_name: str):
            return name.startswith(os.path.join("torch", "utils", "data", "dataloader.py")) and name.endswith(
                function_name
            )
...
class GradNotSetToNonePattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = (
            "Detected gradient set to zero instead of None. " "Please add 'set_to_none=True' when calling zero_grad()."
        )
        self.url = (
            "https://pytorch.org/tutorials/recipes/recipes/tuning_guide.html"
...
class Conv2dBiasFollowedByBatchNorm2dPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = (
            "Detected bias enabled in Conv2d that is followed by BatchNorm2d. Please set 'bias=False' in Conv2d."
        )
        self.url = (
            "https://pytorch.org/tutorials/recipes/recipes/tuning_guide.html"
