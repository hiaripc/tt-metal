...
def unpack_int8_to_int4(weight):
    assert weight.dim() == 2
    assert weight.dtype == torch.int8
    return torch.stack((weight & 0xF, (weight >> 4) & 0xF), dim=2).view(weight.shape[0], 2 * weight.shape[1])
...
def quantized_weight_reorder_for_mixed_dtypes_linear_cutlass(weight, dtypeq, transpose=False):
    ...
    if dtypeq == torch.quint4x2:
        cols_permuted = (
            ...
            + (torch.arange(0, nrows // 16, device=device).reshape(-1, 1) * 16).expand(nrows // 16, 16)
        ).view(-1)
    else:
        cols_permuted = (
            torch.tensor(
