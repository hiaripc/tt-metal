...
def register_package(
    priority: int, tagger: Callable[[STORAGE], Optional[str]], deserializer: Callable[[STORAGE, str], Optional[STORAGE]]
):
    """
    ...
    if pickle_module is not None and pickle_module.__name__ == "dill":
        ...
        if not check_module_version_greater_or_equal(pickle_module, required_dill_version, False):
            raise ValueError(
                ...
                ).format(".".join([str(num) for num in required_dill_version]), pickle_module.__version__)
            )
    ...
    with _open_file_like(f, "rb") as opened_file:
        if _is_zipfile(opened_file):
            ...
            with _open_zipfile_reader(opened_file) as opened_zipfile:
                ...
                return _load(
                    opened_zipfile, map_location, pickle_module, overall_storage=overall_storage, **pickle_load_args
                )
        if mmap:
            raise RuntimeError(
...
def _legacy_load(f, map_location, pickle_module, **pickle_load_args):
        ...
        if typename == "module":
            ...
            if root_key not in deserialized_objects:
                ...
                typed_storage = torch.storage.TypedStorage(
                    wrap_storage=restore_location(obj, location), dtype=dtype, _internal=True
                )
                ...
                if typed_storage._data_ptr() == 0:
                    typed_storage = torch.storage.TypedStorage(
                        device=typed_storage._untyped_storage.device, dtype=dtype, _internal=True
                    )
    ...
    for key in deserialized_storage_keys:
        ...
        typed_storage._untyped_storage._set_from_file(
            f, offset, f_should_read_directly, torch._utils._element_size(typed_storage.dtype)
        )
