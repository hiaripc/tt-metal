...
function(dedent outvar text)
    ...
    if(NOT _dedent_exitcode EQUAL 0)
        message(
            ERROR
            " Failed to remove indentation from: \n\"\"\"\n${text}\n\"\"\"
...
macro(caffe2_interface_library SRC DST)
    ...
    if(${__src_target_type} STREQUAL "STATIC_LIBRARY")
        ...
        if(APPLE)
            target_link_libraries(${DST} INTERFACE -Wl,-force_load,\"$<TARGET_FILE:${SRC}>\")
        elseif(MSVC)
            # In MSVC, we will add whole archive in default.
            target_link_libraries(${DST} INTERFACE "$<TARGET_FILE:${SRC}>")
            target_link_options(${DST} INTERFACE "-WHOLEARCHIVE:$<TARGET_FILE:${SRC}>")
        else()
    ...
    elseif(${__src_target_type} STREQUAL "SHARED_LIBRARY")
        if("${CMAKE_CXX_COMPILER_ID}" MATCHES "GNU")
            target_link_libraries(${DST} INTERFACE "-Wl,--no-as-needed,\"$<TARGET_FILE:${SRC}>\" -Wl,--as-needed")
        else()
        ...
        target_link_libraries(${DST} INTERFACE $<TARGET_PROPERTY:${SRC},INTERFACE_LINK_LIBRARIES>)
    else()
        message(
            FATAL_ERROR
            "You made a CMake build file error: target "
            ${SRC}
...
macro(torch_hip_get_arch_list store_var)
    if(DEFINED ENV{PYTORCH_ROCM_ARCH})
        ...
        execute_process(
            ...
            COMMAND
                bash "-c" "grep -v gfx000 | sort -u | xargs | tr -d '\n'"
...
function(torch_compile_options libname)
    ...
    target_compile_options(${libname} PRIVATE $<$<COMPILE_LANGUAGE:CXX>:${private_compile_options}>)
    if(USE_CUDA)
        ...
        target_compile_options(${libname} PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:-Xcompiler=${private_compile_options}>)
    endif()
