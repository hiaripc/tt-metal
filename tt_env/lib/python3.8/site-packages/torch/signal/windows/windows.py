...
def hann(
    ...
    *,
    ...
    return general_hamming(
        M, alpha=0.5, sym=sym, dtype=dtype, layout=layout, device=device, requires_grad=requires_grad
    )
...
def blackman(
    ...
    *,
    ...
    return general_cosine(
        M, a=[0.42, 0.5, 0.08], sym=sym, dtype=dtype, layout=layout, device=device, requires_grad=requires_grad
    )
...
def general_cosine(
    ...
    *,
    ...
    k = torch.linspace(
        start=0, end=(M - 1) * constant, steps=M, dtype=dtype, layout=layout, device=device, requires_grad=requires_grad
    )

    a_i = torch.tensor(
        [(-1) ** i * w for i, w in enumerate(a)], device=device, dtype=dtype, requires_grad=requires_grad
    )
...
def general_hamming(
    ...
    *,
    ...
    return general_cosine(
        M, a=[alpha, 1.0 - alpha], sym=sym, dtype=dtype, layout=layout, device=device, requires_grad=requires_grad
    )
