...
def check_dtype(f_name, t, dtype, *additional_dtypes):
    check(
        t.dtype == dtype and t.dtype in ((torch.half, torch.bfloat16, torch.float) + tuple(*additional_dtypes)),
...
def scatter_mm_meta(
    M, K, N, Ms, Ks, GROUP_SIZE=None, TILE_M=None, TILE_N=None, SPLIT_N=None, num_warps=None, num_stages=None, **extra
):
...
def bsr_dense_addmm_meta(
    ...
    **extra,
    ...
    if {SPLIT_N, num_warps, num_stages, GROUP_SIZE_ROW} == {None}:
        device_name = torch.cuda.get_device_name()
        meta = get_meta(
            "bsr_dense_addmm", (M, K, N, Ms, Ks, beta == 0, beta == 1, alpha == 1), device_name, version=(0, dtype, 0.5)
        )
        if meta is not None:
            meta.update(**extra)
            return meta
...
def _bsr_scatter_mm_indices_data(indices_format, M, K, N, Ms, Ks, nbatches, SPLIT_N, compressed_sparse_tensor_as_key):
    ...
    if indices_format == "bsr_strided_mm_compressed":
        ...
        r_offsets = (a + b).view(-1)
        c_indices = torch.cat(
            (crow_indices[:1], torch.cumsum(crow_indices_diff[non_zero_row_indices].repeat_interleave(SPLIT_N), 0))
        )
...
def bsr_scatter_mm(bsr, other, indices_data=None, out=None):
    ...
    if bsr._nnz() == 0:
        ...
        accumulators = torch.zeros(
            (nbatches * Ms // blocksize[0] * Ns // blocksize[0], blocksize[0], blocksize[0]),
            dtype=bsr.dtype,
            device=bsr.device,
        )
        others = (
        ...
        out.copy_(
            accumulators.unflatten(0, (nbatches, Ms // blocksize[0], Ns // blocksize[0]))
...
if has_triton():
    ...
    def _sampled_addmm_kernel(
        ...
        crow_indices_offset_ptr = (
            crow_indices_ptr + crow_indices_batch_stride * batch_pid + crow_indices_stride * row_block_pid
        )
        ...
        for _ in range(row_nnz):
            ...
            for k_tile in range(0, k, TILE_K):
                ...
                mat1_block = tl.load(
                    mat1_block_ptrs + mat1_col_block_stride * k_offsets[None, :], mask=mask_k[None, :], other=0.0
                )
    ...
    def _run_dense_rowspace_kernel(blocksize, values, crow_indices, col_indices, dense, output, max_grid, meta):
        ...
        def kernel(grid, *sliced_tensors):
            _bsr_strided_dense_rowspace_kernel[grid](
                *ptr_stride_extractor(*sliced_tensors), *blocksize, acc_dtype=acc_dtype, allow_tf32=allow_tf32, **meta
            )

        launch_kernel(kernel, tensor_dims_map, full_grid, grid_blocks)

    def _run_sampled_addmm_kernel(
        alpha, beta, is_beta_zero, blocksize, k, tile_k, values, crow_indices, col_indices, mat1, mat2, max_grid
    ):
    ...
    def sampled_addmm(
        ...
        *,
        ...
        _run_sampled_addmm_kernel(
            alpha, beta, beta == 0.0, blocksize, k, tile_k, values, crow_indices, col_indices, mat1, mat2, max_grid
        )
    ...
    def bsr_dense_mm(
        ...
        *,
        ...
        if not skip_checks:
            ...
            check(
                not n % row_block,
                f"bsr_dense_mm(): dense.size(-1) == {n} should be divisible by " f"blocksize[0] == {row_block}.",
            )
        ...
        if enable_bsr_scatter_mm and max(blocksize) == 16 and bsr.dense_dim() == 0 and bsr.ndim == 2:
            ...
            if (
                ...
                and ((m >= 4096 and n >= 8192) or (m == 2048 and n >= 32768) or (n >= 131072))
            ) or (dtype == torch.float32 and (m >= 1024 or (m == 512 and n >= 512) or (m == 256 and n >= 2048))):
    ...
    def bsr_softmax(input, max_row_nnz=None):
        ...
        if input.values().transpose(-3, -2).is_contiguous():
            ...
            values = input.values()
        values = (
            values.transpose(-3, -2).contiguous().unsqueeze(0).flatten(0, -4).reshape(-1, row_block, nnz * col_block)
        )
        ...
        return torch.sparse_compressed_tensor(
            input.crow_indices().clone(), input.col_indices().clone(), values, size=input.shape, layout=input.layout
        )
