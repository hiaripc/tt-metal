...
def get_meta(op, key, device_name=None, version=(0, torch.float16, 0.5), exact=False):
    ...
    if values is not None:
        if op == "scatter_mm":
            ...
            return dict(zip(("GROUP_SIZE_ROW", "SPLIT_N", "num_stages", "num_warps"), values))
...
def dump():
    ...
    if begin_data_index == -1 or end_data_index == -1:
        warnings.warn(
            f"{current_file} cannot be updated:" " BEGIN/END GENERATED DATA comment blocks appear to be corrupted"
        )
        return
...
def minimize(target_func, initial_parameters, reference_parameters, step_func, max_step=2):
    ...
    while True:
        ...
        if new_minimizer:
            ...
            minimizer_keys = {
                k for k, v in all_values.items() if isinstance(v, float) and abs(1 - v / minimizer_target) < 0.001
            }
            ...
            for _, rel_diffs, name in sorted(sensitivity, reverse=True):
                left_diffs = "|".join([f"{rel_diff:.1f}" for rel_diff in rel_diffs[:max_step]])
                right_diffs = "|".join([f"{rel_diff:.1f}" for rel_diff in rel_diffs[max_step:]])
                sensitivity_message.append(f"{name}={parameters[name]} ({left_diffs}...{right_diffs} %)")
        ...
        try:
            for (M, K), N, (BM, BK) in itertools.product(shapes_lst, sizes_lst, blocksize_lst):
                if op == "scatter_mm":
                    ...
                    optimize_bsr_dense_addmm(M, K, N, BM, BK, force=force, sparsity=sparsity, dtype=dtype)
                else:
