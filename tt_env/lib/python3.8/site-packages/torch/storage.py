...
class _StorageBase:
    ...
    def __len__(self) -> int:
        ...  # type: ignore[empty-body] # noqa: E704
    ...
    def element_size(self) -> int:
        ...  # type: ignore[empty-body, type-var] # noqa: E704
    ...
    def _get_filename(self, *args, **kwargs) -> _Optional[str]:
        ...  # type: ignore[empty-body, misc] # noqa: E704

    def __str__(self):
        info_str = f"[{torch.typename(self)}(device={self.device}) " f"of size {len(self)}]"
    ...
    def _to(self, dtype):
        ...
        storage = (
            torch.tensor([], dtype=torch.uint8, device=self.device).set_(cast(Storage, self)).to(dtype)._typed_storage()
        )
    ...
    def pin_memory(self, device: Union[str, torch.device] = "cuda"):
        ...
        pinned_tensor = (
            torch.tensor([], dtype=torch.uint8, device=self.device).set_(cast(Storage, self)).pin_memory(device)
        )
...
class TypedStorage:
    ...
    def __new__(cls, *args, wrap_storage=None, dtype=None, device=None, _internal=False):
        ...
        if cls == TypedStorage:
            ...
            if wrap_storage is None:
                ...
                return TypedStorage(
                    *args, dtype=cls._dtype, device=_get_device_from_module(cls.__module__), _internal=True
                )
    ...
    def _getitem(self, idx):
        ...
        if self.dtype in [torch.quint8, torch.quint4x2, torch.quint2x4, torch.qint32, torch.qint8]:
            ...
            return TypedStorage(
                wrap_storage=self._untyped_storage, dtype=interpret_dtypes[self.dtype], _internal=True
            )._getitem(idx)
    ...
    def __str__(self):
        _warn_typed_storage_removal()
        info_str = f"[{torch.typename(self)}(dtype={self.dtype}, " f"device={self.device}) of size {len(self)}]"
        if self.device.type == "meta":
            return "...\n" + info_str
        else:
            data_str = " " + "\n ".join(str(self[i]) for i in range(self.size()))
...
def _get_dtype_from_pickle_storage_type(pickle_storage_type: str):
    try:
        ...
        raise KeyError(f'pickle storage type "{pickle_storage_type}" is not recognized') from e
