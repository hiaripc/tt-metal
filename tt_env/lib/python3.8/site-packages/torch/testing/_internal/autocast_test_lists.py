...
class AutocastTestLists:
    def _rnn_cell_args(self, n, num_chunks, is_lstm, dev, dtype):
        ...
        hx = (
            (torch.randn((n, n), device=dev, dtype=torch.float32), torch.randn((n, n), device=dev, dtype=torch.float32))
            if is_lstm
            else torch.randn((n, n), device=dev, dtype=torch.float32),
        )

        weights = (
            torch.randn((num_chunks * n, n), device=dev, dtype=torch.float32),  # weight_ih
    ...
    def __init__(self, dev):
        ...
        conv_args_fp32 = [
            (torch.randn(dimset, dtype=torch.float32, device=dev), torch.randn(dimset, dtype=torch.float32, device=dev))
        ...
        self.torch_fp16 = [
            ...
            (
                "_convolution",
                conv_args_fp32[1] + bias_fp32 + ((1, 1), (0, 0), (1, 1), False, (0, 0), 1, False, True, True),
            ),
            # the current  _convolution
            (
                "_convolution",
                conv_args_fp32[1] + bias_fp32 + ((1, 1), (0, 0), (1, 1), False, (0, 0), 1, False, True, True, True),
            ),
            ...
            (
                ...
                conv_args_fp32[1] + ((0, 0), (0, 0), (1, 1), (1, 1), 1, False, True, True),
                TEST_WITH_ROCM,
            ),
        ...
        self.torch_fp32 = [
            ...
            ("poisson_nll_loss", mat0_fp16 + mat1_fp16 + (True, False, 1.0e-8, torch.nn._reduction.get_enum("mean"))),
            (
        ...
        self.torch_need_autocast_promote = [
            ...
            (
                "cross",
                (torch.randn(3, dtype=torch.float32, device=dev), torch.randn(3, dtype=torch.float16, device=dev)),
            ),
            ...
            (
                "index_put",
                pointwise0_fp32
                + ((torch.tensor([1], device=dev, dtype=torch.long),), torch.randn(1, device=dev, dtype=torch.float16)),
            ),
            (
                "index_put",
                pointwise0_fp16
                + ((torch.tensor([1], device=dev, dtype=torch.long),), torch.randn(1, device=dev, dtype=torch.float32)),
            ),
            (
                "tensordot",
                (
        ...
        self.nn_fp32 = [
            ...
            (
                "nll_loss",
                (torch.rand((n, n), device=dev, dtype=torch.float), torch.zeros((n,), device=dev, dtype=torch.long)),
            ),
            (
                "nll_loss2d",
                (
...
class AutocastCPUTestLists:
    ...
    def __init__(self, dev):
        ...
        self.torch_fp32 = [
            ("poisson_nll_loss", mat0_bf16 + mat1_bf16 + (True, False, 1.0e-8, torch.nn._reduction.get_enum("mean"))),
            (
                "cosine_embedding_loss",
                (
        ...
        self.nn_fp32 = [
            ...
            (
                "nll_loss",
                (torch.rand((n, n), device=dev, dtype=torch.bfloat16), torch.zeros((n,), device=dev, dtype=torch.long)),
            ),
            (
                "nll_loss2d",
                (
