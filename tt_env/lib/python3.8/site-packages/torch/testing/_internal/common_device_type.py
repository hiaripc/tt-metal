    ...
    def instantiate_test(cls, name, test, *, generic_cls=None):
        ...
        for test, test_suffix, param_kwargs, decorator_fn in parametrize_fn(test, generic_cls, cls):  # noqa: B020
            ...
            instantiate_test_helper(
                cls=cls, name=test_name, test=test, param_kwargs=param_kwargs, decorator_fn=decorator_fn
            )
...
def instantiate_device_type_tests(
    generic_test_class, scope, except_for=None, only_for=None, include_lazy=False, allow_mps=False
):
...
class ops(_TestParametrizer):
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
            ...
            for dtype in dtypes:
                ...
                try:
                    ...
                    decorator_fn = partial(
                        op.get_decorators, generic_cls.__name__, test.__name__, device_cls.device_type, dtype
                    )
        ...
        if op is check_exhausted_iterator:
            raise ValueError(
                "An empty op_list was passed to @ops. " "Note that this may result from reuse of a generator."
            )
...
def skipCUDAIfNotMiopenSuggestNHWC(fn):
    return skipCUDAIf(not TEST_WITH_MIOPEN_SUGGEST_NHWC, "test doesn't currently work without MIOpen NHWC activation")(
        fn
    )
