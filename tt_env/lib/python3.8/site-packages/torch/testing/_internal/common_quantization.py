...
def _make_conv_add_extra_input_tensor(scale, zero_point, sizes):
    (X_value_min, X_value_max) = (0, 4)
    X_init = torch.randint(X_value_min, X_value_max, sizes)  # Infer the size of tensor to do the add
...
class QuantizationTestCase(TestCase):
    def setUp(self):
        ...
        self.train_data = [
            [torch.rand(2, 5, dtype=torch.float), torch.randint(0, 1, (2,), dtype=torch.long)] for _ in range(2)
        ]
        ...
        self.img_data_1d_train = [
            [torch.rand(2, 3, 10, dtype=torch.float), torch.randint(0, 1, (1,), dtype=torch.long)] for _ in range(2)
        ]
        self.img_data_2d_train = [
            [torch.rand(1, 3, 10, 10, dtype=torch.float), torch.randint(0, 1, (1,), dtype=torch.long)] for _ in range(2)
        ]
        self.img_data_3d_train = [
            [torch.rand(1, 3, 5, 5, 5, dtype=torch.float), torch.randint(0, 1, (1,), dtype=torch.long)]
            for _ in range(2)
        ]
    ...
    def checkObservers(self, module, propagate_qconfig_list=None, prepare_custom_config_dict=None):
        ...
        if (
            ...
            self.assertTrue(
                hasattr(module, "activation_post_process"), "module: " + str(type(module)) + " do not have observer"
            )
        ...
        for debug in [True, False]:
            if dynamic:
                ...
                models[debug] = quantize_jit(
                    model, qconfig_dict, test_only_eval_fn, [inputs_copy], inplace=False, debug=debug
                )
                # make sure it runs
    ...
    def checkGraphModuleNodes(
        self, graph_module, expected_node=None, expected_node_occurrence=None, expected_node_list=None
    ):
        ...
        if expected_node is not None:
            self.assertTrue(
                expected_node in nodes_in_graph, "node:" + str(expected_node) + " not found in the graph module"
            )
        ...
        if expected_node_occurrence is not None:
            for expected_node, occurrence in expected_node_occurrence.items():
                if occurrence != 0:
                    self.assertTrue(
                        expected_node in nodes_in_graph, "Check failed for node:" + str(expected_node) + " not found"
                    )
                    self.assertTrue(
                    ...
                    self.assertTrue(
                        ...
                        "Check failed for node:" + str(expected_node) + " expected no occurrence but found",
                    )
    ...
    if HAS_FX:
        ...
        def assert_ns_compare_dict_valid(
            ...
            for layer_name, result_type_to_data in act_compare_dict.items():
                for result_type, layer_data in result_type_to_data.items():
                    self.assertTrue(
                        len(layer_data) == 2, f"Layer {layer_name} does not have exactly two model results."
                    )
                    ...
                    for res_idx in range(len(layer_data[model_name_0])):
                        ...
                        is_weight_functional_conv1d = result_type == NSSingleResultValuesType.WEIGHT.value and (
        ...
        def checkGraphModeFxOp(
            ...
            self.checkGraphModuleNodes(
                prepared, prepare_expected_node, prepare_expected_node_occurrence, prepare_expected_node_list
            )
    ...
    def checkEmbeddingSerialization(
        self, qemb, num_embeddings, embedding_dim, indices, offsets, set_qconfig, is_emb_bag, dtype=torch.quint8
    ):
...
class AnnotatedCustomConfigNestedModel(torch.nn.Module):
    def __init__(self):
        ...
        custom_qconfig = QConfig(
            activation=default_observer.with_args(**custom_options), weight=default_weight_observer
        )
...
class EmbeddingBagModule(torch.nn.Module):
    def __init__(self):
        ...
        self.emb = torch.nn.EmbeddingBag(
            num_embeddings=10, embedding_dim=12, include_last_offset=True, scale_grad_by_freq=False, mode="sum"
        )
...
class SparseNNModel(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.dense_top = DenseTopMLP(
            self._DENSE_DIM, self._DENSE_OUTPUT, self._EMBEDDING_DIM, self._TOP_OUT_IN, self._TOP_OUT_OUT
        )
