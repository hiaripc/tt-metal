...
subclass_db = {
    ...
    NonWrapperTensor: SubclassInfo("non_wrapper_tensor", create_fn=lambda shape: NonWrapperTensor(torch.randn(shape))),
    LoggingTensor: SubclassInfo("logging_tensor", create_fn=lambda shape: LoggingTensor(torch.randn(shape))),
    SparseTensor: SubclassInfo(
        "sparse_tensor", create_fn=lambda shape: SparseTensor.from_dense(torch.randn(shape).relu())
    ),
