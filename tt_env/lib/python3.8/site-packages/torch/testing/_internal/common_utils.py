...
class TestEnvironment:
    ...
    def def_flag(
        ...
        enabled_fn=lambda env_var_val, default: ((env_var_val != "0") if default else (env_var_val == "1")),
...
TestEnvironment.def_flag(
    "IS_CI", env_var="CI", include_in_repro=False, enabled_fn=lambda env_var_value, _: bool(env_var_value)
)
TestEnvironment.def_flag(
    "IS_SANDCASTLE",
    env_var="SANDCASTLE",
...
class TrackedInputIter:
    ...
    def __next__(self):
        ...
        self._set_tracked_input(
            TrackedInput(index=input_idx, val=self.callback(input_val), type_desc=self.input_type_desc)
        )
        return input_val
...
def compose_parametrize_fns(old_parametrize_fn, new_parametrize_fn):
    ...
    def composite_fn(
        ...
        for old_test, old_test_name, old_param_kwargs, old_dec_fn in old_tests:
            for new_test, new_test_name, new_param_kwargs, new_dec_fn in new_parametrize_fn(
                old_test, generic_cls, device_cls
            ):
...
parser.add_argument(
    "--save-xml", nargs="?", type=str, const=_get_test_report_path(), default=_get_test_report_path() if IS_CI else None
)
...
def run_tests(argv=UNITTEST_ARGS):
    ...
    if TEST_IN_SUBPROCESS:
        ...
        test_cases = (
            ...
            if USE_PYTEST
            else [case.id().split(".", 1)[1] for case in discover_test_cases_recursively(suite)]
        )
        ...
        unittest.main(
            ...
            testRunner=xmlrunner.XMLTestRunner(
                output=test_report_path, verbosity=2 if verbose else 1, resultclass=XMLTestResultVerbose
            ),
        )
    elif REPEAT_COUNT > 1:
...
TEST_CUDA_GRAPH = (
    ...
    and (
        ...
        or (torch.version.hip and float(".".join(torch.version.hip.split(".")[0:2])) >= 5.3)
    )
)
...
def check_if_enable(test: unittest.TestCase):
    ...
    if not IS_SANDCASTLE:
        ...
        if not should_skip and RERUN_DISABLED_TESTS:
            skip_msg = (
                "Test is enabled but --rerun-disabled-tests verification mode is set, so only" " disabled tests are run"
            )
            raise unittest.SkipTest(skip_msg)

    if TEST_SKIP_FAST:
        if hasattr(test, test._testMethodName) and not getattr(test, test._testMethodName).__dict__.get(
            "slow_test", False
        ):
...
This message can be suppressed by setting PYTORCH_PRINT_REPRO_ON_FAILURE=0"""
                    self.wrap_with_policy(
                        method_name, lambda repro_str=repro_str: print_repro_on_failure(repro_str=repro_str)
                    )
                except Exception as e:
    ...
    def _run_with_retry(self, result=None, num_runs_left=0, report_only=True, num_red=0, num_green=0):
        ...
        if failures_before < len(result.failures):
            ...
            self._run_with_retry(
                result=result, num_runs_left=0, report_only=report_only, num_red=num_red, num_green=num_green + 1
            )
    ...
    def run(self, result=None):
        with contextlib.ExitStack() as stack:
            ...
            self._run_with_retry(
                result=result, num_runs_left=num_runs, report_only=not OVERRIDE_FLAKY_SIGNAL, num_red=0, num_green=0
            )
    ...
    def genSparseCompressedTensor(self, size, nnz, *, layout, device, dtype, index_dtype, blocksize=(), dense_dims=0):
        ...
        return torch.sparse_compressed_tensor(
            compressed_indices, plain_indices, values, size=size, dtype=dtype, layout=layout, device=device
        )
    ...
    def generate_simple_inputs(
        ...
        def get_blockpattern(pattern, blocksize):
            ...
            blockpattern = (
                pattern.reshape(-1, blocksize[0], basesize[1] // blocksize[1], blocksize[1])
                .transpose(-3, -2)
                .any(-1)
                .any(-1)
            )
        ...
        def get_sparse_data_with_block(pattern, blocksize):
            ...
            bsr_values = torch.stack(
                [
                    strided_values[
                        bi * blocksize[0] : (bi + 1) * blocksize[0], bj * blocksize[1] : (bj + 1) * blocksize[1]
                    ]
        ...
        if patterns is None:
            ...
            patterns = [
                ...
                (
                    [
                        [[[1, 2, 0], [1, 0, 3]], [[1, 2, 3], [1, 0, 0]], [[1, 0, 0], [1, 2, 3]]],
                        [[[0, 2, 0], [1, 2, 3]], [[1, 0, 3], [1, 2, 0]], [[1, 2, 3], [0, 2, 0]]],
                    ],
                    [(2, 1), (2, 3)],
                    [(), (2,)],
                ),
                ...
                ([[[1, 2], [3, 4]], [[1, 0], [0, 0]]], [(1, 1)], ([()] if enable_batch_variable_nse else [])),
            ]
        ...
        def non_contiguous_copy(t, dim=-1, offset=0):
            ...
            step = max(2, offset + 1)
            tmp = torch.zeros(
                (*t.shape[:dim], t.shape[dim] * step, *t.shape[dim + 1 :]), dtype=t.dtype, device=t.device
            )
                    ...
                    for densesize in densesizes:
                        if layout == torch.strided:
                            ...
                            crow_indices = torch.tensor(
                                [0] * (basesize[0] // blocksize[0] + 1), device=device, dtype=index_dtype
                            )
                            col_indices = torch.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)
                            ...
                            ccol_indices = torch.tensor(
                                [0] * (basesize[1] // blocksize[1] + 1), device=device, dtype=index_dtype
                            )
                            row_indices = torch.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)
                            values = torch.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
    ...
    def assertExpected(self, s, subname=None):
        ...
        expected = expected.replace(
            'producer_version: "CURRENT_VERSION"', f'producer_version: "{torch.onnx.producer_version}"'
        )
        ...
        with DeterministicGuard(True):
            if should_alert:
                with self.assertRaisesRegex(
                    RuntimeError, alert_message, msg="expected a non-deterministic error, but it was not raised"
                ):
                    fn()
                ...
                try:
                    ...
                    if "does not have a deterministic implementation" in str(e):
                        self.fail(
                            "did not expect non-deterministic error message, " + 'but got one anyway: "' + str(e) + '"'
                        )
...
skipIfSlowGradcheckEnv = unittest.skipIf(
    TEST_WITH_SLOW_GRADCHECK, "Tests that don't use gradcheck don't need to run on slow_gradcheck CI"
)
