...
class Trainer:
    def __init__(
        ...
        gLogger.info(
            "Succeeded in creating a HybridModel instance with " "%s ddp params and %s other local params.",
            len(self.ddp_params),
    ...
    def train_batch(
        ...
        if not simulate_uneven_inputs:
            ...
            if trainer_has_less_inputs:
                ...
                gLogger.info(
                    "Trainer reduced input patches from %s " "to %s to simulate uneven inputs.",
                    len(batches),
                    len(input_batches),
                )
            else:
...
def get_training_examples():
    ...
    return [
        FeatureSet(
            dense_features=training_examples.dense_features[start : start + examples_per_trainer, :],
            sparse_features=training_examples.sparse_features[start : start + examples_per_trainer],
...
class CudaDdpComparisonTest(CommonDdpComparisonTest):
    ...
    def test_ddp_dist_autograd_local_vs_remote_gpu(self):
        ...
        with dist_autograd.context() as context_id:
            loss = ddp_layer4(
                remote_layer3(ddp_layer2(remote_layer1(inputs).cuda(self.rank)).cpu()).cuda(self.rank)
            ).sum()
            dist_autograd.backward(context_id, [loss])
            grads_dict = dist_autograd.get_gradients(context_id)
            dist.barrier()
