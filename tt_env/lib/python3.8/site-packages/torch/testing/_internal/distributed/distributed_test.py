...
skipIfNoTorchVision = skip_but_pass_in_sandcastle_if(not HAS_TORCHVISION, "no torchvision")
...
class DistributedTest:
    class _DistTestBase:
        ...
        @skip_but_pass_in_sandcastle_if(
            ...
            "Requires file:// initialization method. " + "Both tcp:// and env:// rely on the TCP store for which "
        ...
        def test_3_level_hierarchical_model_averager(self):
            ...
            expect_group_ranks_res1 = (
                rank // subgroup_size1 * subgroup_size1 + np.array(list(range(subgroup_size1)))
            ).tolist()
            expect_group_ranks_res2 = (
                rank // subgroup_size2 * subgroup_size2 + np.array(list(range(subgroup_size2)))
            ).tolist()
        ...
        def _test_reduce_twice_helper(
            ...
            for src in group:
                tensors = [
                    _build_tensor(src + 1).fill_(master_value if rank == src else worker_value) for i in range(2)
                ]
                if cuda:
                    for i in range(2):
        ...
        def test_reduce_scatter_v_cuda(self):
            ...
            for async_val in [True, False]:
                ...
                out_tensor = (
                    torch.empty(input_split_sizes[rank], sum_len, sum_len, dtype=torch.float).fill_(-1).cuda(device_id)
                )
        ...
        def test_all_gather_v_cuda(self):
            ...
            for async_val in [True, False]:
                tensor = (
                    torch.empty(output_split_sizes[rank], sum_len, sum_len, dtype=torch.float)
                    .fill_(value)
                    .cuda(device_id)
                )
        ...
        def _test_all_gather_coalesced_helper(self, group, group_id, rank, dtype=torch.float):
            ...
            if group_id is not None:
                for test_case_id in range(2, 5):
                    ...
                    input_tensors = [
                        _build_multidim_tensor(tensor_id, tensor_id, rank + tensor_id, dtype=dtype)
                        for tensor_id in range(1, test_case_id)
                    ]
                    output_tensor_lists = [
                        [
                            _build_multidim_tensor(tensor_id, tensor_id, -1, dtype=dtype)
                            for tensor_id in range(1, test_case_id)
                        ]
                        for _ in group
                    ]
                    expected_tensors = [
                        [
                            _build_multidim_tensor(tensor_id, tensor_id, rank_iter + tensor_id, dtype=dtype)
                            for tensor_id in range(1, test_case_id)
                        ]
            ...
            for i in range(6):
                ...
                for n, param in net.named_parameters():
                    ...
                    if param.grad is None:
                        ...
                        tensor_list = [
                            torch.zeros_like(param.grad) for _ in range(dist.get_world_size(net.process_group))
                        ]
        ...
        def test_nccl_backend_bool_broadcast(self):
            ...
            bcast_tensor = torch.tensor(
                [(random.random() < 0.5 if self.rank == 0 else False) for _ in range(tensor_size)]
            ).to(self.rank)
            dist.broadcast(bcast_tensor, src=0)
            # Now allgather and ensure the tensors are equal.
            tensor_list = [
                torch.tensor([False for _ in range(tensor_size)]).to(self.rank) for _ in range(dist.get_world_size())
            ]
                        ...
                        mapping.update(
                            {
                                ...
                                for rank in range(num_early_join_ranks, dist.get_world_size())
                            }
                        )
        ...
        def _test_ddp_ignore_params_arg(self, static_graph=False):
            ...
            for find_unused, broadcast_buffers in itertools.product([False, True], [False, True]):
                ...
                model_fc2_name = next(
                    module_name for module_name, module in model.named_modules() if module is model.fc2
                )
        ...
        def test_scatter_object_list(self):
            src_rank = 0
            scatter_list = (
                COLLECTIVES_OBJECT_TEST_LIST if self.rank == src_rank else [None for _ in COLLECTIVES_OBJECT_TEST_LIST]
            )
            world_size = dist.get_world_size()
        ...
        def _test_ddp_buffer_hook_allreduce(self, return_futures):
            ...
            def buffer_comm_hook(ddp, named_buffers):
                ...
                futs = [
                    dist.all_reduce(buffer, group=ddp.process_group, async_op=True).get_future() for buffer in buffers
                ]
                if return_futures:
                    return futs
                else:
