...
class TensorPipeAgentDistAutogradTest(CommonDistAutogradTest):
    ...
    def test_backward_no_grad_on_tensor_sparse(self):
        self._backward_no_grad_on_tensor(
            build_sparse_tensor(requires_grad=True), build_sparse_tensor(requires_grad=True), True
        )
    ...
    def test_backward_simple_self_sparse(self):
        self._backward_simple(
            self.rank, build_sparse_tensor(requires_grad=True), build_sparse_tensor(requires_grad=True), None, True
        )
    ...
    def test_mixed_requires_grad_sparse(self):
        self._mixed_requires_grad(
            build_sparse_tensor(requires_grad=True), build_sparse_tensor(requires_grad=False), True
        )
...
class DistAutogradTest(CommonDistAutogradTest):
    ...
    def test_backward_no_grad_on_tensor(self):
        self._backward_no_grad_on_tensor(
            torch.rand((3, 3), requires_grad=True), torch.rand((3, 3), requires_grad=True), False
        )
    ...
    def test_backward_simple_self(self):
        self._backward_simple(
            self.rank, torch.rand((3, 3), requires_grad=True), torch.rand((3, 3), requires_grad=True), None, False
        )
    ...
    def test_multiple_backward_with_errors(self):
        ...
        with dist_autograd.context() as context_id:
            loss = rpc.rpc_sync(
                f"worker{self._next_rank()}", DistAutogradTest._python_udf_with_backward_error, args=(t1, t2)
            ).sum()
