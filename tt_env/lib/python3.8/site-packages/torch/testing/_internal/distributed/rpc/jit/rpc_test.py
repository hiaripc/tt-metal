...
class LocalRRefTest:
    ...
    def test_create_local_script_module_rref_in_py(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            ("The RRef being created contains a ScriptModule, " "must provide its ModuleInterface type hint."),
        ):
...
class JitRpcTest(
    ...
    def test_rpc_async_jit_profiled(self):
        ...
        if self.rank == 0:
            ...
            rpc_async_jit_event = [
                event for event in function_events if qual_name in event.name and event.node_id == self.rank
            ]
        ...
        if self.rank == 0:
            ...
            expected_key = (
                _build_rpc_profiling_key(
                    RPCExecMode.ASYNC_JIT,
                    torch._jit_internal._qualified_name(script_add_ones_with_record_function),
                    worker_name(self.rank),
                    dst_worker_name,
                )
                + REMOTE_OP_STR
