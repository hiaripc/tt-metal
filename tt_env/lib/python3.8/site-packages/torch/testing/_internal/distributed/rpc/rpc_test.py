...
def udf_with_torch_ops(device=-1, use_record_function=False):
    ...
    record_function_ctx = (
        torch.autograd.profiler.record_function("##forward##") if use_record_function else contextlib.nullcontext()
    )
...
class StubRpcAgent:
    ...
    def get_worker_infos(self):
        return {WorkerInfo(name=worker_name(rank), id=rank) for rank in range(self.world_size)}
...
def nested_rpc_sparse(dst):
    return rpc.rpc_sync(dst, torch.add, args=(build_sparse_tensor(), build_sparse_tensor()))
...
def async_add_with_future_ctor(to, x, y, z):
    ...
    rpc.rpc_async(to, torch.add, args=(x, y)).then(lambda fut1: fut.set_result(fut1.wait() + z))
    return fut
...
def async_add_nested(to, x, y, z):
    return rpc.rpc_async(to, async_add, args=(to, x, y)).then(lambda fut: fut.wait() + z)
...
class RpcTest(RpcAgentTestFixture, RpcTestCommon):
    ...
    def _test_rref_proxy_class(self, dst):
        ...
        self.assertEqual(
            expected.my_class_method(2, torch.zeros(2, 2)), rref.rpc_sync().my_class_method(2, torch.zeros(2, 2))
        )
        self.assertEqual(
            expected.my_class_method(2, torch.ones(3, 3)), rref.rpc_async().my_class_method(2, torch.ones(3, 3)).wait()
        )
        self.assertEqual(
            expected.my_class_method(2, torch.ones(4, 4)), rref.remote().my_class_method(2, torch.ones(4, 4)).to_here()
        )
    ...
    def test_duplicate_name(self):
        with self.assertRaisesRegex(RuntimeError, "is not unique"):
            store, _, _ = next(
                torch.distributed.rendezvous(self.init_method, rank=self.rank, world_size=self.world_size)
            )
    ...
    def test_pg_init_no_rpc_init(self):
        dist.init_process_group(
            backend="gloo", init_method=self.file_init_method, rank=self.rank, world_size=self.world_size
        )
    ...
    def _run_test_profiler_with_sync_rpc_builtin(self):
        ...
        self._profiler_test_with_rpc(
            RPCExecMode.SYNC, torch.mul, args=(torch.ones(1), torch.ones(1)), use_record_function=True
        )
    ...
    def _run_test_profiler_with_async_rpc_builtin(self):
        ...
        self._profiler_test_with_rpc(
            RPCExecMode.ASYNC, torch.mul, args=(torch.ones(1), torch.ones(1)), use_record_function=True
        )
    ...
    def _run_test_profiler_with_remote_builtin(self):
        ...
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE, torch.mul, args=(torch.ones(1), torch.ones(1)), use_record_function=True
        )
        # test remote to self
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE,
...
class TensorPipeAgentRpcTest(RpcAgentTestFixture, RpcTestCommon):
    ...
    def test_self_py_udf_remote_sparse(self):
        self._self_py_udf_remote(
            rpc.get_worker_info(), build_sparse_tensor(), build_sparse_tensor(), build_sparse_tensor()
        )
...
class TensorPipeAgentCudaRpcTest(RpcAgentTestFixture, RpcTestCommon):
    ...
    def test_device_maps_invalid_max_local_device(self):
        ...
        self._test_device_maps(
            options, errMsg="Node worker0 has source devices with invalid indices in its device map for worker1"
        )
    ...
    def test_device_maps_invalid_max_remote_device(self):
        ...
        self._test_device_maps(
            options, errMsg="Node worker0 has target devices with invalid indices in its device map for worker1"
        )
    ...
    def test_device_maps_many_to_one(self):
        ...
        self._test_device_maps(
            options, errMsg="Node worker0 has duplicated target devices in its device map for worker1"
        )
    ...
    def test_device_maps_in_options(self):
        ...
        rets = rpc.rpc_sync(
            dst, TensorPipeAgentCudaRpcTest._gpu_add_multi_gpu, args=(torch.zeros(2).to(0), torch.ones(2).to(1))
        )
        self.assertEqual(rets[0].device, torch.device(1))
        self.assertEqual(rets[1].device, torch.device(0))
        self.assertEqual(rets[0], (torch.zeros(2) + torch.ones(2)).to(1))
    ...
    def _test_device_maps_return_to_gpu(self, dst):
        ...
        rets = rpc.rpc_sync(
            dst, TensorPipeAgentCudaRpcTest._gpu_add_return_to_gpu, args=(torch.zeros(2), torch.ones(2))
        )
    ...
    def _test_stream_nested_multi_async(self, dst):
        if self.rank == 0:
            ...
            for i in range(n):
                ...
                futs.append(
                    rpc.rpc_async(
                        dst, TensorPipeAgentCudaRpcTest._nested_slow_add_on_user_stream, args=(nested_dst, x, y, z)
                    )
                )
    ...
    def test_devices_option_mismatch(self):
        with self.assertRaisesRegex(
            ValueError, "Node worker0 has unexpected source devices in its device map for worker1"
        ):
            dst = worker_name((self.rank + 1) % self.world_size)
            options = self.rpc_backend_options
            options.set_device_map(dst, {0: 0})
    ...
    def test_devices_option_mismatch_reverse(self):
        with self.assertRaisesRegex(
            ValueError, "Node worker0 has unexpected target devices in its device map for worker1"
        ):
            dst = worker_name((self.rank + 1) % self.world_size)
