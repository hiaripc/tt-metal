...
def _floats_wrapper(*args, **kwargs):
    if "width" in kwargs and hypothesis.version.__version_info__ < (3, 67, 0):
        ...
        no_nan_and_inf = (("allow_nan" in kwargs and not kwargs["allow_nan"]) or "allow_nan" not in kwargs) and (
            ("allow_infinity" in kwargs and not kwargs["allow_infinity"]) or "allow_infinity" not in kwargs
        )
...
def tensor_conv(
    ...
    X = draw(
        tensor(shapes=((batch_size, input_channels) + tuple(feature_map_shape),), elements=elements, qparams=qparams[0])
    )
