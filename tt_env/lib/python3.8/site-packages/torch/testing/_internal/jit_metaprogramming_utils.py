...
def create_input(call_args, requires_grad=True, non_contiguous=False, call_kwargs=None, dtype=torch.float, device=None):
    ...
    def map_arg(arg):
        ...
        if isinstance(arg, (torch.Size, dont_convert)):
            ...
            return conjugate(maybe_non_contig(torch.randn(*arg, dtype=dtype, device=device))).requires_grad_(
                requires_grad
            )
...
nn_functional_tests = [
    ...
    ("max_unpool2d", torch.tensor([[[[2.0, 4]]]]), (torch.tensor([[[[1, 3]]]]), 2, 2, 0)),
    ("max_unpool3d", torch.tensor([[[[[2.0, 4]]]]]), (torch.tensor([[[[[1, 3]]]]]), 2, 2, 0)),
    (
        "lp_pool1d",
        (S, S, S),
        (
    ...
    ("layer_norm", (S, S, S, S), ([5],), "", (False, ["aten::contiguous", "aten::_batch_norm_impl_index"])),
    (
        "layer_norm",
        (S, S, S, S),
        (
    ...
    ("gumbel_softmax", (S, S), (2.0,), "", (True, ["aten::softmax", "aten::add", "aten::div"], ["aten::neg"])),
    (
        "gumbel_softmax",
        (S, S),
        (
    ...
    (
        "multi_margin_loss",
        (S, S),
        (non_differentiable(torch.randint(S, (S,), dtype=torch.int64)), 1, 1.0, non_differentiable(torch.randn(S))),
    ),
    (
    ...
    (
        "binary_cross_entropy",
        torch.randn(3, 2).sigmoid(),
        (non_differentiable(torch.rand(3, 2)), non_differentiable(torch.randn(3, 2)), None, None, "mean"),
        "size_average",
    ),
    (
        "ctc_loss",
...
additional_module_tests = [
    {"module_name": "Bilinear", "constructor_args": (S, S, M), "input_size": (S, S), "extra_args": ((S, S),)},
    {
        "module_name": "RNNCell",
