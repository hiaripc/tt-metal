...
def get_execution_plan(graph_executor_state):
    ...
    if num_plans != 1:
        raise RuntimeError(
            "This test assumes this GraphExecutor should " f"only have one execution plan, got: {num_plans}"
        )
...
class JitTestCase(JitCommonTestCase):
        ...
        def get_nodes_and_parents_recursively(block, kind, acc):
            for node in block.nodes():
                if node.kind() == kind:
                    ...
                    or node.inputs().__next__().node().kind() == "prim::RequiresGradCheck"
                ):
    ...
    def checkTrace(
        ...
        ge = torch.jit.trace(
            func, input_tensors, check_tolerance=check_tolerance, _force_outplace=_force_outplace, check_trace=False
        )
