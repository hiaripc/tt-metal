...
methods of construction cannot be mixed!"""
            ...
            assert (
                output_process_fn_grad is None and broadcasts_input is None and name is None
            ), """
...
class OpInfo:
    ...
    def __post_init__(self):
        ...
        self.dynamic_dtypes = any(isinstance(dtypes, utils._dynamic_dispatch_dtypes) for dtypes in dtypes_args)
        ...
        if self.supports_inplace_autograd is None:
            ...
            assert not (
                self.supports_inplace_autograd and not self.supports_autograd and not self.supports_forward_ad
            ), (
...
def _reference_inputs_elementwise_unary(op, device, dtype, requires_grad, **kwargs):
    ...
    if dtype not in (torch.bool, torch.uint8, torch.int8) and (
        op.handles_large_floats or (not dtype.is_floating_point and not dtype.is_complex)
    ):
