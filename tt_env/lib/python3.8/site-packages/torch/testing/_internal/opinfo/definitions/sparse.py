...
def _check_validate(op_info, sample):
    ...
    def _check_success(sample):
        try:
            ...
            raise AssertionError(  # noqa: TRY200
                f"{op_info.name} on {sample=} expected to succeed " f", got {type(msg).__name__}: {msg}"
            )
...
def sample_inputs_sparse_reduction(op_info, device, dtype, requires_grad, layout, blocksize=None, **kwargs):
    ...
    for sample_input in sample_inputs_reduction(op_info, device, dtype, requires_grad, **kwargs):
        ...
        yield SampleInput(
            sample_input.input.detach().to_sparse(layout=layout, blocksize=blocksize).requires_grad_(requires_grad),
...
def _validate_sample_input_sparse_reduction(op_info, sample, check_validate=False):
    ...
    if op_info.name in {"masked.sum"}:
        ...
        if mask not in {None, UNSPECIFIED} and mask.ndim > 2 and mask.layout is torch.strided and (mask == 0).any():
            # TODO: remove this if-block after gh-98495 is fixed.
            sample = ErrorInput(
                sample,
    ...
    if op_info.name in {"masked.amax", "masked.amin", "masked.mean", "masked.prod"}:
        ...
        if mask is not None and mask.ndim > 2 and mask.layout is torch.strided and (mask == 0).any():
            # TODO: remove this if-block after gh-98495 is fixed.
            sample = ErrorInput(
                sample,
                error_regex="Expect the same number of specified elements per batch.",
        ...
        elif mask.layout is sample.input.layout and mask.ndim > 2 and op_info.name == "masked.mean":
            sample = ErrorInput(
                sample,
                error_type=TypeError,
                error_regex=(
                    "where[(][)] received an invalid combination of arguments" " - got [(]Tensor, Tensor, NoneType[)]"
                ),
            )
        elif not sample.kwargs.get("keepdim"):
            sample = ErrorInput(
        ...
        elif mask.layout is t_inp.layout and mask._nnz() != t_inp._nnz() and t_inp.dense_dim() > 0:
            sample = ErrorInput(
                sample,
    ...
    if isinstance(dim, (int, list, tuple)):
        if layout in {
            ...
            if layout in {torch.sparse_csr, torch.sparse_csc} and not keepdim:
                return ErrorInput(
                    sample,
                    error_regex=("reduction operations on CSR tensors with keepdim=False is unsupported"),
                )
            if t_inp.dim() != 2:
                return ErrorInput(
                    sample,
            ...
            if layout == torch.sparse_csr:
                ...
                if t_inp.dtype == torch.complex32:
                    return ErrorInput(
                        sample,
                        error_regex=("_sparse_csr_sum_cuda not implemented for 'ComplexHalf'"),
                    )
    return sample
...
def _maybe_failing_sample_inputs_sparse_reduction_sum(op_info, device, dtype, requires_grad, layout, **kwargs):
    ...
    if layout in [
        ...
        yield SampleInput(
            torch.tensor([[0, 1], [2, 3]], dtype=dtype).to_sparse(layout=layout).requires_grad_(requires_grad),
...
def sample_inputs_sparse_elementwise_binary_operation(op_info, device, dtype, requires_grad, layout, **kwargs):
        ...
        **kwargs,
        ...
        for dense_dim in range(min_dense_dim, max_dense_dim + 1):
            ...
            for blocksize in blocksizes:
                to_sparse_kwargs = dict(layout=layout, dense_dim=dense_dim, blocksize=blocksize)
...
def _maybe_failing_sample_inputs_sparse_elementwise_binary_mul(op_info, device, dtype, requires_grad, layout, **kwargs):
    ...
    batch = torch.tensor([[[1, 2], [3, 4]], [[4, 5], [6, 7]]], device=device, dtype=dtype).to_sparse(
        layout=layout, dense_dim=0, blocksize=blocksize
    )
    hybrid = torch.tensor([[[1], [2]], [[3], [4]]], device=device, dtype=dtype).to_sparse(
        layout=layout, dense_dim=1, blocksize=blocksize
    )
...
def _sample_inputs_sparse_like_fns(op_info, device, dtype, requires_grad, layout, **kwargs):
    ...
    for tensor in TestCase().generate_simple_inputs(
        ...
        yield SampleInput(tensor, args=(), kwargs=dict(device=device, dtype=dtype, layout=layout))
...
def _validate_sample_input_sparse_like_fns(op_info, sample, check_validate=False):
    ...
    if sample.input.layout is torch.sparse_coo:
        return ErrorInput(
            sample,
            error_regex=("Could not run 'aten::normal_' with arguments from the 'Sparse(CPU|CUDA)' backend."),
        )
    if check_validate:
        _check_validate(op_info, sample)
    return sample
...
def _maybe_failing_sample_inputs_sparse_like_fns(op_info, device, dtype, requires_grad, layout, **kwargs):
    if torch.cuda.is_available() and layout is not torch.sparse_coo:
        ...
        yield SampleInput(
            torch.tensor([[0, 1], [2, 3]], dtype=dtype, device=device).to_sparse(layout=layout, blocksize=blocksize),
...
def _validate_sample_input_sparse_default(op_info, sample, check_validate=False):
    if op_info.name == "to_sparse":
        if (
            sample.input.layout in {torch.sparse_csr, torch.sparse_csc, torch.sparse_bsr, torch.sparse_bsc}
