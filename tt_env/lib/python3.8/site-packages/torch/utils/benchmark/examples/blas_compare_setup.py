...
def main():
    ...
    git_root = (
        subprocess.check_output(
            "git rev-parse --show-toplevel", shell=True, cwd=os.path.dirname(os.path.realpath(__file__))
        )
        .decode("utf-8")
        .strip()
    )
    ...
    for env_name, env_spec in SUB_ENVS.items():
        ...
        build_run = subprocess.run(
            f"source activate {env_path} && " f"cd {git_root} && " "python setup.py install --cmake",
            shell=True,
            capture_output=True,
            check=True,
        )
