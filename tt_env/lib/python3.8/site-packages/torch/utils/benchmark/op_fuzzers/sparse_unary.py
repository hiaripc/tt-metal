    ...
    def __init__(self, seed, dtype=torch.float32, cuda=False):
        super().__init__(
            parameters=[
                ...
                FuzzedParameter(name="sparse_dim", distribution={1: 0.4, 2: 0.4, 3: 0.2}, strict=True),
                # Shapes for `x`.
                ...
                [
                    FuzzedParameter(
                        name=f"k_pow2_{i}", distribution={size: 1.0 / len(_POW_TWO_SIZES) for size in _POW_TWO_SIZES}
                    )
                    for i in range(3)
                ],
                [
