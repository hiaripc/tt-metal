...
def _infer_device_type(*args):
    device_types = list(
        {arg.device.type for arg in args if isinstance(arg, torch.Tensor) and not arg.device.type == "cpu"}
    )
...
def get_device_states(*args) -> Tuple[List[int], List[torch.Tensor]]:
    ...
    fwd_device_ids = list(
        {arg.get_device() for arg in args if isinstance(arg, torch.Tensor) and not arg.device.type == "cpu"}
    )
...
def _supports_autocast(device):
    ...
    return device == "cuda" or (
        hasattr(device_module, "is_autocast_enabled") and hasattr(device_module, "get_autocast_dtype")
    )
...
class _CheckpointFrame:
    def __init__(self, recompute_fn, early_stop, unpack_error_cb, metadata_fn):
        ...
        self.recomputed: DefaultDict[int, weakref.WeakKeyDictionary[_Handle, torch.Tensor]] = defaultdict(
...
def _checkpoint_without_reentrant_generator(
    ...
    **kwargs,
    ...
    if getattr(device_module, "_initialized", False) and preserve_rng_state and not had_device_in_fwd:
