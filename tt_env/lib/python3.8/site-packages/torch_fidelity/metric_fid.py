...
def fid_statistics_to_metric(stat_1, stat_2, verbose):
    ...
    if not np.isfinite(covmean).all():
        vprint(
            verbose,
            f"WARNING: fid calculation produces singular product; " f"adding {eps} to diagonal of cov estimates",
        )
    ...
    out = {KEY_METRIC_FID: float(diff.dot(diff) + np.trace(sigma1) + np.trace(sigma2) - 2 * tr_covmean)}
