...
def calculate_ppl(input_id, **kwargs):
        ...
        for begin_id in range(0, input_model_num_samples, batch_size):
            ...
            if is_cond:
                rgb_e01 = model.forward(
                    torch.cat((batch_lat_e0, batch_lat_e1), dim=0), torch.cat((batch_labels, batch_labels), dim=0)
                )
            else:
    ...
    out = {KEY_METRIC_PPL_MEAN: float(np.mean(distances)), KEY_METRIC_PPL_STD: float(np.std(distances))}
    if reduction == "none":
