...
def is_reference_for_foreach(
    ...
    return (
        ...
        and (not function_schema.name.name.inplace or str(f.func.name) in _foreach_with_inplace_ref)
        and all(
...
def gen_foreach_derivativeinfo(
    ...
    functional_info_by_signature: Dict[FunctionSchema, Dict[str, DifferentiabilityInfo]],
    non_functional_info_by_signature: Dict[FunctionSchema, Dict[str, DifferentiabilityInfo]],
            ...
            for ref_output in derivative.saved_outputs:
                if ref_output.nctype.name == "result":
                    saved_outputs.append(
                        SavedAttribute(
                            nctype=NamedCType(name="result", type=BaseCType(tensorListT)),
                            expr="result",
                        )
                    )
                else:
...
def match_differentiability_info(
    ...
    def find_info(
        ...
        if "generated" in f.tags and f_sig in non_functional_info_by_signature:
            ...
            assert not any(
                any("self" in str(inpt.nctype.name) for inpt in info.all_saved_inputs) for info in info_dict.values()
            ), f"""\
    ...
    for f in native_functions:
        ...
        for key, info in info_dict.items():
            ...
            if f.func.kind() == SchemaKind.inplace:
                ...
                required_original_self_value = bool(re.search(IDENT_REGEX.format("original_self_p"), formula)) or bool(
                    re.search(IDENT_REGEX.format("original_self_t"), formula)
                )
        ...
        result.append(
            NativeFunctionWithDifferentiabilityInfo(func=f, info=info_dict, fw_derivatives=fw_derivative_dict)
        )
