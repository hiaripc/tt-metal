    ...
    def solve(goal: NamedCType, *, direct: bool) -> str:
        ...
        # We can always do translations from value types to reference types, like vector<int> -> IntArrayRef
            ...
            try:
                ...
                symIntArrayRef_type = direct_solve(NamedCType(goal.name, BaseCType(symIntArrayRefT)))
            ...
            argname = direct_solve(NamedCType(goal.name, OptionalCType(BaseCType(longT))))
            ...
            argname = direct_solve(NamedCType(goal.name, OptionalCType(BaseCType(SymIntT))))
            return (
                f"{argname}.has_value() ? c10::make_optional({argname}->guard_int(__FILE__, __LINE__)) : c10::nullopt"
            )
        ...
        if goal.type == MutRefCType(BaseCType(tensorT)):
            const_ref_tensor_ctype = NamedCType(goal.name, ConstRefCType(BaseCType(tensorT)))
