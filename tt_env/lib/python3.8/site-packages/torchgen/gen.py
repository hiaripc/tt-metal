    ...
    *,
    ...
    for name, functions in functions_by_root_name.items():
        ...
        structured_functions = [
            fn for fn in grouped_functions if isinstance(fn, NativeFunctionsGroup) and fn.structured
        ]
            ...
            ops_fm.write_with_template(
                ...
                lambda: {
                    "meta_function_declarations": list(
                        mapMaybe(compute_meta_function_declaration, structured_functions)
                    ),
                },
            )
        declarations = get_native_function_declarations(
        ...
        ops_fm.write_with_template(
            ...
            "NativeFunction.h",
            lambda: {
                "extra_includes": (f"#include <ATen/ops/{name}_meta.h>" if is_structured else []),
        ...
        cpu_fm.write(
            ...
            lambda: {
                f"{category}_includes": [
                    f"#include <ATen/ops/{name}{suffix}.h>" for name in sorted(functions_by_root_name.keys())
                ],
    ...
    for dispatch_key in dispatch_keys:
        ...
        fm.write_with_template(
            ...
            lambda: {
                "dispatch_namespace": dispatch_namespace,
                "DispatchKeyFunctions_inl_includes": [
                    f"#include <ATen/ops/{name}_{dispatch_namespace}_dispatch.h>" for name in sorted(dispatch_names)
                ],
...
def gen_headers(
    *,
    ...
    cpu_fm.write(
        ...
        lambda: {
            "registration_declarations": [
                compute_registration_declarations(f, backend_indices) for f in native_functions
            ],
        },
    )
    ...
    def gen_aten_interned_strings() -> Dict[str, str]:
        ...
        return {
            "aten_symbols": " \\\n".join([f"_(aten, {name})" for name in sorted(names)]),
...
#include <ATen/hip/HIPContext.h>"""
    ...
    def gen_backend_select() -> Dict[str, List[str]]:
        ...
        return {
            ...
            "backend_select_method_definitions": list(
                mapMaybe(ComputeBackendSelect(Target.DEFINITION, selector), relevant_fns)
            ),
            "backend_select_function_registrations": list(
                mapMaybe(ComputeBackendSelect(Target.REGISTRATION, selector), relevant_fns)
            ),
        }
    ...
    (
        aten_schema_registrations,
        schema_registrations,
    ) = get_native_function_schema_registrations(native_functions=native_functions, schema_selector=schema_selector)
    cpu_fm.write(
        "RegisterSchema.cpp",
        lambda: {
            "aten_schema_registrations": [] if skip_dispatcher_op_registration else aten_schema_registrations,
            "schema_registrations": [] if skip_dispatcher_op_registration else schema_registrations,
        },
    )
    ...
    all_groups: List[Union[NativeFunction, NativeFunctionsGroup, NativeFunctionsViewGroup]] = list(
    ...
    structured_map: Dict[OperatorName, NativeFunction] = {
        f.func.name: f for f in concatMap(lambda g: list(g.functions()), structured_native_functions)
    }
        ...
        lambda: {
            "ops_headers": [
                "\n".join(
                    ...
                    for f in ([g.view] if g.view_copy is None else [g.view, g.view_copy])
                )
            ...
            "CompositeViewCopyKernel_Definitions": list(
                mapMaybe(
                    GenCompositeViewCopyKernel(backend_indices[DispatchKey.CompositeExplicitAutogradNonFunctional]),
                    view_groups,
                )
            ),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--backend_whitelist",
        nargs="*",
        help="filter dispatch backend by the whitelist (if set), " "e.g.: CPU CUDA QuantizedCPU ...",
    )
    parser.add_argument(
