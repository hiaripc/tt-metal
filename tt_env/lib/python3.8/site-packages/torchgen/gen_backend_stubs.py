    ...
    for g in grouped_native_functions:
        if isinstance(g, NativeFunction):
            forward_kernels = (
                ...
                else [m for m in [backend_indices[backend_key].get_kernel(g)] if m is not None]
            )
            backward_kernels = (
                []
                if autograd_key is None
                else [m for m in [backend_indices[autograd_key].get_kernel(g)] if m is not None]
            )
        else:
            forward_kernels = (
                []
                if backend_key is None
                else [m for m in [backend_indices[backend_key].get_kernel(f) for f in g.functions()] if m is not None]
            )
            backward_kernels = (
                []
                if autograd_key is None
                else [m for m in [backend_indices[autograd_key].get_kernel(f) for f in g.functions()] if m is not None]
            )
...
def error_on_missing_kernels(
    ...
    expected_backend_native_funcs: List[NativeFunction] = [
        ...
        if f.func.name in expected_backend_op_names.keys() and f.func.name not in full_codegen
    ]
    ...
    autograd_declarations = sorted(
        set(
            concatMap(
                ...
                else dest.compute_native_function_declaration(f, backend_indices[autograd_dispatch_key]),
                grouped_native_functions,
            )
        )
    )
