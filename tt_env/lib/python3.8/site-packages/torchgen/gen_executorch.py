    ...
    *,
    ...
    cpu_fm.write_sharded(
        ...
        env_callable=lambda unbox_kernel_entry: {
            "unboxed_kernels": [ComputeCodegenUnboxedKernels(selector, use_aten_lib)(unbox_kernel_entry)],
            "fn_header": header if unbox_kernel_entry == items[0] else [],  # Only write header once
        },
        num_shards=1,
        ...
        declarations = list(
            mapMaybe(
                ComputeFunction(
                    ...
                    is_custom_op=lambda f: custom_ops_native_functions is not None and f in custom_ops_native_functions,
                ),
    ...
    *,
    ...
    cpu_fm.write_with_template(
        ...
        lambda: {
            ...
            "dispatch_namespace": dispatch_key.lower(),
            "dispatch_namespaced_definitions": "",
            "dispatch_anonymous_definitions": list(mapMaybe(ComputeNativeFunctionStub(), native_functions)),
            "static_init_dispatch_registrations": static_init_dispatch_registrations,
        },
    )
...
def parse_yaml(
...
) -> Tuple[List[NativeFunction], Union[Dict[DispatchKey, Dict[OperatorName, BackendMetadata]], ETKernelIndex],]:
    if path and os.path.exists(path) and os.stat(path).st_size > 0:
        ...
        if kernel_index is not None:
            filtered_index = {
                op_name: kernel_mapping for op_name, kernel_mapping in kernel_index.index.items() if op_name in op_names
            }
