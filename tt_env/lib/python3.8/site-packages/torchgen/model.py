...
DTYPE_CLASSES["Complex"] = OrderedSet([ScalarType.ComplexFloat, ScalarType.ComplexDouble])
...
class NativeFunction:
    ...
    def from_yaml(
        ...
        assert structured_delegate_s is None or "::" not in structured_delegate_s, (
            "namespace is not supported in structured delegate," " using the same namespace as the native function"
        )
        ...
        assert structured_inherits is None or "::" not in structured_inherits, (
            "namespace is not supported in structured inherits," " using the same namespace as the native function"
        )
        ...
        if raw_dispatch is not None:
            ...
            assert not (num_dispatch_keys == 1 and redundant_composite_implicit_autograd), (
                "unnecessary dispatch table for this function; just delete the dispatch " "key entirely"
            )
        ...
        assert len(composites_in_dispatch) <= 1 or (
            ...
            and (DispatchKey.CompositeExplicitAutogradNonFunctional not in composites_in_dispatch)
            and (DispatchKey.CompositeImplicitAutogradNestedTensor in composites_in_dispatch)
        ), (
        ...
        if structured_delegate is not None:
            for key in STRUCTURED_DISPATCH_KEYS:
                assert key not in dispatch, (
                    f"if structured_delegate, then must not have {key} in dispatch dictionary " "(it is delegated!)"
                )
    ...
    def __post_init__(self) -> None:
        ...
        if self.structured_delegate:
            assert self.func.kind() != SchemaKind.out, (
                "structured_delegate field not allowed " "on out= functions; did you mean structured?"
            )
            assert self.device_guard, "device_guard: False is not respected by structured kernels"
        ...
        if (
            ...
            or (
                ("dropout" in str(self.func.name) or any("dropout" in arg.name for arg in self.func.arguments.flat_all))
    ...
    def has_composite_kernel(self) -> bool:
        ...
        ) or (self.has_composite_implicit_autograd_kernel and self.has_composite_implicit_autograd_nested_tensor_kernel)
    ...
    def is_view_op(self) -> bool:
        ...
        is_inplace_view = (
            "inplace_view" in self.tags and str(self.func.name) != "resize_" and str(self.func.name) != "resize_as_"
        )
...
class FunctionSchema:
    ...
    def kind(self) -> SchemaKind:
        ...
        is_mutable = any(
            a.annotation is not None and a.annotation.is_write for a in self.arguments.post_self_positional
        )
    ...
    def aliased_return_names(self) -> List[Optional[str]]:
        ...
        for r in self.returns:
            aliased_args = [
                a for a in self.arguments.flat_all if a.annotation is not None and a.annotation == r.annotation
            ]
        ...
        *,
        ...
        returns_from_mutable_inputs = tuple(
            ...
            for a in itertools.chain(
                ...
                [self.arguments.self_arg.argument] if self.arguments.self_arg is not None else [],
                self.arguments.out,
...
class Annotation:
    ...
    def parse(ann: str) -> "Annotation":
        ...
        is_write = m.group(3) == "!"
        assert not (is_write and len(alias_set) > 1), f"alias set larger than 1 is not mutable, got {ann} instead."
...
class Arguments:
    ...
    def signature(self, *, strip_default: bool = False) -> "Arguments":
        ...
        return Arguments(
            ...
            self_arg=SelfArgument(strip_arg_annotation(self.self_arg.argument)) if self.self_arg is not None else None,
    ...
    def remove_self_annotation(self) -> "Arguments":
        ...
        return dataclasses.replace(
            self,
            self_arg=SelfArgument(dataclasses.replace(self.self_arg.argument, annotation=None)),
        )
    ...
    def __post_init__(self) -> None:
        ...
        mutable_pre_self_positionals = [
            a for a in self.pre_self_positional if a.annotation is not None and a.annotation.is_write
        ]
        assert (
...
def get_view_copy_name(f: NativeFunction) -> "OperatorName":
    ...
    view_copy_name = OperatorName(
        name=BaseOperatorName(base=base_name, inplace=False, dunder_method=f.func.name.name.dunder_method),
    ...
    def parse(src: object) -> "Precompute":
        ...
        for raw_replace_item in src:
            ...
            assert " -> " in raw_replace_item, (
                "precomputed parameters without replacement" " are allowed only in the last line"
            )
