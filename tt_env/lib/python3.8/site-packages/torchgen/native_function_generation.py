...
def self_to_out_signature(func: FunctionSchema) -> FunctionSchema:
    ...
    return FunctionSchema(
        name=func.name.remove_inplace().with_overload(get_expected_out_variant_overload_name(func.name.overload_name)),
...
def functional_to_out_signature(func: FunctionSchema) -> FunctionSchema:
    ...
    return FunctionSchema(
        name=func.name.with_overload(get_expected_out_variant_overload_name(func.name.overload_name)),
    ...
    for d in pre_grouped_native_functions.values():
        ...
        if has_mutable or has_inplace or has_out or has_functional:
            ...
            if not has_out and not base_fn_valid:
                if (
                    str(base_fn.func.name) not in MUTABLE_OPS_THAT_CANNOT_GET_AN_OUT_VARIANT
                    and str(base_fn.func.name) not in FUNCTIONAL_OPS_THAT_CANNOT_GET_AN_OUT_VARIANT
                ):
                    raise AssertionError(
