...
def emit_view_lambda(f: NativeFunction, unpacked_bindings: List[Binding]) -> str:
    ...
    for unpacked_binding in unpacked_bindings:
        ...
        elif (arg == "nested_size_" or arg == "nested_strides_" or arg == "offsets_") and arg_type == ConstRefCType(
            BaseCType(tensorT)
        ):
...
def emit_view_body(fn: NativeFunctionWithDifferentiabilityInfo, var: str) -> Tuple[str, str]:
    ...
    if len(differentiable_output_vars) == 0:
        ...
        rhs_value = (
            f"as_view({view_info}, {var}, " f"/* is_bw_differentiable */ false, /* is_fw_differentiable */ false)"
        )
        ...
        raise RuntimeError(
            "Function that return multiple differentiable output " "when at least one of them is view is not supported."
        )
...
def gen_inplace_or_view_type_env(
    ...
    return {
        "ops_headers": ([f"#include <ATen/ops/{fn.func.root_name}_ops.h>"] if definition is not None else []),
...
def gen_inplace_or_view_type(
    ...
    fm.write_sharded(
        ...
        base_env={
            "generated_comment": "@" + f"generated from {fm.template_dir_for_comments()}/ADInplaceOrViewType.cpp",
        },
