...
def format_trace_inputs(f: NativeFunction) -> str:
    ...
    if f.func.is_out_fn():
        ...
        inplace = [
            ADD_TRACE_INPUT.substitute(name=f.func.arguments.out[i].name, input=f.func.arguments.out[i].name)
...
def emit_trace_body(f: NativeFunction) -> List[str]:
    ...
    assign_return_values = (
        ...
        if f.func.kind() in [SchemaKind.functional, SchemaKind.mutable] and f.func.returns
        else ""
    )
...
def gen_trace_type(out: str, native_functions: List[NativeFunction], template_path: str) -> None:
    ...
    fm.write_sharded(
        ...
        base_env={
            "generated_comment": "@" + f"generated from {fm.template_dir_for_comments()}/TraceType.cpp",
        },
