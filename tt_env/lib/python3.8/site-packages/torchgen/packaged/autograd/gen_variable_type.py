...
def gen_variable_type(
    ...
    fm.write(
        "VariableType.h",
        lambda: {"generated_comment": "@" + f"generated from {fm.template_dir_for_comments()}/VariableType.h"},
    )
    ...
    fm2.write_sharded(
        ...
        base_env={
            "generated_comment": "@" + f"generated from {fm.template_dir_for_comments()}/VariableType.cpp",
        },
...
def emit_body(fn: NativeFunctionWithDifferentiabilityInfo, key: str = "Default") -> List[str]:
    ...
    if is_inplace_foreach:
        ...
        if info is None:
            assert (
                base_name_and_overload_name in _foreach_ops_without_differentiability_info
            ), f"{'.'.join(base_name_and_overload_name)} should have a differentiability info"
        else:
            assert (len(f.func.arguments.flat_non_out) == len(info.func.func.arguments.flat_non_out)) or (
                base_name_and_overload_name in _foreach_ops_with_different_arity
            ), (
    ...
    try_jit_decomposition = (
        requires_derivative and len(fw_derivatives) == 0 and (not modifies_arguments(f)) and (not returns_void)
    )
    ...
    def check_tensorimpl_and_storage(call: str, unpacked_bindings: List[Binding]) -> str:
        ...
        for unpacked_binding in unpacked_bindings:
            ...
            if noref_cpp_type == BaseCType(tensorListT) or noref_cpp_type == BaseCType(iTensorListRefT):
                ...
                stmts_after_call += [
                    ENFORCE_SAME_OPTIONALTENSORLIST_STORAGE.substitute(tensorlist_name=arg),
                ...
                stmts_after_call += [
                    ENFORCE_SAME_TENSOR_STORAGE.substitute(tensor_name=arg, out_tensor_name=arg),
                ...
                for inp in differentiable_inputs:
                    ...
                    if is_list_type:
                        ...
                        cur_derivative_conditions.append(
                            FW_DERIVATIVE_CHECK_TEMPLATE.substitute(req_inp=inp_name + "[i]")
                        )
                    else:
        ...
        for derivative in fw_derivatives:
            ...
            for inp in differentiable_inputs:
                ...
                is_input_tensorlist = is_foreach and is_tensor_list_type(
                    inp.type if not inplace else refargname2inplace_foreacharg[inp.name].type
                )
            ...
            if all((isinstance(var_type, BaseType) and var_type.is_tensor_like()) for var_type in derivative.var_types):
                ...
                if len(derivative.var_types) == 1:
                    ...
                    if not is_foreach:
                        fw_grad_setters.append(
                            FW_DERIVATIVE_SETTER_TENSOR.substitute(out_arg=res[0], is_inplace=is_inplace_str)
                        )
                    else:
                        assert res[0] == ("result" if not inplace else "self")
                        fw_grad_setters.append(
                            FW_DERIVATIVE_SETTER_TENSOR_FOREACH.substitute(out_arg=res[0], is_inplace=is_inplace_str)
                        )
                ...
                if not is_foreach:
                    ...
                    fw_grad_setters.append(
                        FW_DERIVATIVE_SETTER_TENSOR_LIST.substitute(out_arg=res[0], is_inplace=is_inplace_str)
                    )
                else:
            ...
            if not is_foreach:
                ...
                fw_grad_opt_definition = (
                    f"std::vector<{opt_res_grad_type}> {'_'.join(res)}_new_fw_grad_opts" "(self.size(), c10::nullopt);"
                )
                ...
                if inplace:
                    ...
                    if "result" in foreach_forward_grad_formula and "result[i]" not in foreach_forward_grad_formula:
                ...
                content.append(
                    FW_DERIVATIVE_FOREACH_TEMPLATE.substitute(
                        ...
                        any_has_forward_grad_for_current_index=" || ".join(
                            get_any_has_forward_grad_name(derivative.var_names) + "[i]" for derivative in fw_derivatives
                        ),
