...
def create_differentiability_info(
    ...
    def set_up_derivatives(
        f: NativeFunction,
    ) -> Tuple[Sequence[Derivative], Sequence[ForwardDerivative], Sequence[Binding], Sequence[str], Sequence[str],]:
    ...
    if not schema_function:
        ...
        raise RuntimeError(
            f"could not find ATen function for schema: {specification} " f".  Available signatures:\n{avail}"
        )
...
def saved_variables(
    ...
    REPLACEMENTS: List[Tuple[str, Dict[str, Any]]] = [
        ...
        (
            r"{}->sym_sizes\(\)",
            {
                ...
                "nctype": lambda name: NamedCType(name, OptionalCType(BaseCType(symIntArrayRefT))),
        ...
        (
            r"to_args_sizes_symint\({}\)",
            {
                ...
                "nctype": lambda name: NamedCType(name, VectorCType(VectorCType(BaseCType(SymIntT)))),
            },
        ),
    ...
    if re.search(r"\.size\([-]?\d+\)", formula) or re.search(r"->size\([-]?\d+\)", formula):
        raise RuntimeError(
