...
class OptionParser(object):
    ...
    def group_dict(self, group: str) -> Dict[str, Any]:
        ...
        """
        return dict(
            (opt.name, opt.value()) for name, opt in self._options.items() if not group or group == opt.group_name
        )
                ...
                if option.multiple:
                    if not isinstance(config[name], (list, str)):
                        raise Error(
                            ...
                            "or a comma-separated string" % (option.name, option.type.__name__)
                        )
    ...
    def set(self, value: Any) -> None:
        if self.multiple:
            ...
            if value is not None and not isinstance(value, self.type):
                raise Error(
                    "Option %r is required to be a %s (%s given)" % (self.name, self.type.__name__, type(value))
                )
        self._value = value
        if self.callback is not None:
            self.callback(self._value)
