...
class SimpleHTTPClientTestMixin(object):
    ...
    def get_app(self: typing.Any):
        ...
        self.triggers = collections.deque()  # type: typing.Deque[typing.Callable[[], None]]
        return Application(
            [
                url(
                    "/trigger",
        ...
        with closing(self.create_client(max_clients=1)) as client:
            ...
            with self.assertRaises(HTTPTimeoutError) as cm:
                self.io_loop.run_sync(
                    lambda: client.fetch(self.get_url("/hello"), connect_timeout=0.1, raise_error=True)
                )
...
class ChunkedWithContentLengthTest(AsyncHTTPTestCase):
    ...
    def test_chunked_with_content_length(self):
        ...
        with ExpectLog(
            gen_log,
            ("Malformed HTTP message from None: Message " "with both Transfer-Encoding and Content-Length"),
            level=logging.INFO,
        ):
            with self.assertRaises(HTTPStreamClosedError):
