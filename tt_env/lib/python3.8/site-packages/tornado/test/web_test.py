...
class HostMatchingTest(WebTestCase):
    ...
    def test_host_matching(self):
        ...
        self.app.add_handlers(r"www\.example\.com", [("/bar", HostMatchingTest.Handler, {"reply": "[1]"})])
...
class GzipTestCase(SimpleHandlerTestCase):
    ...
    def assert_compressed(self, response):
        ...
        self.assertEqual(
            response.headers.get("Content-Encoding", response.headers.get("X-Consumed-Content-Encoding")),
            "gzip",
        )
...
class UIMethodUIModuleTest(SimpleHandlerTestCase):
    ...
    def test_ui_method(self):
        ...
        self.assertEqual(
            response.body,
            b"In my_ui_method(42) with handler value asdf. " b"In MyModule(123) with handler value asdf.",
        )
...
class Default404Test(WebTestCase):
    ...
    def test_404(self):
        ...
        self.assertEqual(
            response.body,
            b"<html><title>404: Not Found</title>" b"<body>404: Not Found</body></html>",
        )
...
class IncorrectContentLengthTest(SimpleHandlerTestCase):
    ...
    def test_content_length_too_high(self):
        ...
        with ExpectLog(app_log, "(Uncaught exception|Exception in callback)"):
            with ExpectLog(
                gen_log,
                "(Cannot send error response after headers written" "|Failed to flush partial response)",
            ):
...
class URLSpecReverseTest(unittest.TestCase):
    ...
    def test_reverse_arguments(self):
        self.assertEqual("/api/v1/foo/bar", url(r"^/api/v1/foo/(\w+)$", None).reverse("bar"))
        self.assertEqual(
