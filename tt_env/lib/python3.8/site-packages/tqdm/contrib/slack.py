...
class SlackIO(MonoWorker):
    ...
    def write(self, s):
        ...
        try:
            future = self.submit(
                self.client.chat_update, channel=message["channel"], ts=message["ts"], text="`" + s + "`"
            )
        except Exception as e:
            tqdm_auto.write(str(e))
        else:
...
class tqdm_slack(tqdm_auto):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not kwargs.get("disable"):
            ...
            self.sio = SlackIO(
                kwargs.pop("token", getenv("TQDM_SLACK_TOKEN")), kwargs.pop("channel", getenv("TQDM_SLACK_CHANNEL"))
            )
    ...
    def display(self, **kwargs):
        ...
        if fmt.get("bar_format", None):
            fmt["bar_format"] = fmt["bar_format"].replace("<bar/>", "`{bar:10}`").replace("{bar}", "`{bar:10u}`")
        else:
            fmt["bar_format"] = "{l_bar}`{bar:10}`{r_bar}"
        if fmt["ascii"] is False:
            fmt["ascii"] = [":black_square:", ":small_blue_diamond:", ":large_blue_diamond:", ":large_blue_square:"]
            fmt["ncols"] = 336
