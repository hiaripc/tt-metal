...
class TqdmCallback(keras.callbacks.Callback):
    ...
    def __init__(self, epochs=None, data_size=None, batch_size=None, verbose=1, tqdm_class=tqdm_auto, **tqdm_kwargs):
        """
        Parameters
        ----------
        epochs  : int, optional
        ...
        if verbose == 1:
            ...
            self.on_batch_end = self.bar2callback(
                self.batch_bar, pop=["batch", "size"], delta=lambda logs: logs.get("size", 1)
            )
        ...
        if self.verbose:
            ...
            if self.verbose == 2:
                ...
                self.batch_bar = self.tqdm_class(
                    total=total, unit="batch", leave=True, unit_scale=1 / (params("batch_size", 1) or 1)
                )
                self.on_batch_end = self.bar2callback(
                    self.batch_bar, pop=["batch", "size"], delta=lambda logs: logs.get("size", 1)
                )
