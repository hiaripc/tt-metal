...
class Bar(object):
    ...
    def colour(self, value):
        ...
        try:
            ...
            warn(
                "Unknown colour (%s); valid choices: [hex (#00ff00), %s]" % (value, ", ".join(self.COLOURS)),
                TqdmWarning,
                stacklevel=2,
            )
            self._colour = None
...
class tqdm(Comparable):
    ...
    def format_meter(
        ...
        **extra_kwargs,
        ...
        rate_noinv_fmt = ((format_sizeof(rate) if unit_scale else f"{rate:5.2f}") if rate else "?") + unit + "/s"
        rate_inv_fmt = (
            ((format_sizeof(inv_rate) if unit_scale else f"{inv_rate:5.2f}") if inv_rate else "?") + "s/" + unit
        )
        ...
        if total:
            ...
            return f'{(prefix + ": ") if prefix else ""}' f"{n_fmt}{unit} [{elapsed_str}, {rate_fmt}{postfix}]"
        ...
        with cls.get_lock():  # also constructs lock if non-existent
            ...
            if cls.monitor_interval and (cls.monitor is None or not cls.monitor.report()):
                try:
                    ...
                    warn(
                        "tqdm:disabling monitor support" " (monitor_interval = 0) due to:\n" + str(e),
                        TqdmMonitorWarning,
                        stacklevel=2,
                    )
    ...
    @envwrap(
        "TQDM_", is_method=True, types={"total": float, "ncols": int, "miniters": float, "position": int, "nrows": int}
    )
    def __init__(
        self,
        iterable=None,
