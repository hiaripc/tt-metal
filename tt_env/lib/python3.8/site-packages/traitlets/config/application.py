...
class Application(SingletonConfigurable):
    ...
    log_datefmt = Unicode("%Y-%m-%d %H:%M:%S", help="The date format used by logging formatters for %(asctime)s").tag(
        config=True
    )
    ...
    def get_default_logging_config(self) -> StrDict:
        ...
        config: StrDict = {
            ...
            "formatters": {
                "console": {
                    "class": (f"{self._log_formatter_cls.__module__}" f".{self._log_formatter_cls.__name__}"),
    ...
    show_config = Bool(help="Instead of starting the Application, dump configuration to stdout").tag(config=True)

    show_config_json = Bool(help="Instead of starting the Application, dump configuration to stdout (as JSON)").tag(
        config=True
    )
    ...
    def emit_flag_help(self) -> t.Generator[str, None, None]:
        ...
        for flags, (cfg, fhelp) in self.flags.items():
            try:
                ...
                cfg_list = " ".join(
                    f"--{clname}.{prop}={val}" for clname, props_dict in cfg.items() for prop, val in props_dict.items()
                )
    ...
    def _create_loader(
        ...
        return KVArgParseConfigLoader(argv, aliases, flags, classes=classes, log=self.log, subcommands=self.subcommands)
    ...
    def load_config_file(self, filename: str, path: str | t.Sequence[str | None] | None = None) -> None:
        ...
        for config, fname in self._load_config_files(
            ...
            if fname not in self._loaded_config_files:  # only add to list of loaded files if not previously loaded
    ...
    def _classes_with_config_traits(
        ...
        cls_to_config = OrderedDict(
            (cls, bool(cls.class_own_traits(config=True))) for cls in self._classes_inc_parents(classes)
        )
