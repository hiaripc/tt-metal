    ...
    def __setitem__(self, key: str, value: t.Any) -> None:
        if _is_section_key(key):
            if not isinstance(value, Config):
                raise ValueError(
                    "values whose keys begin with an uppercase " f"char must be Config instances: {key!r}, {value!r}"
                )
...
class _FlagAction(argparse.Action):
    ...
    def __call__(self, parser: t.Any, namespace: t.Any, values: t.Any, option_string: str | None = None) -> None:
...
class KeyValueConfigLoader(KVArgParseConfigLoader):
    ...
    def __init__(self, *args: t.Any, **kwargs: t.Any) -> None:
        warnings.warn(
            "KeyValueConfigLoader is deprecated since Traitlets 5.0." " Use KVArgParseConfigLoader instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        super().__init__(*args, **kwargs)
