            ...
            if isinstance(value, TraitType):
                ...
                for c in mro_trait:
                    ...
                    if (
                        ...
                        cls._static_immutable_initial_values[name] = trait.default_value
                    elif type(trait) in [CBool, Bool] and (isinstance(trait.default_value, bool) or none_ok):
                        cls._static_immutable_initial_values[name] = trait.default_value
                    elif type(trait) in [CUnicode, Unicode] and (isinstance(trait.default_value, str) or none_ok):
                        cls._static_immutable_initial_values[name] = trait.default_value
                    elif type(trait) == Any and (isinstance(trait.default_value, (str, int, float, bool)) or none_ok):
                        cls._static_immutable_initial_values[name] = trait.default_value
    ...
    def validate(self, obj: t.Any, value: t.Any) -> G:
        ...
        if isinstance(value, str):
            try:
                ...
                raise TraitError(
                    f"The '{self.name}' trait of {obj} instance must be a type, but " f"{value!r} could not be imported"
                ) from e
        try:
...
class Container(Instance[T]):
    ...
    def __init__(
        ...
        **kwargs : any
        ...
        super().__init__(klass=self.klass, args=args, help=help, read_only=read_only, config=config, **kwargs)
    ...
    def from_string_list(self, s_list: list[str]) -> T | None:
        ...
        if len(s_list) == 1:
            ...
            if r == "None" and self.allow_none:
                return None
            if len(r) >= 2 and any(
                r.startswith(start) and r.endswith(end) for start, end in self._literal_from_string_pairs
            ):
                if self.this_class:
...
class Dict(Instance["dict[K, V]"]):
    ...
    def item_from_string(self, s: str) -> dict[K, V]:
        ...
        if "=" not in s:
            raise TraitError(f"'{self.__class__.__name__}' options must have the form 'key=value', got {s!r}")
        key, value = s.split("=", 1)
