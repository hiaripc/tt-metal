...
class PackageFile:
    ...
    def from_filename(cls, filename: str, comment: Optional[str]) -> "PackageFile":
        ...
        for ext, dtype in DIST_EXTENSIONS.items():
            if filename.endswith(ext):
                try:
                    ...
                    raise exceptions.InvalidDistribution("Invalid distribution file: '%s'" % os.path.basename(filename))
                else:
                    break
        else:
        ...
        # setup.cfg.
        missing_fields = [f.capitalize() for f in ["name", "version"] if not getattr(meta, f)]
        if missing_fields:
    ...
    def add_gpg_signature(self, signature_filepath: str, signature_filename: str) -> None:
        if self.gpg_signature is not None:
    ...
    def run_gpg(cls, gpg_args: Tuple[str, ...]) -> None:
        try:
            ...
            if gpg_args[0] != "gpg":
                raise exceptions.InvalidSigningExecutable(f"{gpg_args[0]} executable not available.")
