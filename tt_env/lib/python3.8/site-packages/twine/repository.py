...
class Repository:
    ...
    def _make_user_agent_string() -> str:
        user_agent_string = user_agent.UserAgentBuilder("twine", twine.__version__).include_implementation().build()
    ...
    def _upload(self, package: package_file.PackageFile) -> requests.Response:
        ...
        with open(package.filename, "rb") as fp:
            data_to_send.append(("content", (package.basefilename, fp, "application/octet-stream")))
            encoder = requests_toolbelt.MultipartEncoder(data_to_send)
    ...
    def upload(self, package: package_file.PackageFile, max_redirects: int = 5) -> requests.Response:
    ...
    def package_is_uploaded(self, package: package_file.PackageFile, bypass_cache: bool = False) -> bool:
    ...
    def release_urls(self, packages: List[package_file.PackageFile]) -> Set[str]:
        ...
        return {f"{url}project/{package.safe_name}/{package.metadata.version}/" for package in packages}
