...
def _validate_repository_url(repository_url: str) -> None:
    ...
    validator = rfc3986.validators.Validator().allow_schemes("http", "https").require_presence_of("scheme", "host")
    try:
        validator.validate(rfc3986.uri_reference(repository_url))
    except rfc3986.exceptions.RFC3986Exception as exc:
        raise exceptions.UnreachableRepositoryURLDetected(f"Invalid repository URL: {exc.args[0]}.")
...
def check_status_code(response: requests.Response, verbose: bool) -> None:
    ...
    try:
        ...
        if not verbose:
            logger.warning("Error during upload. " "Retry with the --verbose option for more details.")
