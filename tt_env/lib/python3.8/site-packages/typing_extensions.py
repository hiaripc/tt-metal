...
if _PEP_728_IMPLEMENTED:
    ...
    def TypedDict(typename, fields=_marker, /, *, total=True, closed=None, extra_items=NoExtraItems, **kwargs):
...
if hasattr(typing, "ReadOnly"):  # 3.13+
    ...
    def get_type_hints(obj, globalns=None, localns=None, include_extras=False):
        ...
        if sys.version_info < (3, 9):
            ...
            hint = {k: (t if get_origin(t) != Union else Union[t.__args__]) for k, t in hint.items()}
        if include_extras:
            return hint
    ...
    def _clean_optional(obj, hints, globalns=None, localns=None):
        ...
        for name, value in hints.items():
            ...
            if not _could_be_inserted_optional(value) or name not in defaults or defaults[name] is not None:
                continue
            ...
            if isinstance(original_value, str):
                ...
                if sys.version_info < (3, 9):
                    ...
                    original_value = ForwardRef(original_value, is_argument=not isinstance(obj, _types.ModuleType))
...
if not hasattr(typing, "Concatenate"):
    ...
    class _ConcatenateGenericAlias(list):
        ...
        def __getitem__(self, args):
            ...
            if alen != plen:
                raise TypeError(
                    f"Too {'many' if alen > plen else 'few'} arguments for {self};" f" actual {alen}, expected {plen}"
                )
...
def _create_concatenate_alias(origin, parameters):
    ...
    if parameters[-1] is not _EllipsisDummy:
        return concatenate
    # Remove dummy again
    concatenate.__args__ = tuple(p if p is not _EllipsisDummy else ... for p in concatenate.__args__)
    if sys.version_info < (3, 10):
        # backport needs __args__ adjustment only
        return concatenate
    concatenate.__parameters__ = tuple(p for p in concatenate.__parameters__ if p is not _EllipsisDummy)
    return concatenate
...
if hasattr(typing, "_collect_type_vars"):
    ...
    def _collect_type_vars(types, typevar_types=None):
        ...
        for t in types:
            ...
            elif isinstance(t, typevar_types) and not isinstance(t, _UnpackAlias) and t not in tvars:
        ...
        def __init__(self, name: str, value, *, type_params=()):
            ...
            for type_param in type_params:
                ...
                if default_value_encountered and not has_default:
                    raise TypeError(f"non-default type parameter '{type_param!r}'" " follows default type parameter")
                if has_default:
        ...
        * STRING: return annotations as strings, in a format close to the original
        ...
        if not eval_str:
            if format is Format.STRING:
                return {
                    key: value if isinstance(value, str) else typing._type_repr(value) for key, value in ann.items()
                }
            return dict(ann)
...
if hasattr(typing, "evaluate_forward_ref"):
    ...
    def _eval_with_owner(forward_ref, *, owner=None, globals=None, locals=None, type_params=None):
        ...
        if globals is None and getattr(forward_ref, "__forward_module__", None) is not None:
            globals = getattr(sys.modules.get(forward_ref.__forward_module__, None), "__dict__", None)
        if globals is None:
        ...
        if type_params is not None:
            ...
            for param in type_params:
                ...
                if (_FORWARD_REF_HAS_CLASS and not forward_ref.__forward_is_class__) or param_name not in globals:
