...
class Expansion:
    ...
    def _encode_list_item(
        self, variable: Variable, name: str, index: int, item: Any, delim: str, prefix: str, joiner: str, first: bool
    ) -> str | None:
    ...
    def _encode_var(
        ...
        if isinstance(value, str):
            ...
            if len(value):
                encoded_items = [
                    self._encode_dict_item(variable, name, key, value[key], delim, prefix, joiner, first)
            ...
            if len(value):
                encoded_items = [
                    self._encode_list_item(variable, name, index, item, delim, prefix, joiner, first)
...
class ExpressionExpansion(Expansion):
    ...
    def expand(self, values: Mapping[str, Any]) -> str | None:
        ...
        if expanded_vars:
            return (
                (self.output_prefix if (not self.trailing_joiner) else "")
                + self.var_joiner.join(expanded_vars)
    ...
    def partial(self, values: Mapping[str, Any]) -> str:
        ...
        for index, (expanded, missing) in enumerate(result):
            ...
            if expanded:
                output += (
                    (self.output_prefix if (first and (not self.trailing_joiner)) else "")
                    + self.var_joiner.join(expanded)
                    + self.trailing_joiner
                )
            else:
                output += (
                    (self.output_prefix if (first and not last) else (self.var_joiner if (not last) else ""))
                    + "{"
...
class LabelExpansion(ExpressionExpansion):
    ...
    def _expand_var(self, variable: Variable, value: Any) -> str | None:
        """Expand a single variable."""
        return self._encode_var(
            variable, self._uri_encode_name(variable.name), value, delim=("." if variable.explode else ",")
        )
...
class PathStyleExpansion(ExpressionExpansion):
    ...
    def _encode_dict_item(
        ...
        if variable.array:
            ...
            joiner = ","
        return self._encode_var(
            variable, self._uri_encode_name(key) if (not variable.array) else "", item, delim, prefix, joiner, False
        )
