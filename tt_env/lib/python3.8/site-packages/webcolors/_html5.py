...
def html5_parse_simple_color(value: str) -> HTML5SimpleColor:
    ...
    #
    # 9. Return result.
    return HTML5SimpleColor(int(value[1:3], 16), int(value[3:5], 16), int(value[5:7], 16))
...
def html5_parse_legacy_color(value: str) -> HTML5SimpleColor:
    ...
    *extremely* forgiving of input, but the results of parsing inputs
    ...
    if len(value) == 4 and value.startswith("#") and all(c in string.hexdigits for c in value[1:]):
        ...
        result = HTML5SimpleColor(int(value[1], 16) * 17, int(value[2], 16) * 17, int(value[3], 16) * 17)
