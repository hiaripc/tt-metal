...
def _get_environ(obj: WSGIEnvironment | Request) -> WSGIEnvironment:
    ...
    assert isinstance(env, dict), f"{type(obj).__name__!r} is not a WSGI environment (has to be a dict)"
    return env
...
class _DictAccessorProperty(t.Generic[_TAccessorValue]):
    ...
    def __get__(self, instance: t.Any | None, owner: type) -> _TAccessorValue | _DictAccessorProperty[_TAccessorValue]:
        if instance is None:
            return self
