...
class LocalManager:
    ...
    def __init__(
        self,
        locals: None | (Local | LocalStack[t.Any] | t.Iterable[Local | LocalStack[t.Any]]) = None,
    ) -> None:
        if locals is None:
            self.locals = []
...
class LocalProxy(t.Generic[T]):
    ...
    __repr__ = _ProxyLookup(repr, fallback=lambda self: f"<{type(self).__name__} unbound>")  # type: ignore[assignment]
