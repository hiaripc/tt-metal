    ...
    def __str__(self) -> str:
        ...
        if self.suggested:
            if self.endpoint == self.suggested.endpoint:
                if self.method and self.suggested.methods is not None and self.method not in self.suggested.methods:
                    message.append(" Did you mean to use methods" f" {sorted(self.suggested.methods)!r}?")
                missing_values = self.suggested.arguments.union(set(self.suggested.defaults or ())) - set(
                    self.values.keys()
                )
                if missing_values:
                    message.append(f" Did you forget to specify values {sorted(missing_values)!r}?")
            else:
