...
def parse_converter_args(argstr: str) -> tuple[tuple[t.Any, ...], dict[str, t.Any]]:
    ...
    for item in _converter_args_re.finditer(argstr):
        if item.start() != position:
            raise ValueError(f"Cannot parse converter argument '{argstr[position:item.start()]}'")
...
class Rule(RuleFactory):
    ...
    def _compile_builder(self, append_unknown: bool = True) -> t.Callable[..., tuple[str, str]]:
        ...
        def _parts(ops: list[tuple[bool, str]]) -> list[ast.expr]:
            parts: list[ast.expr] = [_convert(elem) if is_dynamic else ast.Constant(elem) for is_dynamic, elem in ops]
        ...
        pargs = [elem for is_dynamic, elem in dom_ops + url_ops if is_dynamic and elem not in defaults]
