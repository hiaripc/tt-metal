...
class Client:
    ...
    def __init__(
        ...
        elif response_wrapper is not None and not issubclass(response_wrapper, TestResponse):
    ...
    def set_cookie(
        ...
        **kwargs: t.Any,
        ...
        cookie = Cookie._from_response_header(domain, "/", dump_cookie(key, value, domain=domain, path=path, **kwargs))
    ...
    def _add_cookies_to_wsgi(self, environ: WSGIEnvironment) -> None:
        ...
        value = "; ".join(
            c._to_request_header() for c in self._cookies.values() if c._matches_request(server_name, url.path)
        )
    ...
    def _update_cookies_from_response(self, server_name: str, path: str, headers: list[str]) -> None:
        ...
        while response.status_code in {
            ...
            if new_redirect_entry in redirects:
                raise ClientRedirectError(
                    f"Loop detected: A {response.status_code} redirect" f" to {response.location} was already made."
                )
...
class Cookie:
    ...
    def _matches_request(self, server_name: str, path: str) -> bool:
        return (
            ...
            path == self.path
            or (path.startswith(self.path) and path[len(self.path) - self.path.endswith("/") :].startswith("/"))
        )
    ...
    def _should_delete(self) -> bool:
        return self.max_age == 0 or (self.expires is not None and self.expires.timestamp() == 0)
