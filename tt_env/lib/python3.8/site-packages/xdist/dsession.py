...
class DSession:
    ...
    def pytest_runtestloop(self) -> bool:
        self.sched = self.config.hook.pytest_xdist_make_scheduler(config=self.config, log=self.log)
    ...
    def worker_internal_error(self, node: WorkerController, formatted_error: str) -> None:
        """
    ...
    def worker_errordown(self, node: WorkerController, error: object | None) -> None:
        ...
        maximum_reached = self._max_worker_restart is not None and self._failed_nodes_count > self._max_worker_restart
        if maximum_reached:
    ...
    def worker_collectionfinish(self, node: WorkerController, ids: Sequence[str]) -> None:
        ...
        if self.terminal:
            self.trdist.setstatus(node.gateway.spec, WorkerStatus.CollectionDone, tests_collected=len(ids))
        if self.sched.collection_is_completed:
            if self.terminal and not self.sched.has_pending:
                self.trdist.ensure_show_status()
                self.terminal.write_line("")
                if self.config.option.verbose > 0:
                    self.terminal.write_line(f"scheduling tests via {self.sched.__class__.__name__}")
            self.sched.schedule()
    ...
    def worker_runtest_protocol_complete(self, node: WorkerController, item_index: int, duration: float) -> None:
        """
    ...
    def worker_unscheduled(self, node: WorkerController, indices: Sequence[int]) -> None:
        """
    ...
    def worker_warning_recorded(
        ...
        kwargs = dict(warning_message=warning_message, when=when, nodeid=nodeid, location=location)
    ...
    def _handlefailures(
        ...
        if rep.failed:
            ...
            if self.maxfail and self.countfailures >= self.maxfail and not self.shouldstop:
...
class TerminalDistReporter:
    ...
    def pytest_testnodeready(self, node: WorkerController) -> None:
        ...
        self.setstatus(node.gateway.spec, WorkerStatus.ReadyForCollection, tests_collected=0)
