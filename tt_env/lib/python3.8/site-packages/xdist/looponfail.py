...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group._addoption(
        ...
        dest="looponfail",
        default=False,
        help="Run tests in subprocess: wait for files to be modified, then " "re-run failing test set until all pass.",
    )
    ...
    try:
        while 1:
            ...
            if not remotecontrol.failures and remotecontrol.wasfailing:
                ...
                continue
            repr_pytest_looponfailinfo(failreports=remotecontrol.failures, rootdirs=rootdirs)
...
def repr_pytest_looponfailinfo(failreports: Sequence[str], rootdirs: Sequence[Path]) -> None:
    tr = TerminalWriter()
    if failreports:
            ...
            for path in visit_path(rootdir, filter=self.fil, recurse=self.rec):
                ...
                try:
                    ...
                    if oldstat is not None:
                        if oldstat.st_mtime != curstat.st_mtime or oldstat.st_size != curstat.st_size:
                            changed = True
                            print("# MODIFIED", path)
