...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group.addoption(
        ...
        help="Maximum number of workers that can be restarted " "when crashed (set to zero to disable this feature)",
    )
    group.addoption(
        "--dist",
        metavar="distmode",
...
def pytest_cmdline_main(config: pytest.Config) -> None:
    ...
    if not val("collectonly") and _is_distribution_mode(config) and usepdb:
        raise pytest.UsageError("--pdb is incompatible with distributing tests; try using -n0 or -nauto.")
...
def is_xdist_controller(
    ...
    :param request_or_session: the `pytest` `request` or `session` object
    """
    return not is_xdist_worker(request_or_session) and request_or_session.config.option.dist != "no"
