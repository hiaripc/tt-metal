...
class WorkerInteractor:
    ...
    def handle_command(self, command: tuple[str, dict[str, Any]] | Literal[Marker.SHUTDOWN]) -> None:
    ...
    def run_one_test(self) -> None:
        ...
        self.sendevent("runtest_protocol_complete", item_index=self.item_index, duration=duration)
    ...
    def pytest_collection_modifyitems(
        ...
        if config.getvalue("loadgroup"):
            for item in items:
                ...
                if not mark:
                    continue
                gname = mark.args[0] if len(mark.args) > 0 else mark.kwargs.get("name", "default")
