...
def parse_spec_config(config: pytest.Config) -> list[str]:
    ...
    if not xspeclist:
        raise pytest.UsageError("MISSING test execution (tx) nodes: please specify --tx")
    return xspeclist
...
class NodeManager:
    ...
    def rsync(
        ...
        notify: (Callable[[str, execnet.XSpec, str | os.PathLike[str]], Any] | None) = None,
        verbose: int = False,
...
class WorkerController:
    ...
    def process_from_remote(self, eventcall: tuple[str, dict[str, Any]] | Literal[Marker.END]) -> None:
        ...
        try:
            ...
            if eventname in ("collectionstart",):
                ...
                rep = self.config.hook.pytest_report_from_serializable(config=self.config, data=kwargs["data"])
