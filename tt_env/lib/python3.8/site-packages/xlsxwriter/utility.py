...
def supported_datetime(dt):
    ...
    return isinstance(dt, (datetime.datetime, datetime.date, datetime.time, datetime.timedelta))
...
def datetime_to_excel_datetime(dt_obj, date_1904, remove_timezone):
    ...
    excel_time = delta.days + (float(delta.seconds) + float(delta.microseconds) / 1e6) / (60 * 60 * 24)
    ...
    if isinstance(date_type, datetime.datetime) and dt_obj.isocalendar() == (1900, 1, 1):
        excel_time -= 1
