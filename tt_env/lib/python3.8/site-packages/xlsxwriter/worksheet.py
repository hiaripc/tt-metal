...
class Worksheet(xmlwriter.XMLwriter):
    ...
    def write_array_formula(self, first_row, first_col, last_row, last_col, formula, cell_format=None, value=0):
        ...
        return self._write_array_formula(
            first_row, first_col, last_row, last_col, formula, cell_format, value, "static"
        )
    ...
    def write_dynamic_array_formula(self, first_row, first_col, last_row, last_col, formula, cell_format=None, value=0):
        ...
        error = self._write_array_formula(
            first_row, first_col, last_row, last_col, formula, cell_format, value, "dynamic"
        )
    ...
    def _prepare_formula(self, formula):
        ...
        formula = re.sub(r"\bFORECAST.ETS.CONFINT\(", "_xlfn.FORECAST.ETS.CONFINT(", formula)
        formula = re.sub(r"\bFORECAST.ETS.SEASONALITY\(", "_xlfn.FORECAST.ETS.SEASONALITY(", formula)
    ...
    def _write_array_formula(
        self, first_row, first_col, last_row, last_col, formula, cell_format=None, value=0, atype="static"
    ):
    ...
    def insert_textbox(self, row, col, text, options=None):
        ...
        self.shapes.append(
            [row, col, x_offset, y_offset, x_scale, y_scale, text, anchor, options, description, decorative]
        )
        return 0
            ...
            for col in range(first_col, last_col + 1):
                if self.merged_cells.get((row, col)):
                    ...
                    raise OverlappingRange(
                        "Merge range '%s' overlaps previous merge range '%s'." % (cell_range, previous_range)
                    )
                elif self.table_cells.get((row, col)):
                    previous_range = self.table_cells.get((row, col))
                    raise OverlappingRange(
                        "Merge range '%s' overlaps previous table range '%s'." % (cell_range, previous_range)
                    )
                else:
    ...
    def data_validation(self, first_row, first_col, last_row, last_col, options=None):
        ...
        if "error_type" not in options:
            ...
            warn(
                "Unknown criteria type '%s' for parameter 'error_type' " "in data_validation()" % options["error_type"]
            )
            return -2
        else:
    ...
    def conditional_format(self, first_row, first_col, last_row, last_col, options=None):
        ...
        if options["type"] == "iconSet":
            ...
            if not options.get("icon_style"):
                warn(
                    "The 'icon_style' parameter must be specified when " "'type' == 'icon_set' in conditional_format()"
                )
                return -3
        ...
        if options["type"] == "timePeriod":
            if options["criteria"] == "yesterday":
                ...
                options["formula"] = (
                    ...
                    "TODAY()),AND(MONTH(%s)=1,YEAR(A1)=YEAR(TODAY())-1)))" % (start_cell, start_cell, start_cell)
                )
    ...
    def add_table(self, first_row, first_col, last_row, last_col, options=None):
        ...
        for row in range(first_row, last_row + 1):
            for col in range(first_col, last_col + 1):
                if self.table_cells.get((row, col)):
                    previous_range = self.table_cells.get((row, col))
                    raise OverlappingRange(
                        "Table range '%s' overlaps previous table range '%s'." % (cell_range, previous_range)
                    )
                elif self.merged_cells.get((row, col)):
                    previous_range = self.merged_cells.get((row, col))
                    raise OverlappingRange(
                        "Table range '%s' overlaps previous merge range '%s'." % (cell_range, previous_range)
                    )
                else:
        ...
        if "name" in options:
            ...
            if not re.match(r"^[\w\\][\w\\.]*$", name, re.UNICODE) or re.match(r"^\d", name):
    ...
    def set_header(self, header="", options=None, margin=None):
        ...
        if placeholder_count != image_count:
            warn(
                ...
                "count (%s) in string: %s" % (image_count, placeholder_count, header_orig)
            )
            self.header_images = []
            return
    ...
    def set_footer(self, footer="", options=None, margin=None):
        ...
        if placeholder_count != image_count:
            warn(
                ...
                "count (%s) in string: %s" % (image_count, placeholder_count, footer_orig)
            )
            self.footer_images = []
            return
        ...
        if re.match("blanks|nonblanks", token.lower()):
            ...
            if operator != 2 and operator != 5:
                warn(
                    ...
                    "is not valid in relation to Blanks/NonBlanks'" % (tokens[1], expression)
                )
    ...
    def _prepare_image(self, index, image_id, drawing_id, width, height, name, image_type, x_dpi, y_dpi, digest):
        ...
        drawing_type = 2
        (row, col, _, x_offset, y_offset, x_scale, y_scale, url, tip, anchor, _, description, decorative) = self.images[
            index
        ]
            ...
            if target is not None:
                if len(target) > self.max_url_length:
                    warn(
                        ...
                        "characters since it exceeds Excel's limit for URLS" % (url, self.max_url_length)
                    )
                else:
    ...
    def _prepare_shape(self, index, drawing_id):
        ...
        (row, col, x_offset, y_offset, x_scale, y_scale, text, anchor, options, description, decorative) = self.shapes[
            index
        ]
    ...
    def _position_object_emus(self, col_start, row_start, x1, y1, width, height, anchor):
        ...
        # pixel
        (col_start, row_start, x1, y1, col_end, row_end, x2, y2, x_abs, y_abs) = self._position_object_pixels(
                ...
                if user_props[i].get("type"):
                    ...
                    if user_props[i]["type"] not in valid_types:
                        warn(
                            ...
                            "property 'type' in conditional_format()" % user_props[i]["type"]
                        )
                    else:
    ...
    def _write_cell(self, row, col, cell):
        ...
        if type_cell_name == "Number" or type_cell_name == "Datetime":
            ...
            if not self.constant_memory:
                ...
                string = re_control_chars_2.sub(lambda match: "_x%04X_" % ord(match.group(1)), string)
