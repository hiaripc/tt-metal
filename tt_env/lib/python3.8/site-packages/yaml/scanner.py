    ...
    def stale_possible_simple_keys(self):
        ...
        for level in list(self.possible_simple_keys):
            ...
            if key.line != self.line or self.index - key.index > 1024:
                if key.required:
                    raise ScannerError(
                        "while scanning a simple key", key.mark, "could not find expected ':'", self.get_mark()
                    )
            ...
            for k in range(2):
                if self.peek(k) not in "0123456789ABCDEFabcdef":
                    raise ScannerError(
                        ...
                        start_mark,
                        "expected URI escape sequence of 2 hexadecimal numbers, but found %r" % self.peek(k),
                        self.get_mark(),
                    )
