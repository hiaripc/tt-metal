...
def run(argv=None):
    ...
    files_group.add_argument("files", metavar="FILE_OR_DIR", nargs="*", default=(), help="files to check")
    files_group.add_argument("-", action="store_true", dest="stdin", help="read from standard input")
    config_group = parser.add_mutually_exclusive_group()
    config_group.add_argument(
        "-c", "--config-file", dest="config_file", action="store", help="path to a custom configuration"
    )
    config_group.add_argument(
        "-d", "--config-data", dest="config_data", action="store", help="custom configuration (as YAML source)"
    )
    ...
    parser.add_argument(
        "-s", "--strict", action="store_true", help="return non-zero exit code on warnings " "as well as errors"
    )
    parser.add_argument("--no-warnings", action="store_true", help="output only error level problems")
    parser.add_argument("-v", "--version", action="version", version="{} {}".format(APP_NAME, APP_VERSION))
    ...
    for file in find_files_recursively(args.files, conf):
        ...
        try:
            ...
            print(e, file=sys.stderr)
            sys.exit(-1)
        prob_level = show_problems(problems, file, args_format=args.format, no_warn=args.no_warnings)
    ...
    if args.stdin:
        try:
            ...
            print(e, file=sys.stderr)
            sys.exit(-1)
        prob_level = show_problems(problems, "stdin", args_format=args.format, no_warn=args.no_warnings)
        max_level = max(max_level, prob_level)
