...
class YamlLintConfig:
    ...
    def enabled_rules(self, filepath):
        return [
            ...
            if val is not False and (filepath is None or "ignore" not in val or not val["ignore"].match_file(filepath))
        ]
...
def validate_rule_conf(rule, conf):
    ...
    if isinstance(conf, dict):
        if "ignore" in conf and not isinstance(conf["ignore"], pathspec.pathspec.PathSpec):
            if isinstance(conf["ignore"], str):
        ...
        for optkey in conf:
            ...
            if optkey not in options:
                raise YamlLintConfigError('invalid config: unknown option "%s" for rule "%s"' % (optkey, rule.ID))
            ...
            if isinstance(options[optkey], tuple):
                if conf[optkey] not in options[optkey] and type(conf[optkey]) not in options[optkey]:
                    raise YamlLintConfigError(
                        'invalid config: option "%s" of "%s" should be in %s' % (optkey, rule.ID, options[optkey])
                    )
            # Example: CONF = {option: ['flag1', 'flag2', int]}
            #          → {option: [flag1]}      → {option: [42, flag1, flag2]}
            elif isinstance(options[optkey], list):
                if type(conf[optkey]) is not list or any(
                    flag not in options[optkey] and type(flag) not in options[optkey] for flag in conf[optkey]
                ):
                    raise YamlLintConfigError(
                        ('invalid config: option "%s" of "%s" should only ' "contain values in %s")
            ...
            #          → {option: 42}
                ...
                if not isinstance(conf[optkey], options[optkey]):
                    raise YamlLintConfigError(
                        'invalid config: option "%s" of "%s" should be %s' % (optkey, rule.ID, options[optkey].__name__)
                    )
        ...
        if hasattr(rule, "VALIDATE"):
            ...
            if res:
                raise YamlLintConfigError("invalid config: %s: %s" % (rule.ID, res))
    else:
        raise YamlLintConfigError(
            ('invalid config: rule "%s": should be ' 'either "enable", "disable" or a dict') % rule.ID
        )
