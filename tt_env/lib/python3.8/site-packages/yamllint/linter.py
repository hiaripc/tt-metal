    ...
    for elem in parser.token_or_comment_or_line_generator(buffer):
        if isinstance(elem, parser.Token):
            ...
            for problem in cache:
                if not (
                    disabled_for_line.is_disabled_by_directive(problem) or disabled.is_disabled_by_directive(problem)
                ):
                    yield problem
...
def get_syntax_error(buffer):
    try:
        ...
        problem = LintProblem(
            e.problem_mark.line + 1, e.problem_mark.column + 1, "syntax error: " + e.problem + " (syntax)"
        )
        problem.level = "error"
        return problem
