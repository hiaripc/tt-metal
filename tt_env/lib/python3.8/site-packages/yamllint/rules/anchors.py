...
ID = "anchors"
TYPE = "token"
CONF = {"forbid-undeclared-aliases": bool, "forbid-duplicated-anchors": bool, "forbid-unused-anchors": bool}
DEFAULT = {"forbid-undeclared-aliases": True, "forbid-duplicated-anchors": False, "forbid-unused-anchors": False}
...
def check(conf, token, prev, next, nextnext, context):
    ...
    if (
        ...
        yield LintProblem(
            token.start_mark.line + 1, token.start_mark.column + 1, f'found undeclared alias "{token.value}"'
        )

    if conf["forbid-duplicated-anchors"] and isinstance(token, yaml.AnchorToken) and token.value in context["anchors"]:
        yield LintProblem(
            token.start_mark.line + 1, token.start_mark.column + 1, f'found duplicated anchor "{token.value}"'
        )
    ...
    if conf["forbid-unused-anchors"]:
        ...
        if isinstance(next, (yaml.StreamEndToken, yaml.DocumentStartToken, yaml.DocumentEndToken)):
            for anchor, info in context["anchors"].items():
                if not info["used"]:
                    yield LintProblem(info["line"] + 1, info["column"] + 1, f'found unused anchor "{anchor}"')
