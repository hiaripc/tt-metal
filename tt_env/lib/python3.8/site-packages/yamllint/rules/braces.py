...
def check(conf, token, prev, next, nextnext, context):
    if conf["forbid"] is True and isinstance(token, yaml.FlowMappingStartToken):
        yield LintProblem(token.start_mark.line + 1, token.end_mark.column + 1, "forbidden flow mapping")
    ...
    elif isinstance(token, yaml.FlowMappingStartToken) and isinstance(next, yaml.FlowMappingEndToken):
