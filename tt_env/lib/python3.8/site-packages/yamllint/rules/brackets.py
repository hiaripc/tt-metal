...
def check(conf, token, prev, next, nextnext, context):
    if conf["forbid"] is True and isinstance(token, yaml.FlowSequenceStartToken):
        yield LintProblem(token.start_mark.line + 1, token.end_mark.column + 1, "forbidden flow sequence")
    ...
    elif isinstance(token, yaml.FlowSequenceStartToken) and isinstance(next, yaml.FlowSequenceEndToken):
        problem = spaces_after(
            token,
            prev,
            next,
            min=(
                conf["min-spaces-inside-empty"] if conf["min-spaces-inside-empty"] != -1 else conf["min-spaces-inside"]
            ),
            max=(
                conf["max-spaces-inside-empty"] if conf["max-spaces-inside-empty"] != -1 else conf["max-spaces-inside"]
            ),
