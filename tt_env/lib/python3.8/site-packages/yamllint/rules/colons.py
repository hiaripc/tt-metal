...
def check(conf, token, prev, next, nextnext, context):
    if isinstance(token, yaml.ValueToken) and not (
        ...
        problem = spaces_before(
            token, prev, next, max=conf["max-spaces-before"], max_desc="too many spaces before colon"
        )
        if problem is not None:
            yield problem

        problem = spaces_after(token, prev, next, max=conf["max-spaces-after"], max_desc="too many spaces after colon")
        if problem is not None:
            yield problem

    if isinstance(token, yaml.KeyToken) and is_explicit_key(token):
        problem = spaces_after(
            token, prev, next, max=conf["max-spaces-after"], max_desc="too many spaces after question mark"
        )
        if problem is not None:
            yield problem
