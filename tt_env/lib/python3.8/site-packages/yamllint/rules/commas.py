...
def check(conf, token, prev, next, nextnext, context):
    if isinstance(token, yaml.FlowEntryToken):
        if prev is not None and conf["max-spaces-before"] != -1 and prev.end_mark.line < token.start_mark.line:
            yield LintProblem(
                token.start_mark.line + 1, max(1, token.start_mark.column), "too many spaces before comma"
            )
        else:
            problem = spaces_before(
                token, prev, next, max=conf["max-spaces-before"], max_desc="too many spaces before comma"
            )
