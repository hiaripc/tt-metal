...
def check(conf, token, prev, next, nextnext, context):
    if conf["present"]:
        ...
        prev_is_end_or_stream_start = isinstance(prev, (yaml.DocumentEndToken, yaml.StreamStartToken))
        ...
        if isinstance(token, yaml.DocumentEndToken):
            yield LintProblem(
                token.start_mark.line + 1, token.start_mark.column + 1, 'found forbidden document end "..."'
            )
