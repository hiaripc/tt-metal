...
def check(conf, token, prev, next, nextnext, context):
    if conf["present"]:
        if isinstance(prev, (yaml.StreamStartToken, yaml.DocumentEndToken, yaml.DirectiveToken)) and not isinstance(
        ...
        if isinstance(token, yaml.DocumentStartToken):
            yield LintProblem(
                token.start_mark.line + 1, token.start_mark.column + 1, 'found forbidden document start "---"'
            )
