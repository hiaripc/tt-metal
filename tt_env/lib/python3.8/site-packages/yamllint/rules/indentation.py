...
def check_scalar_indentation(conf, token, context):
    ...
    while True:
        ...
        if indent != expected_indent:
            yield LintProblem(
                line_no, indent + 1, "wrong indentation: expected %d but found %d" % (expected_indent, indent)
            )
...
def _check(conf, token, prev, next, nextnext, context):
    ...
    if first_in_line:
        ...
        if found_indentation != expected:
            if expected < 0:
                message = "wrong indentation: expected at least %d" % (found_indentation + 1)
            else:
                message = "wrong indentation: expected %d but found %d" % (expected, found_indentation)
    ...
    if isinstance(token, yaml.BlockMappingStartToken):
        ...
        if not isinstance(
            next, (yaml.BlockEndToken, yaml.FlowMappingEndToken, yaml.FlowSequenceEndToken, yaml.KeyToken)
        ):
...
def check(conf, token, prev, next, nextnext, context):
    try:
        ...
        yield LintProblem(
            token.start_mark.line + 1, token.start_mark.column + 1, "cannot infer indentation: unexpected token"
        )
