...
def check(conf, token, prev, next, nextnext, context):
    if not (
        ...
        and isinstance(
            prev,
            (yaml.BlockEntryToken, yaml.FlowEntryToken, yaml.FlowSequenceStartToken, yaml.TagToken, yaml.ValueToken),
        )
    ):
        return
    ...
    if conf["required"] is True:
        ...
        if token.style is None or not (
            _quote_match(quote_type, token.style) or (conf["allow-quoted-quotes"] and _has_quoted_quotes(token))
        ):
        ...
        if token.style and tag == DEFAULT_SCALAR_TAG and token.value and not _quotes_are_needed(token.value):
            is_extra_required = any(re.search(r, token.value) for r in conf["extra-required"])
            is_extra_allowed = any(re.search(r, token.value) for r in conf["extra-allowed"])
            if not (is_extra_required or is_extra_allowed):
                msg = "string value is redundantly quoted with %s quotes" % (quote_type)
