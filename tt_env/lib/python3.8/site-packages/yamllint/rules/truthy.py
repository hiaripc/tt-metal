...
def check(conf, token, prev, next, nextnext, context):
    ...
    if not conf["check-keys"] and isinstance(prev, yaml.tokens.KeyToken) and isinstance(token, yaml.tokens.ScalarToken):
        return

    if isinstance(token, yaml.tokens.ScalarToken):
        if token.value in (set(TRUTHY) - set(conf["allowed-values"])) and token.style is None:
            yield LintProblem(
