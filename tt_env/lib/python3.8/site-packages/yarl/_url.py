...
class URL:
    ...
    def __new__(
        ...
        *,
        ...
        if not encoded:
            ...
            if not netloc:  # netloc
                ...
                if host is None:
                    if scheme in SCHEME_REQUIRES_HOST:
                        msg = "Invalid URL: host is required for " f"absolute urls with the {scheme} scheme"
            ...
            if val.netloc != netloc or val.path != path or val.query != query or val.fragment != fragment:
    ...
    def build(
        ...
        *,
        ...
        if authority and (user or password or host or port):
            raise ValueError('Can\'t mix "authority" with "user", "password", "host" or "port".')
        ...
        if encoded:
            ...
            query_string = cls._QUERY_QUOTER(query_string) if query_string else query_string
    ...
    @staticmethod
    def _unsplit_result(scheme: str, netloc: str, url: str, query: str, fragment: str) -> str:
    ...
    def _parse_host(cls, host: str) -> Tuple[bool, str, Union[bool, None], str, str, str]:
    ...
    @classmethod
    def _encode_host(cls, host: str, human: bool = False, validate_host: bool = True) -> str:
    ...
    def with_scheme(self, scheme: str) -> "URL":
        ...
        if not self._val.netloc and lower_scheme in SCHEME_REQUIRES_HOST:
            msg = "scheme replacement is not allowed for " f"relative URLs for the {lower_scheme} scheme"
            raise ValueError(msg)
    ...
    def _get_str_query_from_sequence_iterable(
        ...
        pairs = [
            ...
            for k, val in items
            for v in (val if type(val) is not str and isinstance(val, (list, tuple)) else (val,))
        ]
        return "&".join(pairs)
    ...
    def _query_var(v: QueryVariable) -> str:
        ...
        if cls is not bool and isinstance(cls, SupportsInt):
            return str(int(v))
        raise TypeError(
            "Invalid variable type: value " "should be str, int or float, got {!r} " "of type {}".format(v, cls)
        )

    def _get_str_query_from_iterable(self, items: Iterable[Tuple[Union[str, istr], str]]) -> str:
        ...
        pairs = [f"{quoter(k)}={quoter(v if type(v) is str else self._query_var(v))}" for k, v in items]
        return "&".join(pairs)
    ...
    def without_query_params(self, *query_params: str) -> "URL":
        ...
        return self.with_query(
            tuple((name, value) for name, value in self.query.items() if name not in params_to_remove)
        )
    ...
    def with_name(self, name: str) -> "URL":
        ...
        if self._val.netloc:
            ...
            if parts[0] == "/":
                parts[0] = ""  # replace leading '/'
        return self._from_val(self._val._replace(path="/".join(parts), query="", fragment=""))
    ...
    def human_repr(self) -> str:
        ...
        query_string = "&".join(
            "{}={}".format(_human_quote(k, "#&+;="), _human_quote(v, "#&+;=")) for k, v in self.query.items()
        )
...
def _host_validate(host: str) -> None:
    ...
    if value == "@" or (value == ":" and "@" in host[pos:]):
        ...
        extra = ", if the value includes a username or password, " "use 'authority' instead of 'host'"
    raise ValueError(f"Host {host!r} cannot contain {value!r} (at position " f"{pos}){extra}") from None
...
def cache_configure(
    *,
    ...
    _ip_compressed_version = lru_cache(ip_address_size)(_ip_compressed_version.__wrapped__)
