...
class Socket:
    ...
    def bind(self, address):
        ...
        if rc < 0:
            if IPC_PATH_MAX_LEN and C.zmq_errno() == errno_mod.ENAMETOOLONG:
                ...
                msg = (
                    f'ipc path "{path}" is longer than {IPC_PATH_MAX_LEN} ' "characters (sizeof(sockaddr_un.sun_path))."
                )
    ...
    def get(self, option):
        ...
        _retry_sys_call(C.zmq_getsockopt, self._zmq_socket, option, c_value_pointer, c_sizet_pointer)

        sz = c_sizet_pointer[0]
        v = value_from_opt_pointer(option, c_value_pointer, sz)
        if option != zmq.SocketOption.ROUTING_ID and opt_type == _OptType.bytes and v.endswith(b"\0"):
            v = v[:-1]
        return v
