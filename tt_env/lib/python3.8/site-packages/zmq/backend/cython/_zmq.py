...
class Socket:
    ...
    def get(self, option: C.int):
        ...
        if opt_type == _OptType.bytes:
            ...
            if option != ZMQ_IDENTITY and sz > 0 and (cast(p_char, identity_str_c))[sz - 1] == b"\0":
                sz -= 1
...
def zmq_poll(sockets, timeout: C.int = -1):
    ...
    for i in range(nsockets):
        ...
        if isinstance(s, Socket):
            ...
            free(pollitems)
            raise TypeError("Socket must be a 0MQ socket, an integer fd or have " f"a fileno() method: {s!r}")
