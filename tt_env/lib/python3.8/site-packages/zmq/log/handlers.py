...
class PUBHandler(logging.Handler):
    ...
    def __init__(
        ...
        self.formatters = {
            logging.DEBUG: logging.Formatter("%(levelname)s %(filename)s:%(lineno)d - %(message)s\n"),
...
for name in "debug warn warning error critical fatal".split():
    ...
    setattr(
        TopicLogger,
        name,
        lambda self, level, topic, msg, *args, **kwargs: meth(self, level, topic + TOPIC_DELIM + msg, *args, **kwargs),
    )
