...
class Socket(SocketBase, AttributeSetter, Generic[SocketReturnType]):
        ...
        **kwargs,
        ...
        for i, msg in enumerate(msg_parts):
            ...
            try:
                ...
                if len(rmsg) > 32:
                    rmsg = rmsg[:32] + "..."
                raise TypeError(f"Frame {i} ({rmsg}) does not support the buffer interface.")
    ...
    def send_pyobj(self, obj: Any, flags: int = 0, protocol: int = DEFAULT_PROTOCOL, **kwargs) -> zmq.Frame | None:
        """
    ...
    def get_monitor_socket(self: _SocketType, events: int | None = None, addr: str | None = None) -> _SocketType:
        ...
        if zmq.zmq_version_info() < (4,):
            raise NotImplementedError(f"get_monitor_socket requires libzmq >= 4, have {zmq.zmq_version()}")
